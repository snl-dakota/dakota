...
class Parameters:
    ...
    def __init__(
        ...
        if format not in (APREPRO, STANDARD, JSON, DIRECT):
            raise TypeError(
                "format parameter must be APREPRO, STANDARD, JSON, or DIRECT"
            )
        ...
        if self.format in (
            APREPRO,
            STANDARD,
        ):  # JSON and DIRECT formats are presumed to have the proper types
    ...
    def _set_types_from_list(self, types: dict) -> None:
        if len(types) != len(self._variables):
            raise BadTypesOverride(
                "Length of types list is %d but number of variables is %d"
                % (len(types), len(self._variables))
            )
...
class Response:
    ...
    def __init__(
        self, descriptor: str, num_deriv_vars: int, ignore_asv: bool, asv: int
    ) -> None:
    ...
    def shape_matches(self, other: "Response") -> bool:
        ...
        return (
            ...
            and self._num_deriv_vars == other._num_deriv_vars
            and self.asv == other.asv
            and self._ignore_asv == other._ignore_asv
    ...
    def function(self, val: float) -> None:
        if not (self._ignore_asv or self.asv.function):
            raise ResponseError(
                "Function value not requested for '%s'." % self._descriptor
            )
    ...
    def gradient(self, val: Gradient) -> None:
        ...
        if len(self._gradient) != self._num_deriv_vars:
            raise ResponseError(
                "Length of gradient must equal number of derivative variables."
            )
    ...
    def hessian(self, val: Hessian) -> None:
        ...
        for r in val:
            ...
            if cctr != self._num_deriv_vars:
                raise ResponseError(
                    "Hessian must be square and size num_deriv_variables."
                )
...
class Results:
    ...
    def format_matches(self, other: "Results") -> bool:
        ...
        matches = matches and all(
            s == o for s, o in zip_longest(self._deriv_vars, other._deriv_vars)
        )
        matches = matches and all(
            s[1].shape_matches(o[1])
            for s, o in zip_longest(self._responses.items(), other._responses.items())
        )
    ...
    def write(
        self, stream: io.IOBase = None, ignore_asv: bool = None, json: bool = False
    ) -> None:
        ...
        if self._batch:
            raise BatchWriteError(
                "write() called on Results object in "
...
class BatchParameters:
    ...
    def __init__(self, param_sets: List[Parameters]) -> None:
        ...
        try:
            ...
            raise BatchSettingError(
                "batch set to True, but parameters "
...
class BatchResults:
    ...
    def write(
        ...
        write_function = (
            self._write_batch_to_json if json else self._write_batch_to_standard
        )
    ...
    def _write_batch_to_json(self, stream: io.IOBase, ignore_asv: bool) -> None:
        json_batch = [
            _encode_results_as_json(r, ignore_asv) for r in self._eval_results
        ]
...
class BatchSplitter:
    ...
    def __init__(self, parameters_file: str = None):
        self._parameters_file = (
            sys.argv[1] if parameters_file is None else parameters_file
        )
        self._format = self._detect_format()
        self._ending_line_numbers, self._batch_id, self._eval_nums = (
            self._parse_parameters_file()
        )
    ...
    def _parse_parameters_file(self) -> Tuple[List[int], str, List[int]]:
        ...
        try:
            ...
            raise BatchSettingError(
                "Parameters file does not appear to be a batch file."
            )
    ...
    def write(self, filename: str, index: int = None, eval_num: int = None) -> None:
        ...
        if (index is None and eval_num is None) or (
            index is not None and eval_num is not None
        ):
            raise ValueError(
                "BatchSplitter.write must be called with either an index or eval_num"
            )
    ...
    def _index_from_eval_num(self, eval_num: int) -> int:
        try:
            ...
            raise EvalNumberError(
                f"Batch does not contain a parameter set with evaluation number {eval_num}"
            )
        return idx
...
def _write_standard_results(
    stream: io.IOBase, results: Results, ignore_asv: bool
) -> None:
    # Write FAIL if set
    if results.failed():
...
def _encode_results_as_json(results: Results, ignore_asv: bool) -> dict:
    ...
    for t, v in results.items():
        if (v.asv.hessian or ignore_asv) and v.hessian is not None:
            eval_results["hessians"][t] = [
                [_replace_non_finite(e) for e in row] for row in v.hessian
            ]
...
def _check_against_asv(results: Results, ignore_asv: bool) -> None:
    ...
    if not ignore_asv and not results.failed():
        for t, v in results.items():
            if v.asv.function and v.function is None:
                raise ResponseError(
                    "Response '" + t + "' is missing requested function result."
                )
            if v.asv.gradient and v.gradient is None:
                raise ResponseError(
                    "Response '" + t + "' is missing requested gradient result."
                )
            if v.asv.hessian and v.hessian is None:
                raise ResponseError(
                    "Response '" + t + "' is missing requested Hessian result."
                )
    ...
    if not results.failed():
        for k, v in results.metadata.items():
            if v is None:
                raise ResultsError(
                    "Results object is missing requested metadata field '" + k + "'"
                )
...
_pRE = {
    "APREPRO": {
        "num_variables": re.compile(
            _aprepro_re_base.format(
                value=r"(?P<value>\d+)", tag=r"(?P<tag>DAKOTA_VARS)"
            )
        ),
        "num_functions": re.compile(
            _aprepro_re_base.format(value=r"(?P<value>\d+)", tag=r"(?P<tag>DAKOTA_FNS)")
        ),
        "num_deriv_vars": re.compile(
            _aprepro_re_base.format(
                value=r"(?P<value>\d+)", tag=r"(?P<tag>DAKOTA_DER_VARS)"
            )
        ),
        "num_an_comps": re.compile(
            _aprepro_re_base.format(
                value=r"(?P<value>\d+)", tag=r"(?P<tag>DAKOTA_AN_COMPS)"
            )
        ),
        "num_metadata": re.compile(
            _aprepro_re_base.format(
        ...
        "eval_id": re.compile(
            _aprepro_re_base.format(
                value=r"(?P<value>\d+(?::\d+)*)", tag="(?P<tag>DAKOTA_EVAL_ID)"
            )
        ),
        "variable": re.compile(
            _aprepro_re_base.format(value=r"\"?(?P<value>.+?)\"?", tag=r"(?P<tag>\S+)")
        ),
        "function": re.compile(
            _aprepro_re_base.format(
        ...
        "metadata": re.compile(
            _aprepro_re_base.format(value=r"\"(?P<value>.+?)\"", tag=r"(?P<tag>MD_\d+)")
        ),
    },
    "DAKOTA": {
        "num_variables": re.compile(
            _dakota_re_base.format(value=r"(?P<value>\d+)", tag=r"(?P<tag>variables)")
        ),
        "num_functions": re.compile(
            _dakota_re_base.format(value=r"(?P<value>\d+)", tag=r"(?P<tag>functions)")
        ),
        "num_deriv_vars": re.compile(
            _dakota_re_base.format(
        ...
        "num_metadata": re.compile(
            _dakota_re_base.format(value=r"(?P<value>\d+)", tag=r"(?P<tag>metadata)")
        ),
        "eval_id": re.compile(
            _dakota_re_base.format(
        ...
        "deriv_var": re.compile(
            _dakota_re_base.format(value=r"(?P<value>\d+)", tag=r"DVV_\d+:(?P<tag>\S+)")
        ),
        "an_comp": re.compile(
            _dakota_re_base.format(value=r"(?P<value>.+?)", tag=r"AC_\d+:(?P<tag>.+?)")
        ),
        "metadata": re.compile(
            _dakota_re_base.format(value=r"(?P<value>.+?)", tag=r"(?P<tag>MD_\d+)")
        ),
    },
}
...
def _extract_block(stream, numRE, dataRE, handle):
    ...
    m = numRE.match(line)
    if m is None:
        raise ParamsFormatError(
            "Improper format for section header in " + "parameters file"
        )
...
def _read_eval_from_stream(
    stream=None, ignore_asv=False, results_file=None, infer_types=True, types=None
):
    ...
    _extract_block(
        stream, useRE["num_deriv_vars"], useRE["deriv_var"], store_deriv_vars
    )
    ...
    _extract_optional_block(
        stream, useRE["num_metadata"], useRE["metadata"], store_metadata
    )
    return (
        Parameters(format, variables, an_comps, eval_id, metadata, infer_types, types),
        Results(
            format, responses, deriv_vars, eval_id, metadata, ignore_asv, results_file
        ),
    )


def _read_parameters_stream(
    stream=None,
    ignore_asv=False,
    batch=False,
    results_file=None,
    ...
    param_sets, results_sets = _read_evals_from_stream(
        stream, ignore_asv, results_file, infer_types, types
    )
    ...
    if batch:
        ...
        raise BatchSettingError(
            "batch is False, but parameters for "
...
def _read_evals_from_stream(
    stream: io.IOBase, ignore_asv: bool, results_file: str, infer_types: bool, types
) -> Tuple[List[Parameters], List[Results]]:
    try:
        return _read_evals_from_json(
            stream, ignore_asv, results_file, infer_types, types
        )
...
def _read_evals_from_json(stream, ignore_asv, results_file, infer_types, types):
    ...
    for params in j:
        try:
            ...
            raise ParamsFormatError(
                "'variables' portion of JSON-format evaluation missing or not formatted as expected"
            )
        try:
            ...
            raise ParamsFormatError(
                "'responses' portion of JSON-format evaluation missing or not formatted as expected"
            )
        try:
            deriv_vars = [var_labels[i - 1] for i in params["derivative_variables"]]
        except (KeyError, IndexError):
            raise ParamsFormatError(
                "'derivative_variables' portion of JSON-format evaluation missing or not formatted as expected"
            )
        try:
            an_comps = [a["component"] for a in params["analysis_components"]]
        except KeyError:
            raise ParamsFormatError(
                "'analysis_components' portion of JSON-format evaluation missing or not formatted as expected"
            )
        try:
            eval_id = params["eval_id"]
        except KeyError:
            raise ParamsFormatError(
                "'eval_id' portion of JSON-format evaluation missing"
            )
        try:
            metadata = params["metadata"]
        except KeyError:
            raise ParamsFormatError(
                "'metadata' portion of JSON-format evaluation missing"
            )
        params_set.append(
            Parameters(JSON, var_dict, an_comps, eval_id, metadata, infer_types, types)
        )
        results_set.append(
            Results(
                JSON,
...
def _read_params_from_dict(parameters=None):
    ...
    for p in parameters:
        ...
        results_list.append(
            Results(DIRECT, responses, dvv, p["eval_id"], p["metadata_labels"], False)
        )
    if batch:
...
def read_parameters_file(
    parameters_file: Union[str, None] = None,
    ...
    if parameters_file is None:
        try:
            ...
            raise MissingSourceError(
                "No parameters filename provided and no command line argument."
            )
    ...
    if results_file is None:
        try:
            ...
            raise MissingSourceError(
                "No results filename provided and no command line argument."
            )
    ...
    with open(parameters_file, "r", encoding="utf8") as ifp:
        return _read_parameters_stream(
            ifp, ignore_asv, batch, results_file, infer_types, types
        )


def dprepro(
    template: Union[str, io.IOBase],
    parameters: Parameters = None,
    results: Results = None,
    include: dict = None,
    output: Union[str, io.IOBase] = None,
    fmt: str = "%0.10g",
    code: str = "%",
    code_block: str = "{% %}",
    inline: str = "{ }",
