...
def _get_node_list(
    tile=None, tile_size=None, tasks_per_node=None, dedicated_master=None
):
...
def _mpirun(node_list, user_commands, **kwargs):
    ...
    proc = subprocess.Popen(
        mpi_command + host_args + user_command,
        stderr=stderr,
        stdout=stdout,
        stdin=stdin,
        **kwargs,
...
def _calc_num_tiles(
    tile_size=None, tasks_per_node=None, num_nodes=None, dedicated_master=None
):
    ...
    if dedicated_master == "NODE":
        if num_nodes == 1:
            raise ResourceError(
                "Dedicated master node requested, but job has only one node."
            )
        ...
        if num_tiles == 0:
            raise ResourceError(
                "Dedicated master tile requested, but job has only one tile."
            )
    else:
...
def tile_run_static(
    ...
    **kwargs,
    ...
    if eval_num:
        ...
        params, results = read_parameters_file(
            parameters_file=parameters_file, results_file=UNNAMED
        )
    ...
    if not tile_size:
        ...
        raise ResourceError(
            "Requested tile size (%d) is smaller than total "
...
def tile_run_dynamic(
    ...
    **kwargs,
    ...
    if not tile_size:
        ...
        raise ResourceError(
            "Requested tile size (%d) is smaller than " % tile_size
