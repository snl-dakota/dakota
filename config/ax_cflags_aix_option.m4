...
dnl variant.
dnl
dnl Modified by Shannon Brown to use AC_LANG_CPLUSPLUS
...
AC_DEFUN([AX_CFLAGS_AIX_OPTION],[ifelse(regexp([$2],[-]),-1,
[AX_CFLAGS_AIX_OPTION_NEW($@)],[AX_CFLAGS_AIX_OPTION_OLD($@)])])

AC_DEFUN([AX_CXXFLAGS_AIX_OPTION],[ifelse(regexp([$2],[-]),-1,
