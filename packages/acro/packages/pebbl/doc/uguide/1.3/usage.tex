...
environments, for example, you could use the linker directives
\begin{codeblock}
...
%%
in directory \url{acro-pebbl/packages/pebbl/src/example/pebbl}.
...
the solution's objective value, and no information, for example, about
values of decision variables.  To construct such an object, use the
constructor \texttt{solution(branching*)}, and then set the
\texttt{value} of the resulting object.  For example, for code in the
...
\texttt{foundSolution(sol)} cedes ownership of \texttt{*sol}
to PEBBL.
...
this method if \texttt{haveIncumbentHeuristic()} returns
...
\item[\textmd{\emph{defaultValue}}] is the default value for the
  parameter.  The class data member \emph{internalName} will take this
  value if the command-line parameter \texttt{--}\emph{commandLineName} is
  not specified.
...
\item[\textmd{\emph{error check object}}] is optional and 
  ...
  \texttt{Parameter} and related class documentation.
\end{description}
...
available in \emph{error check object} by overloading the
...
\}
\end{codeblock}
The \texttt{driver<\emph{myBranching}>(argc,argv)} template invocation
...
\texttt{setupProblem}, and \texttt{solve()}.  Further details
are not covered in this version of the user's guide.
...
to configure PEBBL with MPI; see Section~\ref{sec:compiling}, and
...
stubs.  You may wish to follow the same pattern within your own
...
\method{parallelBranchSub* blankParallelSub()} This method is
...
datatypes, along with many UTILIB-supplied datatypes and STL
\texttt{set}s or
...
compilers.  For UTILIB arrays, STL \texttt{set}s, or STL
...
\label{sec:parsol}
User-defined \texttt{solution}-derived classes, for example
\texttt{\emph{mySolution}} from Section~\ref{sec:solclassdetails},
need a few additional methods in order to function in a parallel
setting.  Specifically, you should define implementations of the
following \texttt{virtual} methods:

\method{solution* blankClone()} Return a ``blank'' copy of the current
solution.  The blank copy should contain all information that is
typically obtained from the object describing your problem instance
(for example, from the \texttt{\emph{myBranching}} class).  This
information should include any pointers to the problem instance class,
and the correct setting of \texttt{sense}.  For all such information
embedded in the \texttt{solution} base class, the constructor
...
\method{void packContents(PackBuffer\& outBuf)}
Pack the application-specific information in the current solution into
the \texttt{PackBuffer} object \texttt{outBuf}, typically using the
\texttt{>>} operator.
...
the order of packing in \texttt{packContents}.

\method{int maxContentsBufSize()} Return the maximum number of bytes
that calls to \texttt{packContents(outBuf)} might write to the buffer
\texttt{outBuf} for the current problem instance.
...
the a possible new incumbent value.  If enumeration is active, then
new solutions may be ``hashed'' to other processor to check for
duplicates and for incorporation into the global solution repository.
...
subproblem.  The default implementation is a stub.  You need not
...
subsequent intervention.  PEBBL will select one of the best solutions
...
\texttt{sync} which is relevant only during the ramp-up phase.  Its
default value is the \texttt{enum} constant \texttt{notSynchronous}.
Supplying instead the value \texttt{synchronous} indicates to PEBBL
...
the next \texttt{rampUpIncumbentSync()} operation.  The \texttt{sync}
argument to \texttt{foundSolution} is ignored outside of the ramp-up
phase.  
...
checkpointing feature will neither save nor restore them.  In that
...
available parallelism.  If more than one processor is available or the
...
MPI.  Once it has detected whether multiple processors are available
