...
const int A::serializer_registered
   = Serializer().register_serializer<A>("sss:A", A::serializer);
...
int test_sserialstream(int , char**)
{
   int    i1  = 5;
   double d1  = 3.1415;
   double d1a = 1.234;
   ...
   // Test 0: generate a test string
   try
   {
      utilib::osSerialStream oss;
      oss << i1 << d1 << a1;
      data = oss.str();

      OUT << "(data created)" << endl;
   }
   ...
   try
   {
      ...
      utilib::isSerialStream iss(data);
      iss >> i2;
   ...
   // Test 4: appending to a string
   OUT << "Appending to an existing string" << endl;
   try
   {
      int    i2 = 0;
      double d2 = 1.0;
      ...
      utilib::osSerialStream oss(data);
      oss << d1a;
      OUT << "   (appended d1a)" << endl;

      utilib::isSerialStream iss(oss.str());
      iss >> i2 >> d2 >> a2 >> d2a;
   ...
   OUT << "Appending to an existing string (mixed read/write)" << endl;
   try
   {
      int    i2 = 0;
      double d2 = 1.0;
   ...
   // Test 6: 
   OUT << "sSerialStream to a truncated string" << endl;
   try
   {
      int    i2 = 0;
      double d2 = 1.0;
