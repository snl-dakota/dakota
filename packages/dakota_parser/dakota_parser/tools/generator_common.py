...
class CodeGenerator:
    ...
    def generate_actions(self, block_dir: Path):
        ...
        lines.append("                    }")
        lines.append("                }")
        lines.append("                // Only reuse an existing node when the keyword acts as a")
        lines.append("                // container in the schema. Repeated leaf keywords must remain")
        lines.append("                // distinct AST nodes so occurrence validation can reject them.")
        lines.append("                bool can_reuse_as_container =")
        lines.append("                    parent_child_map.find(processing_stack[parent_index].dotted_path + \".\" + resolved_name)")
        lines.append("                    != parent_child_map.end();")
        lines.append("")
        ...
        lines.append("                if (can_reuse_as_container) {")
        ...
        lines.append('                            DEBUG_OUT("[DEBUG]   Found existing container \'" << resolved_name << "\' on stack at level " << si << "\\n");')
        ...
        lines.append("                    // Only do this for container keywords; repeated leaf keywords")
        lines.append("                    // must remain distinct so occurrence validation can see them.")
        lines.append("                    {")
        ...
        lines.append('                                    DEBUG_OUT("[DEBUG]   Re-using committed child container " << resolved_name << "\\n");' )
        lines.append("                                }")
        lines.append("                            } else {")
        ...
        lines.append("                                    if (ki->second.empty()) block->keywords.erase(ki);")
        lines.append('                                    DEBUG_OUT("[DEBUG]   Re-using committed block container " << resolved_name << "\\n");' )
        lines.append("                                }")
        lines.append("                            }")
        lines.append("                        }")
        ...
        lines.append("                    block->keywords[orphan->name].push_back(orphan);")
        lines.append("                }")
    ...
    def generate_block_parser(self, block_dir: Path):
        ...
        lines.append(f'            result = tao::pegtl::standard_trace<tao::pegtl::must<tao::pegtl::seq<{self.block.name}_content, tao::pegtl::eof>>, actions::action>(input, state);')
        lines.append('        } else {')
        lines.append(f'            result = tao::pegtl::parse<tao::pegtl::must<tao::pegtl::seq<{self.block.name}_content, tao::pegtl::eof>>, actions::action>(input, state);')
        lines.append('        }')
        ...
        lines.append('        const auto& pos = e.positions().front();')
        lines.append('        std::string message(e.message());')
        lines.append('        if (message.find("tao::pegtl::seq<") != std::string::npos &&')
        lines.append('            message.find("tao::pegtl::eof") != std::string::npos) {')
        lines.append('            message = "unexpected trailing content in block";')
        lines.append('        }')
        lines.append(f'        std::cerr << "Parse error in {self.block.name} block at line " << pos.line')
        lines.append('                  << ", column " << pos.column << ": " << message << "\\n";')
        lines.append('        try {')
        lines.append('            std::string line(input.line_at(pos));')
        lines.append('            std::cerr << line << "\\n";')
        lines.append("            if (pos.column > 0) { std::cerr << std::string(pos.column - 1, ' ') << \"^\\n\"; }")
        lines.append('        } catch (...) {')
        lines.append('        }')
