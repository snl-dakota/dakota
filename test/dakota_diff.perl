...
sub compare_output {
  ...
  while ( $test_found && scalar(@tst_excerpt) > 0 ) { 
    ...
    my $uq_re = '^(\s+(Response Level|Resp Level Set)\s+Probability Level\s+Reliability Index\s+General Rel Index|\s+Response Level\s+Belief (Prob Level|Gen Rel Lev)\s+Plaus (Prob Level|Gen Rel Lev)|\s+(Probability|General Rel) Level\s+Belief Resp Level\s+Plaus Resp Level|[ \w]+Correlation Matrix[ \w]+input[ \w]+output\w*:|\w+ Sobol\' indices:|(Expansion moment statistics|Sample moment statistics|95% confidence intervals) for each response function:)$';
