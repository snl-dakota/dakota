...
while ( $test_found == 0 && defined ($base = <DAKOTA_BASE>) ) {
  ...
  if ($base =~ /$mod_testin/) {
	...
	while ( ( ($t_val) = $test =~ /^\s+($e)/ ) &&
      ...
      while ( ($base =~ /^(\s+Response Level\s+Probability Level\s+Reliability Index\s+General Rel Index|\s+Response Level\s+Belief (Prob Level|Gen Rel Lev)\s+Plaus (Prob Level|Gen Rel Lev)|\s+(Probability|General Rel) Level\s+Belief Resp Level\s+Plaus Resp Level|[ \w]+Correlation Matrix[ \w]+input[ \w]+output\w*:|\w+ Sobol indices:|(Moment-based statistics|95% confidence intervals) for each response function:)$/o) &&
	      ($test =~ /^(\s+Response Level\s+Probability Level\s+Reliability Index\s+General Rel Index|\s+Response Level\s+Belief (Prob Level|Gen Rel Lev)\s+Plaus (Prob Level|Gen Rel Lev)|\s+(Probability|General Rel) Level\s+Belief Resp Level\s+Plaus Resp Level|[ \w]+Correlation Matrix[ \w]+input[ \w]+output\w*:|\w+ Sobol indices:|(Moment-based statistics|95% confidence intervals) for each response function:)$/o) ) {
