...
//KRD modified this starting from shubert.C
int main(int argc, char** argv)
{
  ...
  if (ASV[0] >= 1) 
    for (i=0; i<num_vars; i++) {
      ...
      w[i]=std::exp(-dtemp1*dtemp1)+std::exp(-0.8*dtemp2*dtemp2);
    }
  ...
  if (ASV[0] >= 2) {
    for (i=0; i<num_deriv_vars; i++) {
      ...
      d1w[ii]=-2.0*dtemp1*std::exp(-dtemp1*dtemp1)-1.6*dtemp2*std::exp(-0.8*dtemp2*dtemp2);
    }
  }
  double d1;
  if (ASV[0] & 2) {
  ...
  //                    -1.6*\exp(-0.8*(x_i+1)^2)+2.56*(x_i+1)^2*\exp(-0.8*(x_i+1)^2) //for smoothed herbie
  if (ASV[0] & 4) {
    fout << "[[ ";
    double d2;
    for (i=0; i<num_deriv_vars; ++i) {
