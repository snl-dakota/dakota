#!/bin/csh -f
# Matlab driver script for Gary Vanderplaats beam
#
# Robert A. Cafield, 2/19/2018
...
cp *beamGV*.m workdir.$num/

# ------------------------------------
# RUN the simulation from workdir.num
# ------------------------------------
# launch Matlab with command mode (-r)
# matlab_beamGV_wrapper.m will call the actual analysis files
# fbeamGV.m and gbeamGV.m
cd workdir.$num

echo matlab -nojvm -nodesktop -nodisplay -nosplash -r "matlab_beamGV_wrapper('params.in', 'results.out'); exit"

matlab -nojvm -nodesktop -nodisplay -nosplash -r "matlab_beamGV_wrapper('params.in', 'results.out'); exit"
