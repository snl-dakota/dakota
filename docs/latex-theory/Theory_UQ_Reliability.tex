...
Eq.~\ref{eq:beta_cdf} or~\ref{eq:beta_ccdf} in the latter case.
For a specified generalized reliability level $\bar{\beta^*}$ or
second-order probability level $\bar{p}$, the equality constraint is 
reformulated in terms of the generalized reliability index:
\begin{eqnarray}
{\rm minimize}     & \pm G({\bf u}) \nonumber \\
{\rm subject \ to} & \beta^* = \bar{\beta^*} \label{eq:pma2_opt}
\end{eqnarray}
where $\bar{\beta^*}$ is computed from $\bar{p}$ using 
Eq.~\ref{eq:gen_beta} or its complement in the latter case.
...
$G({\bf u})$) is similarly defined by either $\bar{\beta}$ (specified 
reliability or first-order probability level) or from a $\bar{\beta}$ 
computed from $\bar{\beta^*}$ (specified generalized reliability or 
second-order probability level)
\begin{eqnarray}
