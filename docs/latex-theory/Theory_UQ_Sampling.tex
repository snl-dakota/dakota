...
To obtain the sample allocation, in the MLMC context, it is necessary to evaluate the variance of $c^{ML}[Q]$, which can be written as 
\begin{equation}
...
\begin{equation}
\begin{split}
 \mathbb{C}ov\left[ \hat{Q}_{L}^{ML}, \hat{\sigma}_L^{ML} \right] &= \mathbb{C}ov\left[ \sum_{\ell_i=0}^{L} \hat{Q}_{\ell_i} - \hat{Q}_{\ell_i-1}, \sum_{\ell_i=0}^{L} \hat{\sigma}_{\ell_i} - \hat{\sigma}_{\ell_i-1} \right] \\
                                                                  &= \sum_{\ell_i=0}^{L} \mathbb{C}ov\left[ \hat{Q}_{\ell_i} - \hat{Q}_{\ell_i-1}, \hat{\sigma}_{\ell_i} - \hat{\sigma}_{\ell_i-1} \right],
\end{split}
\end{equation}
which leads to the need for evaluating the following four contributions 
\begin{equation}
 \mathbb{C}ov\left[ \hat{Q}_{\ell_i} - \hat{Q}_{\ell_i-1}, \hat{\sigma}_{\ell_i} - \hat{\sigma}_{\ell_i-1} \right] =
 \mathbb{C}ov\left[ \hat{Q}_{\ell_i} , \hat{\sigma}_{\ell_i} \right] - \mathbb{C}ov\left[ \hat{Q}_{\ell_i} , \hat{\sigma}_{\ell_i-1} \right]
 - \mathbb{C}ov\left[ \hat{Q}_{\ell_i-1}, \hat{\sigma}_{\ell_i} \right] + \mathbb{C}ov\left[ \hat{Q}_{\ell_i-1}, \hat{\sigma}_{\ell_i-1} \right].
\end{equation}

In Dakota, we adopt the following approximation, for two arbitrary levels $\ell$ and $\kappa \in \left\{ \ell-1, \ell, \ell+1 \right\}$ 
\begin{equation}
 \rho\left[ \hat{Q}_{L}^{\ell}, \hat{\sigma}_{\kappa} \right] \approx \rho\left[ \hat{Q}_{\ell}, \hat{Q}_{\kappa,2} \right]
