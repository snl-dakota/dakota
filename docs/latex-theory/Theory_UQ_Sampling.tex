...
\label{eq: variance_est_single_level}
 \mathbb{V}ar\left[Q_M\right] \approx \frac{1}{N_L - 1} \sum_{i=1}^{N_L} \left( Q_M^{(i)} - \mathbb{E}\left[Q_M\right] \right)^2.
\end{equation}

There are two necessary steps to obtain a multilevel version of Eq.~\eqref{eq: variance_est_single_level}, namely the telescopic expansion in term of difference of estimators
over subsequent levels and the telescopic expansion for the expected value $\mathbb{E}\left[Q_M\right]$.
...
\label{eq: variance_est_ML}
 \mathbb{V}ar\left[Q_M\right] &\approx       \frac{1}{N_L - 1} \sum_{i=1}^{N_L} \left( Q_M^{(i)} - \mathbb{E}\left[Q_M\right] \right)^2 \\
                              &\approx \sum_{\ell=0}^L  \frac{1}{N_\ell - 1} \left( \left( Q_{M_\ell}^{(i)} - \mathbb{E}\left[Q_{M_\ell}\right] \right)^2 
                                                                                  - \left( Q_{M_{\ell-1}}^{(i)} - \mathbb{E}\left[Q_{\ell-1}\right] \right)^2 \right).
\end{split}
\end{equation}
...
\begin{split}
\min\limits_{N_\ell} \sum_{\ell=0}^L \mathcal{C}_{\ell} N_\ell \quad \mathrm{s.t.} \quad \mathbb{V}ar\left[ \hat{Q}_{L,2}^{\mathrm{ML}} \right] = \varepsilon^2.
% 
% 
%  f(N_\ell,\lambda) = \sum_{\ell=0}^{L} N_\ell \, \mathcal{C}_{\ell} 
%                    + \lambda \left( \sum_{\ell=0}^{L} N_\ell^{-1} \mathbb{V}ar\left({Y_\ell}\right) - \varepsilon^2/2 \right). 
...
currently implemented in Dakota. We refer to the reference manual for a discussion of the keywords that enable to select these different options.
