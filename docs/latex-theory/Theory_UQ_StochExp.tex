...
Eqs.~\ref{eq:lagrange_interp_1d}--\ref{eq:hermite_interp_1d} and 
$I^{l-1}(R)$ is defined as $I^l(I^{l-1}(R))$, indicating evaluation of
the lower level interpolant on the higher level point set.
...
%For simplicity, we demonstrate this for the value-based interpolants without loss of generality.
For the case where we used nested point sets among the interpolation
levels, the differences at points contained in set $m_{l-1}$ are zero,
allowing us to restrict the summation to
\begin{equation}
\Delta^l(R) = 
\sum_{j=1}^{m_{\Delta_l}} \left[ r(\xi_j) - I^{l-1}(R)(\xi_j) \right] \,L_j(\xi)
\end{equation}
where we define $m_{\Delta_l} = m_l \setminus m_{l-1}$.  We then
define the hierarchical surplus $s$ at a point $\xi_j$ as $r(\xi_j) -
I^{l-1}(R)(\xi_j)$, which can be interpreted as a local interpolation
error estimate since it captures the difference between the true value
and the value predicted by the previous interpolant.  $\Delta^l(R)$
then becomes
\begin{equation}
\Delta^l(R) = \sum_{j=1}^{m_{\Delta_l}} s(\xi_j)\,L_j(\xi)
\end{equation}
...
collocation on sparse grids in Section~\ref{uq:expansion:sc}.
