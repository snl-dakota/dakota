...
\begin{equation}
R(\boldsymbol{\xi}) \cong \sum_{j=1}^{N_p} r_j(\boldsymbol{\xi}) 
\boldsymbol{L}_j(\boldsymbol{\xi}) \label{eq:lagrange_interp_nd}
\end{equation}
...
\prod_{i=1}^{n} H^{(1)}_{c^j_i}(\xi_i) \label{eq:multivar_H1}
\end{equation}
