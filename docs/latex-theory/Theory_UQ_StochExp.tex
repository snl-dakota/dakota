...
formulation~\cite{BerTref04,Higham04} is used.  Defining
the barycentric weights $w_j$ as
\begin{equation}
w_j = \prod_{\stackrel{\scriptstyle k=1}{k \ne j}}^m 
\frac{1}{\xi_j - \xi_k} \label{eq:barycentric_weights}
\end{equation}
and defining the quantity $l(x)$ as 
\begin{equation}
l(\xi) = \prod_{k=1}^m (\xi - \xi_k) \label{eq:barycentric_prod}
\end{equation}
we can pull out the common term and rewrite Eq.~\ref{eq:lagrange_interp_1d} as
\begin{equation}
R(\xi) = l(\xi) \sum_{j=1}^m \frac{w_j}{x-x_j} r(\xi_j) \label{eq:barycentric_lagrange1_1d}
\end{equation}
which is the first form of barycentric interpolation.  Utilizing an
identity for the interpolation of unity to eliminate $l(x)$, we arrive
at the second form of the barycentric interpolation formula as
\begin{equation}
R(\xi) = \frac{\sum_{j=1}^m \frac{w_j}{x-x_j} r(\xi_j)}{\sum_{j=1}^m \frac{w_j}{x-x_j}}
