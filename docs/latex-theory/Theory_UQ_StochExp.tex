...
\subsection{Global value-based} \label{uq:expansion:interp:Lagrange}
...
\subsection{Local value-based} \label{uq:expansion:interp:linear}
...
\subsection{Value-based} \label{uq:expansion:sc:value}
...
\begin{equation}
\frac{d\Psi_j(\boldsymbol{\xi})}{d\xi_i} = \frac{d\psi_{t_i^j}}{d\xi_i}(\xi_i)
...
\begin{equation}
\frac{d\boldsymbol{L}_j(\boldsymbol{\xi})}{d\xi_i} = 
...
Experiments Chapter of the Users Manual.  The concepts are summarized
here as well.  Variance-based decomposition is a global sensitivity
method that summarizes how the uncertainty in model output can be
apportioned to uncertainty in individual input variables.  VBD uses
two primary measures, the main effect sensitivity index $S_{i}$ and
the total effect index $T_{i}$.  These indices are also called the
...
\subsection{Uniform p-refinement with isotropic grids}
...
\subsection{Adaptive p-refinement with anisotropic grids}
...
\subsection{Goal-oriented p-refinement with generalized sparse grids}
...
for the original high-fidelity model.
...
Models chapter of the Users Manual).  The exact discrepancy functions are
\begin{eqnarray}
...
\begin{eqnarray}
\hat{R}_{hi_{\alpha}}(\boldsymbol{\xi}) & = & R_{lo}(\boldsymbol{\xi}) + 
\hat{A}(\boldsymbol{\xi}) \label{eq:correct_val_add} \\
\hat{R}_{hi_{\beta}}(\boldsymbol{\xi}) & = & 
...
than that required to resolve the discrepancy ($P_{lo} >> P_{hi}$ or
$N_{lo} >> N_{hi}$; either enforced statically through level
...
\item in the multiplicative case, the form of the high-fidelity
  expansion must first be expanded to include all polynomial orders
  indicated by the products of each of the basis polynomials in the
  low fidelity and discrepancy expansions (most easily estimated
  from total-order, tensor, or sum of tensor expansions).  Then the
  coefficients of this product expansion are computed as follows (shown 
  generically for expansion $z$ = expansion $x$ times expansion $y$):
\begin{eqnarray}
...
\end{eqnarray}
  where the basis triple products $\langle \Psi_i \Psi_j \Psi_k \rangle$ 
  are typically sparse and can be precomputed efficiently using one 
  dimensional quadrature for fast lookup within multidimensional integrals.
\end{itemize}
...
polynomial form of the more resolved expansion (typically the low
fidelity expansion) and only the coefficients are updated in order to
interpolate the sum or product values (and potentially their
derivatives).  Since we will typically need to produce a value of a
less resolved expansion on a more resolved grid to perform this
combination, we utilize the less resolved expansion rather than the
original response functions for both interpolated and non-interpolated 
point values (and derivatives), in order to ensure consistency.
