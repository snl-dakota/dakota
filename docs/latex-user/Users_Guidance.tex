...
simulation (see Section~\ref{models:ex:sbuq}). However, if this
...
of evidence (DAKOTA methods \texttt{nond\_local\_evidence} and 
\texttt{nond\_global\_evidence}). Another method is pure interval 
analysis (\texttt{nond\_local\_interval\_est} and 
\texttt{nond\_global\_interval\_est}), where intervals on inputs are 
...
is an interval propagation approach (\texttt{nond\_local\_interval\_est} or 
\texttt{nond\_global\_interval\_est}), the nested approach is
...
evidence-based approach (\texttt{nond\_local\_evidence} or 
\texttt{nond\_global\_evidence}), the approach generates epistemic belief 
...
nested UQ (second-order probability, \\
 ...
 & & and aleatory inner loop, nond\_sampling \\
\hline
\end{tabular}
\end{table}
...
\texttt{nond\_sampling} in a design of experiments mode (as opposed to
...
experiments        & space filling designs  & fsu\_quasi\_mc (halton, 
                   ...
                   & uniformly distributed) & \\
\hline
sampling           & space filling designs      & nond\_sampling
                                                  (Monte Carlo or LHS) \\
