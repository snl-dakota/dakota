...
{\bf Optimization Darts (Opt\_darts)} Exact deterministic methods are guaranteed to find all global solutions (or at least one of them); however, the associated computational burden can become excessive as the function dimensionality increases.
Heuristic stochastic techniques are not associated with strict convergence guarantees; however, they are usually better (and sometimes the only option) in practice.  Opt\_darts follows the basic steps in DIRECT but improve it in two major aspects: using Voronoi regions instead of hyper-rectangular cells, and placing samples via stochastic blue noise instead of deterministic cell division. Implicit tracing of the Voronoi cells enables handling of higher dimensional functions with very little storage requirements. Opt\_darts is associated with the same convergence guarantee that DIRECT has. 
