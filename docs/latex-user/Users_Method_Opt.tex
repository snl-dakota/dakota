...
another option. Also available is a method related to SQP: sequential 
linear programming (\texttt{dot\_slp}).
...
Although ROL accepts Dakota-provided finite-differencing  
approximations to the gradient of both objective and constraints, the  
user is better advised in utilizing alternatives such as DOT. Although 
ROL is a general gradient-based library, its past use cases 
predominantly involved analytic gradients (and Hessians) with little 
understanding of how ROL behaves when numerical gradients (and 
Hessians) are provided. Future releases would aim to address this lack 
of understanding.

We offer the following recommendations in deciding upon a 
suitable gradient-based method for a given problem
\begin{itemize}
\item For {\bf unonstrained and bound-constrained problems}, conjugate 
...
small- to intermediate-sized problems.
