...
\begin{itemize}
\item Although ROL (Rapid Optimization Library for large-scale 
optimization, part of the Trilinos software suite~\cite{Kou2014}) 
accepts Dakota-provided finite-differencing  approximations to the 
gradient of both objective and constraints, the  user is better 
advised in utlizing alternatives such as DOT. Although  ROL is a 
general gradient-based library, its past use cases  predominantly 
involved analytic gradients (and Hessians) with little  understanding 
of how ROL behaves when numerical gradients (and  Hessians) are 
provided. Future releases would aim to address this lack  of 
understanding.
\item As the problem size (i.e. number of variables) increases, ROL's 
trust region method 
and conjugate gradient methods exhibits the best scalability out of 
the available suite of gradient-based optimization libraries for 
unconstrained problems. In-house experiments were used to verify the 
superior scalability of ROL and conjugate gradient methods 
(\texttt{conmin\_frcg}, \texttt{dot\_frcg} and \texttt{optpp\_cg}) 
with respect to other optimizers for the bound-constrained extended 
Rosenbrock function.
\item For problems with large number of constraints with respect to 
number of variables, Method of Feasible Directions (MFD) methods 
(\texttt{conmin\_mfd} and \texttt{dot\_mmfd}) exhibit the best 
scalability, as was observed for the Vanderplaats beam problem using 
additional numerical experiments performed by Dakota developers.
\end{itemize}
...
conjugate gradient method (\texttt{optpp\_cg}) and ROL's trust-region 
method with truncated conjugate gradient subproblem solver 
(\texttt{rol}). ROL relies on secant updates for the Hessian, with the 
an approximation to the Hessian matrix at each iteration provided 
using only values of the gradient at current and previous iterates.
...
available. DOT (\texttt{dot\_bfgs}) provides a quasi-Newton method for 
such problems. \emph{We here provide a caution regarding 
...
\texttt{npsol\_sqp}~\cite{Gil86}. Also available is ROL's 
composite-step method (\texttt{rol}), utilizing SQP with trust 
regions, for equality-constrained problems. The particular implementation in
...
scalable performance for large-scale problems. Note that ROL internally
converts inequality constraints into equality constraints with bounded
slack variables.
...
see \texttt{dakota/share/dakota/examples/users/rosen\_opt\_patternsearch.in} }
...
\begin{figure}[ht!]
  ...
  Dakota input file --
see \texttt{dakota/share/dakota/examples/users/rosen\_opt\_ea.in} }
...
see \texttt{dakota/share/dakota/examples/users/rosen\_opt\_ego.in} }
...
see \texttt{dakota/share/dakota/examples/users/textbook\_opt\_multiobj1.in} }
...
\begin{figure}[htp!]
  ...
  \caption{Multiple objective genetic algorithm (MOGA) example: the
    Dakota input file --
see \texttt{dakota/share/dakota/examples/users/mogatest1.in} }
...
see \texttt{dakota/share/dakota/examples/users/rosen\_opt\_scaled.in} }
