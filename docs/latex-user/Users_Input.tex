...
By default, deterministic least squares and Bayesian calibration methods rely 
on the processes in the \texttt{response} block to provide estimates of the 
residuals,
\begin{equation}
\boldsymbol{r}(\boldsymbol{\theta}) = \boldsymbol{d} - 
	\boldsymbol{q}(\boldsymbol{\theta}),
\end{equation}
between the data $\boldsymbol{d}$ and the model predictions 
$\boldsymbol{q}(\boldsymbol{\theta})$. However, if \texttt{response} instead 
provides the model predictions $\boldsymbol{q}(\boldsymbol{\theta})$ themselves, 
experimental data $\boldsymbol{d}$ may be specified directly in a separate 
text file through the \texttt{calibration\_data\_file} command. This indicates 
that the residuals need to be calculated outside of the user-specified 
\texttt{response}. The data file containing experimental observations, if 
provided, must agree with the following guidelines. 
