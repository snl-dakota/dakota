...
Denote the variance estimate at the estimated parameters $\hat{x}$ by
\begin{equation}
\hat{\sigma}^2 = \frac{1}{N_{dof}}\sum_{i=1}^{n} T_i(\hat{x})^2,
\end{equation}
where $N_{dof} = n - p$ denotes the number of degrees of freedom
(total residuals $n$ less the number of calibrated parameters
$p$). Let
\begin{equation}
J = \left[ \frac{\partial T(\hat{x})}{\partial x} \right]
\end{equation}
denote the $N \times p$ matrix of partial derivatives of the residuals
with respect to the calibrated paramters. Then the standard error $SE$
...
inaccurate as well.  (Confidence intervals cannot be calculated when
the number of least-squares terms is less than the number of
parameters to be estimated, or when using vendor numerical gradients.)
See~\cite{Vug07} for more details about confidence intervals, and note
