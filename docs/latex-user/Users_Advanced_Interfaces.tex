...
partially listed in Figure~\ref{advint:figure03} will be used here for
...
\begin{figure}
  ...
  \begin{bigbox}
    \begin{small}
      \verbatimtabinput[8]{dprepro_head}
    \end{small}
  \end{bigbox}
...
ModelCenter framework, and The Mathworks' Matlab scientific computing
environment.
...
compiled with the Matlab feature enabled; see the INSTALL file
included with the DAKOTA distribution for more information.  The
DAKOTA/Matlab interface has been built and tested on 32-bit Linux with
Matlab 7.0 (R14) and on 64-bit Linux with Matlab 7.1 (R14SP3).
...
modifications to the DAKOTA build system as described in INSTALL.

Since the Matlab libraries are linked dynamically, they must be
accessible at run time.  Before running a DAKOTA binary that includes
Matlab support, make sure the path to the Matlab shared object
libraries is included in your {\tt LD\_LIBRARY\_PATH} environment
variable.  For example to accomplish this in BASH on 32-bit Linux, one
might type
\begin{small}
\begin{verbatim}
...
Example files corresponding to the following tutorial are available in \\ {\tt Dakota/examples/linked\_interfaces/Matlab}.
...
Dakota.
  ...
  numADV              number active discrete variables
  numDerivVars        number of derivative variables specified in directFnDVV
  xC                  continuous variable values ([1 x numACV]) 
  xD                  discrete variable values ([1 x numADV])
  xCLabels            continuous variable labels (cell array of numACV strings)
  xDLabels            discrete variable labels (cell array of  numADV strings)
