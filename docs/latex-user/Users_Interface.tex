...
\texttt{dprepro}, a template processing utility, to extract the
...
and is distributed with it (in \texttt{Dakota/scripts/pyprepro/}, or 
{\tt Dakota/bin} in a binary distribution). Dakota also provides a second,
...
variables. As shown in \texttt{simulator\_script}, the names for the Dakota
...
command line using \{\texttt{-\/-var} will be set as Immutable.
...
Immutable, it cannot be rassigned without first explicitly make it Mutable.

Note: Variables defined in files that are include using the command line 
\texttt{-\/-include} options are Immutable by default. Likewise, variables
defined using the \texttt{-\/-var} command line option are Immutable. However,
variables from files read in by the \texttt{include()} function are not 
