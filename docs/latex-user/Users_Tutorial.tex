...
\subsection{Running DAKOTA with simple input file}\label{tutorial:installation:quickstart:running}
...
\begin{enumerate}
  ...
  \item Copy \texttt{dakota\_rosenbrock\_2d.in} from the \texttt{Dakota/examples/tutorial/} directory to the working directory
  \item From the working directory, run \texttt{dakota -i dakota\_rosenbrock\_2d.in -o Dakota\_rosenbrock\_2d.out > Dakota\_rosenbrock\_2d.stdout}
\end{enumerate}
...
\begin{enumerate}
  \item The screen output has been redirected to the file \texttt{Dakota\_rosenbrock\_2d.stdout}. \\
  The contents are messages from DAKOTA and notes about the progress of the iterator (i.e. method/algorithm).
  \item The output file \texttt{Dakota\_rosenbrock\_2d.out} contains information about the function evaluations.
...
\begin{itemize}
  ...
  \item This analysis used a gradient-based optimization method, and the \texttt{Rosenbrock} test problem. More details about the analysis are in Section~\ref{tutorial:example:gradient2} and the example problem is described in Sections~\ref{tutorial:rosenbrock} and~\ref{additional:rosenbrock}.
...
\subsection {Examples of DAKOTA output}\label{tutorial:installation:quickstart:output}
...
\subsection {OLD Getting Started section} 
** We need to determine how much of this to put somewhere else, what to delete
...
directory \texttt{Dakota/examples/tutorial}. 
...
The results are shown later, in Section~\ref{tutorial:example:param_study}.
...
Figure~\ref{tutorial:rosenbrock_2d} is the \emph{strategy} block.
...
\section{Examples}\label{tutorial:example}
...
\subsection{Rosenbrock Test Problem}\label{tutorial:example:rosenbrock}
...
\subsection{Two-Dimensional Grid Parameter Study}\label{tutorial:example:param_study}
...
listed in Figure~\ref{tutorial:rosenbrock_2d}. This is executed by DAKOTA using the
following command:
\begin{small}
\begin{verbatim}
...
\subsection{Gradient-based Unconstrained Optimization}\label{tutorial:example:optimization}
...
\subsection{Uncertainty Quantification with Monte Carlo Sampling}\label{tutorial:example:uncert_quant}
