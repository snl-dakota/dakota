...
More detailed information about installation can be found 
on the Dakota website and wiki pages ****.
...
\end{verbatim}
    ...
    Section~\ref{tutorial:quickstart:running}.
...
{\bf First Steps}
\begin{enumerate}
  \item Make sure Dakota runs.  To perform a quick check, one can enter
 \texttt{dakota -v} to make sure a Dakota executable is present in the directory being 
run.  This command will print the Dakota version.  For more detailed command line 
options, see Section~\ref{tutorial:installation:running}.
  \item Create a working directory 
  \item Copy \texttt{rosen\_multidim.in} from the \texttt{Dakota/examples/tutorial/}
...
\item Example input files are included throughout this manual, and
  also in {\tt Dakota/examples/tutorial}, {\tt
    Dakota/examples/methods}, and {\tt Dakota/test} in Dakota
