...
\subsection{Running Dakota with simple input file}\label{tutorial:quickstart:running}
...
\begin{enumerate}
  ...
  \item Copy \texttt{dakota\_rosenbrock\_multidim.in} from the \texttt{Dakota/examples/tutorial/}
 directory to the working directory
  \item From the working directory, run \texttt{dakota -i 
dakota\_rosenbrock\_multidim.in \textbackslash \\
-o Dakota\_rosenbrock\_multidim.out > Dakota\_rosenbrock\_multidim.stdout}
\end{enumerate}
...
A simple Dakota
input file (that is named \texttt{dakota\_rosenbrock\_multidim.in})
...
\begin{figure}[ht!]
  ...
  \begin{bigbox}
    \begin{small}
      \verbatimtabinput[8]{dakota_rosenbrock_multidim.in}
    \end{small}
  \end{bigbox}
...
\begin{small}
\begin{verbatim}
    dakota -i dakota_rosenbrock_multidim.in \\
      -o Rosenbrock_multidim.out > Rosenbrock_multidim.stdout
\end{verbatim}
\end{small}

The output of the Dakota run is written to the file named
\texttt{Rosenbrock\_multidim.out} while the screen output, 
or standard output, is redirect to \texttt{Rosenbrock\_multidim}.
 For comparison, files named 
\texttt{Rosenbrock\_multidim.out.sav} and
\texttt{Rosenbrock\_multidim.stdout.sav} are
...
are in the \texttt{Dakota/examples/tutorial} directory, with \texttt{.sav} 
