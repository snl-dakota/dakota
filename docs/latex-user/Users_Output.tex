...
output capability that is useful as a monitoring tool.  The JAGUAR
Dakota GUI is an emerging capability that will provide more advanced
...
column is the interface ID (which is EMPTY if the user does not
specify a name for the interface), the third and fourth columns are
the design parameter values (labeled in the example as ``\texttt{H}''
and ``\texttt{D}''), the fifth column is the objective function
(labeled ``\texttt{obj\_fn}''), and the sixth column is the nonlinear
equality constraint (labeled ``\texttt{nln\_eq\_con\_1}'').
...
of inputs (variables) and outputs (responses).
...
\texttt{export\_points\_file = \emph{STRING}}.  When specified, any
approximate evaluations performed on the surrogate model will be
output to the specified data file.  The data file will be exported in
the same format as top-level tabular output data described in
Section~\ref{output:tabular}.  This facilitates plotting or external
diagnostics of the surrogate model.
...
(variables) data in tabular format (see Section~\ref{output:tabular})
with data columns corresponding to each variable.  This file can be
generated with sampling, parameter study, and DACE methods by invoking
\begin{small}
\begin{verbatim}
...
be imported using Dakota's post-run mode.
