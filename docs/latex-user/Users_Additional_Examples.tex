...
for each example can be found in the {\tt Dakota/examples/methods} directory.}
...
The following command runs this example:
\begin{small}
\begin{verbatim}
...
\texttt{Dakota/examples/methdos} for comparison purposes. The
...
% % requires modification to \texttt{Dakota\_Source/test/text\_book.C} and will
...
The file \texttt{leastsquares.out.sav} is included in the
\texttt{Dakota/examples/methods} directory for comparison purposes. The
...
\texttt{objective\_functions = 1} in the responses specification,
and select an optimizer (e.g., \texttt{optpp\_q\_newton}) in the
method specification, e.g., as shown in 
{\tt dakota\_addtnl\_rosen\_opt.in}:
\begin{center}
  \begin{small}
    \begin{bigbox}
...
changed to \texttt{least\_squares\_terms = 2} and the method
specification is changed to a least squares method, e.g.,
\texttt{optpp\_g\_newton}, as shown in {\tt dakota\_addtnl\_rosen\_ls.in}:
\begin{center}
  \begin{small}
    \begin{bigbox}
...
An example using the cylinder head test problem is shown below in
{\tt dakota\_addtnl\_cylhead.in}:
\begin{center}
  \begin{small}
    \begin{bigbox}
...
\texttt{dakota\_container.in}. The solution to this example
...
\subsection{Constrained Gradient Based Optimization}
The test problem is solved using {\tt dakota\_addtnl\_cantilever.in}:
\begin{center}
  \begin{small}
    \begin{bigbox}
...
Figure~\ref{additional:moga2inp} and provided in {\tt dakota\_mogatest2.in} . It differs from
...
The input file for this example is shown in
Figure~\ref{additional:moga3inp} and provided in {\tt dakota\_mogatest3.in}. It differs from
...
The dakota input {\tt dakota\_psuade.in} exercises
...
reliability analyses. {\bf These are NOT included in the {\tt Dakota/examples/methods} 
...
\texttt{dakota\_logratio\_taylor2.in} input files in directory 
{\tt Dakota/test/}. 
