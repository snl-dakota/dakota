...
\begin{equation*}
  \mathcal{L}\left( {\theta ;d} \right) = f\left( {q(\theta) - d} \right)
\end{equation*}
where $\theta$ are the parameters of model quantity of interest $q$. 
...
simulation quantity of interest $q_i$. We further 
...
\left[ - \frac{\left(d_i-q_i({\theta})\right)^2}{2{\sigma_d}^2} \right]
...
In Equation~\ref{eqn:Likelihood}, the ${\sigma_d}^2$ term refers to the measurement error of the data, assumed 
constant across all data observations in this case.  A more general case, which we support, is to 
have a full covariance matrix, $\Sigma_d$, that specifies the covariance between each observation $i$ and $j$. 
...
\log{\mathcal{L}({\theta};d)} \propto % =-0.5 {R}^{T} {\Sigma}^{-1} {R}
-\frac{1}{2} \boldsymbol{r}^T \boldsymbol{\Sigma}_d^{-1} \boldsymbol{r} 
\end{equation}

where $\boldsymbol{r}$ is the vector of residuals between the data points and the model quantity of interest, $q(\theta)-d$.  
There are now four options to specify 
...
\texttt{scalar} where a constant value ${\sigma_d}^2$ is given for all observations, \texttt{diagonal} where a 
value is specified for the diagonal elements of the covariance matrix $\Sigma_d$ meaning that each observation 
has its own measurement error but there are no cross terms, and \texttt{matrix} where the full covariance 
matrix $\Sigma_d$ is specified.  Note that the \texttt{diagonal} and \texttt{matrix} terms are only available 
for field response data.  For scalar responses, the only variance values that are specified are with \texttt{none} or 
\texttt{sigma}.  Finally, all the sigma values should be given as variances (${\sigma_d}^2$).
...
Since each iteration involves an evaluation of the model to obtain 
$q(\theta)$, often surrogate models of the simulation 
model are employed.
...
For more detail about the formulation of the various proposal covariance options, see the Theory Manual. 
