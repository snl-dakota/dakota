...
%may involve either cumulative or complementary cumulative distribution functions.  
...
and \texttt{Dakota/test/dakota\_textbook\_lhs.in}
...
file is \texttt{textbook\_uq\_sampling.in} in the 
\texttt{Dakota/examples/users} directory.  The number of samples to
...
\texttt{logratio\_uq\_reliability.in} in the {Dakota/examples/users}
...
is \texttt{rosen\_uq\_sc.in} in the \texttt{Dakota/examples/users}
...
the test file \texttt{cantilever\_uq\_global\_interval.in}, and also
...
uncertain variables. This input file is \texttt{textbook\_uq\_glob\_evidence.in}, found in the \texttt{Dakota/examples/users} 
...
\begin{small}
\begin{verbatim}
         dakota textbook_uq_meanvalue.in > mv.out
\end{verbatim}
\end{small}
See the file \texttt{mv.out.sav} in \texttt{Dakota/examples/users} 
...
A typical Dakota input file for performing an uncertainty
quantification using polynomial chaos expansions is shown in
Figure~\ref{uq:examples:pce_input}, \texttt{rosen\_uq\_pce.in}.
...
is found in the test file \texttt{cantilever\_uq\_global\_interval.in}, 
and also in Figure~\ref{uq:examples:interval_input}, with example results in 
