...
configuration, $M_i(\boldsymbol{\theta}, x_{k,new}) + \delta_i(x_{k,new})$ 
...
When each corrected model value $M_i(\boldsymbol{\theta}^{*}, x_{k, new}) +
...
By setting \texttt{max\_hifi\_evaluations} to zero, Dakota will run the intial
