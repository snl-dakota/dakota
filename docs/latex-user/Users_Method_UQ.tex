...
In some situations, there are multiple models that represent a phenomenon and 
the user is left with the task to determine which is most appropriate. 
In this case, Bayesian model selection may help.  Suppose that the user 
has a set of models, $\mathcal{M}$={$M_1,M_2...M_m$} from which to choose.  
In the Bayesian setting, each of these models may be updated according to 
Bayes' rule: 
\begin{equation}
...
as the likelihood in a version of Bayes' rule which calculates the posterior
model plausibility as: 
\begin{equation}
...
that the sum of all model plausibilities is 1.0.  Model selection 
involves choosing the model with the highest posterior model plausibility. 
Model evidence is defined as the likelihood in 
Equation~\ref{eq:model_plausibility}.  Model evidence is determined by 
averaging the likelihood of its model parameters over all possible values 
of the model parameters, according to their prior distributions.  It is 
...
There are many ways to calculate model evidence. There are currently two implemented 
in Dakota. The user first specifies \texttt{model\_evidence}, then either 
...
distribution of the parameters, calculating the likelihood of that model, and 
estimating the integral given in Eq.~\ref{eq:uq:model_evidence} by 
brute force.  The number of samples used in the sampling of the integral is 
...
as outlined in~\cite{Wasserman}.  It has the assumption that the posterior distribution is nearly Gaussian, which is not always a safe assumption. 
...
\end{equation}
where $N_i$ is the number of parameters in the i-th model and $H$ is the negative 
Hessian of the log-posterior.  This implementation only requires 
the evaluation of the model likelihood at the MAP point, although 
it also requires the Hessian calculation of the log-posterior density. 
