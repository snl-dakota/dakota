...
Sampling techniques are selected using the \texttt{nond\_sampling}
...
specifies \texttt{lhs} as the sampling method under \texttt{nond\_sampling}.
...
\texttt{nond\_local\_reliability} method, followed by the MPP search 
...
If the user specifies \texttt{nond\_local\_reliability} as a method 
...
Additional expansion form alternatives can also be considered.  Of
...
%The number of random dimensions, the order of the expansion, and the
%order of a numerical integration scheme are all potential sources for
...
The use of independent standard random variables is the critical
...
% would be interesting to add comparisons among strictly Hermite 
% PCE vs. generalized PCE for correlated Askey variables.
...
Figure~\ref{fig:pascal_tensor_quad5_Gauss} displays the monomial
...
Given an index set of levels, linear or nonlinear growth rules may be
...
\caption{For a two-dimensional parameter space ($n=2$) and maximum level
  ...
  and the Gaussian abscissas (right).} % for ${\rm w}=5$. }
\label{fig:isogrid_N2_q7}
\end{center}
\end{figure}

Figure~\ref{fig:pascal_sparse_lev4_Gauss} displays the monomial
...
\begin{figure}[htbp]
  ...
  \subfigure[Nonlinear growth rule.]{\includegraphics{images/SparseLevel4_NonlinGauss}}
  \subfigure[Linear growth rule.]{\includegraphics{images/SparseLevel4_LinGauss}}
  \end{subfigmatrix}
  \caption{Pascal's triangle depiction of integrand monomial coverage 
...
Given a mechanism for defining anisotropy, we must extend the
...
methods to calculate these bounds.  \texttt{nond\_local\_interval\_est}
...
recommend using either \texttt{nond\_global\_interval\_est} or 
\texttt{nond\_local\_interval\_est}.  An example of interval estimation 
...
either \texttt{nond\_global\_evidence} or 
\texttt{nond\_local\_evidence} in the DAKOTA input file.  
...
As in other nondeterministic methods, with \texttt{nond\_local\_evidence}
or \texttt{nond\_global\_evidence},
...
To elaborate on the differences between \texttt{nond\_global\_evidence}
and \texttt{nond\_local\_evidence}: both of these methods
...
methods to calculate these bounds: \texttt{nond\_local\_evidence}
allows the user to specify either \texttt{sqp} which is sequential 
quadratic programming, or \texttt{nip} which is a nonlinear interior point 
method.  We also offer the capability to use global methods to 
assess these interval cell bounds. \texttt{nond\_global\_evidence}
allows the user to specify either \texttt{lhs}, which performs 
Latin Hypercube Sampling and takes the minimum and maximum of 
