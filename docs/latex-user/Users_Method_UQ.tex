...
value-based or gradient-enhanced, and nodal or hierarchical. Both sets
of methods provide analytic response moments and variance-based
metrics; however, CDF/CCDF probabilities are evaluated numerically by
...
Section~\ref{uq:examples:pce}, which illustrates PCE defined from an
...
polynomials. This section presents a more sophisticated example, where
we use stochastic collocation built on an anisotropic sparse grid
defined from numerically-generated orthogonal polynomials. The
uncertain variables are lognormal in this example and the orthogonal
polynomials are generated from Gauss-Wigert recursion
...
the \texttt{Dakota/examples/users/cantilever\_uq\_global\_interval.in}, and also
...
\section{Additional UQ Examples}
...
The PCE and SC methods have been described in Section~\ref{uq:expansion},
with an SC example provided in Section~\ref{uq:uncertainty2}.  This section
provides an additional PCE example.
