...
of steps, as discussed earlier in this section. Assessing the convergence
of the chain is an active area of research, and the implemention of a number 
of metrics for chain convergence is undergoing active development in Dakota,
and can be triggered during a Bayesian calibration through the use of the
keyword \texttt{chain\_diagnostics}. 

As of Dakota 6.10, 
\texttt{confidence\_intervals} is the only diagnostic implemented. These
confidence intervals are calculated using the asympotically valid
interval estimator~\cite{Fle10}
\begin{equation}
  \bar{g}_{n} \pm t_{*} \frac{\hat{\sigma}_{n}}{\sqrt{n}},
\end{equation}
where $\bar{g}_{n}$ is the estimated moment (i.e. mean or variance), $t_{*}$ 
is the $t$-value for the 95th quantile, $n$ is the sample size, and
$\hat{\sigma}_{n}$ is approximated using batch means estimation. To 
approximate $\hat{\sigma}_{n}$, the Markov chain produced during calibration 
is broken up into ``batches," the sample moment is calculated for each batch, 
and $\hat{\sigma}_{n}$ is an unbiased estimate of the standard deviation in 
the moment calculations over the batches. The confidence intervals produced
are 95\% confidence intervals, and they are calculated for the mean and
variance (first and second moments) for each parameter and each response.
Further details regarding the default settings for these calculations can be
found in the Dakota Theory Manual~\cite{TheoMan}. 
...
say by 10\%, and the diagnostics reevaluated for signs of chain convergence.
Furthermore, if \texttt{output} is set to \texttt{debug}, the sample moment
...
calibration problems, using the \texttt{calibration\_term\_scales}
...
residuals-squared can be weighted via the \texttt{calibration\_terms
...
after error variance weighting and before \texttt{weights}
application. The \texttt{calibration\_terms} keyword documentation in
