...
algorithm~\cite{Gilks}, though this is an active research area.  MCMC
...
Dakota offers three approaches for Bayesian calibration: QUESO, DREAM,
and GPMSA.  They are specified with the \texttt{bayes\_calibration}
keyword in combination with the \texttt{queso}, \texttt{dream}, or
\texttt{gpmsa} selections, respectively.  The QUESO method uses
components from the QUESO library (Quantification of Uncertainty for
Estimation, Simulation, and Optimization) developed at The University
of Texas at Austin.  It implements the Delayed Rejection and Adaptive
...
elements.
%
DREAM uses an implementation of DiffeRential Evolution Adaptive
Metropolis developed by John Burkardt.  The DREAM approach runs
concurrent chains for global exploration, and automatically tunes the
proposal covariance during the process by a self-adaptive randomized
subspace sampling~\cite{Vrugt}.
%
...
\texttt{chains} specification.  The total number of generations per chain in i
...
\subsection{Comparison of features for Bayesian Methods}
...
\begin{tabular}{|c|c|c|}
\hline
Capability           &  QUESO  & DREAM \\
\hline
Prior Distribution Types                &  Any continuous uncertain & Only uniform priors \\
                                        &  distributions in Dakota  &                    \\
\hline
MCMC Type                               &  Delayed Rejection or &  DREAM (Differential \\
                               &  Adaptive Metropolis  or &  Evolution Adaptive  \\
                               &  Metropolis Hastings  or &  Metropolis) \\
                               &  Delayed Rejection  &   \\
                               &  Adaptive Metropolis (DRAM)  &   \\
\hline
Can use PCE/SC Emulator                 &  Yes                      &  Yes               \\
\hline
Can use a GP Emulator                   &  Yes                      &  Yes               \\
\hline
Can update the emulator adaptively            &  Yes                      &  No          \\     
to focus on high likelihood points            &                       &                \\
\hline
Use MAP optimal point           &  Yes                      &  No               \\
as starting point for MCMC chain             &                       &                \\
(optimization can use emulator or actual model)   &                       &                \\
\hline
Can import build points for emulator            &  Yes                      &  Yes               \\             
\hline
Can specify proposal covariance from            &  Yes                      &  No             \\            
prior, user data, or derivatives               &                       &                \\
\hline
Can calibrate error covariance multipliers           &  Yes                      &  Yes         \\                
which multiply the $\Sigma$ matrix used                &                       &                \\
in the log-likelihood function                 &                       &                 \\             
\hline
Can be used in standardized space           &  Yes                    &  Yes               \\             
\hline
Logit transform           &  Yes                     &  No              \\             
\hline
Can export samples from the chain           &  Yes                     &  Yes              \\             
%Can export samples from the posterior density           &  No                     &  No              \\             
\hline
\end{tabular}
\end{table}
...
input file such as the one shown in~\ref{uq:figure18}.  Here, the
QUESO DRAM (delayed rejection adaptive metropolis) solver is
...
\texttt{mod\_cantilever} algebraic model, so an emulator is not warranted,
but the commented lines hint at related user options.  The proposal
covariance used has diagonal element values of 1.e6 and 0.1. Two credible and
prediction intervals will be calculated for each model output: a 5/95 interval
and a 10/90 interval. The calibration terms in the responses section refers to 
the number of outputs that will be used in the calibration process: in this 
case, it is just two. The calibration data file has the observational data: in
this case, it is a freeform file (e.g. no header or annotation) with
ten experiments. For each experiment, there are two experiment values,
one for stress and one for displacement, followed by two variance
values for the error associated with that experiment for each quantity
of interest. 
...
accordance with Bayes Theorem~\ref{eq:BayesThm} and the likelihood
...
posterior probability (an estimate of the MAP point).  The final output 
also summarizes the moments of the posterior samples from the chain (e.g. 
mean of the chain, standard deviation of the chain samples, etc.), as well as
the credible and prediction intervals for each model output.

A variety of auxilliary output also exists.  The MCMC sample output is
put into a directory called \texttt{QuesoDiagnostics} in the directory from
...
the \texttt{QuesoDiagnostics} directory contain the chain files.  The files
...
chain values of the parameters\footnote{The full chain will be output
  ...
  refinement data or restarting points, respectively.}. 
In addition, the accepted chain values that Dakota
...
\texttt{sub\_sampling\_period} are specified, the filtered acceptance chain
is written to the file \texttt{dakota\_mcmc\_tabular.dat}. This file 
contains the posterior values of the filtered MCMC chain, as well as the values 
of state variables and the resulting model responses. Finally, if one wants to 
see the likelihood of each point, specifying verbose output in the method 
section will result in the likelihoods being printed. 
...
which to run the high-fidelity model to add to the calibration data. The low-
fidelity posterior parameter distribution is then updated again using Bayesian
calibration. The mutual information is used as the selection criterion to guide
the process of high-fidelity data acquisition.
...
%Further theoretical and implementation details of this algorithm may be found 
%in~\cite{TheoMan}.
%From these candidate designs, that which maximizes the mutual information with 
%respect to the current state of the low-fidelity model parameters is deemed 
%``optimal." The mutual information between low-fidelity model parameters 
%$\boldsymbol{\theta}$ and the high-fidelity data $\textbf{d}(\xi)$, gathered at 
%design conditions $\xi$, is given by
%\begin{equation}
%I(\boldsymbol{\theta}, \textbf{d}(\xi)) = \iint p(\boldsymbol{\theta}, 
	%\textbf{d}(\xi)) \log \frac{ p(\boldsymbol{\theta}, \textbf{d}(\xi)) }
	%{p(\boldsymbol{\theta}) p(\textbf{d}(\xi))} d\boldsymbol{\theta}
	%d\textbf{d}.
%\end{equation}
%This quantity is approximated using the low-fidelity model and the $k$-nearest
%neighbor algorithm proposed by Kraskov~\cite{Kra04}, as detailed 
%in~\cite{Lew16}. %and the Dakota Theory Manual \ref{}.
