...
documentation.  Dakota currently exposes two types of MCMC approaches
as supplied by MUQ: Metropolis-Hastings and Adaptive Metropolis.  The
MUQ implementation in Dakota is a preliminary one, anticipated to grow
in future releases of Dakotato to leverage components in MUQ that
support Hamiltonian Monte Carlo and Langevin-based sampling
strategies.  Note that the MUQ is an experimental capability within
Dakota. Thus, the library is not exposed by default and must be
enabled explicitely when compiling Dakota.
