...
generated~\cite{paul_tpq}.  Moreover, this equivalence can be extended
to sparse grids based on standard Gauss rules, provided that a sparse
PCE is formed based on a weighted sum of tensor expansions~\cite{paul_ssg}.
...
expansions (see Section~\ref{sec:spectral_sparse}).
...
general, and possible approaches include the Rosenblatt~\cite{rosen},
Nataf~\cite{nataf}, and Box-Cox~\cite{boxcox} transformations.
