...
addition, the mean value methods additionally output estimates of the
mean and standard deviation of the response functions along with
importance factors for each of the uncertain variables in the case of
...
We also support the more general case of a full covariance matrix, $\boldsymbol{\Sigma_d}$, that specifies the covariance between each observation $i$ and $j$. 
...
matrix $\boldsymbol{\Sigma_d}$ meaning that each observation has its own
measurement error but there are no cross terms, and \texttt{matrix}
where the full covariance matrix $\boldsymbol{\Sigma_d}$ is specified.  
Note that the \texttt{diagonal} and \texttt{matrix} terms are only 
available for field response data.  For scalar responses, variance types
...
faster chain mixing.  
%Note that the derivative-based proposal covariance 
...
Another feature is that the user can perform a deterministic gradient-based optimization before the MCMC sampling to get a good starting point for the chain.  The goal of the optimization is to maximize the log posterior, which is defined as the log-likelihood minus the log-prior. The Markov Chain in Bayesian calibration will be started at the best point found in the optimization (the maximum a posteriori probability point, also called the MAP point).  This will give an optimal starting point for the MCMC chain, which can help the process avoid a lot of the initial searching for high posterior probability points. Use of the optimization to generate a MAP starting point is specified with the keyword \texttt{pre\_solve}.  The pre-solve option can be used with an emulator or with no emulator.   
...
\hline
Uses MAP optimal point           &  Yes                      &  No               \\
as starting point for MCMC chain             &                       &                \\
(optimization can use emulator or actual model)   &                       &                \\
\hline
Uses inverse Hessian of MAP point            &  Yes                      &  No        \\       
as initial proposal covariance              &                       &                \\
\hline
Can import build points for emulator            &  Yes                      &  Yes               \\             
\hline
Can specify the proposal covariance from            &  Yes                      &  No             \\            
the variance of the prior distribution or               &                       &                \\
from values specified by the user or                &                       &                \\
