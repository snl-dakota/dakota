...
https://bitbucket.org/mituq/muq2/src/master/ and https://mituq.bitbucket.io/index.html for additional documentation. 
Dakota currently exposes two types of MCMC 
approaches as supplied by MUQ: Metropolis-Hastings and Adaptive Metropolis. 
The MUQ implementation in Dakota is a preliminary one, anticipated to grow 
in future releases of Dakotato to leverage components in MUQ that
support Hamiltonian Monte Carlo and Langevin-based sampling strategies. 
Note that the MUQ is an experimental capability within Dakota. Thus, the library is not exposed by
default and must be enabled explicitely when compiling Dakota.
...
\hline
Prior Distributions  &  Any continuous &  Any continuous &  Any continuous & Uniform only & Uniform only \\
                     &  variable type  &  variable type  &  variable type  &              &              \\
\hline
Inference Type                 & MCMC with DR,   & MCMC with MH,   & MCMC with DR,   & MCMC with              & MCMC-free         \\
                               & AM, DRAM, or MH & AM & AM, DRAM, or MH & Differential Evolution & interval analysis \\
                               &                 &                 &                 & Adaptive Metropolis    &                   \\
\hline
Can use PCE/SC Emulator        &  Yes            & Yes              & Yes                    & Yes \\
\hline
Can use GP Emulator            &  Yes            &  Yes            & Yes (required)  & Yes                    & Yes \\
\hline
Likelihood-based adaptive      &  Yes            &  No             &  No             & No                     & No \\     
emulator update                &                 &                 &                 &                        &    \\
\hline
