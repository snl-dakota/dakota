...
\begin{equation*}
  \mathcal{L}\left( {\theta ;d} \right) =  \mathcal{F}(q(\theta) - d)
\end{equation*}
...
in a likelihood function for $\theta$ that is the product of $n$
...
points and the model quantity of interest, $q(\theta)-d$.
...
evaluation of the model to obtain $q(\theta)$, surrogate models of
...
the MCMC algorithm and generate a posterior sample of $\theta$ in
