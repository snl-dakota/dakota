...
#define find_dlsym(a, b, c) \
  (a = (dl_constructor_t)GetProcAddress((HINSTANCE)(b), c))
...
// (This machinery is compiled into DAKOTA unless configured with
// --without-npsol). The corresponding dl_npsol.dll is not loaded until the
// database is locked, but dl_npsol.dll then invokes "new NPSOLOptimizer(...)",
// which needs to have the database unlocked.  Note that unlocking then may give
// wrong results in multi-specification runs.  The configure machinery does not
// know about compilation with -DDLSOLVER_DEBUG_UNLOCK, since it is only meant
// for use in special cases by knowledgeable developers.
...
static RealVector const *nonlinear_ineq_constraint_lower_bounds1(
    Optimizer1 *o) {
  return &o->M->nonlinear_ineq_constraint_lower_bounds();
}

static RealVector const *nonlinear_ineq_constraint_upper_bounds1(
    Optimizer1 *o) {
...
static void GetFuncs1(Optimizer1 *o, int m0, int m1, double *f) {
  ...
  for (int i = m0; i < m1; i++) f[i - m0] = (*RV)[i];
}

static void GetGrads1(Optimizer1 *o, int m0, int m1, int n, int is, int js,
                      double *g) {
  int i, i1, j;
  Response const *R = &o->M->current_response();
...
Dakota_funcs DakFuncs0 = {fprintf,
                          abort_handler,
                          dlsolver_option,
...
void DLSolver::cleanup() {
  void *h;
  if (h = dlLib) {
    dlLib = NULL;
    if (dl_destructor) (*dl_destructor)(&dl_Optimizer);
    dlclose(h);
    if (pdlLib) *pdlLib = 0;
  }
  if (details) {
...
void DLSolver::botch(const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  fprintf(stderr, "\nDLSolver Error: ");
...
DLSolver::DLSolver(std::shared_ptr<Model> model)
  ...
  if (!s) botch("new(%lu) failure in DLSolver::DLSolver.", (unsigned long)L);
  memcpy(s, s0, L);
  vp = probDescDB.get_voidss("method.dl_solver.dlLib");
  pdlLib = *vp ? vp : 0;
}

DLSolver::~DLSolver() { cleanup(); }

void DLSolver::core_run() {
  Dakota_funcs *df;
  Dakota_probsize ps;
  char *s, *s0;
  ...
  if (!dl_core_run) {  // Load the shared library if this is
    ...
    if (!df) botch("new Dakota_Funcs failure");
    memcpy(df, &DakFuncs0, sizeof(Dakota_funcs));
    for (s0 = s = details; *s > ' '; s++);
    if (s == s0) botch("dlDetails not given for dl_solver.");
    if (*s) *s++ = 0;
    if (pdlLib)
      h = *pdlLib;
    else
    ...
    DEBUG_unlock dl_Optimizer =
        (*dl_constructor)(this, df, &dl_core_run, &dl_destructor);
    DEBUG_lock
  }
  if (dl_core_run) {
    df = DF;
...
int dlsolver_option(Opt_Info *Oi) {
  char *in, *in0, *s, *val0;
  int c;

  if (!(in = Oi->begin)) return 0;
  while (*in <= ' ')
    if (!*in++) return 0;
  Oi->name = in0 = in;
  for (; (c = *in) > ' '; ++in) {
