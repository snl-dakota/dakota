 ...
 */
class JEGAOptimizer::Evaluator : public GeneticAlgorithmEvaluator {
  /*
  ===========================================================================
 ...
 protected:
   ...
   *             continuous values.
   */
  void SeparateVariables(const Design& from, RealVector& intoCont,
                         IntVector& intoDiscInt, RealVector& intoDiscReal,
                         StringMultiArray& intoDiscString) const;
 ...
 public:
  ...
  Evaluator(GeneticAlgorithm& algorithm, Model& model)
      : GeneticAlgorithmEvaluator(algorithm),
        _model(model){EDDY_FUNC_DEBUGSCOPE}
      ...
      : GeneticAlgorithmEvaluator(copy),
        _model(copy._model){EDDY_FUNC_DEBUGSCOPE}
         ...
         * \param model The model through which evaluations will be done.
         */
        Evaluator(const Evaluator& copy, GeneticAlgorithm& algorithm,
                  Model& model)
      : GeneticAlgorithmEvaluator(copy, algorithm), _model(model) {
    EDDY_FUNC_DEBUGSCOPE
  }
...
class JEGAOptimizer::EvaluatorCreator
 ...
 public:
   ...
   *                 created evaluator.
   */
  EvaluatorCreator(Model& theModel) : _theModel(theModel) {
    EDDY_FUNC_DEBUGSCOPE
  }
 ...
 */
class JEGAOptimizer::Driver : public JEGA::FrontEnd::Driver {
  /*
  ===========================================================================
  Member Data Declarations
  ===========================================================================
 ...
 public:
   ...
   */
  Driver(const ProblemConfig& probConfig) : JEGA::FrontEnd::Driver(probConfig) {
    EDDY_FUNC_DEBUGSCOPE
  }
...
void JEGAOptimizer::core_run() {
  ...
  if (!this->_initPts.empty()) {
    ...
    pdb.AddIntegralParam("method.population_size",
                         static_cast<int>(oldInit.GetSize()));

    pdb.AddDoubleMatrixParam("method.jega.design_matrix",
                             ToDoubleMatrix(initial_points()));

    GeneticAlgorithmInitializer* newInit =
        AllOperators::FullInstance().GetInitializer("double_matrix", *theGA);
    ...
    JEGAIFLOG_II_F(
        !theGA->SetInitializer(newInit), theGA->GetLogger(), this,
        text_entry(
            lfatal(),
            name + ": Unable to set the initializer to "
                   "double_matrix because it is incompatible with the other "
                   "operators."))

    JEGAIFLOG_II_F(
        !newInit->ExtractParameters(pdb), theGA->GetLogger(), this,
        text_entry(lfatal(), name +
                                 ": Failed to retrieve the parameters for \"" +
                                 newInit->GetName() + "\"."));
  }

  JEGALOG_II_G(
      lverbose(), this,
      text_entry(lverbose(), name + ": About to perform algorithm execution."))
  ...
  JEGAIFLOG_II_G(
      designSortMap.size() == 0, lquiet(), this,
      text_entry(
          lquiet(),
          name + ": was unable to identify at least one "
  ...
  for (; best_it != best_end; ++best_it, ++index) {
    this->LoadDakotaResponses(*(best_it->second),
  ...
  JEGALOG_II_G(
      lquiet(), this,
      text_entry(lquiet(), name +
                               ": find optimum completed and all "
...
bool JEGAOptimizer::accepts_multiple_points() const { return true; }

bool JEGAOptimizer::returns_multiple_points() const { return true; }

void JEGAOptimizer::initial_points(const VariablesArray& pts) {
  this->_initPts = pts;
}

const VariablesArray& JEGAOptimizer::initial_points() const {
...
void JEGAOptimizer::LoadDakotaResponses(const JEGA::Utilities::Design& des,
  ...
  for (size_t i = 0; i < this->numDiscreteIntVars; ++i)
    di_vars[i] =
        static_cast<int>(des.GetVariableValue(i + this->numContinuousVars));

  // The next set of variables represent the discrete real variables.
  for (size_t i = 0; i < this->numDiscreteRealVars; ++i)
    dr_vars[i] = des.GetVariableValue(i + this->numContinuousVars +
                                      this->numDiscreteIntVars);
  ...
  mvd_dist_rep->pull_parameters(
      this->numContinuousVars + this->numDiscreteIntVars,
      this->numDiscreteStringVars, Pecos::DSS_VALUES, ddss_values);
  for (size_t i = 0; i < this->numDiscreteStringVars; ++i) {
    const int& element_index = static_cast<int>(des.GetVariableValue(
  ...
  for (size_t i = 0; i < static_cast<size_t>(this->numNonlinearConstraints);
       ++i)
...
void JEGAOptimizer::ReCreateTheParameterDatabase() {
  EDDY_FUNC_DEBUGSCOPE
  delete this->_theParamDB;
  this->_theParamDB = new BasicParameterDatabaseImpl();
}

void JEGAOptimizer::LoadTheParameterDatabase() {
  EDDY_FUNC_DEBUGSCOPE
  ...
  if (random_seed != 0)
    this->_theParamDB->AddIntegralParam("method.random_seed", random_seed);
  else
    this->_theParamDB->AddIntegralParam("method.random_seed", -1);

  // now all the reals
  const Real& constraint_penalty =
      probDescDB.get_real("method.constraint_penalty");
  if (constraint_penalty >= 0.)
    this->_theParamDB->AddDoubleParam("method.constraint_penalty",
                                      constraint_penalty);
  else
    this->_theParamDB->AddDoubleParam("method.constraint_penalty", 1.0);
  const Real& crossover_rate = probDescDB.get_real("method.crossover_rate");
  if (crossover_rate >= 0.)
    this->_theParamDB->AddDoubleParam("method.crossover_rate", crossover_rate);
  else
    this->_theParamDB->AddDoubleParam("method.crossover_rate", 0.75);
  if (this->probDescDB.get_string("method.mutation_type") != "")
    this->_theParamDB->AddDoubleParam(
        "method.mutation_rate", probDescDB.get_real("method.mutation_rate"));
  else
    this->_theParamDB->AddDoubleParam("method.mutation_rate", 0.1);
  this->_theParamDB->AddDoubleParam(
      "method.mutation_scale", probDescDB.get_real("method.mutation_scale"));
  double perc_change = probDescDB.get_real("method.jega.percent_change");
  this->_theParamDB->AddDoubleParam("method.jega.percent_change",
                                    (perc_change < 0) ? 1.0e-4 : perc_change);
  double conv_tol = probDescDB.get_real("method.convergence_tolerance");
  this->_theParamDB->AddDoubleParam("method.convergence_tolerance",
  ...
  this->_theParamDB->AddSizeTypeParam("method.max_function_evaluations",
                                      static_cast<size_t>(probDescDB.get_sizet(
  ...
  this->_theParamDB->AddBooleanParam(
      "method.print_each_pop", probDescDB.get_bool("method.print_each_pop"));
  ...
  if (selector != "")
    this->_theParamDB->AddStringParam("method.replacement_type", selector);
  else if (this->methodName == SOGA)
    this->_theParamDB->AddStringParam("method.replacement_type", "elitist");
  else if (this->methodName == MOGA)
    this->_theParamDB->AddStringParam("method.replacement_type", "below_limit");
  ...
  if (selector == "favor_feasible") {
    JEGALOG_II_G(
        ...
        text_entry(
            ...
            "fitness assessor.  Therefore, use of the \"" +
                fitness +
                "\" will be changed to use of the \"weighted_sum_only\"."))

    this->_theParamDB->AddStringParam("method.fitness_type",
                                      "weighted_sum_only");
  } else {
    if (fitness != "")
  ...
  if (crossover_operator != "")
    this->_theParamDB->AddStringParam("method.crossover_type",
                                      crossover_operator);
  else
    this->_theParamDB->AddStringParam("method.crossover_type",
                                      "shuffle_random");
  ...
  if (mutation_operator != "")
    this->_theParamDB->AddStringParam("method.mutation_type",
                                      mutation_operator);
  else
    this->_theParamDB->AddStringParam("method.mutation_type",
                                      "replace_uniform");
  ...
  this->_theParamDB->AddIntegralParam(
      "method.output", this->probDescDB.get_short("method.output"));
  this->_theParamDB->AddStringParam(
      "method.initialization_type",
      this->probDescDB.get_string("method.initialization_type"));
  this->_theParamDB->AddStringParam(
      "method.flat_file", this->probDescDB.get_string("method.flat_file"));
  ...
  this->_theParamDB->AddStringParam("method.jega.data_directory", "./");

  // Dakota does not currently expose the final data file name pattern
  // through the interface.
  this->_theParamDB->AddStringParam("method.jega.final_data_filename",
                                    "finaldata#.dat");

  // Dakota does not currently expose the main loop operator selection
  // through the interface.
  this->_theParamDB->AddStringParam("method.jega.mainloop_type",
                                    "duplicate_free");
  ...
  this->_theParamDB->AddDoubleVectorParam("method.jega.niche_vector",
                                          niche_vector);
  ...
  this->_theParamDB->AddDoubleVectorParam("method.jega.distance_vector",
                                          distance_vector);
  ...
  this->_theParamDB->AddDoubleVectorParam("responses.multi_objective_weights",
                                          mow_vector);
  ...
  this->_theParamDB->AddStringVectorParam("method.flat_files",
...
void JEGAOptimizer::LoadAlgorithmConfig(
  ...
  aConfig.SetAlgorithmName(this->method_id().empty()
                               ? method_enum_to_string(this->methodName)
                               : this->method_id());
}

void JEGAOptimizer::LoadProblemConfig(JEGA::FrontEnd::ProblemConfig& pConfig) {
  EDDY_FUNC_DEBUGSCOPE
...
void JEGAOptimizer::LoadTheDesignVariables(
  ...
  StringMultiArrayConstView clabels =
      m.current_variables().continuous_variable_labels();
  ...
  StringMultiArrayConstView dilabels =
      m.current_variables().discrete_int_variable_labels();
  ...
  for (i = 0, dsi_cntr = 0; i < this->numDiscreteIntVars; ++i) {
    if (di_set_bits[i]) {  // discrete set variables
      ...
      pConfig.AddDiscreteIntegerVariable(
          dilabels[i], JEGA::IntVector(dak_set.begin(), dak_set.end()));
      ++dsi_cntr;
  ...
  StringMultiArrayConstView drlabels =
      m.current_variables().discrete_real_variable_labels();
  for (i = 0; i < this->numDiscreteRealVars; ++i) {
    const RealSet& dak_set = dsrv[i];
    pConfig.AddDiscreteRealVariable(
        drlabels[i], JEGA::DoubleVector(dak_set.begin(), dak_set.end()));
  }
  ...
  StringMultiArrayConstView dslabels =
      m.current_variables().discrete_string_variable_labels();
  ...
  for (i = 0; i < this->numDiscreteStringVars; ++i) {
    ...
    for (j = 0; j < num_elements; ++j) element_index[j] = j;
    pConfig.AddDiscreteIntegerVariable(
        dslabels[i],
        JEGA::IntVector(element_index.begin(), element_index.end()));
  }

  // Now make sure that an info was created for each variable.
  EDDY_ASSERT(pConfig.GetDesignTarget().GetNDV() ==
              (this->numContinuousVars + this->numDiscreteIntVars +
               this->numDiscreteRealVars + this->numDiscreteStringVars));
}

void JEGAOptimizer::LoadTheObjectiveFunctions(
    JEGA::FrontEnd::ProblemConfig& pConfig) {
  EDDY_FUNC_DEBUGSCOPE
  ...
  for (size_t i = 0; i < this->numObjectiveFns; ++i)
    if (use_sense && max_sense[i])
      pConfig.AddNonlinearMaximizeObjective("Non-Linear Maximize " + labels[i]);
    else
      pConfig.AddNonlinearMinimizeObjective("Non-Linear Minimize " + labels[i]);

  // see to it that the numbers match up.
  EDDY_ASSERT(pConfig.GetDesignTarget().GetNOF() == this->numObjectiveFns);
}

void JEGAOptimizer::LoadTheConstraints(JEGA::FrontEnd::ProblemConfig& pConfig) {
  EDDY_FUNC_DEBUGSCOPE
  ...
  **************************************************************************/
  ...
  for (size_t i = 0; i < this->numNonlinearIneqConstraints; ++i)
    pConfig.AddNonlinearTwoSidedInequalityConstraint(
        "Non-Linear Two-Sided Inequality " + asstring(i), nln_ineq_lwr_bnds[i],
        nln_ineq_upr_bnds[i]);
  ...
  for (size_t i = 0; i < this->numNonlinearEqConstraints; ++i)
    pConfig.AddNonlinearEqualityConstraint("Non-Linear Equality " + asstring(i),
                                           nln_eq_targets[i]);
  ...
  for (size_t i = 0; i < numLinearIneqConstraints; ++i) {
    ...
    pConfig.AddLinearTwoSidedInequalityConstraint(
        "Linear Two-Sided Inequality " + asstring(i), lin_ineq_lwr_bnds[i],
        lin_ineq_upr_bnds[i], lin_ineq_coeffs_row);
  }
  ...
  const RealVector& lin_eq_targets =
      ModelUtils::linear_eq_constraint_targets(m);
  ...
  for (size_t i = 0; i < numLinearEqConstraints; ++i) {
    ...
    pConfig.AddLinearEqualityConstraint("Linear Equality " + asstring(i),
  ...
  EDDY_ASSERT(pConfig.GetDesignTarget().GetNCN() ==
              (this->numNonlinearIneqConstraints +
               this->numLinearIneqConstraints +
               this->numNonlinearEqConstraints + this->numLinearEqConstraints));
}

void JEGAOptimizer::GetBestSolutions(
...
void JEGAOptimizer::GetBestMOSolutions(
    const JEGA::Utilities::DesignOFSortSet& from,
    const JEGA::Algorithms::GeneticAlgorithm& theGA,
    std::multimap<RealRealPair, JEGA::Utilities::Design*>& designSortMap) {
  EDDY_FUNC_DEBUGSCOPE
  ...
  for (; design_it != design_end; ++design_it) {
    ...
    if (constraintViolation > 0.0)
      ...
      for (size_t i = 0; i < nof; ++i)
        utopiaDistance +=
            Math::Pow((*design_it)->GetObjective(i) - extremeSet.get_min(i), 2);
    }
...
void JEGAOptimizer::GetBestSOSolutions(
    const JEGA::Utilities::DesignOFSortSet& from,
    const JEGA::Algorithms::GeneticAlgorithm& theGA,
    std::multimap<RealRealPair, JEGA::Utilities::Design*>& designSortMap) {
  EDDY_FUNC_DEBUGSCOPE
  ...
  for (; design_it != design_end; ++design_it) {
    ...
    double objectiveFunction =
        SingleObjectiveStatistician::ComputeWeightedSum(**design_it, weights);
...
JEGA::DoubleMatrix JEGAOptimizer::ToDoubleMatrix(
    const VariablesArray& variables) const {
  EDDY_FUNC_DEBUGSCOPE
...
===============================================================================
*/
JEGAOptimizer::JEGAOptimizer(ProblemDescDB& problem_db,
                             ParallelLibrary& parallel_lib,
                             std::shared_ptr<Model> model)
    :  // Optimizer(problem_db, model, std::shared_ptr<TraitsBase>(new
       // JEGATraits())),
      Optimizer(problem_db, parallel_lib, model,
                std::shared_ptr<TraitsBase>(new JEGATraits())),
      _theParamDB(0x0),
      _theEvalCreator(0x0) {
  EDDY_FUNC_DEBUGSCOPE
  ...
  if (!JEGA::FrontEnd::Driver::IsJEGAInitialized()) {
    ...
    unsigned int rSeed =
        (rseed_temp < 0) ? 0 : static_cast<unsigned int>(rseed_temp);
    ...
    JEGA::FrontEnd::Driver::InitializeJEGA("JEGAGlobal.log", jegaLev, rSeed,
  ...
  JEGAIFLOG_II_G_F(
      !JEGA::FrontEnd::Driver::IsJEGAInitialized(), this,
...
JEGAOptimizer::~JEGAOptimizer() {
  ...
  delete this->_theParamDB;
  this->numInstances--;
  if (this->numInstances == 0)  // Force JEGA's global log file to close. See
                                // the comment on numInstances.
    JEGA::Logging::Logger::Global()
        .Gate()
        .get_log()
        .get_first_log()
        .close_stream();
...
===============================================================================
*/
void JEGAOptimizer::Evaluator::SeparateVariables(
    const Design& from, RealVector& intoCont, IntVector& intoDiscInt,
    RealVector& intoDiscReal, StringMultiArray& intoDiscString) const {
  EDDY_FUNC_DEBUGSCOPE

  size_t num_cv = ModelUtils::cv(this->_model),
         num_div = ModelUtils::div(this->_model),
         num_drv = ModelUtils::drv(this->_model),
         num_dsv = ModelUtils::dsv(this->_model);
  ...
  for (i = 0; i < num_dsv; ++i, ++dvi_cntr) {
    ...
    const int& element_index =
        static_cast<int>(dvis[dvi_cntr]->WhichValue(from));
    intoDiscString[i] = set_index_to_value(element_index, dssv_values[i]);
  }
}

void JEGAOptimizer::Evaluator::RecordResponses(const RealVector& from,
                                               Design& into) const {
  EDDY_FUNC_DEBUGSCOPE
  ...
  for (size_t i = 0; i < nof; ++i, ++loc) into.SetObjective(i, from[loc]);
...
bool JEGAOptimizer::Evaluator::Evaluate(DesignGroup& group) {
  EDDY_FUNC_DEBUGSCOPE
