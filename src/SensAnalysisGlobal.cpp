...
static const char rcsId[] =
    "@(#) $Id: SensAnalysisGlobal.cpp 6170 2009-10-06 22:42:15Z lpswile $";
...
size_t SensAnalysisGlobal::find_valid_samples(
    const IntResponseMap& resp_samples, BoolDeque& valid_sample) {
  ...
  for (size_t j = 0; j < num_obs; ++j, ++it) {
    ...
    if (valid_sample[j]) ++num_valid_samples;
  }

  return num_valid_samples;
}

void SensAnalysisGlobal::valid_sample_matrix(const VariablesArray& vars_samples,
...
void SensAnalysisGlobal::valid_sample_matrix(const RealMatrix& vars_samples,
                                             const IntResponseMap& resp_samples,
                                             const BoolDeque is_valid_sample,
                                             RealMatrix& valid_data) {
...
void SensAnalysisGlobal::values_to_ranks(RealMatrix& valid_data) {
  ...
  for (int i = 0; i < num_corr; ++i) {
    ...
    for (int rank = 0; vi_it != vi_end;) {
      ...
      std::pair<RealIntMultiMap::const_iterator,
                RealIntMultiMap::const_iterator>
...
/** This version is used when full variables objects are being
    ...
    coefficients. */
void SensAnalysisGlobal::compute_correlations(
    const VariablesArray& vars_samples, const IntResponseMap& resp_samples,
    const StringSetArray& dss_vals) {
  ...
  valid_sample_matrix(vars_samples, resp_samples, dss_vals, is_valid_sample,
                      valid_data);
  partial_corr(valid_data, numVars, simpleCorr, partialCorr,
               numericalIssuesRaw);
...
/** This version is used when compact samples matrix is being
    ...
    coefficients. */
void SensAnalysisGlobal::compute_correlations(
    const RealMatrix& vars_samples, const IntResponseMap& resp_samples) {
...
void SensAnalysisGlobal::simple_corr(RealMatrix& total_data, const int& num_in,
  ...
  for (int i = 0; i < num_corr; i++) {
    ...
    for (int j = 0; j < num_obs; j++) total_data(i, j) /= row_sumsq;
  }
  ...
  if (num_corr == num_in) {
    ...
    if (num_obs <= 1)
      ...
      corr_matrix.multiply(Teuchos::NO_TRANS, Teuchos::TRANS, 1.0, total_data,
                           total_data, 0.0);
      for (int i = 0; i < num_corr; ++i) {
        // set finite diagonal values to 1.0
        if (std::isfinite(corr_matrix(i, i))) corr_matrix(i, i) = 1.0;
    ...
    if (num_obs <= 1)
      ...
      RealMatrix total_data_in(Teuchos::View, total_data, num_in, num_obs, 0,
                               0);
      ...
      for (int i = 0; i < num_in; ++i)
        for (int j = 0; j < num_out; ++j) correl_adjust(corr_matrix(i, j));
    }
  }
}
...
void SensAnalysisGlobal::partial_corr(RealMatrix& total_data, const int num_in,
  ...
  for (int i = 0; i < num_in; ++i) {
    ...
    for (int j = 0; j < num_obs; ++j) {
      ...
      // V~i
      for (int k = 0; k < i; ++k) control_factors_Z(j, k) = total_data(k, j);
    ...
    if (use_qr) {
      ...
      for (; sv_keep < sing_vals.length(); ++sv_keep)
        if (sing_vals[sv_keep] < tol) break;
      ...
      if (sv_keep == 0)
        ...
        Zinv_Zt_X.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0, v_trans,
                           Zt_X, 0.0);
    ...
    for (int k = 0; k < num_out; ++k)
      if (numerical_except[i])
        ...
        corr_matrix(i, k) = partial_cov(0, k + 1) /
                            std::sqrt(partial_cov(0, 0)) /
...
bool SensAnalysisGlobal::has_nan_or_inf(const RealMatrix& corr) const {
  ...
  for (int j = 0; j < num_cols; ++j)
    for (int i = 0; i < num_rows; ++i)
      if (!std::isfinite(corr(i, j))) return true;
  return false;
}

// TODO: combine archive with print
void SensAnalysisGlobal::archive_correlations(const StrStrSizet& run_identifier,
  ...
  for (const String& label : var_labels) combined_desc.push_back(label.c_str());
  for (const String& label : resp_labels)
    combined_desc.push_back(label.c_str());

  archive_simple_correlations(run_identifier, iterator_results, var_labels,
                              resp_labels, combined_desc, inc_id, false);
  archive_simple_correlations(run_identifier, iterator_results, var_labels,
                              resp_labels, combined_desc, inc_id, true);
  archive_partial_correlations(run_identifier, iterator_results, var_labels,
                               resp_labels, inc_id, false);
  archive_partial_correlations(run_identifier, iterator_results, var_labels,
                               resp_labels, inc_id, true);
}

void SensAnalysisGlobal::archive_simple_correlations(
    const StrStrSizet& run_identifier, ResultsManager& iterator_results,
    const StringArray& var_labels, const StringArray& resp_labels,
    const std::vector<const char*>& combined_desc, const size_t& inc_id,
    bool rank) const {
  ...
  if (corr_matrix.numRows() == num_in_out &&
    ...
    iterator_results.insert(run_identifier, location, corr_matrix, scales);
...
void SensAnalysisGlobal::archive_partial_correlations(
    const StrStrSizet& run_identifier, ResultsManager& iterator_results,
    const StringArray& var_labels, const StringArray& resp_labels,
    const size_t& inc_id, bool rank) const {
  StringArray location;
  if (inc_id) location.push_back(String("increment:") + std::to_string(inc_id));
  if (rank)
  ...
  location.push_back("");
  if (corr_matrix.numRows() == numVars && corr_matrix.numCols() == numFns) {
    DimScaleMap scales;
    scales.emplace(0, StringScale("variables", var_labels));
    for (size_t i = 0; i < resp_labels.size(); ++i) {
      location.back() = resp_labels[i];
      iterator_results.insert(
          run_identifier, location,
          Teuchos::getCol<int, Real>(Teuchos::View,
                                     *const_cast<RealMatrix*>(&corr_matrix), i),
          scales);
    }
  }
}

void SensAnalysisGlobal::archive_std_regress_coeffs(
    const StrStrSizet& run_identifier, ResultsManager& iterator_results,
    const StringArray& var_labels, const StringArray& resp_labels,
    const size_t& inc_id) const {
  if (!iterator_results.active()) return;
  ...
  for (size_t i = 0; i < resp_labels.size(); ++i) {
    ...
    iterator_results.insert(
        run_identifier, location,
        Teuchos::getCol<int, Real>(
            Teuchos::View, *const_cast<RealMatrix*>(&stdRegressCoeffs), i),
        scales);
    // Archive Coeff of Determination (R^2) as an attribute
    AttributeArray ns_attr({ResultAttribute<Real>(
        "coefficient_of_determination", stdRegressCODs[i])});
    iterator_results.add_metadata_to_object(run_identifier, location, ns_attr);
  }
}

void SensAnalysisGlobal::print_correlations(
    std::ostream& s, const StringArray& var_labels,
  ...
  if (var_labels.size() != numVars) {
    Cerr << "Error: Number of variable labels (" << var_labels.size()
         ...
         << "variables (" << numVars << ") in compute_correlations()."
         << std::endl;
    abort_handler(-1);
  }
...
void SensAnalysisGlobal::check_correlations_for_nan_or_inf(
    std::ostream& s) const {
  if (has_nan_or_inf(simpleCorr) || has_nan_or_inf(partialCorr) ||
      has_nan_or_inf(simpleRankCorr) || has_nan_or_inf(partialRankCorr))
...
void SensAnalysisGlobal::print_simple_correlations(
    std::ostream& s, const StringArray& var_labels,
    const StringArray& resp_labels, bool rank) const {
  int num_in_out = numVars + numFns;
  auto& corr_matrix = (rank) ? simpleRankCorr : simpleCorr;
  ...
  if (corr_matrix.numRows() == num_in_out &&
    ...
    if (rank)
      ...
      s << "\nSimple Correlation Matrix among all inputs and outputs:\n"
        << "             ";
    for (const String& label : var_labels) s << std::setw(12) << label << ' ';
    for (const String& label : resp_labels) s << std::setw(12) << label << ' ';
    s << '\n';
...
void SensAnalysisGlobal::print_partial_correlations(
    std::ostream& s, const StringArray& var_labels,
    const StringArray& resp_labels, bool rank) const {
  int num_in_out = numVars + numFns;
  auto& corr_matrix = (rank) ? partialRankCorr : partialCorr;
...
void SensAnalysisGlobal::compute_std_regress_coeffs(
    const RealMatrix& vars_samples, const IntResponseMap& resp_samples) {
  ...
  RealMatrix vars_view(Teuchos::View, valid_data, numVars,
                       valid_data.numCols());
  RealMatrix vars_copy_trans(vars_view, Teuchos::TRANS);
  RealMatrix resp_view(Teuchos::View, valid_data, numFns, valid_data.numCols(),
                       numVars);
  RealMatrix resp_copy_trans(resp_view, Teuchos::TRANS);

  compute_std_regression_coeffs(vars_copy_trans, resp_copy_trans,
                                stdRegressCoeffs, stdRegressCODs);
#endif
}

void SensAnalysisGlobal::print_std_regress_coeffs(
    std::ostream& s, StringArray var_labels,
    const StringArray& resp_labels) const {
#ifdef HAVE_DAKOTA_SURROGATES
  // output standardized regression coefficients and coefficients of
  // determination (R^2)

  if (has_nan_or_inf(stdRegressCoeffs))
    s << "\nAt least one standardized regression coefficient is nan or inf. "
         "This "
  ...
  if (resp_labels.size() != numFns) {
    Cerr
        << "Error: Number of response labels (" << resp_labels.size()
  ...
  s << "\nStandardized Regression Coefficients and Coefficients of "
       "Determination (R^2):\n"
    << "             ";
  for (j = 0; j < numFns; ++j) s << std::setw(12) << resp_labels[j] << ' ';
  s << '\n';
  for (i = 0; i < numVars; ++i) {
    s << std::setw(12) << var_labels[i] << ' ';
    for (j = 0; j < numFns; ++j)
  ...
  s << std::setw(12) << "R^2" << ' ';
  for (j = 0; j < numFns; ++j) s << std::setw(12) << stdRegressCODs(j) << ' ';
  s << '\n';
...
void SensAnalysisGlobal::compute_vbd_stats_via_sampling(
    const unsigned short method, const int numBins, const size_t numFunctions,
    const size_t num_vars, const size_t num_samples,
    const RealMatrix& vars_samples, const IntResponseMap& resp_samples) {
  if (method == VBD_BINNED) {
    this->compute_binned_vbd_stats(numBins, numFunctions, num_vars, num_samples,
                                   vars_samples, resp_samples);
  } else {
    this->compute_pick_and_freeze_vbd_stats(numFunctions, num_vars, num_samples,
                                            resp_samples);
  }
}

void SensAnalysisGlobal::compute_pick_and_freeze_vbd_stats(
    const size_t numFunctions, const size_t num_vars, const size_t num_samples,
    const IntResponseMap& resp_samples) {
  if (resp_samples.size() != num_samples * (num_vars + 2)) {
    Cerr << "\nError in Analyzer::compute_vbd_stats_with_Saltelli()"
         ...
         << "; received " << resp_samples.size() << std::endl;
  ...
  boost::multi_array<Real, 3> total_fn_vals(
      boost::extents[numFunctions][num_vars + 2][num_samples]);  // [k][i][j]
  ...
  //   estimator for the total sensitivity index.", Comp Physics Comm, 181 (2),
  //   pp. 259--270, Feb. 2010.
  //
  // We decided to use formulas indexSi and indexTi based on testing with a
  // shock physics problem that had significant nonlinearities, interactions,
  // and several response functions. The results are documented in the paper:
  ...
  boost::multi_array<Real, 3> total_norm_vals(
      boost::extents[numFunctions][num_vars + 2][num_samples]);  // [k][i][j]
  ...
  for (size_t k(0); k < numFunctions; ++k) {
    ...
    var_hatYC = var_hatYC / (2. * dNumSamples) - mean_C * mean_C;
...
void SensAnalysisGlobal::compute_binned_vbd_stats(
    const int numBins, const size_t numFunctions, const size_t num_vars,
    const size_t num_samples, const RealMatrix& vars_samples,
    const IntResponseMap& resp_samples) {
  // TNP NOTE: the algorithm would need to be modified to support discrete
  // random variables. Currently there is a check before calling
  // compute_vbs_stats_via_sampling that errors out if there are discrete random
  // variables.

  check_num_samples(num_samples, resp_samples.size(),
                    "compute_binned_vbd_stats");
  ...
  // However, since the binned Sobol' indices are computed per response, we
  // could technically do this filtering per response. For now doing it all at
  // once.
...
void SensAnalysisGlobal::print_main_and_total_effects_indices(
    std::ostream& s, const StringArray& var_labels,
    const StringArray& resp_labels, const Real dropTol) const {
...
void SensAnalysisGlobal::print_main_effects_indices(
    std::ostream& s, const StringArray& var_labels,
    const StringArray& resp_labels, const Real dropTol) const {
  for (size_t k(0); k < resp_labels.size(); ++k) {
    s << resp_labels[k] << " Sobol' indices:\n";
...
void SensAnalysisGlobal::archive_sobol_indices(
    const StrStrSizet& run_identifier, ResultsManager& resultsDB,
    const StringArray& var_labels, const StringArray& resp_labels,
    const Real dropTol) const {
  if (!resultsDB.active()) return;

  archive_main_effects_indices(run_identifier, resultsDB, var_labels,
                               resp_labels, dropTol);
  if (indexTi.size() > 0) {
    archive_total_effects_indices(run_identifier, resultsDB, var_labels,
                                  resp_labels, dropTol);
  }
}

void SensAnalysisGlobal::archive_main_effects_indices(
    const StrStrSizet& run_identifier, ResultsManager& resultsDB,
    const StringArray& var_labels, const StringArray& resp_labels,
    const Real dropTol) const {
  for (size_t k(0); k < resp_labels.size(); ++k) {
...
void SensAnalysisGlobal::archive_total_effects_indices(
    const StrStrSizet& run_identifier, ResultsManager& resultsDB,
    const StringArray& var_labels, const StringArray& resp_labels,
    const Real dropTol) const {
  for (size_t k(0); k < resp_labels.size(); ++k) {
...
void SensAnalysisGlobal::compute_binned_sobol_indices_from_valid_samples(
    const RealMatrix& valid_data, size_t num_bins) {
  ...
  // TODO: only implemented "option 2" algorithm from the paper; could add
  // "option 1" if there is demand.
  ...
  RealMatrix response_samples(Teuchos::View, valid_data, numFns, num_samples,
                              numVars);
  RealVector total_means, total_variances;
  compute_response_means_and_variances(response_samples, total_means,
                                       total_variances);
  ...
  for (int i = 0; i < numVars; ++i) {
    ...
    reorder_matrix_columns_from_index_vector(
        response_samples, sorted_response_samples, sort_inds);

    // Compute response variances in each bin
    for (int j = 0; j < num_bins; ++j) {
      RealMatrix binned_response_samples(Teuchos::View, sorted_response_samples,
                                         numFns, num_samples_per_bin, 0,
                                         num_samples_per_bin * j);
      RealMatrix binned_response_samples_T(binned_response_samples,
                                           Teuchos::TRANS);
      compute_col_means(binned_response_samples_T, binned_means);
      compute_col_variances(binned_response_samples_T, binned_means,
                            binned_variances);
      Teuchos::setCol(binned_variances, j, binned_variance_samples);
    }

    // Compute expectation of binned variances over bins
    RealMatrix binned_variance_samples_T(binned_variance_samples,
                                         Teuchos::TRANS);
...
IntMatrix SensAnalysisGlobal::get_var_samples_argsort(
    const RealMatrix& valid_data) {
  ...
  RealMatrix var_samples_T(Teuchos::View, valid_samples_T, valid_data.numCols(),
                           numVars);

  RealMatrix sorted_var_samples_T;
  IntMatrix sorted_var_indices_T;
  sort_matrix_columns(var_samples_T, sorted_var_samples_T,
                      sorted_var_indices_T);
  return sorted_var_indices_T;
}

void SensAnalysisGlobal::compute_response_means_and_variances(
    const RealMatrix& response_samples, RealVector& total_means,
