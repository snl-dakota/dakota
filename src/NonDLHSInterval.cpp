...
NonDLHSInterval::NonDLHSInterval(ProblemDescDB& problem_db,
  ...
  if (!numSamples) numSamples = 10000;

  maxEvalConcurrency *= numSamples;

  unsigned short sample_type = SUBMETHOD_DEFAULT;
  bool vary_pattern = false;  // for consistency across outer loop invocations
  lhsSampler = std::make_unique<NonDLHSSampling>(iteratedModel, sample_type,
                                                 numSamples, seedSpec, rngName,
                                                 vary_pattern, ACTIVE);
}
...
void NonDLHSInterval::initialize() {}  // default is no-op
