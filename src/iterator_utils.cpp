    ...
    namespace IteratorUtils {
        ...
        switch (method_name) {
            case HYBRID:
                ...
                return std::make_shared<EmbedHybridMetaIterator>(problem_db, parallel_lib); break;
                case SUBMETHOD_SEQUENTIAL:
                return std::make_shared<SeqHybridMetaIterator>(problem_db, parallel_lib); break;
                default:
            ...
            case PARETO_SET: case MULTI_START:
                return std::make_shared<ConcurrentMetaIterator>(problem_db, parallel_lib); break;
            default:
                // rather than create additional derived constructors for non-meta-iterators
                // that differ only in creation of their own Model instance, perform the
                // Model instantiation here and leverage the existing constructors.
        ...
        switch (problem_db.get_ushort("method.sub_method")) {
            ...
            case SUBMETHOD_EMBEDDED:
            return std::make_shared<EmbedHybridMetaIterator>(problem_db, parallel_lib, model); break;
            case SUBMETHOD_SEQUENTIAL:
            return std::make_shared<SeqHybridMetaIterator>(problem_db, parallel_lib, model); break;
            default:
            ...
            break;
        case PARETO_SET: case MULTI_START:
            return std::make_shared<ConcurrentMetaIterator>(problem_db, parallel_lib, model); break;
        case CENTERED_PARAMETER_STUDY: case   LIST_PARAMETER_STUDY: 
        case MULTIDIM_PARAMETER_STUDY: case VECTOR_PARAMETER_STUDY: 
            return std::make_shared<ParamStudy>(problem_db, parallel_lib, model); break;
        case RICHARDSON_EXTRAP:
            return std::make_shared<RichExtrapVerification>(problem_db, parallel_lib, model); break;
        case LOCAL_RELIABILITY:
            return std::make_shared<NonDLocalReliability>(problem_db, parallel_lib, model); break;
        case LOCAL_INTERVAL_EST:
            return std::make_shared<NonDLocalSingleInterval>(problem_db, parallel_lib, model); break;
        case LOCAL_EVIDENCE:
            return std::make_shared<NonDLocalEvidence>(problem_db, parallel_lib, model); break;
        ...
        case GLOBAL_INTERVAL_EST:
            ...
            default:
            return std::make_shared<NonDGlobalSingleInterval>(problem_db, parallel_lib, model);
            break;
            }
            break;
        case GLOBAL_EVIDENCE:
            ...
            default:
            return std::make_shared<NonDGlobalEvidence>(problem_db, parallel_lib, model); break;
            }
            break;
        case POLYNOMIAL_CHAOS:
            return std::make_shared<NonDPolynomialChaos>(problem_db, parallel_lib, model); break;
        case MULTILEVEL_POLYNOMIAL_CHAOS: case MULTIFIDELITY_POLYNOMIAL_CHAOS:
            return std::make_shared<NonDMultilevelPolynomialChaos>(problem_db, parallel_lib, model);
            break;
        case STOCH_COLLOCATION:
            return std::make_shared<NonDStochCollocation>(problem_db, parallel_lib, model); break;
        case MULTIFIDELITY_STOCH_COLLOCATION:
            return std::make_shared<NonDMultilevelStochCollocation>(problem_db, parallel_lib, model);
            break;
        #ifdef HAVE_C3
        case C3_FUNCTION_TRAIN:
            return std::make_shared<NonDC3FunctionTrain>(problem_db, parallel_lib, model); break;
        case MULTILEVEL_FUNCTION_TRAIN: case MULTIFIDELITY_FUNCTION_TRAIN:
            return std::make_shared<NonDMultilevelFunctionTrain>(problem_db, parallel_lib, model);
            break;
        #endif
        ...
        case BAYES_CALIBRATION:
        ...
        #ifdef HAVE_QUESO
            return std::make_shared<NonDQUESOBayesCalibration>(problem_db, parallel_lib, model);
            break;
        #else
            ...
            std::shared_ptr<Iterator> sp = std::make_shared<NonDDREAMBayesCalibration>(problem_db, parallel_lib, model);
            return sp;
            break;
            }
        #endif
            ...
            return std::make_shared<NonDWASABIBayesCalibration>(problem_db, parallel_lib, model);
            break;
            default:
            ...
            break;
        case GPAIS:
            return std::make_shared<NonDGPImpSampling>(problem_db, parallel_lib, model); break;
        case POF_DARTS:
            return std::make_shared<NonDPOFDarts>(problem_db, parallel_lib, model); break;
        case RKD_DARTS:
            return std::make_shared<NonDRKDDarts>(problem_db, parallel_lib, model); break;
        case IMPORTANCE_SAMPLING:
            return std::make_shared<NonDAdaptImpSampling>(problem_db, parallel_lib, model); break;
        #ifdef HAVE_ADAPTIVE_SAMPLING
        case ADAPTIVE_SAMPLING:
            return std::make_shared<NonDAdaptiveSampling>(problem_db, parallel_lib, model); break;
        #endif
        //#ifdef HAVE_MUQ
        //  case MUQ_SAMPLING:
        //    return std::make_shared<NonDMUQBayesCalibration>(problem_db, parallel_lib, model);break;
        ...
        case APPROX_CONTROL_VARIATE:
            ...
            return std::make_shared<NonDACVSampling>(problem_db, parallel_lib, model);
            break;
        case GEN_APPROX_CONTROL_VARIATE: // not currently reachable
            return std::make_shared<NonDGenACVSampling>(problem_db, parallel_lib, model);
            break;
        case MULTILEVEL_BLUE:
            return std::make_shared<NonDMultilevBLUESampling>(problem_db, parallel_lib, model);
            break;
        case DATA_FIT_SURROGATE_BASED_LOCAL:
            return std::make_shared<DataFitSurrBasedLocalMinimizer>(problem_db, parallel_lib, model);
            break;
        case HIERARCH_SURROGATE_BASED_LOCAL:
            return std::make_shared<HierarchSurrBasedLocalMinimizer>(problem_db, parallel_lib, model);
            break;
        case SURROGATE_BASED_LOCAL:
            if (model->surrogate_type() == "ensemble")
            return
            std::make_shared<HierarchSurrBasedLocalMinimizer>(problem_db, parallel_lib, model);
            else
            return
            std::make_shared<DataFitSurrBasedLocalMinimizer>(problem_db, parallel_lib, model);
            break;
        case SURROGATE_BASED_GLOBAL:
            return std::make_shared<SurrBasedGlobalMinimizer>(problem_db, parallel_lib, model); break;
        case EFFICIENT_GLOBAL:
            return std::make_shared<EffGlobalMinimizer>(problem_db, parallel_lib, model); break;
        case NONLINEAR_CG:
            return std::make_shared<NonlinearCGOptimizer>(problem_db, parallel_lib, model);  break;
        case GENIE_OPT_DARTS:
            return std::make_shared<OptDartsOptimizer>(problem_db, parallel_lib, model); break;
        case GENIE_DIRECT: 
            return std::make_shared<OptDartsOptimizer>(problem_db, parallel_lib, model); break;
        #ifdef HAVE_OPTPP
        case OPTPP_G_NEWTON:
            return std::make_shared<SNLLLeastSq>(problem_db, parallel_lib, model); break;
        case OPTPP_Q_NEWTON: case OPTPP_FD_NEWTON: case OPTPP_NEWTON:
        case OPTPP_CG:       case OPTPP_PDS:
            return std::make_shared<SNLLOptimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef DAKOTA_HOPS
        case ASYNCH_PATTERN_SEARCH:
            return std::make_shared<APPSOptimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_ACRO
        case COLINY_BETA: case COLINY_COBYLA:         case COLINY_DIRECT:
        case COLINY_EA:   case COLINY_PATTERN_SEARCH: case COLINY_SOLIS_WETS:
            return std::make_shared<COLINOptimizer>(problem_db, parallel_lib, model); break;
        case BRANCH_AND_BOUND:
            return std::make_shared<PebbldMinimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_JEGA
        case MOGA: case SOGA:
        ...
        case MIT_NOWPAC: case MIT_SNOWPAC:
            return std::make_shared<NOWPACOptimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_NPSOL
        case NPSOL_SQP:
            return std::make_shared<NPSOLOptimizer>(problem_db, parallel_lib, model); break;
        case NLSSOL_SQP:
            return std::make_shared<NLSSOLLeastSq>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_NLPQL
        case NLPQL_SQP:
            return std::make_shared<NLPQLPOptimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_NL2SOL
        case NL2SOL:
            return std::make_shared<NL2SOLLeastSq>(problem_db, parallel_lib, model);   break;
        #endif
        //#ifdef DAKOTA_RSQP
        //  case REDUCED_SQP: return std::make_shared<rSQPOptimizer>(problem_db, parallel_lib, model);   break;
        //#endif
        #ifdef HAVE_DOT
        case DOT_BFGS: case DOT_FRCG: case DOT_MMFD: case DOT_SLP: case DOT_SQP:
            return std::make_shared<DOTOptimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_CONMIN
        case CONMIN_FRCG: case CONMIN_MFD:
            return std::make_shared<CONMINOptimizer>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_DDACE
        case DACE:
            return std::make_shared<DDACEDesignCompExp>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_FSUDACE
        case FSU_CVT: case FSU_HALTON: case FSU_HAMMERSLEY:
            return std::make_shared<FSUDesignCompExp>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_PSUADE
        case PSUADE_MOAT:
            return std::make_shared<PSUADEDesignCompExp>(problem_db, parallel_lib, model); break;
        #endif
        #ifdef HAVE_NCSU
        case NCSU_DIRECT:
