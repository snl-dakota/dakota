...
void Sampler::enrich_samples(int num_dims, const RealMatrix &initial_samples,
  ...
  RealMatrix initial_samples_view(Teuchos::View, selected_samples, num_dims,
                                  num_initial_samples);
  ...
  RealMatrix candidate_samples_view(Teuchos::View, selected_samples, num_dims,
                                    num_new_samples, 0, num_initial_samples);
...
void Sampler::get_unique_samples(const RealMatrix &initial_samples,
  ...
  Pecos::util::extract_submatrix_from_column_indices(
      candidate_samples, unique_candidate_indices, unique_candidate_samples);
}

void LejaSampler::get_enriched_sample_indices(
    int num_dims, const RealMatrix &initial_samples, int num_new_samples,
    const RealMatrix &candidate_samples,
    IntVector &selected_candidate_indices) {
  ...
  if (precondition_) apply_preconditioning(basis_matrix);

  int num_initial_samples = initial_samples.numCols();
  int num_total_samples = num_new_samples + num_initial_samples;
  RealMatrix A, L_factor, U_factor;
  IntVector selected_sample_indices;
  Pecos::util::truncated_pivoted_lu_factorization(
      basis_matrix, L_factor, U_factor, selected_sample_indices,
...
void LejaSampler::enrich_samples(int num_dims,
                                 ...
                                 int num_new_samples, RealMatrix &samples) {
  RealMatrix candidate_samples;
...
void LejaSampler::set_seed(int seed) { seed_ = seed; }
...
void LejaSampler::set_total_degree_basis_from_num_samples(int num_vars,
                                                          int num_samples) {
  int degree =
      Pecos::util::get_total_degree_from_num_samples(num_vars, num_samples);
  UShortArray exp_order(num_vars, degree);
  Pecos::SharedPolyApproxData::total_order_multi_index(exp_order, multiIndex);
}

void LejaSampler::set_polynomial_basis(
