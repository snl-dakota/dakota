...
DataMethodRep::DataMethodRep()
    : methodOutput(NORMAL_OUTPUT),
      maxIterations(SZ_MAX),
      maxRefineIterations(SZ_MAX),
      maxSolverIterations(SZ_MAX),
      maxFunctionEvals(SZ_MAX),
      ...
      subMethod(SUBMETHOD_DEFAULT),
      // Meta-iterators
      iteratorServers(0),
      procsPerIterator(0),  // 0 defaults to detect user spec
      iteratorScheduling(DEFAULT_SCHEDULING),
      hybridLSProb(0.1),
      ...
      trustRegionMinSize(1.e-6),
      trustRegionContractTrigger(0.25),
      trustRegionExpandTrigger(0.75),
      trustRegionContract(0.25),
      trustRegionExpand(2.0),
      ...
      surrBasedLocalAcceptLogic(FILTER),
      surrBasedLocalConstrRelax(NO_RELAX),
      ...
      // NPSOL
      verifyLevel(-1),
      functionPrecision(1.e-10),
      lineSearchTolerance(0.9),
...
#endif
      stepLenToBoundary(-1.),
      centeringParam(-1.),  // dummy defaults (see SNLLBase)
      searchSchemeSize(32),
      // APPSPACK
      initStepLength(1.0),
      contractStepLength(0.5),
      threshStepLength(0.01),
      meritFunction("merit2_squared"),
      constrPenalty(1.0),
      smoothFactor(0.0),
      // COLINY
      constantPenalty(false),
      globalBalanceParam(-1.),
      localBalanceParam(-1.),
      maxBoxSize(-1.),
      minBoxSize(-1.),
      // boxDivision("major_dimension"), // leave empty string as default
      mutationAdaptive(true),
      showMiscOptions(false),
      ...
      // defaults:
      solnTarget(
          -DBL_MAX),  // COLINY default of 1.e-5 can cause premature term.
      // These attributes use dummy defaults which are used to trigger
      // conditional option processing (since we don't want to get out of synch
      // with SGOPT's defaults which may change).  The dummy defaults should be
      // values which are _not_ reasonable user inputs.
      mutationMinScale(-1.),
      initDelta(-1.),
      threshDelta(-1.),
      newSolnsGenerated(-9999),
      numberRetained(-9999),
      expansionFlag(true),  // default = on, no_expansion spec turns off
      expandAfterSuccess(0),
      contractAfterFail(0),
      mutationRange(-9999),
      ...
      numCrossPoints(2),
      numParents(2),
      numOffspring(2),  // convergenceType(""),
      percentChange(0.1),
      numGenerations(15),
      fitnessLimit(6.0),
      shrinkagePercent(0.9),
      ...
      initMeshSize(1.0),
      minMeshSize(1.e-6),
      historyFile("mads_history"),
      displayFormat("bbe obj"),
      vns(0.0),
      neighborOrder(1),
      showAllEval(false),
      useSurrogate("none"),
      // C3 FT
      maxCrossIterations(1),
      solverTol(1.e-10),
      solverRoundingTol(1.e-10),
      statsRoundingTol(1.e-10),
      startOrder(2),
      kickOrder(1),
      maxOrder(USHRT_MAX),
      adaptOrder(false),
      ...
      maxCVOrderCandidates(USHRT_MAX),
      ...
      vbdFlag(false),
      vbdDropTolerance(-1.),
      vbdViaSamplingMethod(VBD_PICK_AND_FREEZE),
      ...
      wilksFlag(false),
      wilksOrder(1),
      wilksConfidenceLevel(0.95),
      wilksSidedInterval(ONE_SIDED_UPPER),
      // NonD
      toleranceIntervalsFlag(false),
      tiCoverage(0.95),
      tiConfidenceLevel(0.90),
      stdRegressionCoeffs(false),
      respScalingFlag(false),
      vbdOrder(0),
      covarianceControl(DEFAULT_COVARIANCE),
      rngName("mt19937"),
      refinementType(Pecos::NO_REFINEMENT),
      refinementMetric(Pecos::DEFAULT_METRIC),
      refinementControl(Pecos::NO_CONTROL),
      nestingOverride(Pecos::NO_NESTING_OVERRIDE),
      growthOverride(Pecos::NO_GROWTH_OVERRIDE),
      expansionType(EXTENDED_U),
      piecewiseBasis(false),
      expansionBasisType(Pecos::DEFAULT_BASIS),
      quadratureOrder(USHRT_MAX),
      sparseGridLevel(USHRT_MAX),
      expansionOrder(USHRT_MAX),
      collocationPoints(SZ_MAX),
      expansionSamples(SZ_MAX),
      ensemblePilotSolnMode(ONLINE_PILOT),
      pilotGroupSampling(SHARED_PILOT),
      groupThrottleType(NO_GROUP_THROTTLE),
      groupSizeThrottle(USHRT_MAX),
      rCondBestThrottle(SZ_MAX),
      rCondTolThrottle(DBL_MAX),
      truthPilotConstraint(false),
      dagRecursionType(NO_GRAPH_RECURSION),
      dagDepthLimit(USHRT_MAX),
      modelSelectType(NO_MODEL_SELECTION),
      relaxFixedFactor(0.),
      relaxRecursiveFactor(0.),
      allocationTarget(TARGET_MEAN),
      useTargetVarianceOptimizationFlag(false),
      ...
      cubIntOrder(USHRT_MAX),
      collocationRatio(0.),
      collocRatioTermsOrder(1.),
      regressionType(Pecos::DEFAULT_REGRESSION),
      lsRegressionType(DEFAULT_LS),
      regressionL2Penalty(0.),
      crossValidation(false),
      crossValidNoiseOnly(false),
      // adaptedBasisInitLevel(0),
      adaptedBasisAdvancements(3),
      normalizedCoeffs(false),
      tensorGridFlag(false),
      sampleType(SUBMETHOD_DEFAULT),
      rank1LatticeFlag(false),
      noRandomShiftFlag(false),
      log2MaxPoints(0),
      kuo(false),
      cools_kuo_nuyens(false),
      naturalOrdering(false),
      radicalInverseOrdering(false),
      digitalNetFlag(false),
      noDigitalShiftFlag(false),
      noScramblingFlag(false),
      ...
      dOptimal(false),
      numCandidateDesigns(0),
      // reliabilitySearchType(MV),
      integrationRefine(NO_INT_REFINE),
      optSubProbSolver(SUBMETHOD_DEFAULT),
      numericalSolveMode(NUMERICAL_FALLBACK),
      estVarMetricType(DEFAULT_ESTVAR_METRIC),
      estVarMetricNormOrder(2.),
      multilevAllocControl(DEFAULT_MLMF_CONTROL),
      multilevEstimatorRate(2.),
      ...
      responseLevelTarget(PROBABILITIES),
      responseLevelTargetReduce(COMPONENT),
      chainSamples(0),
      buildSamples(0),
      samplesOnEmulator(0),
      emulatorOrder(0),
      ...
      mcmcType("dram"),
      standardizedSpace(false),
      adaptPosteriorRefine(false),
      logitTransform(false),
      gpmsaNormalize(false),
      posteriorStatsKL(false),
      posteriorStatsMutual(false),
      posteriorStatsKDE(false),
      chainDiagnostics(false),
      chainDiagnosticsCI(false),
      modelEvidence(false),
      modelEvidMC(false),
      modelEvidLaplace(false),
      priorPropCovMult(1.0),
      proposalCovUpdatePeriod(std::numeric_limits<int>::max()),
      fitnessMetricType("predicted_variance"),
      batchSelectionType("naive"),
      lipschitzType("local"),
      calibrateErrorMode(CALIBRATE_NONE),
      burnInSamples(0),
      subSamplingPeriod(1),
      ...
      numChains(3),
      numCR(3),
      crossoverChainPairs(3),
      grThreshold(1.2),
      jumpStep(5),
      generatePosteriorSamples(false),
      evaluatePosteriorDensity(false),
      // Wasabi
      ...
      // Parameter Study
      numSteps(0),
      pstudyFileFormat(TABULAR_ANNOTATED),
      pstudyFileActive(false),
      // Verification
      refinementRate(2.),
      // Point import/export files
      importBuildFormat(TABULAR_ANNOTATED),
      importBuildActive(false),
      importApproxFormat(TABULAR_ANNOTATED),
      importApproxActive(false),
      exportApproxFormat(TABULAR_ANNOTATED),
      exportSampleSeqFlag(false),
      exportSamplesFormat(TABULAR_ANNOTATED),
      exportSurrogate(false),
      modelExportPrefix("exported_surrogate"),
...
void DataMethodRep::write(MPIPackBuffer& s) const {
  ...
  s << iteratorServers << procsPerIterator << iteratorScheduling
    << hybridMethodNames << hybridModelPointers
    << hybridMethodPointers
    //<< hybridProgThresh
    << hybridGlobalMethodName << hybridGlobalModelPointer
    << hybridGlobalMethodPointer << hybridLocalMethodName
    << hybridLocalModelPointer << hybridLocalMethodPointer
    << hybridLSProb
    //<< branchBndNumSamplesRoot << branchBndNumSamplesNode
    << concurrentRandomJobs << concurrentParameterSets;

  // Surrogate-based
  s << softConvLimit << surrBasedLocalLayerBypass << trustRegionInitSize
    << trustRegionMinSize << trustRegionContractTrigger
    << trustRegionExpandTrigger << trustRegionContract << trustRegionExpand
  ...
  s << maxCrossIterations << solverTol << solverRoundingTol << statsRoundingTol
    << startOrder << kickOrder << maxOrder << adaptOrder << startRank
    << kickRank << maxRank << adaptRank << maxCVRankCandidates
    << maxCVOrderCandidates << c3AdvanceType << startOrderSeq << startRankSeq;

  // NonD & DACE
  s << numSamples << fixedSeedFlag << fixedSequenceFlag << vbdFlag
    << vbdDropTolerance << vbdViaSamplingMethod << vbdViaSamplingNumBins
    << backfillFlag << pcaFlag << percentVarianceExplained << wilksFlag
    << wilksOrder << wilksConfidenceLevel << wilksSidedInterval;
  ...
  s << toleranceIntervalsFlag << tiCoverage << tiConfidenceLevel
    ...
    << anisoDimPref << cubIntOrder << collocationRatio << collocRatioTermsOrder
    << regressionType << lsRegressionType << regressionNoiseTol
    << regressionL2Penalty << crossValidation
    ...
    << exportExpansionFile << sampleType << rank1LatticeFlag
    << noRandomShiftFlag << log2MaxPoints << kuo << cools_kuo_nuyens
    << naturalOrdering << radicalInverseOrdering << digitalNetFlag
    << noDigitalShiftFlag << noScramblingFlag << mostSignificantBitFirst
    << leastSignificantBitFirst << numberOfBits << scrambleSize << joe_kuo
    << sobol_order_2 << grayCodeOrdering << dOptimal
    ...
    << rCondBestThrottle << rCondTolThrottle << truthPilotConstraint
    << dagRecursionType << dagDepthLimit << modelSelectType
    << relaxFactorSequence << relaxFixedFactor << relaxRecursiveFactor
    << allocationTarget << useTargetVarianceOptimizationFlag << qoiAggregation
    << scalarizationRespCoeffs << convergenceToleranceType
    << convergenceToleranceTarget << multilevAllocControl
    ...
    << modelEvidence << modelEvidLaplace << modelEvidMC << proposalCovType
    << priorPropCovMult << proposalCovUpdatePeriod << proposalCovInputType
    << proposalCovData << proposalCovFile << advancedOptionsFilename
    ...
    << batchSizeExplore << mutualInfoKSG2 << numChains << numCR
    << crossoverChainPairs << grThreshold << jumpStep << numPushforwardSamples
    << drNumStages << drScaleType << drScale << amPeriodNumSteps
    << amStartingStep << amScale << malaStepSize << diliHessianType
    << diliAdaptInterval << diliAdaptStart << diliAdaptEnd << diliInitialWeight
    << diliHessTolerance << diliLISTolerance << diliSesNumEigs << diliSesRelTol
    << diliSesAbsTol << diliSesExpRank << diliSesOversFactor << diliSesBlockSize
    << mlmcmcInitialChainSamples << mlmcmcSubsamplingSteps
    << mlmcmcTargetVariance << dataDistType << dataDistCovInputType
    << dataDistMeans << dataDistCovariance << dataDistFile
    << posteriorDensityExportFilename << posteriorSamplesExportFilename
    << posteriorSamplesImportFilename << generatePosteriorSamples
    << evaluatePosteriorDensity;

  // Parameter Study
  s << finalPoint << stepVector << numSteps << stepsPerVariable << listOfPoints
    << pstudyFilename << pstudyFileFormat << pstudyFileActive << varPartitions;
...
void DataMethodRep::read(MPIUnpackBuffer& s) {
  ...
  s >> initStepLength >> contractStepLength >> threshStepLength >>
      meritFunction >> constrPenalty >> smoothFactor;
  ...
  // C3 FT
  s >> maxCrossIterations >> solverTol >> solverRoundingTol >>
      statsRoundingTol >> startOrder >> kickOrder >> maxOrder >> adaptOrder >>
  ...
  s >> toleranceIntervalsFlag >> tiCoverage >> tiConfidenceLevel >>
      ...
      expansionBasisType >> quadratureOrderSeq >> sparseGridLevelSeq >>
      expansionOrderSeq >> collocationPointsSeq >> expansionSamplesSeq >>
      quadratureOrder >> sparseGridLevel >> expansionOrder >>
      collocationPoints >> expansionSamples  //>> expansionSampleType
      ...
      evidenceSamples >> calModelDiscrepancy >> numPredConfigs >>
      predictionConfigList >> importPredConfigs >> importPredConfigFormat >>
      modelDiscrepancyType >> polynomialOrder >> exportCorrModelFile >>
      ...
      batchSize >> batchSizeExplore >> mutualInfoKSG2 >> numChains >> numCR >>
      crossoverChainPairs >> grThreshold >> jumpStep >> numPushforwardSamples >>
      drNumStages >> drScaleType >> drScale >> amPeriodNumSteps >>
      amStartingStep >> amScale >> malaStepSize >> diliHessianType >>
      diliAdaptInterval >> diliAdaptStart >> diliAdaptEnd >>
      diliInitialWeight >> diliHessTolerance >> diliLISTolerance >>
      diliSesNumEigs >> diliSesRelTol >> diliSesAbsTol >> diliSesExpRank >>
      diliSesOversFactor >> diliSesBlockSize >> mlmcmcInitialChainSamples >>
      mlmcmcSubsamplingSteps >> mlmcmcTargetVariance >> dataDistType >>
  ...
  // Parameter Study
  s >> finalPoint >> stepVector >> numSteps >> stepsPerVariable >>
      listOfPoints >> pstudyFilename >> pstudyFileFormat >> pstudyFileActive >>
      varPartitions;
...
DataMethod::DataMethod()
    : dataMethodRep(new DataMethodRep()) { /* empty ctor */ }

DataMethod::DataMethod(const DataMethod& data_method)
    : dataMethodRep(data_method.dataMethodRep) { /* empty ctor */ }
...
DataMethod::~DataMethod() { /* empty dtor */ }
