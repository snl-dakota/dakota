...
class boost_ls_eval {
 public:
  // constructor
  boost_ls_eval(NonlinearCGOptimizer& pimpl_)
      : pImpl(pimpl_) { /* empty constructor */ }
...
NonlinearCGOptimizer::NonlinearCGOptimizer(ProblemDescDB& problem_db,
                                           ParallelLibrary& parallel_lib,
                                           std::shared_ptr<Model> model)
    : Optimizer(problem_db, parallel_lib, model,
                std::shared_ptr<TraitsBase>(new NonlinearCGTraits())),
      initialStep(0.01),
      linesearchTolerance(1.0e-2),
      linesearchType(CG_LS_SIMPLE),
...
void NonlinearCGOptimizer::core_run() {
  ...
  for (iterCurr = 0; iterCurr < maxIterations; iterCurr++) {
    ...
    if (iterCurr == 0) gradDotGrad_init = gradDotGrad_curr;
...
void NonlinearCGOptimizer::compute_direction() {
  if (iterCurr == 0 || (iterCurr % restartIter == 0) ||
    ...
    if (updateType == CG_FLETCHER_REEVES)
      ...
      gradDiff -= gradPrev;
      Real gradCurr_dot_gradDiff = gradCurr.dot(gradDiff);
      ...
      if (updateType == CG_POLAK_RIBIERE)
        ...
        beta = gradCurr_dot_gradDiff / gradDiff.dot(searchDirection);
    }
    ...
    searchDirection.AXPY(searchDirection.length(), -1.0, gradCurr.values(), 1,
...
bool NonlinearCGOptimizer::compute_step() {
  ...
  if (resetStep) stepLength = initialStep;
  ...
  switch (linesearchType) {
    ...
    case CG_LS_BRENT: {
      ...
      if (outputLevel > NORMAL_OUTPUT)
        Cout << "INFO (NonlinearCG_LS): Initiating bracketing procedure."
             << endl;
...
void NonlinearCGOptimizer::bracket_min(Real& xa, Real& xb, Real& xc, Real& fa,
                                       Real& fb, Real& fc) {
...
Real NonlinearCGOptimizer::brent_minimize(Real a, Real b, Real tol) {
  ...
  unsigned iter = 1;
  while (iter < maxLinesearchIters &&
         std::fabs(x - xm) > (t2 - 0.5 * (b - a))) {
    d = 0.0;
    p = 0.0;
    q = 0.0;
    r = 0.0;
