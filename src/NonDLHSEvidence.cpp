...
NonDLHSEvidence::NonDLHSEvidence(ProblemDescDB& problem_db,
...
void NonDLHSEvidence::initialize() { calculate_cells_and_bpas(); }
      ...
      for (cellCntr = 0; cellCntr < numCells; ++cellCntr) {
        ...
        if (in_bounds)
          for (j = 0; j < numDiscSetIntUncVars; ++j)
            if (di_vars[j + numDiscIntervalVars] !=
                cellIntSetBounds[cellCntr][j])  // TO DO
            {
              in_bounds = false;
              break;
            }
