...
ApproximationFieldInterface::ApproximationFieldInterface(
    ProblemDescDB& problem_db, const Variables& am_vars, bool am_cache,
    const String& am_interface_id, const Response& response)
  ...
  // Cout << "\nApproximationInterface: numSurfaces, numFields, numScalars --> "
  // << numSurfaces << ", "
  //                                                                             << numFields << ", "
  //                                                                             << numScalars << std::endl;
  ...
  std::copy(approximation_fn_indices().begin(),
            approximation_fn_indices().end(), std::back_inserter(all_indices));
  ...
  for (int i = 0; i < numScalars; ++i)
    functionSurfaces[all_indices[i]] =
        Approximation(problem_db, shared_approximation(),
                      response.function_labels()[all_indices[i]]);
  for (int i = 0; i < numFields; ++i)
    functionSurfaces[numScalars + i] =
        FieldApproximation(problem_db, shared_approximation(), field_labels[i]);
}

Approximation& ApproximationFieldInterface::function_surface(size_t index) {
  ...
  auto field_approx =
      std::dynamic_pointer_cast<FieldApproximation>(approx.approx_rep());
  // FieldApproximation* field_approx =
  // dynamic_cast<FieldApproximation*>(&approx);
  if (field_approx) field_approx->active_component(activeComponent);
  return functionSurfaces.at(fn2SurfaceIdMap.at(index));
}

const Approximation& ApproximationFieldInterface::function_surface(
    size_t index) const {
  activeSurface = fn2SurfaceIdMap.at(index);
  activeComponent = fn2SurfaceComponentMap.at(index);
  return functionSurfaces.at(fn2SurfaceIdMap.at(index));
