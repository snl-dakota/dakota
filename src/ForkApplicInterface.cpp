...
ForkApplicInterface::ForkApplicInterface(const ProblemDescDB& problem_db,
...
#endif  // MPI_DEBUG
    analysisProcessIdMap.erase(an_it);
    ++completed;
...
pid_t ForkApplicInterface::create_analysis_process(bool block_flag,
                                                   bool new_group) {
...
pid_t ForkApplicInterface::wait(pid_t process_group_id,
                                std::map<pid_t, int>& process_id_map,
                                bool block_flag) {
  int status;
  ...
  pid_t pid = (block_flag) ? waitpid(-process_group_id, &status, 0)
                           :  // block for completion w/i group
                  waitpid(-process_group_id, &status,
                          WNOHANG);  // don't block for completion
  ...
  if (pid == -1 && errno == ECHILD) {  // special case: mitigate w/ fallback
    ...
    bool done = false;
    while (!done) {
      for (gp_it = process_id_map.begin(); gp_it != process_id_map.end();
           ++gp_it) {
...
void ForkApplicInterface::join_process_group(pid_t& process_group_id,
                                             bool new_group) {
  ...
  // int err1 = setpgid(0, process_group_id);//pid=0 -> use pid from this
  // process if (err1 && !new_group) { // && errno == E****)
  //  process_group_id = getpid(); // this child pid becomes group leader
    ...
    switch (errno) {
      case EACCES:
        Cerr << "An attempt was made to change the process group ID of one\n  "
             ...
             << "already performed an exec." << std::endl;
        break;
      case EINVAL:
        Cerr << "pgid is less than 0." << std::endl;
        break;
      case EPERM:
        Cerr
            ...
            << "ID of a session leader." << std::endl;
        break;
      case ESRCH:
        ...
        break;
      default:
        Cerr << std::strerror(errno) << std::endl;
        break;
    }
    abort_handler(-1);
  }
