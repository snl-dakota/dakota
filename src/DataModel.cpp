...
DataModelRep::DataModelRep()
    : modelType("simulation"),
      ...
      modelImportPrefix("exported_surrogate"),
      modelImportFormat(NO_MODEL_FORMAT),
      importBuildFormat(TABULAR_ANNOTATED),
      importUseVariableLabels(false),
      importBuildActive(false),
      // importApproxFormat(TABULAR_ANNOTATED), importApproxActive(false),
      exportApproxFormat(TABULAR_ANNOTATED),
      exportApproxVarianceFormat(TABULAR_ANNOTATED),
      numRestarts(10),
      approxCorrectionType(NO_CORRECTION),
      approxCorrectionOrder(0),
      modelUseDerivsFlag(false),
      ...
      annRange(0.),
      domainDecomp(false),
      decompCellType("voronoi"),
      decompSupportLayers(0),
      decompDiscontDetect(false),
      discontJumpThresh(0.0),
      discontGradThresh(0.0),
      ...
      percentFold(0.),
      pressFlag(false),
      importChallengeFormat(TABULAR_ANNOTATED),
      importChalUseVariableLabels(false),
      importChallengeActive(false),
      identityRespMap(false),
      subMethodServers(0),
      subMethodProcs(0),  // 0 defaults to detect user spec
      subMethodScheduling(DEFAULT_SCHEDULING),
      initialSamples(0),
      maxIterations(SZ_MAX),
      convergenceTolerance(1.e-4),
      softConvergenceLimit(0),
      subspaceIdBingLi(false),
      subspaceIdConstantine(false),
      subspaceIdEnergy(false),
      subspaceIdCV(false),
      subspaceBuildSurrogate(false),
      subspaceSampleType(SUBMETHOD_DEFAULT),
      subspaceDimension(0),
      subspaceNormalization(SUBSPACE_NORM_DEFAULT),
      numReplicates(100),
      relTolerance(1.0e-6),
      decreaseTolerance(1.0e-6),
      subspaceCVMaxRank(-1),
      subspaceCVIncremental(true),
      subspaceIdCVMethod(CV_ID_DEFAULT),
      regressionType(FT_LS),
      ...
      refineCVMetric("root_mean_squared"),
      refineCVFolds(10),
      adaptedBasisSparseGridLev(0),
      adaptedBasisExpOrder(0),
      adaptedBasisCollocRatio(1.),
...
void DataModelRep::write(MPIPackBuffer& s) const {
  s << idModel << modelType << variablesPointer << interfacePointer
    ...
    << importUseVariableLabels
    << importBuildActive
    //<< importApproxPtsFile << importApproxFormat << importApproxActive
    << exportApproxPtsFile << exportApproxFormat << exportApproxVarianceFile
    << exportApproxVarianceFormat << numRestarts << approxCorrectionType
    << approxCorrectionOrder << modelUseDerivsFlag << respScalingFlag
    << polynomialOrder << krigingCorrelations << krigingOptMethod
    << krigingMaxTrials << krigingMaxCorrelations << krigingMinCorrelations
    << krigingNugget << krigingFindNugget << mlsWeightFunction << rbfBases
    ...
    << initialSamples << refineSamples << maxIterations << convergenceTolerance
    << softConvergenceLimit << subspaceIdBingLi << subspaceIdConstantine
    << subspaceIdEnergy << subspaceBuildSurrogate << subspaceDimension
    << subspaceNormalization << numReplicates << regressionType
    << regressionL2Penalty << maxSolverIterations << maxCrossIterations
    << solverTol << solverRoundingTol << statsRoundingTol << tensorGridFlag
    << startOrder << kickOrder << maxOrder << adaptOrder << startRank
    << kickRank << maxRank << adaptRank << maxCVRankCandidates
    << maxCVOrderCandidates << c3AdvanceType << collocationPoints
    << collocationRatio << autoRefine << maxFunctionEvals << refineCVMetric
    ...
    << subspaceSampleType << subspaceIdCV << relTolerance << decreaseTolerance
    << subspaceCVMaxRank << subspaceCVIncremental << subspaceIdCVMethod
    << method_rotation << adaptedBasisTruncationTolerance;
}
...
void DataModelRep::read(MPIUnpackBuffer& s) {
  s >> idModel >> modelType >> variablesPointer >> interfacePointer >>
      ...
      exportApproxVarianceFile >> exportApproxVarianceFormat >> numRestarts >>
      approxCorrectionType >> approxCorrectionOrder >> modelUseDerivsFlag >>
      respScalingFlag >> polynomialOrder >> krigingCorrelations >>
      ...
      importChallengePtsFile >> importChallengeFormat >>
      importChalUseVariableLabels >> importChallengeActive >>
      advancedOptionsFilename >> optionalInterfRespPointer >> primaryVarMaps >>
      secondaryVarMaps >> primaryRespCoeffs >> secondaryRespCoeffs >>
      identityRespMap >> subMethodServers >> subMethodProcs >>
      subMethodScheduling >> initialSamples >> refineSamples >> maxIterations >>
      ...
      maxCrossIterations >> solverTol >> solverRoundingTol >>
      statsRoundingTol >> tensorGridFlag >> startOrder >> kickOrder >>
      maxOrder >> adaptOrder >> startRank >> kickRank >> maxRank >> adaptRank >>
      maxCVRankCandidates >> maxCVOrderCandidates >> c3AdvanceType >>
      collocationPoints >> collocationRatio >> autoRefine >> maxFunctionEvals >>
      refineCVMetric >> refineCVFolds >> adaptedBasisSparseGridLev >>
      adaptedBasisExpOrder >> adaptedBasisCollocRatio >>
      propagationModelPointer >> truncationTolerance >> rfDataFileName >>
      randomFieldIdForm >> analyticCovIdForm >> subspaceSampleType >>
      subspaceIdCV >> relTolerance >> decreaseTolerance >> subspaceCVMaxRank >>
      subspaceCVIncremental >> subspaceIdCVMethod >> method_rotation >>
      adaptedBasisTruncationTolerance;
}
...
DataModel::DataModel() : dataModelRep(new DataModelRep()) { /* empty ctor */ }

DataModel::DataModel(const DataModel& data_model)
    : dataModelRep(data_model.dataModelRep) { /* empty ctor */ }
...
DataModel::~DataModel() { /* empty dtor*/ }
