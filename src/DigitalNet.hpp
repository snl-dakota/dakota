...
class DigitalNet : public LowDiscrepancySequence {
 public:
  ...
  DigitalNet(
      ...
      bool mostSignificantBit,      /// Generating matrices are stored with most
 ...
 private:
  ...
  inline UInt64 reorder_natural(UInt64 k);

  /// Position of the `k`th digital net point in DIGITAL_NET_GRAY_CODE_ORDERING
  inline UInt64 reorder_gray_code(UInt64 k);
