...
static const char rcsId[] =
    "@(#) $Id: RichExtrapVerification.cpp 6972 2010-09-17 22:18:50Z briadam $";

// #define DEBUG

namespace Dakota {

RichExtrapVerification::RichExtrapVerification(ProblemDescDB& problem_db,
                                               ParallelLibrary& parallel_lib,
                                               std::shared_ptr<Model> model)
    : Verification(problem_db, parallel_lib, model),
      studyType(probDescDB.get_ushort("method.sub_method")),
      refinementRate(
          probDescDB.get_real("method.verification.refinement_rate")) {
...
void RichExtrapVerification::pre_run()
{
  ...
  if (studyType == ) {
    copy_data(ModelUtils::continuous_variables(iteratedModel), initialCVPoint);
// copy copy_data(ModelUtils::discrete_int_variables(iteratedModel),
initialDIVPoint);// copy
    copy_data(ModelUtils::discrete_string_variables(iteratedModel),initialDSVPoint);//
copy copy_data(ModelUtils::discrete_real_variables(iteratedModel),
initialDRVPoint);// copy
  }
...
void RichExtrapVerification::core_run() {
  ...
  */
  ...
  if (refinementRefPt.empty()) refinementRefPt.sizeUninitialized(numFunctions);
...
void RichExtrapVerification::extrapolation(const RealVector& refine_triple,
                                           RealMatrix& qoi_triples) {
  if (outputLevel == DEBUG_OUTPUT) Cout << "refine_triple:\n" << refine_triple;
  ...
  ModelUtils::continuous_variables(*iteratedModel,
                                   initialCVars);  // reset prior to eval triple

  for (size_t i = 0; i < 3; i++) {
    ModelUtils::continuous_variable(*iteratedModel, refine_triple[i],
                                    factorIndex);
  ...
  ++r_cit;
  const Response& resp1 = r_cit->second;
  ++r_cit;
  const Response& resp2 = r_cit->second;

  Real r21, r32, e21, e32, *conv_order = convOrder[factorIndex];
  if (qoi_triples.empty()) qoi_triples.shapeUninitialized(numFunctions, 3);
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    r21 =
        refine_triple[1] / refine_triple[0];  // could replace w/ refinementRate
    ...
    conv_order[i] = (std::abs(e32) > DBL_MIN && std::abs(e21) > DBL_MIN)
                        ? std::log(std::abs(e32 / e21)) / std::log(r21)
                        : 0.;
  }
  if (outputLevel == DEBUG_OUTPUT) {
    Cout << "qoi_triples:\n";
...
void RichExtrapVerification::estimate_order() {
  ...
  for (factorIndex = 0; factorIndex < numFactors; ++factorIndex) {
    ...
    for (i = 0; i < 3; ++i) {
      refine_triple[i] = refine_level;
      refine_level /= refinementRate;
    }
    ...
    qoi_0 = qoi_triples[0];
    err_qoi = numErrorQOI[factorIndex];
    extrap_qoi = extrapQOI[factorIndex];
    for (i = 0; i < numFunctions; ++i) err_qoi[i] = extrap_qoi[i] - qoi_0[i];
    ...
    while (tolerance > convergenceTol && ++counter <= maxIterations) {
      ...
      if (outputLevel == DEBUG_OUTPUT)
        Cout << "Change in orders:\n"
             << delta_order << "converge_order() tolerance = " << tolerance
             << '\n';
...
void RichExtrapVerification::extrapolate_result(const RealVector& refine_triple,
