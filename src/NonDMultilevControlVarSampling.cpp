...
static const char rcsId[] =
    "@(#) $Id: NonDMultilevControlVarSampling.cpp 7035 2010-10-22 21:45:39Z "
    "mseldre $";
...
NonDMultilevControlVarSampling::NonDMultilevControlVarSampling(
    ...
    : NonDMultilevelSampling(problem_db, parallel_lib, model),
      ...
      NonDHierarchSampling(problem_db, parallel_lib,
                           model),  // top of virtual inheritance
...
void NonDMultilevControlVarSampling::pre_run() {
  ...
  for (i = 0; i < num_mf; ++i) {
    ...
    num_lev = Nl_i.size();
    for (j = 0; j < num_lev; ++j) Nl_i[j].assign(numFunctions, 0);
...
void NonDMultilevControlVarSampling::core_run() {
  ...
  switch (sequenceType) {  // 1d fallbacks selected in configure_2d_sequence()
    case Pecos::RESOLUTION_LEVEL_1D_SEQUENCE:
      ...
      return;
      break;
      // MFMC not inherited as CVMC was, so rely on MLCV with 2 models,
      // num_hf_lev=1
  ...
  switch (pilotMgmtMode) {
    case ONLINE_PILOT:
      // if (true) // reformulated approach using 1 new QoI correlation per
      // level
      //   multilevel_control_variate_mc_Qcorr();
      // else      // original approach using 1 discrepancy correlation per
      // level
...
void NonDMultilevControlVarSampling::assign_active_key() {
  ...
  size_t num_hf_lev = NLevActual[hf_form].size(),
         num_cv_lev = (num_mf > 1)
                          ? std::min(num_hf_lev, NLevActual[lf_form].size())
                          : 0;
...
{
  size_t qoi, lev,
      num_mf = NLevActual.size(), N_alloc_l,
      ...
      num_cv_lev =
          (num_mf > 1)
              ...
              : 0;
  RealVector hf_targets(num_hf_lev);
  Real eps_sq_div_2;
  ...
  IntRealMatrixMap sum_Ll, sum_Llm1, sum_Hl, sum_Hlm1, sum_Ll_Ll,
      sum_Ll_Llm1,               // for Var(Q_l^L), Covar(Q_l^L,Q_lm1^L)
      sum_Llm1_Llm1, sum_Hl_Ll,  // for Var(Q_lm1^L), Covar(Q_l^H,Q_l^L)
      sum_Hl_Llm1,
      sum_Hlm1_Ll,    // for Covar(Q_l^H,Q_lm1^L), Covar(Q_lm1^H,Q_l^L)
  ...
  unsigned short group, lf_form = 0,
                        hf_form = num_mf - 1;  // 2 models @ extremes
  ...
  if (finalStatsType == QOI_STATISTICS) {
    ...
    for (lev = 0, group = 0; lev < num_cv_lev; ++lev, ++group)  // no relaxation
      delta_N_lf[lev] =
          (backfillFailures) ?  // increment LF samples
              ...
              lf_increment(eval_ratios[lev], N_alloc_lf[lev], hf_targets[lev],
                           lf_targets[lev]);  // relative to allocated
    ...
    RealMatrix Y_mom(4, numFunctions);
    RealVector2DArray beta_2d, gamma_2d;
    ...
    RealMatrix& sum_Hl_1 = sum_Hl[1];
    RealMatrix& sum_Hl_2 = sum_Hl[2];
...
void NonDMultilevControlVarSampling::
  ...
  IntRealMatrixMap sum_Ll_pilot, sum_Llm1_pilot, sum_Hl_pilot, sum_Hlm1_pilot,
      ...
      sum_Hl_Llm1_pilot, sum_Hlm1_Ll_pilot, sum_Hlm1_Llm1_pilot,
      sum_Hl_Hl_pilot, sum_Hl_Hlm1_pilot, sum_Hlm1_Hlm1_pilot;
  initialize_mlmf_sums(sum_Ll_pilot, sum_Llm1_pilot, sum_Hl_pilot,
                       ...
                       sum_Hl_Hlm1_pilot, sum_Hlm1_Hlm1_pilot, num_hf_lev,
                       num_cv_lev);

  // -----------------------------------------
  // Initial loop for offline (overkill) pilot
  // -----------------------------------------
  SizetArray N_alloc_pilot;
  Sizet2DArray N_actual_pilot(num_hf_lev);
  ...
  for (lev = 0, group = 0; lev < num_cv_lev; ++lev, ++group)  // no relaxation
    delta_N_lf[lev] = (backfillFailures) ?  // increment LF samples
                          lf_increment(eval_ratios_pilot[lev], N_actual_lf[lev],
                                       hf_targets_pilot[lev], lf_targets[lev])
                                         :  // rel to successful
                          lf_increment(eval_ratios_pilot[lev], N_alloc_lf[lev],
                                       hf_targets_pilot[lev],
                                       lf_targets[lev]);  // rel to allocated
  ...
  compute_mlmf_controls(sum_Ll_pilot, sum_Llm1_pilot, sum_Hl_pilot,
                        ...
                        sum_Llm1_Llm1_pilot, sum_Hl_Ll_pilot, sum_Hl_Llm1_pilot,
                        sum_Hlm1_Ll_pilot, sum_Hlm1_Llm1_pilot, sum_Hl_Hl_pilot,
                        sum_Hl_Hlm1_pilot, sum_Hlm1_Hlm1_pilot, N_actual_pilot,
                        num_cv_lev, beta_2d, gamma_2d);
  apply_mlmf_controls(sum_Ll, sum_Llm1, sum_Hl, sum_Hlm1, N_actual_hf,
                      sum_Ll_refined, sum_Llm1_refined, N_actual_lf, num_cv_lev,
                      beta_2d, gamma_2d, Y_mom);
  if (num_hf_lev > num_cv_lev)
    ml_Y_raw_moments(sum_Hl[1], sum_Hl[2], sum_Hl[3], sum_Hl[4], N_actual_hf,
                     num_cv_lev, num_hf_lev, Y_mom);
...
void NonDMultilevControlVarSampling::
  ...
  SizetArray delta_N_hf;
  delta_N_hf.assign(num_hf_lev, 0);
...
void NonDMultilevControlVarSampling::evaluate_levels(
    ...
    RealVectorArray& eval_ratios, RealMatrix& Lambda, RealMatrix& var_YH,
    Real& eps_sq_div_2, SizetArray& delta_N_hf, Sizet2DArray& N_actual,
    SizetArray& N_alloc, RealVector& hf_targets, bool accumulate_cost,
    bool pilot_estvar) {
  ...
  accumulate_increments(
      delta_N_hf, N_actual, N_alloc, hf_targets, sum_Ll, sum_Llm1, sum_Hl,
      sum_Hlm1, sum_Ll_Ll, sum_Ll_Llm1, sum_Llm1_Llm1, sum_Hl_Ll, sum_Hl_Llm1,
      sum_Hlm1_Ll, sum_Hlm1_Llm1, sum_Hl_Hl, sum_Hl_Hlm1, sum_Hlm1_Hlm1, false);

  // THIRD PASS: allocations
  compute_allocations(
      eval_ratios, Lambda, var_YH, eps_sq_div_2, delta_N_hf, N_actual, N_alloc,
      hf_targets, accumulate_cost, pilot_estvar, sum_Ll, sum_Llm1, sum_Hl,
      sum_Hlm1, sum_Ll_Ll, sum_Ll_Llm1, sum_Llm1_Llm1, sum_Hl_Ll, sum_Hl_Llm1,
      sum_Hlm1_Ll, sum_Hlm1_Llm1, sum_Hl_Hl, sum_Hl_Hlm1, sum_Hlm1_Hlm1);
}

void NonDMultilevControlVarSampling::evaluate_pilot(
    RealVectorArray& eval_ratios, RealMatrix& Lambda, RealMatrix& var_YH,
    Real& eps_sq_div_2, Sizet2DArray& N_actual, SizetArray& N_alloc,
    RealVector& hf_targets, RealMatrix& pilot_mom, bool accumulate_cost,
    bool pilot_estvar) {
  size_t qoi, lev,
      num_mf = NLevActual.size(),
      num_hf_lev = iteratedModel->truth_model()->solution_levels(),
  ...
  RealMatrix sum_Ll(numFunctions, num_cv_lev),
      ...
      sum_Hl_Ll(numFunctions, num_cv_lev),
      sum_Hl_Llm1(numFunctions, num_cv_lev),
      sum_Hlm1_Ll(numFunctions, num_cv_lev),
      sum_Hlm1_Llm1(numFunctions, num_cv_lev),
      sum_Hl_Hl(numFunctions, num_hf_lev),
      sum_Hl_Hlm1(numFunctions, num_hf_lev),
      sum_Hlm1_Hlm1(numFunctions, num_hf_lev),
      rho_dot2_LH(numFunctions, num_cv_lev, false);
  IntRealMatrixMap sum_Hl;
  initialize_ml_Ysums(sum_Hl, num_hf_lev, 2);
  IntRealMatrixMap sum_Hlm1;
  initialize_ml_Ysums(sum_Hlm1, num_hf_lev, 2);
  ...
  RealMatrix& sum_Hl_2 = sum_Hl[2];
  // For levels 0:num_cv_lev, sum_Hl[p] is Ql^p
  ml_Q_raw_moments(sum_Hl_1, sum_Hlm1[1], sum_Hl_2, sum_Hlm1[2], N_actual, 0,
                   num_cv_lev, pilot_mom);
  // For levels num_cv_lev:num_hf_lev, sum_Hl[p] is level discrepancy in Q^p,
  // i.e. Ql^p - Qlm1^p, and can be summed directly into raw moments:
...
void NonDMultilevControlVarSampling::mlmf_increments(
    const SizetArray& delta_N_l, String prepend) {
  ...
  size_t lev,
      num_mf = NLevActual.size(), num_hf_lev = NLevActual.back().size(),
      num_cv_lev =
          (num_mf > 1) ? std::min(num_hf_lev, NLevActual.front().size()) : 0;
...
/** The version in NonDNonHierarchSampling would be sufficiently general here
    ...
    MLCV MC employ AGGREGATED_MODEL_PAIR without ASV subsetting, so we
    specialize for that case. */
void NonDMultilevControlVarSampling::lf_increments(const SizetArray& delta_N_lf,
                                                   String prepend) {
  if (mlmfIter == 0) Cout << "\nPerforming pilot sample for model groups.\n";
  // else Cout << "\nSampling iteration " << mlmfIter << ": sample increment
  // =\n"
...
void NonDMultilevControlVarSampling::compute_eval_ratios(
    RealMatrix& sum_L_shared, RealMatrix& sum_H, RealMatrix& sum_LL,
...
void NonDMultilevControlVarSampling::compute_eval_ratios(
    RealMatrix& sum_Ll, RealMatrix& sum_Llm1, RealMatrix& sum_Hl,
    RealMatrix& sum_Hlm1, RealMatrix& sum_Ll_Ll, RealMatrix& sum_Ll_Llm1,
    RealMatrix& sum_Llm1_Llm1, RealMatrix& sum_Hl_Ll, RealMatrix& sum_Hl_Llm1,
    RealMatrix& sum_Hlm1_Ll, RealMatrix& sum_Hlm1_Llm1, RealMatrix& sum_Hl_Hl,
    RealMatrix& sum_Hl_Hlm1, RealMatrix& sum_Hlm1_Hlm1, Real cost_ratio,
    size_t lev, const SizetArray& N_shared, RealMatrix& var_YHl,
  ...
  if (lev == 0)
    ...
    for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
      ...
      compute_mlmf_control(
          sum_Ll(qoi, lev), sum_Llm1(qoi, lev), sum_Hl(qoi, lev),
          sum_Hlm1(qoi, lev), sum_Ll_Ll(qoi, lev), sum_Ll_Llm1(qoi, lev),
          sum_Llm1_Llm1(qoi, lev), sum_Hl_Ll(qoi, lev), sum_Hl_Llm1(qoi, lev),
          sum_Hlm1_Ll(qoi, lev), sum_Hlm1_Llm1(qoi, lev), sum_Hl_Hl(qoi, lev),
          sum_Hl_Hlm1(qoi, lev), sum_Hlm1_Hlm1(qoi, lev), N_shared[qoi],
      ...
      eval_ratios[qoi] =
          (rho_dot_sq < 1.)
              ...
              : std::sqrt(cost_ratio /
                          Pecos::SMALL_NUMBER);  // should not happen
      if (outputLevel >= DEBUG_OUTPUT)
        Cout << "eval_ratio() QoI " << qoi + 1
             << ": cost_ratio = " << cost_ratio
             << " rho_dot_sq = " << rho_dot_sq
...
size_t NonDMultilevControlVarSampling::lf_increment(
    const RealVector& eval_ratios, const SizetArray& N_lf, Real hf_target,
  ...
  size_t num_samp =
      one_sided_delta(N_lf, lf_targets);  // averaged, no relaxation
...
size_t NonDMultilevControlVarSampling::lf_increment(
    const RealVector& eval_ratios, size_t N_lf, Real hf_target,
    RealVector& lf_targets) {
  if (lf_targets.empty()) lf_targets.sizeUninitialized(numFunctions);
  for (size_t qoi = 0; qoi < numFunctions; ++qoi)
    lf_targets[qoi] = eval_ratios[qoi] * hf_target;
  size_t num_samp =
      one_sided_delta((Real)N_lf, average(lf_targets));  // no relax
...
void NonDMultilevControlVarSampling::compute_mlmf_controls(
    IntRealMatrixMap& sum_Ll, IntRealMatrixMap& sum_Llm1,
    IntRealMatrixMap& sum_Hl, IntRealMatrixMap& sum_Hlm1,
    IntRealMatrixMap& sum_Ll_Ll, IntRealMatrixMap& sum_Ll_Llm1,
    IntRealMatrixMap& sum_Llm1_Llm1, IntRealMatrixMap& sum_Hl_Ll,
...
void NonDMultilevControlVarSampling::compute_mlmf_controls(
    const Real* sum_Ll_m, const Real* sum_Llm1_m, const Real* sum_Hl_m,
    const Real* sum_Hlm1_m, const Real* sum_Ll_Ll_m, const Real* sum_Ll_Llm1_m,
    ...
    const SizetArray& N_shared, RealVector& beta_dot, RealVector& gamma) {
  if (beta_dot.length() != numFunctions)
    beta_dot.sizeUninitialized(numFunctions);
  if (gamma.length() != numFunctions) gamma.sizeUninitialized(numFunctions);
  Real var_YHl, rho_dot2_LH;  // not needed for this context
  for (size_t qoi = 0; qoi < numFunctions; ++qoi)
    compute_mlmf_control(
        sum_Ll_m[qoi], sum_Llm1_m[qoi], sum_Hl_m[qoi], sum_Hlm1_m[qoi],
        sum_Ll_Ll_m[qoi], sum_Ll_Llm1_m[qoi], sum_Llm1_Llm1_m[qoi],
        sum_Hl_Ll_m[qoi], sum_Hl_Llm1_m[qoi], sum_Hlm1_Ll_m[qoi],
        sum_Hlm1_Llm1_m[qoi], sum_Hl_Hl_m[qoi], sum_Hl_Hlm1_m[qoi],
        sum_Hlm1_Hlm1_m[qoi], N_shared[qoi], var_YHl, rho_dot2_LH,
        beta_dot[qoi], gamma[qoi]);
}

void NonDMultilevControlVarSampling::apply_mf_controls(
    const Real* sum_H, const Real* sum_L_shared, const SizetArray& N_shared,
    const Real* sum_L_refined, const SizetArray& N_refined,
    const RealVector& beta, RealMatrix& H_raw_mom, int m_index) {
  for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
    Cout << "   QoI " << qoi + 1 << ": control variate beta = " << std::setw(9)
         << beta[qoi] << '\n';
...
void NonDMultilevControlVarSampling::apply_mlmf_controls(
    IntRealMatrixMap& sum_Ll, IntRealMatrixMap& sum_Llm1,
    IntRealMatrixMap& sum_Hl, IntRealMatrixMap& sum_Hlm1,
    const Sizet2DArray& N_shared, IntRealMatrixMap& sum_Ll_refined,
    IntRealMatrixMap& sum_Llm1_refined, const Sizet2DArray& N_refined,
    size_t num_cv_lev, const RealVector2DArray& beta_2d,
  ...
  for (lev = 1; lev < num_cv_lev; ++lev) {
    ...
    for (int m = 1; m <= num_mom; ++m) {
      ...
      Cout << "Moment " << m << ":\n";
      apply_mlmf_controls(
          sum_Hl.at(m)[lev], sum_Hlm1.at(m)[lev], sum_Ll.at(m)[lev],
          sum_Llm1.at(m)[lev], N_shared[lev], sum_Ll_refined.at(m)[lev],
          sum_Llm1_refined.at(m)[lev], N_refined[lev], beta_2d[m_index][lev],
          gamma_2d[m_index][lev], H_raw_mom, m_index);
    }
    Cout << '\n';  // for loop over levels
  }
}
...
void NonDMultilevControlVarSampling::apply_mlmf_controls(
    const Real* sum_Hl, const Real* sum_Hlm1, const Real* sum_Ll,
    const Real* sum_Llm1, const SizetArray& N_shared,
    const Real* sum_Ll_refined, const Real* sum_Llm1_refined,
    const SizetArray& N_refined, const RealVector& beta_dot,
    const RealVector& gamma, RealMatrix& H_raw_mom, int m_index) {
  for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
    Cout << "   QoI " << qoi + 1
         << ": control variate beta_dot = " << std::setw(9) << beta_dot[qoi]
         << '\n';
    apply_mlmf_control(sum_Hl[qoi], sum_Hlm1[qoi], sum_Ll[qoi], sum_Llm1[qoi],
                       N_shared[qoi], sum_Ll_refined[qoi],
...
void NonDMultilevControlVarSampling::compute_mlmf_control(
    Real sum_Ll, Real sum_Llm1, Real sum_Hl, Real sum_Hlm1, Real sum_Ll_Ll,
    Real sum_Ll_Llm1, Real sum_Llm1_Llm1, Real sum_Hl_Ll, Real sum_Hl_Llm1,
    Real sum_Hlm1_Ll, Real sum_Hlm1_Llm1, Real sum_Hl_Hl, Real sum_Hl_Hlm1,
    Real sum_Hlm1_Hlm1, size_t N_shared, Real& var_YHl, Real& rho_dot2_LH,
  ...
  // theta, tau, beta:
  Real cov_YHl_YLldot =
      gamma * (cov_Hl_Ll - cov_Hlm1_Ll) - cov_Hl_Llm1 + cov_Hlm1_Llm1;
...
void NonDMultilevControlVarSampling::update_projected_samples(
    const RealVector& hf_targets, const RealVectorArray& eval_ratios,
    const Sizet2DArray& N_actual_hf, SizetArray& N_alloc_hf,
    const Sizet2DArray& N_actual_lf, SizetArray& N_alloc_lf,
  ...
  size_t hf_actual_incr, hf_alloc_incr, lf_actual_incr, lf_alloc_incr, lev,
      num_hf_lev = iteratedModel->truth_model()->solution_levels(),
      num_cv_lev = std::min(
          num_hf_lev, iteratedModel->surrogate_model()->solution_levels());
  ...
  for (lev = 0; lev < num_hf_lev; ++lev) {
    ...
    hf_actual_incr = (backfillFailures)
                         ? one_sided_delta(N_actual_hf[lev], hf_target_l)
                         : hf_alloc_incr;  // no relax
    ...
    if (lev < num_cv_lev) {
      ...
      lf_actual_incr = (backfillFailures)
                           ? one_sided_delta(N_actual_lf[lev], lf_targets)
                           : lf_alloc_incr;
      // increment_samples(N_actual_lf[lev], lf_actual_incr);
      // delta_N_actual_lf[lev] += lf_actual_incr;
      increment_mlmf_equivalent_cost(
          hf_actual_incr, level_cost(sequenceCost, lev, num_cv_lev),
          lf_actual_incr, level_cost(sequenceCost, lev), hf_ref_cost,
          delta_equiv_hf);
    } else
      increment_ml_equivalent_cost(hf_actual_incr,
                                   level_cost(sequenceCost, lev, num_cv_lev),
                                   hf_ref_cost, delta_equiv_hf);
  }
}

void NonDMultilevControlVarSampling::update_projected_lf_samples(
    const RealVector& hf_targets, const RealVectorArray& eval_ratios,
    const Sizet2DArray& N_actual_lf, SizetArray& N_alloc_lf,
    // SizetArray& delta_N_actual_lf,
    Real& delta_equiv_hf) {
  size_t lf_actual_incr, lf_alloc_incr, lev,
      num_cv_lev =
...
void NonDMultilevControlVarSampling::initialize_mlmf_sums(
    IntRealMatrixMap& sum_Ll, IntRealMatrixMap& sum_Llm1,
    IntRealMatrixMap& sum_Hl, IntRealMatrixMap& sum_Hlm1,
    IntRealMatrixMap& sum_Ll_Ll, IntRealMatrixMap& sum_Ll_Llm1,
    IntRealMatrixMap& sum_Llm1_Llm1, IntRealMatrixMap& sum_Hl_Ll,
  ...
  for (int i = 1; i <= num_mom; ++i) {
    ...
    sum_Llm1_Llm1.insert(empty_pr).first->second.shape(numFunctions,
                                                       num_cv_lev);
    ...
    sum_Hlm1_Llm1.insert(empty_pr).first->second.shape(numFunctions,
                                                       num_cv_lev);
    // num_ml_lev:
    ...
    sum_Hlm1_Hlm1.insert(empty_pr).first->second.shape(numFunctions,
                                                       num_ml_lev);
  }
}
...
void NonDMultilevControlVarSampling::accumulate_mlmf_Qsums(
...
void NonDMultilevControlVarSampling::accumulate_mlmf_Qsums(
    const IntResponseMap& resp_map, IntRealMatrixMap& sum_L_shared,
    IntRealMatrixMap& sum_H, IntRealMatrixMap& sum_LL, IntRealMatrixMap& sum_LH,
    IntRealMatrixMap& sum_HH, size_t lev, SizetArray& N_shared) {
  using std::isfinite;
  Real lf_l, hf_l, lf_l_prod, hf_l_prod;
  IntRespMCIter r_it;
  IntRMMIter ls_it, h_it, ll_it, lh_it, hh_it;
  ...
  for (r_it = resp_map.begin(); r_it != resp_map.end(); ++r_it) {
    ...
    for (qoi = 0; qoi < numFunctions; ++qoi) {
      ...
      if (isfinite(lf_l) && isfinite(hf_l)) {  // neither NaN nor +/-Inf
        ...
        while (ls_it != sum_L_shared.end() || h_it != sum_H.end() ||
...
void NonDMultilevControlVarSampling::accumulate_mlmf_Qsums(
    const IntResponseMap& resp_map, IntRealMatrixMap& sum_Ll,
    IntRealMatrixMap& sum_Llm1, IntRealMatrixMap& sum_Hl,
    IntRealMatrixMap& sum_Hlm1, IntRealMatrixMap& sum_Ll_Ll,
    IntRealMatrixMap& sum_Ll_Llm1, IntRealMatrixMap& sum_Llm1_Llm1,
    IntRealMatrixMap& sum_Hl_Ll, IntRealMatrixMap& sum_Hl_Llm1,
    IntRealMatrixMap& sum_Hlm1_Ll, IntRealMatrixMap& sum_Hlm1_Llm1,
    IntRealMatrixMap& sum_Hl_Hl, IntRealMatrixMap& sum_Hl_Hlm1,
    IntRealMatrixMap& sum_Hlm1_Hlm1, size_t lev, SizetArray& N_shared) {
  ...
  if (lev == 0)  // level lm1 not available; accumulate only level l
    ...
    using std::isfinite;
    Real lf_l_prod, lf_l, lf_lm1_prod, lf_lm1, hf_l_prod, hf_l, hf_lm1_prod,
        hf_lm1;
    IntRespMCIter r_it;
    IntRMMIter l1_it, l2_it, h1_it, h2_it, ll1_it, ll2_it, ll3_it, lh1_it,
        lh2_it, lh3_it, lh4_it, hh1_it, hh2_it, hh3_it;
    ...
    for (r_it = resp_map.begin(); r_it != resp_map.end(); ++r_it) {
      ...
      for (qoi = 0; qoi < numFunctions; ++qoi) {
        ...
        // sync sample counts for all L and H interactions at this level
        if (isfinite(lf_l) && isfinite(lf_lm1) && isfinite(hf_l) &&
            isfinite(hf_lm1)) {  // neither NaN nor +/-Inf
          // Low: Ll, Llm1
          l1_it = sum_Ll.begin();
...
void NonDMultilevControlVarSampling::accumulate_mlmf_Qsums(
    const IntResponseMap& resp_map, RealMatrix& sum_L_shared,
    IntRealMatrixMap& sum_H, RealMatrix& sum_LL, RealMatrix& sum_LH,
    RealMatrix& sum_HH, size_t lev, SizetArray& N_shared) {
  using std::isfinite;
  Real lf_l, hf_l, hf_prod;
  int h_ord, active_ord;
...
void NonDMultilevControlVarSampling::accumulate_mlmf_Qsums(
    const IntResponseMap& resp_map, RealMatrix& sum_Ll, RealMatrix& sum_Llm1,
    IntRealMatrixMap& sum_Hl, IntRealMatrixMap& sum_Hlm1, RealMatrix& sum_Ll_Ll,
    RealMatrix& sum_Ll_Llm1, RealMatrix& sum_Llm1_Llm1, RealMatrix& sum_Hl_Ll,
    RealMatrix& sum_Hl_Llm1, RealMatrix& sum_Hlm1_Ll, RealMatrix& sum_Hlm1_Llm1,
    RealMatrix& sum_Hl_Hl, RealMatrix& sum_Hl_Hlm1, RealMatrix& sum_Hlm1_Hlm1,
...
void NonDMultilevControlVarSampling::print_variance_reduction(
    std::ostream& s) const {
  if (delegateMethod ==
      MULTILEVEL_SAMPLING)  // not currently overridden by MLMC
  {
    NonDMultilevelSampling::print_variance_reduction(s);
    return;
  }
