...
static const char rcsId[] =
    "@(#) $Id: DOTOptimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
DOTOptimizer::DOTOptimizer(ProblemDescDB& problem_db,
                           ParallelLibrary& parallel_lib,
                           std::shared_ptr<Model> model)
    : Optimizer(problem_db, parallel_lib, model,
                std::shared_ptr<TraitsBase>(new DOTTraits())),
...
DOTOptimizer::DOTOptimizer(const String& method_string,
                           std::shared_ptr<Model> model)
...
void DOTOptimizer::initialize() {
  ...
  realCntlParmArray[2] =
      convergenceTol;  // RPRM(3) =DABOBJ=MAX(0.0001*ABS(F0),1.E-20)
  realCntlParmArray[3] = convergenceTol;  // RPRM(4) =DELOBJ
  realCntlParmArray[11] =
      convergenceTol;  // RPRM(12)=DABSTR=MAX(0.0001*ABS(F0),1.E-20)
    ...
    if (dotInfo == 2) {
      ...
      for (dot_constr = 0; dot_constr < numDotConstr; ++dot_constr) {
        ...
        if (dot_constr < numDotNlnConstr)  // nonlinear ineq & eq
          ...
          constraintValues[dot_constr] =
              constraintMapOffsets[dot_constr] +
