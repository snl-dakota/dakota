...
RelaxedVarConstraints::RelaxedVarConstraints(const ProblemDescDB& problem_db,
                                             const SharedVariablesData& svd)
    : Constraints(BaseConstructor(), problem_db, svd) {
  const RealVector& cdv_l_bnds =
      problem_db.get_rv("variables.continuous_design.lower_bounds");
  const RealVector& cdv_u_bnds =
      problem_db.get_rv("variables.continuous_design.upper_bounds");
  const RealVector& cauv_l_bnds =
      problem_db.get_rv("variables.continuous_aleatory_uncertain.lower_bounds");
  const RealVector& cauv_u_bnds =
      problem_db.get_rv("variables.continuous_aleatory_uncertain.upper_bounds");
  ...
  const RealVector& csv_l_bnds =
      problem_db.get_rv("variables.continuous_state.lower_bounds");
  const RealVector& csv_u_bnds =
      problem_db.get_rv("variables.continuous_state.upper_bounds");

  const IntVector& ddrv_l_bnds =
      problem_db.get_iv("variables.discrete_design_range.lower_bounds");
  const IntVector& ddrv_u_bnds =
      problem_db.get_iv("variables.discrete_design_range.upper_bounds");
  const IntVector& ddsiv_l_bnds =
      problem_db.get_iv("variables.discrete_design_set_int.lower_bounds");
  const IntVector& ddsiv_u_bnds =
      problem_db.get_iv("variables.discrete_design_set_int.upper_bounds");
  ...
  const IntVector& dsrv_l_bnds =
      problem_db.get_iv("variables.discrete_state_range.lower_bounds");
  const IntVector& dsrv_u_bnds =
      problem_db.get_iv("variables.discrete_state_range.upper_bounds");
  const IntVector& dssiv_l_bnds =
      problem_db.get_iv("variables.discrete_state_set_int.lower_bounds");
  const IntVector& dssiv_u_bnds =
      problem_db.get_iv("variables.discrete_state_set_int.upper_bounds");

  // discrete string variable bounds?  (not needed for exclusively categorical)

  const RealVector& ddsrv_l_bnds =
      problem_db.get_rv("variables.discrete_design_set_real.lower_bounds");
  const RealVector& ddsrv_u_bnds =
      problem_db.get_rv("variables.discrete_design_set_real.upper_bounds");
  ...
  const RealVector& dssrv_l_bnds =
      problem_db.get_rv("variables.discrete_state_set_real.lower_bounds");
  const RealVector& dssrv_u_bnds =
      problem_db.get_rv("variables.discrete_state_set_real.upper_bounds");

  size_t i,
      ardi_cntr = 0, ardr_cntr = 0, acv_offset = 0, adiv_offset = 0,
      adrv_offset = 0, num_ddrv = ddrv_l_bnds.length(),
      num_ddsiv = ddsiv_l_bnds.length(), num_ddsrv = ddsrv_l_bnds.length(),
      num_dauiv = dauiv_l_bnds.length(), num_daurv = daurv_l_bnds.length(),
      num_deuiv = deuiv_l_bnds.length(), num_deurv = deurv_l_bnds.length(),
      num_dsrv = dsrv_l_bnds.length(), num_dssiv = dssiv_l_bnds.length(),
      num_dssrv = dssrv_l_bnds.length();
...
void RelaxedVarConstraints::read(std::istream& s) {
  ...
  size_t num_cdv = vc_totals[TOTAL_CDV], num_ddiv = vc_totals[TOTAL_DDIV],
         ...
         /*num_dausv = vc_totals[TOTAL_DAUSV],*/ num_daurv =
             vc_totals[TOTAL_DAURV],
         num_ceuv = vc_totals[TOTAL_CEUV], num_deuiv = vc_totals[TOTAL_DEUIV],
         /*num_deusv = vc_totals[TOTAL_DEUSV],*/ num_deurv =
