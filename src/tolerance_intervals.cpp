...
static const char rcsId[] =
    "@(#) $Id: tolerance_intervals.cpp 9999 2010-10-22 23:20:24Z mseldre $";
...
Real std_normal_coverage_inverse(const Real coverage) {
  if ((0. <= coverage) && (coverage <= 1.)) {
    ...
    Cerr << "Error in std_normal_coverage_inverse()"
         << ": coverage (" << coverage
         << ") must belong to the closed interval [0,1]" << std::endl;
    abort_handler(-1);
  }
...
Real computeDSTIEN_conversion_factor(const size_t number_of_samples,
  ...
  if ((0. <= alpha) && (alpha <= 1.)) {
    ...
    Cerr << "Error in computeDSTIEN_conversion_factor()"
         << ": alpha (" << alpha << ") must belong to the closed interval [0,1]"
         << std::endl;
    abort_handler(-1);
  }
  ...
  if (number_of_samples == 1) {
    ...
    conversionFactor = std::sqrt(1. + 1. / m) * std::sqrt((m - 1.) / quant) *
                       std::sqrt(1. + tmpRatio);
  }
...
void computeDSTIEN(const IntResponseMap& resp_samples, const Real coverage,
  ...
  for (IntRespMCIter it = resp_samples.begin(); it != resp_samples.end();
       ++it) {
    if (it->second.num_functions() != num_responses) {
      Cerr << "Error in computeDSTIEN()"
           << ": all response samples must have the same size ("
           << num_responses << ") as the first sample" << std::endl;
      abort_handler(-1);
    }
  }
  ...
  if (num_valid_samples == 0) {
    ...
    if (num_valid_samples == 1) {
      ...
      Real conversionFactor =
          computeDSTIEN_conversion_factor(num_valid_samples, alpha);
