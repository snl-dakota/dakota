...
class Sampler {
 public:
  ...
  void enrich_samples(int num_dims, const RealMatrix &initial_samples,
                      int num_new_samples, const RealMatrix &candidate_samples,
                      RealMatrix &result);
...
class LejaSampler : public Sampler {
 ...
 public:
  ...
  /// Default constructor
  LejaSampler() : numCandidateSamples_(10000), seed_(0), precondition_(true) {};
  ...
  /** \brief Draw samples from the equilibrium measure. For all bounded
   * variables the equilibrium measure is the arcsine measure, i.e. "Chebyshev"
   * density
   */
  ...
  /// Enrinch a set of M samples with N new samples
  void enrich_samples(int num_dims, const RealMatrix &initial_samples,
                      int num_new_samples, RealMatrix &result) override;
   ...
   */
  void build_basis_matrix(const RealMatrix &samples, RealMatrix &result);

  /// Set the polynomial basis
  void set_polynomial_basis(
