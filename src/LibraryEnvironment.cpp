...
static const char rcsId[] =
    "@(#) $Id: LibraryEnvironment.cpp 6492 2009-12-19 00:04:28Z briadam $";

namespace Dakota {

LibraryEnvironment::LibraryEnvironment() : Environment(BaseConstructor()) {}

/** Construct library environment, optionally performing check/bcast
    of database and iterator construction */
LibraryEnvironment::LibraryEnvironment(ProgramOptions prog_opts,
  ...
  if (check_bcast_construct) construct();
}
...
LibraryEnvironment::LibraryEnvironment(MPI_Comm dakota_mpi_comm,
...
void LibraryEnvironment::insert_nodes(Dakota::DataMethod& dme,
...
bool LibraryEnvironment::plugin_interface(
    const String& model_type, const String& interf_type,
    const String& an_driver, std::shared_ptr<Interface> plugin_iface) {
...
InterfaceList LibraryEnvironment::filtered_interface_list(
    const String& interf_type, const String& an_driver) {
...
ModelList LibraryEnvironment::filtered_model_list(const String& model_type,
                                                  const String& interf_type,
                                                  const String& an_driver) {
