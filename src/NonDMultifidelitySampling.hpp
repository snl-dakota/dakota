...
inline const MFSolutionData& NonDMultifidelitySampling::final_solution_data()
    const {
  return mfmcSolnData;
}

inline void NonDMultifidelitySampling::initialize_mf_sums(
    IntRealMatrixMap& sum_L_baseline, IntRealVectorMap& sum_H,
    IntRealMatrixMap& sum_LL, IntRealMatrixMap& sum_LH, RealVector& sum_HH) {
...
inline void NonDMultifidelitySampling::mfmc_estvar_ratios(
...
inline void NonDMultifidelitySampling::correlation_sq_to_covariance(
    const RealMatrix& rho2_LH, const RealMatrix& var_L, const RealVector& var_H,
    RealMatrix& cov_LH) {
...
inline void NonDMultifidelitySampling::matrix_to_diagonal_array(
    const RealMatrix& var_L, RealSymMatrixArray& cov_LL) {
  size_t qoi, approx;
  if (cov_LL.empty()) {
...
inline void NonDMultifidelitySampling::print_analytic_solution(
    const RealMatrix& rho2_LH, const RealVector& avg_eval_ratios) const {
  if (outputLevel < NORMAL_OUTPUT) return;

  Cout << "MFMC analytic solution:\n";
  bool ordered = corrApproxSequence.empty();
  size_t i, qoi, approx;
  for (qoi = 0; qoi < numFunctions; ++qoi)
    for (i = 0; i < numApprox; ++i) {
      approx = (ordered) ? i : corrApproxSequence[i];
      Cout << "  QoI " << qoi + 1 << " Approx "
           << approx + 1
...
inline void NonDMultifidelitySampling::print_variance_reduction(
    std::ostream& s) const {
