...
ProcessHandleApplicInterface::ProcessHandleApplicInterface(
    const ProblemDescDB& problem_db, ParallelLibrary& parallel_lib)
...
void ProcessHandleApplicInterface::process_local_evaluation(PRPQueue& prp_queue,
                                                            const pid_t pid) {
  ...
  // (shallow) replace_by_eval_id(prp_queue, fn_eval_id, *queue_it);// not
  // needed (shallow)
...
pid_t ProcessHandleApplicInterface::create_evaluation_process(bool block_flag) {
  ...
  if (evalCommRank == 0 && !suppressOutput) {
    ...
    if (!iFilterName.empty()) {
      Cout << substitute_params_and_results(iFilterName, paramsFileName,
                                            resultsFileName);
      if (commandLineArgs)
    ...
    for (i = 0; i < numAnalysisDrivers; i++) {
      ...
      if (multipleParamsFiles) params_file += '.' + std::to_string(i + 1);
      if (numAnalysisDrivers > 1) results_file += '.' + std::to_string(i + 1);
      Cout << substitute_params_and_results(programNames[i], params_file,
                                            results_file);
      if (commandLineArgs) {
        Cout << ' ' << params_file << ' ' << results_file;
      }
      if (i != numAnalysisDrivers - 1) Cout << "; ";
    }
  ...
  if (iFilterName.empty() && oFilterName.empty() && numAnalysisDrivers == 1) {
    ...
    if (block_flag || pid == 0) {
      ...
      if (asynchLocalAnalysisFlag)  // asynch w/ concurrency limit>1 or
                                    // unlimited
      ...
      // process now.
      if (!block_flag) _exit(1);
    } else  // parent process from fork() above
      if (new_group) evaluation_process_group_id(pid);
  }
...
void ProcessHandleApplicInterface::asynchronous_local_analyses(int start,
                                                               int end,
                                                               int step) {
...
void ProcessHandleApplicInterface::serve_analyses_asynch() {
  ...
  do {  // main loop
    ...
    while (mpi_test_flag && analysis_id &&
      ...
      // test for completion
      if (recv_request) parallelLib.test(recv_request, mpi_test_flag, status);
  ...
  if (pid == -1) {
    ...
    switch (errno) {
      ...
      case EINTR:
        ...
        break;
      case EINVAL:
        Cerr << "The options argument was invalid";
        break;
      default:
...
void ProcessHandleApplicInterface::join_evaluation_process_group(
    bool new_group) {}  // no-op
...
*/
void ProcessHandleApplicInterface::create_command_arguments(
    boost::shared_array<const char*>& av, StringArray& driver_and_args) {
  String driver_subbed =
      substitute_params_and_results(argList[0], argList[1], argList[2]);
  ...
  if (commandLineArgs) nargs += 2;
  // ideally would use char *const argv[],
  av.reset(new const char*[nargs + 1]);  // need extra NULL

  size_t i = 0;
  for (; i < driver_and_args.size(); ++i) av[i] = driver_and_args[i].c_str();
