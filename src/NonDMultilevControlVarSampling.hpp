...
//, public NonDMultifidelitySampling
{
 ...
 private:
  ...
  void evaluate_levels(
      IntRealMatrixMap& sum_Ll_pilot, IntRealMatrixMap& sum_Llm1_pilot,
      IntRealMatrixMap& sum_Hl_pilot, IntRealMatrixMap& sum_Hlm1_pilot,
      IntRealMatrixMap& sum_Ll_Ll_pilot, IntRealMatrixMap& sum_Ll_Llm1_pilot,
      IntRealMatrixMap& sum_Llm1_Llm1_pilot, IntRealMatrixMap& sum_Hl_Ll_pilot,
      IntRealMatrixMap& sum_Hl_Llm1_pilot, IntRealMatrixMap& sum_Hlm1_Ll_pilot,
      IntRealMatrixMap& sum_Hlm1_Llm1_pilot, IntRealMatrixMap& sum_Hl_Hl_pilot,
      IntRealMatrixMap& sum_Hl_Hlm1_pilot,
      IntRealMatrixMap& sum_Hlm1_Hlm1_pilot, RealVectorArray& eval_ratios,
      RealMatrix& Lambda, RealMatrix& var_YH, Real& eps_sq_div_2,
      SizetArray& delta_N_hf, Sizet2DArray& N_actual, SizetArray& N_alloc,
  ...
  void accumulate_increments(
      const SizetArray& delta_N_hf, Sizet2DArray& N_actual, SizetArray& N_alloc,
      const RealVector& hf_targets, SumContainer1& sum_Ll,
      SumContainer1& sum_Llm1, SumContainer2& sum_Hl, SumContainer2& sum_Hlm1,
      ...
      SumContainer1& sum_Hl_Hlm1, SumContainer1& sum_Hlm1_Hlm1, bool incr_cost);
  ...
  void compute_allocations(
      ...
      bool pilot_estvar, SumContainer1& sum_Ll, SumContainer1& sum_Llm1,
      SumContainer2& sum_Hl, SumContainer2& sum_Hlm1, SumContainer1& sum_Ll_Ll,
  ...
  void compute_eval_ratios(
      ...
      RealMatrix& var_YHl, RealMatrix& rho_dot2_LH, RealVector& eval_ratios);
  ...
  void compute_mlmf_controls(const Real* sum_Ll_m, const Real* sum_Llm1_m,
                             ...
                             const Real* sum_Hl_Ll_m, const Real* sum_Hl_Llm1_m,
                             const Real* sum_Hlm1_Ll_m,
                             const Real* sum_Hlm1_Llm1_m,
                             const Real* sum_Hl_Hl_m, const Real* sum_Hl_Hlm1_m,
  ...
  void initialize_mlmf_sums(
      ...
      IntRealMatrixMap& sum_Hlm1_Llm1, IntRealMatrixMap& sum_Hl_Hl,
      IntRealMatrixMap& sum_Hl_Hlm1, IntRealMatrixMap& sum_Hlm1_Hlm1,
      size_t num_ml_lev, size_t num_cv_lev, size_t num_mom = 4);
  /*
  /// initialize the MLMF accumulators for computing means, variances, and
  /// covariances across fidelity levels
  ...
  */
  ...
  void accumulate_mlmf_Qsums(
      const IntResponseMap& mlmf_resp_map, IntRealMatrixMap& sum_Ll,
...
inline void NonDMultilevControlVarSampling::compute_eval_ratios(
    RealMatrix& sum_Ll, RealMatrix& sum_Llm1, IntRealMatrixMap& sum_Hl,
    IntRealMatrixMap& sum_Hlm1, RealMatrix& sum_Ll_Ll, RealMatrix& sum_Ll_Llm1,
...
inline void NonDMultilevControlVarSampling::variance_Ysum(
    IntRealMatrixMap& sum_Hl, IntRealMatrixMap& sum_Hl_Hl,
...
inline void NonDMultilevControlVarSampling::variance_Ysum(
    IntRealMatrixMap& sum_Hl, RealMatrix& sum_Hl_Hl,
    const Sizet2DArray& N_actual, RealMatrix& var_YH, size_t lev) {
...
template <typename SumContainer1, typename SumContainer2>
void NonDMultilevControlVarSampling::accumulate_increments(
    const SizetArray& delta_N_hf, Sizet2DArray& N_actual_hf,
    SizetArray& N_alloc_hf, const RealVector& hf_targets, SumContainer1& sum_Ll,
    SumContainer1& sum_Llm1, SumContainer2& sum_Hl, SumContainer2& sum_Hlm1,
    SumContainer1& sum_Ll_Ll, SumContainer1& sum_Ll_Llm1,
    SumContainer1& sum_Llm1_Llm1, SumContainer1& sum_Hl_Ll,
    SumContainer1& sum_Hl_Llm1, SumContainer1& sum_Hlm1_Ll,
...
template <typename SumContainer>
void NonDMultilevControlVarSampling::accumulate_lf_increments(
    const SizetArray& delta_N_lf, Sizet2DArray& N_actual_lf,
    SizetArray& N_alloc_lf, const RealVectorArray& lf_targets,
    SumContainer& sum_Ll_refined, SumContainer& sum_Llm1_refined) {
  ...
  for (lev = 0, group = 0; lev < num_cv_lev; ++lev, ++group) {
    ...
    N_alloc_lf[lev] +=
        (backfillFailures)
            ? one_sided_delta(N_alloc_lf[lev], average(lf_targets[lev]))
            : numSamples;
  }
  clear_batches();
}
...
template <typename SumContainer1, typename SumContainer2>
void NonDMultilevControlVarSampling::compute_allocations(
    RealVectorArray& eval_ratios, RealMatrix& Lambda, RealMatrix& var_YH,
    Real& eps_sq_div_2, SizetArray& delta_N_hf, Sizet2DArray& N_actual,
    SizetArray& N_alloc, RealVector& hf_targets, bool accumulate_cost,
    bool pilot_estvar, SumContainer1& sum_Ll, SumContainer1& sum_Llm1,
  ...
  Real sum_sqrt_var_cost = 0., agg_estvar_iter0 = 0., r_lq, budget, lf_lev_cost,
       hf_lev_cost, hf_ref_cost = sequenceCost[numApprox];
  ...
  for (lev = 0; lev < num_hf_lev; ++lev) {
    ...
    numSamples = delta_N_hf[lev];
    Real& agg_var_hf_l = agg_var_hf[lev];  // carried over from prev iter
                                           // if!samp
      ...
      sum_sqrt_var_cost +=
          (budget_constrained)
              ? std::sqrt(agg_var_hf_l / hf_lev_cost * om_rho2) *
                    (hf_lev_cost +
                     (1. + average(eval_ratios[lev])) * lf_lev_cost)
              : std::sqrt(agg_var_hf_l * hf_lev_cost / om_rho2) *
                    avg_lambda[lev];
  ...
  Real fact = (budget_constrained) ? budget / sum_sqrt_var_cost
                                   :                 //        budget constraint
                  sum_sqrt_var_cost / eps_sq_div_2;  // error balance constraint
  for (lev = 0; lev < num_hf_lev; ++lev) {
    hf_lev_cost = level_cost(sequenceCost, lev, num_cv_lev);
    hf_targets[lev] = (lev < num_cv_lev)
...
inline Real NonDMultilevControlVarSampling::compute_mlmf_equivalent_cost(
    const SizetArray& raw_N_hf, const RealVector& hf_cost,
  ...
  for (lev = 1; lev < num_hf_lev;
       ++lev)  // subsequent levels incur 2 model costs
    if (raw_N_hf[lev])
      equiv_hf += raw_N_hf[lev] * (hf_cost[lev] + hf_cost[lev - 1]);
  for (lev = 1; lev < num_cv_lev;
       ++lev)  // subsequent levels incur 2 model costs
    if (raw_N_lf[lev])
...
inline void NonDMultilevControlVarSampling::increment_mlmf_equivalent_cost(
    size_t new_N_hf, Real hf_lev_cost, size_t new_N_lf, Real lf_lev_cost,
    Real hf_ref_cost, Real& equiv_hf) {
  // increment the equivalent number of HF evaluations
  Real incr = 0.;
...
inline void NonDMultilevControlVarSampling::compute_mlmf_estimator_variance(
    const RealMatrix& var_Y, const Sizet2DArray& num_Y,
    const RealMatrix& Lambda, RealVector& mlmf_est_var) {
...
inline void NonDMultilevControlVarSampling::compute_mf_correlation(
    Real sum_L, Real sum_H, Real sum_LL, Real sum_LH, Real sum_HH,
...
inline void NonDMultilevControlVarSampling::compute_mf_controls(
    const Real* sum_L, const Real* sum_H, const Real* sum_LL,
    const Real* sum_LH, const SizetArray& N_shared, RealVector& beta) {
...
inline void NonDMultilevControlVarSampling::apply_mf_control(
    Real sum_H, Real sum_L_shared, size_t N_shared, Real sum_L_refined,
    size_t N_refined, Real beta, Real& H_raw_mom) {
  // apply control for HF uncentered raw moment estimates:
  H_raw_mom +=
      sum_H / N_shared                       // mu_H from shared samples
      - beta * (sum_L_shared / N_shared -    // mu_L from shared samples
                sum_L_refined / N_refined);  // refined_mu_L incl increment
}

inline void NonDMultilevControlVarSampling::apply_mlmf_control(
    Real sum_Hl, Real sum_Hlm1, Real sum_Ll, Real sum_Llm1, size_t N_shared,
    Real sum_Ll_refined, Real sum_Llm1_refined, size_t N_refined, Real beta_dot,
