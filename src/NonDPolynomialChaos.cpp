...
NonDPolynomialChaos::NonDPolynomialChaos(ProblemDescDB& problem_db,
    ...
    : NonDExpansion(problem_db, parallel_lib, model),
      ...
      numAdvance(
          problem_db.get_ushort("method.nond.adapted_basis.advancements")),
...
// resizedFlag(false), callResize(false)
{
  ...
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, pce_set, pce_view, approx_type, exp_orders,
      corr_type, corr_order, data_order, outputLevel, pt_reuse,
      importBuildPointsFile,
...
NonDPolynomialChaos::NonDPolynomialChaos(
    std::shared_ptr<Model> model, short exp_coeffs_approach,
    unsigned short num_int, const RealVector& dim_pref, short u_space_type,
    ...
    : NonDExpansion(POLYNOMIAL_CHAOS, model, model->current_variables().view(),
                    exp_coeffs_approach, dim_pref, 0, refine_type,
...
// resizedFlag(false), callResize(false), initSGLevel(0)
{
  ...
  switch (exp_coeffs_approach) {
    ...
    default:
      Cerr << "Error: Unsupported PCE coefficient estimation approach in "
           << "NonDPolynomialChaos constructor." << std::endl;
      abort_handler(METHOD_ERROR);
      break;
  }
  ...
  String pt_reuse;  // empty for integration approaches
  short corr_order = -1, corr_type = NO_CORRECTION;
  // DFSModel consumes QoI aggregations. Helper mode: support surrogate Hessians
  const ActiveSet& recast_set = g_u_model->current_response().active_set();
  ShortArray pce_asv(g_u_model->qoi(),
                     7);  // TO DO: consider passing in data_mode
  ActiveSet pce_set(pce_asv, recast_set.derivative_vector());
  const ShortShortPair& pce_view = g_u_model->current_variables().view();
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, pce_set, pce_view, approx_type, exp_orders,
...
NonDPolynomialChaos::NonDPolynomialChaos(
    std::shared_ptr<Model> model, short exp_coeffs_approach,
    unsigned short exp_order, const RealVector& dim_pref, size_t colloc_pts,
    Real colloc_ratio, int seed, short u_space_type, short refine_type,
    ...
    bool import_build_active_only, String exp_expansion_file)
    : NonDExpansion(POLYNOMIAL_CHAOS, model, model->current_variables().view(),
                    ...
                    piecewise_basis, use_derivs),
      crossValidation(cv_flag),
      crossValidNoiseOnly(false),
      maxCVOrderCandidates(USHRT_MAX),
      respScaling(false),
      importBuildPointsFile(import_build_pts_file),
      l2Penalty(0.),
      numAdvance(3),
      expOrderSpec(exp_order),
      collocPtsSpec(colloc_pts),
      normalizedCoeffOutput(false),
      uSpaceType(u_space_type),
      expansionExportFile(exp_expansion_file)
// resizedFlag(false), callResize(false)
{
  ...
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, pce_set, pce_view, approx_type, exp_orders,
      corr_type, corr_order, data_order, outputLevel, pt_reuse,
      importBuildPointsFile, import_build_format, import_build_active_only);
  initialize_u_space_model();
...
NonDPolynomialChaos::NonDPolynomialChaos(std::shared_ptr<Model> model,
    ...
    : NonDExpansion(POLYNOMIAL_CHAOS, model, approx_view, -1, RealVector(), 0,
      ...
      uSpaceType(u_space_type),
      expansionImportFile(exp_import_file)
// expansionExportFile(
//   problem_db.get_string("method.nond.export_expansion_file"))
{
  ...
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, pce_set, approx_view, approx_type, exp_orders,
      corr_type, corr_order, data_order, outputLevel, pt_reuse);
...
NonDPolynomialChaos::NonDPolynomialChaos(unsigned short method_name,
...
NonDPolynomialChaos::NonDPolynomialChaos(
    ...
    short exp_coeffs_approach, const RealVector& dim_pref, short u_space_type,
    short refine_type, short refine_control, short covar_control,
...
NonDPolynomialChaos::NonDPolynomialChaos(
    ...
    short exp_coeffs_approach, const RealVector& dim_pref, short u_space_type,
    short refine_type, short refine_control, short covar_control,
    const SizetArray& colloc_pts_seq, Real colloc_ratio, short ml_alloc_control,
    short ml_discrep,
    // short rule_nest, short rule_growth,
    bool piecewise_basis, bool use_derivs, bool cv_flag)
    : NonDExpansion(method_name, model, model->current_variables().view(),
...
bool NonDPolynomialChaos::config_integration(
    unsigned short quad_order, unsigned short ssg_level, unsigned short cub_int,
    std::shared_ptr<Iterator>& u_space_sampler,
  ...
  if (quad_order != USHRT_MAX) {
    ...
    expansionCoeffsApproach = (refineControl) ? Pecos::INCREMENTAL_SPARSE_GRID
...
bool NonDPolynomialChaos::config_expectation(
    size_t exp_samples, unsigned short sample_type, int seed, const String& rng,
    std::shared_ptr<Iterator>& u_space_sampler,
    std::shared_ptr<Model> g_u_model, String& approx_type) {
  if (exp_samples == SZ_MAX) return false;
...
bool NonDPolynomialChaos::config_regression(
    const UShortArray& exp_orders, size_t colloc_pts,
    Real colloc_ratio_terms_order, short regress_type, short ls_regress_type,
    const UShortArray& tensor_grid_order, unsigned short sample_type, int seed,
  ...
  if (regress_type == Pecos::DEFAULT_LEAST_SQ_REGRESSION)
    switch (ls_regress_type) {
      case SVD_LS:
        expansionCoeffsApproach = Pecos::SVD_LEAST_SQ_REGRESSION;
        break;
      case EQ_CON_LS:
        expansionCoeffsApproach = Pecos::EQ_CON_LEAST_SQ_REGRESSION;
        break;
  ...
  switch (expansionCoeffsApproach) {
    case Pecos::ORTHOG_LEAST_INTERPOLATION:
      ...
      break;
    case Pecos::DEFAULT_REGRESSION:
    case Pecos::DEFAULT_LEAST_SQ_REGRESSION:
    case Pecos::SVD_LEAST_SQ_REGRESSION:
    case Pecos::EQ_CON_LEAST_SQ_REGRESSION:
    case Pecos::BASIS_PURSUIT:
    case Pecos::BASIS_PURSUIT_DENOISING:
    case Pecos::ORTHOG_MATCH_PURSUIT:
    case Pecos::LASSO_REGRESSION:
    case Pecos::LEAST_ANGLE_REGRESSION: {
      ...
      if (!expansionBasisType)
        expansionBasisType = (tensorRegression && numContinuousVars <= 5)
                                 ? Pecos::TENSOR_PRODUCT_BASIS
                                 : Pecos::TOTAL_ORDER_BASIS;
      size_t exp_terms;
...
bool NonDPolynomialChaos::resize() {
  ...
  if (expansionCoeffsApproach == Pecos::QUADRATURE ||
    ...
    // orig_dfs_model->import_build_active_only();
    uSpaceModel = std::make_shared<DataFitSurrModel>(
        u_space_sampler, g_u_model, pce_set, pce_view, approx_type, exp_orders,
        corr_type, corr_order, data_order, outputLevel,
        pt_reuse);  // no import after resize
  ...
  unsigned short int_refine(NO_INT_REFINE);
  IntVector refine_samples;
  if (imp_sampler_rep) {
...
void NonDPolynomialChaos::resolve_inputs(short& u_space_type,
                                         short& data_order) {
...
void NonDPolynomialChaos::initialize_u_space_model() {
  ...
  if (expansionCoeffsApproach >= Pecos::DEFAULT_REGRESSION) {
    ...
    Pecos::RegressionConfigOptions rc_options(
        crossValidation, crossValidNoiseOnly, maxCVOrderCandidates, respScaling,
        random_seed(), noiseTols, l2Penalty, false, 0 /*initSGLevel*/, 2,
        numAdvance);
...
void NonDPolynomialChaos::compute_expansion() {
  if (expansionImportFile.empty())
    ...
    uSpaceModel->approximation_coefficients(coeffs_array,
                                            normalizedCoeffOutput);
  }
}

void NonDPolynomialChaos::select_refinement_points(
  ...
  sampler.get_enriched_sample_indices((int)numContinuousVars, current_samples,
                                      (int)batch_size, unique_candidate_samples,
                                      selected_candidate_indices);
  best_samples.shapeUninitialized((int)numContinuousVars, (int)batch_size);
  Pecos::util::extract_submatrix_from_column_indices(
      unique_candidate_samples, selected_candidate_indices, best_samples);
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    ...
    Dakota::write_data(export_file_stream, best_samples, brackets, row_rtn,
                       final_rtn);
    export_file_stream.close();
  }
}

void NonDPolynomialChaos::select_refinement_points_deprecated(
    const RealVectorArray& candidate_samples, unsigned short batch_size,
    RealMatrix& best_samples) {
  // from initial candidate_samples, select the best batch_size points in terms
  // of information content, as determined by pivoted LU factorization
  ...
  int new_size = numSamplesOnModel + batch_size;
  UShortArray exp_order(numContinuousVars, 0);
  UShort2DArray multi_index;
  ...
  for (b = 0, t = numSamplesOnModel; b < batch_size; ++b, ++t) {
    ...
    for (j = 0; j < numContinuousVars; ++j) b_col[j] = selected_rv[j];
  }
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "Select refinement pts: best_samples =\n" << best_samples;
}
...
void NonDPolynomialChaos::ratio_samples_to_order(Real colloc_ratio,
                                                 int num_samples,
  ...
  size_t i, incr = 0,
            data_size = (useDerivs) ? num_samples * (numContinuousVars + 1)
                                    : num_samples;
  size_t exp_terms =
      (expansionBasisType == Pecos::TENSOR_PRODUCT_BASIS)
          ? Pecos::SharedPolyApproxData::tensor_product_terms(exp_order)
  ...
  size_t data_reqd = (size_t)std::floor(
      std::pow((Real)exp_terms, termsOrder) * colloc_ratio + .5);
  while (data_reqd < data_size) {
    // uniform order increment
    for (i = 0; i < numContinuousVars; ++i) ++exp_order[i];
    // terms in total order expansion
    exp_terms =
        (expansionBasisType == Pecos::TENSOR_PRODUCT_BASIS)
            ? Pecos::SharedPolyApproxData::tensor_product_terms(exp_order)
            : Pecos::SharedPolyApproxData::total_order_terms(exp_order);
    data_reqd = (size_t)std::floor(
        std::pow((Real)exp_terms, termsOrder) * colloc_ratio + .5);
    ++incr;
  }
...
void NonDPolynomialChaos::sample_allocation_metric(Real& sparsity_metric,
                                                   Real power) {
  ...
  for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
    ...
    if (outputLevel >= DEBUG_OUTPUT)
      Cout << "Sparsity(" /*lev " << lev << ", "*/ << "qoi "
           << qoi
           /* << ", iter " << iter */
           << ") = " << sparsity_q << '\n';

    if (pow_inf) {
      if (sparsity_q > max) max = sparsity_q;
    } else
...
void NonDPolynomialChaos::post_run(std::ostream& s) {
  ...
  if (!summaryOutputFlag && !expansionExportFile.empty()) export_coefficients();
...
void NonDPolynomialChaos::print_coefficients(std::ostream& s) {
  ...
  for (i = 0; i < numFunctions; i++) {
    if (normalizedCoeffOutput)
      s << "Normalized coefficients of ";
    else
      s << "Coefficients of ";
    ...
    for (j = 0; j < startCAUV; ++j) {
      std::sprintf(tag, "d%i", j + 1);
      s << std::setw(5) << tag;
    }
    for (j = 0; j < numCAUV; ++j) {
      std::sprintf(tag, "u%i", j + 1);
      s << std::setw(5) << tag;
    }
    for (j = 0; j < num_ceuv; ++j) {
      std::sprintf(tag, "e%i", j + 1);
      s << std::setw(5) << tag;
    }
    for (j = 0; j < num_csv; ++j) {
      std::sprintf(tag, "s%i", j + 1);
      s << std::setw(5) << tag;
    }
    s << "\n  " << std::setw(width) << "-----------";
    for (j = 0; j < numContinuousVars; ++j) s << " ----";
...
void NonDPolynomialChaos::archive_coefficients() {
  ...
  for (size_t i = 0; i < numFunctions; i++) {
    ...
    resultsDB.array_insert<RealVector>(run_identifier(),
                                       resultsNames.pce_coeffs, i, coeffs);
