...
class EffGlobalMinimizer : public SurrBasedMinimizer {
 public:
  ...
  EffGlobalMinimizer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
                     std::shared_ptr<Model> model);
  /// alternate constructor for instantiations "on the fly"
  EffGlobalMinimizer(std::shared_ptr<Model> model, const String& approx_type,
                     int samples, int seed, bool use_derivs, size_t max_iter,
...
inline Real EffGlobalMinimizer::augmented_lagrangian(const RealVector& mean) {
  return augmented_lagrangian_merit(
      mean, iteratedModel->primary_response_fn_sense(),
...
inline void EffGlobalMinimizer::update_constraints(
...
inline void EffGlobalMinimizer::update_convergence_counters(
    const Variables& vars_star, const Response& resp_star) {
...
inline int EffGlobalMinimizer::extract_id(IntVarsMCIter cit,
                                          const IntVariablesMap& map) {
...
inline void EffGlobalMinimizer::debug_print_values(const Variables& vars) {
  ...
  for (size_t i = 0; i < numFunctions; i++) stdv[i] = std::sqrt(variance[i]);
  Cout << "\nexpected values    =\n"
       << mean << "\nstandard deviation =\n"
       << stdv << "\nexpected violation =\n"
       << ev << std::endl;
...
inline void EffGlobalMinimizer::debug_print_dist_counters(Real dist_cv_star) {
  Cout << "EGO Iteration " << globalIterCount
       << ": dist_cv_star = " << dist_cv_star
       << " distance convergence cntr = " << distConvergenceCntr << '\n';
}
