...
SharedSurfpackApproxData::SharedSurfpackApproxData(ProblemDescDB& problem_db,
                                                   size_t num_vars)
  ...
  if (approxType == "global_polynomial")
    ...
    if (trend_string == "constant")
      ...
      approxOrder = 2;  // empty, reduced_quadratic, quadratic
  }
}

/// On-the-fly constructor which uses mostly Surfpack model defaults
SharedSurfpackApproxData::SharedSurfpackApproxData(
...
void SharedSurfpackApproxData::add_sd_to_surfdata(
    const Pecos::SurrogateDataVars& sdv, const Pecos::SurrogateDataResp& sdr,
...
void SharedSurfpackApproxData::copy_matrix(
    const RealSymMatrix& rsm, SurfpackMatrix<Real>& surfpack_matrix) {
  ...
  for (size_t i = 0; i < nr; ++i)
    for (size_t j = 0; j < nc; ++j) surfpack_matrix(i, j) = rsm(i, j);
}

StringArray SharedSurfpackApproxData::variable_labels(
...
void SharedSurfpackApproxData::validate_metrics(
  ...
  if (err_found) abort_handler(PARSE_ERROR);
}

void SharedSurfpackApproxData::map_variable_labels(
    const Variables& dfsm_vars, const StringArray& approx_labels) {
