...
String method_hdf5_link_name(const StrStrSizet &iterator_id) {
  ...
  // throwing error
  if (method_id.empty()) method_id = "anonymous";
...
String execution_hdf5_link_name(const StrStrSizet &iterator_id) {
  ...
  String rval = method_results_hdf5_link_name(iterator_id) +
                "/execution:" + std::to_string(exec_num);
  return rval;
}
...
String object_hdf5_link_name(const StrStrSizet &iterator_id,
  ...
  for (const String &s : location) rval += "/" + s;
  return rval;
}
...
void ResultsDBHDF5::allocate_vector(const StrStrSizet &iterator_id,
  ...
  if (!method_exists) add_name_to_method(iterator_id);
}
...
void ResultsDBHDF5::insert_into(const StrStrSizet &iterator_id,
                                const StringArray &location,
                                const boost::any &data, const int &index,
                                const bool &row) {
  // Store the results
  String dset_name = object_hdf5_link_name(iterator_id, location);
  // Need to fix this to use incoming "data"
  if (data.type() == typeid(std::vector<double>)) {
    hdf5Stream->set_vector(
        dset_name, boost::any_cast<std::vector<double> >(data), index, row);
  } else if (data.type() == typeid(RealVector)) {
    hdf5Stream->set_vector(dset_name, boost::any_cast<RealVector>(data), index,
                           row);
  } else if (data.type() == typeid(IntVector)) {
    hdf5Stream->set_vector(dset_name, boost::any_cast<IntVector>(data), index,
                           row);
  } else if (data.type() == typeid(StringMultiArrayConstView)) {
    hdf5Stream->set_vector(dset_name,
                           boost::any_cast<StringMultiArrayConstView>(data),
                           index, row);
    ...
    Cerr << "Warning: dset " << dset_name
         << " of unknown type of any: " << data.type().name() << std::endl;
    abort_handler(-1);
  }
}
...
void ResultsDBHDF5::insert(const StrStrSizet &iterator_id,
                           const StringArray &location, const boost::any &data,
                           const DimScaleMap &scales,
                           const AttributeArray &attrs, const bool &transpose) {
  bool method_exists = method_in_cache(iterator_id);
  // Store the results
  String dset_name = object_hdf5_link_name(iterator_id, location);
  // Need to fix this to use incoming "data"
  if (data.type() == typeid(std::vector<double>)) {
    hdf5Stream->store_vector(dset_name,
                             boost::any_cast<std::vector<double> >(data));
  } else if (data.type() == typeid(RealVector)) {
    hdf5Stream->store_vector(dset_name, boost::any_cast<RealVector>(data));
  } else if (data.type() == typeid(IntVector)) {
    hdf5Stream->store_vector(dset_name, boost::any_cast<IntVector>(data));
  } else if (data.type() == typeid(StringMultiArrayConstView)) {
    hdf5Stream->store_vector(dset_name,
                             boost::any_cast<StringMultiArrayConstView>(data));
  } else if (data.type() == typeid(RealMatrix)) {
    hdf5Stream->store_matrix(dset_name, boost::any_cast<RealMatrix>(data),
                             transpose);
  } else if (data.type() == typeid(Real)) {
  ...
  //  hdf5Stream->store_vector(dset_name,
  //  boost::any_cast<std::vector<std::string> >(data));
  //}
  // else if (data.type() == typeid(std::vector<RealVector>)) {
  //  hdf5Stream->store_vector(dset_name,
  //  boost::any_cast<std::vector<RealVector> >(data));
  //}
  // else if (data.type() == typeid(std::vector<RealMatrix>)) {
  //  hdf5Stream->store_vector(dset_name,
  //  boost::any_cast<std::vector<RealMatrix> >(data));
  //}
  else {
    Cerr << "Warning: dset " << dset_name
         << " of unknown type of any: " << data.type().name() << std::endl;
...
void ResultsDBHDF5::attach_scales(const String &dset_name,
                                  const StrStrSizet &iterator_id,
                                  const StringArray &location,
                                  const DimScaleMap &scales) {
  // Store and attach the dimension scales.
  // Iteration must be explicit for the dimension scales because they are stored
  // in a multimap, which is a container of pairs, not of boost::variants
  for (auto &s : scales) {  // s is a std::pair<int, boost::variant<StringScale,
                            // RealScale> >
    int dimension = s.first;
    AttachScaleVisitor visitor(iterator_id, location, dimension, dset_name,
                               hdf5Stream);
    boost::apply_visitor(visitor, s.second);
  }
}

void ResultsDBHDF5::add_attributes(const String &linkname,
                                   const AttributeArray &attrs) {
  // Add metadata to the dataset
  AddAttributeVisitor attribute_adder(linkname, hdf5Stream);
  std::for_each(attrs.begin(), attrs.end(),
                boost::apply_visitor(attribute_adder));
}

void ResultsDBHDF5::flush() const { hdf5Stream->flush(); }
