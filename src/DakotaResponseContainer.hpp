...
class ModelGroup {
 public:
  ...
  ModelGroup(const std::vector<unsigned short>& model_indices, int num_qois,
  ...
  int numSamples(int model = 0, int qoi = 0, int moment = 1,
                 int replication = 0);
  ...
  Real getCovariance(int model, int other_model, int qoi, int moment,
                     int replication);
 ...
 private:
  std::vector<unsigned short>
      model_indices_;     // Indices of models in this group.
  ...
  std::vector<std::vector<std::vector<std::vector<MeanAccumulator<Real>>>>>
      mean_accumulators_;  // [moment][model][quantity_of_interest][replication]

  // Accumulators for covariance statistics.
  std::vector<std::vector<
      std::vector<std::vector<std::vector<CovarianceAccumulator<Real>>>>>>
      covariance_accumulators_;  // [moment][model][other_model][quantity_of_interest][replication]
};
...
class ResponseContainer {
 public:
  ...
  //   num_replications: The number of replications for the statistics (default
  //   is 1).
  void addModelGroup(const std::vector<unsigned short>& model_indices,
                     int num_qois = 1, int num_moments = 1,
  ...
  // Gets the mean for a specific group, model, quantity of interest, moment,
  // and replication.
  //
  // Args:
  //   group: The index of the model group.
  ...
  // Returns:
  //   The computed mean for the specified parameters.
  Real getMean(int group, int model, int qoi = 0, int moment = 1,
               int replication = 0);
  ...
  // Returns:
  //   The computed covariance for the specified parameters.
  Real getCovariance(int group, int model, int other_model, int qoi = 0,
                     int moment = 1, int replication = 0);

  // Empties the response container of all stored samples.
  void reset();

 private:
  std::vector<std::unique_ptr<ModelGroup>>
      model_groups_;  // Unique pointers to model groups.
};
