...
EnsembleSurrModel::EnsembleSurrModel(ProblemDescDB& problem_db,
    ...
    : SurrogateModel(problem_db, parallel_lib),
      ...
      sameInterfaceInstance(false),
      ensemblePrecedence(DEFAULT_PRECEDENCE),
      modeKeyBufferSize(0),
  ...
  if (truth_model_spec)
    ...
    problem_db.set_db_model_nodes(ensemble_model_ptrs[num_approx]);
  ...
  for (i = 0; i < num_approx; ++i) approxModels[i]->serialize_threshold(0);
...
void EnsembleSurrModel::assign_default_keys(short mode) {
  ...
  switch (mode) {
    ...
    case AGGREGATED_MODEL_PAIR:  // first and last model form (no soln levels)
      if (multilevel_multifidelity()) {  // first and last model form / soln
                                         // levels
      ...
      } else if (multifidelity()) {  // first and last model form (no soln
                                     // levels)
        truthModelKey =
            Pecos::ActiveKey(id, Pecos::RAW_DATA, num_approx,
                             truthModel->solution_level_cost_index());
        surrModelKeys.resize(1);
        // Note: for modeKeyBufferSize estimation, must maintain consistency
        ...
        surrModelKeys[0] =
            Pecos::ActiveKey(id, Pecos::RAW_DATA, 0,
    ...
    case BYPASS_SURROGATE:
      truthModelKey =
          Pecos::ActiveKey(id, Pecos::RAW_DATA, num_approx, truth_soln_lev - 1);
      surrModelKeys.clear();
      break;
...
IntIntPair EnsembleSurrModel::estimate_partition_bounds(
...
void EnsembleSurrModel::derived_init_communicators(ParLevLIter pl_iter,
                                                   int max_eval_concurrency,
                                                   bool recurse_flag) {
  ...
  if (recurse_flag) {
    ...
    bool extra_deriv_config =
        (method_name & MINIMIZER_BIT || method_name == BAYES_CALIBRATION ||
         method_name == LOCAL_RELIABILITY ||
         method_name == LOCAL_INTERVAL_EST || method_name == LOCAL_EVIDENCE);
...
void EnsembleSurrModel::derived_set_communicators(ParLevLIter pl_iter,
                                                  int max_eval_concurrency,
                                                  bool recurse_flag) {
  miPLIndex =
      modelPCIter->mi_parallel_level_index(pl_iter);  // run time setting
  ...
  if (recurse_flag) {
    ...
    */
    ...
    switch (responseMode) {
      ...
      case BYPASS_SURROGATE: {
        auto hf_model = active_truth_model();
      ...
      default: {  // AUTO_CORRECTED_SURROGATE, MODEL_DISCREPANCY,
        ...
        bool use_deriv_conc =
            (responseMode == AUTO_CORRECTED_SURROGATE && corrType && corrOrder);
        ...
        for (i = 0; i < num_models; ++i) {
          auto model_i = model_from_index(i);
          if (use_deriv_conc) {
            int deriv_conc_i = model_i->derivative_concurrency();
            model_i->set_communicators(pl_iter, deriv_conc_i);
            if (deriv_conc_i > 1 && model_i->asynch_flag())
              asynchEvalFlag = true;
          } else {
            model_i->set_communicators(pl_iter, max_eval_concurrency);
            if (model_i->asynch_flag()) asynchEvalFlag = true;
...
void EnsembleSurrModel::derived_free_communicators(ParLevLIter pl_iter,
                                                   int max_eval_concurrency,
                                                   bool recurse_flag) {
  if (recurse_flag) {
    size_t i, num_models = approxModels.size();
...
bool EnsembleSurrModel::finalize_mapping() {
  size_t i, num_approx = approxModels.size();
  for (i = 0; i < num_approx; ++i) approxModels[i]->finalize_mapping();
  ...
  */
  ...
  for (i = 0; i < num_map; ++i) {
    ...
    if (ac_index1 != _NPOS) {
      ...
      m_index = find_index(ModelUtils::all_continuous_variable_labels(model),
                           surr_label);
      // push value from currentVariables to sub-ordinate variables
      if (m_index != _NPOS)
        ModelUtils::all_continuous_variable(
            model, currentVariables.all_continuous_variables()[ac_index1],
            m_index);
    } else if (adi_index1 != _NPOS) {
      const String& surr_label =
          currentVariables.all_discrete_int_variable_labels()[adi_index1];
      m_index = find_index(ModelUtils::all_discrete_int_variable_labels(model),
                           surr_label);
      if (m_index != _NPOS)
        ModelUtils::all_discrete_int_variable(
            model, currentVariables.all_discrete_int_variables()[adi_index1],
            m_index);
    } else if (ads_index1 != _NPOS) {
      const String& surr_label =
          currentVariables.all_discrete_string_variable_labels()[ads_index1];
      m_index = find_index(
          ModelUtils::all_discrete_string_variable_labels(model), surr_label);
      if (m_index != _NPOS)
        ModelUtils::all_discrete_string_variable(
            model, currentVariables.all_discrete_string_variables()[ads_index1],
            m_index);
      ...
      m_index = find_index(ModelUtils::all_discrete_real_variable_labels(model),
                           surr_label);
      if (m_index != _NPOS)
        ModelUtils::all_discrete_real_variable(
            model, currentVariables.all_discrete_real_variables()[adr_index1],
            m_index);
    } else {
...
void EnsembleSurrModel::derived_evaluate(const ActiveSet& set) {
  ...
  switch (responseMode) {
    ...
    case AGGREGATED_MODELS: {
      ...
      for (i = 0; i < num_steps; ++i) {
        ...
        if (test_asv(asv_i)) {
          ...
          auto model_i = model_from_index(m_index);
    ...
    case BYPASS_SURROGATE: {
      ...
      auto hf_model = model_from_index(m_index);
    ...
    default: {  // paired cases: manage LF/HF evaluation requirements
      ShortArray lo_fi_asv, hi_fi_asv;
      bool lo_fi_eval, hi_fi_eval, mixed_eval;
      ...
      switch (responseMode) {
        ...
        case MODEL_DISCREPANCY:
          lo_fi_eval = hi_fi_eval = mixed_eval = true;
          break;
        default:  // {UN,AUTO_}CORRECTED_SURROGATE, AGGREGATED_MODEL_PAIR
          ...
          mixed_eval = (lo_fi_eval && hi_fi_eval);
          break;
      }
      ...
      if (hierarchicalTagging) {
        String eval_tag =
            evalTagPrefix + '.' + std::to_string(surrModelEvalCntr + 1);
      ...
      // > when LF & HF are the same model, then setting the index for low or
      // high
      //   invalidates the other fidelity definition.
      // > within a single derived_evaluate(), could protect these updates with
      //   "if (sameModelInstance && mixed_eval)", but this does not guard
      //   against changes in eval requirements from the previous evaluation.
      //   Detecting the current solution index state is currently as expensive
      //   as resetting it, so just reset each time.
          ...
          case MODEL_DISCREPANCY:
            ...
            hi_fi_response = (sameModelInstance) ?  // shared rep
                                 hf_model->current_response().copy()
                                                 : hf_model->current_response();
            break;
          default: {  // {UN,AUTO_}CORRECTED_SURROGATE, AGGREGATED_MODEL_PAIR
            ActiveSet hi_fi_set(hi_fi_asv, set.derivative_vector());
            hf_model->evaluate(hi_fi_set);
            if (mixed_eval)
              hi_fi_response = (sameModelInstance)
                                   ?  // deep copy or shared rep
                                   hf_model->current_response().copy()
                                   : hf_model->current_response();
            else {
      ...
      if (lo_fi_eval) {
        ...
        switch (responseMode) {
          case AUTO_CORRECTED_SURROGATE:
            ...
            if (!approxBuilds || force_rebuild()) build_approximation();
            break;
        }
        ...
        switch (responseMode) {
          case AUTO_CORRECTED_SURROGATE: {
            // LF resp should not be corrected directly (see
            // derived_synchronize())
      ...
      switch (responseMode) {
        case MODEL_DISCREPANCY: {
          ...
          // just update currentResponse (managed as surrogate data at higher
          // level)
...
void EnsembleSurrModel::derived_evaluate_nowait(const ActiveSet& set) {
  ...
  switch (responseMode) {
    ...
    default: {  // paired cases: manage LF/HF evaluation requirements
      ShortArray lo_fi_asv, hi_fi_asv;
      bool lo_fi_eval, hi_fi_eval;
      switch (responseMode) {
        case MODEL_DISCREPANCY:
          lo_fi_eval = hi_fi_eval = true;
          break;
        default:  // {UN,AUTO_}CORRECTED_SURROGATE, AGGREGATED_MODEL_PAIR
          asv_split(set.request_vector(), lo_fi_asv, hi_fi_asv);
          lo_fi_eval = !lo_fi_asv.empty();
      ...
      // asynchEvalFlag is set if _either_ LF or HF is asynchronous, resulting
      // in use of derived_evaluate_nowait().  To manage general case of mixed
      // asynch, launch nonblocking evals first, followed by blocking evals.

      // For notes on repetitive setting of model.solution_level_cost_index(),
      // see derived_evaluate() above.

      // launch nonblocking evals before any blocking ones
      if (hi_fi_eval &&
          asynch_hi_fi) {  // HF model may be executed asynchronously
      ...
      if (lo_fi_eval &&
          asynch_lo_fi) {  // LF model may be executed asynchronously
        // don't need to set component parallel mode since only queues the job
        assign_surrogate_key(0);  // only 1 surrogate key when paired
      ...
      if (hi_fi_eval &&
          !asynch_hi_fi) {  // execute HF synchronously & cache resp
        m_index = truthModelKey.retrieve_model_form();
        component_parallel_mode(m_index + 1);  // index to id (0 is reserved)
        assign_truth_key();
        hf_model->evaluate(hi_fi_set);
        // not part of rekey_synch(); can rekey to surrModelEvalCntr immediately
        cachedRespMaps[1][surrModelEvalCntr] =
            hf_model->current_response().copy();
      }
      if (lo_fi_eval &&
          !asynch_lo_fi) {  // execute LF synchronously & cache resp
        m_index = surrModelKeys[0].retrieve_model_form();
        component_parallel_mode(m_index + 1);  // index to id (0 is reserved)
        ...
        // not part of rekey_synch(); can rekey to surrModelEvalCntr
        // immediately.
        cachedRespMaps[0][surrModelEvalCntr] =
            lo_fi_response;  // deep copied above
      }
      break;
    }
  }
...
void EnsembleSurrModel::derived_synchronize_sequential(
  ...
  if (sameModelInstance) {
    ...
    for (i = 0; i < num_steps; ++i) {
      ...
      if (!model_id_map_i.empty()) {  // synchronize evals for i-th Model
        ...
        rekey_synch(model_i, block, model_id_map_i, model_resp_maps_rekey[i],
                    deep_copy);
      }
    }
...
void EnsembleSurrModel::derived_synchronize_combine(
    IntResponseMapArray& model_resp_maps, IntResponseMap& combined_resp_map) {
  // --------------------------------------------
  ...
  switch (responseMode) {
    case AGGREGATED_MODELS: {
      ...
      size_t i, num_steps = model_resp_maps.size();
      IntRespMCIter r_cit;
      for (i = 0; i < num_steps; ++i) {
    ...
    case BYPASS_SURROGATE:
      ...
      // std::swap(combined_resp_map, model_resp_maps[0]); // can't swap w/
      // const
      break;
      ...
      switch (responseMode) {
        ...
        default: {  // {UNCORRECTED,AUTO_CORRECTED,BYPASS}_SURROGATE modes
          ...
          while (hf_cit != hf_resp_map.end() || lf_it != lf_resp_map.end()) {
            ...
            int hf_eval_id =
                (hf_cit == hf_resp_map.end()) ? INT_MAX : hf_cit->first;
            int lf_eval_id =
                (lf_it == lf_resp_map.end()) ? INT_MAX : lf_it->first;
...
void EnsembleSurrModel::derived_synchronize_combine_nowait(
    IntResponseMapArray& model_resp_maps, IntResponseMap& combined_resp_map) {
  // ------------------------------------------------
  ...
  switch (responseMode) {
    ...
    default: {  // paired cases
      ...
      if (lf_resp_map.empty() &&
          lf_id_map.empty()) {            // none completed or pending
        combined_resp_map = hf_resp_map;  // can't swap w/ const
        return;
      }
      if (responseMode == AUTO_CORRECTED_SURROGATE)
        compute_apply_delta(lf_resp_map);
      if (hf_resp_map.empty() &&
          hf_id_map.empty()) {  // none completed or pending
        std::swap(combined_resp_map, lf_resp_map);
        return;
      }
      ...
      if (responseMode == MODEL_DISCREPANCY) dc_it = deltaCorr.find(activeKey);
      while (hf_cit != hf_resp_map.end() || lf_it != lf_resp_map.end()) {
        // these have been rekeyed already to top-level surrModelEvalCntr:
        int hf_eval_id =
            (hf_cit == hf_resp_map.end()) ? INT_MAX : hf_cit->first;
          ...
          switch (responseMode) {
            ...
            default:  // {UNCORRECTED,AUTO_CORRECTED,BYPASS}_SURROGATE modes
              if (remain_surr_ids.find(hf_eval_id) != remain_surr_ids.end())
                // LF contribution is pending -> cache HF response
                hf_cached_map[hf_eval_id] = hf_cit->second;
              else  // no LF component is pending -> HF contribution is
                    // sufficient
                response_combine(hf_cit->second, empty_resp,
          ...
          switch (responseMode) {
            ...
            default:  // {UNCORRECTED,AUTO_CORRECTED,BYPASS}_SURROGATE modes
              if (remain_truth_ids.find(lf_eval_id) != remain_truth_ids.end())
                // HF contribution is pending -> cache LF response
                lf_cached_map[lf_eval_id] = lf_it->second;
              else  // no HF component is pending -> LF contribution is
                    // sufficient
                response_combine(empty_resp, lf_it->second,
...
std::shared_ptr<Model> EnsembleSurrModel::model_from_index(
    unsigned short m_index) {
  size_t num_approx = approxModels.size();
  if (m_index < num_approx)
    return approxModels[m_index];
  else if (m_index == num_approx)
    return truthModel;
  else {  // includes _NPOS
...
std::shared_ptr<const Model> EnsembleSurrModel::model_from_index(
    unsigned short m_index) const {
  size_t num_approx = approxModels.size();
  if (m_index < num_approx)
    return approxModels[m_index];
  else if (m_index == num_approx)
...
std::shared_ptr<Model> EnsembleSurrModel::approx_model_from_index(
    unsigned short m_index) {
  size_t num_approx = approxModels.size();
  if (m_index < num_approx)
    return approxModels[m_index];
...
std::shared_ptr<const Model> EnsembleSurrModel::approx_model_from_index(
    unsigned short m_index) const {
  size_t num_approx = approxModels.size();
  if (m_index < num_approx)
    return approxModels[m_index];
...
void EnsembleSurrModel::asv_split(const ShortArray& orig_asv,
  ...
  switch (responseMode) {
    case AGGREGATED_MODEL_PAIR: {
      ...
      approx_asv.resize(num_qoi);
      actual_asv.resize(num_qoi);
      // aggregated response uses {HF,LF} order:
      for (i = 0; i < num_qoi; ++i) approx_asv[i] = orig_asv[i];
      for (i = 0; i < num_qoi; ++i) actual_asv[i] = orig_asv[i + num_qoi];
      break;
    }
...
void EnsembleSurrModel::asv_split(const ShortArray& aggregate_asv,
                                  Short2DArray& indiv_asv) {
  ...
  for (i = 0; i < num_indiv; ++i) {
    ...
    for (j = 0; j < num_qoi; ++j, ++cntr) asv_i[j] = aggregate_asv[cntr];
  }
}
  ...
  if (sameModelInstance) {  // all subordinate keys involve same model form
    switch (responseMode) {
      ...
      case AUTO_CORRECTED_SURROGATE:
        assign_surrogate_key(0);
        break;
        // case AGGREGATED_MODELS: break; // defer setting active solution
        // levels
    }
...
std::shared_ptr<Model> EnsembleSurrModel::active_surrogate_model(size_t i) {
  ...
  switch (responseMode) {
    case AGGREGATED_MODELS:  // array of surrogates: require valid index
      ...
      return model_from_index(lf_form);
      break;
    case BYPASS_SURROGATE:
    case NO_SURROGATE:
      return nullptr;
      break;
    case AGGREGATED_MODEL_PAIR:
    case MODEL_DISCREPANCY:  // paired cases
    case UNCORRECTED_SURROGATE:
    case AUTO_CORRECTED_SURROGATE:
...
std::shared_ptr<const Model> EnsembleSurrModel::active_surrogate_model(
    size_t i) const {
  unsigned short lf_form;
  switch (responseMode) {
    case AGGREGATED_MODELS:  // array of surrModelKeys: require valid index
      ...
      break;
    case BYPASS_SURROGATE:
    case NO_SURROGATE:  // surrModelKeys are empty
      return nullptr;
      break;
    case AGGREGATED_MODEL_PAIR:
    case MODEL_DISCREPANCY:  // 1 surrModelKey
    case UNCORRECTED_SURROGATE:
    case AUTO_CORRECTED_SURROGATE:  // (paired cases)
      // One surrModelKey: allow client to quietly rely on default (_NPOS)
      lf_form = (i == _NPOS) ? active_surrogate_model_form(0)
                             : active_surrogate_model_form(i);
      return  //(lf_form == USHRT_MAX) ? model_from_index(0) :
...
void EnsembleSurrModel::create_tabular_datastream() {
  ...
  switch (responseMode) {
    case AGGREGATED_MODELS: {
      ...
      if (truthModel) {
        ++num_m;
        num_m_l += truthModel->solution_levels();
      }
      // -------------
      // Interface ids
      // -------------
      // To flatten into one composite tabular format, we must rely on invariant
      // quantities rather than run-time flags like
      // same{Model,Interface}Instance
      StringArray iface_ids;
      bool one_iface_id = matching_all_interface_ids();
      if (one_iface_id)  // invariant (sameInterfaceInstance can vary at run
                         // time)
      ...
      // soln control variable label for each model that has one.  Since
      ...
      // a reference state variable ordering. > Note that reading data back in
      // from this composed tabular format is
      ...
      else {  // output leading and trailing vars (spec order), skip soln
              // control
      ...
      if (num_curr_labels == num_m_l * num_qoi)
        labels = curr_labels;  // copy
      else
        inflate(curr_labels, num_m_l, labels);
      ...
      for (m = 0; m < num_m; ++m) {
        ...
        for (l = 0; l < num_l; ++l) {
          ...
          for (q = 0; q < num_qoi; ++q, ++cntr) labels[cntr].append(m_l_append);
        }
      }
    ...
    case AGGREGATED_MODEL_PAIR:
    case MODEL_DISCREPANCY:   // two models/resolutions
    case BYPASS_SURROGATE: {  // use same row len since commonly alternated
      ...
      if (responseMode == AGGREGATED_MODEL_PAIR) {
        if (num_curr_labels == 2 * num_qoi)
          labels = curr_labels;
        else
          inflate(curr_labels, 2, labels);
      ...
      if (responseMode == MODEL_DISCREPANCY)
        for (q = 0; q < num_qoi; ++q) labels[q].insert(0, "Del_");
      ...
      // (violates encapsulation) or detection of the changing
      // models/resolutions (not known until run time)
...
void EnsembleSurrModel::derived_auto_graphics(const Variables& vars,
                                              const Response& resp) {
  ...
  switch (responseMode) {
    case AGGREGATED_MODELS: {  // use same #Cols since commonly alternated
      ...
      if (asynchEvalFlag)
        export_vars.active_variables(vars);  // reqd for parallel
      if (scv_index == _NPOS)
    ...
    case BYPASS_SURROGATE: {  // use same #Cols since commonly alternated
      auto lf_model = active_surrogate_model(0);  // rtns dummy_model for BYPASS
      auto hf_model = active_truth_model();
      ...
      if (one_iface_id)  // invariant (sameInterfaceInstance can vary at run
        ...
        if (truth_key)
          ...
          iface_ids.push_back("N/A");  // preserve row len
        if (surr_key)
          iface_ids.push_back(lf_model->interface_id());
        else
          iface_ids.push_back("N/A");  // preserve row len
      }
      output_mgr.add_tabular_data(iface_ids);  // includes graphics cntr
      ...
      if (solnCntlAVIndex == _NPOS)
        ...
        // but data count is)
        if (sameModelInstance && truth_key &&
            surr_key) {  // data count is invariant
          // HF soln cntl was overwritten by LF and must be temporarily restored
          assign_truth_key();
          add_tabular_solution_level_value(*hf_model);
          assign_surrogate_key(0);
          add_tabular_solution_level_value(*lf_model);
        } else {  // HF and LF soln levels are not overlapping
          if (truth_key)
            add_tabular_solution_level_value(*hf_model);
          else
            output_mgr.add_tabular_scalar(
                "N/A");  // preserve consistent row len
          if (surr_key)
            add_tabular_solution_level_value(*lf_model);
          else
            output_mgr.add_tabular_scalar(
        ...
        size_t start = solnCntlAVIndex + 1;
        output_mgr.add_tabular_data(export_vars, start,
                                    export_vars.tv() - start);
      }
...
void EnsembleSurrModel::resize_response(bool use_virtual_counts) {
  size_t num_meta,
      ...
      num_truth_fns = (use_virtual_counts)
                          ? truthModel->qoi()
                          :  // allow models to consume lower-level aggregations
                          ModelUtils::response_size(
                              *truthModel);  // raw counts align w/
                                             // currentResponse raw count

  switch (responseMode) {
    case AGGREGATED_MODELS: {
      size_t i, num_surr = surrModelKeys.size();
      numFns = num_truth_fns;
      num_meta = num_truth_md;
      for (i = 0; i < num_surr; ++i) {
        unsigned short form = surrModelKeys[i].retrieve_model_form();
        Model& model_i = *model_from_index(form);
        numFns += (use_virtual_counts)
                      ? model_i.qoi()
                      : model_i.current_response().num_functions();
    ...
    default:
      ...
      size_t num_lf_meta = lf_model.current_response().metadata().size(),
             num_lf_fns = (use_virtual_counts)
                              ? lf_model.qoi()
                              : ModelUtils::response_size(lf_model);
      switch (responseMode) {
        case AGGREGATED_MODEL_PAIR:
          numFns = num_truth_fns + num_lf_fns;
          num_meta = num_truth_md + num_lf_meta;
          break;
        case MODEL_DISCREPANCY:
          if (num_lf_fns != num_truth_fns) {
            Cerr << "Error: mismatch in response sizes for MODEL_DISCREPANCY "
                    "mode "
...
void EnsembleSurrModel::component_parallel_mode(short model_id) {
  ...
  if (componentParallelMode !=
      model_id) {  //||componentParallelKey != activeKey)
...
void EnsembleSurrModel::serve_run(ParLevLIter pl_iter,
                                  int max_eval_concurrency) {
...
void EnsembleSurrModel::add_tabular_solution_level_value(Model& model) {
  ...
  switch (model.solution_control_variable_type()) {
    ...
    case DISCRETE_STATE_RANGE:
    case DISCRETE_STATE_SET_INT:
      output_mgr.add_tabular_scalar(model.solution_level_int_value());
      break;
    case DISCRETE_DESIGN_SET_STRING:
    case DISCRETE_UNCERTAIN_SET_STRING:
    case DISCRETE_STATE_SET_STRING:
      output_mgr.add_tabular_scalar(model.solution_level_string_value());
      break;
    case DISCRETE_DESIGN_SET_REAL:
    case DISCRETE_UNCERTAIN_SET_REAL:
    case DISCRETE_STATE_SET_REAL:
      output_mgr.add_tabular_scalar(model.solution_level_real_value());
      break;
  }
}
...
void EnsembleSurrModel::build_approximation() {
  ...
  if (it == truthResponseRef.end())
    it = truthResponseRef
             .insert(std::pair<Pecos::ActiveKey, Response>(
                 truthModelKey, currentResponse.copy()))
             .first;
...
void EnsembleSurrModel::single_apply(const Variables& vars, Response& resp,
  ...
  if (!delta_corr.computed()) {
    ...
    extract_truth_key(paired_key, truth_key);
    std::map<Pecos::ActiveKey, Response>::iterator it =
        truthResponseRef.find(truth_key);
    if (it == truthResponseRef.end())
      apply_corr = false;  // not found
    else
      delta_corr.compute(vars, it->second, resp, quiet_flag);
  }
  if (apply_corr) delta_corr.apply(vars, resp, quiet_flag);
}
  ...
  switch (correctionMode) {
    ...
    case FULL_SOLUTION_LEVEL_CORRECTION: {
      ...
      for (i = lf_lev; i < num_levels - 1; ++i) {
        ...
        paired_key.assign_resolution_level(i + 1,
                                           1);  //   fine res in KeyData[1]
        single_apply(vars, resp, paired_key);
      }
      break;
    }
