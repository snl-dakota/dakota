...
bool register_dakota_cast() {
  TypeManager()->register_lexical_cast(typeid(RealVector),
                                       typeid(vector<double>),
                                       &cast_from_realvector_to_vector);
  TypeManager()->register_lexical_cast(typeid(vector<double>),
                                       typeid(RealVector),
                                       &cast_from_vector_to_realvector);
  TypeManager()->register_lexical_cast(typeid(IntVector), typeid(vector<int>),
                                       &cast_from_intvector_to_vector);
  TypeManager()->register_lexical_cast(typeid(vector<int>), typeid(IntVector),
                                       &cast_from_vector_to_intvector);
  TypeManager()->register_lexical_cast(typeid(int), typeid(unsigned int),
                                       &cast_from_int_to_unsignedint);
  TypeManager()->register_lexical_cast(typeid(int), typeid(unsigned long long),
                                       &cast_from_int_to_unsignedlonglong);
  TypeManager()->register_lexical_cast(typeid(unsigned int), typeid(int),
                                       &cast_from_unsignedint_to_int);
  TypeManager()->register_lexical_cast(typeid(unsigned long long), typeid(int),
                                       &cast_from_unsignedlonglong_to_int);
  TypeManager()->register_lexical_cast(typeid(char const*), typeid(string),
...
COLINOptimizer::COLINOptimizer(ProblemDescDB& problem_db,
...
COLINOptimizer::COLINOptimizer(const String& method_string,
                               std::shared_ptr<Model> model, int seed,
...
void COLINOptimizer::core_run() {
  ...
  try {
    ...
    colinEvalMgr = colin::EvalManagerFactory().create(
        iteratedModel->asynch_flag() ? "Concurrent" : "Serial");
    ...
    const RealVector& cv_init =
        ModelUtils::continuous_variables(*iteratedModel);
    const IntVector& div_init =
        ModelUtils::discrete_int_variables(*iteratedModel);
    const RealVector& drv_init =
    ...
    MixedIntVars miv_init(
        0, numDiscreteIntVars + numDiscreteRealVars + numDiscreteStringVars,
        cv_init.length());
...
void COLINOptimizer::set_solver_parameters() {
  ...
  if (probDescDB.is_null()) {
    ...
    const Real& init_delta = probDescDB.get_real("method.coliny.initial_delta");
    ...
    const String& selection_type = probDescDB.get_string("method.fitness_type");
    ...
    double mutation_rate = probDescDB.get_real("method.mutation_rate");
    ...
    const String& mutation_type = probDescDB.get_string("method.mutation_type");
    if (colinSolver->has_property("realarray_mutation_type")) {
      if (mutation_type == "replace_uniform" ||
        ...
        colinSolver->property("realarray_mutation_type") =
            mutation_type.c_str();
      else  // default mutation type = "offset_uniform"
        colinSolver->property("realarray_mutation_type") =
            string("offset_uniform");
    }
    ...
    if (mutation_range >= 0 &&
        colinSolver->has_property("intarray_mutation_range")) {
    ...
    const bool& expansion_flag = probDescDB.get_bool("method.coliny.expansion");
    if (expansion_flag) {
    ...
    if (!exploratory_moves.empty() &&
        colinSolver->has_property("exploratory_move"))
    ...
    total_pattern_size = probDescDB.get_int("method.coliny.total_pattern_size");
    ...
    if (num_augmented_trials > 0 &&
        colinSolver->has_property("num_augmented_trials"))
    ...
    if (solution_accuracy > -DBL_MAX &&
        colinSolver->has_property("sufficient_objective_value"))
    ...
    if (show_misc_options) {
      Cout << "---------------------------SOLVER OPTIONS"
           << "---------------------------\n";
      Cout << "The functionality to show solver options is temporarily "
              "unavailable.\n"
...
void COLINOptimizer::post_run(std::ostream& s) {
  ...
  Model& model_for_responses = localObjectiveRecast
                                   ? *iteratedModel->subordinate_model()
                                   : *iteratedModel;
  ...
  for (i = 0; cache_it != cache_end; ++i, ++cache_it) {
    ...
    for (j = 0; j < numDiscreteStringVars; ++j) {
      String dakota_value = set_index_to_value(
          ddv[j + numDiscreteIntVars + numDiscreteRealVars], dssv_values[j]);
    ...
    double obj_fn_metric = objective(
        fn_vals, numObjectiveFns, iteratedModel->primary_response_fn_sense(),
    ...
    if (variableSortMap.size() < numFinalSolutions) {
      ...
      if (metrics < var_worst_it->first) {
        ...
        variableSortMap.insert(
            std::make_pair(metrics, tmpVariableHolder.copy()));
      }
      if (metrics < resp_worst_it->first) {
        responseSortMap.erase(resp_worst_it);
        responseSortMap.insert(
            std::make_pair(metrics, tmpResponseHolder.copy()));
      }
    }
  }
  ...
  if (colin_cache_warn)
    Cerr
        << "Warning: Lookup of some COLIN final points failed.\n         If you"
  ...
  for (; var_best_it != var_best_end; ++var_best_it, ++resp_best_it, ++index) {
    ...
    if (localObjectiveRecast)  // local_recast_retrieve used for objective...
      for (size_t i = numUserPrimaryFns;
        ...
        bestResponseArray[index].function_value(
            resp_best_it->second.function_value(i), i);
    else
...
/** Encapsulated Colin Cache response extraction, which will
    ...
    successful lookup. */
std::pair<bool, bool> COLINOptimizer::colin_cache_lookup(
...
double COLINOptimizer::constraint_violation(const Response& tmpResponseHolder) {
  ...
  for (size_t j = 0; j < num_nln_ineq; j++) {
    if (fn_vals[j + numIterPrimaryFns] > nln_ineq_upr_bnds[j])
      cons_violation +=
          std::pow(fn_vals[j + numIterPrimaryFns] - nln_ineq_upr_bnds[j], 2);
    else if (fn_vals[j + numIterPrimaryFns] < nln_ineq_lwr_bnds[j])
      cons_violation +=
          std::pow(nln_ineq_lwr_bnds[j] - fn_vals[j + numIterPrimaryFns], 2);
  }
  for (size_t j = 0; j < num_nln_eq; j++) {
    if (std::fabs(fn_vals[j + numIterPrimaryFns + num_nln_ineq] -
                  nln_eq_targets[j]) > 0.)
      cons_violation += std::pow(
          fn_vals[j + numIterPrimaryFns + num_nln_ineq] - nln_eq_targets[j], 2);
  }
