...
OptDartsOptimizer::OptDartsOptimizer(ProblemDescDB& problem_db,
                                     ParallelLibrary& parallel_lib,
                                     std::shared_ptr<Model> model)
    : Optimizer(problem_db, parallel_lib, model,
                std::shared_ptr<TraitsBase>(new OptDartsTraits())) {
  // load_parameters
...
OptDartsOptimizer::OptDartsOptimizer(std::shared_ptr<Model> model)
    : Optimizer(GENIE_OPT_DARTS, model,
                std::shared_ptr<TraitsBase>(new OptDartsTraits())) {
  // load_parameters
  this->load_parameters(*model);
}
...
void OptDartsOptimizer::core_run() {
  ...
  opt_darts_execute(num_dim, budget, xmin, xmax, TOL, problem_index, fw_MC,
                    fb_MC);

  delete[] xmin;
  delete[] xmax;

  // at the end, you need to return the best variables and corresponding
  // responses see NOMADOptimizer as an example
  //  for example,
...
double OptDartsOptimizer::opt_darts_f() {
  ...
  if (_problem_index == 2) {
    ...
    for (size_t idim = 0; idim < _num_dim; idim++) {
      ...
      double wherb = pow(E, -xm * xm) + pow(E, -0.8 * xp * xp) -
                     0.05 * sin(8 * (_dart[idim] + 0.1));
      fval *= wherb;
    }
    fval = -fval;
    return fval;
  ...
  if (_problem_index == 3) {
    ...
    for (size_t idim = 0; idim < _num_dim; idim++) {
      ...
      double wherb =
          pow(E, -xm * xm) +
          pow(E, -0.8 * xp * xp);  // - 0.05 * sin(8 * (x[idim] + 0.1));
      fval *= wherb;
    }
    fval = -fval;
    return fval;
...
void OptDartsOptimizer::opt_darts_execute(size_t num_dim, size_t budget,
                                          double* xmin, double* xmax,
                                          double TOL, size_t problem_index,
  ...
  if (problem_index == 0 || problem_index == 1 || problem_index == 6 ||
      problem_index == 7) {
    for (size_t idim = 0; idim < _num_dim; idim++) {
  ...
  while (_num_samples < _budget) {
    ...
    if (count == 100) {
      /*
       ...
       std::cout<< "Value of first corner = " << _f[_corners[0]][0] <<
       std::endl; std::cout<< "best point = " << _ib << std::endl; std::cout<<
       "size of best point = " << _h[_ib] << std::endl; std::cout<< "Value of
       best point = " << _f[_ib][0] << std::endl;
       */
      count = 0;
    }
  ...
  std::cout << "Method has converged after " << _num_samples
            << " function evaluations" << std::endl;
  // std::cout<< "worst value (MC) = " << std::setprecision(10) << _fw_MC <<
  // std::endl; std::cout<< "best value (MC) = " << std::setprecision(10) <<
  // _fb_MC << std::endl;
  ...
  if (_problem_index == 4) {
    ...
    for (size_t idim = 1; idim < _num_dim; idim++)
...
void OptDartsOptimizer::opt_darts_initiate(double* xmin, double* xmax) {
  ...
  for (size_t idim = 0; idim < _num_dim; idim++)
    _alpha_Deceptive[idim] =
        0.699719487;  // 0.5 + 0.2 * generate_a_random_number();
    ...
    for (size_t isample = 0; isample < _num_samples; isample++) {
      ...
      if (_estimate_K && num_neighbors > 2 * _num_dim) {
        ...
        if (hK > h * _K[isample][ifunc]) {
          // Current cell should be discarded according to estimate of K from
          // neighbors which could be inaccurate
...
void OptDartsOptimizer::opt_darts_sample_from_candidate_neighborhood(
    size_t icandidate, size_t ifunc) {
  // simply add furthest witness
  for (size_t idim = 0; idim < _num_dim; idim++)
    _dart[idim] = _xc[icandidate][idim];
...
void OptDartsOptimizer::DIRECT_sample_from_candidate_neighborhood(
    size_t icandidate) {
  double dx_max(0.0);
  size_t dir(0);
  for (size_t idim = 0; idim < _num_dim; idim++) {
...
void OptDartsOptimizer::opt_darts_add_dart() {
  ...
  for (size_t idim = 0; idim < _num_dim; idim++)
    _x[_num_samples][idim] = _dart[idim];
...
void OptDartsOptimizer::opt_darts_update_K_h_approximate_Voronoi(
    size_t isample) {
  for (size_t ifunc = 0; ifunc < numFunctions; ifunc++)
    _K[isample][ifunc] = 0.0;
  ...
  for (size_t j = 0; j < _num_ext_neighbors; j++) {
    ...
    // retrieve mid-point
    for (size_t idim = 0; idim < _num_dim; idim++)
      _end[idim] = 0.5 * (_x[isample][idim] + _x[jsample][idim]);

    // retrieve closest sample point to mid point
    double dmin(_diag * _diag);
    double dst_j(0.0);
    for (size_t k = 0; k < _num_ext_neighbors; k++) {
  ...
  while (true) {
    ...
    for (size_t idim = 0; idim < _num_dim; idim++)
      _end[idim] = _st[idim] + t_end * (_end[idim] - _st[idim]);
...
void OptDartsOptimizer::opt_darts_plot_discs_2d(size_t icandidate) {
  ...
  for (size_t index = 0; index < _num_samples; index++) {
    // filled discs
    file << _x[index][0] * scale << "  " << _x[index][1] * scale << "  "
         << _r[index] * scale << "  ";
    file << "greenfcirc" << std::endl;  // non-active disk
  }

  if (icandidate < _num_samples) {
    file << _x[icandidate][0] * scale << "  " << _x[icandidate][1] * scale
         << "  " << _r[icandidate] * scale << "  ";
  ...
  for (size_t index = 0; index < _num_samples; index++) {
    ...
    /*
     ...
     if (h > 1E-10)
     {
     file << _x[index][0] * scale << "  " << _x[index][1] * scale << "  " << h *
  ...
  file << _x[_ib][0] * scale << "  " << _x[_ib][1] * scale << "  "
       << 0.02 * scale << "  ";
  ...
  for (size_t index = 0; index < _num_samples; index++) {
    ...
    for (size_t i = 1; i <= num_neighbors; i++) {
      ...
      // file << st[0] * scale << "  " << st[1] * scale << "  " << end[0] *
      // scale << "  " << end[1] * scale << " blackseg" << std::endl;
      ...
      file << st[0] * scale << "  " << st[1] * scale << "  " << end[0] * scale
           << "  " << end[1] * scale << " blackseg" << std::endl;
    }
  }
  delete[] st;
  delete[] end;
...
void OptDartsOptimizer::opt_darts_plot_hull_2d(size_t icandidate,
                                               size_t ifunc) {
  std::stringstream ss;
  ...
  if (false) {
    // plot axis
    file << xmin * scale << "  " << ymin * scale << "  " << xmax * scale << "  "
         << ymin * scale << " blackseg_bold" << std::endl;
    file << xmin * scale << "  " << ymax * scale << "  " << xmax * scale << "  "
         << ymax * scale << " blackseg_bold" << std::endl;
    file << xmin * scale << "  " << ymin * scale << "  " << xmin * scale << "  "
         << ymax * scale << " blackseg_bold" << std::endl;
    file << xmax * scale << "  " << ymin * scale << "  " << xmax * scale << "  "
         << ymax * scale << " blackseg_bold" << std::endl;
  }
  ...
  for (size_t index = 0; index < _num_samples; index++) {
    // filled discs
    file << _h[index] * scale_x << "  " << _f[index][ifunc] * scale_y << "  "
         << 0.2 * scale << "  ";
    file << "greenfcirc" << std::endl;  // non-active disk
  }
  ...
  for (size_t icorner = 0; icorner < _num_corners; icorner++) {
    ...
    file << ho * scale_x << "  " << fo * scale_y << "  " << hn * scale_x << "  "
         << fn * scale_y << " blackseg" << std::endl;
  ...
  // candidate disc
  file << _h[icandidate] * scale_x << "  " << _f[icandidate][ifunc] * scale_y
       << "  " << 0.2 * scale << "  ";
...
void OptDartsOptimizer::sample_uniformly_from_unit_sphere_surface(
    double* dart, size_t num_dim) {
  // unbiased method
  double sf = 0.0;
  for (size_t idim = 0; idim < num_dim; idim++) {
    double sum(0.0);
...
bool OptDartsOptimizer::trim_line_using_Hyperplane(
