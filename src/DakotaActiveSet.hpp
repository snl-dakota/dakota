...
inline ShortArray& ActiveSet::request_vector() { return requestVector; }
...
inline void ActiveSet::request_values(const short rv_val, size_t start,
                                      size_t end) {
  for (size_t i = start; i < end; ++i) requestVector[i] = rv_val;
}
...
inline SizetArray& ActiveSet::derivative_vector() { return derivVarsVector; }
...
inline void ActiveSet::derivative_start_value(size_t dvv_start_val) {
  ...
  for (i = 0; i < dvv_len; ++i) derivVarsVector[i] = dvv_start_val + i;
}
...
BOOST_CLASS_TRACKING(Dakota::ActiveSet, boost::serialization::track_never)
