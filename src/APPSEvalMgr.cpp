...
APPSEvalMgr::APPSEvalMgr(Optimizer& opt, std::shared_ptr<Model> model)
    : dakOpt(opt),
      ...
      numWorkersUsed(0),
      numWorkersTotal(1),
      xTrial(ModelUtils::continuous_variables(*model)) {
...
bool APPSEvalMgr::submit(const int apps_tag,
  ...
  if (numWorkersUsed < numWorkersTotal) {
    set_variables<>(apps_xtrial, *iteratedModel,
...
int APPSEvalMgr::recv(int& apps_tag, HOPSPACK::Vector& apps_f,
  ...
  if (modelAsynchFlag) {
    if (dakotaResponseMap.empty())
      dakotaResponseMap = (blockingSynch) ? iteratedModel->synchronize()
    ...
    if (!dakotaResponseMap.empty()) {  // synchronize_nowait may return none
      ...
      if (find_tag != tagList.end()) {
        ...
        dakOpt.get_responses_from_dakota(local_fn_vals, apps_f, apps_cEqs,
                                         apps_cIneqs);
