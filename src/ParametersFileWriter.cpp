...
void ParametersFileWriter::write_parameters_file(
    const PRPQueue& prp_queue, const std::string& prog,
    const std::vector<std::string>& an_comps,
    const std::string& eval_prefix_tag, int batch_id,
  ...
  populate_labels(vars, set, response, prog, an_comps, asv_labels, dvv_labels,
                  ac_labels, md_tags);

  std::string eval_and_batch =
      (eval_prefix_tag.empty())
  ...
  for (const auto& pair : prp_queue) {
    ...
    write_evaluation_to_stream(pair.variables(), pair.active_set(),
                               pair.response(), an_comps, asv_labels,
                               dvv_labels, ac_labels, md_tags, full_eval_id,
                               parameter_stream);  // appear in params file
  }
...
void ParametersFileWriter::write_parameters_file(
  ...
  write_evaluation_to_stream(vars, set, response, an_comps, asv_labels,
                             dvv_labels, ac_labels, md_tags, formatted_eval_id,
                             parameter_stream);
...
void ParametersFileWriter::write_evaluation_to_stream(
    const Variables& vars, const ActiveSet& set, const Response& response,
...
void ParametersFileWriter::populate_labels(
    const Variables& vars, const ActiveSet& set, const Response& response,
    const std::string prog, const std::vector<std::string>& an_comps,
    StringArray& asv_labels, StringArray& dvv_labels, StringArray& ac_labels,
  ...
  for (i = 0; i < asv_len; ++i) asv_labels[i] += ":" + resp_labels[i];
  for (i = 0; i < dvv_len; ++i) {
    size_t acv_index = find_index(acv_ids, dvv[i]);
    if (acv_index != _NPOS) dvv_labels[i] += ":" + acv_labels[acv_index];
  }
