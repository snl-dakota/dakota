...
void ExtPythonMethod::initialize_python()
{
  ...
  // Prepend sys.path with empty string so Python can find modules in the
  // current working directory (e.g. the user's ext_py_methods module)
  PyRun_SimpleString("import sys\nsys.path.insert(0,\"\")");

  if( !py11Active )
  {
    // Import the dakota.ext_method extension module to register the
    // ModelExecutor type with pybind11 before passing it to Python
    try {
      py::module_::import("dakota.ext_method");
    }
    catch(py::error_already_set &e) {
      Cerr << "Error: Could not import the dakota.ext_method extension module.\n"
           << "Ensure the Dakota Python package is on PYTHONPATH.\n"
           << e.what() << std::endl;
      abort_handler(-1);
    }
