...
class ApproximationInterface : public Interface {
 ...
 protected:
  ...
  void approximation_function_indices(
      const SizetSet& approx_fn_indices) override;
  ...
  Real2DArray cv_diagnostics(const StringArray& metrics,
...
inline const Approximation& ApproximationInterface::function_surface(
    size_t index) const {
  return functionSurfaces[index];
}
...
inline int ApproximationInterface::minimum_points(bool constraint_flag) const {
  ...
  int min_points = 0;
  for (StSCIter cit = approxFnIndices.begin(); cit != approxFnIndices.end();
       cit++)
...
inline int ApproximationInterface::recommended_points(
    bool constraint_flag) const {
  ...
  for (StSCIter cit = approxFnIndices.begin(); cit != approxFnIndices.end();
       cit++)
    rec_points = std::max(
        rec_points, function_surface(*cit).recommended_points(constraint_flag));
  return rec_points;
}

inline void ApproximationInterface::active_model_key(
...
inline void ApproximationInterface::restore_data_key() {
  ...
  bool reduce_key =
      (active_key.aggregated() && active_key.raw_with_reduction_data());
...
inline void ApproximationInterface::qoi_set_to_key_index(size_t qoi_set,
                                                         size_t& key_index) {
    ...
    */
    ...
    switch (sharedData.discrepancy_emulation_mode()) {
      case RECURSIVE_EMULATION: {
        ...
        key_index =
            last_index - qoi_set;  // reverse indexing (maps 0->1 for now)
...
inline void ApproximationInterface::approximation_function_indices(
...
inline bool ApproximationInterface::advancement_available() {
  ...
  if (sharedData.advancement_available())
    return true;  // check Shared first
  else {
    bool refine = false;
    for (StSIter it = approxFnIndices.begin(); it != approxFnIndices.end();
         ++it)
...
inline const SizetSet& ApproximationInterface::approximation_fn_indices()
    const {
...
inline const Pecos::SurrogateData& ApproximationInterface::approximation_data(
    size_t fn_index) {
...
inline void ApproximationInterface::sample_to_variables(
    const Real* sample_c_vars, size_t num_cv, Variables& vars) {
  if (vars.cv() == num_cv)
