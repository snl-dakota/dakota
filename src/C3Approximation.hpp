...
class C3Approximation : public Approximation {
 ...
 private:
  ...
  /// compute variance corresponding to the passed FT expansion
  Real covariance(const RealVector& x, C3FnTrainData& ftd1,
                  C3FnTrainData& ftd2);
...
inline void C3Approximation::active_model_key(const Pecos::ActiveKey& key) {
  ...
  // Test for change
  if (levApproxIter != levelApprox.end() && levApproxIter->first == key) return;
...
inline const RealVector& C3Approximation::numerical_integration_moments()
    const {
...
inline Real C3Approximation::mean() { return mean(levApproxIter->second); }
...
inline Real C3Approximation::combined_mean() { return mean(combinedC3FTData); }
...
inline Real C3Approximation::covariance(const RealVector& x,
...
inline Real C3Approximation::combined_covariance(const RealVector& x,
                                                 Approximation& approx_2) {
  std::shared_ptr<C3Approximation> c3_approx_rep_2 =
      std::static_pointer_cast<C3Approximation>(approx_2.approx_rep());
