...
void load(Archive& ar, Teuchos::SerialDenseVector<OrdinalType, ScalarType>& v,
  ...
  OrdinalType i, len;
  ar & len;
  if (len != v.length()) v.sizeUninitialized(len);
  for (i = 0; i < len; ++i) ar& v[i];
}
...
inline void save(Archive& ar, const boost::dynamic_bitset<Block, Allocator>& bs,
                 const unsigned int version) {
  size_t size = bs.size();
  ar & size;
...
template <class Archive, typename Block, typename Allocator>
inline void load(Archive& ar, boost::dynamic_bitset<Block, Allocator>& bs,
                 const unsigned int version) {
  size_t size;
  ar & size;
...
// Forward declarations since these operators are used in read/write data and
// vice-versa
...
std::istream& operator>>(
    std::istream& s, Teuchos::SerialDenseVector<OrdinalType, ScalarType>& data);
...
template <typename OrdinalType, typename ScalarType>
std::istream& operator>>(
    std::istream& s, Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& data);
...
void read_sized_data(std::istream& s, RealVectorArray& va,
                     size_t num_experiments, int num_state_variables);

/// istream extraction operator for response data of known dim and
/// unknown length
void read_fixed_rowsize_data(std::istream& s, RealVectorArray& va,
                             int num_responses, bool row_major = true);

/// istream extraction operator for coordinate data of unknown dim and
/// unknown length
void read_unsized_data(std::istream& s, RealVectorArray& va,
                       bool row_major = true);

/// file reader for configuration data supplied via multiple files
void read_config_vars_multifile(const std::string& basename, int num_expts,
                                int ncv, std::vector<Variables>& config_vars);

/// file reader for configuration data supplied via a single file
void read_config_vars_singlefile(const std::string& basename, int num_expts,
                                 int ncv, std::vector<Variables>& config_vars);

/// file reader for vector field (response) data
void read_field_values(const std::string& basename, int expt_num,
...
void read_coord_values(const std::string& basename, int expt_num,
                       RealMatrix& coords);

/// file reader for CONSTANT covariance data
void read_covariance(const std::string& basename, int expt_num,
                     RealMatrix& cov_vals);

/// file reader for VECTOR and MATRIX covariance data
void read_covariance(const std::string& basename, int expt_num,
                     Dakota::CovarianceMatrix::FORMAT, int num_vals,
                     RealMatrix& cov_vals);
...
void read_data(std::istream& s,
  ...
  OrdinalType i, len = v.length();
  if (label_array.size() != len) label_array.resize(boost::extents[len]);
  for (i = 0; i < len; ++i) s >> v[i] >> label_array[i];
}
...
void read_data_partial(
    std::istream& s, OrdinalType2 start_index, OrdinalType2 num_items,
  ...
  for (i = start_index; i < end; ++i) s >> v[i];
}

/// standard istream extraction operator for partial SerialDenseVector
/// with labels (as StringMultiArray&)
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void read_data_partial(std::istream& s, OrdinalType2 start_index,
  ...
  if (label_array.size() != len) label_array.resize(boost::extents[len]);
  for (i = start_index; i < end; ++i) s >> v[i] >> label_array[i];
}
...
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void read_data_partial_tabular(
    std::istream& s, OrdinalType2 start_index, OrdinalType2 num_items,
    Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& v) {
  OrdinalType2 i, end = start_index + num_items;
  if (end > v.length()) {
...
void read_data_annotated(std::istream& s,
  ...
  s >> len;
  if (len != v.length()) v.sizeUninitialized(len);
  if (len != label_array.size()) label_array.resize(boost::extents[len]);
  for (i = 0; i < len; ++i) s >> v[i] >> label_array[i];
}
...
inline void read_data_annotated(std::istream& s, StringMultiArray& v,
  ...
  size_t i, len;
  s >> len;
  if (len != v.size()) v.resize(boost::extents[len]);
  if (len != label_array.size()) {
    Cerr << "Error: size of label_array in read_data_annotated(std::istream) "
         << "does not equal length of StringMultiArray." << std::endl;
    abort_handler(-1);
...
void read_lower_triangle(
  ...
  OrdinalType i, j, nrows = sm.numRows();
  for (i = 0; i < nrows; ++i)
    for (j = 0; j <= i; ++j) s >> sm(i, j);
}

/// istream partial specialization for reading a column vector of a
/// SerialDenseMatrix
template <typename OrdinalType, typename ScalarType>
void read_col_vector_trans(
    std::istream& s, OrdinalType col,
  ...
  for (OrdinalType row = 0; row < nr; ++row) {
    s >> token;
    sdm_c[row] = std::atof(token.c_str());
  }
}
...
template <typename IStreamType, typename OrdinalType, typename ScalarType>
void read_col_vector_trans(
    IStreamType& s, OrdinalType col,
    Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& sdm) {
  OrdinalType nr = sdm.numRows();
  ScalarType* sdm_c = sdm[col];  // column vector
  for (OrdinalType row = 0; row < nr; ++row) s >> sdm_c[row];
}
...
inline void array_read(std::istream& s, ArrayT& v) {
  ...
  for (typename ArrayT::size_type i = 0; i < len; ++i) s >> v[i];
}
...
void write_data_aprepro(
  ...
  for (i = 0; i < len; ++i)
    s << "                    { " << std::setw(15)
      << std::setiosflags(std::ios::left) << label_array[i].data()
...
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void write_data_partial(
    std::ostream& s, OrdinalType2 start_index, OrdinalType2 num_items,
...
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void write_data_partial_aprepro(
    std::ostream& s, OrdinalType2 start_index, OrdinalType2 num_items,
    const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& v,
    const StringMultiArray& label_array) {
  OrdinalType2 i, end = start_index + num_items;
...
void write_data_partial_json(json& s, OrdinalType start_index,
  ...
  for (i = start_index; i < end; ++i) {
    json tempVarObject = {{"label", label_array[i]}, {"value", v[i]}};
...
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void write_data_partial_json(
    json& s, OrdinalType2 start_index, OrdinalType2 num_items,
    const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& v,
    const StringMultiArray& label_array) {
  OrdinalType2 i, end = start_index + num_items;
  OrdinalType1 len = v.length();
...
template <typename OrdinalType, typename ScalarType>
void write_data_tabular(
    std::ostream& s,
    const Teuchos::SerialDenseVector<OrdinalType, ScalarType>& v) {
  OrdinalType i, len = v.length();
  s << std::setprecision(write_precision)
    << std::resetiosflags(std::ios::floatfield);
  for (i = 0; i < len; ++i) s << std::setw(write_precision + 4) << v[i] << ' ';
}
...
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void write_data_partial_tabular(
    std::ostream& s, OrdinalType2 start_index, OrdinalType2 num_items,
    const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& v) {
  OrdinalType2 end = start_index + num_items;
  if (end > v.length()) {
...
template <typename OrdinalType>
void write_data_partial_tabular(std::ostream& s, OrdinalType start_index,
                                OrdinalType num_items,
                                const StringMultiArray& v) {
...
template <typename OrdinalType, typename ScalarType>
void write_col_vector_trans(
    std::ostream& s, OrdinalType col, OrdinalType num_items,
...
template <typename OStreamType, typename OrdinalType, typename ScalarType>
void write_col_vector_trans(
    OStreamType& s, OrdinalType col, OrdinalType num_items,
    const Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& sdm) {
...
inline void array_write_aprepro(std::ostream& s, const StringArray& v,
  ...
  for (StringArray::size_type i = 0; i < len; ++i)
    s << "                    { " << std::setw(15)
      << std::setiosflags(std::ios::left) << label_array[i].c_str()
      << std::resetiosflags(std::ios::adjustfield) << " = "
      << std::setw(write_precision + 7) << '"' << v[i] << '"' << " }\n";
}
...
using IteratorCategoryOf = typename std::iterator_traits<
    typename ContainerT::iterator>::iterator_category;
                    ...
                    std::forward_iterator_tag)  // generic version
{
  ...
  for (i = 0; i < len; ++i) {
    typename ContainerT::value_type
        data;  // fresh alloc in case T is ref-counted
    s >> data;
    c.push_back(data);
  }
#endif
...
{
  container_read(s, c, IteratorCategoryOf<ContainerT>());
  return s;
}
...
MPIPackBuffer& operator<<(MPIPackBuffer& s, const ContainerT& c)  // one version
{
  ...
  s << len;
  for (const typename ContainerT::value_type& entry : c) s << entry;
#endif
  return s;
}
...
inline MPIPackBuffer& operator<<(
    MPIPackBuffer& s, const boost::dynamic_bitset<Block, Allocator>& bs) {
  size_t size = bs.size();
  s << size;
...
template <typename Block, typename Allocator>
inline MPIUnpackBuffer& operator>>(
    MPIUnpackBuffer& s, boost::dynamic_bitset<Block, Allocator>& bs) {
  size_t size;
  s >> size;
...
MPIPackBuffer& operator<<(MPIPackBuffer& s,
                          const std::map<KeyT, ValueT>& data) {
  size_t len = data.size();
  s << len;
...
inline std::ostream& operator<<(
    std::ostream& s,
    const Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& data) {
  write_data(s, data, true, true, true);
  return s;
}
