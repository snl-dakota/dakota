...
class PluginInterface : public ApplicationInterface {
 ...
 protected:
  ...
  DakotaPlugins::EvalRequest form_eval_request(const Variables& vars,
                                               const ActiveSet& set,
                                               int fn_eval_id) const;

  /// map plugin response to Dakota response
  void populate_response(const DakotaPlugins::EvalResponse& plugin_response,
