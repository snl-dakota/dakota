...
class MeanAccumulator : public Accumulator<T> {
 public:
  ...
  // Args:
  //   sample: The sample to be added.
  void addSample(T sample) override { acc_(sample); }

  // Returns the number of samples in this mean accumulator.
  //
  // Returns:
  //   The number of samples.
  std::size_t numSamples() override { return boost::accumulators::count(acc_); }

  // Computes the mean of the accumulated samples.
  //
  // Returns:
  //   The computed mean of type T.
  T computeStatistic() override { return mean(acc_); }

  // Empties the accumulator of all stored samples.
  void reset() override { acc_ = mean_accumulator(); }
...
class VarianceAccumulator : public Accumulator<T> {
 public:
  ...
  T computeStatistic() override { return variance(acc_); }

  // Empties the accumulator of all stored samples.
  void reset() override { acc_ = variance_accumulator(); }
...
class CovarianceAccumulator : public Accumulator<T> {
 public:
  ...
  // Empties the accumulator of all stored samples.
  void reset() override { acc_ = covariance_accumulator(); }

 private:
  typedef accumulator_set<T, stats<tag::covariance<T, tag::covariate1>>>
