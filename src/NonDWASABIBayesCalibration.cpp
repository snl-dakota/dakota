...
NonDWASABIBayesCalibration::NonDWASABIBayesCalibration(
    ...
    : NonDBayesCalibration(problem_db, parallel_lib, model),
      numPushforwardSamples(
          probDescDB.get_int("method.nond.pushforward_samples")),
      ...
      posteriorSamplesImportFile(
          probDescDB.get_string("method.nond.posterior_samples_import_file")),
      posteriorSamplesImportFormat(0),  // not used
      exportPosteriorDensityFile(
          probDescDB.get_string("method.nond.posterior_density_export_file")),
      exportPosteriorSamplesFile(
          probDescDB.get_string("method.nond.posterior_samples_export_file")),
      exportFileFormat(0),  // not used
      generateRandomPosteriorSamples(
          probDescDB.get_bool("method.nond.generate_posterior_samples")),
      evaluatePosteriorDensity(
          probDescDB.get_bool("method.nond.evaluate_posterior_density")) {
...
void NonDWASABIBayesCalibration::calibrate() {
  ...
  RealMatrix samples_from_prior((int)numContinuousVars, numPushforwardSamples,
                                false);
...
#ifdef DEBUG
  Cout << "pushforward_responses_from_prior "
       << pushforward_responses_from_prior;
#endif
  ...
  response_kde.pdf(responses_for_posterior_eval,
                   response_density_vals_for_posterior_eval, Teuchos::TRANS);
  ...
  if (dataDistMeans.length() > 0) {
    ...
    std::ifstream file_stream;
    TabularIO::open_file(file_stream, dataDistFilename, "obs_data");
    RealVectorArray va;
    read_unsized_data(file_stream, va);
    dataDistMeans = va[0];
  ...
  for (int k = 0; k < num_obs_data; k++)
    datadists.push_back(
        boost::math::normal(dataDistMeans[k], dataDistCovariance[k]));
  ...
  for (int j = 0; j < samples_for_posterior_eval.numCols(); j++) {
    ...
    posterior_density[j] =
        (data_density_vals[j] / response_density_vals_for_posterior_eval[j]);
  }
  ...
  if (generateRandomPosteriorSamples)
    for (int j = 0; j < samples_for_posterior_eval.numCols(); j++) {
      ...
      if (ratio > rnum) points_to_keep.push_back(j);
    }
  ...
  for (int k = 0; k < numFunctions; k++) momentStatistics(0, k) = 0.0;
  for (int j = 0; j < samples_for_posterior_eval.numCols(); j++) {
    posterior_density[j] = posterior_density[j] * prior_density_vals[j];
    for (int k = 0; k < numFunctions; k++)
      momentStatistics(0, k) +=
          responses_for_posterior_eval(k, j) * posterior_density[j];
  }
  for (int k = 0; k < numFunctions; k++) {
    momentStatistics(0, k) =
        momentStatistics(0, k) / samples_for_posterior_eval.numCols();
  }
#ifdef DEBUG
  Cout << "Posterior density values " << posterior_density << '\n';
#endif
...
void NonDWASABIBayesCalibration::extract_selected_posterior_samples(
  ...
  posterior_data.shapeUninitialized(num_points_to_keep, numContinuousVars + 1);
  ...
  for (size_t i = 0; i < num_points_to_keep; i++)
    for (size_t j = 0; j < numContinuousVars; j++)
      posterior_samples(i, j) =
          samples_for_posterior_eval(j, points_to_keep[i]);

  RealVector posterior_values(Teuchos::View, posterior_data[numContinuousVars],
                              num_points_to_keep);
  for (size_t i = 0; i < num_points_to_keep; i++)
    posterior_values[i] = posterior_density[points_to_keep[i]];
}

void NonDWASABIBayesCalibration::export_posterior_samples_to_file(
  ...
  Dakota::write_data(export_file_stream, posterior_data, brackets, row_rtn,
                     final_rtn);
...
void NonDWASABIBayesCalibration::print_results(std::ostream& s,
  ...
  NonDSampling::print_moments(s, momentStatistics, RealMatrix(),
                              "response function", Pecos::STANDARD_MOMENTS,
                              resp_labels, false);
}

void NonDWASABIBayesCalibration::compute_responses(RealMatrix& samples,
