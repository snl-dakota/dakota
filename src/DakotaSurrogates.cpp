...
SurrogatesBaseApprox::SurrogatesBaseApprox(const ProblemDescDB& problem_db,
                                           const SharedApproxData& shared_data,
                                           const String& approx_label)
    : FieldApproximation(BaseConstructor(), problem_db, shared_data,
                         StringArray{approx_label}) {
...
SurrogatesBaseApprox::SurrogatesBaseApprox(const SharedApproxData& shared_data)
...
RealArray SurrogatesBaseApprox::cv_diagnostic(const StringArray& metric_types,
                                              unsigned num_folds) {
  MatrixXd vars, resp;
  convert_surrogate_data(vars, resp);
...
void SurrogatesBaseApprox::challenge_diagnostics(
...
void SurrogatesBaseApprox::convert_surrogate_data(MatrixXd& vars,
                                                  MatrixXd& resp,
                                                  int num_resp) {
...
Real SurrogatesBaseApprox::value(const RealVector& c_vars) {
  if (!model) {
...
RealVector SurrogatesBaseApprox::values(const RealVector& c_vars) {
  if (!model) {
  ...
  for (size_t j = 0; j < vals.size(); j++) ret_vals[j] = vals(j);
...
const RealVector& SurrogatesBaseApprox::gradient(const RealVector& c_vars) {
  ...
  for (size_t j = 0; j < num_vars; j++) approxGradient[j] = pred_grad(0, j);
...
const RealSymMatrix& SurrogatesBaseApprox::hessian(const RealVector& c_vars) {
  ...
  for (size_t i = 0; i < num_vars; i++)
    for (size_t j = 0; j <= i; j++) approxHessian(i, j) = pred_hess(i, j);

  return approxHessian;
}

void SurrogatesBaseApprox::import_model(const ProblemDescDB& problem_db) {
  auto import_prefix =
  ...
  std::string filename =
      import_prefix + "." + approxLabel + (is_binary ? ".bin" : ".txt");
...
void SurrogatesBaseApprox::map_variable_labels(const Variables& dfsm_vars) {
...
void SurrogatesBaseApprox::export_model(const Variables& vars,
...
void SurrogatesBaseApprox::export_model(const StringArray& var_labels,
                                        const String& fn_label,
                                        const String& export_prefix,
                                        const unsigned short export_format) {
