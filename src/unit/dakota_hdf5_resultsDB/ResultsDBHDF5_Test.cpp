...
TEST(ResultsDBHDF5_tests, test_tpl_hdf5_test_results_manager_init) {
  ...
  std::shared_ptr<HDF5IOHelper> hdf5_helper_ptr(
      new HDF5IOHelper(database_name, true));
  std::unique_ptr<ResultsDBHDF5> db_ptr(
      new ResultsDBHDF5(false /* in_core */, hdf5_helper_ptr));
...
TEST(ResultsDBHDF5_tests, test_tpl_hdf5_test_insert_into) {
  // TODO: This is not a very good test, yet. It passes if no exceptions are
  // throw, but it doesn't read the matrices back in to confirm that they were
  // written correctly.
  ...
  Dakota::StrStrSizet iterator_id = boost::make_tuple(
      std::string("test_method"), std::string("test_method_id"), 1);

  std::vector<double> row0 = {1.0, 2.0, 3.0, 4.0};
  std::vector<double> row1 = {5.0, 6.0, 7.0, 8.0};
  Dakota::RealVector row2(4);
  for (int i = 0; i < 4; ++i) row2[i] = double(i) + 9.0;

  Dakota::DimScaleMap m_scales;
  m_scales.emplace(0,
                   Dakota::StringScale("dim_0_test", {"a", "scale", "test"}));
  m_scales.emplace(1,
                   Dakota::StringScale("dim_1_test", {"do", "re", "me", "fa"}));
  ...
  results_manager.allocate_matrix(iterator_id, {std::string("row_result")},
                                  Dakota::ResultsOutputType::REAL, 3, 4,
                                  m_scales, attrs);
  results_manager.insert_into(iterator_id, {std::string("row_result")}, row0,
                              0);
  results_manager.insert_into(iterator_id, {std::string("row_result")}, row1,
                              1);
  results_manager.insert_into(iterator_id, {std::string("row_result")}, row2,
                              2);

  results_manager.allocate_matrix(iterator_id, {std::string("column_result")},
                                  Dakota::ResultsOutputType::REAL, 4, 3);
  results_manager.insert_into(iterator_id, {std::string("column_result")}, row0,
                              0, false);
  results_manager.insert_into(iterator_id, {std::string("column_result")}, row1,
                              1, false);
  results_manager.insert_into(iterator_id, {std::string("column_result")}, row2,
                              2, false);
  ...
  for (int i = 0; i < 3; ++i)
    results_manager.insert_into(iterator_id, {std::string("vector_result")},
                                double(i + 5), i);
}
