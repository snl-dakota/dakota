...
std::shared_ptr<Dakota::Optimizer> get_optimizer(
    std::shared_ptr<Dakota::LibraryEnvironment> p_env, bool verbose = false) {
  Dakota::ProblemDescDB &problem_db = p_env->problem_description_db();
  Dakota::IteratorList &iter_list =
      Dakota::Iterator::iterator_cache(problem_db);
  ...
  std::shared_ptr<Dakota::Optimizer> optimizer =
      std::static_pointer_cast<Dakota::Optimizer>(dak_iter);
...
TEST(opt_tpl_adapters_tests, test_nln_ineq_demo) {
  ...
  std::shared_ptr<Dakota::LibraryEnvironment> p_env(
      Opt_TPL_Test::create_env(text_book_input.c_str()));
  ...
  std::shared_ptr<Dakota::DemoTPLOptimizer> demo_optimizer =
      std::static_pointer_cast<Dakota::DemoTPLOptimizer>(get_optimizer(p_env));

  std::shared_ptr<TPLDataTransfer> data_xfer =
      demo_optimizer->get_data_transfer_helper();
  DemoOptTraits::VecT nln_ineqs(data_xfer->num_tpl_nonlin_ineq_constraints());
  data_xfer->get_nonlinear_ineq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_ineqs);
  ...
  text_book_input += "    nonlinear_inequality_upper_bounds =  5.0  ";
...
TEST(opt_tpl_adapters_tests, test_nln_eq_demo) {
  ...
  data_xfer->get_nonlinear_eq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_eqs);
...
TEST(opt_tpl_adapters_tests, test_nln_mixed_demo) {
  ...
  // Now test adapters when using nonzero nonlinear equality targets and
  // nonlinear ineq upper bounds
  ...
  // Now test adapters when using nonzero nonlinear equality targets and
  // inequality lower and upper bounds
...
class UnitTestTraits : public TraitsBase {
 public:
  ...
  /// Return the flag indicating whether method supports nonlinear equality
  /// constrinats
  bool supports_nonlinear_equality() override { return true; }

  /// Return the flag indicating whether method supports nonlinear inequality
  /// constrinats
...
TEST(opt_tpl_adapters_tests, test_nln_ineq_traits) {
  ...
  using TRAITS_TYPE1 =
      UnitTestTraits<NONLINEAR_INEQUALITY_FORMAT::ONE_SIDED_LOWER,
  ...
  data_xfer->configure_data_adapters(test_traits1,
                                     *demo_optimizer->iterated_model());
  ...
  data_xfer->configure_data_adapters(test_traits2,
                                     *demo_optimizer->iterated_model());
...
TEST(opt_tpl_adapters_tests, test_nln_eq_traits) {
  ...
  using TRAITS_TYPE1 =
      UnitTestTraits<NONLINEAR_INEQUALITY_FORMAT::ONE_SIDED_UPPER,
                     NONLINEAR_EQUALITY_FORMAT::TWO_INEQUALITY>;
  using TRAITS_TYPE2 =
      UnitTestTraits<NONLINEAR_INEQUALITY_FORMAT::ONE_SIDED_LOWER,
                     NONLINEAR_EQUALITY_FORMAT::TWO_INEQUALITY>;
  using TRAITS_TYPE3 =
      UnitTestTraits<NONLINEAR_INEQUALITY_FORMAT::TWO_SIDED,
                     NONLINEAR_EQUALITY_FORMAT::TWO_INEQUALITY>;
  ...
  data_xfer1->configure_data_adapters(test_traits1,
                                      *demo_optimizer->iterated_model());
  data_xfer2->configure_data_adapters(test_traits2,
                                      *demo_optimizer->iterated_model());
  data_xfer3->configure_data_adapters(test_traits3,
                                      *demo_optimizer->iterated_model());
  ...
  data_xfer1->get_nonlinear_eq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_eqs1);
  data_xfer2->get_nonlinear_eq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_eqs2);
  data_xfer3->get_nonlinear_eq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_eqs3);

  // There should be no equality values because they are treated as two-sided
  // inequalities
  ...
  data_xfer1->get_nonlinear_ineq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_ineqs1);
  data_xfer2->get_nonlinear_ineq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_ineqs2);
  data_xfer3->get_nonlinear_ineq_constraints_from_dakota(
      demo_optimizer->iterated_model()->current_response(), nln_ineqs3);

  // BUT there should be two inequality constraints for each model equality
  // constraint
  EXPECT_TRUE((2 == nln_ineqs1.size()));
  EXPECT_TRUE((2 == nln_ineqs2.size()));
  EXPECT_TRUE((2 == nln_ineqs3.size()));
  // ... and they are all treated the same because they are transformed in a
  // manner that works with any of the
  //     three inequality constraint formats
  EXPECT_LT(std::fabs(1. - -0.5 / nln_ineqs1[0]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(1. - 0.5 / nln_ineqs1[1]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(1. - -0.5 / nln_ineqs2[0]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(1. - 0.5 / nln_ineqs2[1]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(1. - -0.5 / nln_ineqs3[0]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(1. - 0.5 / nln_ineqs3[1]), 1.e-12 / 100.);
  ...
  EXPECT_TRUE((2 == nln_ineqs2.size()));
  EXPECT_TRUE((2 == nln_ineqs3.size()));
  EXPECT_LT(std::fabs(/*1. - 0.0 / */ nln_ineqs1[0]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(/*1. - 0.0 / */ nln_ineqs1[1]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(/*1. - 0.0 / */ nln_ineqs2[0]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(/*1. - 0.0 / */ nln_ineqs2[1]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(/*1. - 0.0 / */ nln_ineqs3[0]), 1.e-12 / 100.);
  EXPECT_LT(std::fabs(/*1. - 0.0 / */ nln_ineqs3[1]), 1.e-12 / 100.);
}
