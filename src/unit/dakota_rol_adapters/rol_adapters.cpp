...
class DakotaModelObjective : public FactoryT::RObjT {
 ...
 public:
  DakotaModelObjective(Model *model) : dakModel(model) {}
  ...
  // void hessVec( std::vector<Real> &hv, const std::vector<Real> &v, const
  // std::vector<Real> &x, Real &tol )
  //{
  // }
...
void rol_quad_solv() {
  ...
  Dakota::Model &model =
      **(Dakota::Model::model_cache(env.problem_description_db()).begin());
  ...
  RCP<ROL::Objective<Real> > obj =
      rcp(new DakotaModelObjective<FactoryT>(&model));
...
TEST(rol_adapters_test, a_rol_std_quad) { rol_quad_solv<StdFactory>(); }
