...
TEST(opt_api_core_data_xfers_tests, test_opt_api_data_xfers_basic_vec1) {
  ...
  for (size_t i = 0; i < vec.length(); ++i)
    max_diff =
        (diff = std::fabs(vec[i] - tpl1_vec[i])) > max_diff ? diff : max_diff;
  max_diff += 1.0;
  Real real_tol = std::pow(
      10,
      -std::numeric_limits<dummyGradFreeOpt1::Traits::scalarType>::digits10);
  ...
  for (size_t i = 0; i < vec.length(); ++i)
    max_diff =
        (diff = std::fabs(vec[i] - tpl2_vec[i])) > max_diff ? diff : max_diff;
  max_diff += 1.0;
  float float_tol =
      100.0 *
      std::pow(10, -std::numeric_limits<
                       dummyGradFreeOpt2::Traits::scalarType>::digits10);
...
TEST(opt_api_core_data_xfers_tests, test_opt_api_data_xfers_basic_mat1) {
  ...
  for (size_t i = 0; i < tpl1_vec.size(); ++i)
    max_diff = (diff = std::fabs(mat(MAT_ROW, i) - tpl1_vec[i])) > max_diff
                   ? diff
                   : max_diff;
  max_diff += 1.0;
  Real real_tol = std::pow(
  ...
  // Test that matrix gets resized correctly and values are correnct in the
  // propoer row
  EXPECT_TRUE((mat2.numRows() == 5));
  EXPECT_TRUE((mat2.numCols() == VEC_SIZE));
  max_diff = 0.0;
  for (size_t i = 0; i < tpl1_vec.size(); ++i)
    max_diff = (diff = std::fabs(mat2(4, i) - tpl1_vec[i])) > max_diff
                   ? diff
                   : max_diff;
  max_diff += 1.0;
  EXPECT_LT(std::fabs(1. - max_diff / 1.0), real_tol / 100.);
