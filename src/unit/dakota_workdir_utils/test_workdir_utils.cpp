...
void test_save_current_path(const std::string& pwd_str,
  ...
  std::filesystem::path temp_name =
      WorkdirHelper::unique_path("daktst_%%%%%%%%");
  ...
  if (std::filesystem::exists(wd) && is_directory(wd)) {
    // Change directory into newly created dir (verify current path is as
    // expected)
...
#if defined(DEBUG)
    std::cout << "Listing driver scripts in " << search_dir << " by wildcard "
              << wild_card << std::endl;
#endif
...
void test_which_driver(std::filesystem::path& wd,
  ...
  EXPECT_TRUE((
      std::filesystem::equivalent(driver, std::filesystem::path(driver_name))));
}

void test_cp_template_files_into_wd(std::filesystem::path& wd) {
  // std::cout << "OK to COPY template files into EMPTY dir:  " << wd <<
  // std::endl;
  ...
  BOOST_FOREACH (std::filesystem::path const& p, std::make_pair(it, eod)) {
    if (is_regular_file(p)) {
      ++file_count;
      // std::cout << "counting files poplated wdir.. currentFCount is: " <<
      // file_count << std::endl;
    } else if (is_symlink(p)) {
      ++link_count;
      // std::cout << "counting links poplated wdir.. currentLCount is: " <<
      // link_count << std::endl;
    } else if (is_directory(p)) {
      ++dir_count;
      // std::cout << "counting dirs poplated wdir.. currentDCount is: " <<
      // dir_count << std::endl;
    }
  }
...
void test_ln_template_files_into_wd(std::filesystem::path& wd) {
  // std::cout << "OK to LINK template files into EMPTY dir:  " << wd <<
  // std::endl;
  std::string template_path_str = WorkdirHelper::startup_pwd();
  template_path_str += "/../test/dakota_workdir.templatedir/*";
  ...
  // EXPECT_TRUE(( std::filesystem::exists(wd +=
  // "/dakota_workdir.templatedir.dat") ));
}

void test_rmdir(std::filesystem::path& wd) {
  // 7.	Remove the directory (verify its gone)
  WorkdirHelper::recursive_remove(
      wd, 0);  // 0 indicates silent (appropriate for unit test)
  EXPECT_TRUE((!std::filesystem::exists(wd)));
}
...
void test_create_and_remove_tmpdir(bool copy = false) {
  ...
  //		ToDo: does this work right for pre-existing dir when it’s
  // supposed to tolerate vs. error?
  // 5.	ToDo:  Make a subdir and a contained file so we can verify rm –rf
  // (verify subdir is there)
  ...
  if (std::filesystem::exists(wd) && is_directory(wd)) {
    ...
    // std::cout << "OK to remove dir (with contained items): " << wd <<
    // std::endl;
...
TEST(workdir_utils_tests, all_tests) {
  ...
  /* WJB: consider refactor count_driver_scripts test -- std::filesystem::path
  fq_search(argv[1]); std::string fq_search(rundir_str); fq_search +=
  "/../test/d*.sh"; int run_result = 0;
