...
void test_multiple_scalar_covariance_matrix() {
  ...
  EXPECT_LT(std::fabs(1. - exper_cov.log_determinant() / std::log(8.0)),
            1.0e-12 / 100.);
  ...
  EXPECT_TRUE(
      (std::abs(prod - 7.) < 10. * std::numeric_limits<double>::epsilon()));
...
void test_single_full_block_covariance_matrix() {
  ...
  EXPECT_LT(std::fabs(1. - exper_cov.log_determinant() / std::log(6.75)),
            1.0e-12 / 100.);
...
void test_mixed_scalar_diagonal_full_block_covariance_matrix() {
  ...
  EXPECT_LT(std::fabs(1. - exper_cov.log_determinant() / std::log(432.0)),
            1.0e-12 / 100.);
  ...
  // gradient vectors
  RealMatrix scaled_grads;
  Real grads_array[] = {1., 2., 1., 2., 2., 4., 4., 8., 1.,
                        2., 2., 4., 4., 8., 2., 4., 4., 8.};
  ...
  RealSymMatrix hessian_1(hessian_0), hessian_2(hessian_0),
      hessian_3(hessian_0), hessian_4(hessian_0), hessian_5(hessian_0),
      hessian_6(hessian_0), hessian_7(hessian_0), hessian_8(hessian_0);
  hessian_1 *= 1.;
  hessian_2 *= 2.;
  hessian_3 *= 4.;
  hessian_4 *= 1.;
  ...
  hessians[1] = hessian_1;
  hessians[2] = hessian_2;
  hessians[3] = hessian_3;
  hessians[4] = hessian_4;
  hessians[5] = hessian_5;
  hessians[6] = hessian_6;
  hessians[7] = hessian_7;
  ...
  EXPECT_TRUE(
      (calc_correl.normInf() < 10. * std::numeric_limits<double>::epsilon()));
}
...
void test_linear_interpolate_1d_no_extrapolation() {
  ...
  Real diff_array[] = {-2.16574074074074e-1, 1.8e-1,
                       3.91261574074074e-1,  4.29259259259259e-1,
                       3.17084548104956e-1,  0.0};
  ...
  EXPECT_TRUE(
      (true_grads.normInf() < 10. * std::numeric_limits<double>::epsilon()));
...
void test_linear_interpolate_1d_with_extrapolation() {
  ...
  int num_field_pts = 6;
  Real field_pts_array[] = {-1. / 6., 2. / 5., 7. / 12.,
                            2. / 3.,  6. / 7., 11. / 10.};
  RealMatrix field_pts(Teuchos::View, field_pts_array, 1, num_field_pts, 1);
  // Generate field data which is the 3rd degree Legendre polynomial
  // 1/2(5x^3-3x)
  ...
  Real diff_array[] = {-7.38425925925926e-1, 1.8e-1,
                       3.91261574074074e-1,  4.29259259259259e-1,
                       3.17084548104956e-1,  -6.775e-1};
  RealVector diff(Teuchos::View, diff_array, num_field_pts);
  diff -= interp_vals;
  EXPECT_TRUE((diff.normInf() < 10. * std::numeric_limits<double>::epsilon()));
}
...
void test_symmetric_eigenvalue_decomposition() {
  ...
  RealMatrix truth_eigenvectors(Teuchos::View, truth_eigenvectors_array, 2, 2,
                                2);
...
void test_get_positive_definite_covariance_from_hessian() {
  ...
  NonDBayesCalibration::get_positive_definite_covariance_from_hessian(
      misfit_hessian1, prior_L1, covariance1, NORMAL_OUTPUT);
  ...
  NonDBayesCalibration::get_positive_definite_covariance_from_hessian(
      misfit_hessian2, prior_L2, covariance2, NORMAL_OUTPUT);
...
void test_matrix_symmetry() {
  ...
  Real matrix_array[] = {1.0, 0.7, 0.7, 0.7, 0.7, 1.0, 0.7, 0.7,
                         0.7, 0.7, 1.0, 0.7, 0.7, 0.7, 0.7, 1.0};
