...
class RolTesterInterface : public DirectApplicInterface {
  ...
  int derived_map_ac(const Dakota::String& ac_name) {
    ...
    ROL::ZOO::getSimpleEqConstrained<Real, ROL::StdVector<Real>,
                                     ...
                                     ROL::StdVector<Real>>(obj, eq_cons, x0,
                                                           sol);
    ...
    if (ac_name == "simple_eq_constrained") {
      ...
      //      fail_code = simple_quad(xC, directFnASV[0], fnVals[0], fn_grad,
      //      fn_hess);
      ...
      Real tol = 0.0;
      if (directFnASV[0] & 2) fnVals[0] = obj->value(x_curr, tol);
      if (directFnASV[0] & 2) {
        Teuchos::RCP<std::vector<Real>> obj_grad_rcp(
            new std::vector<Real>(5, 0.0));
      ...
      if (true) {
        ...
        for (int i = 0; i < 5; ++i) {
          ...
          for (int j = 0; j < 3; ++i) fnGrads(i, 1 + j) = (*cons_grad_rcp)[j];
        }
      }
