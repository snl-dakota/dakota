...
TEST(rank_1_lattice_tests, lattice_check_points) {
  ...
  std::vector<std::vector<double> > exact = {
      {0, 0},         {0.5, 0.5},     {0.25, 0.75},   {0.75, 0.25},
      {0.125, 0.375}, {0.625, 0.875}, {0.375, 0.125}, {0.875, 0.625}};
...
TEST(rank_1_lattice_tests, lattice_check_throws_mMax) {
  ...
  Dakota::UInt32Vector generatingVector(Teuchos::View,
                                        Dakota::cools_kuo_nuyens_d250_m20, 250);

  // Check that mMax < 1 throws an exception
  EXPECT_THROW(Dakota::Rank1Lattice(generatingVector, -1), std::system_error);
}
...
TEST(rank_1_lattice_tests, lattice_check_throws_dMax) {
  ...
  Dakota::UInt32Vector generatingVector(Teuchos::View,
                                        Dakota::cools_kuo_nuyens_d250_m20, 0);

  // Check that dMax < 1 throws an exception
  EXPECT_THROW(Dakota::Rank1Lattice(generatingVector, 20), std::system_error);
}
...
TEST(rank_1_lattice_tests, lattice_check_throws_seedValue) {
  ...
  // Check that dMax < 1 throws an exception
  EXPECT_THROW(Dakota::Rank1Lattice(-1), std::system_error);
}
...
TEST(rank_1_lattice_tests, lattice_test_check_sizes) {
  ...
  // Define a generating vector
  Dakota::UInt32Vector generatingVector(Teuchos::View,
                                        Dakota::cools_kuo_nuyens_d250_m20, 10);

  /// Define a lattice
  Dakota::Rank1Lattice lattice(generatingVector, 2, true, 17,
  ...
  EXPECT_THROW(lattice.get_points(points), std::system_error);
  ...
  EXPECT_THROW(lattice.get_points(0, 3, points), std::system_error);
...
TEST(rank_1_lattice_tests, lattice_check_throws_mMax_in_input_file) {
  ...
  EXPECT_THROW(Dakota::Opt_TPL_Test::create_env(dakota_input),
               std::system_error);
}
...
TEST(rank_1_lattice_tests, lattice_check_throws_max_dimension_exceeded) {
  ...
  // Define a generating vector
  Dakota::UInt32Vector generatingVector(Teuchos::View,
                                        Dakota::cools_kuo_nuyens_d250_m20, 6);
...
TEST(rank_1_lattice_tests, lattice_check_throws_mismatched_samples_matrix) {
  ...
  EXPECT_THROW(lattice.get_points(10, 21, points), std::system_error);
}
...
TEST(rank_1_lattice_tests, lattice_check_random_seed_lattice) {
  ...
  for (size_t row = 0; row < numPoints; row++) {
    for (size_t col = 0; col < 2; col++) {
      ...
      EXPECT_LT(std::fabs(1. - points_17a[row][col] / points_17b[row][col]),
                1e-4 / 100.);
...
TEST(rank_1_lattice_tests, lattice_check_disabled_randomization) {
  ...
  // Exact lattice points
  std::vector<std::vector<double> > exact = {{0, 0}, {0.5, 0.5}, {0.25, 0.75}};
...
TEST(rank_1_lattice_tests, lattice_check_lattice_negative_seed) {
  ...
  EXPECT_THROW(Dakota::Rank1Lattice lattice(-1), std::system_error);
}
