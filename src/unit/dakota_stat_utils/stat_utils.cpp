...
TEST(stat_utils_tests, test_stat_utils_system_seed) {
  ...
  for (unsigned int i = 0; i < 1000; ++i) {
    ...
    if (seed < 1 || seed > 1000000) ++num_out_of_bounds_seeds;
...
TEST(stat_utils_tests, test_stat_utils_batch_means_mean) {
  ...
  EXPECT_LT(std::fabs(1. - interval_matrix[0][0] / gold_lower_int),
            1.e-3 / 100.);
  EXPECT_LT(std::fabs(1. - interval_matrix[0][1] / gold_upper_int),
            1.e-3 / 100.);
}
