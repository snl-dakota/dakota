...
TEST(hdf5_test_serial_tests, test_tpl_hdf5_test_serial) {
  ...
  try {
    ...
    DataSet dataset(
        file.createDataSet(DATASET_NAME, PredType::STD_I32BE, dataspace));
    ...
    for (j = 0; j < DIM0; j++) {
      for (i = 0; i < DIM1; i++) cout << " " << data[j][i];
      cout << endl;
    }
    ...
    for (j = 0; j < DIM0_SUB; j++) {
      for (i = 0; i < DIM1_SUB; i++) sdata[j][i] = 5;
    }
    ...
    for (i = 0; i < DIM0; i++) {
      for (j = 0; j < DIM1; j++) cout << " " << rdata[i][j];
      cout << endl;
    }
    cout << endl;
