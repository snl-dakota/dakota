...
void create_field_data_file(const std::string &filename,
                            const RealVectorArray &field_data) {
...
void remove_field_data_files() {
  for (size_t i = 0; i < used_filenames.size(); ++i)
    if (remove(used_filenames[i].c_str()) != 0)
      std::cout << "Error removing test data file \"" << used_filenames[i]
                << "\"" << std::endl;
}
...
class Cleanup_Helper {
 public:
  ...
  ~Cleanup_Helper() { remove_field_data_files(); }
};

}  // namespace

// Automatically deletes generated test data files
// Could make deletion optional to allow the files to remain if running with
// verbosity enabled
...
TEST(file_reader_tests, test_read_sized_data) {
  ...
  for (size_t i = 0; i < test_vec_array.size(); ++i)
    for (int j = 0; j < test_vec_array[i].length(); ++j)
      EXPECT_LT(std::fabs(1. - field_data[i][j] / test_vec_array[i][j]),
                1.e-12 / 100.);
}
...
TEST(file_reader_tests, test_read_sigma_scalar) {
  ...
  // Verify contents of what we wrote and what we read
  EXPECT_LT(std::fabs(1. - field_data[0][0] / test_vec_array[0][0]),
            1.e-12 / 100.);
}
...
TEST(file_reader_tests, test_read_sigma_vector_row) {
  ...
  std::ifstream in_file;
  TabularIO::open_file(in_file, filename,
                       "unit test test_sigma_vector_row_data");
...
TEST(file_reader_tests, test_read_sigma_vector_col) {
  ...
  std::ifstream in_file;
  TabularIO::open_file(in_file, filename,
                       "unit test test_sigma_vector_col_data");
