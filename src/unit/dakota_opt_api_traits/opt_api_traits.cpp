...
bool check_variables(unsigned short methodName, int num_cont_vars,
                     int num_disc_int_vars, int num_disc_real_vars,
                     int num_disc_string_vars, bool &continuous_only) {
  if (methodName == MOGA || methodName == SOGA || methodName == COLINY_EA ||
      methodName == SURROGATE_BASED_GLOBAL || methodName == COLINY_BETA ||
      methodName == MESH_ADAPTIVE_SEARCH ||
      methodName == ASYNCH_PATTERN_SEARCH || methodName == BRANCH_AND_BOUND) {
    continuous_only = false;
    if (!num_cont_vars && !num_disc_int_vars && !num_disc_real_vars &&
        !num_disc_string_vars)
      return false;
  } else {  // methods supporting only continuous design variables
    continuous_only = true;
    if (!num_cont_vars) return false;
  }
  return true;
}
...
bool check_variables(std::shared_ptr<TraitsBase> traits, int num_cont_vars,
                     int num_disc_int_vars, int num_disc_real_vars,
                     int num_disc_string_vars, bool &continuous_only) {
  ...
  for (int i = 0; i < 2; ++i)
    for (int j = 0; j < 2; ++j)
      for (int k = 0; k < 2; ++k)
        for (int l = 0; l < 2; ++l) {
          is_consistent_enums =
              check_variables(methodName, i, j, k, l, continuous_only_enum);
          is_consistent_traits = check_variables(method_iter->traits(), i, j, k,
                                                 l, continuous_only_traits);
...
#ifdef DAKOTA_HOPS
  check_variable_consistency(ASYNCH_PATTERN_SEARCH,
                             std::shared_ptr<TraitsBase>(new AppsTraits()));
#endif
#ifdef HAVE_JEGA
  check_variable_consistency(MOGA,
                             std::shared_ptr<TraitsBase>(new JEGATraits()));
  check_variable_consistency(SOGA,
                             std::shared_ptr<TraitsBase>(new JEGATraits()));
#endif
...
#ifdef HAVE_NOMAD
  check_variable_consistency(MESH_ADAPTIVE_SEARCH,
                             std::shared_ptr<TraitsBase>(new NomadTraits()));
#endif
#ifdef HAVE_ACRO
  check_variable_consistency(BRANCH_AND_BOUND,
                             std::shared_ptr<TraitsBase>(new PebbldTraits()));
#endif
}
