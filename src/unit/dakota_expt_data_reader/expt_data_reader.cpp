...
TEST(expt_data_reader_tests, test_expt_data_reader_read_bad_config_vars_size) {
  ...
  EXPECT_THROW(
      read_config_vars_multifile(base_name, NUM_EXPTS, NCV + 1, config_data),
      std::runtime_error);
  EXPECT_THROW(
      read_config_vars_singlefile(base_name, NUM_EXPTS, NCV + 1, config_data),
      std::runtime_error);
...
TEST(expt_data_reader_tests, test_expt_data_reader_read_field_values_scalar) {
  ...
  for (size_t i = 0; i < NUM_FIELD_VALUES; ++i) {
    EXPECT_LT(std::fabs(1. - field_vals_array[0][i] / gold_values[i]),
              1.e-12 / 100.);
...
TEST(expt_data_reader_tests, test_expt_data_reader_read_field_values_vector) {
  ...
  for (int i = 0; i < VECTOR_FIELD_DIM; ++i)
    for (int j = 0; j < NUM_FIELD_VALUES; ++j)
      EXPECT_LT(
          std::fabs(1. - field_vals[i][j] /
                             (double(j + 1) + double(j + 1) * pow(0.1, i))),
          1.e-12 / 100.);
}
...
TEST(expt_data_reader_tests, test_expt_data_reader_read_coord_values) {
  ...
  for (int i = 0; i < VECTOR_FIELD_DIM; ++i)
    for (int j = 0; j < NUM_FIELD_VALUES; ++j)
      EXPECT_LT(
          std::fabs(1. - coords[i][j] / (double(i) + double(j + 1) * 0.1)),
          1.e-12 / 100.);
}
