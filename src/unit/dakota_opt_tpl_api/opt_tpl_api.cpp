...
TEST(opt_tpl_api_tests, test_hopspack) {
  Dakota::LibraryEnvironment* p_env =
      Opt_TPL_Test_Fixture::create_default_env(Dakota::ASYNCH_PATTERN_SEARCH);
  ...
  // dynamic_cast<Dakota::Optimizer*>(dak_iter.iterator_rep());
  ...
  EXPECT_LT(std::fabs(1. - vars.continuous_variable(0) / -1.50),
            max_tol / 100.);
  ...
  // resp.write(Cout);
  EXPECT_LT(std::fabs(1. - (resp.function_value(0) + 1.0) / 1.0),
            max_tol / 100.);
}
...
TEST(opt_tpl_api_tests, test_optpp) {
  Dakota::LibraryEnvironment* p_env =
      Opt_TPL_Test_Fixture::create_default_env(Dakota::OPTPP_PDS);
  Dakota::LibraryEnvironment& env = *p_env;
  ...
  EXPECT_LT(std::fabs(1. - vars.continuous_variable(0) / -1.50),
            0.6 / 100. /* not so good tol */);
  EXPECT_LT(std::fabs(1. - vars.continuous_variable(1) / 0.75),
            2.0 / 100. /* ditto */);
...
// {
//   Dakota::LibraryEnvironment * p_env =
//   Opt_TPL_Test_Fixture::create_default_env(Dakota::NLPQL_SQP);
//   Dakota::LibraryEnvironment & env = *p_env;
