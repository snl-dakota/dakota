...
std::pair<RealMatrix, RealMatrix> get_parameter_and_response_submatrices() {
  ...
  if (param_sub_mat.empty() || response_sub_mat.empty()) {
    ...
    // Create submatrix consisting of the two input parameter columns
    param_sub_mat =
        RealMatrix(Teuchos::Copy, test_mat, test_mat.numRows(), 2, 0, 1);

    // Create submatrix consisting of the 50 response columns
    response_sub_mat =
        RealMatrix(Teuchos::Copy, test_mat, test_mat.numRows(), 50, 0, 3);
  }
...
TEST(reduced_bases_tests, test_reduced_basis_compute_col_means) {
  ...
  EXPECT_TRUE((column_means.length() == 2));
  EXPECT_LT(std::fabs(1. - column_means[0] / 0.299958060862), 1.e-10 / 100.);
  EXPECT_LT(std::fabs(1. - column_means[1] / 0.299814773583), 1.e-10 / 100.);
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_sort_vectors) {
  RealMatrix matrix = get_parameter_and_response_submatrices().first;
  ...
  for (int i = 0; i < matrix.numCols(); ++i) {
    ...
    EXPECT_TRUE((!std::is_sorted(unsrt_vec.values(),
                                 unsrt_vec.values() + unsrt_vec.length())));

    // --------------- What we are testing
    sort_vector(Teuchos::getCol(Teuchos::View, matrix, i), sorted_vector,
                sorted_indices);
    ...
    EXPECT_TRUE(
        (std::is_sorted(sorted_vector.values(),
                        sorted_vector.values() + sorted_vector.length())));
    // but indices reflecting the sorting order should not be sorted
    EXPECT_TRUE(
        (!std::is_sorted(sorted_indices.values(),
                         sorted_indices.values() + sorted_indices.length())));
  }
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_sort_matrix_cols) {
  RealMatrix matrix = get_parameter_and_response_submatrices().first;
  ...
  for (int i = 0; i < matrix.numCols(); ++i) {
    const RealVector& unsrt_vec = Teuchos::getCol(Teuchos::View, matrix, i);
    const RealVector& sorted_vec =
        Teuchos::getCol(Teuchos::View, sorted_matrix, i);
    const IntVector& sort_indices =
        Teuchos::getCol(Teuchos::View, sorted_indices, i);

    // starting column vector should be unsorted
    EXPECT_TRUE((!std::is_sorted(unsrt_vec.values(),
                                 unsrt_vec.values() + unsrt_vec.length())));
    // sorted matrix column should be ... well, sorted
    EXPECT_TRUE((std::is_sorted(sorted_vec.values(),
                                sorted_vec.values() + sorted_vec.length())));
    // but indices reflecting the sorting order should not be sorted
    EXPECT_TRUE((!std::is_sorted(
        sort_indices.values(), sort_indices.values() + sort_indices.length())));
  }
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_compute_svd) {
  RealMatrix matrix = get_parameter_and_response_submatrices().first;
  ...
  EXPECT_TRUE((singular_values.length() == 2));
  EXPECT_LT(std::fabs(1. - singular_values[0] / 4.39718), 1.e-3 / 100.);
  EXPECT_LT(std::fabs(1. - singular_values[1] / 1.14583), 1.e-3 / 100.);
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_simple_api) {
  RealMatrix matrix = get_parameter_and_response_submatrices().first;
  ...
  const RealMatrix& VT_mat =
      reduced_basis.get_right_singular_vector_transpose();

  RealMatrix S(2, 2);
  S(0, 0) = singular_values(0);
  S(1, 1) = singular_values(1);

  // Reconstruct matrix using SVD and compare to original (implies original
  // matrix does not get changed).
  RealMatrix reconstructed_mat(matrix.numRows(), matrix.numCols());
  int ierr = reconstructed_mat.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS,
                                        1.0, U_mat, S, 0.0);
  RealMatrix temp_matrix(reconstructed_mat);
  ierr += reconstructed_mat.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0,
                                     temp_matrix, VT_mat, 0.0);
  EXPECT_TRUE((ierr == 0));

  reconstructed_mat -= matrix;
  Real diff = 1.0 + reconstructed_mat.normFrobenius();
  EXPECT_LT(std::fabs(1. - diff / 1.0), 1.e-12 / 100.);
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_simple_api2) {
  ...
  EXPECT_TRUE((singular_values1.length() == 2));
  EXPECT_LT(std::fabs(1. - singular_values1[0] / 1.161389978), 1.e-4 / 100.);
  EXPECT_LT(std::fabs(1. - singular_values1[1] / 1.145814014), 1.e-4 / 100.);
  EXPECT_TRUE((singular_values1.length() == singular_values2.length()));
  for (int i = 0; i < singular_values1.length(); ++i)
    EXPECT_LT(std::fabs(1. - singular_values1[i] / singular_values2[i]),
              1.e-4 / 100.);
}
...
TEST(reduced_bases_tests, test_reduced_basis_gp_surr0) {
  ...
  eval_vars(0) = -1.0;
  eval_vars(1) = 1.0;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / 7.0), 1.e-6 / 100.);
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_spectral_sums) {
  ...
  EXPECT_LT(std::fabs(1. - eigen_sum / 86.00739691478532),
            1.e-12 / 100.);  // from Matlab eig
  EXPECT_LT(std::fabs(1. - computed_eigen_sum / eigen_sum), 1.e-6 / 100.);
  EXPECT_LT(std::fabs(1. - computed_sval_sum / sval_sum), 1.e-6 / 100.);
}

//----------------------------------------------------------------

TEST(reduced_bases_tests, test_reduced_basis_truncations) {
  // Use the response submatrix
  RealMatrix matrix = get_parameter_and_response_submatrices().second;
  ...
  // Test lengths
  EXPECT_TRUE((num_values == 50));
  EXPECT_TRUE((num_values1 == 50));
  EXPECT_TRUE((num_values2 == 5));
  EXPECT_TRUE((num_values3 == 4));
  EXPECT_TRUE((num_values4 == 5));

  Real ratio3 = 0.0;
  for (int i = 0; i < num_values3; ++i)
    ratio3 += singular_values3(i) * singular_values3(i);
  ratio3 /= reduced_basis.get_eigen_values_sum();
  EXPECT_LT(std::fabs(1. - ratio3 / 0.9994559642736607), 1.e-10 / 100.);

  Real ratio4 = singular_values(num_values4 - 1) *
                singular_values(num_values4 - 1) /
  ...
  EXPECT_LT(std::fabs(1. - test_val / matlab_based_value), 1.e-10 / 100.);
}
...
// test construction and evaluation of a GP surrogate from data
// matrices; one approximation per response
TEST(reduced_bases_tests, test_reduced_basis_gp_surr_module0) {
  ...
  // configure the surrogate
  String approx_type(
      "global_exp_gauss_proc");  // Tom's new GP from surrogate module
  UShortArray approx_order;
  short data_order = 1;  // assume only function values
  short output_level = Dakota::QUIET_OUTPUT;
  SharedApproxData shared_approx_data(approx_type, approx_order, num_vars,
  ...
  RealVector eval_vars(
      num_vars);  // Dakota::Approximation only supports single eval pt
  eval_vars(0) = 0.4;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / -0.0333528),
            1.e-4 / 100.);
  eval_vars(0) = 1.0;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / -0.0506785),
            1.5e-4 / 100.);
}
...
// matrices
TEST(reduced_bases_tests, test_reduced_basis_gp_surr_module1) {
  // Set things up to correspond to the unit test in
  // src/surrogates/unit/gp_approximation_ts.cpp
  ...
  eval_vars(0) = 0.20;
  eval_vars(1) = 0.45;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / 0.782031),
            100.0 * tol / 100.);

  eval_vars(0) = -0.30;
  eval_vars(1) = -0.70;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / 0.847901),
            100.0 * tol / 100.);

  eval_vars(0) = 0.40;
  eval_vars(1) = -0.10;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / 0.744622),
            100.0 * tol / 100.);

  eval_vars(0) = -0.25;
  eval_vars(1) = 0.33;
  EXPECT_LT(std::fabs(1. - gp_approx.value(eval_vars) / 0.745461),
            100.0 * tol / 100.);
}
