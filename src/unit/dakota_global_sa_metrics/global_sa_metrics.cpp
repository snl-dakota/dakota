...
MatrixXd create_samples() {
  ...
  samplesXd << 0.680375, -0.967399, 0.05349, -0.52344, -0.211234, -0.514226,
      0.539828, 0.941268, 0.566198, -0.725537, -0.199543, 0.804416, 0.59688,
      0.608354, 0.783059, 0.70184, 0.823295, -0.686642, -0.433371, -0.466669,
      -0.604897, -0.198111, -0.295083, 0.0795207, -0.329554, -0.740419,
      0.615449, -0.249586, 0.536459, -0.782382, 0.838053, 0.520497, -0.444451,
      0.997849, -0.860489, 0.0250707, 0.10794, -0.563486, 0.898654, 0.335448,
      -0.0452059, 0.0258648, 0.0519907, 0.0632129, 0.257742, 0.678224,
      -0.827888, -0.921439, -0.270431, 0.22528, -0.615572, -0.124725, 0.0268018,
      -0.407937, 0.326454, 0.86367, 0.904459, 0.275105, 0.780465, 0.86162,
      0.83239, 0.0485744, -0.302214, 0.441905, 0.271423, -0.012834, -0.871657,
      -0.431413, 0.434594, 0.94555, -0.959954, 0.477069, -0.716795, -0.414966,
      -0.0845965, 0.279958, 0.213938, 0.542715, -0.873808, -0.291903;
...
TEST(global_sa_matrices_tests, test_standard_reg_coeffs) {
  ...
  /////////////////////  What we want to test --> Reg. Coeffs. (not
  /// standardized)
  RealMatrix rcoeffs;
  RealVector cods;
  ...
  // ... should be 1.0 because we fit the surrogate using exact polynomial
  // objective values
  EXPECT_LT(std::fabs(1. - cods(0) / 1.0), 1.e-13 / 100.);
  // double polynomial_intercept = pr.get_polynomial_intercept(); // not used or
  // needed? - RWH
...
TEST(global_sa_matrices_tests, test_standard_reg_coeffs_multi_resp) {
  ...
  gold_srcs << bad, 0.996027, 0.996027, bad, 0.122718, 0.122718, bad, 0.0131245,
      0.0131245, bad, 0.0010858, 0.0010858;
...
class SensAnalysisGlobalTest : public SensAnalysisGlobal {
 public:
  void compute_binned_sobol_indices_from_valid_samples(
      const RealMatrix& valid_samples, size_t n_bins) {
    return SensAnalysisGlobal::compute_binned_sobol_indices_from_valid_samples(
        valid_samples, n_bins);
  }
...
class SobolG {
 ...
 public:
  ...
  SobolG(const Eigen::ArrayXd& a_in) : a(a_in.size()) {
    a = a_in;
    compute_analytical_main_effects();
  }
  ...
  Eigen::ArrayXXd generate_input_samples(int n_samples) {
    // Eigen's Random method generates samples from U[-1,1], so we add 1 and
    // scale by 0.5 to generate samples from U[0,1].
