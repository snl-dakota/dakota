...
TEST(rol_integration_tests, test_rol_basic1) {
  ...
  try {
    ...
    RCP<std::vector<RealT> > lu_rcp =
        rcp(new std::vector<RealT>(1, ROL::ROL_INF<RealT>()));
    ...
    RCP<ROL::BoundConstraint<RealT> > bnd =
        rcp(new ROL::Bounds<RealT>(lower, upper));
    RCP<ROL::Constraint<RealT> > ineq = rcp(new InequalityQL<RealT>());
    RCP<ROL::BoundConstraint<RealT> > ibnd =
        rcp(new ROL::Bounds<RealT>(ll, lu));

    ROL::OptimizationProblem<RealT> problem(obj, x, bnd, ineq, li, ibnd);

    /* checkAdjointJacobianConsistency fails for the OptimizationProblem if we
     * don't do this first... why? */
...
TEST(rol_integration_tests, test_rol_text_book_nln_ineq_const) {
  ...
  Teuchos::RCP<std::ostream> outStream = Teuchos::rcp(&rol_diags, false);
