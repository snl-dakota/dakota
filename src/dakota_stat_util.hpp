...
void compute_regression_coeffs(const RealMatrix& samples,
                               const RealMatrix& resps, RealMatrix& coeffs,
                               RealVector& cods);
...
ScalarType sum(const ScalarType* ptr, OrdinalType ptr_len) {
  ScalarType sum = 0.;
  for (OrdinalType i = 0; i < ptr_len; ++i) sum += ptr[i];
  return sum;
}
...
Real average(const ScalarType* ptr, OrdinalType ptr_len) {
  switch (ptr_len) {
    ...
    case 1:
      ...
      break;
    default:
      return (Real)sum(ptr, ptr_len) / (Real)ptr_len;
      break;
  }
}
...
inline void average(const RealMatrix& mat, size_t avg_index,
  ...
  switch (avg_index) {
    case 0:  // average over index 0, retaining index 1
      ...
      for (i = 0; i < nc; ++i) avg_vec[i] = average(mat[i], nr);
      break;
    case 1:  // average over index 1 (cols), retaining index 0 (rows)
      ...
      switch (nc) {
        case 0:
          for (i = 0; i < nr; ++i)
            avg_vec[i] = std::numeric_limits<Real>::quiet_NaN();
          break;
        case 1:
...
inline void average(const Sizet2DArray& N_2D, RealVector& N_1D) {
  ...
  for (i = 0; i < len; ++i) N_1D[i] = average(N_2D[i]);
}

inline Real power_sum(const RealVector& vec, Real p) {
  Real sum = 0.;
  size_t i, len = vec.length();
  for (i = 0; i < len; ++i) sum += std::pow(std::abs(vec[i]), p);
  return sum;
}
