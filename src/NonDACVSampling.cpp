...
static const char rcsId[] =
    "@(#) $Id: NonDACVSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDACVSampling::NonDACVSampling(ProblemDescDB& problem_db,
                                 ParallelLibrary& parallel_lib,
                                 std::shared_ptr<Model> model)
    : NonDNonHierarchSampling(problem_db, parallel_lib,
                              model)  //, multiStartACV(true)
{
  ...
  for (size_t i = 0; i < numApprox; ++i) fullApproxSet[i] = i;
...
void NonDACVSampling::approximate_control_variate_online_pilot() {
  ...
  while (numSamples && mlmfIter <= maxIterations) {
    ...
    N_H_alloc +=
        (backfillFailures && mlmfIter)
            ? one_sided_delta(N_H_alloc, acvSolnData.solution_reference(),
                              relaxFactor)
            : numSamples;
  ...
  } else  // N_H is final --> do not compute any deltaNActualHF (from maxIter
          // exit)
...
void NonDACVSampling::approximate_control_variate_offline_pilot() {
  ...
  compute_LH_statistics(sum_L_pilot[1], sum_H_pilot[1], sum_LL_pilot[1],
                        sum_LH_pilot[1], sum_HH_pilot, N_pilot, varH, covLL,
                        covLH);
...
void NonDACVSampling::approximate_control_variate_pilot_projection() {
  ...
  SizetArray N_L_alloc;  // inflated from N_H
...
void NonDACVSampling::evaluate_pilot(RealMatrix& sum_L_pilot,
                                     RealVector& sum_H_pilot,
                                     RealSymMatrixArray& sum_LL_pilot,
                                     RealMatrix& sum_LH_pilot,
                                     RealVector& sum_HH_pilot,
...
void NonDACVSampling::evaluate_pilot(IntRealMatrixMap& sum_L_pilot,
                                     IntRealVectorMap& sum_H_pilot,
                                     IntRealSymMatrixArrayMap& sum_LL_pilot,
                                     IntRealMatrixMap& sum_LH_pilot,
                                     RealVector& sum_HH_pilot,
                                     SizetArray& N_pilot, bool incr_cost) {
  initialize_acv_sums(sum_L_pilot, sum_H_pilot, sum_LL_pilot, sum_LH_pilot,
                      sum_HH_pilot);
...
void NonDACVSampling::compute_LH_statistics(
    RealMatrix& sum_L_pilot, RealVector& sum_H_pilot,
    RealSymMatrixArray& sum_LL_pilot, RealMatrix& sum_LH_pilot,
    RealVector& sum_HH_pilot,
    SizetArray& N_pilot,  // RealMatrix& var_L,
    RealVector& var_H, RealSymMatrixArray& cov_LL, RealMatrix& cov_LH,
...
void NonDACVSampling::approx_increments(IntRealMatrixMap& sum_L_baseline,
  ...
  update_model_groups(r_approx_sequence);
  update_model_group_costs();
...
void NonDACVSampling::overlay_peer_group_sums(
...
void NonDACVSampling::compute_acv_controls(
...
void NonDACVSampling::solve_for_C_F_c_f(RealSymMatrix& C_F, RealVector& c_f,
                                        RealVector& lhs, bool copy_C_F,
                                        bool copy_c_f) {
  ...
  if (mlmfIter == 0) {
    ...
    switch (optSubProblemSolver) {
      ...
      default: {
        ...
        // CVMC) for best initial guess, where each initial guess may
        // additionally employ multiple varianceMinimizers in
        // ensemble_numerical_solution()
        RealMatrix rho2_LH;
...
void NonDACVSampling::analytic_initialization_from_mfmc(
    const RealMatrix& rho2_LH, Real avg_N_H, MFSolutionData& soln) {
...
void NonDACVSampling::analytic_initialization_from_ensemble_cvmc(
    const RealMatrix& rho2_LH, Real avg_N_H, MFSolutionData& soln) {
...
void NonDACVSampling::analytic_ratios_to_solution_variables(
    RealVector& avg_eval_ratios, Real avg_N_H, MFSolutionData& soln) {
  Real hf_target;
  if (maxFunctionEvals ==
      SZ_MAX) {  // HF tgt from ACV estvar using analytic soln
    hf_target =
  ...
  } else  // allocate_budget(), then manage lower bounds and pilot
          // over-estimation
...
void NonDACVSampling::print_model_allocations(std::ostream& s,
                                              const MFSolutionData& soln,
                                              const UShortArray& approx_set) {
  const RealVector& soln_vars = soln.solution_variables();
  size_t i, num_approx = approx_set.size();
  for (i = 0; i < num_approx; ++i)
    s << "Approx " << approx_set[i] + 1
      << ": sample allocation = " << soln_vars[i] << '\n';
...
void NonDACVSampling::accumulate_acv_sums(
...
void NonDACVSampling::accumulate_acv_sums(
    RealMatrix& sum_L_baseline, RealVector& sum_H,
...
void NonDACVSampling::accumulate_acv_sums(IntRealMatrixMap& sum_L,
                                          Sizet2DArray& N_L_actual,
...
void NonDACVSampling::accumulate_hf_qoi(const RealVector& fn_vals, size_t qoi,
                                        IntRealVectorMap& sum_H,
                                        RealVector& sum_HH) {
...
void NonDACVSampling::accumulate_lf_qoi(const RealVector& fn_vals, size_t qoi,
                                        size_t approx,
  ...
  while (l_ord) {
    if (l_ord == active_ord) {  // support general key sequence
      l_it->second(qoi, approx) += lf_prod;
      ++l_it;
    ...
    if (ll_ord == active_ord) {  // support general key sequence
      ...
      for (approx2 = 0; approx2 < approx; ++approx2) {
        ...
        if (asv[lf2_index] & 1) {
          ...
          for (m = 1; m < active_ord; ++m) lf2_prod *= lf2_fn;
...
void NonDACVSampling::accumulate_lf_hf_qoi(const RealVector& fn_vals,
                                           const ShortArray& asv, size_t qoi,
                                           size_t approx,
                                           IntRealMatrixMap& sum_L_shared,
                                           IntRealSymMatrixArrayMap& sum_LL,
  ...
  int ls_ord = (ls_it == sum_L_shared.end()) ? 0 : ls_it->first,
      ll_ord = (ll_it == sum_LL.end()) ? 0 : ll_it->first,
      lh_ord = (lh_it == sum_LH.end()) ? 0 : lh_it->first, active_ord = 1;
  Real lf_fn = fn_vals[lf_index], lf_prod = lf_fn, lf2_fn, lf2_prod,
...
void NonDACVSampling::compute_LH_covariance(const RealMatrix& sum_L_shared,
                                            const RealVector& sum_H,
                                            const RealMatrix& sum_LH,
                                            const SizetArray& N_shared,
                                            RealMatrix& cov_LH,
                                            const UShortArray& approx_set) {
  if (cov_LH.numRows() != numFunctions)
    cov_LH.shape(numFunctions, numApprox);
  else
    cov_LH = 0.;
...
void NonDACVSampling::compute_LL_covariance(
    const RealMatrix& sum_L_shared, const RealSymMatrixArray& sum_LL,
  ...
  if (cov_LL.size() != numFunctions) {
    ...
    for (qoi = 0; qoi < numFunctions; ++qoi) cov_LL[qoi] = 0.;

  Real sum_L_aq;
  size_t N_sh_q, num_approx = approx_set.size();
  for (qoi = 0; qoi < numFunctions; ++qoi) {
...
/** LF and HF */
void NonDACVSampling::update_projected_samples(
    const MFSolutionData& soln, const UShortArray& approx_set,
    const SizetArray& N_H_actual, size_t& N_H_alloc, Sizet2DArray& N_L_actual,
  ...
  size_t alloc_incr = one_sided_delta(N_H_alloc, hf_target),
         actual_incr = (backfillFailures)
                           ? one_sided_delta(average(N_H_actual), hf_target)
                           : alloc_incr;
...
/** LF only */
void NonDACVSampling::update_projected_lf_samples(
    const MFSolutionData& soln, const UShortArray& approx_set,
    const SizetArray& N_H_actual, size_t N_H_alloc, Sizet2DArray& N_L_actual,
    SizetArray& N_L_alloc,
    // SizetArray& delta_N_L_actual,
  ...
  for (i = 0; i < num_approx; ++i) {
    ...
    actual_incr = (backfillFailures)
                      ? one_sided_delta(average(N_L_actual_a), lf_target)
                      : alloc_incr;
...
void NonDACVSampling::inflate_lf_samples(const UShortArray& approx_set,
                                         const SizetArray& N_H_actual,
                                         size_t N_H_alloc,
                                         Sizet2DArray& N_L_actual,
                                         SizetArray& N_L_alloc) {
