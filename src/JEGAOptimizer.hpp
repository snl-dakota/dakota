 ...
 */
class JEGAOptimizer : public Optimizer {
  /*
  ===========================================================================
 ...
 private:
  ...
  // JEGA's global logger when it reaches 0. The logger keeps JEGAGlobal.log
  // open until process termination, which can cause problems when Dakota is
  // used in library mode, such as with the environment Python binding, because
  // it outlives the environment. See the destructor.
 ...
 protected:
   ...
   */
  void LoadDakotaResponses(const JEGA::Utilities::Design& from, Variables& vars,
                           Response& resp) const;
 ...
 public:
  ...
  void initial_points(const VariablesArray& pts) override;
 ...
 public:
   ...
   */
  JEGAOptimizer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
