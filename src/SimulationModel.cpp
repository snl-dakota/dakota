...
static const char rcsId[] =
    "@(#) $Id: SimulationModel.cpp 6492 2009-12-19 00:04:28Z briadam $";
...
SimulationModel::SimulationModel(ProblemDescDB& problem_db,
    ...
    : Model(problem_db, parallel_lib),
      userDefinedInterface(Interface::get_interface(problem_db, parallel_lib)),
      solnCntlVarType(EMPTY_TYPE),
      solnCntlADVIndex(_NPOS),
      solnCntlAVIndex(_NPOS),
      costMetadataIndex(_NPOS),
      simModelEvalCntr(0) {
...
void SimulationModel::initialize_solution_recovery(const String& cost_label) {
  // returns _NPOS if no metadata label match
  costMetadataIndex =
      find_index(currentResponse.shared_data().metadata_labels(), cost_label);
}

void SimulationModel::initialize_solution_control(const String& control,
  ...
  if (control.empty()) {
    ...
    switch (cost_len) {
      case 0:  // invalid cost of 0 must be replaced by online recovery
        solnCntlCostMap.insert(std::pair<Real, size_t>(0., _NPOS));
        break;
  ...
  solnCntlADVIndex =
      find_index(currentVariables.all_discrete_int_variable_labels(), control);
  if (solnCntlADVIndex != _NPOS) {
    solnCntlAVIndex = svd.adiv_index_to_all_index(solnCntlADVIndex);
    solnCntlVarType =
        currentVariables.all_discrete_int_variable_types()[solnCntlADVIndex];
  ...
  switch (solnCntlVarType) {
    ...
    case DISCRETE_STATE_RANGE:
      num_lev = userDefinedConstraints
                    .all_discrete_int_upper_bounds()[solnCntlADVIndex] -
                userDefinedConstraints
                    .all_discrete_int_lower_bounds()[solnCntlADVIndex];
      break;
    case DISCRETE_DESIGN_SET_INT:
    case DISCRETE_STATE_SET_INT:
      num_lev = mvd_rep->pull_parameter_size<IntSet>(solnCntlAVIndex,
                                                     Pecos::DSI_VALUES);
      break;
    case DISCRETE_DESIGN_SET_STRING:
    case DISCRETE_STATE_SET_STRING:
      num_lev = mvd_rep->pull_parameter_size<StringSet>(solnCntlAVIndex,
                                                        Pecos::DSS_VALUES);
      break;
    case DISCRETE_DESIGN_SET_REAL:
    case DISCRETE_STATE_SET_REAL:
      num_lev = mvd_rep->pull_parameter_size<RealSet>(solnCntlAVIndex,
                                                      Pecos::DSR_VALUES);
      break;
...
/* Real */ void SimulationModel::solution_level_cost_index(size_t cost_index) {
  ...
  switch (solnCntlVarType) {
    ...
    case DISCRETE_STATE_RANGE: {
      int val =
          val_index + userDefinedConstraints
                          .all_discrete_int_lower_bounds()[solnCntlADVIndex];
    ...
    case DISCRETE_STATE_SET_INT: {
      ...
      ISIter is_it = is.begin();
      std::advance(is_it, val_index);
    ...
    case DISCRETE_STATE_SET_STRING: {
      StringSet ss;
      mvd_rep->pull_parameter<StringSet>(solnCntlAVIndex, Pecos::DSS_VALUES,
                                         ss);
      SSCIter ss_it = ss.begin();
      std::advance(ss_it, val_index);
    ...
    case DISCRETE_STATE_SET_REAL: {
      ...
      RSIter rs_it = rs.begin();
      std::advance(rs_it, val_index);
    ...
    case DISCRETE_UNCERTAIN_SET_INT: {
      ...
      IRMIter ir_it = irm.begin();
      std::advance(ir_it, val_index);
      currentVariables.all_discrete_int_variable(ir_it->first,
                                                 solnCntlADVIndex);
      break;
    }
    case DISCRETE_UNCERTAIN_SET_STRING: {
      ...
      SRMIter sr_it = srm.begin();
      std::advance(sr_it, val_index);
    ...
    case DISCRETE_UNCERTAIN_SET_REAL: {
      ...
      RRMIter rr_it = rrm.begin();
      std::advance(rr_it, val_index);
      currentVariables.all_discrete_real_variable(rr_it->first,
                                                  solnCntlADVIndex);
      break;
    }
  }
...
RealVector SimulationModel::solution_level_costs() const {
  ...
  std::map<Real, size_t>::const_iterator cit;
  size_t i;
  for (cit = solnCntlCostMap.begin(), i = 0; cit != solnCntlCostMap.end();
       ++cit, ++i)
...
Real SimulationModel::solution_level_cost() const {
  ...
  if (cit == solnCntlCostMap.end())
    return 0.;
  else {
    size_t cost_index = solution_level_cost_index();
    if (cost_index != _NPOS) std::advance(cit, cost_index);
    return cit->first;
  }
}
...
ActiveSet SimulationModel::default_interface_active_set() {
  ...
  if (has_deriv_vars) {
    if (gradientType == "analytic") {
      ...
      for (const auto& gi : gradIdAnalytic) asv[gi - 1] |= 2;
    }

    if (hessianType == "analytic") {
      for (auto& a : asv) a |= 4;
    } else if (hessianType == "mixed") {
      for (const auto& hi : hessIdAnalytic) asv[hi - 1] |= 4;
    }
  }
  set.request_vector(asv);
  return set;
...
void SimulationModel::derived_init_communicators(ParLevLIter pl_iter,
...
void SimulationModel::derived_set_communicators(ParLevLIter pl_iter,
                                                int max_eval_concurrency,
                                                bool recurse_flag) {
  // allow recursion to progress - don't store/set/restore
  parallelLib.parallel_configuration_iterator(modelPCIter);
  userDefinedInterface->set_communicators(messageLengths, max_eval_concurrency);
  set_ie_asynchronous_mode(
      max_eval_concurrency);  // asynchEvalFlag, evalCapacity
}

IntIntPair SimulationModel::estimate_partition_bounds(
    int max_eval_concurrency) {
