...
class MinimizerAdapterModel : public AdapterModel {
 public:
  ...
  MinimizerAdapterModel(
      const RealVector& cv_initial_pt, const RealVector& cv_lower_bnds,
  ...
  void assign_constraints(
      ...
      const RealVector& lin_ineq_upper_bnds, const RealMatrix& lin_eq_coeffs,
      const RealVector& lin_eq_targets, const RealVector& nln_ineq_lower_bnds,
      const RealVector& nln_ineq_upper_bnds, const RealVector& nln_eq_targets);
...
inline void MinimizerAdapterModel::initialize_variables(
    size_t num_cdv)  //, num_div, num_drv, ...
{
  ShortShortPair view(MIXED_DESIGN, EMPTY_VIEW);
  SizetArray vc_totals;
  vc_totals.assign(NUM_VC_TOTALS, 0);
...
inline void MinimizerAdapterModel::assign_variables(
    const RealVector& c_vars)  //, di_vars, dr_vars, ...
{
...
inline void MinimizerAdapterModel::assign_constraints(
    const RealVector& cv_lower_bnds, const RealVector& cv_upper_bnds,
    const RealMatrix& lin_ineq_coeffs, const RealVector& lin_ineq_lower_bnds,
    const RealVector& lin_ineq_upper_bnds, const RealMatrix& lin_eq_coeffs,
    const RealVector& lin_eq_targets, const RealVector& nln_ineq_lower_bnds,
    const RealVector& nln_ineq_upper_bnds, const RealVector& nln_eq_targets) {
  if (userDefinedConstraints.is_null()) initialize_constraints();
