...
class FileReadException : public std::runtime_error {
 public:
  FileReadException(const std::string& msg)
      : std::runtime_error(msg) { /* empty ctor */ }
};
...
class TabularDataTruncated : public FileReadException {
 public:
  TabularDataTruncated(const std::string& msg)
      : FileReadException(msg) { /* empty ctor */ }
};
...
enum {
  PARAMETERS_FILE_STANDARD = 0,
  PARAMETERS_FILE_APREPRO,
  PARAMETERS_FILE_JSON
};
...
/// INACTIVE: The database is either closed, or this model or interface should
/// not
///           write to it.
