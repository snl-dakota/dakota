...
typedef boost::tuple<std::string, std::string, size_t, std::string>
    ResultsKeyType;
...
/// create MetaDataValueType from the passed strings
MetaDataValueType make_metadatavalue(StringMultiArrayConstView cv_labels,
...
inline MetaDataValueType make_metadatavalue(StringMultiArrayConstView labels) {
  MetaDataValueType mdv;
  for (size_t i = 0; i < labels.size(); ++i) mdv.push_back(labels[i]);
  return mdv;
}

inline MetaDataValueType make_metadatavalue(
    StringMultiArrayConstView cv_labels, StringMultiArrayConstView div_labels,
    StringMultiArrayConstView dsv_labels, StringMultiArrayConstView drv_labels,
    const StringArray &resp_labels) {
  // could use iterators for some of these
  MetaDataValueType mdv;
  for (size_t i = 0; i < cv_labels.size(); ++i) mdv.push_back(cv_labels[i]);
  for (size_t i = 0; i < div_labels.size(); ++i) mdv.push_back(div_labels[i]);
  for (size_t i = 0; i < dsv_labels.size(); ++i) mdv.push_back(dsv_labels[i]);
  for (size_t i = 0; i < drv_labels.size(); ++i) mdv.push_back(drv_labels[i]);
  for (size_t i = 0; i < resp_labels.size(); ++i) mdv.push_back(resp_labels[i]);

  return mdv;
}

inline MetaDataValueType make_metadatavalue(const StringArray &resp_labels) {
  // could use iterators for some of these
  MetaDataValueType mdv;
  for (size_t i = 0; i < resp_labels.size(); ++i) mdv.push_back(resp_labels[i]);
...
inline MetaDataValueType make_metadatavalue(const std::string &s1) {
  MetaDataValueType mdv;
  mdv.push_back(s1);
  return mdv;
}
...
inline ResultsKeyType make_key(const StrStrSizet &iterator_id,
                               const std::string &data_name) {
  return ResultsKeyType(iterator_id.get<0>(), iterator_id.get<1>(),
                        iterator_id.get<2>(), data_name);
}
...
struct RealScale {
  ...
  RealScale(const std::string &label, const Real *in_items, const int len,
            ScaleScope scope = ScaleScope::UNSHARED)
      : label(label), scope(scope) {
    items = RealVector(Teuchos::View, const_cast<Real *>(in_items), len);
    numCols = items.length();
    isMatrix = false;
  }

  /// Constructor that takes an initializer_list.
  RealScale(const std::string &in_label, std::initializer_list<Real> in_items,
  ...
  RealScale(const std::string &in_label, const RealVectorArray &in_items,
            ScaleScope in_scope = ScaleScope::UNSHARED) {
    label = in_label;
    ...
    for (int i = 0; i < num_rows; ++i)
      for (int j = 0; j < numCols; ++j)
        items[j * num_rows + i] =
            in_items[i][j];  // deliberately transposed. See AttachScaleVisitor
                             // in ResultsDBHDF5.hpp.
    scope = in_scope;
    isMatrix = true;
  }
...
struct IntegerScale {
  ...
  IntegerScale(const std::string &label, const int *in_items, const int len,
               ScaleScope scope = ScaleScope::UNSHARED)
      : label(label), scope(scope) {
    items = IntVector(Teuchos::View, const_cast<int *>(in_items), len);
    numCols = items.length();
    isMatrix = false;
  }

  /// Constructor that takes an initializer_list.
  IntegerScale(const std::string &in_label, std::initializer_list<int> in_items,
               ScaleScope in_scope = ScaleScope::UNSHARED) {
    label = in_label;
    int len = in_items.size();
    items = IntVector(len);
...
struct StringScale {
  ...
  StringScale(const std::string &in_label, std::vector<const char *> in_items,
              ScaleScope in_scope = ScaleScope::UNSHARED) {
    label = in_label;
    items.resize(in_items.size());
    std::copy(in_items.begin(), in_items.end(), items.begin());
  ...
  StringScale(const std::string &in_label,
              ...
              ScaleScope in_scope = ScaleScope::UNSHARED) {
    label = in_label;
    for (const auto &s : in_items) items.push_back(s.c_str());
    scope = in_scope;
    numCols = items.size();
    isMatrix = false;
  }
  ...
  /// Constructor that takes a vector<vector<const char *> > to produce a 2D
  /// scale
  StringScale(const std::string &in_label,
    ...
    int num_rows = in_items.size();
    numCols = in_items[0]
                  .size();  // TODO: error checking to confirm a "square matrix"
...
typedef std::multimap<int,
                      boost::variant<StringScale, RealScale, IntegerScale> >
    DimScaleMap;
...
typedef std::vector<boost::variant<
    ResultAttribute<int>, ResultAttribute<String>, ResultAttribute<Real> > >
    AttributeArray;
...
struct VariableParametersField {
  VariableParametersField(const String &in_name, ResultsOutputType in_type,
                          const SizetArray &in_dims = SizetArray())
      : name(in_name), dims(in_dims), type(in_type) {};
  String name;
  SizetArray dims;
