...
shared_ptr<dakota::surrogates::Surrogate> findSurrogate(long inPointer) {
  for (int i = 0; i < surrogateCache.size(); i++) {
    long pointerAsLong =
        reinterpret_cast<unsigned long long>(surrogateCache.at(i).get());
...
Java_gov_sandia_dakota_jni_surrogates_java_DakotaSurrogates_loadSurrogate(
    JNIEnv *env, jobject thisObj, jstring inJNIStr, jboolean inJNIBoolean) {
  // Convert the JNI String (jstring) into C-string (char*), then into C++
  // std::string
  jboolean isCopy;
...
Java_gov_sandia_dakota_jni_surrogates_java_DakotaSurrogates_getVariables(
    JNIEnv *env, jobject thisObj, jlong inPointer) {
  // Step 1. Get the original surrogate shared_ptr.
  shared_ptr<dakota::surrogates::Surrogate> loadedSurrogate =
      findSurrogate(inPointer);

  // Step 2. Get the variable labels out of the surrogate and stuff them into
  // the jobjectArray.
  if (loadedSurrogate != NULL) {
    vector<string> variableLabels = loadedSurrogate->variable_labels();
    jobjectArray outJNIArray = env->NewObjectArray(
        variableLabels.size(), env->FindClass("java/lang/String"),
        env->NewStringUTF(""));
...
Java_gov_sandia_dakota_jni_surrogates_java_DakotaSurrogates_getResponses(
  ...
  // Step 2. Get the response labels out of the surrogate and stuff them into
  // the jobjectArray.
  if (loadedSurrogate != NULL) {
    vector<string> responseLabels = loadedSurrogate->response_labels();
    jobjectArray outJNIArray = env->NewObjectArray(
        responseLabels.size(), env->FindClass("java/lang/String"),
        env->NewStringUTF(""));
...
Java_gov_sandia_dakota_jni_surrogates_java_DakotaSurrogates_value(
    JNIEnv *env, jobject thisObj, jlong inPointer, jdoubleArray inJNIArr) {
  jboolean isCopy;
...
Java_gov_sandia_dakota_jni_surrogates_java_DakotaSurrogates_nativeDeleteSurrogate(
  ...
  if (loadedSurrogate != NULL) {
    ...
    surrogateCache.erase(
        remove(surrogateCache.begin(), surrogateCache.end(), loadedSurrogate),
        surrogateCache.end());
  }
}
