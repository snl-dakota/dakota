...
inline const Eigen::VectorXd& as_VectorXd(
    const ROL::Vector<double>& x) noexcept;
...
class ROLVectorXd : public ROL::Vector<double> {
 public:
  ...
  void plus(const ROL::Vector<double>& x) override { vec_ += as_VectorXd(x); }

  void scale(const double alpha) override { vec_ *= alpha; }

  double dot(const ROL::Vector<double>& x) const override {
    return vec_.dot(as_VectorXd(x));
  }

  double norm() const override { return vec_.norm(); }
  ...
  void zero() override { vec_.setZero(); }

  int dimension() const override { return vec_.size(); }

  void set(const ROL::Vector<double>& x) override { vec_ = as_VectorXd(x); }

  const ROL::Vector<double>& dual() const override { return *this; }

  double apply(const ROL::Vector<double>& x) const override { return dot(x); }
  ...
  void print(std::ostream& os) const override { os << vec_; }
  ...
  void setScalar(const double C) override { vec_.setConstant(C); }

  // Mutable segment access
  inline auto segment(int start, int size) { return vec_.segment(start, size); }
  ...
  inline auto head(int size) { return vec_.head(size); }

  inline auto head(int size) const { return vec_.head(size); }

  inline auto tail(int size) { return vec_.tail(size); }

  inline auto tail(int size) const { return vec_.tail(size); }
  ...
  inline double& operator()(int i) { return vec_(i); }

  inline const double& operator()(int i) const { return vec_(i); }

  inline Eigen::VectorXd& getVector() noexcept { return vec_; }

  inline const Eigen::VectorXd& getVector() const noexcept { return vec_; }

  inline static ROLVectorXd& downcast(ROL::Vector<double>& x) noexcept {
    return static_cast<ROLVectorXd&>(x);
  }

  inline static const ROLVectorXd& downcast(
      const ROL::Vector<double>& x) noexcept {
