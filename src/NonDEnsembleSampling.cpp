...
static const char rcsId[] =
    "@(#) $Id: NonDEnsembleSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDEnsembleSampling::NonDEnsembleSampling(ProblemDescDB& problem_db,
  ...
  size_t num_mf = model_ensemble.size(), num_lev, prev_lev = SZ_MAX, md_index,
         num_md;
  int m;
...
void NonDEnsembleSampling::accumulate_online_cost(
    const IntResponseMap& resp_map, RealVector& accum_cost,
    SizetArray& num_cost) {
...
void NonDEnsembleSampling::initialize_final_statistics() {
  switch (finalStatsType) {
    case ESTIMATOR_PERFORMANCE: {  // MSE in stat goal(s) used for method
                                   // selection
      size_t num_final = 2;
      ActiveSet set(num_final);  //, num_active_vars); // default RV = 1
      set.derivative_vector(
      ...
      {
        stats_labels[0] = "equiv_HF_cost";
        stats_labels[1] = "est_var_metric";
      } else  // budget spec: equiv cost is constraint
      {
        stats_labels[0] = "est_var_metric";
        stats_labels[1] = "equiv_HF_cost";
      }
    ...
    case QOI_STATISTICS:  // final stats: moments + level mappings
      NonD::initialize_final_statistics();
      break;
  }
}
  ...
  */
  switch (finalStatsType) {
    case ESTIMATOR_PERFORMANCE:
      if (maxFunctionEvals ==
          SZ_MAX) {  // accuracy spec: equiv cost is objective
...
void NonDEnsembleSampling::active_set_mapping() {
  switch (finalStatsType) {
    case ESTIMATOR_PERFORMANCE:
      ...
      break;
    case QOI_STATISTICS:  // final stats: moments + level mappings
      NonDSampling::active_set_mapping();
      break;
  }
}

void NonDEnsembleSampling::compute_mean_confidence_intervals(
    const RealMatrix& moment_stats, const RealVector& mean_estvar,
    RealMatrix& mean_conf_ints) {
  size_t q, num_qoi = moment_stats.numCols();
  if (mean_conf_ints.empty()) mean_conf_ints.shapeUninitialized(2, num_qoi);
  ...
  Real mean, sqrt_estvar,
      z = Pecos::NormalRandomVariable::inverse_std_cdf(
          .975);  // ~ 2 stdev on 1 side
  for (q = 0; q < num_qoi; ++q) {
...
void NonDEnsembleSampling::print_results(std::ostream& s, short results_state) {
  ...
  if (!projections) {
    ...
    // print_statistics(s);
    print_moments(
        s, momentStats, meanCIs, "response function", finalMomentsType,
        ModelUtils::response_labels(*iteratedModel->truth_model()), true);
    archive_moments();
  }
}

void NonDEnsembleSampling::print_multimodel_summary(std::ostream& s,
...
void NonDEnsembleSampling::export_all_samples(const Model& model,
                                              const String& tabular_filename) {
...
void NonDEnsembleSampling::convert_moments(const RealMatrix& raw_mom,
                                           RealMatrix& final_mom) {
