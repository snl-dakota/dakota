...
void SurrBasedLevelData::initialize_data(const Variables& vars,
                                         const Response& approx_resp,
                                         const Response& truth_resp,
                                         bool uncorr) {
...
void SurrBasedLevelData::active_set_center(const ActiveSet& set,
                                           short response_type, bool uncorr) {
...
void SurrBasedLevelData::active_set_star(const ActiveSet& set,
                                         short response_type, bool uncorr) {
  switch (response_type) {
    case TRUTH_RESPONSE:
...
Response& SurrBasedLevelData::response_star(short corr_response_type) {
  switch (corr_response_type) {
    case CORR_TRUTH_RESPONSE:
      return responseStarTruthCorrected.second;
      break;
    case UNCORR_TRUTH_RESPONSE:
      return responseStarTruthUncorrected;
      break;
    case CORR_APPROX_RESPONSE:
      return responseStarApproxCorrected;
      break;
    default:  // case UNCORR_APPROX_RESPONSE:
      return responseStarApproxUncorrected;
      break;
  }
}

Response& SurrBasedLevelData::response_center(short corr_response_type) {
  switch (corr_response_type) {
    case CORR_TRUTH_RESPONSE:
      return responseCenterTruthCorrected.second;
      break;
    case UNCORR_TRUTH_RESPONSE:
      return responseCenterTruthUncorrected;
      break;
    case CORR_APPROX_RESPONSE:
      return responseCenterApproxCorrected;
      break;
    default:  // case UNCORR_APPROX_RESPONSE:
      return responseCenterApproxUncorrected;
      break;
  }
}

void SurrBasedLevelData::response_star(const Response& resp,
  ...
  switch (corr_response_type) {
    case CORR_TRUTH_RESPONSE:
      ...
      break;
    case UNCORR_TRUTH_RESPONSE:
      responseStarTruthUncorrected.update(resp);
      break;
    case CORR_APPROX_RESPONSE:
      responseStarApproxCorrected.update(resp);
      break;
    case UNCORR_APPROX_RESPONSE:
      responseStarApproxUncorrected.update(resp);
      break;
  }
}

void SurrBasedLevelData::response_center(const Response& resp,
                                         short corr_response_type) {
  switch (corr_response_type) {
    case CORR_TRUTH_RESPONSE:
      ...
      break;
    case UNCORR_TRUTH_RESPONSE:
      responseCenterTruthUncorrected.update(resp);
      break;
    case CORR_APPROX_RESPONSE:
      responseCenterApproxCorrected.update(resp);
      break;
    case UNCORR_APPROX_RESPONSE:
      responseCenterApproxUncorrected.update(resp);
      break;
  }
}

IntResponsePair& SurrBasedLevelData::response_star_pair(
    short corr_response_type) {
...
IntResponsePair& SurrBasedLevelData::response_center_pair(
    short corr_response_type) {
  if (corr_response_type != CORR_TRUTH_RESPONSE) {
    Cerr << "Error: IntResponsePair return not supported in SurrBasedLevelData "
         << "for this response type" << std::endl;
...
void SurrBasedLevelData::response_star_id(int eval_id,
                                          short corr_response_type) {
  switch (corr_response_type) {
    case CORR_TRUTH_RESPONSE:
      ...
      break;
    default:
      Cerr << "Error: eval_id assignment not supported in SurrBasedLevelData "
              "for "
...
void SurrBasedLevelData::response_center_id(int eval_id,
                                            short corr_response_type) {
  switch (corr_response_type) {
    case CORR_TRUTH_RESPONSE:
...
void SurrBasedLevelData::response_star_pair(int eval_id, const Response& resp,
                                            short corr_response_type) {
  if (corr_response_type != CORR_TRUTH_RESPONSE) {
...
void SurrBasedLevelData::response_center_pair(int eval_id, const Response& resp,
                                              short corr_response_type) {
  if (corr_response_type != CORR_TRUTH_RESPONSE) {
    Cerr << "Error: eval_id + response assignment not supported in "
         << "SurrBasedLevelData for this response type" << std::endl;
...
bool SurrBasedLevelData::update_filter(Real new_f, Real new_g) {
  ...
  for (; filt_it != paretoFilter.end(); ++filt_it) {
    filt_f = filt_it->first;
    filt_g = filt_it->second;
    ...
    if (new_f + gamma * new_g > filt_f &&
        new_g > beta * filt_g)  // slanting logic
