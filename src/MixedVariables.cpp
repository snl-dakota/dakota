...
MixedVariables::MixedVariables(const ProblemDescDB& problem_db,
  ...
  copy_data_partial(cdv, allContinuousVars, start);
  start += cdv.length();
  copy_data_partial(cauv, allContinuousVars, start);
  start += cauv.length();
  copy_data_partial(ceuv, allContinuousVars, start);
  start += ceuv.length();
  ...
  copy_data_partial(ddrv, allDiscreteIntVars, start);
  start += ddrv.length();
  copy_data_partial(ddsiv, allDiscreteIntVars, start);
  start += ddsiv.length();
  copy_data_partial(dauiv, allDiscreteIntVars, start);
  start += dauiv.length();
  copy_data_partial(deuiv, allDiscreteIntVars, start);
  start += deuiv.length();
  copy_data_partial(dsrv, allDiscreteIntVars, start);
  start += dsrv.length();
  ...
  copy_data_partial(ddssv, allDiscreteStringVars, start);
  start += ddssv.size();
  copy_data_partial(dausv, allDiscreteStringVars, start);
  start += dausv.size();
  copy_data_partial(deusv, allDiscreteStringVars, start);
  start += deusv.size();
  ...
  copy_data_partial(ddsrv, allDiscreteRealVars, start);
  start += ddsrv.length();
  copy_data_partial(daurv, allDiscreteRealVars, start);
  start += daurv.length();
  copy_data_partial(deurv, allDiscreteRealVars, start);
  start += deurv.length();
...
void MixedVariables::write_tabular(std::ostream& s,
                                   unsigned short vars_part) const {
  write_core(s, TabularWriter(), vars_part);
}

void MixedVariables::write_tabular_labels(std::ostream& s,
                                          unsigned short vars_part) const {
  write_core(s, LabelsWriter(), vars_part);
}

void MixedVariables::write_tabular_partial(
...
void MixedVariables::write_tabular_partial_labels(std::ostream& s,
                                                  size_t start_index,
...
template <typename Writer, typename Stream>
bool MixedVariables::write_partial_core(Stream& s, Writer write_handler,
                                        size_t start_index, size_t end_index,
                                        size_t& acv_offset, size_t& adiv_offset,
                                        size_t& adsv_offset,
                                        size_t& adrv_offset, size_t& av_cntr,
                                        size_t num_cv, size_t num_div,
