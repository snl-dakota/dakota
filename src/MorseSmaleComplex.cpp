...
double Vertex::GetXi(int i) { return x[i]; }
...
void Vertex::CreateANNpoint(ANNpoint &p) {
  if (p != NULL) delete[] p;
  p = new ANNcoord[d];
  for (int i = 0; i < d; i++) p[i] = this->x[i];
}
...
int Vertex::Find_Max(Vertex *V[]) {
  if (this->UF_max == this->ID) return this->ID;
...
double Vertex::SDistance(Vertex *v) {
  double sDist = 0;
  for (int i = 0; i < d; i++) sDist += (x[i] - v->x[i]) * (x[i] - v->x[i]);
  return sDist;
}
...
int Vertex::Find_Min(Vertex *V[]) {
  if (this->UF_min == this->ID) return this->ID;
...
MS_Crystal::MS_Crystal(MS_Crystal *a, MS_Crystal *b, bool mergeMax,
                       int persistence) {
  minP = a->maxP = b->maxP = persistence;
  V = a->V;
  v = new int[a->count + b->count];
  int i = 0;
  for (; i < a->count; i++) v[i] = a->v[i];
  for (; i < a->count + b->count; i++) v[i] = b->v[i - a->count];
...
MS_Crystal::MS_Crystal(MS_Crystal *a, bool mergeMax, int nuExtrema,
                       int persistence) {
...
void SortSaddles(Saddle **S, int n) { quickSort(S, 0, n - 1); }
MS_Complex::MS_Complex(double *points, int dimension, int count, int _k,
                       bool perturb) {
  perturbed = perturb;
  d = dimension - 1;
  numKneighbors = _k;
  ...
  for (int i = 0; i < count; i++) {
    ...
    eps = eps * 1e-6;
    if (rand() > RAND_MAX / 2) eps = -eps;

    for (int j = 0; j < dimension; j++) p[j] = points[i * dimension + j];
...
MS_Complex::MS_Complex(MS_Complex &Complex, double *new_point) {
  ...
  for (int i = 0; i < Complex.numV; i++) {
    for (int j = 0; j < d; j++) p[j] = Complex.V[i]->GetXi(j);
  ...
  V[numV - 1] =
      new Vertex(d, new_point, new_point[d] + (perturbed ? eps : 0), numV - 1);
  delete[] p;
  KNN();
  Compute();
  maxDist = -1;
...
void MS_Complex::Compute() {
  ...
  for (i = 0; i < numV; i++) {
    ...
    if (globalMax < v->Value()) globalMax = v->Value();
    if (globalMin > v->Value()) globalMin = v->Value();
  ...
  // Go through each vertex, examine its neighbors, if maxima are different,
  // create or update the entry in a map
  //  where the key is the two maxima (always use lower maxima as first in
  //  pair), if the value of this point is higher then update or create the
  //  entry with this point as the pseudo-saddle.
  // When we are done looking at each point, read off the saddles.
  std::map<std::pair<int, int>, int> maxSaddles =
  ...
  for (i = 0; i < numV; i++) {
    ...
    for (int Bindex = 0; Bindex < j; Bindex++) {
      ...
      if (AmaxIndex != BmaxIndex) {
        ...
        if (maxSaddles.find(AB) == maxSaddles.end() ||
            V[maxSaddles[AB]]->Value() < V[i]->Value()) {
          maxSaddles[AB] = i;
        }
      }
    }
  ...
  std::map<std::pair<int, int>, int> minSaddles =
      std::map<std::pair<int, int>, int>();
  for (i = 0; i < numV; i++) {
    if (V[i]->classification == LOCAL_MAX || V[i]->classification == LOCAL_MIN)
      continue;
    ...
    for (; Bindex < j; Bindex++) {
      ...
      if (AminIndex != BminIndex) {
        ...
        if (minSaddles.find(AB) == minSaddles.end() ||
            V[minSaddles[AB]]->Value() > V[i]->Value()) {
          minSaddles[AB] = i;
        }
      }
    }
  ...
  for (; sIter != maxSaddles.end(); sIter++) {
    ...
    if (V[sIter->first.first]->Value() < V[sIter->first.second]->Value() ||
        (V[sIter->first.first]->Value() == V[sIter->first.second]->Value() &&
         sIter->first.first < sIter->first.second)) {
    ...
    S[curIdx]->persistence = std::max(
        eps, fabs(V[sIter->second]->Value() - V[S[curIdx]->less]->Value()));

    if (V[S[curIdx]->less]->persistence == 0 ||
  ...
  //  as opposed to just reporting the extrema without persistence, this is
  //  wrong as an extrema's persistence may get updated when we cancel saddles
  ...
  std::map<std::pair<int, int>, std::vector<Vertex *> >::iterator cIter =
      Crystals.begin();
  numC = 10000;
  ...
  while (cIter != Crystals.end()) {
    ...
    tempCrystalList.push_back(
        new MS_Crystal(V, vIds, cIter->second.size() + 2, d));
  ...
  p_levels = numSaddles;
  V_to_C = new int[numV * p_levels];  // Stores the crystal id that a vertex
                                      // belongs to at a given persistence level
  // Assign every vertex to its original Crystal for every persistence level,
  // update when new crystal emerges
  for (i = 0; i < Crystals.size(); i++) {
    for (j = 0; j < tempCrystalList[i]->size(); j++) {
      int vIndex = tempCrystalList[i]->GetVertexID(j);
      for (int p = 0; p < p_levels; p++) V_to_C[vIndex * p_levels + p] = i;
    }
  }
  ...
  while (current != NULL) {
    ...
    V[idxToRemove]->persistence = V[current->idx]->persistence = std::max(
        eps, fabs(V[current->less]->Value() - V[current->idx]->Value()));
    // printf("Combining %d and %d\n", current->idx, idxToRemove);

    // Find the rest of the saddles that have this extremum and delete them,
    // then reenter them into the
        ...
        if (idxToReplace != findIter->more) {
          ...
          if (V[idxToReplace]->classification == LOCAL_MAX) {
            if (V[idxToReplace]->Value() > V[findIter->more]->Value() ||
                (V[idxToReplace]->Value() == V[findIter->more]->Value() &&
                 idxToReplace > findIter->more)) {
          ...
          nuSaddle->persistence = std::max(
              eps,
              fabs(V[nuSaddle->less]->Value() - V[nuSaddle->idx]->Value()));
          V[nuSaddle->less]->persistence =
              std::min(nuSaddle->persistence, V[nuSaddle->less]->persistence);
          ...
          // Insert the next one (note we will always gain persistence, so we
          // can start
          ...
          if (insert == NULL || nuSaddle->persistence < insert->persistence) {
            ...
            while (insert != NULL &&
                   nuSaddle->persistence > insert->persistence) {
        ...
        if (idxToReplace != findIter->less) {
          ...
          if (V[idxToReplace]->classification == LOCAL_MAX) {
            if (V[idxToReplace]->Value() > V[findIter->less]->Value() ||
                (V[idxToReplace]->Value() == V[findIter->less]->Value() &&
                 idxToReplace > findIter->less)) {
              nuSaddle->more = idxToReplace;
    ...
    for (i = 0; i < totalCrystals; i++) {
      if (tempCrystalList[i] == NULL ||
          !tempCrystalList[i]->Exists(p_level - 1))
        continue;

      bool success = false;
      if (V[idxToRemove]->classification == LOCAL_MAX &&
          tempCrystalList[i]->MaxV() == idxToRemove) {
        for (j = 0; j < totalCrystals; j++) {
          if (tempCrystalList[j] == NULL ||
              !tempCrystalList[j]->Exists(p_level - 1) || i == j)
            continue;

          if (tempCrystalList[j]->MaxV() == idxToReplace &&
              tempCrystalList[i]->MinV() == tempCrystalList[j]->MinV()) {
            // C[nextCID] = new MS_Crystal(C[i], C[j], true, p_level);
            MS_Crystal *nuCrystal = new MS_Crystal(
                tempCrystalList[i], tempCrystalList[j], true, p_level);
        ...
        if (!success) {
          ...
          MS_Crystal *nuCrystal =
              new MS_Crystal(tempCrystalList[i], true, idxToReplace, p_level);
          tempCrystalList.push_back(nuCrystal);
          for (int k = 0; k < nuCrystal->size(); k++) {
            int vIndex = nuCrystal->GetVertexID(k);
            for (int p = p_level; p < p_levels; p++)
      ...
      } else if (V[idxToRemove]->classification == LOCAL_MIN &&
                 tempCrystalList[i]->MinV() == idxToRemove) {
        for (j = 0; j < totalCrystals; j++) {
          if (tempCrystalList[j] == NULL ||
              !tempCrystalList[j]->Exists(p_level - 1) || i == j)
            continue;

          if (tempCrystalList[j]->MinV() == idxToReplace &&
              tempCrystalList[i]->MaxV() == tempCrystalList[j]->MaxV()) {
            // C[nextCID] = new MS_Crystal(C[i], C[j], false, p_level);
            MS_Crystal *nuCrystal = new MS_Crystal(
                tempCrystalList[i], tempCrystalList[j], false, p_level);
            tempCrystalList.push_back(nuCrystal);
            for (int k = 0; k < nuCrystal->size(); k++) {
              int vIndex = nuCrystal->GetVertexID(k);
              for (int p = p_level; p < p_levels; p++)
        ...
        if (!success) {
          ...
          MS_Crystal *nuCrystal =
              new MS_Crystal(tempCrystalList[i], false, idxToReplace, p_level);
          tempCrystalList.push_back(nuCrystal);
          for (int k = 0; k < nuCrystal->size(); k++) {
            int vIndex = nuCrystal->GetVertexID(k);
            for (int p = p_level; p < p_levels; p++)
  ...
  for (i = 0; i < numV; i++) {
    if (V[i]->classification == LOCAL_MIN &&
        V[i]->Value() < V[globalMinIdx]->Value())
      globalMinIdx = i;
    else if (V[i]->classification == LOCAL_MAX &&
             V[i]->Value() > V[globalMaxIdx]->Value())
      globalMaxIdx = i;
  }

  for (i = 0; i < numV; i++) {
    if ((V[i]->classification == LOCAL_MIN ||
         V[i]->classification == LOCAL_MAX) &&
        V[i]->persistence == 0) {
      if (i == globalMinIdx || i == globalMaxIdx)
        V[i]->persistence = V[globalMaxIdx]->Value() - V[globalMinIdx]->Value();
      else {
        std::cerr << "ERROR: There exists an extrema with zero "
                     "persistence!\n\tVertex "
                  << i << "\n\tClassification = " << V[i]->classification
                  << "\n\tValue = " << V[i]->Value() << std::endl;
        //                  << "\nPrinting points for reconstruction to
        //                  badMSC.txt" << std::endl;
        //        FILE *badMSC =  fopen("badMSC.txt", "w");
        //        for(int i = 0; i < numV; i++)
        //        {
        //          fprintf(badMSC, "%f\t%f\t%f\n", V[i]->GetXi(0),
        //          V[i]->GetXi(1), V[i]->Value());
        //        }
        //        fclose(badMSC);
      }
    }
...
double MS_Complex::CompareExtremaCount(MS_Complex &_C) {
  ...
  return abs(countThisMin - countThatMin + countThisMax - countThatMax +
             countThisSaddle - countThatSaddle);
}
...
double MS_Complex::CompareClassChange(MS_Complex &_C) {
  ...
  for (int i = 0; i < minNumV; i++, _ci++) {
    if (V[i]->classification != _C.V[_ci]->classification) countChanged++;
  }
...
double MS_Complex::CompareBottleneck(MS_Complex &_C) {
  ...
  PDPoint<double> globalMinMax(V[globalMinIdx]->Value(),
                               V[globalMaxIdx]->Value());
  ...
  PDPoint<double> globalMinMax2(_C.V[_C.globalMinIdx]->Value(),
                                _C.V[_C.globalMaxIdx]->Value());
...
double MS_Complex::CompareLabels(MS_Complex &_C, double p) {
  ...
  for (int i = 0; i < numV && i < _C.numV; i++) {
    ...
    if (idx1 != idx2) diff_labels++;
    // if(idx1 < numC && idx2 < _C.numC)
    //   if(C[idx1]->MaxV() != _C.C[idx2]->MaxV() || C[idx1]->MinV() !=
    //   _C.C[idx2]->MinV())
    //     diff_labels++;
  }
...
double MS_Complex::FarFromExtrema(double *v, double p) {
  ...
  for (int i = 0; i < numV; i++) {
    if (V[i]->classification == REGULAR || V[i]->classification == SADDLE ||
        V[i]->persistence < testPersistence)
      continue;
...
double MS_Complex::FarFromSaddle(double *v, double p) {
  ...
  for (int i = 0; i < numV; i++) {
    if (V[i]->classification == REGULAR || V[i]->classification == LOCAL_MIN ||
        V[i]->classification == LOCAL_MAX ||
        V[i]->persistence < testPersistence)
      continue;
...
MS_Complex::~MS_Complex() {
  for (int i = 0; i < numV; i++) delete V[i];
  delete[] V;
  for (int i = 0; i < numE; i++) delete E[i];
  delete[] E;
  for (int i = 0; i < numC; i++) delete C[i];
  delete[] C;
...
int MS_Complex::GetIthHighestPersistence(int i) {
  ...
  for (int j = 0; j < numV; j++) {
    if (V[j]->classification != REGULAR) numCPs++;
  }
  if (numCPs <= i) {
    return -1;
  }
...
int MS_Complex::GetIthHighestPersistenceSaddle(int i) {
  ...
  for (int j = 0; j < numV; j++)
    if (V[j]->classification == SADDLE) numS++;
...
bool MS_Complex::IsExtrema(int i) {
  int j = i;
  return j < numV && j >= 0 &&
         (V[j]->classification == LOCAL_MIN ||
          V[j]->classification == LOCAL_MAX);
}
...
std::vector<int> ScoreTOPOHP(int dimension, int knn, double *training,
                             double *trainingY, int n_training,
                             double *candidates, double *candidateY,
                             int n_candidates) {
  ...
  for (int i = 0; i < n_candidates; i++) {
    ...
    if (count < Complex.numV) returnOrder.push_back(tempIdx);
  }
  ...
  for (int i = 0; i < n_candidates; i++) {
    ...
    if (!found) returnOrder.push_back(i);
  }
...
void MS_Complex::Print(std::ostream &out) {
  ...
  for (int i = 0; i < numV; i++) {
    if (V[i]->classification == LOCAL_MIN) {
      for (int j = 0; j < d; j++) out << V[i]->GetXi(j) << ' ';
...
void MS_Complex::Draw(double gMin, double gMax, bool flatMode) {
  ...
  for (int i = 0; i < numE; i++) {
    ...
    glVertex3f(V[E[i]->start]->GetXi(0), V[E[i]->start]->GetXi(1),
               flatMode ? 0
                        : ((V[E[i]->start]->Value() - gMin) / (gMax - gMin) -
                           1. / 2.));
    glVertex3f(
        V[E[i]->end]->GetXi(0), V[E[i]->end]->GetXi(1),
        flatMode ? 0
                 : ((V[E[i]->end]->Value() - gMin) / (gMax - gMin) - 1. / 2.));
  }
  glEnd();
  ...
  for (int i = 0; i < numV; i++) {
    ...
    glTranslatef(
        curV->GetXi(0), curV->GetXi(1),
        flatMode ? 0 : ((curV->Value() - gMin) / (gMax - gMin) - 1. / 2.));
  ...
  for (int i = 0; i < numV; i++) {
    ...
    for (int k = 0; k < numKneighbors; k++) {
      ...
      if (nextIdx == curV->NeighborMax() || nextIdx == curV->NeighborMin()) {
        ...
        glVertex3f(
            nextV->GetXi(0), nextV->GetXi(1),
            flatMode ? 0 : ((nextV->Value() - gMin) / (gMax - gMin) - 1. / 2.));
        glEnd();
      }
    }
...
void MS_Complex::DrawBurst() {
  ...
  if (maxDist < 0) {
    for (int vi = 0; vi < numV; vi++) {
      for (int vj = vi + 1; vj < numV; vj++) {
        ...
        if (sdist > maxSdist) maxSdist = sdist;
      }
    }
  ...
  for (int i = 0; i < numV; i++) {
    if (V[i]->classification == LOCAL_MIN) mins.push_back(i);
    if (V[i]->classification == LOCAL_MAX) maxs.push_back(i);
    if (V[i]->classification == SADDLE) saddles.push_back(i);
  }
  ...
  for (int i = 0; i < mins.size(); i++) {
    ...
    for (int j = 0; j < numV; j++) {
      if (V[j]->Find_Min(V) == midx && midx != j) {
        ...
        glVertex3f(x, 0.5, y);
        double radius =
            0.5 * (V[j]->Value() - V[midx]->Value()) / (gMax - gMin);
  ...
  for (int i = 0; i < maxs.size(); i++) {
    ...
    for (int j = 0; j < numV; j++) {
      if (V[j]->Find_Max(V) == midx && midx != j) {
        ...
        glVertex3f(x, 0.5, y);
        double radius =
            0.5 * (V[midx]->Value() - V[j]->Value()) / (gMax - gMin);
        double dist = sqrt(V[midx]->SDistance(V[j]));
        double theta = dist * factor + M_PI / 2.;
        double xtemp = x + radius * cos(theta);
        double ytemp = y - radius * sin(theta);
