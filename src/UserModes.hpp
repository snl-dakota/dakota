...
/// @brief  UserModes stores the user-requested run modes, file names, and file
/// formats
class UserModes {
 public:
  ...
  // In ProgramOptions:: parse() and validate_run_modes(), preRun, run, and
  // postRun are treated as MODES: If a user specified none of them, all three
  // are set to true because Dakota should do all three. userModes, on the other
  // hand, is only true when a user made a non-default selection.
