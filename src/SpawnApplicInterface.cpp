...
SpawnApplicInterface::SpawnApplicInterface(const ProblemDescDB &problem_db,
...
static HANDLE *wait_setup(std::map<pid_t, int> *M, size_t *pn) {
  ...
  if ((n = M->size()) > 0) {
    ...
    e = M->end();
    for (i = 0, p = M->begin(); p != e; ++p) h[i++] = (HANDLE)p->first;
  }
  *pn = n;
  return h;
}
...
static int wait_for_one(size_t n, HANDLE *h, int req1, size_t *pi) {
  ...
  for (j = 0; j < n; j += k) {
    if ((k = n - j) > MAXIMUM_WAIT_OBJECTS) k = MAXIMUM_WAIT_OBJECTS;
    i = WaitForMultipleObjects(k, h + j, 0, 0);
    if (i != WAIT_TIMEOUT) goto ret;
  }
  if (!req1) return 0;
  if (n > MAXIMUM_WAIT_OBJECTS) {
...
pid_t SpawnApplicInterface::create_analysis_process(bool block_flag,
                                                    bool new_group) {
  ...
  if (block_flag)
    status = _spawnvp(_P_WAIT, av[0], av.get());
  else
    pid = _spawnvp(_P_NOWAIT, av[0], av.get());
...
size_t SpawnApplicInterface::wait_local_analyses() {
  ...
  if ((h = wait_setup(&analysisProcessIdMap, &n))) {
    ...
    while (wait_for_one(n, h, req, &i)) {
      GetExitCodeProcess(h[i], &dw);
      check_wait((pid_t)h[i], (int)dw);
      std::map<pid_t, int>::iterator an_it =
          analysisProcessIdMap.find((pid_t)h[i]);
