...
/* Portability adapter: return the cwd string in OS-native
   format.  TODO: change paths throughout code to use std::filesystem::path
   where possible, since Windows (and Cygwin) use wchar_t instead of char_t. */
...
void WorkdirHelper::change_directory(const std::filesystem::path& new_dir) {
  ...
  if (err_code) {
    ...
    Cout << "\nEXPECTED: successful change directory to " << new_dir
         << std::endl;
#endif
}
...
void WorkdirHelper::prepend_preferred_env_path(const std::string& extra_path) {
  ...
  std::string abs_extra_path =
      std::filesystem::path(extra_path).is_absolute()
          ? extra_path
 ...
 */
std::vector<std::string> WorkdirHelper::tokenize_env_path(
  ...
  for (const std::string& d : dirs) Cout << "\tDir = " << d << '\n';
  Cout << std::endl;
#endif
...
bool WorkdirHelper::resolve_driver_path(String& an_driver) {
  ...
  if (strbegins(driver0, dot_str) || strbegins(driver0, dotdot_str)) {
    ...
    for (; daa_it != daa_end; ++daa_it) an_driver += " " + *daa_it;
    return true;
  }
 ...
 */
inline bool contains(const std::filesystem::path& dir_path,
...
void WorkdirHelper::set_preferred_path(
  ...
  std::filesystem::path abs_extra_path =
      extra_path.is_absolute() ? extra_path : rel_to_abs(extra_path);
...
std::filesystem::path WorkdirHelper::unique_path(const std::string& input) {
  // Function to replace '%' characters in a string with random alphanumeric
  // values
...
std::filesystem::path WorkdirHelper::system_tmp_file(
...
bool WorkdirHelper::create_directory(const std::filesystem::path& dir_path,
  ...
  try {
    ...
    if (std::filesystem::exists(dir_path)) {
      ...
      if ((dir_perms & std::filesystem::perms::owner_write) ==
          std::filesystem::perms::none) {
        // BMA: Make this a warning until we get permission checking fixed
        Cout << "\nWarning: Directory " << dir_path
             << " exists (permitted), but "
             << "not writable." << std::endl;
      }

    } else {
      try {
        // directory does not exist, create recursively (as if mkdir -p, we
        // hope)
        ...
        // std::filesystem::permissions(dir_path, std::filesystem::add_perms |
        // std::filesystem::owner_all);
      } catch (const std::filesystem::filesystem_error& e) {
...
void WorkdirHelper::recursive_remove(const std::filesystem::path& rm_path,
                                     short fileop_opt) {
  try {
...
void WorkdirHelper::rename(const std::filesystem::path& old_path,
...
bool WorkdirHelper::file_op_items(const file_op_function& file_op,
  ...
  for (; src_it != src_end; ++src_it) {
    ...
    if (strcontains(*src_it, "*") || strcontains(*src_it, "?")) {
      ...
      glob_iterator fit(wc_predicate,
                        std::filesystem::directory_iterator(root_dir));
      ...
      for (; fit != fitend; ++fit) {
        ...
        if (std::filesystem::exists(src_path)) {
          ...
          if (file_op(src_path, dest_dir, overwrite)) return_code = true;
        } else {
...
bool WorkdirHelper::check_equivalent_dest(
    const StringArray& source_items, const std::filesystem::path& dest_dir) {
...
bool WorkdirHelper::find_driver(const StringArray& source_items,
  ...
  // BMA TODO: file_op_items should short-circuit when found and not be
  // verbose...
...
bool WorkdirHelper::link(const std::filesystem::path& src_path,
                         const std::filesystem::path& dest_dir,
                         bool overwrite) {
  ...
  try {
    ...
    Cerr << "\nError: could not create symlink from " << dest_dir << " to "
         << src_path << ";\n       " << e.what() << std::endl;
    abort_handler(IO_ERROR);
  }
...
bool WorkdirHelper::recursive_copy(const std::filesystem::path& src_path,
  ...
  try {
    ...
    if (!std::filesystem::exists(dest_dir) ||
        !std::filesystem::is_directory(dest_dir)) {
    ...
    Cerr << "\nError: could not recursive copy " << src_path << " to "
         << dest_dir << ";\n       " << e.what() << std::endl;
    abort_handler(IO_ERROR);
  }
  return false;
...
std::filesystem::path WorkdirHelper::concat_path(
    const std::filesystem::path& p_in, const String& tag) {
