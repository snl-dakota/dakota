...
DigitalNet::DigitalNet(
    ...
    bool mostSignificantBitFirst,  /// Generating matrices are stored with most
                                   /// significant bit first if true
    short outputLevel              /// Verbosity
    )
    : LowDiscrepancySequence(mMax, generatingMatrices.numRows(), seedValue,
                             outputLevel),
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    Cout << "The maximum dimension of this digital net is " << dMax << "."
         << std::endl;
    ...
    Cout << "The number of bits of the integers in the generating matrices "
         << "is " << tMax << "." << std::endl;
    Cout << "The number of rows in the linear scramble matrix is " << tScramble
         << "." << std::endl;
    ...
    Cout << "Assuming generating matrix is stored with "
         ...
         << "first." << std::endl;
    Cout << "Found generating matrices of shape " << numRows << " x " << numCols
         << ":" << std::endl;
  ...
  if (tScramble < tMax) {
    Cerr << "Error: number of rows of the scramble matrices 't_scramble' must "
         << "be greater than or equal to the number of bits in the "
            "representation "
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    if (ordering == DIGITAL_NET_NATURAL_ORDERING) {
      ...
      Cout << "Using gray code ordering of the digital net points" << std::endl;
    }
  }
...
DigitalNet::DigitalNet(const UInt64Matrix& generatingMatrices, int mMax,
                       int tMax)
    : DigitalNet(generatingMatrices, mMax, tMax, tMax, true, true,
                 generate_system_seed(), DIGITAL_NET_GRAY_CODE_ORDERING, false,
                 NORMAL_OUTPUT) {}

/// A constructor with only the random seed as argument
DigitalNet::DigitalNet(int seedValue)
    : DigitalNet(UInt64Matrix(Teuchos::View, &joe_kuo_d1024_t32_m32[0][0], 1024,
                              1024, 32),
                 32, 32, 32, true, true, seedValue,
                 DIGITAL_NET_GRAY_CODE_ORDERING, false, NORMAL_OUTPUT) {}

/// A constructor with no arguments
DigitalNet::DigitalNet() : DigitalNet(generate_system_seed()) {}

/// A constructor that takes a problem description database
DigitalNet::DigitalNet(ProblemDescDB& problem_db)
    : DigitalNet(get_data(problem_db), problem_db) {}

/// A constructor that takes a tuple and a problem description database
DigitalNet::DigitalNet(std::tuple<UInt64Matrix, int, int> data,
                       ProblemDescDB& problem_db)
    : DigitalNet(std::get<0>(data), std::get<1>(data), std::get<2>(data),
...
/// Case I: the generating matrices are provided in an external file
const std::tuple<UInt64Matrix, int, int>
DigitalNet::get_generating_matrices_from_file(ProblemDescDB& problem_db) {
  ...
  try {
    ...
    return std::make_tuple(generatingMatrices,
...
/// Case II: the generating matrices are provided in the input file
const std::tuple<UInt64Matrix, int, int>
DigitalNet::get_inline_generating_matrices(ProblemDescDB& problem_db) {
  /// Get the inline generating matrices
  ...
  return std::make_tuple(generatingMatrices, numCols,
                         problem_db.get_int("method.t_max"));
}

/// Case III: a set of default generating matrices has been selected
const std::tuple<UInt64Matrix, int, int>
DigitalNet::get_default_generating_matrices(ProblemDescDB& problem_db) {
  /// NOTE: outputLevel has not been set yet, so gettting it directly from
  /// the 'problem_db' instead
  bool outputLevel = problem_db.get_short("method.output");
  ...
  if (problem_db.get_bool("method.sobol_order_2")) {
    if (outputLevel >= DEBUG_OUTPUT) {
      Cout << "Found predefined generating matrices 'sobol_order_2'."
           << std::endl;
    }
    ...
    return std::make_tuple(
        UInt64Matrix(Teuchos::View, &joe_kuo_d1024_t32_m32[0][0], 1024, 1024,
                     32),
        32, 64);
  }
}
...
void DigitalNet::digital_shift(int seed) {
...
void DigitalNet::scramble(int seed) {
  auto numRows = generatingMatrices.numRows();
  auto numCols = generatingMatrices.numCols();
  if (seed < 0) {
...
UInt64Matrix DigitalNet::get_linear_scrambling_matrices(int seed) {
...
void DigitalNet::unsafe_get_points(const size_t nMin, const size_t nMax,
                                   RealMatrix& points) {
  /// Check if a power of 2 number of points is request when using natural
  /// ordering
  if (ordering == DIGITAL_NET_NATURAL_ORDERING && !ispow2(points.numCols())) {
    Cerr << "Error: natural ordering requires the requested number of points "
            "to be "
  ...
  double oneOnPow2tScramble =
      1 / Real(UInt64(1) << digits - 1) / 2;  /// 1 / 2^(-tMax)
  for (UInt64 k = nMin; k < nMax; ++k)        /// Loop over all points
  {
    ...
    {
      points(j, idx - nMin) = (current_point[j] ^ digitalShift[j]) *
                              oneOnPow2tScramble;  // apply digital shift
    }
  }
}
...
void DigitalNet::next(int k, UInt64Vector& current_point) {
  if (k == 0) return;
...
inline UInt64 DigitalNet::reorder_natural(UInt64 k) { return binary2gray(k); }

/// Position of the `k`th digital net point in DIGITAL_NET_GRAY_CODE_ORDERING
inline UInt64 DigitalNet::reorder_gray_code(UInt64 k) { return k; }
