...
SurrogatesGPApprox::SurrogatesGPApprox(const ProblemDescDB& problem_db,
  ...
  if (trend_string == "constant")
    ...
    surrogateOpts.sublist("Trend").sublist("Options").set("max degree", 2);
    surrogateOpts.sublist("Trend").sublist("Options").set("reduced basis",
                                                          true);
  } else if (trend_string == "none")
    surrogateOpts.sublist("Trend").set("estimate trend", false);
  surrogateOpts.sublist("Trend").sublist("Options").set(
      "verbosity", surrogateOpts.get<int>("verbosity"));
...
SurrogatesGPApprox::SurrogatesGPApprox(const SharedApproxData& shared_data)
...
int SurrogatesGPApprox::min_coefficients() const {
...
void SurrogatesGPApprox::build() {
  ...
  if (!advanced_options_file.empty()) {
    model.reset(new dakota::surrogates::GaussianProcess(vars, resp,
                                                        advanced_options_file));
  } else {
    model.reset(
        new dakota::surrogates::GaussianProcess(vars, resp, surrogateOpts));
  }
...
void set_model_gp_options(Model& model, const String& options_file) {
  ...
  for (int i = 0; i < exp_gp_approxs.size(); ++i) {
    auto exp_gp_derived = std::static_pointer_cast<SurrogatesGPApprox>(
