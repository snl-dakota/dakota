...
void lagrange_interpolation_1d(const RealVector &samples,
                               ...
                               const RealVector &values, RealVector &result) {
...
void kronecker_product_2d(const RealMatrix &matrix1, const RealMatrix &matrix2,
  ...
  if ((matrix1.numRows() != matrix2.numRows()) ||
    ...
    throw(
        std::runtime_error("kronecker_product_2d: matrices are inconsistent"));
  }
...
void get_chebyshev_points(int order, RealVector &points) {
  ...
  for (int i = 0; i < order + 1; i++) points[i] = -std::cos((Real)i * h);
}
...
void SpectralDiffusionModel::initialize(int order, String kernel,
                                        const RealVector &bndry_conds,
  ...
  for (int i = 0; i < order_ + 1; i++)
    collocationPoints_(0, i) =
        range * (chebyshev_points[i] + 1.) / 2. + physicalDomainLimits_[0];

  // Transform derivative matrix to account for mapping of points
  // from [-1,1] to [a,b]
  for (int j = 0; j < order_ + 1; j++)
    for (int i = 0; i < order_ + 1; i++) derivativeMatrix_(i, j) *= 2. / range;
  ...
  if (kernel_ == "exponential") {
    ...
    for (int i = 0; i < num_mesh_points; i++) {
      for (int j = 0; j < num_mesh_points; j++) {
        Corr(i, j) = std::exp(-std::pow(
            std::abs(collocationPoints_(0, i) - collocationPoints_(0, j)) / L_,
            p_));
      }
    }
...
void SpectralDiffusionModel::form_collocation_matrix(
    const RealVector &diagonal, RealMatrix &collocation_matrix) const {
...
void SpectralDiffusionModel::apply_boundary_conditions(
    RealMatrix &collocation_matrix, RealVector &forcing) const {
  for (int j = 0; j < order_ + 1; j++) {
    collocation_matrix(0, j) = 0.;
    collocation_matrix(order_, j) = 0.;
  }
  collocation_matrix(0, 0) = 1.;
  collocation_matrix(order_, order_) = 1.;
  forcing[0] = boundaryConditions_[0];
  forcing[order_] = boundaryConditions_[1];
}

void SpectralDiffusionModel::solve(const RealVector &diffusivity,
                                   const RealVector &forcing_in,
...
void SpectralDiffusionModel::interpolate(const RealVector &mesh_values,
  ...
  RealVector mesh_points(Teuchos::View, collocationPoints_.values(),
                         order_ + 1);
...
void SpectralDiffusionModel::diffusivity_function(
    const RealVector &sample, const RealMatrix &mesh_points,
  ...
  if (kernel_ == "exponential") {
    for (int d = 0; d < num_stoch_dims; d++) {
      for (int i = 0; i < num_mesh_points; i++) {
        diffusivity[i] +=
            singularValues_[d] * leftSingularVectors_(i, d) * sample[d];
      }
    }
    for (int i = 0; i < num_mesh_points; i++) {
      if (positivity_)
    ...
    for (int d = 0; d < num_stoch_dims; d++) {
      ...
      for (int i = 0; i < num_mesh_points; i++) {
        diffusivity[i] +=
            std::cos(2. * dPI * mesh_points(0, i)) / (dPI * dPI) * sample[d];
      }
    }
    for (int i = 0; i < num_mesh_points; i++)
      diffusivity[i] = diffusivity[i] * fieldStdDev_ + fieldMean_;
...
void SpectralDiffusionModel::set_num_qoi(int num_qoi) { numQOI_ = num_qoi; }
