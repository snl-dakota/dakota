...
bool nearby(const RealVector& rv1, const RealVector& rv2, Real rel_tol) {
  ...
  if (rv2.length() != len) return false;
...
bool operator==(const ShortArray& dsa1, const ShortArray& dsa2) {
  ...
  if (dsa2.size() != len) return false;

  // Check each value
  for (size_t i = 0; i < len; ++i)
    if (dsa2[i] != dsa1[i]) return false;
...
Real rel_change_L2(const RealVector& curr_rv, const RealVector& prev_rv) {
  ...
  if (!zero_prev) {  // change relative to previous
    ...
    return !Pecos::is_small(scaling) ? std::sqrt(norm / scaling)
                                     : std::sqrt(norm);
  }
}
...
void sort_vector(const RealVector& vec, RealVector& sort_vec,
  ...
  std::sort(indices.values(), indices.values() + vec.length(),
            [&](Real i, Real j) { return vec[i] < vec[j]; });
  ...
  for (int i = 0; i < vec.length(); ++i) sort_vec[i] = vec[indices[i]];
}
...
void sort_matrix_columns(const RealMatrix& mat, RealMatrix& sort_mat,
  ...
  if ((mat.numRows() != sort_mat.numRows()) ||
    ...
    sort_mat.shapeUninitialized(mat.numRows(),
                                mat.numCols());  // avoid initialization to 0.0

  if ((mat.numRows() != indices.numRows()) ||
      (mat.numCols() != indices.numCols()))
    indices.shapeUninitialized(mat.numRows(),
                               mat.numCols());  // avoid initialization to 0.0

  for (int i = 0; i < mat.numCols(); ++i) {
    RealMatrix& nonconst_mat = const_cast<RealMatrix&>(mat);
    const RealVector& unsrt_vec =
        Teuchos::getCol(Teuchos::View, nonconst_mat, i);
...
void reorder_matrix_columns_from_index_vector(const RealMatrix& mat,
  ...
  for (int i = 0; i < num_inds; ++i) {
    Teuchos::setCol(Teuchos::getCol(Teuchos::View, nonconst_mat, indices[i]), i,
                    reordered_mat);
  }
}
...
void center_matrix_rows(RealMatrix& mat) {
  ...
  for (int i = 0; i < num_row; i++) {
    ...
    Real row_mean = 0.0;
    for (int j = 0; j < num_col; j++) row_mean += mat(i, j);
    row_mean /= (Real)num_col;
    for (int j = 0; j < num_col; j++) mat(i, j) -= row_mean;
  }
}
...
void center_matrix_cols(RealMatrix& mat) {
  ...
  for (int j = 0; j < num_col; j++) {
    // normalize each column (input/output factor) by its mean across
    // observations
    Real col_mean = 0.0;
    for (int i = 0; i < num_row; i++) col_mean += mat(i, j);
    col_mean /= (Real)num_row;
    for (int i = 0; i < num_row; i++) mat(i, j) -= col_mean;
  }
}
...
bool is_matrix_symmetric(const RealMatrix& matrix) {
  ...
  if (num_cols != num_rows) return false;
...
void copy_data(const MatrixXd& src_mat, RealMatrix& dst_mat) {
  ...
  for (int i = 0; i < nrows; ++i)
    for (int j = 0; j < ncols; ++j) dst_mat(i, j) = src_mat(i, j);
}
...
void iround(const RealVector& input_vec, IntVector& rounded_vec) {
  ...
  if (rounded_vec.length() != len) rounded_vec.resize(len);
  for (int i = 0; i < len; ++i)
