...
Real rel_change_L2(const RealVector& curr_rv, const RealVector& prev_rv)
{
  ...
  for (i=0; i<rv_len; ++i)
    if (Pecos::is_below(std::abs(prev_rv[i])))
      { zero_prev = true; break; }
  // check current vector for zeros
  if (zero_prev)
    for (i=0; i<rv_len; ++i)
      if (Pecos::is_below(std::abs(curr_rv[i])))
		   ...
		   const RealVector& curr_rv2, const RealVector& prev_rv2)
{
  ...
  bool zero_prev = false, zero_curr = false;
  for (i=0; i<rv1_len; ++i)
    if (Pecos::is_below(std::abs(prev_rv1[i])))
      { zero_prev = true; break; }
  if (!zero_prev)
    for (i=0; i<iv_len; ++i)
  ...
  if (!zero_prev)
    for (i=0; i<rv2_len; ++i)
      if (Pecos::is_below(std::abs(prev_rv2[i])))
	{ zero_prev = true; break; }
  // check current vectors for zeros
  if (zero_prev) {
    for (i=0; i<rv1_len; ++i)
      if (Pecos::is_below(std::abs(curr_rv1[i])))
	{ zero_curr = true; break; }
    if (!zero_prev)
      for (i=0; i<iv_len; ++i)
    ...
    if (!zero_prev)
      for (i=0; i<rv2_len; ++i)
	if (Pecos::is_below(std::abs(curr_rv2[i])))
	  { zero_curr = true; break; }
  }
