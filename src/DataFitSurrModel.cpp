...
static const char rcsId[] =
    "@(#) $Id: DataFitSurrModel.cpp 7034 2010-10-22 20:16:32Z mseldre $";
...
DataFitSurrModel::DataFitSurrModel(ProblemDescDB& problem_db,
    ...
    : SurrogateModel(problem_db, parallel_lib),
      ...
      pointsManagement(
          problem_db.get_short("model.surrogate.points_management")),
      ...
      exportFormat(
          problem_db.get_ushort("model.surrogate.export_approx_format")),
      exportVarianceFile(
          problem_db.get_string("model.surrogate.export_approx_variance_file")),
      exportVarianceFormat(problem_db.get_ushort(
  ...
  if (model_construct) {
    ...
    if (strends(surrogateType, "_orthogonal_polynomial") ||
      ...
      u_space_type =
          PARTIAL_ASKEY_U;  // problem_db.get_short("model.surrogate.expansion_type");
    } else {
  ...
  const Variables& vars =
      (!actualModel) ? currentVariables : actualModel->current_variables();
  bool cache = false;
  ...
  if (0 == currentResponse.field_lengths().length())
    ...
    approxInterface = std::make_shared<ApproximationFieldInterface>(
        problem_db, vars, cache, am_interface_id, currentResponse);

  // initialize the basis, if needed
  if (basis_expansion) shared_approximation().construct_basis(mvDist);
...
DataFitSurrModel::DataFitSurrModel(
    ...
    std::shared_ptr<Model> actual_model,
    // const SharedVariablesData& svd,const SharedResponseData& srd,
    const ActiveSet& dfs_set, const ShortShortPair& dfs_view,
    const String& approx_type, const UShortArray& approx_order, short corr_type,
    short corr_order, short data_order, short output_level,
    const String& point_reuse, const String& import_build_points_file,
    unsigned short import_build_format, bool import_build_active_only,
    const String& export_approx_points_file,
    unsigned short export_approx_format)
    :  // SVD can be shared, but don't share SRD as QoI aggregations are
       // consumed:
      ...
      daceIterator(dace_iterator),
      actualModel(actual_model),
      pointsTotal(0),
      pointsManagement(DEFAULT_POINTS),
      pointReuse(point_reuse),
      exportSurrogate(false),
      exportPointsFile(export_approx_points_file),
      exportFormat(export_approx_format),
      importPointsFile(import_build_points_file),
      autoRefine(false),
      maxIterations(100),
      maxFuncEvals(1000),
      convergenceTolerance(1e-4),
      softConvergenceLimit(0),
      refineCVMetric("root_mean_square"),
      refineCVFolds(10) {
  ...
  userDefinedConstraints.reshape(
      ModelUtils::num_nonlinear_ineq_constraints(*actualModel),
  ...
  approxInterface = std::make_shared<ApproximationInterface>(
      /*dfs_view,*/ approx_type, approx_order,
  ...
  if (import_pts)
    import_points(import_build_format, import_use_var_labels,
...
void DataFitSurrModel::check_submodel_compatibility(const Model& sub_model) {
  ...
  if (err1 || err2 || err3) abort_handler(MODEL_ERROR);
}
...
void DataFitSurrModel::derived_init_communicators(ParLevLIter pl_iter,
...
bool DataFitSurrModel::build_approximation(const Variables& vars,
  ...
  // build a local, multipoint, or global data fit approximation.
  if (strbegins(surrogateType, "local_") ||
      strbegins(surrogateType,
                "multipoint_")) {  // NOTE: branch not used by TRMM
    update_local_reference();
...
void DataFitSurrModel::rebuild_approximation() {
  ...
  if (strbegins(surrogateType, "local_") ||
      strbegins(surrogateType, "multipoint_")) {
    // update_local_reference();//updates from build_approximation() remain
    // valid
    build_local_multipoint();  // no change for build vs. rebuild
  } else {                     // global approximation
    // update_global_reference();// updates from build_approximation remain
    // valid
    rebuild_global();
  }
  ...
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\n<<<<< " << surrogateType
         << " approximation rebuilds completed.\n";
}
...
/** This function populates/replaces SurrogateData::anchor{Vars,Resp}
    ...
    updates data from a previous call to build_approximation(), and is
    not intended to be used in isolation. */
void DataFitSurrModel::update_approximation(const Variables& vars,
                                            const IntResponsePair& response_pr,
                                            bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\n>>>>> Updating " << surrogateType << " approximations.\n";
  ...
  if (rebuild_flag) rebuild_approximation(response_pr);
...
/** This function populates/replaces SurrogateData::{vars,resp}Data
    ...
    updates data from a previous call to build_approximation(), and is
    not intended to be used in isolation. */
void DataFitSurrModel::update_approximation(const VariablesArray& vars_array,
  ...
  if (rebuild_flag) rebuild_approximation(resp_map);
...
/** This function populates/replaces SurrogateData::{vars,resp}Data
    ...
    updates data from a previous call to build_approximation(), and is
    not intended to be used in isolation. */
void DataFitSurrModel::update_approximation(const RealMatrix& samples,
                                            const IntResponseMap& resp_map,
                                            bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\n>>>>> Updating " << surrogateType << " approximations.\n";
...
void DataFitSurrModel::append_approximation(const Variables& vars,
                                            const IntResponsePair& response_pr,
                                            bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
...
void DataFitSurrModel::append_approximation(const RealMatrix& samples,
                                            const IntResponseMap& resp_map,
                                            bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\n>>>>> Appending to " << surrogateType << " approximations.\n";
...
/** This function appends multiple points to SurrogateData::{vars,resp}Data
    ...
    from a previous call to build_approximation(), and is not intended to
    be used in isolation. */
void DataFitSurrModel::append_approximation(const VariablesArray& vars_array,
                                            const IntResponseMap& resp_map,
                                            bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\n>>>>> Appending to " << surrogateType << " approximations.\n";
...
/** This function appends multiple points to SurrogateData::{vars,resp}Data
    ...
    from a previous call to build_approximation(), and is not intended to
    be used in isolation. */
void DataFitSurrModel::append_approximation(const IntVariablesMap& vars_map,
                                            const IntResponseMap& resp_map,
                                            bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\n>>>>> Appending to " << surrogateType << " approximations.\n";
...
void DataFitSurrModel::replace_approximation(const IntResponsePair& response_pr,
                                             bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
...
void DataFitSurrModel::replace_approximation(const IntResponseMap& resp_map,
                                             bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
...
void DataFitSurrModel::pop_approximation(bool save_surr_data,
                                         bool rebuild_flag) {
  if (outputLevel >= NORMAL_OUTPUT)
...
void DataFitSurrModel::update_global_reference() {
  ...
  if (actualModel && actualModel->model_type() == "recast") {
    ...
    copy_data(ModelUtils::discrete_int_lower_bounds(*sub_model),
              referenceDILBnds);
    copy_data(ModelUtils::discrete_int_upper_bounds(*sub_model),
              referenceDIUBnds);
    copy_data(ModelUtils::discrete_real_lower_bounds(*sub_model),
              referenceDRLBnds);
    copy_data(ModelUtils::discrete_real_upper_bounds(*sub_model),
              referenceDRUBnds);
  } else {
    const Constraints& cons = (!actualModel)
...
void DataFitSurrModel::update_approx_interface(
...
void DataFitSurrModel::build_approx_interface() {
  ...
  if (exportSurrogate) {
    ...
    for (auto approx : approximations()) approx.export_model(currentVariables);
  }
}
...
void DataFitSurrModel::build_global() {
  ...
  if (pointReuse == "all" || pointReuse == "region") {
    ...
    for (prp_iter = data_pairs.begin(); prp_iter != data_pairs.end();
         ++prp_iter) {
      ...
      if (prp_iter->interface_id() == am_interface_id && consistent(prp_vars)) {
        ...
        if (inside(db_vars) &&
          ...
          approxInterface->append_approximation(
              db_vars, std::make_pair(prp_iter->eval_id(), db_resp));
          ++reuse_points;
  ...
  if (!daceIterator) {  // reused/imported data only (no new data)
    ...
    if (reuse_points < min_points) {
      Cerr << "Error: a minimum of " << min_points << " points is required by "
           ...
           << reuse_points << " were provided." << std::endl;
      abort_handler(MODEL_ERROR);
    }
  } else {  // new data
    ...
    daceIterator->sampling_reset(diff_points, true,
                                 false);  // update s.t. lwr bnd
  ...
  Cout << "Constructing global approximations with " << anchor_str
       ...
       << " reused points.\n";
  if (autoRefine)
    refine_surrogate();  // BMA TODO: Move to an external refiner
  else
    build_approx_interface();
...
void DataFitSurrModel::rebuild_global() {
  ...
  if (!daceIterator) {  // reused/imported data only (no new data)
    ...
    if (curr_points < min_points) {
      Cerr << "Error: a minimum of " << min_points << " points is required by "
           << "DataFitSurrModel::build_global.\n"
           << curr_points << " were provided." << std::endl;
      abort_handler(MODEL_ERROR);
    }
  } else {  // new data
    ...
    if (diff_points) {  // only run the iterator if work to do
      ...
      build_approx_interface();
      ++approxBuilds;  // Note: this is a replacement rather than an
                       // increment...
    } else if (outputLevel >= DEBUG_OUTPUT)
...
void DataFitSurrModel::refine_surrogate() {
  ...
  accumulator_set<Real, stats<tag::rolling_mean> > mean_err(
      tag::rolling_window::window_size = softConvergenceLimit);
  ...
  for (size_t i = 0; i < resp_fns; ++i) cv_per_fn[i] = cv_diags[i][0];
...
bool DataFitSurrModel::consistent(const Variables& vars) const {
  size_t i, num_acv = vars.acv(), num_adiv = vars.adiv(),
            num_adsv = vars.adsv(), num_adrv = vars.adrv(),
            cv_start = vars.cv_start(), div_start = vars.div_start(),
            dsv_start = vars.dsv_start(), drv_start = vars.drv_start(),
            num_cv = vars.cv(), num_div = vars.div(), num_dsv = vars.dsv(),
            num_drv = vars.drv();
  ...
  for (i = 0; i < div_start; ++i)
    if (adiv[i] != am_adiv[i]) return false;
  ...
  for (i = 0; i < dsv_start; ++i)
    if (adsv[i] != am_adsv[i]) return false;
...
bool DataFitSurrModel::inside(const Variables& vars) const {
  ...
  if (pointReuse == "region") {
    ...
    for (i = 0; i < num_drv; ++i)
      if (drv[i] < dr_l_bnds[i] || drv[i] > dr_u_bnds[i]) return false;
  }
...
void DataFitSurrModel::derived_evaluate(const ActiveSet& set) {
  ...
  ShortArray approx_asv, actual_asv;
  bool actual_eval, approx_eval, mixed_eval;
  ...
  switch (responseMode) {
    ...
    case AUTO_CORRECTED_SURROGATE:
      ...
      approx_eval = !approx_asv.empty();
      actual_eval = !actual_asv.empty();
      mixed_eval = (approx_eval && actual_eval);
      break;
  ...
  if (hierarchicalTagging) {
    String eval_tag =
        evalTagPrefix + '.' + std::to_string(surrModelEvalCntr + 1);
    if (actual_eval) actualModel->eval_tag_prefix(eval_tag);
  }
  ...
  if (approx_eval) {  // normal case: evaluation of approxInterface
    ...
    // component_parallel_mode(SURROGATE_MODEL_MODE); // does not use
    // parallelism ParConfigLIter pc_iter =
    ...
    if (interfEvaluationsDBState == EvaluationsDBState::UNINITIALIZED)
      interfEvaluationsDBState = evaluationsDB.interface_allocate(
          modelId, approxInterface->interface_id(), "approximation",
    ...
    switch (responseMode) {
      ...
      case AUTO_CORRECTED_SURROGATE: {
        ...
        approx_response =
            (mixed_eval) ? currentResponse.copy() : currentResponse;
        ...
        if (interfEvaluationsDBState == EvaluationsDBState::ACTIVE) {
          evaluationsDB.store_interface_variables(
              modelId, approxInterface->interface_id(),
    ...
    switch (responseMode) {
      case AUTO_CORRECTED_SURROGATE: {
        ...
        //   deltaCorr.compute(currentVariables, centerResponse,
        //   approx_response,
        //                     quiet_flag);
...
void DataFitSurrModel::derived_evaluate_nowait(const ActiveSet& set) {
  ...
  if (actual_eval) {
    ...
    switch (responseMode) {
      ...
      case AUTO_CORRECTED_SURROGATE: {
        ActiveSet actual_set = set;
        actual_set.request_vector(actual_asv);
        actualModel->evaluate_nowait(actual_set);
        break;
      }
      case BYPASS_SURROGATE:
      case MODEL_DISCREPANCY:
      case AGGREGATED_MODEL_PAIR:
        actualModel->evaluate_nowait(set);
        break;
    }
  ...
  if (approx_eval) {  // normal case: evaluation of approxInterface
    ...
    if (interfEvaluationsDBState == EvaluationsDBState::ACTIVE)
      evaluationsDB.interface_allocate(
          modelId, approxInterface->interface_id(), "approximation",
          currentVariables, currentResponse, default_interface_active_set(),
          approxInterface->analysis_components());
    ...
    switch (responseMode) {
      ...
      case AUTO_CORRECTED_SURROGATE: {
        ActiveSet approx_set = set;
        approx_set.request_vector(approx_asv);
        approxInterface->map(currentVariables, approx_set, currentResponse,
                             true);
        if (interfEvaluationsDBState == EvaluationsDBState::ACTIVE)
          evaluationsDB.store_interface_variables(
              modelId, approxInterface->interface_id(),
  ...
  switch (responseMode) {
    ...
    default:  // {UN,AUTO_}CORRECTED_SURROGATE modes
      ...
      while (act_it != actual_resp_map_rekey.end() ||
             app_it != approx_resp_map_rekey.end()) {
        int act_eval_id =
            (act_it == actual_resp_map_rekey.end()) ? INT_MAX : act_it->first;
        int app_eval_id =
            (app_it == approx_resp_map_rekey.end()) ? INT_MAX : app_it->first;
  ...
  while (act_it != actual_resp_map_rekey.end() ||
    ...
    if (act_eval_id < app_eval_id) {  // only actual available
      ...
      switch (responseMode) {
        case MODEL_DISCREPANCY:
          deltaCorr.compute(act_it->second, app_it->second,
                            surrResponseMap[act_eval_id], quiet_flag);
          break;
        case AGGREGATED_MODEL_PAIR:
          aggregate_response(app_it->second, act_it->second,
                             surrResponseMap[act_eval_id]);
...
void DataFitSurrModel::derived_synchronize_approx(
    bool block, IntResponseMap& approx_resp_map_rekey) {
  ...
  if (responseMode == AUTO_CORRECTED_SURROGATE && corrType) {
    ...
    for (r_it = approx_resp_map_rekey.begin(), v_it = rawVarsMap.begin();
      ...
      if (export_pts) export_point(r_it->first, v_it->second, r_it->second);
    }
    rawVarsMap.clear();
  } else if (export_pts) {
    IntVarsMIter v_it;
  ...
  for (r_it = cachedApproxRespMap.begin(); r_it != cachedApproxRespMap.end();
       ++r_it)
    approx_resp_map_rekey[r_it->first] = r_it->second;
  cachedApproxRespMap.clear();
}

void DataFitSurrModel::asv_inflate_build(const ShortArray& orig_asv,
                                         ShortArray& actual_asv) {
  // DataFitSurrModel consumes replicates from any response aggregations
  // occurring in actualModel
  size_t num_orig = orig_asv.size(),
         num_actual = ModelUtils::response_size(*actualModel);
...
void DataFitSurrModel::asv_split(const ShortArray& orig_asv,
                                 ShortArray& approx_asv,
                                 ShortArray& actual_asv) {
  if (!actualModel || surrogateFnIndices.size() == numFns) {
    approx_asv = orig_asv;
    return;
  }  // don't inflate approx_asv
  // else mixed response set
    ...
    if (orig_asv_val) {
      if (surrogateFnIndices.count(index)) {
        ...
        if (actual_asv.empty())  // keep empty if no active requests
          actual_asv.assign(num_actual, 0);
        for (size_t i = index; i < num_actual;
             i += num_orig)  // inflate actual_asv
          actual_asv[i] = orig_asv_val;
      }
    }
  }
...
void DataFitSurrModel::import_points(unsigned short tabular_format,
  ...
  String context_msg =
      "Surrogate model with id '" + model_id() + "' import_build_points";
  ...
  if (cache || restart) {
    ...
    for (prp_it = import_prp_list.begin(); prp_it != import_prp_list.end();
         ++prp_it) {
      ...
      if ((tabular_format & TABULAR_IFACE_ID) == 0 &&
          !am_iface_id.empty()) {      // not imported: dangerous!
...
void DataFitSurrModel::export_point(int eval_id, const Variables& vars,
                                    const Response& resp) {
  ...
  if (recastings()) {
    ...
    if (!exportPointsFile.empty())
      TabularIO::write_data_tabular(exportFileStream, export_vars,
                                    interface_id(), export_resp, eval_id,
                                    exportFormat);
    if (!exportVarianceFile.empty()) {
      // BMA TODO WARN or SKIP: exported variance not transformed?!?
      // Can't in general map it through a recast model...
      TabularIO::write_data_tabular(exportVarianceFileStream, export_vars,
                                    interface_id(), response_variance, eval_id,
                                    exportVarianceFormat);
    }

  } else {
    if (!exportPointsFile.empty())
      TabularIO::write_data_tabular(exportFileStream, vars, interface_id(),
                                    resp, eval_id, exportFormat);
    if (!exportVarianceFile.empty()) {
      TabularIO::write_data_tabular(exportVarianceFileStream, vars,
                                    interface_id(), response_variance, eval_id,
                                    exportVarianceFormat);
    }
  }
...
IntIntPair DataFitSurrModel::estimate_partition_bounds(
    int max_eval_concurrency) {
...
void DataFitSurrModel::declare_sources() {
  switch (responseMode) {
    ...
    case AUTO_CORRECTED_SURROGATE:
      if (!actualModel || surrogateFnIndices.size() == numFns) {
        evaluationsDB.declare_source(modelId, "surrogate",
                                     approxInterface->interface_id(),
                                     "approximation");
      } else if (surrogateFnIndices
                     .empty()) {  // don't know if this can happen.
        evaluationsDB.declare_source(modelId, "surrogate",
...
ActiveSet DataFitSurrModel::default_interface_active_set() {
  ...
  const bool has_gradients =
      gradientType != "none" && has_deriv_vars &&
  ...
  if (responseMode == MODEL_DISCREPANCY ||
      responseMode == AGGREGATED_MODEL_PAIR || !actualModel ||
    ...
    for (size_t i = 0; i < numFns; ++i) {
      if (surrogateFnIndices.count(i)) {
        asv[i] = 1;
        if (has_gradients) asv[i] |= 2;
        if (has_hessians) asv[i] |= 4;
      }
    }
  }
