...
{
  lineColor[0] = 192;
  lineColor[1] = lineColor[2] = 0;  // red (default)
...
void Graph2D::add_window(Widget &parent) {
  ...
  plotBack =
      XmCreateForm(parent, const_cast<char *>(name_str.c_str()), args, 2);
  XtManageChild(plotBack);
  // workaround to pass string literal to char* interface
  std::string text_str("Options");
  XmString xmstr1 = XmStringCreateLtoR(const_cast<char *>(text_str.c_str()),
                                       XmFONTLIST_DEFAULT_TAG);
  vertWheel = XtVaCreateManagedWidget(
      "wheel_R", xfwfThumbWheelWidgetClass, plotBack, XtNvertical, True,
      XmNrightAttachment, XmATTACH_POSITION, XmNtopAttachment,
      XmATTACH_POSITION, XmNtopPosition, 25, XmNrightPosition, 99, XmNresizable,
      True, XmNrubberPositioning, True, NULL);
  horizWheel = XtVaCreateManagedWidget(
      "wheel_R", xfwfThumbWheelWidgetClass, plotBack, XtNvertical, False,
      XmNtopAttachment, XmATTACH_POSITION, XmNtopPosition, 90,
      XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 70, XmNresizable,
      True, XmNrubberPositioning, True, NULL);
  optButton = XtVaCreateManagedWidget(
      "opt_button", xmPushButtonWidgetClass, plotBack, XtNwidth, 55, XtNheight,
      20, XmNtopAttachment, XmATTACH_POSITION, XmNtopPosition, 90,
      XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 25,
      XmNlabelString, xmstr1,
      // XmNrightAttachment,  XmATTACH_WIDGET,
      // XmNrightWidget , horizWheel,
      XmNresizable, True, XmNrubberPositioning, False, NULL);
  ...
  plotWidget = XtVaCreateManagedWidget(
      "plot", sciplotWidgetClass, plotBack, XtNwidth, 150, XtNheight, 150,
      XtNshowTitle, True, XtNshowLegend, False, XtNshowYLabel, False,
      XtNshowXLabel, False, XtNplotTitle, "Iteration History", XtNxLabel,
      "Evaluation Number", XtNyLabel, "Attribute Value", XtNtitleFont,
      XtFONT_TIMES | 18, XtNlabelFont, XtFONT_TIMES | 14, XtNtitleMargin, 2,
      XtNdrawMinorTics, False, XtNdrawMinor,
      False,  // XtNyLog, True,
      XtNdefaultMarkerSize, 3, XtNchartType, XtCARTESIAN, XmNresizable, True,
      XmNtopAttachment, XmATTACH_POSITION, XmNleftAttachment, XmATTACH_POSITION,
      XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget, vertWheel,
      XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, horizWheel,
...
void Graph2D::change_colors() {
  ...
  // yMin marker
  bColor =
      SciPlotAllocRGBColor(plotWidget, 0, 0, 255);  // blue (max saturation)
...
void Graph2D::draw_plot() {
  ...
  if (numEntries > 1 && (hWheelIncr || vWheelIncr)) {
    double x_view_min = xMin, x_view_max = xMax, y_view_min = yMin,
           y_view_max = yMax;
      ...
      for (int i = xyListFirstId; i <= xyListCurrId; i++) {
        ...
        for (int j = 0; j < num_xy_pairs; j++) {
          ...
          if (x >= x_view_min && x <= x_view_max) {  // visible w/i reduced view
            ...
            if (y > y_view_max) y_view_max = y;
            if (y < y_view_min) y_view_min = y;
          }
        }
      }
      // hWheelChange = false;
    ...
    cout << "x: (" << x_view_min << ", " << x_view_max << ") y: (" << y_view_min
         << ", " << y_view_max << ")" << endl;
...
void Graph2D::set_options() {
  ...
  if (markersOn == True) {  // activate markers by specifying a point style
    ...
    SciPlotListSetStyle(plotWidget, yMinListId, bColor, XtMARKER_NONE, bColor,
                        XtLINE_NONE);
    SciPlotListSetStyle(plotWidget, yMaxListId, rColor, XtMARKER_NONE, rColor,
                        XtLINE_NONE);
  }
...
void Graph2D::options_callback(Widget widget, XtPointer client_data,
  ...
  Widget Shell =
      XtVaCreateWidget("Options", xmDialogShellWidgetClass, graph->topLevel,
  ...
  Widget Label = XtVaCreateManagedWidget(
      "Graph Options", xmLabelWidgetClass, rc, XmNleftAttachment, XmATTACH_FORM,
      XmNleftOffset, 24, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 8,
      XmNnoResize, True, NULL);
  Widget Frame = XtVaCreateManagedWidget(
      "frame1", xmFrameWidgetClass, rc, XmNleftAttachment, XmATTACH_FORM,
      XmNleftOffset, 24, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 24,
      ...
      NULL);
  Widget RowCol =
      XtVaCreateManagedWidget("RowCol", xmRowColumnWidgetClass, Frame, NULL);
  ...
  widget_name = "    Markers";
  graph->markersButton = XmCreateToggleButton(
      RowCol, const_cast<char *>(widget_name.c_str()), NULL, 0);
  ...
  graph->axisLabelsButton = XmCreateToggleButton(
      RowCol, const_cast<char *>(widget_name.c_str()), NULL, 0);
  // set the button to selected/checked or unselected/unchecked
  ...
  widget_name = "    Legend";
  graph->legendButton = XmCreateToggleButton(
      RowCol, const_cast<char *>(widget_name.c_str()), NULL, 0);
  // set the button to selected/checked or unselected/unchecked
  ...
  graph->yLogButton = XmCreateToggleButton(
      RowCol, const_cast<char *>(widget_name.c_str()), NULL, 0);
  ...
  graph->xLogButton = XmCreateToggleButton(
      RowCol, const_cast<char *>(widget_name.c_str()), NULL, 0);
  ...
  Widget print_button = XtVaCreateManagedWidget(
      "Print", xmPushButtonWidgetClass, RowCol, XmNx, 10, XmNy, 70, NULL, 0);
  Widget RowCol2 = XtVaCreateManagedWidget("RowCol", xmRowColumnWidgetClass, rc,
                                           XmNtopAttachment, XmATTACH_WIDGET,
                                           XmNtopWidget, Frame, NULL);
  Widget okay_button = XtVaCreateManagedWidget(
      "  OK  ", xmPushButtonWidgetClass, rc, XmNx, 10, XmNy, 10,
      XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 8, XmNtopAttachment,
      XmATTACH_WIDGET, XmNtopWidget, Frame, XmNtopOffset, 8, XmNleftAttachment,
      XmATTACH_FORM, XmNleftOffset, 24, NULL, 0);
  Widget cancel_button = XtVaCreateManagedWidget(
      "Cancel", xmPushButtonWidgetClass, rc, XmNbottomAttachment, XmATTACH_FORM,
      XmNbottomOffset, 8, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
      Frame, XmNtopOffset, 8, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset,
      24, XmNx, 50, XmNy, 10, NULL, 0);
