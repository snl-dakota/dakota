...
class NonDSampling : public NonD {
 public:
  ...
  static int compute_wilks_sample_size(unsigned short order, Real alpha,
                                       Real beta, bool twosided = false);

  /// Helper function - calculates the Wilks residual
  static Real compute_wilks_residual(unsigned short order, int nsamples,
                                     Real alpha, Real beta, bool twosided);

  /// calculates the alpha paramter given number of samples using the Wilks
  /// formula Static so I can test without instantiating a NonDSampling object -
  /// RWH
  static Real compute_wilks_alpha(unsigned short order, int nsamples, Real beta,
                                  bool twosided = false);

  /// calculates the beta parameter given number of samples using the Wilks
  /// formula Static so I can test without instantiating a NonDSampling object -
  /// RWH
  static Real compute_wilks_beta(unsigned short order, int nsamples, Real alpha,
                                 bool twosided = false);
 ...
 protected:
  ...
  /// constructor
  NonDSampling(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
  ...
  void get_parameter_sets(std::shared_ptr<Model> model,
                          const size_t num_samples, RealMatrix& design_matrix,
                          bool write_msg);
  ...
  void get_parameter_sets(const RealVector& means, const RealVector& std_devs,
...
inline void NonDSampling::pre_run() {
  ...
  if (subIteratorFlag) active_set_mapping();
}

inline void NonDSampling::compute_moments(const RealVectorArray& fn_samples) {
  SizetArray sample_counts;
  compute_moments(fn_samples, sample_counts, momentStats, finalMomentsType,
...
inline void NonDSampling::print_intervals(std::ostream& s) const {
  print_intervals(s, "response function",
                  ModelUtils::response_labels(*iteratedModel));
}

inline void NonDSampling::print_moments(
    std::ostream& s, String qoi_type, const StringArray& moment_labels) const {
...
inline size_t NonDSampling::num_samples() const { return numSamples; }
...
inline void NonDSampling::sampling_reset(size_t min_samples, bool all_data_flag,
                                         bool stats_flag) {
...
inline void NonDSampling::random_seed(int seed) { /*seedSpec = */
  randomSeed = seed;
}
...
inline void NonDSampling::transform_samples(Model& src_model,
                                            Model& tgt_model) {
  Pecos::ProbabilityTransformation& nataf =
      tgt_model.probability_transformation();

  transform_samples(nataf, allSamples,
                    ModelUtils::continuous_variable_ids(src_model),
                    ModelUtils::continuous_variable_ids(tgt_model), true);
}

/** transform x_samples to u_samples for use by expansionSampler */
inline void NonDSampling::transform_samples(
    Pecos::ProbabilityTransformation& nataf, bool x_to_u) {
...
inline void NonDSampling::transform_samples(
    Pecos::ProbabilityTransformation& nataf, RealMatrix& sample_matrix,
    bool x_to_u) {
  // No model recursion available, assume same x/u ids for mapping:
  SizetMultiArrayConstView cv_ids =
      ModelUtils::continuous_variable_ids(*iteratedModel);
...
inline void NonDSampling::sample_to_cv(const Real* sample_vars, Variables& vars,
                                       size_t& acv_index, size_t num_acv,
                                       size_t& samp_index) {
...
inline void NonDSampling::sample_to_div(const Real* sample_vars,
                                        Variables& vars, size_t& adiv_index,
...
inline void NonDSampling::sample_to_dsv(const Real* sample_vars,
                                        Variables& vars, size_t& adsv_index,
...
inline void NonDSampling::sample_to_drv(const Real* sample_vars,
                                        Variables& vars, size_t& adrv_index,
...
inline void NonDSampling::sample_to_type(
    const Real* sample_vars, Variables& vars, size_t& cv_index, size_t num_cv,
    size_t& div_index, size_t num_div, size_t& dsv_index, size_t num_dsv,
    size_t& drv_index, size_t num_drv, size_t& samp_index, Model& model) {
...
inline void NonDSampling::sample_to_cv_type(
    const Real* sample_vars, Variables& vars, size_t& cv_index, size_t num_cv,
    size_t& div_index, size_t num_div, size_t& dsv_index, size_t num_dsv,
    size_t& drv_index, size_t num_drv, size_t& samp_index)  //, Model& model)
{
...
inline void NonDSampling::update_model_from_sample(Model& model,
                                                   const Real* sample_vars) {
  sample_to_variables(sample_vars, model.current_variables(), model);
}

inline void NonDSampling::sample_to_variables(const Real* sample_vars,
                                              Variables& vars) {
