...
static const char rcsId[] =
    "@(#) $Id: NonDGPImpSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDGPImpSampling::NonDGPImpSampling(ProblemDescDB& problem_db,
  ...
  gpBuild = std::make_shared<NonDLHSSampling>(iteratedModel, sample_type,
  ...
  gpModel = std::make_shared<DataFitSurrModel>(
      gpBuild, iteratedModel, gp_set, gp_view, approx_type, approx_order,
  ...
  if (numEmulEval == 0) numEmulEval = 10000;
  construct_lhs(gpEval, gpModel, sample_type, numEmulEval, randomSeed, rngName,
                vary_pattern);
...
void NonDGPImpSampling::core_run() {
  ...
  // This piece of code prints out the build points, not the approximation
  // points. const Pecos::SurrogateData& gp_data =
  // gpModel->approximation_data(0);
  ...
  // function and response level and calculate the failure probability. We will
  // need to add error handling:  we will only be calculating results per
  // response level, not probability level or reliability index.
  ...
  for (resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++) {
    ...
    const Pecos::SurrogateData& gp_data =
        gpModel->approximation_data(resp_fn_count);
    ...
    for (level_count = 0; level_count < num_levels; level_count++) {
      ...
      Cout << "Threshold level is "
           << requestedRespLevels[resp_fn_count][level_count] << '\n';
      ...
      for (j = 0; j < numSamples; j++) {
        indicator(j) = static_cast<double>(
            (z - sdr_array[j].response_function()) * cdfMult > 0.0);
        if (outputLevel > NORMAL_OUTPUT)
          Cout << "indicator(" << j << ")=" << indicator(j) << '\n';
      }

      // rho0 is the original PDF, rho1 is the compon importance pdf, rho2 is
      // the distribution used to generate candidate points for the emulator For
      // the initial implementation, we assume rho1 is uniform, and rho0 and
      // rho2 are uniform also with the same distribution.
      ...
      for (k = 0; k < numPtsAdd; k++) {
        ...
        for (j = 0, resp_it = all_resp.begin(); j < numEmulEval;
             ++j, ++resp_it) {
        ...
        // calculate distribution pdfs required to calculate the draw
        // distribution distribution_pdf(rhoEmul2); distribution_pdf(rhoEmul0);
        ...
        while ((iter < 20) && (temp_norm_const * numEmulEval < 25)) {
          ...
          for (i = 0; i < numEmulEval; i++) {
            temp_cvars = Teuchos::getCol(
                Teuchos::View, const_cast<RealMatrix&>(this_samples), i);
            gpCvars[i] = temp_cvars;
            // Cout << "input is " << gpCvars[i] << '\n';
            //  update gpModel currentVariables for use in approx_variances()
            ModelUtils::continuous_variables(*gpModel, temp_cvars);
          ...
          for (j = 0, resp_it = this_resp.begin(); j < numEmulEval;
               ++j, ++resp_it) {
            RealVector temp_resp(numFunctions);
            for (i = 0; i < numFunctions; i++)
              temp_resp(i) = resp_it->second.function_value(i);
          ...
          for (j = 0; j < numEmulEval; j++) temp_norm_this += rhoDraw(j);
        ...
        gpModel->append_approximation(iteratedModel->current_variables(),
                                      resp_truth, true);
        indicator(numSamples + k) = static_cast<double>(
            (z - sdr_array[numSamples + k].response_function()) * cdfMult >
            0.0);
      ...
      //
      // This is where we need some re-architecting.  I want to evaluate the
      // GPmodel at a set of pre-defined points.  We will need to use a
      // parameter list study. something like the following Iterator listStudy;
      // The list of points will be the full X data, consisting of the original
      // plus added points We have this stored in the Surrogate data
      ...
      // for now, since we are assuming rho0=rho1=rho2=all uniform, just set
      // rho1 to 1.
      for (j = 0; j < numPtsTotal; j++)
        rhoOne(j) =
            rho1const;  // for uniform this should be 1.0/prod(xmax-xmin) across
                        // all dimensions... ok now I see this isn't rhoOne it's
                        // rhoOne/rhoZero... if rho0=rho1=rho2 this should be ok

      for (j = 0; j < numPtsTotal; j++) rhoMix(j) = rhoOne(j) * numSamples;

      for (j = numPtsAdd - 1; j >= 0; j--) {
        if (normConst(j) == 0.0) {
          for (k = 0; k < numPtsTotal; k++) rhoMix(k) = rhoMix(k) + rhoOne(k);
        } else {
          ...
          for (k = 0; k < numPtsTotal; k++) {
            ...
            this_var =
                gpModel->approximation_variances(gpModel->current_variables());
            exp_ind_this(k) =
                calcExpIndPoint(resp_fn_count, z, this_mean, this_var);
            if (outputLevel > NORMAL_OUTPUT)
        ...
        // Since we need to evaluate the SUCCESSIVE SEQUENCES of GPs built using
        // numSamples-->numPtsTotal it might be most efficient to "pop" the data
        // and go backward: listStudy.run(pl_iter); obtain results and expected
        // indicator functions gpModel->pop_approximation();
        //
      }
      if (outputLevel > NORMAL_OUTPUT) {
        Cout << "rhoMix " << rhoMix << '\n';
      ...
      for (j = 0; j < numPtsTotal; j++) fract_fail_mix += indicator(j);
...
void NonDGPImpSampling::calcRhoDraw() {
  ...
  for (i = 0; i < numEmulEval; i++) {
    if (expIndicator(i) != 0.0) {
      ...
      // for now this is OK because rho0const = rho2const, will need to change
      // this
...
RealVector NonDGPImpSampling::calcExpIndicator(const int resp_fn_count,
                                               const Real respThresh) {
  int i, j;
  ...
  for (i = 0; i < numEmulEval; i++) {
    ...
    // Cout << "EI " << ei(i) << " respThresh= " << respThresh << " mu= " <<
    // gpMeans[i][resp_fn_count] << " stdv= " << stdv << '\n';
  }
  return ei;
}

Real NonDGPImpSampling::calcExpIndPoint(const int resp_fn_count,
                                        const Real respThresh,
                                        const RealVector this_mean,
                                        const RealVector this_var) {
  int i, j;
  Real ei;
  ...
  // Cout << "EI " << ei << " respThresh= " << respThresh << " mu= " <<
  // this_mean(resp_fn_count) << " stdv= " << stdv << '\n';
