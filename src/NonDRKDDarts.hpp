...
/** The NonDRKDDart class recursively implements the numerical integration of a
 * domain based on k-d flat samples. */
...
class NonDRKDDarts : public NonD {
 public:
  ...
  NonDRKDDarts(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
 ...
 protected:
  ...
  void create_new_sample(size_t parent, size_t left, size_t right,
                         double position);
  ...
  double interpolate_lagrange(size_t num_data_points, double* data_x,
                              double* data_f, double x);

  double integrate_legendre_gauss(double xmin, double xmax,
                                  size_t num_data_points, double* data_x,
                                  double* data_f, double& err_est);
 ...
 private:
  ...
  size_t _num_dim, _num_samples, _max_num_samples, _num_evaluations,
      _evaluation_budget;  // numberof dimensions, size of samples containers,
                           // _number of function evaluations

  size_t _max_num_neighbors;  // maximum number of neighbor to conside in
                              // building lagrange interpolant in each cell

  double _bounding_box_volume;

  double _discont_jump_threshold;  // discontinuity jump threshold for
                                   // disconnecting neighbors across a
                                   // discontinuity
  ...
  double* _sample_value;  // Sample value (either through integration of
                          // children or direct function evaluation)
