...
static const char rcsId[] =
    "@(#) $Id: NPSOLOptimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
NPSOLOptimizer::NPSOLOptimizer(ProblemDescDB& problem_db,
...
NPSOLOptimizer::NPSOLOptimizer(std::shared_ptr<Model> model)
    : Optimizer(NPSOL_SQP, model,
                std::shared_ptr<TraitsBase>(new NPSOLTraits())),
      SOLBase(model),
      setUpType("model") {
  // invoke SOLBase set function (shared with NLSSOLLeastSq)
  set_options(speculativeFlag, vendorNumericalGradFlag, outputLevel, -1, 1.e-10,
              0.9, maxIterations, constraintTol, convergenceTol,
              iteratedModel->gradient_type(),
              iteratedModel->fd_gradient_step_size());
}
...
NPSOLOptimizer::NPSOLOptimizer(
    ...
    Real fn_precision, Real feas_tol, Real lin_feas_tol, Real nonlin_feas_tol)
  ...
  aggregate_bounds(cv_lower_bnds, cv_upper_bnds, lin_ineq_lower_bnds,
                   ...
                   nonlin_ineq_upper_bnds, nonlin_eq_targets, lowerBounds,
                   upperBounds);
...
NPSOLOptimizer::~NPSOLOptimizer() {
  ...
  if (setUpType == "user_functions") deallocate_arrays();
}
...
NPSOLOptimizer* new_NPSOLOptimizer3(
    const RealVector& cv_initial, const RealVector& cv_lower_bnds,
    const RealVector& cv_upper_bnds, const RealMatrix& lin_ineq_coeffs,
    const RealVector& lin_ineq_lower_bnds,
    const RealVector& lin_ineq_upper_bnds, const RealMatrix& lin_eq_coeffs,
  ...
  return 0;
#else
  return new NPSOLOptimizer(
      cv_initial, cv_lower_bnds, cv_upper_bnds, lin_ineq_coeffs,
      lin_ineq_lower_bnds, lin_ineq_upper_bnds, lin_eq_coeffs, lin_eq_targets,
      nonlin_ineq_lower_bnds, nonlin_ineq_upper_bnds, nonlin_eq_targets,
      user_obj_eval, user_con_eval, derivative_level, conv_tol);
#endif  // DAKOTA_DYNLIB
}
#endif  // HAVE_DYNLIB_FACTORIES

void NPSOLOptimizer::update_callback_data(
    const RealVector& cv_initial, const RealVector& cv_lower_bnds,
    const RealVector& cv_upper_bnds, const RealMatrix& lin_ineq_coeffs,
...
void NPSOLOptimizer::objective_eval(int& mode, int& n, double* x, double& f,
                                    double* gradf, int& nstate) {
  ...
  // if ( !(solInstance->derivLevel & 1) && (asv_request & 2) ) { // more
  // general
  ...
  if (asv_request && npsolInstance->numNonlinearConstraints == 0) {
    ...
    ModelUtils::continuous_variables(*npsolInstance->iteratedModel,
                                     local_des_vars);
    npsolInstance->activeSet.request_values(asv_request);
    npsolInstance->iteratedModel->evaluate(npsolInstance->activeSet);
  ...
  if (asv_request & 2) {
    ...
    if (max_flag)
      for (size_t i = 0; i < n; ++i) gradf[i] = -local_grad[i];
    else
...
void NPSOLOptimizer::core_run() {
  if (setUpType == "model")
    ...
    Cerr << "Error: bad setUpType in NPSOLOptimizer::core_run()." << std::endl;
    abort_handler(-1);
  }
}
...
void NPSOLOptimizer::send_sol_option(std::string sol_option) {
  ...
  NPOPTN2_F77(
      sol_option.data());  // NO Null terminator with std::string::data()
}
...
void NPSOLOptimizer::find_optimum_on_model() {
  ...
  if (localObjectiveRecast) {
    ...
    if (iteratedModel) {
      ...
      best_fns[0] =
          (!max_sense.empty() && max_sense[0]) ? -local_f_val : local_f_val;
    } else {
...
void NPSOLOptimizer::find_optimum_on_user_functions() {
  ...
  int i, num_cv = numContinuousVars, num_lin_constraints = numLinearConstraints,
  ...
  NPSOL_F77(
      num_cv, num_lin_constraints, num_nln_constraints, linConstraintArraySize,
      nlnConstraintArraySize, num_cv, linConstraintMatrixF77,
      lowerBounds.values(), upperBounds.values(), userConstraintEval,
      userObjectiveEval, informResult, numberIterations, &constraintState[0],
      local_c_vals.values(), constraintJacMatrixF77, &cLambda[0], local_f_val,
      local_f_grad.values(), upperFactorHessianF77, local_des_vars.values(),
      &intWorkSpace[0], intWorkSpaceSize, &realWorkSpace[0], realWorkSpaceSize);
  ...
  best_fns[0] = local_f_val;
  if (numNonlinearConstraints) copy_data_partial(local_c_vals, best_fns, 1);
  bestResponseArray.front().function_values(best_fns);
}
