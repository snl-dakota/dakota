...
C3Approximation::C3Approximation(ProblemDescDB& problem_db,
                                 const SharedApproxData& shared_data,
                                 const String& approx_label)
    : Approximation(BaseConstructor(), problem_db, shared_data, approx_label),
      levApproxIter(levelApprox.end()),
      expansionCoeffFlag(true),
...
void C3Approximation::recover_function_train_orders(
    const std::vector<OneApproxOpts*>& a_opts) {
  // returns the recovered orders, reflecting the latest CV if adapt_order
...
bool C3Approximation::advancement_available() {
  ...
  switch (shared_data_rep->c3AdvancementType) {
    ...
    case MAX_ORDER_ADVANCEMENT:
      ...
      m_o_advance = max_order_advancement_available();
      break;
...
void C3Approximation::build() {
  ...
  unsigned short max_o = data_rep->max_order(), kick_o, max_cv_o,
                 start_o =
                     find_max(data_rep->start_orders());  // flatten dim_pref
  ...
  if (adapt_r) {
    ...
    if (max_r == SZ_MAX)  // default upper bound -> define appropriate range
      ...
      int start_cand = max_r, start_lb = start_r;
      i = 0;
      ...
      if (start_r > min_recov_r)
        start_r = min_recov_r;  // include prev recovery
    }
  }
  ...
  if (adapt_o) {
    ...
    if (max_o == USHRT_MAX)  // default upper bound -> define appropriate range
      ...
      // use signed operands to avoid type coercion error in comparing unsigned
      int start_cand = max_o, start_lb = start_o;
      i = 0;
      ...
      if (start_o > min_recov_o)
        start_o = min_recov_o;  // include prev recovery
    }
  }
  for (i = 0; i < num_v; ++i) start_orders[i] = start_o;
  ...
  if (adapt_r) {
    ...
    if (max_r != SZ_MAX) ft_regress_set_maxrank(ftr, max_r);

    ft_regress_set_kfold(ftr, 5);  // kfold);//match Alex's Python (C3
                                   // default=3)
  }
  ...
  if (data_rep->respScaling)
    approxData.compute_response_function_scaling();
  else
    approxData.clear_response_function_scaling();
  ...
  for (i = 0; i < ndata; ++i) {
    ...
    for (j = 0; j < num_v; j++) xtrain[j + i * num_v] = c_vars[j];
  ...
  if (adapt_o) {  // CV for basis orders, with/without adapt_rank
    ...
    size_t kfold =
        (ndata > 5) ? 5 : ndata;  // ndata = max folds = leave-one-out
    ...
    for (i = 1; i < num_opts; ++i) np_opts[i] = np_opts[i - 1] + kick_o;
  ...
  if (apply_scaling) {  // unscale the FT expansion: fn = y * range + min
    ...
    function_train_free(ft_soln);
    function_train_free(ft_offset);
  } else
...
void C3Approximation::combine_coefficients() {
  ...
  for (; it != levelApprox.end(); ++it) {
    ...
    {
      y = function_train_round(z, rnd_tol, ma_opts);
      function_train_free(z);
    } else
      y = z;
  }
  ...
  if (rand_ind.empty() || rand_ind.size() == data_rep->numVars)
    ...
    compute_derived_statistics_av(combinedC3FTData, num_mom,
                                  true);  // overwrite
}
...
void C3Approximation::generate_synthetic_data(
    Pecos::SurrogateData& surr_data, const Pecos::ActiveKey& active_key,
    short combine_type) {
  ...
  lf0_key =
      surr_data.filtered_key(Pecos::SINGLETON_FILTER,
                             0);  // *** Note: ActiveKey first sorts on group id
  ...
  const std::map<Pecos::ActiveKey, Pecos::SDRArray>& discrep_resp_map =
      surr_data.filtered_response_data_map(
          Pecos::RAW_WITH_REDUCTION_DATA_FILTER);
  ...
  switch (combine_type) {
    case Pecos::MULT_COMBINE: {
      ...
      for (i = 0; i < num_pts; ++i) {
        ...
        //  augment w/ emulation of discrepancies (Delta-hat) preceding
        //  active_key
        for (cit = discrep_resp_map.begin(), j = 0; cit->first != active_key;
             ++cit, ++j) {
        ...
        if (lf_hat_bits & 1) lf_hat_sdr.response_function(fn_val_j);
        // if (lf_hat_bits & 2)
        //   lf_hat_sdr.response_gradient(fn_grad_j);
      }
      break;
    }
    default: {  // case Pecos::ADD_COMBINE: (correction specification not
                // required)
      Real sum_val;
      RealVector sum_grad;
      for (i = 0; i < num_pts; ++i) {
...
void C3Approximation::compute_derived_statistics(C3FnTrainData& ftd,
                                                 size_t num_mom,
                                                 bool overwrite) {
  std::shared_ptr<SharedC3ApproxData> data_rep =
      std::static_pointer_cast<SharedC3ApproxData>(sharedDataRep);
...
void C3Approximation::compute_derived_statistics_av(C3FnTrainData& ftd,
                                                    size_t num_mom,
                                                    bool overwrite) {
  std::shared_ptr<SharedC3ApproxData> data_rep =
      std::static_pointer_cast<SharedC3ApproxData>(sharedDataRep);
...
void C3Approximation::compute_moments(bool full_stats, bool combined_stats) {
  ...
  if (combined_stats) {
    ...
    if (full_stats) {  // should not happen: combined exp is an intermediate
                       // state
    ...
    primary_mom[0] = mean();
    primary_mom[1] = variance();
    if (full_stats) {
      primary_mom[2] = third_central();
      primary_mom[3] = fourth_central();
    }
  }
...
void C3Approximation::compute_moments(const RealVector& x, bool full_stats,
  ...
  if (combined_stats) {
    ...
    primary_mom[0] = mean(x);
    primary_mom[1] = variance(x);
  }
}
...
Real C3Approximation::mean(const RealVector& x,
                           C3FnTrainData& ftd)  //, size_t num_mom)
{
  compute_derived_statistics_av(ftd, 2);  // num_mom); // if not already
                                          // computed
...
const RealVector& C3Approximation::mean_gradient(const RealVector& x,
...
const RealVector& C3Approximation::variance_gradient(const RealVector& x,
                                                     const SizetArray& dvv) {
...
Real C3Approximation::covariance(const RealVector& x, C3FnTrainData& ftd1,
                                 C3FnTrainData& ftd2) {
...
Real C3Approximation::stored_value(const RealVector& c_vars,
...
size_t C3Approximation::regression_size() {
  ...
  switch (data_rep->c3AdvancementType) {
    case START_RANK_ADVANCEMENT: {  // use start ranks + recovered orders
      SizetVector start_r;
      data_rep->assign_start_ranks(start_r);
      const UShortArray& ft_o = (data_rep->adaptOrder)
                                    ? levApproxIter->second.recovered_orders()
                                    : data_rep->start_orders();
      return regression_size(start_r, max_r, ft_o, max_o);
      break;
    }
    case START_ORDER_ADVANCEMENT: {  // use start orders + recovered ranks
      const UShortArray& start_o =
          data_rep->start_orders();  // anisotropic is OK
      if (data_rep->adaptRank)
        return regression_size(levApproxIter->second.recovered_ranks(), max_r,
                               start_o, max_o);
      else {
        SizetVector start_r;
        data_rep->assign_start_ranks(start_r);
...
size_t C3Approximation::regression_size(const SizetVector& ranks,
                                        size_t max_rank,
  ...
  if (err_flag) abort_handler(APPROX_ERROR);
...
void C3Approximation::sobol_iterate_apply(
    void (*f)(double val, size_t ninteract, size_t* interactions, void* arg),
    void* args) {
