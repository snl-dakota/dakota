...
static const char rcsId[] =
    "@(#) $Id: NonDSparseGrid.cpp,v 1.57 2004/06/21 19:57:32 mseldre Exp $";
...
NonDSparseGrid::NonDSparseGrid(ProblemDescDB& problem_db,
  ...
  Pecos::ExpansionConfigOptions ec_options(
      ssgDriverType, exp_basis_type, model->correction_type(),
      ...
      probDescDB.get_ushort(
          "method.nond.vbd_interaction_order"),  // refine_type,
  ...
  Pecos::BasisConfigOptions bc_options(
      nested_rules, piecewise_basis, equidist_rules,
  ...
  switch (ssgDriverType) {
    case Pecos::COMBINED_SPARSE_GRID: {
      ...
      std::static_pointer_cast<Pecos::CombinedSparseGridDriver>(ssgDriver)
          ->initialize_grid(ssgLevelSpec, dimPrefSpec, u_dist, ec_options,
                            bc_options, growth_rate, track_colloc,
                            track_uniq_prod_wts);
      break;
    }
    case Pecos::INCREMENTAL_SPARSE_GRID: {
      ...
      std::static_pointer_cast<Pecos::IncrementalSparseGridDriver>(ssgDriver)
          ->initialize_grid(ssgLevelSpec, dimPrefSpec, u_dist, ec_options,
                            bc_options, growth_rate, track_uniq_prod_wts);
      break;
    }
    ...
    default:  // SparseGridDriver
      ssgDriver->initialize_grid(ssgLevelSpec, dimPrefSpec, u_dist, ec_options,
  ...
  switch (ssgDriverType) {
    case Pecos::COMBINED_SPARSE_GRID: {
      std::shared_ptr<Pecos::CombinedSparseGridDriver> csg_driver =
          std::static_pointer_cast<Pecos::CombinedSparseGridDriver>(ssgDriver);
      csg_driver->track_collocation_details(
          true);  // SC & sparse-grid PCE (SPAM)
...
void NonDSparseGrid::initialize_grid(
...
void NonDSparseGrid::get_parameter_sets(std::shared_ptr<Model> model) {
  ...
  if (outputLevel > NORMAL_OUTPUT &&
      ssgDriverType !=
          Pecos::HIERARCHICAL_SPARSE_GRID)  // t1 wts not implemented
...
void NonDSparseGrid::sampling_reset(size_t min_samples, bool all_data_flag,
                                    bool stats_flag) {
  ...
  while (ssgDriver->grid_size() < min_samples) ssgDriver->level(++ssg_level);
...
void NonDSparseGrid::increment_grid() {
  ...
  while (ssgDriver->grid_size() == orig_size) ssgDriver->level(++ssg_lev);
}
