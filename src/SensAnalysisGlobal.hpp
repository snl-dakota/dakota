...
class SensAnalysisGlobal {
 public:
  ...
  void print_std_regress_coeffs(std::ostream& s, StringArray var_labels,
                                const StringArray& resp_labels) const;

  /// compute VBD-based Sobol indices
  void compute_vbd_stats_via_sampling(
  ...
  void print_sobol_indices(std::ostream& s, const StringArray& var_labels,
                           const StringArray& resp_labels,
 ...
 private:
  ...
  /// Checks there are a nonzero number of samples and that the number of
  /// samples agree between variables and responses. Method name passed for
  /// error handling.
  ...
  void partial_corr(RealMatrix& total_data, const int num_in,
                    const RealMatrix& simple_corr_mat, RealMatrix& corr_matrix,
  ...
  void print_simple_correlations(std::ostream& s, const StringArray& var_labels,
                                 const StringArray& resp_labels,
                                 bool rank) const;

  /// print partial (rank = false) or partial rank (rank = true) correlations
  void print_partial_correlations(std::ostream& s,
                                  const StringArray& var_labels,
                                  const StringArray& resp_labels,
                                  bool rank) const;

  /// archive simple (rank = false) or simple rank (rank = true) correlations
  void archive_simple_correlations(
  ...
  void compute_binned_vbd_stats(const int numBins, const size_t numFunctions,
                                const size_t num_vars, const size_t num_samples,
 ...
 protected:
  /// compute binned sobol indices from valid samples (having screened out
  /// samples with non-numeric response)
  void compute_binned_sobol_indices_from_valid_samples(
      const RealMatrix& valid_samples, size_t num_bins);
