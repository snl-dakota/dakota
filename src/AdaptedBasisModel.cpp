...
AdaptedBasisModel::AdaptedBasisModel(ProblemDescDB& problem_db,
    ...
    : method_rotation(
          problem_db.get_short("model.adapted_basis.rotation_method")),
      adaptedBasisTruncationTolerance(
          probDescDB.get_real("model.adapted_basis.truncation_tolerance")),
      subspaceDimension(probDescDB.get_int("model.subspace.dimension")),
  ...
  pcePilotExpansion = std::shared_ptr<NonDPolynomialChaos>(pcePilotExpRepPtr);
...
std::shared_ptr<Model> AdaptedBasisModel::get_sub_model(
  ...
  if (ssg_level) {
    ...
    pcePilotExpRepPtr = new NonDPolynomialChaos(
        actual_model, Pecos::COMBINED_SPARSE_GRID, ssg_level, dim_pref,
        EXTENDED_U, refine_type, refine_cntl, cov_cntl, rule_nest, rule_growth,
        pw_basis, use_derivs);
  } else if (exp_order) {  // regression PCE: LeastSq/CS
                           // (exp_order,colloc_ratio)
    short exp_coeffs_approach = Pecos::DEFAULT_REGRESSION;
    String import_file;
    int seed = 12347;
    size_t colloc_pts = SZ_MAX;
    pcePilotExpRepPtr = new NonDPolynomialChaos(
        actual_model, exp_coeffs_approach, exp_order, dim_pref, colloc_pts,
        colloc_ratio, seed, EXTENDED_U, refine_type, refine_cntl,
        cov_cntl,  // rule_nest, rule_growth,
        pw_basis, use_derivs,
        problem_db.get_bool("method.nond.cross_validation"), import_file,
        TABULAR_ANNOTATED, false);
  } else {
...
void AdaptedBasisModel::derived_init_communicators(ParLevLIter pl_iter,
...
void AdaptedBasisModel::derived_set_communicators(ParLevLIter pl_iter,
                                                  int max_eval_concurrency,
                                                  bool recurse_flag) {
  miPLIndex =
...
void AdaptedBasisModel::derived_free_communicators(ParLevLIter pl_iter,
                                                   int max_eval_concurrency,
                                                   bool recurse_flag) {
  if (recurse_flag) {
...
void AdaptedBasisModel::validate_inputs() {
  ...
  if (subspaceDimension > numFullspaceVars) {
    error_flag = true;
    Cerr << "\nError (dimension): Required rotation dimension larger than the "
            "full problem dimension;"
         << "\n                        Please select dimension < number of "
            "variables\n"
         << std::endl;
  }
...
void AdaptedBasisModel::compute_subspace() {
  ...
  for (i = 0; i < num_pce; ++i) {
    num_ones = std::count(std::begin(pce_multi_index[i]),
                          std::end(pce_multi_index[i]), 1);
    num_zeros = std::count(std::begin(pce_multi_index[i]),
                           std::end(pce_multi_index[i]), 0);
  ...
  if (method_rotation == ROTATION_METHOD_UNRANKED) {
    method = 0;
    std::cout
        << "\nSelecting UNRANKED Gaussian for the rotation matrix construction"
        << std::endl;

  } else if (method_rotation == ROTATION_METHOD_RANKED) {
    method = 1;
    std::cout
        << "\nSelecting RANKED Gaussian for the rotation matrix construction"
        << std::endl;
  }
  ...
  for (i = 0; i < numFns; ++i) {
    ...
    if (method == 0) {
      ...
      for (j = 0; j < numFullspaceVars; ++j)
        A_i(0, j) = pce_coeffs[i][first_ord_index[j]];  // offset by 1 to
                                                        // neglect constant/mean
      for (j = 1; j < numFullspaceVars; ++j) A_i(j, j) = 1.;
    }

    else if (method == 1) {
      // Step 1b: same as 1a except permuted location of 1's determined from
      // q_i's
      ...
      // by descending order, the column indices are the same as in the first
      // row
    ...
    la.GEQRF(numFullspaceVars, numFullspaceVars, A_i_trans.values(),
             numFullspaceVars, tau, work, lwork, &info);
    ...
    for (j = 0; j < numFullspaceVars; ++j, ++row_cntr)
      for (k = 0; k < numFullspaceVars; ++k) A_q(row_cntr, k) = A_i_trans(k, j);
  }
  ...
  //// Cout << "\nAdapted Basis Model: Composing composite reduction"  <<
  /// std::endl;
...
void AdaptedBasisModel::truncate_rotation() {
  ...
  double threshold_ratio =
      adaptedBasisTruncationTolerance;  // default value 0.8

  std::cout << "\nThreshold Ratio: " << adaptedBasisTruncationTolerance
            << std::endl;
  ...
  int seed = 12345;
  int nsamples = 200;
  if (nsamples < numFullspaceVars * 2 + 1) nsamples = numFullspaceVars * 2 + 1;
  size_t i, j, k;
  ...
  for (i = 0; i < numFullspaceVars; i++) correl_matrix(i, i) = 1.;
  ...
  const RealMatrix A(Teuchos::Copy, reducedBasis, numFullspaceVars,
                     numFullspaceVars);
  int m = A.numRows(), n = A.numCols();
  Real alpha = 1.0, beta = 0.0;
  RealMatrix eta(numFullspaceVars, nsamples, false);
  Teuchos::BLAS<int, Real> teuchos_blas;
  teuchos_blas.GEMM(Teuchos::NO_TRANS, Teuchos::NO_TRANS, numFullspaceVars,
                    nsamples, numFullspaceVars, alpha, A.values(),
                    numFullspaceVars, xi.values(), numFullspaceVars, beta,
                    eta.values(), numFullspaceVars);

  // 1) map \eta of reduced dimensions to the full dimensional space as
  // \xi_trans 2) compute the weighted difference between \xi and \xi_trans 3)
  // compute metric w = 1/N \sum (\xi_weigt - \xi_trans_weigt)
  ...
  for (i = 0; i < numFullspaceVars; i++) {
    ...
    for (j = 0; j < i + 1; j++) {
      for (k = 0; k < nsamples; k++) eta_extd(j, k) = eta(j, k);
    }

    // matrix multiplication: \hat \xi = A^T * \eta_{ex}
    RealMatrix xi_trans(numFullspaceVars, nsamples, false);
    teuchos_blas.GEMM(Teuchos::TRANS, Teuchos::NO_TRANS, numFullspaceVars,
                      nsamples, numFullspaceVars, alpha, A.values(),
                      numFullspaceVars, eta_extd.values(), numFullspaceVars,
  ...
  for (j = 1; j < numFullspaceVars; j++) w_diff(j) = w(j - 1) - w(j);
...
void AdaptedBasisModel::uncertain_vars_to_subspace() {
  ...
  for (int i = 0; i < reducedRank; i++)
    adapted_basis_var_labels[i] = "abv_" + std::to_string(i + 1);
...
void AdaptedBasisModel::variables_mapping(const Variables& reduced_vars,
                                          Variables& full_vars) {
  ...
  for (int i = 0; i < dim_eta; i++) eta_ex(i) = eta(i);
  //  y <-- alpha*A*x + beta*y
  //  Calculate \xi = 1.0 * A^T \eta + + 0.0 * \xi
  Teuchos::BLAS<int, Real> teuchos_blas;
  teuchos_blas.GEMV(Teuchos::TRANS, m, n, alpha, A.values(), m, eta_ex.values(),
                    incy, beta, xi.values(), incx);
