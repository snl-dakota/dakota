...
ParallelDirectApplicInterface::ParallelDirectApplicInterface(
    const Dakota::ProblemDescDB& problem_db,
    Dakota::ParallelLibrary& parallel_lib, const MPI_Comm& analysis_comm)
    : Dakota::DirectApplicInterface(
          problem_db, parallel_lib)  //, analysisComm(analysis_comm)
{
...
int ParallelDirectApplicInterface::derived_map_ac(
...
void ParallelDirectApplicInterface::wait_local_evaluations(
...
int ParallelDirectApplicInterface::text_book(
    const Dakota::RealVector& c_vars, const Dakota::ShortArray& asv,
  ...
  if (asv[0] & 1) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      if (analysisCommRank == 0) fn_vals[0] = global_val;
    } else
      fn_vals[0] = local_val;
  }
  ...
  if (asv[0] & 2) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      parallelLib.reduce_sum_a(fn_grads[0], sum_fns, numDerivVars);
      if (analysisCommRank == 0) {
        Teuchos::setCol(Teuchos::SerialDenseVector<int, double>(
                            Teuchos::View, sum_fns, numDerivVars),
                        0, fn_grads);
        delete[] sum_fns;
      }
    }
  ...
  if (num_fns > 1 && (asv[1] & 1)) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      // evalCommRank 0 in overlay_response.  evalCommRank 0 then returns the
      // results to the iterator in ApplicationInterface::serve_evaluations().
      if (analysisCommRank == 0) fn_vals[1] = global_val;
    } else
      fn_vals[1] = local_val;
  }
  ...
  if (num_fns > 1 && (asv[1] & 2)) {
    ...
    if (multiProcAnalysisFlag) {
      double* sum_fns = (analysisCommRank) ? NULL : new double[numDerivVars];
      parallelLib.reduce_sum_a(fn_grads[1], sum_fns, numDerivVars);
      if (analysisCommRank == 0) {
        Teuchos::setCol(Teuchos::SerialDenseVector<int, double>(
                            Teuchos::View, sum_fns, numDerivVars),
                        1, fn_grads);
  ...
  if (num_fns > 1 && (asv[1] & 4)) {
    ...
    for (i = analysisCommRank; i < numDerivVars; i += analysisCommSize) {
      int var_index = directFnDVV[i] - 1;  // assumes no discrete vars
      if (var_index == 0) fn_hessians[1](i, i) = 2.;
    }
    if (multiProcAnalysisFlag) {
      int num_doubles = numDerivVars * numDerivVars;
      double* local_fns = new double[num_doubles];
  ...
  if (num_fns > 2 && (asv[2] & 1)) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      // evalCommRank 0 in overlay_response.  evalCommRank 0 then returns the
      // results to the iterator in ApplicationInterface::serve_evaluations().
      if (analysisCommRank == 0) fn_vals[2] = global_val;
    } else
      fn_vals[2] = local_val;
  }
  ...
  if (num_fns > 2 && (asv[2] & 2)) {
    ...
    if (multiProcAnalysisFlag) {
      double* sum_fns = (analysisCommRank) ? NULL : new double[numDerivVars];
      parallelLib.reduce_sum_a(fn_grads[2], sum_fns, numDerivVars);
      if (analysisCommRank == 0) {
        Teuchos::setCol(Teuchos::SerialDenseVector<int, double>(
                            Teuchos::View, sum_fns, numDerivVars),
                        2, fn_grads);
  ...
  if (num_fns > 2 && (asv[2] & 4)) {
    ...
    for (i = analysisCommRank; i < numDerivVars; i += analysisCommSize) {
      int var_index = directFnDVV[i] - 1;  // assumes no discrete vars
      if (var_index == 1) fn_hessians[2](i, i) = 2.;
    }
    if (multiProcAnalysisFlag) {
      int num_doubles = numDerivVars * numDerivVars;
      double* local_fns = new double[num_doubles];
