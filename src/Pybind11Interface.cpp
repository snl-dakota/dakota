...
Pybind11Interface::Pybind11Interface(const ProblemDescDB& problem_db,
  ...
  if (userNumpyFlag) {
    ...
    Cerr << "\nError: Direct Python interface 'numpy' option requested, but "
         << "Dakota was not built with numpy support enabled." << std::endl;
    abort_handler(-1);
#endif
  }
...
int Pybind11Interface::derived_map_ac(const String& ac_name) {
  ...
  try {
    ...
    unpack_python_response(directFnASV, directFnDVV.size(), ret_val, fnVals,
                           fnGrads, fnHessians, metaData);
...
void Pybind11Interface::derived_map_asynch(
    const ParamResponsePair& pair) { /* no-op*/ }
...
void Pybind11Interface::wait_local_evaluations(PRPQueue& prp_queue) {
  ...
  for (auto& prp : prp_queue) {
    ...
    unpack_python_response(
        active_set.request_vector(), active_set.derivative_vector().size(),
        py_responses[i], fnVals, fnGrads, fnHessians, metaData);
...
RetT Pybind11Interface::copy_array_to_pybind11(const ArrayT& src) const {
  ...
  for (auto const& a : src) tmp_vec.push_back(a);
  return py::cast(tmp_vec);
}

template <typename RetT, class O, class S>
RetT Pybind11Interface::copy_array_to_pybind11(
    const Teuchos::SerialDenseVector<O, S>& src) const {
...
py::dict Pybind11Interface::pack_kwargs() const {
  py::list all_var_labels =
      copy_array_to_pybind11<py::list, StringArray, String>(xAllLabels);
  py_arrayT cv = copy_array_to_pybind11<py_arrayT>(xC);
  py::list cv_labels =
      copy_array_to_pybind11<py::list, StringMultiArray, String>(xCLabels);
  py_arrayT div = copy_array_to_pybind11<py_arrayT>(xDI);
  py::list div_labels =
      copy_array_to_pybind11<py::list, StringMultiArray, String>(xDILabels);
  py_arrayT dsv =
      copy_array_to_pybind11<py::list, StringMultiArray, String>(xDS);
  py::list dsv_labels =
      copy_array_to_pybind11<py::list, StringMultiArray, String>(xDSLabels);
  py_arrayT drv = copy_array_to_pybind11<py_arrayT>(xDR);
  py::list drv_labels =
      copy_array_to_pybind11<py::list, StringMultiArray, String>(xDRLabels);
  py_arrayT asv =
      copy_array_to_pybind11<py_arrayT, ShortArray, int>(directFnASV);
  py_arrayT dvv =
      copy_array_to_pybind11<py_arrayT, SizetArray, size_t>(directFnDVV);
  py_arrayT an_comps =
      (analysisComponents.size() > 0)
          ? copy_array_to_pybind11<py_arrayT, StringArray, String>(
                analysisComponents[analysisDriverIndex])
          : py_arrayT();
  py::list fn_labels =
      copy_array_to_pybind11<py::list, StringArray, String>(fnLabels);
  py::list md_labels =
      copy_array_to_pybind11<py::list, StringArray, String>(metaDataLabels);

  std::string eval_id(eval_id_string());

  py::dict kwargs = py::dict(
      "variables"_a = numVars, "functions"_a = numFns,
      "metadata"_a = metaData.size(), "variable_labels"_a = all_var_labels,
      "function_labels"_a = fn_labels, "metadata_labels"_a = md_labels,
      "cv"_a = cv, "cv_labels"_a = cv_labels, "div"_a = div,
      "div_labels"_a = div_labels, "dsv"_a = dsv, "dsv_labels"_a = dsv_labels,
      "drv"_a = drv, "drv_labels"_a = drv_labels, "asv"_a = asv, "dvv"_a = dvv,
      "analysis_components"_a = an_comps, "eval_id"_a = eval_id);
...
void Pybind11Interface::unpack_python_response(
    ...
    RealMatrix& gradients, RealSymMatrixArray& hessians, RealArray& metadata) {
  ...
  if (expect_derivative(asv, 1)) {
    if (py_response.contains("fns")) {
      ...
      if (values.size() != num_fns) {
        throw(
            std::runtime_error("Pybind11 Direct Interface [\"fns\"]: "
      ...
      throw(
          std::runtime_error("Pybind11 Direct Interface: required key "
  ...
  if (expect_derivative(asv, 2)) {
    if (py_response.contains("fnGrads")) {
      auto grads =
          py_response["fnGrads"].cast<std::vector<std::vector<double>>>();
      if (grads.size() != num_fns) {
        throw(
            std::runtime_error("Pybind11 Direct Interface [\"fnGrads\"]: "
                               "incorrect size for # of functions"));
      }
      for (size_t i = 0; i < num_fns; ++i) {
      ...
      throw(std::runtime_error(
          "Pybind11 Direct Interface: required key "
  ...
  if (expect_derivative(asv, 4)) {
    if (py_response.contains("fnHessians")) {
      auto hess = py_response["fnHessians"]
                      .cast<std::vector<std::vector<std::vector<double>>>>();
      if (hess.size() != num_fns) {
        throw(
            std::runtime_error("Pybind11 Direct Interface [\"fnHessians\"]: "
                               "incorrect size for # of functions"));
      }
      for (size_t i = 0; i < num_fns; ++i) {
  ...
  if (num_md > 0) {
    if (py_response.contains("metadata")) {
      ...
      if (md.size() != num_md) {
        throw(
            std::runtime_error("Pybind11 Direct Interface [\"metadata\"]: "
...
std::string Pybind11Interface::eval_id_string() const {
  std::string eval_id;
  if (!evalTagPrefix.empty()) eval_id = evalTagPrefix + ":";
  if (batchEval) eval_id += std::to_string(batchIdCntr) + ":";
