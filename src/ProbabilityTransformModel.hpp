...
class ProbabilityTransformModel : public RecastModel {
 public:
  ...
  ProbabilityTransformModel(std::shared_ptr<Model> sub_model,
                            short u_space_type,
...
inline void ProbabilityTransformModel::nested_variable_mappings(
    const SizetArray& c_index1, const SizetArray& di_index1,
    const SizetArray& ds_index1, const SizetArray& dr_index1,
    const ShortArray& c_target2, const ShortArray& di_target2,
...
inline short
ProbabilityTransformModel::query_distribution_parameter_derivatives() const {
  ...
  if (num_outer_cv) {
    ...
    for (i = 0; i < num_outer_cv; ++i)
      if (secondaryACVarMapTargets[i] == Pecos::NO_TARGET)
        ...
        tgt = true;
    if (tgt && no_tgt)
      dist_param_derivs = MIXED_DERIVS;
    else if (tgt)
      dist_param_derivs = ALL_DERIVS;
  }
  return dist_param_derivs;
}

inline void
ProbabilityTransformModel::activate_distribution_parameter_derivatives() {
  distParamDerivs = query_distribution_parameter_derivatives();
}

inline void
ProbabilityTransformModel::deactivate_distribution_parameter_derivatives() {
  distParamDerivs = NO_DERIVS;
}

inline const SizetArray& ProbabilityTransformModel::nested_acv1_indices()
    const {
  return primaryACVarMapIndices;
}

inline const ShortArray& ProbabilityTransformModel::nested_acv2_targets()
    const {
  return secondaryACVarMapTargets;
}
...
inline void ProbabilityTransformModel::initialize_transformation(
    short u_space_type) {
...
inline void ProbabilityTransformModel::update_from_subordinate_model(
    size_t depth) {
...
inline bool ProbabilityTransformModel::nonlinear_variables_mapping(
    const Pecos::MultivariateDistribution& x_dist,
    const Pecos::MultivariateDistribution& u_dist) const {
    ...
    if (active_v[i]) {
      switch (u_types[i]) {
        case Pecos::STD_NORMAL:
          nln_vars_map = (x_types[i] != Pecos::NORMAL);
          break;
        case Pecos::STD_UNIFORM:
          switch (x_types[i]) {
            case Pecos::CONTINUOUS_RANGE:
            case Pecos::UNIFORM:
        ...
        case Pecos::STD_EXPONENTIAL:
          nln_vars_map = (x_types[i] != Pecos::EXPONENTIAL);
          break;
        case Pecos::STD_BETA:
          nln_vars_map = (x_types[i] != Pecos::BETA);
          break;
        case Pecos::STD_GAMMA:
          nln_vars_map = (x_types[i] != Pecos::GAMMA);
          break;
        default:
          nln_vars_map = (x_types[i] != u_types[i]);
          break;
      }
...
ProbabilityTransformModel::probability_transformation() {
  return natafTransform;
}

inline void ProbabilityTransformModel::assign_instance() { ptmInstance = this; }

/** Map the variables from iterator space (u) to simulation space (x). */
inline void ProbabilityTransformModel::trans_U_to_X(const RealVector& u_c_vars,
                                                    RealVector& x_c_vars) {
  ...
  if (u_active_view == x_active_view)
    natafTransform.trans_U_to_X(u_c_vars,
                                ...
                                x_c_vars, x_vars.continuous_variable_ids());
  else {
    ...
    if (!u_all && x_all)
      natafTransform.trans_U_to_X(
          u_c_vars, currentVariables.all_continuous_variable_ids(), x_c_vars,
          x_vars.continuous_variable_ids());
    else if (!x_all && u_all)
      natafTransform.trans_U_to_X(
          u_c_vars, currentVariables.continuous_variable_ids(), x_c_vars,
...
/** Map the variables from iterator space (u) to simulation space (x). */
inline void ProbabilityTransformModel::trans_U_to_X(const Variables& u_vars,
                                                    Variables& x_vars) {
  ...
  if (u_active_view == x_active_view)
    ...
    bool u_all = (u_active_view == RELAXED_ALL || u_active_view == MIXED_ALL),
         x_all = (x_active_view == RELAXED_ALL || x_active_view == MIXED_ALL);
    if (!u_all && x_all)
      natafTransform.trans_U_to_X(u_vars.all_continuous_variables(),
                                  u_vars.all_continuous_variable_ids(),
                                  x_vars.continuous_variables_view(),
                                  x_vars.continuous_variable_ids());
    else if (!x_all && u_all) {
      RealVector x_acv;
      natafTransform.trans_U_to_X(u_vars.continuous_variables(),
                                  u_vars.continuous_variable_ids(), x_acv,
                                  x_vars.all_continuous_variable_ids());
...
inline void ProbabilityTransformModel::trans_X_to_U(const RealVector& x_c_vars,
                                                    RealVector& u_c_vars) {
  const Variables& x_vars = subModel->current_variables();
  short u_active_view = currentVariables.shared_data().view().first,
        x_active_view = x_vars.shared_data().view().first;
  ...
  if (u_active_view == x_active_view)
    ...
    if (!u_all && x_all)
      natafTransform.trans_X_to_U(
          x_c_vars, x_vars.continuous_variable_ids(), u_c_vars,
          currentVariables.all_continuous_variable_ids());
    else if (!x_all && u_all)
      natafTransform.trans_X_to_U(
          x_c_vars, x_vars.all_continuous_variable_ids(), u_c_vars,
          currentVariables.continuous_variable_ids());
    else {
      Cerr << "Error: unsupported variable view differences in "
...
/** Map the variables from simulation space (x) to iterator space (u). */
inline void ProbabilityTransformModel::trans_X_to_U(const Variables& x_vars,
                                                    Variables& u_vars) {
  short u_active_view = u_vars.shared_data().view().first,
        x_active_view = x_vars.shared_data().view().first;
  ...
  if (u_active_view == x_active_view)
    ...
    if (!u_all && x_all) {
      RealVector u_acv;
      natafTransform.trans_X_to_U(x_vars.continuous_variables(),
                                  x_vars.continuous_variable_ids(), u_acv,
                                  u_vars.all_continuous_variable_ids());
      u_vars.all_continuous_variables(u_acv);
    } else if (!x_all && u_all)
      natafTransform.trans_X_to_U(x_vars.all_continuous_variables(),
                                  x_vars.all_continuous_variable_ids(),
                                  u_vars.continuous_variables_view(),
                                  u_vars.continuous_variable_ids());
    else {
      Cerr << "Error: unsupported variable view differences in "
           << "ProbabilityTransformModel::trans_X_to_U()." << std::endl;
      abort_handler(MODEL_ERROR);
...
/** Map the variables from iterator space (u) to simulation space (x). */
inline void ProbabilityTransformModel::vars_u_to_x_mapping(
    const Variables& u_vars, Variables& x_vars) {
  ptmInstance->trans_U_to_X(u_vars, x_vars);
}

/** Map the variables from simulation space (x) to iterator space (u). */
inline void ProbabilityTransformModel::vars_x_to_u_mapping(
    const Variables& x_vars, Variables& u_vars) {
  ptmInstance->trans_X_to_U(x_vars, u_vars);
}

inline void ProbabilityTransformModel::trans_grad_X_to_U(
    const RealVector& fn_grad_x, RealVector& fn_grad_u,
    const RealVector& x_vars) {
  SizetMultiArrayConstView x_cv_ids =
                               ModelUtils::continuous_variable_ids(*subModel),
                           u_cv_ids =
                               currentVariables.continuous_variable_ids();
  SizetArray x_dvv;
  copy_data(x_cv_ids, x_dvv);
  natafTransform.trans_grad_X_to_U(fn_grad_x, x_cv_ids, fn_grad_u, u_cv_ids,
                                   x_vars, x_dvv);
}

inline void ProbabilityTransformModel::trans_grad_U_to_X(
    const RealVector& fn_grad_u, RealVector& fn_grad_x,
    const RealVector& x_vars) {
  SizetMultiArrayConstView x_cv_ids =
                               ModelUtils::continuous_variable_ids(*subModel),
                           u_cv_ids =
...
inline void ProbabilityTransformModel::trans_grad_X_to_S(
    const RealVector& fn_grad_x, RealVector& fn_grad_s,
    const RealVector& x_vars) {
  SizetMultiArrayConstView x_cv_ids =
      ModelUtils::continuous_variable_ids(*subModel);
  SizetArray x_dvv;
  copy_data(x_cv_ids, x_dvv);
  natafTransform.trans_grad_X_to_S(
      fn_grad_x, fn_grad_s, x_vars, x_dvv, x_cv_ids,
      currentVariables.continuous_variable_ids(),          // u_cv_ids
      ModelUtils::all_continuous_variable_ids(*subModel),  // x_acv_ids
      primaryACVarMapIndices, secondaryACVarMapTargets);
}

inline void ProbabilityTransformModel::trans_hess_X_to_U(
    const RealSymMatrix& fn_hess_x, RealSymMatrix& fn_hess_u,
...
inline size_t ProbabilityTransformModel::rv_index_to_corr_index(
    size_t rv_index) {
  ...
  if (active_corr.empty())
    ...
    for (i = 0; i < rv_index; ++i)
      if (active_corr[i]) ++corr_index;
    return corr_index;
  } else  // RV not active in correlated variable subset
    return _NPOS;
}

inline size_t ProbabilityTransformModel::acv_index_to_corr_index(
    size_t acv_index) {
