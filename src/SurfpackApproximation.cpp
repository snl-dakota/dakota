...
// sharedDataRep((SharedSurfpackApproxData*)shared_data.data_rep())
{
  ...
  // For Kriging surface fits
    ...
    if (bdo & 2) surfpack_derivative_order = 1;
    if (bdo & 4) {
      if (bdo & 2)
    ...
    if (!optimization_method.empty()) {
      if (optimization_method == "none" || optimization_method == "sampling" ||
          optimization_method == "local" || optimization_method == "global")
    ...
    if (max_trials > 0) args["max_trials"] = toString<short>(max_trials);
  ...
  // For ANN surface fits
    ...
    short nodes = problem_db.get_short("model.surrogate.neural_network_nodes");
    if (nodes > 0) {
  ...
  //// For moving least squares
    ...
    args["type"] = "mls";
    short weight = problem_db.get_short("model.surrogate.mls_weight_function");
    if (weight > 0) {
  ...
  //// For radial basis function networks
    ...
    short max_subsets = problem_db.get_short("model.surrogate.rbf_max_subsets");
    if (max_subsets > 0) {
...
void SurfpackApproximation::build() {
  ...
  if (!spFactory) {
    Cerr << "Error: Uninitialized model factory in "
            "SurfpackApproximation::build()."
         << std::endl;
    abort_handler(-1);
  }
...
void SurfpackApproximation::map_variable_labels(const Variables& dfsm_vars) {
...
void SurfpackApproximation::export_model(const Variables& vars,
...
void SurfpackApproximation::export_model(const StringArray& var_labels,
                                         const String& fn_label,
                                         const String& export_prefix,
                                         const unsigned short export_format) {
...
const RealSymMatrix& SurfpackApproximation::hessian(const Variables& vars) {
  ...
  try {
    ...
    for (size_t i = 0; i < num_cv; i++)
      for (size_t j = 0; j < num_cv; j++) approxHessian(i, j) = sm(i, j);
  } catch (...) {
...
void SurfpackApproximation::primary_diagnostics(size_t fn_index) {
  String func_description = approxLabel.empty()
                                ? "function " + std::to_string(fn_index + 1)
                                : approxLabel;
  std::shared_ptr<SharedSurfpackApproxData> shared_surf_data_rep =
      std::static_pointer_cast<SharedSurfpackApproxData>(sharedDataRep);
  ...
  if (diag_set.empty()) {
    ...
    Cout << "\nSurrogate quality metrics at build (training) points for "
         << func_description << ":\n";
    for (const auto& req_diag : diag_set) diagnostic(req_diag);
    ...
    if (shared_surf_data_rep->crossValidateFlag) {
      ...
      for (int j = 0; j < diag_set.size(); ++j) {
        ...
        if (metric_type == "rsquared")
          Cout << std::setw(20) << metric_type << std::setw(20)
               << std::numeric_limits<Real>::quiet_NaN()
               << "  (n/a for cross-validation)" << std::endl;
        else
    ...
    if (shared_surf_data_rep->pressFlag) {
      ...
      for (int j = 0; j < diag_set.size(); ++j) {
        ...
        if (metric_type == "rsquared")
          Cout << std::setw(20) << metric_type << std::setw(20)
               << std::numeric_limits<Real>::quiet_NaN() << "  (n/a for PRESS)"
               << std::endl;
        else
          Cout << std::setw(20) << metric_type << std::setw(20) << cv_metrics[j]
               << std::endl;
...
void SurfpackApproximation::challenge_diagnostics(
    size_t fn_index, const RealMatrix& challenge_points,
...
RealArray SurfpackApproximation::cv_diagnostic(const StringArray& metric_types,
  ...
  try {
    ...
    std::fill(cv_metrics.begin(), cv_metrics.end(),
              std::numeric_limits<Real>::quiet_NaN());
  }
  return cv_metrics;
}

RealArray SurfpackApproximation::challenge_diagnostic(
...
void SurfpackApproximation::import_model(const ProblemDescDB& problem_db) {
  ...
  std::string filename =
      import_prefix + "." + approxLabel + (is_binary ? ".bsps" : ".sps");
...
void SurfpackApproximation::add_constraints_to_surfdata(
    const Pecos::SurrogateDataVars& anchor_vars,
    const Pecos::SurrogateDataResp& anchor_resp, short fail_code) {
