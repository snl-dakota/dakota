...
static const char rcsId[] =
    "@(#) $Id: DakotaVariables.cpp 7037 2010-10-23 01:18:08Z mseldre $";
...
std::map<const ProblemDescDB*, std::list<Variables>>
...
Variables::Variables(BaseConstructor, const ProblemDescDB& problem_db,
...
/** This constructor is the one which must build the base class data for all
    ...
    calling get_variables() again).  Since the letter IS the representation,
    its representation pointer is set to NULL. */
Variables::Variables(BaseConstructor, const SharedVariablesData& svd)
    : sharedVarsData(svd) {
  shape();        // size all*Vars arrays
  build_views();  // construct active/inactive views of all arrays
}
...
Variables::Variables() { /* empty ctor */ }
...
/** Initializes variablesRep to the appropriate derived type, as given
    ...
    are invoked.  */
std::shared_ptr<Variables> Variables::get_variables(
  ...
  switch (active_view) {
    ...
    case MIXED_STATE:
      return std::make_shared<MixedVariables>(problem_db, view);
      break;
    case RELAXED_ALL:
    case RELAXED_DESIGN:
...
std::shared_ptr<Variables> Variables::get_variables(
  ...
  switch (active_view) {
    ...
    case MIXED_UNCERTAIN:
    case MIXED_STATE:
      return std::make_shared<MixedVariables>(svd);
      break;
    case RELAXED_ALL:
    case RELAXED_DESIGN:
    case RELAXED_ALEATORY_UNCERTAIN:
    ...
    case RELAXED_UNCERTAIN:
    case RELAXED_STATE:
      return std::make_shared<RelaxedVariables>(svd);
      break;
    default:
      Cerr << "Variables active view " << active_view << " not currently "
           << "supported in derived Variables classes." << std::endl;
...
Variables::Variables(const Variables& vars)
    : variablesRep(vars.variablesRep) { /* empty ctor */ }
...
Variables::~Variables() { /* empty dtor */ }
...
void Variables::active_view(short view1) {
  if (variablesRep)
    ...
    if (view1 == RELAXED_ALL || view1 == MIXED_ALL) {
      ...
      build_inactive_views();  // unconditional: avoid Vars/Cons order
                               // dependence
    }
...
short Variables::method_map(short view_spec, bool relaxed) const {
  switch (view_spec) {
    case ALL_VIEW:  // precedence level 1 view selection
      return (relaxed) ? RELAXED_ALL : MIXED_ALL;
      break;
    case DESIGN_VIEW:  // precedence level 1 view selection
      return (relaxed) ? RELAXED_DESIGN : MIXED_DESIGN;
      break;
    case UNCERTAIN_VIEW:  // precedence level 1 view selection
      return (relaxed) ? RELAXED_UNCERTAIN : MIXED_UNCERTAIN;
      break;
    ...
    case EPISTEMIC_UNCERTAIN_VIEW:  // precedence level 1 view selection
      return (relaxed) ? RELAXED_EPISTEMIC_UNCERTAIN
                       : MIXED_EPISTEMIC_UNCERTAIN;
      break;
    case STATE_VIEW:  // precedence level 1 view selection
      return (relaxed) ? RELAXED_STATE : MIXED_STATE;
      break;
    default:
...
short Variables::method_view(const ProblemDescDB& problem_db) const {
  ...
  if (method_name & PSTUDYDACE_BIT)
    ...
    } else if (method_name > RANDOM_SAMPLING)
      return EPISTEMIC_UNCERTAIN_VIEW;
    else
      return ALEATORY_UNCERTAIN_VIEW;
  } else if (method_name & VERIF_BIT)
    return STATE_VIEW;
  else
    return DESIGN_VIEW;
}
...
void Variables::check_view_compatibility() {
  ...
  if (((active_view == RELAXED_ALL ||
       ...
       (inactive_view == MIXED_ALL ||
        (inactive_view >= MIXED_DESIGN && inactive_view <= MIXED_STATE))) ||
      ((active_view == MIXED_ALL ||
        (active_view >= MIXED_DESIGN && active_view <= MIXED_STATE)) &&
       (inactive_view == RELAXED_ALL ||
        (inactive_view >= RELAXED_DESIGN && inactive_view <= RELAXED_STATE)))) {
  ...
  if (active_view == inactive_view ||
      ...
      ((active_view == RELAXED_UNCERTAIN || active_view == MIXED_UNCERTAIN) &&
...
void Variables::build_active_views() {
  ...
  continuousVars =
      (num_cv) ? RealVector(Teuchos::View,
                            &allContinuousVars[sharedVarsData.cv_start()],
                            num_cv)
               : RealVector();  // return to default empty initialization
  discreteIntVars =
      (num_div) ? IntVector(Teuchos::View,
                            &allDiscreteIntVars[sharedVarsData.div_start()],
                            num_div)
  ...
  //}
  discreteRealVars =
      (num_drv) ? RealVector(Teuchos::View,
                             &allDiscreteRealVars[sharedVarsData.drv_start()],
                             num_drv)
                : RealVector();  // return to default empty initialization
}
...
void Variables::build_inactive_views() {
  ...
  inactiveContinuousVars =
      (num_icv) ? RealVector(Teuchos::View,
                             &allContinuousVars[sharedVarsData.icv_start()],
                             num_icv)
                : RealVector();  // return to default empty initialization
  inactiveDiscreteIntVars =
      (num_idiv) ? IntVector(Teuchos::View,
                             &allDiscreteIntVars[sharedVarsData.idiv_start()],
                             num_idiv)
                 : IntVector();  // return to default empty initialization
  // as for label arrays, generate StringMultiArrayViews on the fly
  // if (num_idsv) {
  ...
  inactiveDiscreteRealVars =
      (num_idrv) ? RealVector(Teuchos::View,
                              &allDiscreteRealVars[sharedVarsData.idrv_start()],
                              num_idrv)
                 : RealVector();  // return to default empty initialization
}
...
template void Variables::load<boost::archive::binary_iarchive>(
    boost::archive::binary_iarchive& ar, const unsigned int version);
template void Variables::save<boost::archive::binary_oarchive>(
...
void Variables::read_annotated(std::istream& s) {
  ...
  for (i = 0; i < NUM_VC_TOTALS; ++i) s >> vars_comps_totals[i];
  ...
  s >> ardi_size;
  if (ardi_size > 0) s >> all_relax_di;
  s >> ardr_size;
  if (ardr_size > 0) s >> all_relax_dr;
...
void Variables::write_annotated(std::ostream& s) const {
  ...
  if (variablesRep)
    ...
    size_t i;
    for (i = 0; i < NUM_VC_TOTALS; ++i) s << vc_totals[i] << ' ';
    // for annotated (neutral), write the binary representation (string of 0/1)
    // have to write the size as a flag since they may be length 0 in the mixed
    // case
...
void Variables::write_tabular_partial(std::ostream& s, size_t start_index,
...
void Variables::write_tabular_labels(std::ostream& s,
...
void Variables::write_tabular_partial_labels(std::ostream& s,
                                             size_t start_index,
                                             size_t num_items) const {
  if (variablesRep)
...
Variables Variables::copy(bool deep_svd) const {
  ...
  if (variablesRep) {
    ...
    vars.variablesRep =
        deep_svd ? get_variables(variablesRep->sharedVarsData.copy())
                 :                                        //    deep SVD copy
...
void Variables::inactive_into_all_variables(const Variables& vars) {
  ...
  if (variablesRep)
    ...
    size_t icv_start = vars.icv_start(), idiv_start = vars.idiv_start(),
           ...
           num_icv = vars.icv(), num_idiv = vars.idiv(), num_idsv = vars.idsv(),
           num_idrv = vars.idrv();
    ...
    copy_data_partial(vars.inactive_continuous_variables(), allContinuousVars,
                      icv_start);
...
void Variables::as_vector(const StringSetArray& dss_vals,
                          RealVector& var_values) const {
  if (variablesRep)
