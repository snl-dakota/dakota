...
static const char rcsId[] =
    "@(#) $Id: SurrogateModel.cpp 7024 2010-10-16 01:24:42Z mseldre $";
...
SurrogateModel::SurrogateModel(ProblemDescDB& problem_db,
    ...
    : Model(problem_db, parallel_lib),
      surrogateFnIndices(
          problem_db.get_szs("model.surrogate.function_indices")),
  ...
  if (surrogateFnIndices.empty())  // default: all fns are approximated
    for (size_t i = 0; i < numFns; ++i) surrogateFnIndices.insert(i);
  else {
...
SurrogateModel::SurrogateModel(ProblemDescDB& problem_db,
                               ...
                               short output_level)
    :  // Allow DFSModel to employ sizing differences (e.g., consuming
       // aggregations)
      Model(surr_view, svd, share_svd, srd, share_srd, surr_set, output_level,
...
bool SurrogateModel::check_active_variables(const Model& sub_model) {
  ...
  if (active_view == sm_active_view) {
    ...
    size_t sm_cv = ModelUtils::cv(sub_model),
           sm_div = ModelUtils::div(sub_model),
           sm_dsv = ModelUtils::dsv(sub_model),
           sm_drv = ModelUtils::drv(sub_model), cv = currentVariables.cv(),
           div = currentVariables.div(), dsv = currentVariables.dsv(),
           drv = currentVariables.drv();
    ...
    size_t sm_cv = ModelUtils::cv(sub_model),
           sm_div = ModelUtils::div(sub_model),
           sm_dsv = ModelUtils::dsv(sub_model),
           sm_drv = ModelUtils::drv(sub_model), acv = currentVariables.acv(),
           adiv = currentVariables.adiv(), adsv = currentVariables.adsv(),
           adrv = currentVariables.adrv();
    ...
    size_t sm_acv = ModelUtils::acv(sub_model),
           sm_adiv = ModelUtils::adiv(sub_model),
           sm_adsv = ModelUtils::adsv(sub_model),
           sm_adrv = ModelUtils::adrv(sub_model), cv = currentVariables.cv(),
           div = currentVariables.div(), dsv = currentVariables.dsv(),
           drv = currentVariables.drv();
...
bool SurrogateModel::check_inactive_variables(const Model& sub_model) {
  ...
  size_t sm_icv = ModelUtils::icv(sub_model),
         sm_idiv = ModelUtils::idiv(sub_model),
         sm_idsv = ModelUtils::idsv(sub_model),
         sm_idrv = ModelUtils::idrv(sub_model), icv = currentVariables.icv(),
         idiv = currentVariables.idiv(), idsv = currentVariables.idsv(),
         idrv = currentVariables.idrv();
...
void SurrogateModel::init_model_constraints(Model& sub_model) {
  size_t num_cv = currentVariables.cv(), num_div = currentVariables.div(),
         ...
         num_sm_cv = ModelUtils::cv(sub_model),
         num_sm_div = ModelUtils::div(sub_model),
         num_sm_dsv = ModelUtils::dsv(sub_model),
         num_sm_drv = ModelUtils::drv(sub_model);
  ...
  if ((lin_ineq || lin_eq) &&
      (ModelUtils::cv(sub_model) != currentVariables.cv() ||
  ...
  if (lin_ineq) {
    ModelUtils::linear_ineq_constraint_coeffs(
        sub_model, userDefinedConstraints.linear_ineq_constraint_coeffs());
    ModelUtils::linear_ineq_constraint_lower_bounds(
        sub_model,
        userDefinedConstraints.linear_ineq_constraint_lower_bounds());
    ModelUtils::linear_ineq_constraint_upper_bounds(
        sub_model,
        userDefinedConstraints.linear_ineq_constraint_upper_bounds());
  }
  if (lin_eq) {
    ModelUtils::linear_eq_constraint_coeffs(
        sub_model, userDefinedConstraints.linear_eq_constraint_coeffs());
    ModelUtils::linear_eq_constraint_targets(
        sub_model, userDefinedConstraints.linear_eq_constraint_targets());
  }
  ...
  if (userDefinedConstraints.num_nonlinear_eq_constraints())
    ModelUtils::nonlinear_eq_constraint_targets(
        sub_model, userDefinedConstraints.nonlinear_eq_constraint_targets());
}
  ...
  if (ModelUtils::response_labels(sub_model).empty())  // should not happen
    switch (responseMode) {
      ...
      case AGGREGATED_MODEL_PAIR: {
        ...
        copy_data_partial(currentResponse.function_labels(), 0, sub_model.qoi(),
                          qoi_labels);
...
void SurrogateModel::init_model_inactive_variables(Model& sub_model) {
  ...
  if (inactive_view && inactive_view == sm_vars.view().second) {
    ...
    sub_model.user_defined_constraints().inactive_bounds(
...
void SurrogateModel::update_model_distributions(Model& sub_model) {
  ...
  if (svd.id() == sm_svd.id())  // same set of variables
    ...
    StringArray pull_labels;
    svd.assemble_all_labels(pull_labels);
    StringArray push_labels;
    sm_svd.assemble_all_labels(push_labels);
    sub_model.multivariate_distribution().pull_distribution_parameters(
        mvDist, pull_labels, push_labels);
  }
}
...
void SurrogateModel::update_distributions_from_model(const Model& sub_model) {
  ...
  if (svd.id() == sm_svd.id())  // same variables specification
    ...
    StringArray pull_labels;
    sm_svd.assemble_all_labels(pull_labels);
    StringArray push_labels;
    svd.assemble_all_labels(push_labels);
    mvDist.pull_distribution_parameters(sub_model.multivariate_distribution(),
...
void SurrogateModel::update_response_from_model(const Model& sub_model) {
  if (!approxBuilds &&
    ...
    switch (responseMode) {
      case AGGREGATED_MODELS:
      case AGGREGATED_MODEL_PAIR: {
        const StringArray& model_labels =
            ModelUtils::response_labels(sub_model);
      ...
      default:
        currentResponse.function_labels(ModelUtils::response_labels(sub_model));
        break;
    }
...
void SurrogateModel::update_all_variables_from_model(const Model& sub_model) {
  ...
  if (!approxBuilds) currentVariables.all_labels(sub_model.current_variables());
}

/** Update values and labels in currentVariables and
    bound/linear/nonlinear constraints in userDefinedConstraints from
    variables and constraints data within model. */
void SurrogateModel::update_complement_variables_from_model(
  ...
  for (i = 0; i < div_begin; ++i) {
    ...
    if (index != _NPOS) {
      ...
      userDefinedConstraints.all_discrete_int_lower_bound(adiv_l_bnds[i],
                                                          index);
      userDefinedConstraints.all_discrete_int_upper_bound(adiv_u_bnds[i],
                                                          index);
    }
  }
...
bool SurrogateModel::check_rebuild(
    const RealVector& ref_icv, const IntVector& ref_idiv,
  ...
  if (!actual_model) {
    ...
    if (strbegins(surrogateType, "global_") &&
        ...
        (ref_c_l_bnds != userDefinedConstraints.continuous_lower_bounds() ||
         ...
         ref_di_l_bnds != userDefinedConstraints.discrete_int_lower_bounds() ||
         ref_di_u_bnds != userDefinedConstraints.discrete_int_upper_bounds() ||
         // no discrete string bounds
         ref_dr_l_bnds != userDefinedConstraints.discrete_real_lower_bounds() ||
         ref_dr_u_bnds != userDefinedConstraints.discrete_real_upper_bounds()))
      return true;
    ...
    if (strbegins(surrogateType, "global_")) {
      if (actual_model->model_type() == "recast") {
        ...
        if (ref_c_l_bnds != ModelUtils::continuous_lower_bounds(*sub_model) ||
            ...
            ref_di_l_bnds !=
                ModelUtils::discrete_int_lower_bounds(*sub_model) ||
            ref_di_u_bnds !=
                ModelUtils::discrete_int_upper_bounds(*sub_model) ||
            // no discrete string bounds
            ref_dr_l_bnds !=
                ModelUtils::discrete_real_lower_bounds(*sub_model) ||
...
void SurrogateModel::asv_combine(const ShortArray& actual_asv,
                                 const ShortArray& approx_asv,
...
void SurrogateModel::response_combine(const Response& actual_response,
...
void SurrogateModel::aggregate_response(const Response& resp1,
  ...
  for (i = 0; i < num_fns1; ++i) {
    ...
    if (asv_i & 4) agg_resp.function_hessian(resp1.function_hessian(i), i);
  }
...
void SurrogateModel::insert_response(
    const Response& response, size_t position,
    Response& agg_response)  //, bool include_metadata)
{
  if (agg_response.is_null())
    agg_response =
        currentResponse.copy();  // resize_response() -> aggregate size
...
void SurrogateModel::insert_metadata(const RealArray& md, size_t position,
