...
template<typename T>
class NonDGlobalReliability: public NonDReliability<T>
{
public:
  ...
  using Iterator::export_final_surrogates;
  using Iterator::maxIterations;
  using Iterator::method_enum_to_string;
...
private:
  ...
  //
  // Set up uSpaceModel
  void build_uspacemodel(Iterator &dace_iterator, String approx_type, String sample_reuse, String import_pts_file);
...
template<typename T>
NonDGlobalReliability<T>* NonDGlobalReliability<T>::nondGlobRelInstance(NULL);

template<typename T>
NonDGlobalReliability<T>::
NonDGlobalReliability(ProblemDescDB& problem_db, Model& model): 
  ...
  meritFunctionType(AUGMENTED_LAGRANGIAN_MERIT), dataOrder(1)
{
  ...
  build_uspacemodel(dace_iterator, approx_type, sample_reuse, import_pts_file);
  ...
  auto importance_sampler_rep = std::make_shared<NonDAdaptImpSampling>
    (env, sample_type, refine_samples, refine_seed,
...
template<>
void NonDGlobalReliability<ProbabilityTransformModel>::
build_uspacemodel(Iterator &dace_iterator, String approx_type, String sample_reuse, String import_pts_file) {
  // instantiate the ProbabilityTransform and GP DataFit recursions
...
template <>
void NonDGlobalReliability<DataFitSurrModel>::
build_uspacemodel(Iterator &dace_iterator, String approx_type, String sample_reuse, String import_pts_file) {
...
template<typename T>
NonDGlobalReliability<T>::~NonDGlobalReliability()
{ }


template<typename T>
bool NonDGlobalReliability<T>::resize()
{
  bool parent_reinit_comms = NonDReliability<T>::resize();
...
template<typename T>
void NonDGlobalReliability<T>::derived_init_communicators(ParLevLIter pl_iter)
{
...
template<typename T>
void NonDGlobalReliability<T>::derived_set_communicators(ParLevLIter pl_iter)
{
...
template<typename T>
void NonDGlobalReliability<T>::derived_free_communicators(ParLevLIter pl_iter)
{
...
template<typename T>
void NonDGlobalReliability<T>::pre_run()
{
  Analyzer::pre_run();
...
template<typename T>
void NonDGlobalReliability<T>::core_run()
{
...
template<typename T>
void NonDGlobalReliability<T>::optimize_gaussian_process()
{
  ...
  for (respFnCount=0; respFnCount<numFunctions; respFnCount++) {
	...
	ModelUtils::continuous_variables(*uSpaceModel, c_vars_u);
	...
	= uSpaceModel->approximation_data(respFnCount);
      size_t num_data_pts = gp_data.size(), num_vars = ModelUtils::cv(*uSpaceModel);
...
template<typename T>
void NonDGlobalReliability<T>::importance_sampling()
{
...
template<typename T>
void NonDGlobalReliability<T>::get_best_sample()
{
...
constraint_penalty(const Real& c_viol, const RealVector& u)
{
  if (meritFunctionType == PENALTY_MERIT)
    ...
    ModelUtils::continuous_variables(*uSpaceModel, u);
    uSpaceModel->evaluate();
    RealVector m_grad_f
...
template<typename T>
void NonDGlobalReliability<T>::print_results(std::ostream& s, short results_state)
{
...
template<typename T>
inline void NonDGlobalReliability<T>::x_truth_evaluation(short mode)
{
...
template<typename T>
inline void NonDGlobalReliability<T>::
