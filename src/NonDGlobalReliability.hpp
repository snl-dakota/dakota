...
class NonDGlobalReliability : public NonDReliability {
 public:
  ...
  /// constructor
  NonDGlobalReliability(ProblemDescDB& problem_db,
...
inline void NonDGlobalReliability::x_truth_evaluation(short mode) {
  ...
  set.request_value(mode, respFnCount);
...
inline void NonDGlobalReliability::x_truth_evaluation(
    const RealVector& c_vars_u, short mode) {
  RealVector c_vars_x;
  SizetMultiArrayConstView x_cv_ids = ModelUtils::continuous_variable_ids(
                               *iteratedModel),
                           u_cv_ids = (mppSearchType == SUBMETHOD_EGRA_X)
                                          ? ModelUtils::continuous_variable_ids(
                                                *uSpaceModel)
                                          : uSpaceModel->subordinate_model()
                                                ->current_variables()
...
inline void NonDGlobalReliability::u_truth_evaluation(
    const RealVector& c_vars_u, short mode) {
  uSpaceModel->component_parallel_mode(TRUTH_MODEL_MODE);  // Recast forwards
...
inline void NonDGlobalReliability::u_evaluation(const RealVector& c_vars_u,
                                                short mode) {
  ModelUtils::continuous_variables(*uSpaceModel, c_vars_u);
  ActiveSet set = uSpaceModel->current_response().active_set();
  set.request_values(0);
