...
/// Derived approximation class for QMEA Quadratic Multipoint Exponential
...
class QMEApproximation : public Approximation {
 ...
 private:
  ...
  RealVector
      beta;  ///< vector of QMEA reduced space diagonal Hessian coefficients
  RealMatrix G_reduced_xfm;  ///< Grahm-Schmidt orthonormal reduced subspace
                             ///< transformation
  size_t numUsed;  ///< number of previous data points used (size of reduced
                   ///< subspace)
...
inline QMEApproximation::QMEApproximation(const SharedApproxData& shared_data)
