...
static const char rcsId[] =
    "@(#) $Id: SeqHybridMetaIterator.cpp 6972 2010-09-17 22:18:50Z briadam $";
...
void SeqHybridMetaIterator::run_sequential() {
  ...
  const ParallelLevel& parent_pl =
      (iterSched.miPLIndex)
          ? methodPCIter->mi_parallel_level(iterSched.miPLIndex - 1)
          : mi_pl;
  ...
  for (seqCount = 0; seqCount < num_iterators; seqCount++) {
    ...
    if (server_id <= iterSched.numIteratorServers) {
      ...
      // graphics window.
      if (rank0 && server_id > 0) curr_iterator.initialize_graphics(server_id);
      ...
      if (rank0) prpResults.resize(iterSched.numIteratorJobs);
      ...
      if (iterSched.messagePass && rank0) {
        ...
        ParamResponsePair prp_star(
            curr_iterator.variables_results(), curr_model->interface_id(),
            curr_iterator.response_results());  // shallow
        ...
        for (size_t i = 0; i < prp_return_size; ++i) results_buffer << prp_star;
...
void SeqHybridMetaIterator::run_sequential_adaptive() {
  ...
  for (seqCount = 0; seqCount < num_iterators; seqCount++) {
    ...
    if (seqCount + 1 <
        num_iterators) {  // prevent index out of range on last pass
      ...
      selectedModels[seqCount + 1]->current_variables().active_variables(
          vars_star);
    }
...
void SeqHybridMetaIterator::update_local_results(PRPArray& prp_results,
                                                 int job_id) {
  Iterator& curr_iterator = *selectedIterators[seqCount];
  auto curr_model =
      (selectedModels.empty()) ? iteratedModel : selectedModels[seqCount];
  ...
  if (curr_iterator.returns_multiple_points()) {
    ...
    for (size_t i = 0; i < num_results; ++i) {
      ...
      prp_results[i] =
          ParamResponsePair(vars, curr_model->interface_id(), resp, job_id);
    }
  } else {
    // need a deep copy for case where multiple instances of
    // best{Variables,Response}Array.front() will be assimilated
    prp_results.resize(1);
    prp_results[0] = ParamResponsePair(
        curr_iterator.variables_results(), curr_model->interface_id(),
...
void SeqHybridMetaIterator::declare_sources() {
  for (const auto si : selectedIterators)
    evaluationsDB.declare_source(method_id(), "iterator", si->method_id(),
                                 "iterator");
}
