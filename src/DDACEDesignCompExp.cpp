...
static const char rcsId[] =
    "@(#) $Id: DDACEDesignCompExp.cpp 7035 2010-10-22 21:45:39Z mseldre $";

namespace Dakota {

/** This constructor is called for a standard iterator built with data from
    probDescDB. */
DDACEDesignCompExp::DDACEDesignCompExp(ProblemDescDB& problem_db,
    ...
    : PStudyDACE(problem_db, parallel_lib, model),
      ...
      symbolsSpec(probDescDB.get_int("method.symbols")),
  ...
  if (daceMethod == SUBMETHOD_BOX_BEHNKEN)
    maxEvalConcurrency *=
        1 + 4 * numContinuousVars * (numContinuousVars - 1) / 2;
  ...
  if (mainEffectsFlag &&
      (daceMethod == SUBMETHOD_RANDOM || daceMethod == SUBMETHOD_BOX_BEHNKEN ||
...
/** This alternate constructor is used for instantiations on-the-fly,
    ...
    queries are used. */
DDACEDesignCompExp::DDACEDesignCompExp(std::shared_ptr<Model> model,
                                       int samples, int symbols, int seed,
                                       unsigned short sampling_method)
    : PStudyDACE(DACE, model),
      daceMethod(sampling_method),
      samplesSpec(samples),
      numSamples(samples),
      symbolsSpec(symbols),
      numSymbols(symbols),
...
void DDACEDesignCompExp::post_run(std::ostream& s) {
  ...
  if (vbdFlag) {
    pStudyDACESensGlobal.compute_vbd_stats_via_sampling(
...
void DDACEDesignCompExp::get_parameter_sets(std::shared_ptr<Model> model,
  ...
  } else if (!varyPattern)  // force same sample pattern: reset to previous
                            // value
    DistributionBase::setSeed(randomSeed);
  ...
  if (mainEffectsFlag) symbolMapping = ddace_sampler->getP();
  ...
  for (int i = 0; i < num_samples; ++i) {
    ...
    for (int j = 0; j < numContinuousVars; ++j) all_samp_i[j] = sample_pt_i[j];
  }
...
std::shared_ptr<DDaceSamplerBase> DDACEDesignCompExp::create_sampler(
    Model& model) {
  ...
  switch (daceMethod) {
    case SUBMETHOD_OAS: {
      return std::make_shared<DDaceOASampler>(numSamples, noise,
                                              ddace_distribution);
      break;
    }
    case SUBMETHOD_OA_LHS: {
      int strength = 2;
      bool randomize = true;
      return std::make_shared<DDaceOALHSampler>(numSamples, numContinuousVars,
                                                strength, randomize,
                                                ddace_distribution);
      break;
    }
    case SUBMETHOD_LHS: {
      ...
      return std::make_shared<DDaceLHSampler>(numSamples, replications, noise,
                                              ddace_distribution);
      break;
    }
    case SUBMETHOD_RANDOM: {
      return std::make_shared<DDaceRandomSampler>(numSamples,
                                                  ddace_distribution);
      break;
    }
    case SUBMETHOD_GRID: {
      return std::make_shared<DDaceFactorialSampler>(numSamples, numSymbols,
                                                     noise, ddace_distribution);
      break;
    }
...
void DDACEDesignCompExp::resolve_samples_symbols() {
  ...
  switch (daceMethod) {
    ...
    case SUBMETHOD_OA_LHS: {
      ...
      if (numSymbols <= 0 || numSamples % (numSymbols * numSymbols) ||
          !prime_flag) {
    ...
    case SUBMETHOD_LHS: {
      ...
      if (numSymbols <= 0)
        ...
        Cout
            ...
            << "symbols.\n";
        int replications =
            (int)std::ceil((double)numSamples / (double)numSymbols);
    ...
    case SUBMETHOD_BOX_BEHNKEN: {
      ...
      if (num_samples_bb >= numSamples)
        ...
        // In this case, the BBD cannot produce enough samples to meet either
        // the user request (which may not be decreased) or the minimum required
        // by a surrogate.
    ...
    case SUBMETHOD_CENTRAL_COMPOSITE: {
      ...
      size_t num_samples_ccd = 1 + 2 * numContinuousVars +
                               (int)std::pow(2.0, (double)numContinuousVars);
      if (num_samples_ccd >= numSamples)
        ...
        // In this case, the CCD cannot produce enough samples to meet either
        // the user request (which may not be decreased) or the minimum required
        // by a surrogate.
        Cerr << "\nError: DDACE method central_composite generates "
             ...
             << "       Either reduce the requirement or select another "
                "sampling "
             << "method.\n"
             << std::endl;
        abort_handler(-1);
    ...
    case SUBMETHOD_GRID: {
      ...
      if (numSamples !=
          std::pow((double)numSymbols, (double)numContinuousVars)) {
        Cout << "\n\nWarning: For grid sampling the number of samples should "
             << "be\n         (num_symbols)^(num_variables).\n";
        numSymbols = (int)std::ceil(
            std::pow((double)numSamples, 1. / (double)numContinuousVars));
        numSamples =
            (int)std::pow((double)numSymbols, (double)numContinuousVars);
      }
      break;
    }
    default:
