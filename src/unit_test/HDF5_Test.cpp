...
TEUCHOS_UNIT_TEST(tpl_hdf5, new_hdf5_test) {
	...
	for(int i = 1; i <= num_evaluations; i++) {
		...
		DataSet* ds_lower_bounds = HDF5_create_1D_dimension_scale ( &scales_group, 4, PredType::IEEE_F64LE, "lower_bounds" );
		DataSet* ds_upper_bounds = HDF5_create_1D_dimension_scale ( &scales_group, 4, PredType::IEEE_F64LE, "upper_bounds" );

		ds_lower_bounds->write( lower_bounds_arr, PredType::IEEE_F64LE );
		ds_upper_bounds->write( upper_bounds_arr, PredType::IEEE_F64LE );
		...
		for(int j = 0; j < 3; j++) {
			...
			herr_t ret_code;
			ret_code = H5DSattach_scale(ds_lower_bounds->getId(), ds_id, 0);
			ret_code = H5DSattach_scale(ds_upper_bounds->getId(), ds_id, 0);
