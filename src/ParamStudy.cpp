...
static const char rcsId[] =
    "@(#) $Id: ParamStudy.cpp 7024 2010-10-16 01:24:42Z mseldre $";

// #define DEBUG

namespace Dakota {

ParamStudy::ParamStudy(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
  ...
  switch (methodName) {
    ...
    case VECTOR_PARAMETER_STUDY: {
      ...
      if (step_vector.empty()) {  // final_point & num_steps spec.
        ...
        if (check_num_steps(
          ...
          err_flag = true;
        // precompute steps (using construct-time initialPoint) and perform
        // error checks only if in check mode; else avoid additional overhead
        // and rely on run-time checks for run-time initialPoint.
        if (numSteps && parallelLib.command_line_check()) {
          initialCVPoint =
              ModelUtils::continuous_variables(*iteratedModel);  // view
          initialDIVPoint =
              ModelUtils::discrete_int_variables(*iteratedModel);  // view
          initialDSVPoint.resize(boost::extents[numDiscreteStringVars]);
          initialDSVPoint =
              ModelUtils::discrete_string_variables(*iteratedModel);  // copy
          initialDRVPoint =
              ModelUtils::discrete_real_variables(*iteratedModel);  // view
          final_point_to_step_vector();  // covers check_ranges_sets(numSteps)
        }
      } else {  // step_vector & num_steps spec.
        // check length and distribute
        if (check_step_vector(step_vector)) err_flag = true;
        // check value
        if (check_num_steps(
                probDescDB.get_int("method.parameter_study.num_steps")))
          err_flag = true;
        ...
        initialDRVPoint =
            ModelUtils::discrete_real_variables(*iteratedModel);  // view
        if (check_ranges_sets(numSteps)) err_flag = true;
      }
      break;
    }
    ...
    case MULTIDIM_PARAMETER_STUDY:
      if (check_variable_partitions(probDescDB.get_usa("method.partitions")))
        err_flag = true;
      if (check_finite_bounds()) err_flag = true;
      ...
      if (parallelLib.command_line_check()) distribute_partitions();
      break;
    default:
...
void ParamStudy::pre_run() {
  ...
  if (av_size != numEvals) {
    ...
    for (size_t i = av_size; i < numEvals; ++i) allVariables[i] = vars.copy();
    if (outputLevel > SILENT_OUTPUT && (methodName == VECTOR_PARAMETER_STUDY ||
                                        methodName == CENTERED_PARAMETER_STUDY))
    ...
    case CENTERED_PARAMETER_STUDY:
      if (outputLevel > SILENT_OUTPUT) {
        ...
        write_ordered(Cout, svd.active_components_totals(),
                      contStepsPerVariable, discIntStepsPerVariable,
...
void ParamStudy::core_run() {
  ...
  bool log_resp_flag =
      (methodName == MULTIDIM_PARAMETER_STUDY) ? (!subIteratorFlag) : false;
  ...
  if (resultsDB.active()) {
    ...
    if (numContinuousVars)
      resultsDB.insert_into(
          run_identifier(),
          {String("parameter_sets"), String("continuous_variables")}, c_vars,
          idx);
    ...
    if (methodName == CENTERED_PARAMETER_STUDY) archive_cps_vars(model, idx);
  }
}

void ParamStudy::archive_model_response(const Response& response,
                                        size_t idx) const {
  // Let's try to write resultsDB stuff here - RWH
  if (resultsDB.active()) {
    const RealVector& resp_vec = response.function_values();
    resultsDB.insert_into(run_identifier(),
                          {String("parameter_sets"), String("responses")},
                          resp_vec, idx);

    if (methodName == CENTERED_PARAMETER_STUDY) archive_cps_resp(response, idx);
  }
}
...
void ParamStudy::archive_allocate_sets() const {
  if (resultsDB.active()) {
    ...
    if (methodName == CENTERED_PARAMETER_STUDY) archive_allocate_cps();
  }
}
...
void ParamStudy::post_run(std::ostream& s) {
  ...
  if (methodName == MULTIDIM_PARAMETER_STUDY && log_resp_flag) {
    pStudyDACESensGlobal.compute_correlations(
        allVariables, allResponses,
        ModelUtils::discrete_set_string_values(
            *iteratedModel));  // to map string variable
                               // values back to indices
    if (resultsDB.active()) {
      pStudyDACESensGlobal.archive_correlations(
  ...
  for (size_t i = 0; i < numEvals; ++i) {
    ...
    if (numDiscreteStringVars)
      allVariables[i].discrete_string_variables(
          listDSVPoints[boost::indices[i]
                                      [idx_range(0, numDiscreteStringVars)]]);
    if (numDiscreteRealVars)
...
void ParamStudy::vector_loop() {
  ...
  for (i = 0; i <= numSteps; ++i) {
    ...
    // active continuous
    for (j = 0; j < numContinuousVars; ++j) c_step(j, i, vars);
...
bool ParamStudy::load_distribute_points(const String& points_filename,
                                        unsigned short tabular_format,
                                        bool active_only) {
  bool err = false;
  ...
  numEvals = TabularIO::read_data_tabular(
      points_filename, "List Parameter Study", listCVPoints, listDIVPoints,
      listDSVPoints, listDRVPoints, tabular_format, active_only,
  ...
  for (size_t i = 0; i < numEvals; ++i) {
    ...
    const BitArray& di_set_bits = ModelUtils::discrete_int_sets(*iteratedModel);
    const IntSetArray& dsi_vals =
        ModelUtils::discrete_set_int_values(*iteratedModel);
    const IntVector& di_lb =
        ModelUtils::discrete_int_lower_bounds(*iteratedModel);
    const IntVector& di_ub =
...
bool ParamStudy::distribute_list_of_points(const RealVector& list_of_pts) {
  size_t i, j, dsi_cntr, start,
      len_lop = list_of_pts.length(),
  ...
  for (i = 0, start = 0; i < numEvals; ++i) {
    ...
    StringMultiArrayView list_dsv_i =
        (numDiscreteStringVars) ? listDSVPoints[boost::indices[i][idx_range(
                                      0, numDiscreteStringVars)]]
                                : empty_sa[boost::indices[idx_range(0, 0)]];
    RealVector& list_drv_i =
        (numDiscreteRealVars) ? listDRVPoints[i] : empty_rv;
    ...
    for (j = 0, dsi_cntr = 0; j < numDiscreteIntVars; ++j) {
      if (di_set_bits[j]) {
        // if set values:
        // if (set_value_to_index(list_div_i[j], dsi_values[dsi_cntr]) ==
        // _NPOS){
        ...
        try {
          ...
          Cerr << e.what() << " for variable '"
               << ModelUtils::discrete_int_variable_labels(*iteratedModel)[j]
               << "' in method '" << method_id() << "'\n";
          abort_handler(-1);
        }
        ++dsi_cntr;
    ...
    for (j = 0; j < numDiscreteStringVars; ++j) {
      ...
      try {
        ...
        Cerr << e.what() << " for variable '"
             << ModelUtils::discrete_string_variable_labels(*iteratedModel)[j]
             << "' in method '" << method_id() << "'\n";
        abort_handler(-1);
      }
    }
    ...
    for (j = 0; j < numDiscreteRealVars; ++j) {
      ...
      try {
        ...
        Cerr << e.what() << " for variable '"
             << ModelUtils::discrete_real_variable_labels(*iteratedModel)[j]
             << "' in method '" << method_id() << "'\n";
        abort_handler(-1);
      }
    }
  ...
  for (i = 0; i < numEvals; ++i) {
    ...
    if (numDiscreteStringVars) {
      ...
      write_data(Cout, listDSVPoints[boost::indices[i][idx_range(
                           0, numDiscreteStringVars)]]);
    }
    if (numDiscreteRealVars)
...
void ParamStudy::distribute_partitions() {
  ...
  const RealVector& dr_vars =
      ModelUtils::discrete_real_variables(*iteratedModel);
  ...
  for (i = 0, dsi_cntr = 0; i < numDiscreteIntVars; ++i) {
    ...
    if (part) {
      ...
      initialDIVPoint[i] = di_vars[i];
      discIntStepVector[i] = 0;
    }
  ...
  for (i = 0; i < numDiscreteStringVars; ++i) {
    ...
    if (part) {
      ...
      initialDSVPoint[i] = ds_vars[i];
      discStringStepVector[i] = 0;
    }
  }
  for (i = 0; i < numDiscreteRealVars; ++i) {
    ...
    if (part) {
      ...
      initialDRVPoint[i] = dr_vars[i];
      discRealStepVector[i] = 0;
    }
  }
  ...
  if (numContinuousVars)
    Cout << "c_vars:\n"
         ...
         << initialCVPoint << "contStepVector:\n"
         << contStepVector;
  if (numDiscreteIntVars)
    Cout << "di_vars:\n"
         << di_vars << "di_l_bnds:\n"
         << di_l_bnds << "di_u_bnds:\n"
         << di_u_bnds << "initialDIVPoint:\n"
         << initialDIVPoint << "discIntStepVector:\n"
         << discIntStepVector;
  if (numDiscreteStringVars) {
    Cout << "ds_vars:\n";
  ...
  if (numDiscreteRealVars)
    Cout << "dr_vars:\n"
         << dr_vars << "dr_l_bnds:\n"
         << dr_l_bnds << "dr_u_bnds:\n"
         << dr_u_bnds << "initialDRVPoint:\n"
         << initialDRVPoint << "discRealStepVector:\n"
         << discRealStepVector;
#endif  // DEBUG
}
...
void ParamStudy::final_point_to_step_vector() {
  ...
  discStringStepVector.sizeUninitialized(numDiscreteStringVars);
  for (j = 0; j < numDiscreteStringVars; ++j)
    discStringStepVector[j] =
        index_step(set_value_to_index(initialDSVPoint[j], dss_values[j]),
                   // for final point defined as index:
  ...
  discRealStepVector.sizeUninitialized(numDiscreteRealVars);
  for (j = 0; j < numDiscreteRealVars; ++j)
    discRealStepVector[j] =
        index_step(set_value_to_index(initialDRVPoint[j], dsr_values[j]),
                   // for final point defined as index:
  ...
  if (numContinuousVars) Cout << "continuous step vector:\n" << contStepVector;
  if (numDiscreteIntVars)
...
bool ParamStudy::check_sets(const IntVector& c_steps, const IntVector& di_steps,
                            const IntVector& ds_steps,
...
void ParamStudy::archive_allocate_cps() const {
  ...
  for (size_t i = 0; i < numContinuousVars; ++i) {
    resultsDB.allocate_vector(
        ...
        Dakota::ResultsOutputType::REAL, 2 * contStepsPerVariable[i] + 1);
    resultsDB.allocate_matrix(
        run_identifier(),
        {String("variable_slices"), cv_labels[i], String("responses")},
        Dakota::ResultsOutputType::REAL, 2 * contStepsPerVariable[i] + 1,
        numFunctions, resp_scales);
  }
  for (size_t i = 0; i < numDiscreteIntVars; ++i) {
    resultsDB.allocate_vector(
        run_identifier(),
        {String("variable_slices"), div_labels[i], String("steps")},
        Dakota::ResultsOutputType::INTEGER, 2 * discIntStepsPerVariable[i] + 1);
    resultsDB.allocate_matrix(
        run_identifier(),
        {"variable_slices", div_labels[i], String("responses")},
        Dakota::ResultsOutputType::REAL, 2 * discIntStepsPerVariable[i] + 1,
        numFunctions, resp_scales);
  }
  for (size_t i = 0; i < numDiscreteStringVars; ++i) {
    ...
    resultsDB.allocate_matrix(
        ...
        Dakota::ResultsOutputType::REAL, 2 * discStringStepsPerVariable[i] + 1,
        numFunctions, resp_scales);
  }
  for (size_t i = 0; i < numDiscreteRealVars; ++i) {
    resultsDB.allocate_vector(
        ...
        Dakota::ResultsOutputType::REAL, 2 * discRealStepsPerVariable[i] + 1);
    resultsDB.allocate_matrix(
        run_identifier(),
...
void ParamStudy::archive_cps_vars(const Model& model, size_t idx) const {
  ...
  if (idx == 0) {
    for (size_t i = 0; i < numContinuousVars; ++i)
      resultsDB.insert_into(
          run_identifier(),
          {String("variable_slices"), cv_labels[i], String("steps")}, c_vars[i],
          contStepsPerVariable[i]);
    for (size_t i = 0; i < numDiscreteIntVars; ++i)
      resultsDB.insert_into(
          run_identifier(),
          {String("variable_slices"), div_labels[i], String("steps")},
  ...
  if (var_idx < numContinuousVars) {
    ...
    size_t drv_idx = var_idx - numContinuousVars - numDiscreteIntVars -
                     numDiscreteStringVars;
    resultsDB.insert_into(
        run_identifier(),
...
void ParamStudy::index_to_var_step(const size_t study_idx, size_t& var_idx,
  ...
  for (var_idx = 0; var_idx < num_vars; ++var_idx) {
    if (study_idx < offset + 2 * stepsPerVariable[var_idx]) break;
