...
void JSONParametersFileWriter::write_parameters_file(
  ...
  json json_out;
  write_evaluation_to_json(vars, set, response, prog, an_comps,
                           formatted_eval_id, json_out);
...
void JSONParametersFileWriter::write_parameters_file(
  ...
  for (const auto& pair : prp_queue) {
    ...
    json single_eval;
    write_evaluation_to_json(pair.variables(), pair.active_set(),
                             pair.response(), prog, an_comps, full_eval_id,
                             single_eval);
...
void JSONParametersFileWriter::write_evaluation_to_json(
    const Variables& vars, const ActiveSet& set, const Response& response,
    const std::string prog, const std::vector<std::string>& an_comps,
    std::string full_eval_id, json& json_obj) const {
  ...
  for (size_t i = 0; i < asv_len; ++i) {
    json jsonfuncs = {{"label", resp_labels[i]}, {"active_set", asv[i]}};
  ...
  for (size_t i = 0; i < ac_len; ++i) {
    json json_an_comps = {{"driver", prog}, {"component", an_comps[i]}};
