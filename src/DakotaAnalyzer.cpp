...
static const char rcsId[] =
    "@(#) $Id: DakotaAnalyzer.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
Analyzer::Analyzer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
                   std::shared_ptr<Model> model)
    : Iterator(problem_db, parallel_lib),
      compactMode(true),
      numObjFns(0),
      numLSqTerms(0),  // default: no best data tracking
  ...
  if (vbdFlag) vbdDropTol = probDescDB.get_real("method.vbd_drop_tolerance");

  if (!numFinalSolutions)   // default is zero
    numFinalSolutions = 1;  // iterator-specific default assignment
}

Analyzer::Analyzer(unsigned short method_name, std::shared_ptr<Model> model)
    : Iterator(method_name, model),
      compactMode(true),
      numObjFns(0),
      numLSqTerms(0),  // default: no best data tracking
      vbdFlag(false),
...
void Analyzer::pre_run() { bestVarsRespMap.clear(); }
...
void Analyzer::evaluate_parameter_sets(Model& model, bool log_resp_flag,
                                       bool log_best_flag) {
  ...
  size_t i,
      num_evals = (compactMode) ? allSamples.numCols() : allVariables.size();
  ...
  if (asynch_flag) {
    ...
    if (compactMode)
      log_response_map(allSamples, resp_map, log_best_flag);
    else
      log_response_map(allVariables, resp_map, log_best_flag);
  }
}
...
void Analyzer::evaluate_batch(Model& model, int batch_id, bool log_best_flag) {
  ...
  if (compactMode) {
    ...
    if (!rtn_pr.second)  // not inserted since batch_id already present
      rv2_it->second.clear();
    num_cv = allSamples.numRows();
    batch_size = allSamples.numCols();
  } else {
  ...
  for (i = 0; i < batch_size; i++) {
    ...
    if (compactMode)
      ...
      v2_it->second[eval_id] =
          allVariables[i];  // *** shallow copy is safe unless allVariables gets
                            // updated in place
  }
}

const IntIntResponse2DMap& Analyzer::synchronize_batches(Model& model,
                                                         bool log_best_flag) {
  // synchronize all batches at once
  const IntResponseMap& full_resp_map = model.synchronize();

  // arrange full set into batch groupings
  ...
  if (compactMode) {
    for (IntIntRealVector2DMap::iterator s_it = batchSamplesMap.begin();
      ...
      // > evals within each batch are sequential, but ordering of batches may
      //   be inverted (e.g., pyramid sampling)
      // if (initial) {
...
void Analyzer::update_model_from_variables(Model& model,
...
void Analyzer::sample_to_variables(const Real* sample_c_vars, Variables& vars) {
...
void Analyzer::get_vbd_parameter_sets(std::shared_ptr<Model> model,
                                      size_t num_samples) {
  if (!compactMode) {
  ...
  if (sample_1.numCols() != num_samples) {
    Cerr << "\nError in Analyzer::variance_based_decomp(): Expected "
         << num_samples << " variable samples; received " << sample_1.numCols()
         << std::endl;
    abort_handler(METHOD_ERROR);
  }
  ...
  if (sample_2.numCols() != num_samples) {
    Cerr << "\nError in Analyzer::variance_based_decomp(): Expected "
         << num_samples << " variable samples; received " << sample_2.numCols()
         << std::endl;
    abort_handler(METHOD_ERROR);
  }
  ...
  for (int i = 0; i < num_vars; ++i) {
    ...
    for (int j = 0; j < num_samples; ++j) sample_r(i, j) = sample_1(i, j);
  }
}
...
void Analyzer::pre_output() {
  ...
  // TODO: consider passing precision to helper functions instead of using
  // global
  int save_precision;
  if (writePrecision == 0) {
  ...
  TabularIO::write_header_tabular(tabular_file,
                                  ...
                                  iteratedModel->current_response(), "eval_id",
  ...
  if (writePrecision == 0) write_precision = save_precision;
  if (outputLevel > QUIET_OUTPUT)
...
void Analyzer::read_variables_responses(int num_evals, size_t num_vars) {
  ...
  for (i = 0, prp_it = import_prp_list.begin(); i < num_evals; ++i, ++prp_it) {
    ...
    if (map_to_iter_space)
      ...
      iter_vars = pr.variables();
      iter_resp = pr.response();
    }
    ...
    if (numObjFns || numLSqTerms) update_best(iter_vars, i + 1, iter_resp);
  }
...
void Analyzer::compute_best_metrics(const Response& response,
  ...
  Real& obj_fn = metrics.second;
  obj_fn = 0.0;
  if (numObjFns) {
    constr_offset = numObjFns;
    if (primary_wts.empty()) {
      for (i = 0; i < numObjFns; i++) obj_fn += fn_vals[i];
      if (numObjFns > 1) obj_fn /= (Real)numObjFns;
    } else
      for (i = 0; i < numObjFns; i++) obj_fn += primary_wts[i] * fn_vals[i];
  } else if (numLSqTerms) {
    constr_offset = numLSqTerms;
    if (primary_wts.empty())
      for (i = 0; i < numLSqTerms; i++) obj_fn += std::pow(fn_vals[i], 2);
    else
    ...
    return;
  Real& constr_viol = metrics.first;
  constr_viol = 0.0;
  size_t num_nln_ineq =
             ModelUtils::num_nonlinear_ineq_constraints(*iteratedModel),
  ...
  for (i = 0; i < num_nln_ineq;
       i++) {  // ineq constraint violation (default tol=0)
...
void Analyzer::update_best(const Real* sample_c_vars, int eval_id,
  ...
  if (num_best_map < numFinalSolutions) {  // initialization of best map
    ...
    if (metrics < it->first) {  // new best
      ...
      ParamResponsePair prp(vars, iteratedModel->interface_id(), copy_resp,
                            eval_id,
                            false);  // shallow copy since previous deep
      std::pair<RealRealPair, ParamResponsePair> new_pr(metrics, prp);
      bestVarsRespMap.insert(new_pr);
    }
  }
...
void Analyzer::update_best(const Variables& vars, int eval_id,
                           const Response& response) {
  RealRealPair metrics;
  compute_best_metrics(response, metrics);
#ifdef DEBUG
  ...
  size_t num_best_map = bestVarsRespMap.size();
  if (num_best_map < numFinalSolutions) {  // initialization of best map
    ParamResponsePair prp(vars, iteratedModel->interface_id(), response,
                          eval_id);  // deep copy
    std::pair<RealRealPair, ParamResponsePair> new_pr(metrics, prp);
    bestVarsRespMap.insert(new_pr);
  } else {
...
void Analyzer::print_results(std::ostream& s, short results_state) {
  if (!numObjFns && !numLSqTerms) {
    s << "<<<<< Best parameters/responses are not reported for generic "
         "response_functions"
      << std::endl;
    return;
  }
