...
static const char rcsId[] =
    "@(#) $Id: SimulationResponse.cpp 7029 2010-10-22 00:17:02Z mseldre $";

namespace Dakota {

SimulationResponse::SimulationResponse(const Variables& vars,
                                       const ProblemDescDB& problem_db)
    : Response(BaseConstructor(), vars, problem_db) {
  sharedRespData.response_type(SIMULATION_RESPONSE);
}

SimulationResponse::SimulationResponse(const SharedResponseData& srd,
                                       const ActiveSet& set)
