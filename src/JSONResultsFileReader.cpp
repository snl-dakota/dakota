...
JSONResultsFileReader::JSONResultsFileReader(bool labeled)
    : ResultsFileReader(labeled) {}

void JSONResultsFileReader::read_results_file(
...
void JSONResultsFileReader::read_results_file(PRPQueue& prp_queue,
  ...
  if (!j.is_array())
    throw ResultsFileError(
        "Batch JSON results file is expected to contain an array at the top "
        "level");
  if (j.size() != num_pairs)
    throw ResultsFileError(
        std::string("Batch JSON results file expected to have ") +
        std::to_string(num_pairs) +
        ", but the top-level array it contains has " +
