...
void compute_regression_coeffs(const RealMatrix& samples,
                               const RealMatrix& resps, RealMatrix& rcoeffs,
                               RealVector& cods) {
  if (samples.numRows() == 0) return;
  ...
  for (int i = 0; i < num_resp; ++i) {
    dakota::surrogates::PolynomialRegression pr(
        copy_samples, copy_responses.col(i), param_list);
    const MatrixXd& coeffs = pr.get_polynomial_coeffs();

    // A workaround to deal with the inability of RealVector to use const
    // double* data
    VectorXd nonconst_coeffs =
        coeffs.col(0).tail(num_vars);  // slice off the constant coeff values
    std::copy(nonconst_coeffs.data(), nonconst_coeffs.data() + num_vars,
              rcoeffs[i]);

    // Compute coefficient of determination, R^2
    VectorXd sur_vals = pr.value(copy_samples);
    cods[i] = dakota::util::compute_metric(sur_vals, copy_responses.col(i),
                                           "rsquared");
  }
...
void compute_std_regression_coeffs(const RealMatrix& samples,
  ...
  for (int j = 0; j < copy_responses.cols(); ++j) {
    ...
    //// Handle the case of resp_stddev = 0 (based on Dakota's definition of
    /// small)
    ...
    VectorXd std_polynomial_coeffs =
        (polynomial_coeffs.array() * stddevs.array()) / resp_stddev;

    std::copy(std_polynomial_coeffs.data(),
              std_polynomial_coeffs.data() + num_vars, std_reg_coeffs[j]);
  }
