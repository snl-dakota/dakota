...
class MFSolutionData {
 public:
  ...
  static void update_estimator_variance_metric(
      short metric_type, Real norm_order, const RealVector& est_var_ratios,
      const RealVector& est_var, Real& est_var_metric, size_t& metric_index);
...
inline MFSolutionData::MFSolutionData(const RealVector& soln_vars,
                                      const RealVector& est_var,
...
inline void MFSolutionData::solution_variables(const SizetArray& samples) {
  ...
  for (i = 0; i < len; ++i) solutionVars[i] = (Real)samples[i];
}

inline std::pair<RealVector, Real> MFSolutionData::anchored_solution_ratios()
    const {
  RealVector ratios;
  Real ratio_ref = 0.;
  int v_len = solutionVars.length();
  if (v_len) {
    int r_len = v_len - 1;
...
inline void MFSolutionData::anchored_solution_ratios(
    const RealVector& soln_ratios, Real soln_ref) {
  int r_len = soln_ratios.length(), v_len = r_len + 1;
  if (solutionVars.length() != v_len) solutionVars.sizeUninitialized(v_len);
  for (int i = 0; i < r_len; ++i) solutionVars[i] = soln_ratios[i] * soln_ref;
...
inline void MFSolutionData::initialize_estimator_variances(size_t num_fns) {
...
inline void MFSolutionData::update_estimator_variance_metric(
    short metric_type, Real norm_order, const RealVector& est_var_ratios,
    const RealVector& est_var, Real& metric, size_t& metric_index) {
  ...
  switch (metric_type) {
    ...
    case AVG_ESTVAR_METRIC:
      metric_index = _NPOS;
      metric = average(est_var);
      break;
    case NORM_ESTVAR_METRIC:
      metric_index = _NPOS;
      metric = p_norm(est_var, norm_order);
      break;
    case MAX_ESTVAR_METRIC:
      metric_index = find_max_index(est_var);
      metric = est_var[metric_index];
      break;
    // metrics relative to MC estvar (see validity note above):
    case AVG_ESTVAR_RATIO_METRIC:
      metric_index = _NPOS;
      metric = average(est_var_ratios);
      break;
    case NORM_ESTVAR_RATIO_METRIC:
      metric_index = _NPOS;
      metric = p_norm(est_var_ratios, norm_order);
      break;
    case MAX_ESTVAR_RATIO_METRIC:
      metric_index = find_max_index(est_var_ratios);
      metric = est_var_ratios[metric_index];
      break;
  }
}

inline void MFSolutionData::update_estimator_variance_metric(
    short metric_type, Real norm_order, const RealVector& est_var,
    Real& metric) {
...
inline void MFSolutionData::initialize_estimator_variance_ratios(
    size_t num_fns) {
  if (estVarRatios.length() != num_fns) estVarRatios.sizeUninitialized(num_fns);
}

inline void MFSolutionData::initialize_estimator_variance_ratios(size_t num_fns,
                                                                 Real val) {
  if (estVarRatios.length() != num_fns) estVarRatios.sizeUninitialized(num_fns);
  estVarRatios.putScalar(val);
}

inline void MFSolutionData::estimator_variance_ratios(
    const RealVector& est_var_ratios) {
  copy_data(est_var_ratios, estVarRatios);
}
...
inline void MFSolutionData::estimator_variance_ratio(Real est_var_ratio,
                                                     size_t i) {
