...
MinimizerAdapterModel(const RealVector& cv_initial_point,
		      ...
		      void (*resp_map) (const Variables& vars,
					const ActiveSet& set,
					Response& response)):
  //Model(LightWtBaseConstructor(), svd, true, srd, true, set, SILENT_OUTPUT),
  adapterModelEvalCntr(0)
{
  modelType = "adapter"; 
  modelId   = "MINIMIZER_ADAPTER";
  supportsEstimDerivs = true; // no subModel to delegate this to
  ...
  init_variables(cv_initial_point);
  init_constraints(cv_lower_bnds, cv_upper_bnds, lin_ineq_coeffs,
...
/** The MinimizerAdapterModel is evaluated by an Iterator for a recast problem
...
void MinimizerAdapterModel::derived_evaluate(const ActiveSet& set)
{
...
void MinimizerAdapterModel::initialize_data_from_submodel()
{
...
void MinimizerAdapterModel::update_variable_values(const Model& model)
{
...
void MinimizerAdapterModel::update_discrete_variable_values(const Model& model)
{
...
void MinimizerAdapterModel::update_variable_bounds(const Model& model)
{
...
void MinimizerAdapterModel::update_discrete_variable_bounds(const Model& model)
{
...
void MinimizerAdapterModel::update_variable_labels(const Model& model)
{
...
void MinimizerAdapterModel::update_discrete_variable_labels(const Model& model)
{
...
void MinimizerAdapterModel::update_linear_constraints(const Model& model)
{
...
void MinimizerAdapterModel::update_response_from_model(Model& model)
{
...
void MinimizerAdapterModel::update_secondary_response(const Model& model)
{
...
void MinimizerAdapterModel::assign_instance()
{ } // no static instance pointer to assign at base (default is no-op)


ActiveSet MinimizerAdapterModel::default_active_set()
{
...
void MinimizerAdapterModel::declare_sources()
{
