...
//- Description: Class implementation for Voronoi Piecewise Surrogate
// Approximation
...
VPSApproximation::VPSApproximation(const ProblemDescDB& problem_db,
    ...
    : Approximation(BaseConstructor(), problem_db, shared_data, approx_label),
      _disc_min_jump(
          problem_db.get_real("model.surrogate.discont_jump_thresh")),
      _disc_min_grad(
          problem_db.get_real("model.surrogate.discont_grad_thresh")) {
  const String& surrogate_type = problem_db.get_string("model.surrogate.type");

  if (surrogate_type != "global_kriging" &&
      surrogate_type != "global_polynomial" &&
      surrogate_type != "global_radial_basis") {
    Cerr << "\nError: Domain decomposition option is not available for "
         << surrogate_type
         << " surrogate; consider polynomial regression, GP, or RBF"
         << std::endl;
    abort_handler(-1);
  }
  ...
  else if (surrogate_type == "global_radial_basis")
    _vps_subsurrogate_basis = radial;
  ...
  std::cout << ".: VPS :.   Initializing, Surrogate order " << surrogateOrder
            << "." << std::endl;
  std::cout << ".: VPS :.   Initializing, Surrogate type " << surrogate_type
            << "." << std::endl;

  _use_derivatives = problem_db.get_bool("model.surrogate.derivative_usage");

  if (_use_derivatives)
    std::cout << ".: VPS :.   Derivatives' information will be used for "
                 "approximation."
              << std::endl;
  else
    std::cout
        << ".: VPS :.   Approximation will not use derivatives' information."
        << std::endl;
}
...
VPSApproximation::VPSApproximation(const SharedApproxData& shared_data)
  ...
  std::shared_ptr<SharedSurfpackApproxData> dat =
      std::static_pointer_cast<SharedSurfpackApproxData>(
          shared_data.data_rep());

  surrogateOrder = dat->approxOrder;

  // std::cout << ".: VPS :.   Initializing, Surrogate order = " <<
  // surrogateOrder << "." << std::endl;
...
/// destructor
VPSApproximation::~VPSApproximation() { VPS_destroy_global_containers(); }
...
// Functions available for debugging: {SmoothHerbie, Herbie, Cone, Cross,
// UnitSphere, Linear34}
#endif
  ...
  for (size_t isample = 0; isample < _num_inserted_points; isample++) {
    ...
    if (_sample_neighbors[isample] != 0)
      num_neighbors = _sample_neighbors[isample][0];
    ...
    for (size_t ineighbor = 1; ineighbor <= num_neighbors; ineighbor++) {
      _vps_ext_neighbors[isample][ineighbor] =
          _sample_neighbors[isample][ineighbor];
    }
  }
    ...
    for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
      ...
      for (size_t i = 1; i <= _vps_ext_neighbors[ipoint][0]; i++) {
        ...
        for (size_t idim = 0; idim < _n_dim; idim++) {
          double dx =
              _sample_points[ipoint][idim] - _sample_points[neighbor][idim];
          dst += dx * dx;
        }
        dst = std::sqrt(dst);
  ...
  if (_vps_subsurrogate == LS) {
    ...
    for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++)
      av_num_basis += _num_cell_basis_functions[ipoint];
    av_num_basis /= _num_inserted_points;

    //
    std::cout << std::endl;
    std::cout << "-- Least Squares results --" << std::endl;
    std::cout << ".: VPS :.   Average Number of polynomial coefficients = "
              << std::fixed << av_num_basis << "." << std::endl;
    std::cout << ".: VPS :.   Number of GMRES solves = " << std::fixed
              << _num_GMRES << "." << std::endl;
  }
  ...
  std::cout
      << "================================================================== "
      << std::endl;
  std::cout << ".: VPS :.   Global surrogate constructed in " << std::fixed
  ...
  for (size_t i = 0; i < num_contours; i++)
    contours.push_back(_f_min + (1.0 / num_contours) * i * (_f_max - _f_min));
...
void VPSApproximation::VPS_create_containers() {
  ...
  std::cout << ".: VPS :.   Constructing a surrogate using " << numObs
  ...
  std::cout << ".: VPS :.   Surrogate order = " << surrogateOrder << "."
            << std::endl;
  ...
  if (_vps_subsurrogate == LS) {
    if (_vps_subsurrogate_basis == polynomial)
      std::cout << ".: VPS :.   Solving Least Squares (LS) for a polynomial "
                   "regression surrogate."
                << std::endl;
    else
      std::cout << ".: VPS :.   Solving Least Squares (LS) for a Radial Basis "
                   "Functions (RBF) surrogate."
                << std::endl;
  } else if (_vps_subsurrogate == GP) {
    std::cout << ".: VPS :.   Constructing a Gaussian Process (GP) surrogate."
              << std::endl;
  }
  ...
  if (_disc_min_jump < 1E-10) {
    _disc_min_jump = DBL_MAX;
    std::cout << ".: VPS :.   Based on function evaluations: disabled."
              << std::endl;
  } else {
    std::cout << ".: VPS :.   Based on function evaluations: Enabled."
              << std::endl;
    std::cout << ".: VPS :.   Discontinuity jump threshold = " << _disc_min_jump
              << std::endl;
  }
  ...
  if (_disc_min_grad < 1E-10) {
    ...
    std::cout << ".: VPS :.   Discontinuity gradient threshold = "
              << _disc_min_grad << std::endl;
  }
  // =======================
  ...
  // Retrieve function gradients: If first point has gradients, I am assuming
  // all points have
  if (_use_derivatives && active_bits_0 & 2) {
    ...
    for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
      ...
      for (size_t idim = 0; idim < _n_dim; idim++) {
        ...
        // std::cout<< "*** For ipoint = " << ipoint << " and idim = " << idim
        // << ":" << std::endl; std::cout<< "*** VPS::  fgrad = " <<
        // _fgrad[ipoint][idim] << std::endl;
      }
      delete[] fn_grad;
    }
  ...
  // Retrieve function hessians: If first point has gradients, I am assuming all
  // points have
      ...
      for (size_t idim = 0; idim < _n_dim; idim++) {
        ...
        for (size_t jdim = 0; jdim < _n_dim; jdim++) {
          _fhess[ipoint][idim][jdim] = fn_hessian[idim][jdim] *
                                       (_xmax[idim] - _xmin[idim]) *
                                       (_xmax[jdim] - _xmin[jdim]);
        }
  ...
  for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
    for (size_t idim = 0; idim < _n_dim; idim++) {
      _sample_points[ipoint][idim] =
          (_sample_points[ipoint][idim] - _xmin[idim]) /
          (_xmax[idim] - _xmin[idim]);
    }
  }
  // =====
  ...
  for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      ...
      if (_sample_points[ipoint][idim] < _xmin[idim])
        _xmin[idim] = _sample_points[ipoint][idim];
      if (_sample_points[ipoint][idim] > _xmax[idim])
        _xmax[idim] = _sample_points[ipoint][idim];
      double DX = _xmax[idim] - _xmin[idim];
      _diag += DX * DX;
    }
    _diag = std::sqrt(_diag);
  ...
  // Retrieve function gradients: If first point has gradient, I am assuming all
  // points have gradients
  if (_use_derivatives && active_bits_0 & 2) {
    _use_gradient = true;
    _fgrad = new double*[_num_inserted_points];
    for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
  ...
  // Retrieve function hessians: If first point has hessian, I am assuming all
  // points have hessians
  if (_use_derivatives && active_bits_0 & 4) {
    _use_hessian = true;
    _fhess = new double**[_num_inserted_points];
    for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
  ...
  if (_vps_subsurrogate == LS) {
    if (_vps_subsurrogate_basis == polynomial) {
      ...
      for (size_t icell = 0; icell < _num_inserted_points; icell++) {
        retrieve_permutations(_num_cell_basis_functions[icell], _vps_t[icell],
                              _n_dim, _vps_order, true, _vps_order);
      }
    }
    ...
    sharedData = SharedApproxData(approx_type, approx_order, _n_dim, data_order,
                                  output_level);
...
void VPSApproximation::VPS_retrieve_neighbors(size_t ipoint,
                                              bool update_point_neighbors) {
  ...
  while (num_misses < max_misses) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      if (tmp_pnt[idim] > 1.0) {
        double t = (1.0 - _sample_points[ipoint][idim]) /
                   (tmp_pnt[idim] - _sample_points[ipoint][idim]);
        if (t < t_end) t_end = t;
      }
      if (tmp_pnt[idim] < 0.0) {
        double t = _sample_points[ipoint][idim] /
                   (_sample_points[ipoint][idim] - tmp_pnt[idim]);
        if (t < t_end) t_end = t;
      }
    }
    for (size_t idim = 0; idim < _n_dim; idim++)
      tmp_pnt[idim] = _sample_points[ipoint][idim] +
                      t_end * (tmp_pnt[idim] - _sample_points[ipoint][idim]);
    ...
    for (size_t jpoint = 0; jpoint < _num_inserted_points; jpoint++) {
      ...
      for (size_t idim = 0; idim < _n_dim; idim++) {
        qH[idim] =
            0.5 * (_sample_points[ipoint][idim] + _sample_points[jpoint][idim]);
      ...
      if (trim_line_using_Hyperplane(_n_dim, _sample_points[ipoint], tmp_pnt,
                                     qH, nH)) {
        ineighbor = jpoint;
      }
    }
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      double dx =
          _sample_points[ipoint][idim] - _sample_points[ineighbor][idim];
      h += dx * dx;
    }
    h = std::sqrt(h);
    ...
    if ((jump > _disc_min_jump) || (grad > _disc_min_grad))
      new_neighbor = false;  // gradient of two point exceeds threshold
  ...
  for (size_t i = 0; i < num_neighbors; i++)
    _sample_neighbors[ipoint][i + 1] = tmp_neighbors[i];
...
void VPSApproximation::VPS_adjust_extend_neighbors_of_all_points() {
  for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++) {
    while (_vps_ext_neighbors[ipoint][0] <
               2 * _num_cell_basis_functions[ipoint] &&
           _vps_ext_neighbors[ipoint][0] < _num_inserted_points - 1) {
      size_t num_old_neighbors = _vps_ext_neighbors[ipoint][0];
      VPS_extend_neighbors(ipoint);

      size_t num_new_neighbors = _vps_ext_neighbors[ipoint][0];
      if (num_old_neighbors == num_new_neighbors)
...
void VPSApproximation::VPS_extend_neighbors(size_t ipoint) {
  ...
  size_t* tmp_neighbors = new size_t[_num_inserted_points];
  for (size_t i = 1; i <= _vps_ext_neighbors[ipoint][0]; i++)
    tmp_neighbors[i - 1] = _vps_ext_neighbors[ipoint][i];

  for (size_t i = 1; i <= _vps_ext_neighbors[ipoint][0]; i++) {
    size_t neighbor = _vps_ext_neighbors[ipoint][i];
    for (size_t j = 1; j <= _sample_neighbors[neighbor][0];
         j++)  // extend neighbors of ipoint using direct neighbors of neighbor
    {
...
void VPSApproximation::VPS_build_local_surrogate(size_t cell_index) {
  if (_vps_subsurrogate == LS) {
    ...
    for (size_t j = 0; j <= _vps_ext_neighbors[cell_index][0];
         j++)  // loop over neighbors
    {
      size_t neighbor = cell_index;
      if (j > 0) neighbor = _vps_ext_neighbors[cell_index][j];

      gpApproximations[cell_index].add(
          training_vars[neighbor], false, training_resp[neighbor], false, false,
          INT_MAX);  // shallow, shallow, not anchor,no eval id tracking
    }
...
double VPSApproximation::VPS_evaluate_surrogate(double* x) {
  ...
  for (size_t idim = 0; idim < _n_dim; idim++)
    x_vps[idim] = (x[idim] - _xmin[idim]) / (_xmax[idim] - _xmin[idim]);
  ...
  if (_vps_subsurrogate == LS) {
    // LS Surrogate
    double f_VPS = 0.0;
    for (size_t ibasis = 0; ibasis < _num_cell_basis_functions[iclosest];
         ibasis++) {
      ...
      //{
      //     std::cout<< "(x,y)[iclosest] = (" << _sample_points[iclosest][0] <<
      //     "," << _sample_points[iclosest][1] << ")" << std::endl; std::cout<<
      //     "f[iclosest] = " << _fval[iclosest] << std::endl; std::cout <<
      //     "Weight = " << wi << std::endl; std::cout << "Poly term = " << yi
      //     << std::endl;
      // }
    }
    delete[] x_vps;
...
void VPSApproximation::VPS_destroy_global_containers() {
  ...
  if (_vps_subsurrogate == LS) {
    ...
    if (_vps_subsurrogate_basis == polynomial) {
      delete[] _vps_dfar;
      for (size_t icell = 0; icell < _num_inserted_points; icell++) {
        for (size_t ibasis = 0; ibasis < _num_cell_basis_functions[icell];
             ibasis++) {
...
void VPSApproximation::retrieve_permutations(size_t& m, size_t**& perm,
                                             size_t num_dim, size_t upper_bound,
                                             bool force_sum_constraint,
                                             size_t sum_constraint) {
  ...
  while (true) {
    while (t[k_dim] <= upper_bound) {
      ...
      if (valid) {
        ...
        // for (size_t idim = 0; idim < num_dim; idim++) std::cout <<
        // perm[m][idim] << " "; std::cout << std::endl;
  ...
  if (num_basis >= _n_dim * (_n_dim + 1) / 2 + _n_dim + 1) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      for (size_t jdim = idim; jdim < _n_dim; jdim++) {
        // search fot the corresponding basis and bring it to its proper
        // location
        for (size_t ibasis = 0; ibasis < num_basis; ibasis++) {
          ...
          for (size_t kdim = 0; kdim < _n_dim; kdim++) {
            ...
            if (perm[ibasis][kdim] > 0 && sig_dim_i == _n_dim) {
              sig_dim_i = kdim;
              sig_dim_j = sig_dim_i;
            } else if (perm[ibasis][kdim] > 0 && sig_dim_i < _n_dim)
              sig_dim_j = kdim;
          }
  ...
  {
      for (size_t idim = 0; idim < num_dim; idim++) std::cout <<
  perm[ibasis][idim] << " "; std::cout << std::endl;
  }
  */
...
void VPSApproximation::build_radial_basis_function(size_t icell) {
  ...
  for (size_t idim = 0; idim < _n_dim; idim++)
    dart[idim] = _sample_points[icell][idim];
  ...
  while (true) {
    ...
    if (valid_dart) {
      ...
      if (num_successive_misses == max_num_successive_misses)
        break;  // iactive++; create only a single layer of basis functions
                // around seed
    }
  }
  ...
  for (size_t ibasis = 0; ibasis < num_basis; ibasis++)
    _sample_basis[icell][ibasis] = rbfs[ibasis];
  rbfs.clear();
  delete[] dart;
  return;
}
...
void VPSApproximation::VPS_LS_retrieve_weights(size_t cell_index) {
  ...
  if (_use_derivatives && _use_gradient && surrogateOrder >= 1 &&
      _vps_subsurrogate_basis == polynomial) {
  ...
  if (_use_derivatives && _use_hessian && surrogateOrder >= 2 &&
      _vps_subsurrogate_basis == polynomial) {
    size_t iloc = _n_dim + 1;
  ...
  // std::cout << "*** num_basis_resolved = " << num_basis_resolved <<
  // std::endl;
  ...
  if (num_basis > num_basis_resolved + num_neighbors + 1)
    num_basis = num_basis_resolved + num_neighbors + 1;
  ...
  for (size_t ibasis = 0; ibasis < num_basis; ibasis++) {
    ...
    for (size_t irow = num_basis_resolved; irow < num_rows; irow++) {
      size_t neighbor = cell_index;
      if (irow > num_basis_resolved)
        neighbor = _vps_ext_neighbors[cell_index][irow - num_basis_resolved];

      H[ibasis][irow] =
          evaluate_basis_function(_sample_points[neighbor], cell_index, ibasis);
    }
  }
  ...
  if (num_basis_resolved > 0) {
    // std::cout << "******* Number of bases resvoled " << num_basis_resolved <<
    // std::endl; Case (Some coefficients have been already found) Reconstruct
    // "reduced system", and solve for the rest of coefficients
  ...
  constrained_LeastSquare(num_basis, num_neighbors + 1, H, _vps_w[cell_index],
                          b);

  for (size_t ibasis = 0; ibasis < num_basis; ibasis++) {
    // std::cout << "******* Basis Coefficient " << _vps_w[cell_index][ibasis]
    // << std::endl;
    delete[] H[ibasis];
  }
  delete[] H;
...
double VPSApproximation::evaluate_basis_function(double* x, size_t icell,
                                                 size_t ibasis) {
  if (_vps_subsurrogate_basis == polynomial) {
    double* y = new double[_n_dim];
    // shift origin
    for (size_t idim = 0; idim < _n_dim; idim++)
      y[idim] = (x[idim] - _sample_points[icell][idim]);  // / _vps_dfar[icell];
...
// =======================
int VPSApproximation::constrained_LeastSquare(size_t n, size_t m, double** H,
                                              double* w, double* f) {
  ...
  if (fabs(H[0][0]) < 1E-10) {
    std::cout << ".: VPS :.   Contrained Least Square: Dividing by zero."
              << std::endl;
    return 1;
  }

  // taking away constrained equation
  double* crow = new double[n];
  double fo = f[0];
...
double VPSApproximation::vec_pow_vec(size_t num_dim, double* vec_a,
                                     size_t* vec_b) {
  double ans = 1.0;
...
void VPSApproximation::Cholesky_solver(int n, double** LD, double* b,
                                       double* x) {
...
void VPSApproximation::GMRES(size_t n, double** A, double* b, double* x,
                             double eps) {
  size_t im = 0;
  ...
  for (im = 0; im < n; im++) {
    ...
    if (rNorm < eps * ro && rNorm < eps) break;
  }
...
void VPSApproximation::initiate_random_number_generator(unsigned long x) {
  ...
  c = 0.0;
  zc = 0.0; /* current CSWB and SWB `borrow` */
...
bool VPSApproximation::trim_line_using_Hyperplane(
    ...
    double* qH, double* nH)   // a point on the hyperplane and it normal
{
  ...
  if (u > 0.0 && u < 1.0) {
    for (size_t idim = 0; idim < num_dim; idim++) {
      if (dote > 0.0)
        end[idim] = st[idim] + u * (end[idim] - st[idim]);
      else
        st[idim] = st[idim] + u * (end[idim] - st[idim]);
    }
    return true;
  }
...
double VPSApproximation::f_test(double* x) {
  if (_vps_test_function == SmoothHerbie) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      ...
      double xp = x[idim] + 1.0;
      double wherb =
          exp(-xm * xm) + exp(-0.8 * xp * xp) - 0.05 * sin(8 * (x[idim] + 0.1));
      fval *= wherb;
    }
    fval = -fval;
    return fval;
...
double** VPSApproximation::hessian_f_test(double* x) {
  ...
  for (size_t idim = 0; idim < _n_dim; idim++) H[idim] = new double[_n_dim];
...
void VPSApproximation::generate_poisson_disk_sample(double r) {
  ...
  while (num_points < _num_inserted_points) {
    ...
    for (size_t ipoint = 0; ipoint < num_points; ipoint++) {
      ...
      // std::cout << "num_point = " << num_points << " , dst_sq = " << dst_sq
      // << " , r_sq = "<< r_sq << std::endl;
      if (dst_sq < r_sq) {
        valid_dart = false;
        break;
...
void VPSApproximation::isocontouring(std::string file_name,
                                     bool plot_test_function,
                                     bool plot_surrogate,
                                     std::vector<double> contours) {
  ...
  for (size_t i = 0; i < 1000; i++) {
    ...
    for (size_t j = 0; j < 1000; j++) {
      for (size_t ifunc = 0; ifunc < 2; ifunc++) {
        ...
        xx[0] = xo;
        xx[1] = yo;
        if (plot_test_function && ifunc != 1)
          fo = f_test(xx);
        else if (plot_surrogate)
          fo = VPS_evaluate_surrogate(xx);
        xx[0] = xo + sx;
        xx[1] = yo;
        if (plot_test_function && ifunc != 1)
          f1 = f_test(xx);
        else if (plot_surrogate)
          f1 = VPS_evaluate_surrogate(xx);
        xx[0] = xo + sx;
        xx[1] = yo + sy;
        if (plot_test_function && ifunc != 1)
          f2 = f_test(xx);
        else if (plot_surrogate)
          f2 = VPS_evaluate_surrogate(xx);
        xx[0] = xo;
        xx[1] = yo + sy;
        if (plot_test_function && ifunc != 1)
          f3 = f_test(xx);
        else if (plot_surrogate)
          f3 = VPS_evaluate_surrogate(xx);
        ...
        for (size_t icont = 0; icont < num_isocontours; icont++) {
          ...
          if ((fo > contour && f1 < contour) ||
              (fo < contour && f1 > contour)) {
          ...
          if ((f2 > contour && f3 < contour) ||
              (f2 < contour && f3 > contour)) {
          ...
          if ((f3 > contour && fo < contour) ||
              (f3 < contour && fo > contour)) {
          ...
          if (num_points == 2) {
            if (ifunc == 0) {
              ...
              if (gs < go)
                ...
                g = 1.0 / (1.0 - gn) - gs / (1.0 - gn);

              if (gs < 0.5)
                b = 0.0;
              else if (gs < 0.75)
                b = 1.0 - 16.0 * (gs - 0.75) * (gs - 0.75);
              else
                b = 1.0;
...
void VPSApproximation::isocontouring_solid(std::string file_name,
                                           bool plot_test_function,
                                           bool plot_surrogate,
                                           std::vector<double> contours) {
  ...
  for (size_t i = 0; i < num_cells; i++) {
    ...
    for (size_t j = 0; j < num_cells; j++) {
      ...
      for (size_t icont = 0; icont < num_isocontours; icont++) {
        ...
        // std::cout<< "contour_m = " << contour_m << " , contour = " << contour
        // << std::endl;
        ...
        if (fo >= contour_m - 1E-10 && fo < contour + 1E-10) {
          ...
          if ((fo > contour && f1 < contour) ||
            ...
            poly_y.push_back(yo);
          } else if ((fo > contour_m && f1 < contour_m) ||
                     (fo < contour_m && f1 > contour_m)) {
        ...
        if (f1 >= contour_m - 1E-10 && f1 < contour + 1E-10) {
          ...
          } else if ((f1 > contour_m && f2 < contour_m) ||
                     (f1 < contour_m && f2 > contour_m)) {
        ...
        if (f2 >= contour_m - 1E-10 && f2 < contour + 1E-10) {
          ...
          if ((f2 > contour && f3 < contour) ||
            ...
            poly_y.push_back(yo + sy);
          } else if ((f2 > contour_m && f3 < contour_m) ||
                     (f2 < contour_m && f3 > contour_m)) {
        ...
        if (f3 >= contour_m - 1E-10 && f3 < contour + 1E-10) {
          ...
          } else if ((f3 > contour_m && fo < contour_m) ||
                     (f3 < contour_m && fo > contour_m)) {
        ...
        if (num_corners > 1) {
          ...
          file << "newpath" << std::endl;
          file << poly_x[0] * scale << " " << poly_y[0] * scale << " moveto"
               << std::endl;
          // std::cout<< "*** x = " <<  poly_x[0] << ", y = " << poly_y[0] <<
          // std::endl;
          for (size_t icorner = 1; icorner < num_corners; icorner++) {
            file << poly_x[icorner] * scale << " " << poly_y[icorner] * scale
                 << " lineto" << std::endl;
            // std::cout << "*** x = " <<  poly_x[icorner] << ", y = " <<
            // poly_y[icorner] << std::endl;
          }
          // std::cout << std::endl;
...
void VPSApproximation::plot_neighbors() {
  ...
  for (size_t isample = 0; isample < _num_inserted_points; isample++) {
    ...
    for (size_t i = 1; i <= num_neighbors; i++) {
      ...
      double sample_x =
          _xmin[0] + _sample_points[isample][0] * (_xmax[0] - _xmin[0]);
      double sample_y =
          _xmin[1] + _sample_points[isample][1] * (_xmax[1] - _xmin[1]);
      double neighbor_x =
          _xmin[0] + _sample_points[neighbor][0] * (_xmax[0] - _xmin[0]);
      double neighbor_y =
          _xmin[1] + _sample_points[neighbor][1] * (_xmax[1] - _xmin[1]);
  ...
  for (size_t index = 0; index < _num_inserted_points; index++) {
    // plot vertex
    double sample_x =
        _xmin[0] + _sample_points[index][0] * (_xmax[0] - _xmin[0]);
    double sample_y =
        _xmin[1] + _sample_points[index][1] * (_xmax[1] - _xmin[1]);
...
void VPSApproximation::VPSmodel_apply(const RealVector& approx_pt,
