...
NonDGlobalEvidence::NonDGlobalEvidence(ProblemDescDB& problem_db,
...
void NonDGlobalEvidence::initialize() { calculate_cells_and_bpas(); }

void NonDGlobalEvidence::set_cell_bounds() {
  size_t j;
  for (j = 0; j < numContIntervalVars; ++j) {
    ModelUtils::continuous_lower_bound(*intervalOptModel,
                                       cellContLowerBounds[cellCntr][j], j);
    ModelUtils::continuous_upper_bound(*intervalOptModel,
                                       cellContUpperBounds[cellCntr][j], j);
  }

  for (j = 0; j < numDiscIntervalVars; ++j) {
    ModelUtils::discrete_int_lower_bound(
        *intervalOptModel, cellIntRangeLowerBounds[cellCntr][j], j);
    ModelUtils::discrete_int_upper_bound(
        *intervalOptModel, cellIntRangeUpperBounds[cellCntr][j], j);
  }

  for (j = 0; j < numDiscSetIntUncVars; ++j)
    ModelUtils::discrete_int_variable(*intervalOptModel,
                                      cellIntSetBounds[cellCntr][j],
                                      j + numDiscIntervalVars);

  for (j = 0; j < numDiscSetRealUncVars; ++j)
    ModelUtils::discrete_real_variable(*intervalOptModel,
                                       cellRealSetBounds[cellCntr][j], j);
}
  ...
  for (i = 0; i < numCells; ++i) {
    ...
    for (j = 0, j < numContIntervalVars; ++j)
      Cout << "Cell bounds for continuous variable " << j << ": ("
           << cellContLowerBounds[i][j] << ", " << cellContUpperBounds[i][j]
           << ")\n";
    for (j = 0; j < numDiscIntervalVars; ++j)
  ...
  if (eval_approx) {
    if ((!maximize && truthFnStar == DBL_MAX) ||
      ...
      for (i = 0; i < numContIntervalVars; ++i)
        ModelUtils::continuous_variable(*fHatModel,
                                        (cellContLowerBounds[cellCntr][i] +
                                         cellContUpperBounds[cellCntr][i]) /
                                            2.,
                                        i);
      for (i = 0; i < numDiscIntervalVars; ++i)
        ModelUtils::discrete_int_variable(
            *fHatModel,
            (cellIntRangeLowerBounds[cellCntr][i] +
             cellIntRangeUpperBounds[cellCntr][i]) /
                2,
            i);
      for (i = 0; i < numDiscSetIntUncVars; ++i)
        ModelUtils::discrete_int_variable(
            *fHatModel, cellIntSetBounds[cellCntr][i], i + numDiscIntervalVars);
      ...
      if (numContIntervalVars)
        ModelUtils::continuous_variables(*fHatModel,
                                         sdv.continuous_variables());
