...
class NonDEnsembleSampling : public NonDSampling {
 public:
  ...
  NonDEnsembleSampling(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
...
inline Real NonDEnsembleSampling::inactive_budget_deduction() const {
  return 0.;
}  // default implementation (overridden by GenACV,ML BLUE)
...
inline Real NonDEnsembleSampling::active_budget() const {
  ...
  if (!offline && !accuracy_constrained) budget -= inactive_budget_deduction();
  return budget;
}

inline void NonDEnsembleSampling::print_multigroup_summary(
    std::ostream& s, const String& summary_type, bool projections) const {
}  // default is no-op

inline void NonDEnsembleSampling::print_variance_reduction(
    std::ostream& s) const {}  // default is no-op
...
inline void NonDEnsembleSampling::recover_online_cost(
  ...
  SizetArray num_cost;
  num_cost.assign(len, 0);  // init to 0
...
inline int NonDEnsembleSampling::seed_sequence(size_t index) {
  ...
  if (seq_len == 0)
    ...
    seedIndex = std::min(index, seq_len - 1);  // use end if sequence exhausted
           ...
           0)  // pilot sample: only advance until sequence exhausted
...
inline void NonDEnsembleSampling::compute_mc_estimator_variance(
    const RealVector& var_l, const SizetArray& N_l, RealVector& mc_est_var) {
...
inline void NonDEnsembleSampling::increment_samples(SizetArray& N_samp,
                                                    size_t incr) {
  if (!incr) return;
  size_t q, nq = N_samp.size();
  for (q = 0; q < nq; ++q) N_samp[q] += incr;
}

inline void NonDEnsembleSampling::increment_samples(SizetArray& N_samp,
                                                    const SizetArray& incr) {
  size_t q, nq = N_samp.size();
  if (incr.size() != nq) {
  ...
  for (q = 0; q < nq; ++q) N_samp[q] += incr[q];
}

inline void NonDEnsembleSampling::increment_samples(Sizet2DArray& N_samp,
                                                    const SizetArray& incr) {
  ...
  for (l = 0; l < nl; ++l) increment_samples(N_samp[l], incr[l]);
}

inline void NonDEnsembleSampling::increment_sums(Real* sum_l, const Real* incr,
                                                 size_t len) {
  for (size_t i = 0; i < len; ++i) sum_l[i] += incr[i];
}
...
inline void NonDEnsembleSampling::compute_mf_control(Real sum_L, Real sum_H,
                                                     Real sum_LL, Real sum_LH,
                                                     size_t N_shared,
                                                     Real& beta) {
...
inline void NonDEnsembleSampling::project_mc_estimator_variance(
    const RealVector& var_l, const SizetArray& N_l, size_t new_samp,
    RealVector& mc_est_var) {
...
inline void NonDEnsembleSampling::uncentered_to_centered(Real rm1, Real rm2,
                                                         Real rm3, Real rm4,
                                                         Real& cm1, Real& cm2,
                                                         Real& cm3, Real& cm4) {
...
inline void NonDEnsembleSampling::uncentered_to_centered(Real rm1, Real rm2,
  ...
  if (Nlq > 3) {
    ...
    cm4 = (n_sq * Nlq * cm4 / nm1 -
           (6. * Nlq - 9.) * (n_sq - Nlq) / (n_sq - 2. * Nlq + 3) * cm2 * cm2) /
          ((n_sq - 3. * Nlq + 3.) -
           (6. * Nlq - 9.) * (n_sq - Nlq) / (Nlq * (n_sq - 2. * Nlq + 3.)));
  } else
...
/** For single-level moment calculations without a sample count. */
inline void NonDEnsembleSampling::uncentered_to_centered(const Real* rm,
                                                         Real* cm,
                                                         size_t num_mom) {
  if (num_mom >= 1) {
    Real& cm1 = cm[0];
...
inline void NonDEnsembleSampling::uncentered_to_centered(
    const RealMatrix& raw_mom, RealMatrix& cent_mom) {
...
inline void NonDEnsembleSampling::centered_to_standard(Real cm1, Real cm2,
                                                       Real cm3, Real cm4,
                                                       Real& sm1, Real& sm2,
                                                       Real& sm3, Real& sm4) {
  ...
  if (cm2 > 0.) {
    ...
    sm2 = 0.;
    sm3 = cm3;
    sm4 = cm4;  // or assign NaN to sm{3,4}
  }
}

inline void NonDEnsembleSampling::centered_to_standard(const Real* cm, Real* sm,
                                                       size_t num_mom) {
  if (num_mom >= 1) {
    sm[0] = cm[0];  // mean
    if (num_mom >= 2) {
      ...
      if (cm2 > 0.) {
        ...
        sm2 = 0.;
        sm[2] = cm[2];
        sm[3] = cm[3];  // or assign NaN to sm{3,4}
      }
    }
  }
}
...
/** Convert full matrix of moments. */
inline void NonDEnsembleSampling::centered_to_standard(
    const RealMatrix& cent_mom, RealMatrix& std_mom) {
...
inline bool NonDEnsembleSampling::valid_variances(
    const RealVector& estvar) const {
  size_t q, num_q = estvar.length();
  for (q = 0; q < num_q; ++q)
    if (!valid_variance(estvar[q])) return false;
  return true;
}
