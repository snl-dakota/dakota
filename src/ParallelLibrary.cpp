...
static const char rcsId[] =
    "@(#) $Id: ParallelLibrary.cpp 7013 2010-10-08 01:03:38Z wjbohnh $";
...
/** TODO: Update comment.
    ...
    (programOptions)
 */
ParallelLibrary::ParallelLibrary(const MPIManager& mpi_mgr,
    ...
    : mpiManager(mpi_mgr),
      ...
      outputManager(output_mgr),
      dummyFlag(false),
      outputTimings(programOptions.proceed_to_run()),
      startClock(0),
      currPCIter(parallelConfigurations.end()) {
...
void ParallelLibrary::init_mpi_comm() {
  ...
  if (pl.serverCommSize > 1) {
    ...
    start_msg += std::to_string(pl.serverCommSize) + " processors.";
  } else
  ...
  increment_parallel_configuration();  // define 1st parallelConfigurations
                                       // entry
}
...
void ParallelLibrary::init_communicators(
    const ParallelLevel& parent_pl, int num_servers, int procs_per_server,
    int min_procs_per_server, int max_procs_per_server, int max_concurrency,
  ...
  if (child_pl.messagePass) ++currPCIter->numParallelLevels;
...
void ParallelLibrary::resolve_inputs(
    ParallelLevel& child_pl, int avail_procs, int min_procs_per_server,
    int max_procs_per_server, int max_concurrency, int capacity_multiplier,
    short default_config, short scheduling_override, bool peer_dynamic_avail,
    bool print_rank) {
  ...
  if (print_rank)
    Cout << "ParallelLibrary::resolve_inputs() called with num_servers = "
         ...
         << " avail_procs = " << avail_procs
         << " min_procs_per_server = " << min_procs_per_server
         << " max_procs_per_server = " << max_procs_per_server
         << " max_concurrency = " << max_concurrency
         << " capacity_multiplier = " << capacity_multiplier
         << " default_config = " << default_config
         << " scheduling_override = " << scheduling_override << std::endl;
#endif

  const bool ded_sched_override =
      (scheduling_override == DEDICATED_SCHEDULER_DYNAMIC);
  const bool peer_override = (scheduling_override == PEER_SCHEDULING ||
  ...
  if (avail_procs == 1) {
    ...
    num_servers = 1;
    procs_per_server = 1;
    ded_scheduler = false;                               // peer partition
    ...
    if (total_request == avail_procs) {
      ...
      ded_scheduler =
          !peer_override;  // dedicate a scheduler if no peer override
      ...
      if (total_request < avail_procs && print_rank)
        Cerr << "\nWarning: user override of server count and server size "
             ...
             << total_request << ", avail = " << avail_procs
             << ", idle = " << avail_procs - total_request << ")\n\n";
    ...
    if (procs_per_server >=
        max_procs_per_server) {                 // include = due to remainder
      ...
      if (print_rank) {
        ...
        if (idle)
          Cerr << "\nWarning: user override of servers (" << num_servers
               ...
               << "processors (avail = " << server_procs
               << ", utilized = " << utilized << ", idle = " << idle << ")\n\n";
      }
    } else
      proc_remainder = server_procs % num_servers;
  } else if (procs_per_server >
             0) {  // Data default is 0: user request of 1
                   // executes this block as a manual override
    // -------------------------------
    ...
    if (procs_per_server == avail_procs) {
      ...
      else if (peer_servers ==
                   1 ||  // avail_procs not enough for 2 full servers
    ...
    if (server_procs % procs_per_server && print_rank)
      Cerr << "\nWarning: user override of server size results in idle "
           << "processors\n         (server size request = " << procs_per_server
           << ", avail = " << server_procs
           << ", idle = " << server_procs - num_servers * procs_per_server
           << ")\n\n";
  } else {
    // --------------------------------------------------
    ...
    if (min_procs_per_server == avail_procs) {
      ...
      int server_procs = (ded_scheduler) ? avail_procs - 1 : avail_procs;
      num_servers =
          std::min(server_procs / min_procs_per_server, loaded_servers);
      procs_per_server = server_procs / num_servers;
      if (procs_per_server >=
          max_procs_per_server) {                 // = included for remainder
        procs_per_server = max_procs_per_server;  // truncate if necessary
        proc_remainder = 0;  // don't reallocate; leave any remainder as idle
  ...
  if (print_rank)
    Cout << "ParallelLibrary::resolve_inputs() returns num_servers = "
         << num_servers << " procs_per_server = " << procs_per_server
         << " proc_remainder = " << proc_remainder
         << " dedicated scheduler = " << ded_scheduler << std::endl;
#endif
}

void ParallelLibrary::split_communicator_dedicated_scheduler(
  ...
  int i,
      color_cntr = 1, end_rank = 0,
      addtl_procs = child_pl.procRemainder / child_pl.numServers,  // truncated
      proc_rem_cntr =
          child_pl.procRemainder - addtl_procs * child_pl.numServers;
  ...
  if (parent_pl.serverCommRank > end_rank) color = color_cntr;
  if (end_rank + 1 < parent_pl.serverCommSize) child_pl.idlePartition = true;
  ...
  if (child_pl.procsPerServer == 1 && !child_pl.procRemainder &&
    ...
    child_pl.serverId =
        parent_pl.serverCommRank;  // 0=ded sched, 1/2/...=servers
    return;
  }
  ...
  child_pl.serverId =
      color;  // scheduler = 0, servers = 1/2/3/.../n, idle = n+1
...
#ifdef MPI_DEBUG
  Cout << "worldRank = " << mpiManager.world_rank()
       << " child comm rank = " << child_pl.serverCommRank
       << " child comm size = " << child_pl.serverCommSize << '\n';
  ...
  color =
      (child_pl.serverCommRank == 0 && child_pl.serverId <= child_pl.numServers)
          ? 1
          : 0;
  MPI_Comm_split(parent_pl.serverIntraComm, color, parent_pl.serverCommRank,
...
void ParallelLibrary::split_communicator_peer_partition(
    const ParallelLevel& parent_pl, ParallelLevel& child_pl) {
  // ----------------------------------------------------------------
  // Check to see if resulting partition sizes require comm splitting
  // ----------------------------------------------------------------
  ...
  if (!child_pl.idlePartition) {
    ...
    // Note: can safely comment out this special case if heavier weight
    //       approach required for future contexts.
    if (child_pl.procsPerServer == 1 &&
        !child_pl.procRemainder) {  // 1-proc peers
      copy_as_hub_server_comm(parent_pl, child_pl);
      child_pl.serverLeaderFlag = true;
      child_pl.serverId =
          parent_pl.serverCommRank + 1;  // peer id's = 1/2/3/.../n
      return;
    }
    // special case: assign child = parent without partitioning
...
void ParallelLibrary::print_configuration() {
  ...
  for (i = last_index; i > 0; --i) {
    ...
    for (j = 1; j < i;
         ++j)  // stop one short of mi_iter[i] (see serverId==0 below)
    ...
    if (server1_messaging) {
      ...
      if (mi_iter->dedicatedSchedulerFlag) {
        ...
        if (mi_iter->serverId == 1 && mi_iter->serverCommRank == 0) {
          ...
          for (j = last_index; j > i; --j) send_buffer << mi_pls[j];
          buffer_len = send_buffer.size();
        ...
        // RECV: if ded scheduler, must receive data from iter server 1
          ...
          for (j = last_index; j > i; --j) recv_buffer >> mi_pls[j];
        }
      }
    }
  }
  ...
  if (mpiManager.world_rank() == 0) {  // does all output
    ...
    for (i = 1; i < num_mi; ++i) {
      ...
      if (mi_pl.messagePass) {
        ...
        if (mi_pl.dedicatedSchedulerFlag)
          Cout << "ded. sched.\n";
        else
          Cout << "peer\n";
      }
    ...
    if (ie_pl.dedicatedSchedulerFlag)
      Cout << "ded. sched.\n";
    else
      Cout << "peer\n";
    ...
    if (ea_pl.dedicatedSchedulerFlag)
      Cout << "ded. sched.\n";
    else
      Cout << "peer\n";
    ...
    Cout << "multiprocessor analysis\t  " << std::setw(4)
         << ea_pl.procsPerServer
...
void ParallelLibrary::push_output_tag(const ParallelLevel& pl) {
  ...
  // restart files, etc.
  if (pl.serverId > pl.numServers) return;
  ...
  if (pl.serverCommRank == 0) {
    ...
    if (pl.hubServerCommSize > 1) {
      ...
      if (pl.hubServerCommRank == 0) {
        MPIPackBuffer send_buffer;
        send_buffer << programOptions << outputManager.graph2DFlag
        ...
        recv_buffer >> programOptions >> outputManager.graph2DFlag >>
            outputManager.tabularDataFlag >> outputManager.tabularDataFile >>
            outputManager.resultsOutputFlag >> outputManager.resultsOutputFile;
      }
    }
  }
...
void ParallelLibrary::terminate_modelcenter() {
  ...
  if (mc_ptr_int) mc_release_com(ireturn, iprint, mc_ptr_int);
  if (ireturn == -1) {
    Cerr << "Error: mc_release of API pointer unsuccessful." << std::endl;
    abort_handler(-1);
  } else
    mc_ptr_int = 0;
  if (dc_ptr_int) mc_release_com(ireturn, iprint, dc_ptr_int);
  if (ireturn == -1) {
...
void ParallelLibrary::output_timers() {
...
#ifdef DAKOTA_UTILIB
  Real parentCPU = CPUSeconds() - startCPUTime, childCPU = totalCPU - parentCPU,
  ...
  if (mpiManager.mpirun_flag()) {  // MPI functions are available
    ...
    if (mpiManager.world_rank() == 0) {
      ...
      Cout << " [parent   = " << std::setw(10) << parentCPU
           << ", child = " << std::setw(10) << childCPU
           << "]\n  Total wall clock = " << std::setw(10) << totalWC
           << " [MPI_Init = " << std::setw(10) << initWC
      ...
      time_attrs.push_back(
          ResultAttribute<Real>("total_wallclock_time", totalWC));
      time_attrs.push_back(
          ResultAttribute<Real>("mpi_init_wallclock_time", initWC));
    ...
    Cout << " [parent = " << std::setw(10) << parentCPU
         << ", child = " << std::setw(10) << childCPU
         << "]\n  Total wall clock = " << std::setw(10) << totalWC << std::endl;
    time_attrs.push_back(ResultAttribute<Real>("parent_cpu_time", parentCPU));
    time_attrs.push_back(ResultAttribute<Real>("child_cpu_time", childCPU));
    time_attrs.push_back(
        ResultAttribute<Real>("total_wallclock_time", totalWC));
