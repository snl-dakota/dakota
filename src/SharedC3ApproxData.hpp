...
class SharedC3ApproxData : public SharedApproxData {
 ...
 protected:
  ...
  // void configuration_options(const Pecos::ExpansionConfigOptions&
  // ec_options); void configuration_options(const Pecos::BasisConfigOptions&
  // bc_options);
  ...
  // BitArray ranVarsKey; // stored locally rather than passed to library
  // (Pecos)
...
inline void SharedC3ApproxData::active_model_key(const Pecos::ActiveKey& key) {
  ...
  if (startOrdersMap.find(key) == startOrdersMap.end()) {
    startOrdersMap[key] = startOrders;
    form = true;
  }
  if (startRankMap.find(key) == startRankMap.end()) {
    startRankMap[key] = startRank;
    form = true;
  }
  if (maxOrderMap.find(key) == maxOrderMap.end()) {
    maxOrderMap[key] = maxOrder;
    if (adaptOrder) form = true;
  }
  if (maxRankMap.find(key) == maxRankMap.end()) {
    maxRankMap[key] = maxRank;
    if (adaptRank) form = true;
  }
...
inline void SharedC3ApproxData::assign_start_ranks(
    SizetVector& start_ranks) const {
  size_t v, num_vp1 = numVars + 1, start_r = start_rank();
  if (start_ranks.length() != num_vp1) start_ranks.sizeUninitialized(num_vp1);
...
SharedC3ApproxData::max_cross_validation_order_candidates() const {
  return maxCVOrderCandidates;
}
...
inline size_t SharedC3ApproxData::regression_size(size_t num_v, size_t rank,
                                                  size_t max_rank,
                                                  const UShortArray& orders,
                                                  unsigned short max_order) {
  // Each dimension has its own rank within the product of function cores.
  ...
  switch (num_v) {
    ...
    default: {  // first, last, and num_v-2 middle cores
      ...
      p = std::min(orders[0], max_order) + 1;
      sum = p;  // first
      p = std::min(orders[num_vm1], max_order) + 1;
      sum += p;  // last
...
inline size_t SharedC3ApproxData::regression_size() {
  return regression_size(numVars, start_rank(), max_rank(), start_orders(),
                         max_order());
...
inline size_t SharedC3ApproxData::max_order_regression_size() {
  // UShortArray max_orders; RealVector dim_pref;//isotropic for now: no XML
  // spec Pecos::dimension_preference_to_anisotropic_order(max_o, dim_pref,
  //   numVars, max_orders);

  unsigned short max_o = max_order();
  UShortArray max_orders(numVars,
                         max_o);  // no dim_pref; adaptOrder is isotropic
...
inline void SharedC3ApproxData::set_parameter(String var,
                                              const UShortArray& val) {
  if (var.compare("start_order") == 0)
    startOrders = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}

inline void SharedC3ApproxData::set_parameter(String var, unsigned short val) {
  if (var.compare("kick_order") == 0)
    kickOrder = val;
  else if (var.compare("max_order") == 0)
    maxOrder = val;
  else if (var.compare("max_cv_order") == 0)
    maxCVOrderCandidates = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}
...
inline void SharedC3ApproxData::set_parameter(String var, size_t val) {
  if (var.compare("start_rank") == 0)
    ...
    maxRank = val;
  else if (var.compare("max_cv_rank") == 0)
    maxCVRankCandidates = val;
  else if (var.compare("max_solver_iterations") == 0)
    maxSolverIterations = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}

inline void SharedC3ApproxData::set_parameter(String var, bool val) {
  if (var.compare("adapt_rank") == 0)
    adaptRank = val;
  else if (var.compare("adapt_order") == 0)
    adaptOrder = val;
  else if (var.compare("response_scaling") == 0)
    respScaling = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}
...
inline void SharedC3ApproxData::set_parameter(String var, short val) {
  if (var.compare("regress_type") == 0)
    regressType = val;
  else if (var.compare("discrepancy_type") == 0)
    discrepReduction = val;
  else if (var.compare("alloc_control") == 0)
    allocControl = val;
  else if (var.compare("combine_type") == 0)
    combineType = val;
  else if (var.compare("advancement_type") == 0)
    c3AdvancementType = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}

inline void SharedC3ApproxData::set_parameter(String var, double val) {
  if (var.compare("solver_tol") == 0)
    solverTol = val;
  else if (var.compare("solver_rounding_tol") == 0)
    solverRoundingTol = val;
  else if (var.compare("stats_rounding_tol") == 0)
    statsRoundingTol = val;
  else if (var.compare("regularization_parameter") == 0)
    regressRegParam = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}
...
inline void SharedC3ApproxData::set_parameter(String var, int val) {
  if (var.compare("max_cross_iterations") == 0)
    crossMaxIter = val;
  else if (var.compare("random_seed") == 0)
    randomSeed = val;
  else
    Cerr << "Unrecognized C3 parameter: " << var << std::endl;
}

inline void SharedC3ApproxData::set_active_parameter(String var,
                                                     const UShortArray& val) {
  if (var.compare("start_order") == 0) {
    startOrdersMap[activeKey] = val; /*update_basis();*/
  } else
    Cerr << "Unrecognized C3 active parameter: " << var << std::endl;
}
...
inline void SharedC3ApproxData::set_active_parameter(String var, size_t val) {
  if (var.compare("start_rank") == 0)
    startRankMap[activeKey] = val;
  else if (var.compare("max_rank") == 0)
    maxRankMap[activeKey] = val;
  else
    Cerr << "Unrecognized C3 active parameter: " << var << std::endl;
}

inline void SharedC3ApproxData::set_active_parameter(String var, int val) {
  if (var.compare("random_seed") == 0)
    randomSeed = val;
  else
    Cerr << "Unrecognized C3 active parameter: " << var << std::endl;
}

inline void SharedC3ApproxData::random_variables_key(
    const BitArray& random_vars_key) {
  // store incoming mask:
  ...
  if (random_vars_key.empty()) {
    ...
    for (size_t i = 0; i < numVars; ++i) randomIndices[i] = i;
  } else {
    ...
    for (i = 0, cntr = 0; i < numVars; ++i)
      if (random_vars_key[i]) randomIndices[cntr++] = i;
  }
}
