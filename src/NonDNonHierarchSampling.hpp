...
class NonDNonHierarchSampling : public NonDEnsembleSampling {
 ...
 protected:
  ...
  virtual Real augmented_linear_ineq_violations(
  ...
  void increment_sample_range(SizetArray& N_L, size_t incr,
                              const SizetArray& approx_sequence, size_t start,
                              size_t end);
  void increment_sample_range(SizetArray& N_L, size_t incr,
                              const UShortArray& approx_set);
  void increment_sample_range(SizetArray& N_L, size_t incr,
                              const SizetArray& approx_sequence, size_t start,
                              size_t end, const UShortArray& approx_set);
  ...
  void pick_mfmc_cvmc_solution(
      const MFSolutionData& mf_soln,  // size_t mf_samp,
 ...
 private:
  ...
  static void optpp_fdnlf1_constraint(int n, const RealVector& x, RealVector& c,
                                      int& result_mode);
...
inline void NonDNonHierarchSampling::initialize_sums(
    IntRealMatrixMap& sum_L_baseline, IntRealVectorMap& sum_H,
    IntRealMatrixMap& sum_LH, RealVector& sum_HH) {
  // sum_* are running sums across all increments
  std::pair<int, RealVector> vec_pr;
  std::pair<int, RealMatrix> mat_pr;
  for (int i = 1; i <= 4; ++i) {
...
inline void NonDNonHierarchSampling::initialize_counts(
    Sizet2DArray& num_L_baseline, SizetArray& num_H, Sizet2DArray& num_LH) {
  num_H.assign(numFunctions, 0);
  num_L_baseline.resize(numApprox);
  num_LH.resize(numApprox);
  for (size_t approx = 0; approx < numApprox; ++approx) {
...
inline void NonDNonHierarchSampling::initialize_group_sums(
...
inline void NonDNonHierarchSampling::initialize_group_sums(
  ...
  for (int i = 1; i <= 4; ++i) sum_G[i] = mat1;  // copies
}

inline void NonDNonHierarchSampling::initialize_group_counts(
    Sizet2DArray& num_G) {
  size_t g, num_groups = modelGroups.size(), num_models;
  num_G.resize(num_groups);
  for (g = 0; g < num_groups; ++g) num_G[g].assign(numFunctions, 0);
}
...
inline void NonDNonHierarchSampling::competed_initial_guesses(
    MFSolutionData& mf_soln, MFSolutionData& cv_soln,
    MFSolutionData& selected_soln) {
...
inline void NonDNonHierarchSampling::estimator_variance_ratios(
    const RealVector& cd_vars, RealVector& estvar_ratios) {
...
inline void NonDNonHierarchSampling::estimator_variance_ratio_gradients(
    const RealVector& cd_vars, RealMatrix& evr_grads) {
...
inline void NonDNonHierarchSampling::estimator_variances(
    const RealVector& cd_vars, RealVector& est_var) {
  RealVector estvar_ratios(numFunctions, false);
  estimator_variance_ratios(cd_vars,
                            estvar_ratios);  // virtual: MFMC,ACV,GenACV
  estimator_variances_from_ratios(cd_vars, estvar_ratios, est_var);
}

inline void NonDNonHierarchSampling::estimator_variances_and_ratios(
    const RealVector& cd_vars, RealVector& estvar_ratios, RealVector& estvar) {
  estimator_variance_ratios(cd_vars,
                            estvar_ratios);  // virtual: MFMC,ACV,GenACV
...
inline void NonDNonHierarchSampling::estimator_variances_and_ratios(
    const RealVector& cd_vars, MFSolutionData& soln) {
...
inline void NonDNonHierarchSampling::estimator_variance_gradients(
...
inline void NonDNonHierarchSampling::estimator_variances_and_gradients(
...
inline void NonDNonHierarchSampling::estimator_variance_ratios_and_gradients(
    const RealVector& cd_vars, RealVector& estvar_ratios,
    RealMatrix& evr_grads) {
  // default implementation
...
inline void NonDNonHierarchSampling::estvar_ratios_to_estvar(
    const RealVector& estvar_ratios, const RealVector& var_H,
    const SizetArray& N_H, RealVector& estvar) {
...
inline void NonDNonHierarchSampling::estvar_ratios_to_estvar(
    const RealVector& estvar_ratios, const RealVector& var_H, Real N_H,
    RealVector& estvar) {
  if (estvar.length() != numFunctions) estvar.sizeUninitialized(numFunctions);
  for (size_t qoi = 0; qoi < numFunctions; ++qoi)
...
inline void NonDNonHierarchSampling::estvar_ratios_to_estvar(
    const RealVector& var_H, const SizetArray& N_H, MFSolutionData& soln) {
...
inline Real NonDNonHierarchSampling::estimator_variance_metric(
    const RealVector& cd_vars) {
  RealVector estvar_ratios, estvar;
  Real metric;
  size_t metric_index;
  ...
  MFSolutionData::update_estimator_variance_metric(
      estVarMetricType, estVarMetricNormOrder, estvar_ratios, estvar, metric,
      metric_index);
  return metric;
}

inline void NonDNonHierarchSampling::estimator_variance_metric_gradient(
    const RealVector& cd_vars, RealVector& evm_grad) {
  RealVector ev_vec;
  RealMatrix ev_grads;
  switch (estVarMetricType) {
    case NORM_ESTVAR_METRIC:
    case MAX_ESTVAR_METRIC:
      estimator_variances_and_gradients(cd_vars, ev_vec, ev_grads);
      break;
    case NORM_ESTVAR_RATIO_METRIC:
    case MAX_ESTVAR_RATIO_METRIC:
      estimator_variance_ratios_and_gradients(cd_vars, ev_vec, ev_grads);
      break;
    default:
      estimator_variance_gradients(cd_vars, ev_grads);
      break;
...
inline void NonDNonHierarchSampling::estimator_variance_metric_and_gradient(
    const RealVector& cd_vars, Real& ev_metric, RealVector& evm_grad) {
  RealVector estvar_ratios, estvar;
  RealMatrix ev_grads;
  switch (estVarMetricType) {
    case AVG_ESTVAR_RATIO_METRIC:
  ...
  size_t metric_index;
  MFSolutionData::update_estimator_variance_metric(
      estVarMetricType, estVarMetricNormOrder, estvar_ratios, estvar, ev_metric,
      metric_index);
}

inline Real NonDNonHierarchSampling::log_estvar_metric(
    const RealVector& cd_vars) {
...
inline void NonDNonHierarchSampling::log_estvar_metric_gradient(
    const RealVector& cd_vars, RealVector& log_evm_grad) {
  RealVector evm_grad;
  estimator_variance_metric_gradient(cd_vars, evm_grad);
  ...
  Real ev_metric = estimator_variance_metric(cd_vars);
  if (ev_metric > 0.)
    for (v = 0; v < num_v; ++v) log_evm_grad[v] = evm_grad[v] / ev_metric;
  else
    log_evm_grad = 0.;
}

inline void NonDNonHierarchSampling::log_estvar_metric_and_gradient(
    const RealVector& cd_vars, Real& log_evm, RealVector& log_evm_grad) {
  Real ev_metric;
  RealVector evm_grad;
...
inline void NonDNonHierarchSampling::finalize_counts(
    const Sizet2DArray& N_L_actual, const SizetArray& N_L_alloc) {
  ...
  for (approx = 0; approx < numApprox; ++approx) {
    ...
    if (N_l_actual_fl.empty())
      N_l_actual_fl = N_L_actual[approx];
    else
      increment_samples(N_l_actual_fl, N_L_actual[approx]);
  }
}

inline Real NonDNonHierarchSampling::compute_equivalent_cost(
    Real hf_target, const RealVector& avg_eval_ratios, const RealVector& cost) {
  size_t a, hf_index = cost.length() - 1;
  Real cost_ref = cost[hf_index], rc_sum = 0.;
  for (a = 0; a < numApprox; ++a) rc_sum += avg_eval_ratios[a] * cost[a];
...
inline void NonDNonHierarchSampling::increment_equivalent_cost(
    size_t new_samp, const RealVector& cost, size_t index,
    Real& equiv_hf_evals) {
  size_t len = cost.length(), hf_index = len - 1;
  equiv_hf_evals += (index == hf_index)
                        ? new_samp
                        : (Real)new_samp * cost[index] / cost[hf_index];
}

inline void NonDNonHierarchSampling::increment_equivalent_cost(
    size_t new_samp, const RealVector& cost, size_t start, size_t end,
    Real& equiv_hf_evals) {
  size_t index, len = cost.length(), hf_index = len - 1;
  if (end == len) {
    equiv_hf_evals += (Real)new_samp;
    --end;
  }
  Real sum_cost = 0.;
  for (index = start; index < end; ++index) sum_cost += cost[index];
  equiv_hf_evals += (Real)new_samp * sum_cost / cost[hf_index];
}

inline void NonDNonHierarchSampling::increment_equivalent_cost(
    size_t new_samp, const RealVector& cost, const SizetArray& approx_sequence,
...
inline void NonDNonHierarchSampling::increment_equivalent_cost(
    ...
    Real& equiv_hf_evals) {
  size_t i, approx, num_approx = approx_set.size(),
                    hf_index = cost.length() - 1;
  bool ordered = approx_sequence.empty();
  // This fn is only used for LF sample increments:
...
inline void NonDNonHierarchSampling::increment_equivalent_cost(
    size_t new_samp, const RealVector& cost, unsigned short root,
    const UShortSet& reverse_dag, Real& equiv_hf_evals) {
  Real sum_cost = 0;
...
inline void NonDNonHierarchSampling::increment_equivalent_cost(
    const SizetArray& delta_N_g, const RealVector& group_cost, Real hf_cost,
    Real& equiv_hf_evals) {
  ...
  for (g = 0; g < group_len; ++g)
    if (delta_N_g[g]) sum += (Real)delta_N_g[g] * group_cost[g];
  equiv_hf_evals += sum / hf_cost;
}

inline void NonDNonHierarchSampling::increment_sample_range(
    SizetArray& N_L, size_t incr, const SizetArray& approx_sequence,
    size_t start, size_t end) {
  if (!incr) return;
  bool ordered = approx_sequence.empty();
  size_t i, approx;
...
inline void NonDNonHierarchSampling::increment_sample_range(
    SizetArray& N_L, size_t incr, const UShortArray& approx_set) {
  if (!incr) return;
  size_t i, num_approx = approx_set.size();
  for (i = 0; i < num_approx; ++i) N_L[approx_set[i]] += incr;
}
...
inline void NonDNonHierarchSampling::increment_sample_range(
    SizetArray& N_L, size_t incr, unsigned short root,
    const UShortSet& reverse_dag) {
  if (!incr) return;
...
inline void NonDNonHierarchSampling::ensemble_active_set(
...
inline void NonDNonHierarchSampling::root_reverse_dag_to_group(
    unsigned short root, const UShortSet& rev_dag, UShortArray& model_group) {
  model_group.clear();
  model_group.reserve(rev_dag.size() + 1);
  model_group.insert(model_group.end(), rev_dag.begin(), rev_dag.end());
  model_group.push_back(root);  // by convention
}

inline void NonDNonHierarchSampling::group_to_root_reverse_dag(
    const UShortArray& model_group, unsigned short& root, UShortSet& rev_dag) {
  root = model_group.back();  // by convention
  rev_dag.clear();
  rev_dag.insert(model_group.begin(), --model_group.end());
}

inline void NonDNonHierarchSampling::mfmc_model_group(
    size_t last_index, UShortArray& model_group) const {
...
inline void NonDNonHierarchSampling::mfmc_model_group(
    size_t last_index, const SizetArray& approx_sequence,
    UShortArray& model_group) const {
  if (approx_sequence.empty()) {
    mfmc_model_group(last_index, model_group);
    return;
  }
  ...
  for (m = seq_mapping_len; m < group_size; ++m) model_group[m] = m;
...
inline void NonDNonHierarchSampling::singleton_model_group(
    size_t index, UShortArray& model_group) const {
  model_group.resize(1);
  model_group[0] = index;
}

inline void NonDNonHierarchSampling::singleton_model_group(
    size_t index, const SizetArray& approx_sequence,
    UShortArray& model_group) const {
  if (approx_sequence.empty()) {
    singleton_model_group(index, model_group);
    return;
  }
  model_group.resize(1);
  model_group[0] =
      (index < approx_sequence.size()) ? approx_sequence[index] : index;
}

inline void NonDNonHierarchSampling::cvmc_model_group(
    size_t index, UShortArray& model_group) const {
  if (index < numApprox)
    singleton_model_group(index, model_group);
  else
...
inline void NonDNonHierarchSampling::cvmc_model_group(
    ...
    UShortArray& model_group) const {
  if (approx_sequence.empty()) {
    cvmc_model_group(index, model_group);
    return;
  }
  if (index < numApprox)
...
inline void NonDNonHierarchSampling::mlmc_model_group(
    size_t index, UShortArray& model_group) const {
  ...
  if (index == 0) {
    ...
    model_group[0] = index - 1;
    model_group[1] = index;  // ordered low to high
  }
}
...
inline void NonDNonHierarchSampling::mlmc_model_group(
    ...
    UShortArray& model_group) const {
  if (approx_sequence.empty()) {
    mlmc_model_group(index, model_group);
    return;
  }
  // MLMC or ACV-RD (ACV-IS differs in shared group)
  if (index == 0)
...
inline void NonDNonHierarchSampling::print_group(std::ostream& s,
                                                 size_t g) const {
  const UShortArray& group_g = modelGroups[g];
  size_t m, num_models = group_g.size();
  s << " (models";
  for (m = 0; m < num_models; ++m) s << ' ' << group_g[m];
  s << ")\n";
}

inline Real NonDNonHierarchSampling::allocate_budget(
    const RealVector& avg_eval_ratios, const RealVector& cost, Real budget) {
...
inline Real NonDNonHierarchSampling::allocate_budget(
    const UShortArray& approx_set, const RealVector& avg_eval_ratios,
    const RealVector& cost, Real budget) {
  // version with scalar HF target (eval_ratios already averaged over QoI
  // due to formulation of optimization sub-problem)
...
inline void NonDNonHierarchSampling::pick_mfmc_cvmc_solution(
    const MFSolutionData& mf_soln,  // size_t mf_samp,
    const MFSolutionData& cv_soln,  // size_t cv_samp,
    MFSolutionData& soln)           //, size_t& num_samp)
{
...
inline void NonDNonHierarchSampling::apply_mc_reference(
...
inline void NonDNonHierarchSampling::enforce_bounds(RealVector& x0,
...
inline void NonDNonHierarchSampling::r_and_N_to_N_vec(
    const RealVector& avg_eval_ratios, Real N_H, RealVector& N_vec) {
  size_t num_approx = avg_eval_ratios.length(), vec_len = num_approx + 1;
  if (N_vec.length() != vec_len) N_vec.sizeUninitialized(vec_len);
  for (size_t i = 0; i < num_approx; ++i) N_vec[i] = avg_eval_ratios[i] * N_H;
  N_vec[num_approx] = N_H;
}

inline void NonDNonHierarchSampling::r_and_N_to_design_vars(
    const RealVector& avg_eval_ratios, Real N_H, RealVector& cd_vars) {
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT:  // embed N at end of cd_vars for GenACV
                                    // usage
    ...
    case N_MODEL_LINEAR_CONSTRAINT:
      r_and_N_to_N_vec(avg_eval_ratios, N_H, cd_vars);
      break;
  }
}

inline void NonDNonHierarchSampling::solution_to_design_vars(
    const MFSolutionData& soln, RealVector& cd_vars) {
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT:  // embed N at end of cd_vars for GenACV
                                    // usage
    case R_AND_N_NONLINEAR_CONSTRAINT: {
      RealVector r = soln.solution_ratios();
      size_t num_r = r.length();
      cd_vars.sizeUninitialized(num_r + 1);
      copy_data_partial(r, cd_vars, 0);
      cd_vars[num_r] = soln.solution_reference();
      break;
    }
    default:  // N_i for models or groups
      copy_data(soln.solution_variables(), cd_vars);
      break;
  }
}

inline Real NonDNonHierarchSampling::update_hf_target(
    const RealVector& estvar_ratios, size_t metric_index,
    const RealVector& var_H, const RealVector& estvar_0) {
...
inline Real NonDNonHierarchSampling::update_hf_target(
  ...
  switch (estVarMetricType) {
    ...
    case MAX_ESTVAR_RATIO_METRIC:
      hf_target =
          estvar_ratios[metric_index] * var_H[metric_index] / convergenceTol;
      break;
  }
  Cout << "Scaling profile for convergenceTol = " << convergenceTol
       << ": HF target = " << hf_target << std::endl;
...
inline Real NonDNonHierarchSampling::update_hf_target(
    const RealVector& estvar, size_t metric_index, const SizetArray& N_H,
    const RealVector& estvar_0) {
...
inline Real NonDNonHierarchSampling::update_hf_target(const RealVector& estvar,
                                                      size_t metric_index,
                                                      const SizetArray& N_H) {
  // Starting from last expression above: convert estvar_ratio back to
  // incoming estvar using estvar_ratio var_H = estvar N_H
...
inline Real NonDNonHierarchSampling::update_hf_target(
    const RealVector& estvar, size_t metric_index, size_t N_H,
    const RealVector& estvar_0) {
  SizetArray N_H_array;
  N_H_array.assign(estvar.length(), N_H);
...
inline bool NonDNonHierarchSampling::ordered_approx_sequence(
    const RealVector& metric, SizetArray& approx_sequence,
    bool descending_keys) {
  size_t i, len = metric.length(), metric_order;
  bool ordered = true;
  ...
  if (descending_keys) {
    ...
    for (i = 0, it = descending_map.begin(); it != descending_map.end();
         ++it, ++i) {
    ...
    for (i = 0, it = ascending_map.begin(); it != ascending_map.end();
         ++it, ++i) {
      approx_sequence[i] = metric_order = it->second;
      if (i != metric_order) ordered = false;
    }
...
inline bool NonDNonHierarchSampling::ordered_approx_sequence(
    const RealMatrix& metric, SizetArray& approx_sequence,
    bool descending_keys) {
...
inline bool NonDNonHierarchSampling::ordered_approx_sequence(
    const RealMatrix& metric, const UShortArray& approx_set) {
  // this checks for metric ordering for each QoI (not averaged QoI)
...
inline void NonDNonHierarchSampling::compute_variance(Real sum_Q, Real sum_QQ,
                                                      size_t num_Q, Real& var_Q)
...
inline void NonDNonHierarchSampling::compute_variance(const RealVector& sum_Q,
                                                      const RealVector& sum_QQ,
                                                      const SizetArray& num_Q,
                                                      RealVector& var_Q) {
...
inline void NonDNonHierarchSampling::compute_correlation(
    Real sum_Q1, Real sum_Q2, Real sum_Q1Q1, Real sum_Q1Q2, Real sum_Q2Q2,
    size_t N_shared, Real& var_Q1, Real& var_Q2, Real& rho2_Q1Q2) {
  if (N_shared <= 1) {
...
inline void NonDNonHierarchSampling::compute_covariance(
    Real sum_Q1, Real sum_Q2, Real sum_Q1Q2, size_t N_shared, Real& cov_Q1Q2) {
  if (N_shared <= 1)
...
inline void NonDNonHierarchSampling::covariance_to_correlation_sq(
    const RealMatrix& cov_LH, const RealSymMatrixArray& cov_LL,
    const RealVector& var_H, RealMatrix& rho2_LH) {
  if (rho2_LH.empty()) rho2_LH.shapeUninitialized(numFunctions, numApprox);

  size_t qoi, approx;
  Real var_H_q, cov_LH_aq;
  for (qoi = 0; qoi < numFunctions; ++qoi) {
    ...
    for (approx = 0; approx < numApprox; ++approx) {
      ...
      rho2_LH(qoi, approx) =
          cov_LH_aq / cov_LL_q(approx, approx) * cov_LH_aq / var_H_q;
    }
  }
}
...
inline void NonDNonHierarchSampling::apply_control(Real sum_L_shared,
                                                   size_t num_L_shared,
                                                   Real sum_L_refined,
                                                   size_t num_L_refined,
                                                   Real beta, Real& H_raw_mom) {
  // apply control for HF uncentered raw moment estimates:
  H_raw_mom -=
      beta * (sum_L_shared / num_L_shared -    // mu from shared samples
              sum_L_refined / num_L_refined);  // refined mu w/ increment

  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "apply_control(): sum_L_sh = " << sum_L_shared
         << " sum_L_ref = " << sum_L_refined << " num_L_sh = " << num_L_shared
...
inline void NonDNonHierarchSampling::inflate(size_t N_0D, SizetArray& N_1D,
  ...
  size_t i, num_approx = approx_set.size();
  for (i = 0; i < num_approx; ++i) N_1D[approx_set[i]] = N_0D;
}

inline void NonDNonHierarchSampling::inflate(const SizetArray& N_1D,
                                             Sizet2DArray& N_2D) {
  N_2D.resize(numApprox);
  for (size_t approx = 0; approx < numApprox; ++approx) N_2D[approx] = N_1D;
}
...
inline void NonDNonHierarchSampling::inflate(const SizetArray& N_1D,
  ...
  N_2D.resize(numApprox);
  size_t i, num_approx = approx_set.size();
  for (i = 0; i < num_approx; ++i) N_2D[approx_set[i]] = N_1D;
...
inline void NonDNonHierarchSampling::inflate(const RealVector& avg_eval_ratios,
                                             RealMatrix& eval_ratios) {
  // inflate avg_eval_ratios back to eval_ratios
  size_t qoi, approx;
  Real r_i, *eval_ratios_a;
  for (approx = 0; approx < numApprox; ++approx) {
    r_i = avg_eval_ratios[approx];
    eval_ratios_a = eval_ratios[approx];
    for (qoi = 0; qoi < numFunctions; ++qoi) eval_ratios_a[qoi] = r_i;
  }
}

inline void NonDNonHierarchSampling::inflate(Real r_i, size_t num_rows,
                                             Real* eval_ratios_col) {
  // inflate scalar to column vector
  for (size_t row = 0; row < num_rows; ++row) eval_ratios_col[row] = r_i;
}

inline void NonDNonHierarchSampling::inflate(const RealVector& vec,
                                             const BitArray& mask,
                                             RealVector& inflated_vec) {
  if (mask.empty())
    ...
    for (i = 0; i < inflated_len; ++i)
      if (mask[i]) inflated_vec[i] = vec[cntr++];
  }
}
...
inline void NonDNonHierarchSampling::deflate(const RealVector& vec,
                                             const BitArray& mask,
                                             RealVector& deflated_vec) {
  if (mask.empty())
    ...
    for (i = 0; i < len; ++i)
      if (mask[i]) deflated_vec[cntr++] = vec[i];
  }
}

inline void NonDNonHierarchSampling::deflate(const SizetArray& vec,
                                             const BitArray& mask,
                                             RealVector& deflated_vec) {
  if (mask.empty())
    copy_data(vec, deflated_vec);
    ...
    deflated_vec.sizeUninitialized(deflate_len);  // init to 0
    for (i = 0; i < len; ++i)
      if (mask[i]) deflated_vec[cntr++] = (Real)vec[i];
  }
}
