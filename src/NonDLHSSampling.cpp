...
static const char rcsId[] =
    "@(#) $Id: NonDLHSSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDLHSSampling::NonDLHSSampling(ProblemDescDB& problem_db,
  ...
  if (sampleType == SUBMETHOD_DEFAULT) sampleType = SUBMETHOD_LHS;

  if (model->primary_fn_type() == GENERIC_FNS)
    numResponseFunctions = model->num_primary_fns();

  if ((vbdFlag == true) && (vbdViaSamplingMethod == VBD_BINNED) &&
  ...
  if (dOptimal) {
    ...
    if (leja) {
      ...
      if (numCandidateDesigns == 0) numCandidateDesigns = 100;
    }
...
NonDLHSSampling::NonDLHSSampling(std::shared_ptr<Model> model,
                                 unsigned short sample_type, int samples,
...
NonDLHSSampling::NonDLHSSampling(unsigned short sample_type, int samples,
...
NonDLHSSampling::NonDLHSSampling(unsigned short sample_type, int samples,
    ...
    : NonDSampling(sample_type, samples, seed, rng, means, std_devs, lower_bnds,
                   upper_bnds, correl),
      numResponseFunctions(0),
      dOptimal(false),
      oversampleRatio(0.0),
...
void NonDLHSSampling::pre_run() {
  ...
  int seq_len = 1;
  if (sample_all_batches) seq_len += refineSamples.length();
  // the user may have fixed the seed; we have to advance it
  if (refineSamples.length() > 0) varyPattern = true;

  IntVector samples_vec(seq_len);
  samples_vec[0] = numSamples;
  if (sample_all_batches) copy_data_partial(refineSamples, samples_vec, 1);
  ...
  if (allSamples.numRows() != num_vars || allSamples.numCols() != total_samples)
    allSamples.shape(num_vars, total_samples);
  IntMatrix all_ranks;
  if (increm_lhs_active) all_ranks.shape(num_vars, total_samples);
  ...
  for (int batch_ind = 0; batch_ind < seq_len; ++batch_ind) {
    ...
    if (increm_lhs_active) {
      ...
      if (batch_ind == 0)
        ...
        increm_lhs_parameter_set(previous_samples, new_samples, allSamples,
                                 all_ranks);
    } else if (dOptimal) {
      ...
      RealMatrix batch_samples(Teuchos::View, allSamples, num_vars,
                               new_samples,           // num row/col
...
void NonDLHSSampling::initial_increm_lhs_set(int new_samples,
                                             RealMatrix& full_samples,
                                             IntMatrix& full_ranks) {
  int num_vars = numContinuousVars + numDiscreteIntVars + numDiscreteRealVars +
                 numDiscreteStringVars;

  // sub-matrix to populate
  RealMatrix batch_samples(Teuchos::View, full_samples, num_vars, new_samples,
                           0, 0);
  ...
  if (sampleType != SUBMETHOD_LOW_DISCREPANCY_SAMPLING) {
    IntMatrix batch_ranks(Teuchos::View, full_ranks, num_vars, new_samples, 0,
                          0);
    store_ranks(batch_samples, batch_ranks);
  }
}

void NonDLHSSampling::increm_lhs_parameter_set(int previous_samples,
                                               int new_samples,
                                               RealMatrix& full_samples,
                                               IntMatrix& full_ranks) {
  ...
  const RealMatrix initial_samples(Teuchos::View, full_samples, num_vars,
                                   previous_samples, 0, 0);
  // new samples to populate, Latin w.r.t. initial samples
  RealMatrix increm_samples(Teuchos::View, full_samples, num_vars, new_samples,
                            0, previous_samples);

  if (sampleType != SUBMETHOD_LOW_DISCREPANCY_SAMPLING) {
    // ranks of the previous_samples are already cached here
    const IntMatrix initial_ranks(Teuchos::View, full_ranks, num_vars,
                                  previous_samples, 0, 0);
    // ranks of the new (increment) samples will be cached here
    IntMatrix increm_ranks(Teuchos::View, full_ranks, num_vars, new_samples, 0,
                           previous_samples);
#ifdef DEBUG
    Cout << "initial_ranks\n" << initial_ranks << '\n';
#endif  // DEBUG
    ...
    for (int v = 0; v < numContinuousVars; ++v) {
      const Pecos::RandomVariable& rv =
          x_ran_vars[svd.cv_index_to_all_index(v)];
    ...
    RealMatrix concat_samples(Teuchos::View, all_samples, num_vars, new_samples,
                              0, previous_samples);
...
void NonDLHSSampling::store_ranks(const RealMatrix& sample_values,
                                  IntMatrix& sample_ranks) {
  ...
  for (size_t v = numContinuousVars; v < num_vars; ++v) {
    ...
    for (size_t s = 0; s < num_samples; ++s) final_rank[rank_col[s]] = s;
    for (size_t s = 0; s < num_samples;
         ++s)  // can't be combined with loop above
...
void NonDLHSSampling::store_ranks(IntMatrix& full_ranks) {
  ...
  IntMatrix combined_ranks(Teuchos::View, full_ranks, num_vars, total_samples,
                           0, 0);
...
void NonDLHSSampling::combine_discrete_ranks(const RealMatrix& initial_values,
  ...
  for (size_t v = numContinuousVars; v < num_vars; ++v) {
    ...
    std::sort(rank_col.begin(), rank_col.end(), rank_sort);
    for (size_t s = 0; s < total_samples; ++s) final_rank[rank_col[s]] = s + 1;
#ifdef DEBUG
    Cout << "final ranks " << final_rank << '\n';
    Cout << "rawData " << rawData << '\n';
#endif
    for (size_t s = 0; s < total_samples;
         ++s)  // can't be combined with loop above
...
void NonDLHSSampling::d_optimal_parameter_set(int previous_samples,
                                              int new_samples,
                                              RealMatrix& full_samples) {
  ...
  RealMatrix initial_samples(Teuchos::View, full_samples, num_vars,
                             previous_samples, 0, 0);
  ...
  RealMatrix selected_samples(Teuchos::View, full_samples, num_vars,
                              total_samples, 0, 0);
  ...
  ProbabilityTransformModel::initialize_distribution_types(
      EXTENDED_U, x_dist.active_variables(), x_dist, u_dist);
  ...
  Pecos::SharedOrthogPolyApproxData::construct_basis(
      u_dist, bc_options, poly_basis, basis_types, colloc_rules);
  ...
  if (leja) {
    ...
    for (size_t vi = 0; vi < num_vars; ++vi) multi_index[vi][vi] = 1;

    // repeatedly generate a parameter set of size new_samples
    // the newly populated points go in this block of full_samples
    RealMatrix curr_new_samples(Teuchos::View, full_samples, num_vars,
                                new_samples, 0, previous_samples);
    ...
    for (int cand_i = 0; cand_i < numCandidateDesigns; ++cand_i) {
      ...
      Pecos::OrthogPolyApproximation::basis_matrix(selected_samples, poly_basis,
                                                   multi_index, basis_matrix);
...
void NonDLHSSampling::post_run(std::ostream& s) {
  ...
  if (statsFlag) {
    if (vbdFlag) {
      nonDSampCorr.compute_vbd_stats_via_sampling(
          vbdViaSamplingMethod, vbdViaSamplingNumBins, numFunctions,
          numContinuousVars + numDiscreteIntVars + numDiscreteRealVars +
              numDiscreteStringVars,
          numSamples, allSamples, allResponses);
      nonDSampCorr.archive_sobol_indices(
          run_identifier(), resultsDB,
      ...
      // iteratively called by print_results. However, when the sampling
      // iterator is a subiterator (e.g. in a nested model), print_results isn't
      // called. Compute stats here for all samples.
  ...
  for (i = 0; i < numFunctions; ++i) {
    switch (finalMomentsType) {
      case Pecos::STANDARD_MOMENTS:
        ...
        if (std::isnan(finalStatErrors(2 * i, 2 * i)) ||
            std::isinf(finalStatErrors(2 * i, 2 * i))) {
          Cerr << "NonDLHSSampling::update_final_statistics() std(mean) is nan "
                  "or inf for qoi = "
               << i << ": " << finalStatErrors(2 * i, 2 * i)
               << ". Reparing to zero.\n";
        ...
        // << "\n";
        finalStatErrors(2 * i + 1, 2 * i + 1) =
            (qoi_stdev == 0)
                ? 0
                : 1. / (2. * qoi_stdev) *
                      std::sqrt(qoi_stdev * qoi_stdev * qoi_stdev * qoi_stdev *
                                (qoi_exckurt / ns + 2. / (ns - 1.)));
        if (std::isnan(finalStatErrors(2 * i + 1, 2 * i + 1)) ||
            std::isinf(finalStatErrors(2 * i + 1, 2 * i + 1))) {
          Cerr << "Values for exckurt = " << qoi_stdev << ", " << qoi_exckurt
               << "\n";
          Cerr << "NonDLHSSampling::update_final_statistics() std(std) is nan "
                  "or inf for qoi = "
               << i << ": " << finalStatErrors(2 * i + 1, 2 * i + 1)
               << ". Reparing to zero.\n";
        ...
        // //COV_BOOTSTRAP finalStatErrors(2*i+1, 2*i) = finalStatErrors(2*i,
        // 2*i)*finalStatErrors(2*i+1, 2*i+1); //COV_PEARSON
        finalStatErrors(2 * i + 1, 2 * i) = qoi_cm3 / ns;  // COV_CORRLIFT
        if (std::isnan(finalStatErrors(2 * i + 1, 2 * i)) ||
            std::isinf(finalStatErrors(2 * i + 1, 2 * i))) {
          Cerr << "Values for cov(mean, std) = " << qoi_skewness << ", "
               << qoi_stdev << ", " << qoi_cm3 << "\n";
          Cerr << "NonDLHSSampling::update_final_statistics() cov(mean, std) "
                  "is nan or inf for qoi = "
               << i << ": " << finalStatErrors(2 * i + 1, 2 * i)
               << ". Reparing to zero.\n";
      ...
      case Pecos::CENTRAL_MOMENTS:
        qoi_var = momentStats(1, i);
        qoi_stdev = std::sqrt(qoi_var);
        ...
        //[fm] Introduction to the Theory of Statistics, Var[Var] = bias
        // correction * 1/N (cm4 - (N-3)/(N-1) cm2^2)
        finalStatErrors(2 * i + 1, 2 * i + 1) =
            std::sqrt((ns - 1.) / (ns * ns - 2. * ns + 3.) *
                      (qoi_cm4 - (ns - 3.) / (ns - 1.) * qoi_var * qoi_var));
        if (outputLevel >= DEBUG_OUTPUT) Cout << "QoICM4 = " << qoi_cm4 << "\n";
        Cout << "QoICM2 = " << qoi_var << "\n";
        Cout << "ns = " << ns << "\n";
        Cout << "Estimator SE for variance = "
             << finalStatErrors(2 * i + 1, 2 * i + 1) << "\n\n";
        break;
    }
...
void NonDLHSSampling::compute_pca(std::ostream& s) {
  ...
  */
  ...
  RealMatrix f_scores(Teuchos::Copy, factor_scores, numSamples, numSamples, 0,
                      0);
  if (outputLevel == DEBUG_OUTPUT) {
    s << "myerr" << myerr << '\n';
    s << "FactorScores\n";
    for (samp = 0; samp < numSamples; ++samp) {
      for (fn = 0; fn < numSamples; ++fn) s << f_scores(samp, fn) << " ";
      s << '\n';
    }
    s << std::endl;
  }
  ...
  for (int j = 0; j < numSamples; j++) {
    ...
    for (int i = 0; i < num_signif_Pcomps; ++i) {
      ...
      if (outputLevel == DEBUG_OUTPUT) s << "pca_coeff " << pca_coeff << '\n';
      // need Row, not column
  ...
  if (outputLevel == DEBUG_OUTPUT) {
    s << "\nPrediction Matrix\n";
    for (samp = 0; samp < numSamples; ++samp) {
      for (fn = 0; fn < numFunctions; ++fn) s << predMatrix(samp, fn) << "  ";
      s << '\n';
    }
  }
}
...
void NonDLHSSampling::print_results(std::ostream& s, short results_state) {
  ...
  if (vbdFlag)
    nonDSampCorr.print_sobol_indices(
    ...
    if (refineSamples.length() == 0) {
      ...
      for (size_t i = 0; i < samples_vec.size(); ++i) {
        ...
        RealMatrix inc_samples(
            Teuchos::View, allSamples,             // block of samples for
            allSamples.numRows(), running_total);  // this increment
        IntResponseMap::iterator end_resp =
            allResponses.find(running_total + start_id);
...
void NonDLHSSampling::archive_results(int num_samples, size_t inc_id) {
  ...
  if (!subIteratorFlag) {
    nonDSampCorr.archive_correlations(
        run_identifier(), resultsDB,
        iteratedModel->current_variables().ordered_labels(ACTIVE_VARS),
  ...
  if (stdRegressionCoeffs) {
    nonDSampCorr.archive_std_regress_coeffs(
        run_identifier(), resultsDB,
        iteratedModel->current_variables().ordered_labels(ACTIVE_VARS),
        ModelUtils::response_labels(*iteratedModel), inc_id);
  }
  ...
  // Associate number of samples attribute with the increment for incremental
  // samplee
  ...
  // Associate number of samples with the execution on the final increment (or
  // when there are no increments). 1 must be added to the length of
  // refineSamples to account for the initial samples
