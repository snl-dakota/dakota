...
int Iterator::get_min_procs_per_iterator()
{
  if (iteratorRep)
    return iteratorRep->get_min_procs_per_iterator();
  else {
    // Define min_procs_per_iterator to accomodate any lower level overrides.
    // With default_config = PUSH_DOWN, this is less critical.
    int ppa = probDescDB.get_int("interface.direct.processors_per_analysis"),
        num_a_serv = probDescDB.get_int("interface.analysis_servers"),
        ppe = probDescDB.get_int("interface.processors_per_evaluation"),
        num_e_serv = probDescDB.get_int("interface.evaluation_servers");
...
int Iterator::
get_max_procs_per_iterator(int max_eval_concurrency)
{
  if (iteratorRep)
    return iteratorRep->get_max_procs_per_iterator(max_eval_concurrency);
  else {
    ...
    if (probDescDB.get_string("interface.type") == "direct")
      return probDescDB.parallel_library().world_size();
	...
	int num_drivers
	  = probDescDB.get_sa("interface.application.analysis_drivers").size();
	short a_sched = probDescDB.get_short("interface.analysis_scheduling");
	if (num_a_serv) {
	  max_procs_per_eval = num_a_serv;
	  int alac = std::max(1, 
	    probDescDB.get_int("interface.asynch_local_analysis_concurrency"));
      ...
      int num_e_serv = probDescDB.get_int("interface.evaluation_servers");
      short e_sched = probDescDB.get_short("interface.evaluation_scheduling");
      if (num_e_serv) {
	max_procs_per_iterator = max_procs_per_eval * num_e_serv;
	int alec = std::max(1, 
	  probDescDB.get_int("interface.asynch_local_evaluation_concurrency"));
	...
	bool peer_dynamic_avail
	  = (probDescDB.get_short("interface.local_evaluation_scheduling") !=
