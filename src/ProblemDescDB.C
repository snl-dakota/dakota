...
static const char rcsId[]="@(#) $Id: ProblemDescDB.C 7007 2010-10-06 15:54:39Z wjbohnh $";


namespace Dakota {

extern ParallelLibrary dummy_lib; // defined in global_defs.C
extern ProblemDescDB *Dak_pddb;	  // defined in global_defs.C
	      ...
	      void (*callback)(void*), void *callback_data)
{
  if (dbRep)
    ...
    if( !contains(startup_path, dakota_input_file, false, path_found) ) {
      bfs::path abs_dak_inp_parent( parent_path(bfs::path(dakota_input_file)) );
...
const Real& ProblemDescDB::get_real(const String& entry_name) const
{
  ...
  if (entry_name.begins("strategy.")) {
	...
	{"proposal_covariance_scale", P proposalCovScale},
