...
const char *GetLongOpt::retrieve(const char *const opt) const {
  ...
  for (t = table; t != 0; t = t->next) {
    if (std::strcmp(opt, t->option) == 0) return t->value;
  }
  Cerr << "GetLongOpt::retrieve - unenrolled option " << optmarker << opt
       << '\n';
  return 0;
}
...
int GetLongOpt::parse(int argc, char *const *argv) {
  ...
  while (argc >= 1) {
    ...
    if (matchStatus == PartialMatch) {
      ...
      Cerr << pname << ": unrecognized option " << optmarker
           << std::strtok(token, "= ") << '\n';
      return -1; /* no match */
    }
...
int GetLongOpt::parse(char *const str, char *const p) {
  ...
  while (token) {
    ...
    if (matchStatus == PartialMatch) {
      ...
      Cerr << name << ": unrecognized option " << optmarker
           << std::strtok(token, "= ") << '\n';
      return -1; /* no match */
    }
...
int GetLongOpt::setcell(Cell *c, char *valtoken, char *nexttoken,
  ...
  switch (c->type) {
    case GetLongOpt::Valueless:
      if (*valtoken == '=') {
        Cerr << name << ": unsolicited value for flag " << optmarker
             << c->option << '\n';
    ...
    case GetLongOpt::MandatoryValue:
      if (*valtoken == '=') {
        ...
        if (nexttoken != 0 && nexttoken[0] != optmarker) {
          ...
          Cerr << name << ": mandatory value for " << optmarker << c->option
               << " not specified\n";
...
void CommandLineHandler::initialize_options() {
  ...
  // I/O options
  enroll("input", GetLongOpt::MandatoryValue, "REQUIRED DAKOTA input file $val",
         NULL);

  enroll("preproc", GetLongOpt::OptionalValue,
         "Pre-process input file with pyprepro or tool $val", NULL);
...
void CommandLineHandler::check_usage(int argc, char **argv) {
  ...
  // Cout << "CommandLineHandler::check_usage: optind = " << optind <<
  // std::endl;
  ...
  if (retrieve("read_restart") == NULL && retrieve("stop_restart") != NULL) {
    usage();
    output_helper("\nread_restart is REQUIRED for use with stop_restart.",
                  Cerr);
    abort_handler(-1);
  }
...
void CommandLineHandler::usage(std::ostream &outfile) const {
  if (worldRank == 0) GetLongOpt::usage(outfile);
}

/** When there is a valid ParallelLibrary, output only on rank 0 */
void CommandLineHandler::output_helper(const std::string &message,
