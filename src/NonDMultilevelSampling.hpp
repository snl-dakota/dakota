...
class NonDMultilevelSampling : public virtual NonDHierarchSampling {
 ...
 protected:
  ...
  void ml_Q_raw_moments(const RealMatrix& sum_Hl_1,
                        const RealMatrix& sum_Hlm1_1,
                        const RealMatrix& sum_Hl_2,
                        const RealMatrix& sum_Hlm1_2,
  ...
  void configure_indices(unsigned short group, unsigned short form, size_t lev,
                         short seq_type);
 ...
 private:
  ...
  static Real variance_Ysum_static(Real sum_Y, Real sum_YY, /*Real offset,*/
                                   size_t Nlq_pilot, size_t Nlq,
                                   bool compute_gradient, Real& grad);

  /// compute variance from sum accumulators
  /// necessary for sample allocation optimization
  static Real variance_Qsum_static(Real sum_Ql, Real sum_Qlm1, Real sum_QlQl,
                                   Real sum_QlQlm1, Real sum_Qlm1Qlm1,
                                   size_t Nlq_pilot, size_t Nlq,
                                   bool compute_gradient, Real& grad);

  Real var_lev_l(Real sum_Ql, Real sum_Qlm1, Real sum_QlQl, Real sum_Qlm1Qlm1,
                 size_t Nlq);
  static Real var_lev_l_static(Real sum_Ql, Real sum_Qlm1, Real sum_QlQl,
                               Real sum_Qlm1Qlm1, size_t Nlq_pilot, size_t Nlq,
                               bool compute_gradient, Real& grad);
  ...
  Real variance_mean_Qsum(const IntRealMatrixMap& sum_Ql,
                          const IntRealMatrixMap& sum_Qlm1,
                          const IntIntPairRealMatrixMap& sum_QlQlm1,
  ...
  Real aggregate_variance_Qsum(const Real* sum_Ql, const Real* sum_Qlm1,
                               ...
                               const size_t lev);
  /// evaluate variance for given level and QoI (using sum_YY with means from
  /// sum_Y)
  ...
  Real variance_variance_Qsum(const IntRealMatrixMap& sum_Ql,
                              const IntRealMatrixMap& sum_Qlm1,
                              const IntIntPairRealMatrixMap& sum_QlQlm1,
                              const Sizet2DArray& N_l, const size_t step,
                              const size_t qoi);
  ...
  Real variance_sigma_Qsum(const IntRealMatrixMap& sum_Ql,
                           const IntRealMatrixMap& sum_Qlm1,
                           const IntIntPairRealMatrixMap& sum_QlQlm1,
                           const Sizet2DArray& N_l, const size_t step,
                           const size_t qoi);
  ...
  static Real compute_bootstrap_covariance(
  ...
  static Real compute_cov_mean_sigma(const IntRealMatrixMap& sum_Ql,
                                     const IntRealMatrixMap& sum_Qlm1,
                                     const IntIntPairRealMatrixMap& sum_QlQlm1,
                                     const size_t Nlq_pilot, const Real Nlq,
                                     const size_t qoi, const size_t lev,
                                     const bool compute_gradient, Real& grad_g);

  static RealVector compute_cov_mean_sigma_fd(
      const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
      const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
      const Real Nlq, const size_t qoi, const size_t lev);
  ...
  static Real compute_cov(const RealVector& samples_X,
                          const RealVector& samples_hat);
  ...
  /// compute epsilon^2/2 term for each qoi based on reference estimator_var0
  /// and relative convergence tolereance
  void set_convergence_tol(const RealVector& estimator_var0_qoi,
                           const RealVector& cost,
                           RealVector& eps_sq_div_2_qoi);
  ...
  /// compute sample allocation delta based on current samples and based on
  /// allocation target. Single allocation target for each qoi, aggregated using
  /// max operation.
  void compute_sample_allocation_target(
      const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
      const IntIntPairRealMatrixMap& sum_QlQlm1,
  ...
  static Real unbiased_mean_product_triplet(
      const Real sumQ1, const Real sumQ2, const Real sumQ3, const Real sumQ1Q2,
      const Real sumQ1Q3, const Real sumQ2Q3, const Real sumQ1Q2Q3,
      const size_t Nlq);
  /// compute the unbiased product of two pairs of products of sampling means
  static Real unbiased_mean_product_pairpair(
      const Real sumQ1, const Real sumQ2, const Real sumQ1Q2,
      const Real sumQ1sq, const Real sumQ2sq, const Real sumQ1sqQ2,
      const Real sumQ1Q2sq, const Real sumQ1sqQ2sq, const size_t Nlq);

  static Real var_of_var_ml_l0(const IntRealMatrixMap& sum_Ql,
                               const IntRealMatrixMap& sum_Qlm1,
                               const IntIntPairRealMatrixMap& sum_QlQlm1,
                               const size_t Nlq_pilot, const Real Nlq,
                               const size_t qoi, const bool compute_gradient,
                               Real& grad_g);
  ...
  static Real compute_cov_meanl_varlmone(
      const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
      const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
      const Real Nlq, const size_t qoi, const size_t lev,
      const bool compute_gradient, Real& grad_g);

  static Real compute_cov_meanlmone_varl(
      const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
      const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
      const Real Nlq, const size_t qoi, const size_t lev,
      const bool compute_gradient, Real& grad_g);

  static Real compute_grad_cov_meanl_vark(
      const Real cov_mean_var, const Real var_of_var, const Real var_of_sigma,
      const Real grad_var_of_var, const Real grad_var_of_sigma, const Real Nlq);

  /// OPTPP definition
  static void target_cost_objective_eval_optpp(int mode, int n,
                                               const RealVector& x, double& f,
                                               RealVector& grad_f,
                                               int& result_mode);
  ...
  static void target_scalarization_constraint_eval_optpp(int mode, int n,
                                                         ...
                                                         RealMatrix& grad_g,
                                                         int& result_mode);
  static void target_scalarization_constraint_eval_logscale_optpp(
  ...
  static void target_scalarization_objective_eval_optpp(int mode, int n,
                                                        ...
                                                        RealVector& grad_f,
                                                        int& result_mode);
  static void target_scalarization_objective_eval_logscale_optpp(
      int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
      int& result_mode);

  static void target_scalarization_objective_eval_optpp_fd(int mode, int n,
                                                           const RealVector& x,
                                                           double& f,
                                                           int& result_mode);
  ...
  static void target_var_constraint_eval_npsol(int& mode, int& m, int& n,
                                               ...
                                               double* g, double* grad_g,
                                               int& nstate);
  static void target_var_constraint_eval_logscale_npsol(
  ...
  static void target_sigma_constraint_eval_npsol(int& mode, int& m, int& n,
                                                 ...
                                                 double* x, double* g,
                                                 double* grad_g, int& nstate);
  static void target_sigma_constraint_eval_logscale_npsol(
      int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
      double* grad_g, int& nstate);

  static void target_scalarization_constraint_eval_npsol(
      int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
      double* grad_g, int& nstate);
  static void target_scalarization_constraint_eval_logscale_npsol(
      int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
      double* grad_g, int& nstate);
  ...
  void assign_static_member_problem18(Real& var_L_exact, Real& var_H_exact,
                                      Real& mu_four_L_exact,
                                      Real& mu_four_H_exact, Real& Ax,
                                      RealVector& level_cost_vec) const;
...
inline void NonDMultilevelSampling::configure_indices(size_t group, size_t form,
                                                      size_t lev,
                                                      short seq_type) {
...
inline Real NonDMultilevelSampling::level_cost(const RealVector& cost,
                                               size_t step, size_t offset) {
  // discrepancies incur two level costs
  size_t offset_step = offset + step;
  return (step) ? cost[offset_step] + cost[offset_step - 1]
                :                // aggregated {HF,LF} mode
...
inline Real NonDMultilevelSampling::estimator_accuracy_metric() const {
  ...
  switch (estVarMetricType) {
    ...
    case AVG_ESTVAR_METRIC:
      return average(estVar);
      break;
    case NORM_ESTVAR_METRIC:
      return p_norm(estVar, estVarMetricNormOrder);
      break;
    case MAX_ESTVAR_METRIC:
      return maximum(estVar);
      break;
    default:
...
inline void NonDMultilevelSampling::nested_response_mappings(
    const RealMatrix& primary_coeffs, const RealMatrix& secondary_coeffs) {
  if (scalarizationCoeffs
          .empty()) {  // && allocationTarget == TARGET_SCALARIZATION){
    if (primary_coeffs.empty()) {
      Cerr << "\nPrimary_response_mapping should not be empty at this point. "
              "If you are sure this is correct, "
           << "you need to specify scalarization_response_mapping seperately "
              "in multilevel_sampling."
           << std::endl;
      abort_handler(METHOD_ERROR);
    }
    if (primary_coeffs.numCols() != 2 * numFunctions ||
      ...
      Cerr
          << "\nWrong size for primary_response_mapping. If you are sure, it "
             "is the right size, e.g.,"
          << " you are interested in quantiles, you need to specify "
             "scalarization_response_mapping seperately in multilevel_sampling."
          << std::endl;
      abort_handler(METHOD_ERROR);
    }
    if (!secondary_coeffs.empty()) {
      if (secondary_coeffs.numCols() != 2 * numFunctions) {
        Cerr << "\nWrong size for columns of secondary_response_mapping. If "
                "you are sure, it is the right size, e.g.,"
             << " you are interested in quantiles, you need to specify "
                "scalarization_response_mapping seperately in "
                "multilevel_sampling."
             << std::endl;
        abort_handler(METHOD_ERROR);
      }
    ...
    if (nb_rows > numFunctions) {
      Cerr << "\nWrong size for rows of response_mapping. If you are sure, it "
              "is the right size, e.g.,"
           << ", you need to specify scalarization_response_mapping seperately "
              "in multilevel_sampling."
           << std::endl;
    ...
    for (size_t row_qoi = 0; row_qoi < numFunctions; ++row_qoi) {
      ...
      scalarizationCoeffs(0, row_qoi * 2 + 1) =
          primary_coeffs(0, row_qoi * 2 + 1);
    }
    ...
    for (size_t qoi = nb_rows; qoi < numFunctions; ++qoi) {
      for (size_t row_qoi = 0; row_qoi < numFunctions; ++row_qoi) {
        scalarizationCoeffs(qoi, row_qoi * 2) = 0.;
        scalarizationCoeffs(qoi, row_qoi * 2 + 1) = 0.;
      }
    }
  }
}

inline Real NonDMultilevelSampling::variance_Ysum(Real sum_Y, Real sum_YY,
                                                  /*Real offset,*/ size_t Nlq) {
...
inline void NonDMultilevelSampling::variance_Ysum(const Real* sum_Y,
                                                  const Real* sum_YY,
                                                  const SizetArray& N_l,
                                                  Real* var_Y) {
...
inline Real NonDMultilevelSampling::variance_Ysum_static(
    Real sum_Y, Real sum_YY, /*Real offset,*/ size_t Nlq_pilot, size_t Nlq,
    bool compute_gradient, Real& grad) {
  ...
  Real var_Y =
      variance_tmp * (Real)Nlq / (Real)(Nlq - 1);  // Bessel's correction
  if (var_Y < 0) {
    Cerr << "NonDMultilevelSampling::variance_Ysum_static: var_Y < 0"
         << std::endl;
  }
  check_negative(var_Y);
  return var_Y;
...
inline Real NonDMultilevelSampling::variance_Qsum(Real sum_Ql, Real sum_Qlm1,
                                                  Real sum_QlQl,
                                                  Real sum_QlQlm1,
                                                  Real sum_Qlm1Qlm1,
                                                  size_t Nlq) {
...
inline Real NonDMultilevelSampling::variance_Qsum_static(
    Real sum_Ql, Real sum_Qlm1, Real sum_QlQl, Real sum_QlQlm1,
  ...
  // var_Y = var_Ql - 2.* covar_QlQlm1 + var_Qlm1;
  Real variance_tmp =
      (sum_QlQl / Nlq_pilot - mu_Ql * mu_Ql               // var_Ql
       - 2. * (sum_QlQlm1 / Nlq_pilot - mu_Ql * mu_Qlm1)  // covar_QlQlm1
       + sum_Qlm1Qlm1 / Nlq_pilot - mu_Qlm1 * mu_Qlm1);   // var_Qlm1

  if (compute_gradient) {
    grad = variance_tmp * (-1.) /
           ((Real)(Nlq_pilot - 1.) * (Real)(Nlq_pilot - 1.));
    grad = 0;  // TODO_SCALARBUGFIX
  }
  if (variance_tmp < 0) {
    Cerr << "NonDMultilevelSampling::variance_Qsum: variance_tmp < 0"
         << std::endl;
  }
...
inline Real NonDMultilevelSampling::var_lev_l(Real sum_Ql, Real sum_Qlm1,
                                              Real sum_QlQl, Real sum_Qlm1Qlm1,
                                              size_t Nlq) {
  Real mu_Ql = sum_Ql / Nlq, mu_Qlm1 = sum_Qlm1 / Nlq;
  ...
  // if(var_lev_Q < 0){
  //   Cerr << "NonDMultilevelSampling::var_lev_l: var_lev_Q < 0: " << var_lev_Q
...
inline Real NonDMultilevelSampling::var_lev_l_static(
    Real sum_Ql, Real sum_Qlm1, Real sum_QlQl, Real sum_Qlm1Qlm1,
    size_t Nlq_pilot, size_t Nlq, bool compute_gradient, Real& grad) {
  Real mu_Ql = sum_Ql / Nlq_pilot, mu_Qlm1 = sum_Qlm1 / Nlq_pilot;
  // var_Y = var_Ql - 2.* covar_QlQlm1 + var_Qlm1;
  // Cerr << "1NonDMultilevelSampling::var_lev_l_static: " << sum_QlQl << ", "
  // << Nlq_pilot << ", " << mu_Ql << ", " <<" \n"; Cerr <<
  // "1NonDMultilevelSampling::var_lev_l_static: " << sum_Qlm1Qlm1 << ", " <<
  // Nlq_pilot << ", " << mu_Qlm1 << ", " <<" \n";
  Real var_lev_Q = (sum_QlQl / Nlq_pilot - mu_Ql * mu_Ql)  // var_Ql
                   -
                   (sum_Qlm1Qlm1 / Nlq_pilot - mu_Qlm1 * mu_Qlm1);  // var_Qlm1
  // sCerr << "2NonDMultilevelSampling::var_lev_l_static: " << var_lev_Q << ", "
  // << Nlq << " But that should be fine.\n";
  if (compute_gradient) {
  ...
  // if(var_lev_Q < 0){
  //   Cerr << "NonDMultilevelSampling::var_lev_l_static: var_lev_Q < 0: " <<
  //   var_lev_Q << " But that should be fine.\n";
  // check_negative(var_of_var);
  //}
  return var_lev_Q;
}

inline Real NonDMultilevelSampling::aggregate_variance_Ysum(
    const Real* sum_Y, const Real* sum_YY, const SizetArray& N_l) {
...
inline Real NonDMultilevelSampling::aggregate_variance_Qsum(
    const Real* sum_Ql, const Real* sum_Qlm1, const Real* sum_QlQl,
    const Real* sum_QlQlm1, const Real* sum_Qlm1Qlm1, const SizetArray& N_l,
    const size_t lev) {
  Real agg_var_l = 0.;  //, var_Y;
  for (size_t qoi = 0; qoi < numFunctions; ++qoi)
...
inline Real NonDMultilevelSampling::variance_Qsum(
    const Real* sum_Ql, const Real* sum_Qlm1, const Real* sum_QlQl,
    const Real* sum_QlQlm1, const Real* sum_Qlm1Qlm1, const SizetArray& N_l,
    const size_t lev, const size_t qoi) {
  Real var_l = (lev)
                   ? variance_Qsum(sum_Ql[qoi], sum_Qlm1[qoi], sum_QlQl[qoi],
                                   sum_QlQlm1[qoi], sum_Qlm1Qlm1[qoi], N_l[qoi])
                   : variance_Ysum(sum_Ql[qoi], sum_QlQl[qoi], N_l[qoi]);
  return var_l;
}

inline void NonDMultilevelSampling::variance_Qsum(
    const Real* sum_Ql, const Real* sum_Qlm1, const Real* sum_QlQl,
    const Real* sum_QlQlm1, const Real* sum_Qlm1Qlm1, const SizetArray& N_l,
    const size_t lev, Real* var_Yl) {
  if (lev)
...
inline Real NonDMultilevelSampling::aggregate_mse_Yvar(const Real* var_Y,
                                                       const SizetArray& N_l) {
  Real agg_mse = 0.;
...
inline Real NonDMultilevelSampling::aggregate_mse_Ysum(const Real* sum_Y,
                                                       const Real* sum_YY,
                                                       const SizetArray& N_l) {
...
inline void NonDMultilevelSampling::aggregate_mse_target_Qsum(
    RealMatrix& agg_var_qoi, const Sizet2DArray& N_l, const size_t step,
    RealVector& estimator_var0_qoi) {
  for (size_t qoi = 0; qoi < numFunctions; ++qoi)
    estimator_var0_qoi[qoi] += agg_var_qoi(qoi, step) / N_l[step][qoi];
}

inline void NonDMultilevelSampling::set_convergence_tol(
    const RealVector& estimator_var0_qoi, const RealVector& cost,
    RealVector& eps_sq_div_2_qoi) {
  for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
    switch (convergenceTolTarget) {
      case VARIANCE_CONSTRAINT_TARGET:
        ...
        switch (convergenceTolType) {
          case RELATIVE_CONVERGENCE_TOLERANCE:
            eps_sq_div_2_qoi[qoi] =
                estimator_var0_qoi[qoi] * convergenceTolVec[qoi];
            break;
          ...
          case ABSOLUTE_CONVERGENCE_TOLERANCE:
            ...
            break;
          default:
            Cerr << "NonDMultilevelSampling::set_convergence_tol: "
                 << "convergenceTolType is not known." << std::endl;
            abort_handler(METHOD_ERROR);
            break;
        }
        break;
      ...
      case COST_CONSTRAINT_TARGET:
        // Relative cost with respect to convergenceTol evaluations on finest
        // grid
        switch (convergenceTolType) {
          case RELATIVE_CONVERGENCE_TOLERANCE:
            eps_sq_div_2_qoi[qoi] =
                cost[cost.length() - 1] * convergenceTolVec[qoi];
            break;
      ...
      default:
        Cerr << "NonDMultilevelSampling::set_convergence_tol: "
             << "convergenceTolTarget is not known." << std::endl;
        abort_handler(METHOD_ERROR);
        break;
    }
  }
  if (outputLevel == DEBUG_OUTPUT)
    Cout << "Epsilon squared target per QoI = " << eps_sq_div_2_qoi
         << std::endl;
}

inline void NonDMultilevelSampling::ml_Q_raw_moments(
    const RealMatrix& sum_Hl_1, const RealMatrix& sum_Hlm1_1,
...
inline void NonDMultilevelSampling::ml_Y_raw_moments(const RealMatrix& sum_H1,
                                                     const RealMatrix& sum_H2,
                                                     const Sizet2DArray& N_l,
...
inline void NonDMultilevelSampling::ml_Y_raw_moments(
    const RealMatrix& sum_H1, const RealMatrix& sum_H2,
    const RealMatrix& sum_H3, const RealMatrix& sum_H4, const Sizet2DArray& N_l,
    size_t start, size_t end, RealMatrix& ml_raw_mom) {
  // MLMC without CV: sum_H = HF Q sums for lev 0 and HF Y sums for lev > 0
...
inline void NonDMultilevelSampling::compute_ml_estimator_variance(
    const RealMatrix& var_Y, const Sizet2DArray& num_Y,
    RealVector& ml_est_var) {
...
inline void NonDMultilevelSampling::update_projected_samples(
    const SizetArray& delta_N_l,  // Sizet2DArray& N_actual,
    SizetArray& N_alloc, const RealVector& cost, Real& delta_equiv_hf) {
  size_t actual_incr, alloc_incr, offline_N_lwr = 0, lev,
                                  num_lev = cost.length();
...
inline void NonDMultilevelSampling::increment_ml_equivalent_cost(
    size_t new_N_l, Real lev_cost, Real ref_cost, Real& equiv_hf) {
...
inline Real NonDMultilevelSampling::compute_ml_equivalent_cost(
    const SizetArray& raw_N_l, const RealVector& cost) {
  size_t step, num_steps = raw_N_l.size();
  Real equiv_hf = 0.;
  equiv_hf = raw_N_l[0] * cost[0];  // first level is single eval
  for (step = 1; step < num_steps;
       ++step)  // subsequent levels incur 2 model costs
    equiv_hf += raw_N_l[step] * (cost[step] + cost[step - 1]);
  equiv_hf /= cost[num_steps - 1];  // normalize into equivalent HF evals
  return equiv_hf;
}

inline void NonDMultilevelSampling::recover_variance(
    const RealMatrix& moment_stats, RealVector& var_H) {
...
inline Real NonDMultilevelSampling::var_of_var_ml_l0(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const bool compute_gradient,
    Real& grad_g) {
  Real cm1l, cm2l, cm3l, cm4l, cm2l_sq, var_of_var;

  IntIntPair pr11(1, 1), pr12(1, 2), pr21(2, 1), pr22(2, 2);
  const RealMatrix &sum_Q1l = sum_Ql.at(1),
                   &sum_Q1lm1 = sum_Qlm1.at(1),  //.at() returns a const member
  ...
  uncentered_to_centered(
      sum_Q1l(qoi, 0) / Nlq_pilot, sum_Q2l(qoi, 0) / Nlq_pilot,
      sum_Q3l(qoi, 0) / Nlq_pilot, sum_Q4l(qoi, 0) / Nlq_pilot, cm1l, cm2l,
      cm3l, cm4l, Nlq_pilot);

  cm2l_sq = cm2l * cm2l;
  var_of_var = (Nlq - 1.) / (Nlq * Nlq - 2. * Nlq + 3.) *
               (cm4l - (Nlq - 3.) / (Nlq - 1.) * cm2l_sq);

  if (compute_gradient) {
    grad_g = ((Nlq * Nlq - 2. * Nlq + 3.) - (Nlq - 1.) * (2. * Nlq - 2.)) /
                 ((Nlq * Nlq - 2. * Nlq + 3.) * (Nlq * Nlq - 2. * Nlq + 3.)) *
                 cm4l -
  ...
  // Cout << "NonDMultilevelSampling::var_of_var_ml_l0: (Qoi, 0): " << qoi << ",
  // 0" << ") Var[Var]: " << var_of_var << std::endl;
  if (var_of_var < 0) {
    Cerr << "NonDMultilevelSampling::var_of_var_ml_l0(qoi = " << qoi
         << "): var_of_var < 0.";
    check_negative(var_of_var);
  }
  return var_of_var;
...
inline Real NonDMultilevelSampling::var_of_var_ml_lmax(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const bool compute_gradient,
    Real& grad_g) {
  ...
  uncentered_to_centered(
      sum_Q1l(qoi, 1) / Nlq_pilot, sum_Q2l(qoi, 1) / Nlq_pilot,
      sum_Q3l(qoi, 1) / Nlq_pilot, sum_Q4l(qoi, 1) / Nlq_pilot, cm1l, cm2l,
      cm3l, cm4l, Nlq_pilot);
  ...
  if (var_of_var < 0) {
    Cerr << "NonDMultilevelSampling::var_of_var_ml_lmax(qoi = " << qoi
         << "): var_of_var < 0.";
    check_negative(var_of_var);
  }
  return var_of_var;
...
inline Real NonDMultilevelSampling::var_of_var_ml_l(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const size_t lev,
    const bool compute_gradient, Real& grad_g) {
  Real cm1l, cm2l, cm3l, cm4l, cm1lm1, cm2lm1, cm3lm1, cm4lm1, cm1l_sq, cm2l_sq,
      cm2lm1_sq, mu_Q2l, mu_Q2lm1, mu_Q2lQ2lm1, mu_Q1lm1_mu_Q2lQ1lm1,
      ...
      covar_P2lP2lm1, term, var_of_var;
  ...
  uncentered_to_centered(
      sum_Q1l(qoi, lev) / Nlq_pilot, mu_Q2l, sum_Q3l(qoi, lev) / Nlq_pilot,
      sum_Q4l(qoi, lev) / Nlq_pilot, cm1l, cm2l, cm3l, cm4l, Nlq_pilot);
  mu_Q2lm1 = sum_Q2lm1(qoi, lev) / Nlq_pilot;
  uncentered_to_centered(sum_Q1lm1(qoi, lev) / Nlq_pilot, mu_Q2lm1,
                         sum_Q3lm1(qoi, lev) / Nlq_pilot,
                         sum_Q4lm1(qoi, lev) / Nlq_pilot, cm1lm1, cm2lm1,
                         cm3lm1, cm4lm1, Nlq_pilot);
  cm2l_sq = cm2l * cm2l;
  cm2lm1_sq = cm2lm1 * cm2lm1;

  // [fm] bias correction for var_P2l and var_P2lm1
  var_P2l = (Nlq - 1.) / (Nlq * Nlq - 2. * Nlq + 3.) *
            (cm4l - (Nlq - 3.) / (Nlq - 1.) * cm2l_sq);
  var_P2lm1 = (Nlq - 1.) / (Nlq * Nlq - 2. * Nlq + 3.) *
              (cm4lm1 - (Nlq - 3.) / (Nlq - 1.) * cm2lm1_sq);

  //[fm] unbiased products of mean
  mu_Q2lQ2lm1 = sum_Q2lQ2lm1(qoi, lev) / Nlq_pilot;
  mu_Q1lm1_mu_Q2lQ1lm1 =
      unbiased_mean_product_pair(sum_Q1lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev),
                                 sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  mu_Q1lm1_mu_Q1lm1_muQ2l = unbiased_mean_product_triplet(
      sum_Q1lm1(qoi, lev), sum_Q1lm1(qoi, lev), sum_Q2l(qoi, lev),
      sum_Q2lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev),
      sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  mu_Q1l_mu_Q1lQ2lm1 =
      unbiased_mean_product_pair(sum_Q1l(qoi, lev), sum_Q1lQ2lm1(qoi, lev),
                                 sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  mu_Q1l_mu_Q1l_mu_Q2lm1 = unbiased_mean_product_triplet(
      sum_Q1l(qoi, lev), sum_Q1l(qoi, lev), sum_Q2lm1(qoi, lev),
      sum_Q2l(qoi, lev), sum_Q1lQ2lm1(qoi, lev), sum_Q1lQ2lm1(qoi, lev),
      sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  mu_Q1l_mu_Qlm1_mu_Q1lQ1lm1 = unbiased_mean_product_triplet(
      sum_Q1l(qoi, lev), sum_Q1lm1(qoi, lev), sum_Q1lQ1lm1(qoi, lev),
      sum_Q1lQ1lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev), sum_Q1lQ2lm1(qoi, lev),
      sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  mu_Q1l_mu_Q1l_mu_Q1lm1_muQ1lm1 = unbiased_mean_product_pairpair(
      sum_Q1l(qoi, lev), sum_Q1lm1(qoi, lev), sum_Q1lQ1lm1(qoi, lev),
      sum_Q2l(qoi, lev), sum_Q2lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev),
      sum_Q1lQ2lm1(qoi, lev), sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  mu_Q2l_muQ2lm1 =
      unbiased_mean_product_pair(sum_Q2l(qoi, lev), sum_Q2lm1(qoi, lev),
                                 sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  ...
  mu_Q1lQ1lm1_mu_Q1lQ1lm1 =
      unbiased_mean_product_pair(sum_Q1lQ1lm1(qoi, lev), sum_Q1lQ1lm1(qoi, lev),
                                 sum_Q2lQ2lm1(qoi, lev), Nlq_pilot);
  term = mu_Q1lQ1lm1_mu_Q1lQ1lm1 - 2. * mu_Q1l_mu_Qlm1_mu_Q1lQ1lm1 +
         mu_Q1l_mu_Q1l_mu_Q1lm1_muQ1lm1;

  //[fm] Using only unbiased estimators the sum is also unbiased
  covar_P2lP2lm1 = (mu_P2lP2lm1 + term / (Nlq - 1.)) / Nlq;
  ...
  if (compute_gradient) {
    grad_g =
        ...
        ((Nlq * Nlq - 2. * Nlq + 3.) - (Nlq - 3.) * (2. * Nlq - 2.)) /
            ...
            cm2lm1_sq -
        2. *
            (-1. / (Nlq * Nlq) * mu_P2lP2lm1 +
             (-2. * Nlq + 1.) / ((Nlq * Nlq - Nlq) * (Nlq * Nlq - Nlq)) * term);
  }

  // Cout << "NonDMultilevelSampling::var_of_var_ml_l: (Qoi, lev): " << qoi <<
  // ", " << lev << ") Var[Var]: " << var_of_var << std::endl;
  if (var_of_var < 0) {
    Cerr << "NonDMultilevelSampling::var_of_var_ml_l(qoi, lev) = (" << qoi
         << ", " << lev << "): var_of_var < 0.";
    check_negative(var_of_var);
  }
  return var_of_var;
...
inline Real NonDMultilevelSampling::compute_cov_mean_sigma(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const size_t lev,
    const bool compute_gradient, Real& grad_g) {
  ...
  Real cov_meanl_varl = 0, cov_meanlm1_varlm1 = 0, cov_meanl_varlm1 = 0,
       cov_meanlm1_varl = 0;
  Real cov_meanl_sigmal = 0, cov_meanlm1_sigmalm1 = 0, cov_meanl_sigmalm1 = 0,
       cov_meanlm1_sigmal = 0, cov_mean_sigma = 0;
  ...
  var_var_l = (Nlq - 1.) / (Nlq * Nlq - 2. * Nlq + 3.) *
              (cm4l - (Nlq - 3.) / (Nlq - 1.) * cm2l_sq);
  if (compute_gradient) {
    grad_var_var_l =
        ((Nlq * Nlq - 2. * Nlq + 3.) - (Nlq - 1.) * (2. * Nlq - 2.)) /
  ...
  if (lev > 0) {
    ...
    var_var_lm1 = (Nlq - 1.) / (Nlq * Nlq - 2. * Nlq + 3.) *
                  (cm4lm1 - (Nlq - 3.) / (Nlq - 1.) * cm2lm1_sq);
    if (compute_gradient) {
      grad_var_var_lm1 =
          ((Nlq * Nlq - 2. * Nlq + 3.) - (Nlq - 1.) * (2. * Nlq - 2.)) /
  ...
  if (lev > 0) {
    ...
    if (compute_gradient) {
      grad_var_sigma_lm1 =
          var_var_lm1 > 0 ? 1. / (4. * cm2lm1) * grad_var_var_lm1 : 0;
    }
  }
  ...
  if (lev > 0) {
    ...
    cov_meanlm1_varl = compute_cov_meanlmone_varl(
        sum_Ql, sum_Qlm1, sum_QlQlm1, Nlq_pilot, Nlq, qoi, lev,
        compute_gradient, grad_cov_meanlm1_varl);
    cov_meanl_varlm1 = compute_cov_meanl_varlmone(
        sum_Ql, sum_Qlm1, sum_QlQlm1, Nlq_pilot, Nlq, qoi, lev,
        compute_gradient, grad_cov_meanl_varlm1);
    cov_meanlm1_varlm1 = cm3lm1 / Nlq;
  }

  // Compute cov_mean_var to cov_mean_sigma transformation using the assumption
  // that pearson correlation is the same for both, i.e. p[mean, sigma] =
  // p[mean, var]
  cov_meanl_sigmal =
      var_var_l > 0 ? cov_meanl_varl * std::sqrt(var_sigma_l / var_var_l) : 0;
  if (lev > 0) {
    cov_meanlm1_sigmal =
        var_var_l > 0 ? cov_meanlm1_varl * std::sqrt(var_sigma_l / var_var_l)
                      : 0;
    cov_meanl_sigmalm1 =
        var_var_lm1 > 0
            ? cov_meanl_varlm1 * std::sqrt(var_sigma_lm1 / var_var_lm1)
            : 0;
    cov_meanlm1_sigmalm1 =
        var_var_lm1 > 0
            ? cov_meanlm1_varlm1 * std::sqrt(var_sigma_lm1 / var_var_lm1)
            : 0;
  }

  if (compute_gradient) {
    grad_cov_meanl_varl = var_var_l > 0 && var_sigma_l > 0
                              ? compute_grad_cov_meanl_vark(
                                    cov_meanl_varl, var_var_l, var_sigma_l,
                                    grad_var_var_l, grad_var_sigma_l, Nlq)
                              : 0;
    if (lev > 0) {
      grad_cov_meanlm1_varl =
          var_var_l > 0 && var_sigma_l > 0
              ? compute_grad_cov_meanl_vark(cov_meanlm1_varl, var_var_l,
                                            var_sigma_l, grad_var_var_l,
                                            grad_var_sigma_l, Nlq)
              : 0;
      grad_cov_meanl_varlm1 =
          var_var_lm1 > 0 && var_sigma_lm1 > 0
              ? compute_grad_cov_meanl_vark(cov_meanl_varlm1, var_var_lm1,
                                            var_sigma_lm1, grad_var_var_lm1,
                                            grad_var_sigma_lm1, Nlq)
              : 0;
      grad_cov_meanlm1_varlm1 =
          var_var_lm1 > 0 && var_sigma_lm1 > 0
              ? compute_grad_cov_meanl_vark(cov_meanlm1_varlm1, var_var_lm1,
                                            var_sigma_lm1, grad_var_var_lm1,
                                            grad_var_sigma_lm1, Nlq)
              : 0;
    }
  ...
  if (lev > 0) {
    cov_mean_sigma +=
        cov_meanlm1_sigmalm1 - cov_meanlm1_sigmal - cov_meanl_sigmalm1;
  }

  if (compute_gradient) {
    grad_g = grad_cov_meanl_varl;
    if (lev > 0) {
      grad_g += grad_cov_meanlm1_varlm1 - grad_cov_meanlm1_varl -
                grad_cov_meanl_varlm1;
    }
  }
  ...
  /*if(compute_gradient){
    Real h = 1e-8;
    RealVector cov_mean_sigma_fd_plus = compute_cov_mean_sigma_fd(sum_Ql,
  sum_Qlm1, sum_QlQlm1, Nlq_pilot, Nlq+h, qoi, lev); RealVector
  cov_mean_sigma_fd_minus = compute_cov_mean_sigma_fd(sum_Ql, sum_Qlm1,
                    sum_QlQlm1, Nlq_pilot, Nlq-h, qoi, lev);

    Cout << "QOI LEV: " << qoi << ", " << lev << std::endl;
    Cout << "\t\t\t var_var_l: " << var_var_l << ", var_var_lm1: " <<
  var_var_lm1 << std::endl; Cout << "\t\t\t var_sigma_l: " << var_sigma_l << ",
  var_sigma_lm1: " << var_sigma_lm1 << std::endl; Cout << "\t\t\t
  grad_cov_mean_sigma: " << grad_g << ", " << (cov_mean_sigma_fd_plus[0] -
  cov_mean_sigma_fd_minus[0])/(2*h) << std::endl; Cout << "\t\t\t
  grad_cov_meanl_sigmal: " << grad_cov_meanl_varl << ", " <<
  (cov_mean_sigma_fd_plus[1] - cov_mean_sigma_fd_minus[1])/(2*h) << std::endl;
    Cout << "\t\t\t grad_cov_meanlm1_sigmal: " << grad_cov_meanlm1_varl << ", "
  << (cov_mean_sigma_fd_plus[2] - cov_mean_sigma_fd_minus[2])/(2*h) <<
  std::endl; Cout << "\t\t\t grad_cov_meanl_sigmalm1: " << grad_cov_meanl_varlm1
  << ", " << (cov_mean_sigma_fd_plus[3] - cov_mean_sigma_fd_minus[3])/(2*h) <<
  std::endl; Cout << "\t\t\t grad_cov_meanlm1_sigmalm1: " <<
  grad_cov_meanlm1_varlm1 << ", " << (cov_mean_sigma_fd_plus[4] -
  cov_mean_sigma_fd_minus[4])/(2*h) << std::endl;
  }*/
  ...
  if(lev > 2){
    std::ofstream myfile;
    if(qoi == 0)
      myfile.open("covariance_file_problem18_4lev.txt", std::ofstream::out |
  std::ofstream::app); if(qoi == 1)
      myfile.open("covariance_file_cantilever_stress.txt", std::ofstream::out |
  std::ofstream::app); if(qoi == 2)
      myfile.open("covariance_file_cantilever_displ.txt", std::ofstream::out |
  std::ofstream::app); myfile << std::setprecision(16) << qoi << ", " << lev <<
  ", " << cov_mean_sigma << ",  "
           << cov_meanl_sigmal;
    myfile                             << ", " << cov_meanlm1_sigmalm1 << ", "
  << cov_meanlm1_sigmal << ", " << cov_meanl_sigmalm1; myfile <<
  std::setprecision(16) << ", " << cov_meanl_varl   << ", " <<
  cov_meanlm1_varlm1 << ", " << cov_meanlm1_varl << ", " << cov_meanl_varlm1;
    myfile << std::setprecision(16) << ", " << cm3l   << ", " << cm3lm1 << ", "
  << std::sqrt(var_var_l) << ", " << std::sqrt(var_var_lm1); myfile <<
  std::setprecision(16) << ", " << std::sqrt(var_sigma_l) << ", " <<
  std::sqrt(var_sigma_lm1); myfile << std::setprecision(16) << std::endl;
    myfile.close();
  }
  else if(lev > 0){
    std::ofstream myfile;
    if(qoi == 0)
      myfile.open("covariance_file_problem18.txt", std::ofstream::out |
  std::ofstream::app); if(qoi == 1)
      myfile.open("covariance_file_cantilever_stress.txt", std::ofstream::out |
  std::ofstream::app); if(qoi == 2)
      myfile.open("covariance_file_cantilever_displ.txt", std::ofstream::out |
  ...
  if (std::isnan(cov_mean_sigma)) {
    Cerr << "Cov_mean_sigma is nan since variance is zero or negative for qoi: "
         << qoi << " with values: cm2l: " << cm2l << " and cm2lm1: " << cm2lm1;
...
inline RealVector NonDMultilevelSampling::compute_cov_mean_sigma_fd(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const size_t lev) {
  Real mu_Q2l = 0, mu_Q2lm1 = 0, cm1l = 0, cm2l = 0, cm3l = 0, cm4l = 0,
  ...
  if (lev > 0) {
    cov_meanlm1_varlm1 = cm3lm1 / Nlq;
    cov_meanl_varlm1 =
        compute_cov_meanl_varlmone(sum_Ql, sum_Qlm1, sum_QlQlm1, Nlq_pilot, Nlq,
                                   qoi, lev, false, dummy_grad);
    cov_meanlm1_varl =
        compute_cov_meanlmone_varl(sum_Ql, sum_Qlm1, sum_QlQlm1, Nlq_pilot, Nlq,
                                   qoi, lev, false, dummy_grad);
  }
...
inline Real NonDMultilevelSampling::compute_cov_meanl_varlmone(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const size_t lev,
    const bool compute_gradient, Real& grad_g) {
  const RealMatrix &sum_Q1l = sum_Ql.at(1), &sum_Q1lm1 = sum_Qlm1.at(1),
                   &sum_Q2lm1 = sum_Qlm1.at(2);
  IntIntPair pr11(1, 1), pr12(1, 2);
  const RealMatrix &sum_Q1lQ1lm1 = sum_QlQlm1.at(pr11),
                   &sum_Q1lQ2lm1 = sum_QlQlm1.at(pr12);

  Real term1_mu_Q1lQ2lm1 =
      sum_Q1lQ2lm1(qoi, lev) / Nlq_pilot;  // mean(Ql.*(Qlmone).^2)
  Real term2_mu_Q1l_mu_Q2lm1 = unbiased_mean_product_pair(
      sum_Q1l(qoi, lev), sum_Q2lm1(qoi, lev), sum_Q1lQ2lm1(qoi, lev),
      Nlq_pilot);  // mean(Ql)*mean(Qlmone.^2)
  Real term3_mu_Q1lm1_mu_Q1lQ1lm1 = unbiased_mean_product_pair(
      sum_Q1lm1(qoi, lev), sum_Q1lQ1lm1(qoi, lev), sum_Q1lQ2lm1(qoi, lev),
      Nlq_pilot);  // mean(Qlmone)*mean(Ql.*Qlmone)
  Real term4_mu_Q1l_mu_Q1lm1_mu_Q1lm1 = unbiased_mean_product_triplet(
      sum_Q1l(qoi, lev), sum_Q1lm1(qoi, lev), sum_Q1lm1(qoi, lev),
      sum_Q1lQ1lm1(qoi, lev), sum_Q1lQ1lm1(qoi, lev), sum_Q2lm1(qoi, lev),
      sum_Q1lQ2lm1(qoi, lev), Nlq_pilot);  // mean(Ql)*mean(Qlmone)^2

  Real cov_numerator =
      (term1_mu_Q1lQ2lm1 - term2_mu_Q1l_mu_Q2lm1 -
       2. * term3_mu_Q1lm1_mu_Q1lQ1lm1 + 2. * term4_mu_Q1l_mu_Q1lm1_mu_Q1lm1);
  /*
  if(lev > 2){
    std::ofstream myfile;
    if(qoi == 0)
      myfile.open("covariance_file_covmeanlvarlmoneterms_problem18_4lev.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 1)
      myfile.open("covariance_file_covmeanlvarlmoneterms_cantilever_stress.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 2)
      myfile.open("covariance_file_covmeanlvarlmoneterms_cantilever_displ.txt",
  std::ofstream::out | std::ofstream::app); myfile << qoi << ", " << lev; myfile
  << std::setprecision(16) << ", " << term1_mu_Q1lQ2lm1   << ", " <<
  term2_mu_Q1l_mu_Q2lm1 << ", " << 2.*term3_mu_Q1lm1_mu_Q1lQ1lm1 << ", "
  << 2.*term4_mu_Q1l_mu_Q1lm1_mu_Q1lm1; myfile << std::endl; myfile.close();
  }
  else if(lev > 0){
    std::ofstream myfile;
    if(qoi == 0)
      myfile.open("covariance_file_covmeanlvarlmoneterms_problem18.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 1)
      myfile.open("covariance_file_covmeanlvarlmoneterms_cantilever_stress.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 2)
      myfile.open("covariance_file_covmeanlvarlmoneterms_cantilever_displ.txt",
...
inline Real NonDMultilevelSampling::compute_cov_meanlmone_varl(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const size_t Nlq_pilot,
    const Real Nlq, const size_t qoi, const size_t lev,
    const bool compute_gradient, Real& grad_g) {
  ...
  cov_meanlmone_varl_term2_unbiased = @(Ql, Qlmone) mean_mean_unbiased(Qlmone,
  Ql.^2, size(Ql, 2)); cov_meanlmone_varl_term3_unbiased = @(Ql, Qlmone)
  2*mean_mean_unbiased(Ql, Qlmone.*Ql, size(Ql, 2));
  cov_meanlmone_varl_term4_unbiased = @(Ql, Qlmone)
  2*mean_mean_mean_unbiased(Qlmone, Ql, Ql, size(Ql, 2));
  */

  Real term1_mu_Q2lQ1lm1 =
      sum_Q2lQ1lm1(qoi, lev) / Nlq_pilot;  // mean(Qlmone.*(Ql).^2)
  Real term2_mu_Q2l_mu_Q1lm1 = unbiased_mean_product_pair(
      sum_Q2l(qoi, lev), sum_Q1lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev),
      Nlq_pilot);  // mean(Qlmone)*mean(Ql.^2)
  Real term3_mu_Q1l_mu_Q1lQ1lm1 = unbiased_mean_product_pair(
      sum_Q1l(qoi, lev), sum_Q1lQ1lm1(qoi, lev), sum_Q2lQ1lm1(qoi, lev),
      Nlq_pilot);  // mean(Ql)*mean(Qlmone.*Ql)
  Real term4_mu_Q1l_mu_Q1l_mu_Q1lm1 = unbiased_mean_product_triplet(
      sum_Q1l(qoi, lev), sum_Q1l(qoi, lev), sum_Q1lm1(qoi, lev),
      sum_Q2l(qoi, lev), sum_Q1lQ1lm1(qoi, lev), sum_Q1lQ1lm1(qoi, lev),
      sum_Q2lQ1lm1(qoi, lev), Nlq_pilot);  // mean(Qlmone)*mean(Ql)^2

  Real cov_numerator =
      (term1_mu_Q2lQ1lm1 - term2_mu_Q2l_mu_Q1lm1 -
       2. * term3_mu_Q1l_mu_Q1lQ1lm1 + 2. * term4_mu_Q1l_mu_Q1l_mu_Q1lm1);
  /*
  if(lev > 2){
    std::ofstream myfile;
    if(qoi == 0)
      myfile.open("covariance_file_covmeanlmonevarlterms_problem18_4lev.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 1)
      myfile.open("covariance_file_covmeanlmonevarlterms_cantilever_stress.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 2)
      myfile.open("covariance_file_covmeanlmonevarlterms_cantilever_displ.txt",
  std::ofstream::out | std::ofstream::app); myfile << qoi << ", " << lev; myfile
  << std::setprecision(16) << ", " << term1_mu_Q2lQ1lm1   << ", " <<
  term2_mu_Q2l_mu_Q1lm1 << ", " << 2.*term3_mu_Q1l_mu_Q1lQ1lm1 << ", "
  << 2.*term4_mu_Q1l_mu_Q1l_mu_Q1lm1; myfile << std::endl; myfile.close();
  }
  else if(lev > 0){
    std::ofstream myfile;
    if(qoi == 0)
      myfile.open("covariance_file_covmeanlmonevarlterms_problem18.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 1)
      myfile.open("covariance_file_covmeanlmonevarlterms_cantilever_stress.txt",
  std::ofstream::out | std::ofstream::app); if(qoi == 2)
      myfile.open("covariance_file_covmeanlmonevarlterms_cantilever_displ.txt",
...
inline Real NonDMultilevelSampling::compute_grad_cov_meanl_vark(
    const Real cov_mean_var, const Real var_of_var, const Real var_of_sigma,
    const Real grad_var_of_var, const Real grad_var_of_sigma, const Real Nlq) {
  ...
  */
  ...
  denominator =
      2 * Nlq * var_of_var * var_of_var * sqrt(var_of_sigma / var_of_var);

  return cov_mean_var * enumerator / denominator;
}

inline Real NonDMultilevelSampling::unbiased_mean_product_pair(
    const Real sumQ1, const Real sumQ2, const Real sumQ1Q2, const size_t Nlq) {
...
inline Real NonDMultilevelSampling::unbiased_mean_product_triplet(
    const Real sumQ1, const Real sumQ2, const Real sumQ3, const Real sumQ1Q2,
    const Real sumQ1Q3, const Real sumQ2Q3, const Real sumQ1Q2Q3,
    const size_t Nlq) {
...
inline Real NonDMultilevelSampling::unbiased_mean_product_pairpair(
    const Real sumQ1, const Real sumQ2, const Real sumQ1Q2, const Real sumQ1sq,
    const Real sumQ2sq, const Real sumQ1sqQ2, const Real sumQ1Q2sq,
    const Real sumQ1sqQ2sq, const size_t Nlq) {
  Real mean1, mean2, mean3, mean4, bessel_corr1, bessel_corr2, bessel_corr3,
      bessel_corr4 = 0.;

  mean1 =
      1. / Nlq * 1. / Nlq * 1. / Nlq * 1. / Nlq * sumQ1 * sumQ1 * sumQ2 * sumQ2;

  mean2 = unbiased_mean_product_triplet(sumQ1sq, sumQ2, sumQ2, sumQ1sqQ2,
                                        sumQ1sqQ2, sumQ2sq, sumQ1sqQ2sq, Nlq);
  mean2 +=
      4. * unbiased_mean_product_triplet(sumQ1Q2, sumQ1, sumQ2, sumQ1sqQ2,
                                         sumQ1Q2sq, sumQ1Q2, sumQ1sqQ2sq, Nlq);
  mean2 += unbiased_mean_product_triplet(
      sumQ1, sumQ1, sumQ2sq, sumQ1sq, sumQ1Q2sq, sumQ1Q2sq, sumQ1sqQ2sq, Nlq);
  ...
  bessel_corr1 = ((Real)Nlq * (Real)Nlq * (Real)Nlq) /
                 ((Nlq - 1.) * (Nlq - 2.) * (Nlq - 3.));
