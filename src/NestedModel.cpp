...
static const char rcsId[] =
    "@(#) $Id: NestedModel.cpp 7024 2010-10-16 01:24:42Z mseldre $";
...
NestedModel::NestedModel(ProblemDescDB& problem_db,
  ...
  if (oi_ptr.empty())
    ...
    if (oi_resp_ptr_defined) problem_db.set_db_responses_node(oi_resp_ptr);
    ...
    optInterfaceResponse = Response::get_response(
        problem_db, SIMULATION_RESPONSE, currentVariables);
    ...
    if (oi_resp_ptr_defined) {
      ...
      for (i = 0; i < numOptInterfEqCon; ++i)
        if (interf_eq_targets[i] != 0.) warning_flag = true;
      if (warning_flag)
  ...
  if (outputLevel > NORMAL_OUTPUT) subModel->fine_grained_evaluation_counters();
  ...
  size_t i,
      num_var_map_1 = primary_var_mapping.size(),
  ...
  size_t curr_i, sm_acv_cntr = 0, sm_adiv_cntr = 0, sm_adsv_cntr = 0,
                 sm_adrv_cntr = 0, sm_acv_avail = 0, sm_adiv_avail = 0,
                 sm_adsv_avail = 0, sm_adrv_avail = 0;
  ...
  size_t num_curr_ccv =
             currentVariables.acv() - num_curr_cv,  // active complement
      num_curr_cdiv =
          currentVariables.adiv() - num_curr_div,  // active complement
      num_curr_cdsv =
          currentVariables.adsv() - num_curr_dsv,  // active complement
      num_curr_cdrv =
          currentVariables.adrv() - num_curr_drv;  // active complement
  // complement can be sized based on corresponding currentVariables sizes
  // due to restriction to default mappings
  if (num_curr_ccv) complement1ACVarMapIndices.resize(num_curr_ccv);
  if (num_curr_cdiv) complement1ADIVarMapIndices.resize(num_curr_cdiv);
  if (num_curr_cdsv) complement1ADSVarMapIndices.resize(num_curr_cdsv);
  if (num_curr_cdrv) complement1ADRVarMapIndices.resize(num_curr_cdrv);
...
void NestedModel::declare_sources() {
  ...
  if (optionalInterface)
    evaluationsDB.declare_source(
        modelId, modelType, optionalInterface->interface_id(), "interface");
}

IntIntPair NestedModel::estimate_partition_bounds(int max_eval_concurrency) {
  ...
  subIteratorSched.construct_sub_iterator(probDescDB, parallelLib, subIterator,
                                          subModel, subMethodPointer, empty_str,
                                          empty_str);
...
void NestedModel::derived_init_communicators(ParLevLIter pl_iter,
...
void NestedModel::derived_init_serial() {
  ...
  if (optionalInterface) optionalInterface->init_serial();
  subModel->init_serial();
}

void NestedModel::derived_set_communicators(ParLevLIter pl_iter,
                                            int max_eval_concurrency,
                                            bool recurse_flag) {
  // Outer context:
  ...
  if (recurse_flag) {
    ...
    if (subIteratorSched.messagePass) asynchEvalFlag = true;
    if (subIteratorSched.numIteratorServers > evaluationCapacity)
      evaluationCapacity = subIteratorSched.numIteratorServers;
  }
}

void NestedModel::derived_free_communicators(ParLevLIter pl_iter,
                                             int max_eval_concurrency,
                                             bool recurse_flag) {
  /*
  if (optionalInterface) {
...
void NestedModel::init_sub_iterator() {
  ...
  subIterator->nested_variable_mappings(
      active1ACVarMapIndices, active1ADIVarMapIndices, active1ADSVarMapIndices,
      active1ADRVarMapIndices, active2ACVarMapTargets, active2ADIVarMapTargets,
      active2ADSVarMapTargets, active2ADRVarMapTargets);
  ...
  size_t num_mapped_ineq_con = probDescDB.get_sizet(
             "responses.num_nonlinear_inequality_constraints"),
         num_mapped_eq_con = probDescDB.get_sizet(
  ...
  if (identityRespMap) {
    ...
    if (num_mapped_total != numSubIterFns) {
      Cerr << "\nError: For identity_response_mapping, number of nested model "
           << "responses (primary + secondary functions) must equal the number "
              "of "
           << "sub-method final results. Specified nested model has "
           << num_mapped_total << " functions, while there are "
           << numSubIterFns << " sub-method results.\n";
  ...
  if (!secondary_resp_coeffs.empty()) {
    ...
    copy_data(secondary_resp_coeffs, secondaryRespCoeffs, 0,
              (int)numSubIterFns);
...
void NestedModel::resolve_map1(const String& map1, size_t& ac_index1,
                               size_t& adi_index1, size_t& ads_index1,
                               size_t& adr_index1, size_t curr_index,
                               short& inactive_sm_view) {
  adi_index1 = ads_index1 = adr_index1 = _NPOS;
  ac_index1 =
      find_index(ModelUtils::all_continuous_variable_labels(*subModel), map1);
  if (ac_index1 == _NPOS) {
    adi_index1 = find_index(
        ModelUtils::all_discrete_int_variable_labels(*subModel), map1);
    if (adi_index1 == _NPOS) {
      ads_index1 = find_index(
          ModelUtils::all_discrete_string_variable_labels(*subModel), map1);
      if (ads_index1 == _NPOS) {
        adr_index1 = find_index(
            ModelUtils::all_discrete_real_variable_labels(*subModel), map1);
        if (adr_index1 == _NPOS) {
          Cerr << "\nError: primary mapping " << map1 << " could not be "
               << "matched within any sub-model variable labels." << std::endl;
          abort_handler(MODEL_ERROR);
        } else if (find_index(
                       ModelUtils::discrete_real_variable_labels(*subModel),
                       map1) == _NPOS)  // inactive DRV target
          update_inactive_view(ModelUtils::all_discrete_real_variable_types(
                                   *subModel)[adr_index1],
                               inactive_sm_view);
      } else if (find_index(
                     ModelUtils::discrete_string_variable_labels(*subModel),
                     map1) == _NPOS)  // inactive DSV target
        update_inactive_view(ModelUtils::all_discrete_string_variable_types(
                                 *subModel)[ads_index1],
                             inactive_sm_view);
    } else if (find_index(ModelUtils::discrete_int_variable_labels(*subModel),
                          map1) == _NPOS)  // inactive DIV target
      update_inactive_view(
          ModelUtils::all_discrete_int_variable_types(*subModel)[adi_index1],
          inactive_sm_view);
  } else if (find_index(ModelUtils::continuous_variable_labels(*subModel),
                        map1) == _NPOS)  // inactive CV target
    update_inactive_view(
        ModelUtils::all_continuous_variable_types(*subModel)[ac_index1],
        inactive_sm_view);
...
void NestedModel::resolve_real_variable_mapping(const String& map1,
                                                const String& map2,
                                                size_t curr_index,
                                                short& inactive_sm_view) {
  size_t ac_index1, adi_index1, ads_index1, adr_index1;
  resolve_map1(map1, ac_index1, adi_index1, ads_index1, adr_index1, curr_index,
               inactive_sm_view);
  ...
  if (!active2ACVarMapTargets.empty()) {  // indicates num_var_map_2
    if (map2.empty())
      ...
      active2ADIVarMapTargets[curr_index] =
          active2ADSVarMapTargets[curr_index] =
              active2ADRVarMapTargets[curr_index] = Pecos::NO_TARGET;
...
void NestedModel::resolve_integer_variable_mapping(const String& map1,
                                                   const String& map2,
                                                   size_t curr_index,
                                                   short& inactive_sm_view) {
  size_t ac_index1, adi_index1, ads_index1, adr_index1;
...
void NestedModel::resolve_string_variable_mapping(const String& map1,
                                                  const String& map2,
                                                  size_t curr_index,
                                                  short& inactive_sm_view) {
  size_t ac_index1, adi_index1, ads_index1, adr_index1;
...
void NestedModel::derived_evaluate(const ActiveSet& set) {
  ...
  set_mapping(set, opt_interface_set, opt_interface_map, sub_iterator_set,
              sub_iterator_map);
  ...
  if (opt_interface_map) {
    ...
    if (hierarchicalTagging) {
      String eval_tag =
          evalTagPrefix + '.' + std::to_string(nestedModelEvalCntr);
    ...
    if (interfEvaluationsDBState == EvaluationsDBState::UNINITIALIZED)
      interfEvaluationsDBState = evaluationsDB.interface_allocate(
          ...
          optInterfaceResponse, default_interface_active_set(),
    ...
    if (interfEvaluationsDBState == EvaluationsDBState::ACTIVE) {
      evaluationsDB.store_interface_variables(
          modelId, interface_id(), optionalInterface->evaluation_id(),
          opt_interface_set, currentVariables);
      evaluationsDB.store_interface_response(modelId, interface_id(),
                                             optionalInterface->evaluation_id(),
                                             optInterfaceResponse);
    }
...
void NestedModel::derived_evaluate_nowait(const ActiveSet& set) {
  ...
  if (opt_interface_map) {
    Cout << "\n----------------------------------------------------------------"
         << "--\nNestedModel Evaluation " << std::setw(4) << nestedModelEvalCntr
         << ": queueing optional interface mapping\n"
         << "------------------------------------------------------------------"
         << '\n';
...
const IntResponseMap& NestedModel::derived_synchronize() {
  ...
  if (!subIteratorPRPQueue.empty()) {
    ...
    subIteratorIdMap.clear();
    subIteratorJobCntr = 0;
  }

  // nestedVarsMap.clear();
  for (r_cit = nestedResponseMap.begin(); r_cit != nestedResponseMap.end();
       ++r_cit)
    Cout << "\n---------------------------\nNestedModel Evaluation "
...
void NestedModel::set_mapping(const ActiveSet& mapped_set,
                              ActiveSet& opt_interface_set,
                              bool& opt_interface_map,
                              ActiveSet& sub_iterator_set,
  ...
  if (mapped_asv.size() !=
      num_mapped_primary + num_opt_interf_con + subIterMappedSec) {
    Cerr << "\nError: mismatch is ASV lengths in NestedModel::set_mapping()."
         << "\n       expected " << mapped_asv.size() << " total, received "
         << num_mapped_primary << " primary plus "
         << num_opt_interf_con + subIterMappedSec << " secondary." << std::endl;
    abort_handler(MODEL_ERROR);
  }
  ...
  for (i = 0; i < subIterMappedSec; ++i) {
    short mapped_asv_val =
        mapped_asv[i + num_mapped_primary + num_opt_interf_con];
    if (mapped_asv_val) {
      if (identityRespMap)
  ...
  if (sub_iterator_map) {
    ...
    */
    ...
    if (!var_map_2) sub_iterator_dvv.resize(num_mapped_dvv);
  ...
  for (i = 0; i < num_opt_interf_con; ++i)
    opt_interface_asv[i + numOptInterfPrimary] =
        mapped_asv[i + num_mapped_primary];
...
void NestedModel::interface_response_overlay(
    const Response& opt_interface_response, Response& mapped_response) {
  ...
  for (i = 0; i < num_mapped_fns; ++i) {
    if (mapped_asv[i] & 6) deriv_flag = true;
    break;
  }
  ...
  for (i = 0; i < numOptInterfPrimary; ++i) {
    if (mapped_asv[i] & 1)
      mapped_response.function_value(opt_interface_response.function_value(i),
                                     i);
    if (mapped_asv[i] & 2)
...
void NestedModel::iterator_response_overlay(
  ...
  for (i = 0; i < subIterMappedSec; ++i) {
    m_index =
        num_mapped_1 + numOptInterfIneqCon + i;  // {a_l} <= [A]{S} <= {a_u}
      ...
      for (j = 0; j < num_mapped_deriv_vars; ++j) {
        for (k = 0; k <= j; ++k) {
          if (identityRespMap)
            ...
            Real& inner_prod = mapped_hess(j, k);
            inner_prod = 0.;
            for (l = 0; l < numSubIterFns; ++l) {
...
void NestedModel::iterator_error_estimation(
    const RealSymMatrix& sub_iterator_errors, RealVector& mapped_errors) {
  ...
  /*
  Cout << "## sub_iterator_erros size: " << sub_iterator_errors.length() <<
  std::endl; Cout << "## subIterMappedPri: " << subIterMappedPri << "
  numSubIterFns: " << numSubIterFns << std::endl; Cout << "## subIterMappedSec:
  " << subIterMappedSec << " numSubIterMappedIneqCon: " <<
  numSubIterMappedIneqCon << std::endl; Cout << "## numOptInterfPrimary: " <<
  numOptInterfPrimary << " numOptInterfIneqCon: " << numOptInterfIneqCon << "
  numOptInterfEqCon: " << numOptInterfEqCon << std::endl;
  */
  ...
  for (i = 0; i < subIterMappedPri; ++i) {
    if (identityRespMap)
      ...
      for (j = 0; j < numSubIterFns / 2; ++j) {  // Nb of sub responses
        ...
        if (coeff1 != 0.) {
          term = coeff1 * sub_iterator_errors(
                              2 * j, 2 * j);  // [W]{S} Mean term (Diagonal)
          sum += term * term;
        }
        if (coeff2 != 0.) {
          term = coeff2 *
                 sub_iterator_errors(2 * j + 1,
                                     2 * j + 1);  // [W]{S} Var/Sigma (Diagonal)
          sum += term * term;
        }
        if (coeff1 != 0 && coeff2 != 0.) {
          term = 2. * coeff1 * coeff2 *
                 sub_iterator_errors(2 * j + 1,
                                     2 * j);  // [W]{S} Cov (Subdiagonal)
          sum += term;
        }
      }
  ...
  for (i = 0; i < subIterMappedSec; ++i) {
    ...
    if (identityRespMap)
      ...
      for (j = 0; j < numSubIterFns / 2; ++j) {  // Nb of sub responses
        ...
        if (coeff1 != 0.) {
          term =
              coeff1 * sub_iterator_errors(2 * j, 2 * j);  // [W]{S} Mean term
          sum += term * term;
        }
        if (coeff2 != 0.) {
          term = coeff2 *
                 sub_iterator_errors(2 * j + 1, 2 * j + 1);  // [W]{S} Var/Sigma
          sum += term * term;
        }
        if (coeff1 != 0 && coeff2 != 0.) {
          term = 2. * coeff1 * coeff2 *
                 sub_iterator_errors(2 * j + 1, 2 * j);  // [W]{S} Cov
          sum += term;
        }
      }
...
void NestedModel::component_parallel_mode(short mode) {
  ...
  */
  ...
  if ((componentParallelMode != mode ||
    ...
    if (mi_pl.server_communicator_size() > 1) parallelLib.bcast(mode, mi_pl);
  }
    ...
    if (componentParallelMode == INTERFACE_MODE && optionalInterface) {
      ...
      if (subIteratorSched.messagePass)  // serve concurrent subIterator execs
        ...
        ParLevLIter si_pl_iter  // inner context
            = modelPCIter->mi_parallel_level_iterator(
                subIteratorSched.miPLIndex);
  ...
  if (new_view == RELAXED_ALL || new_view == MIXED_ALL) {
    ...
    for (i = 0; i < num_sm_acv; ++i)
      if (i < sm_cv_start || i >= sm_cv_start + num_sm_cv) {  // inactive
        ...
        if (type_i == CONTINUOUS_DESIGN || (type_i >= DISCRETE_DESIGN_RANGE &&
...
void NestedModel::update_inactive_view(unsigned short type, short& view) {
  ...
  bool relaxed =
      (active_sm_view == RELAXED_ALL ||
       (active_sm_view >= RELAXED_DESIGN && active_sm_view <= RELAXED_STATE));
  ...
  if (type >= CONTINUOUS_DESIGN && type <= DISCRETE_DESIGN_SET_REAL) {
    ...
    if (type >= CONTINUOUS_INTERVAL_UNCERTAIN)
      new_view =
          (relaxed) ? RELAXED_EPISTEMIC_UNCERTAIN : MIXED_EPISTEMIC_UNCERTAIN;
    else
      new_view =
          (relaxed) ? RELAXED_ALEATORY_UNCERTAIN : MIXED_ALEATORY_UNCERTAIN;
    update_inactive_view(new_view, view);
  }
}

void NestedModel::update_sub_model(const Variables& vars,
  ...
  if (num_curr_cv) {
    ...
    StringMultiArrayConstView curr_c_labels = vars.continuous_variable_labels();
    for (i = 0; i < num_curr_cv; ++i) {
      curr_i = svd.cv_index_to_active_index(i);
      ...
      if (pacvm_index != _NPOS) {
        ...
        if (sacvm_target == Pecos::NO_TARGET) {
          ModelUtils::all_continuous_variable(*subModel, curr_c_vars[i],
                                              pacvm_index);
          if (extraCVarsData[i]) {  // default mapping between consistent types
            ModelUtils::all_continuous_lower_bound(*subModel, curr_c_l_bnds[i],
                                                   pacvm_index);
            ModelUtils::all_continuous_upper_bound(*subModel, curr_c_u_bnds[i],
                                                   pacvm_index);
            // Note: this is more general than just bounds (all dist params):
            sm_mvd_rep->pull_distribution_parameters(
                mvDist, svd.cv_index_to_all_index(i),
                sm_svd.acv_index_to_all_index(pacvm_index));
            if (firstUpdate)
              ModelUtils::all_continuous_variable_label(
                  *subModel, curr_c_labels[i], pacvm_index);
          }
        } else
    ...
    for (i = 0; i < num_curr_div; ++i) {
      ...
      if (pacvm_index != _NPOS) {
        ...
        if (sadivm_target == Pecos::NO_TARGET) {
          ModelUtils::all_discrete_int_variable(*subModel, curr_di_vars[i],
                                                padivm_index);
          if (extraDIVarsData[i]) {  // default mapping between consistent types
            ModelUtils::all_discrete_int_lower_bound(
                *subModel, curr_di_l_bnds[i], padivm_index);
            ModelUtils::all_discrete_int_upper_bound(
                *subModel, curr_di_u_bnds[i], padivm_index);
            // Note: this is more general than just bounds (all dist params):
            sm_mvd_rep->pull_distribution_parameters(
                mvDist, svd.div_index_to_all_index(i),
                sm_svd.adiv_index_to_all_index(padivm_index));
            if (firstUpdate)
              ModelUtils::all_discrete_int_variable_label(
                  *subModel, curr_di_labels[i], padivm_index);
          }
        } else
          integer_variable_mapping(curr_di_vars[i], padivm_index,
                                   sadivm_target);
      } else if (padsvm_index != _NPOS) {
        short sadsvm_target = (num_var_map_2) ? active2ADSVarMapTargets[curr_i]
                                              : Pecos::NO_TARGET;
    ...
    for (i = 0; i < num_curr_dsv; ++i) {
      ...
      if (pacvm_index != _NPOS) {
        ...
        if (sadsvm_target == Pecos::NO_TARGET) {
          ModelUtils::all_discrete_string_variable(*subModel, curr_ds_vars[i],
                                                   padsvm_index);
          if (extraDSVarsData[i]) {  // default mapping between consistent types
            // Note: this is more general than just bounds (all dist params):
            sm_mvd_rep->pull_distribution_parameters(
                mvDist, svd.dsv_index_to_all_index(i),
                sm_svd.adsv_index_to_all_index(padsvm_index));
            if (firstUpdate)
              ModelUtils::all_discrete_string_variable_label(
                  *subModel, curr_ds_labels[i], padsvm_index);
          }
        } else
    ...
    for (i = 0; i < num_curr_drv; ++i) {
      ...
      if (pacvm_index != _NPOS) {
        ...
        if (sadrvm_target == Pecos::NO_TARGET) {
          ModelUtils::all_discrete_real_variable(*subModel, curr_dr_vars[i],
                                                 padrvm_index);
          if (extraDRVarsData[i]) {  // default mapping between consistent types
            ModelUtils::all_discrete_real_lower_bound(
                *subModel, curr_dr_l_bnds[i], padrvm_index);
            ModelUtils::all_discrete_real_upper_bound(
                *subModel, curr_dr_u_bnds[i], padrvm_index);
            // Note: this is more general than just bounds (all dist params):
            sm_mvd_rep->pull_distribution_parameters(
                mvDist, svd.drv_index_to_all_index(i),
                sm_svd.adrv_index_to_all_index(padrvm_index));
            if (firstUpdate)
              ModelUtils::all_discrete_real_variable_label(
                  *subModel, curr_dr_labels[i], padrvm_index);
          }
        } else
  ...
  if (num_curr_ccv) {
    ...
    for (i = 0; i < num_curr_ccv; ++i) {
      ...
      ModelUtils::all_continuous_variable(*subModel, curr_ac_vars[curr_i],
                                          c1_index);
      ModelUtils::all_continuous_lower_bound(*subModel, curr_ac_l_bnds[curr_i],
                                             c1_index);
      ModelUtils::all_continuous_upper_bound(*subModel, curr_ac_u_bnds[curr_i],
                                             c1_index);
      if (firstUpdate) {
        ModelUtils::all_continuous_variable_label(
            *subModel, curr_ac_labels[curr_i], c1_index);
        // Note: this is more general than just bounds (all dist params):
        sm_mvd_rep->pull_distribution_parameters(
            mvDist, svd.ccv_index_to_all_index(i),
  ...
  if (num_curr_cdiv) {
    ...
    for (i = 0; i < num_curr_cdiv; ++i) {
      ...
      ModelUtils::all_discrete_int_variable(*subModel, curr_adi_vars[curr_i],
                                            c1_index);
      ModelUtils::all_discrete_int_lower_bound(
          *subModel, curr_adi_l_bnds[curr_i], c1_index);
      ModelUtils::all_discrete_int_upper_bound(
          *subModel, curr_adi_u_bnds[curr_i], c1_index);
      if (firstUpdate) {
        ModelUtils::all_discrete_int_variable_label(
            *subModel, curr_adi_labels[curr_i], c1_index);
        // Note: this is more general than just bounds (all dist params):
        sm_mvd_rep->pull_distribution_parameters(
            mvDist, svd.cdiv_index_to_all_index(i),
  ...
  if (num_curr_cdsv) {
    ...
    for (i = 0; i < num_curr_cdsv; ++i) {
      ...
      ModelUtils::all_discrete_string_variable(*subModel, curr_ads_vars[curr_i],
                                               c1_index);
      if (firstUpdate) {
        ModelUtils::all_discrete_string_variable_label(
            *subModel, curr_ads_labels[curr_i], c1_index);
        // Note: this is more general than just bounds (all dist params):
        sm_mvd_rep->pull_distribution_parameters(
            mvDist, svd.cdsv_index_to_all_index(i),
  ...
  if (num_curr_cdrv) {
    ...
    for (i = 0; i < num_curr_cdrv; ++i) {
      ...
      ModelUtils::all_discrete_real_variable(*subModel, curr_adr_vars[curr_i],
                                             c1_index);
      ModelUtils::all_discrete_real_lower_bound(
          *subModel, curr_adr_l_bnds[curr_i], c1_index);
      ModelUtils::all_discrete_real_upper_bound(
          *subModel, curr_adr_u_bnds[curr_i], c1_index);
      if (firstUpdate) {
        ModelUtils::all_discrete_real_variable_label(
            *subModel, curr_adr_labels[curr_i], c1_index);
        // Note: this is more general than just bounds (all dist params):
        sm_mvd_rep->pull_distribution_parameters(
            mvDist, svd.cdrv_index_to_all_index(i),
...
void NestedModel::real_variable_mapping(Real r_var, size_t av_index,
                                        short svm_target) {
  ...
  switch (svm_target) {
    ...
    // or scaling.  They are mapped to a convenient definition believed to be
    // the most natural for a user, where the definition changes from
    // distribution to distribution (a consistent meaning of mu/sigma would be
    // more awkward for a user to convert).  For Normal, location & scale are
    // mean & std deviation.
    ...
    // the most natural for a user, where the definition changes from
    // distribution to distribution (a consistent meaning of mu/sigma would be
    // more awkward for a user to convert).  For Uniform, location & scale are
    // center & range.
    ...
    // the most natural for a user, where the definition changes from
    // distribution to distribution (a consistent meaning of mu/sigma would be
    // more awkward for a user to convert).  For Triangular, location & scale
    // are mode & range.
...
void NestedModel::integer_variable_mapping(int i_var, size_t av_index,
                                           short svm_target) {
  Pecos::MultivariateDistribution& sm_mvd =
      subModel->multivariate_distribution();
  std::shared_ptr<Pecos::MarginalsCorrDistribution> sm_mvd_rep =
  ...
  switch (svm_target) {
    ...
    default:
      Cerr << "\nError: secondary mapping target unmatched for integer value "
           << "insertion in NestedModel::integer_variable_mapping()"
           << std::endl;
      abort_handler(MODEL_ERROR);
  }
}

void NestedModel::string_variable_mapping(const String& s_var, size_t av_index,
                                          short svm_target) {
  Pecos::MultivariateDistribution& sm_mvd =
      subModel->multivariate_distribution();
  std::shared_ptr<Pecos::MarginalsCorrDistribution> sm_mvd_rep =
  ...
  switch (svm_target) {
    ...
    default:
      Cerr << "\nError: secondary mapping target unmatched for string value "
           << "insertion in NestedModel::string_variable_mapping()"
           << std::endl;
      abort_handler(MODEL_ERROR);
  }
}
...
ActiveSet NestedModel::default_interface_active_set() {
  ...
  if (has_deriv_vars) {
    if (optInterfGradientType == "analytic") {
      ...
      for (const auto& gi : optInterfGradIdAnalytic) asv[gi - 1] |= 2;
    }

    if (optInterfHessianType == "analytic") {
      for (auto& a : asv) a |= 4;
    } else if (optInterfHessianType == "mixed") {
      for (const auto& hi : optInterfHessIdAnalytic) asv[hi - 1] |= 4;
    }
  }
