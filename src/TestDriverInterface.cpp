...
TestDriverInterface::TestDriverInterface(const ProblemDescDB& problem_db,
  ...
  if (localDataView & VARIABLES_MAP) {
    ...
    // case SOBOL_ISHIGAMI:
    varTypeMap["x1"] = VAR_x1;
    varTypeMap["x2"] = VAR_x2;
    ...
    varTypeMap["E"] = VAR_E;
    varTypeMap["X"] = VAR_X;
    varTypeMap["area_type"] = VAR_area_type;
    // varTypeMap["Y"] = VAR_Y; break;
    // case STEEL_COLUMN:
    varTypeMap["Fs"] = VAR_Fs;
    varTypeMap["P1"] = VAR_P1;
    varTypeMap["P2"] = VAR_P2;
    varTypeMap["P3"] = VAR_P3;
    varTypeMap["B"] = VAR_B;
    varTypeMap["D"] = VAR_D;
    ...
    // case PROBLEM18:
    varTypeMap["x"] = VAR_x;
    varTypeMap["xi"] = VAR_xi;
    varTypeMap["Af"] = VAR_Af;
    varTypeMap["Ac"] = VAR_Ac;
    // case TUNABLE_MODEL:
    varTypeMap["y"] = VAR_y;
    varTypeMap["theta"] = VAR_theta;
    varTypeMap["theta1"] = VAR_theta1;
    varTypeMap["theta2"] = VAR_theta2;
    varTypeMap["delta"] = VAR_delta;
    varTypeMap["gamma"] = VAR_gamma;
    //}
  }
}
...
int TestDriverInterface::derived_map_ac(const String& ac_name) {
  ...
  switch (ac_type) {
    case CANTILEVER_BEAM:
      ...
      break;
    case MOD_CANTILEVER_BEAM:
      fail_code = mod_cantilever();
      break;
    case CANTILEVER_BEAM_ML:
      fail_code = cantilever_ml();
      break;
    case CYLINDER_HEAD:
      fail_code = cyl_head();
      break;
    ...
    case EXTENDED_ROSENBROCK:
      ...
      break;
    case LF_ROSENBROCK:
      fail_code = lf_rosenbrock();
      break;
    case EXTRA_LF_ROSENBROCK:
      fail_code = extra_lf_rosenbrock();
      break;
    case MF_ROSENBROCK:
      fail_code = mf_rosenbrock();
      break;
    case LF_POLY_PROD:
      fail_code = lf_poly_prod();
      break;
    case POLY_PROD:
      fail_code = poly_prod();
      break;
    ...
    case SHORT_COLUMN:
      ...
      break;
    case LF_SHORT_COLUMN:
      fail_code = lf_short_column();
      break;
    case MF_SHORT_COLUMN:
      fail_code = mf_short_column();
      break;
    case SIDE_IMPACT_COST:
      fail_code = side_impact_cost();
      break;
    case SIDE_IMPACT_PERFORMANCE:
      fail_code = side_impact_perf();
      break;
    case SOBOL_RATIONAL:
      fail_code = sobol_rational();
      break;
    case SOBOL_G_FUNCTION:
      fail_code = sobol_g_function();
      break;
    case SOBOL_ISHIGAMI:
      fail_code = sobol_ishigami();
      break;
    case STEEL_COLUMN_COST:
    ...
    case TEXT_BOOK_OUU:
      ...
      break;
    case SCALABLE_TEXT_BOOK:
      fail_code = scalable_text_book();
      break;
    case SCALABLE_MONOMIALS:
      fail_code = scalable_monomials();
      break;
    case MOGATEST1:
      fail_code = mogatest1();
      break;
    ...
    case GENZ:
      ...
      break;
    case DAMPED_OSCILLATOR:
      fail_code = damped_oscillator();
      break;
    case STEADY_STATE_DIFFUSION_1D:
      fail_code = steady_state_diffusion_1d();
      break;
    case SS_DIFFUSION_DISCREPANCY:
      fail_code = ss_diffusion_discrepancy();
      break;
    case TRANSIENT_DIFFUSION_1D:
      fail_code = transient_diffusion_1d();
      break;
    case TUNABLE_MODEL:
      fail_code = tunable_model();
      break;
    case PREDATOR_PREY:
      fail_code = predator_prey();
      break;
    case ANISOTROPIC_QUADRATIC_FORM:
      fail_code = aniso_quad_form();
      break;
    case BAYES_LINEAR:
      fail_code = bayes_linear();
      break;
    case PROBLEM18:
      fail_code = problem18();
      break;
...
int TestDriverInterface::cantilever() {
  ...
  if ((numVars != 4 && numVars != 6) || numADIV ||
      numADRV ||                                          // var count, no dv
  ...
  // **** f:
  if (objective && (directFnASV[0] & 1)) fnVals[0] = area;

  // **** c1:
  if (directFnASV[c1i] & 1) fnVals[c1i] = stress / R - 1.;

  // **** c2:
  if (directFnASV[c2i] & 1) fnVals[c2i] = D4 - 1.;

  // **** df/dx:
  if (objective && (directFnASV[0] & 2))
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_w:
          fnGrads[0][i] = t;
          break;  // design var derivative
        case VAR_t:
          fnGrads[0][i] = w;
          break;  // design var derivative
        default:
          fnGrads[0][i] = 0.;
          break;  // uncertain var derivative
      }
  ...
  if (directFnASV[c1i] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_w:
          fnGrads[c1i][i] = -600. * (Y / t + 2. * X / w) / w_sq / t / R;
          break;  // dv
        case VAR_t:
          fnGrads[c1i][i] = -600. * (2. * Y / t + X / w) / w / t_sq / R;
          break;  // dv
        case VAR_R:
          fnGrads[c1i][i] = -stress / R_sq;
          break;  // uncertain var deriv
        case VAR_E:
          fnGrads[c1i][i] = 0.;
          break;  // uncertain var deriv
        case VAR_X:
          fnGrads[c1i][i] = 600. / w_sq / t / R;
          break;  // uncertain var deriv
        case VAR_Y:
          fnGrads[c1i][i] = 600. / w / t_sq / R;
          break;  // uncertain var deriv
      }
  ...
  if (directFnASV[c2i] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_w:
          fnGrads[c2i][i] = -D3 * 2. * X_sq / w_sq / w_sq / w - D4 / w;
          break;  // dv
        case VAR_t:
          fnGrads[c2i][i] = -D3 * 2. * Y_sq / t_sq / t_sq / t - D4 / t;
          break;  // dv
        case VAR_R:
          fnGrads[c2i][i] = 0.;
          break;  // unc var deriv
        case VAR_E:
          ...
          break;  // unc var deriv
        case VAR_X:
          fnGrads[c2i][i] = D3 * X / w_sq / w_sq;
          break;  // unc var deriv
        case VAR_Y:
          fnGrads[c2i][i] = D3 * Y / t_sq / t_sq;
          break;  // unc var deriv
      }
    ...
    for (size_t i = 0; i < numDerivVars; ++i)
      for (size_t j = 0; j <= i; ++j)
        if (varTypeDVV[i] == VAR_w && varTypeDVV[j] == VAR_w)
          fnHessians[c1i](i, j) =
              1200. * (Y / t + 3. * X / w) / w_sq / area / R;
        else if (varTypeDVV[i] == VAR_t && varTypeDVV[j] == VAR_t)
          fnHessians[c1i](i, j) =
              1200. * (3. * Y / t + X / w) / t_sq / area / R;
  ...
  if (directFnASV[c2i] & 4) {
    ...
    Real dD2_dX = 2. * X / w_sq / w_sq, dD3_dX = D6 * dD2_dX,
         dD4_dX = D8 * dD2_dX;
    Real dD2_dY = 2. * Y / t_sq / t_sq, dD3_dY = D6 * dD2_dY,
         dD4_dY = D8 * dD2_dY;
      ...
      for (size_t j = 0; j <= i; ++j)
        if (varTypeDVV[i] == VAR_w && varTypeDVV[j] == VAR_w)
          fnHessians[c2i](i, j) = D3 * 10. * X_sq / pow(w_sq, 3) -
                                  2. * X_sq / w_sq / w_sq / w * dD3_dw +
                                  D4 / w_sq - dD4_dw / w;
        else if (varTypeDVV[i] == VAR_t && varTypeDVV[j] == VAR_t)
          fnHessians[c2i](i, j) = D3 * 10. * Y_sq / pow(t_sq, 3) -
                                  2. * Y_sq / t_sq / t_sq / t * dD3_dt +
                                  D4 / t_sq - dD4_dt / t;
          ...
          fnHessians[c2i](i, j) = D3 / t_sq / t_sq + Y / t_sq / t_sq * dD3_dY;
                 ...
                 (varTypeDVV[i] == VAR_t && varTypeDVV[j] == VAR_w))
          fnHessians[c2i](i, j) =
              -2. * X_sq / w_sq / w_sq / w * dD3_dt - dD4_dt / w;
          ...
          fnHessians[c2i](i, j) = -dD4_dw / E;
                 ...
                 (varTypeDVV[i] == VAR_X && varTypeDVV[j] == VAR_w))
          fnHessians[c2i](i, j) =
              -4. * X * D3 / w_sq / w_sq / w + X / w_sq / w_sq * dD3_dw;
        else if ((varTypeDVV[i] == VAR_w && varTypeDVV[j] == VAR_Y) ||
                 (varTypeDVV[i] == VAR_Y && varTypeDVV[j] == VAR_w))
          ...
          fnHessians[c2i](i, j) = X / w_sq / w_sq * dD3_dt;
                 ...
                 (varTypeDVV[i] == VAR_Y && varTypeDVV[j] == VAR_t))
          fnHessians[c2i](i, j) =
              -4. * Y * D3 / t_sq / t_sq / t + Y / t_sq / t_sq * dD3_dt;
...
int TestDriverInterface::mod_cantilever() {
  ...
  // **** c1:
  if (directFnASV[c1i] & 1) fnVals[c1i] = stress - R;

  // **** c2:
  if (directFnASV[c2i] & 1) fnVals[c2i] = displ - D0;
  ...
  if (directFnASV[c1i] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_w:
          fnGrads[c1i][i] = -600. * (Y / t + 2. * X / w) / w_sq / t;
          break;  // des var
        case VAR_t:
          fnGrads[c1i][i] = -600. * (2. * Y / t + X / w) / w / t_sq;
          break;  // des var
        case VAR_R:
          fnGrads[c1i][i] = -1.;
          break;  // uncertain var deriv
        case VAR_E:
          ...
          break;  // uncertain var deriv
        case VAR_X:
          fnGrads[c1i][i] = 600. / w_sq / t;
          break;  // uncertain var deriv
        case VAR_Y:
          fnGrads[c1i][i] = 600. / w / t_sq;
          break;  // uncertain var deriv
      }
  ...
  if (directFnASV[c2i] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_w:
          fnGrads[c2i][i] = -D3 * 2. * X_sq / w_sq / w_sq / w - displ / w;
          break;
        case VAR_t:
          fnGrads[c2i][i] = -D3 * 2. * Y_sq / t_sq / t_sq / t - displ / t;
          break;
        case VAR_R:
          fnGrads[c2i][i] = 0.;
          break;  // unc var deriv
        case VAR_E:
          fnGrads[c2i][i] = -displ / E;
          break;  // unc var deriv
        case VAR_X:
          fnGrads[c2i][i] = D3 * X / w_sq / w_sq;
          break;  // unc var deriv
...
int TestDriverInterface::cantilever_ml() {
  ...
  // of augmenting the uncertain variables, then the number of variables is 4.
  // Design gradients are not supported for the case of design var insertion.
  /*if ( (numVars != 5 && numVars != 7) || (numADIV != 1) || numADRV ||//var
  count, no dv (gradFlag && numVars == 5 && (numDerivVars != 4 && numDerivVars
  != 7 ) ) ) { // design insertion Cerr << "Error: Bad number of variables in
  cantilever direct fn."
         << std::endl;
  ...
  int area_type = (area_type_iter == xDIM.end())
                      ? 1.
                      : area_type_iter->second;  // Correlation Af for objective
  ...
  if (area_type == 1 || area_type == 5) {  // Rectangle
    ...
    stress = area_type == 1 ? 600 * Y / w / t_sq + 600 * X / w_sq / t
                            : 600 * Y / w / t_sq;

    D1 = 4. * pow(L, 3) / E / area;
    D2 =
        area_type == 1 ? pow(Y / t_sq, 2) + pow(X / w_sq, 2) : pow(Y / t_sq, 2);
    D3 = D1 / std::sqrt(D2);
    ...
    stress =
        (4. * L) / (m_pi * a * b) * std::sqrt(pow(Y / a, 2) + pow(X / b, 2));

    Real I_x = (m_pi * b * pow(a, 3)) / 4.;
    Real I_y = (m_pi * pow(b, 3) * a) / 4.;
    displ = std::sqrt(pow((pow(L, 3) * X) / (3. * E * I_y), 2) +
    ...
    const Real radius =
        (area_type == 3) ? std::sqrt(w * t) / 2. : std::sqrt(w * t / m_pi);
...
int TestDriverInterface::cyl_head() {
  ...
  if (numVars != 2 || numADIV || numADRV || (gradFlag && numDerivVars != 2)) {
    Cerr << "Error: Bad number of variables in cyl_head direct fn."
         << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (numFns != 4) {
    Cerr << "Error: Bad number of functions in cyl_head direct fn."
         << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (hessFlag) {
  ...
  Real wall_thickness =
      intake_offset - exhaust_offset - (xC[0] + exhaust_dia) / 2.;
  ...
  // **** c1:
  if (directFnASV[1] & 1) fnVals[1] = max_stress / 1500. - 1.;

  // **** c2:
  if (directFnASV[2] & 1) fnVals[2] = 1. - warranty / 100000.;

  // **** c3:
  if (directFnASV[3] & 1) fnVals[3] = cycle_time / 60. - 1.;
...
int TestDriverInterface::multimodal() {
  ...
  if (directFnASV[0] & 1)
    fnVals[0] =
        (xC[0] * xC[0] + 4) * (xC[1] - 1) / 20 - std::sin(5 * xC[0] / 2) - 2;

  // **** df/dx:
  if (directFnASV[0] & 2) {
    fnGrads[0][0] =
        xC[0] * (xC[1] - 1) / 10 - (5 / 2) * std::cos(5 * xC[0] / 2);
...
int TestDriverInterface::rosenbrock() {
  ...
  if (least_sq_flag) {
    // **** Residual R1:
    if (directFnASV[0] & 1) fnVals[0] = 10. * f1;
    // **** Residual R2:
    if (directFnASV[1] & 1) fnVals[1] = f2;
      ...
      for (size_t i = 0; i < numDerivVars; ++i)
        for (size_t j = 0; j <= i; ++j)
          if (varTypeDVV[i] == VAR_x1 && varTypeDVV[j] == VAR_x1)
            ...
            fnHessians[0](i, j) = 0.;
    // **** d^2R2/dx^2:
    if (directFnASV[1] & 4) fnHessians[1] = 0.;
  } else {
    // **** f:
    if (directFnASV[0] & 1) fnVals[0] = 100. * f1 * f1 + f2 * f2;

    // **** df/dx:
    if (directFnASV[0] & 2)
      for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
          case VAR_x1:
            fnGrads[0][i] = -400. * f1 * x1 - 2. * f2;
            break;
          case VAR_x2:
            fnGrads[0][i] = 200. * f1;
            break;
...
int TestDriverInterface::modified_rosenbrock() {
  if (multiProcAnalysisFlag) {
    Cerr << "Error: modified rosenbrock direct fn does not yet support "
            "multiprocessor "
         << "analyses." << std::endl;
    abort_handler(-1);
  }
  ...
  if (least_sq_flag) {
    ...
    if (directFnASV[1] & 1) fnVals[1] = f2;
    // **** Residual R3:
    if (directFnASV[2] & 1) fnVals[2] = f3;
    ...
    if (directFnASV[2] & 2)
      for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
          case VAR_x1:
            fnGrads[2][i] = 2. * PI * std::cos(2. * PI * x1 + x2);
            break;
          case VAR_x2:
            fnGrads[2][i] = std::cos(2. * PI * x1 + x2);
            break;
        }
    ...
    // **** f:
    if (directFnASV[0] & 1) fnVals[0] = 100. * f1 * f1 + f2 * f2 + f3 * f3;
    ...
    if (directFnASV[0] & 2)
      for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
          case VAR_x1:
            ...
            break;
          case VAR_x2:
            fnGrads[0][i] = 200. * f1 + std::sin(2. * (2. * PI * x1 + x2));
            break;
        }
          ...
          if (varTypeDVV[i] == VAR_x1 && varTypeDVV[j] == VAR_x1)
            fnHessians[0](i, j) =
                ...
                8. * PI * PI * std::cos(2. * (2. * PI * x1 + x2));
                   ...
                   (varTypeDVV[i] == VAR_x2 && varTypeDVV[j] == VAR_x1))
            fnHessians[0](i, j) =
                -400. * x1 + 4. * PI * std::cos(2. * (2. * PI * x1 + x2));
          else if (varTypeDVV[i] == VAR_x2 && varTypeDVV[j] == VAR_x2)
            fnHessians[0](i, j) =
                200. + 2. * std::cos(2. * (2. * PI * x1 + x2));
  }
...
int TestDriverInterface::generalized_rosenbrock() {
  ...
  for (size_t i = 1; i < numVars; ++i) {
    ...
    if (least_sq_flag) {
      ...
      // **** R_2im1:
      if (directFnASV[rindex_2im1] & 1) fnVals[rindex_2im1] = 10. * f1;
      // **** R_2i:
      if (directFnASV[rindex_2i] & 1) fnVals[rindex_2i] = f2;
      ...
      // **** d^2R_2i/dx^2:
      if (directFnASV[rindex_2i] & 4) fnHessians[rindex_2i] = 0.;
...
int TestDriverInterface::extended_rosenbrock() {
  ...
  for (size_t i = 1; i <= numVars / 2; ++i) {
    ...
    if (least_sq_flag) {
      // **** Residual R_2im1:
      if (directFnASV[index_2im1] & 1) fnVals[index_2im1] = sqrt_alpha * f1;
      // **** Residual R_2i:
      if (directFnASV[index_2i] & 1) fnVals[index_2i] = f2;
      ...
      // **** d^2R_2i/dx^2:
      if (directFnASV[index_2i] & 4) fnHessians[index_2i] = 0.;

    } else {
      // **** f:
      if (directFnASV[0] & 1) fnVals[0] += alpha * f1 * f1 + f2 * f2;
...
int TestDriverInterface::lf_rosenbrock() {
  ...
  Real x1 = xCM[VAR_x1], x2 = xCM[VAR_x2], f1 = x2 - x1 * x1 + .2,
       f2 = 0.8 - x1;  // terms offset by +/- .2
...
int TestDriverInterface::extra_lf_rosenbrock() {
  ...
  Real x1 = xCM[VAR_x1], x2 = xCM[VAR_x2], f1 = x2 - a * x1 * x1 + b,
       f2 = c - d * x1;
  ...
  if (directFnASV[0] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_x1:
          fnGrads[0][i] = -400. * a * f1 * x1 - 2. * d * f2;
          break;
        case VAR_x2:
          fnGrads[0][i] = 200. * f1;
          break;
...
int TestDriverInterface::lf_poly_prod() {
  ...
  if (directFnASV[0] & 1) fnVals[0] = xC[0] * xC[0] - 0.5 * xC[1];
  ...
  if (directFnASV[0] & 4) fnHessians[0](0, 0) = 2.0;
...
int TestDriverInterface::poly_prod() {
  ...
  if (directFnASV[0] & 1) fnVals[0] = t1 * t2;
...
int TestDriverInterface::gerstner() {
  ...
  if (numVars != 2 || numADIV || numADRV || (gradFlag && numDerivVars != 2)) {
    Cerr << "Error: Bad number of variables in gerstner direct fn."
         << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (numFns != 1) {
    Cerr << "Error: Bad number of functions in gerstner direct fn."
         << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (hessFlag) {
  ...
  String an_comp = (!analysisComponents.empty() &&
                    !analysisComponents[analysisDriverIndex].empty())
                       ? analysisComponents[analysisDriverIndex][0]
                       : "iso1";
  short test_fn;
  ...
  if (directFnASV[0] & 1) {
    switch (test_fn) {
      case 1:
        fnVals[0] = x_coeff * std::exp(-x * x) + y_coeff * std::exp(-y * y);
        break;
      case 2:
        ...
        break;
      case 3:
        fnVals[0] = std::exp(-x_coeff * x * x - y_coeff * y * y);
        break;
    }
  }
...
int TestDriverInterface::scalable_gerstner() {
  ...
  String an_comp = (!analysisComponents.empty() &&
                    !analysisComponents[analysisDriverIndex].empty())
                       ...
                       : "iso1";
  short test_fn;
  Real even_coeff, odd_coeff, inter_coeff;
  if (an_comp == "iso1") {
    test_fn = 1;
    even_coeff = odd_coeff = 10.;
  } else if (an_comp == "iso2") {
    test_fn = 2;
    even_coeff = odd_coeff = inter_coeff = 1.;
  } else if (an_comp == "iso3") {
    test_fn = 3;
    even_coeff = odd_coeff = 10.;
  } else if (an_comp == "aniso1") {
  ...
  if (directFnASV[0] & 1) {
    switch (test_fn) {
      ...
      case 3: {
        ...
        for (size_t i = 0; i < numVars; ++i)
          sum -=
              (i % 2) ? odd_coeff * xC[i] * xC[i] : even_coeff * xC[i] * xC[i];
        fnVals[0] = std::exp(sum);
        break;
      }
    }
  }
  ...
  if (directFnASV[0] & 2) {
    ...
    switch (test_fn) {
      case 1:
        for (size_t i = 0; i < numVars; ++i)
          fnGrads[0][i] =
              (i % 2) ? -2. * xC[i] * odd_coeff * std::exp(-xC[i] * xC[i])
      ...
      case 3:
        if (directFnASV[0] & 1)
          ...
          Real sum = 0;
          for (size_t i = 0; i < numVars; ++i)
            sum -= (i % 2) ? odd_coeff * xC[i] * xC[i]
                           : even_coeff * xC[i] * xC[i];
          val = std::exp(sum);
        }
        for (size_t i = 0; i < numVars; ++i)
...
void TestDriverInterface::steady_state_diffusion_core(
    SpectralDiffusionModel& model, RealVector& domain_limits) {
  // ------------------------------------------------------------- //
  // Pre-processing
  // ------------------------------------------------------------- //
  ...
  if (err_flag) abort_handler(INTERFACE_ERROR);
  ...
  // defaults:
  for (int i = 0; i < numFns; i++) {
    size_t coord_index = find_index(xDRLabels, "coord_" + std::to_string(i));
    if (coord_index != _NPOS) qoi_coords[i] = xDR[coord_index];
  }
...
int TestDriverInterface::transient_diffusion_1d() {
  ...
  Real dx = 1. / N_x;
  x[0] = 0.;
  for (i = 1; i <= N_x; ++i) x[i] = x[i - 1] + dx;

  Real gamma = 1., T_f = 0.5, sum_A, int_u_n = 0., f_t_1, f_t_2, x_i, xpi,
       npi_g, u_exp, xi2_sq = xi[2] * xi[2], sin_xi0 = std::sin(xi[0]),
       sin_xi1 = std::sin(xi[1]),
       i_fn = 3.5 * (sin_xi0 + 7. * sin_xi1 * sin_xi1 +
                     xi2_sq * xi2_sq * sin_xi0 / 10.);
  ...
  for (n = 0; n < N_mod; ++n) {
    ...
    for (i = 1; i <= N_x; ++i) {
      ...
      A[i - 1] =
          (f_tilde[i] + f_tilde[i - 1]) * dx / 2.;  // area (trapezoidal rule)
      sum_A += A[i - 1];
    ...
    for (i = 1; i <= N_x; ++i)
      int_u_n +=
          u_n[i - 1] + u_n[i];  // ends counted once, interior counted twice
  }
  ...
  if (md_index != _NPOS) {
    switch (N_mod) {
      case 21:  // HF model form
        switch (N_x) {
          case 30:
            metaData[md_index] = 5.67e+5;
            break;
          case 60:
            metaData[md_index] = 4.536e+6;
            break;
          case 100:
            metaData[md_index] = 2.1e+7;
            break;
          case 200:
            metaData[md_index] = 1.68e+8;
            break;
        }
        break;
      case 3:  // LF model form
        switch (N_x) {
          case 5:
            metaData[md_index] = 375.;
            break;
          case 15:
            metaData[md_index] = 10125.;
            break;
          case 30:
            metaData[md_index] = 81000.;
            break;
          case 60:
            metaData[md_index] = 648000.;
            break;
        }
        break;
    }
  ...
  if (numACV == 3 && numADIV >= 1) {  // x,y,theta1 and model form integer
    ...
    if (mform == "HF" || mform == "Fine_HF") {
      ...
      if (mform == "LF1" || mform == "Coarse_HF") {
        ...
        w_lo *= gamma2;
        w_hi *= gamma2;  // new for LF: reduce MF cost by gamma
      }
      // cost metadata for tunable problem (extended cost model)
      Real log_w =
          std::log(w_lo) - std::log(w_lo / w_hi) / th_range *
...
int TestDriverInterface::predator_prey() {
  ...
  init_conditions[0] = 0.7;
  init_conditions[1] = 0.5;
...
int TestDriverInterface::genz() {
  ...
  if (numADIV || numADRV) {
    Cerr << "Error: Bad variable types in genz direct fn." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (numFns != 1) {
    Cerr << "Error: Bad number of functions in genz direct fn." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (hessFlag) {
    Cerr << "Error: Hessians not supported in genz direct fn." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  ...
  if (directFnASV[0] & 1) {
    switch (fn_type) {
      case 0:
        fnVals[0] = 2.0 * pi * w[0];
        for (int d = 0; d < numVars; d++) fnVals[0] += c[d] * xC[d];
...
int TestDriverInterface::damped_oscillator() {
  ...
  Real B1 = -F * (bw) / zeta2, B2 = F * kw / zeta2, A1 = x0 - B1,
       A2 = (v0 + g * A1 - w * B2) / wd;
  ...
  for (size_t i = 0; i < numFns; ++i) {
    ...
    if (directFnASV[i] & 1) {
      ...
      y_trans = std::exp(-g * time) *
                (A1 * std::cos(wd * time) + A2 * std::sin(wd * time));
...
int TestDriverInterface::log_ratio() {
  ...
  // **** f:
  if (directFnASV[0] & 1) fnVals[0] = x1 / x2;
...
int TestDriverInterface::short_column() {
  ...
  if (numFns > 1 && (directFnASV[ai] & 1)) fnVals[ai] = b * h;
  ...
  if (directFnASV[lsi] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_b:  // design variable derivative
          fnGrads[lsi][i] =
              4. * M / (b_sq * h_sq * Y) + 2. * P_sq / (b_sq * b * h_sq * Y_sq);
          break;
        case VAR_h:  // design variable derivative
          fnGrads[lsi][i] = 8. * M / (b * h_sq * h * Y) +
                            2. * P_sq / (b_sq * h_sq * h * Y_sq);
          break;
        case VAR_P:  // uncertain variable derivative
          fnGrads[lsi][i] = -2. * P / (b_sq * h_sq * Y_sq);
          break;
        case VAR_M:  // uncertain variable derivative
          fnGrads[lsi][i] = -4. / (b * h_sq * Y);
          break;
        case VAR_Y:  // uncertain variable derivative
          fnGrads[lsi][i] =
              4. * M / (b * h_sq * Y_sq) + 2. * P_sq / (b_sq * h_sq * Y_sq * Y);
          break;
      }
...
int TestDriverInterface::alternate_short_column_forms(int form) {
  ...
  if (directFnASV[lsi] & 1) {
    switch (form) {
      ...
      case 2:
        fnVals[lsi] =
            1. - 4. * P / (b * h_sq * Y) - P_sq / (b_sq * h_sq * Y_sq);
        break;
      case 3:
        fnVals[lsi] =
            1. - 4. * M / (b * h_sq * Y) - M_sq / (b_sq * h_sq * Y_sq);
        break;
      case 4:
        fnVals[lsi] = 1. - 4. * M / (b * h_sq * Y) -
                      P_sq / (b_sq * h_sq * Y_sq) - 4. * (P - M) / (b * h * Y);
        break;
      default:
        return 1;
        break;
...
int TestDriverInterface::side_impact_cost() {
  ...
  // **** f:
  if (directFnASV[0] & 1)
    fnVals[0] = 1.98 + 4.9 * xC[0] + 6.67 * xC[1] + 6.98 * xC[2] +
                4.01 * xC[3] + 1.78 * xC[4] + 2.73 * xC[6];
  ...
  // **** d^2f/dx^2:
  if (directFnASV[0] & 4) fnHessians[0] = 0.;
...
int TestDriverInterface::side_impact_perf() {
  ...
  if (directFnASV[1] & 1)
    fnVals[1] = 28.98 + 3.818 * xC[2] - 4.2 * xC[0] * xC[1] +
                0.0207 * xC[4] * xC[9] + 6.63 * xC[5] * xC[8] -
                7.7 * xC[6] * xC[7] + 0.32 * xC[8] * xC[9];
  if (directFnASV[2] & 1)
    fnVals[2] = 33.86 + 2.95 * xC[2] + 0.1792 * xC[9] - 5.057 * xC[0] * xC[1] -
                11 * xC[1] * xC[7] - 0.0215 * xC[4] * xC[9] -
                9.98 * xC[6] * xC[7] + 22 * xC[7] * xC[8];
  if (directFnASV[3] & 1)
    fnVals[3] =
        46.36 - 9.9 * xC[1] - 12.9 * xC[0] * xC[7] + 0.1107 * xC[2] * xC[9];
  if (directFnASV[4] & 1)
    fnVals[4] = 0.261 - 0.0159 * xC[0] * xC[1] - 0.188 * xC[0] * xC[7] -
                0.019 * xC[1] * xC[6] + 0.0144 * xC[2] * xC[4] +
                0.0008757 * xC[4] * xC[9] + 0.08045 * xC[5] * xC[8] +
                0.00139 * xC[7] * xC[10] + 0.00001575 * xC[9] * xC[10];
  if (directFnASV[5] & 1)
    fnVals[5] = 0.214 + 0.00817 * xC[4] - 0.131 * xC[0] * xC[7] -
                0.0704 * xC[0] * xC[8] + 0.03099 * xC[1] * xC[5] -
                0.018 * xC[1] * xC[6] + 0.0208 * xC[2] * xC[7] +
                0.121 * xC[2] * xC[8] - 0.00364 * xC[4] * xC[5] +
                0.0007715 * xC[4] * xC[9] - 0.0005354 * xC[5] * xC[9] +
                0.00121 * xC[7] * xC[10];
  if (directFnASV[6] & 1)
    fnVals[6] = 0.74 - 0.61 * xC[1] - 0.163 * xC[2] * xC[7] +
                0.001232 * xC[2] * xC[9] - 0.166 * xC[6] * xC[8] +
                0.227 * xC[1] * xC[1];
  if (directFnASV[7] & 1)
    fnVals[7] = 4.72 - 0.5 * xC[3] - 0.19 * xC[1] * xC[2] -
                0.0122 * xC[3] * xC[9] + 0.009325 * xC[5] * xC[9] +
                0.000191 * xC[10] * xC[10];
  if (directFnASV[8] & 1)
    fnVals[8] = 10.58 - 0.674 * xC[0] * xC[1] - 1.95 * xC[1] * xC[7] +
                0.02054 * xC[2] * xC[9] - 0.0198 * xC[3] * xC[9] +
                0.028 * xC[5] * xC[9];
  if (directFnASV[9] & 1)
  ...
  if (hess_flag)
    Cerr << "Error: Hessians not currently supported in side_impact_perf()."
         << std::endl;
  if (grad_flag || hess_flag) abort_handler(INTERFACE_ERROR);
...
int TestDriverInterface::steel_column_cost() {
  ...
  if (directFnASV[0] & 1) fnVals[0] = b * d + 5. * h;
...
int TestDriverInterface::steel_column_perf() {
  ...
  Real F0 = xCM[VAR_F0], B = xCM[VAR_B], D = xCM[VAR_D], H = xCM[VAR_H],
       Fs = xCM[VAR_Fs], E = xCM[VAR_E],
       P = xCM[VAR_P1] + xCM[VAR_P2] + xCM[VAR_P3], Pi = 3.1415926535897932385,
       Pi2 = Pi * Pi, Pi4 = Pi2 * Pi2, Pi6 = Pi2 * Pi4, B2 = B * B, D2 = D * D,
       H2 = H * H, H3 = H * H2, H5 = H2 * H3, E2 = E * E, E3 = E * E2,
       s2 = s * s, X = Pi2 * E * B * D * H2 - 2. * s2 * P, X2 = X * X,
       X3 = X * X2;
  ...
  if (directFnASV[0] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_F0:  // df/dF0
          fnGrads[0][i] = -E * H * P * Pi2 / X;
          break;
        case VAR_P1:
        case VAR_P2:
        case VAR_P3:  // df/dP1, df/dP2, df/dP3
          fnGrads[0][i] = -1. / 2. / B / D - B * D * E2 * F0 * H3 * Pi4 / X2;
          break;
        case VAR_B:  // df/dB
          fnGrads[0][i] = P * (1. / 2. / B2 / D + D * E2 * F0 * H3 * Pi4 / X2);
          break;
        case VAR_D:  // df/dD
          fnGrads[0][i] = P * (1. / 2. / B / D2 + B * E2 * F0 * H3 * Pi4 / X2);
          break;
        case VAR_H:  // df/dH
          fnGrads[0][i] = E * F0 * P * Pi2 * (X + 4. * P * s2) / X2;
          break;
        case VAR_Fs:  // df/dFs
          fnGrads[0][i] = 1.;
          break;
        case VAR_E:  // df/dE
          fnGrads[0][i] = 2. * F0 * H * P * P * Pi2 * s2 / X2;
          break;
      }
    ...
    for (size_t i = 0; i < numDerivVars; ++i)
      for (size_t j = 0; j <= i; ++j)
        if (varTypeDVV[i] == VAR_Fs || varTypeDVV[j] == VAR_Fs)
          ...
          fnHessians[0](i, j) =
              -P * (1. / B2 / B / D + 2. * D2 * E3 * F0 * H5 * Pi6 / X3);
          ...
          fnHessians[0](i, j) =
              -2. * B * D * E2 * F0 * H * P * Pi4 * (X + 8. * P * s2) / X3;
...
int TestDriverInterface::sobol_rational() {
  ...
  // **** df/dx:
  if (directFnASV[0] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (directFnDVV[i]) {
        case 1:  // x1
  ...
  if (directFnASV[0] & 2) {
    for (size_t i = 0; i < numDerivVars; ++i) {
      ...
      if (4. * xC[var_index] == 2.)  // zero gradient assumed at discontinuity
        ...
        fn_grad_0i = (4. * xC[var_index] > 2.) ? 8. / (1. + a[var_index])
                                               : -8. / (1. + a[var_index]);
...
int TestDriverInterface::sobol_ishigami() {
  ...
  if (directFnASV[0] & 1)
    fnVals[0] =
        (1. + 0.1 * pow(2. * pi * x3 - pi, 4.0)) * sin(2. * pi * x1 - pi) +
        7. * pow(sin(2 * pi * x2 - pi), 2.0);

  // **** df/dx:
  if (directFnASV[0] & 2)
    for (size_t i = 0; i < numDerivVars; ++i) switch (varTypeDVV[i]) {
        case VAR_x1:
          fnGrads[0][i] = 2. * pi * (1. + 0.1 * pow(2. * pi * x3 - pi, 4.)) *
                          cos(2. * pi * x1 - pi);
          break;
        case VAR_x2:
          fnGrads[0][i] =
              28. * pi * sin(2. * pi * x2 - pi) * cos(2. * pi * x2 - pi);
          break;
        case VAR_x3:
          fnGrads[0][i] =
              0.8 * pow(2. * pi * x3 - pi, 3.) * sin(2. * pi * x1 - pi);
          break;
      }
...
int TestDriverInterface::text_book() {
  ...
  if (numFns > 1) text_book2();  // constraint 1 val/grad/Hessian
  if (numFns > 2) text_book3();  // constraint 2 val/grad/Hessian
...
int TestDriverInterface::text_book1() {
  ...
  if (directFnASV[0] & 1) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      if (analysisCommRank == 0) fnVals[0] = global_val;
    } else
      fnVals[0] = local_val;
  }
  ...
  if (directFnASV[0] & 2) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      parallelLib.reduce_sum_a((Real*)fnGrads[0], sum_fns, numDerivVars);
...
int TestDriverInterface::text_book2() {
  ...
  if (directFnASV[1] & 1) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      // evalCommRank 0 in overlay_response.  evalCommRank 0 then returns the
      // results to the iterator in ApplicationInterface::serve_evaluations().
      if (analysisCommRank == 0) fnVals[1] = global_val;
    } else
      fnVals[1] = local_val;
  }
  ...
  if (directFnASV[1] & 2) {
    ...
    if (multiProcAnalysisFlag) {
      Real* sum_fns = (analysisCommRank) ? NULL : new Real[numDerivVars];
      parallelLib.reduce_sum_a((Real*)fnGrads[1], sum_fns, numDerivVars);
      if (analysisCommRank == 0) {
  ...
  if (directFnASV[1] & 4) {
    ...
    for (i = analysisCommRank; i < numDerivVars; i += analysisCommSize) {
      size_t var_index = directFnDVV[i] - 1;  // assumes no discrete vars
      if (var_index == 0) fnHessians[1](i, i) = 2.;
#ifdef TB_EXPENSIVE
      Real x_i = xC[var_index];
      for (size_t j = 0; j < numDerivVars; ++j)
        for (size_t k = 1; k <= 5000; ++k)
...
int TestDriverInterface::text_book3() {
  ...
  if (directFnASV[2] & 1) {
    ...
    if (multiProcAnalysisFlag) {
      ...
      // evalCommRank 0 in overlay_response.  evalCommRank 0 then returns the
      // results to the iterator in ApplicationInterface::serve_evaluations().
      if (analysisCommRank == 0) fnVals[2] = global_val;
    } else
      fnVals[2] = local_val;
  }
  ...
  if (directFnASV[2] & 2) {
    ...
    if (multiProcAnalysisFlag) {
      Real* sum_fns = (analysisCommRank) ? NULL : new Real[numDerivVars];
      parallelLib.reduce_sum_a((Real*)fnGrads[2], sum_fns, numDerivVars);
      if (analysisCommRank == 0) {
  ...
  if (directFnASV[2] & 4) {
    ...
    for (i = analysisCommRank; i < numDerivVars; i += analysisCommSize) {
      size_t var_index = directFnDVV[i] - 1;  // assumes no discrete vars
      if (var_index == 1) fnHessians[2](i, i) = 2.;
#ifdef TB_EXPENSIVE
      Real x_i = xC[var_index];
      for (size_t j = 0; j < numDerivVars; ++j)
        for (size_t k = 1; k <= 5000; ++k)
...
int TestDriverInterface::text_book_ouu() {
  ...
  size_t i,
      split = 2 + (numVars - 2) / 2;  // split the uncertain vars among d1,d2
  // xC[0], xC[1]     = design
  // xC[2], xC[3],... = uncertain
  ...
  if (directFnASV[0] & 1) {
    Real f = 0.;
    for (i = 2; i < split; ++i) f += std::pow(xC[i] - 10. * xC[0], 4.0);
    for (i = split; i < numVars; ++i) f += std::pow(xC[i] - 10. * xC[1], 4.0);
    fnVals[0] = f;
  }
  ...
  // **** df/dx:
  if (directFnASV[0] & 2)
    for (i = 0; i < numDerivVars; ++i) switch (directFnDVV[i]) {
        ...
        default: {  // uncertain variable derivative
          ...
          fnGrads[0][i] = (var_index < split)
                              ? 4. * std::pow(xCvi - 10. * xC[0], 3)
  ...
  if (numFns > 1 && (directFnASV[1] & 2))
    for (i = 0; i < numDerivVars; ++i) switch (directFnDVV[i]) {
        case 1:  // design variable derivative
          fnGrads[1][i] = xC[2] * xC[2] - 0.5 * xC[3];
          break;
        case 3:  // uncertain variable derivative
          fnGrads[1][i] = 2 * xC[0] * xC[2];
          break;
        case 4:  // uncertain variable derivative
          fnGrads[1][i] = -0.5 * xC[0];
          break;
  ...
  if (numFns > 2 && (directFnASV[2] & 2))
    for (i = 0; i < numDerivVars; ++i) switch (directFnDVV[i]) {
        case 2:  // design variable derivative
          fnGrads[2][i] = xC[3] * xC[3] - 0.5 * xC[2];
          break;
        case 3:  // uncertain variable derivative
          fnGrads[2][i] = -0.5 * xC[1];
          break;
        case 4:  // uncertain variable derivative
          fnGrads[2][i] = 2 * xC[1] * xC[3];
          break;
...
int TestDriverInterface::scalable_text_book() {
  ...
  if (directFnASV[0] & 1) {
    fnVals[0] = 0.;
    for (i = 0; i < numACV; ++i) fnVals[0] += std::pow(xC[i] - POW_VAL, 4);
  }
...
int TestDriverInterface::scalable_monomials() {
  ...
  if (directFnASV[0] & 1) {
    fnVals[0] = 0.;
    for (size_t i = 0; i < numACV; ++i) fnVals[0] += std::pow(xC[i], power);
  }
    ...
    for (size_t i = 0; i < numDerivVars; ++i) {
      ...
      switch (power) {
        case 0:
          ...
          break;
        default:
          fnGrads[0][i] = power * std::pow(xC[var_index], power - 1);
          break;
      }
    }
  }
  ...
  if (directFnASV[0] & 4) {
    ...
    for (size_t i = 0; i < numDerivVars; ++i) {
      ...
      switch (power) {
        ...
        default:
          fnHessians[0](i, i) =
              power * (power - 1) * std::pow(xC[var_index], power - 2);
          break;
      }
    }
  }
...
int TestDriverInterface::mogatest1() {
  ...
  if (directFnASV[0] & 1) fnVals[0] = f0;
  if (directFnASV[1] & 1) fnVals[1] = f1;
  ...
  if (directFnASV[0] & 4 || directFnASV[1] & 4) {
    Cerr << "Error: Analytic Hessians not supported in mogatest1." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
...
int TestDriverInterface::mogatest2() {
  ...
  if (directFnASV[0] & 4 || directFnASV[1] & 4) {
    Cerr << "Error: Analytic Hessians not supported in mogatest2." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
...
int TestDriverInterface::mogatest3() {
  ...
  if (directFnASV[0] & 1) fnVals[0] = f0;
  if (directFnASV[1] & 1) fnVals[1] = f1;
  if (directFnASV[2] & 1) fnVals[2] = g0;
  if (directFnASV[3] & 1) fnVals[3] = g1;
  if (directFnASV[0] & 2 || directFnASV[1] & 2 || directFnASV[2] & 2 ||
      directFnASV[3] & 2) {
    Cerr << "Error: Analytic gradients not supported in mogatest3."
         << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (directFnASV[0] & 4 || directFnASV[1] & 4 || directFnASV[2] & 4 ||
      directFnASV[3] & 4) {
    Cerr << "Error: Analytic Hessians not supported in mogatest3." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
...
int TestDriverInterface::illumination() {
  ...
  size_t Ider, Jder;
  double grad[7];
  for (Ider = 0; Ider < num_vars; ++Ider) grad[Ider] = 0.0;

  // **** f: (f is required to calculate any derivative of f; perform always)
  double U = 0.0;
  for (size_t i = 0; i < 11; i++) {
    double dtmp = 0.0;
    for (size_t j = 0; j < num_vars; j++) dtmp += A[i][j] * xC[j];
    dtmp = 1.0 - dtmp;
    U += dtmp * dtmp;
  ...
  double fx = sqrt(U);
  if (directFnASV[0] & 1) fnVals[0] = fx;

  // **** df/dx:
  if (directFnASV[0] &
      6) {  // gradient required for itself and to calcuate the Hessian
    for (Ider = 0; Ider < num_vars; ++Ider)
      grad[Ider] *= (0.5 / fx);  // this is now updated to be grad(f)

    if (directFnASV[0] & 2) {  // if ASV demands gradient, print it
      for (int Ider = 0; Ider < num_vars; ++Ider) fnGrads[0][Ider] = grad[Ider];
    }
  }
  ...
  if (directFnASV[0] & 4) {
    for (Ider = 0; Ider < num_vars; ++Ider) {
      for (Jder = Ider; Jder < num_vars; ++Jder) {
        ...
        for (size_t i = 0; i < 11; ++i)
          fnHessians[0](Ider, Jder) +=
              A[i][Ider] * A[i][Jder];  // this is 0.5*hess(U)

        fnHessians[0](Jder, Ider) =
            fnHessians[0](Ider, Jder)  // this is hess(f)
...
int TestDriverInterface::barnes() {
  ...
  if (numACV < 2 || numACV > 23) {
    Cerr << "Error: Bad number of variables in barnes direct fn." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }

  if (numFns != 4) {
    Cerr << "Error: Bad number of functions in barnes direct fn." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  ...
  double a[] = {75.196,  -3.8112,    0.12694,     -2.0567e-3, 1.0345e-5,
                ...
                0.0005};
  double x1 = xC[0], x2 = xC[1], x1x2 = x1 * x2, x2_sq = x2 * x2,
         x1_sq = x1 * x1;
  ...
  size_t i, cntr;
  for (i = 23 - numACV, cntr = 2; i < 21; ++i, ++cntr) a[i] = xC[cntr];

  // **** f
  if (directFnASV[0] & 1) {
    double f = a[0] + a[1] * x1 + a[2] * x1_sq + a[3] * x1_sq * x1 +
               a[4] * x1_sq * x1_sq + a[5] * x2 + a[6] * x1x2 +
               a[7] * x1 * x1x2 + a[8] * x1x2 * x1_sq +
               a[9] * x2 * x1_sq * x1_sq + a[10] * x2_sq + a[11] * x2 * x2_sq +
               a[12] * x2_sq * x2_sq + a[13] / (x2 + 1.) +
               a[14] * x2_sq * x1_sq + a[15] * x1 * x1_sq * x2_sq +
               a[16] * x1x2 * x2_sq * x1_sq + a[17] * x1 * x2_sq +
               a[18] * x1x2 * x2_sq + a[19] * exp(a[20] * x1x2);
    fnVals[0] = f;
  }

  // **** g1
  if (directFnASV[1] & 1) fnVals[1] = x1x2 / 700. - 1.;

  // **** g2
  if (directFnASV[2] & 1) fnVals[2] = x2 / 5. - x1_sq / 625.;

  // **** g3
  if (directFnASV[3] & 1) fnVals[3] = pow(x2 / 50. - 1., 2.) - x1 / 500. + 0.11;
  ...
  if (directFnASV[0] & 2) {
    for (size_t i = 0; i < numDerivVars; i++) {
      ...
      switch (var_index) {
        case 0:
          fnGrads[0][i] =
              a[1] + 2. * a[2] * x1 + 3. * a[3] * x1_sq +
              4. * a[4] * x1_sq * x1 + a[6] * x2 + 2. * a[7] * x1x2 +
              3. * a[8] * x2 * x1_sq + 4. * a[9] * x1x2 * x1_sq +
              2. * a[14] * x2_sq * x1 + 3. * a[15] * x1_sq * x2_sq +
              3. * a[16] * x2 * x2_sq * x1_sq + a[17] * x2_sq +
              a[18] * x2 * x2_sq + a[19] * a[20] * x2 * exp(a[20] * x1x2);
          break;
        case 1:
          fnGrads[0][i] =
              a[5] + a[6] * x1 + a[7] * x1_sq + a[8] * x1 * x1_sq +
              a[9] * x1_sq * x1_sq + 2. * a[10] * x2 + 3. * a[11] * x2_sq +
              4. * a[12] * x2 * x2_sq - a[13] / pow(x2 + 1., 2.) +
              2. * a[14] * x2 * x1_sq + 2. * a[15] * x1 * x1_sq * x2 +
              3. * a[16] * x1 * x2_sq * x1_sq + 2. * a[17] * x1x2 +
              3. * a[18] * x1 * x2_sq + a[19] * a[20] * x1 * exp(a[20] * x1x2);
          break;
      }
    }
  }
...
int TestDriverInterface::barnes_lf() {
  ...
  // **** f
  if (directFnASV[0] & 1) {
    fnVals[0] = -2.74465943148169 + 0.01213957527281 * x1 +
                0.00995748775273 * x12 - 5.557060816484793e-04 * x13 +
                (1.15084419109172 + 0.00947331101091 * x1 +
                 2.994070392732408e-05 * x12) *
                    x2 +
  ...
  // **** g1
  if (directFnASV[1] & 1) fnVals[1] = (xC[0] + xC[1] - 50.) / 10.;

  // **** g2
  if (directFnASV[2] & 1) fnVals[2] = (-0.64 * xC[0] + xC[1]) / 6.;

  // **** g3
  if (directFnASV[3] & 1) {
    if (xC[1] > 50)
      fnVals[3] = -0.00599508167546 * xC[0] + 0.0134054101569 * xC[1] -
                  0.34054101569933;
    else
      fnVals[3] = -0.00599508167546 * xC[0] - 0.01340541015699 * xC[1] + 1.;
  }
  ...
  if (directFnASV[0] & 2) {
    for (size_t i = 0; i < numDerivVars; i++) {
      ...
      switch (var_index) {
        case 0:
          fnGrads[0][i] =
              -0.58530968989099 + 0.01991497550546 * xC[0] -
              0.00166711824495 * x12 +
              (0.00767686877527 + 5.988140785464816e-05 * xC[0]) * x2 -
              1.676054720545071e-04 * x22;
          break;
        case 1:
          fnGrads[0][i] =
              0.86664486076442 + 0.00947331101091 * xC[0] +
              2.994070392732408e-05 * x12 +
...
void TestDriverInterface::herbie1D(size_t der_mode, Real xc_loc,
  ...
  if (der_mode &
      1)  // 1=2^0: the 0th derivative of the response (the response itself)
    w_and_ders[0] = std::exp(-rtemp1_sq) + std::exp(-0.8 * rtemp2_sq) -
                    0.05 * std::sin(rtemp3);
  if (der_mode & 2)  // 2=2^1: the 1st derivative of the response
    w_and_ders[1] = -2.0 * rtemp1 * std::exp(-rtemp1_sq) -
                    1.6 * rtemp2 * std::exp(-0.8 * rtemp2_sq) -
                    0.4 * std::cos(rtemp3);
  if (der_mode & 4)  // 4=2^2: the 2nd derivative of the response
    w_and_ders[2] = (-2.0 + 4.0 * rtemp1_sq) * std::exp(-rtemp1_sq) +
...
/// 1D Smoothed Herbie= 1DHerbie minus the high frequency sine term, and its
/// derivatives (apart from a multiplicative factor)
void TestDriverInterface::smooth_herbie1D(size_t der_mode, Real xc_loc,
                                          std::vector<Real>& w_and_ders) {
  w_and_ders[0] = w_and_ders[1] = w_and_ders[2] = 0.0;
  ...
  if (der_mode & 4)  // 4=2^2: the 2nd derivative of the response
    w_and_ders[2] = (-2.0 + 4.0 * rtemp1_sq) * std::exp(-rtemp1_sq) +
                    (-1.6 + 2.56 * rtemp2_sq) * std::exp(-0.8 * rtemp2_sq);
  if (der_mode > 7) {
    Cerr << "only 0th through 2nd derivatives are implemented for "
            "smooth_herbie1D()\n";
    assert(false);  // should throw an exception get brian to help
  }
}
...
void TestDriverInterface::shubert1D(size_t der_mode, Real xc_loc,
                                    std::vector<Real>& w_and_ders) {
  w_and_ders[0] = w_and_ders[1] = w_and_ders[2] = 0.0;
  ...
  if (der_mode & 2) {
    for (k = 1; k <= 5; ++k) {
      k_real = static_cast<Real>(k);
      w_and_ders[1] +=
          k_real * (k_real + 1.0) * -std::sin(xc_loc * (k_real + 1.0) + k_real);
    }
  }
  if (der_mode & 4) {
    for (k = 1; k <= 5; ++k) {
      k_real = static_cast<Real>(k);
      w_and_ders[2] += k_real * (k_real + 1.0) * (k_real + 1.0) *
                       -std::cos(xc_loc * (k_real + 1.0) + k_real);
    }
  }
  if (der_mode > 7) {
    Cerr
        << "only 0th through 2nd derivatives are implemented for shubert1D()\n";
    assert(false);  // should throw an exception get brian to help
  }
}
...
int TestDriverInterface::herbie() {
  ...
  for (i = 0; i < numVars; ++i) der_mode[i] = 1;
  if (directFnASV[0] >= 2)
    for (i = 0; i < numDerivVars; ++i) der_mode[directFnDVV[i] - 1] += 2;
  if (directFnASV[0] >= 4)
    for (i = 0; i < numDerivVars; ++i) der_mode[directFnDVV[i] - 1] += 4;
...
int TestDriverInterface::bayes_linear() {
  ...
  if (numVars < 1 || numVars > 500 || numADIV || numADRV) {
    Cerr << "Error: Bad variable types in Bayes linear fn." << std::endl;
    abort_handler(INTERFACE_ERROR);
  }
  if (numFns < 1) {
  ...
  RealVector temp_cvars = Teuchos::getCol(Teuchos::View,
  const_cast<RealMatrix&>(lhs_samples), 0);
  ...
  */
  ...
  Real pred = 0.0;
  for (int i = 0; i < numVars; i++) pred += xC[i];
  fnVals[0] = pred;
...
int TestDriverInterface::problem18() {
  ...
  Real Af = (Af_iter == xDRM.end())
                ? 1.
                : Af_iter->second;  // Correlation Af for objective

  std::map<var_t, Real>::iterator Ac_iter = xDRM.find(VAR_Ac);
  Real Ac = (Ac_iter == xDRM.end())
                ? 1.
                : Ac_iter->second;  // Correlation Ac for constraint
...
double TestDriverInterface::problem18_f(const double& x) {
  return x <= 3. ? (x - 2.) * (x - 2.) : 2. * std::log(x - 2.) + 1;
}

double TestDriverInterface::problem18_g(const double& x) {
  return (2. * std::log(3.5 - 2)) / (3.5 - 1) * x + 0 -
         (2. * std::log(3.5 - 2)) / (3.5 - 1);
}
...
/// this function combines N 1D functions and their derivatives to compute a N-D
/// separable function and its derivatives, logic is general enough to support
/// different 1D functions in different dimensions (can mix and match)
void TestDriverInterface::separable_combine(Real mult_scale_factor,
                                            std::vector<Real>& w,
                                            std::vector<Real>& d1w,
                                            std::vector<Real>& d2w) {
  // *************************************************************
  ...
  if (directFnASV[0] & 1) {
    ...
    for (i = 0; i < numVars; ++i) local_val *= w[i];
    fnVals[0] = local_val;
  }
  ...
  if (directFnASV[0] & 2) {
    ...
    for (i = 0; i < numDerivVars; ++i) {
      ...
      for (j = 0; j < i_var_index; ++j) local_val *= w[j];
      for (j = i_var_index + 1; j < numVars; ++j) local_val *= w[j];
  ...
  if (directFnASV[0] & 4) {
    ...
    for (i = 0; i < numDerivVars; ++i) {
      ...
      for (j = 0; j < numDerivVars; ++j) {
        ...
        for (k = 0; k < numVars; ++k)
          if ((k != i_var_index) && (k != j_var_index)) local_val *= w[k];
...
Real TestDriverInterface::levenshtein_distance(const String& v) {
  ...
  if (d_match != levenshteinDistanceCache.end()) return d_match->second;
  const String& w = LEV_REF;
  size_t v_len = v.size(), w_len = w.size();
  IntMatrix d(v_len + 1, w_len + 1);
  size_t i, j;
  for (i = 0; i <= v_len; ++i) d(i, 0) = i;
  for (j = 0; j <= w_len; ++j) d(0, j) = j;
  for (j = 1; j <= w_len; ++j) {
    for (i = 1; i <= v_len; ++i) {
      if (v[i - 1] == w[j - 1])
        d(i, j) = d(i - 1, j - 1);
...
int TestDriverInterface::salinas() {
  ...
  while (fin) {
    ...
    if (token == "//")
      ...
      char se[32];
      std::sprintf(se, "'./salinas%d.exo' ",
                   evalServerId);  // tag root in root.exo
      fout << se;
  ...
  // Real min_margin = 0.;
  Real lambda1, mass, mass_low_bnd = 3.,
                      mass_upp_bnd = 6.;  // 4.608 nominal mass
  ...
  while (f2in) {
    ...
    if (token == "Total") {
      for (size_t i = 0; i < 4;
           ++i)  // After "Total", parse "Mass of Structure is"
        f2in >> token;
      f2in >> mass;
    } else if (token == "1:") {
      f2in >> lambda1;
...
int TestDriverInterface::aniso_quad_form() {
  ...
  if (!initialized) {
    ...
    for (int i = 0; i < num_prin_direc; ++i) {
      ...
      for (int j = i + 1; j < num_prin_direc; ++j) {
        RealVector::scalarType proj_val =
            getCol(Teuchos::View, prin_vecs, j).dot(prev_ortho_vec);

        RealVector prev_ortho_vec_copy(Teuchos::Copy, prev_ortho_vec.values(),
  ...
  quad_prod.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0, q_mat, xC, 0.0);
