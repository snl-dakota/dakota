...
NonDBayesCalibration::NonDBayesCalibration(ProblemDescDB& problem_db,
    ...
    : NonDCalibration(problem_db, parallel_lib, model),
      ...
      scalarDataFilename(
          probDescDB.get_string("responses.scalar_data_filename")),
      importCandPtsFile(
          probDescDB.get_string("method.import_candidate_points_file")),
      importCandFormat(probDescDB.get_ushort("method.import_candidate_format")),
      ...
      importPredConfigs(
          probDescDB.get_string("method.import_prediction_configs")),
      ...
      exportCorrVarFormat(probDescDB.get_ushort(
          "method.nond.export_corrected_variance_format")),
      discrepPolyOrder(probDescDB.get_short(
      ...
      posteriorStatsKL(
          probDescDB.get_bool("method.posterior_stats.kl_divergence")),
      ...
      chainDiagnosticsCI(
          probDescDB.get_bool("method.chain_diagnostics.confidence_intervals")),
  ...
  switch (emulatorType) {
    ...
    case SC_EMULATOR:
      ...
      break;
    case MF_PCE_EMULATOR:
    case ML_PCE_EMULATOR:
    case MF_SC_EMULATOR:
      standardizedSpace = true;  // nataf defined w/i ProbTransformModel
  ...
  if (ensemble_model) iteratedModel->surrogate_response_mode(mode);
  ...
  if (calibrationData) {
    residualModel = std::make_shared<DataTransformModel>(
        mcmcModel, expData, orig_view, numHyperparams, obsErrorMultiplierMode,
        mcmcDerivOrder);
  ...
  else  // convert back from surrogate view to iteratedModel view for use in
        // MCMC
  ...
  // Cout << "\n  iteratedModel num cv = " << ModelUtils::cv(iteratedModel) << "
  // mvd active = " <<
  // iteratedModel.multivariate_distribution().active_variables().count()
  //      << "\n  mcmcModel     num cv = " << ModelUtils::cv(mcmcModel) << " mvd
  //      active = " <<
  //      mcmcModel->multivariate_distribution().active_variables().count()
  //      << "\n  residualModel num cv = " << ModelUtils::cv(residualModel) << "
  //      mvd active = " <<
  //      residualModel->multivariate_distribution().active_variables().count()
  //      << std::endl;
  // if (mapOptAlgOverride != SUBMETHOD_NONE)
  //   Cout << "\n  negLogPostModel num cv = "<<
  //   ModelUtils::cv(negLogPostModel)<<std::endl;
  ...
  switch (emulatorType) {
    ...
    case MF_SC_EMULATOR: {
      ...
      short refine_type =
            ...
            refine_cntl = probDescDB.get_short(
      ...
      if (emulatorType == SC_EMULATOR) {  // SC sparse grid interpolation
        ...
        if (ssg_level != USHRT_MAX) {
          ...
          stochExpIterator = std::make_shared<NonDStochCollocation>(
              inbound_model, exp_coeff_approach, ssg_level, dim_pref,
              u_space_type, refine_type, refine_cntl, cov_cntl, rule_nest,
              rule_growth, pw_basis, use_derivs);
        } else if (tpq_order != USHRT_MAX)
          stochExpIterator = std::make_shared<NonDStochCollocation>(
              inbound_model, Pecos::QUADRATURE, tpq_order, dim_pref,
              u_space_type, refine_type, refine_cntl, cov_cntl, rule_nest,
              rule_growth, pw_basis, use_derivs);
        ...
        if (!exp_import_file.empty()) {
          ...
          stochExpIterator = std::make_shared<NonDPolynomialChaos>(
              inbound_model, exp_import_file, u_space_type,
              approx_view);                   // no export since imported
        } else if (ssg_level != USHRT_MAX) {  // PCE sparse grid
          short exp_coeff_approach = (refine_cntl)
                                         ? Pecos::INCREMENTAL_SPARSE_GRID
                                         : Pecos::COMBINED_SPARSE_GRID;
          stochExpIterator = std::make_shared<NonDPolynomialChaos>(
              inbound_model, exp_coeff_approach, ssg_level, dim_pref,
              u_space_type, refine_type, refine_cntl, cov_cntl, rule_nest,
              rule_growth, pw_basis, use_derivs, exp_export_file);
        } else if (tpq_order != USHRT_MAX)
          stochExpIterator = std::make_shared<NonDPolynomialChaos>(
              inbound_model, Pecos::QUADRATURE, tpq_order, dim_pref,
              u_space_type, refine_type, refine_cntl, cov_cntl, rule_nest,
          ...
          stochExpIterator = std::make_shared<NonDPolynomialChaos>(
              inbound_model, Pecos::CUBATURE, cub_int, dim_pref, u_space_type,
              refine_type, refine_cntl, cov_cntl, rule_nest, rule_growth,
              pw_basis, use_derivs, exp_export_file);
          ...
          stochExpIterator = std::make_shared<NonDPolynomialChaos>(
              ...
              probDescDB.get_real(
                  "method.nond.collocation_ratio"),  // single scalar
        ...
        short ml_alloc_cntl = probDescDB.get_short(
                  "method.nond.multilevel_allocation_control"),
              ml_discrep = probDescDB.get_short(
        ...
        if (!ssg_level_seq.empty()) {
          ...
          if (probDescDB.get_short("method.nond.expansion_basis_type") ==
            ...
            exp_coeff_approach = Pecos::INCREMENTAL_SPARSE_GRID;
          stochExpIterator = std::make_shared<NonDMultilevelStochCollocation>(
              inbound_model, exp_coeff_approach, ssg_level_seq, dim_pref,
              u_space_type, refine_type, refine_cntl, cov_cntl, ml_alloc_cntl,
              ml_discrep, rule_nest, rule_growth, pw_basis, use_derivs);
        } else if (!tpq_order_seq.empty())
          stochExpIterator = std::make_shared<NonDMultilevelStochCollocation>(
              inbound_model, Pecos::QUADRATURE, tpq_order_seq, dim_pref,
              u_space_type, refine_type, refine_cntl, cov_cntl, ml_alloc_cntl,
              ml_discrep, rule_nest, rule_growth, pw_basis, use_derivs);
        mcmcDerivOrder = 3;  // Hessian computations not yet implemented for SC
      }
      ...
      stochExpIterator->requested_levels(
          empty_rv_array, empty_rv_array, empty_rv_array, empty_rv_array,
          respLevelTarget, respLevelTargetReduce, cdfFlag, false);
    ...
    case KRIGING_EMULATOR: {
      ...
      // these purposes, but +/-3 sigma has little to no effect in current
      // tests.
      ...
      lhs_iterator = std::make_shared<NonDLHSSampling>(
      ...
      mcmcModel = std::make_shared<DataFitSurrModel>(
          lhs_iterator, lhs_model, gp_set, gp_view, approx_type, approx_order,
          corr_type, corr_order, data_order, outputLevel, sample_reuse,
    ...
    case NO_EMULATOR:
      ...
      if (standardizedSpace)
        mcmcModel =
            std::make_shared<ProbabilityTransformModel>(inbound_model, ASKEY_U);
      else
...
void NonDBayesCalibration::construct_map_model() {
  ...
  negLogPostModel = std::make_shared<RecastModel>(
      residualModel, vars_map_indices, recast_vc_totals, all_relax_di,
      all_relax_dr, nonlinear_vars_map,
      iteratedModel->current_variables().view(), nullptr, set_recast,
      primary_resp_map_indices, secondary_resp_map_indices, 0, nlp_resp_order,
...
void NonDBayesCalibration::construct_map_optimizer() {
  ...
  switch (mapOptAlgOverride) {
    ...
    case SUBMETHOD_OPTPP:
      ...
      mapOptimizer =
          std::make_shared<SNLLOptimizer>("optpp_newton", negLogPostModel);
      break;
#endif
  }
}
...
void NonDBayesCalibration::derived_init_communicators(ParLevLIter pl_iter) {
  ...
  switch (emulatorType) {
    ...
    case MF_PCE_EMULATOR:
    case MF_SC_EMULATOR:
      stochExpIterator->init_communicators(pl_iter);
      break;
      // default:
  ...
  if (mapOptimizer) mapOptimizer->init_communicators(pl_iter);

  if (hifiSampler) hifiSampler->init_communicators(pl_iter);
}
...
void NonDBayesCalibration::derived_set_communicators(ParLevLIter pl_iter) {
  ...
  switch (emulatorType) {
    ...
    case MF_PCE_EMULATOR:
    case MF_SC_EMULATOR:
      stochExpIterator->set_communicators(pl_iter);
      break;
      // default:
  ...
  if (mapOptimizer) mapOptimizer->set_communicators(pl_iter);

  if (hifiSampler) hifiSampler->set_communicators(pl_iter);
}

void NonDBayesCalibration::derived_free_communicators(ParLevLIter pl_iter) {
  if (hifiSampler) hifiSampler->free_communicators(pl_iter);

  if (mapOptimizer) mapOptimizer->free_communicators(pl_iter);
  ...
  switch (emulatorType) {
    ...
    case MF_PCE_EMULATOR:
    case MF_SC_EMULATOR:
      stochExpIterator->free_communicators(pl_iter);
      break;
      // default:
...
void NonDBayesCalibration::best_to_all() {
  ...
  std::/*multi*/ map<Real, RealVector>::const_iterator bs_it =
                                                           bestSamples.begin(),
                                                       bs_end =
                                                           bestSamples.end();
...
void NonDBayesCalibration::update_model() {
  ...
  switch (emulatorType) {
    ...
    case KRIGING_EMULATOR:
      if (standardizedSpace)
        nondInstance = (NonD*)mcmcModel->subordinate_iterator().get();
      evaluate_parameter_sets(*mcmcModel);
      if (standardizedSpace) nondInstance = this;  // restore
      break;
  }
  ...
  switch (emulatorType) {
    ...
    case MF_SC_EMULATOR: {
      ...
      // (more explicit than embedded logic w/i
      // mcmcModel->append_approximation).
    ...
    case GP_EMULATOR:
    case KRIGING_EMULATOR:
      mcmcModel->append_approximation(allSamples, allResponses,
                                      true);  // rebuild
      break;
  }
}
...
Real NonDBayesCalibration::assess_emulator_convergence() {
  ...
  if (prevCoeffs.empty()) {
    switch (emulatorType) {
      ...
      case ML_PCE_EMULATOR:
      case MF_PCE_EMULATOR:
        prevCoeffs = mcmcModel->approximation_coefficients(true);
        break;
      case SC_EMULATOR:
      case MF_SC_EMULATOR:
        prevCoeffs = mcmcModel->approximation_coefficients(false);
        break;
      case GP_EMULATOR:
      case KRIGING_EMULATOR:
  ...
  switch (emulatorType) {
    ...
    case MF_PCE_EMULATOR: {
      ...
      const RealVectorArray& coeffs =
          mcmcModel->approximation_coefficients(true);
      size_t i, j, num_qoi = coeffs.size(), num_curr_coeffs, num_prev_coeffs,
                   num_coeffs;

      // This approach assumes a well-ordered progression in multiIndex, which
      // is acceptable for regression PCE using consistently incremented
      // (candidate) expansion definitions.  Sparsity is not a concern as
      // returned coeffs are inflated to be dense w.r.t.
      // SharedOrthogPolyApproxData::multiIndex. Could implement as resize
      // (inflat smaller w/ 0's) + vector difference + Frobenious norm, but
      // current approach should have lower overhead.
      for (i = 0; i < num_qoi; ++i) {
    ...
    case MF_SC_EMULATOR: {
      ...
      const RealVectorArray& coeffs =
          mcmcModel->approximation_coefficients(false);
...
void NonDBayesCalibration::calibrate_to_hifi() {
  ...
  */
  ...
  // TODO? Make a struct?
  const RealVector initial_pt(
      Teuchos::Copy, ModelUtils::continuous_variables(*mcmcModel).values(),
  ...
  int num_design_vars =
      ModelUtils::cv(*hifiModel) + ModelUtils::div(*hifiModel) +
      ModelUtils::dsv(*hifiModel) + ModelUtils::drv(*hifiModel);
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    for (size_t i = 0; i < initHifiSamples;
         i++)  // TNP ? Why is this not num_exp?
  ...
  while (!stop_metric) {
    eval_hi2lo_stop(stop_metric, prev_MI, MI_vec, num_hifi, max_hifi,
                    design_matrix.size());
    ...
    if (!stop_metric || max_hifi == 0) {
      ...
      choose_batch_from_mutual_info(random_seed, num_it, max_hifi, num_hifi,
                                    mi_chain, design_matrix,
                                    optimal_config_matrix, MI_vec);
      ...
      print_hi2lo_file(out_file, num_it, optimal_config_matrix, MI_vec,
                       resp_matrix);
    }  // end MI loop
  }  // end while loop
}
...
void NonDBayesCalibration::print_hi2lo_chain_moments() {
  if (outputLevel < DEBUG_OUTPUT) return;
  ...
  NonDSampling::print_moments(Cout, chainStats, RealMatrix(),
                              "posterior variable", Pecos::STANDARD_MOMENTS,
                              combined_labels, false);
  // Print response moments
  StringArray resp_labels = ModelUtils::response_labels(*mcmcModel);
  NonDSampling::print_moments(Cout, fnStats, RealMatrix(), "response function",
                              Pecos::STANDARD_MOMENTS, resp_labels, false);
}

void NonDBayesCalibration::print_hi2lo_batch_status(
    int num_it, int batch_n, int batchEvals, const Variables& optimal_config,
    double max_MI) {
  Cout << "\n----------------------------------------------\n";
  Cout << "Experimental Design Iteration " << num_it << " Progress";
  Cout << "\n----------------------------------------------\n";
  Cout << "Point " << batch_n << " of " << batchEvals << " selected\n";
  Cout << "Optimal design:\n";
...
void NonDBayesCalibration::print_hi2lo_selected(
    int num_it, const VariablesArray& optimal_config_matrix,
    const RealVector& MI_vec) {
...
void NonDBayesCalibration::print_hi2lo_file(
    std::ostream& out_file, int num_it,
    const VariablesArray& optimal_config_matrix, const RealVector& MI_vec,
  ...
  for (int batch_n = 0; batch_n < batch_evals; batch_n++) {
    const Variables& col = optimal_config_matrix[batch_n];
    if (batch_evals > 1) {
      out_file << "Design point ";
    }
    col.write(out_file, ACTIVE_VARS);
    out_file << "Mutual Information = " << MI_vec[batch_n] << '\n';
    if (resp_matrix.numCols() > 0) {
      RealVector col = Teuchos::getCol(Teuchos::View, resp_matrix, batch_n);
      out_file << "Hifi Response = " << col << '\n';
    }
  }
}

void NonDBayesCalibration::choose_batch_from_mutual_info(
    int random_seed, int num_it, int max_hifi, int num_hifi,
  ...
  if (max_hifi != 0)
    if (design_matrix.size() < batchEvals || max_hifi - num_hifi < batchEvals) {
  ...
  for (int batch_n = 1; batch_n < batchEvals + 1; batch_n++) {
    Xmatrix.reshape(numContinuousVars + batch_n * numFunctions, num_filtered);

    // Build simulation error matrix
    RealMatrix sim_error_matrix;
    const RealVector& sim_error_vec =
        mcmcModel->current_response().shared_data().simulation_error();
    ...
    for (size_t i = 0; i < design_matrix.size(); i++) {
      ...
      if (outputLevel >= NORMAL_OUTPUT) print_hi2lo_status(num_it, i, xi_i, MI);
    ...
    if (batchEvals > 1) {
      ...
      mcmcModel->current_variables().active_to_inactive_variables(
          optimal_config);
      Model::evaluate(mi_chain, *mcmcModel, lofi_resp_matrix);
      if (sim_error_matrix.numRows() > 0) lofi_resp_matrix += sim_error_matrix;
    ...
    if (batchEvals > 1)
      if (outputLevel >= NORMAL_OUTPUT)
        print_hi2lo_batch_status(num_it, batch_n, batchEvals, optimal_config,
                                 max_MI);
  }  // end batch_n loop
}
...
void NonDBayesCalibration::add_lhs_hifi_data() {
  ...
  if (initHifiSamples <= expData.num_experiments()) return;
...
void NonDBayesCalibration::apply_hifi_sim_error(int& random_seed, int num_exp,
                                                int exp_offset) {
  // Apply hifi error
  const RealVector& hifi_sim_error =
      hifiModel->current_response().shared_data().simulation_error();
...
void NonDBayesCalibration::apply_error_vec(const RealVector& sim_error_vec,
  ...
  if (sim_error_vec.length() == 1) {
    ...
    for (size_t j = 0; j < numFunctions; j++) error_vec[j] = err_gen();
...
void NonDBayesCalibration::build_designs(VariablesArray& design_matrix) {
  ...
  if (!importCandPtsFile.empty()) {
    ...
    auto numin_havemore = TabularIO::read_data_tabular(
        importCandPtsFile, "user-provided candidate points", numCandidates,
        design_matrix, importCandFormat);
  ...
  if (num_candidates_in < numCandidates) {
    ...
    auto lhs_iterator2 = std::make_shared<NonDLHSSampling>(
        hifiModel, sample_type, new_candidates, random_seed_1, rng,
        vary_pattern, ACTIVE_UNIFORM);
...
void NonDBayesCalibration::build_hi2lo_xmatrix(RealMatrix& Xmatrix, int i,
  ...
  RealMatrix xmatrix_theta(Teuchos::View, Xmatrix, numContinuousVars,
                           Xmatrix.numCols());
...
void NonDBayesCalibration::run_hifi(const VariablesArray& optimal_config_matrix,
                                    RealMatrix& resp_matrix) {
  ...
  for (col_ind = 0; col_ind < num_evals; ++col_ind) {
    ...
    expData.add_data(mcmcModel->current_variables().shared_data(), config_vars,
                     hifi_resp);
  }
}
...
void NonDBayesCalibration::build_scalar_discrepancy() {
  ...
  for (size_t i = 0; i < numFunctions; ++i) fn_indices.insert(i);
  ...
  vars_comps_totals =
      mcmcModel->current_variables().shared_data().inactive_components_totals();
  SharedVariablesData svd(view, vars_comps_totals);
  Variables configvars(svd);
  VariablesArray configvar_array(num_exp);
  for (int i = 0; i < num_exp; i++) {
    const RealVector& config_i =
        Teuchos::getCol(Teuchos::View, allConfigInputs, i);
    ...
    configvars.discrete_int_variables(
        vars_copy.inactive_discrete_int_variables());
    configvars.discrete_real_variables(
        vars_copy.inactive_discrete_real_variables());
  ...
  if (!importPredConfigs.empty()) {
    TabularIO::read_data_tabular(
        importPredConfigs, "user-provided prediction configurations",
        configpred_mat, num_configvars, importPredConfigFormat, false);
  ...
  for (int i = 0; i < num_pred; i++) {
    ...
    Model::inactive_variables(config_vec, *mcmcModel);
    ModelUtils::continuous_variables(*mcmcModel,
                                     ave_params);  // KAM -delete later
    mcmcModel->evaluate();
  ...
  if (expData.variance_active()) {
    ...
    for (int i = 0; i < numFunctions; i++) {
      ...
      for (int j = 0; j < num_exp; j++)
        if (exp_stddevs[j][i] > max_var) max_var = exp_stddevs[j][i];
      RealVector discrep_varvec =
          Teuchos::getCol(Teuchos::View, discrep_var, i);
  ...
  */
    ...
    for (int j = 0; j < field_lengths.length(); j++) {
      ...
      if (expData.interpolate_flag()) {
        ...
        for (int k = 0; k < field_lengths[j]; k++)
          concat_disc[ind + k] =
              expData.all_data(i)[k] - interpolated_resp.function_values()[k];
      } else {
        for (int k = 0; k < field_lengths[j]; k++)
          concat_disc[ind + k] =
              expData.all_data(i)[k] -
  ...
  if (!importPredConfigs.empty()) {
    ...
    for (int i = 0; i < num_configvars; i++)
      config_step[i] =
          (configUpperBnds[i] - configLowerBnds[i]) / (numPredConfigs - 1);
  ...
  for (int i = 0; i < num_field_groups; i++) {
    for (int j = 0; j < num_pred; j++) {
      ...
      int dim_indepvars = vars_mat.numCols();
      vars_mat.reshape(num_indepvars, dim_indepvars + num_configvars);
      const RealVector& config_vec =
          Teuchos::getCol(Teuchos::Copy, configpred_mat, j);
      col_vec.resize(num_indepvars);
      for (int k = 0; k < num_configvars; k++) {
        col_vec.putScalar(config_vec[k]);
        Teuchos::setCol(col_vec, dim_indepvars + k, vars_mat);
  ...
  for (int i = 0; i < num_pred; i++) {
    const RealVector& config_vec =
        Teuchos::getCol(Teuchos::Copy, configpred_mat, i);
    Model::inactive_variables(config_vec, *mcmcModel);
    mcmcModel->evaluate();
    const RealVector sim_resp = mcmcModel->current_response().function_values();
    for (int j = 0; j < sim_resp.length(); j++) {
      correctedFieldResponses[ind + j] =
          sim_resp[j] + discrepancyFieldResponses[ind + j];
    }
    ind += sim_resp.length();
  }
  ...
  // expData.form_residuals(mcmcModel->current_response(), j, my_asv, 0,
  // residual_response); expData.form_residuals(mcmcModel->current_response(),
  // j, residual_response); build_GP_field(vector view(indep_coordinates,
  // t_new_coord, concat_disc, disc_pred); Cout <<
  // residual_response.function_values();
  //}

  // We are getting coordinates for the experiment and ultimately we need to
  // handle the simulation coordinates if interpolation.
  //
  ...
  // One possibility is to use the GP construction from NonDLHSSampling, line
  // 794 Another is to use the DiscrepancyCorrection class
...
void NonDBayesCalibration::build_GP_field(const RealMatrix& discrep_vars_mat,
                                          ...
                                          const RealVector& concat_disc,
                                          RealVector& disc_pred,
                                          RealVector& disc_var) {
  String approx_type;
  ...
  for (int i = 0; i < pred_length; i++) {
    RealVector new_sample =
        Teuchos::getCol(Teuchos::View, discrep_vars_pred, i);
    disc_pred(i) = gpApproximation.value(new_sample);
    disc_var(i) = gpApproximation.prediction_variance(new_sample);
  }
}

void NonDBayesCalibration::export_discrepancy(RealMatrix& pred_config_mat) {
  ...
  const StringArray& resp_labels = ModelUtils::response_labels(*mcmcModel);
  size_t wpp4 = write_precision + 4;

  // Discrepancy responses file output
  unsigned short discrep_format = exportDiscrepFormat;
  String discrep_filename = exportDiscrepFile.empty()
                                ? "dakota_discrepancy_tabular.dat"
                                : exportDiscrepFile;
  ...
  for (int i = 0; i < num_pred; ++i) {
    TabularIO::write_leading_columns(discrep_stream, i + 1,
                                     mcmcModel->interface_id(), discrep_format);
    const RealVector& config_vec =
        Teuchos::getCol(Teuchos::View, pred_config_mat, i);
    Model::inactive_variables(config_vec, *mcmcModel);
  ...
  String corrmodel_filename = exportCorrModelFile.empty()
                                  ? "dakota_corrected_model_tabular.dat"
                                  : exportCorrModelFile;
  ...
  corrmodel_stream << std::setprecision(write_precision)
                   << std::resetiosflags(std::ios::floatfield);
  for (int i = 0; i < num_pred; ++i) {
    TabularIO::write_leading_columns(
        corrmodel_stream, i + 1, mcmcModel->interface_id(), corrmodel_format);
    const RealVector& config_vec =
        Teuchos::getCol(Teuchos::View, pred_config_mat, i);
    Model::inactive_variables(config_vec, *mcmcModel);
    output_vars = mcmcModel->current_variables().copy();
  ...
  String var_filename = exportCorrVarFile.empty()
                            ? "dakota_discrepancy_variance_tabular.dat"
                            : exportCorrVarFile;
  ...
  TabularIO::write_header_tabular(discrepvar_stream, output_vars, var_labels,
                                  "pred_config", "interface",
                                  discrepvar_format);
  discrepvar_stream << std::setprecision(write_precision)
                    << std::resetiosflags(std::ios::floatfield);
  for (int i = 0; i < num_pred; ++i) {
    TabularIO::write_leading_columns(
        discrepvar_stream, i + 1, mcmcModel->interface_id(), discrepvar_format);
    const RealVector& config_vec =
        Teuchos::getCol(Teuchos::View, pred_config_mat, i);
    Model::inactive_variables(config_vec, *mcmcModel);
    output_vars = mcmcModel->current_variables().copy();
    output_vars.write_tabular(discrepvar_stream);
    const RealVector& var_vec =
        Teuchos::getCol(Teuchos::View, corrected_var_transpose, i);
    for (size_t j = 0; j < numFunctions; ++j)
...
void NonDBayesCalibration::export_field_discrepancy(RealMatrix& pred_vars_mat) {
  ...
  for (int i = 0; i < num_pred; ++i) {
    TabularIO::write_leading_columns(discrep_stream, i + 1,
                                     mcmcModel->interface_id(), discrep_format);
    const RealVector& pred_vec =
        Teuchos::getCol(Teuchos::View, pred_vars_mat, int(numFunctions) * i);
...
void NonDBayesCalibration::extract_selected_posterior_samples(
...
void NonDBayesCalibration::export_posterior_samples_to_file(
    const std::string filename, const RealMatrix& posterior_data) const {}
...
Real NonDBayesCalibration::log_likelihood(const RealVector& residuals,
                                          const RealVector& all_params) {
  // if needed, extract the trailing hyper-parameters
  RealVector hyper_params;
  if (numHyperparams > 0)
    hyper_params = RealVector(
        Teuchos::View, all_params.values() + numContinuousVars, numHyperparams);
...
void NonDBayesCalibration::get_positive_definite_covariance_from_hessian(
  ...
  Teuchos::symMatTripleProduct(Teuchos::TRANS, 1., hessian, prior_chol_fact,
                               LT_H_L);
  ...
  */
  ...
  for (n = 0; n < num_rows; ++n) covariance(n, n) += 1.;
...
void NonDBayesCalibration::neg_log_post_resp_mapping(
    const Variables& residual_vars, const Variables& nlpost_vars,
    const Response& residual_resp, Response& nlpost_resp) {
  ...
  if (nonDBayesInstance->numHyperparams > 0)
    hyper_params = RealVector(
        Teuchos::View, c_vars.values() + nonDBayesInstance->numContinuousVars,
...
void NonDBayesCalibration::compute_statistics() {
  ...
  if (burnInSamples > 0 || num_skip > 1) {
    ...
    filtered_chain = RealMatrix(
        Teuchos::View, acceptanceChain.values(), acceptanceChain.stride(),
        acceptanceChain.numRows(), acceptanceChain.numCols());

    filteredFnVals = RealMatrix(
        Teuchos::View, acceptedFnVals.values(), acceptedFnVals.stride(),
  ...
  NonDSampling::compute_moments(filteredFnVals, fnStats,
                                Pecos::STANDARD_MOMENTS);
  if (!requestedProbLevels[0].empty()) compute_intervals();
  ...
  if (posteriorStatsKL) kl_post_prior(acceptanceChain);
  if (posteriorStatsMutual) mutual_info_buildX();
  if (posteriorStatsKDE) calculate_kde();
  if (calModelEvidence) calculate_evidence();
}
...
void NonDBayesCalibration::filter_matrix_cols(const RealMatrix& orig_matrix,
                                              int start_index, int stride,
                                              RealMatrix& filtered_matrix) {
...
void NonDBayesCalibration::compute_intervals() {
  ...
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    interval_stream << Cred_interval_minima[i] << ", "
                    << Cred_interval_maxima[i] << '\n';
  }
  interval_stream << "\n";
  ...
  if (expData.variance_active()) {
    compute_prediction_vals(filteredFnVals, predVals, num_filtered, num_exp,
                            num_concatenated);
  ...
  if (num_levels > 0)
    print_intervals_file(interval_stream, filtered_fn_vals_transpose, predVals,
                         num_filtered, num_concatenated);

  interval_stream << "acceptedVals = " << acceptedFnVals << '\n';
  interval_stream << "predVals = " << predVals << '\n';
}

void NonDBayesCalibration::compute_prediction_vals(RealMatrix& filtered_fn_vals,
                                                   RealMatrix& predVals,
                                                   int num_filtered,
                                                   size_t num_exp,
  ...
  for (e = 0; e < num_exp; ++e) {
    ...
    lhsDriver.generate_normal_samples(means_vec, std_deviations[e], lower_bnds,
                                      upper_bnds, correl_matrices[e],
                                      num_filtered, lhs_normal_samples);
...
void NonDBayesCalibration::export_chain(RealMatrix& filtered_chain,
                                        RealMatrix& filtered_fn_vals) {
  String mcmc_filename = exportMCMCFilename.empty() ? "dakota_mcmc_tabular.dat"
                                                    : exportMCMCFilename;
  ...
  */
  ...
  for (int i = 0; i < num_filtered; ++i) {
    TabularIO::write_leading_columns(
        export_mcmc_stream, i + 1, mcmcModel->interface_id(), exportMCMCFormat);
...
void NonDBayesCalibration::calculate_kde() {
...
void NonDBayesCalibration::calculate_evidence() {
  ...
  if (!calModelEvidMC && !calModelEvidLaplace) calModelEvidMC = true;
  if (calModelEvidMC) {
    // int num_prior_samples = chainSamples; //KAM: replace later
    int num_prior_samples =
        (evidenceSamples > 0) ? evidenceSamples : chainSamples;
  ...
  if (calModelEvidLaplace) {
    ...
    neg_log_post_resp_mapping(mapOptimizer->variables_results(),
                              mapOptimizer->variables_results(),
                              residualModel->current_response(), nlpost_resp);
    if (outputLevel >= DEBUG_OUTPUT) {
      Cout << "Negative log posterior function values "
           << nlpost_resp.function_values() << '\n';
      Cout << "Negative log posterior Hessian "
           << nlpost_resp.function_hessian_view(0) << '\n';
      // Cout << nlpost_resp << '\n';
    }
    ...
    // for now, the matrix passed to set_covariance is a RealMatrix, not
    // RealSymMatrix
    ...
    try {
      ...
      Cout << "log determinant post" << local_log_post_hess.log_determinant()
           << std::endl;
      double lpres = laplace_prior + laplace_like +
                     numContinuousVars * HALF_LOG_2PI -
                     0.5 * local_log_post_hess.log_determinant();
      Cout << "Model evidence (Laplace) = " << std::exp(lpres) << '\n';
    } catch (const std::runtime_error& re) {
      Cout << "Can't compute model evidence because the Hessian of the "
...
void NonDBayesCalibration::print_intervals_file(
    std::ostream& s, RealMatrix& filteredFnVals_transpose, RealMatrix& predVals,
    int num_filtered, size_t num_concatenated) {
  const StringArray& resp = ModelUtils::response_labels(*mcmcModel);
  size_t width = write_precision + 7;
  double alpha;
  int lower_index;
  ...
  for (int i = 0; i < numFunctions; ++i) {
    ...
    const RealVector& col_vec =
        Teuchos::getCol(Teuchos::View, filteredFnVals_transpose, i);
    ...
    if (num_prob_levels > 0) {
      ...
      for (size_t j = 0; j < num_prob_levels; ++j) {
        ...
        s << std::setw(width) << ' ' << std::setw(width) << col_vec[lower_index]
          << ' ' << std::setw(width) << alpha << '\n'
          << std::setw(width) << ' ' << std::setw(width) << col_vec[upper_index]
          << ' ' << std::setw(width) << 1 - alpha << '\n'
          << std::setw(width) << ' ' << "        -----             -----\n";
      }
    }
  }
  ...
  if (expData.variance_active()) {
    ...
    for (int i = 0; i < numFunctions; ++i) {
      // Sort function values
      const RealVector& col_vec1 =
          Teuchos::getCol(Teuchos::View, predVals_transpose, i);
      ...
      if (num_prob_levels > 0) {
        ...
        for (size_t j = 0; j < num_prob_levels; ++j) {
          ...
          s << std::setw(width) << ' ' << std::setw(width)
            << col_vec1[lower_index] << ' ' << std::setw(width) << alpha << '\n'
            << std::setw(width) << ' ' << std::setw(width)
            << col_vec1[upper_index] << ' ' << std::setw(width) << 1 - alpha
            << '\n'
            << std::setw(width) << ' ' << "        -----             -----\n";
        }
      }
...
void NonDBayesCalibration::print_results(std::ostream& s, short results_state) {
  ...
  copy_data(ModelUtils::continuous_variable_labels(*residualModel),
            combined_labels);
  NonDSampling::print_moments(s, chainStats, RealMatrix(), "posterior variable",
                              Pecos::STANDARD_MOMENTS, combined_labels, false);
  // Print response moments
  StringArray resp_labels = ModelUtils::response_labels(*mcmcModel);
  NonDSampling::print_moments(s, fnStats, RealMatrix(), "response function",
                              Pecos::STANDARD_MOMENTS, resp_labels, false);

  // Print chain diagnostics for variables
  if (chainDiagnostics) print_chain_diagnostics(s);
  ...
  if (requestedProbLevels[0].length() > 0 && outputLevel >= NORMAL_OUTPUT) {
    ...
    RealMatrix predVals_transpose(predVals, Teuchos::TRANS);
    print_intervals_screen(s, filteredFnVals_transpose, predVals_transpose,
                           num_filtered);
  }

  // Print posterior stats
  if (posteriorStatsKL) print_kl(s);
}

void NonDBayesCalibration::print_variables(std::ostream& s,
    ...
    for (int i = burn_in_post + 1; i < num_post_samples; ++i) {
      ...
      if (it_cntr % num_skip == 0) {
        RealVector param_vec =
            Teuchos::getCol(Teuchos::View, acceptanceChain, i);
  ...
  for (int i = 0; i < num_samples; i++) {
    if (alg == MI_ALG_KSG2) {  // alg=1, ksg2
      ...
      */
      ...
      n_x = kdTreeX->annkFRSearch(dataX[i], XYdistances[i], 0, NULL, NULL, eps);
      n_y = kdTreeY->annkFRSearch(dataY[i], XYdistances[i], 0, NULL, NULL, eps);
    }
...
void NonDBayesCalibration::ann_dist(const ANNpointArray matrix1,
                                    const ANNpointArray matrix2,
                                    RealVector& distances, int NX, int NY,
...
void NonDBayesCalibration::ann_dist(const ANNpointArray matrix1,
                                    const ANNpointArray matrix2,
                                    RealVector& distances, Int2DArray& indices,
  ...
  for (unsigned int i = 0; i < NX; ++i) {
    ...
    double dist = knn_dist[k_i];
    IntArray ind(k_i + 1);
    for (int j = 0; j < k_i + 1; j++) ind[j] = knn_ind[j];
    if (dist == 0.0) {
      ANNdistArray knn_dist_i = new ANNdist[NY];
      ANNidxArray knn_ind_i = new ANNidx[NY];
      // calc distances for whole array
      ...
      for (unsigned int j = k_i + 1; j < NY; ++j) {
        if (knn_dist_i[j] > 0.0) {
          dist = knn_dist_i[j];
          ind.resize(j);
          for (int k = 0; k < j; k++) ind[k] = knn_ind_i[k];
          k_vec[i] = j;
          break;
        }
      }
...
void NonDBayesCalibration::print_chain_diagnostics(std::ostream& s) {
  s << "\nChain diagnostics\n";
  if (chainDiagnosticsCI) print_batch_means_intervals(s);
}
...
void NonDBayesCalibration::print_batch_means_intervals(std::ostream& s) {
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    for (int i = 0; i < num_vars; i++) {
      ...
      s << var_labels[i] << '\n';
      const RealVector& mean_vec =
          Teuchos::getCol(Teuchos::View, variables_mean_batch_means, i);
      s << mean_vec;
      s << "\tBatch means of variance for variable ";
      const RealVector& var_vec =
          Teuchos::getCol(Teuchos::View, variables_var_batch_means, i);
      s << var_labels[i] << '\n';
      s << var_vec;
    }
    for (int i = 0; i < num_responses; i++) {
      ...
      s << resp_labels[i] << '\n';
      const RealVector mean_vec =
          Teuchos::getCol(Teuchos::View, responses_mean_batch_means, i);
      s << mean_vec;
      s << "\tBatch means of variance for response ";
      s << resp_labels[i] << '\n';
      const RealVector var_vec =
          Teuchos::getCol(Teuchos::View, responses_var_batch_means, i);
      s << var_vec;
    }
  }
  ...
  for (int i = 0; i < num_vars; i++) {
    RealVector col_vec =
        Teuchos::getCol(Teuchos::View, variables_mean_interval_mat, i);
    s << '\t' << std::setw(width) << var_labels[i] << " = [" << col_vec[0]
      << ", " << col_vec[1] << "]\n";
  }
  for (int i = 0; i < num_responses; i++) {
    RealVector col_vec =
        Teuchos::getCol(Teuchos::View, responses_mean_interval_mat, i);
    s << '\t' << std::setw(width) << resp_labels[i] << " = [" << col_vec[0]
      << ", " << col_vec[1] << "]\n";
  }
  s << "\t95% Confidence Intervals of variances\n";
  for (int i = 0; i < num_vars; i++) {
    RealVector col_vec =
        Teuchos::getCol(Teuchos::View, variables_var_interval_mat, i);
    s << '\t' << std::setw(width) << var_labels[i] << " = [" << col_vec[0]
      << ", " << col_vec[1] << "]\n";
  }
  for (int i = 0; i < num_responses; i++) {
    RealVector col_vec =
        Teuchos::getCol(Teuchos::View, responses_var_interval_mat, i);
    s << '\t' << std::setw(width) << resp_labels[i] << " = [" << col_vec[0]
      << ", " << col_vec[1] << "]\n";
  }
}
