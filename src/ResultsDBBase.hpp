...
class ResultsDBBase {
 public:
  ...
  virtual void flush() const { return; }
  ...
  virtual void allocate_vector(
      const StrStrSizet& iterator_id, const StringArray& location,
  ...
  virtual void allocate_matrix(
      const StrStrSizet& iterator_id, const StringArray& location,
      ResultsOutputType stored_type, const int& num_rows, const int& num_cols,
      const DimScaleMap& scales = DimScaleMap(),
      const AttributeArray& attrs = AttributeArray()) = 0;

  /// addition with dimension scales and attributes
  virtual void insert(const StrStrSizet& iterator_id,
  ...
  virtual void insert_into(const StrStrSizet& iterator_id,
                           const StringArray& location, const boost::any& data,
                           const int& index, const bool& row) = 0;

  /// Add key:value metadata to a method
  virtual void add_metadata_to_method(const StrStrSizet& iterator_id,
                                      const AttributeArray& attrs) = 0;

  /// Add key:value metadata to an execution
  virtual void add_metadata_to_execution(const StrStrSizet& iterator_id,
                                         const AttributeArray& attrs) = 0;
  ...
  virtual void insert(const StrStrSizet& iterator_id,
                      const std::string& data_name, const boost::any& result,
...
template <typename StoredType>
void ResultsDBBase::array_allocate(const StrStrSizet& iterator_id,
                                   const std::string& data_name,
                                   size_t array_size,
                                   const MetaDataType& metadata) {
  // add a vector of the StoredType, with metadata
  insert(iterator_id, data_name, std::vector<StoredType>(array_size), metadata);
}

/** insert requires previous allocation, and does not allow metadata update */
template <typename StoredType>
void ResultsDBBase::array_insert(const StrStrSizet& iterator_id,
                                 const std::string& data_name, size_t index,
                                 const StoredType& sent_data) {
