...
ActiveSubspaceModel::ActiveSubspaceModel(ProblemDescDB& problem_db,
    ...
    : SubspaceModel(problem_db, parallel_lib,
                    get_sub_model(problem_db, parallel_lib)),
      initialSamples(problem_db.get_int("model.initial_samples")),
      subspaceIdBingLi(probDescDB.get_bool(
          "model.active_subspace.truncation_method.bing_li")),
      subspaceIdConstantine(probDescDB.get_bool(
          "model.active_subspace.truncation_method.constantine")),
      subspaceIdEnergy(probDescDB.get_bool(
          "model.active_subspace.truncation_method.energy")),
      subspaceIdCV(
          probDescDB.get_bool("model.active_subspace.truncation_method.cv")),
      numReplicates(
          problem_db.get_int("model.active_subspace.bootstrap_samples")),
      totalSamples(0),
      gradientScaleFactors(RealArray(numFns, 1.)),
      truncationTolerance(
          probDescDB.get_real("model.active_subspace.truncation_method.energy."
                              "truncation_tolerance")),
      buildSurrogate(
          probDescDB.get_bool("model.active_subspace.build_surrogate")),
      refinementSamples(0),
      subspaceNormalization(
          probDescDB.get_ushort("model.active_subspace.normalization")),
      cvIncremental(
          probDescDB.get_bool("model.active_subspace.cv.incremental")),
...
ActiveSubspaceModel::ActiveSubspaceModel(std::shared_ptr<Model> sub_model,
  ...
  // subModel.derivative_concurrency();

  RealMatrix reduced_basis_W1(Teuchos::View, rotation_matrix, numFullspaceVars,
                              reducedRank);
...
std::shared_ptr<Model> ActiveSubspaceModel::get_sub_model(
...
void ActiveSubspaceModel::initialize_subspace() {
  ...
  if (buildSurrogate) build_surrogate();
...
void ActiveSubspaceModel::uncertain_vars_to_subspace() {
  ...
  RealVector mu_x(numFullspaceVars), sd_x(numFullspaceVars);
  size_t cntr = 0;
  for (i = 0; i < num_rv; ++i)
    if (active_vars[i]) {
      switch (native_rv_types[i]) {
        case Pecos::
            STD_NORMAL:  // ActiveSubspace(ProbabilityTransform(Simulation))
          mu_x[cntr] = 0.;
          sd_x[cntr] = 1.;
          break;
  ...
  int m = reducedBasis.numRows(), n = reducedBasis.numCols(), incx = 1,
      incy = 1;
  Real alpha = 1., beta = 0.;
  ...
  UTVx.multiply(Teuchos::TRANS, Teuchos::NO_TRANS, alpha, reducedBasis, V_x,
                beta);
  RealMatrix V_y(reducedRank, reducedRank, false);
  V_y.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, alpha, UTVx, reducedBasis,
               beta);
  if (outputLevel >= DEBUG_OUTPUT) Cout << "\nSubspace Model: V_y = \n" << V_y;

  // compute the standard deviations in reduced space
  for (int i = 0; i < reducedRank; ++i) sd_y(i) = std::sqrt(V_y(i, i));
  ...
  for (int i = 0; i < reducedRank; i++) cv_types[i] = NORMAL_UNCERTAIN;
...
void ActiveSubspaceModel::generate_fullspace_samples(
  ...
  fullspaceSampler->sampling_reset(diff_samples, true,
                                   false);  // all_data, stats
...
void ActiveSubspaceModel::populate_matrices(unsigned int diff_samples) {
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    Cout << "\nSubspace Model: DACE iterator returned " << all_responses.size()
         << " samples. (expected " << diff_samples << " samples)" << std::endl;
  }
      ...
      if (subspaceNormalization == SUBSPACE_NORM_MEAN_VALUE) {
        ...
        for (unsigned int fn_ind = 0; fn_ind < numFns; ++fn_ind) {
          gradientScaleFactors[fn_ind] +=
              resp_vector(fn_ind) / static_cast<Real>(diff_samples);
        }
        ...
        for (unsigned int fn_ind = 0; fn_ind < numFns; ++fn_ind) {
          ...
          gradientScaleFactors[fn_ind] +=
              norm_grad / static_cast<Real>(diff_samples);
        }
      }
    }
  }
  ...
  for (; resp_it != resp_end; ++resp_it, ++diff_sample_ind) {
    ...
    for (unsigned int fn_ind = 0; fn_ind < numFns; ++fn_ind) {
      ...
      for (unsigned int var_ind = 0; var_ind < numFullspaceVars; ++var_ind) {
        Real scale = 1.0;
        if (numFns > 1 && (subspaceNormalization == SUBSPACE_NORM_DEFAULT ||
...
void ActiveSubspaceModel::truncate_subspace() {
  ...
  if (reducedRank > 0 && reducedRank <= singularValues.length()) {
    ...
    if (subspaceIdBingLi) {
      ...
      if (reducedRank < bing_li_rank) reducedRank = bing_li_rank;
    }
    ...
    if (subspaceIdConstantine) {
      ...
      if (reducedRank < constantine_rank) reducedRank = constantine_rank;
    }
    ...
    if (subspaceIdEnergy) {
      ...
      if (reducedRank < energy_rank) reducedRank = energy_rank;
    }
    ...
    if (subspaceIdCV) {
      ...
      if (reducedRank < cv_rank) reducedRank = cv_rank;
    }
  ...
  if (singularValues[reducedRank - 1] < mach_svtol) {
    ...
    Cout << "\nSubspace Model: New subspace size is dimension = " << reducedRank
         << "." << std::endl;
  }

  int k_temp =
      std::ceil(initialSamples / (2.0 * std::log10((double)numFullspaceVars)));
...
unsigned int ActiveSubspaceModel::compute_bing_li_criterion(
  ...
  for (size_t i = 0; i < num_vals; ++i) bing_li_criterion[i] /= eigen_sum;
  ...
  for (size_t i = 0; i < numReplicates; ++i) {
    ...
    for (size_t j = 1; j < bootstrapped_det.size(); ++j) {
      ...
      Teuchos::SerialDenseVector<RealMatrix::ordinalType,
                                 RealMatrix::ordinalType>
          pivot(num_sing_vec);
      RealMatrix::ordinalType info;

      lapack.GETRF(num_sing_vec, num_sing_vec, submatrix.values(), num_sing_vec,
                   pivot.values(), &info);
  ...
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "\nSubspace Model: Bing Li metric subspace size estimate = " << rank
         << std::endl;

  return rank;
}

unsigned int ActiveSubspaceModel::compute_constantine_metric(
    RealVector& singular_values) {
  int num_vars = derivativeMatrix.numRows();
  int num_vals;
  if (derivativeMatrix.numCols() < num_vars)
  ...
  RealArray constantine_metric(
      (num_vals < num_vars - 1) ? num_vals : num_vars - 1, 0);
  ...
  for (size_t i = 0; i < numReplicates; ++i) {
    ...
    for (size_t j = 0; j < constantine_metric.size(); ++j) {
      ...
      dist_mat.multiply(Teuchos::NO_TRANS, Teuchos::TRANS, 1.0, submatrix,
                        submatrix, 0.0);
...
unsigned int ActiveSubspaceModel::compute_energy_criterion(
    RealVector& singular_values) {
  int num_vars = derivativeMatrix.numRows();
  int num_vals;
  if (derivativeMatrix.numCols() < num_vars)
  ...
  for (size_t i = 1; i < num_vals; ++i) {
    energy_metric[i] =
        std::pow(singular_values[i], 2) / total_energy + energy_metric[i - 1];
  }
...
unsigned int ActiveSubspaceModel::compute_cross_validation_metric() {
  ...
  if (cvMaxRank >= 0 && max_rank > cvMaxRank) max_rank = cvMaxRank;

  // Loop over all feasible subspace sizes
  std::vector<Real> cv_error;
  bool found_acceptable_subspace = false, rel_tol_met = false,
       decrease_tol_met = false;
  for (unsigned int ii = 1; ii <= max_rank; ii++) {
    if (found_acceptable_subspace && cvIncremental) break;
    ...
    auto cv_surr_model = std::make_shared<DataFitSurrModel>(
        dace_iterator, asm_model_tmp, surr_set, surr_view, approx_type,
        approx_order, corr_type, corr_order, data_order, QUIET_OUTPUT,
        sample_reuse);
    ...
    for (int ind = 0; ind < n; ++ind) random_index_vec.push_back(ind);
    ...
    for (int jj = 0; jj < n % num_folds; jj++) fold_size[jj]++;
    ...
    for (int jj = 0; jj < num_folds; jj++) {
      ...
      for (int kk = 0; kk < jj; kk++) fold_start_ind += fold_size[kk];
...
unsigned int ActiveSubspaceModel::determine_rank_cv(
  ...
  unsigned int rank_rel_tol =
      tolerance_met_index(cv_error, cvRelTolerance, rel_tol_met) + 1;
  unsigned int rank_decrease_tol =
      tolerance_met_index(negative_diff(cv_error), cvDecreaseTolerance,
                          decrease_tol_met) +
      1;
...
void ActiveSubspaceModel::build_surrogate() {
  ...
  auto asm_model = std::make_shared<ActiveSubspaceModel>(
      subModel, reducedRank, leftSingularVectors,
      QUIET_OUTPUT);  // partitioned to W1,W2
  ...
  surrogateModel = std::make_shared<DataFitSurrModel>(
      dace_iterator, asm_model, surr_set, surr_view, approx_type, approx_order,
  ...
  if (refinementSamples > 0) {
    ...
    surrogateModel->append_approximation(all_vars_y_ref, all_responses_ref,
                                         true);
  }
}
...
Real ActiveSubspaceModel::build_cv_surrogate(Model& cv_surr_model,
                                             RealMatrix training_x,
  ...
  surr_set.derivative_vector(
      ModelUtils::continuous_variable_ids(cv_surr_model));
  for (int ii = 0; ii < num_test_points; ii++) {
    ModelUtils::continuous_variables(
        cv_surr_model, Teuchos::getCol(Teuchos::Copy, test_x, ii));
  ...
  IntRespMCIter resp_it = test_y.begin(), resp_end = test_y.end(),
                resp_surr_it = test_y_surr.begin(),
                resp_surr_end = test_y_surr.end();
  RealVector error_norm(numFns), mean(numFns), stdev(numFns);
  for (; resp_it != resp_end && resp_surr_it != resp_surr_end;
       resp_it++, resp_surr_it++) {
    const RealVector& resp_vector = resp_it->second.function_values();
  ...
  for (size_t ii = 0; ii < numFns; ii++) stdev[ii] = std::sqrt(stdev[ii]);

  Real max_error_norm = 0;
  for (size_t ii = 0; ii < numFns; ii++) {
    Real norm_rms_error = std::sqrt(error_norm[ii] / test_y.size()) / stdev[ii];

    if (norm_rms_error > max_error_norm) max_error_norm = norm_rms_error;
  }
...
/**  This specialization is because the model is used in multiple
     ...
     into any submodels. */
void ActiveSubspaceModel::derived_init_communicators(ParLevLIter pl_iter,
  ...
  if (recurse_flag) {
    if (!mappingInitialized) fullspaceSampler->init_communicators(pl_iter);

    subModel->init_communicators(pl_iter, max_eval_concurrency);
  }
}

void ActiveSubspaceModel::derived_set_communicators(ParLevLIter pl_iter,
                                                    int max_eval_concurrency,
                                                    bool recurse_flag) {
  miPLIndex =
  ...
  if (recurse_flag) {
    if (!mappingInitialized) fullspaceSampler->set_communicators(pl_iter);
...
void ActiveSubspaceModel::derived_free_communicators(ParLevLIter pl_iter,
                                                     int max_eval_concurrency,
                                                     bool recurse_flag) {
  if (recurse_flag) {
...
void ActiveSubspaceModel::init_fullspace_sampler(unsigned short sample_type) {
  ...
  fullspaceSampler = std::make_unique<NonDLHSSampling>(
      subModel, sample_type, initialSamples, randomSeed, rng, true);
...
void ActiveSubspaceModel::variables_mapping(const Variables& recast_y_vars,
  ...
  Real alpha = 1., beta = 0.;
  teuchos_blas.GEMV(Teuchos::NO_TRANS, m, n, alpha, W1.values(), m, y.values(),
                    incy, beta, x.values(), incx);
  ...
  n = W2.numCols();
  alpha = beta = 1.;
  teuchos_blas.GEMV(Teuchos::NO_TRANS, m, n, alpha, W2.values(), m, z.values(),
                    incz, beta, x.values(), incx);

  if (asmInstance->output_level() >= DEBUG_OUTPUT)
    Cout << "\nSubspace Model: Subspace vars are\n"
         << recast_y_vars << "\n\nSubspace Model: Fullspace vars are\n"
         << sub_model_x_vars << std::endl;
}
