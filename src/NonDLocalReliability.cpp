...
static const char rcsId[] =
    "@(#) $Id: NonDLocalReliability.cpp 4058 2006-10-25 01:39:40Z mseldre $";
...
NonDLocalReliability::NonDLocalReliability(ProblemDescDB& problem_db,
    ...
    : NonDReliability(problem_db, parallel_lib, model),
      ...
      npsolFlag(false),
      warmStartFlag(true),
      nipModeOverrideFlag(true),
      curvatureDataAvailable(false),
      kappaUpdated(false),
      secondOrderIntType(HOHENRACK),
      curvatureThresh(1.e-10),
      warningBits(0) {
  bool err_flag = false;
  ...
  if (mppSearchType) {  // default is MV = 0
    ...
    switch (sub_optimizer_select(
      ...
      case SUBMETHOD_OPTPP:
        ...
        break;
      default:
        Cerr << "\nError: invalid MPP optimizer selection in "
  ...
  // startCV = ac_totals[TOTAL_CDV]; numContAleatUncVars =
  // ac_totals[TOTAL_CAUV];
  ...
  switch (mppSearchType) {
    case SUBMETHOD_AMV_X:
    case SUBMETHOD_AMV_PLUS_X:
    case SUBMETHOD_TANA_X:
    case SUBMETHOD_QMEA_X: {
      ...
      if (mppSearchType == SUBMETHOD_TANA_X)
        approx_type = "multipoint_tana";
      else if (mppSearchType == SUBMETHOD_QMEA_X)
        approx_type = "multipoint_qmea";
      else
        approx_type = "local_taylor";
      ...
      ActiveSet dfs_set =
          iteratedModel->current_response().active_set();  // copy
      dfs_set.request_values(dfs_set_order);
      g_hat_x_model = std::make_shared<DataFitSurrModel>(
          dace_iterator, iteratedModel, dfs_set, orig_view, approx_type,
          approx_order, corr_type, corr_order, ai_data_order, outputLevel,
          sample_reuse);
    ...
    case SUBMETHOD_AMV_U:
    case SUBMETHOD_AMV_PLUS_U:
    case SUBMETHOD_TANA_U:
    case SUBMETHOD_QMEA_U: {
      ...
      String sample_reuse, approx_type;
      if (mppSearchType == SUBMETHOD_TANA_U)
        approx_type = "multipoint_tana";
      else if (mppSearchType == SUBMETHOD_QMEA_U)
        approx_type = "multipoint_qmea";
      else
        approx_type = "local_taylor";
      UShortArray approx_order(1, taylorOrder);
      ...
      dfs_set.request_values(dfs_set_order);
      uSpaceModel = std::make_shared<DataFitSurrModel>(
          dace_iterator, g_u_model, dfs_set, orig_view, approx_type,
          approx_order, corr_type, corr_order, ai_data_order, outputLevel,
          sample_reuse);
      break;
    }
  ...
  if (mppSearchType) {
    ...
    mppModel = std::make_shared<RecastModel>(
        uSpaceModel, recast_vars_comps_total, all_relax_di, all_relax_dr,
        orig_view, 1, 1, 0, recast_resp_order);
    ...
    if (npsolFlag) {
      ...
      mppOptimizer =
          std::make_shared<SNLLOptimizer>("optpp_q_newton", mppModel);
    }
#endif
  }
    ...
    switch (mppSearchType) {
      ...
      case SUBMETHOD_QMEA_X: {
        ...
        import_sampler_rep = std::make_shared<NonDAdaptImpSampling>(
            g_u_model, sample_type, refine_samples, refine_seed, rng,
            vary_pattern, integrationRefinement, cdfFlag, x_model_flag,
      ...
      case SUBMETHOD_QMEA_U:
        import_sampler_rep = std::make_shared<NonDAdaptImpSampling>(
            uSpaceModel->truth_model(), sample_type, refine_samples,
      ...
      case SUBMETHOD_NO_APPROX:
        import_sampler_rep = std::make_shared<NonDAdaptImpSampling>(
            uSpaceModel, sample_type, refine_samples, refine_seed, rng,
            vary_pattern, integrationRefinement, cdfFlag, x_model_flag,
            use_model_bounds, track_extreme);
        break;
    }
...
void NonDLocalReliability::derived_init_communicators(ParLevLIter pl_iter) {
  ...
  if (mppSearchType) {
    ...
    if (integrationRefinement) importanceSampler->init_communicators(pl_iter);
  }
}
...
void NonDLocalReliability::derived_set_communicators(ParLevLIter pl_iter) {
  ...
  if (mppSearchType) {
    ...
    if (integrationRefinement) importanceSampler->set_communicators(pl_iter);
  }
}

void NonDLocalReliability::derived_free_communicators(ParLevLIter pl_iter) {
  if (mppSearchType) {
    if (integrationRefinement) importanceSampler->free_communicators(pl_iter);
...
void NonDLocalReliability::check_sub_iterator_conflict() {
  ...
  if (mppSearchType == SUBMETHOD_NO_APPROX && npsolFlag) {
    ...
    if (sub_iterator && (sub_iterator->method_name() == NPSOL_SQP ||
...
void NonDLocalReliability::initialize_graphics(int iterator_server_id) {
  ...
  if (totalLevelRequests) {
    ...
    if (mgr.graph2DFlag &&
        iterator_server_id == 1) {  // initialize the 2D plots
...
void NonDLocalReliability::mean_value() {
  ...
  size_t num_imp_fact = (correlation_flag)
                            ? (numContinuousVars * (numContinuousVars + 1)) / 2
                            : numContinuousVars;
  ...
  for (respFnCount = 0; respFnCount < numFunctions; respFnCount++) {
    ...
    mean = momentStats(0, respFnCount);
    mom2 = momentStats(1, respFnCount);
    if (finalMomentsType == Pecos::CENTRAL_MOMENTS) {
      var = mom2;
      std_dev = std::sqrt(mom2);
    } else {
      std_dev = mom2;
      var = mom2 * mom2;
    }
    ...
    if (!need_dg_ds)
      for (i = 0, j = statCount + moment_offset; i < total_lev;
           ++i, ++j)  // all lev map
        if (final_asv[j] & 2) {
          need_dg_ds = true;
          break;
    ...
    if (!Pecos::is_small(std_dev)) {
      ...
      if (correlation_flag) {
        ...
        for (i = 0, cntr = numContinuousVars; i < numContinuousVars; ++i) {
          ...
          // TO DO: model.current_variables().cv_index_to_corr_index() for
          // general use cases
        }
      } else
        for (i = 0; i < numContinuousVars; ++i)
    ...
    for (levelCount = 0; levelCount < rl_len; levelCount++) {
      ...
      if (!Pecos::is_small(std_dev)) {
        ...
        if ((cdfFlag && mean <= z) || (!cdfFlag && mean > z)) {
          beta = computedRelLevels[respFnCount][levelCount] =
              computedGenRelLevels[respFnCount][levelCount] =
                  -Pecos::LARGE_NUMBER;
      ...
      switch (respLevelTarget) {
        case PROBABILITIES:
          ...
          if (final_asv[statCount] & 2) {
            ...
            if (cdfFlag)
              final_stat_grad.scale(-phi_m_beta / std_dev);
            else
              final_stat_grad.scale(phi_m_beta / std_dev);
            finalStatistics.function_gradient(final_stat_grad, statCount);
          }
        ...
        case GEN_RELIABILITIES:
          ...
          if (final_asv[statCount] & 2) {
            ...
            RealVector final_stat_grad(dg_ds_meanx);  // deep copy
            if (cdfFlag)
              final_stat_grad.scale(1. / std_dev);
            else
              final_stat_grad.scale(-1. / std_dev);
            finalStatistics.function_gradient(final_stat_grad, statCount);
          }
      ...
      if (!subIteratorFlag) dakota_graphics.add_datapoint(respFnCount, z, p);
    }
    for (i = 0; i < pl_len; i++) {
      levelCount = i + rl_len;
      // computed = requested in MV case since no validation fn evals
      ...
      Real z = computedRespLevels[respFnCount][levelCount] =
          (cdfFlag) ? mean - beta * std_dev : mean + beta * std_dev;
...
void NonDLocalReliability::mpp_search() {
  ...
  // ModelUtils::continuous_variables(iteratedModel) assignment in
  // initial_taylor_series() and needs to follow nataf.transform_correlations()
  if (numRelAnalyses == 0) {
    if (initialPtUserSpec)
      uSpaceModel->trans_X_to_U(
          ModelUtils::continuous_variables(*iteratedModel), initialPtUSpec);
    else {
  ...
  // sets ModelUtils::continuous_variables(iteratedModel) to mean values
  initial_taylor_series();
  ...
  for (respFnCount = 0; respFnCount < numFunctions; ++respFnCount) {
    ...
    size_t rl_len = requestedRespLevels[respFnCount].length(),
           pl_len = requestedProbLevels[respFnCount].length(),
           bl_len = requestedRelLevels[respFnCount].length(),
           gl_len = requestedGenRelLevels[respFnCount].length(), index,
    ...
    if (num_levels) initialize_level_data();
    ...
    for (levelCount = 0; levelCount < num_levels; ++levelCount) {
      ...
      if (ria_flag) {
        ...
        Real beta_cdf =
            (cdfFlag) ? requestedTargetLevel : -requestedTargetLevel;
      ...
      if (levelCount) initialize_mpp_search_data();
      ...
      while (!approxConverged) {
        ...
        std::shared_ptr<RecastModel> mpp_model_rep =
            std::static_pointer_cast<RecastModel>(mppModel);
        if (ria_flag) {                  // RIA: g is in constraint
          ...
          mpp_model_rep->init_maps(
              vars_map, false, NULL, NULL, primary_resp_map, secondary_resp_map,
              nonlinear_resp_map, RIA_objective_eval, RIA_constraint_eval);
          ...
          if (pma2_flag)
            mpp_model_rep->init_maps(vars_map, false, NULL, PMA2_set_mapping,
                                     ...
                                     nonlinear_resp_map, PMA_objective_eval,
        ...
        if (ria_flag)
          ...
          Cout << "G(u)]\n                     "
               << std::setw(write_precision + 7) << fns_star[1];
          if (pma2_flag)
            Cout << " [B* - bar-B*]\n";
          else
            Cout << " [u'u - B^2]\n";
  ...
  if (warmStartFlag && subIteratorFlag)  // view->copy
    copy_data(ModelUtils::inactive_continuous_variables(*iteratedModel),
              prevICVars);
...
void NonDLocalReliability::initial_taylor_series() {
  ...
  switch (mppSearchType) {
    ...
    case SUBMETHOD_QMEA_U:
      for (i = 0; i < numFunctions; ++i)
        if (!requestedRespLevels[i].empty() ||
            !requestedProbLevels[i].empty() || !requestedRelLevels[i].empty() ||
            !requestedGenRelLevels[i].empty())
          asrv[i] = mode;
  ...
  if (init_ts_flag) {
    ...
    ModelUtils::continuous_variables(*iteratedModel, ranVarMeansX);
    ...
    if (mode & 4) fnHessiansMeanX = curr_resp.function_hessians();
      ...
      if (asrv[i]) {
        ...
        for (j = 0; j < numContinuousVars; ++j) {
          ...
          if (correlation_flag)
            ...
            Real cov_jj =
                ranVarStdDevsX[j] * ranVarStdDevsX[j];  // covariance(j,j);
        ...
        if (t2nq) mean += v1 / 2.;
        mom2 =
            (finalMomentsType == Pecos::CENTRAL_MOMENTS) ? v2 : std::sqrt(v2);
      }
    }
...
void NonDLocalReliability::initialize_level_data() {
  ...
  if (warmStartFlag && subIteratorFlag && numRelAnalyses) {
    ...
    if (inactive_grads && lev0_ria) {
      RealVector fn_grad_d =
          Teuchos::getCol(Teuchos::View, prevFnGradDLev0, respFnCount);
      RealVector fn_grad_u =
          Teuchos::getCol(Teuchos::View, prevFnGradULev0, respFnCount);
      const RealVector& d_k_plus_1 =
          ModelUtils::inactive_continuous_variables(*iteratedModel);  // view
    ...
    if (levelCount < rl_len) {
      ...
      if (norm_grad_u_sq > 1.e-10) {  // also handles NPSOL numerical case
        Real alpha = (requestedTargetLevel -
                      requestedRespLevels[respFnCount][levelCount - 1]) /
                     norm_grad_u_sq;
        for (size_t i = 0; i < numContinuousVars; i++)
          initialPtU[i] = mostProbPointU[i] + alpha * fnGradU[i];
      } else
        initialPtU =
            initialPtUSpec;  // mostProbPointU: premature conv w/ some opt
    } else {
      ...
      size_t pl_len = requestedProbLevels[respFnCount].length(),
             bl_len = requestedRelLevels[respFnCount].length();
      Real prev_bl =
          (integrationOrder == 2 && (levelCount < rl_len + pl_len ||
                                     levelCount >= rl_len + pl_len + bl_len))
      ...
      Real high_tol = 1.e+3,
           low_tol = ((taylorOrder == 2 || integrationOrder == 2) &&
                      iteratedModel->hessian_type() == "quasi")
                         ? 1.e-3
                         : 1.e-10;
...
void NonDLocalReliability::update_mpp_search_data(const Variables& vars_star,
  ...
  switch (mppSearchType) {
    ...
    case SUBMETHOD_QMEA_U: {
      ...
      if (approxConverged) {
        ...
        const ShortArray& final_asv =
            finalStatistics.active_set_request_vector();
        if (warmStartFlag || (final_asv[statCount] & 2)) mode |= 2;
        if (integrationOrder == 2)
          mode |=
              4;  // RecastModel::transform_set() augments if nonlinear_vars_map
      } else {    // not converged
    ...
    case SUBMETHOD_NO_APPROX: {  // FORM/SORM
      ...
      // fnGradX/U needed for warm starting by projection, final_stat_grad,
      // and/or 2nd-order integration (for nonlinear transformations), and
      // should be retrievable from previous evals.  If second-order integration
      // for RIA, fnHessX/U also needed, but not retrievable.  If second-order
      // PMA with specified p-level, Hessian should be retrievable since it was
      // computed during the update of requestedCDFRelLevel from
      // requestedCDFProbLevel. When data should be retrievable, we cannot in
      // general assume that the last grad/Hessian eval corresponds to the
      // converged MPP; therefore, we use a DB search.  If the DB search fails
      // (e.g., the eval cache is deactivated), then we resort to reevaluation.
      short mode = 0, found_mode = 0;  // computedRespLevel already retrieved
      const ShortArray& final_asv = finalStatistics.active_set_request_vector();
      if (warmStartFlag || (final_asv[statCount] & 2)) mode |= 2;
      if (integrationOrder ==
          2) {  // apply 2nd-order integr in all RIA/PMA cases
        mode |= 4;
      ...
      if (mode & 6) uSpaceModel->trans_U_to_X(mostProbPointU, mostProbPointX);
      // retrieve previously evaluated gradient information, if possible
      if (mode &
          2) {  // avail in all RIA/PMA cases (exception: numerical grads)
        ...
        PRPCacheHIter cache_it = lookup_by_val(
            data_pairs, iteratedModel->interface_id(), search_vars, search_set);
      ...
      if ((mode & 4) && !ria_flag &&
        ...
        if (cache_it != data_pairs.get<hashed>().end()) {
          ...
          uSpaceModel->trans_hess_X_to_U(fnHessX, fnHessU, mostProbPointX,
                                         fnGradX);
...
void NonDLocalReliability::update_level_data() {
  ...
  if (!ria_flag && integrationOrder == 2) {
    ...
    computedProbLevels[respFnCount][levelCount] = computed_prob_level =
        probability(computedRelLevel, cdfFlag, mostProbPointU, fnGradU,
                    fnHessU);
    computedGenRelLevels[respFnCount][levelCount] = computedGenRelLevel =
  ...
  if (respLevelTargetReduce &&
    ...
    if (final_asv[sys_stat_count] & 2) system_grad_contrib = true;
  }
  if ((final_asv[statCount] & 2) || system_grad_contrib) {
    ...
    if (ria_flag) {
      ...
      size_t i, num_final_grad_vars =
                    finalStatistics.active_set_derivative_vector().size();
      for (i = 0; i < num_final_grad_vars; ++i) final_stat_grad[i] *= factor;
    }
    finalStatistics.function_gradient(final_stat_grad, statCount);
  }
...
void NonDLocalReliability::update_limit_state_surrogate() {
  bool x_space =
      (mppSearchType == SUBMETHOD_AMV_X ||
       mppSearchType == SUBMETHOD_AMV_PLUS_X ||
       mppSearchType == SUBMETHOD_TANA_X || mppSearchType == SUBMETHOD_QMEA_X);

  // construct local Variables object
  Variables mpp_vars(iteratedModel->current_variables().shared_data());
  if (x_space)
    mpp_vars.continuous_variables(mostProbPointX);
  else
    mpp_vars.continuous_variables(mostProbPointU);
  ...
  set.derivative_vector(ModelUtils::continuous_variable_ids(*iteratedModel));
  ...
  if (x_space) {
    ...
    if (taylorOrder == 2) response.function_hessian(fnHessX, respFnCount);
  } else {
    response.function_gradient(fnGradU, respFnCount);
    if (taylorOrder == 2) response.function_hessian(fnHessU, respFnCount);
  }
...
void NonDLocalReliability::update_pma_maximize(const RealVector& mpp_u,
  ...
  if (levelCount < rl_pl_len) {
    ...
    Real gen_beta =
        requestedGenRelLevels[respFnCount][levelCount - rl_pl_bl_len];
...
void NonDLocalReliability::truth_evaluation(short mode) {
  ...
  copy_data(ModelUtils::continuous_variables(*iteratedModel), mostProbPointX);
  const Response& x_resp = iteratedModel->current_response();
  const Response& u_resp = uSpaceModel->current_response();
  if (mode & 1) computedRespLevel = x_resp.function_value(respFnCount);
  if (mode & 2) {
...
void NonDLocalReliability::RIA_objective_eval(
    const Variables& sub_model_vars, const Variables& recast_vars,
...
void NonDLocalReliability::RIA_constraint_eval(
    const Variables& sub_model_vars, const Variables& recast_vars,
    const Response& sub_model_response, Response& recast_response) {
  // --------------------------------------------------------
...
void NonDLocalReliability::PMA_objective_eval(
    const Variables& sub_model_vars, const Variables& recast_vars,
    const Response& sub_model_response, Response& recast_response) {
  // ----------------------------------
  ...
  if (asv_val & 1) {
    ...
    if (pma_max)
      recast_response.function_value(-fn, 0);
    else
      recast_response.function_value(fn, 0);
  }
  if (asv_val & 2) {  // dG/du: no additional transformation needed
    if (pma_max) {
      ...
      for (i = 0; i < num_vars; ++i) recast_grad[i] = -fn_grad_u[i];
    } else
  ...
  if (asv_val & 4) {  // d^2G/du^2: no additional transformation needed
    if (pma_max) {
      ...
      for (i = 0; i < num_vars; ++i)
        for (j = 0; j <= i; ++j) recast_hess(i, j) = -fn_hess_u(i, j);
    } else
...
void NonDLocalReliability::PMA_constraint_eval(
    const Variables& sub_model_vars, const Variables& recast_vars,
    const Response& sub_model_response, Response& recast_response) {
  // ------------------------------------------------------
  ...
  if (asv_val & 1) {
    ...
    for (i = 0; i < num_vars; ++i)
      beta_sq +=
          std::pow(u[i], 2);  // use beta^2 to avoid singular grad @ origin
...
void NonDLocalReliability::PMA2_constraint_eval(
    const Variables& sub_model_vars, const Variables& recast_vars,
    const Response& sub_model_response, Response& recast_response) {
  // -----------------------------------------------------
  ...
  Real computed_prob_level =
      (nondLocRelInstance->mppSearchType == SUBMETHOD_NO_APPROX)
          ? nondLocRelInstance->probability(
                comp_rel, cdf, u, fn_grad_u,
  ...
  if (asv_val & 2) {
    ...
    Real factor = -nondLocRelInstance->dp2_dbeta_factor(comp_rel, cdf) /
                  comp_rel /
                  Pecos::NormalRandomVariable::std_pdf(-comp_gen_rel);
    size_t i, num_vars = u.length();
    RealVector grad_f = recast_response.function_gradient_view(1);
    for (i = 0; i < num_vars; ++i) grad_f[i] = factor * u[i];
#ifdef DEBUG
...
void NonDLocalReliability::PMA2_set_mapping(const Variables& recast_vars,
...
void NonDLocalReliability::dg_ds_eval(const RealVector& x_vars,
  ...
  if (final_stat_grad.empty()) final_stat_grad.resize(num_final_grad_vars);
  ...
  if (dist_param_derivs == NO_DERIVS || dist_param_derivs == MIXED_DERIVS) {
    ...
    if (mppSearchType && mppSearchType < SUBMETHOD_NO_APPROX)
      uSpaceModel->component_parallel_mode(TRUTH_MODEL_MODE);
    ModelUtils::continuous_variables(*iteratedModel, x_vars);
    ...
    SizetMultiArrayConstView icv_ids =
        ModelUtils::inactive_continuous_variable_ids(*iteratedModel);
...
Real NonDLocalReliability::signed_norm(Real norm_mpp_u, const RealVector& mpp_u,
                                       const RealVector& fn_grad_u,
                                       bool cdf_flag) {
...
Real NonDLocalReliability::probability(Real beta, bool cdf_flag,
  ...
  if (integrationOrder == 2 && curvatureDataAvailable) {
    ...
    for (i = 0; i < num_kappa; i++) {
      ...
      if (1. + kterm * kappa[i] <= curvatureThresh) apply_correction = false;
    }
  ...
  if (integrationRefinement &&                                   // IS/AIS/MMAIS
    ...
    import_sampler_rep->initialize(mpp_u, x_data_flag, respFnCount, p,
                                   requestedTargetLevel);
    ParLevLIter pl_iter = methodPCIter->mi_parallel_level_iterator(miPLIndex);
  ...
  if (outputLevel > NORMAL_OUTPUT) Cout << '\n';
#ifdef DEBUG
  if (integrationOrder == 2 && curvatureDataAvailable)
...
Real NonDLocalReliability::dp2_dbeta_factor(Real beta, bool cdf_flag) {
  ...
  size_t i, j, num_kappa;
  Real kterm, dpsi_m_beta_dbeta, beta_corr;
  RealVector kappa;
  if (curvatureDataAvailable) {
    ...
    switch (secondOrderIntType) {
      ...
      case HOHENRACK:
        ...
        dpsi_m_beta_dbeta =
            kterm * (beta_corr - kterm);  // orig (kterm + beta_corr)
        break;
    }
    ...
    if (apply_correction) {
      ...
      for (i = 0; i < num_kappa; ++i) {
        ...
        for (j = 0; j < num_kappa; ++j)
          if (j != i) prod1 /= std::sqrt(1. + kterm * kappa[j]);
        prod1 *= kappa[i] / 2. / std::pow(1. + ktk, 1.5);
        if (secondOrderIntType != BREITUNG) prod1 *= dpsi_m_beta_dbeta;
        sum -= prod1;
      }
...
Real NonDLocalReliability::reliability(Real p, bool cdf_flag,
                                       const RealVector& mpp_u,
                                       const RealVector& fn_grad_u,
                                       const RealSymMatrix& fn_hess_u) {
      ...
      while (!reduction && !terminate) {  // enter loop even if converged
        ...
        if (!terminate) {
          if (std::fabs(res_step) < std::fabs(res)) {  // accept step
            ...
            if (backtrack_iters++ >=
                max_iters) {  // backtrack iter must complete
...
bool NonDLocalReliability::reliability_residual(const Real& p, const Real& beta,
...
Real NonDLocalReliability::reliability_residual_derivative(
    const Real& p, const Real& beta, const RealVector& kappa) {
  ...
  for (i = 0; i < num_kappa; i++) {
    prod = 1.;
    for (j = 0; j < num_kappa; j++)
      if (j != i) prod *= std::sqrt(1. + kterm * kappa[j]);
    prod *= kappa[i] / 2. / std::sqrt(1. + kterm * kappa[i]);
    if (secondOrderIntType != BREITUNG) prod *= dpsi_m_beta_dbeta;
    sum += prod;
  }
...
void NonDLocalReliability::principal_curvatures(const RealVector& mpp_u,
                                                const RealVector& fn_grad_u,
                                                const RealSymMatrix& fn_hess_u,
                                                RealVector& kappa_u) {
  ...
  for (i = 0; i < num_kappa; i++) R0(i, i) = 1.;
  ...
  for (i = num_vars - 2; i >= 0; i--) {   // update the ith row vector
    ...
    Real len = 0.;
    for (j = 0; j < num_vars; j++) len += std::pow(R(i, j), 2);
    len = std::sqrt(len);
    for (j = 0; j < num_vars; j++) R(i, j) /= len;
  }
  // Cout << "\nR:" << R;
  ...
  if (kappa_u.length() != num_kappa) kappa_u.sizeUninitialized(num_kappa);
      ...
      if (Pecos::is_small(std_dev))
        ...
        for (j = 0; j < numContinuousVars; j++)
          s << "  Importance Factor for " << std::setiosflags(std::ios::left)
            ...
            << std::resetiosflags(std::ios::adjustfield) << std::setw(width)
            << imp_fact_i[j] << '\n';
