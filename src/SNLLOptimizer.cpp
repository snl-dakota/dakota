...
static const char rcsId[] =
    "@(#) $Id: SNLLOptimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
/** This constructor is used for normal instantiations using data from
    the ProblemDescDB. */
SNLLOptimizer::SNLLOptimizer(ProblemDescDB& problem_db,
    ...
    : Optimizer(problem_db, parallel_lib, model,
      ...
      userObjective0(NULL),
      userObjective1(NULL),
      userConstraint0(NULL),
      userConstraint1(NULL) {
  ...
  switch (methodName) {
    case OPTPP_PDS: {
      // ************************************************************************
      // NOTE: the parallelism of PDS is peer partition, not dedicated
      // scheduler. That is, PDS cannot use a simple launch/synchronize protocol
      // like most other optimizers in DAKOTA.  Rather it will be necessary to
      // implement a communicator partition for the PDS processors (with hidden
      // layers of underlying parallelism) as was done for PICO.
      // ************************************************************************
      if (numConstraints) {
    ...
    // quasi-Newton: unconstrained, bound-constrained, & nonlinear
    // interior-point
    case OPTPP_Q_NEWTON:
      if (vendorNumericalGradFlag) {
        fdnlf1 = new OPTPP::FDNLF1(numContinuousVars, nlf0_evaluator, init_fn);
        ...
        if (numConstraints) {  // nonlinear interior-point
          ...
          if (outputLevel == DEBUG_OUTPUT)
            Cout
                << "Instantiating OptBCQNewton optimizer with FDNLF1 evaluator."
                << '\n';
          ...
          if (searchStrat == OPTPP::TrustRegion)
            optbcqnewton->setTRSize(maxStep);
        } else {  // unconstrained
          if (numContinuousVars < LARGE_SCALE) {
            if (outputLevel == DEBUG_OUTPUT)
              Cout
                  << "Instantiating OptQNewton optimizer with FDNLF1 evaluator."
                  << '\n';
            theOptimizer = optqnewton = new OPTPP::OptQNewton(fdnlf1);
            optqnewton->setSearchStrategy(searchStrat);
            if (searchStrat == OPTPP::TrustRegion)
              optqnewton->setTRSize(maxStep);
          } else {
            if (outputLevel == DEBUG_OUTPUT)
              Cout << "Instantiating OptLBFGS optimizer with FDNLF1 "
                      "evaluator.\n";
    ...
    case OPTPP_FD_NEWTON:
      if (vendorNumericalGradFlag) {
        ...
        if (numConstraints) {  // nonlinear interior-point
          if (outputLevel == DEBUG_OUTPUT)
            Cout
                << "Instantiating OptFDNIPS optimizer with FDNLF1 evaluator.\n";
          ...
          if (outputLevel == DEBUG_OUTPUT)
            Cout << "Instantiating OptBCFDNewton optimizer with FDNLF1 "
                    "evaluator."
                 << '\n';
        ...
        if (numConstraints) {  // nonlinear interior-point
          ...
          if (outputLevel == DEBUG_OUTPUT)
            Cout
                << "Instantiating OptFDNewton optimizer with NLF1 evaluator.\n";
      ...
      if (numConstraints) {  // nonlinear interior-point
        ...
        // optfdnips->setSearchStrategy(searchStrat);// search strat. not
        // supported
        ...
        if (searchStrat == OPTPP::TrustRegion)
          optbcfdnewton->setTRSize(maxStep);
      } else {  // unconstrained
  ...
  snll_post_instantiate(numContinuousVars, vendorNumericalGradFlag,
                        ...
                        theOptimizer, nlfObjective, fdnlf1, fdnlf1Con);
}

/** This is an alternate constructor for instantiations on the fly
    using a Model but no ProblemDescDB. */
SNLLOptimizer::SNLLOptimizer(const String& method_string,
                             std::shared_ptr<Model> model)
  ...
  switch (methodName) {
    case OPTPP_Q_NEWTON:
      default_instantiate_q_newton(nlf1_evaluator);
      if (numConstraints) default_instantiate_constraint(constraint1_evaluator);
      break;
    case OPTPP_NEWTON:
      default_instantiate_newton(nlf2_evaluator, constraint2_evaluator);
      break;
...
SNLLOptimizer::SNLLOptimizer(
    ...
    size_t max_iter, size_t max_eval, Real conv_tol, Real grad_tol,
    Real max_step)
    :  // use default SNLLBase ctor
      ...
      fdnlf1Con(NULL),
      theOptimizer(NULL),
      setUpType("user_functions"),
      linIneqCoeffs(lin_ineq_coeffs),
      linIneqLowerBnds(lin_ineq_l_bnds),
      linIneqUpperBnds(lin_ineq_u_bnds),
      linEqCoeffs(lin_eq_coeffs),
      linEqTargets(lin_eq_tgts),
      nlnIneqLowerBnds(nln_ineq_l_bnds),
      nlnIneqUpperBnds(nln_ineq_u_bnds),
      nlnEqTargets(nln_eq_tgts),
      userObjective0(NULL),
      userObjective1(nlf1_obj_eval),
      userConstraint0(NULL),
      userConstraint1(nlf1_con_eval)  //,
// fdStepSize(fdss), fdIntervalType(interval_type)
{
  maxIterations = max_iter;
  maxFunctionEvals = max_eval;
  convergenceTol = conv_tol;
  gradientTol = grad_tol;
  maxStep = max_step;
  ...
  if (numConstraints) default_instantiate_constraint(nlf1_con_eval);
  ...
  snll_post_instantiate(numContinuousVars, vendorNumericalGradFlag,
                        ...
                        numConstraints, outputLevel, theOptimizer, nlfObjective,
                        fdnlf1, fdnlf1Con);
}
...
SNLLOptimizer::SNLLOptimizer(
    ...
    void (*nlf1_con_eval)(int mode, int n, const RealVector& x, RealVector& g,
                          RealMatrix& grad_g, int& result_mode),
    const RealVector& fdss, const String& interval_type, size_t max_iter,
    size_t max_eval, Real conv_tol, Real grad_tol, Real max_step)
    :  // use default SNLLBase ctor
      Optimizer(OPTPP_Q_NEWTON, initial_pt.length(), 0, 0, 0,
                lin_ineq_coeffs.numRows(), lin_eq_coeffs.numRows(),
                nln_ineq_l_bnds.length(), nln_eq_tgts.length(),
      ...
      nlnIneqUpperBnds(nln_ineq_u_bnds),
      nlnEqTargets(nln_eq_tgts),
      userObjective0(nlf0_obj_eval),
      userObjective1(NULL),
      userConstraint0(NULL),
      userConstraint1(nlf1_con_eval),
      fdStepSize(fdss),
      fdIntervalType(interval_type) {
  maxIterations = max_iter;
  maxFunctionEvals = max_eval;
  convergenceTol = conv_tol;
  gradientTol = grad_tol;
...
SNLLOptimizer::SNLLOptimizer(
  ...
  // quasi-Newton: unconstrained, bound-constrained, & nonlinear interior-point
  default_instantiate_q_newton(nlf1_obj_eval);
  if (numConstraints) default_instantiate_constraint(nlf0_con_eval);
...
void SNLLOptimizer::update_callback_data(
  ...
  bool reshape = false;
  size_t num_cv = cv_initial.length(), num_lin_ineq = lin_ineq_coeffs.numRows(),
  ...
  if (reshape) {
    ...
    // else {
    if (userObjective0)
      default_instantiate_q_newton(userObjective0);
    else if (userObjective1)
      default_instantiate_q_newton(userObjective1);
    if (numConstraints) {
      if (userConstraint0)
        default_instantiate_constraint(userConstraint0);
      else if (userConstraint1)
        default_instantiate_constraint(userConstraint1);
    }
    //}
    snll_post_instantiate(
        numContinuousVars, vendorNumericalGradFlag, fdIntervalType, fdStepSize,
        maxIterations, maxFunctionEvals, convergenceTol, gradientTol, maxStep,
        boundConstraintFlag, numConstraints, outputLevel, theOptimizer,
...
void SNLLOptimizer::default_instantiate_constraint(void (*con_eval)(
    int n, const RealVector& x, RealVector& g, int& result_mode)) {
  nlfConstraint = fdnlf1Con = new OPTPP::FDNLF1(
      numContinuousVars, numNonlinearConstraints, con_eval, init_fn);
  nlpConstraint = new OPTPP::NLP(fdnlf1Con);
}
...
void SNLLOptimizer::default_instantiate_constraint(
    void (*con_eval)(int mode, int n, const RealVector& x, RealVector& g,
                     RealMatrix& grad_g, int& result_mode)) {
  nlfConstraint = nlf1Con = new OPTPP::NLF1(
      numContinuousVars, numNonlinearConstraints, con_eval, init_fn);
  nlpConstraint = new OPTPP::NLP(nlf1Con);
}
...
void SNLLOptimizer::default_instantiate_newton(
    ...
    void (*con_eval)(int mode, int n, const RealVector& x, RealVector& g,
                     ...
                     int& result_mode)) {
  nlfObjective = nlf2 = new OPTPP::NLF2(numContinuousVars, obj_eval, init_fn);
  if (numConstraints) {  // nonlinear interior-point
    if (outputLevel == DEBUG_OUTPUT)
    ...
    nlfConstraint = nlf2Con = new OPTPP::NLF2(
        numContinuousVars, numNonlinearConstraints, con_eval, init_fn);
...
void SNLLOptimizer::nlf0_evaluator(int n, const RealVector& x, double& f,
                                   int& result_mode) {
  ...
  if (!snllOptInstance->numNonlinearConstraints ||
    ...
    ModelUtils::continuous_variables(*snllOptInstance->iteratedModel, x);
    ...
    // if (snllOptInstance->numNonlinearConstraints) // turn off asv for
    // constr's
  ...
  f = (!max_sense.empty() && max_sense[0])
          ? -snllOptInstance->iteratedModel->current_response().function_value(
                0)
          : snllOptInstance->iteratedModel->current_response().function_value(
                0);
...
void SNLLOptimizer::nlf1_evaluator(int mode, int n, const RealVector& x,
...
void SNLLOptimizer::nlf2_evaluator(int mode, int n, const RealVector& x,
                                   double& f, RealVector& grad_f,
...
void SNLLOptimizer::constraint0_evaluator(int n, const RealVector& x,
...
void SNLLOptimizer::constraint1_evaluator(int mode, int n, const RealVector& x,
  ...
  if (mode & 1) {  // 1st bit is present, mode = 1 or 3
    snllOptInstance->copy_con_vals_dak_to_optpp(
        local_response.function_values(), g, snllOptInstance->numObjectiveFns);
    result_mode = OPTPP::NLPFunction;
  }
  if (mode & 2) {  // 2nd bit is present, mode = 2 or 3
...
void SNLLOptimizer::constraint2_evaluator(
...
void SNLLOptimizer::core_run() { theOptimizer->optimize(); }
...
void SNLLOptimizer::post_run(std::ostream& s) {
  ...
  if (!localObjectiveRecast) {  // else local_objective_recast_retrieve()
    ...
    if (iteratedModel) {
      ...
      best_fns[0] = (!max_sense.empty() && max_sense[0]) ? -nlfObjective->getF()
                                                         : nlfObjective->getF();
    } else {
...
// This override exists purely to prevent an optimizer/minimizer from declaring
// sources when it's being used to evaluate a user-defined function (e.g.
// finding the correlation lengths of Dakota's GP).
