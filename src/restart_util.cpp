...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    general_opts.add_options()("help", "show dakota_restart_util help message")(
        "custom_annotated",
    ...
    hidden_opts.add_options()("util_command",
                              bpo::value<std::string>(&util_command),
    ...
    if (vm.count("freeform")) freeform = true;
...
void print_usage(std::ostream& s) {
  s << "Usage:\n  dakota_restart_util command <arg1> [<arg2> <arg3> ...] "
       "--options\n"
...
void print_restart(StringArray pos_args, String print_dest) {
  ...
  try {
    ...
    if (print_dest == "neutral_file") neutral_file_stream.close();
...
void print_restart_pdb(StringArray pos_args, String print_dest) {
  ...
  RestartVersion rst_ver = RestartVersion::check_restart_version(pos_args[0]);
  ...
  const StringArray& fn_labels = prp_iter->response().function_labels();
  ...
  for (i = 0; i < num_evals; ++i, ++prp_iter) {
    ...
    for (j = 0; j < num_cv; ++j) tabular_rdata[j][i] = local_c_vars[j];
    for (j = 0; j < num_div; ++j) tabular_idata[j][i] = local_di_vars[j];
    for (j = 0; j < num_drv; ++j)
    ...
    while (restart_input_fs.good() && !restart_input_fs.eof()) {
      ...
      if (num_evals == 0 || new_interf != curr_interf) {
        ...
        if (curr_vars.all_continuous_variable_labels() != curr_acv_labels ||
            ...
            curr_vars.all_discrete_string_variable_labels() !=
                curr_adsv_labels ||
      ...
      current_pair.write_tabular(tabular_text,
                                 tabular_format);  // also writes IDs
      ++num_evals;
...
void read_neutral(StringArray pos_args) {
  ...
  try {
    ...
    Cerr << "\nError: Could not write restart file '" << write_restart_filename
         << "'.\nDetails (Boost archive exception): " << e.what() << std::endl;
  }
}
...
void repair_restart(StringArray pos_args, String identifier_type) {
  ...
  if (identifier_type == "by_value") {
    ...
    by_value = false;
    write_restart_filename = pos_args.back();
    pos_args.pop_back();
    read_restart_filename = pos_args.back();
    pos_args.pop_back();
    try {
      for (const String& pa : pos_args) bad_ids.push_back(std::stoi(pa));
    } catch (const std::logic_error& le_except) {
  ...
  try {
    ...
    cout << "Restart repair completed: " << cntr << " evaluations retrieved"
         << ", " << cntr - good_cntr << " removed, " << good_cntr
         << " saved.\n";
    restart_output_fs.close();
...
void concatenate_restart(StringArray pos_args) {
  ...
  try {
    String write_restart_filename = pos_args.back();
    pos_args.pop_back();
    std::ofstream restart_output_fs(write_restart_filename.c_str(),
                                    std::ios::binary);
    if (!restart_output_fs.good()) {
    ...
    for (const String& rst_file : pos_args) {
      RestartVersion rst_ver = RestartVersion::check_restart_version(rst_file);

      std::ifstream restart_input_fs(rst_file.c_str(), std::ios::binary);
      if (!restart_input_fs.good()) {
        Cerr << "\nError: could not open restart file '" << rst_file
             << "' for reading." << std::endl;
        exit(-1);
      }
      boost::archive::binary_iarchive restart_input_archive(restart_input_fs);
      ...
      int cntr = 0;
      restart_input_fs
          .peek();  // peek to force EOF if no records in restart file
      while (restart_input_fs.good() && !restart_input_fs.eof()) {
        ParamResponsePair current_pair;
        try {
          restart_input_archive & current_pair;
