...
class QuesoJointPdf : public QUESO::BaseJointPdf<V, M> {
 public:
  ...
  QuesoJointPdf(const char* prefix, const QUESO::VectorSet<V, M>& domainSet,
  ...
  double lnValue(const V& domainVector, const V* domainDirection, V* gradVector,
...
class QuesoVectorRV : public QUESO::BaseVectorRV<V, M> {
 public:
  ...
  QuesoVectorRV(const char* prefix, const QUESO::VectorSet<V, M>& imageSet,
                NonDQUESOBayesCalibration* nond_queso_ptr);
  //! Virtual destructor
...
class DerivInformedPropCovLogitTK
    : public QUESO::TransformedScaledCovMatrixTKGroup<V, M> {
 public:
...
class TKFactoryDIPC : public QUESO::TransitionKernelFactory {
 public:
  ...
  TKFactoryDIPC(const std::string& name)
      : QUESO::TransitionKernelFactory(name),
        nonDQUESOInstance(NULL) { /* empty ctor */ }
 ...
 protected:
  ...
  virtual QUESO::SharedPtr<
      QUESO::BaseTKGroup<QUESO::GslVector, QUESO::GslMatrix> >::Type
  build_tk() override {
    QUESO::SharedPtr<
        QUESO::BaseTKGroup<QUESO::GslVector, QUESO::GslMatrix> >::Type new_tk;

    new_tk.reset(new DerivInformedPropCovTK<QUESO::GslVector, QUESO::GslMatrix>(
        this->m_options->m_prefix.c_str(), *(this->m_vectorSpace),
        *(this->m_dr_scales), *(this->m_initial_cov_matrix),
...
/// Custom Logit RW TKfactory: passed Dakota QUESO instance pointer to the TK at
/// build
/** Can't share this factory between random walk and logit as their
    constructor arguments differ */
class TKFactoryDIPCLogit : public QUESO::TransitionKernelFactory {
 ...
 protected:
  ...
  build_tk() override {
    ...
    new_tk.reset(
        new DerivInformedPropCovLogitTK<QUESO::GslVector, QUESO::GslMatrix>(
            this->m_options->m_prefix.c_str(),
            (this->m_target_pdf->domainSet()), *(this->m_dr_scales),
            *(this->m_initial_cov_matrix), this->nonDQUESOInstance));
