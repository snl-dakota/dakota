  ...
  try {
    ...
    gradient_only.receive([&](auto available) {
      if constexpr( available ) {
        ...
        dispatch_count++; // Still valid scenario, just different method
      }
    });
    ...
    hessian_no_grad.receive([&](auto available) {
      if constexpr( available ) {
        ...
        dispatch_count++; // Still valid scenario, just standard case
      }
    });
    ...
    std::cout << "✓ Algorithm selection scenarios: " << (dispatch_count == 4) << std::endl;
    ...
    bool overall_success = all_correct && (dispatch_count == 4) && 
