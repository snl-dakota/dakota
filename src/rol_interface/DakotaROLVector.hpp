...
inline Dakota::RealVector& get_vector(
    ROL::Ptr<ROL::Vector<Dakota::Real>>& x) noexcept;

inline const Dakota::RealVector& get_vector(
    const ROL::Vector<Dakota::Real>& x) noexcept;

inline const Dakota::RealVector& get_vector(
    const ROL::Ptr<const ROL::Vector<Dakota::Real>>& x) noexcept;

inline Dakota::Real* get_vector_values(ROL::Vector<Dakota::Real>& x) noexcept;

inline Dakota::Real* get_vector_values(
    ROL::Ptr<ROL::Vector<Dakota::Real>>& x) noexcept;

inline const Dakota::Real* get_vector_values(
    const ROL::Vector<Dakota::Real>& x) noexcept;
...
class Vector : public ROL::Vector<Dakota::Real> {
 ...
 public:
  Vector(const ROL::Ptr<Dakota::RealVector>& vec) : vec_(vec) {}
  ...
  Dakota::Real norm() const override { return std::sqrt(vec_->dot(*vec_)); }

  void set(const ROL::Vector<Dakota::Real>& x) override {
    *vec_ = get_vector(x);
  }

  void set(const Dakota::RealVector& x) { *vec_ = x; }

  void plus(const ROL::Vector<Dakota::Real>& x) override {
    *vec_ += get_vector(x);
  }

  void scale(const Dakota::Real alpha) override { vec_->scale(alpha); }

  void setScalar(const Dakota::Real alpha) override { vec_->putScalar(alpha); }

  void zero() override { vec_->putScalar(0); }

  int dimension() const override { return vec_->length(); }
  ...
  void randomize(const Dakota::Real l = 0.0,
    ...
    Dakota::Real x(0);
    for (int i = 0; i < vec_->length(); ++i) {
      x = static_cast<Dakota::Real>(rand()) /
          static_cast<Dakota::Real>(RAND_MAX);
      (*vec_)(i) = a * x + b;
    }
  }
  ...
  ROL::Ptr<Dakota::RealVector> getVector() { return vec_; }

  ROL::Ptr<const Dakota::RealVector> getVector() const { return vec_; }

  void print(std::ostream& outStream) const override { vec_->print(outStream); }

  // Add element access operators to circumvent needing to create and get
  // Dakota::RealVectors
  Dakota::Real& operator()(int i) { return (*vec_)(i); }

  const Dakota::Real& operator()(int i) const { return (*vec_)(i); }
