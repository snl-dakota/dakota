...
class Hessian : public ROL::LinearOperator<Dakota::Real> {
 public:
  Hessian() = delete;
  Hessian(ModelInterface* model_interface, int num_rows,
          bool has_inverse = false);

  virtual ~Hessian() = default;

  void apply(ROL::Vector<Dakota::Real>& Hv, const ROL::Vector<Dakota::Real>& v,
 ...
 private:
  ...
  std::unique_ptr<Dakota::RealMatrix>
      workMat;                  // LU Storage for Hessian inversion
