...
ModelInterface::ModelInterface(Optimizer* opt)
  ...
  opt->problem =
      makePtr<ROL::Problem<Dakota::Real>>(obj, obj->make_opt_vector());
  ...
  if (has_linear_equality) {
    ...
    problem->addLinearConstraint(econ->get_name(), econ,
                                 econ->make_lagrange_multiplier());
  }

  if (has_linear_inequality) {
    auto icon = makePtr<LinearInequalityConstraint>(this);
    auto l =
        Vector::make_from(dakotaModel.linear_ineq_constraint_lower_bounds());
    auto u =
        Vector::make_from(dakotaModel.linear_ineq_constraint_upper_bounds());
    auto ibnd = ROL::makePtr<Bounds>(l, u);
    problem->addLinearConstraint(icon->get_name(), icon,
                                 icon->make_lagrange_multiplier(), ibnd);
  }

  if (has_nonlinear_equality) {
    auto econ = makePtr<NonlinearEqualityConstraint>(this);
    problem->addConstraint(econ->get_name(), econ,
                           econ->make_lagrange_multiplier());
  }
  ...
  if (has_nonlinear_inequality) {
    ...
    auto l =
        Vector::make_from(dakotaModel.nonlinear_ineq_constraint_lower_bounds());
    auto u =
        Vector::make_from(dakotaModel.nonlinear_ineq_constraint_upper_bounds());
    auto ibnd = ROL::makePtr<Bounds>(l, u);
    problem->addConstraint(icon->get_name(), icon,
                           icon->make_lagrange_multiplier(), ibnd);
  }
...
void ModelInterface::set_default_parameters(Optimizer* opt) {
  ...
  // Set the verbosity level.
  bool has_verbose_output =
      Dakota::Iterator::outputLevel >= Dakota::VERBOSE_OUTPUT;
  bool has_normal_output =
      Dakota::Iterator::outputLevel >= Dakota::NORMAL_OUTPUT;
  ...
  status.set("Gradient Tolerance",
             prob_desc_db.get_real("method.gradient_tolerance"));
  status.set("Constraint Tolerance",
             prob_desc_db.get_real("method.constraint_tolerance"));
  status.set("Step Tolerance",
             prob_desc_db.get_real("method.variable_tolerance"));
  ...
  String adv_opts_file =
      prob_desc_db.get_string("method.advanced_options_file");
  ...
  void ModelInterface::set_jacobian(NonlinearInequalityConstraint * con) {
    ...
    // The inequality constraint jacobian data begins after the objective
    // gradient data
  ...
  void ModelInterface::set_jacobian(NonlinearEqualityConstraint * con) {
    ...
    int num_ineq_con = dakotaModel.num_nonlinear_ineq_constraints();
    const Dakota::RealMatrix& J = resp.function_gradients();
    // The equality constraint jacobian data begins after the objective gradient
    // data and the inequality constraint jacobian data (if it exists)
    auto jvalues = J.values() + num_opt * (1 + num_ineq_con);
    con->set_jacobian(jvalues);
  }

  ModelInterface::update(const Dakota::RealVector& x, UpdateType type,
                         int iter) {
    //  switch(type) {
  ...
  const Dakota::RealSymMatrixArray& ModelInterface::get_hessians()
      const noexcept {
