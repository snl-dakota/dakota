...
Constraint::Constraint(ModelInterface* model_interface,
                       Constraint::Type con_type, std::string con_name)
    : targetData(nullptr),
      valueData(nullptr),
      jacobianData(nullptr),
      modelInterface(model_interface),
      conType(con_type),
      conName(con_name) {}  // Constraint::Constraint
...
void Constraint::update(const Dakota::RealVector& x, ROL::UpdateType type,
                        int iter) {
...
void Constraint::value(ROL::Vector<Dakota::Real>& c,
                       const ROL::Vector<Dakota::Real>& x, Dakota::Real& tol) {
  value(get_vector(c), get_vector(x), tol);
}  // Constraint:::value

void Constraint::value(Dakota::RealVector& c, const Dakota::RealVector& x,
                       Dakota::Real& tol) {
...
void Constraint::applyJacobian(Dakota::RealVector& jv,
                               ...
                               const Dakota::RealVector& x, Dakota::Real& tol) {
  auto& blas = modelInterface->get_blas();
  auto jvdata = jv.values();
  auto vdata = v.values();
  blas.GEMV(Teuchos::NO_TRANS, numCon, numOpt, 1, jacobianData, numCon, vdata,
            1, 0, jvdata, 1);
}  // Constraint::applyJacobian
...
void Constraint::applyAdjointJacobian(Dakota::RealVector& ajv,
  ...
  auto vdata = v.values();
  blas.GEMV(Teuchos::TRANS, numCon, numOpt, 1, jacobianData, numCon, vdata, 1,
            0, ajvdata, 1);
}  // Constraint::applyJacobian
...
void Constraint::applyAdjointHessian(ROL::Vector<Dakota::Real>& ahuv,
  ...
  if (modelInterface->has_second_derivatives()) {
    applyAdjointHessian(get_vector(ahuv), get_vector(u), get_vector(v),
                        get_vector(x), tol);
  } else {
...
void Constraint::applyAdjointHessian(Dakota::RealVector& ahuv,
  ...
  for (int i = 0; i < numCon; ++i) {
    ...
    blas.SYMM(Teuchos::LEFT_SIDE, Teuchos::UPPER_TRI, numOpt, 1, u[i], hdata,
              vdata, numOpt, beta, ahuvdata, numOpt);
  }

}  // Constraint::applyAdjointHessian

ROL::Ptr<ROL::Vector<Dakota::Real>> Constraint::make_lagrange_multiplier()
    const {
