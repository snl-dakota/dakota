...
//- Class:        Constraint
//- Description:  Base class for providing a ROL::Constraint interface to
// Dakota's
...
class Constraint : public ROL::Constraint<Dakota::Real> {
 public:
  // This Constraint type enum convention allows bit masking to extract
  // properties
  ...
  Constraint() = delete;
  Constraint(ModelInterface* model_interface, Type con_type,
             std::string con_name = "Constraint");
  ...
  void update(const ROL::Vector<Dakota::Real>& x, ROL::UpdateType type,
              int iter = -1) override;

  virtual void update(const Dakota::RealVector& x, ROL::UpdateType type,
                      int iter = -1);

  void value(ROL::Vector<Dakota::Real>& c, const ROL::Vector<Dakota::Real>& x,
