...
class Objective : public ROL::Objective<Dakota::Real> {
 public:
  Objective() = delete;
  Objective(const ROL::Ptr<ModelInterface>& model_interface, bool has_inverse);
  ...
  virtual Dakota::Real value(const Dakota::RealVector& x, Dakota::Real& tol);

  void gradient(ROL::Vector<Dakota::Real>& g,
                const ROL::Vector<Dakota::Real>& x,
                Dakota::Real& tol) override final;

  virtual void gradient(Dakota::RealVector& g, const Dakota::RealVector& x,
                        Dakota::Real& tol);
  ...
  inline auto make_opt_vector() noexcept { return make_vector(numOpt); }
