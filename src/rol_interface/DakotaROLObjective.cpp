...
Objective::Objective(ModelInterface* model_interface, bool has_inverse)
...
void Objective::set_dimension(int num_opt) { numOpt = numOpt; }

void Objective::update(const ROL::Vector<Dakota::Real>& x, ROL::UpdateType type,
                       int iter) {
...
void Objective::update(const Dakota::RealVector& x, ROL::UpdateType type,
                       int iter) {
  modelInterface->update(x, type, iter);
}  // Objective::update

Dakota::Real Objective::value(const Dakota::RealVector& x, Dakota::Real& tol) {
...
void Objective::gradient(Dakota::RealVector& g, const Dakota::RealVector& x,
                         Dakota::Real& tol) {
