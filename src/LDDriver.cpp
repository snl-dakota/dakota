...
LDDriver::LDDriver(ProblemDescDB& problem_db)
    : sequence_(problem_db.get_bool("method.rank_1_lattice")
                    ? static_cast<LowDiscrepancySequence*>(
...
int LDDriver::get_seed() { return sequence_->get_seed(); }

// Set the seed of the wrapped low-discrepancy sequence
void LDDriver::set_seed(int seed) { sequence_->set_seed(seed); }

// Generate samples of the wrapped low-discrepancy sequence
void LDDriver::generate_samples(
    std::shared_ptr<Model>
        model,  // The model to get the multivariate distribution from
    const size_t numSamples,  // Number of samples to generate
    RealMatrix& sampleMatrix  // Matrix to store the generated samples (needs to
                              // be resized to numVariables x numSamples)
) {
  ...
  // Transform points from [0, 1) to the multivariate distribution in the given
  // model
...
void LDDriver::check_support(Pecos::MultivariateDistribution& mv_dist) {
  ...
  for (Pecos::RandomVariable variable : variables) {
    ...
    if ((variable_type != Pecos::STD_NORMAL) &&
        ...
        (variable_type != Pecos::STD_BETA) && (variable_type != Pecos::BETA) &&
        (variable_type != Pecos::STD_GAMMA) &&
        (variable_type != Pecos::GAMMA) && (variable_type != Pecos::GUMBEL) &&
...
void LDDriver::reshape_sample_matrix(RealMatrix& sampleMatrix, int numRows,
                                     int numCols) {
...
// Function to scale a given sample matrix from a uniform distribution over [0,
// 1) to a uniform distribution between given lower and upper bounds
void LDDriver::scale(
    const RealVector& lowerBounds, const RealVector& upperBounds,
    RealMatrix& sampleMatrix  // The matrix of samples to transform (shape
                              // numVariables x numSamples)
) {
  ...
  for (size_t col = 0; col < numSamples; col++) {
    ...
    for (size_t row = 0; row < numVariables; row++) {
      ...
      sampleMatrix[col][row] =
          sampleMatrix[col][row] * (upperBound - lowerBound) + lowerBound;
    }
  }
}

// Function to transform a given sample matrix from a uniform distribution over
// [0, 1) to the multivariate distribution defined by the given model
void LDDriver::transform(
    std::shared_ptr<Model>
        model,  // The model to get the target multivariate distribution from
    RealMatrix& sampleMatrix  // The matrix of samples to transform (shape
                              // numVariables x numSamples)
) {
  ...
  lowerBounds = -1.0;
  upperBounds = 1.0;
  scale(lowerBounds, upperBounds,
        sampleMatrix);  // transform from [0, 1) to [-1, 1)
  ...
  if (model->multivariate_distribution().correlation()) {
    ...
    for (size_t sample = 0; sample < numSamples; ++sample) {
      ...
      uNataf.trans_U_to_X(uSample, ModelUtils::continuous_variable_ids(*model),
    ...
    for (size_t sample = 0; sample < numSamples; ++sample) {
      RealVector uSample(Teuchos::Copy, sampleMatrix[sample], numVariables);
      RealVector xSample(Teuchos::View, sampleMatrix[sample], numVariables);
      vNataf.trans_U_to_X(uSample, ModelUtils::continuous_variable_ids(*model),
                          xSample, ModelUtils::continuous_variable_ids(*model));
    }
