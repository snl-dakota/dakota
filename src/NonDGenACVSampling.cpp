...
static const char rcsId[] =
    "@(#) $Id: NonDGenACVSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDGenACVSampling::NonDGenACVSampling(ProblemDescDB& problem_db,
  ...
  switch (methodName) {
    case MULTILEVEL_SAMPLING: {  // MLMC promotion: hierarch search, model
                                 // selection
      ...
      // > for model graph search on, ACV-RD searches all graphs whereas
      // weighted
      //   MLMC is restricted to the hierarchical subset
      dagWidthLimit = 1;
      dagDepthLimit = numApprox;  // a hierarchical DAG
      mlmfSubMethod = SUBMETHOD_ACV_RD;
      // check for unsupported allocation targets from MLMC spec
      bool err_flag = false;
      if (problem_db.get_short("method.nond.allocation_target") !=
          TARGET_MEAN) {
    ...
    case MULTIFIDELITY_SAMPLING:  // MFMC promotion: hierarch search, model
                                  // select
      ...
      dagWidthLimit = 1;
      dagDepthLimit = numApprox;  // a hierarchical DAG
      mlmfSubMethod = SUBMETHOD_ACV_MF;
      break;
    default:  // not a promotion: ACV specification + search options
      ...
      switch (dagRecursionType) {
        ...
        case FULL_GRAPH_RECURSION:
          dagDepthLimit = dagWidthLimit = numApprox;
          break;
      }
      break;
  }
  ...
  */
  ...
  switch (modelSelectType) {
    case ALL_MODEL_COMBINATIONS: {
      ...
      UShort2DArray tp;
      UShortArray tp_orders(numApprox, 1);
      ...
      for (i = 0; i < num_tp; ++i) {  // include first = {0} --> retain MC case
        ...
        UShortArraySet& mapped_set = modelDAGs[nodes];
        mapped_set.clear();
    ...
    case NO_MODEL_SELECTION:
      ...
      generate_dags(root, nodes, modelDAGs[nodes]);
      break;
  }
  ...
  for (d_cit = modelDAGs.begin(); d_cit != modelDAGs.end(); ++d_cit) {
    ...
    if (approx_set.size()) {  // suppress MC case
      dag_set_size = dag_set.size();
      total_dag_size += dag_set_size;
      Cout << "For approximation set:\n"
           << approx_set << "searching array of DAGs of size " << dag_set_size;
...
void NonDGenACVSampling::prune_ensembles(const UShortArray& active_approx_set) {
  ...
  for (i = 0; i < numApprox; ++i)
    if (cntr < num_active && active_approx_set[cntr] == i)
      ++cntr;
    else
      inactive_approx_set.push_back(i);
...
void NonDGenACVSampling::generate_dags(unsigned short root,
...
void NonDGenACVSampling::generate_sub_trees(unsigned short root,
                                            const UShortArray& nodes,
  ...
  Pecos::SharedPolyApproxData::tensor_product_multi_index(tp1_orders, tp1,
                                                          true);
  size_t j, k, num_tp1 = tp1.size(), num_tp2, num_sub_roots, num_sub_nodes;
  unsigned short node_j, dm1 = depth - 1;
  for (i = 1; i < num_tp1;
       ++i) {  // skip 1st entry ({0} = no sub-root is invalid)
    ...
    for (j = 0; j < num_nodes; ++j) {
      node_j = nodes[j];
      if (tp1_i[j]) {
        sub_roots.push_back(node_j);
        dag[node_j] = root;
      } else
        sub_nodes.push_back(node_j);  // "nexts"
    }
    if (sub_nodes.empty()) {  // optimize out unnecessary recursion
      ...
      Pecos::SharedPolyApproxData::tensor_product_multi_index(tp2_orders, tp2,
                                                              false);
      num_tp2 = tp2.size();
      for (j = 0; j < num_tp2;
           ++j) {  // 1st entry (all 0's) is valid for this TP
...
void NonDGenACVSampling::generate_hierarchical_sub_trees(
    unsigned short root, const UShortArray& nodes, unsigned short depth,
    UShortArray& dag, UShortArraySet& dag_set) {
  ...
  num_sub_roots = 1;
  num_sub_nodes = num_nodes - num_sub_roots;
  sub_roots.resize(num_sub_roots);
  sub_nodes.resize(num_sub_nodes);
...
void NonDGenACVSampling::generate_reverse_dag(const UShortArray& approx_set,
  ...
  SizetArray index_map;
  inflate_approx_set(approx_set, index_map);
...
void NonDGenACVSampling::unroll_reverse_dag_from_root(unsigned short root,
...
void NonDGenACVSampling::unroll_reverse_dag_from_root(
    unsigned short root, const RealVector& avg_eval_ratios,
    UShortList& root_list) {
  ...
  std::pair<Real, unsigned short> ratio_root_pr;
  unsigned short node;
  ...
  for (; l_it != default_root_list.end(); ++l_it) {
    node = *l_it;
    ratio_root_pr.first =
        (node == root) ? 1. : avg_eval_ratios[index_map[node]];
  ...
  root_list.clear();
  for (; m_it != root_ratios.end(); ++m_it) root_list.push_back(m_it->second);
...
void NonDGenACVSampling::update_model_groups() {
  ...
  const UShortArray& approx_set = activeModelSetIter->first;
  size_t g, num_approx = approx_set.size();
  unsigned short root;
...
void NonDGenACVSampling::update_model_groups(const UShortList& root_list) {
  ...
  modelGroups.resize(num_approx + 1);
  UShortList::const_iterator r_cit;
  int g = num_approx;
...
void NonDGenACVSampling::pre_run() {
  ...
  if (modelDAGs.empty()) generate_ensembles_dags();
...
void NonDGenACVSampling::generalized_acv_online_pilot() {
  ...
  while (numSamples && mlmfIter <= maxIterations) {
    ...
    alloc_incr = (backfillFailures && mlmfIter)
                     ? one_sided_delta(N_H_alloc, hf_target, relaxFactor)
                     : numSamples;
    ...
    if (no_solve)
      ...
      for (activeModelSetIter = modelDAGs.begin();
        ...
        for (activeDAGIter = dag_set.begin(); activeDAGIter != dag_set.end();
             ++activeDAGIter) {
      ...
      if (modelSelectType) prune_ensembles(soln_key.first);
    }
  }
  restore_best();
...
void NonDGenACVSampling::generalized_acv_offline_pilot() {
  ...
  IntRealMatrixMap sum_L_pilot, sum_LH_pilot;
  IntRealSymMatrixArrayMap sum_LL_pilot;
  RealVector sum_HH_pilot;
  ...
  compute_LH_statistics(sum_L_pilot[1], sum_H_pilot[1], sum_LL_pilot[1],
                        sum_LH_pilot[1], sum_HH_pilot, N_shared_pilot, varH,
                        covLL, covLH);
  ...
  genacv_raw_moments(sum_L_pilot, sum_H_pilot, sum_LL_pilot, sum_LH_pilot,
                     N_shared_pilot, sum_H, N_H_actual, sum_L_shared,
                     N_L_actual_shared, sum_L_refined, N_L_actual_refined,
                     soln);
  finalize_counts(N_L_actual_refined, NApproxAlloc);
}
...
void NonDGenACVSampling::generalized_acv_pilot_projection() {
  ...
  // Evaluate shared increment and update correlations, {eval,EstVar}_ratios
  // --------------------------------------------------------------------
  RealVector sum_H, sum_HH;
  RealMatrix sum_L, sum_LH;
  ...
  update_projected_samples(soln, soln_key.first, N_H_actual, N_H_alloc,
                           N_L_actual, NApproxAlloc, deltaNActualHF,
                           deltaEquivHF);
...
void NonDGenACVSampling::no_solve_solution() {
  ...
  if (mlmfIter == 0) {  // default to peer DAG with all models
    UShortArray peer_dag;
    peer_dag.assign(numApprox, numApprox);  // peer DAG
    ...
    if (m_it == modelDAGs.end()) {  // should not happen
      std::pair<UShortArray, UShortArraySet> soln_set(fullApproxSet,
                                                      UShortArraySet());
      ...
      if (d_it == dag_set.end()) d_it = dag_set.insert(peer_dag).first;
    }
    bestModelSetIter = m_it;
    bestDAGIter = d_it;
...
void NonDGenACVSampling::approx_increments(
    IntRealMatrixMap& sum_L_baseline, const SizetArray& N_H_actual,
    size_t N_H_alloc, IntRealMatrixMap& sum_L_shared,
    Sizet2DArray& N_L_actual_shared, IntRealMatrixMap& sum_L_refined,
  ...
  // inflate if needed
  inflate_lf_samples(approx_set, N_H_actual, N_H_alloc, N_L_actual_shared,
                     N_L_alloc);
  ...
  for (int g = num_approx - 1; g >= 0;
       --g)  // base to top, excluding all-model group
    delta_N_G[g] = group_approx_increment(
        soln_vars, approx_set, N_L_actual_refined, N_L_alloc, modelGroups[g]);
  ...
  initialize_group_sums(sum_G);
  Sizet2DArray N_G_actual;
  initialize_group_counts(N_G_actual);
  ...
  overlay_group_sums(sum_G, N_G_actual, sum_L_shared, N_L_actual_shared,
                     sum_L_refined, N_L_actual_refined);
}

void NonDGenACVSampling::genacv_raw_moments(
    const IntRealMatrixMap& sum_L_covar, const IntRealVectorMap& sum_H_covar,
  ...
  if (mlmfIter == 0) {
    ...
    switch (optSubProblemSolver) {  // no initial guess
      ...
      default: {  // competed initial guesses with (competed) local methods
        ...
        analytic_initialization_from_mfmc(approx_set, rho2_LH, avg_N_H,
                                          mf_soln);
        analytic_initialization_from_ensemble_cvmc(approx_set, *activeDAGIter,
                                                   orderedRootList, rho2_LH,
                                                   avg_N_H, cv_soln);
...
void NonDGenACVSampling::analytic_initialization_from_mfmc(
    const UShortArray& approx_set, const RealMatrix& rho2_LH, Real avg_N_H,
    MFSolutionData& soln) {
  ...
  Real hf_target;
  if (maxFunctionEvals ==
      SZ_MAX)  // HF target from GenACV estvar using MFMC soln
    hf_target =
...
void NonDGenACVSampling::analytic_initialization_from_ensemble_cvmc(
    const UShortArray& approx_set, const UShortArray& dag,
    const UShortList& root_list, const RealMatrix& rho2_LH, Real avg_N_H,
    MFSolutionData& soln) {
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "GenACV scaled initial guess from ensemble CVMC:\n"
         << "  average eval ratios:\n"
         << avg_eval_ratios << "  HF target = " << hf_target << std::endl;
}

void NonDGenACVSampling::cvmc_ensemble_solutions(
    const RealSymMatrixArray& cov_LL, const RealMatrix& cov_LH,
    const RealVector& var_H, const RealVector& cost,
    const UShortArray& approx_set, const UShortArray& dag,
    const UShortList& root_list, RealVector& avg_eval_ratios) {
  size_t qoi, source, target, d, dag_size = dag.size();
  if (avg_eval_ratios.length() != dag_size)
    avg_eval_ratios.size(dag_size);
  else
    avg_eval_ratios = 0.;
  ...
  inflate_approx_set(approx_set, index_map);
  UShortList::const_iterator r_cit;
  UShortSet::const_iterator d_cit;
  Real r_tgt;
  for (r_cit = ++root_list.begin(); r_cit != root_list.end(); ++r_cit) {
    ...
    for (d_cit = reverse_dag.begin(); d_cit != reverse_dag.end(); ++d_cit) {
      source = *d_cit;
      avg_eval_ratios[index_map[source]] *= r_tgt;
    }
  }
}

void NonDGenACVSampling::numerical_solution_counts(size_t& num_cdv,
                                                   size_t& num_lin_con,
                                                   size_t& num_nln_con) {
  ...
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT:
      ...
      for (i = 0; i < num_approx; ++i)
        if (dag[i] != numApprox) ++num_lin_con;
      break;
    case R_AND_N_NONLINEAR_CONSTRAINT:
      num_cdv = num_approx + 1;
      num_nln_con = 1;
      num_lin_con = 0;
      for (i = 0; i < num_approx; ++i)
        if (dag[i] != numApprox) ++num_lin_con;
    ...
    case N_MODEL_LINEAR_CONSTRAINT:
      num_cdv = num_lin_con = num_approx + 1;
      num_nln_con = 0;
      break;
    case N_MODEL_LINEAR_OBJECTIVE:
      num_cdv = num_approx + 1;
      num_nln_con = 1;
      num_lin_con = num_approx;
      break;
  }
}

void NonDGenACVSampling::numerical_solution_bounds_constraints(
  ...
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT: {
      ...
      lin_ineq_ub[0] =
          (avg_N_H > 1.) ?  // protect N_H==0 for offline pilot
              activeBudget / avg_N_H - 1.
                         :  // normal case
              activeBudget -
                  1.;  // bound N_H at 1 (TO DO: need to perform sample)
      for (approx = 0; approx < num_approx; ++approx)
    ...
    case N_MODEL_LINEAR_OBJECTIVE: {
      ...
      if (optSubProblemForm == N_MODEL_LINEAR_CONSTRAINT) {
        ...
        for (approx = 0; approx < num_approx; ++approx)
          lin_ineq_coeffs(0, approx) =
              sequenceCost[approx_set[approx]] / cost_H;
...
void NonDGenACVSampling::augment_linear_ineq_constraints(
...
void NonDGenACVSampling::enforce_augmented_linear_ineq_constraints(
    RealVector& avg_eval_ratios, const UShortArray& approx_set,
...
void NonDGenACVSampling::enforce_augmented_linear_ineq_constraints(
  ...
  switch (optSubProblemForm) {
    ...
    default: {  // use N directly rather than r
      ...
      SizetArray index_map;
      inflate_approx_set(approx_set, index_map);
      for (r_cit = orderedRootList.begin(); r_cit != orderedRootList.end();
           ++r_cit) {
        target = *r_cit;
        deflate_tgt = (target == numApprox) ? num_approx : index_map[target];
        N_tgt = cd_vars[deflate_tgt];
        N_tgt_nudge = N_tgt * nudge_p1;
        const UShortSet& reverse_dag = reverseActiveDAG[target];
        for (d_cit = reverse_dag.begin(); d_cit != reverse_dag.end(); ++d_cit) {
          source = *d_cit;
...
void NonDGenACVSampling::scale_to_target(Real avg_N_H, const RealVector& cost,
                                         RealVector& avg_eval_ratios,
                                         Real& avg_hf_target,
  ...
  if (avg_N_H >
    ...
    size_t approx, num_approx = approx_set.size();
    for (approx = 0; approx < num_approx; ++approx)
    ...
    for (r_cit = root_list.begin(); r_cit != root_list.end(); ++r_cit) {
      ...
      budget -= budget_decr;
      approx_inner_prod -= inner_prod_decr;
      factor = (budget / avg_N_H - 1.) / approx_inner_prod * cost_H;
    }
    if (outputLevel >= DEBUG_OUTPUT) {
      ...
      Cout << "Rescale to budget: average evaluation ratios\n"
           << avg_eval_ratios << "avg_hf_target = " << avg_hf_target
...
void NonDGenACVSampling::solve_for_C_G_c_g(RealSymMatrix& C_G, RealVector& c_g,
                                           RealVector& lhs, bool copy_C_G,
                                           bool copy_c_g) {
  ...
  if (hardenNumericSoln) {
    ...
    if (outputLevel >= DEBUG_OUTPUT)
      Cout << "GenACV pseudo-inverse solve for LHS:\n"
           << lhs << "has rcond = " << rcond << std::endl;
  } else
    cholesky_solve(C_G, lhs, c_g, copy_C_G, copy_c_g);
}

void NonDGenACVSampling::estimator_variance_ratios(const RealVector& cd_vars,
  ...
  size_t num_approx = approx_set.size();
  RealVector N_vec;
  inflate_variables(cd_vars, N_vec, approx_set);
  Real R_sq,
      N_H = N_vec[numApprox];  // R_ONLY: N_vec inflated w/ avg NLevActual
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT:
    case R_AND_N_NONLINEAR_CONSTRAINT:
      for (size_t i = 0; i < numApprox; ++i) N_vec[i] *= N_H;  // N_i = r_i * N
...
void NonDGenACVSampling::minimizer_results_to_solution_data(
  ...
  Real estvar_metric = (optSubProblemForm == N_MODEL_LINEAR_OBJECTIVE)
                           ? std::exp(fn_star[1])
                           : std::exp(fn_star(0));
  soln.estimator_variance_metric(
      estvar_metric);  // QoI-vec evaluated downstream
  ...
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT: {
      ...
      soln.anchored_solution_ratios(avg_eval_ratios, hf_target);
      soln.equivalent_hf_allocation(compute_equivalent_cost(
          hf_target, avg_eval_ratios, sequenceCost, approx_set));
      break;
    }
    case R_AND_N_NONLINEAR_CONSTRAINT: {
...
Real NonDGenACVSampling::linear_model_cost(const RealVector& N_vec) {
  ...
  Real cost_H = sequenceCost[numApprox], cost_i, N_H = N_vec[num_approx],
       sum = 0.;
...
Real NonDGenACVSampling::nonlinear_model_cost(const RealVector& r_and_N) {
  const UShortArray& approx_set = activeModelSetIter->first;
  size_t i, num_approx = approx_set.size();
  Real cost_H = sequenceCost[numApprox], cost_i, N_H = r_and_N[num_approx],
       approx_inner_prod = 0.;
...
void NonDGenACVSampling::linear_model_cost_gradient(const RealVector& N_vec,
                                                    RealVector& grad_c) {
  const UShortArray& approx_set = activeModelSetIter->first;
  size_t i, num_approx = approx_set.size();
  unsigned short inflate_i;
...
void NonDGenACVSampling::nonlinear_model_cost_gradient(
...
void NonDGenACVSampling::accumulate_genacv_sums(
    IntRealMatrixMap& sum_L_shared, IntRealVectorMap& sum_H,
...
void NonDGenACVSampling::accumulate_genacv_sums(IntRealMatrixMap& sum_L_shared,
...
void NonDGenACVSampling::accumulate_genacv_sums(
    IntRealMatrixMap& sum_L_shared, IntRealMatrixMap& sum_L_refined,
  ...
  if (ordered)
    ...
    for (s = sequence_start; s < sequence_end; ++s) {
      ...
      source = approx_set[seq_s];
      target = active_dag[seq_s];
  ...
  // led by the approx Model responses of interest
  IntRespMCIter r_it;
  UShortSet::iterator z1_it;
  size_t approx, inflate_approx;
  for (r_it = allResponses.begin(); r_it != allResponses.end(); ++r_it) {
    const Response& resp = r_it->second;
    const RealVector& fn_vals = resp.function_values();
    const ShortArray& asv = resp.active_set_request_vector();
  ...
  switch (mlmfSubMethod) {
    case SUBMETHOD_ACV_MF: {  // Bomarito Eqs. 16-17
      ...
      for (i = 0; i < dag_size; ++i) {
        src_i = approx_set[i];
        tgt_i = active_dag[i];
        z1_i = /*z2_bi = z_bi =*/N_vec[tgt_i];
        z2_i = /*z_i =*/N_vec[src_i];
        gVec[i] =
            (std::min(z1_i, z_H) / z1_i - std::min(z2_i, z_H) / z2_i) / z_H;
        for (j = 0; j <= i; ++j) {
          src_j = approx_set[j];
          tgt_j = active_dag[j];
          z1_j = /*z2_bj = z_bj =*/N_vec[tgt_j];
          z2_j = /*z_j =*/N_vec[src_j];
          GMat(i, j) =
              (std::min(z1_i, z1_j) / z1_j - std::min(z1_i, z2_j) / z2_j) /
                  z1_i +
              (std::min(z2_i, z2_j) / z2_j - std::min(z2_i, z1_j) / z1_j) /
                  z2_i;
        }
      }
      break;
    ...
    case SUBMETHOD_ACV_IS: {
      ...
      for (i = 0; i < dag_size; ++i) {
        ...
        gVec[i] = (tgt_i == numApprox) ? 1. / z1_i - 1. / z_i
                                       : 0.;  // N_0 -> N_beta_i
        for (j = 0; j <= i; ++j) {
          src_j = approx_set[j];
          tgt_j = active_dag[j];
          ...
          GMat(i, j) = 0.;
          if (tgt_i == tgt_j)
            GMat(i, j) += 1. / z1_i - 1. / z_i - 1. / z_j + z1_i / zi_zj;
    ...
    case SUBMETHOD_ACV_RD: {
      ...
      for (i = 0; i < dag_size; ++i) {
        ...
        for (j = 0; j <= i; ++j) {
          ...
          if (tgt_i == src_j)
            GMat(i, j) -= 1. / z1_i;  // always false for root dag
          if (src_i == tgt_j)
            GMat(i, j) -= 1. / z2_i;  // always false for root dag
...
void NonDGenACVSampling::unroll_z1_z2(const RealVector& N_vec, RealVector& z1,
                                      RealVector& z2) {
  ...
  z1.size(numGroups);
  z1[numApprox] = 0;  // JCP ACV Fig 2 "z"
  z2.size(numGroups);
  z2[numApprox] = N_vec[numApprox];  // JCP ACV Fig 2 "z"

  switch (mlmfSubMethod) {
    case SUBMETHOD_ACV_MF: {  // not used (special unroll logic not required)
      const UShortArray& approx_set = activeModelSetIter->first;
      const UShortArray& active_dag = *activeDAGIter;
      unsigned short i, source, target;
      size_t dag_size = active_dag.size();
      for (i = 0; i < dag_size; ++i) {
        source = approx_set[i];
        target = active_dag[i];
        // z1[i] = N_vec[target];  z2[i] = N_vec[source];    // COMPACTED z1,z2
        z1[source] = N_vec[target];
        z2[source] = N_vec[source];  // FULL z1,z2
      }
      break;
    }
    case SUBMETHOD_ACV_IS: {
