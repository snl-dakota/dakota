...
{
  // z1, z2 denote z_m^1 and z_m^2 from ACV paper: Gorodetsky et al., JCP 2020
  // > {z1,z2}_grads use active (not inflated) indexing as for other grad
  //   arrays, differing from z1,z2 in unroll_z1_z2()

  const UShortArray& approx_set = activeModelSetIter->first;
  const UShortArray& active_dag = *activeDAGIter;
  size_t i, num_a = approx_set.size(), num_v = num_a + 1, index_src, cntr;
  unsigned short source, target;
  SizetArray index_map;  inflate_approx_set(approx_set, index_map);
  z1_grads.shape(num_v, num_v); // init to 0. (entries are sparse)
  ...
  case SUBMETHOD_ACV_IS: {
    ...
    for (i=0; i<num_a; ++i) { // walk DAG from i-th source to root
      ...
      //z1_grads(i, index_src) = 1.; // no: z1[src] advances to first N[tgt]
      while (target != numApprox) {
  ...
  case SUBMETHOD_ACV_RD: {
    ...
    for (i=0; i<num_a; ++i) { // walk DAG from i-th source to root
      source = approx_set[i];  index_src = index_map[source];
      target = active_dag[i];  cntr = 0;
      z2_grads(i, index_src) = 1.; //z1_grads(i, index_src) = 0.;
      while (target != numApprox) {
	source = target;  index_src = index_map[source];
	target = active_dag[index_src];  ++cntr;
			     ...
			     RealSymMatrixArray& dG_dN, RealVectorArray& dg_dN)
{
  ...
  case SUBMETHOD_ACV_MF: {
	...
	dg_dN[deflate_tgt_i](i) -= inv_z1_sq_i;//term 1, Bomarito eq 17 (z1=z_H)
      else if (z1_i <= z_H) // for completeness (root should have smallest N)
	dg_dN[deflate_tgt_i](i) -= 1./(z_H * z_H);
      // derivs of g(i) w.r.t. z2_i = N[src_i]
      if (src_i == numApprox || z2_i > z_H)
	dg_dN[deflate_src_i](i) += inv_z2_sq_i; // term 2, Bomarito eq 17
      else if (z2_i <= z_H) // for completeness (root should have smallest N)
	dg_dN[deflate_tgt_i](i) += 1./(z_H * z_H);

      // G(i,i) =       1/z1_i - 2 min(z1_i,z2_i)/(z1_i z2_i) + 1/z2_i
      // Bomarito eq 16:  term 1            terms 2,3             term 4
      // derivs of G for i==j w.r.t. z1_i = N[tgt_i] and z2_i = N[src_i]
      Real& dG_dN_ti_ii = dG_dN[deflate_tgt_i](i,i);// deriv of Gii w.r.t. z1_i
      Real& dG_dN_si_ii = dG_dN[deflate_src_i](i,i);// deriv of Gii w.r.t. z2_i
      dG_dN_ti_ii -= inv_z1_sq_i; // term 1
      dG_dN_si_ii -= inv_z2_sq_i; // term 4
  ...
  case SUBMETHOD_ACV_IS: { // refer to Eq. 30 in JCP ACV paper
    ...
    for (i=0; i<dag_size; ++i) {
      ...
      if (tgt_i == numApprox) { // g[i] = 1/N_H - 1/N_i (since N_H is N_beta_i)
	dg_dN_A(i)  -= 1./(z_H  * z_H);
	dg_dN[i](i) += inv_z2_sq_i;
      }
...
estimator_variance_gradients(const RealVector& cd_vars, RealMatrix& ev_grads)
{
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "GenACV estimator variance gradients:\n" << ev_grads << std::endl;
}
				  ...
				  RealVector& est_var, RealMatrix& ev_grads)
{
  ...
  */
  ...
  for (q=0; q<numFunctions; ++q) {
    ...
    //copy_data(CG_inv_rm, CG_inv);
    est_var[q] = varH_q / N_H - cg.dot(lhs);
    for (v=0; v<num_v; ++v) {
      // compute dCG_inv_dN = -CG_inv^T dCG_dN[v] CG_inv
      matrixTripleProduct(-1., dCG_dN[v], CG_inv, dCG_inv_dN); //A,W,B
      //if (outputLevel >= DEBUG_OUTPUT) Cout << "dCG_inv/dN:\n" << dCG_inv_dN;
