...
struct Opt_Info {
  char *begin;  // I/O, updated by dlsolver_option for use in next
                // dlsolver_option call
...
struct Dakota_funcs {
  ...
  void (*GetGrads1)(Optimizer1 *, int m0, int m1, int n, int is, int js,
                    double *g);
  // g[(i-m0)*is + j*js] <-- partial(Response i)/partial(var j),
  // m0 <= i < m1, 0 <= j < n
  void (*GetContVars)(Optimizer1 *, int n, double *x);
  void (*SetBestContVars)(Optimizer1 *, int n,
                          double *x);  // set best continuous var values
  void (*SetBestDiscVars)(Optimizer1 *, int n,
                          int *x);  // set best discrete var values
  void (*SetBestRespFns)(Optimizer1 *, int n,
                         double *x);               // set best resp func values
...
#define nonlinear_ineq_constraint_lower_bounds(o) \
  DF->nonlinear_ineq_constraint_lower_bounds(o)
#define nonlinear_ineq_constraint_upper_bounds(o) \
  DF->nonlinear_ineq_constraint_upper_bounds(o)
#define nonlinear_eq_constraint_targets(o) \
  DF->nonlinear_eq_constraint_targets(o)
#define linear_ineq_constraint_lower_bounds(o) \
  DF->linear_ineq_constraint_lower_bounds(o)
#define linear_ineq_constraint_upper_bounds(o) \
  DF->linear_ineq_constraint_upper_bounds(o)
...
extern "C" void *dl_constructor(Optimizer1 *, Dakota_funcs *, dl_core_run_t *,
                                dl_destructor_t *);
