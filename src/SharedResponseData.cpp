...
static const char rcsId[] =
    "@(#) $Id: SharedResponseData.cpp 6886 2010-08-02 19:13:01Z mseldre $";
...
SharedResponseDataRep::SharedResponseDataRep(const ProblemDescDB& problem_db)
  ...
  if (num_field_responses) {
    ...
    if (num_scalar_primary + num_field_primary != num_total_primary) {
      Cerr << "Error: number of scalar (" << num_scalar_primary
           << ") and field (" << num_field_primary << ") " << primary_fn_name()
           << " must sum to total number (" << num_total_primary << ") of "
    ...
    // can't use num_scalar_responses, as constraints are only specified via
    // total
...
void SharedResponseDataRep::serialize(Archive& ar, const unsigned int version) {
  ...
  if (version >= 1) ar & metadataLabels;
#ifdef SERIALIZE_DEBUG
  Cout << "Serializing SharedResponseDataRep:\n"
       << responseType << '\n'
       << responsesId << '\n'
       << functionLabels << numScalarResponses << priFieldLengths << std::endl;
#endif
}
...
void SharedResponseDataRep::build_field_labels(
    const StringArray& labels_per_group) {
  ...
  for (size_t i = 0; i < priFieldLengths.length(); ++i)
    for (size_t j = 0; j < priFieldLengths[i]; ++j)
      build_label(functionLabels[unrolled_index++], priFieldLabels[i], j + 1,
                  "_");
...
void SharedResponseDataRep::resize_field_labels(
    const StringArray& old_full_labels, size_t old_field_elements) {
...
void SharedResponseData::reshape_labels(StringArray& resp_labels,
                                        size_t num_new) {
...
long SharedResponseData::reference_count() const { return srdRep.use_count(); }
