...
RandomFieldModel::RandomFieldModel(ProblemDescDB& problem_db,
    ...
    : RecastModel(problem_db, parallel_lib,
...
std::shared_ptr<Model> RandomFieldModel::get_sub_model(
...
void RandomFieldModel::rf_suite_identify_field_model() {
  ...
  // dprepro to insert covariance type / parameters / correl lengths, bases,
  // percent variance
...
void RandomFieldModel::identify_field_model() {
  ...
  switch (expansionForm) {
    ...
    case RF_PCA_GP: {
      ...
      // num_samples > numFns...
      RealMatrix f_scores(Teuchos::Copy, factor_scores, num_samples,
                          num_samples, 0, 0);
      ...
      sharedData =
          SharedApproxData(approx_type, approx_order, ModelUtils::cv(*subModel),
      ...
      for (int i = 0; i < actualReducedRank; ++i) {
        ...
        gpApproximations[i].add_array(rfBuildVars, false, factor_i,
                                      true);  // shallow,deep
...
void RandomFieldModel::initialize_recast() {
  ...
  size_t submodel_vars =
      ModelUtils::cv(*subModel) + ModelUtils::div(*subModel) +
      ModelUtils::dsv(*subModel) + ModelUtils::drv(*subModel);
  ...
  UShortMultiArrayConstView sm_cv_types =
      ModelUtils::continuous_variable_types(*subModel);
  ...
  for (size_t i = 0; i < submodel_vars; ++i) {
    ...
    size_t j = 0;
    for (; j < num_sm_normal; ++j) vars_map_indices[i][j] = j;
  ...
  RecastModel::init_maps(vars_map_indices, nonlinear_vars_mapping, vars_mapping,
                         set_mapping, primary_resp_map_indices,
...
void RandomFieldModel::initialize_rf_coeffs() {
  ...
  if (expansionForm == RF_KARHUNEN_LOEVE) {
    ...
    for (int i = 0; i < actualReducedRank; ++i) {
      ...
      currentVariables.continuous_variable_label(xi_label, num_sm_normal + i);
    }
    for (int i = num_sm_normal; i < sm_cv_labels.size(); ++i)
      currentVariables.continuous_variable_label(sm_cv_labels[i],
                                                 actualReducedRank + i);
...
void RandomFieldModel::vars_mapping(const Variables& recast_augmented_vars,
  ...
  if (rfmInstance->expansionForm == RF_KARHUNEN_LOEVE) {
    ...
    for (; i < num_sm_normal; ++i) sm_cvars[i] = augmented_cvars[i];
    // skip the N(0,1) coeffs, if they exist
    for (; i < num_sm_cv; ++i)
    ...
    sub_model_vars.discrete_int_variables(
        recast_augmented_vars.discrete_int_variables());
    sub_model_vars.discrete_string_variables(
        recast_augmented_vars.discrete_string_variables());
    sub_model_vars.discrete_real_variables(
        recast_augmented_vars.discrete_real_variables());
  } else
...
void RandomFieldModel::generate_kl_realization() {
  ...
  if (covarianceForm != NOCOVAR) std::system("./run_kl_realize.sh");
