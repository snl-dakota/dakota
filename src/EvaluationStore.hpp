...
class HDF5IOHelper;
#endif
// Hold the default/maximal ActiveSet (and number of metadata responses) for a
// model or interface+model
struct DefaultSet {
  ActiveSet set;
...
class EvaluationStore {
 public:
  ...
  EvaluationsDBState iterator_allocate(const String &iterator_id,
                                       const bool &top_level);
  /// Allocate storage for model evaluations
  EvaluationsDBState model_allocate(
      const String &model_id, const String &model_type,
      const Variables &variables,
      const Pecos::MultivariateDistribution &mv_dist, const Response &response,
      const ActiveSet &set);
  /// Allocate storage for evalulations of interface+model pairs
  EvaluationsDBState interface_allocate(const String &model_id,
                                        const String &interface_id,
                                        const String &interface_type,
                                        const Variables &variables,
  ...
  void store_model_variables(const String &model_id, const String &model_type,
                             const int &eval_id, const ActiveSet &set,
                             const Variables &variables);
  ...
  void store_interface_variables(const String &model_id,
                                 const String &interface_id, const int &eval_id,
                                 const ActiveSet &set,
                                 const Variables &variables);

  /// Store response for an interface+model evaluation
  void store_interface_response(const String &model_id,
                                const String &interface_id, const int &eval_id,
                                const Response &response);
 ...
 private:
  ...
  void declare_iterator_source(const String owner_id, const String source_id,
  ...
  String create_interface_root(const String &model_id,
                               const String &interface_id);
  ...
  // all Dakota types
#define DECLARE_STORE_PARAMETERS_FOR(vtype)                               \
  void store_parameters_for_##vtype(                                      \
  ...
  /// Allocate storage for variable paramters
  void allocate_variable_parameters(const String &root_group,
                                    const Variables &variables,
                                    Pecos::MarginalsCorrDistribution *mvd_rep);
  ...
  /// Store properties information (ASV, DVV, analysis components, distribution
  /// parameters)
  void store_properties(const String &root_group, const ActiveSet &set,
                        const DefaultSet &default_set_s);

  /// Store metadata
  void store_metadata(const String &root_group, const int &resp_idx,
                      const Response &response);
  ...
  /// Default ActiveSets and whether they have gradients and hessians for
  /// interfaces
  ...
  /// Models that have been declared as sources to iterators. Only populated
  /// when TOP_METHOD or ALL_METHODS model evals are stored.
  std::set<String> sourceModels;

  /// Ids of models that have tried to store a different number of functions
