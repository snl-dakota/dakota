...
void C3FnTrainDataRep::ft_derived_functions_create_av(
    struct MultiApproxOpts* opts, const SizetArray& rand_indices,
    Real round_tol) {
  ...
  ft_derived_fns.ft_nonrand =
      function_train_integrate_weighted_subset(ft, num_rand, rand_ptr);
...
void C3FnTrainDataRep::ft_derived_functions_create(struct MultiApproxOpts* opts,
                                                   size_t num_mom,
                                                   Real round_tol) {
  ...
  if (num_mom >= 2 && prev_alloc < 2) {
    ...
    // ft_derived_fns.ft_squared = function_train_round(ft_tmp, round_tol,
    // opts); function_train_free(ft_tmp); ft_derived_fns.second_moment
    //  = function_train_integrate_weighted(ft_derived_fns.ft_squared);

    // Alternate approach for sanity checking:
    // ft_derived_fns.ft_squared = function_train_product(ft, ft); // no
    // rounding Real raw1 = ft_derived_fns.first_moment,
    ...
    ft_derived_fns.second_central_moment = function_train_inner_weighted(
        ft_derived_fns.ft_diff_from_mean, ft_derived_fns.ft_diff_from_mean);
  ...
  if (num_mom >= 3 && prev_alloc < 3) {
    ...
    ft_derived_fns.third_central_moment =
        function_train_inner_weighted(ft_derived_fns.ft_diff_from_mean_squared,
                                      ft_derived_fns.ft_diff_from_mean);
  ...
  if (num_mom >= 4 && prev_alloc < 4) {
    ...
    ft_derived_fns.excess_kurtosis =
        ft_derived_fns.fourth_central_moment / var / var -
        3.;  // excess kurtosis
  }
...
C3FnTrainData::C3FnTrainData()
    : ftdRep(new C3FnTrainDataRep()) {}  // body allocated with null FT pointers

// TO DO: shallow copy would be better for this case, but requires ref counting
C3FnTrainData::C3FnTrainData(const C3FnTrainData& ftd) { ftdRep = ftd.ftdRep; }
...
C3FnTrainData C3FnTrainData::copy() const {
  ...
  ftd.ftdRep->ft =
      (ftdRep->ft == NULL) ? NULL : function_train_copy(ftdRep->ft);
  ftd.ftdRep->ft_gradient = (ftdRep->ft_gradient == NULL)
                                ? NULL
                                : ft1d_array_copy(ftdRep->ft_gradient);
  ftd.ftdRep->ft_hessian =
      (ftdRep->ft_hessian == NULL) ? NULL : ft1d_array_copy(ftdRep->ft_hessian);
