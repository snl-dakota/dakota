...
static const char rcsId[] =
    "@(#) $Id: NLSSOLLeastSq.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
void NLSSOL_F77(
    int& m, int& n, int& nclin, int& ncnln, int& nrowa, int& nrowcj,
    int& nrowfj, int& nrowr, double* a, double* bl, double* bu,
    void (*funcon)(int& mode, int& ncnln, int& n, int& nrowj, int* needc,
                   double* x, double* c, double* cjac, int& nstate),
    void (*funobj)(int& mode, int& m, int& n, int& nrowfj, double* x, double* f,
                   double* gradf, int& nstate),
    int& inform, int& iter, int* istate, double* c, double* cjac, double* y,
    double* f, double* fjac, double* clambda, double& objf, double* r,
...
NLSSOLLeastSq::NLSSOLLeastSq(ProblemDescDB& problem_db,
...
void NLSSOLLeastSq::least_sq_eval(int& mode, int& m, int& n, int& nrowfj,
                                  double* x, double* f, double* gradf,
                                  int& nstate) {
  ...
  if (asv_request && nlssolInstance->numNonlinearConstraints == 0) {
    ...
    ModelUtils::continuous_variables(*nlssolInstance->iteratedModel,
                                     local_des_vars);
  ...
  if (asv_request & 1) {
    ...
    for (size_t i = 0; i < m; i++) f[i] = local_fn_vals[i];
  }
  if (asv_request & 2) {
    ...
    for (size_t j = 0; j < n; j++)
      for (size_t i = 0; i < m; i++) gradf[cntr++] = local_fn_grads(j, i);
  }
}
...
void NLSSOLLeastSq::core_run() {
  ...
  NLSSOL_F77(
      num_least_sq_terms, num_cv, num_linear_constraints,
      num_nonlinear_constraints, linConstraintArraySize, nlnConstraintArraySize,
      num_least_sq_terms, num_cv, linConstraintMatrixF77,
      augmented_l_bnds.values(), augmented_u_bnds.values(), constraint_eval,
      least_sq_eval, informResult, numberIterations, &constraintState[0],
      local_c_vals.values(), constraintJacMatrixF77, local_lsq_offsets.values(),
      local_lsq_vals.values(), local_lsq_grads, &cLambda[0], local_f_val,
      upperFactorHessianF77, local_des_vars.values(), &intWorkSpace[0],
      intWorkSpaceSize, &realWorkSpace[0], realWorkSpaceSize);
