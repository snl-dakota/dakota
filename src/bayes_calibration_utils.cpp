...
void batch_means_interval(RealMatrix& mcmc_matrix, RealMatrix& interval_matrix,
  ...
  int num_batches =
      num_samples / batch_size;  // What about when this doesn't divide evenly?
                                 // Do we put the extras in the last batch?
  ...
  if (moment == 1) func_totalchain = mean_totalchain;
  if (moment == 2) {
  ...
  for (int i = 0; i < num_batches; i++) {
    RealMatrix batch_j_matrix(Teuchos::View, mcmc_matrix_transpose, batch_size,
                              num_qoi, i * batch_size, 0);
...
void batch_means_percentile(RealMatrix& mcmc_matrix,
  ...
  for (int i = 0; i < num_qoi; i++) {
    RealVector qoi_vec =
        Teuchos::getCol(Teuchos::Copy, mcmc_matrix_transpose, i);
  ...
  for (int i = 0; i < num_batches; i++) {
    ...
    RealVector func_subchain(num_qoi);
    for (int j = 0; j < num_qoi; j++) {
      RealVector batch_qoi_vec =
          Teuchos::getCol(Teuchos::Copy, batch_j_matrix, j);
