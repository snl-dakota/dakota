...
Rank1Lattice::Rank1Lattice(
    ...
    : LowDiscrepancySequence(mMax, generatingVector.length(), seedValue,
                             outputLevel),
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    Cout << "The maximum dimension of this rank-1 lattice rule is " << dMax
         << "." << std::endl;
    ...
    for (size_t j = 0; j < length; ++j) Cout << generatingVector[j] << " ";
    Cout << std::endl;
  }
...
Rank1Lattice::Rank1Lattice(const UInt32Vector& generatingVector, int mMax)
    : Rank1Lattice(generatingVector, mMax, true, generate_system_seed(),
                   RANK_1_LATTICE_RADICAL_INVERSE_ORDERING, NORMAL_OUTPUT) {}

/// A constructor with only the random seed as argument
Rank1Lattice::Rank1Lattice(int seedValue)
    : Rank1Lattice(
          UInt32Vector(Teuchos::View, &cools_kuo_nuyens_d250_m20[0], 250), 20,
          true, seedValue, RANK_1_LATTICE_RADICAL_INVERSE_ORDERING,
          NORMAL_OUTPUT) {}
...
/// Case I: the generating vector is provided in an external file
const std::tuple<UInt32Vector, int>
Rank1Lattice::get_generating_vector_from_file(ProblemDescDB& problem_db) {
  ...
  try {
    ...
    return std::make_tuple(generatingVector,
...
const std::tuple<UInt32Vector, int> Rank1Lattice::get_inline_generating_vector(
  ...
  return std::make_tuple(generatingVector, problem_db.get_int("method.m_max"));
}
...
const std::tuple<UInt32Vector, int> Rank1Lattice::get_default_generating_vector(
  ...
  if (problem_db.get_bool("method.kuo")) {
    ...
    return std::make_tuple(UInt32Vector(Teuchos::View, &kuo_d3600_m20[0], 3600),
                           20);
  } else {
    if (outputLevel >= DEBUG_OUTPUT) {
...
void Rank1Lattice::random_shift(int seed) {
...
void Rank1Lattice::unsafe_get_points(const size_t nMin, const size_t nMax,
                                     RealMatrix& points) {
...
inline UInt32 Rank1Lattice::reorder_natural(UInt32 k) { return k; }

/// Position of the `k`th lattice point in
/// RANK_1_LATTICE_RADICAL_INVERSE_ORDERING
inline UInt32 Rank1Lattice::reorder_radical_inverse(UInt32 k) {
  return bitreverse(k);
}
