...
BitArray discrete_int_sets(const Model &model, short active_view) {
  ...
  bool relax = (active_view == RELAXED_ALL || (active_view >= RELAXED_DESIGN &&
  ...
  if (relax) {
    ...
    if (num_ddiv = active_totals[TOTAL_DDIV]) {
      ...
      for (i = 0; i < set_ddiv; ++i, ++di_cntr) dis.set(di_cntr);
    }
    if (num_dauiv = active_totals[TOTAL_DAUIV]) {
      ...
      for (i = 0; i < set_dauiv; ++i, ++di_cntr) dis.set(di_cntr);
    }
    if (num_deuiv = active_totals[TOTAL_DEUIV]) {
      size_t set_deuiv = svd.vc_lookup(DISCRETE_UNCERTAIN_SET_INT);
      di_cntr +=
          num_deuiv - set_deuiv;  // vc_lookup(DISCRETE_INTERVAL_UNCERTAIN)
      for (i = 0; i < set_deuiv; ++i, ++di_cntr) dis.set(di_cntr);
    }
    if (num_dsiv = active_totals[TOTAL_DSIV]) {
      ...
      for (i = 0; i < set_dsiv; ++i, ++di_cntr) dis.set(di_cntr);
    }
  }
  ...
  switch (active_view) {
    ...
    case MIXED_STATE: {
      size_t num_cv, num_div, num_dsv, num_drv,
          start_rv = 0, num_rv = svd.vc_lookup(DISCRETE_STATE_SET_INT);
    ...
    default: {  // RELAXED_*
      ...
      const SizetArray &all_totals = svd.components_totals();
      const SizetArray &active_totals = svd.active_components_totals();
      size_t i, num_cv, num_div, num_dsv, num_drv, di_cntr = 0, ardi_cntr = 0,
                                                   rv_cntr = 0;
      // discrete design
      svd.design_counts(num_cv, num_div, num_dsv, num_drv);
      if (active_totals[TOTAL_DDIV]) {
...
StringSetArray discrete_set_string_values(const Model &model) {
  return discrete_set_string_values(model,
                                    model.current_variables().view().first);
}
  ...
  switch (active_view) {
    ...
    case RELAXED_UNCERTAIN: {
      ...
      mvd_rep->pull_parameters<StringRealMap>(
          ...
          deuss_vals_probs);
      size_t i, num_dauss = h_pt_prs.size(),
                num_deuss = deuss_vals_probs.size();
      adss_vals.resize(num_dauss + num_deuss);
      for (i = 0; i < num_dauss; ++i)
        map_keys_to_set(h_pt_prs[i], adss_vals[i]);
      for (i = 0; i < num_deuss; ++i)
        map_keys_to_set(deuss_vals_probs[i], adss_vals[i + num_dauss]);
      break;
    }
    case MIXED_STATE:
    case RELAXED_STATE: {
    ...
    case RELAXED_ALL: {
      ...
      mvd_rep->pull_parameters<StringRealMap>(
          ...
          deuss_vals_probs);
      size_t i,
          ds_cntr = 0, num_ddss = svd.vc_lookup(DISCRETE_DESIGN_SET_STRING),
          num_dauss = h_pt_prs.size(), num_deuss = deuss_vals_probs.size(),
          num_dsss = svd.vc_lookup(DISCRETE_STATE_SET_STRING);
      adss_vals.resize(num_ddss + num_dauss + num_deuss + num_dsss);
      size_t num_cv, num_div, num_dsv, num_drv;
      svd.design_counts(num_cv, num_div, num_dsv, num_drv);
...
RealSetArray discrete_set_real_values(const Model &model) {
  return discrete_set_real_values(model,
                                  model.current_variables().view().first);
}
...
RealSetArray discrete_set_real_values(const Model &model, short active_view) {
  ...
  switch (active_view) {
    ...
    default: {  // RELAXED_*
      ...
      const SizetArray &all_totals = svd.components_totals();
      const SizetArray &active_totals = svd.active_components_totals();
      size_t i, num_cv, num_div, num_dsv, num_drv, dr_cntr = 0, ardr_cntr = 0,
                                                   rv_cntr = 0;
      // discrete design
      svd.design_counts(num_cv, num_div, num_dsv, num_drv);
      ...
      if (active_totals[TOTAL_DEURV]) {
        RealRealMapArray deusr_vals_probs;
        mvd_rep->pull_parameters<RealRealMap>(
...
size_t tv(const Model &model) { return model.current_variables().tv(); }

size_t cv(const Model &model) { return model.current_variables().cv(); }

size_t div(const Model &model) { return model.current_variables().div(); }

size_t dsv(const Model &model) { return model.current_variables().dsv(); }

size_t drv(const Model &model) { return model.current_variables().drv(); }

size_t icv(const Model &model) { return model.current_variables().icv(); }

size_t idiv(const Model &model) { return model.current_variables().idiv(); }

size_t idsv(const Model &model) { return model.current_variables().idsv(); }

size_t idrv(const Model &model) { return model.current_variables().idrv(); }

size_t acv(const Model &model) { return model.current_variables().acv(); }

size_t adiv(const Model &model) { return model.current_variables().adiv(); }

size_t adsv(const Model &model) { return model.current_variables().adsv(); }

size_t adrv(const Model &model) { return model.current_variables().adrv(); }
...
void continuous_variable_types(Model &model,
...
void discrete_int_variable_types(Model &model,
                                 UShortMultiArrayConstView div_types) {
  model.current_variables().discrete_int_variable_types(div_types);
}

void discrete_int_variable_type(Model &model, unsigned short div_type,
                                size_t i) {
...
void discrete_string_variable_types(Model &model,
                                    UShortMultiArrayConstView dsv_types) {
  model.current_variables().discrete_string_variable_types(dsv_types);
}

void discrete_string_variable_type(Model &model, unsigned short dsv_type,
                                   size_t i) {
...
void discrete_real_variable_types(Model &model,
                                  UShortMultiArrayConstView drv_types) {
  model.current_variables().discrete_real_variable_types(drv_types);
}

void discrete_real_variable_type(Model &model, unsigned short drv_type,
                                 size_t i) {
...
StringMultiArrayConstView inactive_discrete_string_variables(
    const Model &model) {
  return model.current_variables().inactive_discrete_string_variables();
}

void inactive_discrete_string_variables(Model &model,
                                        StringMultiArrayConstView i_d_vars) {
...
void inactive_discrete_real_variables(Model &model,
                                      const RealVector &i_d_vars) {
  model.current_variables().inactive_discrete_real_variables(i_d_vars);
}

UShortMultiArrayConstView inactive_continuous_variable_types(
    const Model &model) {
...
void all_discrete_string_variables(Model &model,
                                   StringMultiArrayConstView a_d_vars) {
  model.current_variables().all_discrete_string_variables(a_d_vars);
}

void all_discrete_string_variable(Model &model, const String &a_d_var,
                                  size_t i) {
...
UShortMultiArrayConstView all_discrete_string_variable_types(
    const Model &model) {
...
void continuous_variable_labels(Model &model,
                                StringMultiArrayConstView c_v_labels) {
...
void discrete_int_variable_labels(Model &model,
                                  StringMultiArrayConstView d_v_labels) {
...
void discrete_string_variable_labels(Model &model,
                                     StringMultiArrayConstView d_v_labels) {
...
void inactive_discrete_string_variable_labels(
    Model &model, StringMultiArrayConstView i_d_v_labels) {
  model.current_variables().inactive_discrete_string_variable_labels(
      i_d_v_labels);
}

StringMultiArrayConstView inactive_discrete_real_variable_labels(
    const Model &model) {
...
void inactive_discrete_real_variable_labels(
    Model &model, StringMultiArrayConstView i_d_v_labels) {
  model.current_variables().inactive_discrete_real_variable_labels(
      i_d_v_labels);
}
...
void all_continuous_variable_labels(Model &model,
                                    StringMultiArrayConstView a_c_v_labels) {
  model.current_variables().all_continuous_variable_labels(a_c_v_labels);
}

void all_continuous_variable_label(Model &model, const String &a_c_v_label,
                                   size_t i) {
...
void inactive_continuous_lower_bounds(Model &model,
                                      const RealVector &i_c_l_bnds) {
...
void inactive_continuous_upper_bounds(Model &model,
                                      const RealVector &i_c_u_bnds) {
...
void inactive_discrete_int_lower_bounds(Model &model,
                                        const IntVector &i_d_l_bnds) {
  model.user_defined_constraints().inactive_discrete_int_lower_bounds(
      i_d_l_bnds);
  auto &mvd = model.multivariate_distribution();
  if (mvd.global_bounds())
    mvd.lower_bounds(
        i_d_l_bnds, model.current_variables().shared_data().idiv_to_all_mask());
}
...
void inactive_discrete_int_upper_bounds(Model &model,
                                        const IntVector &i_d_u_bnds) {
  model.user_defined_constraints().inactive_discrete_int_upper_bounds(
      i_d_u_bnds);
  auto &mvd = model.multivariate_distribution();
  if (mvd.global_bounds())
    mvd.upper_bounds(
        i_d_u_bnds, model.current_variables().shared_data().idiv_to_all_mask());
}
...
void inactive_discrete_real_lower_bounds(Model &model,
                                         const RealVector &i_d_l_bnds) {
  model.user_defined_constraints().inactive_discrete_real_lower_bounds(
      i_d_l_bnds);
  auto &mvd = model.multivariate_distribution();
  if (mvd.global_bounds())
    mvd.lower_bounds(
        i_d_l_bnds, model.current_variables().shared_data().idrv_to_all_mask());
}
...
void inactive_discrete_real_upper_bounds(Model &model,
                                         const RealVector &i_d_u_bnds) {
  model.user_defined_constraints().inactive_discrete_real_upper_bounds(
      i_d_u_bnds);
  auto &mvd = model.multivariate_distribution();
  if (mvd.global_bounds())
    mvd.upper_bounds(
        i_d_u_bnds, model.current_variables().shared_data().idrv_to_all_mask());
}
...
void all_discrete_int_lower_bounds(Model &model, const IntVector &a_d_l_bnds) {
  ...
  if (mvd.global_bounds())
    mvd.lower_bounds(
        a_d_l_bnds, model.current_variables().shared_data().adiv_to_all_mask());
}
...
void all_discrete_int_upper_bounds(Model &model, const IntVector &a_d_u_bnds) {
  ...
  if (mvd.global_bounds())
    mvd.upper_bounds(
        a_d_u_bnds, model.current_variables().shared_data().adiv_to_all_mask());
}
...
void all_discrete_real_lower_bounds(Model &model,
                                    const RealVector &a_d_l_bnds) {
  model.user_defined_constraints().all_discrete_real_lower_bounds(a_d_l_bnds);
  auto &mvd = model.multivariate_distribution();
  if (mvd.global_bounds())
    mvd.lower_bounds(
        a_d_l_bnds, model.current_variables().shared_data().adrv_to_all_mask());
}
...
void all_discrete_real_upper_bounds(Model &model,
                                    const RealVector &a_d_u_bnds) {
  model.user_defined_constraints().all_discrete_real_upper_bounds(a_d_u_bnds);
  auto &mvd = model.multivariate_distribution();
  if (mvd.global_bounds())
    mvd.upper_bounds(
        a_d_u_bnds, model.current_variables().shared_data().adrv_to_all_mask());
}
...
void linear_ineq_constraint_coeffs(Model &model,
                                   const RealMatrix &lin_ineq_coeffs) {
  model.user_defined_constraints().linear_ineq_constraint_coeffs(
      lin_ineq_coeffs);
}
...
void linear_ineq_constraint_lower_bounds(Model &model,
                                         const RealVector &lin_ineq_l_bnds) {
  model.user_defined_constraints().linear_ineq_constraint_lower_bounds(
      lin_ineq_l_bnds);
}
...
void linear_ineq_constraint_upper_bounds(Model &model,
                                         const RealVector &lin_ineq_u_bnds) {
  model.user_defined_constraints().linear_ineq_constraint_upper_bounds(
      lin_ineq_u_bnds);
}
...
void linear_eq_constraint_coeffs(Model &model,
...
void linear_eq_constraint_targets(Model &model,
...
const RealVector &nonlinear_ineq_constraint_lower_bounds(const Model &model) {
  return model.user_defined_constraints()
      .nonlinear_ineq_constraint_lower_bounds();
}

void nonlinear_ineq_constraint_lower_bounds(Model &model,
                                            const RealVector &nln_ineq_l_bnds) {
  model.user_defined_constraints().nonlinear_ineq_constraint_lower_bounds(
      nln_ineq_l_bnds);
}
...
void nonlinear_ineq_constraint_upper_bounds(Model &model,
                                            const RealVector &nln_ineq_u_bnds) {
  model.user_defined_constraints().nonlinear_ineq_constraint_upper_bounds(
      nln_ineq_u_bnds);
}
...
void nonlinear_eq_constraint_targets(Model &model,
                                     const RealVector &nln_eq_targets) {
  model.user_defined_constraints().nonlinear_eq_constraint_targets(
      nln_eq_targets);
}
