...
NonDLocalEvidence::NonDLocalEvidence(ProblemDescDB& problem_db,
...
void NonDLocalEvidence::initialize() { calculate_cells_and_bpas(); }

void NonDLocalEvidence::set_cell_bounds() {
  size_t j;
  for (j = 0; j < numContIntervalVars; j++) {
    ModelUtils::continuous_lower_bound(*minMaxModel,
                                       cellContLowerBounds[cellCntr][j], j);
    ModelUtils::continuous_upper_bound(*minMaxModel,
                                       cellContUpperBounds[cellCntr][j], j);
  }

  for (j = 0; j < numDiscIntervalVars; j++) {
    ModelUtils::discrete_int_lower_bound(
        *minMaxModel, cellIntRangeLowerBounds[cellCntr][j], j);
    ModelUtils::discrete_int_upper_bound(
        *minMaxModel, cellIntRangeUpperBounds[cellCntr][j], j);
  }

  for (j = 0; j < numDiscSetIntUncVars; j++) {
    ModelUtils::discrete_int_variable(
        *minMaxModel, cellIntSetBounds[cellCntr][j], j + numDiscIntervalVars);
  }

  for (j = 0; j < numDiscSetRealUncVars; j++) {
    ModelUtils::discrete_real_variable(*minMaxModel,
