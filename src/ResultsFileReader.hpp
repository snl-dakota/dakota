...
class FunctionBatchEvalFailure : public FunctionEvalFailure {
 public:
  FunctionBatchEvalFailure(const std::string& msg,
                           const ParamResponsePair& in_prp,
                           Response& in_response)
      : FunctionEvalFailure(msg),
        prp(in_prp),
        response(in_response) { /* empty ctor */ }
