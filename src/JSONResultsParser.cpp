...
void from_json(const json &j, JSONRealVector &e) {
  if (!j.is_array()) throw JSONStoreError("expected an array object");
  auto n = j.size();
  auto &v = e.value;
  v.size(n);
...
void from_json(const json &j, JSONRealSymMatrix &e) {
  if (!j.is_array()) throw JSONStoreError(std::string("expected array object"));
  auto &h = e.value;
  auto n = j.size();
  h.shape(n);
  for (const auto &row : j) {
    if (!row.is_array())
      throw JSONStoreError(
          std::string("expected element to be an array object"));
    if (row.size() != n)
...
JSONResultsParser::JSONResultsParser(const json &j)
    : jref(j), failedFlag(false) {
  if (!jref.is_object())
...
bool JSONResultsParser::failed() const { return failedFlag; }

double JSONResultsParser::metadata(const std::string &label) const {
  if (!hasMetadata)
    throw JSONResultsParserError(
        "JSON results object does not contain 'metadata', or it's not an "
        "object");
  if (!jref["metadata"].contains(label))
    throw JSONResultsParserError(
        std::string(
            "'metadata' object in JSON results object does not contain '") +
        label + "'");
  double result;
  try {
    ...
    throw JSONResultsParserError(
        std::string("In JSON results object, metadata value for '") + label +
...
double JSONResultsParser::function(const std::string &label) const {
  if (!hasFunctions)
    throw JSONResultsParserError(
        "JSON results object does not contain 'functions', or it's not an "
        "object");
  if (!jref["functions"].contains(label))
    throw JSONResultsParserError(
        std::string(
            "'functions' object in JSON results object does not contain '") +
        label + "'");
  double result;
  try {
    ...
    throw JSONResultsParserError(
        std::string("In JSON results object, function value for '") + label +
        "' could not be converted to a number");
  } catch (const std::invalid_argument &e) {
    throw JSONResultsParserError(
        std::string("In JSON results object, function value for '") + label +
...
RealVector JSONResultsParser::gradient(const std::string &label) const {
  if (!hasGradients)
    throw JSONResultsParserError(
        "JSON results object does not contain 'gradients', or it's not an "
        "object");
  if (!jref["gradients"].contains(label))
    throw JSONResultsParserError(
        std::string(
            "'gradients' object in JSON results object does not contain '") +
        label + "'");
  RealVector grad;
  try {
    ...
    throw JSONResultsParserError(
        std::string("In JSON results object, gradient for '") + label +
        "' contains " + "an element that could not be converted to a number");
  } catch (const std::invalid_argument &e) {
    throw JSONResultsParserError(
        std::string("In JSON results object, gradient for '") + label +
        "' contains " + "an element that could not be converted to a number");
...
RealSymMatrix JSONResultsParser::hessian(const std::string &label) const {
  if (!hasHessians)
    throw JSONResultsParserError(
        "JSON results object does not contain 'hessians', or it's not an "
        "object");
  if (!jref["hessians"].contains(label))
    throw JSONResultsParserError(
        std::string(
            "'hessians' object in JSON results object does not contain '") +
        label + "'");
  ...
  try {
    ...
    throw JSONResultsParserError(
        std::string("In JSON results object, Hessian for '") + label +
        "' contains an element that could not be converted to a number");
  } catch (const std::invalid_argument &e) {
    throw JSONResultsParserError(
        std::string("In JSON results object, Hessian for '") + label +
