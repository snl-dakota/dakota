...
static const char rcsId[] =
    "@(#) $Id: dakota_global_defs.cpp 6716 2010-04-03 18:35:08Z wjbohnh $";
...
std::ostream *dakota_cout =
    &std::cout;  ///< DAKOTA stdout initially points to
                 ///< std::cout, but may be redirected to a tagged ofstream if
                 ///< there are concurrent iterators.
std::ostream *dakota_cerr =
    &std::cerr;  ///< DAKOTA stderr initially points to
                 ///< std::cerr, but may be redirected to a tagged ofstream if
                 ///< there are concurrent iterators.
...
ProblemDescDB *Dak_pddb =
    nullptr;  ///< set by ProblemDescDB, for use in parsing

ParallelLibrary *Dak_parallel_lib =
    nullptr;  ///< set by ParallelLibrary; used by the abort handler to shut
              ///< down MPI
...
void abort_handler(int code) {
  ...
  if (Dak_pddb) {
    ...
    if (Dak_parallel_lib) Dak_parallel_lib->abort_helper(code);
  } else {
...
void abort_throw_or_exit(int dakota_code) {
  ...
  if (abort_mode == ABORT_THROWS) {
    ...
    std::exit(
        os_code);  // or std::exit(EXIT_FAILURE) from /usr/include/stdlib.h
}
