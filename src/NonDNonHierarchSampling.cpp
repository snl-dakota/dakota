...
static const char rcsId[] =
    "@(#) $Id: NonDNonHierarchSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDNonHierarchSampling::NonDNonHierarchSampling(ProblemDescDB& problem_db,
                                                 ParallelLibrary& parallel_lib,
                                                 std::shared_ptr<Model> model)
    : NonDEnsembleSampling(problem_db, parallel_lib, model),
      activeBudget((Real)maxFunctionEvals),
      truthFixedByPilot(
          problem_db.get_bool("method.nond.truth_fixed_by_pilot")),
      ...
      hardenNumericSoln(true)  // Cholesky option not currently exposed in spec
{
  ...
  costSource = configure_cost(num_forms_resolutions, sequenceType, sequenceCost,
                              costMetadataIndices);
}
...
void NonDNonHierarchSampling::assign_active_key() {
  ...
  size_t m, l, num_lev, cntr = 0;
  ModelLIter m_iter;
  for (m = 0, m_iter = sub_models.begin(); m_iter != sub_models.end();
       ++m_iter, ++m) {
    num_lev = (*m_iter)->solution_levels();  // lower bound of 1
    for (l = 0; l < num_lev; ++l, ++cntr) form_res_keys[cntr].form_key(0, m, l);
  }
...
void NonDNonHierarchSampling::hf_indices(size_t& hf_form_index,
                                         size_t& hf_lev_index) const {
  // case Pecos::FORM_RESOLUTION_ENUMERATION
...
bool NonDNonHierarchSampling::approx_increment(
    String prepend, const SizetArray& approx_sequence, size_t start,
    size_t end) {
...
bool NonDNonHierarchSampling::approx_increment(
    String prepend, const SizetArray& approx_sequence, size_t start, size_t end,
    const UShortArray& approx_set) {
  if (numSamples && start < end) {
    Cout << "\nApprox sample increment = " << numSamples << " for approximation"
         << " sequence [" << start + 1 << ", " << end << ']' << std::endl;
...
bool NonDNonHierarchSampling::approx_increment(String prepend,
                                               unsigned short root,
...
void NonDNonHierarchSampling::group_increments(SizetArray& delta_N_G,
                                               String prepend,
                                               bool reverse_order) {
  if (mlmfIter == 0)
...
size_t NonDNonHierarchSampling::group_approx_increment(
    const RealVector& soln_vars, const UShortArray& approx_set,
  ...
  size_t num_samp, root = model_group.back(),  // root must be last model
      deflate_root = (approx_set.size() == numApprox)
                         ? root
...
void NonDNonHierarchSampling::ensemble_sample_increment(const String& prepend,
                                                        size_t step,
                                                        bool new_samples) {
...
void NonDNonHierarchSampling::ensemble_sample_batch(const String& prepend,
                                                    size_t step,
                                                    bool new_samples) {
...
void NonDNonHierarchSampling::export_sample_sets(const String& prepend,
                                                 size_t step) {
...
void NonDNonHierarchSampling::export_all_samples(const String& root_prepend,
                                                 const Model& model,
  ...
  if (iface_id.empty())
    tabular_filename += "NO_ID_i";
  else
    tabular_filename += iface_id + "_i";
  tabular_filename += std::to_string(iter) + "_s" + std::to_string(step) + '_' +
                      std::to_string(num_samp) + ".dat";

  NonDEnsembleSampling::export_all_samples(model, tabular_filename);
}

void NonDNonHierarchSampling::mfmc_analytic_solution(
    const UShortArray& approx_set, const RealMatrix& rho2_LH,
    const RealVector& cost, RealVector& avg_eval_ratios, bool lower_bounded_r,
    bool monotonic_r) {
  int a;
  size_t qoi, num_approx = approx_set.size(), num_am1 = num_approx - 1;
  if (avg_eval_ratios.length() != num_approx)
    avg_eval_ratios.size(num_approx);
  else
    avg_eval_ratios = 0.;
  ...
  for (a = 0; a < num_approx; ++a) {
    ...
    const Real* rho2_LH_m = rho2_LH[approx];
    cost_L = cost[approx];                      // full
  ...
  if (lower_bounded_r)
    for (a = num_am1; a >= 0; --a) {
      ...
      if (avg_eval_ratio < nudge_p1) {
        avg_eval_ratio = nudge_p1;
        nudge_p1 += RATIO_NUDGE;
      }
    }
...
void NonDNonHierarchSampling::mfmc_reordered_analytic_solution(
    const UShortArray& approx_set, const RealMatrix& rho2_LH,
    const RealVector& cost, SizetArray& corr_approx_sequence,
    RealVector& avg_eval_ratios, bool lower_bounded_r, bool monotonic_r) {
  int a;
  size_t qoi, num_approx = approx_set.size(), num_am1 = num_approx - 1;
  if (avg_eval_ratios.length() != num_approx)
  ...
  Real cost_L,
      cost_H = cost[numApprox], rho2, prev_rho2, rho2_diff,
  ...
  for (a = 0; a < num_approx; ++a) {
    ...
    cost_L = cost[inflate_approx];  // full
...
void NonDNonHierarchSampling::cvmc_ensemble_solutions(
    const RealMatrix& rho2_LH, const RealVector& cost,
    RealVector& avg_eval_ratios, bool lower_bounded_r) {
  if (avg_eval_ratios.empty())
    avg_eval_ratios.size(numApprox);
  else
    avg_eval_ratios = 0.;
...
void NonDNonHierarchSampling::apply_controls(
    const IntRealVectorMap& sum_H_baseline, const SizetArray& N_baseline,
    const IntRealMatrixMap& sum_L_shared, const Sizet2DArray& N_L_shared,
    const IntRealMatrixMap& sum_L_refined, const Sizet2DArray& N_L_refined,
  ...
  for (int mom = 1; mom <= 4; ++mom) {
    ...
    for (qoi = 0; qoi < numFunctions; ++qoi) {
      ...
      H_raw_mq =
          sum_H_base_m[qoi] / N_baseline[qoi];  // 1st term to be augmented
...
void NonDNonHierarchSampling::update_model_group_costs() {
  ...
  for (g = 0; g < num_groups; ++g) {
    ...
    for (m = 0; m < num_models; ++m) group_cost += sequenceCost[group_g[m]];
  }
}

void NonDNonHierarchSampling::overlay_group_sums(
    const IntRealMatrixArrayMap& sum_G, const Sizet2DArray& N_G_actual,
    IntRealMatrixMap& sum_L_shared, Sizet2DArray& N_L_actual_shared,
    IntRealMatrixMap& sum_L_refined, Sizet2DArray& N_L_actual_refined) {
...
void NonDNonHierarchSampling::scale_to_target(Real avg_N_H,
                                              const RealVector& cost,
                                              RealVector& avg_eval_ratios,
                                              Real& avg_hf_target, Real budget,
                                              Real offline_N_lwr) {
  ...
  if (avg_N_H >
      avg_hf_target) {  // replace N* with N_pilot, rescale r* to budget
    avg_hf_target = avg_N_H;
    scale_to_budget_with_pilot(avg_eval_ratios, cost, avg_N_H, budget);
  }
}

void NonDNonHierarchSampling::scale_to_budget_with_pilot(
    RealVector& avg_eval_ratios, const RealVector& cost, Real avg_N_H,
    Real budget) {
  ...
  Real cost_r_i, approx_inner_prod = 0.;
  size_t approx;
  for (approx = 0; approx < numApprox; ++approx)
  ...
  for (int i = numApprox - 1; i >= 0;
       --i) {  // repair r_i < 1 first if possible
    Real& r_i = avg_eval_ratios[i];
    r_i *= factor;
    if (r_i <=
        1.) {  // fix at 1+NUDGE --> scale remaining r_i to reduced budget
      ...
      budget -= avg_N_H * cost_r_i / cost_H;
      approx_inner_prod -= cost_r_i;
      factor = (budget / avg_N_H - 1.) / approx_inner_prod * cost_H;
    }
  }
  if (outputLevel >= DEBUG_OUTPUT) {
    ...
    Cout << "Rescale to budget: average evaluation ratios\n"
         << avg_eval_ratios << "Equiv HF = " << avg_N_H * inner_prod / cost_H
         << std::endl;
  }
}

void NonDNonHierarchSampling::ensemble_numerical_solution(
    MFSolutionData& soln) {
  ...
  RealVector x0(num_cdv, false), x_lb(num_cdv, false), x_ub(num_cdv, false),
      ...
      nln_ineq_lb(num_nln_con, false), nln_ineq_ub(num_nln_con, false),
      nln_eq_tgt;
  RealMatrix lin_ineq_coeffs(num_lin_con, num_cdv), lin_eq_coeffs;
  numerical_solution_bounds_constraints(
      soln, x0, x_lb, x_ub, lin_ineq_lb, lin_ineq_ub, lin_eq_tgt, nln_ineq_lb,
      nln_ineq_ub, nln_eq_tgt, lin_ineq_coeffs, lin_eq_coeffs);
...
void NonDNonHierarchSampling::process_model_allocations(MFSolutionData& soln,
                                                        size_t& num_samples) {
  ...
  if (!iterating || graph_enumerate) {  // estvar needed for this soln
    ...
    // soln.estimator_variances() precedes (and ignores subsequent) sample
    // increment relaxation, don't use relaxed increments here.
    // RealVector proj_mc_estvar;
    // project_mc_estimator_variance(varH, N_H_actual,
    // soln_incr,proj_mc_estvar);
    // soln.estimator_variance_ratios(soln.estimator_variances(),proj_mc_estvar);
...
void NonDNonHierarchSampling::numerical_solution_counts(size_t& num_cdv,
                                                        size_t& num_lin_con,
                                                        size_t& num_nln_con) {
  // Notes:
  ...
  switch (optSubProblemForm) {
    ...
    case N_MODEL_LINEAR_CONSTRAINT:
      num_cdv = num_lin_con = numGroups;
      num_nln_con = 0;
      break;
    case N_MODEL_LINEAR_OBJECTIVE:
      num_cdv = numGroups;
      num_nln_con = 1;
      num_lin_con = numApprox;
      break;
  }
}

void NonDNonHierarchSampling::numerical_solution_bounds_constraints(
  ...
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT: {
      ...
      lin_ineq_ub[0] =
          (avg_N_H > 1.) ?  // protect N_H==0 for offline pilot
              budget / avg_N_H - 1.
                         :  // normal case
    ...
    case R_AND_N_NONLINEAR_CONSTRAINT: {
      // Could allow optimal profile to emerge from pilot by allowing N* less
      // than the incurred cost (e.g., setting N_lb to 1), but instead we bound
      // with the incurred cost by setting x_lb = latest N_H and retaining r_lb
      // = 1.
      x_lb = 1.;  // r_i
...
void NonDNonHierarchSampling::augment_linear_ineq_constraints(
  ...
  switch (optSubProblemForm) {
    ...
    case N_MODEL_LINEAR_OBJECTIVE: {  // no other lin ineq
      ...
      for (size_t approx = 0; approx < numApprox; ++approx) {
        ...
        lin_ineq_coeffs(approx + offset, numApprox) =
            1. + RATIO_NUDGE;  // N_i > N
      }
      break;
    }
...
Real NonDNonHierarchSampling::augmented_linear_ineq_violations(
    const RealVector& cd_vars, const RealMatrix& lin_ineq_coeffs,
  ...
  size_t r, c,
      num_v = cd_vars.length(), num_lin_con = lin_ineq_coeffs.numRows(),
  ...
  for (r = lin_ineq_offset; r < num_lin_con; ++r) {
    ...
    u_bnd = lin_ineq_ub[r];
    if (inner_prod < l_bnd) {
      viol = (std::abs(l_bnd) > Pecos::SMALL_NUMBER) ? (1. - inner_prod / l_bnd)
                                                     : l_bnd - inner_prod;
      quad_viol += viol * viol;
    } else if (inner_prod > u_bnd) {
      viol = (std::abs(u_bnd) > Pecos::SMALL_NUMBER) ? (inner_prod / u_bnd - 1.)
                                                     : inner_prod - u_bnd;
      quad_viol += viol * viol;
    }
  }
...
void NonDNonHierarchSampling::enforce_augmented_linear_ineq_constraints(
    RealVector& cd_vars) {
  ...
  switch (optSubProblemForm) {
    ...
    case N_MODEL_LINEAR_OBJECTIVE: {  // no other lin ineq
      ...
      for (size_t approx = 0; approx < numApprox; ++approx)
        if (cd_vars[approx] < N_H_nudge) cd_vars[approx] = N_H_nudge;
      break;
    }
    case R_ONLY_LINEAR_CONSTRAINT:
    case R_AND_N_NONLINEAR_CONSTRAINT:
...
void NonDNonHierarchSampling::finite_solution_bounds(const RealVector& x0,
                                                     RealVector& x_lb,
                                                     RealVector& x_ub) {
  ...
  if (varMinIndices.first == 0 &&
    ...
    switch (optSubProblemForm) {
      ...
      case N_GROUP_LINEAR_OBJECTIVE: {
        ...
        // target accuracy via MC: varH[q] / tgt = tol * estvar0[q]; for
        // minimizer sequencing, a downstream refinement can omit this
        // approximated bound
        RealVector hf_targets;
        apply_mc_reference(hf_targets);
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "Finite bounds (lb, ub):\n"
         << x_lb << x_ub << "from modelGroupCost:\n"
         << modelGroupCost << std::endl;
}

void NonDNonHierarchSampling::derived_finite_solution_bounds(
  ...
  switch (optSubProblemForm) {
    ...
    case R_AND_N_NONLINEAR_CONSTRAINT: {  // not valid for group allocations
      ...
      size_t hf_index = x_len - 1;
      Real cost_sh = modelGroupCost[hf_index];
...
void NonDNonHierarchSampling::configure_minimizers(
    ...
    const RealMatrix& lin_ineq_coeffs, const RealMatrix& lin_eq_coeffs) {
  ...
  switch (optSubProblemSolver) {
    case SUBMETHOD_SBLO:
    case SUBMETHOD_SBGO:
      use_adapter = use_dfs = true;
      break;
    case SUBMETHOD_EGO:
    case SUBMETHOD_EA:  // case SUBMETHOD_CPS:
      use_adapter = true;
      break;
  ...
  if (varianceMinimizers.empty()) {
    ...
    switch (optSubProblemSolver) {
      case SUBMETHOD_NPSOL_OPTPP:  // competed
        solvers_0.resize(2);
        solvers_0[0] = SUBMETHOD_NPSOL;
        solvers_0[1] = SUBMETHOD_OPTPP;
        break;
      case SUBMETHOD_DIRECT_NPSOL: {  // sequenced global-local
        solvers_0.resize(1);
        solvers_0[0] = SUBMETHOD_DIRECT;
        solvers[1].resize(1);
        solvers[1][0] = SUBMETHOD_NPSOL;
        break;
      }
      case SUBMETHOD_DIRECT_OPTPP: {  // sequenced global-local
        ...
        solvers_0[0] = SUBMETHOD_DIRECT;
        solvers[1].resize(1);
        solvers[1][0] = SUBMETHOD_OPTPP;
        break;
      }
      case SUBMETHOD_DIRECT_NPSOL_OPTPP: {  // sequenced global-local+competed
                                            // local
        solvers_0.resize(1);
        solvers_0[0] = SUBMETHOD_DIRECT;
        ShortArray& solvers_1 = solvers[1];
        solvers_1.resize(2);
        solvers_1[0] = SUBMETHOD_NPSOL;
        solvers_1[1] = SUBMETHOD_OPTPP;
        break;
      }
      default:
        solvers_0.resize(1);
        solvers_0[0] = optSubProblemSolver;
        break;
    }
    ...
    if (use_adapter) {
      ...
      adapt_model = std::make_shared<MinimizerAdapterModel>(
          x0, x_lb, x_ub, lin_ineq_coeffs, lin_ineq_lb, lin_ineq_ub,
      ...
      if (use_dfs) {
        ...
        auto dace_iterator = std::make_shared<NonDLHSSampling>(
            adapt_model, sample_type, samples, seed, rng, vary_pattern,
            ACTIVE_UNIFORM);
        dace_iterator->active_set_request_values(data_order);

        String approx_type("global_kriging"),
            point_reuse("none");   // TO DO: spec
        UShortArray approx_order;  // empty
        ActiveSet dfs_set =
            adapt_model->current_response().active_set();  // copy
        dfs_set.request_values(1);
        const ShortShortPair& dfs_view =
            adapt_model->current_variables().view();
      ...
      for (j = 0; j < num_solvers; ++j) {
        ...
        switch (solvers_i[j]) {
...
#ifdef HAVE_NPSOL
            min_i[j] = std::make_shared<NPSOLOptimizer>(
                x0, x_lb, x_ub, lin_ineq_coeffs, lin_ineq_lb, lin_ineq_ub,
                lin_eq_coeffs, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub, nln_eq_tgt,
                npsol_objective, npsol_constraint, deriv_level, conv_tol,
                max_iter, fdss);
#endif
            break;
          }
          case SUBMETHOD_OPTPP: {
            ...
            String fd_type = "forward";
            RealVector fdss(1, false);
            fdss[0] = 1.e-7;  // ~7x > default
#ifdef HAVE_OPTPP
            if (analyticEstVarDerivs)
              min_i[j] = std::make_shared<SNLLOptimizer>(
                  x0, x_lb, x_ub, lin_ineq_coeffs, lin_ineq_lb, lin_ineq_ub,
                  lin_eq_coeffs, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub,
                  nln_eq_tgt, optpp_nlf1_objective,
                  optpp_nlf1_constraint,  // fdss, fd_type,
              ...
              switch (optSubProblemForm) {
                ...
                case N_GROUP_LINEAR_OBJECTIVE:
                  min_i[j] = std::make_shared<SNLLOptimizer>(
                      x0, x_lb, x_ub, lin_ineq_coeffs, lin_ineq_lb, lin_ineq_ub,
                      lin_eq_coeffs, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub,
                      nln_eq_tgt, optpp_nlf1_objective, optpp_fdnlf1_constraint,
                      fdss, fd_type, max_iter, max_eval, conv_tol, conv_tol,
                      max_step);
                  break;
                default:
                  min_i[j] = std::make_shared<SNLLOptimizer>(
                      x0, x_lb, x_ub, lin_ineq_coeffs, lin_ineq_lb, lin_ineq_ub,
                      lin_eq_coeffs, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub,
                      nln_eq_tgt, optpp_fdnlf1_objective, optpp_nlf1_constraint,
                      fdss, fd_type, max_iter, max_eval, conv_tol, conv_tol,
                      max_step);
                  break;
              }
          ...
          case SUBMETHOD_DIRECT: {  // global search + local refinement
            // > For DIRECT, we have the option of either passing an adapter
            // model
            //   or passing a fn callback.  Since we don't need parallel or
            //   other Model features, passing a function callback seems lighter
            //   weight.
            // > We also have the option of forming a merit fn for constraints
            //   (seems preferred for ordering the search) or passing
            //   "infeasible" codes intended for implicit constraints.  A merit
            //   fn requires another wrapper layer, either via model recursion
            //   or fn callback.
            // min_i[j].assign_rep(std::make_shared<NCSUOptimizer>(sub_prob_model,
            //  max_iter, max_eval, min_box_size, vol_box_size,
            //  solution_target));
            ...
            Real min_box_size = 1.e-6,  //-1. activates NCSU default = 1.e-4
                ...
                soln_target =
                    -DBL_MAX;  // no target, deactivates convergenceTol
#ifdef HAVE_NCSU
            min_i[j] = std::make_shared<NCSUOptimizer>(
                x_lb, x_ub, lin_ineq_coeffs, lin_ineq_lb, lin_ineq_ub,
                lin_eq_coeffs, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub, nln_eq_tgt,
                max_iter, max_eval, direct_penalty_merit, min_box_size,
          ...
          case SUBMETHOD_EGO: {
            ...
            min_i[j] = std::make_shared<EffGlobalMinimizer>(
                sub_prob_model, approx_type, samples, seed, use_derivs,
  ...
  for (i = start; i < sequence_len; ++i) {
    ...
    if (i < last_seq_index) {  // propagate best final pt to next initial pt(s)
      ...
      for (j = 0; j < num_solvers; ++j) {
        ...
        if (model_ij)
          model_ij->current_variables().active_variables(vars_star);
        else
          min_ij->initial_point(vars_star);
      }
    }
  }
...
void NonDNonHierarchSampling::minimizer_results_to_solution_data(
    const RealVector& cv_star, const RealVector& fn_star,
    MFSolutionData& soln) {
  ...
  Real estvar_metric = (optSubProblemForm == N_MODEL_LINEAR_OBJECTIVE ||
                        optSubProblemForm == N_GROUP_LINEAR_OBJECTIVE)
                           ? std::exp(fn_star[1])
                           : std::exp(fn_star(0));
  soln.estimator_variance_metric(estvar_metric);  // re-evaluated downstream
  ...
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT: {
      ...
      if (maxFunctionEvals != SZ_MAX) {
      ...
      } else {  // if (convergenceTol != -DBL_MAX) { // *** TO DO: detect user
                // spec
        // EstVar target = convTol * estvar_iter0 = estvar_ratio * varH /
        // N_target
        ...
        MFSolutionData::update_estimator_variance_metric(
            estVarMetricType, estVarMetricNormOrder, estvar_ratios, estvar,
            metric, metric_index);
        size_t hf_form, hf_lev;
        hf_indices(hf_form, hf_lev);
        if (backfillFailures)
          hf_target =
              (convergenceTolType == ABSOLUTE_CONVERGENCE_TOLERANCE)
                  ? update_hf_target(estvar, metric_index,
                                     NLevActual[hf_form][hf_lev])
                  : update_hf_target(estvar, metric_index,
                                     NLevActual[hf_form][hf_lev], estVarIter0);
        else
          hf_target =
              (convergenceTolType == ABSOLUTE_CONVERGENCE_TOLERANCE)
                  ? update_hf_target(estvar, metric_index,
                                     NLevAlloc[hf_form][hf_lev])
                  : update_hf_target(estvar, metric_index,
                                     NLevAlloc[hf_form][hf_lev], estVarIter0);
      ...
      soln.equivalent_hf_allocation(
          compute_equivalent_cost(hf_target, avg_eval_ratios, sequenceCost));
      break;
    }
    case R_AND_N_NONLINEAR_CONSTRAINT: {
...
void NonDNonHierarchSampling::estimator_variances_from_ratios(
    const RealVector& cd_vars, const RealVector& estvar_ratios,
    RealVector& estvar) {
  ...
  switch (optSubProblemForm) {
    case R_ONLY_LINEAR_CONSTRAINT:  // N is a vector constant for opt
                                    // sub-problem
      if (cd_vars.length() == num_approx) {
        ...
        estvar_ratios_to_estvar(estvar_ratios, varH, cd_vars[num_approx],
                                estvar);
      break;
    case N_MODEL_LINEAR_OBJECTIVE:
    case N_MODEL_LINEAR_CONSTRAINT:
...
void NonDNonHierarchSampling::estvar_gradients_to_metric_gradient(
    const RealVector& ev_vec, const RealMatrix& ev_grad, RealVector& evm_grad) {
  ...
  switch (estVarMetricType) {
    ...
    case NORM_ESTVAR_RATIO_METRIC: {
      ...
      size_t q, v;
      Real sum_q,
          p = estVarMetricNormOrder, pm1 = p - 1.,
    ...
    case MAX_ESTVAR_RATIO_METRIC:
      ...
      for (v = 0; v < num_v; ++v) evm_grad[v] = ev_grad(v, metric_index);
      break;
  }
}
...
void NonDNonHierarchSampling::method_recourse(unsigned short outer_method) {
  ...
  if (sol_conflict) switch (optSubProblemSolver) {
      case SUBMETHOD_NPSOL:
...
Real NonDNonHierarchSampling::nh_penalty_merit(const RealVector& cd_vars,
  ...
  switch (optSubProblemForm) {
    case N_MODEL_LINEAR_OBJECTIVE:
    case N_GROUP_LINEAR_OBJECTIVE: {
      Real nln_ineq_ub = (convergenceTolType == ABSOLUTE_CONVERGENCE_TOLERANCE)
                             ? std::log(convergenceTol)
                             : std::log(convergenceTol * estVarMetric0);
    ...
    case N_MODEL_LINEAR_CONSTRAINT:
      return nh_penalty_merit(fn_vals[0], linear_model_cost(cd_vars),
                              activeBudget);
      break;
    case N_GROUP_LINEAR_CONSTRAINT:
      return nh_penalty_merit(fn_vals[0], linear_group_cost(cd_vars),
                              activeBudget);
      break;
    case R_AND_N_NONLINEAR_CONSTRAINT:
...
Real NonDNonHierarchSampling::nh_penalty_merit(Real obj, Real nln_con,
                                               Real nln_u_bnd) {
...
void NonDNonHierarchSampling::linear_model_cost_gradient(
    const RealVector& N_vec, RealVector& grad_c) {
  // linear objective: N + Sum(w_i N_i) / w
  ...
  for (size_t i = 0; i < numApprox; ++i) grad_c[i] = sequenceCost[i] / cost_H;
  grad_c[numApprox] = 1.;
  if (outputLevel >= DEBUG_OUTPUT)
...
void NonDNonHierarchSampling::linear_group_cost_gradient(
    const RealVector& N_vec, RealVector& grad_c) {
  // default version for case where retainedModelGroups is undefined

  Real cost_H = sequenceCost[numApprox];
  for (size_t i = 0; i < numGroups; ++i) grad_c[i] = modelGroupCost[i] / cost_H;
  if (outputLevel >= DEBUG_OUTPUT)
...
Real NonDNonHierarchSampling::nonlinear_model_cost(const RealVector& r_and_N) {
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "nonlinear cost: design vars:\n"
         << r_and_N << "cost = " << nln_cost << std::endl;
  return nln_cost;
}

void NonDNonHierarchSampling::nonlinear_model_cost_gradient(
    const RealVector& r_and_N, RealVector& grad_c) {
...
void NonDNonHierarchSampling::npsol_objective(int& mode, int& n, double* x,
                                              double& f, double* grad_f,
                                              int& nstate) {
  ...
  switch (nonHierSampInstance->optSubProblemForm) {
    case N_MODEL_LINEAR_OBJECTIVE:
      if (asv_request & 1) f = nonHierSampInstance->linear_model_cost(x_rv);
      if (asv_request & 2) {
      ...
      break;
    case N_GROUP_LINEAR_OBJECTIVE:
      if (asv_request & 1) f = nonHierSampInstance->linear_group_cost(x_rv);
      if (asv_request & 2) {
        RealVector grad_f_rv(Teuchos::View, grad_f, n);
    ...
    default:
      if ((asv_request & 3) ==
          3) {  // compute metric + grads, avoiding redundancy
        ...
        if (asv_request & 1) f = nonHierSampInstance->log_estvar_metric(x_rv);
...
void NonDNonHierarchSampling::npsol_constraint(int& mode, int& ncnln, int& n,
                                               int& nrowj, int* needc,
                                               double* x, double* c,
  ...
  switch (nonHierSampInstance->optSubProblemForm) {
    ...
    case N_GROUP_LINEAR_OBJECTIVE:
      if ((asv_request & 3) ==
        ...
        nonHierSampInstance->log_estvar_metric_and_gradient(x_rv, c[0],
                                                            grad_c_rv);
        ...
        // NPSOL estimates unspecified components of the constr grad, so ASV
        // grad request is not an error
        if (asv_request & 2) {
...
void NonDNonHierarchSampling::optpp_nlf1_objective(int mode, int n,
                                                   const RealVector& x,
                                                   double& f,
                                                   RealVector& grad_f,
                                                   int& result_mode) {
...
void NonDNonHierarchSampling::optpp_nlf1_constraint(int mode, int n,
                                                    const RealVector& x,
                                                    RealVector& c,
                                                    RealMatrix& grad_c,
                                                    int& result_mode) {
...
void NonDNonHierarchSampling::optpp_fdnlf1_objective(int n, const RealVector& x,
                                                     double& f,
                                                     int& result_mode) {
  f = nonHierSampInstance->log_estvar_metric(x);
...
void NonDNonHierarchSampling::optpp_fdnlf1_constraint(int n,
                                                      const RealVector& x,
                                                      RealVector& c,
                                                      int& result_mode) {
  c[0] = nonHierSampInstance->log_estvar_metric(x);
...
Real NonDNonHierarchSampling::direct_penalty_merit(const RealVector& cd_vars) {
  ...
  Real lin_ineq_viol = nonHierSampInstance->augmented_linear_ineq_violations(
      cd_vars, direct_min->callback_linear_ineq_coefficients(),
      direct_min->callback_linear_ineq_lower_bounds(),
      direct_min->callback_linear_ineq_upper_bounds());
  bool protect_numerics =
      (lin_ineq_viol > 0.);  // RATIO_NUDGE reflected in viol
  ...
  if (protect_numerics) {
    ...
    log_metric = nonHierSampInstance->log_estvar_metric(cd_vars);

  switch (nonHierSampInstance->optSubProblemForm) {
    case N_MODEL_LINEAR_OBJECTIVE:
      obj = nonHierSampInstance->linear_model_cost(cd_vars);
      break;
    case N_GROUP_LINEAR_OBJECTIVE:
      obj = nonHierSampInstance->linear_group_cost(cd_vars);
      break;
    default:
      obj = log_metric;
      break;
...
void NonDNonHierarchSampling::response_evaluator(const Variables& vars,
                                                 const ActiveSet& set,
                                                 Response& response) {
  ...
  switch (nonHierSampInstance->optSubProblemForm) {
    case N_MODEL_LINEAR_OBJECTIVE:
      if (asv[0] & 1)
        response.function_value(nonHierSampInstance->linear_model_cost(c_vars),
                                0);
      if (asv[0] & 2) {
    ...
    case N_GROUP_LINEAR_OBJECTIVE:
      if (asv[0] & 1)
        response.function_value(nonHierSampInstance->linear_group_cost(c_vars),
                                0);
      if (asv[0] & 2) {
        RealVector grad_c = response.function_gradient_view(0);
      ...
      if ((asv[0] & 3) == 3) {
        ...
        if (asv[0] & 1)
          response.function_value(
              nonHierSampInstance->log_estvar_metric(c_vars), 0);
        if (asv[0] & 2) {
          RealVector grad_f = response.function_gradient_view(0);
  ...
  switch (nonHierSampInstance->optSubProblemForm) {
    ...
    case N_GROUP_LINEAR_OBJECTIVE:
      if (nln_con && ((asv[1] & 3) == 3)) {
        ...
        if (nln_con && (asv[1] & 1))
          response.function_value(
              nonHierSampInstance->log_estvar_metric(c_vars), 1);
        if (nln_con && (asv[1] & 2)) {
          RealVector grad_c = response.function_gradient_view(1);
    ...
    default:
      ...
      if (nln_con && (asv[1] & 1))
        response.function_value(
            nonHierSampInstance->nonlinear_model_cost(c_vars), 1);
      if (nln_con && (asv[1] & 2)) {
        RealVector grad_c = response.function_gradient_view(1);
...
void NonDNonHierarchSampling::accumulate_group_sums(
    IntRealMatrixArrayMap& sum_G, Sizet2DArray& num_G,
...
void NonDNonHierarchSampling::accumulate_group_sums(
    IntRealMatrixArrayMap& sum_G, Sizet2DArray& num_G, size_t group,
  ...
  for (r_cit = resp_map.begin(); r_cit != resp_map.end(); ++r_cit) {
    ...
    for (qoi = 0; qoi < numFunctions; ++qoi) {
      ...
      for (m = 0; m < num_models; ++m) {
        ...
        if (!isfinite(fn_vals[g_index])) all_finite = false;  // break;
      }
...
void NonDNonHierarchSampling::print_estimator_performance(
    std::ostream& s, const MFSolutionData& soln) const {
  ...
  Real proj_equiv_hf = equivHFEvals + deltaEquivHF, nh_est_var_q, nh_ratio_q,
       proj_mc_est_var_q, budget_mc_est_var_q;
  size_t qoi, proj_N_H_q, hf_form_index, hf_lev_index,
  ...
  switch (methodName) {
    ...
    case APPROX_CONTROL_VARIATE:
      method = "    ACV";
      break;
    case GEN_APPROX_CONTROL_VARIATE:
      method = " GenACV";
      break;
  }
  ...
  for (qoi = 0; qoi < numFunctions; ++qoi) {
    ...
    if (valid_variance(nh_est_var_q) && valid_variance(budget_mc_est_var_q))
      s << "   Equivalent" << method
        << " ratio:              " << std::setw(wpp7)
        << nh_est_var_q / budget_mc_est_var_q << '\n';
  }
}
