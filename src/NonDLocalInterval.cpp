...
NonDLocalInterval::NonDLocalInterval(ProblemDescDB& problem_db,
...
#ifdef HAVE_OPTPP
      minMaxOptimizer =
          std::make_unique<SNLLOptimizer>("optpp_q_newton", minMaxModel);
// #elif // handled within NonD::sub_optimizer_select()
#endif  // HAVE_OPTPP
      break;
    default:
...
void NonDLocalInterval::core_run() {
  ...
  copy_data(ModelUtils::continuous_variables(*minMaxModel),
            min_initial_pt);        // view->copy
  ...
  for (respFnCntr = 0; respFnCntr < numFunctions; ++respFnCntr) {
    ...
    for (cellCntr = 0; cellCntr < numCells; ++cellCntr) {
      ...
      ModelUtils::continuous_variables(*minMaxModel,
                                       min_initial_pt);  // set starting pt
      ...
      ModelUtils::continuous_variables(*minMaxModel,
                                       max_initial_pt);  // set starting point
      minMaxOptimizer->run(pl_iter);
...
void NonDLocalInterval::initialize() {}  // default is no-op

void NonDLocalInterval::set_cell_bounds() {}  // default is no-op
...
void NonDLocalInterval::post_process_final_results() {}  // default is no-op

void NonDLocalInterval::extract_objective(const Variables& sub_model_vars,
  ...
  if (recast_asv[0] & 2)
    recast_response.function_gradient(
        sub_model_response.function_gradient_view(nondLIInstance->respFnCntr),
        0);
  if (recast_asv[0] & 4)
...
void NonDLocalInterval::method_recourse(unsigned short method_name) {
  ...
  if (npsolFlag) {
    ...
    minMaxOptimizer->init_communicators(
        pl_iter);  // restore methodPCIter et al.
#else
