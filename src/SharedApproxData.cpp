...
SharedApproxData::SharedApproxData(BaseConstructor, ProblemDescDB& problem_db,
                                   size_t num_vars)
    :  // See base constructor in DakotaIterator.cpp for full discussion of
       // output verbosity.  For approximations, verbose adds quad poly coeff
       // reporting.
      numVars(num_vars),
      approxType(problem_db.get_string("model.surrogate.type")),
      buildDataOrder(1),
...
/** This constructor is the one which must build the base class data
    ...
    letter IS the representation, its rep pointer is set to NULL (an
    uninitialized pointer causes problems in ~SharedApproxData). */
SharedApproxData::SharedApproxData(NoDBBaseConstructor,
                                   const String& approx_type, size_t num_vars,
...
std::shared_ptr<SharedApproxData> SharedApproxData::get_shared_data(
    ProblemDescDB& problem_db, size_t num_vars) {
...
SharedApproxData::SharedApproxData(const String& approx_type,
                                   const UShortArray& approx_order,
                                   size_t num_vars, short data_order,
                                   short output_level)
    :  // Set the rep pointer to the appropriate derived type
      dataRep(get_shared_data(approx_type, approx_order, num_vars, data_order,
                              output_level)) {
  if (!dataRep)  // bad type or insufficient memory
    abort_handler(APPROX_ERROR);
}
...
/** Used only by the envelope constructor to initialize dataRep to the
    appropriate derived type. */
std::shared_ptr<SharedApproxData> SharedApproxData::get_shared_data(
    const String& approx_type, const UShortArray& approx_order, size_t num_vars,
    short data_order, short output_level) {
  // if (approx_type == "local_taylor")
...
#endif
    // else if (approx_type == "global_gaussian")
    //   approx = new SharedGaussProcApproxData(num_vars,
...
SharedApproxData::SharedApproxData(const SharedApproxData& shared_data)
    : dataRep(shared_data.dataRep) { /* empty ctor */ }

SharedApproxData SharedApproxData::operator=(
    const SharedApproxData& shared_data) {
  dataRep = shared_data.dataRep;
  return *this;  // calls copy constructor since returned by value
}

SharedApproxData::~SharedApproxData() { /* empty dtor */ }
...
void SharedApproxData::integration_iterator(const Iterator& iterator) {
  if (dataRep)
    dataRep->integration_iterator(iterator);
...
short SharedApproxData::discrepancy_reduction() const {
  if (dataRep)
    return dataRep->discrepancy_reduction();
  else
    return Pecos::NO_DISCREPANCY;  // this enum is 0
}
...
void SharedApproxData::rebuild() {
  if (dataRep)
    ...
    build();  // if incremental not defined, fall back to full build
}
...
void SharedApproxData::pre_push() {
  if (dataRep) dataRep->pre_push();
  // else no-op (derived implementation not required)
}

void SharedApproxData::post_push() {
  if (dataRep) dataRep->post_push();
  // else no-op (derived implementation not required)
}
...
void SharedApproxData::construct_basis(
    const Pecos::MultivariateDistribution& mv_dist) {
  if (dataRep)
    dataRep->construct_basis(mv_dist);
  else {  // virtual fn: no default, error if not supplied by derived
...
void SharedApproxData::update_basis_distribution_parameters(
    const Pecos::MultivariateDistribution& mvd) {
  if (dataRep) dataRep->update_basis_distribution_parameters(mvd);
  // else no-op (derived implementation not required)
}

bool SharedApproxData::formulation_updated() const {
  if (dataRep)
    return dataRep->formulation_updated();
  else {  // not virtual
...
void SharedApproxData::formulation_updated(bool update) {
  if (dataRep)
    dataRep->formulation_updated(update);
  else
    formUpdated[activeKey] = update;
}

void SharedApproxData::configuration_options(
    const Pecos::ExpansionConfigOptions& ec_options) {
  if (dataRep) dataRep->configuration_options(ec_options);
  // else no-op (derived implementation not required)
}

void SharedApproxData::configuration_options(
    const Pecos::BasisConfigOptions& bc_options) {
  if (dataRep) dataRep->configuration_options(bc_options);
  // else no-op (derived implementation not required)
}

void SharedApproxData::configuration_options(
    const Pecos::RegressionConfigOptions& rc_options) {
  if (dataRep) dataRep->configuration_options(rc_options);
  // else no-op (derived implementation not required)
}

void SharedApproxData::random_variables_key(const BitArray& random_vars_key) {
  if (dataRep) dataRep->random_variables_key(random_vars_key);
  // else no-op (derived implementation not required)
}

void SharedApproxData::refinement_statistics_mode(short stats_mode) {
  if (dataRep) dataRep->refinement_statistics_mode(stats_mode);
  // else no-op (derived implementation not required)
}
