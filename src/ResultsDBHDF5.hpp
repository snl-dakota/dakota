...
String scale_hdf5_link_name(const StrStrSizet &iterator_id,
  ...
  for (size_t i = 0; i < location.size() - 1; ++i) rval += "/" + location[i];
...
class AttachScaleVisitor : public boost::static_visitor<> {
 public:
  ...
  AttachScaleVisitor(const StrStrSizet &iterator_id,
                     const StringArray &location, const int &dim,
  ...
  void operator()(const RealScale &scale) {
    String name = scale_hdf5_link_name(iteratorID, location, scale);
    if (!hdf5Stream->exists(name)) {
      if (scale.isMatrix) {
        Teuchos::SerialDenseMatrix<int, Real> scale_matrix(
            Teuchos::View, scale.items.values(), sizeof(scale.items[0]),
            scale.items.length() / scale.numCols, scale.numCols);
...
/// Manage interactions between ResultsManager and the low-level HDFIOHelper
/// class
class ResultsDBHDF5 : public ResultsDBBase {
 public:
  ...
  ResultsDBHDF5(bool in_core, std::shared_ptr<HDF5IOHelper> hdf5_helper_ptr)
    ...
    AttributeArray root_attrs = {
        ResultAttribute<String>("dakota_version",
                                DakotaBuildInfo::get_release_num()),
        ResultAttribute<String>("dakota_revision",
                                DakotaBuildInfo::get_rev_number()),
        ResultAttribute<String>("output_version",
                                ResultsDBHDF5::outputVersion)};
  ...
  /// attributes. Insert elements using insert_into(...)
  ...
  void insert(const StrStrSizet &iterator_id, const std::string &data_name,
              const boost::any &result, const MetaDataType &metadata) override {
    return;
  }
 ...
 private:
  ...
  void attach_scales(const String &dset_name, const StrStrSizet &iterator_id,
                     const StringArray &location, const DimScaleMap &scales);
  ...
  /// Instance of HDF5IOHelper (must be a pointer because it's shared with the
