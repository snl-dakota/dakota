...
class ReducedBasis {
 public:
  ...
  const RealMatrix &get_matrix() { return matrix; }
  ...
  bool is_valid() const { return is_valid_svd; }

  const Real &get_singular_values_sum() const { return singular_values_sum; }

  const Real &get_eigen_values_sum() const { return eigen_values_sum; }
  ...
  const RealVector &get_singular_values() const { return S_values; }
  ...
  const RealMatrix &get_left_singular_vector() const { return U_matrix; }
