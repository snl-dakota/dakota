...
class DataTransformModel : public RecastModel {
 public:
  ...
  void print_best_responses(std::ostream& s,
                            ...
                            const Response& best_submodel_resp, size_t num_best,
                            size_t best_ind);
 ...
 protected:
  ...
  void scale_response(const Variables& submodel_vars,
                      const Variables& recast_vars, Response& recast_response);
  ...
  /// Archive the best configuration variables associated with each model
  /// response
  ...
  void archive_best_residuals(const ResultsManager& results_db,
                              const StrStrSizet& iterator_id, const int num_fns,
                              const RealVector& best_terms, const Real wssr,
...
inline void DataTransformModel::assign_instance() { dtModelInstance = this; }
