...
void NomadOptimizer::find_optimum()
{
     ...
     for(int i=0;i<this->numInt+this->numReal;i++)
     {
     ...
     IntVector  discIntVars(numInt);
     RealVector discRealVars(numReal);
     ...
     for(j=0, dsi_cntr=0; j<numInt; j++)
     { 
       if (int_set_bits[j]) { // this active discrete int var is a set type
	 discIntVars[j] = set_index_to_value((*bestX)[j+numCont].value(), set_int_vars[dsi_cntr]);
	 ++dsi_cntr;
       }
       else  {                // this active discrete int var is a range type
	 discIntVars[j] = (*bestX)[j+numCont].value();
       }
     }
     bestVariablesArray.front().discrete_int_variables(discIntVars);
     for (j=0; j<numReal; j++) {
       discRealVars = set_index_to_value((*bestX)[j+numCont+numInt].value(), set_real_vars[j]);
     }
...
void NomadOptimizer::load_parameters(Model &model)
{
     ...
     numInt = model.div();
     numReal = model.drv();
     numTotal = numCont + numInt + numReal;
	...
	if (index == _NPOS) {
	  ...
	  _initial_point[i+numCont] = (int)index;
	  _lower_bound[i+numCont] = 0;
	  _upper_bound[i+numCont] = initial_point_set_int[dsi_cntr].size() - 1;
	}
	++dsi_cntr;
       }
       else {
	 _initial_point[i+numCont] = initial_point_int[i];
	 _lower_bound[i+numCont] = lower_bound_int[i];
	 _upper_bound[i+numCont] = upper_bound_int[i];
       }
     }
     for (i=0; i<numReal; i++) {
	 ...
	 _initial_point[i+numCont+numInt] = (int)index;
	 _lower_bound[i+numCont+numInt] = 0;
	 _upper_bound[i+numCont+numInt] = initial_point_set_real[i].size() - 1;
       }
     }
...
NomadOptimizer::NomadOptimizer(NoDBBaseConstructor, Model& model):
