...
static const char rcsId[] =
    "@(#) $Id: DakotaOptimizer.cpp 7031 2010-10-22 16:23:52Z mseldre $";
...
Optimizer::Optimizer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
  ...
  if ((methodName == COLINY_DIRECT) || (methodName == COLINY_EA)) {
    ...
    for (size_t i = 0; i < numContinuousVars; ++i)
      if (c_l_bnds[i] <= -bigRealBoundSize || c_u_bnds[i] >= bigRealBoundSize) {
        Cerr << "\nError: finite bound constraints are required for global "
                "optimizer "
             << method_enum_to_string(methodName)
             << ".\n       Please specify both "
  ...
  if (methodName == OPTPP_NEWTON) {  // || ...) {
    ...
    if (have_lsq) {
      if (iteratedModel->gradient_type() == "none") {
        Cerr << "\nError: full Newton optimization of least-squares problem "
                "requires calibration term gradients."
             << std::endl;
        err_flag = true;
      }
      if (numNonlinearConstraints &&
          (iteratedModel->hessian_type() == "none")) {
        Cerr << "\nError: full Newton optimization of least-squares problem "
                "with nonlinear constraints "
             << "requires constraint Hessians.  Alternatively, consider using "
                "optpp_g_newton."
             << std::endl;
        err_flag = true;
      }
      ...
      Cerr << "\nError: full Newton optimization requires objective Hessians. "
           << "Alternatively, consider using optpp_q_newton." << std::endl;
      err_flag = true;
    }
  }
  ...
  if (have_lsq) {
    ...
    if (numUserPrimaryFns > 1 && (methodName != MOGA && methodName != SOGA))
      localObjectiveRecast = true;
  } else {
  ...
  if (calibrationDataFlag) data_transform_model();
  if (scaleFlag) scale_model();
  if (localObjectiveRecast) reduce_model(have_lsq, require_hessians);
}

Optimizer::Optimizer(unsigned short method_name, std::shared_ptr<Model> model,
                     std::shared_ptr<TraitsBase> traits)
...
Optimizer::Optimizer(unsigned short method_name, size_t num_cv, size_t num_div,
  ...
  vc_totals[TOTAL_CDV] = num_cv;
  vc_totals[TOTAL_DDIV] = num_div;
  vc_totals[TOTAL_DDSV] = num_dsv;
  vc_totals[TOTAL_DDRV] = num_drv;
  ...
  activeSet.derivative_start_value(1);
  ...
  for (i = 0; i < num_best; ++i) {
    ...
    if (optimizationFlag) {
      ...
      if (calibrationDataFlag) {
        ...
        print_residuals(numUserPrimaryFns, best_fns, lsq_weights, num_best, i,
                        s);  // the weights are only used to compute the norm
      }
    }
...
void Optimizer::get_common_stopping_criteria(int& max_fn_evals, int& max_iters,
                                             double& conv_tol,
                                             double& min_var_chg,
                                             double& obj_target) {
...
void Optimizer::reduce_model(bool local_nls_recast, bool require_hessians) {
  ...
  if (local_nls_recast) {
    ...
    size_t total_calib_terms =
        (calibrationDataFlag) ? numTotalCalibTerms : numUserPrimaryFns;
  ...
  void (*set_recast)(const Variables&, const ActiveSet&, ActiveSet&) =
      (local_nls_recast && require_hessians &&
       iteratedModel->hessian_type() == "none")
          ? gnewton_set_recast
          : NULL;
  void (*pri_resp_recast)(const Variables&, const Variables&, const Response&,
                          Response&) = primary_resp_reducer;
  void (*sec_resp_recast)(const Variables&, const Variables&, const Response&,
                          Response&) = NULL;
  ...
  iteratedModel = std::make_shared<RecastModel>(
      ...
      iteratedModel->current_variables().view(), vars_recast, set_recast,
      primary_resp_map_indices, secondary_resp_map_indices,
      recast_secondary_offset, recast_resp_order, nonlinear_resp_map,
...
/** Objective function map from multiple primary responses (objective
    ...
    goal-oriented */
void Optimizer::primary_resp_reducer(const Variables& full_vars,
  ...
  optimizerInstance->objective_reduction(
      full_response, sub_model->primary_response_fn_sense(),
      sub_model->primary_response_fn_weights(), reduced_response);
...
void Optimizer::objective_reduction(const Response& full_response,
  ...
  for (int i = 0; i < full_wts.length(); ++i) Cout << full_wts[i] << std::endl;
  ...
  if (reduced_asv0 & 1) {  // build objective fn from full_response functions
    Real sum =
        objective(full_response.function_values(), num_fns, sense, full_wts);
...
void Optimizer::configure_constraint_maps() {
  if (traits()->supports_nonlinear_inequality()) {
    ...
    if (traits()->nonlinear_inequality_format() ==
        NONLINEAR_INEQUALITY_FORMAT::NONE) {
      Cerr << "\nError: inconsistent format for NONLINEAR_INEQUALITY_FORMAT in "
              "traits."
           << std::endl;
      abort_handler(-1);
    }

    Real scaling = (traits()->nonlinear_inequality_format() ==
                    NONLINEAR_INEQUALITY_FORMAT::ONE_SIDED_LOWER)
                       ? 1.0
                       : -1.0;

    numNonlinearIneqConstraintsFound = configure_inequality_constraint_maps(
        *iteratedModel, bigRealBoundSize, CONSTRAINT_TYPE::NONLINEAR,
        constraintMapIndices, constraintMapMultipliers, constraintMapOffsets,
        scaling);
  }
}
...
void Optimizer::initialize_run() {
  ...
  if (iteratedModel) configure_constraint_maps();
}
  ...
  for (size_t point_index = 0; point_index < num_points; ++point_index) {
    ...
    if (localObjectiveRecast) {
      ...
      if (!resp_found && scaleFlag && numNonlinearConstraints > 0) {
        ...
        // only requesting scaling of constraints, so no need for variable
        // Jacobian
        activeSet.request_values(1);
        ...
        scale_model_rep->secondary_resp_scaled2native(
            best_fns, activeSet.request_vector(), numUserPrimaryFns, best_fns);
      }
    }
