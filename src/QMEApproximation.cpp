...
QMEApproximation::QMEApproximation(ProblemDescDB& problem_db,
...
void QMEApproximation::build() {
  ...
  if (num_pts == 1) {
    ...
    if (prevGradIndex == _NPOS)  // no previous gradient point
      ...
      for (size_t i = 0; i < num_v; i++) minX[i] = std::min(x1[i], x2[i]);
    }
...
void QMEApproximation::find_scaled_coefficients() {
  ...
  size_t num_pts = approxData.points(), num_v = sharedDataRep->numVars;
  size_t k = num_pts - 1,
         p = num_pts - 2;  // indices to current and previous points
  ...
  */
  ...
  D.multiply(Teuchos::TRANS, Teuchos::NO_TRANS, 1.0, G_reduced_xfm, dy, 0.0);
  if (D.numCols() != numUsed) {
    Cerr << "Error: QMEApproximation num sing vectors=" << D.numCols()
         << "num_pts_used= " << numUsed << std::endl;
    abort_handler(-1);
  }
  dy.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0, G_reduced_xfm, D, 0.0);
#ifdef DEBUG
  Cout << "D=" << D << '\n';
  ...
  approx_delta.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0, D2T, beta,
                        0.0);
#ifdef DEBUG
  Cout << "after qr_solve" << '\n';
...
Real QMEApproximation::apxfn_value(const RealVector& x) {
  ...
  if (num_pts == 1) {  // First-order Taylor series (interim approx)
...
#ifdef DEBUG
    Cout << "epsilon: " << epsilon << " sum1: " << sum1
         << " Lin_val: " << lin_val << " TANA3 approx value: " << approx_val
         << '\n';
#endif  // DEBUG
    ...
    Real quad_term = 0.0;
    // Project transformed design point difference from anchor onto reduced
    // subspace
