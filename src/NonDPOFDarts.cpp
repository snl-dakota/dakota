...
static const char rcsId[] =
    "@(#) $Id: NonDPOFDart.cpp 6080 2009-09-08 19:03:20Z gtang $";

namespace Dakota {

NonDPOFDarts::NonDPOFDarts(ProblemDescDB& problem_db,
  ...
  */
  ...
  if (lipschitzType == "local") {
    ...
    _use_local_L =
        false;  // Global Lipschitz: less sampling time - less accuracy
...
void NonDPOFDarts::core_run() {
  ...
  if (fabs(seed - 1.0) < 1E-10)
    seed = time(0);  // Automated seed
  else if (fabs(seed - 1.0) < 2) {
    seed = 1234567890;

    _eval_error = true;

    std::cout << "Evaluate Error: (0: No, 1: SmoothHerbie, 2: Herbie, 3: "
                 "PlanarCross, 4: CircularCone) ";
  ...
  generate_a_random_number()<<std::endl;
  }
  */
...
void NonDPOFDarts::init_pof_darts() {
  ...
  // Estmate max radius of spheres based on number of evaluation and unifrom
  // distribution
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
    _fval[resp_fn_count] = new double[_total_budget];

  _Lip = new double[numFunctions];
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
    _Lip[resp_fn_count] = 0.0;
}
...
void NonDPOFDarts::execute(size_t kd) {
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions;
    ...
    for (size_t level_count = 0; level_count < num_levels; level_count++) {
      ...
      // adjust prior sphere radii to reflect current response function and
      // threshold
      for (size_t isample = 0; isample < _num_inserted_points; isample++)
      ...
      std::cout << "pof::    Number of inserted points = " << std::fixed
                << _num_inserted_points << std::endl;
      std::cout << "pof::    Number of thrown darts = " << std::fixed
                << _num_darts << std::endl;
      std::cout.precision(4);
      std::cout << "pof::    Execution Time = " << std::fixed << cpu_time
                << " seconds." << std::endl;

      /*
      for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++)
      {
          std::cout << "Sample Point :" << ipoint << " - num neighbors = " <<
      _sample_neighbors[ipoint][0] << std::endl;

          for (size_t i = 1; i < _sample_neighbors[ipoint][0]; i++)
          {
              std::cout << "   Neighbor: " << i << " : " <<
      _sample_neighbors[ipoint][i] << std::endl;
          }
      }
      */
    }
...
void NonDPOFDarts::classical_dart_throwing_games(size_t game_index) {
  ...
  while (true) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++)
      _dart[idim] = generate_a_random_number();
    ...
    if (valid_dart(_dart)) {
      ...
      if (_num_inserted_points - num_prior_disks == samples) {
        std::cout << "\npof:: Simulation Budget has been exhausted!"
                  << std::endl;
        return;
      }
    } else {
      _num_successive_misses_p++;
      if (_num_successive_misses_p + _num_successive_misses_m >
          _max_num_successive_misses) {
        std::cout << "\npof:: Void-finding budget has been exhausted, "
                     "Increaing Lipschitz Multiplier!"
                  << std::endl;
        _safety_factor *= 1.5;
        for (size_t isample = 0; isample < _num_inserted_points; isample++) {
          assign_sphere_radius_POF(isample);
        ...
        // std::cout<< "\npof:: Void-finding budget has been exhausted,
        // shrinking BIG disks!" << std::endl; shrink_big_spheres();
      }
    }
  }
}
...
void NonDPOFDarts::line_dart_throwing_games(size_t game_index) {
  ...
  while (true) {
    ...
    if (_use_local_L && nVsampling == 25) {
      ...
      _max_vsize = 0.0;
      size_t ilargest(_num_inserted_points);
      for (size_t isample = 0; isample < _num_inserted_points; isample++) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      ...
      for (size_t jdim = 0; jdim < _n_dim; jdim++)
        _dart[jdim] = generate_a_random_number();

      _flat_dim = _line_flat[idim];
      _num_flat_segments = 1;
      _line_flat_start[0] = 0.0;
      _line_flat_end[0] = 1.0;
    ...
    if (covered) {
      ...
      if (_num_successive_misses_p + _num_successive_misses_m >
        ...
        // std::cout<< "\npof:: Void-finding budget has been exhausted,
        // shrinking all disks!" << std::endl; shrink_big_spheres();
      }
    } else {
      _num_successive_misses_m = _num_successive_misses_p;
      _num_successive_misses_p = 0;
...
bool NonDPOFDarts::valid_dart(double* x) {
  for (size_t index = 0; index < _num_inserted_points; index++) {
    ...
    if (dd < fabs(_sample_points[index][_n_dim]))
      return false;  // prior disk approach
  }
  return true;
}
  ...
  for (size_t index = 0; index < _num_inserted_points; index++) {
    ...
    if (hh < fabs(_sample_points[index][_n_dim])) {
      ...
      while (ist != iend) {
        ...
        if (_line_flat_start[imid] < segmin)
          ist = imid;
        else
          iend = imid;
      }

      // remove the line segment from _sample_points[index][flat_dim] - v to
      // _sample_points[index][flat_dim] + v
      while (iend < _num_flat_segments) {
        double xmin = _line_flat_start[iend];
        double xmax = _line_flat_end[iend];

        if (segmax < xmin) break;

        if (segmax > xmin && segmax < xmax && segmin < xmin) {
          // Adjust line segment start
          _line_flat_start[iend] = segmax;
          break;
        } else if (segmin > xmin && segmax < xmax) {
          // split line segment
          for (size_t jseg = _num_flat_segments; jseg > iend;
               jseg--)  // iend >=0
          {
...
void NonDPOFDarts::add_point(double* x) {
  ...
  for (size_t idim = 0; idim < _n_dim; idim++)
    _sample_points[_num_inserted_points][idim] = x[idim];

  double* x_actual = new double[_n_dim];
  for (size_t idim = 0; idim < _n_dim; idim++)
    x_actual[idim] = _xmin[idim] + x[idim] * (_xmax[idim] - _xmin[idim]);
  ...
  if (_use_local_L) {
    ...
    if (_sample_neighbors[_num_inserted_points - 1] != 0)
      num_neighbors = _sample_neighbors[_num_inserted_points - 1][0];
    for (size_t i = 1; i <= num_neighbors; i++) {
      size_t neighbor = _sample_neighbors[_num_inserted_points - 1][i];
      assign_sphere_radius_POF(
          neighbor);  // adjusting radius of old disks due to new L
    }
...
void NonDPOFDarts::compute_response(double* x) {
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions;
       resp_fn_count++) {
    double fval =
        iteratedModel->current_response().function_value(resp_fn_count);
...
bool NonDPOFDarts::add_neighbor(size_t ipoint, size_t ineighbor) {
  ...
  size_t num_neighbors(_sample_neighbors[ipoint][0]);
  for (size_t i = 1; i <= num_neighbors; i++) {
    if (_sample_neighbors[ipoint][i] == ineighbor) {
      found = true;
      break;
    }
  }
  if (!found) {
    ...
    for (size_t i = 0; i <= num_neighbors; i++)
      tmp[i] = _sample_neighbors[ipoint][i];
...
void NonDPOFDarts::retrieve_neighbors(size_t ipoint,
...
void NonDPOFDarts::update_global_L() {
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions;
    ...
    for (size_t isample = 0; isample < last_point; isample++) {
      ...
      for (size_t idim = 0; idim < _n_dim; idim++) {
        double dx =
            _sample_points[isample][idim] - _sample_points[last_point][idim];
        dst += dx * dx;
      }
      dst = std::sqrt(dst);
      double L = fabs(_fval[resp_fn_count][isample] -
                      _fval[resp_fn_count][last_point]) /
                 dst;
...
void NonDPOFDarts::assign_sphere_radius_POF(size_t isample) {
  ...
  if (_use_local_L) {
    ...
    for (size_t i = 1; i <= num_neighbors; i++) {
      ...
      for (size_t idim = 0; idim < _n_dim; idim++) {
        double dx =
            _sample_points[isample][idim] - _sample_points[neighbor][idim];
        dst += dx * dx;
      }
      dst = std::sqrt(dst);
      double grad = fabs(_fval[_active_response_function][isample] -
                         _fval[_active_response_function][neighbor]) /
                    dst;
  ...
  if (L > 1E-10)
    r = fabs(_fval[_active_response_function][isample] - _failure_threshold) /
        L;  // radius based on Lipschitz

  _sample_points[isample][_n_dim] = r * r;
  if (_fval[_active_response_function][isample] < _failure_threshold)
    _sample_points[isample][_n_dim] = -_sample_points[isample][_n_dim];
  ...
  if (_use_local_L) {
    ...
    for (size_t jsample = 0; jsample < _num_inserted_points; jsample++) {
      // if (_sample_points[isample][_n_dim] * _sample_points[jsample][_n_dim] >
      // 0.0) continue; // same color
      ...
      for (size_t idim = 0; idim < _n_dim; idim++) {
        double dx =
            _sample_points[isample][idim] - _sample_points[jsample][idim];
        dst_sq += dx * dx;
      }
      ...
      if (r_i + r_j > dst) {
        L = fabs(_fval[_active_response_function][isample] -
                 _fval[_active_response_function][jsample]) /
            dst;
        double r_i_new = fabs(_fval[_active_response_function][isample] -
                              _failure_threshold) /
                         L;
        double r_j_new = fabs(_fval[_active_response_function][jsample] -
                              _failure_threshold) /
                         L;
        if (r_i_new < r_i) {
        ...
        if (r_j_new < r_j) {
          ...
          if (_fval[_active_response_function][jsample] < _failure_threshold)
            _sample_points[jsample][_n_dim] = -_sample_points[jsample][_n_dim];
        }
      }
    }
  }
...
void NonDPOFDarts::shrink_big_spheres() {
  double rr_max(0.0);
  for (size_t isample = 0; isample < _num_inserted_points; isample++) {
    if (fabs(_sample_points[isample][_n_dim]) > rr_max)
      rr_max = fabs(_sample_points[isample][_n_dim]);
  }

  std::cout << "\npof:: maximum radius = " << std::sqrt(rr_max) << std::endl;

  for (size_t isample = 0; isample < _num_inserted_points; isample++) {
    if (fabs(_sample_points[isample][_n_dim]) > 0.95 * 0.95 * rr_max)
      _sample_points[isample][_n_dim] *= (0.95 * 0.95);
  }
}

double NonDPOFDarts::area_triangle(double x1, double y1, double x2, double y2,
                                   double x3, double y3) {
...
void NonDPOFDarts::add_surrogate_data(const Variables& vars,
                                      const Response& resp) {
...
void NonDPOFDarts::estimate_pof_surrogate() {
  ...
  std::cout.precision(4);
  std::cout << "pof::    Surrogates built in " << std::fixed << cpu_time
            << " seconds." << std::endl;
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions;
    ...
    for (size_t level_count = 0; level_count < num_levels; level_count++)
      num_fMC_samples[resp_fn_count][level_count] = 0.0;
  }
  ...
  if (pdfOutput) {
    ...
    for (size_t i = 0; i < numFunctions; ++i) {
      extremeValues[i].first = DBL_MAX;
      extremeValues[i].second = -DBL_MAX;
    }
  }
  ...
  while (isample < num_MC_samples) {
    ...
    for (size_t idim = 0; idim < _n_dim; idim++) {
      tmp_pnt[idim] = _xmin[idim] +
                      generate_a_random_number() * (_xmax[idim] - _xmin[idim]);
    }
  ...
  std::cout << "pof::    Surrogate evaluated in " << std::fixed << cpu_time
            << " seconds." << std::endl;

  if (_eval_error) {
    std::cout.precision(15);
    std::cout << "pof::    exact pof " << std::fixed
              << fabs(pof_exact) / num_MC_samples << std::endl;
    std::cout << "pof::    Surrogate error " << std::fixed
              << fabs((num_fMC_samples[0][0] - pof_exact) / num_MC_samples)
              << std::endl;
  }
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions;
    ...
    for (size_t level_count = 0; level_count < num_levels; level_count++) {
      double pof_val =
          num_fMC_samples[resp_fn_count][level_count] / num_MC_samples;
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
    delete[] num_fMC_samples[resp_fn_count];
  delete[] num_fMC_samples;
  delete[] tmp_pnt;
}

bool NonDPOFDarts::trim_line_using_Hyperplane(
...
void NonDPOFDarts::plot_vertices_2d(bool plot_true_function,
                                    bool plot_suurogate) {
  std::stringstream ss;
  ...
  for (size_t index = 0; index < _num_inserted_points; index++) {
    ...
    // if (_sample_vsize[index] >  0.8 * _max_vsize) file << "bluefcirc"     <<
    // std::endl; // large Vcell
    if (_sample_points[index][2] > 0)
      file << "greenfcirc" << std::endl;  // non-failure disk
    else
    ...
    for (size_t i = 0; i < 1000; i++) {
      ...
      for (size_t j = 0; j < 1000; j++) {
        ...
        if ((fo > _failure_threshold && f1 < _failure_threshold) ||
            (fo < _failure_threshold && f1 > _failure_threshold)) {
        ...
        if ((f1 > _failure_threshold && f2 < _failure_threshold) ||
            (f1 < _failure_threshold && f2 > _failure_threshold)) {
        ...
        if ((f2 > _failure_threshold && f3 < _failure_threshold) ||
            (f2 < _failure_threshold && f3 > _failure_threshold)) {
        ...
        if ((f3 > _failure_threshold && fo < _failure_threshold) ||
            (f3 < _failure_threshold && fo > _failure_threshold)) {
        ...
        if (num_points == 2) {
          ...
          if (num_failure_points == 1) {
            double a;
            if (fo < _failure_threshold)
              a = fabs(area_triangle(xo, yo, x1, y1, x2, y2));
            if (f1 < _failure_threshold)
              a = fabs(area_triangle(xo + sx, yo, x1, y1, x2, y2));
            if (f2 < _failure_threshold)
              a = fabs(area_triangle(xo + sx, yo + sy, x1, y1, x2, y2));
            if (f3 < _failure_threshold)
              a = fabs(area_triangle(xo, yo + sy, x1, y1, x2, y2));
            failure += a;
            non_failure += (sx * sy - a);
        ...
        for (size_t index = 0; index < _num_inserted_points; index++) {
          ...
          if (dx * dx + dy * dy < rrs) {
            ...
            if (_sample_points[index][2] < 0.0)
              failure_spheres += sx * sy;
            else
              non_failure_spheres += sx * sy;
            break;  // move to the next cell
          }
        }
      }
    }
    Cout << "pof::       POF (isocontouring) = " << std::setprecision(10)
         << failure / A << '\n';
  ...
  if (plot_suurogate) {
    std::cout << "plotting _active_response_function = "
              << _active_response_function << std::endl;
  ...
  // file << _dart[0] * scale << "  " << _dart[1] * scale << "  " << 1.4142 * s
  // * scale << "  "; file << "circ"     << std::endl;
...
void NonDPOFDarts::plot_neighbors() {
  ...
  for (size_t index = 0; index < _num_inserted_points; index++) {
    ...
    file << _sample_points[index][0] * scale << "  "
         << _sample_points[index][1] * scale << "  " << r * scale << "  ";
  ...
  for (size_t isample = 0; isample < _num_inserted_points; isample++) {
    ...
    for (size_t i = 1; i <= num_neighbors; i++) {
      ...
      file << _sample_points[isample][0] * scale << "  "
           << _sample_points[isample][1] * scale << "  ";
      file << _sample_points[neighbor][0] * scale << "  "
           << _sample_points[neighbor][1] * scale << "  ";
      file << "blueseg" << std::endl;
    }
  }
  ...
  for (size_t index = 0; index < _num_inserted_points; index++) {
    ...
    // plot vertex
    file << _sample_points[index][0] * scale << "  "
         << _sample_points[index][1] * scale << "  " << s * scale << " ";
    file << "blackfcirc" << std::endl;  // non-failure disk
  }
}
