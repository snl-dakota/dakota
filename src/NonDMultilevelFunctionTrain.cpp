...
NonDMultilevelFunctionTrain::NonDMultilevelFunctionTrain(
    ...
    : NonDC3FunctionTrain(DEFAULT_METHOD, problem_db, model),
      startRankSeqSpec(problem_db.get_sza(
          "method.nond.c3function_train.start_rank_sequence")),
      startOrderSeqSpec(problem_db.get_usa(
      ...
      sequenceIndex(0)  // resizedFlag(false), callResize(false)
{
  ...
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, mlft_set, mlft_view, approx_type,
      start_orders, corr_type, corr_order, data_order, outputLevel, pt_reuse,
...
colloc_ratio, const SizetArray& seed_seq, short u_space_type, short refine_type,
    ...
    case MULTILEVEL_FUNCTION_TRAIN:
      ...
      switch (multilevAllocControl) {
        case DEFAULT_MLMF_CONTROL:  // define MLFT-specific default
          multilevAllocControl = RANK_SAMPLING;
          break;
        case RANK_SAMPLING: {
          ...
          //       case of unspecified max rank: [start_r,
          //       start_r+max_cv*kick_r]
          break;
        }
...
void NonDMultilevelFunctionTrain::infer_pilot_sample(
...
void NonDMultilevelFunctionTrain::increment_sample_sequence(size_t new_samp,
                                                            ...
                                                            size_t step) {
  numSamplesOnModel =
      new_samp;  // total_samp,lev not used by this derived class
...
void NonDMultilevelFunctionTrain::compute_sample_increment(
    const RealVector& regress_metrics, const SizetArray& N_l,
  ...
  if (collocRatio > 0.)
    new_N_l.scale(collocRatio);
  else
    new_N_l.scale(2.);  // default: over-sample 2x
...
void NonDMultilevelFunctionTrain::print_results(std::ostream& s,
                                                short results_state) {
