...
/** The VPSApproximation class provides a set of piecewise surrogate
approximations each of which is valid within a Voronoi cell.  */
...
class VPSApproximation : public Approximation {
 public:
  ...
  //////////////////////////////////////////////////////////////
  void retrieve_permutations(size_t& m, size_t**& perm, size_t num_dim,
                             size_t upper_bound, bool force_sum_constraint,
  ...
  int constrained_LeastSquare(size_t n, size_t m, double** H, double* w,
                              double* f);
  ...
  void isocontouring(std::string file_name, bool plot_test_function,
                     bool plot_surrogate, std::vector<double> contours);

  void isocontouring_solid(std::string file_name, bool plot_test_function,
                           bool plot_surrogate, std::vector<double> contours);
 ...
 private:
  ...
  double* _sample_vsize;  // furthest distance between seed and one of its
                          // Voronoi corners
  double*
      _vps_dfar;  // furthest distance between a seed and its extended neighbors
  ...
  std::vector<Approximation>
      gpApproximations;  // One approximation for each cell
