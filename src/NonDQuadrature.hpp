...
class NonDQuadrature : public NonDIntegration {
 ...
 protected:
  ...
  NonDQuadrature(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
...
inline void NonDQuadrature::quadrature_order(
    const Pecos::UShortArray& dim_quad_order) {
...
inline void NonDQuadrature::random_seed(int seed) { randomSeed = seed; }
...
inline void NonDQuadrature::increment_grid_preference(
    const RealVector& dim_pref)  // update dimension pref
{
  UShortArray ref_quad_order = tpqDriver->reference_quadrature_order();  // copy
...
}  // no update to dimension preference
...
inline void NonDQuadrature::evaluate_grid_increment() {
  ...
  tpqDriver->compute_grid(
      allSamples);  // Driver->compute_increment(allSamples);
...
inline short NonDQuadrature::mode() const { return quadMode; }
