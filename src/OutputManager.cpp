...
OutputManager::OutputManager()
    : graph2DFlag(false),
      ...
      tabularInterfLabel("interface"),
      outputLevel(NORMAL_OUTPUT) { /* empty ctor */ }

/// Only get minimal information off ProgramOptions as may be updated
/// later by broadcast
OutputManager::OutputManager(const ProgramOptions& prog_opts,
                             int dakota_world_rank, bool dakota_mpirun_flag)
    : graph2DFlag(false),
      tabularDataFlag(false),
      resultsOutputFlag(false),
      worldRank(dakota_world_rank),
      mpirunFlag(dakota_mpirun_flag),
      coutRedirector(dakota_cout, &std::cout),
      cerrRedirector(dakota_cerr, &std::cerr),
      graphicsCntr(1),
      tabularCntrLabel("eval_id"),
...
OutputManager::~OutputManager() { close_streams(); }
...
void OutputManager::check_inputfile_redirs(const std::string& input_filename,
  ...
  OutputManager::check_input_redirs_impl(infile, output_filename,
                                         error_filename);
}
...
void OutputManager::close_streams() {
  ...
  if (graph2DFlag || tabularDataFlag) {
    if (graph2DFlag) dakotaGraphics.close();
...
void OutputManager::parse(const ProgramOptions& prog_opts,
  ...
  modelEvalsSelection =
      problem_db.get_ushort("environment.model_evals_selection");
  interfEvalsSelection =
      problem_db.get_ushort("environment.interface_evals_selection");
  tabularFormat = problem_db.get_ushort("environment.tabular_format");
  resultsOutputFormat =
      problem_db.get_ushort("environment.results_output_format");
...
void OutputManager::push_output_tag(const String& iterator_tag,
...
void OutputManager::pop_output_tag() {
  ...
  if (outputLevel >= DEBUG_OUTPUT) {
    String file_tag = build_output_tag();
    std::cout << "\nDEBUG: Rank " << worldRank
              << " popping output tag; new tag '" << file_tag << "'"
              << std::endl;
  }
...
void OutputManager::output_version(std::ostream& os) const {
  ...
  if (worldRank == 0) {
    ...
    // subversion revision
    version_info += "Repository revision " + DakotaBuildInfo::get_rev_number() +
...
void OutputManager::output_helper(const String& message,
                                  std::ostream& os) const {
  if (worldRank == 0) os << message << std::endl;
}
...
/** Opens the tabular data file stream and prints headings, one for
    ...
    tabular data is used for post-processing of DAKOTA results in
    Matlab, Tecplot, etc. */
void OutputManager::create_tabular_header(const Variables& vars,
...
void OutputManager::create_tabular_header(const StringArray& iface_ids) {
  TabularIO::write_header_tabular(tabularDataFStream, tabularCntrLabel,
                                  iface_ids, tabularFormat);
}

void OutputManager::append_tabular_header(const Variables& vars) {
...
void OutputManager::append_tabular_header(const StringArray& labels, bool rtn) {
  TabularIO::append_header_tabular(tabularDataFStream, labels, tabularFormat);
  if (rtn) tabularDataFStream << std::endl;
}

void OutputManager::append_tabular_header(const Response& response) {
...
void OutputManager::add_tabular_data(const Variables& vars, const String& iface,
                                     const Response& response) {
...
void OutputManager::add_tabular_data(const Variables& vars, size_t start_index,
                                     size_t num_items) {
  // In the more finely-grained case, forego the check on ASV fns
  // --> always generate a row, even if no active response fns
...
void OutputManager::add_tabular_data(const Response& response,
  ...
  // whether the file is open, not whether the user asked
  if (tabularDataFStream.is_open())
    TabularIO::write_data_tabular(tabularDataFStream, response, start_index,
                                  num_items);
...
void OutputManager::graphics_counter(int cntr) { graphicsCntr = cntr; }

int OutputManager::graphics_counter() const { return graphicsCntr; }

void OutputManager::init_results_db() {
  String file_tag;
  if (mpirunFlag) file_tag = "." + std::to_string(worldRank + 1);
...
#else
    Cerr << "WARNING: HDF5 results output was requested, but is not available "
            "in this build.\n";
#endif
  }
}
...
void OutputManager::read_write_restart(bool restart_requested,
  ...
  if (read_restart_flag) {
    ...
    try {
      ...
      if (stop_restart_evals)  // cmd_line_handler rtns 0 if no setting
        Cout << "Stopping restart file processing at " << stop_restart_evals
      ...
      while (restart_input_fs.good() && !restart_input_fs.eof() &&
             (!stop_restart_evals || cntr < stop_restart_evals)) {
        ...
        // TODO: should it be the default to truncate and warn when bad?!?
          ...
          Cerr << "\nError reading restart file '" << read_restart_filename
               ...
               << ".\nDetails (boost::archive exception): " << e.what()
               << std::endl;
          abort_handler(IO_ERROR);
        }
  ...
  try {
    ...
    std::shared_ptr<RestartWriter> rst_writer(
        new RestartWriter(write_restart_filename));
    restartDestinations.push_back(rst_writer);
    ...
    if (!read_pairs.empty()) {
      ...
      for (it = read_pairs.begin(); it != it_end; ++it) {
        ...
        } else  // should not be negative (see rst append above), but can be
                // zero
...
const String& OutputWriter::filename() const { return outputFilename; }

std::ostream* OutputWriter::output_stream() { return outputStream; }

ConsoleRedirector::ConsoleRedirector(std::ostream*& dakota_stream,
                                     std::ostream* default_dest)
...
void ConsoleRedirector::push_back() {
  ...
  if (ostreamDestinations.empty()) {
    std::shared_ptr<OutputWriter> out_writer_ptr(
        new OutputWriter(defaultOStream));
...
void ConsoleRedirector::push_back(const String& output_filename) {
...
#endif
    std::shared_ptr<OutputWriter> out_writer_ptr(
        new OutputWriter(output_filename));
    ostreamDestinations.push_back(out_writer_ptr);
  }
  ...
  std::cerr << "ConsoleRedirector appending redirect to file "
            << output_filename << ". ostreamHandle now " << ostreamHandle
            << std::endl;
#endif
}
...
RestartWriter::RestartWriter() { /* empty ctor */ }
...
RestartWriter::RestartWriter(const String& write_restart_filename,
  ...
  if (!restartOutputFS.good()) {
    Cerr << "\nError: could not open restart file '" << write_restart_filename
         << "' for writing." << std::endl;
    abort_handler(IO_ERROR);
  }

  restartOutputArchive.reset(
      new boost::archive::binary_oarchive(restartOutputFS));
...
RestartWriter::RestartWriter(std::ostream& write_restart_ostream)
    : restartOutputArchive(
          new boost::archive::binary_oarchive(write_restart_ostream)) {
  RestartVersion rst_version(DakotaBuildInfo::get_release_num(),
                             DakotaBuildInfo::get_rev_number());
  restartOutputArchive->operator&(rst_version);
}

const String& RestartWriter::filename() { return restartOutputFilename; }
...
void RestartWriter::flush() { restartOutputFS.flush(); }
...
extern "C" void dak_sigcatch(int sig) {
  ...
  if (getrusage(RUSAGE_SELF, &ru)) std::memset(&ru, 0, sizeof(ru));
  if (getrusage(RUSAGE_SELF, &ruc)) std::memset(&ruc, 0, sizeof(ru));
