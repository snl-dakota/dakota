...
static const char rcsId[] =
    "@(#) $Id: DakotaMinimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
Minimizer::Minimizer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
                     std::shared_ptr<Model> model,
                     std::shared_ptr<TraitsBase> traits)
    : Iterator(problem_db, parallel_lib, traits),
      constraintTol(probDescDB.get_real("method.constraint_tolerance")),
      bigRealBoundSize(BIG_REAL_BOUND),
      bigIntBoundSize(1000000000),
  ...
  if (!numFinalSolutions && methodName != MOGA) numFinalSolutions = 1;
}

Minimizer::Minimizer(unsigned short method_name, std::shared_ptr<Model> model,
                     std::shared_ptr<TraitsBase> traits)
    : Iterator(method_name, model, traits),
      constraintTol(0.),
      bigRealBoundSize(1.e+30),
      bigIntBoundSize(1000000000),
      boundConstraintFlag(false),
...
Minimizer::Minimizer(std::shared_ptr<Model> model, size_t max_iter,
...
void Minimizer::update_from_model(const Model& model) {
  ...
  numContinuousVars = ModelUtils::cv(model);
  numDiscreteIntVars = ModelUtils::div(model);
  numDiscreteStringVars = ModelUtils::dsv(model);
  numDiscreteRealVars = ModelUtils::drv(model);
  ...
  if (numLinearEqConstraints &&
      (!traits()->supports_linear_equality() ||
  ...
  if (numLinearIneqConstraints &&
      (!traits()->supports_linear_inequality() ||
       (methodName == OPTPP_CG || methodName == OPTPP_PDS ||
        methodName == COLINY_SOLIS_WETS))) {
  ...
  if (numNonlinearEqConstraints &&
      (!traits()->supports_nonlinear_equality() ||
       (methodName == OPTPP_CG || methodName == OPTPP_PDS))) {
    Cerr
  ...
  if (numNonlinearIneqConstraints &&
      (!traits()->supports_nonlinear_inequality() ||
       (methodName == OPTPP_CG || methodName == OPTPP_PDS))) {
    Cerr << "\nError: nonlinear inequality constraints not currently supported "
            "by "
         << method_enum_to_string(methodName) << ".\n       Please select a "
         << "different method." << std::endl;
    err_flag = true;
  ...
  bool discrete_bounds =
      (methodName == MOGA || methodName == SOGA || methodName == COLINY_EA);
  if (discrete_bounds) {
...
void Minimizer::initialize_run() {
  ...
  if (iteratedModel) {
    ...
    // if (!subIteratorFlag)
    if (summaryOutputFlag) iteratedModel->set_evaluation_reference();
  }
...
std::shared_ptr<Model> Minimizer::original_model(
    unsigned short recasts_left) const {
  // Dive into the originally passed model (could keep a shallow copy of it)
  // Don't use a reference here as want a shallow copy, not the instance
  auto usermodel = iteratedModel;
...
Real Minimizer::objective(const RealVector& fn_vals, const BoolDeque& max_sense,
...
/** This "composite" objective is a more general case of the previous
    ...
    of COLIN lookups. */
Real Minimizer::objective(const RealVector& fn_vals, size_t num_fns,
    ...
    if (primary_wts.empty()) {
      for (size_t i = 0; i < num_fns; ++i)
        if (use_sense && max_sense[i])
          ...
          obj_fn += fn_vals[i];
      if (num_fns > 1) obj_fn /= (Real)num_fns;  // default weight = 1/n
    } else {
      for (size_t i = 0; i < num_fns; ++i)
        if (use_sense && max_sense[i])
...
void Minimizer::objective_gradient(const RealVector& fn_vals,
...
void Minimizer::objective_gradient(const RealVector& fn_vals, size_t num_fns,
                                   const RealMatrix& fn_grads,
                                   const BoolDeque& max_sense,
                                   const RealVector& primary_wts,
                                   RealVector& obj_grad) const {
...
void Minimizer::objective_hessian(const RealVector& fn_vals,
                                  const RealMatrix& fn_grads,
...
/** The composite objective Hessian computation combines the
    ...
    available when needed, based on nonlinearRespMapping settings. */
void Minimizer::objective_hessian(const RealVector& fn_vals, size_t num_fns,
                                  const RealMatrix& fn_grads,
                                  const RealSymMatrixArray& fn_hessians,
                                  const BoolDeque& max_sense,
                                  const RealVector& primary_wts,
      ...
      for (i = 0; i < num_fns; ++i) {
        ...
        if (use_sense && max_sense[i])
          for (j = 0; j < numContinuousVars; ++j)
            for (k = 0; k <= j; ++k) obj_hess(j, k) -= fn_hess_i(j, k);
        else
          for (j = 0; j < numContinuousVars; ++j)
            for (k = 0; k <= j; ++k) obj_hess(j, k) += fn_hess_i(j, k);
      }
      if (num_fns > 1) obj_hess *= 1. / (Real)num_fns;  // default weight = 1/n
    } else
      for (i = 0; i < num_fns; ++i) {
        const RealSymMatrix& fn_hess_i = fn_hessians[i];
        const Real& wt_i = primary_wts[i];
        if (use_sense && max_sense[i])
          for (j = 0; j < numContinuousVars; ++j)
            for (k = 0; k <= j; ++k) obj_hess(j, k) -= wt_i * fn_hess_i(j, k);
        else
          for (j = 0; j < numContinuousVars; ++j)
            for (k = 0; k <= j; ++k) obj_hess(j, k) += wt_i * fn_hess_i(j, k);
      }
...
void Minimizer::print_best_eval_ids(const String& search_interface_id,
  ...
  const String
      ...
      best_id_restart =
          "<<<<< Best evaluation ID not found among current execution's "
      ...
      id_warning =
          "(This warning may occur when the best iterate is comprised of "
...
void Minimizer::archive_best_variables(const bool active_only) const {
  ...
  const auto& cv_labels =
      (active_only) ? variables_results().continuous_variable_labels()
                    : variables_results().all_continuous_variable_labels();
  const auto& div_labels =
      (active_only) ? variables_results().discrete_int_variable_labels()
                    : variables_results().all_discrete_int_variable_labels();
  const auto& dsv_labels =
      (active_only) ? variables_results().discrete_string_variable_labels()
                    : variables_results().all_discrete_string_variable_labels();
  const auto& drv_labels =
      (active_only) ? variables_results().discrete_real_variable_labels()
                    : variables_results().all_discrete_real_variable_labels();
  // ##  legacy text output ##
  if (numContinuousVars) {
    // labels
    resultsDB.insert(iterator_id, resultsNames.cv_labels, cv_labels);
    // best variables, with labels in metadata
    MetaDataType md;
    md["Array Spans"] = make_metadatavalue("Best Sets");
    md["Row Labels"] = make_metadatavalue(cv_labels);
    resultsDB.array_allocate<RealVector>(iterator_id, resultsNames.best_cv,
                                         num_points, md);
  }
  if (numDiscreteIntVars) {
    // labels
    resultsDB.insert(iterator_id, resultsNames.div_labels, div_labels);
    // best variables, with labels in metadata
    MetaDataType md;
    md["Array Spans"] = make_metadatavalue("Best Sets");
    md["Row Labels"] = make_metadatavalue(div_labels);
    resultsDB.array_allocate<IntVector>(iterator_id, resultsNames.best_div,
                                        num_points, md);
  }
  if (numDiscreteStringVars) {
    // labels
    resultsDB.insert(iterator_id, resultsNames.dsv_labels, dsv_labels);
    // best variables, with labels in metadata
    MetaDataType md;
    md["Array Spans"] = make_metadatavalue("Best Sets");
    md["Row Labels"] = make_metadatavalue(dsv_labels);
    resultsDB.array_allocate<StringArray>(iterator_id, resultsNames.best_dsv,
                                          num_points, md);
  }
  if (numDiscreteRealVars) {
    // labels
    resultsDB.insert(iterator_id, resultsNames.drv_labels, drv_labels);
    // best variables, with labels in metadata
    MetaDataType md;
    md["Array Spans"] = make_metadatavalue("Best Sets");
    md["Row Labels"] = make_metadatavalue(drv_labels);
    resultsDB.array_allocate<RealVector>(iterator_id, resultsNames.best_drv,
                                         num_points, md);
  }
  ...
  for (const auto& best_vars : bestVariablesArray) {
    ...
    if (numContinuousVars) {
      ...
      resultsDB.array_insert<RealVector>(run_identifier(), resultsNames.best_cv,
                                         point_index,
      ...
      if (active_only)
        resultsDB.insert(iterator_id, location,
                         best_vars.continuous_variables(), cv_scale);
      else
        resultsDB.insert(iterator_id, location,
                         best_vars.all_continuous_variables(), cv_scale);
    }

    if (numDiscreteIntVars) {
      // coreDB backend which will likely be removed in the future - RWH
      resultsDB.array_insert<IntVector>(run_identifier(), resultsNames.best_div,
                                        point_index,
      ...
      if (active_only)
        resultsDB.insert(iterator_id, location,
                         best_vars.discrete_int_variables(), div_scale);
      else
        resultsDB.insert(iterator_id, location,
                         best_vars.all_discrete_int_variables(), div_scale);
    }
    ...
    if (numDiscreteStringVars) {
      ...
      if (active_only)
        resultsDB.insert(iterator_id, location,
                         best_vars.discrete_string_variables(), dsv_scale);
      else
        resultsDB.insert(iterator_id, location,
                         best_vars.all_discrete_string_variables(), dsv_scale);
    }

    if (numDiscreteRealVars) {
      // coreDB backend which will likely be removed in the future - RWH
      resultsDB.array_insert<RealVector>(run_identifier(),
                                         resultsNames.best_drv, point_index,
      ...
      if (active_only)
        resultsDB.insert(iterator_id, location,
                         best_vars.discrete_real_variables(), drv_scale);
      else
        resultsDB.insert(iterator_id, location,
                         best_vars.all_discrete_real_variables(), drv_scale);
    }
    point_index++;
  }
}

void Minimizer::archive_best_objective_functions() const {
  ...
  MetaDataType md;
  md["Array Spans"] = make_metadatavalue("Best Sets");
  md["Row Labels"] = make_metadatavalue(response_results().function_labels());
  resultsDB.array_allocate<RealVector>(iterator_id, resultsNames.best_fns,
                                       num_points, md);

  size_t point_index = 0;
  StringArray location;
  if (num_points > 1) location.push_back("");
  location.push_back("best_objective_functions");
  DimScaleMap scale;
  scale.emplace(0,
                StringScale("responses", response_results().function_labels()));
  for (const auto& best_resp : bestResponseArray) {
    if (num_points > 1)
      location[0] = String("set:") + std::to_string(point_index + 1);
    // coreDB-based API Results output
    resultsDB.array_insert<RealVector>(iterator_id, resultsNames.best_fns,
                                       point_index,
                                       best_resp.function_values());

    // hdf5DB-based API Results output
    const RealVector& fvals = best_resp.function_values();
    Teuchos::SerialDenseVector<int, Real> primary(
        Teuchos::View, const_cast<Real*>(&fvals[0]), numUserPrimaryFns);
...
void Minimizer::archive_best_residuals() const {
  if (!resultsDB.active()) return;
  ...
  for (const auto& resp : bestResponseArray) {
    ...
    Real wssr = std::sqrt(
        sum_squared_residuals(numUserPrimaryFns, best_terms, lsq_weights));
    Teuchos::SerialDenseVector<int, Real> residuals(
        Teuchos::View, const_cast<Real*>(&best_terms[0]), numUserPrimaryFns);
    resultsDB.insert(iterator_id, residuals_location, residuals);
    resultsDB.insert(iterator_id, norm_location, wssr);
    // coreDB
    resultsDB.array_insert<RealVector>(iterator_id, resultsNames.best_fns,
                                       point_index, resp.function_values());
    point_index++;
  }
}

void Minimizer::archive_best_constraints() const {
  ...
  for (const auto& best_resp : bestResponseArray) {
    if (num_points > 1)
      location[0] = String("set:") + std::to_string(point_index + 1);
    const RealVector& fvals = best_resp.function_values();
    Teuchos::SerialDenseVector<int, Real> secondary(
        Teuchos::View, const_cast<Real*>(&fvals[numUserPrimaryFns]),
        numNonlinearConstraints);
...
Real Minimizer::sum_squared_residuals(size_t num_pri_fns,
...
void Minimizer::print_residuals(size_t num_terms, const RealVector& best_terms,
  ...
  if (num_best > 1) s << "(set " << best_index + 1 << ") ";
  s << "= " << std::setw(write_precision + 7) << std::sqrt(wssr)
    << "; 0.5 * norm^2 = " << std::setw(write_precision + 7) << 0.5 * wssr
    << '\n';
}
...
void Minimizer::archive_best_results() {
  ...
  // use asv = 1's
  ActiveSet search_set(ModelUtils::response_size(*orig_model),
                       numContinuousVars);
  int eval_id;

  if (numNonlinearConstraints) archive_best_constraints();
  if (optimizationFlag) {
    ...
    for (i = 0; i < num_best; ++i) {
      ...
      if (!optimizationFlag && calibrationDataFlag) {
        dt_model_rep->archive_best_responses(resultsDB, iterator_id, best_vars,
                                             bestResponseArray[i], num_best, i);
      }
    }
  }
  ...
  for (i = 0; i < num_best; ++i) {
    ...
    const Variables& best_vars = bestVariablesArray[i];
    PRPCacheHIter cache_it =
        lookup_by_val(data_pairs, interface_id, best_vars, search_set);
    if (cache_it == data_pairs.get<hashed>().end())
      eval_id = 0;
    else
...
/** Retrieve a MOO/NLS response based on the data returned by a single
    ...
    inbound Model. */
bool Minimizer::local_recast_retrieve(const Variables& vars,
  ...
  PRPCacheHIter cache_it = lookup_by_val(
      data_pairs, iteratedModel->interface_id(), vars, lookup_set);
  if (cache_it == data_pairs.get<hashed>().end()) {
