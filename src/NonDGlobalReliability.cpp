...
static const char rcsId[] =
    "@(#) $Id: NonDGlobalReliability.cpp 4058 2006-10-25 01:39:40Z mseldre $";
...
NonDGlobalReliability::NonDGlobalReliability(ProblemDescDB& problem_db,
                                             ParallelLibrary& parallel_lib,
                                             std::shared_ptr<Model> model)
    : NonDReliability(problem_db, parallel_lib, model),
      meritFunctionType(AUGMENTED_LAGRANGIAN_MERIT),
      dataOrder(1) {
  ...
  if (mppSearchType ==
      SUBMETHOD_EGRA_X) {  // Recast( DataFit( iteratedModel ) )

    // The following uses on the fly derived ctor:
    dace_iterator = std::make_shared<NonDLHSSampling>(
        iteratedModel, sample_type, samples, lhs_seed, rng, vary_pattern,
        ACTIVE_UNIFORM);
    ...
    g_hat_x_model = std::make_shared<DataFitSurrModel>(
        dace_iterator, iteratedModel, gp_set, gp_view, approx_type,
        approx_order, corr_type, corr_order, dataOrder, outputLevel,
...
#else
      Cerr << "\nError: global_reliability does not support "
    ...
    dace_iterator = std::make_shared<NonDLHSSampling>(
        g_u_model, sample_type, samples, lhs_seed, rng, vary_pattern,
        ACTIVE_UNIFORM);
    // unsigned short dace_method = SUBMETHOD_LHS; // submethod enum
    ...
    uSpaceModel = std::make_shared<DataFitSurrModel>(
        dace_iterator, g_u_model, gp_set, gp_view, approx_type, approx_order,
  ...
  mppModel = std::make_shared<RecastModel>(uSpaceModel, recast_vars_comps_total,
                                           all_relax_di, all_relax_dr, mpp_view,
                                           1, 0, 0, recast_resp_order);

  // For formulations with one objective and one equality constraint,
  // use the following instead:
  // mppModel->assign_rep(new RecastModel(uSpaceModel, ..., 1, 1, 0, ...),
  // false); RealVector nln_eq_targets(1, 0.);
...
#ifdef HAVE_NCSU
  mppOptimizer = std::make_shared<NCSUOptimizer>(mppModel, max_iter, max_eval,
  ...
  importanceSampler = std::make_shared<NonDAdaptImpSampling>(
      uSpaceModel, sample_type, refine_samples, refine_seed, rng, vary_pattern,
...
void NonDGlobalReliability::derived_init_communicators(ParLevLIter pl_iter) {
  ...
  // uSpaceModel->  An additional uSpaceModel->init_communicators() call would
...
void NonDGlobalReliability::optimize_gaussian_process() {
  if (mppSearchType == SUBMETHOD_EGRA_X) {
    ...
    uSpaceModel->trans_U_to_X(ModelUtils::continuous_lower_bounds(*uSpaceModel),
                              x_l_bnds);
    uSpaceModel->trans_U_to_X(ModelUtils::continuous_upper_bounds(*uSpaceModel),
                              x_u_bnds);
  ...
  for (respFnCount = 0; respFnCount < numFunctions; respFnCount++) {
      ...
      while (!approxConverged) {
        ...
        if (ria_flag) {
          ...
          mpp_model_rep->init_maps(
              ...
              nonlinear_resp_map, EFF_objective_eval, NULL);
        } else {
          ...
          mpp_model_rep->init_maps(
              vars_map, false, NULL, NULL, primary_resp_map, secondary_resp_map,
              nonlinear_resp_map, EIF_objective_eval, NULL);
        }
        ...
        Real beta_star = 0.,
             /* TO DO */ exp_fns_star =
                 (ria_flag) ? expected_feasibility(g_hat_fns, vars_star)
        ...
        if (pma_aug_lag_flag) {
          ...
          if (c_violation < lastConstraintViolation) lastIterateAccepted = true;
    ...
    if (num_levels) {  // can only plot if GP was built!
      ...
      size_t num_data_pts = gp_data.size(),
             num_vars = ModelUtils::cv(*uSpaceModel);
      ...
      if (num_vars == 2) {
        ...
        for (size_t i = 0; i < 101; i++) {
          ...
          for (size_t j = 0; j < 101; j++) {
            ...
            gpOut << '\n'
                  << std::setw(13) << u_pt[0] << ' ' << std::setw(13) << u_pt[1]
                  << ' ' << std::setw(13) << gp_fns[respFnCount];
...
void NonDGlobalReliability::importance_sampling() {
  ...
  for (respFnCount = 0; respFnCount < numFunctions; respFnCount++) {
    ...
    if (num_levels == 0) {
      ...
      finalStatistics.function_value(true_resp.function_value(respFnCount),
                                     statCount);
    } else {
    ...
    for (levelCount = 0; levelCount < num_levels; levelCount++) {
      ...
      if (ria_flag) {
        ...
        importance_sampler_rep->initialize(gp_inputs, x_data_flag, respFnCount,
                                           0., z);
      } else  // not operational (see error traps in ctor)
        importance_sampler_rep->initialize(
            gp_inputs, x_data_flag, respFnCount, 0.,
      ...
      switch (respLevelTarget) {
        ...
        case GEN_RELIABILITIES:
          finalStatistics.function_value(gen_beta, statCount++);
          break;
      }
    }
  }
...
void NonDGlobalReliability::EIF_objective_eval(
...
void NonDGlobalReliability::EFF_objective_eval(
    const Variables& sub_model_vars, const Variables& recast_vars,
    const Response& sub_model_response, Response& recast_response) {
  const ShortArray& recast_asv = recast_response.active_set_request_vector();
  if (recast_asv[0] & 1) {
...
Real NonDGlobalReliability::expected_improvement(
    const RealVector& expected_values, const Variables& recast_vars) {
...
Real NonDGlobalReliability::expected_feasibility(
    const RealVector& expected_values, const Variables& recast_vars) {
  // Get variance from the GP; Expected values are passed in
  // If GP built in x-space, transform input point to x-space to get variance
  RealVector variances;
  ...
  Real mean = expected_values[respFnCount],
       stdv = std::sqrt(variances[respFnCount]), zbar = requestedTargetLevel,
  ...
  if (std::fabs(snvz) >= std::fabs(stdv) * 50.) {
    ...
    snvz /= stdv;
    Real snvp = snvz + alpha, snvm = snvz - alpha;
...
void NonDGlobalReliability::get_best_sample() {
  ...
  RealVectorArray true_c_vars_u(num_samples);
  RealVector true_vars_x_cv;
  for (i = 0; i < num_samples; i++) {
    true_vars_x_cv = Teuchos::getCol(
        Teuchos::View, const_cast<RealMatrix&>(true_vars_x), (int)i);
    if (mppSearchType == SUBMETHOD_EGRA_X)
  ...
  fnStar = (pmaMaximizeG) ? -DBL_MAX : DBL_MAX;
  IntRespMCIter it;
...
Real NonDGlobalReliability::constraint_penalty(const Real& c_viol,
                                               const RealVector& u) {
  if (meritFunctionType == PENALTY_MERIT)
    ...
    for (size_t i = 0; i < m; i++) A[i] = 2. * u[i];
