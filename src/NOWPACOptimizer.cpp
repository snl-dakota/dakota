...
static const char rcsId[] =
    "@(#) $Id: NOWPACOptimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";

namespace Dakota {

NOWPACOptimizer::NOWPACOptimizer(ProblemDescDB& problem_db,
...
void NOWPACOptimizer::initialize_options() {
  ...
  nowpacSolver.set_option(
      "eta_1", probDescDB.get_real("method.trust_region.contract_threshold"));
  nowpacSolver.set_option(
      "eta_2", probDescDB.get_real("method.trust_region.expand_threshold"));
  ...
  nowpacSolver.set_option(
      "gamma_inc", probDescDB.get_real("method.trust_region.expansion_factor"));
  nowpacSolver.set_option(
      "gamma", probDescDB.get_real("method.trust_region.contraction_factor"));
  // Reduction factors:
  ...
  nowpacSolver.set_option("max_nb_accepted_steps",
                          (int)maxIterations);  // inf default
  ...
  if (tr_factor < min_factor)
    tr_factor = min_factor;
  else if (tr_factor > 1.)
    tr_factor = 1.;
  ...
  size_t num_v = ModelUtils::cv(*iteratedModel);
  l_bnds.assign(num_v, -1.);
  u_bnds.assign(num_v, 1.);
...
void NOWPACOptimizer::core_run() {
  ...
  nowpacEvaluator.set_unscaled_bounds(
  ...
  RealArray x_star;
  RealArray obj_star;
  nowpacEvaluator.scale(ModelUtils::continuous_variables(*iteratedModel),
                        x_star);
  ...
  if (!localObjectiveRecast) {
    ...
    if (iteratedModel) {
      ...
      best_fns[0] =
          (!max_sense.empty() && max_sense[0]) ? -obj_star[0] : obj_star[0];
    } else {
  ...
  StLCIter i_iter;
  RLCIter m_iter, o_iter;
  size_t cntr = 0;  // numEqConstraints;
  for (i_iter = nln_ineq_map_indices.begin(),
      m_iter = nln_ineq_map_mult.begin(), o_iter = nln_ineq_map_offsets.begin();
       i_iter != nln_ineq_map_indices.end(); ++i_iter, ++m_iter, ++o_iter)
    best_fns[(*i_iter) + numUserPrimaryFns] =
        (obj_star[++cntr] - (*o_iter)) / (*m_iter);
  /*
    size_t i, offset = ModelUtils::num_nonlinear_ineq_constraints(iteratedModel)
    + 1, num_nln_eq = ModelUtils::num_nonlinear_eq_constraints(iteratedModel);
...
void NOWPACBlackBoxEvaluator::allocate_constraints() {
  ...
  if (constraint_err) abort_handler(METHOD_ERROR);
...
void NOWPACBlackBoxEvaluator::evaluate(RealArray const& x, RealArray& vals,
                                       void* param) {
  // NOWPACOptimizer enforces an embedded scaling: incoming x is scaled on
  // [-1,1]
  ...
  for (i_iter = linIneqConMappingIndices.begin(),
    ...
    size_t index = *i_iter;
    Real Ax = 0.;
    for (j = 0; j < num_cv; ++j) Ax += lin_ineq_coeffs(index, j) * x[j];
...
void NOWPACBlackBoxEvaluator::evaluate(RealArray const& x, RealArray& vals,
  ...
  noise[cntr] =
      2. * errors[0];  // for now; TO DO: mapping of noise for MOO/NLS...
  ++cntr;
...
double NOWPACBlackBoxEvaluator::evaluate_samples(
    std::vector<double> const& samples, const unsigned int index,
