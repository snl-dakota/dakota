...
SubspaceModel::SubspaceModel(ProblemDescDB& problem_db,
                             ParallelLibrary& parallel_lib,
                             std::shared_ptr<Model> sub_model)
    : RecastModel(problem_db, parallel_lib, sub_model),
      randomSeed(24620),
      ...
      offlineEvalConcurrency(1),
      onlineEvalConcurrency(1) {
  componentParallelMode = CONFIG_PHASE;
}

SubspaceModel::SubspaceModel(std::shared_ptr<Model> sub_model,
                             unsigned int dimension, short output_level)
    : RecastModel(sub_model),
      numFullspaceVars(ModelUtils::cv(*sub_model)),
...
void SubspaceModel::validate_inputs() {
  ...
  if (ModelUtils::div(*subModel) > 0 || ModelUtils::dsv(*subModel) > 0 ||
      ModelUtils::drv(*subModel) > 0) {
    error_flag = true;
...
void SubspaceModel::initialize_base_recast(
  ...
  size_t submodel_dv = ModelUtils::div(*subModel) + ModelUtils::dsv(*subModel) +
                       ModelUtils::drv(*subModel);
  ...
  for (size_t i = 0; i < submodel_cv; ++i) {
    ...
    for (size_t j = 0; j < recast_cv; ++j) vars_map_indices[i][j] = j;
  }
  ...
  size_t num_primary = subModel->num_primary_fns(),
         ...
         recast_secondary_offset =
             ModelUtils::num_nonlinear_ineq_constraints(*subModel);
  ...
  bool copy_values;
  RecastModel::init_sizes(subModel->current_variables().view(),
                          vars_comps_total, all_relax_di, all_relax_dr,
  ...
  if (reducedRank != ModelUtils::cv(*subModel)) {
    ...
    switch (active_view) {
      ...
      case RELAXED_UNCERTAIN:
        ...
        if (vc_totals[TOTAL_CAUV])
          vc_totals[TOTAL_CAUV] = reducedRank;
        else if (vc_totals[TOTAL_CEUV])
          vc_totals[TOTAL_CEUV] = reducedRank;
        break;
      case MIXED_ALL:
      case RELAXED_ALL:
        // assumption: only one vars type is defined in original space
        // (unclear how to partition dimension reduction among types)
        if (vc_totals[TOTAL_CDV])
          vc_totals[TOTAL_CDV] = reducedRank;
        else if (vc_totals[TOTAL_CAUV])
          vc_totals[TOTAL_CAUV] = reducedRank;
        else if (vc_totals[TOTAL_CEUV])
          vc_totals[TOTAL_CEUV] = reducedRank;
        else if (vc_totals[TOTAL_CSV])
          vc_totals[TOTAL_CSV] = reducedRank;
        break;
      default:
...
void SubspaceModel::uncertain_vars_to_subspace() {
  ...
  for (i = start_reduction; i < end_reduction;
       ++i) {  // reduced space of NORMAL vars
  ...
  // for (i=0; i<num_dauv; ++i)                   // include dauv for active
  // corr
  ...
  for (i = 0; i < start_reduction; ++i)  // same as native
    reduced_dist_rep->random_variable(i) = native_dist_rep->random_variable(i);
  for (i = end_reduction; i < num_reduced_rv; ++i)  // same as native
...
void SubspaceModel::component_parallel_mode(short mode) {
  ...
  // they are inactive
  if (componentParallelMode != mode && componentParallelMode != CONFIG_PHASE) {
...
void SubspaceModel::serve_run(ParLevLIter pl_iter, int max_eval_concurrency) {
  do {
...
int SubspaceModel::serve_init_mapping(ParLevLIter pl_iter) {
  ...
  do {
    ...
    switch (mapping_code) {
      ...
      case SERVE_RUN:
        parallelLib.bcast(max_eval_concurrency, *pl_iter);
        if (max_eval_concurrency) serve_run(pl_iter, max_eval_concurrency);
        break;
...
void SubspaceModel::set_mapping(const Variables& reduced_vars,
                                const ActiveSet& reduced_set,
                                ActiveSet& full_set) {
  ...
  for (size_t i = 0; i < reduced_dvv.size(); ++i)
    if (1 <= reduced_dvv[i] && reduced_dvv[i] <= reduced_cv) {
      for (size_t j = 1; j <= max_sm_id; ++j) full_dvv.push_back(j);
      break;
    }
...
*/
void SubspaceModel::response_mapping(const Variables& reduced_vars,
                                     const Variables& full_vars,
                                     const Response& full_resp,
                                     Response& reduced_resp) {
  ...
  if (!dg_dx.empty()) {
    ...
    teuchos_blas.GEMM(Teuchos::TRANS, Teuchos::NO_TRANS, m, n, k, alpha,
                      W1.values(), k, dg_dx.values(), k, beta, dg_dy.values(),
                      m);
  ...
  if (!H_x_all.empty()) {
    ...
    for (int i = 0; i < H_x_all.size(); i++) {
      ...
      Teuchos::symMatTripleProduct<int, Real>(Teuchos::TRANS, alpha, H_x_all[i],
                                              W1, H_y);
      H_y_all[i] = H_y;
    }
