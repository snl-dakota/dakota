...
class LDDriverAdapter : public SamplerDriver {
 ...
 public:
  ...
  void generate_uniform_samples(const RealVector& lowerBounds,
    ...
    Cerr << "\nError: 'generate_uniform_samples' currently not supported for "
  ...
  void generate_normal_samples(const RealVector& means,
    ...
    //         samplesArray(row, col) = means[row] + standardDeviations[row] *
    //         samplesArray(row, col);
    //     }
    // }
    Cerr << "\nError: 'generate_normal_samples' currently not supported for "
            "low-discrepancy sampling"
         << std::endl;
    abort_handler(METHOD_ERROR);
  }
  ...
  void rng(String rngName) override {
    if (rngName != "mt19937") {
      Cerr << "\nError: low-discrepancy sampling only supports 'mt19937' as "
              "RNG algorithm"
           << std::endl;
      abort_handler(METHOD_ERROR);
    }
  ...
  void advance_seed_sequence() override {
    Cerr << "\nError: low-discrepancy sampling does not support 'rnum2' as RNG "
            "algorithm"
         << std::endl;
    abort_handler(METHOD_ERROR);
  }
