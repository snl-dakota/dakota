...
DataTransformModel::DataTransformModel(std::shared_ptr<Model> sub_model,
                                       ExperimentData& exp_data,
                                       const ShortShortPair& recast_vars_view,
                                       size_t num_hyper,
                                       unsigned short mult_mode,
                                       short recast_resp_deriv_order)
    :  // BMA TODO: should the BitArrays be empty or same as submodel?
       // recast_secondary_offset is the index to the equality constraints
       // within the secondary responses
  ...
  if (num_config_vars &&
    ...
    int num_state_vars =
        ModelUtils::icv(*subModel) + ModelUtils::idiv(*subModel) +
        ModelUtils::idsv(*subModel) + ModelUtils::idrv(*subModel);
  ...
  size_t submodel_dv = ModelUtils::div(*sub_model) +
                       ModelUtils::dsv(*sub_model) +
                       ModelUtils::drv(*sub_model);
  ...
  RecastModel::init_maps(vars_map_indices, nonlinear_vars_mapping,
                         variables_map, set_map, primary_resp_map_indices,
                         secondary_resp_map_indices, nonlinear_resp_mapping,
...
DataTransformModel::~DataTransformModel() { /* empty dtor */ }

void DataTransformModel::data_transform_response(
    const Variables& sub_model_vars, const Response& sub_model_resp,
  ...
  transform_response(recast_vars, sub_model_vars, sub_model_resp,
                     residual_resp);
}
...
void DataTransformModel::update_from_subordinate_model(size_t depth) {
  ...
  if (depth == SZ_MAX)
    subModel->update_from_subordinate_model(
        depth);  // retain special value (inf)
  else if (depth)
...
void DataTransformModel::update_cv_skip_hyperparams(const Model& model) {
  ...
  size_t i,  // indexes the model's all continuous variables
      ...
      cv_end = cv_begin + num_cv, num_acv = sm_vars.acv();
...
void DataTransformModel::update_expanded_response(const Model& model) {
  ...
  expand_primary_array(model.primary_response_fn_weights().length(),
                       model.primary_response_fn_weights(), num_recast_primary,
                       primaryRespFnWts);
  ...
  expand_primary_array(model.primary_response_fn_sense().size(),
                       model.primary_response_fn_sense(), num_recast_primary,
                       primaryRespFnSense);
  ...
  expand_primary_array(model.scaling_options().priScales.length(),
                       model.scaling_options().priScales, num_recast_primary,
                       scalingOpts.priScales);
...
SizetArray DataTransformModel::variables_expand(const Model& sub_model,
                                                size_t num_hyper) {
...
void DataTransformModel::gen_primary_resp_map(
    const SharedResponseData& srd, Sizet2DArray& primary_resp_map_indices,
  ...
  for (size_t exp_ind = 0; exp_ind < num_experiments; ++exp_ind) {
    ...
    for (size_t scalar_ind = 0; scalar_ind < num_scalar; ++scalar_ind) {
      ...
      primary_resp_map_indices[calib_term_ind][0] =
          scalar_ind;  //=calib_term_ind
...
void DataTransformModel::transform_inactive_variables(
    const Variables& exp_config_vars, Variables& sub_model_vars) {
  ...
  if (expData.num_config_vars() == 0)
    ...
    for (size_t i = 0; i < num_exp; ++i) {
      ...
      if (subModel->asynch_flag()) {
        ...
        expData.form_residuals(subModel->current_response(), i,
                               currentResponse);
      }
    }
...
void DataTransformModel::set_mapping(const Variables& recast_vars,
  ...
  size_t max_sm_id =
      dtModelInstance->subordinate_model()->current_variables().cv();
  ...
  if (dtModelInstance->numHyperparams > 0) {
    ...
    for (size_t i = 0; i < sub_model_asv.size(); ++i) {
      if (sub_model_asv[i] & 4) sub_model_asv[i] |= 2;
      if (sub_model_asv[i] & 2) sub_model_asv[i] |= 1;
    }
    sub_model_set.request_vector(sub_model_asv);
  }
}

void DataTransformModel::primary_resp_differencer(
    const Variables& submodel_vars, const Variables& recast_vars,
    const Response& submodel_response, Response& recast_response) {
...
void DataTransformModel::cache_submodel_responses(
    const IntResponseMap& sm_resp_map, bool deep_copy) {
  // Not using BOOST_FOREACH due to potential for iterator invalidation in
  // erase in cache_unmatched_response (shouldn't be a concern with map?)
  IntRespMCIter sm_r_it = sm_resp_map.begin(), sm_r_end = sm_resp_map.end();
  while (sm_r_it != sm_r_end) {
    ...
    if (id_it != recastIdMap.end()) {
      ...
      if (cachedResp.find(recast_id) == cachedResp.end()) {
        ...
        cachedResp[recast_id][sm_id] =
            deep_copy ? sm_r_it->second.copy() : sm_r_it->second;
      } else
        cachedResp[recast_id][sm_id] =
            deep_copy ? sm_r_it->second.copy() : sm_r_it->second;
      recastIdMap.erase(sm_id);
...
/** This transformation assumes the residuals are in submodel eval_id
    order. */
void DataTransformModel::transform_response_map(
    const IntResponseMap& submodel_resp, const Variables& recast_vars,
    Response& residual_resp) {
  size_t num_exp = expData.num_experiments();
...
void DataTransformModel::scale_response(const Variables& submodel_vars,
                                        const Variables& recast_vars,
                                        Response& recast_response) {
  // scale by (error covariance)^{-1/2}
  if (expData.variance_active()) expData.scale_residuals(recast_response);
  ...
  if (obsErrorMultiplierMode > CALIBRATE_NONE) {
    ...
    expData.scale_residuals(hyper_params, obsErrorMultiplierMode,
                            num_calib_params, recast_response);
  }
}
...
void DataTransformModel::init_continuous_vars() {
  ...
  StringMultiArrayConstView sm_acvl =
      ModelUtils::all_continuous_variable_labels(*subModel);
  const RealVector& sm_aclb =
      ModelUtils::all_continuous_lower_bounds(*subModel);
  const RealVector& sm_acub =
      ModelUtils::all_continuous_upper_bounds(*subModel);
  ...
  for (const int& vci : continuous_vc_inds) {
    ...
    for (size_t i = 0; i < num_cvars; ++i) {
      current_variables().all_continuous_variable(sm_acv[sm_offset],
                                                  dtm_offset);
      current_variables().all_continuous_variable_label(sm_acvl[sm_offset],
                                                        dtm_offset);
      ModelUtils::all_continuous_lower_bound(*this, sm_aclb[sm_offset],
                                             dtm_offset);
      ModelUtils::all_continuous_upper_bound(*this, sm_acub[sm_offset],
                                             dtm_offset);
      ++sm_offset;
      ++dtm_offset;
    }
    if (vci == hyperparam_vc_ind) {
      ...
      for (size_t i = 0; i < numHyperparams; ++i) {
        ...
        current_variables().all_continuous_variable_label(hyper_labels[i],
                                                          dtm_offset);
        ModelUtils::all_continuous_lower_bound(*this, 0.0, dtm_offset);
        ModelUtils::all_continuous_upper_bound(
            *this, std::numeric_limits<double>::infinity(), dtm_offset);
        ++dtm_offset;
      }
    }
  }
...
template <typename T>
void DataTransformModel::expand_primary_array(size_t submodel_size,
                                              const T& submodel_array,
                                              size_t recast_size,
                                              T& recast_array) const {
  // Assume that coefficients have been expanded for fields (weights, scales,
  // sense) So each is either len1 or num_elements
  if (submodel_size == 1)
...
void DataTransformModel::print_best_responses(
    std::ostream& s, const Variables& best_submodel_vars,
    const Response& best_submodel_resp, size_t num_best, size_t best_ind) {
...
void DataTransformModel::recover_submodel_responses(
    std::ostream& s, const Variables& best_submodel_vars, size_t num_best,
    size_t best_ind, Response& residual_resp) {
  ...
  for (size_t i = 0; i < num_exp; ++i) {
    ...
    // BMA: why is this necessary?  Should have a reference to same object as
    // PRP
...
void DataTransformModel::archive_best_residuals(
    const ResultsManager& results_db, const StrStrSizet& iterator_id,
    const int num_terms, const RealVector& best_terms, const Real wssr,
    const int num_points, const int point_index) {
  ...
  Teuchos::SerialDenseVector<int, Real> residuals(
      Teuchos::View, const_cast<Real*>(&best_terms[0]), num_terms);
  results_db.insert(iterator_id, residuals_location, residuals);
  results_db.insert(iterator_id, norm_location, wssr);
}

void DataTransformModel::archive_best_original(
    const ResultsManager& results_db, const StrStrSizet& iterator_id,
    const RealVector& function_values, const int& exp_index,
    const int& num_best, const int& best_index) {
  if (!results_db.active()) return;

  DimScaleMap scales;
  scales.emplace(
      0, StringScale("responses", ModelUtils::response_labels(*subModel),
                     ScaleScope::SHARED));
...
void DataTransformModel::archive_submodel_responses(
    const ResultsManager& results_db, const StrStrSizet& iterator_id,
    const Variables& best_submodel_vars, size_t num_best, size_t best_ind,
    Response& residual_resp) {
  // first try cache lookup
  ...
  for (size_t i = 0; i < num_exp; ++i) {
    ...
    if (!lookup_failure) {
      ...
      archive_best_original(results_db, iterator_id,
                            model_resp.function_values(), i, num_best,
                            best_ind);
      if (expData.num_config_vars() != 0)
        archive_best_config_variables(results_db, iterator_id, lookup_vars, i,
                                      num_best, best_ind);
    }
  }
...
void DataTransformModel::archive_best_responses(
    const ResultsManager& results_db, const StrStrSizet iterator_id,
    const Variables& best_submodel_vars, const Response& best_submodel_resp,
  ...
  if (expData.num_config_vars() == 0) {
    const RealVector& best_fns = best_submodel_resp.function_values();
    archive_best_original(results_db, iterator_id, best_fns, 0, num_best,
                          best_ind);
    // form residuals from model responses and apply covariance
    short dt_verbosity = output_level();
    output_level(SILENT_OUTPUT);
  ...
  const RealVector& resid_fns = residual_resp.function_values();
  // print_residuals will archive the residuals. for now, skip archiving the
  // un-weighted ones by passing in an uninitialized ResultsManager.
  const RealVector& lsq_weights = Model::primary_response_fn_weights();
  Real wssr = std::sqrt(Minimizer::sum_squared_residuals(
      num_primary_fns(), resid_fns, lsq_weights));
  // Currently only the weighted residuals are archived. This differs from
  // what's printed to the console (see print_best_responses() above).
  archive_best_residuals(results_db, iterator_id, num_primary_fns(), resid_fns,
                         wssr, num_best, best_ind);
}

/// Archive best configuration variables
void DataTransformModel::archive_best_config_variables(
    const ResultsManager& results_db, const StrStrSizet& iterator_id,
    const Variables& vars, const int& exp_index, const int& num_best,
