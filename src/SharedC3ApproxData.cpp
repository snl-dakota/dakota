...
SharedC3ApproxData::SharedC3ApproxData(ProblemDescDB& problem_db,
                                       size_t num_vars)
    : SharedApproxData(BaseConstructor(), problem_db, num_vars),
      ...
      regressRegParam(
          problem_db.get_real("model.surrogate.regression_penalty")),
      solverTol(problem_db.get_real("model.c3function_train.solver_tolerance")),
      solverRoundingTol(problem_db.get_real(
          "model.c3function_train.solver_rounding_tolerance")),
      statsRoundingTol(problem_db.get_real(
...
SharedC3ApproxData::SharedC3ApproxData(const String& approx_type,
    ...
    : SharedApproxData(NoDBBaseConstructor(), approx_type, num_vars, data_order,
      ...
      crossMaxIter(5),  // adaptConstruct(false),
      c3AdvancementType(NO_C3_ADVANCEMENT),
...
void SharedC3ApproxData::construct_basis(
  ...
  size_t np, max_np = max_order();
  ++max_np;  // no overflow (default is USHRT_MAX)
  for (i = 0; i < num_rv; ++i)
    if (no_mask || active_vars[i]) {
      switch (rv_types[i]) {
        case Pecos::STD_NORMAL:
          o_opts = ope_opts_alloc(HERMITE);
          break;
        case Pecos::STD_UNIFORM:
          o_opts = ope_opts_alloc(LEGENDRE);
          break;
        default:
          o_opts = NULL;
  ...
  if (!activeKey.empty()) formUpdated[activeKey] = true;
}

void SharedC3ApproxData::update_basis(const UShortArray& start_orders,
...
void SharedC3ApproxData::update_basis(size_t v, unsigned short start_order,
                                      unsigned short max_order) {
  size_t max_np = max_order;
  ++max_np;  // no overflow (default is USHRT_MAX)
  ...
  switch (c3AdvancementType) {
    ...
    case START_ORDER_ADVANCEMENT: {
      ...
      for (size_t i = 0; i < num_ord; ++i)
        if (s_ord[i] < m_ord) return true;
      return false;
      break;
    }
    case START_RANK_ADVANCEMENT:
      return (start_rank() < max_rank());
      break;
...
void SharedC3ApproxData::increment_order() {
  switch (c3AdvancementType) {
    ...
    case MAX_RANK_ORDER_ADVANCEMENT:
      ...
      // > unconditionally advancing both is wasteful; only advance
      // non-saturated > could also consider only advancing one when both bounds
      // are active:
...
void SharedC3ApproxData::pre_combine() {
  ...
  for (cit = startOrdersMap.begin(); cit != startOrdersMap.end(); ++cit) {
    ...
    for (i = 0; i < numVars; ++i)
      if (so[i] > combinedOrders[i]) combinedOrders[i] = so[i];
  }
