...
class TPLDataTransfer {
 public:
  ...
  Real get_response_value_from_dakota(const Response &resp) const {
    return (maxSense ? -resp.function_value(0) : resp.function_value(0));
  }
  ...
  void get_nonlinear_eq_constraints_from_dakota(const Response &resp,
                                                VecT &values) {
    // We do not perform checks on incoming vector size or do any
    // resizing/allocation because clients may be passing a view into a larger
    // vector, and the size/resize semantics might overly constrain what type we
    // can support, eg double*, RealVector, etc.
 ...
 protected:
  ...
  /// Construct nonlinear equality maps needed to exchange data to/from Dakota
  /// and the TPL
  void configure_nonlinear_eq_adapters(NONLINEAR_EQUALITY_FORMAT,
                                       const Constraints &);

  /// Construct nonlinear inequality maps needed to exchange data to/from Dakota
  /// and the TPL
  void configure_nonlinear_ineq_adapters(NONLINEAR_INEQUALITY_FORMAT,
                                         const Constraints &, bool split_eqs);
  ...
  /// Single boolean (could be extended to multiple) indicating min/max sense of
  /// optimal value
  bool maxSense;
  ...
  /// ... based conditionally on lower bounds using bigRealBoundSize and on
  /// whether TPL splits equalities into two inequalities
