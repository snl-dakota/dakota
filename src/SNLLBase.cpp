...
static const char rcsId[] =
    "@(#) $Id: SNLLBase.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
SNLLBase::SNLLBase(ProblemDescDB& problem_db)
    : searchMethod(problem_db.get_string("method.optpp.search_method")),
      ...
      stepLenToBndry(
          problem_db.get_real("method.optpp.steplength_to_boundary")),
  ...
  constantASVFlag = (problem_db.interface_locked())
                        ? false
  ...
  switch (problem_db.get_short("method.optpp.merit_function")) {
    case OPTPP::NormFmu:
      meritFn = OPTPP::NormFmu;
      break;
    case OPTPP::ArgaezTapia:
      meritFn = OPTPP::ArgaezTapia;
      break;
    case OPTPP::VanShanno:
      meritFn = OPTPP::VanShanno;
      break;
  }
}
...
void SNLLBase::snll_pre_instantiate(bool bound_constr_flag, int num_constr) {
  ...
  // searchStrat = OPTPP::LineSearch; // default for NIPS optimizers
    ...
    searchStrat =
        OPTPP::TrustRegion;  // all other cases, including unconstr opt
}

void SNLLBase::snll_post_instantiate(
    int num_cv, bool vendor_num_grad_flag, const String& finite_diff_type,
    const RealVector& fdss, size_t max_iter, size_t max_eval, Real conv_tol,
    Real grad_tol, Real max_step, bool bound_constr_flag, int num_constr,
    short output_lev, OPTPP::OptimizeClass* the_optimizer,
    OPTPP::NLP0* nlf_objective, OPTPP::FDNLF1* fd_nlf1,
  ...
  if (vendor_num_grad_flag) {
    ...
    if (fd_nlf1) fd_nlf1->setFcnAccrcy(fcn_accrcy);
    if (fd_nlf1_con && num_constr) fd_nlf1_con->setFcnAccrcy(fcn_accrcy);
  }
  ...
  if (output_lev == DEBUG_OUTPUT) the_optimizer->setDebug();
...
void SNLLBase::snll_initialize_run(
    OPTPP::NLP0* nlf_objective, OPTPP::NLP* nlp_constraint,
    const RealVector& init_pt, bool bound_constr_flag,
    const RealVector& lower_bounds, const RealVector& upper_bounds,
  ...
  if (num_lin_con) {
    if (num_lin_ineq_con) {
      ...
      RealVector optpp_lin_ineq_ubnds(Teuchos::Copy, lin_ineq_u_bnds.values(),
                                      num_lin_ineq_con);
      OPTPP::Constraint li = new OPTPP::LinearInequality(
          optpp_lin_ineq_coeffs, optpp_lin_ineq_lbnds, optpp_lin_ineq_ubnds);
    ...
    if (num_lin_eq_con) {
      ...
      RealVector optpp_lin_eq_targets(Teuchos::Copy, lin_eq_targets.values(),
                                      num_lin_eq_con);
      OPTPP::Constraint le =
          new OPTPP::LinearEquation(optpp_lin_eq_coeffs, optpp_lin_eq_targets);
  ...
  if (num_nln_con) {
    ...
    OPTPP::Constraint nc = new OPTPP::NonLinearConstraint(
        nlp_constraint, augmented_lower_bnds, augmented_upper_bnds,
...
void SNLLBase::snll_post_run(OPTPP::NLP0* nlf_objective) {
  ...
  optLSqInstance->bestVariablesArray.front().continuous_variables(
      nlf_objective->getXc());
...
void SNLLBase::copy_con_vals_dak_to_optpp(const RealVector& local_fn_vals,
...
void SNLLBase::copy_con_vals_optpp_to_dak(const RealVector& g,
...
void SNLLBase::copy_con_grad(const RealMatrix& local_fn_grads,
                             RealMatrix& grad_g, size_t offset) {
  // Unlike DAKOTA, OPT++ expects nonlinear equations followed by nonlinear
...
void SNLLBase::copy_con_hess(const RealSymMatrixArray& local_fn_hessians,
