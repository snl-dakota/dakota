...
static const char rcsId[] =
    "@(#) $Id: DakotaEnvironment.cpp 6749 2010-05-03 17:11:57Z briadam $";
...
Environment::Environment(BaseConstructor, int argc, char* argv[])
    :  // MPIManager potentially parses off MPI options from argc/argv via
       // MPI_Init
...
/** Default envelope constructor.  environmentRep is NULL in this
    case. */
Environment::Environment() { /* empty ctor */ }
...
/** Envelope constructor for LibraryEnvironment.  Selection of
    derived type by get_environment() is not necessary in this case. */
Environment::Environment(ProgramOptions prog_opts)
    :  // set the rep pointer to the appropriate environment type
...
/** Envelope constructor for LibraryEnvironment.  Selection of
    derived type by get_environment() is not necessary in this case. */
Environment::Environment(MPI_Comm dakota_mpi_comm, ProgramOptions prog_opts)
    :  // set the rep pointer to the appropriate environment type
      environmentRep(
          std::make_shared<LibraryEnvironment>(dakota_mpi_comm, prog_opts)) {
  if (!environmentRep)  // insufficient memory
    abort_handler(-1);
}
...
std::shared_ptr<Environment> Environment::get_environment(
    const String& env_type) {
...
Environment::Environment(const Environment& env)
    : environmentRep(env.environmentRep) { /* empty ctor */ }
...
void Environment::preprocess_inputs() {
  ...
  if (parallelLib.world_rank() != 0) return;
  ...
  if (programOptions.preproc_input()) {
    std::string tmpl_file =
        (programOptions.input_string().empty())
            ...
            : string_to_tmpfile(programOptions.input_string());

    // run the pre-processor on the file
    std::string preproc_file =
        pyprepro_input(tmpl_file, programOptions.preproc_cmd());
...
void Environment::parse(bool check_bcast_database,
  ...
  if (!programOptions.input_file().empty() ||
    ...
    probDescDB.parse_inputs(final_input, programOptions.parser_options(),
                            programOptions.user_modes().run, callback,
                            callback_data);
    if (programOptions.preproc_input())
  ...
  if (check_bcast_database)
    ProblemDescDBUtils::check_and_broadcast_pdb(
        probDescDB, programOptions.user_modes(), parallelLib);
}
...
void Environment::construct() {
  ...
  if (method_ptr.empty())
    probDescDB.resolve_top_method(false);  // no model set
  else
  ...
  IteratorScheduler::init_iterator(probDescDB, parallelLib, topLevelIterator,
                                   w_pl_iter);
...
void Environment::execute() {
  if (environmentRep)
    ...
    if (output_rank) outputManager.archive_input(programOptions);
    ...
    if (output_rank) Cout << "<<<<< Environment execution completed.\n";
...
bool Environment::check() const {
  ...
  if (programOptions.help() || programOptions.version()) return true;
