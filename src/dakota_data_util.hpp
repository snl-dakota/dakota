...
bool operator==(
    ...
    typename boost::multi_array<T, 1>::template const_array_view<1>::type mav) {
  // Check for equality in array lengths
  size_t len = vec.size();
  if (mav.size() != len) return false;
  // Check each size_t
  for (size_t i = 0; i < len; ++i)
    if (mav[i] != vec[i]) return false;
  return true;
}
...
bool operator==(
    ...
    typename boost::multi_array<T, 1>::template const_array_view<1>::type mav) {
  if (ma.size() != mav.size()) return false;
  // Note: template dependent names require disambiguation
  typename boost::multi_array<T, 1>::template const_array_view<
      1>::type::const_iterator cit1 = mav.begin(),
                               end1 = mav.end();
...
/// actionable content
template <typename OrdinalType, typename ScalarType>
bool non_zero(const std::vector<ScalarType>& vec, OrdinalType start,
              OrdinalType end) {
...
bool is_equal_vec(const RealVector& vec1, const VectorType& vec2) {
  // Check for equality in array lengths
  int len = vec1.length();
  if ((int)vec2.size() != len) return false;
  // Check each size_t
  for (int i = 0; i < len; ++i)
...
bool is_equal_partial(
  ...
  for (i = 0; i < len; ++i)
    if (vec1[i] != vec2[start_index2 + i]) return false;
  return true;
}
...
inline bool is_equal_partial(const StringMultiArray& ma1,
  ...
  for (i = 0; i < len; ++i)
    if (ma1[i] != ma2[start_index2 + i]) return false;
  return true;
}
...
/// Sort incoming vector with result and corresponding indices returned in
/// passed arguments
void sort_vector(const RealVector& vec, RealVector& sort_vec,
                 IntVector& indices);

/// Sort incoming matrix columns with result and corresponding indices returned
/// in passed arguments
...
void apply_matrix_partial(const MatrixType& M, const VectorType& v1,
                          VectorType& v2) {
  if (M.numCols() > v1.size()) {
    Cerr << "apply_matrix Error: incoming vector size is inconsistent with "
  ...
  if (M.numRows() > v2.size()) v2.resize(M.numRows());

  // Apply the matrix
  for (size_t i = 0; i < M.numRows(); ++i) {
    v2[i] = 0.0;
    for (size_t j = 0; j < M.numCols(); ++j) v2[i] += M(i, j) * v1[j];
  }
}
...
template <typename VectorType>
void apply_matrix_transpose_partial(const RealMatrix& M, const VectorType& v1,
                                    VectorType& v2) {
  if (M.numRows() > v1.size()) {
    Cerr << "apply_matrix_transpose Error: incoming vector size is "
            "inconsistent with matrix row dimension."
         << std::endl;
    abort_handler(-1);
  }

  // Resize target vector if needed
  if (M.numCols() > v2.size()) v2.resize(M.numCols());

  // Apply the matrix
  for (size_t j = 0; j < M.numCols(); ++j) {
    v2[j] = 0.0;
    for (size_t i = 0; i < M.numRows(); ++i) v2[j] += M(i, j) * v1[i];
  }
}
...
void assign_value(vecType& target, valueType val, size_t start, size_t len) {
  ...
  for (i = start; i < end; ++i) target[i] = val;
}
...
// }
//
// template <typename ScalarType1> //, typename ScalarType1, typename
// ScalarType2> void copy_data(const RealVector& vec, ScalarType1* Sptr, const
// size_t ptr_len, size_t dummy)
//{
...
// template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
// void copy_data(
//   const std::vector<Teuchos::SerialDenseVector<OrdinalType1, ScalarType> >&
//   va, ScalarType* ptr, const OrdinalType2 ptr_len, const String& ptr_type)
//{
//   bool c_type;
...
template <typename OrdinalType, typename ScalarType>
void copy_data(const std::vector<
                   Teuchos::SerialDenseVector<OrdinalType, ScalarType> >& sdva,
  ...
  for (i = 0; i < num_vec; ++i) {
    ...
    for (j = 0; j < vec_len; ++j) sdm(i, j) = vec_i[j];
  }
}
...
void copy_data_transpose(
    const std::vector<Teuchos::SerialDenseVector<OrdinalType, ScalarType> >&
        sdva,
    Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& sdm) {
  OrdinalType i, j, num_vec = sdva.size(), max_vec_len = 0;
  for (i = 0; i < num_vec; ++i) {  // loop over vectors in array
  ...
  for (i = 0; i < num_vec; ++i) {
    ...
    for (j = 0; j < vec_len; ++j) sdm_i[j] = vec_i[j];
  }
}
...
/// - used by NestedModel::update_sub_iterator - RWH
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data(const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv,
  ...
  for (OrdinalType1 i = 0; i < nr; ++i)
    for (OrdinalType1 j = 0; j < nc; ++j, ++counter) sdm(i, j) = sdv[counter];
}
...
//
//   // This function is set up to do the copy with either num_a or a_len or
//   both
...
/// - used by ProcessApplicInterface::write_parameters_files - RWH
template <typename T>
void copy_data(const std::vector<std::vector<T> >& d2a, std::vector<T>& da) {
  ...
  for (i = 0; i < num_arrays; ++i) size_d2a += d2a[i].size();
  if (size_d2a != da.size()) da.resize(size_d2a);
  for (i = 0; i < num_arrays; ++i) {
    typename std::vector<T>::size_type array_len = d2a[i].size();
    for (j = 0; j < array_len; ++j) da[cntr++] = d2a[i][j];
  }
}

/// copy map<int, T> to std::vector<T> (discard integer keys) - used by
/// SurrBasedGlobalMinimizer::core_run - RWH
template <typename T>
void copy_data(const std::map<int, T>& im, std::vector<T>& da) {
  size_t i, size_im = im.size();
  if (size_im != da.size()) da.resize(size_im);
  typename std::map<int, T>::const_iterator im_cit = im.begin();
  for (i = 0; i < size_im; ++i, ++im_cit) da[i] = im_cit->second;
}
...
void copy_data(const Teuchos::SerialDenseVector<OrdinalType, ScalarType>& sdv1,
  ...
  if (sdv2.length() != size_sdv1) sdv2.sizeUninitialized(size_sdv1);
  sdv2.assign(sdv1);
}
...
template <typename OrdinalType, typename ScalarType>
void copy_data(
    const Teuchos::SerialSymDenseMatrix<OrdinalType, ScalarType>& ssdm1,
    Teuchos::SerialSymDenseMatrix<OrdinalType, ScalarType>& ssdm2) {
  OrdinalType nr1 = ssdm1.numRows();
  if (ssdm2.numRows() != nr1) ssdm2.shapeUninitialized(nr1);
  ssdm2.assign(ssdm1);
}

template <typename OrdinalType, typename ScalarType>
void copy_data(
    const Teuchos::SerialSymDenseMatrix<OrdinalType, ScalarType>& ssdm,
    Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& sdm) {
  ...
  for (i = 0; i < nr; ++i) {
    sdm(i, i) = ssdm(i, i);
    for (j = 0; j < i; ++j) sdm(i, j) = sdm(j, i) = ssdm(i, j);
  }
}
...
void copy_data(const Teuchos::SerialDenseMatrix<OrdinalType, ScalarType>& sdm,
  ...
  if (ssdm.numRows() != nr) ssdm.shapeUninitialized(nr);
  for (i = 0; i < nr; ++i) {
    ssdm(i, i) = sdm(i, i);
    for (j = 0; j < i; ++j)
      ssdm(i, j) =
          (sdm(i, j) == sdm(j, i)) ? sdm(i, j) : (sdm(i, j) + sdm(j, i)) / 2.;
  }
}

/// Taken from pecos/src/MathTools.hpp, BUT
/// not templated because the implementation is specific to RealMatrix
inline void copy_data(const RealMatrix& source, RealMatrix& dest, int num_rows,
                      int num_cols, int start_row = 0, int start_col = 0) {
  RealMatrix source_subset(Teuchos::View, source, num_rows, num_cols, start_row,
                           start_col);
...
void copy_data(const Teuchos::SerialDenseVector<OrdinalType, ScalarType>& sdv,
  ...
  if (size_sdv != vec.size()) vec.resize(size_sdv);
  for (OrdinalType i = 0; i < size_sdv; ++i) vec[i] = sdv[i];
}
...
void copy_data(const Teuchos::SerialDenseVector<OrdinalType, ScalarType1>& sdv,
  ...
  OrdinalType size_sdv = sdv.length();
  if (size_sdv != vec.size()) vec.resize(size_sdv);
  for (OrdinalType i = 0; i < size_sdv; ++i) vec[i] = (ScalarType2)sdv[i];
}

/// copy Array<ScalarType> to
/// Teuchos::SerialDenseVector<OrdinalType, ScalarType> - used by
/// NOWPACOptimizer - MSE
template <typename OrdinalType, typename ScalarType1, typename ScalarType2>
void copy_data(const std::vector<ScalarType1>& vec,
  ...
  if (size_vec != sdv.length()) sdv.sizeUninitialized(size_vec);
  for (OrdinalType i = 0; i < size_vec; ++i) sdv[i] = (ScalarType2)vec[i];
}

/// copy ScalarType* to Teuchos::SerialDenseVector<OrdinalType, ScalarType> -
/// used by ScalingModel::response_modify_n2s - RWH
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data(const ScalarType* ptr, const OrdinalType2 ptr_len,
               Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv) {
  if (sdv.length() != ptr_len) sdv.sizeUninitialized(ptr_len);
  for (OrdinalType2 i = 0; i < ptr_len; ++i) sdv[i] = ptr[i];
...
/// copy ScalarType* to Teuchos::SerialDenseVector<OrdinalType, ScalarType> -
/// used by NL2SOLLeastSq::core_run - RWH
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data(const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv,
  ...
  for (OrdinalType2 i = 0; i < ptr_len; ++i) ptr[i] = sdv[i];
}
...
void copy_data(const ScalarType* ptr1, ScalarType* ptr2,
  ...
  for (OrdinalType i = 0; i < ptr_len; ++i) ptr2[i] = ptr1[i];
}

/// copy SerialDenseVector<> to Array<SerialDenseVector<> > - used by
/// ConcurrentMetaIterator constructor - RWH
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data(
    const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv,
  ...
  if (sdva.size() != num_vec) sdva.resize(num_vec);
  // sdv is head to tail by rows, which matches the visual matrix format that
  // a user would employ in specifying a matrix as a <LISTof><REAL>
  OrdinalType2 counter = 0;
  for (OrdinalType2 i = 0; i < num_vec; ++i) {
    if (sdva[i].length() != vec_len) sdva[i].sizeUninitialized(vec_len);
    for (OrdinalType2 j = 0; j < vec_len; ++j) sdva[i][j] = sdv[counter++];
  }
}
...
void copy_data_partial(const vecType1& source, size_t source_start_idx,
                       vecType2& target, size_t target_start_idx, size_t len) {
  // This requires that the target type supports the size() method, which
  // RealVectors don't
...
/// used by DataTransformModel::vars_mapping - RWH
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data_partial(
  ...
  if (num_items != sdv2.length()) sdv2.sizeUninitialized(num_items);
  for (OrdinalType2 i = 0; i < num_items; ++i) sdv2[i] = sdv1[start_index1 + i];
}

/// copy all of first SerialDenseVector to portion of second SerialDenseVector -
/// used by MixedVariables - RWH, NLSSOLLeastSq - BMA
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data_partial(
    const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv1,
    Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv2,
  ...
  for (OrdinalType1 i = 0; i < num_items; ++i) sdv2[start_index2 + i] = sdv1[i];
}
...
/// copy all of first SerialDenseVector to portion of second SerialDenseVector -
/// used by SharedSurfpackApproxData::merge_variable_arrays - RWH
template <typename OrdinalType1, typename OrdinalType2, typename ScalarType>
void copy_data_partial(
    const Teuchos::SerialDenseVector<OrdinalType1, ScalarType>& sdv1,
  ...
  for (OrdinalType1 i = 0; i < num_items; ++i) da2[start_index2 + i] = sdv1[i];
}

/// copy portion of first Array<T> to all of second Array<T> - used by
/// SharedResponseDataRep constructor - RWH
template <typename T>
void copy_data_partial(const std::vector<T>& da1, size_t start_index1,
  ...
  if (num_items != da2.size()) da2.resize(num_items);
  for (size_t i = 0; i < num_items; ++i) da2[i] = da1[start_index1 + i];
}

/// copy all of first Array<T> to portion of second Array<T> - used by
/// ParamStudy::multidim_loop - RWH
template <typename T>
void copy_data_partial(const std::vector<T>& da1, std::vector<T>& da2,
  ...
  for (i = 0; i < num_items; ++i) da2[start_index2 + i] = da1[i];
}

/// copy all of first Array<T> to portion of boost::multi_array<T, 1> - used by
/// RelaxedVariables - RWH
template <typename T>
void copy_data_partial(const std::vector<T>& da, boost::multi_array<T, 1>& bma,
  ...
  for (i = 0; i < num_items; ++i) bma[start_index_bma + i] = da[i];
}
...
/// Copies a column of a Teuchos_SerialDenseMatrix<int,Real> to
/// std::vector<Real>
template <typename VectorType>
void copy_column_vector(const RealMatrix& m, RealMatrix::ordinalType j,
                        VectorType& col) {
  RealMatrix::ordinalType i, num_items = m.numRows();
  if (col.size() != num_items) col.resize(num_items);
  for (i = 0; i < num_items; ++i) col[i] = m(i, j);
}
...
void copy_row_vector(const RealMatrix& m, RealMatrix::ordinalType i,
  ...
  if (row.size() != num_items) row.resize(num_items);
  for (j = 0; j < num_items; ++j) row[j] = m(i, j);
}

/// Inserts a std::vector<Real> into a row of a
/// Teuchos_SerialDenseMatrix<int,Real>
template <typename ScalarType>
void insert_row_vector(const std::vector<ScalarType>& row,
                       RealMatrix::ordinalType i, RealMatrix& m) {
  if ((m.numRows() < i + 1) || (m.numCols() != row.size()))
    m.reshape(i + 1, row.size());
  for (size_t j = 0; j < row.size(); ++j) m(i, j) = row[j];
}
...
inline void merge_data_partial(const IntVector& d_vec, RealVector& m_vec,
                               size_t start_index_ma) {
  ...
  for (i = 0; i < num_items; ++i) m_vec[start_index_ma + i] = (Real)d_vec[i];
}

/// merge a discrete integer vector into a single continuous array
inline void merge_data_partial(const IntVector& d_vec, RealArray& m_array,
                               size_t start_index_ma) {
  size_t i, num_items = d_vec.length();
  ...
  for (i = 0; i < num_items; ++i) m_array[start_index_ma + i] = (Real)d_vec[i];
}
...
size_t map_value_to_index(const ValueType& value,
  ...
  typename std::map<KeyType, ValueType>::const_iterator cit;
  for (cit = pairs.begin(); cit != pairs.end(); ++cit, ++index)
    if (cit->second == value) return index;
  return _NPOS;
}
...
void x_y_pairs_to_x_set(
  ...
  for (i = 0; i < num_pairs; ++i) x_set.insert(xy_pairs[2 * i]);
}
...
inline ScalarType find_min(const std::vector<ScalarType>& vec,
  ...
  ScalarType min =
      (start < len) ? vec[start] : std::numeric_limits<ScalarType>::max();
...
inline size_t find_index(SizetMultiArrayConstView bmacv, size_t search_data) {
  ...
  for (size_t i = 0; i < len; i++)
    if (bmacv[i] == search_data) return i;
  return _NPOS;
}
...
template <typename ListT>
typename ListT::const_iterator find_if(
    const ListT& c,
  ...
  for (typename ListT::const_iterator it = c.begin(); it != c.end(); ++it)
    if (test_fn(*it, test_fn_data)) return it;
  return c.end();
...
void copy_data(const std::vector<VectorType>& va, ScalarType* ptr,
               int ptr_len) {
  size_t total_len = 0, cntr = 0, num_vec = va.size();
  for (size_t i = 0; i < num_vec; ++i) total_len += va[i].length();
  if (total_len != ptr_len) {
    Cerr << "copy_data Error: pointer allocation (" << ptr_len
         << ") does not equal "
         << "total std::vector<VecType> length (" << total_len << ")."
         << std::endl;
    abort_handler(-1);
  }
  for (size_t i = 0; i < num_vec; ++i) {
    int vec_len = va[i].length();
    for (int j = 0; j < vec_len; ++j) ptr[cntr++] = va[i][j];
  }
}

/// copy boost::multi_array view to Array - used by ActiveSet::derivative_vector
/// - RWH
inline void copy_data(SizetMultiArrayConstView ma, SizetArray& da) {
  size_t size_ma = ma.size();
  if (size_ma != da.size()) da.resize(size_ma);
  for (size_t i = 0; i < size_ma; ++i) da[i] = ma[i];
}
    ...
    for (i = 0; i < search_len; ++i) {
      ...
      for (j = start; j < vec_len; ++j) {
        ...
        if (val == search_val)
          ...
          start = j;
          break;
        }  // stop inner loop, resume outer search
           // else continue advancing for val < search_val
      }
    }
  else
    for (i = 0; i < search_len; ++i)
      if (contains(vec, search_vec[i])) return true;
...
inline bool isfloat(const Dakota::String token) {
  static boost::regex float_regex(
      "[\\+-]?[0-9]*\\.?[0-9]+\\.?[0-9]*[eEdD]?[\\+-]?[0-9]*|[Nn][Aa][Nn]|[\\+-"
