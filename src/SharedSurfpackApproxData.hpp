...
class SharedSurfpackApproxData : public SharedApproxData {
 ...
 private:
  ...
  /// these are indices into the Model's vars so approx_eval[i] =
  /// [model_vars[ind[i]]]
...
template <typename RealArrayType>
void SharedSurfpackApproxData::merge_variable_arrays(const RealVector& cv,
                                                     const IntVector& div,
                                                     const RealVector& drv,
                                                     RealArrayType& ra) {
...
template <typename RealArrayType>
void SharedSurfpackApproxData::sdv_to_realarray(
    const Pecos::SurrogateDataVars& sdv, RealArrayType& ra) {
  // passed array must be sized due to length/size differences
...
template <typename RealArrayType>
void SharedSurfpackApproxData::vars_to_realarray(const Variables& vars,
                                                 RealArrayType& ra) {
...
template <typename RealArrayType>
void SharedSurfpackApproxData::active_vars_to_realarray(const Variables& vars,
                                                        RealArrayType& ra) {
  // passed array ra must be sized due to length/size differences
...
template <typename RealArrayType>
void SharedSurfpackApproxData::all_vars_to_realarray(const Variables& vars,
                                                     RealArrayType& ra) {
  // passed array ra must be sized due to length/size differences
...
template <typename RealArrayType>
RealArrayType SharedSurfpackApproxData::imported_eval_vars(
