...
double compute_metric(const VectorXd &p, const VectorXd &d,
  ...
  double N = p.size();
  if (N != d.size())
    error("Mismatch between prediction and data vector sizes");

  METRIC_TYPE mtype = metric_type(metric_name);

  switch(mtype) {
   case METRIC_TYPE::SUM_SQUARED : return (p - d).squaredNorm();
   case METRIC_TYPE::MEAN_SQUARED : return (p - d).squaredNorm()/N;
   case METRIC_TYPE::ROOT_MEAN_SQUARED : return std::sqrt((p - d).squaredNorm()/N);
   case METRIC_TYPE::SUM_ABS : return (p - d).lpNorm<1>();
   case METRIC_TYPE::MEAN_ABS : return (p - d).lpNorm<1>()/N;
   case METRIC_TYPE::MAX_ABS : return (p - d).lpNorm<Eigen::Infinity>();
