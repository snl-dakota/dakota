 ...
 *  \param[in] A The first matrix to test
 *  \param[in] B The second matrix to test
 *  \param[in] tol The tolerance to use when comparing double values
 ...
 *  \param[out] R The collection of vectors to be populated
 *  \param[in] num_datasets The number of datasets to read in
 *  \param[in] num_samples The number of data points (e.g. function values, build points) per dataset
*/
void populateVectorsFromFile(const std::string & filename, std::vector<VectorXd> &R, int num_datasets, int num_samples);

/**
 *  \brief Populate a collection of matrices read in a from a text file assuming data layout is a "column-major"
 *  stack of num_samples by num_vars matrices
 *  \param[in] filename The file that contains the data
 *  \param[out] S The collection of vectors to be populated
 *  \param[in] num_datasets The number of datasets to read in
 *  \param[in] num_samples The number of data points (e.g. function values, build points) per dataset (row dim)
 *  \param[in] num_vars The number of variables per dataset (column dim)
*/
