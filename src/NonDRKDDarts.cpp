  ...
  samples(probDescDB.get_int("method.samples"))
{
    ...
    /*
    Cout << "Hello World, RKD Darts is coming! " << '\n';
    ...
    */
    ...
    if (iteratedModel.model_type() != "surrogate") {
      Cerr << "Error: NonDRKDDarts::iteratedModel must be a "
...
void NonDRKDDarts::quantify_uncertainty()
{
    ...
    */
    ...
    init_rkd_darts(); // prepare global variables and containers
    
    execute(1);
    
    exit_rkd_darts(); // delete dynamic arrays from memory
...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    void NonDRKDDarts::execute(size_t kd)
    {
        ...
        for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
        {
            ...
            for (size_t level_count = 0; level_count<num_levels; level_count++)
            {
                ...
                for (size_t isample = 0; isample < _num_inserted_points; isample++) assign_sphere_radius_RKD(isample);
                
                start_time = clock();
                if (kd == 0)
                {
                    Cout <<  "rkd:: Classical Point-Dart Throwing Games ... ";
                    classical_dart_throwing_games(0);
                }
                else if (kd == 1)
                {
                    Cout <<  "rkd:: Classical Line-Dart Throwing Games ... ";
                ...
                std::cout<<  "rkd::    Number of inserted points = " << std::fixed << _num_inserted_points << std::endl;
                std::cout<<  "rkd::    Number of thrown darts = " << std::fixed << _num_darts << std::endl;
                std::cout.precision(4);
                std::cout<<  "rkd::    Execution Time = " << std::fixed << cpu_time << " seconds." << std::endl;
        ...
        std::cout <<  "rkd:: Building Surrogates ..." << std::endl;
        estimate_rkd_surrogate();
        
        if (_n_dim == 2 && _eval_error)
        {
            std::cout <<  "rkd::    Plotting 2d disks ...";
    ...
    void NonDRKDDarts::classical_dart_throwing_games(size_t game_index)
    {
        ...
        while(true)
        {
            ...
            if (valid_dart(_dart))
            {
                ...
                {
                    std::cout<< "\nrkd:: Simulation Budget has been exhausted!" << std::endl;
                    return;
                }
            }
            else
            {
                ...
                {
                    std::cout<< "\nrkd:: Void-finding budget has been exhausted, Increaing Lipschitz Multiplier!" << std::endl;
                    _safety_factor *= 1.5;
                    for (size_t isample = 0; isample < _num_inserted_points; isample++)
                    {
                        assign_sphere_radius_RKD(isample);
                    }
                    
                    //std::cout<< "\nrkd:: Void-finding budget has been exhausted, shrinking BIG disks!" << std::endl;
                    //shrink_big_spheres();
                }
            }
        }
    ...
    void NonDRKDDarts::line_dart_throwing_games(size_t game_index)
    {
        _num_successive_misses_p = 0;
        _num_successive_misses_m = 0;
        size_t num_prior_disks = _num_inserted_points;
        ...
        while (true)
        {
            ...
            if (covered) 
            {
                ...
                if (_num_successive_misses_p + _num_successive_misses_m > _max_num_successive_misses) 
                {
                    ...
                    //std::cout<< "\nrkd:: Void-finding budget has been exhausted, shrinking all disks!" << std::endl;
                    //shrink_big_spheres();
                }
            }
            else 
    ...
    bool NonDRKDDarts::valid_dart(double* x)
    {
    ...
    bool NonDRKDDarts::valid_line_flat(size_t flat_dim, double* flat_dart)
    {
        for (size_t index = 0; index < _num_inserted_points; index++)
        {
            double hh(0.0);
    ...
    void NonDRKDDarts::add_point(double* x)
    {
        ...
        if (_use_local_L)
        {
            ...
            assign_sphere_radius_RKD(_num_inserted_points - 1);
            ...
            for (size_t i = 1; i <= num_neighbors; i++)
            {
                ...
                assign_sphere_radius_RKD(neighbor); // adjusting radius of old disks due to new L
            }
    ...
    void NonDRKDDarts::verify_neighbor_consistency()
    {
        for (size_t ipoint = 0; ipoint < _num_inserted_points; ipoint++)
        {
            ...
            for (size_t i = 1; i <= num_neighbors; i++)
            {
                ...
                if (add_neighbor(neighbor, ipoint)) assign_sphere_radius_RKD(neighbor);
            }
        }
    }

    
    bool NonDRKDDarts::add_neighbor(size_t ipoint, size_t ineighbor)
    {
        bool found(false);
        size_t num_neighbors(_sample_neighbors[ipoint][0]);
        for (size_t i = 1; i <= num_neighbors; i++)
    ...
    void NonDRKDDarts::retrieve_neighbors(size_t ipoint, bool update_point_neighbors)
    {
    ...
    void NonDRKDDarts::sample_furthest_vertex(size_t ipoint, double* fv)
    {
        double* tmp_pnt = new double[_n_dim];    // end of spoke
        double* qH = new double[_n_dim];         // mid-ppint
        double* nH = new double[_n_dim];         // normal vector
    ...
    void NonDRKDDarts::update_global_L()
    {
        for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
        {
    ...
    void NonDRKDDarts::assign_sphere_radius_RKD(size_t isample)
    {
        double r = _diag;
    ...
    void NonDRKDDarts::shrink_big_spheres()
    {
        double rr_max(0.0);
        for (size_t isample = 0; isample < _num_inserted_points; isample++)
        {
        ...
        std::cout << "\nrkd:: maximum radius = " << std::sqrt(rr_max) << std::endl;
    ...
    double NonDRKDDarts::area_triangle(double x1, double y1, double x2, double y2, double x3, double y3)
    {
    ...
    void NonDRKDDarts::estimate_rkd_surrogate()
    {
        ...
        std::cout.precision(4);
        std::cout <<  "rkd::    Surrogates built in " << std::fixed << cpu_time << " seconds." << std::endl;
        ...
        while (isample < num_MC_samples)
        {
            ...
            for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
            {
                ...
                for (size_t level_count = 0; level_count < num_levels; level_count++)
                {
                    ...
                    if (_eval_error && f_exact <  _failure_threshold) rkd_exact += 1.0;
                }
            }
            isample+=1.0;
        }
        ...
        std::cout <<  "rkd::    Surrogate evaluated in " << std::fixed << cpu_time << " seconds." << std::endl;
        
        if (_eval_error)
        {
            std::cout.precision(15);
            std::cout <<  "rkd::    exact rkd " << std::fixed << fabs(rkd_exact)/ num_MC_samples << std::endl;
            std::cout <<  "rkd::    Surrogate error " << std::fixed << fabs((num_fMC_samples[0][0] - rkd_exact) / num_MC_samples) << std::endl;
        }
        ...
        for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
        {
            ...
            for (size_t level_count = 0; level_count < num_levels; level_count++)
            {
                double rkd_val = num_fMC_samples[resp_fn_count][level_count] / num_MC_samples;
                computedProbLevels[resp_fn_count][level_count] = rkd_val;
            }
        }
    ...
    bool NonDRKDDarts::trim_line_using_Hyperplane(size_t num_dim,                               // number of dimensions
    ...
    void NonDRKDDarts::plot_vertices_2d(bool plot_true_function, bool plot_suurogate)
    {
        std::stringstream ss;
        ss << "rkd_" << _active_response_function << ".ps";
        ...
        if (plot_true_function)
        {
            ...
            Cout<< "rkd::       RKD (isocontouring) = " << std::setprecision(10) << failure / A << '\n';
    ...
    void NonDRKDDarts::plot_neighbors( )
    {
        std::stringstream ss;
        ss << "rkd_neighbors" << _active_response_function << ".ps";
        std::fstream file(ss.str().c_str(), std::ios::out);
        file << "%!PS-Adobe-3.0" << std::endl;
        file << "72 72 scale     % one unit = one inch" << std::endl;
    ...
    void NonDRKDDarts::print_results(std::ostream& s)
    {
