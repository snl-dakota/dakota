...
static const char rcsId[] =
    "@(#) $Id: NonDRKDDart.cpp 6080 2009-09-08 19:03:20Z gtang $";

namespace Dakota {

NonDRKDDarts::NonDRKDDarts(ProblemDescDB& problem_db,
...
void NonDRKDDarts::init_rkd_darts() {
  ...
  for (size_t resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++)
    _fval[resp_fn_count] = new double[100 * samples];
}
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
void NonDRKDDarts::exit_rkd_darts() { destroy_rkd_containers(); }
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
// Add a new
void NonDRKDDarts::create_new_sample(size_t parent, size_t left, size_t right,
                                     double position) {
  ...
  _num_samples++;
  _sample_num_children[parent]++;
  _sample_num_children[sample] = 0;
  // std::cout << "New sample created, index = " << sample  << ", left = " <<
  // _sample_left[sample] << " , x = " << _sample_coord[sample] << ", parent =
  // "<< parent << std::endl;
...
double NonDRKDDarts::estimate_surrogate_evaluation_err(size_t parent) {
  ...
  for (size_t i = 0; i < num_samples; i++) {
    ...
    double xmin = _xmin[idim];
    double xmax = _xmax[idim];
...
// ----------------------------------------------------------------
void NonDRKDDarts::get_neighbors(size_t sample, size_t& num_neighbors,
                                 size_t* neighbors) {
  num_neighbors = 0;
  double dF;
  ...
  while (num_neighbors < _max_num_neighbors) {
    ...
    if (left > 0) {
      ...
      num_neighbors++;
      current_left = left;
      left = _sample_left[current_left];
    }
    ...
    if (right > 0) {
      ...
      num_neighbors++;
      current_right = right;
      right = _sample_right[current_right];
    }
...
double NonDRKDDarts::interpolate_lagrange(size_t num_data_points,
                                          double* data_x, double* data_f,
                                          double x) {
  double sum(0.0);
...
// ----------------------------------------------------------------
double NonDRKDDarts::integrate_legendre_gauss(double xmin, double xmax,
  ...
  if (num_quadrature_points == 1) {
    x_quad[0] = 0.0000000000000000;
    w_quad[0] = 2.0000000000000000;
  } else if (num_quadrature_points == 2) {
    x_quad[0] = -0.5773502691896257;
    w_quad[0] = 1.0000000000000000;
    x_quad[1] = 0.5773502691896257;
    w_quad[1] = 1.0000000000000000;
  } else if (num_quadrature_points == 3) {
    x_quad[0] = 0.0000000000000000;
    w_quad[0] = 0.8888888888888888;
    x_quad[1] = -0.7745966692414834;
    w_quad[1] = 0.5555555555555556;
    x_quad[2] = 0.7745966692414834;
    w_quad[2] = 0.5555555555555556;
  } else if (num_quadrature_points == 4) {
    x_quad[0] = -0.3399810435848563;
    w_quad[0] = 0.6521451548625461;
    x_quad[1] = 0.3399810435848563;
    w_quad[1] = 0.6521451548625461;
    x_quad[2] = -0.8611363115940526;
    w_quad[2] = 0.3478548451374538;
    x_quad[3] = 0.8611363115940526;
    w_quad[3] = 0.3478548451374538;
  } else if (num_quadrature_points == 5) {
    x_quad[0] = 0.0000000000000000;
    w_quad[0] = 0.5688888888888889;
    x_quad[1] = -0.5384693101056831;
    w_quad[1] = 0.4786286704993665;
    x_quad[2] = 0.5384693101056831;
    w_quad[2] = 0.4786286704993665;
    x_quad[3] = -0.9061798459386640;
    w_quad[3] = 0.2369268850561891;
    x_quad[4] = 0.9061798459386640;
    w_quad[4] = 0.2369268850561891;
  } else if (num_quadrature_points == 6) {
    x_quad[0] = -0.6612093864662645;
    w_quad[0] = 0.3607615730481386;
    x_quad[1] = 0.6612093864662645;
    w_quad[1] = 0.3607615730481386;
    x_quad[2] = -0.2386191860831969;
    w_quad[2] = 0.4679139345726910;
    x_quad[3] = 0.2386191860831969;
    w_quad[3] = 0.4679139345726910;
    x_quad[4] = -0.9324695142031521;
    w_quad[4] = 0.1713244923791704;
    x_quad[5] = 0.9324695142031521;
    w_quad[5] = 0.1713244923791704;
  } else if (num_quadrature_points == 7) {
    x_quad[0] = 0.0000000000000000;
    w_quad[0] = 0.4179591836734694;
    x_quad[1] = -0.4058451513773972;
    w_quad[1] = 0.3818300505051189;
    x_quad[2] = 0.4058451513773972;
    w_quad[2] = 0.3818300505051189;
    x_quad[3] = -0.7415311855993945;
    w_quad[3] = 0.2797053914892766;
    x_quad[4] = 0.7415311855993945;
    w_quad[4] = 0.2797053914892766;
    x_quad[5] = -0.9491079123427585;
    w_quad[5] = 0.1294849661688697;
    x_quad[6] = 0.9491079123427585;
    w_quad[6] = 0.1294849661688697;
  } else if (num_quadrature_points == 8) {
    x_quad[0] = -0.1834346424956498;
    w_quad[0] = 0.3626837833783620;
    x_quad[1] = 0.1834346424956498;
    w_quad[1] = 0.3626837833783620;
    x_quad[2] = -0.5255324099163290;
    w_quad[2] = 0.3137066458778873;
    x_quad[3] = 0.5255324099163290;
    w_quad[3] = 0.3137066458778873;
    x_quad[4] = -0.7966664774136267;
    w_quad[4] = 0.2223810344533745;
    x_quad[5] = 0.7966664774136267;
    w_quad[5] = 0.2223810344533745;
    x_quad[6] = -0.9602898564975363;
    w_quad[6] = 0.1012285362903763;
    x_quad[7] = 0.9602898564975363;
    w_quad[7] = 0.1012285362903763;
  } else if (num_quadrature_points == 9) {
    x_quad[0] = 0.0000000000000000;
    w_quad[0] = 0.3302393550012598;
    x_quad[1] = -0.8360311073266358;
    w_quad[1] = 0.1806481606948574;
    x_quad[2] = 0.8360311073266358;
    w_quad[2] = 0.1806481606948574;
    x_quad[3] = -0.9681602395076261;
    w_quad[3] = 0.0812743883615744;
    x_quad[4] = 0.9681602395076261;
    w_quad[4] = 0.0812743883615744;
    x_quad[5] = -0.3242534234038089;
    w_quad[5] = 0.3123470770400029;
    x_quad[6] = 0.3242534234038089;
    w_quad[6] = 0.3123470770400029;
    x_quad[7] = -0.6133714327005904;
    w_quad[7] = 0.2606106964029354;
    x_quad[8] = 0.6133714327005904;
    w_quad[8] = 0.2606106964029354;
  } else if (num_quadrature_points == 10) {
    x_quad[0] = -0.1488743389816312;
    w_quad[0] = 0.2955242247147529;
    x_quad[1] = 0.1488743389816312;
    w_quad[1] = 0.2955242247147529;
    x_quad[2] = -0.4333953941292472;
    w_quad[2] = 0.2692667193099963;
    x_quad[3] = 0.4333953941292472;
    w_quad[3] = 0.2692667193099963;
    x_quad[4] = -0.6794095682990244;
    w_quad[4] = 0.2190863625159820;
    x_quad[5] = 0.6794095682990244;
    w_quad[5] = 0.2190863625159820;
    x_quad[6] = -0.8650633666889845;
    w_quad[6] = 0.1494513491505806;
    x_quad[7] = 0.8650633666889845;
    w_quad[7] = 0.1494513491505806;
    x_quad[8] = -0.9739065285171717;
    w_quad[8] = 0.0666713443086881;
    x_quad[9] = 0.9739065285171717;
    w_quad[9] = 0.0666713443086881;
  } else
    std::cout << "*** Error:: Too many quadrature points are needed!!"
              << std::endl;
  ...
  for (size_t i = 0; i < num_quadrature_points; i++) {
    ...
    double f_low =
        interpolate_lagrange(num_data_points - 1, data_x, data_f, xx);
...
void NonDRKDDarts::improve_parent_evaluation(size_t parent) {
  ...
  // if (parent == 0)
  // std::cout<< "*** surrogate = " << parent << " , interp_err = " <<
  // tot_interp_err << " , ev_err = " << tot_ev_err << std::endl;
    ...
    for (size_t i = 0; i < num_samples; i++) {
      ...
      if (u < sum + 1E-10) {
        ...
        if (u < left_err / total_cell_err) {
          // refine left
          size_t left = _sample_left[sample_index];
          double xmin = (5.0 / 6.0) * _xmin[idim] +
                        (1.0 / 6.0) * _sample_coord[sample_index];
          double xmax = 0.5 * _xmin[idim] + 0.5 * _sample_coord[sample_index];
          if (left > 0) {
            xmin = (2.0 / 3.0) * _sample_coord[left] +
                   (1.0 / 3.0) * _sample_coord[sample_index];
            xmax = (1.0 / 3.0) * _sample_coord[left] +
                   (2.0 / 3.0) * _sample_coord[sample_index];
          }
          u = generate_a_random_number();
          ...
          double xmax = (1.0 / 6.0) * _sample_coord[sample_index] +
                        (5.0 / 6.0) * _xmax[idim];

          if (right > 0) {
            xmin = (2.0 / 3.0) * _sample_coord[sample_index] +
                   (1.0 / 3.0) * _sample_coord[right];
            xmax = (1.0 / 3.0) * _sample_coord[sample_index] +
                   (2.0 / 3.0) * _sample_coord[right];
          }
          u = generate_a_random_number();
          double position = xmin + u * (xmax - xmin);
...
void NonDRKDDarts::evaluate_1d_surrogate(size_t parent) {
  ...
  for (size_t i = 0; i < num_samples; i++) {
    ...
    if (num_neighbors == 0) {
      ...
      integ += integrate_legendre_gauss(xmin, xseed, num_neighbors + 1, data_x,
                                        data_f,
                                        _sample_left_interp_err[sample_index]);
      integ += integrate_legendre_gauss(xseed, xmax, num_neighbors + 1, data_x,
                                        data_f,
                                        _sample_right_interp_err[sample_index]);
    }
  ...
  {
    if (rc_left)
      _sample_left_interp_err[rc_seed] = rc;
    else
      _sample_right_interp_err[rc_seed] = rc;
  } else if (fabs(discont_err - total_interp_err) < 1E-10 &&
             rc > 1000 * discont_err)  // All error is due to discontinuity
  {
    if (rc_left)
      _sample_left_interp_err[rc_seed] = rc;
    else
...
// ----------------------------------------------------------------
void NonDRKDDarts::add_surrogate_data(const Variables& vars,
                                      const Response& resp) {
...
void NonDRKDDarts::estimate_rkd_surrogate() {
  ...
  // Build a surrogate over the final sample set. Type: as specified in the
  // input spec
  build_surrogate();
  ...
  std::cout << "<<<<< RKD :: Surrogate model built in " << std::fixed
  ...
  std::cout << "<<<<< RKD :: Surrogate model evaluated in " << std::fixed
            << cpu_time << " seconds." << std::endl
            << std::endl;

  double vol(1.0);
  for (size_t idim = 0; idim < _num_dim; idim++)
    vol *= _xmax[idim] - _xmin[idim];
  ...
  if (_eval_error) {
    ...
    std::cout << "<<<<< RKD :: exact integration " << std::fixed << rkd_exact
              << std::endl;
    std::cout << "<<<<< RKD :: Surrogate error " << std::fixed
              << fabs((_I_RKD[0] - rkd_exact)) << std::endl;
  }
  delete[] tmp_pnt;
}
// ----------------------------------------------------------------
...
void NonDRKDDarts::print_integration_results(std::ostream& s) {
  size_t width = write_precision + 7, w2p2 = 2 * width + 2,
         w3p4 = 3 * width + 4;
  ...
  std::cout << "RKD Numerical Integration results using " << samples
            << " samples:" << std::endl
            << std::endl;

  for (size_t i = 0; i < numFunctions; i++) {
    s << "Estimated integral of " << fn_labels[i] << " = " << _I_RKD[i]
      << std::endl;
  }
}
///////////////////////////////////////////////////////////////////
