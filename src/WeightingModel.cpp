...
WeightingModel::WeightingModel(std::shared_ptr<Model> sub_model
  ...
  for (size_t i = 0; i < num_cv; ++i) vars_map_indices[i][0] = i;
  ...
  for (size_t i = 0; i < num_pri_fns; ++i) primary_resp_map_indices[i][0] = i;
...
WeightingModel::~WeightingModel() { /* empty dtor */ }

void WeightingModel::primary_resp_weighter(const Variables& sub_model_vars,
  ...
  const RealVector& lsq_weights =
      weightModelInstance->subModel->primary_response_fn_weights();
  ...
  if (lsq_weights.length() != num_pri_fns) {
    Cerr << "Error: mismatch in length of weighting vector (" << num_pri_fns
         << " expected and " << lsq_weights.length() << " provided)."
         << std::endl;
  ...
  for (size_t i = 0; i < num_pri_fns; ++i) {
    ...
    // functions
    if (asv[i] & 1) wt_fn_vals[i] = wt_i * sm_fn_vals[i];
