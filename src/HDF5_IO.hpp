...
// We are mixing C and C++ APIs here with an eye to eventually adopt one or the
// other - RWH
#include <cmath>
#include <iostream>
#include <limits>
...
inline H5::DataType h5_file_dtype(const ResultsOutputType t) {
  switch (t) {
    case ResultsOutputType::REAL:
      return h5_file_dtype(double(0.0));
      break;
    case ResultsOutputType::INTEGER:
      return h5_file_dtype(int(0));
      break;
    case ResultsOutputType::UINTEGER:
      return h5_file_dtype(unsigned(0));
      break;
    case ResultsOutputType::STRING:
      return h5_file_dtype("");
      break;
  }
}
...
inline H5::DataType h5_mem_dtype(const ResultsOutputType t) {
  switch (t) {
    case ResultsOutputType::REAL:
      return h5_mem_dtype(double(0.0));
      break;
    case ResultsOutputType::INTEGER:
      return h5_mem_dtype(int(0));
      break;
    case ResultsOutputType::UINTEGER:
      return h5_mem_dtype(unsigned(0));
      break;
    case ResultsOutputType::STRING:
      return h5_mem_dtype("");
      break;
...
class HDF5IOHelper {
   ...
   *   store_vector
   *   store_matrix
   *  - Place data into an existing higher dimensional dataset (call
   * create_empty_dataset first) set_scalar (in a 1D dataset) set_vector (in a
   * 2D dataset) set_matrix (in a 3D dataset) set_vector_matrix (in a 4D
   * dataset) set_vector_scalar_field (assign a scalar field to a 1D dataset of
   * compound type) set_vector_vector_field (assign a vector field to a 1D
   * dataset of compound type)
   *  - Append data into a dataset with an unlimited 0th dimension (call
   * create_empty_dataset first) append_scalar (to a 1D dataset) append_vector
   * (to a 2D dataset) append_matrix (to a 3D dataset) append_vector_matrix (to
   * a 4D dataset) READING
 ...
 public:
  ...
  /// Store matrix (2D) information to a dataset
  template <typename T>
  void store_matrix(const std::string &dset_name, const std::vector<T> &buf,
                    const int &num_cols, const bool &transpose = false) const;
  /// Store matrix (2D) information to a dataset
  void store_matrix(const std::string &dset_name,
  ...
  template <typename T>
  void set_scalar(const String &dset_name, H5::DataSet &ds, const T &data,
                  const int &index);
  /// Set a scalar in a 1D dataset at index using the dataset object
  void set_scalar(const String &dset_name, H5::DataSet &ds, const String &data,
                  const int &index);

  /// Set a row or column in a 2D dataset at index using its name
  template <typename T>
  void set_vector(const String &dset_name, const T &data, const int &index,
                  const bool &row = true);
  /// Set a row or column of Strings in a 2D dataset at index using the dataset
  /// object
  void set_vector(const String &dset_name, H5::DataSet &ds,
                  const StringMultiArrayConstView &data, const int &index,
                  const bool &row = true);
  /// Set a row or column of Strings in a 2D dataset at index using the dataset
  /// object
  void set_vector(const String &dset_name, H5::DataSet &ds,
  ...
  template <typename T>
  void set_vector(const String &dset_name, H5::DataSet &ds, const T &data,
  ...
  void set_matrix(const String &dset_name,
                  ...
                  const int &index, const bool &transpose = false);
  /// Set a matrix in a 3D dataset at the index into the 0th dimension using a
  /// dataset object. Dims of matrix must match those of the trailing dimensions
  /// of the dataset.
  template <typename T>
  ...
  template <typename T>
  void set_vector_matrix(
      const String &dset_name, H5::DataSet &ds,
      const std::vector<Teuchos::SerialDenseMatrix<int, T> > &data,
      const int &index, const bool &transpose = false);

  /// Set a scalar field on all elements of a 1D dataset of compound type using
  /// a ds name.
  template <typename T>
  void set_vector_scalar_field(const String &dset_name, const T &data,
  ...
  /// Set a vector field on all elements of a 1D dataset of compound type using
  /// a ds name.
  template <typename T>
  void set_vector_vector_field(const String &dset_name, const T &data,
  ...
  template <typename T>
  void append_vector(const String &dset_name, const T &data,
                     const bool &row = true);
  /// Append a vector as a row or column to a 2D dataset
  void append_vector(const String &dset_name, const std::vector<String> &data,
                     const bool &row = true);
  /// Append a vector as a row or column to a 2D dataset
  void append_vector(const String &dset_name,
                     const StringMultiArrayConstView &data,
                     const bool &row = true);
  /// Append a SerialDenseMatrix to a 3D dataset. The dataset will be expanded
  /// along the 0th dimension. By default, the shape of the matrix, (nrows,
  /// ncols), must match the size of the 1st and 2nd dimensions of the dataset.
  /// For transpose=true, the reverse must be true.
  template <typename T>
  void append_matrix(const String &dset_name,
                     const Teuchos::SerialDenseMatrix<int, T> &data,
                     const bool &transpose = false);
  /// Append a std::vector of  SerialDenseMatrix's to a 4D dataset. The dataset
  /// will be expanded along the 0th dimension. By default, the size of the
  /// vector must equal the size of the 1st dimension of the dataset, andthe
  /// shape of the SDMs (nrows, ncols), must match the sizes of the 2nd and 2nd
  /// dimensions of the dataset. For transpose=true, the reverse must
  ///  be true of the SDMs.
  template <typename T>
  void append_vector_matrix(
      const String &dset_name,
      const std::vector<Teuchos::SerialDenseMatrix<int, T> > &data,
      const bool &transpose = false);
  ...
  void get_matrix(const std::string &dset_name,
                  Teuchos::SerialDenseMatrix<int, T> &matrix, const int &index,
                  const bool &transpose = false) const;
  /// Read the 3D slice at the index into the 0th dimension of the 4D dataset at
  /// ds_name. Currently this involves a wasteful copy to do the transpose and
  /// is intended only for purposes of testing
  template <typename T>
  ...
  H5::DataSet create_dataset(
      ...
      const H5::DSetCreatPropList &create_plist = H5::DSetCreatPropList(),
      const H5::DSetAccPropList &access_plist = H5::DSetAccPropList()) const;

  /// Create a group
  H5::Group create_group(const H5::H5Location &loc,
                         const std::string &name) const;
  /// Create a soft link
  void create_softlink(const String &link_location,
 ...
 protected:
  ...
  template <typename T>
  H5::Attribute create_attribute(const String &location, const String &label,
                                 const T &data);
...
void HDF5IOHelper::store_scalar(const std::string &dset_name, const T &val) {
  ...
  H5::DataSet dataset(
      create_dataset(h5File, dset_name, h5_file_dtype(val), dataspace));
...
/// Store matrix (2D) information to a dataset
template <typename T>
void HDF5IOHelper::store_matrix(
...
void HDF5IOHelper::store_matrix(const std::string &dset_name,
  ...
  if (transpose) {
    ...
    // sequentially grab rows of the matrix in memory and write them into
    // columns of the dataset in file.
    ...
    m_start[1] = f_start[0] =
        0;  // iterate over rows in memory/columns in the file
...
template <typename T>
void HDF5IOHelper::set_scalar(const String &dset_name, const T &data,
                              const int &index) {
...
void HDF5IOHelper::set_scalar(const String &dset_name, H5::DataSet &ds,
  ...
  if (f_space.getSimpleExtentNdims() != 1) {
    ...
    throw std::runtime_error(
        String("Attempt to insert element into a non-1D datasset ") +
        dset_name + " failed");
  }
  hsize_t dim;
  ...
  if (index < 0 || index >= dim) {
    flush();
    throw std::runtime_error(String("Attempt to insert element into ") +
...
template <typename T>
void HDF5IOHelper::set_vector(const String &dset_name, const T &data,
...
void HDF5IOHelper::set_vector(const String &dset_name, H5::DataSet &ds,
  ...
  if (f_space.getSimpleExtentNdims() != 2) {
    flush();
    throw std::runtime_error(
        String("Attempt to insert row or column into non-2D dataset ") +
        dset_name + " failed");
  }
  hsize_t dims[2];  // assume rank == 2
  f_space.getSimpleExtentDims(dims);
  int len = length(data);  // length is a free function defined in HDF5_IO.hpp
                           // that is overloaded/templated to return the length
                           // of SDVs and std::vectors
  if (row) {
    if (dims[1] != len) {
      flush();
      throw std::runtime_error(
          String("Attempt to insert row into  ") + dset_name +
          " failed; length of data is " + std::to_string(len) +
          " and number of DS columns is " + std::to_string(dims[1]));
    } else if (index >= dims[0] || index < 0) {
      flush();
      throw std::runtime_error(
          String("Attempt to insert row into ") + dset_name +
          " failed; requested index is " + std::to_string(index) +
          " but must be > 0 and < " + std::to_string(dims[0]));
    }
  } else {
    if (dims[0] != len) {
      flush();
      throw std::runtime_error(
          String("Attempt to insert column into  ") + dset_name +
          " failed; length of data is " + std::to_string(len) +
          " and number of DS rows is " + std::to_string(dims[0]));
    } else if (index >= dims[1] || index < 0) {
      flush();
      throw std::runtime_error(
          String("Attempt to insert column into  ") + dset_name +
          " failed; requested index is " + std::to_string(index) +
          " but must be > 0 and < " + std::to_string(dims[1]));
    }
  }
  hsize_t f_count[2], f_start[2],
      m_dim[1];  // f_count and f_start are used to index into the
...
void HDF5IOHelper::set_matrix(const String &dset_name,
                              const Teuchos::SerialDenseMatrix<int, T> &data,
                              const int &index, const bool &transpose) {
  auto ds_iter = datasetCache.find(dset_name);
  if (ds_iter != datasetCache.end())
...
void HDF5IOHelper::set_matrix(const String &dset_name, H5::DataSet &ds,
  ...
  if (f_dataspace.getSimpleExtentNdims() != 3) {
    flush();
    throw std::runtime_error(
        String("Attempt to insert matrix into non-3D dataset ") + dset_name +
        " failed");
  }
  ...
  if (index >= f_dims[0]) {
    flush();
    throw std::runtime_error(
        String("Attempt to insert matrix into  ") + dset_name +
        " failed; requested index greater than 0th dimension " + "of dataset.");
  }
  if ((transpose && !(f_dims[2] == num_rows && f_dims[1] == num_cols)) ||
    ...
    throw std::runtime_error(
        String("Attempt to insert matrix into  ") + dset_name +
        " failed; matrix dimensions do not match " + "dataset dimensions.");
  }
  ...
  if (transpose) {
    ...
    m_dataspace.setExtentSimple(2, m_dims);
    hsize_t m_start[2],
        f_start[3];  // "start" in the C++ API is "offset" in the C API.
    f_start[0] = index;
...
template <typename T>
void HDF5IOHelper::set_vector_matrix(
    const String &dset_name,
    const std::vector<Teuchos::SerialDenseMatrix<int, T> > &data,
    const int &index, const bool &transpose) {
  auto ds_iter = datasetCache.find(dset_name);
...
void HDF5IOHelper::set_vector_matrix(
  ...
  if (f_dataspace.getSimpleExtentNdims() != 4) {
    flush();
    throw std::runtime_error(
        String("Attempt to insert vector-matrix into non-4D dataset ") +
        dset_name + " failed");
  }
  ...
  if (index >= f_dims[0]) {
    flush();
    throw std::runtime_error(
        String("Attempt to insert vector-matrix into  ") + dset_name +
        " failed; requested index greater than 0th dimension " + "of dataset.");
  }
  ...
  if (transpose) {
    ...
    m_dataspace.setExtentSimple(2, m_dims);
    hsize_t m_start[2] = {
        0, 0};  // "start" in the C++ API is "offset" in the C API.
    hsize_t f_start[4] = {hsize_t(index), 0, 0, 0};
    hsize_t m_count[2] = {hsize_t(num_cols), 1};
...
template <typename T>
void HDF5IOHelper::set_vector_scalar_field(const String &dset_name,
                                           const T &data,
                                           const String &field_name) {
  auto ds_iter = datasetCache.find(dset_name);
  if (ds_iter != datasetCache.end())
...
void HDF5IOHelper::set_vector_vector_field(const String &dset_name,
                                           const T &data, const size_t length,
                                           const String &field_name) {
  ...
  // separate functions for scalar and vector fields isn't really necessary,
  // because we could detect whether a field is a scalar or array.  But the docs
  // are a little unclear here and I don't want to spend an hour figuring it
  // out.
  auto ds_iter = datasetCache.find(dset_name);
  if (ds_iter != datasetCache.end())
    set_vector_vector_field(dset_name, ds_iter->second, data, length,
                            field_name);
  else {
    H5::DataSet ds = h5File.openDataSet(dset_name);
...
void HDF5IOHelper::append_scalar(const String &dset_name, const T &data) {
  ...
  if (f_space.getSimpleExtentNdims() != 1) {
    flush();
    throw std::runtime_error(
        String("Attempt to append element to a non-1D datasset ") + dset_name +
        " failed");
  }
  hsize_t dim, maxdim;
  ...
  if (maxdim != H5S_UNLIMITED) {
    flush();
    throw std::runtime_error(
        String("Attempt to append element to a fixed-sized datasset ") +
        dset_name + " failed");
  }
  ++dim;
  ds.extend(&dim);
...
/// Append a vector as a row or column to a 2D dataset
template <typename T>
void HDF5IOHelper::append_vector(const String &dset_name, const T &data,
                                 const bool &row) {
  // 1. open the dataset
  // 2. discover the rank (must be 2) and dimensions
  ...
  if (f_space.getSimpleExtentNdims() != 2) {
    flush();
    throw std::runtime_error(
        String("Attempt to append row or column into non-2D dataset ") +
        dset_name + " failed");
  }
...
void HDF5IOHelper::append_matrix(const String &dset_name,
  ...
  if (f_space.getSimpleExtentNdims() != 3) {
    flush();
    throw std::runtime_error(
        String("Attempt to append matrix to non-3D dataset ") + dset_name +
        " failed");
  }
...
///  be true of the SDMs.
template <typename T>
void HDF5IOHelper::append_vector_matrix(
    const String &dset_name,
    const std::vector<Teuchos::SerialDenseMatrix<int, T> > &data,
    const bool &transpose) {
  // 1. open the dataset
  ...
  if (f_space.getSimpleExtentNdims() != 4) {
    flush();
    throw std::runtime_error(
        String("Attempt to append vector-matrix to non-4D dataset ") +
        dset_name + " failed");
  }
...
void HDF5IOHelper::read_matrix(const std::string &dset_name,
  ...
  if (transpose) {
    ...
    for (int ii = 0; ii < dims[0]; ++ii)
      for (int jj = 0; jj < dims[1]; ++jj) matrix(ii, jj) = tmp(jj, ii);
  }
  return;
}
...
void HDF5IOHelper::get_matrix(const std::string &dset_name,
  ...
  if (index >= dims[0] || index < 0) {
    ...
    throw std::runtime_error(String("Error while getting matrix ") +
                             "slice from 3D dataset " + dset_name +
                             ": index out of range.");
  }
  hsize_t f_start[3] = {hsize_t(index), 0, 0};
  ...
  if (transpose) {
    ...
    dataset.read(&matrix(0, 0), h5_mem_dtype(matrix(0, 0)), m_dataspace,
                 f_dataspace);
  } else {
    Teuchos::SerialDenseMatrix<int, T> tmp(dims[2], dims[1]);
    dataset.read(tmp.values(), h5_mem_dtype(tmp(0, 0)), m_dataspace,
                 f_dataspace);
    matrix.shapeUninitialized(dims[1], dims[2]);
    for (int ii = 0; ii < dims[1]; ++ii)
      for (int jj = 0; jj < dims[2]; ++jj) matrix(ii, jj) = tmp(jj, ii);
  }
  return;
}

/// Read the 3D slice at the index into the 0th dimension of the 4D dataset at
/// ds_name. Currently this involves a wasteful copy to do the transpose and is
/// intended only for purposes of testing
template <typename T>
void HDF5IOHelper::get_vector_matrix(
    const std::string &dset_name,
    std::vector<Teuchos::SerialDenseMatrix<int, T> > &data, const int &index,
    const bool &transpose) const {
  if (!exists(dset_name)) {
    Cerr << "\nError: HDF5 file \"" << fileName << "\""
         << " does not contain data path \"" << dset_name << "\"" << std::endl;
  ...
  if (index >= dims[0] || index < 0) {
    ...
    throw std::runtime_error(String("Error while getting vector-matrix ") +
                             "slice from 4D dataset " + dset_name +
                             ": index out of range.");
  }
  data.resize(dims[1]);
  hsize_t f_start[4] = {hsize_t(index), 0, 0, 0};
  ...
  if (transpose) {
    for (f_start[1] = 0; f_start[1] < dims[1]; ++f_start[1]) {
      ...
      f_dataspace.selectHyperslab(H5S_SELECT_SET, f_count, f_start);
      dataset.read(matrix.values(), h5_mem_dtype(matrix(0, 0)), m_dataspace,
                   f_dataspace);
    }
  } else {
    ...
    for (f_start[1] = 0; f_start[1] < dims[1]; ++f_start[1]) {
      ...
      for (int ii = 0; ii < dims[2]; ++ii)
        for (int jj = 0; jj < dims[3]; ++jj) matrix(ii, jj) = tmp(jj, ii);
    }
  }
  return;
}
...
template <typename T>
H5::Attribute HDF5IOHelper::create_attribute(const String &location,
                                             const String &label,
                                             const T &value) {
  if (!exists(location)) {
  ...
  if (type == H5O_TYPE_GROUP) {
    ...
    return dataset.createAttribute(label, h5_file_dtype(value),
                                   H5::DataSpace());
  } else {
    flush();
    throw std::runtime_error(String("HDF5 object at ") + location +
                             " is of unhandled type.");
  }
}
