...
static const char rcsId[] =
    "@(#) $Id: NonDSampling.cpp 7036 2010-10-22 23:20:24Z mseldre $";
...
NonDSampling::NonDSampling(ProblemDescDB& problem_db,
                           ParallelLibrary& parallel_lib,
                           std::shared_ptr<Model> model)
    : NonD(problem_db, parallel_lib, model),
      seedSpec(probDescDB.get_int("method.random_seed")),
      randomSeed(seedSpec),
      samplesSpec(probDescDB.get_int("method.samples")),
      samplesRef(samplesSpec),
      ...
      numLHSRuns(0),
      samplerDriver((problem_db.get_ushort("method.sample_type") ==
                     SUBMETHOD_LOW_DISCREPANCY_SAMPLING)
                        ? std::unique_ptr<SamplerDriver>(
                              std::make_unique<LDDriverAdapter>(problem_db))
                        : std::unique_ptr<SamplerDriver>(
                              std::make_unique<LHSDriverAdapter>())) {
  ...
  if (wilksFlag) {
    ...
    if (wilksAlpha <=
        0.0)  // Assign a default if probability_levels unspecified
      wilksAlpha = 0.95;

    wilksBeta = probDescDB.get_real("method.confidence_level");
    if (wilksBeta <= 0.0)  // Assign a default if probability_levels unspecified
      wilksBeta = 0.95;
    numSamples = compute_wilks_sample_size(wilksOrder, wilksAlpha, wilksBeta,
                                           wilks_twosided);
...
NonDSampling::NonDSampling(unsigned short method_name,
    ...
    : NonD(method_name, model),
      ...
      sampleRanksMode(IGNORE_RANKS),
      varyPattern(vary_pattern),
      backfillDuplicates(false),
      numLHSRuns(0),
  ...
  if (!sampleType) sampleType = SUBMETHOD_LHS;
...
NonDSampling::NonDSampling(unsigned short sample_type, size_t samples, int seed,
                           const String& rng, const RealVector& lower_bnds,
                           const RealVector& upper_bnds)
    : NonD(RANDOM_SAMPLING, lower_bnds, upper_bnds),
      seedSpec(seed),
      randomSeed(seed),
      samplesSpec(samples),
      samplesRef(samples),
...
/** This alternate constructor defines allSamples from an incoming
    sample matrix. */
NonDSampling::NonDSampling(std::shared_ptr<Model> model,
                           const RealMatrix& sample_matrix)
    : NonD(LIST_SAMPLING, model),
      seedSpec(0),
      randomSeed(0),
      samplesSpec(sample_matrix.numCols()),
      sampleType(SUBMETHOD_DEFAULT),
      wilksFlag(false),
      samplesIncrement(0),
      stdRegressionCoeffs(false),
      toleranceIntervalsFlag(false),
  ...
  // not used but included for completeness
  if (numSamples) maxEvalConcurrency *= numSamples;
}

NonDSampling::~NonDSampling() {}

void NonDSampling::transform_samples(
    Pecos::ProbabilityTransformation& nataf,
    RealMatrix& sample_matrix,  // size_t num_samples,
    SizetMultiArrayConstView src_cv_ids, SizetMultiArrayConstView tgt_cv_ids,
    bool x_to_u) {
...
void NonDSampling::get_parameter_sets(std::shared_ptr<Model> model,
  ...
  switch (samplingVarsMode) {
    ...
    case EPISTEMIC_UNCERTAIN_UNIFORM: {
      ...
      if (samplingVarsMode == ACTIVE_UNIFORM ||
        ...
        // view), or from ALL lower/upper bounds (with model in ALL view). loss
        // of sampleRanks control is OK since NonDIncremLHS uses ACTIVE mode. TO
        // DO: add support for uniform discrete
        samplerDriver->generate_uniform_samples(
            ModelUtils::continuous_lower_bounds(*model),
            ModelUtils::continuous_upper_bounds(*model), corr, num_samples,
            design_matrix);
      } else if (samplingVarsMode == ALL_UNIFORM) {
        // sample uniformly from ALL lower/upper bnds with model in distinct
        // view. loss of sampleRanks control is OK since NonDIncremLHS uses
        // ACTIVE mode. TO DO: add support for uniform discrete
        samplerDriver->generate_uniform_samples(
            ModelUtils::all_continuous_lower_bounds(*model),
            ModelUtils::all_continuous_upper_bounds(*model), corr, num_samples,
            design_matrix);
        ...
        const RealVector& all_c_l_bnds =
            ModelUtils::all_continuous_lower_bounds(*model);
        const RealVector& all_c_u_bnds =
            ModelUtils::all_continuous_upper_bounds(*model);
        RealVector uncertain_c_l_bnds(
            Teuchos::View, const_cast<Real*>(&all_c_l_bnds[start_acv]),
            num_acv);
        RealVector uncertain_c_u_bnds(
            Teuchos::View, const_cast<Real*>(&all_c_u_bnds[start_acv]),
            num_acv);
        // loss of sampleRanks control is OK since NonDIncremLHS uses ACTIVE
        // mode TO DO: add support for uniform discrete
        samplerDriver->generate_uniform_samples(uncertain_c_l_bnds,
                                                uncertain_c_u_bnds, corr,
    ...
    case ACTIVE: {  // utilize model view to sample active variables
      ...
      if (backfillDuplicates)
        samplerDriver->generate_unique_samples(
            model, num_samples, design_matrix, sampleRanks,
      ...
      // sampleRanks remains empty. See LHSDriver::generate_unique_samples()
        ...
        samplerDriver->generate_samples(model, num_samples, design_matrix,
                                        sampleRanks, mv_dist.active_variables(),
                                        mv_dist.active_correlations());
      break;
    }
    ...
    case ALL: {
      ...
      Pecos::MultivariateDistribution& mv_dist =
          model->multivariate_distribution();
      if (backfillDuplicates)
        samplerDriver->generate_unique_samples(model, num_samples,
...
void NonDSampling::get_parameter_sets(const RealVector& lower_bnds,
                                      const RealVector& upper_bnds) {
...
void NonDSampling::get_parameter_sets(const RealVector& means,
                                      const RealVector& std_devs,
                                      const RealVector& lower_bnds,
                                      const RealVector& upper_bnds,
                                      RealSymMatrix& correl) {
  initialize_sample_driver(true, numSamples);
  samplerDriver->generate_normal_samples(
      means, std_devs, lower_bnds, upper_bnds, correl, numSamples, allSamples);
}

// Map the active variables from vars to sample_vars (a column in allSamples)
void NonDSampling::sample_to_variables(const Real* sample_vars, Variables& vars,
                                       Model& model) {
  ...
  if (vars.is_null()) vars = Variables(model.current_variables().shared_data());
  ...
  switch (samplingVarsMode) {
    ...
    case ACTIVE_UNIFORM:
      switch (vars.view().first) {
        case RELAXED_ALL:
        case MIXED_ALL:
          vars_mode = ALL_UNIFORM;
          break;
        case RELAXED_DESIGN:
        case MIXED_DESIGN:
          vars_mode = DESIGN /*_UNIFORM*/;
          break;
        case RELAXED_UNCERTAIN:
        case MIXED_UNCERTAIN:
          vars_mode = UNCERTAIN_UNIFORM;
          break;
        case RELAXED_ALEATORY_UNCERTAIN:
        case MIXED_ALEATORY_UNCERTAIN:
          vars_mode = ALEATORY_UNCERTAIN_UNIFORM;
          break;
        case RELAXED_EPISTEMIC_UNCERTAIN:
        case MIXED_EPISTEMIC_UNCERTAIN:
          vars_mode = EPISTEMIC_UNCERTAIN_UNIFORM;
          break;
        case RELAXED_STATE:
        case MIXED_STATE:
          vars_mode = STATE /*_UNIFORM*/;
          break;
      }
      break;
    default:
      vars_mode = samplingVarsMode;
      break;
  }

  size_t cv_index, num_cv, div_index, num_div, dsv_index, num_dsv, drv_index,
      num_drv, samp_index = 0;
  switch (vars_mode) {
    case DESIGN:
    ...
    //   cv_index = div_index = dsv_index = drv_index = 0;
    //   svd.design_counts(num_cv, num_div, num_dsv, num_drv);
    //   sample_to_cv_type(sample_vars, vars, cv_index, num_cv, div_index,
    //   num_div,
...
void NonDSampling::variables_to_sample(const Variables& vars,
                                       Real* sample_vars) {
...
void NonDSampling::mode_counts(const Variables& vars, size_t& cv_start,
                               size_t& num_cv, size_t& div_start,
                               size_t& num_div, size_t& dsv_start,
                               size_t& num_dsv, size_t& drv_start,
  ...
  switch (samplingVarsMode) {
    ...
    case STATE:
      ...
      break;
    // case STATE_UNIFORM:
    //   // continuous design + aleatory + epistemic vars define starting
    //   indices svd.design_counts(cv_start, dummy, dummy, dummy);
    //   svd.aleatory_uncertain_counts(num_cv, dummy, dummy, dummy);
    //   cv_start += num_cv;
    //   svd.epistemic_uncertain_counts(num_cv, dummy, dummy, dummy);
    //   cv_start += num_cv;
    ...
    case ACTIVE:
      ...
      num_div = vars.div();
      dsv_start = vars.dsv_start();
      num_dsv = vars.dsv();
      drv_start = vars.drv_start();
      num_drv = vars.drv();
      break;
    case ACTIVE_UNIFORM:
      cv_start = vars.cv_start();
...
void NonDSampling::mode_bits(const Variables& vars, BitArray& active_vars,
                             BitArray& active_corr) const {
  const SharedVariablesData& svd = vars.shared_data();
  size_t num_v = vars.tv(), num_cv, num_div, num_dsv, num_drv, num_dv, num_auv,
         num_euv;
  svd.design_counts(num_cv, num_div, num_dsv, num_drv);
  ...
  switch (samplingVarsMode) {
    case DESIGN:
      ...
      assign_value(active_vars, true, 0, num_dv);
      break;
    case ALEATORY_UNCERTAIN:
      active_vars = active_corr;
      break;
    ...
    case ALL:
      active_vars.clear();
      break;  // no subset, all are active
    // case ALEATORY_UNCERTAIN_UNIFORM:
    // case EPISTEMIC_UNCERTAIN_UNIFORM:
    // case UNCERTAIN_UNIFORM:
    default:
...
void NonDSampling::initialize_sample_driver(bool write_message,
                                            size_t num_samples) {
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "Initialize: numLHSRuns = " << numLHSRuns
         << " seedSpec = " << seedSpec << " randomSeed = " << randomSeed
         << " varyPattern = " << varyPattern << std::endl;
  ...
  else if (seed_updated())  // random_seed_sequence advance
  {
    ...
    seed_assigned = true;
             ...
             rngName == "rnum2")  // vary pattern by advancing seed
  {
    samplerDriver->advance_seed_sequence();
    seed_advanced = true;
  } else if (!varyPattern)  // reset orig / machine-generated (don't continue
                            // RNG)
  {
    samplerDriver->seed(randomSeed);
    seed_assigned = true;
  ...
  if (write_message) {
    ...
    if (seed_assigned) {
      ...
      if (seedSpec)
        Cout << " Seed (sequence from user-specified) = ";
      else
        Cout << " Seed (sequence from system-generated) = ";
...
void NonDSampling::active_set_mapping() {
  ...
  for (i = 0; i < numFunctions; ++i) {
    ...
    if (respLevelTarget == RELIABILITIES)
      for (j = 0; j < rl_len;
        ...
        if (final_asv[cntr + j] & 2) {
          moment1_grad = moment2_grad = qoi_fn = true;
          break;
        }
    cntr += rl_len + pl_len;
    ...
    if (qoi_grad /*|| useDerivs*/)
      sampler_asv[i] |= 2;  // future aggregation...
  }
...
void NonDSampling::compute_statistics(const RealMatrix& vars_samples,
                                      const IntResponseMap& resp_samples) {
  StringMultiArrayConstView
      acv_labels = ModelUtils::all_continuous_variable_labels(*iteratedModel),
      adiv_labels =
          ModelUtils::all_discrete_int_variable_labels(*iteratedModel),
      adsv_labels =
          ModelUtils::all_discrete_string_variable_labels(*iteratedModel),
      adrv_labels =
          ModelUtils::all_discrete_real_variable_labels(*iteratedModel);
  size_t cv_start, num_cv, div_start, num_div, dsv_start, num_dsv, drv_start,
      num_drv;
  ...
  StringMultiArrayConstView
      ...
      div_labels = adiv_labels[boost::indices[idx_range(div_start,
                                                        div_start + num_div)]],
      dsv_labels = adsv_labels[boost::indices[idx_range(dsv_start,
                                                        dsv_start + num_dsv)]],
      drv_labels = adrv_labels[boost::indices[idx_range(drv_start,
                                                        drv_start + num_drv)]];
  ...
  if (epistemicStats) {               // Epistemic/mixed
    ...
    if (totalLevelRequests) compute_level_mappings(resp_samples);
  }
  ...
  if (toleranceIntervalsFlag) {
    computeDSTIEN(resp_samples, tiCoverage, 1. - tiConfidenceLevel,
...
void NonDSampling::compute_intervals(RealRealPairArray& extreme_fns,
  ...
  if (resultsDB.active()) {
    ...
    resultsDB.insert(run_identifier(), resultsNames.extreme_values, extreme_fns,
                     md);
  }
}
...
void NonDSampling::compute_moments(const IntResponseMap& samples,
  ...
  if (mom_fns) {
    compute_moments(fn_samples, sample_counts, moment_stats, moments_type,
                    labels);
...
void NonDSampling::compute_moments(const RealVectorArray& fn_samples,
                                   SizetArray& sample_counts,
  ...
  for (i = 0; i < num_qoi; ++i) {
    ...
    if (num_samp != num_obs) {
      std::cout << "In NonDSampling::compute_moments(2)"
                << ", Warning: sampling statistics for quantity " << i + 1
                << " omit " << num_obs - num_samp
                << " failed evaluations out of " << num_obs << " samples."
                << std::endl;
...
void NonDSampling::compute_moments(const RealMatrix& fn_samples,
                                   RealMatrix& moment_stats,
                                   short moments_type) {
...
void NonDSampling::compute_moment_confidence_intervals(
  ...
  if (moment_conf_ints.empty()) moment_conf_ints.shapeUninitialized(4, num_qoi);
  ...
  for (i = 0; i < num_qoi; ++i) {
    if (sample_counts[i] > 1) {
      ...
      if (mean == qnan || std_dev == qnan || var == qnan)
        for (size_t j = 0; j < 4; ++j) moment_ci_i[j] = qnan;
      else {
        ...
        Real mean_ci_delta =
            std_dev * bmth::quantile(t_dist, 0.975) / std::sqrt(ns);
      ...
      for (size_t j = 0; j < 4; ++j) moment_conf_ints(j, i) = 0.;
  }
}

void NonDSampling::compute_moment_gradients(const RealVectorArray& fn_samples,
  ...
  for (q = 0; q < numFunctions; ++q) {
    ...
    Pecos::accumulate_moment_gradients(
        fn_samples, grad_samples, q, moments_type, moment_stats(0, q),
        moment_stats(1, q), moment_grads[m1_index], moment_grads[m2_index]);
    ...
    if (moments_type) {
      ...
      cntr += 1 + requestedRespLevels[q].length() +
              requestedProbLevels[q].length() + requestedRelLevels[q].length() +
              requestedGenRelLevels[q].length();
    }
  }
}

void NonDSampling::archive_moments(size_t inc_id) {
  ...
  md_moments["Row Labels"] =
      (finalMomentsType == Pecos::CENTRAL_MOMENTS)
          ? make_metadatavalue("Mean", "Variance", "3rdCentral", "4thCentral")
          : make_metadatavalue("Mean", "Standard Deviation", "Skewness",
                               "Kurtosis");
  ...
  for (int i = 0; i < numFunctions; ++i) {
    ...
    if (finalMomentsType == Pecos::CENTRAL_MOMENTS)
      ...
      scales.emplace(
          0, StringScale("moments",
                         {"mean", "std_deviation", "skewness", "kurtosis"},
                         ScaleScope::SHARED));
    // extract column or row of moment_stats
    resultsDB.insert(
        run_identifier(), location,
        Teuchos::getCol<int, double>(Teuchos::View,
                                     *const_cast<RealMatrix*>(&momentStats), i),
        scales);
  }
}

void NonDSampling::archive_moment_confidence_intervals(size_t inc_id) {
  if (!resultsDB.active()) return;
  ...
  // archive in HDF5. Store in a 2d dataset with mean and var/stdev on the 1st
  // dimension and lower and upper bounds on the 2nd dimension
  DimScaleMap scales;
...
void NonDSampling::archive_extreme_responses(size_t inc_id) {
  const StringArray& labels = ModelUtils::response_labels(*iteratedModel);
  StringArray location;
  if (inc_id) location.push_back(String("increment:") + std::to_string(inc_id));
...
int NonDSampling::compute_wilks_sample_size(unsigned short order, Real alpha,
  ...
  if (twosided) {
    n = 2.0 * rorder;
    while (boost::math::ibeta<Real>(n - 2.0 * rorder + 1.0, 2.0 * rorder,
                                    alpha) > 1.0 - beta)
      n += 1.0;
  } else {
...
Real NonDSampling::compute_wilks_residual(unsigned short order, int nsamples,
  ...
  if (twosided)
    return boost::math::ibeta<Real>((Real)nsamples - 2.0 * rorder + 1.0,
                                    2.0 * rorder, alpha) -
           (1.0 - beta);
  else
    return boost::math::ibeta<Real>(rorder, (Real)nsamples - rorder + 1.0,
                                    1 - alpha) -
           beta;
}

Real NonDSampling::compute_wilks_alpha(unsigned short order, int nsamples,
                                       Real beta, bool twosided) {
  Real rorder = (Real)order;
  ...
  Real resid_l =
      compute_wilks_residual(order, nsamples, alpha_l, beta, twosided);
  Real resid_u =
      compute_wilks_residual(order, nsamples, alpha_u, beta, twosided);
  if (resid_l * resid_u > 0)
    throw std::runtime_error(
        "Cannot obtain valid bounds for wilks alpha bisection.");
  ...
  while (std::fabs(resid) > tol) {
    ...
    // Real resid_plus  = compute_wilks_residual(order, nsamples, alpha+eps,
    // beta, twosided); Real resid_minus = compute_wilks_residual(order,
    // nsamples, alpha-eps, beta, twosided); alpha -= resid/(
    // (resid_plus-resid_minus)/(2.0*eps) );
  ...
  // Cout << "Converged: alpha = " << alpha << ", resid = " << resid <<
  // std::endl;

  return alpha;
}

Real NonDSampling::compute_wilks_beta(unsigned short order, int nsamples,
                                      Real alpha, bool twosided) {
  Real rorder = (Real)order;

  Real beta_l = get_wilks_beta_min();  // initial lower bound
  Real beta_u = get_wilks_beta_max();  // initial upper bound
  Real resid_l =
      compute_wilks_residual(order, nsamples, alpha, beta_l, twosided);
  Real resid_u =
      compute_wilks_residual(order, nsamples, alpha, beta_u, twosided);
  if (resid_l * resid_u > 0)
    throw std::runtime_error(
        "Cannot obtain valid bounds for wilks beta bisection.");
...
void NonDSampling::compute_level_mappings(const IntResponseMap& samples) {
  ...
  IntRespMCIter s_it;
  std::multiset<Real>::iterator ss_it;
  const ShortArray& final_asv = finalStatistics.active_set_request_vector();
  ...
  for (i = 0; i < numFunctions; ++i) {
    ...
    if (pl_len || gl_len) {  // sort samples array for p/beta* -> z mappings
      ...
      if (pdfOutput) {
        min = *sorted_samples.begin();
        max = *(--sorted_samples.end());
      }
      ...
      if (rl_len && respLevelTarget != RELIABILITIES) {
        ...
        bins.assign(rl_len + 1, 0);
        ss_it = sorted_samples.begin();
        for (j = 0; j < rl_len; ++j)
          while (ss_it != sorted_samples.end() &&
                 *ss_it <= req_rl_i[j])  // p(g<=z)
          {
            ++bins[j];
            ++ss_it;
          }
      ...
      for (s_it = samples.begin(); s_it != samples.end(); ++s_it) {
        ...
        if (std::isfinite(sample)) {
          ...
          if (!found) ++bins[rl_len];
        }
      }
    }
    if (pdfOutput) {
      extremeValues[i].first = min;
      extremeValues[i].second = max;
    }
    ...
    if (rl_len) {
      switch (respLevelTarget) {
        case PROBABILITIES:
        case GEN_RELIABILITIES:  // z -> p/beta* (from binning)
          bin_accumulator = 0;
        ...
        case RELIABILITIES: {  // z -> beta (from moment projection)
          ...
          Real stdev =
              (central_mom) ? std::sqrt(momentStats(1, i)) : momentStats(1, i);
          ...
          for (j = 0; j < rl_len; j++, ++cntr) {
            ...
            if (!Pecos::is_small(stdev))
              computedRelLevels[i][j] =
                  (cdfFlag) ? (mean - z_bar) / stdev : (z_bar - mean) / stdev;
            else
              computedRelLevels[i][j] =
            ...
            // *** beta gradient
            if (final_asv[cntr] & 2) {
              RealVector beta_grad =
                  finalStatistics.function_gradient_view(cntr);
              if (!Pecos::is_small(stdev)) {
                for (k = 0; k < num_deriv_vars; ++k) {
                  Real stdev_grad = (central_mom) ? mom2_grad[k] / (2. * stdev)
                                                  : mom2_grad[k];
                  Real dratio_dx =
                      (stdev * mean_grad[k] - (mean - z_bar) * stdev_grad) /
                      std::pow(stdev, 2);
    ...
    for (j = 0; j < pl_len + gl_len; j++, ++cntr) {  // p/beta* -> z
      Real p = (j < pl_len) ? requestedProbLevels[i][j]
                            : Pecos::NormalRandomVariable::std_cdf(
                                  -requestedGenRelLevels[i][j - pl_len]);
      ...
      if (cdf_incr_id < 1.) {  // extrapolate left of min sample using 1st slope
        lo_id = 1.;
        extrapolated_mappings = true;
        Cerr << "Warning: extrapolation required for response " << i + 1;
        if (j < pl_len)
          Cerr << " for probability level " << j + 1 << ".\n";
        else
          Cerr << " for generalized reliability level " << j + 1 - pl_len
               << ".\n";
      ...
      Real z, z_lo = *ss_it;
      ++ss_it;
      if (ss_it == sorted_samples.end())
        z = z_lo;
      else
        z = z_lo + (cdf_incr_id - lo_id) * (*ss_it - z_lo);
      if (j < pl_len)
        computedRespLevels[i][j] = z;
      else
        computedRespLevels[i][j + bl_len] = z;
    }
    if (bl_len) {
      Real mean = momentStats(0, i);
      Real stdev = (finalMomentsType == Pecos::CENTRAL_MOMENTS)
                       ? std::sqrt(momentStats(1, i))
                       : momentStats(1, i);
      if (!momentGrads.empty()) {
        int i2 = 2 * i;
        mean_grad = Teuchos::getCol(Teuchos::View, momentGrads, i2);
      ...
      for (j = 0; j < bl_len; j++, ++cntr) {
        ...
        computedRespLevels[i][j + pl_len] =
            (cdfFlag) ? mean - beta_bar * stdev : mean + beta_bar * stdev;
        // *** z gradient
        if (final_asv[cntr] & 2) {
          RealVector z_grad = finalStatistics.function_gradient_view(cntr);
          for (k = 0; k < num_deriv_vars; ++k) {
            Real stdev_grad =
                (central_mom) ? mom2_grad[k] / (2. * stdev) : mom2_grad[k];
...
void NonDSampling::print_statistics(std::ostream& s) const {
  ...
  if (!subIteratorFlag) {
    nonDSampCorr.print_correlations(
  ...
  if (wilksFlag) {
    if (epistemicStats) {
      Cerr << "Warning: Wilks printing requested in conjunction with epstemic "
              "variables"
           << std::endl;
    }
  ...
  if (stdRegressionCoeffs) {
    if (epistemicStats) {
      Cerr << "Warning: std regression coefficients printing requested in "
              "conjunction with epstemic variables"
           << std::endl;
    }

    nonDSampCorr.print_std_regress_coeffs(
        s, iteratedModel->current_variables().ordered_labels(ACTIVE_VARS),
        ModelUtils::response_labels(*iteratedModel));
  }
  ...
  if (toleranceIntervalsFlag) {
    if (epistemicStats) {
      Cerr << "Warning: tolerance intervals printing requested in conjunction "
              "with epstemic variables"
           << std::endl;
    }
...
void NonDSampling::print_intervals(std::ostream& s, String qoi_type,
...
void NonDSampling::print_moments(std::ostream& s,
  ...
  if (print_cis && num_ci) {
    ...
    if (num_ci > 2) {
      if (moments_type == Pecos::CENTRAL_MOMENTS)
        s << std::setw(width + 1) << "LowerCI_Variance" << std::setw(width + 2)
          << "UpperCI_Variance";
      else
        s << std::setw(width + 1) << "LowerCI_StdDev" << std::setw(width + 2)
          << "UpperCI_StdDev";
    }
    s << '\n';
    for (i = 0; i < num_qoi; ++i) {
...
void NonDSampling::print_wilks_stastics(std::ostream& s) const {
  ...
  //   {
  //     Cout << "It #" << it2->first << "\t" << it2->second.function_value(i)
  //     << std::endl; sorted_resp.insert(it2->second.function_value(i));
  //   }
  //   int count = 0;
  //   for( it2 = allResponses.begin(), cit2 = sorted_resp.begin(); cit2 !=
  //   sorted_resp.end(); ++cit2, ++count, ++it2 )
  //     Cout << "It #" << count << "\t" << it2->second.function_value(i) <<
  //     "\t" << *cit2 << std::endl;
  // }

  bool wilks_twosided = (wilksSidedness == TWO_SIDED);

  size_t j, width = write_precision + 7, w2p2 = 2 * width + 2,
            w3p4 = 3 * width + 4;
  ...
  for (size_t fn_index = 0; fn_index < numFunctions; ++fn_index) {
    s << "\n\n"
      << "Wilks Statistics for " << (wilks_twosided ? "Two-" : "One-")
      << "Sided " << 100.0 * wilksBeta
      << "% Confidence Level, Order = " << wilksOrder << " for "
      << ModelUtils::response_labels(*iteratedModel)[fn_index] << ":\n\n";

    if (wilks_twosided) {
      s << "    Coverage Level     Lower Bound        Upper Bound     Number "
           "of Samples\n"
        << "    --------------  -----------------  -----------------  "
           "-----------------\n";
    } else {
      s << "    Coverage Level       "
        << (wilksSidedness == ONE_SIDED_UPPER ? "Upper" : "Lower")
    ...
    for (j = 0; j < num_prob_levels; ++j) {
      ...
      int num_samples = compute_wilks_sample_size(wilksOrder, prob_level,
                                                  wilksBeta, wilks_twosided);

      // Grab the first num_samples subset in sorted order (could also randomly
      // sample) - RWH
      ...
      if (wilks_twosided) s << "  " << min;
      s << "   "
        << ((wilks_twosided || wilksSidedness == ONE_SIDED_UPPER) ? max : min)
        << "        " << num_samples << '\n';
    }
  }
}

void NonDSampling::print_tolerance_intervals_statistics(std::ostream& s) const {
  ...
  s << "-----------------------------------------------------------------------"
    ...
    << " with coverage = " << std::fixed << std::setprecision(2)
    << 100.0 * tiCoverage << "%"
    << ", confidence level = " << std::fixed << std::setprecision(2)
    << 100.0 * tiConfidenceLevel << "%"
    << ", and " << tiNumValidSamples << " valid samples" << std::endl
    << "Double-sided tolerance interval equivalent normal statistics for each "
       "response function:"
    << std::endl
    << std::setw(response_label_width + width + 1) << "Sample Mean mu"
    << std::setw(width + 1) << "Sample Stdev s" << std::setw(width + 1)
    << "Stdev Mult. f" << std::setw(width + 1) << "LowerEnd=mu-f*s"
    << std::setw(width + 1) << "UpperEnd=mu+f*s" << std::setw(width + 1)
    << "Eq. Norm. Stdev" << std::endl
    << std::scientific << std::setprecision(write_precision);
  for (size_t i = 0; i < numFunctions; ++i) {
    s << std::setw(response_label_width)
      << ModelUtils::response_labels(*iteratedModel)[i] << ' '
      << std::setw(width) << tiDstienMus[i] << ' ' << std::setw(width)
      << tiSampleSigmas[i] << ' ' << std::setw(width)
      << tiDeltaMultiplicativeFactor << ' ' << std::setw(width)
      << tiDstienMus[i] - tiDeltaMultiplicativeFactor * tiSampleSigmas[i] << ' '
      << std::setw(width)
      << tiDstienMus[i] + tiDeltaMultiplicativeFactor * tiSampleSigmas[i] << ' '
      << std::setw(width) << tiDstienSigmas[i] << std::endl;
  }
}

void NonDSampling::archive_tolerance_intervals(size_t inc_id) {
  StringArray location;
  if (inc_id != 0) {
  ...
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    DimScaleMap scales;
    scales.emplace(0, StringScale("tolerance_intervals",
                                  {"sample_mean", "sample_stdev",
                                   "multiplicative_factor_f", "TI_lower_end",
                                   "TI_upper_end", "TI_equiv_normal_stdev"},
                                  ScaleScope::SHARED));
    ...
    resultsDB.insert(run_identifier(), location, tmpValues, scales);
  }

  AttributeArray ns_attr(
      {ResultAttribute<int>("valid_samples", tiNumValidSamples)});
  if (inc_id == 0) {
