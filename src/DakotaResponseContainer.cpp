...
ModelGroup::ModelGroup(const std::vector<unsigned short>& model_indices,
                       int num_qois, int num_moments, int num_replications)
    : model_indices_(model_indices),
      ...
      mean_accumulators_(
          num_moments,
          std::vector<std::vector<std::vector<MeanAccumulator<Real>>>>(
              model_indices.size(),
              std::vector<std::vector<MeanAccumulator<Real>>>(
                  num_qois,
                  std::vector<MeanAccumulator<Real>>(num_replications)))),
      covariance_accumulators_(
          num_moments,
          std::vector<std::vector<
              std::vector<std::vector<CovarianceAccumulator<Real>>>>>(
              model_indices.size(),
              std::vector<
                  std::vector<std::vector<CovarianceAccumulator<Real>>>>(
                  model_indices.size(),
...
void ModelGroup::addSample(const std::vector<Real>& model_evaluations,
                           int replication) {
  // Update accumulators
    ...
    for (int model = 0; model < model_indices_.size(); ++model) {
      for (int qoi = 0; qoi < num_qois_; ++qoi) {
        ...
        if (!std::isnan(value)) {  // or check out of bounds
          ...
          mean_accumulators_[moment][model][qoi][replication].addSample(
              value_pow);

          // Update covariance accumulator
          for (int other_model = 0; other_model <= model; ++other_model) {
            auto other_value =
                model_evaluations[model_indices_[other_model] * num_qois_ +
                                  qoi];
            if (!std::isnan(other_value)) {
              auto other_value_pow = std::pow(other_value, moment + 1);
              covariance_accumulators_[moment][model][other_model][qoi]
                                      [replication]
                                          .addSample(value_pow,
                                                     other_value_pow);
            }
...
// Gets the number of samples for a specific model, quantity of interest,
// moment, and replication.
//
// Args:
...
// Gets the mean for a specific model, quantity of interest, moment, and
// replication.
//
// Args:
//   model: The index of the model.
...
Real ModelGroup::getMean(int model, int qoi, int moment, int replication) {
  return mean_accumulators_[moment - 1][model][qoi][replication]
      .computeStatistic();
}

// Gets the covariance between two models for a specific quantity of interest,
// moment, and replication.
//
// Args:
...
Real ModelGroup::getCovariance(int model, int other_model, int qoi, int moment,
                               int replication) {
  return covariance_accumulators_[moment - 1][model][other_model][qoi]
                                 [replication]
                                     .computeStatistic();
}
...
void ModelGroup::reset() {
  for (int moment = 0; moment < num_moments_; ++moment) {
    for (int model = 0; model < model_indices_.size(); ++model) {
      for (int qoi = 0; qoi < num_qois_; ++qoi) {
        for (int replication = 0; replication < num_replications_;
             ++replication) {
...
ResponseContainer::ResponseContainer()
    : model_groups_()  // Initialize the vector of unique pointers to model
                       // groups
{}
...
//
// Args:
//   model_indices: A vector of indices representing the models in the new
//   group. num_qois: The number of quantities of interest (default is 1).
//   num_moments: The number of moments to be calculated (default is 1).
//   num_replications: The number of replications for the statistics (default is
//   1).
void ResponseContainer::addModelGroup(
    const std::vector<unsigned short>& model_indices, int num_qois,
    int num_moments, int num_replications) {
  // Create a new ModelGroup and add it to the vector
  model_groups_.emplace_back(std::make_unique<ModelGroup>(
      model_indices, num_qois, num_moments, num_replications));
}
...
void ResponseContainer::addSamples(IntIntResponse2DMap& samples,
                                   int replication) {
  for (const auto& grouped_samples : samples) {
    for (const auto& grouped_sample : grouped_samples.second) {
      ...
      model_groups_[grouped_samples.first]->addSample(function_values,
                                                      replication);
    }
  }
}

// Gets the mean for a specific group, model, quantity of interest, moment, and
// replication.
//
// Args:
...
// Returns:
//   The computed mean for the specified parameters.
Real ResponseContainer::getMean(int group, int model, int qoi, int moment,
                                int replication) {
  return model_groups_[group]->getMean(model, qoi, moment, replication);
}

// Gets the covariance between two models for a specific group, quantity of
// interest, moment, and replication.
//
// Args:
//   group: The index of the model group.
//   model: The index of the first model.
...
// Returns:
//   The computed covariance for the specified parameters.
Real ResponseContainer::getCovariance(int group, int model, int other_model,
                                      int qoi, int moment, int replication) {
  return model_groups_[group]->getCovariance(model, other_model, qoi, moment,
                                             replication);
}
...
int ResponseContainer::numGroups() { return model_groups_.size(); }

// Empties the response container of all stored samples.
void ResponseContainer::reset() {
  for (auto& model_group : model_groups_) model_group->reset();
}
