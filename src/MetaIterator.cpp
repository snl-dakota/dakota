...
static const char rcsId[] =
    "@(#) $Id: MetaIterator.cpp 6715 2010-04-02 21:58:15Z wjbohnh $";
...
void MetaIterator::check_model(const String& method_ptr,
...
void MetaIterator::allocate_by_pointer(const String& method_ptr,
  ...
  if (!the_model) the_model = Model::get_model(probDescDB, parallelLib);
...
void MetaIterator::allocate_by_name(const String& method_string,
                                    const String& model_ptr,
                                    std::shared_ptr<Iterator>& the_iterator,
                                    std::shared_ptr<Model>& the_model) {
...
IntIntPair MetaIterator::estimate_by_pointer(
...
IntIntPair MetaIterator::estimate_by_name(
    const String& method_string, const String& model_ptr,
    std::shared_ptr<Iterator>& the_iterator,
    std::shared_ptr<Model>& the_model) {
  // model instantiation is DB-based, iterator instantiation is not
...
void MetaIterator::post_run(std::ostream& s) {
  if (iterSched.lead_rank()) print_results(s);
}
