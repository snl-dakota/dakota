...
class NonD : public Analyzer {
 ...
 protected:
  ...
  /// constructor
  NonD(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
  ...
  void construct_lhs(std::shared_ptr<Iterator>& u_space_sampler,
                     std::shared_ptr<Model> u_model, unsigned short sample_type,
                     int num_samples, int seed, const String& rng,
                     bool vary_pattern, short sampling_vars_mode = ACTIVE);

  /// utility for vetting sub-method request against optimizers within
  /// the package configuration
  unsigned short sub_optimizer_select(
      unsigned short requested_sub_method,
...
inline short NonD::configure_cost(size_t num_steps, short seq_type,
                                  RealVector& cost) {
  ...
  BitArray model_cost_spec;
  SizetSizetPairArray cost_md_indices(num_mf);
  for (m = 0; m < num_mf; ++m)
    cost_md_indices[m] =
        SizetSizetPair(SZ_MAX, 0);  // no metadata for any model
  short cost_source =
...
inline short NonD::query_cost(size_t num_steps, short seq_type,
                              RealVector& cost) {
  ...
  for (m = 0; m < num_mf; ++m)
    cost_md_indices[m] =
        SizetSizetPair(SZ_MAX, 0);  // no metadata for any model
  return query_cost(num_steps, seq_type, cost, model_cost_spec,
                    cost_md_indices);
}

inline bool NonD::test_cost(bool cost_spec, SizetSizetPair& cost_md_indices,
                            const String& model_id) {
...
inline bool NonD::pdf_output() const { return pdfOutput; }

inline void NonD::pdf_output(bool output) { pdfOutput = output; }

inline short NonD::final_moments_type() const { return finalMomentsType; }

inline void NonD::final_moments_type(short type) { finalMomentsType = type; }
...
inline void NonD::print_level_mappings(std::ostream& s) const {
  print_level_mappings(s, "response function",
                       ModelUtils::response_labels(*iteratedModel));
}

inline void NonD::print_densities(std::ostream& s) const {
  print_densities(s, "response function",
                  ModelUtils::response_labels(*iteratedModel));
}

inline bool NonD::discrepancy_sample_counts() const { return false; }

inline bool NonD::homogeneous(const SizetArray& N_l) const {
  size_t N0 = N_l[0], i, len = N_l.size();
  for (i = 1; i < len; ++i)
    if (N_l[i] != N0) return false;
  return true;
}

inline bool NonD::differ(size_t N_alloc_ij,
                         const SizetArray& N_actual_ij) const {
  if (N_actual_ij.empty())  // allow empty NLevActual[i][j]
    return (N_alloc_ij > 0);
  size_t q, num_q = N_actual_ij.size();
  for (q = 0; q < num_q; ++q)
    if (N_alloc_ij != N_actual_ij[q]) return true;
  return false;
}

inline bool NonD::differ(const SizetArray& N_alloc_i,
                         const Sizet2DArray& N_actual_i) const {
  size_t j, num_lev = N_alloc_i.size();
  if (N_actual_i.size() != num_lev) return true;  // require both non-empty lev
  for (j = 0; j < num_lev; ++j)
    if (differ(N_alloc_i[j], N_actual_i[j])) return true;
  return false;
}

inline bool NonD::differ(const Sizet2DArray& N_alloc,
                         const Sizet3DArray& N_actual) const {
  size_t i, num_mf = N_alloc.size();
  if (N_actual.size() != num_mf) return true;  // require both non-empty mf
  for (i = 0; i < num_mf; ++i)
    if (differ(N_alloc[i], N_actual[i])) return true;
  return false;
}

inline size_t NonD::one_sided_relax_round(Real diff, Real relax_factor) {
  if (relax_factor == 1.)
...
inline size_t NonD::one_sided_delta(const SizetArray& current,
                                    const RealVector& targets,
...
inline size_t NonD::one_sided_delta(const SizetArray& current, Real target,
                                    Real relax_factor, size_t power) {
  size_t i, len = current.size();
  Real diff, pow_mean = 0.;
  switch (power) {
...
inline void NonD::one_sided_delta(const SizetArray& current,
                                  const RealVector& targets,
...
inline void NonD::one_sided_delta(const Sizet2DArray& current,
                                  const RealVector& targets,
                                  SizetArray& delta_N, Real relax_factor) {
  size_t i, c_len = current.size(), t_len = targets.length();
  Real diff_i;
  if (c_len != t_len) {
    Cerr << "Error: inconsistent array sizes in NonD::one_sided_delta()."
         << std::endl;
...
template <typename ArrayType>
void NonD::inflate_approx_samples(const ArrayType& N_l, bool multilev,
                                  size_t secondary_index,
                                  std::vector<ArrayType>& N_l_vec) {
  ...
  if (multilev) {  // ML case
    ...
    for (i = 0; i < num_approx; ++i) N_l_s[i] = N_l[i];
  } else {  // MF case
    num_approx = num_mf - 1;
...
template <typename ArrayType>
void NonD::inflate_sequence_samples(const ArrayType& N_l, bool multilev,
                                    size_t secondary_index,
                                    std::vector<ArrayType>& N_l_vec) {
  // 2D array is num_steps x num_qoi
  // 3D array is num_mf x num_lev x num_qoi which we slice as either:
...
template <typename ArrayType>
void NonD::print_multilevel_model_summary(std::ostream& s,
                                          const std::vector<ArrayType>& N_samp,
                                          String type, short seq_type,
                                          bool discrep_flag) {
  ...
  if (num_mf == 1) {
    ...
    if (discrep_flag)
      print_multilevel_discrepancy_summary(s, N_samp[0]);
    else
