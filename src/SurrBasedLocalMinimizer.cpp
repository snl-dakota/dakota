...
static const char rcsId[] =
    "@(#) $Id: SurrBasedLocalMinimizer.cpp 7031 2010-10-22 16:23:52Z mseldre $";
...
SurrBasedLocalMinimizer::SurrBasedLocalMinimizer(
    ...
    : SurrBasedMinimizer(problem_db, parallel_lib, model, traits),
      ...
      approxSubProbCon(
          probDescDB.get_short("method.sbl.subproblem_constraints")),
...
SurrBasedLocalMinimizer::SurrBasedLocalMinimizer(
    std::shared_ptr<Model> model, short merit_fn, short accept_logic,
    short constr_relax, const RealVector& tr_factors, size_t max_iter,
    size_t max_eval, Real conv_tol, unsigned short soft_conv_limit,
    ...
    : SurrBasedMinimizer(model, max_iter, max_eval, conv_tol, traits),
      ...
      meritFnType(merit_fn),
      acceptLogic(accept_logic),
      trConstraintRelax(constr_relax),
      minimizeCycles(0),
      penaltyIterOffset(-200),
      origTrustRegionFactor(tr_factors),
      minTrustRegionFactor(1.e-6),
      trRatioContractValue(0.25),
      trRatioExpandValue(0.75),
      gammaContract(0.25),
      gammaExpand(2.),
      softConvLimit(soft_conv_limit) {
  initialize();
}
...
void SurrBasedLocalMinimizer::initialize() {
  ...
  if (!softConvLimit) softConvLimit = 5;
...
void SurrBasedLocalMinimizer::initialize_sub_model() {
  ...
  if ((approxSubProbCon == ORIGINAL_CONSTRAINTS || !numNonlinearConstraints) &&
    ...
    size_t i, num_recast_primary = (approxSubProbObj == ORIGINAL_PRIMARY)
                                       ? numUserPrimaryFns
                                       : 1;
    size_t num_recast_secondary =
        (approxSubProbCon == NO_CONSTRAINTS) ? 0 : numNonlinearConstraints;
    ...
    BoolDequeArray nonlinear_resp_map(num_recast_primary +
                                      num_recast_secondary);
    ...
    approxSubProbModel = std::make_shared<RecastModel>(
        iteratedModel, recast_vars_map, recast_vars_comps_total, all_relax_di,
        all_relax_dr, false, iteratedModel->current_variables().view(), nullptr,
        set_recast, recast_primary_resp_map, recast_secondary_resp_map,
        recast_offset, recast_resp_order, nonlinear_resp_map,
        approx_subprob_objective_eval, approx_subprob_constraint_eval);
...
void SurrBasedLocalMinimizer::initialize_sub_minimizer() {
  ...
  if (!approx_method_ptr.empty()) {
    ...
    approxSubProbMinimizer =
        Iterator::get_iterator(probDescDB, parallelLib, approxSubProbModel);
    ...
    if (constraintTol <= 0.) {  // not specified in SBLM method spec
      ...
      if (aspm_constr_tol >
          0.)  // sub-method has spec: enforce SBLM consistency
        ...
        std::shared_ptr<Minimizer> aspm =
            std::static_pointer_cast<Minimizer>(approxSubProbMinimizer);
        aspm->constraint_tolerance(constraintTol);
      }
    } else {  // SBLM method spec takes precedence over approxSubProbMinimizer
              // spec
      std::shared_ptr<Minimizer> aspm =
          std::static_pointer_cast<Minimizer>(approxSubProbMinimizer);
      aspm->constraint_tolerance(constraintTol);
...
void SurrBasedLocalMinimizer::pre_run() {
  ...
  copy_data(ModelUtils::continuous_lower_bounds(*iteratedModel),
            globalLowerBnds);
  copy_data(ModelUtils::continuous_upper_bounds(*iteratedModel),
            globalUpperBnds);
}
...
void SurrBasedLocalMinimizer::update_trust_region_data(
    SurrBasedLevelData& tr_data, const RealVector& parent_l_bnds,
  ...
  // if (cv_truncation) tr_data.set_status_bits(NEW_CENTER);//handled in SBLD
  // set
  ...
  if (tr_lower_truncation)
    Cout << std::setw(wpp9) << "Lower (truncated)";
  else
    Cout << std::setw(wpp9) << "Lower";
  if (cv_truncation)
    Cout << std::setw(wpp9) << "Center (truncated)";
  else
    Cout << std::setw(wpp9) << "Center";
  if (tr_upper_truncation)
    Cout << std::setw(wpp9) << "Upper (truncated)";
  else
    Cout << std::setw(wpp9) << "Upper";
  Cout << '\n';
...
void SurrBasedLocalMinimizer::update_approx_sub_problem(
    SurrBasedLevelData& tr_data) {
  ModelUtils::active_variables(*approxSubProbModel, tr_data.vars_center());
  ModelUtils::continuous_lower_bounds(*approxSubProbModel,
                                      tr_data.tr_lower_bounds());
  ModelUtils::continuous_upper_bounds(*approxSubProbModel,
                                      tr_data.tr_upper_bounds());
...
void SurrBasedLocalMinimizer::compute_trust_region_ratio(
    SurrBasedLevelData& tr_data, bool check_interior) {
  /*
  ...
  */
  ...
  Real rel_obj = (std::fabs(obj_fn_center_truth) > DBL_MIN)
                     ? std::fabs(obj_delta / obj_fn_center_truth)
                     : std::fabs(obj_delta);
  /* Until a variable_tolerance is implemented as Dakota user input, set it to a
    generic default. (Or it could be set to half of relative distance from
    center to trust region bound.) Trust region size ought to be larger than
    design variable tolerance. */
  Real dx, max_dx = 0.0,
           variable_tolerance =
               1e-4;  // variable_tolerance = minTrustRegionFactor / 4;
  if (softConvLimit == 1) {
    for (size_t i = 0; i < numContinuousVars; ++i) {
      dx = std::fabs(c_vars[i] - s_vars[i]) /
           (globalUpperBnds[i] - globalLowerBnds[i]);
  ...
  Real merit_fn_center_truth, merit_fn_star_truth, merit_fn_center_approx,
      merit_fn_star_approx;
  ...
  if (meritFnType == LAGRANGIAN_MERIT) {  // penalty-free (like filter)
    ...
    merit_fn_center_truth =
        lagrangian_merit(fns_center_truth, sense, wts, origNonlinIneqLowerBnds,
                         origNonlinIneqUpperBnds, origNonlinEqTargets);
    merit_fn_star_truth =
        lagrangian_merit(fns_star_truth, sense, wts, origNonlinIneqLowerBnds,
                         origNonlinIneqUpperBnds, origNonlinEqTargets);
    merit_fn_center_approx =
        lagrangian_merit(fns_center_approx, sense, wts, origNonlinIneqLowerBnds,
                         origNonlinIneqUpperBnds, origNonlinEqTargets);
    merit_fn_star_approx =
        lagrangian_merit(fns_star_approx, sense, wts, origNonlinIneqLowerBnds,
                         origNonlinIneqUpperBnds, origNonlinEqTargets);

    // IntRespMCIter d_it;
    // for (i=0, d_it=dace_responses.begin(); i<num_dace_responses; ++i,
    // ++d_it){
    ...
    merit_fn_center_truth = augmented_lagrangian_merit(
        fns_center_truth, sense, wts, origNonlinIneqLowerBnds,
        origNonlinIneqUpperBnds, origNonlinEqTargets);
    merit_fn_star_truth = augmented_lagrangian_merit(
        fns_star_truth, sense, wts, origNonlinIneqLowerBnds,
        origNonlinIneqUpperBnds, origNonlinEqTargets);
    merit_fn_center_approx = augmented_lagrangian_merit(
        fns_center_approx, sense, wts, origNonlinIneqLowerBnds,
        origNonlinIneqUpperBnds, origNonlinEqTargets);
    merit_fn_star_approx = augmented_lagrangian_merit(
        fns_star_approx, sense, wts, origNonlinIneqLowerBnds,
        origNonlinIneqUpperBnds, origNonlinEqTargets);
  }
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "Response truth:\ncenter = "
         ...
         << "star = " << tr_data.response_star(CORR_APPROX_RESPONSE)
         << "Merit fn truth:  center = " << merit_fn_center_truth
         << " star = " << merit_fn_star_truth
         << "\nMerit fn approx: center = " << merit_fn_center_approx
         << " star = " << merit_fn_star_approx << '\n';
  ...
  Real tr_ratio =
      (std::fabs(denominator) > DBL_MIN) ? numerator / denominator : numerator;
  ...
  if (accept_step) {
    ...
    } else if (std::fabs(1. - tr_ratio) <=
               1. - trRatioExpandValue) {  // accept & expand
  ...
  if (accept_step) {
    ...
    if (softConvLimit > 1) {
      Real rel_numer = (std::fabs(merit_fn_center_truth) > DBL_MIN)
                           ? std::fabs(numerator / merit_fn_center_truth)
                           : std::fabs(numerator);
      Real rel_denom = (std::fabs(merit_fn_center_approx) > DBL_MIN)
                           ? std::fabs(denominator / merit_fn_center_approx)
                           : std::fabs(denominator);
      if (numerator <= 0. || rel_numer < convergenceTol || denominator <= 0. ||
          rel_denom < convergenceTol)
    ...
    } else  // SLOWED CONVERGENCE -- based on two consecutive accepted points
    {
      if (rel_obj < convergenceTol && !constraint_viol &&
        ...
        Cout << "Slowed convergence for accepted step: softConvLimit="
             << softConvLimit << "\n";
        Cout << "       rel_obj = " << rel_obj
             << ",  convergenceTol=" << convergenceTol << "\n";
        Cout << "       obj_delta=" << obj_delta
             << ",  center_truth=" << obj_fn_center_truth << "\n";
        Cout << "       max_dx =  " << max_dx
             << ",  variable_tolerance=" << variable_tolerance << "\n";
        tr_data.increment_soft_convergence_count();  // ++ slowed conv counter
      }
    }
  } else if (softConvLimit > 1) {
    tr_data.increment_soft_convergence_count();  // ++ soft conv counter
  }
}
...
void SurrBasedLocalMinimizer::hard_convergence_check(
    SurrBasedLevelData& tr_data, const RealVector& lower_bnds,
  ...
  if (meritFnType == LAGRANGIAN_MERIT ||
    ...
    update_lagrange_multipliers(fns_truth, response_truth.function_gradients(),
                                tr_data);
  if (constraint_viol) return;
...
void SurrBasedLocalMinimizer::update_penalty(const RealVector& fns_center_truth,
                                             const RealVector& fns_star_truth) {
  ...
  if (meritFnType == PENALTY_MERIT) {
    ...
    if (cv_delta < 0. && obj_delta > 0.) {
      ...
      int min_iter =
          (int)std::ceil(10. * std::log(std::fabs(obj_delta / cv_delta)));
    ...
    penaltyParameter *= 10.;
    // penaltyParameter = std::min(penaltyParameter, 1.e+20); // cap max
    // penalty?
    Real mu =
...
void SurrBasedLocalMinimizer::approx_subprob_objective_eval(
    const Variables& surrogate_vars, const Variables& recast_vars,
  ...
  if (sblmInstance->approxSubProbObj == ORIGINAL_PRIMARY) {
    for (size_t i = 0; i < sblmInstance->numUserPrimaryFns; ++i) {
      ...
      if (recast_asv[i] & 4)
        recast_response.function_hessian(surrogate_response.function_hessian(i),
                                         i);
    }
  } else {
    ...
    const RealVector& nln_ineq_l_bnds =
        (no_sub_prob_con) ? sblmInstance->origNonlinIneqLowerBnds
                          : ModelUtils::nonlinear_ineq_constraint_lower_bounds(
                                *sblmInstance->approxSubProbModel);
    const RealVector& nln_ineq_u_bnds =
        (no_sub_prob_con) ? sblmInstance->origNonlinIneqUpperBnds
                          : ModelUtils::nonlinear_ineq_constraint_upper_bounds(
                                *sblmInstance->approxSubProbModel);
    const RealVector& nln_eq_tgts =
        (no_sub_prob_con) ? sblmInstance->origNonlinEqTargets
                          : ModelUtils::nonlinear_eq_constraint_targets(
                                *sblmInstance->approxSubProbModel);
    ...
    if (recast_asv[0] & 1) {
      ...
      switch (sblmInstance->approxSubProbObj) {
        ...
        case LAGRANGIAN_OBJECTIVE:
          recast_fn = sblmInstance->lagrangian_merit(
              surrogate_fns, sense, wts, nln_ineq_l_bnds, nln_ineq_u_bnds,
              nln_eq_tgts);
          break;
        case AUGMENTED_LAGRANGIAN_OBJECTIVE:
          recast_fn = sblmInstance->augmented_lagrangian_merit(
              surrogate_fns, sense, wts, nln_ineq_l_bnds, nln_ineq_u_bnds,
              nln_eq_tgts);
          break;
      }
      ...
      switch (sblmInstance->approxSubProbObj) {
        case SINGLE_OBJECTIVE:
          sblmInstance->objective_gradient(surrogate_fns, surrogate_grads,
                                           sense, wts, recast_grad);
          break;
        case LAGRANGIAN_OBJECTIVE:
          sblmInstance->lagrangian_gradient(
              surrogate_fns, surrogate_grads, sense, wts, nln_ineq_l_bnds,
              nln_ineq_u_bnds, nln_eq_tgts, recast_grad);
          break;
        case AUGMENTED_LAGRANGIAN_OBJECTIVE:
          sblmInstance->augmented_lagrangian_gradient(
              surrogate_fns, surrogate_grads, sense, wts, nln_ineq_l_bnds,
              nln_ineq_u_bnds, nln_eq_tgts, recast_grad);
          break;
      }
      ...
      switch (sblmInstance->approxSubProbObj) {
        case SINGLE_OBJECTIVE:
          sblmInstance->objective_hessian(surrogate_fns, surrogate_grads,
                                          surrogate_hessians, sense, wts,
                                          recast_hess);
          break;
        case LAGRANGIAN_OBJECTIVE:
          sblmInstance->lagrangian_hessian(
              surrogate_fns, surrogate_grads, surrogate_hessians, sense, wts,
              nln_ineq_l_bnds, nln_ineq_u_bnds, nln_eq_tgts, recast_hess);
          break;
        case AUGMENTED_LAGRANGIAN_OBJECTIVE:
          sblmInstance->augmented_lagrangian_hessian(
              surrogate_fns, surrogate_grads, surrogate_hessians, sense, wts,
              nln_ineq_l_bnds, nln_ineq_u_bnds, nln_eq_tgts, recast_hess);
          break;
      }
...
/** Constraint functions evaluator for solution of approximate
    subproblem using a RecastModel. */
void SurrBasedLocalMinimizer::approx_subprob_constraint_eval(
    const Variables& surrogate_vars, const Variables& recast_vars,
    const Response& surrogate_response, Response& recast_response) {
  // RecastModel evaluates its subModel response and then invokes this fn
  const ShortArray& recast_asv = recast_response.active_set_request_vector();
  ...
  size_t num_recast_primary =
      (sblmInstance->approxSubProbObj == ORIGINAL_PRIMARY)
          ? sblmInstance->numUserPrimaryFns
          : 1;
  if (num_recast_fns <= num_recast_primary) return;
  ...
  switch (sblmInstance->approxSubProbCon) {
    case ORIGINAL_CONSTRAINTS: {
      //
      // Note: constraints are NOT converted to std form as done within merit
      // fns
      //
    ...
    case LINEARIZED_CONSTRAINTS: {
      ...
      const Response& center_approx_resp =
          tr_data.response_center(CORR_APPROX_RESPONSE);
      const RealVector& center_approx_fns =
          center_approx_resp.function_values();
      ...
      for (i = 0; i < num_recast_cons; ++i) {
        ...
        if (recast_asv[recast_i] & 1) {
          ...
          for (j = 0; j < num_recast_vars; ++j)
            sum +=
                center_approx_grads(j, surr_i) * (c_vars[j] - center_c_vars[j]);
      ...
      Cout << "center_c_vars =\n"
           ...
           << c_vars << "center_approx_fns =\n"
           << center_approx_fns << "center_approx_grads =\n"
           << center_approx_grads << "recast_response =\n"
...
bool SurrBasedLocalMinimizer::find_response(const Variables& search_vars,
    ...
    if (numNonlinearEqConstraints) {
      ...
      for (size_t i = 0; i < numNonlinearEqConstraints; ++i) {
        const Real& nln_eq_con =
            fns_center_truth[numUserPrimaryFns + numNonlinearIneqConstraints +
                             i];
  ...
  if (constr_viol > constraintTol) {
...
#ifdef HAVE_NPSOL
    tau_minimizer = std::make_unique<NPSOLOptimizer>(
        tau_and_x_initial, tau_and_x_lower_bnds, tau_and_x_upper_bnds,
        lin_ineq_coeffs, lin_ineq_lower_bnds, lin_ineq_lower_bnds,
        lin_eq_coeffs, lin_eq_targets, origNonlinIneqLowerBnds,
        origNonlinIneqUpperBnds, origNonlinEqTargets, hom_objective_eval,
        hom_constraint_eval, deriv_level, conv_tol);
#endif
    ...
    if (tau < 1.) {
      ...
      if (numNonlinearEqConstraints) {
        ...
        ModelUtils::nonlinear_eq_constraint_targets(*approxSubProbModel,
                                                    nln_eq_targets);
      }
...
void SurrBasedLocalMinimizer::hom_objective_eval(int& mode, int& n,
                                                 double* tau_and_x, double& f,
                                                 double* grad_f, int&) {
...
/** NPSOL constraint functions evaluator for solution of homotopy constraint
    ...
    performs the update of the tau parameter in the homotopy heuristic
    approach used to relax the constraints in the original problem. */
void SurrBasedLocalMinimizer::hom_constraint_eval(int& mode, int& ncnln, int& n,
                                                  int& nrowj, int* needc,
                                                  double* tau_and_x, double* c,
  ...
  size_t num_fns = ModelUtils::response_size(*sblmInstance->approxSubProbModel),
         num_obj_fns =
             num_fns - ncnln;  // 1 if recast, numUserPrimaryFns if not
  ...
  ModelUtils::continuous_variables(*sblmInstance->approxSubProbModel,
                                   local_des_vars);
  ...
  const RealVector& nli_lower_slack = sblmInstance->nonlinIneqLowerBndsSlack;
  const RealVector& nli_upper_slack = sblmInstance->nonlinIneqUpperBndsSlack;
  ...
  if (asv_request & 1) {
    ...
    for (i = 0; i < num_nle_constr; ++i)
      c[num_nli_constr + i] =
          resp_fn[nle_offset + i] - (1. - tau) * nle_targets_slack[i];
#ifdef DEBUG
    for (i = 0; i < ncnln; ++i)
  ...
  if (asv_request & 2) {
    ...
    for (j = 0; j < n; ++j) {
      ...
      for (i = 0; i < num_nle_constr; ++i)
        cjac[cntr++] =
            (j == 0) ? nle_targets_slack[i] : resp_grad(j - 1, nle_offset + i);
    }
#ifdef DEBUG
    for (i = 0; i < cntr; ++i)
