...
EffGlobalMinimizer::EffGlobalMinimizer(ProblemDescDB& problem_db,
  ...
  initialize_sub_problem(
      approx_type, samples, probDescDB.get_int("method.random_seed"),
      probDescDB.get_bool("method.derivative_usage"), sample_reuse,
...
EffGlobalMinimizer::EffGlobalMinimizer(std::shared_ptr<Model> model,
                                       const String& approx_type, int samples,
                                       int seed, bool use_derivs,
...
void EffGlobalMinimizer::initialize_sub_problem(
    const String& approx_type, int samples, int seed, bool use_derivs,
    const String& sample_reuse, const String& import_build_points_file,
  ...
  bool vary_pattern =
      false;  // for consistency across any outer loop invocations

  auto dace_iterator = std::make_shared<NonDLHSSampling>(
      iteratedModel, sample_type, samples, seed, rng, vary_pattern,
      ACTIVE_UNIFORM);
  ...
  fHatModel = std::make_shared<DataFitSurrModel>(
      dace_iterator, iteratedModel, gp_set, gp_view, approx_type, approx_order,
      corr_type, corr_order, dataOrder, outputLevel, sample_reuse,
      import_build_points_file, import_build_format, import_build_active_only,
  ...
  maxEvalConcurrency = std::max(
      maxEvalConcurrency, dace_iterator->maximum_evaluation_concurrency());
}
...
void EffGlobalMinimizer::check_parallelism() {
  ...
  if (batchSize > 1) {
    if (iteratedModel->asynch_flag())
      ...
      batchSize = batchSizeAcquisition = 1;
      batchSizeExploration = 0;
    }
  } else
    parallelFlag = batchAsynch = false;
...
void EffGlobalMinimizer::build_gp() {
  ...
  approxSubProbModel->primary_response_fn_weights(RealVector(),
                                                  false);  // no recur
...
void EffGlobalMinimizer::construct_batch_acquisition(size_t new_acq,
  ...
  size_t i;
  for (i = 0; i < numFunctions; i++) primary_resp_map[0][i] = i;
  ...
  std::shared_ptr<RecastModel> asp_model_rep =
      std::static_pointer_cast<RecastModel>(approxSubProbModel);
  ...
  for (i = 0; i < new_acq; ++i, ++batchEvalId) {
    ...
    if (outputLevel >= NORMAL_OUTPUT)
      Cout << "\nResults of EGO iteration:\nFinal point =\n"
           << vars_star << "Expected Improvement    =\n"
           << std::setw(write_precision + 28) << -ei_resp_star.function_value(0)
           << '\n';
...
void EffGlobalMinimizer::append_liar(const Variables& vars_star, int liar_id,
  ...
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "\nParallel EGO: appending liar response for evaluation " << liar_id
         << ".\n";
...
void EffGlobalMinimizer::evaluate_batch(bool rebuild) {
  ...
  if (parallelFlag) {
    ...
    fHatModel->append_approximation(varsAcquisitionMap, truth_resp_map,
                                    rebuild);

    // update constraints (truth resp only, not for liar resp)
    if (numNonlinearConstraints) update_constraints(truth_resp_map);
  } else {
    // no pop: liar was not appended in serial case

    IntVarsMCIter v_cit = (varsAcquisitionMap.empty())
                              ? --varsExplorationMap.end()
                              : --varsAcquisitionMap.end();
  ...
  varsAcquisitionMap.clear();
  varsExplorationMap.clear();
}
...
void EffGlobalMinimizer::launch_batch() {
  ...
  set.request_values(dataOrder);
  IntVarsMIter v_it;
  for (v_it = varsAcquisitionMap.begin(); v_it != varsAcquisitionMap.end();
       ++v_it) {
    ModelUtils::active_variables(*iteratedModel, v_it->second);
    iteratedModel->evaluate_nowait(set);
  }
  for (v_it = varsExplorationMap.begin(); v_it != varsExplorationMap.end();
       ++v_it) {
    ModelUtils::active_variables(*iteratedModel, v_it->second);
    iteratedModel->evaluate_nowait(set);
  }
...
void EffGlobalMinimizer::process_truth_response_map(
    const IntResponseMap& truth_resp_map, bool rebuild) {
...
void EffGlobalMinimizer::update_variable_maps(
    const IntResponseMap& truth_resp_map) {
  ...
  for (r_cit = truth_resp_map.begin(); r_cit != truth_resp_map.end(); ++r_cit) {
    r_id = r_cit->first;
    while (a_id < r_id) a_id = extract_id(++a_it, varsAcquisitionMap);
    while (e_id < r_id) e_id = extract_id(++e_it, varsExplorationMap);
    // Note: use postfix iterator increments to avoid invalidation by erase
    if (a_id == r_id) {
      varsAcquisitionMap.erase(
          a_it++);  // copy a_it, increment orig, erase copy
      a_id = extract_id(a_it, varsAcquisitionMap);
    } else if (e_id == r_id) {
      varsExplorationMap.erase(
          e_it++);  // copy e_it, increment orig, erase copy
      e_id = extract_id(e_it, varsExplorationMap);
    } else {
...
void EffGlobalMinimizer::extract_qoi_build_data(size_t data_index,
                                                RealVector& fn_vals) {
  if (fn_vals.length() != numFunctions) fn_vals.sizeUninitialized(numFunctions);
...
bool EffGlobalMinimizer::converged() {
  ...
  if (conv || outputLevel >= DEBUG_OUTPUT) {
    ...
    if (globalIterCount >= maxIterations)
      Cout << "Stopping criteria met:     globalIterCount (" << globalIterCount
           << ") >= ";
    else
      Cout << "Stopping criteria not met: globalIterCount (" << globalIterCount
           << ") < ";
...
void EffGlobalMinimizer::update_convergence_counters(
  ...
  Real dist_cv_star = (prevSubProbSoln.empty())
                          ? DBL_MAX
                          : rel_change_L2(c_vars, prevSubProbSoln);
  if (dist_cv_star < distanceTol)
    ++distConvergenceCntr;  // increment
  else
    distConvergenceCntr = 0;  // reset
...
void EffGlobalMinimizer::update_convergence_counters(
  ...
  if (eif_star < convergenceTol)
    ++eifConvergenceCntr;  // increment
  else
    eifConvergenceCntr = 0;  // reset
...
/** To maximize variances, the approxSubProbMinimizer will minimize
    -(variances). **/
void EffGlobalMinimizer::Variances_objective_eval(
...
Real EffGlobalMinimizer::compute_probability_improvement(
    const RealVector& means, const RealVector& variances) {
  ...
  if (numNonlinearConstraints) {
    ...
    // stdv_M = sqrt(var_f)
    stdv =
        std::sqrt(variances[0]);  // *** sqrt(sum(variances(1:nUsrPrimaryFns))
  }
...
Real EffGlobalMinimizer::compute_expected_improvement(
    const RealVector& means, const RealVector& variances) {
  // Objective calculation will incorporate any sense changes or
  // weights, such that this is an objective to minimize.
  Real mean = objective(means, iteratedModel->primary_response_fn_sense(),
  ...
  if (numNonlinearConstraints) {
    ...
    for (size_t i = 0; i < numNonlinearConstraints; ++i)
      mean +=
          augLagrangeMult[i] * ev[i] + penaltyParameter * ev[i] * ev[i];  // ***
    stdv = std::sqrt(
        variances[0]);  // *** if variance is only dependent on parameter points
                        // and not on QoI observations, then this would be Ok
  } else {              // extend for NLS/MOO ***
    // mean_M = M(mu_f)
    // stdv_M = sqrt(var_f)
    stdv =
...
Real EffGlobalMinimizer::compute_lower_confidence_bound(
    const RealVector& means, const RealVector& variances) {
  // Objective calculation will incorporate any sense changes or
  // weights, such that this is an objective to minimize.
  Real mean = objective(means, iteratedModel->primary_response_fn_sense(),
  ...
  if (numNonlinearConstraints) {
    ...
    // stdv_M = sqrt(var_f)
    stdv =
        std::sqrt(variances[0]);  // *** sqrt(sum(variances(1:numUserPrimary))
  }

  Real kappa =
      2.;  // in future, vary this parameter as a function of iterations
...
Real EffGlobalMinimizer::compute_variances(const RealVector& variances) {
  ...
  // weights, such that this is an objective to minimize.
  return std::sqrt(
      variances[0]);  // *** sqrt(sum(variances(1:numUserPrimaryFns))
}

/** Compute the expected violation for constraints **/
RealVector EffGlobalMinimizer::expected_violation(const RealVector& means,
                                                  const RealVector& variances) {
  ...
  for (i = 0; i < numNonlinearEqConstraints; i++) {
    const Real& mean =
        means[numUserPrimaryFns + numNonlinearIneqConstraints + i];
    const Real& stdv = std::sqrt(
        variances[numUserPrimaryFns + numNonlinearIneqConstraints + i]);
...
void EffGlobalMinimizer::debug_plots() {
  ...
  for (size_t i = 0; i < numFunctions; i++) {
    ...
    size_t j, k, num_data_pts = gp_data.size(),
                 num_vars = ModelUtils::cv(fHatModel);
    for (j = 0; j < num_data_pts; ++j) {
      s_out << '\n';
      const RealVector& sample = sdv_array[j].continuous_variables();
      for (k = 0; k < num_vars; k++) s_out << std::setw(13) << sample[k] << ' ';
    ...
    if (num_vars == 2) {
      ...
      for (j = 0; j < 101; j++) {
        ...
        for (k = 0; k < 101; k++) {
          ...
          RealVector variances = fHatModel->approximation_variances(
              fHatModel->current_variables());
          var_out << '\n'
                  << std::setw(13) << test_pt[0] << ' ' << std::setw(13)
          ...
          if (i == numFunctions - 1) {
            ...
            eif_out << '\n'
                    << std::setw(13) << test_pt[0] << ' ' << std::setw(13)
                    << test_pt[1] << ' ' << std::setw(13)
                    << compute_expected_improvement(merit, test_pt);
          }
        }
        gp_out << std::endl;
        var_out << std::endl;
        if (i == numFunctions - 1) eif_out << std::endl;
      }
    }
  }
#endif  // DEBUG_PLOTS
