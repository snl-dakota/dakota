...
{
    if (setUpType=="model") {
        build_gp(); // build initial GP once for all response functions: fHatModel.build_approximation()
...
void EffGlobalMinimizer::minimize_surrogates_on_model()
{
    //Parallel EGO: --------------------------
    //     Solve the problem.
    EffGlobalMinimizer* prev_instance = effGlobalInstance; // move to core_run()
    // build initial GP once for all response functions: fHatModel.build_approximation() // move to core_run()
    build_gp(); // build initial GP once for all response functions: fHatModel.build_approximation()
    ...
    while (!approx_converged) {
        ...
        VariablesArray varsArrayBatchAcq(BatchSizeAcquisition);

        // Note: varsStar: input; respStar: output (liar); resp_star_truth: output (true)
        ...
        Variables  varsStar = approxSubProbMinimizer.variables_results();
        RealVector cVars    = varsStar.continuous_variables();
        Response   respStar = approxSubProbMinimizer.response_results();
        Real       eifStar  = respStar.function_value(0);

        // Construct the batch
        Cout << "\n>>>>> Initiating global optimization\n";
        construct_batch_acquisition(BatchSizeAcquisition); // construct the acquisition batch

        // Delete liar responses
        delete_liar_responses(BatchSizeAcquisition); // delete liar responses in fHatModel

        // Query/Update responses/Update constraints
        query_batch(BatchSizeAcquisition); // query the batch -- either in parallel or in sequential
        ...
        for (int i_batch_acquisition = 0; i_batch_acquisition < BatchSizeAcquisition; i_batch_acquisition++) {
            ...
            debug_print_values();
            // check_convergence(eifStar, cVars, prev_cv_star, eif_convergence_cntr, dist_convergence_cntr);
            // Check for convergence based on max EIF
            if ( -eifStar < convergenceTol )
            ...
            Real distCStar = (prev_cv_star.empty()) ? DBL_MAX : rel_change_L2(cVars, prev_cv_star);
            // update prev_cv_star
            copy_data(cVars, prev_cv_star);
            ...
            debug_print_counters(globalIterCount, eifStar, distCStar, dist_convergence_cntr);
        }
    ...
    // cVars, fmin contain the optimal design
...
Real EffGlobalMinimizer::get_augmented_lagrangian(const RealVector& mean,
  ...
  // print out message
   ...
   << std::setw(write_precision+7) << -eifStar
...
void EffGlobalMinimizer::check_convergence(const Real& eifStar,
                                          const RealVector& cVars,
                                          RealVector prev_cv_star,
...
// Construct initialize GP responses
...
// Construct a batch
void EffGlobalMinimizer::construct_batch_acquisition(int BatchSizeAcquisition) {
    for (int i_batch_acquisition = 0; i_batch_acquisition < BatchSizeAcquisition; i_batch_acquisition++) {
        ...
        fHatModel.continuous_variables(cVars);
        fHatModel.evaluate();
        ...
        if (i_batch_acquisition < BatchSizeAcquisition - 1) {// ignore the last point of the batch
            ...
            const IntResponsePair respStar_liar(iteratedModel.evaluation_id() + i_batch_acquisition + 1, approx_response); // implement a liar counter

            // Update GP
            size_t numDataPts = fHatModel.approximation_data(0).points(); // debug
            Cout << "\nParallel EGO: Adding liar response...\n"; // debug

            // Append constant liar to fHatModel (aka heuristic liar)
            fHatModel.append_approximation(varsStar, respStar_liar, true); // debug

            // Update constraints based on the constant liar
            if (numNonlinearConstraints) {
                const RealVector& fns_star_liar = respStar_liar.second.function_values();
        ...
        varsArrayBatchAcq[i_batch_acquisition] = varsStar.copy();
    }
}
...
// Delete liar responses
void EffGlobalMinimizer::delete_liar_responses(int BatchSizeAcquisition) {
    for (int i_batch_acquisition = 0; i_batch_acquisition < BatchSizeAcquisition; i_batch_acquisition++) {
        if (i_batch_acquisition < BatchSizeAcquisition - 1) // ignore the last point of the batch
...
// Query the batch -- either in parallel or in sequential
// Update responses
// Update constraints
void EffGlobalMinimizer::query_batch(int BatchSizeAcquisition) {
    if (parallelFlag) { // Query and update constraints in parallel
        // Query the batch
        for (int i_batch_acquisition = 0; i_batch_acquisition < BatchSizeAcquisition; i_batch_acquisition++) {
            fHatModel.component_parallel_mode(TRUTH_MODEL_MODE);
            iteratedModel.active_variables(varsArrayBatchAcq[i_batch_acquisition]);
        ...
        fHatModel.append_approximation(varsArrayBatchAcq, resp_star_truth, true);
    ...
    { // Query and update constraints in sequential
        ...
        fHatModel.component_parallel_mode(TRUTH_MODEL_MODE);
        iteratedModel.continuous_variables(cVars);
        ActiveSet set = iteratedModel.current_response().active_set();
        set.request_values(dataOrder);
        ...
        fHatModel.append_approximation(varsStar, resp_star_truth, true);
...
void EffGlobalMinimizer::debug_print_values() {
    #ifdef DEBUG
        RealVector variance = fHatModel.approximation_variances(varsStar);
...
// Print counters
void EffGlobalMinimizer::debug_print_counters(unsigned short globalIterCount,
                                             const Real& eifStar,
                                             Real distCStar,
                                             unsigned short dist_convergence_cntr) {
    #ifdef DEBUG
