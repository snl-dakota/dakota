...
extern "C" const size_t nidr_max_errors;
extern "C" char *nidr_parse_errors[];
extern "C" void nidr_alloc_error_cache();
extern "C" void nidr_free_error_cache();
...
void NIDRProblemDescDB::derived_parse_inputs(
    const std::string_view dakota_input, const std::string_view parser_options,
  ...
  if (nidr_parse(parser_options.data(), dump_file)) {
    ...
    for (unsigned int i = 0; i < nidr_max_errors; ++i)
      if (nidr_parse_errors[i][0] != '\0') Cerr << nidr_parse_errors[i];
    nidr_free_error_cache();
    abort_handler(PARSE_ERROR);  // allows proper abort in parallel case
  }
  nidr_free_error_cache();

  if (nerr) abort_handler(PARSE_ERROR);
  if (command_line_run) {
    const char *s;
    // manage dynamic solver plugins specified in the input
#ifdef DAKOTA_DL_SOLVER
    std::list<DataMethod>::iterator Dml = dataMethodList.begin(),
...
struct Var_Info {
  ...
  IntArray *nddsi, *nddss, *nddsr, *nCI, *nDI, *nhbp, *nhpip, *nhpsp, *nhprp,
      *ndusi, *nduss, *ndusr, *ndssi, *ndsss, *ndssr;

  RealVector *ddsr, *CIlb, *CIub, *CIp, *DIp, *DSIp, *DSSp, *DSRp, *dusr, *hba,
      *hbo, *hbc, *hpic, *hpsc, *hpra, *hprc, *ucm, *dssr;
...
void NIDRProblemDescDB::iface_Real(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Iface_Info **)g)->di->**(Real DataInterfaceRep::**)v = *val->r;
}

void NIDRProblemDescDB::iface_Rlit(const char *keyname, Values *val, void **g,
                                   void *v) {
  ...
  r = val->r;
  for (i = 0; i < n; ++i) (*rv)[i] = r[i];
}

void NIDRProblemDescDB::iface_false(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Iface_Info **)g)->di->**(bool DataInterfaceRep::**)v = false;
}

void NIDRProblemDescDB::iface_ilit(const char *keyname, Values *val, void **g,
                                   void *v) {
  DataInterfaceRep *di = (*(Iface_Info **)g)->di;
...
void NIDRProblemDescDB::iface_int(const char *keyname, Values *val, void **g,
                                  void *v) {
  int n = *val->i;
  (*(Iface_Info **)g)->di->**(int DataInterfaceRep::**)v = n;
}

void NIDRProblemDescDB::iface_lit(const char *keyname, Values *val, void **g,
                                  void *v) {
  (*(Iface_Info **)g)->di->*((Iface_mp_lit *)v)->sp = ((Iface_mp_lit *)v)->lit;
}

void NIDRProblemDescDB::iface_start(const char *keyname, Values *val, void **g,
                                    void *v) {
  Iface_Info *ii;

  if (!(ii = new Iface_Info))
  Botch:
    botch("new failure in iface_start");
  if (!(ii->di_handle = new DataInterface)) goto Botch;
  ii->di = ii->di_handle->dataIfaceRep.get();
  *g = (void *)ii;
}

void NIDRProblemDescDB::iface_true(const char *keyname, Values *val, void **g,
                                   void *v) {
...
int not_executable(const char *driver_name, const char *tdir) {
  ...
  if (!stat(driver_name, &sb)) goto ret;
#endif
...
#ifdef _WIN32
      || std::strchr(driver_name, '\\') || (dlen > 2 && driver_name[1] == ':')
#endif
  )
    p = "";
  ...
  break2:
    ...
    stat_check:
      ...
      rc = 2;
      if (sb.st_uid == myuid) {
        if (sb.st_mode & S_IXUSR) goto ret0;
      } else if (sb.st_gid == mygid) {
        if (sb.st_mode & S_IXGRP) goto ret0;
...
void NIDRProblemDescDB::iface_stop(const char *keyname, Values *val, void **g,
                                   void *v) {
  ...
  if (di->batchEvalFlag && (nd > 1 || !ife || !ofe))
    squawk(
        "For batch evaluation, specification of an input_filter, "
        "output_filter,\n\t"
        "or more than one analysis_drivers is disallowed");
  if (di->batchEvalFlag && ec == 1) {
    warn(
        "batch option not required for evaluation concurrency == 1.\n\t"
        "Sequential operation will be used");
    di->batchEvalFlag = false;
  }

  if (di->batchEvalFlag &&
      !(di->failAction == "abort" || di->failAction == "recover"))
    squawk(
        "For batch evaluation, only failure_capture abort and recover are "
        "supported");

  if (di->algebraicMappings == "" && nd == 0)
    squawk(
        "interface specification must provide algebraic_mappings,\n\t"
        "analysis_drivers, or both");
  if (nd > 0 && di->asynchLocalAnalysisConcurrency > nd) {
    warn(
        "analysis_concurrency specification greater than length of\n\t"
        "analysis_drivers list.  Truncating analysis_concurrency to %d",
        nd);
    di->asynchLocalAnalysisConcurrency = nd;
  }
  if (ec && ec < 2 && ac && ac < 2) {
    warn(
        "asynchronous option not required for evaluation and analysis.\n\t"
        "Concurrency limited to %d and %d.\n\t"
        "Synchronous operations will be used",
        ec, ac);
    di->asynchFlag = false;
  }
...
/** returns 1 if not found, 2 if found, but not executable, 0 if found (no
 * error) in case we want to return to error on not found... */
int NIDRProblemDescDB::check_driver(const String &an_driver,
  ...
  if (driver_and_args.size() == 0)
    ...
    if (program_name.empty())
      ...
      if (!driver_found.empty()) return 0;
      ...
      if (WorkdirHelper::find_driver(link_files, program_name)) return 0;

      if (WorkdirHelper::find_driver(copy_files, program_name)) return 0;
...
void NIDRProblemDescDB::iface_str(const char *keyname, Values *val, void **g,
                                  void *v) {
  (*(Iface_Info **)g)->di->**(String DataInterfaceRep::**)v = *val->s;
}

void NIDRProblemDescDB::iface_str2D(const char *keyname, Values *val, void **g,
                                    void *v) {
  DataInterfaceRep *di = (*(Iface_Info **)g)->di;
  ...
  n = val->n;
  if (nd <= 0) botch("num_drivers = %d in iface_str2D", (int)nd);
  if (n % nd) {
    squawk(
        "number of analysis_components not evenly divisible "
  ...
  for (i = k = 0; i < nd; i++) {
    sa = &((*sa2)[i]);
    sa->resize(nc);
    for (j = 0; j < nc; ++j, ++k) (*sa)[j] = s[k];
  }
}

void NIDRProblemDescDB::iface_strL(const char *keyname, Values *val, void **g,
                                   void *v) {
  StringArray *sa =
...
void NIDRProblemDescDB::iface_type(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Iface_Info **)g)->di->*((Iface_mp_type *)v)->sp =
      ((Iface_mp_type *)v)->type;
}

void NIDRProblemDescDB::method_Real(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Meth_Info **)g)->dme->**(Real DataMethodRep::**)v = *val->r;
}

void NIDRProblemDescDB::method_Real01(const char *keyname, Values *val,
                                      void **g, void *v) {
  Real t = *val->r;
  if (t < 0. || t > 1.) botch("%s must be in [0, 1]", keyname);
  (*(Meth_Info **)g)->dme->**(Real DataMethodRep::**)v = t;
}

void NIDRProblemDescDB::method_RealDL(const char *keyname, Values *val,
                                      void **g, void *v) {
  ...
  for (i = 0; i < n; i++) (dm->*sa)[i] = r[i];
}

void NIDRProblemDescDB::method_RealLlit(const char *keyname, Values *val,
                                        void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  Real *r;
...
void NIDRProblemDescDB::method_Realp(const char *keyname, Values *val, void **g,
                                     void *v) {
  Real t = *val->r;
  if (t <= 0.) botch("%s must be positive", keyname);
  (*(Meth_Info **)g)->dme->**(Real DataMethodRep::**)v = t;
}

void NIDRProblemDescDB::method_Realz(const char *keyname, Values *val, void **g,
                                     void *v) {
  Real t = *val->r;
  if (t < 0.) botch("%s must be nonnegative", keyname);
  (*(Meth_Info **)g)->dme->**(Real DataMethodRep::**)v = t;
}

// MSE: This function just sets two values for one keyword.
void NIDRProblemDescDB::method_piecewise(const char *keyname, Values *val,
                                         void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
...
void NIDRProblemDescDB::method_false(const char *keyname, Values *val, void **g,
                                     void *v) {
  (*(Meth_Info **)g)->dme->**(bool DataMethodRep::**)v = false;
}

void NIDRProblemDescDB::method_int(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Meth_Info **)g)->dme->**(int DataMethodRep::**)v = *val->i;
}

void NIDRProblemDescDB::method_ivec(const char *keyname, Values *val, void **g,
                                    void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  int *z = val->i;
  ...
  iv->resize(n);
  for (i = 0; i < n; i++) (*iv)[i] = z[i];
}

void NIDRProblemDescDB::method_ilit2(const char *keyname, Values *val, void **g,
                                     void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
...
void NIDRProblemDescDB::method_ilit2p(const char *keyname, Values *val,
                                      void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
...
void NIDRProblemDescDB::method_lit(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Meth_Info **)g)->dme->*((Method_mp_lit *)v)->sp =
      ((Method_mp_lit *)v)->lit;
}

void NIDRProblemDescDB::method_litc(const char *keyname, Values *val, void **g,
                                    void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
...
void NIDRProblemDescDB::method_litp(const char *keyname, Values *val, void **g,
                                    void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  Real t = *val->r;
  if (t <= 0.) botch("%s must be positive", keyname);
...
void NIDRProblemDescDB::method_litz(const char *keyname, Values *val, void **g,
                                    void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  Real t = *val->r;
  if (t < 0.) botch("%s must be nonnegative", keyname);
...
void NIDRProblemDescDB::method_order(const char *keyname, Values *val, void **g,
                                     void *v) {
  (*(Meth_Info **)g)->dme->*((Method_mp_ord *)v)->sp =
      ((Method_mp_ord *)v)->ord;
}

void NIDRProblemDescDB::method_num_resplevs(const char *keyname, Values *val,
                                            void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  Real *r;
  ...
  n = val->n;
  for (i = k = 0; i < n; ++i) k += z[i];
  if (ra->length() != (int)k)
    botch(
        "number of %s = %u does not match\n"
        "%s specification of %u response levels",
        keyname + 4, (Uint)ra->length(), keyname, (Uint)k);
  r = new Real[k];
  for (i = 0; i < k; i++) r[i] = (*ra)[i];
  (*rva).resize(n);
  for (i = j = je = 0; i < n; i++) {
    m = z[i];
    (*rva)[i].resize(m);
    ra = &(*rva)[i];
    for (i1 = 0, je += m; j < je; ++i1, ++j) (*ra)[i1] = r[j];
  }
  delete[] r;
}

void NIDRProblemDescDB::method_sizet(const char *keyname, Values *val, void **g,
                                     void *v) {
...
void NIDRProblemDescDB::method_resplevs(const char *keyname, Values *val,
                                        void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  Real *r = val->r;
  RealVector *ra;
  ...
  ra = &(*rva)[0];
  ra->resize(n);
  for (i = 0; i < n; ++i) (*ra)[i] = r[i];
}

void NIDRProblemDescDB::method_resplevs01(const char *keyname, Values *val,
                                          void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
  Real *r = val->r, t;
  RealVector *ra;
  ...
  for (i = 0; i < n; ++i) {
    (*ra)[i] = t = r[i];
    if (t < 0. || t > 1.) botch("%s must be between 0 and 1", keyname);
  }
}

void NIDRProblemDescDB::method_shint(const char *keyname, Values *val, void **g,
                                     void *v) {
  (*(Meth_Info **)g)->dme->**(short DataMethodRep::**)v = *val->i;
}

void NIDRProblemDescDB::method_ushint(const char *keyname, Values *val,
                                      void **g, void *v) {
  (*(Meth_Info **)g)->dme->**(unsigned short DataMethodRep::**)v = *val->i;
}

void NIDRProblemDescDB::method_usharray(const char *keyname, Values *val,
                                        void **g, void *v) {
  UShortArray *usa =
...
void NIDRProblemDescDB::method_szarray(const char *keyname, Values *val,
                                       void **g, void *v) {
  SizetArray *sza =
...
void NIDRProblemDescDB::method_slit2(const char *keyname, Values *val, void **g,
                                     void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
...
void NIDRProblemDescDB::method_utype_lit(const char *keyname, Values *val,
                                         void **g, void *v) {
  DataMethodRep *dm = (*(Meth_Info **)g)->dme;
...
void NIDRProblemDescDB::method_start(const char *keyname, Values *val, void **g,
                                     void *v) {
  Meth_Info *mi;

  if (!(mi = new Meth_Info))
  Botch:
    botch("new failure in method_start");
  if (!(mi->dme0 = new DataMethod)) goto Botch;
  mi->dme = mi->dme0->dataMethodRep.get();
  *g = (void *)mi;
}

static void scale_chk(StringArray &ST, RealVector &S, const char *what,
                      const char **univ) {
  const char *s, **u;
  ...
  for (i = nbad = vseen = 0; i < n; ++i) {
    ...
    NIDRProblemDescDB::squawk("\"%s\" cannot appear in %s_scale_types", s,
                              what);
    ++nbad;
  break2:;
  }
...
void NIDRProblemDescDB::method_stop(const char *keyname, Values *val, void **g,
                                    void *v) {
...
void NIDRProblemDescDB::method_str(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Meth_Info **)g)->dme->**(String DataMethodRep::**)v = *val->s;
}

void NIDRProblemDescDB::method_strL(const char *keyname, Values *val, void **g,
                                    void *v) {
  StringArray *sa =
      &((*(Meth_Info **)g)->dme->**(StringArray DataMethodRep::**)v);
  const char **s = val->s;
  size_t i, n = val->n;
...
void NIDRProblemDescDB::method_true(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Meth_Info **)g)->dme->**(bool DataMethodRep::**)v = true;
}

void NIDRProblemDescDB::method_type(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Meth_Info **)g)->dme->*((Method_mp_type *)v)->ip =
      ((Method_mp_type *)v)->type;
}

void NIDRProblemDescDB::method_utype(const char *keyname, Values *val, void **g,
                                     void *v) {
  (*(Meth_Info **)g)->dme->*((Method_mp_utype *)v)->ip =
      ((Method_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::method_augment_utype(const char *keyname, Values *val,
                                             void **g, void *v) {
  (*(Meth_Info **)g)->dme->*((Method_mp_utype *)v)->ip |=
      ((Method_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::model_Real(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Mod_Info **)g)->dmo->**(Real DataModelRep::**)v = *val->r;
}

void NIDRProblemDescDB::model_RealDL(const char *keyname, Values *val, void **g,
                                     void *v) {
  Real *r = val->r;
  ...
  for (i = 0; i < n; i++) (*rdv)[i] = r[i];
}

void NIDRProblemDescDB::model_ivec(const char *keyname, Values *val, void **g,
                                   void *v) {
...
void NIDRProblemDescDB::model_false(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Mod_Info **)g)->dmo->**(bool DataModelRep::**)v = false;
}

void NIDRProblemDescDB::model_int(const char *keyname, Values *val, void **g,
                                  void *v) {
  (*(Mod_Info **)g)->dmo->**(int DataModelRep::**)v = *val->i;
}

void NIDRProblemDescDB::model_lit(const char *keyname, Values *val, void **g,
                                  void *v) {
  (*(Mod_Info **)g)->dmo->*((Model_mp_lit *)v)->sp = ((Model_mp_lit *)v)->lit;
}

void NIDRProblemDescDB::model_order(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Mod_Info **)g)->dmo->*((Model_mp_ord *)v)->sp = ((Model_mp_ord *)v)->ord;
}

void NIDRProblemDescDB::model_type(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Mod_Info **)g)->dmo->*((Model_mp_type *)v)->sp =
      ((Model_mp_type *)v)->type;
}

void NIDRProblemDescDB::model_ushint(const char *keyname, Values *val, void **g,
                                     void *v) {
  (*(Mod_Info **)g)->dmo->**(unsigned short DataModelRep::**)v = *val->i;
}

void NIDRProblemDescDB::model_usharray(const char *keyname, Values *val,
                                       void **g, void *v) {
  UShortArray *usa =
...
void NIDRProblemDescDB::model_utype(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Mod_Info **)g)->dmo->*((Model_mp_utype *)v)->sp =
      ((Model_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::model_augment_utype(const char *keyname, Values *val,
                                            void **g, void *v) {
  (*(Mod_Info **)g)->dmo->*((Model_mp_utype *)v)->sp |=
      ((Model_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::model_shint(const char *keyname, Values *val, void **g,
                                    void *v) {
  (*(Mod_Info **)g)->dmo->**(short DataModelRep::**)v = (short)*val->i;
}

void NIDRProblemDescDB::model_sizet(const char *keyname, Values *val, void **g,
                                    void *v) {
  int n = *val->i;  // test value as int, prior to storage as size_t
#ifdef REDUNDANT_INT_CHECKS
  if (n < 0) /* now handled by INTEGER >= 0 in the .nspec file */
...
void NIDRProblemDescDB::model_id_to_index_set(const char *keyname, Values *val,
                                              void **g, void *v) {
...
void NIDRProblemDescDB::model_start(const char *keyname, Values *val, void **g,
                                    void *v) {
  DataModelRep *dm;
  Mod_Info *mi;
  ...
  if (!(mi = new Mod_Info))
  Botch:
    botch("new failure in model_start");
  if (!(mi->dmo0 = new DataModel)) goto Botch;
  dm = mi->dmo = mi->dmo0->dataModelRep.get();
  *g = (void *)mi;
}

void NIDRProblemDescDB::model_stop(const char *keyname, Values *val, void **g,
                                   void *v) {
...
void NIDRProblemDescDB::model_str(const char *keyname, Values *val, void **g,
                                  void *v) {
  (*(Mod_Info **)g)->dmo->**(String DataModelRep::**)v = *val->s;
}

void NIDRProblemDescDB::model_strL(const char *keyname, Values *val, void **g,
                                   void *v) {
  DataModelRep *dm = (*(Mod_Info **)g)->dmo;
  ...
  (dm->*sa).resize(n);
  for (i = 0; i < n; i++) (dm->*sa)[i] = s[i];
}

void NIDRProblemDescDB::model_true(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Mod_Info **)g)->dmo->**(bool DataModelRep::**)v = true;
}

void NIDRProblemDescDB::resp_RealDL(const char *keyname, Values *val, void **g,
                                    void *v) {
  Real *r = val->r;
  RealVector *rv =
      &((*(Resp_Info **)g)->dr->**(RealVector DataResponsesRep::**)v);
  size_t i, n = val->n;
...
void NIDRProblemDescDB::resp_RealL(const char *keyname, Values *val, void **g,
                                   void *v) {
  NIDRProblemDescDB::resp_RealDL(keyname, val, g, v);
}

void NIDRProblemDescDB::resp_intset(const char *keyname, Values *val, void **g,
                                    void *v) {
  IntSet *is = &((*(Resp_Info **)g)->dr->**(IntSet DataResponsesRep::**)v);
  int *z = val->i;
  size_t i, n = val->n;

  for (i = 0; i < n; ++i) is->insert(z[i]);
}

void NIDRProblemDescDB::resp_ivec(const char *keyname, Values *val, void **g,
                                  void *v) {
...
void NIDRProblemDescDB::resp_lit(const char *keyname, Values *val, void **g,
                                 void *v) {
  (*(Resp_Info **)g)->dr->*((Resp_mp_lit *)v)->sp = ((Resp_mp_lit *)v)->lit;
}

void NIDRProblemDescDB::resp_utype(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Resp_Info **)g)->dr->*((Resp_mp_utype *)v)->sp =
      ((Resp_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::resp_augment_utype(const char *keyname, Values *val,
                                           void **g, void *v) {
  (*(Resp_Info **)g)->dr->*((Resp_mp_utype *)v)->sp |=
      ((Resp_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::resp_sizet(const char *keyname, Values *val, void **g,
                                   void *v) {
  int n = *val->i;  // test value as int, prior to storage as size_t
#ifdef REDUNDANT_INT_CHECKS
  if (n < 0) /* now handled by INTEGER >= 0 in the .nspec file */
...
void NIDRProblemDescDB::resp_start(const char *keyname, Values *val, void **g,
                                   void *v) {
  Resp_Info *ri;
  ...
  if (!(ri->dr0 = new DataResponses)) goto Botch;
  ri->dr = ri->dr0->dataRespRep.get();
  *g = (void *)ri;
}

static void BuildLabels(StringArray *sa, size_t nsa, size_t n1, size_t n2,
                        const char *stub) {
  char buf[64];
  size_t i, n0;
  if (nsa) sa->resize(nsa);
  i = n0 = n1;
  while (n1 < n2) {
...
static int mixed_check(IntSet *S, int n, IntArray *iv, const char *what) {
  ...
  for (ISCIter it = S->begin(), ite = S->end(); it != ite; ++it) {
    j = *it;
    if (j < 1 || j > n) {
      if (!nbad++) Squawk("%s values must be between 1 and %d", what, n);
    } else {
      ++(iv->operator[](j - 1));
    }
  }
...
void NIDRProblemDescDB::resp_stop(const char *keyname, Values *val, void **g,
                                  void *v) {
  size_t k, n;
  ...
  scale_chk(dr->primaryRespFnScaleTypes, dr->primaryRespFnScales,
            dr->numLeastSqTerms ? "least_squares_term" : "objective_function",
            log_scaletypes);
  ...
  // The following check was relevant prior to refactoring the way
  // transformations are applied (now in a layered manner) if (
  // dr->primaryRespFnWeights.length() > 0 && dr->varianceType.size() > 0 ) {
  ...
  if ((n = dr->responseLabels.size()) > 0) {
    if (!(k = dr->numResponseFunctions)) {
      if (!(k = dr->numObjectiveFunctions)) k = dr->numLeastSqTerms;
      k += dr->numNonlinearIneqConstraints + dr->numNonlinearEqConstraints;
    }
    if (n != k)
      squawk("Expected %ld response descriptors but found %ld", (long)k,
             (long)n);
  }
...
void NIDRProblemDescDB::check_descriptors_for_repeats(
  ...
  if (sorted_i != sorted.end())
    Squawk("Repeated descriptors (\"%s\") are not permitted",
           sorted_i->c_str());
}
...
void NIDRProblemDescDB::check_responses(std::list<DataResponses> *drl) {
...
void NIDRProblemDescDB::make_response_defaults(std::list<DataResponses> *drl) {
  IntArray *iv;
  StringArray *rl;
  int ni;
...
#define Schk(a, b, c) {#a, &DataResponsesRep::b, &DataResponsesRep::c}
  static RespStr_chk Str_chk[] = {
      // for StringArray checking
      Schk(least_squares_term_scale_types, numLeastSqTerms,
           primaryRespFnScaleTypes),
      Schk(nonlinear_equality_scale_types, numNonlinearEqConstraints,
           nonlinearEqScaleTypes),
      Schk(nonlinear_inequality_scale_types, numNonlinearIneqConstraints,
           nonlinearIneqScaleTypes),
      Schk(objective_function_scale_types, numObjectiveFunctions,
           primaryRespFnScaleTypes)};
  static RespDVec_chk RespVec_chk_Bound[] = {
      // Bounds:  length must be right
      Schk(least_squares_weights, numLeastSqTerms, primaryRespFnWeights),
      Schk(multi_objective_weights, numObjectiveFunctions,
           primaryRespFnWeights),
      Schk(nonlinear_equality_targets, numNonlinearEqConstraints,
           nonlinearEqTargets),
      Schk(nonlinear_inequality_lower_bounds, numNonlinearIneqConstraints,
           nonlinearIneqLowerBnds),
      Schk(nonlinear_inequality_upper_bounds, numNonlinearIneqConstraints,
           nonlinearIneqUpperBnds)};
  static RespDVec_chk RespVec_chk_Scale[] = {
      // Scales:  length must be right
      // no longer checking LSQ or Obj scales due to fields
      // Schk(least_squares_term_scales,numLeastSqTerms,primaryRespFnScales),
      Schk(nonlinear_equality_scales, numNonlinearEqConstraints,
           nonlinearEqScales),
      Schk(nonlinear_inequality_scales, numNonlinearIneqConstraints,
           nonlinearIneqScales)  //,
...
#define Numberof(x) sizeof(x) / sizeof(x[0])
  ...
  for (; It != Ite; It++) {
    ...
    for (sc = Str_chk, i = 0; i < Numberof(Str_chk); ++sc, ++i)
      if ((n1 = dr->*sc->n) && (n = (dr->*sc->sa).size()) > 0 && n != n1 &&
          n != 1)
        squawk("%s must have length 1 or %lu, not %lu", sc->what, (UL)n1,
               (UL)n);
    ...
    if ((n = dr->numLeastSqTerms)) {
    ...
    } else if ((nf = no + dr->numNonlinearEqConstraints +
                     dr->numNonlinearIneqConstraints)) {
      if (!nrl) {
        rl->resize(nf);
        if (no == 1)
          (*rl)[0] = "obj_fn";
      ...
      if (!nrl) BuildLabels(rl, nf, 0, nf, "response_fn_");
    }
    for (rdvc = RespVec_chk_Bound, i = 0; i < Numberof(RespVec_chk_Bound);
         ++rdvc, ++i)
      if ((n1 = dr->*rdvc->n) && (n = (dr->*rdvc->rv).length()) && n != n1)
        squawk("%s needs %lu elements, not %lu", rdvc->what, (UL)(dr->*rdvc->n),
               (UL)n);
    for (rdvc = RespVec_chk_Scale, i = 0; i < Numberof(RespVec_chk_Scale);
         ++rdvc, ++i)
      if ((n1 = dr->*rdvc->n) && (n = (dr->*rdvc->rv).length()) && n != n1 &&
          n != 1)
        squawk("%s needs %lu elements (or just one), not %lu", rdvc->what,
               (UL)n1, (UL)n);
    if (dr->methodSource == "vendor" && dr->fdGradStepSize.length() > 1)
      squawk(
          "vendor numerical gradients only support a single "
    ...
    if (dr->gradientType == "mixed") {
      ...
      if (!(mixed_check(&dr->idAnalyticGrads, ni, iv, "id_analytic_gradients") +
            mixed_check(&dr->idNumericalGrads, ni, iv,
    ...
    if (dr->hessianType == "mixed") {
      ...
      // iv->resize(nf);
      iv->assign(nf, 0);
      if (!(mixed_check(&dr->idAnalyticHessians, ni, iv,
                        "id_analytic_hessians") +
            mixed_check(&dr->idNumericalHessians, ni, iv,
...
void NIDRProblemDescDB::resp_str(const char *keyname, Values *val, void **g,
                                 void *v) {
  (*(Resp_Info **)g)->dr->**(String DataResponsesRep::**)v = *val->s;
}

void NIDRProblemDescDB::resp_strL(const char *keyname, Values *val, void **g,
                                  void *v) {
  StringArray *sa =
...
void NIDRProblemDescDB::resp_false(const char *keyname, Values *val, void **g,
                                   void *v) {
  (*(Resp_Info **)g)->dr->**(bool DataResponsesRep::**)v = false;
}

void NIDRProblemDescDB::resp_true(const char *keyname, Values *val, void **g,
                                  void *v) {
...
void NIDRProblemDescDB::env_true(const char *keyname, Values *val, void **g,
                                 void *v) {
  (*(DataEnvironmentRep **)g)->**(bool DataEnvironmentRep::**)v = true;
}

void NIDRProblemDescDB::env_int(const char *keyname, Values *val, void **g,
                                void *v) {
...
void NIDRProblemDescDB::env_start(const char *keyname, Values *val, void **g,
                                  void *v) {
  *g = (void *)pDDBInstance->environmentSpec.dataEnvRep.get();
}

void NIDRProblemDescDB::env_str(const char *keyname, Values *val, void **g,
                                void *v) {
  (*(DataEnvironmentRep **)g)->**(String DataEnvironmentRep::**)v = *val->s;
}

/// set a value for an unsigned short type
void NIDRProblemDescDB::env_utype(const char *keyname, Values *val, void **g,
                                  void *v) {
...
void NIDRProblemDescDB::env_augment_utype(const char *keyname, Values *val,
                                          void **g, void *v) {
  (*(DataEnvironmentRep **)g)->*((Env_mp_utype *)v)->sp |=
      ((Env_mp_utype *)v)->utype;
}

void NIDRProblemDescDB::env_strL(const char *keyname, Values *val, void **g,
                                 void *v) {
  StringArray *sa =
...
void NIDRProblemDescDB::method_tr_final(const char *keyname, Values *val,
                                        void **g, void *v) {
  ...
  for (i = 0; i < num_init; ++i) {
    ...
    if (init_size_i < min_init_size) min_init_size = init_size_i;
  }
  if (data_method.trustRegionMinSize > min_init_size) {
    if (num_init)
      botch("specified initial TR size less than minimum TR size");
    else
      botch("minimum TR size must be <= 1.");
  }
  ...
  if (data_method.trustRegionContractTrigger <= 0. ||
    ...
    botch(
        "expand/contract threshold values must satisfy\n\t"
  ...
  if (data_method.trustRegionContract <= 0. ||
    ...
    warn(
        "contraction_factor = 1.0 is valid, but should be < 1\n\t"
...
void NIDRProblemDescDB::var_RealLb(const char *keyname, Values *val, void **g,
                                   void *v) {
  Real b, *r;
  RealVector *rv;
  Var_brv *V;
...
void NIDRProblemDescDB::var_RealUb(const char *keyname, Values *val, void **g,
                                   void *v) {
  Real b, *r;
  RealVector *rv;
  Var_brv *V;
...
void NIDRProblemDescDB::var_IntLb(const char *keyname, Values *val, void **g,
                                  void *v) {
  Var_biv *V = (Var_biv *)v;
...
void NIDRProblemDescDB::var_categorical(const char *keyname, Values *val,
                                        void **g, void *v) {
  BitArray *ba = &((*(Var_Info **)g)->dv->**(BitArray DataVariablesRep::**)v);
  const char **s = val->s;
  size_t i, n = val->n;
...
void NIDRProblemDescDB::var_newrvec(const char *keyname, Values *val, void **g,
                                    void *v) {
  ...
  if (!(rv = new RealVector(n, false))) botch("new failure in var_newrvec");
  vi->**(RealVector * Var_Info::**)v = rv;
  for (i = 0; i < n; i++) (*rv)[i] = r[i];
}

void NIDRProblemDescDB::var_newivec(const char *keyname, Values *val, void **g,
                                    void *v) {
  Var_Info *vi = *(Var_Info **)g;
  IntVector *iv;
  size_t i, n = val->n;
  ...
  if (!(iv = new IntVector(n, false))) botch("new failure in var_newivec");
  vi->**(IntVector * Var_Info::**)v = iv;
  for (i = 0; i < n; i++) (*iv)[i] = z[i];
}

void NIDRProblemDescDB::var_newiarray(const char *keyname, Values *val,
                                      void **g, void *v) {
  Var_Info *vi = *(Var_Info **)g;
  IntArray *iv;
  size_t i, n = val->n;
  ...
  if (!(iv = new IntArray(n))) botch("new failure in var_intarray");
  vi->**(IntArray * Var_Info::**)v = iv;
  for (i = 0; i < n; i++) (*iv)[i] = z[i];
}

void NIDRProblemDescDB::var_newsarray(const char *keyname, Values *val,
                                      void **g, void *v) {
  Var_Info *vi = *(Var_Info **)g;
  StringArray *sa;
  size_t i, n = val->n;
  ...
  if (!(sa = new StringArray(n))) botch("new failure in var_newsarray");
  vi->**(StringArray * Var_Info::**)v = sa;
  for (i = 0; i < n; i++) (*sa)[i] = z[i];
}

void NIDRProblemDescDB::var_rvec(const char *keyname, Values *val, void **g,
                                 void *v) {
  RealVector *rv =
...
void NIDRProblemDescDB::var_ivec(const char *keyname, Values *val, void **g,
                                 void *v) {
...
// {
//   StringArray *sa = &((*(Var_Info**)g)->dv->**(StringArray
//   DataVariablesRep::**)v); size_t i, n = val->n; sa->resize(n);
...
void NIDRProblemDescDB::var_sizet(const char *keyname, Values *val, void **g,
                                  void *v) {
  int n = *val->i;  // test value as int, prior to storage as size_t
#ifdef REDUNDANT_INT_CHECKS
  if (n <= 0) /* now handled by INTEGER > 0 in the .nspec file */
...
void NIDRProblemDescDB::var_type(const char *keyname, Values *val, void **g,
                                 void *v) {
  (*(Var_Info **)g)->dv->*((Var_mp_type *)v)->sp = ((Var_mp_type *)v)->type;
}

void NIDRProblemDescDB::var_start(const char *keyname, Values *val, void **g,
                                  void *v) {
  Var_Info *vi;

  if (!(vi = new Var_Info))
  Botch:
    botch("new failure in var_start");
  memset(vi, 0, sizeof(Var_Info));
  if (!(vi->dv_handle = new DataVariables)) goto Botch;
  vi->dv = vi->dv_handle->dataVarsRep.get();
  *g = (void *)vi;
}

void NIDRProblemDescDB::var_true(const char *keyname, Values *val, void **g,
                                 void *v) {
...
static void wrong_number(const char *what, const char *kind, size_t nsv,
                         size_t m) {
...
static void not_div(const char *kind, size_t nsv, size_t m) {
  Squawk(
      "Number of %s set_values (%d) not evenly divisible by number of "
      "variables (%d); use num_set_values for unequal apportionment",
      kind, (int)nsv, (int)m);
}

static void suppressed(const char *kind, int ndup, int *ip, String *sp,
                       Real *rp) {
  const char *s;
  int i, nother;
  ...
  if (nother) {
    s = "s" + (nother == 1);
    Squawk("Warning%s of %d other duplicate %s value%s suppressed", s, nother,
           kind, s);
  }
}
...
static void Vgen_ContinuousDes(DataVariablesRep *dv, size_t offset) {
  ...
  if (L->length() == 0) Set_rv(L, -dbl_inf, n);
  if (U->length() == 0) Set_rv(U, dbl_inf, n);
  if (V->length() == 0) {
    V->sizeUninitialized(n);
    for (i = 0; i < n;
         i++) {  // init to mean or 0, repairing to bounds if needed
...
static void Vgen_DiscreteDesRange(DataVariablesRep *dv, size_t offset) {
  ...
  if (L->length() == 0) Set_iv(L, INT_MIN, n);
  if (U->length() == 0) Set_iv(U, INT_MAX, n);
  if (V->length() == 0) {
    V->sizeUninitialized(n);
    for (i = 0; i < n; ++i) {
...
static void Vchk_NormalUnc(DataVariablesRep *dv, size_t offset, Var_Info *vi) {
  ...
  if (B->length() && wronglen(n, B, "nuv_lower_bounds")) return;
  B = &dv->normalUncUpperBnds;
  if (B->length() && wronglen(n, B, "nuv_upper_bounds")) return;
}
  ...
  if (IP->length()) {  // in this case, don't nudge since not a default value
    ...
    for (i = offset, j = 0; j < n; ++i, ++j)
      if ((*IP)[j] < (*L)[j])
        ...
        (*V)[i] = (*IP)[j];
  } else {  // > bds is 0, 1, 2, or 3 (0 ==> no bounds given, nothing more to
            // do)
...
static void Vchk_LognormalUnc(DataVariablesRep *dv, size_t offset,
                              Var_Info *vi) {
  size_t n;
  ...
  if (L->length()) {
    if (wronglen(n, L, "lnuv_lambdas")) return;
    if (wronglen(n, Z = &dv->lognormalUncZetas, "lnuv_zetas")) return;
  } else if (M->length()) {
    if (wronglen(n, M, "lnuv_means")) return;
  ...
  if (B->length() && wronglen(n, B, "lnuv_lower_bounds")) return;

  // upper bounds
  B = &dv->lognormalUncUpperBnds;
  if (B->length() && wronglen(n, B, "lnuv_upper_bounds")) return;
}
...
static void Vgen_LognormalUnc(DataVariablesRep *dv, size_t offset) {
  ...
  if (num_U)
    ...
    Set_rv(U, dbl_inf, n);  // default dist; global inferred below

  for (i = offset, j = 0; j < n; ++i, ++j) {
    // extract mean & stdev, if needed
    if (!num_IP || !num_U) {
      if (num_Lam)  // lambda/zeta
        Pecos::LognormalRandomVariable::moments_from_params((*Lam)[j], (*Z)[j],
                                                            mean, stdev);
      else {
        mean = (*M)[j];
...
static void Vgen_UniformUnc(DataVariablesRep *dv, size_t offset) {
  ...
  if (IP->length()) {
    ...
    for (i = offset, j = 0; j < n; ++i, ++j)
      Pecos::UniformRandomVariable::moments_from_params((*L)[j], (*U)[j],
                                                        (*V)[i], stdev);
}

static void Vchk_LoguniformUnc(DataVariablesRep *dv, size_t offset,
                               Var_Info *vi) {
  size_t j, n;
  Real Lj, Uj;
  RealVector *L, *U;
...
static void Vgen_LoguniformUnc(DataVariablesRep *dv, size_t offset) {
  ...
  if (IP->length()) {
    ...
    for (i = offset, j = 0; j < n; ++i, ++j)
      Pecos::LoguniformRandomVariable::moments_from_params((*L)[j], (*U)[j],
                                                           (*V)[i], stdev);
}

static void Vchk_TriangularUnc(DataVariablesRep *dv, size_t offset,
                               Var_Info *vi) {
  size_t j, n;
  Real Lj, Mj, Uj;
  RealVector *L, *M, *U;
  ...
  if (IP->length()) {
    ...
    for (i = offset, j = 0; j < n; ++i, ++j)
      Pecos::TriangularRandomVariable::moments_from_params(
          (*L)[j], (*M)[j], (*U)[j], (*V)[i], stdev);
  }
}

static void Vchk_ExponentialUnc(DataVariablesRep *dv, size_t offset,
                                Var_Info *vi) {
  size_t n = dv->numExponentialUncVars;
  RealVector *B = &dv->exponentialUncBetas;
  if (wronglen(n, B, "euv_betas")) return;
}
...
static void Vgen_BetaUnc(DataVariablesRep *dv, size_t offset) {
  ...
  if (IP->length()) {
    ...
    for (i = offset, j = 0; j < n; ++i, ++j)
      Pecos::BetaRandomVariable::moments_from_params((*A)[j], (*B)[j], (*L)[j],
                                                     (*U)[j], (*V)[i], stdev);
}
...
static void Vchk_GammaUnc(DataVariablesRep *dv, size_t offset, Var_Info *vi) {
  ...
  A = &dv->gammaUncAlphas;
  B = &dv->gammaUncBetas;
  if (wronglen(n, A, "gauv_alphas") || wronglen(n, B, "gauv_betas")) return;
}
...
static void Vgen_GammaUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    Pecos::GammaRandomVariable::moments_from_params((*A)[j], (*B)[j], mean,
                                                    stdev);
    (*L)[i] = 0.;
    (*U)[i] = mean + 3. * stdev;
    if (num_IP)
...
static void Vchk_GumbelUnc(DataVariablesRep *dv, size_t offset, Var_Info *vi) {
  ...
  B = &dv->gumbelUncBetas;
  if (wronglen(n, A, "guuv_alphas") || wronglen(n, B, "guuv_betas")) return;
}
...
static void Vgen_GumbelUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    Pecos::GumbelRandomVariable::moments_from_params((*A)[j], (*B)[j], mean,
                                                     stdev);
    (*L)[i] = mean - 3. * stdev;
    (*U)[i] = mean + 3. * stdev;
    if (num_IP)
...
static void Vchk_FrechetUnc(DataVariablesRep *dv, size_t offset, Var_Info *vi) {
  ...
  if (wronglen(n, A, "fuv_alphas") || wronglen(n, B, "fuv_betas")) return;
}
...
static void Vgen_FrechetUnc(DataVariablesRep *dv, size_t offset) {
  ...
  U = &dv->continuousAleatoryUncUpperBnds;
  V = &dv->continuousAleatoryUncVars;
  IP = &dv->frechetUncVars;
  size_t i, j, n = dv->numFrechetUncVars, num_IP = IP->length();
  if (num_IP) dv->uncertainVarsInitPt = true;

  for (i = offset, j = 0; j < n; ++i, ++j) {
    Pecos::FrechetRandomVariable::moments_from_params((*A)[j], (*B)[j], mean,
                                                      stdev);
    (*L)[i] = 0.;
    (*U)[i] = mean + 3. * stdev;
    if (num_IP)
...
static void Vchk_WeibullUnc(DataVariablesRep *dv, size_t offset, Var_Info *vi) {
  ...
  if (wronglen(n, A, "wuv_alphas") || wronglen(n, B, "wuv_betas")) return;
}
...
static void Vgen_WeibullUnc(DataVariablesRep *dv, size_t offset) {
  ...
  U = &dv->continuousAleatoryUncUpperBnds;
  V = &dv->continuousAleatoryUncVars;
  IP = &dv->weibullUncVars;
  size_t i, j, n = dv->numWeibullUncVars, num_IP = IP->length();
  if (num_IP) dv->uncertainVarsInitPt = true;

  for (i = offset, j = 0; j < n; ++i, ++j) {
    Pecos::WeibullRandomVariable::moments_from_params((*A)[j], (*B)[j], mean,
                                                      stdev);
    (*L)[i] = 0.;
    (*U)[i] = mean + 3. * stdev;
    if (num_IP)
...
static void Vchk_HistogramBinUnc(DataVariablesRep *dv, size_t offset,
                                 Var_Info *vi) {
  IntArray *nhbp;
  ...
  if (hba = vi->hba) {      // abscissas are required
    ...
    hbo = vi->hbo;
    num_o = (hbo) ? hbo->length() : 0;  // ordinates
    hbc = vi->hbc;
    num_c = (hbc) ? hbc->length() : 0;  // counts
    ...
    if (nhbp = vi->nhbp) {
      ...
      if (num_a % m) {
        Squawk(
            "Number of abscissas (%d) not evenly divisible by number of "
            "variables (%d); Use pairs_per_variable for unequal apportionment",
            num_a, m);
        return;
      } else
        avg_nhbpi = num_a / m;
      ...
      for (j = 0; j < nhbpi; ++j, ++cntr) {
        ...
        if (j < nhbpi - 1) {
          ...
          if (num_c) {
            ...
            y /=
                bin_width;  // convert counts to ordinates (probability density)
          } else
...
static void Vgen_HistogramBinUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    ...
    if (num_IP) {
      ...
      Pecos::HistogramBinRandomVariable::moments_from_params(hist_bin_pairs,
                                                             (*V)[i], stdev);
  }
}
...
static void Vchk_PoissonUnc(DataVariablesRep *dv, size_t offset, Var_Info *vi) {
  ...
  if (wronglen(n, A, "lambdas")) return;
}
...
static void Vchk_BinomialUnc(DataVariablesRep *dv, size_t offset,
                             Var_Info *vi) {
  size_t n = dv->numBinomialUncVars;
  RealVector *A = &dv->binomialUncProbPerTrial;
  IntVector *B = &dv->binomialUncNumTrials;
  if (wronglen(n, A, "prob_per_trial") || wronglen(n, B, "num_trials")) return;
}
...
static void Vgen_BinomialUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    ...
    if (num_IP) {
      ...
      Pecos::BinomialRandomVariable::moments_from_params((*NT)[j], (*Pr)[j],
                                                         mean, std_dev);
      (*V)[i] = (int)mean;
    }
  }
...
static void Vchk_NegBinomialUnc(DataVariablesRep *dv, size_t offset,
                                Var_Info *vi) {
...
static void Vgen_NegBinomialUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    Pecos::NegBinomialRandomVariable::moments_from_params((*NT)[j], (*Pr)[j],
                                                          mean, std_dev);
    (*L)[i] = (*NT)[j];
    (*U)[i] = (int)std::ceil(mean + 3. * std_dev);
    if (num_IP) {
...
static void Vchk_GeometricUnc(DataVariablesRep *dv, size_t offset,
                              Var_Info *vi) {
  size_t n = dv->numGeometricUncVars;
  RealVector *A = &dv->geometricUncProbPerTrial;
  if (wronglen(n, A, "prob_per_trial")) return;
}
...
static void Vgen_GeometricUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    (*L)[i] = 0;
    Pecos::GeometricRandomVariable::moments_from_params((*Pr)[j], mean,
                                                        std_dev);
    (*U)[i] = (int)std::ceil(mean + 3. * std_dev);
    if (num_IP)
      (*V)[i] = (*IP)[j];
...
static void Vchk_HyperGeomUnc(DataVariablesRep *dv, size_t offset,
                              Var_Info *vi) {
  IntVector *A, *B, *C;
...
static void Vgen_HyperGeomUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    ...
    if (num_IP) {
      ...
      Pecos::HypergeometricRandomVariable::moments_from_params((*TP)[j], s, d,
                                                               mean, std_dev);
      (*V)[i] = (int)mean;
    }
  }
...
static void Vchk_HistogramPtIntUnc(DataVariablesRep *dv, size_t offset,
                                   Var_Info *vi) {
  IntArray *nhpip;
  IntVector *hpia;
  RealVector *hpic;
...
static void Vgen_HistogramPtIntUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    ...
    if (num_IP) {
      ...
      Real mean, stdev;
      Pecos::DiscreteSetRandomVariable<int>::moments_from_params(hist_pt_prs,
                                                                 mean, stdev);
...
/// (lexicographically)
static void Vchk_HistogramPtStrUnc(DataVariablesRep *dv, size_t offset,
                                   Var_Info *vi) {
  IntArray *nhpsp;
  StringArray *hpsa;
  RealVector *hpsc;
...
static void Vgen_HistogramPtStrUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    ...
    if (num_IP) {
      ...
      Real mean, stdev;
      Pecos::DiscreteSetRandomVariable<String>::moments_from_params(
          hist_pt_prs, mean, stdev);
      if (hist_pt_prs.size() == 1)
        V[i] = hist_pt_prs.begin()->first;
      else {
...
/// are guaranteed unique since the abscissas must increase
static void Vchk_HistogramPtRealUnc(DataVariablesRep *dv, size_t offset,
                                    Var_Info *vi) {
  IntArray *nhprp;
...
static void Vgen_HistogramPtRealUnc(DataVariablesRep *dv, size_t offset) {
  ...
  for (i = offset, j = 0; j < n; ++i, ++j) {
    ...
    if (num_IP) {
      ...
      Real mean, stdev;
      Pecos::DiscreteSetRandomVariable<Real>::moments_from_params(hist_pt_prs,
                                                                  mean, stdev);
      if (hist_pt_prs.size() == 1)
        V[i] = hist_pt_prs.begin()->first;
      else {
...
static void Vchk_ContinuousIntervalUnc(DataVariablesRep *dv, size_t offset,
                                       Var_Info *vi) {
  ...
  if ((Ilb = vi->CIlb) && (Iub = vi->CIub)) {
    ...
    if ((Ip = vi->CIp)) {
      ...
      if (num_lb != num_p || num_ub != num_p) {
        Squawk(
            "Expected as many lower bounds (%d) and upper bounds (%d) as "
            "probabilities (%d)",
            num_lb, num_ub, num_p);
        return;
      }
    } else if (num_lb != num_ub) {
      Squawk("Expected as many lower bounds (%d) as upper bounds (%d)", num_lb,
             num_ub);
      return;
    }
    ...
    if (nI = vi->nCI) {
      ...
      if (num_lb % m) {
        Squawk(
            "Number of bounds (%d) not evenly divisible by number of variables "
            "(%d); Use num_intervals for unequal apportionment",
            num_lb, m);
        return;
      } else
        avg_nI = num_lb / m;
    ...
    for (i = k = 0; i < m; ++i) {
      ...
      if (!num_p)
        ...
        size_t s = k;
        for (j = 0; j < nIi;
             ++j, ++s) {  // normalize the probabilities to sum to one
          total_prob += (*Ip)[s];
        }
        if (fabs(total_prob - 1.0) > 1.E-10) {
          ...
          Warn(
              "Renormalized probability assignments to sum to one for variable "
              "%d",
              i);
        }
      }
      for (j = 0; j < nIi; ++j, ++k) {
        ...
        if (!Pi.insert(make_pair(interval, probability)).second)
          Squawk(
              "Continuous interval [%g, %g] specified more than once for "
              "variable %d",
        ...
        if (lbj > ubj)
          Squawk(
              "Upper bound less than lower bound: [%g, %g] for interval "
              "variable %d",
              lbj, ubj, i);
      }
      if (lb > ub)
...
static void Vgen_ContinuousIntervalUnc(DataVariablesRep *dv, size_t offset) {
  ...
  size_t i, j, n = dv->numContinuousIntervalUncVars, num_IP = IP->length();
  if (num_IP) dv->uncertainVarsInitPt = true;
...
/// intervals) are checked for uniqueness because we don't have a
/// theoretically sound way to combine duplicate intervals
static void Vchk_DiscreteIntervalUnc(DataVariablesRep *dv, size_t offset,
                                     Var_Info *vi) {
  size_t i, j, k, m, num_p = 0, num_lb, num_ub;
  IntArray *nI;
  ...
  if ((Ilb = vi->DIlb) && (Iub = vi->DIub)) {
    ...
    for (i = k = 0; i < m; ++i) {
      ...
      for (j = 0; j < nIi; ++j, ++k) {
        ...
        if (!Pi.insert(make_pair(interval, probability)).second)
          Squawk(
              "Discrete interval [%d, %d] specified more than once for "
              "variable %d",
              interval.first, interval.second, i);
        if (lb > lbj) lb = lbj;
        if (ub < ubj) ub = ubj;
...
/// equally distributed
static bool check_set_keys(size_t num_v, size_t ds_len, const char *kind,
  ...
  if (key) {
    ...
    if (ds_len != total_ds) {
      wrong_number("set_values", kind, total_ds, ds_len);
      return key;
    }
  } else {  // num_set_values is optional; use average len if no spec
    if (ds_len % num_v) {
      not_div(kind, ds_len, num_v);
      return key;
    } else
...
static void Vchk_DIset(size_t num_v, const char *kind, IntArray *input_ndsi,
                       IntVector *input_dsi, IntSetArray &dsi_all,
                       IntVector &dsi_init_pt) {
  if (!input_dsi) return;
  ...
  for (i = cntr = ndup = 0; i < num_v; ++i) {
    ...
    for (j = 0; j < num_dsi_i; ++j, ++cntr) {
      val = (*input_dsi)[cntr];
      if (!dsi_all_i.insert(val)
               .second) {  // insert returns pair<iterator,bool>
        if (++ndup <= 2)   // warnings suppressed beyond two duplicates
          dupval[ndup - 1] = val;
      }
      if (j < num_dsi_i - 1 && val >= (*input_dsi)[cntr + 1]) misordered = true;
    }
  }
  if (ndup) suppressed(kind, ndup, dupval, 0, 0);
  if (misordered) Squawk("Set values for each %s variable must increase", kind);
...
static void Vchk_DIset(size_t num_v, const char *kind, IntArray *input_ndsi,
                       ...
                       IntRealMapArray &dsi_vals_probs,
                       IntVector &dsi_init_pt) {
  if (!input_dsi) return;
...
static void Vchk_DSset(size_t num_v, const char *kind, IntArray *input_ndss,
                       StringArray *input_dss, StringSetArray &dss_all,
                       StringArray &dss_init_pt) {
  if (!input_dss) return;
  ...
  if (ndup) suppressed(kind, ndup, 0, dupval, 0);
  ...
  for (i = 0; i < num_v; ++i) {
    if (misordered[i]) {
      ...
      Squawk(
          "Elements of %s variables must be provided in ascending order ( %s)",
          kind, mss.str().c_str());
    }
  }
...
static void Vchk_DSset(size_t num_v, const char *kind, IntArray *input_ndss,
                       ...
                       StringRealMapArray &dss_vals_probs,
                       StringArray &dss_init_pt) {
  if (!input_dss) return;
  ...
  for (i = cntr = ndup = 0; i < num_v; ++i) {
    ...
    for (j = 0; j < num_dss_i; ++j, ++cntr) {
      ...
      if (j < num_dss_i - 1 && val >= (*input_dss)[cntr + 1]) misordered = true;
    }
  }
  if (ndup) suppressed(kind, ndup, 0, dupval, 0);
  if (misordered) Squawk("Set values for each %s variable must increase", kind);
...
static void Vchk_DRset(size_t num_v, const char *kind, IntArray *input_ndsr,
  ...
  for (i = cntr = ndup = 0; i < num_v; ++i) {
    ...
    for (j = 0; j < num_dsr_i; ++j, ++cntr) {
      val = (*input_dsr)[cntr];
      if (!dsr_all_i.insert(val)
               .second) {  // insert returns pair<iterator,bool>
        if (++ndup <= 2)   // warnings suppressed beyond two duplicates
          dupval[ndup - 1] = val;
      }
      if (j < num_dsr_i - 1 && val >= (*input_dsr)[cntr + 1]) misordered = true;
    }
  }
  if (ndup) suppressed(kind, ndup, 0, 0, dupval);
  if (misordered) Squawk("Set values for each %s variable must increase", kind);
...
static void Vchk_DRset(size_t num_v, const char *kind, IntArray *input_ndsr,
                       ...
                       RealRealMapArray &dsr_vals_probs,
                       RealVector &dsr_init_pt) {
  if (!input_dsr) return;
...
static void Vchk_Adjacency(size_t num_v, const char *kind,
                           const IntArray &num_e, const IntVector &input_ddsa,
                           RealMatrixArray &dda_all) {
  size_t expected_size = 0;
  for (size_t i = 0; i < num_v; ++i) expected_size += num_e[i] * num_e[i];
  if (expected_size != input_ddsa.length())
    ...
    for (size_t i = 0; i < num_v; ++i) {
      ...
      for (size_t j = 0; j < num_e[i]; ++j)
        for (size_t k = 0; k < num_e[i]; ++k) a_tmp[j][k] = input_ddsa[e_ctr++];
      dda_all.push_back(a_tmp);
    }
  }
}

static bool check_LUV_size(size_t num_v, IntVector &L, IntVector &U,
                           IntVector &V, bool aggregate_LUV, size_t offset) {
  bool init_V = true;
  if (aggregate_LUV) {
...
static bool check_LUV_size(size_t num_v, StringArray &L, StringArray &U,
                           StringArray &V, bool aggregate_LUV, size_t offset) {
  bool init_V = true;
  if (aggregate_LUV) {
    int max_index = offset + num_v - 1;
    if (max_index >= L.size() || max_index >= U.size() || max_index >= V.size())
...
static bool check_LUV_size(size_t num_v, RealVector &L, RealVector &U,
                           RealVector &V, bool aggregate_LUV, size_t offset) {
  bool init_V = true;
  if (aggregate_LUV) {
    int max_index = offset + num_v - 1;
    if (max_index >= L.length() || max_index >= U.length() ||
...
static void Vgen_DIset(size_t num_v, IntSetArray &sets, IntVector &L,
                       IntVector &U, IntVector &V, bool aggregate_LUV = false,
                       size_t offset = 0) {
  ISCIter ie, it;
  size_t i, num_set_i;
...
static void Vgen_DSset(size_t num_v, StringSetArray &sets, StringArray &L,
                       StringArray &U, StringArray &V,
...
static void Vgen_DIset(size_t num_v, IntRealMapArray &vals_probs, IntVector &IP,
  ...
  for (i = offset, j = 0; j < num_v; ++i, ++j) {
    ...
    if (num_vp_j == 0) {  // should not occur
      ...
      if (num_IP)
        V[i] = IP[j];  // presence of value w/i set already checked
      else if (init_V) {
        // OLD: Select the initial value to be closest set value to mean value
        // NEW: Select value from middle or next lower index
...
static void Vgen_DRset(size_t num_v, RealSetArray &sets, RealVector &L,
                       RealVector &U, RealVector &V, bool aggregate_LUV = false,
                       size_t offset = 0) {
  RSCIter ie, it;
  size_t i, num_set_i;
...
static void Vgen_DRset(size_t num_v, RealRealMapArray &vals_probs,
                       RealVector &IP, RealVector &L, RealVector &U,
                       RealVector &V, bool aggregate_LUV = false,
                       size_t offset = 0) {
  RRMCIter ite, it;
  size_t i, j, num_vp_j, num_IP = IP.length();
...
static void Vgen_DSset(size_t num_v, StringRealMapArray &vals_probs,
                       StringArray &IP, StringArray &L, StringArray &U,
                       StringArray &V, bool aggregate_LUV = false,
                       size_t offset = 0) {
...
static void Vchk_DiscreteDesSetInt(DataVariablesRep *dv, size_t offset,
                                   Var_Info *vi) {
...
static void Vchk_DiscreteDesSetStr(DataVariablesRep *dv, size_t offset,
                                   Var_Info *vi) {
...
static void Vchk_DiscreteDesSetReal(DataVariablesRep *dv, size_t offset,
                                    Var_Info *vi) {
  ...
  if (vi->ddsra) {
    ...
    Vchk_Adjacency(dv->numDiscreteDesSetRealVars, kind, num_e, *vi->ddsra,
                   dv->discreteDesignSetRealAdj);
  }
}
...
static void Vchk_DiscreteUncSetInt(DataVariablesRep *dv, size_t offset,
                                   Var_Info *vi) {
...
static void Vchk_DiscreteUncSetStr(DataVariablesRep *dv, size_t offset,
                                   Var_Info *vi) {
...
static void Vchk_DiscreteUncSetReal(DataVariablesRep *dv, size_t offset,
                                    Var_Info *vi) {
...
static void Vchk_DiscreteStateSetInt(DataVariablesRep *dv, size_t offset,
                                     Var_Info *vi) {
...
static void Vgen_DiscreteStateSetInt(DataVariablesRep *dv, size_t offset) {
  Vgen_DIset(dv->numDiscreteStateSetIntVars, dv->discreteStateSetInt,
             dv->discreteStateSetIntLowerBnds, dv->discreteStateSetIntUpperBnds,
             dv->discreteStateSetIntVars);  // no offset, not aggregate L/U/V
}

static void Vchk_DiscreteStateSetStr(DataVariablesRep *dv, size_t offset,
                                     Var_Info *vi) {
...
static void Vgen_DiscreteStateSetStr(DataVariablesRep *dv, size_t offset) {
  Vgen_DSset(dv->numDiscreteStateSetStrVars, dv->discreteStateSetStr,
             dv->discreteStateSetStrLowerBnds, dv->discreteStateSetStrUpperBnds,
             dv->discreteStateSetStrVars);  // no offset, not aggregate L/U/V
}

static void Vchk_DiscreteStateSetReal(DataVariablesRep *dv, size_t offset,
                                      Var_Info *vi) {
...
static void Var_RealBoundIPCheck(DataVariablesRep *dv, Var_rcheck *b) {
  ...
  if (b->L) {
    ...
    if (L->length() == 0)
      ...
      Squawk("%s_lower_bounds needs %lu elements, not %lu", b->name, (UL)n,
             (UL)L->length());
      return;
    }
    U = &(dv->*b->U);
    if (U->length() == 0)
      ...
      Squawk("%s_upper_bounds needs %lu elements, not %lu", b->name, (UL)n,
             (UL)U->length());
      return;
    }
  }
  ...
  if (ndflt == 0)
    for (i = 0; i < n; i++) {
      if ((*L)[i] > (*U)[i])
        Squawk("lower bound for %s variable %s exceeds upper bound", b->name,
               Var_Name(sa, namebuf, i));
    }
  if (b->V == 0) return;
  V = &(dv->*b->V);
  if (V->length() == 0) return;
  if (V->length() != n) {
    Squawk("initial point for %s needs %lu elements, not %lu", b->name, (UL)n,
           (UL)V->length());
    return;
  }
  if (L->length() > 0) {
...
static void Var_IntBoundIPCheck(DataVariablesRep *dv, Var_icheck *ib) {
  ...
  if ((n = dv->*ib->n) == 0) return;
  L = &(dv->*ib->L);
  ndflt = 0;
  if (L->length() == 0)
    ++ndflt;
  else if (L->length() != n) {
    Squawk("%s_lower_bounds needs %lu elements, not %lu", ib->name, (UL)n,
           (UL)L->length());
    return;
  }
  U = &(dv->*ib->U);
  if (U->length() == 0)
    ...
    Squawk("%s_upper_bounds needs %lu elements, not %lu", ib->name, (UL)n,
           (UL)L->length());
    return;
  }
  sa = 0;
  ...
  if (ndflt == 0)
    for (i = 0; i < n; i++) {
      if ((*L)[i] > (*U)[i])
        Squawk("lower bound for %s variable %s exceeds upper bound", ib->name,
               Var_Name(sa, namebuf, i));
    }
  if (ib->V == 0) return;  // won't happen for discrete variables
  V = &(dv->*ib->V);
  if (V->length() == 0) return;
  if (V->length() != n) {
    Squawk("initial point for %s needs %lu elements, not %lu", ib->name, (UL)n,
           (UL)V->length());
    return;
  }
  if (L->length() > 0) {
...
static Var_uinfo CAUVLbl[CAUVar_Nkinds] = {
    VarLabelInfo(nuv_, NormalUnc),     VarLabelInfo(lnuv_, LognormalUnc),
    VarLabelInfo(uuv_, UniformUnc),    VarLabelInfo(luuv_, LoguniformUnc),
    VarLabelInfo(tuv_, TriangularUnc), VarLabelInfo(euv_, ExponentialUnc),
    VarLabelInfo(beuv_, BetaUnc),      VarLabelInfo(gauv_, GammaUnc),
    VarLabelInfo(guuv_, GumbelUnc),    VarLabelInfo(fuv_, FrechetUnc),
    VarLabelInfo(wuv_, WeibullUnc),    VarLabelInfo(hbuv_, HistogramBinUnc)};
...
void NIDRProblemDescDB::var_stop(const char *keyname, Values *val, void **g,
                                 void *v) {
  ...
  scale_chk(dv->continuousDesignScaleTypes, dv->continuousDesignScales, "cdv",
            auto_log_scaletypes);
  scale_chk(dv->linearIneqScaleTypes, dv->linearIneqScales, "linear_inequality",
            lincon_scaletypes);
  scale_chk(dv->linearEqScaleTypes, dv->linearEqScales, "linear_equality",
            lincon_scaletypes);
...
static VarLabelChk DesignAndStateLabelsCheck[] = {
    {AVI numContinuousDesVars, AVI continuousDesignLabels, "cdv_",
     "cdv_descriptors"},
    ...
    {AVI numDiscreteDesSetIntVars, AVI discreteDesignSetIntLabels, "ddsiv_",
     "ddsiv_descriptors"},
    {AVI numDiscreteDesSetStrVars, AVI discreteDesignSetStrLabels, "ddssv_",
     "ddssv_descriptors"},
    {AVI numDiscreteDesSetRealVars, AVI discreteDesignSetRealLabels, "ddsrv_",
     "ddsrv_descriptors"},
    {AVI numContinuousStateVars, AVI continuousStateLabels, "csv_",
     "csv_descriptors"},
    ...
    {AVI numDiscreteStateSetIntVars, AVI discreteStateSetIntLabels, "dssiv_",
     "dssiv_descriptors"},
    {AVI numDiscreteStateSetStrVars, AVI discreteStateSetStrLabels, "dsssv_",
     "dsssv_descriptors"},
    {AVI numDiscreteStateSetRealVars, AVI discreteStateSetRealLabels, "dssrv_",
     "dssrv_descriptors"},
    {AVI numContinuousDesVars, AVI continuousDesignScaleTypes, 0,
     "cdv_scale_types"}};
#undef AVI
...
static VLreal VLUncertainReal[NUM_UNC_REAL_CONT] = {
    {CAUVar_Nkinds, AVI CAUv, CAUVLbl, DVR continuousAleatoryUncLabels,
     DVR continuousAleatoryUncLowerBnds, DVR continuousAleatoryUncUpperBnds,
     DVR continuousAleatoryUncVars},
    {CEUVar_Nkinds, AVI CEUv, CEUVLbl, DVR continuousEpistemicUncLabels,
     DVR continuousEpistemicUncLowerBnds, DVR continuousEpistemicUncUpperBnds,
     DVR continuousEpistemicUncVars},
    {DAURVar_Nkinds, AVI DAURv, DAURVLbl, DVR discreteRealAleatoryUncLabels,
     DVR discreteRealAleatoryUncLowerBnds, DVR discreteRealAleatoryUncUpperBnds,
     DVR discreteRealAleatoryUncVars},
    {DEURVar_Nkinds, AVI DEURv, DEURVLbl, DVR discreteRealEpistemicUncLabels,
     DVR discreteRealEpistemicUncLowerBnds,
     DVR discreteRealEpistemicUncUpperBnds, DVR discreteRealEpistemicUncVars}};
...
static VLint VLUncertainInt[NUM_UNC_INT_CONT] = {
    {DAUIVar_Nkinds, AVI DAUIv, DAUIVLbl, DVR discreteIntAleatoryUncLabels,
     DVR discreteIntAleatoryUncLowerBnds, DVR discreteIntAleatoryUncUpperBnds,
     DVR discreteIntAleatoryUncVars},
    {DEUIVar_Nkinds, AVI DEUIv, DEUIVLbl, DVR discreteIntEpistemicUncLabels,
     DVR discreteIntEpistemicUncLowerBnds, DVR discreteIntEpistemicUncUpperBnds,
...
static VLstr VLUncertainStr[NUM_UNC_STR_CONT] = {
    {DAUSVar_Nkinds, AVI DAUSv, DAUSVLbl, DVR discreteStrAleatoryUncLabels,
     DVR discreteStrAleatoryUncLowerBnds, DVR discreteStrAleatoryUncUpperBnds,
     DVR discreteStrAleatoryUncVars},
    {DEUSVar_Nkinds, AVI DEUSv, DEUSVLbl, DVR discreteStrEpistemicUncLabels,
     DVR discreteStrEpistemicUncLowerBnds, DVR discreteStrEpistemicUncUpperBnds,
...
#define Vchk_5(x, y, z)             \
  {#x,                              \
   &DataVariablesRep::num##y##Vars, \
   Vgen_##y,                        \
   &DataVariablesRep::z##LowerBnds, \
   &DataVariablesRep::z##UpperBnds, \
   NULL,                            \
   NULL}
...
#define Vchk_7(x, y, z)             \
  {#x,                              \
   ...
   &DataVariablesRep::z##LowerBnds, \
   &DataVariablesRep::z##UpperBnds, \
   &DataVariablesRep::z##Vars,      \
   &DataVariablesRep::z##Labels}
...
static Var_check
    var_mp_check_cv[] = {Vchk_3(continuous_design, ContinuousDes),
    ...
    var_mp_check_cau[] = {Vchk_3(normal_uncertain, NormalUnc),
    ...
    var_mp_check_daui[] = {Vchk_3(poisson_uncertain, PoissonUnc),
                           ...
                           Vchk_3(histogram_point_int_uncertain,
                                  HistogramPtIntUnc)},
    var_mp_check_daus[] = {Vchk_3(histogram_point_str_uncertain,
                                  HistogramPtStrUnc)},
    var_mp_check_daur[] = {Vchk_3(histogram_point_real_uncertain,
                                  HistogramPtRealUnc)},
    var_mp_check_ceu[] = {Vchk_3(continuous_interval_uncertain,
                                 ContinuousIntervalUnc)},
    var_mp_check_deui[] = {Vchk_3(discrete_interval_uncertain,
                                  DiscreteIntervalUnc),
                           Vchk_3(discrete_uncertain_set_integer,
                                  DiscreteUncSetInt)},
    var_mp_check_deus[] = {Vchk_3(discrete_uncertain_set_string,
                                  DiscreteUncSetStr)},
    var_mp_check_deur[] = {
...
void NIDRProblemDescDB::make_variable_defaults(std::list<DataVariables> *dvl) {
  DataVariablesRep *dv;
  IntVector *IL, *IU, *IV;
  ...
  for (; It != Ite; ++It) {
    ...
    for (k = 0; k < NUM_UNC_REAL_CONT; ++k) {
      ...
      vui = vlr->vui;
      vuie = vui + vlr->n;
      for (nu = 0; vui < vuie; ++vui) nu += dv->*vui->n;
      nursave[k] = nu;
      if (!nu) continue;
      L = &(dv->*vlr->LowerBnds);
      U = &(dv->*vlr->UpperBnds);
    ...
    for (c = var_mp_check_cv, ce = c + Numberof(var_mp_check_cv); c < ce; ++c)
      if ((n = dv->*c->n) > 0) (*c->vgen)(dv, offset);  // offset not used
    ...
    for (c = var_mp_check_daui, ce = c + Numberof(var_mp_check_daui); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) {
        (*c->vgen)(dv, offset);
        offset += n;
    ...
    offset = 0.;
    for (c = var_mp_check_daus, ce = c + Numberof(var_mp_check_daus); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) {
        (*c->vgen)(dv, offset);
        offset += n;
    ...
    offset = 0;
    for (c = var_mp_check_daur, ce = c + Numberof(var_mp_check_daur); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) {
        (*c->vgen)(dv, offset);
        offset += n;
    ...
    offset = 0;
    for (c = var_mp_check_deui, ce = c + Numberof(var_mp_check_deui); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) {
        (*c->vgen)(dv, offset);
        offset += n;
    ...
    offset = 0;
    for (c = var_mp_check_deus, ce = c + Numberof(var_mp_check_deus); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) {
        (*c->vgen)(dv, offset);
        offset += n;
    ...
    offset = 0;
    for (c = var_mp_check_deur, ce = c + Numberof(var_mp_check_deur); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) {
        (*c->vgen)(dv, offset);
        offset += n;
    ...
    offset = 0;
    for (c = var_mp_check_dset, ce = c + Numberof(var_mp_check_dset); c < ce;
         ++c)
      if ((n = dv->*c->n) > 0) (*c->vgen)(dv, offset);  // offset not used
    ...
    for (k = 0; k < NUM_UNC_REAL_CONT; ++k) {
      ...
      V = &(dv->*vlr->UncVars);
      sa = &(dv->*vlr->Labels);
      if (!sa->size()) sa->resize(nu);
      i = 0;
      for (vui = vlr->vui; vui < vuie; ++vui) {
        if ((n = dv->*vui->n) == 0) continue;
        for (j = 0; j < n; ++j)
          if ((*L)[i + j] > (*U)[i + j]) {
    ...
    for (vlc = DesignAndStateLabelsCheck,
        vlce = vlc + Numberof(DesignAndStateLabelsCheck);
         vlc < vlce; ++vlc)
      if (vlc->stub && (n = dv->*vlc->n)) {
        sa = &(dv->*vlc->sa);
        if (sa->size() == 0) BuildLabels(sa, n, 0, n, vlc->stub);
      }
  }
}
...
#define AVI &Var_Info::
  ...
  static IntArray *Var_Info::*Ia_delete[] = {
      AVI nddsi, AVI nddss, AVI nddsr, AVI nCI,   AVI nDI,
      AVI nhbp,  AVI nhpip, AVI nhpsp, AVI nhprp, AVI ndusi,
      AVI nduss, AVI ndusr, AVI ndssi, AVI ndsss, AVI ndssr};
  static RealVector *Var_Info::*Rv_delete[] = {
      AVI ddsr, AVI CIlb, AVI CIub, AVI CIp,  AVI DIp, AVI DSIp,
      AVI DSSp, AVI DSRp, AVI dusr, AVI hba,  AVI hbo, AVI hbc,
      AVI hpic, AVI hpsc, AVI hpra, AVI hprc, AVI ucm, AVI dssr};
  static IntVector *Var_Info::*Iv_delete[] = {AVI ddsi,  AVI DIlb,  AVI DIub,
                                              AVI hpia,  AVI dusi,  AVI dssi,
                                              AVI ddsia, AVI ddssa, AVI ddsra};
  static StringArray *Var_Info::*Sa_delete[] = {AVI ddss, AVI hpsa, AVI duss,
                                                AVI dsss};
#undef AVI
  ...
  for (vlc = DesignAndStateLabelsCheck,
    ...
    if ((n = dv->*vlc->n)) {
      ...
      if ((nu = sa->size())) {
        if (nu != n) {
          if (nu == 1 && !vlc->stub) continue;
          squawk("Found %lu rather than %lu %s values", (UL)nu, (UL)n,
                 vlc->name);
        }
      }
    }
  ...
  for (k = nu = nutot = 0; k < NUM_UNC_REAL_CONT; ++k) {
    ...
    if (nuk > 0) {
      nutot += nuk;
      if (VLR_aleatory[k]) nu += nuk;
      if (havelabels) (sa = &(dv->*vlr->Labels))->resize(nuk);
      i = 0;
      vl = &(vi->*vlr->VL);
      for (vui = vlr->vui; vui < vuie; ++vl, ++vui) {
        ...
        if ((sp = vl->s)) {
          if (vl->n != n)
            squawk("Expected %d %s_descriptors, but got %d", n, vui->lbl,
                   vl->n);
          else
            for (j = 0; j < n; ++i, ++j) (*sa)[i] = sp[j];
          free(sp);
        } else
          i += n;
      }
  ...
  for (k = 0; k < NUM_UNC_INT_CONT; ++k) {
    ...
    if (nuk > 0) {
      nutot += nuk;
      if (VLI_aleatory[k]) nu += nuk;
      if (havelabels) (sa = &(dv->*vli->Labels))->resize(nuk);
      i = 0;
      vl = &(vi->*vli->VL);
  ...
  for (k = 0; k < NUM_UNC_STR_CONT; ++k) {
    ...
    if (nuk > 0) {
      nutot += nuk;
      if (VLS_aleatory[k]) nu += nuk;
      if (havelabels) (sa = &(dv->*vls->Labels))->resize(nuk);
      i = 0;
      vl = &(vi->*vls->VL);
  ...
  nd = 0;
  for (vui = DiscSetLbl, vuie = DiscSetLbl + DiscSetVar_Nkinds; vui < vuie;
       ++vui) {
    if ((n = dv->*vui->n) > 0) {
      vui->vchk(dv, 0, vi);
      nd += n;
  ...
  if (nd + nv + nutot == 0) squawk("at least one variable must be specified");
  ...
  for (i = 0; i < n; i++)
    if ((Ia = vi->*Ia_delete[i])) delete Ia;
  n = Numberof(Rv_delete);
  for (i = 0; i < n; i++)
    if ((Rv = vi->*Rv_delete[i])) delete Rv;
  n = Numberof(Iv_delete);
  for (i = 0; i < n; i++)
    if ((Iv = vi->*Iv_delete[i])) delete Iv;
  n = Numberof(Sa_delete);
  for (i = 0; i < n; i++)
    if ((Sa = vi->*Sa_delete[i])) delete Sa;
  delete vi;
...
static void flatten_rva(RealVectorArray *rva, RealVector **prv) {
  ...
  m = rva->size();
  for (i = n = 0; i < m; ++i) n += (*rva)[i].length();
  *prv = rv = new RealVector(n, false);
  for (i = k = 0; i < m; ++i) {
    rv_i = &(*rva)[i];
    n = rv_i->length();
    for (j = 0; j < n; ++j, ++k) (*rv)[k] = (*rv_i)[j];
  }
}
...
static void flatten_iva(IntVectorArray *iva, IntVector **piv) {
  ...
  m = iva->size();
  for (i = n = 0; i < m; ++i) n += (*iva)[i].length();
  *piv = iv = new IntVector(n, false);
  for (i = k = 0; i < m; ++i) {
    iv_i = &(*iva)[i];
    n = iv_i->length();
    for (j = 0; j < n; ++j, ++k) (*iv)[k] = (*iv_i)[j];
  }
}
...
static void flatten_rsa(RealSetArray *rsa, RealVector **prv) {
  ...
  m = rsa->size();
  for (i = n = 0; i < m; ++i) n += (*rsa)[i].size();
  *prv = rv = new RealVector(n, false);
  for (i = k = 0; i < m; ++i) {
    rs_i = &(*rsa)[i];
    for (rs_it = rs_i->begin(), rs_ite = rs_i->end(); rs_it != rs_ite;
         ++rs_it, ++k)
      (*rv)[k] = *rs_it;
  }
}
...
static void flatten_ssa(StringSetArray *ssa, StringArray **psa) {
  ...
  m = ssa->size();
  for (i = n = 0; i < m; ++i) n += (*ssa)[i].size();
  *psa = sa = new StringArray(n);
  for (i = k = 0; i < m; ++i) {
    ss_i = &(*ssa)[i];
    for (ss_it = ss_i->begin(), ss_ite = ss_i->end(); ss_it != ss_ite;
         ++ss_it, ++k)
      (*sa)[k] = *ss_it;
  }
}
...
static void flatten_isa(IntSetArray *isa, IntVector **piv) {
  ...
  m = isa->size();
  for (i = n = 0; i < m; ++i) n += (*isa)[i].size();
  *piv = iv = new IntVector(n, false);
  for (i = k = 0; i < m; ++i) {
    is_i = &(*isa)[i];
    for (is_it = is_i->begin(), is_ite = is_i->end(); is_it != is_ite;
         ++is_it, ++k)
      (*iv)[k] = *is_it;
  }
}
...
static void flatten_rrma_keys(RealRealMapArray *rrma, RealVector **prv) {
  ...
  m = rrma->size();
  for (i = n = 0; i < m; ++i) n += (*rrma)[i].size();
  *prv = rv = new RealVector(n, false);
  for (i = k = 0; i < m; ++i) {
    rrm_i = &(*rrma)[i];
    for (rrm_it = rrm_i->begin(), rrm_ite = rrm_i->end(); rrm_it != rrm_ite;
         ++rrm_it, ++k)
      (*rv)[k] = rrm_it->first;
  }
}
...
static void flatten_irma_keys(IntRealMapArray *irma, IntVector **piv) {
  ...
  m = irma->size();
  for (i = n = 0; i < m; ++i) n += (*irma)[i].size();
  *piv = iv = new IntVector(n, false);
  for (i = k = 0; i < m; ++i) {
    irm_i = &(*irma)[i];
    for (irm_it = irm_i->begin(), irm_ite = irm_i->end(); irm_it != irm_ite;
         ++irm_it, ++k)
      (*iv)[k] = irm_it->first;
  }
}
...
static void flatten_srma_keys(StringRealMapArray *srma, StringArray **psa) {
  ...
  m = srma->size();
  for (i = n = 0; i < m; ++i) n += (*srma)[i].size();
  *psa = sa = new StringArray(n);
  for (i = k = 0; i < m; ++i) {
    srm_i = &(*srma)[i];
    for (srm_it = srm_i->begin(), srm_ite = srm_i->end(); srm_it != srm_ite;
         ++srm_it, ++k)
      (*sa)[k] = srm_it->first;
  }
}
...
static void flatten_real_intervals(const RealRealPairRealMapArray &rrprma,
                                   RealVector **probs, RealVector **lb,
                                   RealVector **ub) {
  size_t i, j, k, m, n;
  RealVector *rvp, *rvlb, *rvub;

  m = rrprma.size();
  for (i = n = 0; i < m; ++i) n += rrprma[i].size();
...
static void flatten_int_intervals(const IntIntPairRealMapArray &iiprma,
  ...
  m = iiprma.size();
  for (i = n = 0; i < m; ++i) n += iiprma[i].size();
...
void NIDRProblemDescDB::check_descriptor_format(const StringArray &labels) {
  ...
  for (; li != labels.end(); ++li) {
    ...
    if (isfloat(*li))
      Squawk(
          "Descriptor \"%s\" is invalid: floating point numbers not permitted",
          li->c_str());
    if (li->empty())
...
void NIDRProblemDescDB::check_variables(std::list<DataVariables> *dvl) {
  ...
  if (pDDBInstance) {
    ...
    for (; It != Ite; ++It) {
      ...
      if ((m = hbp.size())) {
        ...
        for (i = 0; i < m; ++i) total_prs += (*ia)[i] = hbp[i].size();
      ...
      if ((m = hpip.size())) {
        ...
        for (i = 0; i < m; ++i) total_prs += (*ia)[i] = hpip[i].size();
      ...
      if ((m = hpsp.size())) {
        ...
        for (i = 0; i < m; ++i) total_prs += (*ia)[i] = hpsp[i].size();
      ...
      if ((m = hprp.size())) {
        ...
        for (i = 0; i < m; ++i) total_prs += (*ia)[i] = hprp[i].size();
      ...
      if ((n = dv->numContinuousIntervalUncVars)) {
        ...
        flatten_real_intervals(dv->continuousIntervalUncBasicProbs, &vi->CIp,
                               &vi->CIlb, &vi->CIub);
      }
      ...
      if ((n = dv->numDiscreteIntervalUncVars)) {
        ...
        // unroll the array of maps in to separate variables (p, lb, ub)
        flatten_int_intervals(dv->discreteIntervalUncBasicProbs, &vi->DIp,
                              &vi->DIlb, &vi->DIub);
      }
  ...
  for (; It != Ite; ++It) {
    ...
    check_descriptors_for_repeats(
        dvr.continuousDesignLabels, dvr.discreteDesignRangeLabels,
        dvr.discreteDesignSetIntLabels, dvr.discreteDesignSetStrLabels,
        dvr.discreteDesignSetRealLabels, dvr.continuousStateLabels,
        dvr.discreteStateRangeLabels, dvr.discreteStateSetIntLabels,
        dvr.discreteStateSetStrLabels, dvr.discreteStateSetRealLabels,
        dvr.continuousAleatoryUncLabels, dvr.discreteIntAleatoryUncLabels,
        dvr.discreteStrAleatoryUncLabels, dvr.discreteRealAleatoryUncLabels,
        dvr.continuousEpistemicUncLabels, dvr.discreteIntEpistemicUncLabels,
        dvr.discreteStrEpistemicUncLabels, dvr.discreteRealEpistemicUncLabels);
  }
}

void NIDRProblemDescDB::var_str(const char *keyname, Values *val, void **g,
                                void *v) {
  (*(Var_Info **)g)->dv->**(String DataVariablesRep::**)v = *val->s;
}

void NIDRProblemDescDB::var_strL(const char *keyname, Values *val, void **g,
                                 void *v) {
  StringArray *sa =
      &((*(Var_Info **)g)->dv->**(StringArray DataVariablesRep::**)v);
  const char **s = val->s;
  size_t i, n = val->n;
...
static void var_iulbl(const char *keyname, Values *val, VarLabel *vl) {
  ...
  if (!sl) NIDRProblemDescDB::botch("malloc failure in var_ulbl");
  vl->n = n;
  t = (char *)(sl + n);
  for (i = 0; i < n; i++) {
    strcpy(t, s[i]);
...
void NIDRProblemDescDB::var_caulbl(const char *keyname, Values *val, void **g,
                                   void *v) {
...
void NIDRProblemDescDB::var_ceulbl(const char *keyname, Values *val, void **g,
                                   void *v) {
  VarLabel *vl = &(*(Var_Info **)g)->CEUv[(char *)v - (char *)0];
  var_iulbl(keyname, val, vl);
}

void NIDRProblemDescDB::var_dauilbl(const char *keyname, Values *val, void **g,
                                    void *v) {
  VarLabel *vl = &(*(Var_Info **)g)->DAUIv[(char *)v - (char *)0];
  var_iulbl(keyname, val, vl);
}

void NIDRProblemDescDB::var_deuilbl(const char *keyname, Values *val, void **g,
                                    void *v) {
  VarLabel *vl = &(*(Var_Info **)g)->DEUIv[(char *)v - (char *)0];
  var_iulbl(keyname, val, vl);
}

void NIDRProblemDescDB::var_dauslbl(const char *keyname, Values *val, void **g,
                                    void *v) {
  VarLabel *vl = &(*(Var_Info **)g)->DAUSv[(char *)v - (char *)0];
  var_iulbl(keyname, val, vl);
}

void NIDRProblemDescDB::var_deuslbl(const char *keyname, Values *val, void **g,
                                    void *v) {
  VarLabel *vl = &(*(Var_Info **)g)->DEUSv[(char *)v - (char *)0];
  var_iulbl(keyname, val, vl);
}

void NIDRProblemDescDB::var_daurlbl(const char *keyname, Values *val, void **g,
                                    void *v) {
  VarLabel *vl = &(*(Var_Info **)g)->DAURv[(char *)v - (char *)0];
  var_iulbl(keyname, val, vl);
}

void NIDRProblemDescDB::var_deurlbl(const char *keyname, Values *val, void **g,
                                    void *v) {
...
#define MP3(x, y, z)                                                          \
  iface_mp_TYPE_DATA_##x##_##z = {&DataInterfaceRep::x, &DataInterfaceRep::y, \
                                  #z}
...
static Iface_mp_lit MP2(failAction, abort), MP2(failAction, continuation);

static Iface_mp_type MP2s(analysisScheduling, DEDICATED_SCHEDULER_DYNAMIC),
...
static Iface_mp_utype MP2s(interfaceType, TEST_INTERFACE),
    MP2s(interfaceType, FORK_INTERFACE), MP2s(interfaceType, GRID_INTERFACE),
...
static String MP_(algebraicMappings), MP_(idInterface), MP_(inputFilter),
    MP_(outputFilter), MP_(parametersFile), MP_(pluginLibraryPath),
    MP_(resultsFile), MP_(workDir);

static String2DArray MP_(analysisComponents);

static StringArray MP_(analysisDrivers), MP_(copyFiles), MP_(linkFiles);

static bool MP_(activeSetVectorFlag), MP_(allowExistingResultsFlag),
    MP_(asynchFlag), MP_(batchEvalFlag), MP_(dakotaResultsFileLabeled),
    MP_(dirSave), MP_(dirTag), MP_(evalCacheFlag), MP_(fileSaveFlag),
    MP_(fileTagFlag), MP_(nearbyEvalCacheFlag), MP_(numpyFlag),
    MP_(restartFileFlag), MP_(templateReplace), MP_(useWorkdir),
    MP_(verbatimFlag);

static int MP_(analysisServers), MP_(asynchLocalAnalysisConcurrency),
    MP_(asynchLocalEvalConcurrency), MP_(evalServers), MP_(procsPerAnalysis),
    MP_(procsPerEval);
...
#define MP3(x, y, z) \
  method_mp_TYPE_DATA_##x##_##z = {&DataMethodRep::x, &DataMethodRep::y, #z}
#define MP3s(x, y, z) \
  method_mp_TYPE_DATA_##x##_##z = {&DataMethodRep::x, &DataMethodRep::y, z}

static IntVector MP_(primeBase), MP_(refineSamples), MP_(sequenceLeap),
    MP_(sequenceStart), MP_(stepsPerVariable), MP_(generatingVector),
...
static Method_mp_lit MP2(batchSelectionType, naive),
    MP2(batchSelectionType, distance), MP2(batchSelectionType, topology),
    MP2(batchSelectionType, cl), MP2(boxDivision, all_dimensions),
    MP2(boxDivision, major_dimension),
    MP2(convergenceType, average_fitness_tracker),
    MP2(convergenceType, best_fitness_tracker),
    MP2(convergenceType, metric_tracker), MP2(crossoverType, blend),
    MP2(crossoverType, two_point), MP2(crossoverType, uniform),
    MP2(dataDistCovInputType, diagonal), MP2(dataDistCovInputType, matrix),
    // MP2(dataDistType,gaussian),
    // MP2(dataDistType,user),
    // MP2(expansionSampleType,incremental_lhs),
    MP2(exploratoryMoves, adaptive), MP2(exploratoryMoves, multi_step),
    MP2(exploratoryMoves, simple), MP2(fitnessType, domination_count),
    MP2(fitnessType, layer_rank), MP2(fitnessType, linear_rank),
    MP2(fitnessType, merit_function), MP2(fitnessType, proportional),
    MP2(fitnessMetricType, predicted_variance),
    MP2(fitnessMetricType, distance), MP2(fitnessMetricType, gradient),
    // MP2(hybridCollabType,abo),
    // MP2(hybridCollabType,hops),
    // MP2(seqHybridType,adaptive),
    MP2(initializationType, random), MP2(initializationType, unique_random),
    MP2(lipschitzType, global), MP2(lipschitzType, local),
    MP2(meritFunction, merit_max), MP2(meritFunction, merit_max_smooth),
    MP2(meritFunction, merit1), MP2(meritFunction, merit1_smooth),
    MP2(meritFunction, merit2), MP2(meritFunction, merit2_smooth),
    MP2(meritFunction, merit2_squared), MP2(mcmcType, adaptive_metropolis),
    MP2(mcmcType, delayed_rejection), MP2(mcmcType, dili), MP2(mcmcType, dram),
    MP2(mcmcType, mala), MP2(mcmcType, multilevel_mcmc),
    MP2(mcmcType, metropolis_hastings), MP2(mcmcType, multilevel),
    MP2(modelDiscrepancyType, global_kriging),
    MP2(modelDiscrepancyType, global_polynomial), MP2(mutationType, bit_random),
    MP2(mutationType, offset_cauchy), MP2(mutationType, offset_normal),
    MP2(mutationType, offset_uniform), MP2(mutationType, replace_uniform),
    MP2(patternBasis, coordinate), MP2(patternBasis, simplex),
    MP2(pointReuse, all), MP2(proposalCovInputType, diagonal),
    MP2(proposalCovInputType, matrix), MP2(proposalCovType, derivatives),
    MP2(proposalCovType, prior), MP2(proposalCovType, user),
    MP2(reliabilityIntegration, first_order),
    MP2(reliabilityIntegration, second_order), MP2(replacementType, elitist),
    MP2(replacementType, favor_feasible), MP2(replacementType, roulette_wheel),
    MP2(replacementType, unique_roulette_wheel), MP2(rngName, mt19937),
    MP2(rngName, rnum2), MP2(searchMethod, gradient_based_line_search),
    MP2(searchMethod, tr_pds), MP2(searchMethod, trust_region),
    MP2(searchMethod, value_based_line_search), MP2(trialType, grid),
    MP2(trialType, halton), MP2(trialType, random),
    MP2(useSurrogate, inform_search), MP2(useSurrogate, optimize);

static IntVector MP_(mlmcmcSubsamplingSteps);

static Real MP_(mlmcmcTargetVariance), MP_(mlmcmcGreedyResamplingFactor);

static int MP_(mlmcmcInitialChainSamples);
...
static Method_mp_utype_lit MP3s(methodName, dlDetails,
                                DL_SOLVER);  // struct order: ip, sp, utype
...
static Real MP_(absConvTol), MP_(amScale), MP_(centeringParam),
    MP_(collocationRatio), MP_(collocRatioTermsOrder), MP_(constraintPenalty),
    MP_(constrPenalty), MP_(constraintTolerance), MP_(contractFactor),
    MP_(contractStepLength), MP_(convergenceTolerance), MP_(crossoverRate),
    MP_(diliHessTolerance), MP_(diliLISTolerance), MP_(diliSesAbsTol),
    MP_(diliSesRelTol), MP_(drScale), MP_(estVarMetricNormOrder),
    MP_(falseConvTol), MP_(functionPrecision), MP_(globalBalanceParam),
    MP_(gradientTolerance),
    // MP_(hybridProgThresh),
    MP_(hybridLSProb), MP_(grThreshold), MP_(initDelta), MP_(initMeshSize),
    MP_(initStepLength), MP_(initTRRadius), MP_(lineSearchTolerance),
    MP_(localBalanceParam), MP_(malaStepSize), MP_(maxBoxSize), MP_(maxStep),
    MP_(minBoxSize), MP_(minMeshSize), MP_(multilevEstimatorRate),
    MP_(mutationRate), MP_(mutationScale), MP_(percentVarianceExplained),
    MP_(priorPropCovMult), MP_(rCondTolThrottle), MP_(refinementRate),
    MP_(regressionL2Penalty), MP_(relaxFixedFactor), MP_(relaxRecursiveFactor),
    MP_(shrinkagePercent),  // should be called shrinkageFraction
    MP_(singConvTol), MP_(singRadius), MP_(smoothFactor), MP_(solnTarget),
    MP_(solverRoundingTol), MP_(solverTol), MP_(statsRoundingTol),
    MP_(stepLenToBoundary), MP_(threshDelta), MP_(threshStepLength),
    MP_(tiCoverage), MP_(tiConfidenceLevel), MP_(trustRegionContract),
    MP_(trustRegionContractTrigger), MP_(trustRegionExpand),
    MP_(trustRegionExpandTrigger), MP_(trustRegionMinSize),
    MP_(vbdDropTolerance), MP_(volBoxSize), MP_(vns), MP_(wilksConfidenceLevel),
    MP_(xConvTol);

static RealVector MP_(anisoDimPref), MP_(concurrentParameterSets),
    MP_(dataDistCovariance), MP_(dataDistMeans), MP_(finalPoint),
    MP_(hyperPriorAlphas), MP_(hyperPriorBetas), MP_(listOfPoints),
    MP_(predictionConfigList), MP_(proposalCovData), MP_(regressionNoiseTol),
    MP_(relaxFactorSequence), MP_(scalarizationRespCoeffs), MP_(stepVector),
    MP_(trustRegionInitSize);

static RealVectorArray MP_(genReliabilityLevels), MP_(probabilityLevels),
    MP_(reliabilityLevels), MP_(responseLevels);

static unsigned short MP_(adaptedBasisAdvancements),
    // MP_(adaptedBasisInitLevel),
    MP_(cubIntOrder), MP_(dagDepthLimit), MP_(expansionOrder),
    MP_(groupSizeThrottle), MP_(kickOrder), MP_(maxCVOrderCandidates),
    MP_(maxOrder), MP_(quadratureOrder), MP_(softConvLimit),
    MP_(sparseGridLevel), MP_(startOrder), MP_(vbdOrder), MP_(wilksOrder),
    MP_(vbdViaSamplingMethod);

static SizetArray MP_(collocationPointsSeq), MP_(expansionSamplesSeq),
    MP_(pilotSamples), MP_(randomSeedSeq), MP_(startRankSeq);

static UShortArray MP_(expansionOrderSeq), MP_(quadratureOrderSeq),
    MP_(sparseGridLevelSeq), MP_(startOrderSeq), MP_(tensorGridOrder),
    MP_(varPartitions);

static String MP_(advancedOptionsFilename), MP_(betaSolverName),
    MP_(dataDistFile), MP_(displayFormat), MP_(diliHessianType),
    MP_(drScaleType), MP_(exportApproxPtsFile), MP_(exportCorrModelFile),
    MP_(exportCorrVarFile), MP_(exportDiscrepFile), MP_(exportExpansionFile),
    MP_(exportMCMCPtsFile), MP_(historyFile), MP_(hybridGlobalMethodName),
    MP_(hybridGlobalMethodPointer), MP_(hybridGlobalModelPointer),
    MP_(hybridLocalMethodName), MP_(hybridLocalMethodPointer),
    MP_(hybridLocalModelPointer), MP_(idMethod), MP_(importApproxPtsFile),
    MP_(importBuildPtsFile), MP_(importCandPtsFile), MP_(importExpansionFile),
    MP_(importPredConfigs), MP_(logFile), MP_(lowFidModelPointer),
    MP_(modelExportPrefix), MP_(modelPointer),
    MP_(posteriorDensityExportFilename), MP_(posteriorSamplesExportFilename),
    MP_(posteriorSamplesImportFilename), MP_(proposalCovFile),
    MP_(pstudyFilename), MP_(subMethodName), MP_(subMethodPointer),
    MP_(subModelPointer), MP_(generatingVectorFileName),
    MP_(generatingMatricesFileName);

static StringArray MP_(hybridMethodNames), MP_(hybridMethodPointers),
    MP_(hybridModelPointers), MP_(miscOptions);

static bool MP_(adaptExpDesign), MP_(adaptOrder), MP_(adaptPosteriorRefine),
    MP_(adaptRank), MP_(backfillFlag), MP_(calModelDiscrepancy),
    MP_(chainDiagnostics), MP_(chainDiagnosticsCI), MP_(constantPenalty),
    MP_(crossValidation), MP_(crossValidNoiseOnly), MP_(dOptimal),
    MP_(evaluatePosteriorDensity), MP_(expansionFlag), MP_(exportSampleSeqFlag),
    MP_(exportSurrogate), MP_(fixedSeedFlag), MP_(fixedSequenceFlag),
    MP_(generatePosteriorSamples), MP_(gpmsaNormalize), MP_(importApproxActive),
    MP_(importBuildActive), MP_(latinizeFlag), MP_(logitTransform),
    MP_(mainEffectsFlag), MP_(methodScaling), MP_(methodUseDerivsFlag),
    MP_(modelEvidence), MP_(modelEvidLaplace), MP_(modelEvidMC),
    MP_(mutualInfoKSG2), MP_(mutationAdaptive), MP_(normalizedCoeffs),
    MP_(pcaFlag), MP_(posteriorStatsKL), MP_(posteriorStatsKDE),
    MP_(posteriorStatsMutual), MP_(printPopFlag), MP_(pstudyFileActive),
    MP_(randomizeOrderFlag), MP_(regressDiag), MP_(respScalingFlag),
    MP_(showAllEval), MP_(showMiscOptions), MP_(speculativeFlag),
    MP_(standardizedSpace), MP_(stdRegressionCoeffs),
    MP_(toleranceIntervalsFlag), MP_(surrBasedGlobalReplacePts),
    MP_(surrBasedLocalLayerBypass), MP_(tensorGridFlag),
    MP_(truthPilotConstraint), MP_(useTargetVarianceOptimizationFlag),
    MP_(vbdFlag), MP_(volQualityFlag), MP_(wilksFlag), MP_(rank1LatticeFlag),
    MP_(noRandomShiftFlag), MP_(kuo), MP_(cools_kuo_nuyens),
    MP_(naturalOrdering), MP_(radicalInverseOrdering), MP_(digitalNetFlag),
    MP_(noDigitalShiftFlag), MP_(noScramblingFlag),
    MP_(mostSignificantBitFirst), MP_(leastSignificantBitFirst), MP_(joe_kuo),
    MP_(sobol_order_2), MP_(grayCodeOrdering);
...
static int MP_(amPeriodNumSteps), MP_(amStartingStep), MP_(batchSize),
    MP_(batchSizeExplore), MP_(buildSamples), MP_(burnInSamples),
    MP_(chainSamples), MP_(concurrentRandomJobs), MP_(contractAfterFail),
    MP_(covarianceType), MP_(crossoverChainPairs), MP_(diliAdaptEnd),
    MP_(diliAdaptInterval), MP_(diliAdaptStart), MP_(diliInitialWeight),
    MP_(diliSesBlockSize), MP_(diliSesExpRank), MP_(diliSesNumEigs),
    MP_(diliSesOversFactor), MP_(drNumStages), MP_(emulatorOrder),
    MP_(expandAfterSuccess), MP_(evidenceSamples), MP_(iteratorServers),
    MP_(jumpStep), MP_(maxCrossIterations), MP_(maxHifiEvals),
    MP_(mutationRange), MP_(neighborOrder), MP_(newSolnsGenerated),
    MP_(numChains), MP_(numCR), MP_(numSamples), MP_(numSteps), MP_(numSymbols),
    MP_(numTrials), MP_(populationSize), MP_(procsPerIterator),
    MP_(proposalCovUpdatePeriod), MP_(numPushforwardSamples), MP_(randomSeed),
    MP_(samplesOnEmulator), MP_(searchSchemeSize), MP_(subSamplingPeriod),
    MP_(totalPatternSize), MP_(verifyLevel), MP_(vbdViaSamplingNumBins),
    MP_(log2MaxPoints), MP_(numberOfBits), MP_(scrambleSize);

static size_t MP_(collocationPoints), MP_(expansionSamples), MP_(kickRank),
    MP_(maxCVRankCandidates), MP_(maxFunctionEvals), MP_(maxIterations),
    MP_(maxRank), MP_(maxRefineIterations), MP_(maxSolverIterations),
    MP_(numCandidateDesigns), MP_(numCandidates), MP_(numDesigns),
    MP_(numFinalSolutions), MP_(numGenerations), MP_(numOffspring),
    MP_(numParents), MP_(numPredConfigs), MP_(rCondBestThrottle),
    // MP_(startOrder),
    MP_(startRank);
...
static Method_mp_type MP2s(allocationTarget, TARGET_MEAN),
    ...
    MP2s(convergenceToleranceTarget, COST_CONSTRAINT_TARGET),
    MP2s(convergenceToleranceTarget, VARIANCE_CONSTRAINT_TARGET),
    MP2s(convergenceToleranceType, ABSOLUTE_CONVERGENCE_TOLERANCE),
    MP2s(convergenceToleranceType, RELATIVE_CONVERGENCE_TOLERANCE),
    ...
    MP2s(distributionType, COMPLEMENTARY), MP2s(distributionType, CUMULATIVE),
    MP2s(emulatorType, EXPGP_EMULATOR), MP2s(emulatorType, GP_EMULATOR),
    MP2s(emulatorType, KRIGING_EMULATOR), MP2s(emulatorType, MF_PCE_EMULATOR),
    MP2s(emulatorType, MF_SC_EMULATOR), MP2s(emulatorType, ML_PCE_EMULATOR),
    MP2s(emulatorType, PCE_EMULATOR), MP2s(emulatorType, SC_EMULATOR),
    ...
    MP2p(expansionBasisType, TOTAL_ORDER_BASIS), MP2s(expansionType, ASKEY_U),
    ...
    MP2s(lsRegressionType, EQ_CON_LS), MP2s(lsRegressionType, SVD_LS),
    MP2o(meritFn, ArgaezTapia),  // OPTPP enumeration
    MP2o(meritFn, NormFmu),      // OPTPP enumeration
    MP2o(meritFn, VanShanno),    // OPTPP enumeration
    MP2s(methodOutput, DEBUG_OUTPUT), MP2s(methodOutput, NORMAL_OUTPUT),
    MP2s(methodOutput, QUIET_OUTPUT), MP2s(methodOutput, SILENT_OUTPUT),
    ...
    MP2s(regressionType, FT_LS), MP2s(regressionType, FT_RLS2),
...
static Method_mp_utype MP2s(calibrateErrorMode, CALIBRATE_ONE),
    ...
    MP2s(integrationRefine, AIS), MP2s(integrationRefine, IS),
    MP2s(integrationRefine, MMAIS), MP2s(methodName, APPROX_CONTROL_VARIATE),
    MP2s(methodName, ASYNCH_PATTERN_SEARCH), MP2s(methodName, BRANCH_AND_BOUND),
    MP2s(methodName, C3_FUNCTION_TRAIN), MP2s(methodName, COLINY_BETA),
    MP2s(methodName, COLINY_COBYLA), MP2s(methodName, COLINY_DIRECT),
    MP2s(methodName, COLINY_EA), MP2s(methodName, COLINY_PATTERN_SEARCH),
    MP2s(methodName, COLINY_SOLIS_WETS), MP2s(methodName, CONMIN_FRCG),
    MP2s(methodName, CONMIN_MFD), MP2s(methodName, DACE),
    MP2s(methodName, DATA_FIT_SURROGATE_BASED_LOCAL),
    MP2s(methodName, DOT_BFGS), MP2s(methodName, DOT_FRCG),
    MP2s(methodName, DOT_MMFD), MP2s(methodName, DOT_SLP),
    MP2s(methodName, DOT_SQP), MP2s(methodName, EFFICIENT_GLOBAL),
    MP2s(methodName, FSU_CVT), MP2s(methodName, FSU_HALTON),
    MP2s(methodName, FSU_HAMMERSLEY),
    MP2s(methodName, HIERARCH_SURROGATE_BASED_LOCAL), MP2s(methodName, HYBRID),
    MP2s(methodName, MESH_ADAPTIVE_SEARCH), MP2s(methodName, MOGA),
    MP2s(methodName, MULTI_START), MP2s(methodName, NCSU_DIRECT),
    MP2s(methodName, ROL), MP2s(methodName, DEMO_TPL), MP2s(methodName, NL2SOL),
    MP2s(methodName, NLPQL_SQP), MP2s(methodName, NLSSOL_SQP),
    MP2s(methodName, MIT_NOWPAC), MP2s(methodName, MIT_SNOWPAC),
    MP2s(methodName, ADAPTIVE_SAMPLING), MP2s(methodName, BAYES_CALIBRATION),
    MP2s(methodName, GENIE_DIRECT), MP2s(methodName, GENIE_OPT_DARTS),
    MP2s(methodName, GPAIS), MP2s(methodName, GLOBAL_EVIDENCE),
    MP2s(methodName, GLOBAL_INTERVAL_EST), MP2s(methodName, GLOBAL_RELIABILITY),
    MP2s(methodName, IMPORTANCE_SAMPLING), MP2s(methodName, LOCAL_EVIDENCE),
    MP2s(methodName, LOCAL_INTERVAL_EST), MP2s(methodName, LOCAL_RELIABILITY),
    ...
    MP2s(methodName, MULTILEVEL_SAMPLING), MP2s(methodName, POF_DARTS),
    MP2s(methodName, RKD_DARTS), MP2s(methodName, POLYNOMIAL_CHAOS),
    MP2s(methodName, STOCH_COLLOCATION), MP2s(methodName, SURROGATE_BASED_UQ),
    MP2s(methodName, RANDOM_SAMPLING), MP2s(methodName, NONLINEAR_CG),
    MP2s(methodName, NPSOL_SQP), MP2s(methodName, OPTPP_CG),
    MP2s(methodName, OPTPP_FD_NEWTON), MP2s(methodName, OPTPP_G_NEWTON),
    MP2s(methodName, OPTPP_NEWTON), MP2s(methodName, OPTPP_PDS),
    MP2s(methodName, OPTPP_Q_NEWTON), MP2s(methodName, PARETO_SET),
    MP2s(methodName, PSUADE_MOAT), MP2s(methodName, RICHARDSON_EXTRAP),
    MP2s(methodName, SOGA), MP2s(methodName, SURROGATE_BASED_GLOBAL),
    ...
    MP2s(optSubProbSolver, SUBMETHOD_EA), MP2s(optSubProbSolver, SUBMETHOD_EGO),
    ...
    MP2s(pstudyFileFormat, TABULAR_ANNOTATED), MP2s(sampleType, SUBMETHOD_LHS),
    MP2s(sampleType, SUBMETHOD_RANDOM), MP2s(vbdViaSamplingMethod, VBD_BINNED),
    MP2s(vbdViaSamplingMethod, VBD_PICK_AND_FREEZE),
    MP2s(subMethod, SUBMETHOD_AMV_PLUS_U),
    MP2s(subMethod, SUBMETHOD_AMV_PLUS_X), MP2s(subMethod, SUBMETHOD_AMV_U),
    MP2s(subMethod, SUBMETHOD_AMV_X), MP2s(subMethod, SUBMETHOD_QMEA_U),
    MP2s(subMethod, SUBMETHOD_QMEA_X), MP2s(subMethod, SUBMETHOD_TANA_U),
    MP2s(subMethod, SUBMETHOD_TANA_X), MP2s(subMethod, SUBMETHOD_NO_APPROX),
    MP2s(subMethod, SUBMETHOD_EGRA_U), MP2s(subMethod, SUBMETHOD_EGRA_X),
    MP2s(subMethod, SUBMETHOD_ACV_IS), MP2s(subMethod, SUBMETHOD_ACV_MF),
    MP2s(subMethod, SUBMETHOD_ACV_RD), MP2s(subMethod, SUBMETHOD_WEIGHTED_MLMC),
    MP2s(subMethod, SUBMETHOD_COLLABORATIVE),
    MP2s(subMethod, SUBMETHOD_EMBEDDED), MP2s(subMethod, SUBMETHOD_SEQUENTIAL),
    MP2s(subMethod, SUBMETHOD_MUQ), MP2s(subMethod, SUBMETHOD_DREAM),
    MP2s(subMethod, SUBMETHOD_WASABI), MP2s(subMethod, SUBMETHOD_GPMSA),
    MP2s(subMethod, SUBMETHOD_QUESO), MP2s(subMethod, SUBMETHOD_LHS),
    MP2s(subMethod, SUBMETHOD_RANDOM), MP2s(subMethod, SUBMETHOD_OA_LHS),
    MP2s(subMethod, SUBMETHOD_OAS), MP2s(subMethod, SUBMETHOD_BOX_BEHNKEN),
    MP2s(subMethod, SUBMETHOD_CENTRAL_COMPOSITE),
    MP2s(subMethod, SUBMETHOD_GRID), MP2s(subMethod, SUBMETHOD_CONVERGE_ORDER),
...
static Model_mp_lit MP2(approxPointReuse, all), MP2(approxPointReuse, none),
    MP2(approxPointReuse, region), MP2(marsInterpolation, linear),
    MP2(marsInterpolation, cubic), MP2(modelType, active_subspace),
    MP2(modelType, adapted_basis), MP2(modelType, nested),
    MP2(modelType, random_field), MP2(modelType, simulation),
    MP2(modelType, surrogate), MP2(surrogateType, ensemble),
    MP2(surrogateType, global_exp_gauss_proc),
    MP2(surrogateType, global_exp_poly), MP2(surrogateType, global_exp_python),
    MP2(surrogateType, global_function_train),
    MP2(surrogateType, global_gaussian), MP2(surrogateType, global_kriging),
    ...
    MP2(surrogateType, local_taylor), MP2(surrogateType, multipoint_qmea),
    MP2(surrogateType, multipoint_tana), MP2(trendOrder, none),
    MP2(trendOrder, constant), MP2(trendOrder, linear),
    MP2(trendOrder, reduced_quadratic), MP2(trendOrder, quadratic);

static Model_mp_ord MP2s(approxCorrectionOrder, 0),
    MP2s(approxCorrectionOrder, 1), MP2s(approxCorrectionOrder, 2),
    MP2s(polynomialOrder, 1), MP2s(polynomialOrder, 2),
...
static Model_mp_utype MP2s(analyticCovIdForm, EXP_L2),
    MP2s(analyticCovIdForm, EXP_L1), MP2s(exportApproxFormat, TABULAR_NONE),
    MP2s(exportApproxFormat, TABULAR_HEADER),
    MP2s(exportApproxFormat, TABULAR_EVAL_ID),
    MP2s(exportApproxFormat, TABULAR_IFACE_ID),
    MP2s(exportApproxFormat, TABULAR_ANNOTATED),
...
static Real MP_(adaptedBasisCollocRatio), MP_(annRange), MP_(collocationRatio),
    MP_(convergenceTolerance), MP_(decreaseTolerance), MP_(discontGradThresh),
    MP_(discontJumpThresh), MP_(krigingNugget), MP_(percentFold),
    MP_(regressionL2Penalty), MP_(relTolerance), MP_(solverRoundingTol),
    MP_(solverTol), MP_(statsRoundingTol), MP_(truncationTolerance),
...
static RealVector MP_(krigingCorrelations),
    ...
    MP_(primaryRespCoeffs), MP_(secondaryRespCoeffs), MP_(solutionLevelCost);

static IntVector MP_(refineSamples);

static String MP_(advancedOptionsFilename), MP_(costRecoveryMetadata),
    MP_(decompCellType), MP_(exportApproxPtsFile),
    MP_(exportApproxVarianceFile), MP_(idModel),
    // MP_(importApproxPtsFile),
    MP_(importBuildPtsFile), MP_(importChallengePtsFile), MP_(interfacePointer),
    MP_(krigingOptMethod), MP_(modelExportPrefix), MP_(modelImportPrefix),
    MP_(moduleAndClassName), MP_(optionalInterfRespPointer),
    MP_(propagationModelPointer), MP_(refineCVMetric), MP_(responsesPointer),
    MP_(rfDataFileName), MP_(solutionLevelControl), MP_(subMethodPointer),
    MP_(truthModelPointer), MP_(variablesPointer);

static StringArray MP_(diagMetrics), MP_(ensembleModelPointers),
    MP_(primaryVarMaps), MP_(secondaryVarMaps);

static bool MP_(adaptOrder), MP_(adaptRank), MP_(autoRefine),
    MP_(crossValidateFlag), MP_(decompDiscontDetect), MP_(exportSurrogate),
    MP_(importSurrogate), MP_(hierarchicalTags), MP_(identityRespMap),
    // MP_(importApproxActive),
    MP_(importBuildActive), MP_(importChallengeActive),
    MP_(importChalUseVariableLabels), MP_(importUseVariableLabels),
    MP_(modelUseDerivsFlag), MP_(domainDecomp), MP_(pointSelection),
    MP_(pressFlag), MP_(respScalingFlag), MP_(subspaceIdBingLi),
    MP_(subspaceIdConstantine), MP_(subspaceIdEnergy),
    MP_(subspaceBuildSurrogate), MP_(subspaceIdCV), MP_(subspaceCVIncremental),
    MP_(tensorGridFlag);

static unsigned short MP_(adaptedBasisSparseGridLev), MP_(adaptedBasisExpOrder),
    MP_(kickOrder), MP_(maxCVOrderCandidates), MP_(maxOrder), MP_(startOrder);

static short MP_(annNodes), MP_(annRandomWeight), MP_(c3AdvanceType),
    MP_(krigingFindNugget), MP_(krigingMaxTrials), MP_(marsMaxBases),
    MP_(mlsWeightFunction), MP_(polynomialOrder), MP_(rbfBases), MP_(rbfMaxPts),
    MP_(rbfMaxSubsets), MP_(rbfMinPartition);

static int MP_(decompSupportLayers), MP_(initialSamples),
    MP_(maxCrossIterations), MP_(numFolds), MP_(numReplicates),
    MP_(numRestarts), MP_(pointsTotal), MP_(refineCVFolds),
    MP_(softConvergenceLimit), MP_(subMethodProcs), MP_(subMethodServers),
    MP_(subspaceDimension), MP_(subspaceCVMaxRank);

static size_t MP_(collocationPoints), MP_(kickRank), MP_(maxCVRankCandidates),
    MP_(maxFunctionEvals), MP_(maxIterations), MP_(maxRank),
    MP_(maxSolverIterations), MP_(startRank);
//	MP_(verbosity);
...
static IntSet MP_(idAnalyticGrads), MP_(idAnalyticHessians),
    MP_(idNumericalGrads), MP_(idNumericalHessians), MP_(idQuasiHessians);

static IntVector MP_(fieldLengths), MP_(numCoordsPerField);

static RealVector MP_(expConfigVars), MP_(expObservations),
    MP_(primaryRespFnWeights), MP_(nonlinearEqTargets),
    MP_(nonlinearIneqLowerBnds), MP_(nonlinearIneqUpperBnds), MP_(simVariance),
    MP_(fdGradStepSize), MP_(fdHessStepSize), MP_(primaryRespFnScales),
    MP_(nonlinearEqScales), MP_(nonlinearIneqScales);

static Resp_mp_lit MP2(gradientType, analytic), MP2(gradientType, mixed),
    MP2(gradientType, none), MP2(gradientType, numerical),
    MP2(hessianType, analytic), MP2(hessianType, mixed), MP2(hessianType, none),
    MP2(hessianType, numerical), MP2(hessianType, quasi),
    MP2(intervalType, central), MP2(intervalType, forward),
    MP2(methodSource, dakota), MP2(methodSource, vendor),
    MP2(fdGradStepType, absolute), MP2(fdGradStepType, bounds),
    MP2(fdGradStepType, relative), MP2(fdHessStepType, absolute),
    MP2(fdHessStepType, bounds), MP2(fdHessStepType, relative),
    MP2(quasiHessianType, bfgs), MP2(quasiHessianType, damped_bfgs),
    MP2(quasiHessianType, sr1);

static String MP_(dataPathPrefix), MP_(scalarDataFileName), MP_(idResponses);

static StringArray MP_(metadataLabels), MP_(nonlinearEqScaleTypes),
    MP_(nonlinearIneqScaleTypes), MP_(primaryRespFnScaleTypes),
    MP_(primaryRespFnSense), MP_(responseLabels), MP_(varianceType);

static bool MP_(calibrationDataFlag), MP_(centralHess), MP_(interpolateFlag),
    MP_(ignoreBounds), MP_(readFieldCoords);

static size_t MP_(numExpConfigVars), MP_(numExperiments),
    MP_(numFieldLeastSqTerms), MP_(numFieldObjectiveFunctions),
    MP_(numFieldResponseFunctions), MP_(numLeastSqTerms),
    MP_(numNonlinearEqConstraints), MP_(numNonlinearIneqConstraints),
    MP_(numObjectiveFunctions), MP_(numResponseFunctions),
    MP_(numScalarLeastSqTerms), MP_(numScalarObjectiveFunctions),
...
static Env_mp_utype MP2s(postRunInputFormat, TABULAR_NONE),
    ...
    MP2s(tabularFormat, TABULAR_NONE), MP2s(tabularFormat, TABULAR_HEADER),
    MP2s(tabularFormat, TABULAR_EVAL_ID), MP2s(tabularFormat, TABULAR_IFACE_ID),
...
static String MP_(errorFile), MP_(outputFile), MP_(postRunInput),
    MP_(postRunOutput), MP_(preRunInput), MP_(preRunOutput), MP_(readRestart),
    MP_(resultsOutputFile), MP_(runInput), MP_(runOutput), MP_(tabularDataFile),
    MP_(topMethodPointer), MP_(writeRestart);

static bool MP_(checkFlag), MP_(graphicsFlag), MP_(postRunFlag),
    MP_(preRunFlag), MP_(resultsOutputFlag), MP_(runFlag), MP_(tabularDataFlag);

static int MP_(outputPrecision), MP_(stopRestart);
...
static size_t MP_(numBetaUncVars), MP_(numBinomialUncVars),
    MP_(numContinuousDesVars), MP_(numContinuousIntervalUncVars),
    MP_(numContinuousStateVars), MP_(numDiscreteDesRangeVars),
    MP_(numDiscreteDesSetIntVars), MP_(numDiscreteDesSetStrVars),
    MP_(numDiscreteDesSetRealVars), MP_(numDiscreteIntervalUncVars),
    MP_(numDiscreteStateRangeVars), MP_(numDiscreteStateSetIntVars),
    MP_(numDiscreteStateSetStrVars), MP_(numDiscreteStateSetRealVars),
    MP_(numDiscreteUncSetIntVars), MP_(numDiscreteUncSetStrVars),
    MP_(numDiscreteUncSetRealVars), MP_(numExponentialUncVars),
    MP_(numFrechetUncVars), MP_(numGammaUncVars), MP_(numGeometricUncVars),
    MP_(numGumbelUncVars), MP_(numHistogramBinUncVars),
    MP_(numHistogramPtIntUncVars), MP_(numHistogramPtStrUncVars),
    MP_(numHistogramPtRealUncVars), MP_(numHyperGeomUncVars),
    MP_(numLognormalUncVars), MP_(numLoguniformUncVars),
    MP_(numNegBinomialUncVars), MP_(numNormalUncVars), MP_(numPoissonUncVars),
    MP_(numTriangularUncVars), MP_(numUniformUncVars), MP_(numWeibullUncVars);

static IntVector VP_(ddsi), VP_(DIlb), MP_(discreteDesignRangeLowerBnds),
    MP_(discreteDesignRangeUpperBnds), MP_(discreteDesignRangeVars),
    MP_(discreteDesignSetIntVars), MP_(discreteIntervalUncVars),
    MP_(discreteStateRangeLowerBnds), MP_(discreteStateRangeUpperBnds),
    MP_(discreteStateRangeVars), MP_(discreteStateSetIntVars),
    MP_(discreteUncSetIntVars), VP_(DIub), MP_(histogramPointIntUncVars),
    VP_(hpia), VP_(dssi), VP_(ddsia), VP_(ddssa), VP_(ddsra), VP_(dusi);

static IntArray VP_(nddsi), VP_(nddss), VP_(nddsr), VP_(ndssi), VP_(ndsss),
    VP_(ndssr), VP_(ndusi), VP_(nduss), VP_(ndusr), VP_(nhbp), VP_(nhpip),
    VP_(nhpsp), VP_(nhprp), VP_(nCI), VP_(nDI);

static RealVector MP_(betaUncLowerBnds), MP_(betaUncUpperBnds),
    MP_(betaUncVars), MP_(binomialUncProbPerTrial),
    MP_(continuousDesignLowerBnds), MP_(continuousDesignUpperBnds),
    MP_(continuousDesignVars), MP_(continuousDesignScales),
    MP_(continuousIntervalUncVars), MP_(continuousStateLowerBnds),
    MP_(continuousStateUpperBnds), MP_(continuousStateVars),
    MP_(discreteDesignSetRealVars), MP_(discreteStateSetRealVars),
    MP_(discreteUncSetRealVars), MP_(frechetUncBetas), MP_(frechetUncVars),
    MP_(geometricUncProbPerTrial), MP_(gumbelUncBetas), MP_(gumbelUncVars),
    MP_(histogramBinUncVars), MP_(histogramPointRealUncVars),
    MP_(linearEqConstraintCoeffs), MP_(linearEqScales), MP_(linearEqTargets),
    MP_(linearIneqConstraintCoeffs), MP_(linearIneqLowerBnds),
    MP_(linearIneqUpperBnds), MP_(linearIneqScales),
    MP_(negBinomialUncProbPerTrial), MP_(normalUncLowerBnds),
    MP_(normalUncMeans), MP_(normalUncUpperBnds), MP_(normalUncVars),
    MP_(triangularUncModes), MP_(triangularUncVars), MP_(uniformUncVars),
    MP_(weibullUncVars), VP_(ddsr), VP_(dssr), VP_(dusr), VP_(CIlb), VP_(CIub),
    VP_(CIp), VP_(DIp), VP_(DSIp), VP_(DSSp), VP_(DSRp), VP_(hba), VP_(hbo),
    VP_(hbc), VP_(hpic), VP_(hpsc), VP_(hpra), VP_(hprc), VP_(ucm);

static String MP_(idVariables);

static StringArray MP_(continuousDesignLabels), MP_(continuousDesignScaleTypes),
    MP_(continuousStateLabels), MP_(discreteDesignRangeLabels),
    MP_(discreteDesignSetIntLabels), MP_(discreteDesignSetStrLabels),
    MP_(discreteDesignSetRealLabels), MP_(discreteStateRangeLabels),
    MP_(discreteStateSetIntLabels), MP_(discreteStateSetStrLabels),
    MP_(discreteStateSetRealLabels), MP_(discreteDesignSetStrVars),
    MP_(discreteUncSetStrVars), MP_(discreteStateSetStrVars),
    MP_(histogramPointStrUncVars), MP_(linearEqScaleTypes),
    MP_(linearIneqScaleTypes), VP_(hpsa), VP_(ddss), VP_(duss), VP_(dsss);

static BitArray MP_(discreteDesignSetIntCat), MP_(discreteDesignSetRealCat),
    MP_(discreteStateSetIntCat), MP_(discreteStateSetRealCat),
    MP_(discreteUncSetIntCat), MP_(discreteUncSetRealCat);

static Var_brv MP2s(betaUncAlphas, 0.), MP2s(betaUncBetas, 0.),
    MP2s(exponentialUncBetas, 0.), MP2s(exponentialUncVars, 0.),
    MP2s(frechetUncAlphas, 2.), MP2s(gammaUncAlphas, 0.),
    MP2s(gammaUncBetas, 0.), MP2s(gammaUncVars, 0.), MP2s(gumbelUncAlphas, 0.),
    MP2s(lognormalUncErrFacts, 1.), MP2s(lognormalUncLambdas, 0.),
    MP2s(lognormalUncLowerBnds, 0.), MP2s(lognormalUncMeans, 0.),
    MP2s(lognormalUncStdDevs, 0.),
    MP2s(lognormalUncUpperBnds, std::numeric_limits<Real>::infinity()),
    MP2s(lognormalUncVars, 0.), MP2s(lognormalUncZetas, 0.),
    MP2s(loguniformUncLowerBnds, 0.),
    MP2s(loguniformUncUpperBnds, std::numeric_limits<Real>::infinity()),
    MP2s(loguniformUncVars, 0.), MP2s(normalUncStdDevs, 0.),
    ...
    MP2s(weibullUncAlphas, 0.), MP2s(weibullUncBetas, 0.);

static Var_biv MP2s(binomialUncNumTrials, 0), MP2s(binomialUncVars, 0),
    MP2s(geometricUncVars, 0), MP2s(hyperGeomUncNumDrawn, 0),
    MP2s(hyperGeomUncSelectedPop, 0), MP2s(hyperGeomUncTotalPop, 0),
    MP2s(hyperGeomUncVars, 0), MP2s(negBinomialUncNumTrials, 0),
    MP2s(negBinomialUncVars, 0), MP2s(poissonUncVars, 0);

static Var_mp_type Vtype(varsDomain, MIXED_DOMAIN),
    Vtype(varsDomain, RELAXED_DOMAIN), Vtype(varsView, ALL_VIEW),
    Vtype(varsView, DESIGN_VIEW), Vtype(varsView, UNCERTAIN_VIEW),
    Vtype(varsView, ALEATORY_UNCERTAIN_VIEW),
    Vtype(varsView, EPISTEMIC_UNCERTAIN_VIEW), Vtype(varsView, STATE_VIEW);
...
#define N_vam3(x, y, z) \
  NIDRProblemDescDB::var_##x, y, NIDRProblemDescDB::var_##z
