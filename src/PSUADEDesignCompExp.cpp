...
/** This constructor is called for a standard iterator built with data from
    probDescDB. */
PSUADEDesignCompExp::PSUADEDesignCompExp(ProblemDescDB& problem_db,
  ...
  if (methodName == PSUADE_MOAT) {
    ...
    if (numDACERuns == 1) {  // set initial seed
      if (!seedSpec) {       // no user specification: random behavior
        ...
        // is not reused on subsequent invocations since (1) source granularity
        ...
        // induce correlation between sample sets.  Instead, the
        // system-generated case uses the same seed progression procedure below
        // as the user-specified case.  This has the additional benefit that a
        // random run can be recreated by specifying the system-generated seed
        // in the input file.
        randomSeed = generate_system_seed();
        rng.seed(randomSeed);  // seed for use with varyPattern
      }
    } else if (varyPattern) {  // define sequence of seed values for numLHSRuns
                               // > 1
    ...
    MOATSampling* psuadeSampler =
        new MOATSampling(randomSeed, numPartitions + 1);
    ...
    for (i = 0; i < numSamples; i++) {
      if (i > 0 && i % (numContinuousVars + 1) == 0)
        Cout << "   new replicate --------------------------\n";
      write_data(Cout, allSamples[i], numContinuousVars);
      Cout << '\n';
    }
#endif
  }
...
void PSUADEDesignCompExp::enforce_input_rules() {
  ...
  if (numPartitions <= 0) {
    ...
    Cout << "\nWarning: PSUADE MOAT partitions must be odd (even number of "
         << "levels).\n         Setting to partitions = " << numPartitions
         << " (levels = " << numPartitions + 1 << ").\n";
  }
}
