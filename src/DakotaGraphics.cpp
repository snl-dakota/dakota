...
/** Sets up a single event loop for duration of the dakotaGraphics object,
    ...
    instantiate new). */
void Graphics::create_plots_2d(const Variables& vars,
  ...
  if (win2dOn) {  // graphics already active
    ...
    graphics2D->create_plots2d(num_fns + num_cv + num_div +
                               num_drv);  // set # of plots
  }
  ...
  boost::regex obj_re("obj_fn_[1-9][0-9]*"),
      lsq_re("least_sq_term_[1-9][0-9]*"), resp_re("response_fn_[1-9][0-9]*"),
      ineq_re("nln_ineq_con_[1-9][0-9]*"), eq_re("nln_eq_con_[1-9][0-9]*");
  for (i = 0; i < num_fns; ++i) {
  ...
  for (i = 0; i < num_drv; ++i) {
    ...
    graphics2D->change_line_color2d(i + num_fns + num_cv + num_div, 192, 0,
                                    0);  // red
  }
  ...
  Cerr << "\nWarning: Dakota not compiled with X Windows support; consider "
       << "removing\n       \"graphics\" keyword from input file." << std::endl;
#endif  // HAVE_X_GRAPHICS
}
...
void Graphics::add_datapoint(int graphics_cntr, const Variables& vars,
                             const Response& response) {
#ifdef HAVE_X_GRAPHICS
  if (win2dOn) {
    ...
    for (i = 0; i < num_cv; ++i)  // add to each variable graph
      graphics2D->add_datapoint2d(i + num_fns, (double)graphics_cntr,
                                  c_vars[i]);
...
#ifdef HAVE_X_GRAPHICS
  if (win2dOn) graphics2D->add_datapoint2d(i, x, y);
#endif  // HAVE_X_GRAPHICS
...
#ifdef HAVE_X_GRAPHICS
  if (win2dOn) graphics2D->new_dataset2d(i);
#endif  // HAVE_X_GRAPHICS
}
...
#ifdef HAVE_X_GRAPHICS
  if (win2dOn) graphics2D->set_x_label2d(i, x_label);
#endif  // HAVE_X_GRAPHICS
}

void Graphics::set_y_label2d(int i, const char* y_label) {
#ifdef HAVE_X_GRAPHICS
  if (win2dOn) graphics2D->set_y_label2d(i, y_label);
#endif  // HAVE_X_GRAPHICS
}
