...
class TestDriverInterface : public DirectApplicInterface {
 ...
 private:
  ...
  int cantilever_ml();   ///< unscaled cantilever test function for UQ with
                         ///< levels
  ...
  int alternate_short_column_forms(
      int form);  ///< helper fn for alternate forms
  ...
  /// term so that function can not be exactly approximated by a low degree
  /// polynomial
  ...
  int extra_lf_rosenbrock();  ///< an extra low fidelity version of the
                              ///< Rosenbrock function
  ...
  void get_genz_coefficients(int num_dims, Real factor, int c_type,
  ...
  int scalable_text_book();  ///< scalable version of the text_book test
                             ///< function
