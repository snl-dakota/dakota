...
RealVectorArray NonDAdaptiveSampling::drawNewX(int this_k, int respFnCount) {
  ...
  if (batchSize == 1) {
    ...
    /// expensive / to compute. Thus, the following heuristic only applies in
    /// 2D.
    ////***END ATTENTION***
...
void NonDAdaptiveSampling::compare_complices(int dim, std::ostream& output) {
  ...
  /// between / the surrogate and truth model and write zeros for the rest of
  /// the values
  ////***END ATTENTION***
...
void NonDAdaptiveSampling::output_for_optimization(int dim) {
  ...
  output << std::endl
         ...
         /// database? / Something like, but not exactly the following because
         /// it does not work: /
         /// probDescDB.interface_list().front().interface_id() / In lieu of
         ///this, I am hard-coding the function name and other things like / the
         /// domain boundaries, in a general framework this is inadequate
         ////***END ATTENTION***
