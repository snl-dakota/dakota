...
NonDAdaptiveSampling::NonDAdaptiveSampling(ProblemDescDB& problem_db,
...
#pragma region Class Constructor:
  ...
  if (numEmulEval == 0) numEmulEval = 400;
  batchSize = 1;
  ...
  if (batchStrategy.empty()) batchStrategy = "naive";
  scoringMetric = probDescDB.get_string("method.fitness_metric");
  if (scoringMetric == "predicted_variance") scoringMetric = "alm";
  if (scoringMetric.empty()) scoringMetric = "alm";
  ...
  if (misc_options.size() > 0) parse_options();
  ...
  //**NOTE:  We are hardcoding the sample type to LHS and the approximation type
  //to kriging for now if (sampleDesign == RANDOM_SAMPLING)
  //{
  std::shared_ptr<Iterator> gp_build = std::make_shared<NonDLHSSampling>(
      iteratedModel, SUBMETHOD_DEFAULT, samples, randomSeed, rngName,
      varyPattern, ACTIVE_UNIFORM);
  //}
  // else
  //{
  //	gpBuild.assign_rep(new FSUDesignCompExp(iteratedModel, samples,
  //randomSeed, 					   sampleDesign));
  //}
  ...
  gpModel = std::make_shared<DataFitSurrModel>(
      gp_build, iteratedModel, gp_set, gp_view, approx_type, approx_order,
  ...
  ////***ATTENTION***
  //// Until this starts working, we are forcing the candidates to be selected
  ///by / LHS
  ////***END ATTENTION***
  ...
  if (true) {
    ...
    construct_lhs(gpFinalEval, gpModel, SUBMETHOD_DEFAULT, numFinalEmulEval,
                  randomSeed, rngName, vary_pattern);
  } else {
    gpEval =
        construct_fsu_sampler(gpModel, numEmulEval, randomSeed, sampleDesign);
...
#pragma region Quantify Uncertainity:
  ...
  //  size_t dim = numContinuousVars + numDiscreteIntVars +
  ...
  //// which has just the information I am concerned with and then I
  ///post-process / these files with some python scripts
  ////***END ATTENTION***
  std::stringstream ss;
  ss << "improvement.txt";
  ...
  for (int k = 0; k < numRounds; k++) {
    ...
    if (iteratedModel->asynch_flag()) {
      ...
      for (int i = 0; i < new_Xs.size(); i++) {
        ...
        responses_to_add.insert(IntResponsePair(
            iteratedModel->evaluation_id(), iteratedModel->current_response()));
        points_to_add.push_back(iteratedModel->current_variables());
      }
    }
  ...
  for (int ifunc = 0; ifunc < numFunctions; ifunc++) {
    ...
    for (int ilevel = 0; ilevel < num_levels; ilevel++)
      computedProbLevels[ifunc][ilevel] = 0.0;
  }
...
#pragma region Pick New Candidates from Emulator:
  ...
  for (int i = 0; i < numEmulEval; i++) {
    temp_cvars =
        Teuchos::getCol(Teuchos::View, const_cast<RealMatrix&>(all_samples), i);
...
#pragma region Score Emultor sample points based on their approximation variance:
  ...
  for (int i = 0; i < numEmulEval; i++) {
    ...
    for (int respFnCount = 0; respFnCount < numFunctions; respFnCount++) {
      ...
      Real score = gpModel->approximation_variances(
...
#pragma region Score Emulator sample points based on the closest data point:
  ...
  for (int i = 0; i < numEmulEval; i++) {
    ...
    for (int respFnCount = 0; respFnCount < numFunctions; respFnCount++) {
      ...
      int min_index;
      bool first = true;
      for (int j = 0; j < sdv_array.size();
           j++)  // This is a Naiive way to retrieve closest data point. SHOULD
                 // BE RELACED IN THE FUTURE FOR BETTER PERFORMANCE!
      {
        double sq_dist = 0;
...
#pragma region Score Emulator sample points based on the response function difference of closest data point and current candidate:
  ...
  for (int i = 0; i < numEmulEval; i++) {
    ...
    for (int respFnCount = 0; respFnCount < numFunctions; respFnCount++) {
      ...
      for (int j = 0; j < sdv_array.size();
           j++)  // This is a Naiive way to retriev closet data point. SHOULD BE
                 // RELACED IN THE FUTURE FOR BETTER PERFORMANCE!
      {
        double sq_dist = 0;
        const RealVector& c_vars = sdv_array[j].continuous_variables();
        for (int d = 0; d < c_vars.length(); d++) {
      ...
      Real score = fabs(gpMeans[i][respFnCount] -
                        sdr_array[min_index].response_function());
      if (respFnCount == 0 || score > max_score) max_score = score;
    }
    emulEvalScores(i) = max_score;
  }
...
#pragma region Score Emulator sample points based on Bottleneck distance:
  ...
  if (numEmulEval > 0) temp_x = new double[gpCvars[0].length() + 1];
...
void NonDAdaptiveSampling::output_round_data(int round, int respFnCount) {
  ...
  for (int i = 0; i < validationSetSize; i++) {
    ...
    if (outputValidationData) {
      alm_set[i] = (approx_type == "global_kriging")
                       ? calc_score_alm(respFnCount, validationSet[i])
                       : 0;
      ...
      hybrid_set[i] =
          (approx_type == "global_kriging")
              ? calc_score_topo_alm_hybrid(respFnCount, validationSet[i])
              : 0;
    }
  }
  ...
  if (outputValidationData) {
    ...
    for (int i = 0; i < new_Xs.size(); i++) {
      ...
      candidate_out << iteratedModel->current_response().function_value(
                           respFnCount)
                    << std::endl;
    }
...
RealVectorArray NonDAdaptiveSampling::drawNewX(int this_k, int respFnCount) {
  ...
  if (batchSize == 1) {
    ...
    for (i = 0; i < temp_length; i++)
      if (emulEvalScores[max_index] < emulEvalScores[i]) max_index = i;
    ...
    for (i = 1; i < batchSize; i++) {
      ...
      for (j = 0; j < temp_length; j++) {
        ...
        for (int k = 0; k < gpCvars[j].length(); k++) {
          temp_dist += pow(
              (gpCvars[j][k] - selected_data[selected_data.size() - 1][k]), 2);
        }
    ...
    for (i = 0; i < batchSize; i++) {
      ...
      IntResponsePair response_to_add(gpModel->evaluation_id(),
                                      current_response);
    ...
    ////***ATTENTION***
    //// I do this all over the place, but there has to be a better way to
    ///obtain / the dimensionality of the domain under test
    ////***END ATTENTION***
    // BMA: See note above
    int dim = gpModel->approximation_data(respFnCount)
    ...
    //// dimension higher than 2D. Anything higher can be prohibitively
    ///expensive / to compute. Thus, the following heuristic only applies in 2D.
    ////***END ATTENTION***
    ...
    MS_Complex* score_complex = new MS_Complex(
        raw_points, dim + 1, temp_length,
    ...
    if (selected_data.size() < batchSize) {
      ...
      for (i = 0; i < temp_length; i++) {
        for (j = 0; j < selected_data.size(); j++) {
          ...
          temp_dist = sqrt(temp_dist);
          // If it is the first iteration for i, j==1, then set the
          // min_distances
      ...
      while (selected_data.size() < batchSize) {
        for (j = 0; j < temp_length; j++) {
          ...
          for (int k = 0; k < gpCvars[j].length(); k++) {
            temp_dist += pow(
                (gpCvars[j][k] - selected_data[selected_data.size() - 1][k]),
                2);
          }
          temp_dist = sqrt(temp_dist);
          // If it is the first iteration i==1, then set the min_distances
...
void NonDAdaptiveSampling::calc_score_topo_avg_persistence(int respFnCount) {
  ...
  for (int i = 0; i < numEmulEval; i++) {
    for (int d = 0; d < gpCvars[i].length(); d++) temp_x[d] = gpCvars[i][d];
    temp_x[gpCvars[i].length()] = gpMeans[i][respFnCount];
...
void NonDAdaptiveSampling::calc_score_topo_alm_hybrid(int respFnCount) {
  ...
  for (int i = 0; i < numEmulEval; i++) {
    ...
    ModelUtils::continuous_variables(*gpModel, gpCvars[i]);
    temp_x[gpCvars[i].length()] =
        gpMeans[i][respFnCount] +
        sqrt(gpModel->approximation_variances(
            gpModel->current_variables())[respFnCount]);
    ...
    emulEvalScores(i) =
        (score_val + score_val_plus_std + score_val_minus_std) / 3.;
  }
  delete[] temp_x;
#else
  Cout << "ANN library not enabled, therefore cannot compute approximate "
...
void NonDAdaptiveSampling::calc_score_topo_highest_persistence(
    int respFnCount) {
#ifdef HAVE_MORSE_SMALE
  emulEvalScores.resize(numEmulEval);
  ...
  for (int i = 0; i < n_train; i++) {
    const RealVector& c_vars = sdv_array[i].continuous_variables();
    for (int j = 0; j < dim; j++) train_x[i * dim + j] = c_vars[j];
  ...
  for (int i = 0; i < n_cand; i++) {
    for (int j = 0; j < dim; j++) cand_x[i * dim + j] = gpCvars[i][j];
    cand_y[i] = gpMeans[i][respFnCount];
  }
  std::vector<int> ordered_indices = ScoreTOPOHP(
      dim, numKneighbors, train_x, train_y, n_train, cand_x, cand_y, n_cand);
...
Real NonDAdaptiveSampling::calc_score_topo_bottleneck(int respFnCount,
  ...
  for (int d = 0; d < test_point.length(); d++) temp_x[d] = test_point[d];
...
#ifdef HAVE_MORSE_SMALE
  Cout << "Dionysus library not enabled, therefore cannot compute the "
       << "bottleneck distance score, returning NaN" << std::endl;
#else
  Cout << "ANN library not enabled, therefore cannot compute approximate "
...
Real NonDAdaptiveSampling::calc_score_topo_avg_persistence(
    int respFnCount, RealVector& test_point) {
#ifdef HAVE_MORSE_SMALE
  double* temp_x = new double[test_point.length() + 1];
...
Real NonDAdaptiveSampling::calc_score_topo_alm_hybrid(int respFnCount,
  ...
  temp_x[test_point.length()] =
      surrogate_model.current_response().function_value(respFnCount) +
      gpModel->approximation_variances(
          gpModel->current_variables())[respFnCount];
...
#pragma region Parse Options:
  ...
  for (; db_it != db_end; ++db_it)
    if ((delim = find(db_it->begin(), db_it->end(), '=')) != db_it->end()) {
      ...
      if (opt == "candidate_size")
        ...
        if (!(batchStrategy == "naive" || batchStrategy == "distance" ||
              batchStrategy == "topology" || batchStrategy == "cl")) {
        ...
        scoringMetric = val;
        if (!(scoringMetric == "alm" || scoringMetric == "distance" ||
              scoringMetric == "gradient" || scoringMetric == "bottleneck" ||
        ...
        Cerr << "ERROR (NonDAdaptiveSampling): Unknown misc_option: " << opt
             << std::endl;
        abort_handler(-1);
      }
  ...
  if (batchSize > numEmulEval) {
    Cerr << "ERROR (NonDAdaptiveSampling): Cannot use " << batchSize
         << " as the batch size with only " << numEmulEval << " candidates"
         << std::endl;
    abort_handler(-1);
  }
  ...
  if (numKneighbors > numSamples) {
    Cerr << "ERROR (NonDAdaptiveSampling): Cannot use " << numKneighbors
         << " as the number of neighbors in the Morse-Smale "
  ...
  if (scoringMetric == "bottleneck" || scoringMetric == "avg_persistence" ||
      scoringMetric == "alm_topo_hybrid") {
    Cerr << "ERROR (NonDAdaptiveSampling): Cannot use " << scoringMetric
         << " as the scoring metric because ANN is disabled" << std::endl;
    abort_handler(-1);
  }
#endif
  ...
  if (scoringMetric == "bottleneck") {
    Cerr << "ERROR (NonDAdaptiveSampling): Cannot use " << scoringMetric
         << " as the scoring metric because Dionysus is "
         << "disabled" << std::endl;
    abort_handler(-1);
  }
#endif
...
void NonDAdaptiveSampling::compare_complices(int dim, std::ostream& output) {
  ...
  //// Also, I am not sure comparing topologies has proven fruitful to this
  ///point / so it is a waste of time, instead we will just compute the RMSE
  ///between / the surrogate and truth model and write zeros for the rest of the
  ///values
  ////***END ATTENTION***
...
void NonDAdaptiveSampling::output_for_optimization(int dim) {
  ...
  //// to compute.
  ////***END ATTENTION***
  MS_Complex* modelComplex = new MS_Complex(
      model_points, dim + 1, validationSetSize,
      dim > 2 ? 25 : (int)(std::min(pow(3, dim) - 1, 0.4 * validationSetSize)));
  delete[] model_points;
  ...
  output << std::endl
         ...
         << "\t\tanalysis_driver = \'"
         ////***ATTENTION***
         //// Is there a way to extract the test function used from the
         ///database? / Something like, but not exactly the following because it
         ///does not work: / probDescDB.interface_list().front().interface_id()
         //// In lieu of this, I am hard-coding the function name and other
         ///things like / the domain boundaries, in a general framework this is
         ///inadequate
         ////***END ATTENTION***
...
std::shared_ptr<Iterator> NonDAdaptiveSampling::construct_fsu_sampler(
  ...
  return std::make_shared<FSUDesignCompExp>(u_model, num_samples, seed,
                                            sample_type);
}
