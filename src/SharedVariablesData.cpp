...
static const char rcsId[] =
    "@(#) $Id: SharedVariablesData.cpp 6886 2010-08-02 19:13:01Z mseldre $";
...
SharedVariablesDataRep::SharedVariablesDataRep(const ProblemDescDB& problem_db,
                                               const ShortShortPair& view)
    : variablesId(problem_db.get_string("variables.id")),
      variablesCompsTotals(NUM_VC_TOTALS, 0),
      variablesView(view),
      cvStart(0),
      divStart(0),
      dsvStart(0),
...
SharedVariablesDataRep::SharedVariablesDataRep(
    ...
    : variablesCompsTotals(vars_comps_totals),
      ...
      numIDSV(0),
      numIDRV(0),
      allRelaxedDiscreteInt(all_relax_di),
      allRelaxedDiscreteReal(all_relax_dr) {
...
void SharedVariablesDataRep::initialize_components_totals(
  ...
  if (count =
          problem_db.get_sizet("variables.histogram_uncertain.point_real")) {
...
void SharedVariablesDataRep::components_to_totals() {
  ...
  variablesCompsTotals[TOTAL_DDIV] =
      vc_lookup(DISCRETE_DESIGN_RANGE) + vc_lookup(DISCRETE_DESIGN_SET_INT);
  ...
  variablesCompsTotals[TOTAL_CAUV] =
      vc_lookup(NORMAL_UNCERTAIN) + vc_lookup(LOGNORMAL_UNCERTAIN) +
      vc_lookup(UNIFORM_UNCERTAIN) + vc_lookup(LOGUNIFORM_UNCERTAIN) +
      vc_lookup(TRIANGULAR_UNCERTAIN) + vc_lookup(EXPONENTIAL_UNCERTAIN) +
      vc_lookup(BETA_UNCERTAIN) + vc_lookup(GAMMA_UNCERTAIN) +
      vc_lookup(GUMBEL_UNCERTAIN) + vc_lookup(FRECHET_UNCERTAIN) +
      vc_lookup(WEIBULL_UNCERTAIN) + vc_lookup(HISTOGRAM_BIN_UNCERTAIN);
  // discrete aleatory uncertain integer
  variablesCompsTotals[TOTAL_DAUIV] =
      vc_lookup(POISSON_UNCERTAIN) + vc_lookup(BINOMIAL_UNCERTAIN) +
      vc_lookup(NEGATIVE_BINOMIAL_UNCERTAIN) + vc_lookup(GEOMETRIC_UNCERTAIN) +
      vc_lookup(HYPERGEOMETRIC_UNCERTAIN) +
  ...
  variablesCompsTotals[TOTAL_DSIV] =
      vc_lookup(DISCRETE_STATE_RANGE) + vc_lookup(DISCRETE_STATE_SET_INT);
...
void SharedVariablesDataRep::relax_noncategorical(
    const ProblemDescDB& problem_db) {
  ...
  if (relax) {
    ...
    allRelaxedDiscreteInt.reset();
    allRelaxedDiscreteReal.reset();
  } else {  // use of any() allows Mixed case to employ empty arrays
    allRelaxedDiscreteInt.clear();
    allRelaxedDiscreteReal.clear();
    return;
  }

  // Note: NIDR ensures BitArray is empty or sized by number of variables

  const BitArray& ddrv_cat =
      problem_db.get_ba("variables.discrete_design_range.categorical");
  const BitArray& ddsiv_cat =
      problem_db.get_ba("variables.discrete_design_set_int.categorical");
  const BitArray& ddsrv_cat =
      problem_db.get_ba("variables.discrete_design_set_real.categorical");

  const BitArray& puv_cat =
      problem_db.get_ba("variables.poisson_uncertain.categorical");
  const BitArray& biuv_cat =
      problem_db.get_ba("variables.binomial_uncertain.categorical");
  const BitArray& nbuv_cat =
      problem_db.get_ba("variables.negative_binomial_uncertain.categorical");
  const BitArray& geuv_cat =
      problem_db.get_ba("variables.geometric_uncertain.categorical");
  const BitArray& hguv_cat =
      problem_db.get_ba("variables.hypergeometric_uncertain.categorical");
  const BitArray& hupiv_cat =
      problem_db.get_ba("variables.histogram_uncertain.point_int.categorical");
  const BitArray& huprv_cat =
      problem_db.get_ba("variables.histogram_uncertain.point_real.categorical");

  const BitArray& diuv_cat =
      problem_db.get_ba("variables.discrete_interval_uncertain.categorical");
  const BitArray& dusiv_cat =
      problem_db.get_ba("variables.discrete_uncertain_set_int.categorical");
  const BitArray& dusrv_cat =
      problem_db.get_ba("variables.discrete_uncertain_set_real.categorical");

  const BitArray& dsrv_cat =
      problem_db.get_ba("variables.discrete_state_range.categorical");
  const BitArray& dssiv_cat =
      problem_db.get_ba("variables.discrete_state_set_int.categorical");
  const BitArray& dssrv_cat =
      problem_db.get_ba("variables.discrete_state_set_real.categorical");
...
void SharedVariablesDataRep::set_relax(const BitArray& user_cat_spec,
                                       size_t ucs_index, size_t ard_cntr,
                                       BitArray& ard_container) {
  if (user_cat_spec.empty() || !user_cat_spec[ucs_index])
    ard_container.set(ard_cntr);
}

void SharedVariablesDataRep::initialize_all_labels(
    const ProblemDescDB& problem_db) {
  size_all_labels();
...
void SharedVariablesDataRep::initialize_all_ids() {
  ...
  // DESIGN
  for (i = 0; i < num_cdv; ++i, ++id) allContinuousIds[acv_cntr++] = id;
  ...
  for (i = 0; i < num_ddsv; ++i, ++id) allDiscreteStringIds[adsv_cntr++] = id;
  ...
  // ALEATORY UNCERTAIN
  for (i = 0; i < num_cauv; ++i, ++id) allContinuousIds[acv_cntr++] = id;
  ...
  for (i = 0; i < num_dausv; ++i, ++id) allDiscreteStringIds[adsv_cntr++] = id;
  ...
  // EPISTEMIC UNCERTAIN
  for (i = 0; i < num_ceuv; ++i, ++id) allContinuousIds[acv_cntr++] = id;
  ...
  for (i = 0; i < num_deusv; ++i, ++id) allDiscreteStringIds[adsv_cntr++] = id;
  ...
  // STATE
  for (i = 0; i < num_csv; ++i, ++id) allContinuousIds[acv_cntr++] = id;
  ...
  for (i = 0; i < num_dssv; ++i, ++id) allDiscreteStringIds[adsv_cntr++] = id;
...
void SharedVariablesDataRep::view_start_counts(
    short view, size_t& cv_start, size_t& div_start, size_t& dsv_start,
    size_t& drv_start, size_t& num_cv, size_t& num_div, size_t& num_dsv,
    size_t& num_drv) const {
  size_t i, offset, start_relax_di = 0, start_relax_dr = 0, count_relax_di = 0,
                    count_relax_dr = 0,
                    num_cdv = variablesCompsTotals[TOTAL_CDV],
                    num_ddiv = variablesCompsTotals[TOTAL_DDIV],
                    num_ddsv = variablesCompsTotals[TOTAL_DDSV],
  ...
  switch (view) {
    case EMPTY_VIEW:  // should only happen for inactive views
      cv_start = div_start = dsv_start = drv_start = num_cv = num_div =
          num_dsv = num_drv = 0;
      break;
    case MIXED_ALL:  // should only happen for active views
      ...
      num_drv = num_ddrv + num_daurv + num_deurv + num_dsrv;
      break;
    case MIXED_DESIGN:
      // start at the beginning
      cv_start = div_start = dsv_start = drv_start = 0;
    ...
    case MIXED_UNCERTAIN:
      // skip over the design variables
      cv_start = num_cdv;
      num_cv = num_cauv + num_ceuv;
      div_start = num_ddiv;
      num_div = num_dauiv + num_deuiv;
      dsv_start = num_ddsv;
      num_dsv = num_dausv + num_deusv;
      drv_start = num_ddrv;
      num_drv = num_daurv + num_deurv;
      break;
    case MIXED_STATE:
      // skip over all the design and uncertain variables
      cv_start = num_cdv + num_cauv + num_ceuv;
      num_cv = num_csv;
      div_start = num_ddiv + num_dauiv + num_deuiv;
      num_div = num_dsiv;
      dsv_start = num_ddsv + num_dausv + num_deusv;
      num_dsv = num_dssv;
      drv_start = num_ddrv + num_daurv + num_deurv;
      num_drv = num_dsrv;
      break;
    case RELAXED_ALL:
      // from head to tail
      ...
      cv_start = div_start = dsv_start = drv_start = 0;
      num_cv = num_cdv + num_cauv + num_ceuv + num_csv + count_relax_di +
               count_relax_dr;
    ...
    case RELAXED_DESIGN:
      ...
      for (i = 0; i < num_ddrv; ++i)
        if (allRelaxedDiscreteReal[i]) ++count_relax_dr;
      cv_start = 0;
      num_cv = num_cdv + count_relax_di + count_relax_dr;
      div_start = 0;
      num_div = num_ddiv - count_relax_di;
      dsv_start = 0;
      num_dsv = num_ddsv;
      drv_start = 0;
      num_drv = num_ddrv - count_relax_dr;
      break;
    ...
    case RELAXED_STATE:
      ...
      for (i = 0; i < num_dsrv; ++i)
        if (allRelaxedDiscreteReal[offset + i]) ++count_relax_dr;
      cv_start =
          num_cdv + num_cauv + num_ceuv + start_relax_di + start_relax_dr;
...
void SharedVariablesDataRep::initialize_active_components() {
  switch (variablesView.first) {
    case MIXED_ALL:
    case RELAXED_ALL:
      activeVarsCompsTotals = variablesCompsTotals;
      break;
    case MIXED_DESIGN:
    case RELAXED_DESIGN:
...
void SharedVariablesDataRep::initialize_inactive_components() {
  ...
  switch (variablesView.second) {
    ...
    case RELAXED_STATE:
      ...
      break;
      // default: if inactive view unassigned, leave comp totals initialized to
      // 0
  }
...
void SharedVariablesDataRep::copy_rep_view(SharedVariablesDataRep* svd_rep) {
  ...
  cvStart = svd_rep->cvStart;
  numCV = svd_rep->numCV;
  divStart = svd_rep->divStart;
  numDIV = svd_rep->numDIV;
  dsvStart = svd_rep->dsvStart;
  numDSV = svd_rep->numDSV;
  drvStart = svd_rep->drvStart;
  numDRV = svd_rep->numDRV;
  icvStart = svd_rep->icvStart;
  numICV = svd_rep->numICV;
  idivStart = svd_rep->idivStart;
  numIDIV = svd_rep->numIDIV;
  idsvStart = svd_rep->idsvStart;
  numIDSV = svd_rep->numIDSV;
  idrvStart = svd_rep->idrvStart;
  numIDRV = svd_rep->numIDRV;
}

size_t SharedVariablesDataRep::cv_index_to_all_index(size_t cv_index, bool cdv,
                                                     bool cauv, bool ceuv,
                                                     bool csv) const {
  ...
  if (cdv) {
    bound += num_cv;
    if (cv_index < bound) return offset + cv_index;
  } else
    offset += num_cv;
...
size_t SharedVariablesDataRep::div_index_to_all_index(size_t div_index,
                                                      bool ddv, bool dauv,
                                                      bool deuv,
                                                      bool dsv) const {
  ...
  if (ddv) {
    bound += num_div;
    if (div_index < bound) return offset + div_index;
  } else
    offset += num_div;
...
size_t SharedVariablesDataRep::dsv_index_to_all_index(size_t dsv_index,
                                                      bool ddv, bool dauv,
                                                      bool deuv,
                                                      bool dsv) const {
  ...
  if (ddv) {
    bound += num_dsv;
    if (dsv_index < bound) return offset + dsv_index;
  } else
    offset += num_dsv;
  offset += num_drv;
...
size_t SharedVariablesDataRep::drv_index_to_all_index(size_t drv_index,
                                                      bool ddv, bool dauv,
                                                      bool deuv,
                                                      bool dsv) const {
  ...
  if (ddv) {
    bound += num_drv;
    if (drv_index < bound) return offset + drv_index;
  } else
    offset += num_drv;
...
BitArray SharedVariablesDataRep::active_to_all_mask(bool cdv, bool ddv,
                                                    bool cauv, bool dauv,
                                                    bool ceuv, bool deuv,
                                                    bool csv, bool dsv) const {
  size_t num_cv, num_div, num_dsv, num_drv;
  all_counts(num_cv, num_div, num_dsv, num_drv);
  BitArray all_mask(num_cv + num_div + num_dsv +
                    num_drv);  // init bits to false

  design_counts(num_cv, num_div, num_dsv, num_drv);  // w/ relaxed
  size_t i, all_index = 0, num_dv = num_div + num_dsv + num_drv;
  if (cdv)
    for (i = 0; i < num_cv; ++i, ++all_index) all_mask.set(all_index);
  else
    all_index += num_cv;
  if (ddv)
    for (i = 0; i < num_dv; ++i, ++all_index) all_mask.set(all_index);
  else
    all_index += num_dv;
...
BitArray SharedVariablesDataRep::cv_to_all_mask(bool cdv, bool cauv, bool ceuv,
                                                bool csv) const {
  size_t num_cv, num_div, num_dsv, num_drv;
  all_counts(num_cv, num_div, num_dsv, num_drv);
  BitArray all_mask(num_cv + num_div + num_dsv +
...
BitArray SharedVariablesDataRep::div_to_all_mask(bool ddv, bool dauv, bool deuv,
                                                 bool dsv) const {
  size_t num_cv, num_div, num_dsv, num_drv;
  all_counts(num_cv, num_div, num_dsv, num_drv);
  BitArray all_mask(num_cv + num_div + num_dsv +
  ...
  if (ddv)
    for (i = 0; i < num_div; ++i, ++all_index) all_mask.set(all_index);
  else
    all_index += num_div;
...
BitArray SharedVariablesDataRep::dsv_to_all_mask(bool ddv, bool dauv, bool deuv,
                                                 bool dsv) const {
  size_t num_cv, num_div, num_dsv, num_drv;
  all_counts(num_cv, num_div, num_dsv, num_drv);
  BitArray all_mask(num_cv + num_div + num_dsv +
  ...
  if (ddv)
    for (i = 0; i < num_dsv; ++i, ++all_index) all_mask.set(all_index);
  else
    all_index += num_dsv;
  all_index += num_drv;
...
BitArray SharedVariablesDataRep::drv_to_all_mask(bool ddv, bool dauv, bool deuv,
                                                 bool dsv) const {
  size_t num_cv, num_div, num_dsv, num_drv;
  all_counts(num_cv, num_div, num_dsv, num_drv);
  BitArray all_mask(num_cv + num_div + num_dsv +
  ...
  if (ddv)
    for (i = 0; i < num_drv; ++i, ++all_index) all_mask.set(all_index);
  else
    all_index += num_drv;
...
size_t SharedVariablesData::cv_index_to_active_index(size_t cv_index) const {
  ...
  if (ddv) offset += num_div + num_dsv + num_drv;
  ...
  if (dauv) offset += num_div + num_dsv + num_drv;
  ...
  if (deuv) offset += num_div + num_dsv + num_drv;
...
size_t SharedVariablesData::ccv_index_to_acv_index(size_t ccv_index) const {
  ...
  if (cdv) {
    bound += num_cv;
    if (ccv_index < bound) return offset + ccv_index;
  } else
    offset += num_cv;
...
size_t SharedVariablesData::cdiv_index_to_adiv_index(size_t cdiv_index) const {
  ...
  if (ddv) {
    bound += num_div;
    if (cdiv_index < bound) return offset + cdiv_index;
  } else
    offset += num_div;
...
size_t SharedVariablesData::cdsv_index_to_adsv_index(size_t cdsv_index) const {
  ...
  if (ddv) {
    bound += num_dsv;
    if (cdsv_index < bound) return offset + cdsv_index;
  } else
    offset += num_dsv;
...
size_t SharedVariablesData::cdrv_index_to_adrv_index(size_t cdrv_index) const {
  ...
  if (ddv) {
    bound += num_drv;
    if (cdrv_index < bound) return offset + cdrv_index;
  } else
    offset += num_drv;
...
void SharedVariablesData::assemble_all_labels(StringArray& all_labels) const {
  ...
  size_t num_av = acv_labels.size() + adiv_labels.size() + adsv_labels.size() +
                  adrv_labels.size();
  all_labels.resize(num_av);

  size_t num_cv, num_div, num_dsv, num_drv,
      cv_start = 0, div_start = 0, dsv_start = 0, drv_start = 0, all_start = 0;
  ...
  aleatory_uncertain_counts(num_cv, num_div, num_dsv,
                            num_drv);  // w/ relaxation
  copy_data_partial(acv_labels, cv_start, all_labels, all_start, num_cv);
  all_start += num_cv;
  cv_start += num_cv;
  copy_data_partial(adiv_labels, div_start, all_labels, all_start, num_div);
  ...
  epistemic_uncertain_counts(num_cv, num_div, num_dsv,
                             num_drv);  // w/ relaxation
  copy_data_partial(acv_labels, cv_start, all_labels, all_start, num_cv);
  all_start += num_cv;
  cv_start += num_cv;
...
/** Deep copies are used when recasting changes the nature of a
    Variables set. */
SharedVariablesData SharedVariablesData::copy(
...
void SharedVariablesDataRep::save(Archive& ar,
