...
class SharedResponseDataRep {
 ...
 private:
  ...
  /// the primary function type as a friendly string, e.g.,
  /// "objective_functions"
...
inline SharedResponseDataRep::SharedResponseDataRep()
    :  // copy and reshape immediately apply copy_rep, so don't initialize
       // anything:
...
inline SharedResponseDataRep::~SharedResponseDataRep() { /* empty dtor */ }
...
inline SharedResponseData::SharedResponseData() { /* empty ctor */ }

inline SharedResponseData::SharedResponseData(const ProblemDescDB& problem_db)
    : srdRep(new SharedResponseDataRep(problem_db)) { /* empty ctor */ }

inline SharedResponseData::SharedResponseData(const ActiveSet& set)
    : srdRep(new SharedResponseDataRep(set)) { /* empty ctor */ }
...
inline SharedResponseData& SharedResponseData::operator=(
...
inline SharedResponseData::~SharedResponseData() { /* empty dtor in case we add
                                                      virtual functions */
}
...
inline bool SharedResponseData::is_null() const { return (srdRep == NULL); }
...
void expand_for_fields_sdv(const SharedResponseData& srd, const T& src_array,
  ...
  if (src_size == 0) return;  // leave expanded_array empty

  size_t resp_groups =
      srd.num_scalar_primary() + srd.num_field_response_groups();
