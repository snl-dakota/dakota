...
PebbldMinimizer::PebbldMinimizer(ProblemDescDB& problem_db,
                                 ParallelLibrary& parallel_lib,
                                 std::shared_ptr<Model> model)
    : Minimizer(problem_db, parallel_lib, model,
...
PebbldMinimizer::PebbldMinimizer(std::shared_ptr<Model> model)
    : Minimizer(BRANCH_AND_BOUND, model,
                std::shared_ptr<TraitsBase>(
                    new PebbldTraits())) {  // branchAndBound(model)
      };

PebbldMinimizer::PebbldMinimizer(std::shared_ptr<Model> model, int random_seed,
                                 size_t max_iter, size_t max_eval)
    : Minimizer(BRANCH_AND_BOUND, model,
                std::shared_ptr<TraitsBase>(
                    new PebbldTraits())) {  // branchAndBound(model,random_seed,
                                            // max_iter, max_eval)
      };
...
void PebbldMinimizer::core_run() {
  ...
  pebbl::arraySolution<double>* newSolution =
      dynamic_cast<pebbl::arraySolution<double>*>(
