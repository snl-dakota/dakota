...
class NonDMUQBayesCalibration : public NonDBayesCalibration {
 ...
 protected:
  ...
  /// define solver options, likelihood callback, posterior RV, and inverse
  /// problem
  ...
  std::shared_ptr<MUQPrior>
      MUQPriorPtr;  // Used by all MUQ MCMC methods except DILI
  std::shared_ptr<muq::Modeling::Gaussian>
      muqGaussianPrior;  // Used by DILI only
...
class MUQLikelihood : public muq::Modeling::DensityBase {
 public:
  ...
  double LogDensityImpl(
      muq::Modeling::ref_vector<Eigen::VectorXd> const& inputs);

  Eigen::VectorXd GradLogDensityImpl(
      unsigned int wrt,
      muq::Modeling::ref_vector<Eigen::VectorXd> const& inputs);
...
class MLMCMCLikelihood : public muq::Modeling::DensityBase {
 ...
 public:
  inline MLMCMCLikelihood(Eigen::VectorXi const& input_sizes, int myLevel,
                          NonDMUQBayesCalibration* nonDMUQInstance)
      : muq::Modeling::DensityBase(input_sizes),
        m_myLevel(myLevel),
        m_nonDMUQInstance(nonDMUQInstance) {
    if (m_nonDMUQInstance->outputLevel >= DEBUG_OUTPUT) {
      Cout << "MUQ: MLMCMC: MLMCMCLikelihood:  Constructor : level = "
           << m_myLevel << "\n";
    }
  };

  double LogDensityImpl(
      muq::Modeling::ref_vector<Eigen::VectorXd> const& state) {
    Eigen::VectorXd const& c_vars = state.at(0);
    size_t num_cv = c_vars.size();
    RealVector& all_params =
        m_nonDMUQInstance->residualModel->current_variables()
    ...
    */
    const Pecos::ActiveKey& curr_key =
        m_nonDMUQInstance->residualModel->active_model_key();
    ...
    const RealVector& residuals =
        m_nonDMUQInstance->residualModel->current_response().function_values();
    ...
    if (m_nonDMUQInstance->outputLevel >= DEBUG_OUTPUT) {
      ...
      size_t num_total_params = m_nonDMUQInstance->numContinuousVars +
                                m_nonDMUQInstance->numHyperparams;
...
class MUQMLMCMCInterpolation : public muq::SamplingAlgorithms::MIInterpolation {
 public:
  std::shared_ptr<muq::SamplingAlgorithms::SamplingState> Interpolate(
      std::shared_ptr<muq::SamplingAlgorithms::SamplingState> const&
          coarseProposal,
      std::shared_ptr<
          muq::SamplingAlgorithms::SamplingState> const& /*fineProposal*/
  ) {
    // for now, we do not support calibrating different params count at
    // different levels, so here the state proposed at the coarse level is just
    // copied
    return std::make_shared<muq::SamplingAlgorithms::SamplingState>(
        coarseProposal->state);
  }
};

class MLMCMCComponentFactory
    : public muq::SamplingAlgorithms::MIComponentFactory {
 ...
 public:
  MLMCMCComponentFactory(boost::property_tree::ptree pt, int numLevels,
                         size_t num_cv, const RealVector& init_point_vector,
                         NonDMUQBayesCalibration* nonDMUQInstancePtr)
      : pt(pt),
  ...
  virtual std::shared_ptr<muq::SamplingAlgorithms::MCMCProposal> Proposal(
      ...
      std::shared_ptr<muq::SamplingAlgorithms::AbstractSamplingProblem> const&
          samplingProblem) override {
    if (m_nonDMUQInstancePtr->outputLevel >= DEBUG_OUTPUT) {
      Cout << "MUQ: MLMCMC: MLMCMCComponentFactory: creating the proposal \n";
    }

    // create the proposal for any given level
    // for now, we have the same problem for every level because the same
    // uncertain param dim we use the same proposal using the covariance matrix
    ...
    auto propDist = std::make_shared<muq::Modeling::Gaussian>(
        prop_mu, m_nonDMUQInstancePtr->proposalCovMatrix);
    return std::make_shared<muq::SamplingAlgorithms::MHProposal>(
        pt, samplingProblem, propDist);
#else
    // crank-nic does not work here, something is wrong
    // Eigen::VectorXd prop_mu =
    // Eigen::VectorXd::Zero(m_nonDMUQInstancePtr->numContinuousVars); auto
    // prior = std::make_shared<muq::Modeling::Gaussian>(prop_mu,
    // m_nonDMUQInstancePtr->proposalCovMatrix); return
    // std::make_shared<muq::SamplingAlgorithms::CrankNicolsonProposal>(pt,
    // samplingProblem, prior);
    ...
    return std::make_shared<muq::SamplingAlgorithms::AMProposal>(
        local_pt.get_child("MyProposal"), samplingProblem,
        m_nonDMUQInstancePtr->proposalCovMatrix);
#endif
  }
  ...
  virtual std::shared_ptr<muq::Utilities::MultiIndex> FinestIndex() override {
    ...
    // Here, we need to pass 1 = dimension of the hierarchy, for ML this is 1
    // always.
    auto index = std::make_shared<muq::Utilities::MultiIndex>(1);
    // define the levels: 0,1,2,3..., such that 0 is coarsest, m_numLevels-1 is
    // the finest
    index->SetValue(0, m_numLevels - 1);
    return index;
  }

  virtual std::shared_ptr<muq::SamplingAlgorithms::MCMCProposal> CoarseProposal(
      std::shared_ptr<muq::Utilities::MultiIndex> const& fineIndex,
      std::shared_ptr<muq::Utilities::MultiIndex> const& coarseIndex,
      std::shared_ptr<muq::SamplingAlgorithms::AbstractSamplingProblem> const&
          coarseProblem,
      std::shared_ptr<muq::SamplingAlgorithms::SingleChainMCMC> const&
          coarseChain) override {
    // this proposal drives the coarse chain by a bit and then take the final
    // result Nov. 2024: FRIZZI interacted with Linus Seelinger (author of MLMCM
    // in MUQ) who suggested to leave it as it is for the basecase
    pt::ptree pt_proposal = pt;
    pt_proposal.put("BlockIndex", 0 /*leave = 0 */);
    return std::make_shared<muq::SamplingAlgorithms::SubsamplingMIProposal>(
        pt_proposal, coarseProblem, coarseIndex, coarseChain);
  }

  virtual std::shared_ptr<muq::SamplingAlgorithms::AbstractSamplingProblem>
  SamplingProblem(
      std::shared_ptr<muq::Utilities::MultiIndex> const& index) override {
    ...
    auto vxi =
        Eigen::VectorXi::Constant(1, m_nonDMUQInstancePtr->numContinuousVars);
    auto this_level_lkl =
        std::make_shared<MLMCMCLikelihood>(vxi, level, m_nonDMUQInstancePtr);
    ...
    my_work_graph->AddEdge("Parameters", 0, "Prior",
                           0);  // 0 = index of input,output
    my_work_graph->AddEdge("Parameters", 0, "Likelihood",
                           0);  // 0 = index of input,output
  ...
  Interpolation(
      std::shared_ptr<muq::Utilities::MultiIndex> const& index) override {
    // This is something that is supposed to know how to interpolate from a
    // coarse to a finer level. This is only needed if the # of calibration
    // parameters changes across levels. For example, suppose one is doing
    // calibration on the coefficients of a KL expansion and there is a level 0
    // with 10 KL coeffs, and level 1 has 56 KL coeffs, etc. MUQ MLMCMC needs to
    // have something that specifies what to do across levels, specifically what
    // to do at a fine level when given a propose state at a corser level. For
    // now, calibrating different params count at different levels is not
    // supported so we do not need to specify anything special: the
    // "interpolation" is basically just going to copy the values when needed,
    // see the class for more info.
    return std::make_shared<MUQMLMCMCInterpolation>();
  }

  virtual Eigen::VectorXd StartingPoint(
      std::shared_ptr<muq::Utilities::MultiIndex> const& /*index*/) override {
    // this method is supposed to return the starting point at a given level
    // here we just use the same starting point for all level and this starting
    // point is provided to the constructor and cached.
