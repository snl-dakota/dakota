...
static const char rcsId[] =
    "@(#) $Id: FSUDesignCompExp.cpp 7035 2010-10-22 21:45:39Z mseldre $";

namespace Dakota {

/** This constructor is called for a standard iterator built with data from
    probDescDB. */
FSUDesignCompExp::FSUDesignCompExp(ProblemDescDB& problem_db,
  ...
  switch (methodName) {
    ...
    default:
      Cerr << "Error: FSU DACE method \"" << methodName
...
/** This alternate constructor is used for instantiations on-the-fly,
    ...
    queries are used. */
FSUDesignCompExp::FSUDesignCompExp(std::shared_ptr<Model> model, int samples,
                                   int seed, unsigned short sampling_method)
    : PStudyDACE(sampling_method, model),
      samplesSpec(samples),
      numSamples(samples),
      allDataFlag(true),
      numDACERuns(0),
  ...
  switch (methodName) {
    ...
    case FSU_HAMMERSLEY:
      ...
      if (methodName == FSU_HALTON)
        ...
        primeBase[0] = -numSamples;
        for (size_t i = 1; i < numContinuousVars; i++) primeBase[i] = prime(i);
      }
      break;
    default:
      Cerr << "Error: FSU DACE method \"" << methodName
...
void FSUDesignCompExp::get_parameter_sets(std::shared_ptr<Model> model,
  ...
  switch (methodName) {
    case FSU_HALTON: {
      ...
      fsu_halton(numContinuousVars, num_samples, qmc_step,
    ...
    case FSU_CVT: {
      ...
      if (numCVTTrials < num_samples) numCVTTrials = num_samples * 10;
      ...
      // repeatable behavior but no specification gives you random behavior
      // (seed generated from a system source).  For the cases where core_run()
      // may be called multiple times for the same DACE object (e.g., SBO), a
      ...
      if (numDACERuns == 1) {  // set initial seed
        if (!seedSpec) {       // no user specification: random behavior
          // Generate initial seed from a system source.  NOTE: the system
          // source is not reused on subsequent invocations since (1) clock
          // granularity can be too coarse (can repeat on subsequent runs for
          // inexpensive test fns) and (2) seed progression can be highly
          // structured, which could induce correlation between sample sets.
          // Instead, the system-generated case uses the same seed progression
          // procedure below as the user-specified case.  This has the
          // additional benefit that a random run can be recreated by specifying
          // the system-generated seed in the input file.
          randomSeed = generate_system_seed();
          rng.seed(randomSeed);  // seed for use with varyPattern
        }
      } else if (varyPattern) {  // define sequence of seed values for
                                 // numLHSRuns > 1
...
void FSUDesignCompExp::enforce_input_rules() {
  ...
  if (methodName == FSU_CVT) {
    ...
    for (size_t i = 0; i < numContinuousVars - 1; i++)
      if (temp_prime[i] == temp_prime[i + 1]) primeEqual = true;
    if (primeEqual) {
