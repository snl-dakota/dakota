...
std::shared_ptr<Interface> Interface::get_interface(
  ...
  auto m_it = std::find_if(study_cache.begin(), study_cache.end(),
                           [&id_interface](std::shared_ptr<Interface> m) {
                             return m->interface_id() == id_interface;
                           });
  if (m_it == study_cache.end()) {
    study_cache.push_back(
        InterfaceUtils::get_interface(problem_db, parallel_lib));
    m_it = --study_cache.end();
  }
  return *m_it;
}

std::list<std::shared_ptr<Interface>>& Interface::interface_cache(
...
void Interface::clean_up_all_interfaces() {
  for (auto& icache_pair : Interface::interfaceCache)
    for (auto& interface : icache_pair.second) interface->file_cleanup();
}

std::map<const ProblemDescDB*, std::list<std::shared_ptr<Interface>>>
...
Interface::Interface(const ProblemDescDB& problem_db)
    : interfaceType(problem_db.get_ushort("interface.type")),
      ...
      evalIdRefPt(0),
      newEvalIdRefPt(0),
      multiProcEvalFlag(false),
      ieDedSchedFlag(false),
      // See base constructor in DakotaIterator.cpp for full discussion of
      // output verbosity.  Interfaces support the full granularity in
      // verbosity.
      appendIfaceId(true),
      asl(NULL) {
#ifdef DEBUG
...
#endif  // DEBUG
  if (interfaceId.empty()) interfaceId = user_auto_id();
  ...
  if (!ampl_file_name.empty()) {
    ...
    String stub = (strends(ampl_file_name, ".nl"))
                      ? String(ampl_file_name, 0, ampl_file_name.size() - 3)
                      : ampl_file_name;
    ...
    for (size_t i = 0; i < n_var; i++) {
      ...
      if (ampl_col.good())
        ...
        Cerr << "\nError: failure reading AMPL col file " << col << std::endl;
        abort_handler(IO_ERROR);
      }
    }
    ...
    for (size_t i = 0; i < n_obj + n_con; i++) {
      ...
      if (ampl_row.good()) {
        ...
        Cerr << "\nError: failure reading AMPL row file " << row << std::endl;
        abort_handler(IO_ERROR);
      }
    }
...
/** Copy constructor */
Interface::Interface(const Interface& interface_in) { /* empty ctor */ }

Interface Interface::operator=(const Interface& interface_in) {
  return *this;  // calls copy constructor since returned by value
}

Interface::~Interface() { /* empty dtor */ }
...
void Interface::init_evaluation_counters(size_t num_fns) {
  ...
  if (fnValCounter.size() != num_fns) {
    fnValCounter.assign(num_fns, 0);
    fnGradCounter.assign(num_fns, 0);
    fnHessCounter.assign(num_fns, 0);
    newFnValCounter.assign(num_fns, 0);
    newFnGradCounter.assign(num_fns, 0);
    newFnHessCounter.assign(num_fns, 0);
    fnValRefPt.assign(num_fns, 0);
    fnGradRefPt.assign(num_fns, 0);
    fnHessRefPt.assign(num_fns, 0);
    newFnValRefPt.assign(num_fns, 0);
    newFnGradRefPt.assign(num_fns, 0);
    newFnHessRefPt.assign(num_fns, 0);
  }
}
...
void Interface::print_evaluation_summary(std::ostream& s, bool minimal_header,
  ...
  int fn_evals = (relative_count) ? evalIdCntr - evalIdRefPt : evalIdCntr;
  int new_fn_evals =
      (relative_count) ? newEvalIdCntr - newEvalIdRefPt : newEvalIdCntr;
  ...
  if (fineGrainEvalCounters) {
    ...
    for (i = 0; i < num_fns; i++) {
      int t_v =
          (relative_count) ? fnValCounter[i] - fnValRefPt[i] : fnValCounter[i];
      ...
      int n_h = (relative_count) ? newFnHessCounter[i] - newFnHessRefPt[i]
                                 : newFnHessCounter[i];
      s << std::setw(15) << fnLabels[i] << ": " << t_v << " val (" << n_v
        << " n, " << t_v - n_v << " d), " << t_g << " grad (" << n_g << " n, "
        << t_g - n_g << " d), " << t_h << " Hess (" << n_h << " n, "
        << t_h - n_h << " d)\n";
    }
  }
}
...
void Interface::init_algebraic_mappings(const Variables& vars,
...
void Interface::asv_mapping(const ActiveSet& total_set,
                            ActiveSet& algebraic_set, ActiveSet& core_set) {
...
void Interface::asv_mapping(const ActiveSet& algebraic_set,
                            ActiveSet& total_set) {
...
void Interface::algebraic_mappings(const Variables& vars,
                                   const ActiveSet& algebraic_set,
  ...
  for (i = 0; i < num_alg_fns; ++i) {
    if (algebraic_asv[i] & 2) grad_flag = true;
    if (algebraic_asv[i] & 4) hess_flag = true;
  }
  ...
  for (i = 0; i < num_alg_fns; i++) {
    ...
    if (algebraic_asv[i] & 4) {
      ...
      if (algebraicFnTypes[i] > 0)
        fullhes(fn_hess.values(), num_alg_vars, algebraicFnTypes[i] - 1, NULL,
                NULL);
      else {
  ...
  if (outputLevel > NORMAL_OUTPUT) Cout << "Algebraic mapping applied.\n";
#endif  // HAVE_AMPL
}
...
void Interface::response_mapping(const Response& algebraic_response,
  ...
  for (i = 0; i < num_total_fns; ++i) {
    if (total_asv[i] & 2) grad_flag = true;
    if (total_asv[i] & 4) hess_flag = true;
  }
  ...
  size_t num_alg_fns = algebraic_asv.size(),
         num_alg_vars =
             algebraic_response.active_set_derivative_vector().size();
  ...
  for (i = 0; i < num_alg_fns; ++i) {
    ...
    if (algebraic_asv[i] & 1) total_fn_vals[fn_index] += algebraic_fn_vals[i];
        ...
        if (dvv_index_j != _NPOS) {
          for (k = 0; k <= j; ++k) {
            ...
            if (dvv_index_k != _NPOS)
              total_fn_hess(dvv_index_j, dvv_index_k) +=
                  algebraic_fn_hess(j, k);
          }
        }
      }
    }
...
String Interface::final_eval_id_tag(int iface_eval_id) {
  if (appendIfaceId) return evalTagPrefix + "." + std::to_string(iface_eval_id);
...
int Interface::algebraic_function_type(String functionTag) {
  ...
  int i;
  for (i = 0; i < n_obj; i++)
    if (strcontains(functionTag, obj_name(i))) return i + 1;
  for (i = 0; i < n_con; i++)
    if (strcontains(functionTag, con_name(i))) return -(i + 1);
...
void Interface::approximation_function_indices(
    const SizetSet& approx_fn_indices) {
  // default implementation is no-op
}

void Interface::update_approximation(const Variables& vars,
                                     const IntResponsePair& response_pr) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::update_approximation(const RealMatrix& samples,
                                     const IntResponseMap& resp_map) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::update_approximation(const VariablesArray& vars_array,
                                     const IntResponseMap& resp_map) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::append_approximation(const Variables& vars,
                                     const IntResponsePair& response_pr) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::append_approximation(const RealMatrix& samples,
                                     const IntResponseMap& resp_map) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::append_approximation(const VariablesArray& vars_array,
                                     const IntResponseMap& resp_map) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::append_approximation(const IntVariablesMap& vars_map,
                                     const IntResponseMap& resp_map) {
  InterfaceUtils::no_derived_method_error();
}

void Interface::build_approximation(const RealVector& c_l_bnds,
...
Real2DArray Interface::cv_diagnostics(const StringArray& metric_types,
                                      unsigned num_folds) {
...
std::vector<Approximation>& Interface::approximations() {
  InterfaceUtils::no_derived_method_error();
  // return
  // InterfaceUtils::no_derived_method_error<std::vector<Approximation>>();
  throw INTERFACE_ERROR;
}
