...
const char* FIELD_NAMES[] = {
    "numFns",    "numVars",      "numACV",      "numADIV",  // 0
...
MatlabInterface::MatlabInterface(const ProblemDescDB& problem_db,
  ...
  engine_cmd = env_engine_cmd ? env_engine_cmd : "matlab -nodesktop -nosplash";
#endif
  ...
  if (matlabEngine) {
    ...
    if (env_matlab_startup) engEvalString(matlabEngine, env_matlab_startup);
  } else {
  ...
  dakota_matlab =
      mxCreateStructArray(ndim, dims, NUMBER_OF_FIELDS, FIELD_NAMES);
...
int MatlabInterface::matlab_engine_run(const String& ac_name) {
  ...
  */
  ...
  if ((iF = matlab_field_prep(dakota_matlab, FIELD_NAMES[7])) >= 0) {
    ...
    for (i = 0; i < numADIV; ++i) *(ptr + i) = (double)xDI[i];
  ...
  if ((iF = matlab_field_prep(dakota_matlab, FIELD_NAMES[8])) >= 0) {
    ...
    for (i = 0; i < numADRV; ++i) *(ptr + i) = xDR[i];
  ...
  if ((iF = matlab_field_prep(dakota_matlab, FIELD_NAMES[12])) >= 0) {
    ...
    for (i = 0; i < numFns; ++i) *(mxGetPr(mx_tmp[12]) + i) = directFnASV[i];
    ...
    */
    ...
    if (fn_flag)
      if ((mx_tmp[17] = mxGetField(dakota_matlab, 0, "fnVals")) == NULL) {
        ...
        for (i = 0; i < numFns; ++i) fnVals[i] = *(ptr + i);
      }
