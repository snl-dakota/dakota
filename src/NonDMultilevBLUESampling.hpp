...
class NonDMultilevBLUESampling : public NonDNonHierarchSampling {
 ...
 protected:
  ...
  void numerical_solution_bounds_constraints(
      ...
      RealMatrix& lin_eq_coeffs) override;
  ...
  void enforce_augmented_linear_ineq_constraints(
      RealVector& soln_vars) override;

  //
  //- Heading: member functions
  //

  void compute_C_inverse(RealSymMatrix& cov_GG_gq, RealSymMatrix& cov_GG_inv_gq,
...
inline size_t NonDMultilevBLUESampling::num_active_groups() const {
  if (retainedModelGroups.empty())
    ...
    return retainedModelGroups.count();
}

inline size_t NonDMultilevBLUESampling::active_to_all_group(
...
inline size_t NonDMultilevBLUESampling::all_to_active_group(
    size_t all_index) const {
  if (retainedModelGroups.empty())
    return all_index;
  else if (!retainedModelGroups[all_index])
    return _NPOS;
  else {
    size_t g, cntr = 0;
    for (g = 0; g < all_index; ++g)
      if (retainedModelGroups[g]) ++cntr;
    return cntr;
  }
}

inline Real NonDMultilevBLUESampling::inactive_group_costs(
    const BitArray& retained, const SizetArray& N_G_alloc) const {
  ...
  for (size_t g = 0; g < numGroups; ++g)
    if (!retained[g]) deduct += N_G_alloc[g] * modelGroupCost[g] / cost_H;
  return deduct;
}

inline Real NonDMultilevBLUESampling::inactive_model_costs(
    const BitArray& retained, size_t N_alloc) const {
  ...
  for (size_t m = 0; m <= numApprox; ++m)
    if (!retained[m]) deduct += sequenceCost[m] * N_over_cost;
  return deduct;
}
...
inline Real NonDMultilevBLUESampling::inactive_budget_deduction() const {
  ...
  switch (pilotGroupSampling) {
    ...
    default:  // SHARED_PILOT: budget deductions are model-based
      // > deduct shared pilot cost for an approx model iff all groups
      // containing
...
inline void NonDMultilevBLUESampling::retained_groups_to_models(
    const BitArray& active_g, BitArray& active_m) const {
  size_t g, m, model_m, num_models, retained = 0, num_ap1 = numApprox + 1;
  if (active_m.size() != num_ap1) active_m.resize(num_ap1);
  active_m.reset();
...
inline const MFSolutionData& NonDMultilevBLUESampling::final_solution_data()
    const {
  return blueSolnData;
}
...
inline void NonDMultilevBLUESampling::update_model_group_costs() {
  ...
  for (g = 0; g < numGroups; ++g) {
    ...
    for (m = 0; m < num_models; ++m) group_cost_g += sequenceCost[models[m]];
  }
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "modelGroups:\n"
         << modelGroups << "sequenceCost:\n"
         << sequenceCost << "modelGroupCost:\n"
         << modelGroupCost;
}

inline void NonDMultilevBLUESampling::update_search_algorithm() {
  size_t num_g = num_active_groups();
  bool warn = false;
...
inline void NonDMultilevBLUESampling::increment_allocations(
    const MFSolutionData& soln, SizetArray& N_G_alloc,
    const SizetArray& delta_N_G) {
  ...
  if (backfillFailures) {  // don't use delta_N_G as it may include backfill
    ...
    one_sided_delta(N_G_alloc, soln.solution_variables(), bf_delta_N_G,
                    relaxFactor);  // match under-relaxation
    increment_samples(N_G_alloc, bf_delta_N_G);
  } else  // delta_N_G is the allocation increment, including any
          // under-relaxation
...
inline void NonDMultilevBLUESampling::increment_allocations(
  ...
  if (backfillFailures) {  // don't use delta_N_G as it may include backfill
    ...
    if (diff_g > 0.) curr_g += (size_t)std::floor(relaxFactor * diff_g + .5);
...
inline void NonDMultilevBLUESampling::find_hf_sample_reference(
    const Sizet2DArray& N_G, size_t& ref_group, size_t& ref_model_index) const {
  ...
  for (g = 0; g < num_groups; ++g) {
    ...
    if (group_g.back() == numApprox) {  // HF model is present
      ...
      if (group_samples > ref_samples) {
        ref_group = g;
        ref_model_index = group_g.size() - 1;
        ref_samples = group_samples;
      }
    }
  }
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "HF sample reference located in group " << ref_group << " at index "
         << ref_model_index << std::endl;
}

/** Overload for 1D array: NGroupAlloc */
inline void NonDMultilevBLUESampling::find_hf_sample_reference(
    const SizetArray& N_G, size_t& ref_group, size_t& ref_model_index) const {
  ref_group = ref_model_index = SZ_MAX;
  Real ref_samples = 0., group_samples;
  size_t g, num_groups = modelGroups.size();
  for (g = 0; g < num_groups; ++g) {
...
inline size_t NonDMultilevBLUESampling::best_conditioned_hf_group() {
  ...
  std::multimap<Real, size_t>::reverse_iterator rit;
  size_t group;
  for (rit = groupCovCondMap.rbegin(); rit != groupCovCondMap.rend(); ++rit) {
    group = rit->second;
    if (modelGroups[group].back() == numApprox) return group;
  }
...
inline void NonDMultilevBLUESampling::enforce_diagonal_delta(
  ...
  size_t i, j, num_rsm = Psi.size();
  int n;
  Real delta = RATIO_NUDGE;
  for (i = 0; i < num_rsm; ++i) {
    RealSymMatrix& Psi_i = Psi[i];
    n = Psi_i.numRows();
    for (j = 0; j < n; ++j) Psi_i(n, n) += delta;
  }
}

inline void NonDMultilevBLUESampling::covariance_to_correlation_sq(
    const RealSymMatrixArray& cov_GG_g, RealMatrix& rho2_LH) {
  if (rho2_LH.empty()) rho2_LH.shapeUninitialized(numFunctions, numApprox);

  size_t qoi, approx;
  Real var_H_q, var_L_q, cov_LH_q;
...
inline void NonDMultilevBLUESampling::assign_sub_matrix(
    const RealSymMatrix& sub_mat, const UShortArray& subset,
    RealSymMatrix& mat) {
  size_t r, c, num_sub = subset.size(), num_models = numApprox + 1;
  if (mat.numRows() != num_models)
    mat.shape(num_models);  // init to 0
  else
    mat = 0.;
  for (r = 0; r < num_sub; ++r)
    for (c = 0; c <= r; ++c) mat(subset[r], subset[c]) = sub_mat(r, c);

  // if (outputLevel >= DEBUG_OUTPUT)
  //   Cout << "assign_sub_matrix() =\n" << mat << std::endl;
}

inline void NonDMultilevBLUESampling::add_sub_matrix(
    Real coeff, const RealSymMatrix& sub_mat, const UShortArray& subset,
    RealSymMatrix& mat) {
  size_t r, c, num_sub = subset.size();
  for (r = 0; r < num_sub; ++r)
    for (c = 0; c <= r; ++c) mat(subset[r], subset[c]) += coeff * sub_mat(r, c);

  // if (outputLevel >= DEBUG_OUTPUT)
  //   Cout << "add_sub_matrix() aggregate =\n" << mat << std::endl;
}

inline void NonDMultilevBLUESampling::add_sub_matvec(
    const RealSymMatrix& sub_mat, const RealMatrix& sub_mat2,
...
inline void NonDMultilevBLUESampling::initialize_rsm2a(
    RealSymMatrix2DArray& rsm2a) {
  ...
  if (rsm2a.size() != num_groups) {
    ...
    for (size_t g = 0; g < num_groups; ++g) rsm2a[g].resize(numFunctions);
  }
}

inline void NonDMultilevBLUESampling::initialize_rsma(RealSymMatrixArray& rsma,
                                                      bool init) {
  if (rsma.size() != numFunctions) {
    ...
    for (size_t qoi = 0; qoi < numFunctions; ++qoi) rsma[qoi].putScalar(0.);
}

inline void NonDMultilevBLUESampling::initialize_rva(RealVectorArray& rva,
                                                     bool init) {
  if (rva.size() != numFunctions) {
    ...
    for (size_t qoi = 0; qoi < numFunctions; ++qoi) rva[qoi].putScalar(0.);
}
...
inline void NonDMultilevBLUESampling::linear_group_cost_gradient(
    const RealVector& cdv, RealVector& grad_c) {
  if (retainedModelGroups.empty()) {
  ...
  size_t i, cntr = 0;
  for (i = 0; i < numGroups; ++i)
    if (retainedModelGroups[i]) grad_c[cntr++] = modelGroupCost[i] / cost_H;
  if (outputLevel >= DEBUG_OUTPUT)
    Cout << "linear group cost gradient:\n" << grad_c << std::endl;
}

inline void NonDMultilevBLUESampling::compute_C_inverse(
    RealSymMatrix2DArray& cov_GG, RealSymMatrix2DArray& cov_GG_inv) {
...
inline void NonDMultilevBLUESampling::compute_Psi(
    const RealSymMatrix2DArray& cov_GG_inv, const RealVector& cdv,
    RealSymMatrixArray& Psi) {
      ...
      if (v_g > 0.) {
        ...
        for (qoi = 0; qoi < numFunctions; ++qoi) {
          ...
          if (!cov_GG_inv_gq.empty())
            add_sub_matrix(
                v_g, cov_GG_inv_gq, models_g,
                Psi[qoi]);  // *** can become indefinite here when N_g --> 0,
                            // which depends on online/offline pilot integration
                            // strategy
        }
      }
    }
...
inline void NonDMultilevBLUESampling::compute_Psi(
    const RealSymMatrix2DArray& cov_GG_inv, const Sizet2DArray& N_G,
    RealSymMatrixArray& Psi) {
  // Psi accumulated across groups and sized according to full model ensemble
  initialize_rsma(Psi);
    ...
    if (no_retain_throttle || retainedModelGroups[g]) {
      ...
      for (qoi = 0; qoi < numFunctions; ++qoi) {
        ...
        if (N_gq && !cov_GG_inv_gq.empty())
          add_sub_matrix((Real)N_gq, cov_GG_inv_gq, models_g,
                         Psi[qoi]);  // *** can become indefinite here when N_gq
                                     // --> 0, which depends on online/offline
                                     // pilot integration strategy
      }
    }
  }
  // Add \delta I (Schaden & Ullmann, 2020)
...
inline void NonDMultilevBLUESampling::invert_Psi(RealSymMatrix& Psi,
                                                 RealSymMatrix& Psi_inv) {
...
inline void NonDMultilevBLUESampling::compute_y(
...
inline void NonDMultilevBLUESampling::apply_mc_reference(
  ...
  switch (convergenceTolType) {
    case ABSOLUTE_CONVERGENCE_TOLERANCE:
      for (size_t qoi = 0; qoi < numFunctions; ++qoi)
        mc_targets[qoi] =
            cov_GG_g[qoi](ref_model_index, ref_model_index) / convergenceTol;
      break;
