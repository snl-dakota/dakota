...
int DartSerialDirectApplicInterface::derived_map_ac(
    const Dakota::String& ac_name) {
  int fail_code = 0;
  if (ac_name == "plugin_dart") {
    if (directFnASV[0] & 2) {
      Cerr << "DartSerialDirectApplicInterface does not currently support "
              "analytic gradients."
           << std::endl;
...
void DartSerialDirectApplicInterface::wait_local_evaluations(
...
int DartSerialDirectApplicInterface::evaluate(Dakota::RealVector& fn_val) {
  ...
  for (int i = 0; i < value.size(); ++i) fn_val[i] = value[i];

  return 0;
}

DartSerialDirectApplicInterface::DartSerialDirectApplicInterface(
    const Dakota::ProblemDescDB& problem_db, DakotaFunctor* f)
    : Dakota::DirectApplicInterface(problem_db), functor(f) {}

DartSerialDirectApplicInterface::
    ~DartSerialDirectApplicInterface() { /* Virtual destructor handles
                                            referenceCount at Interface level.
                                          */
}

void DartSerialDirectApplicInterface::derived_map_asynch(
...
void DartSerialDirectApplicInterface::test_local_evaluations(
    Dakota::PRPQueue& prp_queue) {
  wait_local_evaluations(prp_queue);
}

// Hide default run-time error checks at DirectApplicInterface level
void DartSerialDirectApplicInterface::set_communicators_checks(
