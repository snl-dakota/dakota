...
static const char rcsId[] =
    "@(#) $Id: DakotaLeastSq.cpp 7031 2010-10-22 16:23:52Z mseldre $";
...
/** This constructor extracts the inherited data for the least squares
    ...
    settings. */
LeastSq::LeastSq(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
...
LeastSq::LeastSq(unsigned short method_name, std::shared_ptr<Model> model,
                 std::shared_ptr<TraitsBase> traits)
    : Minimizer(method_name, model, traits),
      numLeastSqTerms(numFunctions - numNonlinearConstraints),
      weightFlag(
          false)  //(!model.primary_response_fn_weights().empty()), // TO DO
{
  bool err_flag = false;
  // Check for proper function definition
...
void LeastSq::print_results(std::ostream& s, short results_state) {
  ...
  if (calibrationDataFlag) {
    ...
    if (scaleFlag || weightFlag) s << "Original (as-posed) response:\n";
    // always print the native residuals
    print_residuals(numUserPrimaryFns, best_fns, RealVector(), num_best,
                    best_ind, s);
  }
  ...
  if (!confBoundsLower.empty() && !confBoundsUpper.empty()) {
    ...
    for (size_t i = 0; i < numContinuousVars; i++)
      s << std::setw(14) << cv_labels[i] << ": [ " << setw(write_precision + 6)
        << confBoundsLower[i] << ", " << setw(write_precision + 6)
        << confBoundsUpper[i] << " ]\n";
  }
}
...
void LeastSq::post_run(std::ostream& s) {
  ...
  if (!have_native_pri_fns) {
    ...
    have_native_pri_fns |=
        orig_model->db_lookup(best_vars, search_set, found_resp);
  ...
  if (!retrievedIterPriFns) {
    ...
    ActiveSet search_set(found_resp.active_set());
    search_set.request_values(0);
    for (size_t i = 0; i < numLeastSqTerms; ++i) search_set.request_value(1, i);
    // The receiving response must have the right ASV when using this
    // lookup signature
    found_resp.active_set(search_set);
    retrievedIterPriFns |=
        iteratedModel->db_lookup(iter_vars, search_set, found_resp);
  ...
  ActiveSet search_set(found_resp.active_set());
  search_set.request_values(0);
  for (size_t i = 0; i < numLeastSqTerms; ++i) search_set.request_value(2, i);
  found_resp.active_set(search_set);
  have_iter_pri_grads |=
      iteratedModel->db_lookup(iter_vars, search_set, found_resp);

  if (have_iter_pri_grads) {
    RealMatrix found_gradients(Teuchos::View, found_resp.function_gradients(),
  ...
  if (!retrievedIterPriFns || !have_native_pri_fns) {
    ...
    ModelUtils::continuous_variables(*iteratedModel,
                                     iter_vars.continuous_variables());
    activeSet.request_values(0);
    for (size_t i = 0; i < numLeastSqTerms; ++i) activeSet.request_value(1, i);
    iteratedModel->evaluate(activeSet);

    if (!retrievedIterPriFns) {
      copy_data_partial(iteratedModel->current_response().function_values(), 0,
                        (int)numLeastSqTerms, iter_fns, 0);
  ...
  if (!have_iter_pri_grads && !vendorNumericalGradFlag) {
    ...
    ModelUtils::continuous_variables(*iteratedModel,
                                     iter_vars.continuous_variables());
    activeSet.request_values(0);
    for (size_t i = 0; i < numLeastSqTerms; ++i) activeSet.request_value(2, i);
    iteratedModel->evaluate(activeSet);

    RealMatrix eval_gradients(
        Teuchos::View, iteratedModel->current_response().function_gradients(),
        (int)numContinuousVars, (int)numLeastSqTerms);
    RealMatrix iter_gradients(Teuchos::View, iter_resp.function_gradients(),
                              (int)numContinuousVars, (int)numLeastSqTerms);
  ...
  if (scaleFlag && numNonlinearConstraints > 0) {
    ...
    scale_model_rep->secondary_resp_scaled2native(iter_resp.function_values(),
...
/** Calculate individual confidence intervals for each parameter,
    ...
    must contain the final differenced, scaled, weighted residuals and
   gradients. */
void LeastSq::get_confidence_intervals(const Variables& native_vars,
  ...
  if (scaleFlag) {
    ...
    scale_model_rep->response_modify_s2n(native_vars, iter_resp, ultimate_resp,
                                         0, numLeastSqTerms, unscale_resp);
  }
  ...
  for (int i = 0; i < numContinuousVars; i++) {
    for (int j = i; j < numContinuousVars; j++)
      diag(i) +=
          Jmatrix[j * numLeastSqTerms + i] * Jmatrix[j * numLeastSqTerms + i];
...
void LeastSq::archive_best_results() {
  ...
  resultsDB.allocate_matrix(run_identifier(), {String("confidence_intervals")},
                            ResultsOutputType::REAL, confBoundsLower.length(),
                            2, scales);
