...
void get_initial_values(const Model& model, VecT& values) {
  ...
  for (int i = 0; i < ModelUtils::cv(model); ++i) values[i] = initial_points[i];
}
...
template <typename VecT>
bool get_bounds(const RealVector& lower_source, const RealVector& upper_source,
                VecT& lower_target, VecT& upper_target,
                Real big_real_bound_size, Real no_value) {
  bool allSet = true;
...
void get_bounds(const SetT& source_set, VecT& lower_target, VecT& upper_target,
                int target_offset) {
...
template <typename OrdinalType, typename ScalarType, typename VectorType2,
          typename MaskType, typename SetArray>
bool get_mixed_bounds(
    const MaskType& mask_set, const SetArray& source_set,
    ...
    VectorType2& lower_target, VectorType2& upper_target,
    ScalarType bigBoundSize, ScalarType no_value, int target_offset = 0) {
  bool allSet = true;
...
bool get_variable_bounds(Model& model,  // would like to make const but cannot
                                        // due to discrete_int_sets below
                         Real big_real_bound_size, int big_int_bound_size,
  ...
  const IntVector& lower_bnds_int =
      ModelUtils::discrete_int_lower_bounds(model);
  const IntVector& upper_bnds_int =
      ModelUtils::discrete_int_upper_bounds(model);

  const RealVector& lower_bnds_real =
      ModelUtils::discrete_real_lower_bounds(model);
  const RealVector& upper_bnds_real =
      ModelUtils::discrete_real_upper_bounds(model);

  const BitArray& int_set_bits = ModelUtils::discrete_int_sets(
      model);  // appears to be able to modify the model object ...
  ...
  bool allSet = get_bounds(lower_bnds_cont, upper_bnds_cont, lower, upper,
                           big_real_bound_size, AdapterT::noValue());

  int offset = ModelUtils::cv(model);
  allSet = allSet &&
           get_mixed_bounds(int_set_bits, init_pt_set_int, lower_bnds_int,
                            upper_bnds_int, lower, upper, big_int_bound_size,
                            (int)AdapterT::noValue(), offset);
...
int configure_inequality_constraint_maps(
    const Model& model, Real big_real_bound_size, CONSTRAINT_TYPE ctype,
    IVecT& map_indices, RVecT& map_multipliers, RVecT& map_offsets,
    Real scaling = 1.0 /* should this be tied to a trait ? RWH */) {
  const RealVector& ineq_lwr_bnds =
      (ctype == CONSTRAINT_TYPE::NONLINEAR)
...
template <typename RVecT, typename IVecT>
void configure_equality_constraint_maps(Model& model, CONSTRAINT_TYPE ctype,
                                        IVecT& indices, size_t index_offset,
                                        RVecT& multipliers, RVecT& values,
                                        bool make_one_sided) {
...
template <typename AdapterT>
void get_linear_constraints(Model& model, Real big_real_bound_size,
  ...
  const RealVector& linear_ineq_lower_bnds =
      ModelUtils::linear_ineq_constraint_lower_bounds(model);
  const RealVector& linear_ineq_upper_bnds =
      ModelUtils::linear_ineq_constraint_upper_bounds(model);
  const RealMatrix& linear_eq_coeffs =
      ModelUtils::linear_eq_constraint_coeffs(model);
  const RealVector& linear_eq_targets =
      ModelUtils::linear_eq_constraint_targets(model);

  // These are special cases involving matrices which get delegated to the
  // adapter for now
  ...
  get_bounds(linear_ineq_lower_bnds, linear_ineq_upper_bnds,
             lin_ineq_lower_bnds, lin_ineq_upper_bnds, big_real_bound_size,
             AdapterT::noValue());
...
void apply_linear_constraints(const Model& model,
                              ...
                              const VecT& in_vals, VecT& values,
                              bool adjoint = false) {
  size_t num_linear_consts =
      (etype == CONSTRAINT_EQUALITY_TYPE::EQUALITY)
          ? ModelUtils::num_linear_eq_constraints(model)
          : ModelUtils::num_linear_ineq_constraints(model);
  const RealMatrix& lin_coeffs =
      (etype == CONSTRAINT_EQUALITY_TYPE::EQUALITY)
          ? ModelUtils::linear_eq_constraint_coeffs(model)
          : ModelUtils::linear_ineq_constraint_coeffs(model);
...
void apply_nonlinear_constraints(const Model& model,
                                 ...
                                 const VecT& in_vals, VecT& values,
                                 bool adjoint = false) {
  size_t num_resp = 1;  // does this need to be generalized to more than one
                        // response value? - RWH
  ...
  size_t num_linear_consts =
      (etype == CONSTRAINT_EQUALITY_TYPE::EQUALITY)
          ? ModelUtils::num_linear_eq_constraints(model)
          : ModelUtils::num_linear_ineq_constraints(model);
  size_t num_nonlinear_consts =
      (etype == CONSTRAINT_EQUALITY_TYPE::EQUALITY)
          ? ModelUtils::num_nonlinear_eq_constraints(model)
          : ModelUtils::num_nonlinear_ineq_constraints(model);

  const RealMatrix& gradient_matrix =
      model.current_response().function_gradients();
  ...
  if (adjoint)
    for (size_t i = 0; i < num_continuous_vars; ++i) {
      // BMA --> RWH: can't zero in case compounding linear and nonlinear
      // (usability issue) values[i] = 0.0;
      for (size_t j = 0; j < num_nonlinear_consts; ++j)
        values[i] += gradient_matrix(i, grad_offset + j) *
                     in_vals[num_linear_consts + j];
    }
  else
    for (size_t j = 0; j < num_nonlinear_consts; ++j) {
      values[num_linear_consts + j] = 0.0;
      for (size_t i = 0; i < num_continuous_vars; i++)
        values[num_linear_consts + j] +=
            gradient_matrix(i, grad_offset + j) * in_vals[i];
    }
}
...
class Optimizer : public Minimizer {
 public:
  ...
  void get_common_stopping_criteria(int& max_fn_evals, int& max_iters,
                                    double& conv_tol, double& min_var_chg,
                                    double& obj_target);
  //----------------------------------------------------------------
  ...
  template <typename AdapterT>
  bool get_variable_bounds_from_dakota(typename AdapterT::VecT& lower,
                                       typename AdapterT::VecT& upper) {
    return get_variable_bounds<AdapterT>(*iteratedModel, bigRealBoundSize,
                                         bigIntBoundSize, lower, upper);
  }

  /** Method for transferring responses from Dakota data to TPL data */
  template <typename VecT>
  void get_responses_from_dakota(const RealVector& dak_fn_vals, VecT& funs,
                                 VecT& cEqs, VecT& cIneqs) {
    return get_responses(*iteratedModel, dak_fn_vals, constraintMapIndices,
                         constraintMapMultipliers, constraintMapOffsets, funs,
                         cEqs, cIneqs);
  }
 ...
 protected:
  ...
  Optimizer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
  ...
  /// number of nonlinear ineq constraints actually used (based on conditional
  /// and bigRealBoundSize
  ...
  int configure_inequality_constraints(CONSTRAINT_TYPE ctype) {
    ...
    if (ctype == CONSTRAINT_TYPE::NONLINEAR)
      scaling = (traits()->nonlinear_inequality_format() ==
                 NONLINEAR_INEQUALITY_FORMAT::ONE_SIDED_LOWER)
                    ? 1.0
                    : -1.0;
    else if (ctype == CONSTRAINT_TYPE::LINEAR)
      scaling = (traits()->linear_inequality_format() ==
                 LINEAR_INEQUALITY_FORMAT::ONE_SIDED_LOWER)
                    ? 1.0
                    : -1.0;
    else {
      Cerr << "\nError: inconsistent format for CONSTRAINT_TYPE in "
              "configure_inequality_constraints adapter."
           << std::endl;
      abort_handler(-1);
    }

    return configure_inequality_constraint_maps(
        *iteratedModel, bigRealBoundSize, ctype, constraintMapIndices,
        constraintMapMultipliers, constraintMapOffsets, scaling);
  }

  //----------------------------------------------------------------

  void configure_equality_constraints(CONSTRAINT_TYPE ctype,
                                      size_t index_offset) {
    bool split_into_one_sided = true;
    if ((ctype == CONSTRAINT_TYPE::NONLINEAR) &&
        (traits()->nonlinear_equality_format() ==
         NONLINEAR_EQUALITY_FORMAT::TRUE_EQUALITY))
      split_into_one_sided = false;

    return configure_equality_constraint_maps(
        *iteratedModel, ctype, constraintMapIndices, index_offset,
        constraintMapMultipliers, constraintMapOffsets, split_into_one_sided);
  }
  ...
  void get_linear_constraints_and_bounds(
    ...
    return get_linear_constraints<AdapterT>(
        *iteratedModel, bigRealBoundSize, lin_ineq_lower_bnds,
        lin_ineq_upper_bnds, lin_eq_targets, lin_ineq_coeffs, lin_eq_coeffs);
  }
...
inline Optimizer::Optimizer(std::shared_ptr<TraitsBase> traits)
    : Minimizer(traits), localObjectiveRecast(false) {}
...
void copy_variables(const VectorType1& source, const BitArray& set_bits,
                    const SetArray& set_vars, VectorType2& dest, size_t offset,
                    size_t len) {
  ...
  for (i = 0, set_cntr = 0; i < len; ++i) {
    if (set_bits[i]) {
      ...
      if (index == _NPOS) {
        Cerr << "\ncopy_data Error: bad index in discrete set lookup."
             << std::endl;
        abort_handler(-1);
      } else
...
template <typename VectorType1, typename VectorType2, typename SetArray>
void copy_variables(const VectorType1& source, const SetArray& set_vars,
                    VectorType2& dest, size_t offset, size_t len) {
  size_t i, index;
  for (i = 0; i < len; ++i) {
...
void copy_variables(const VectorType1& source, VectorType2& dest,
                    const BitArray& int_set_bits,
                    const IntSetArray& set_int_vars, size_t offset,
                    size_t len) {
  size_t i, dsi_cntr;
  for (i = 0, dsi_cntr = 0; i < len; ++i) {
    ...
    if (int_set_bits[i])
      dest[i] =
          set_index_to_value(source[i + offset], set_int_vars[dsi_cntr++]);
...
/** Data adapter for use by third-party opt packages to transfer response data
 * to Dakota */
template <typename AdapterT>
void set_best_responses(typename AdapterT::OptT& optimizer, const Model& model,
  ...
  if (set_objectives) {
    ...
    best_fns[0] = (!max_sense.empty() && max_sense[0])
                      ? -AdapterT::getBestObj(optimizer)
                      : AdapterT::getBestObj(optimizer);
  }

  std::vector<double> bestEqs(num_nl_eq_constr);
  std::vector<double> bestIneqs(constraint_map_indices.size() -
                                num_nl_eq_constr);

  // Get best Nonlinear Equality Constraints
  if (num_nl_eq_constr > 0) {
    optimizer.getBestNonlEqs(
        bestEqs);  // we leave this method name the same for now but could
                   // generalize depending on other TPLs
    for (size_t i = 0; i < num_nl_eq_constr; i++)
      // Need to figure out how best to generalize use of 2 index arrays, 1
      // value array and the expression - could use lambdas with c++11
      best_fns[constraint_map_indices[i] + num_user_primary_fns] =
          (bestEqs[i] - constraint_map_offsets[i]) /
          constraint_map_multipliers[i];
  }

  // Get best Nonlinear Inequality Constraints
  if (num_nl_ineq_constr > 0) {
    optimizer.getBestNonlIneqs(
        bestIneqs);  // we leave this method name the same for now but could
                     // generalize depending on other TPLs
    for (size_t i = 0; i < bestIneqs.size(); i++)
      // Need to figure out how best to generalize use of 2 index arrays, 1
      // value array and the expression - could use lambdas with c++11
      best_fns[constraint_map_indices[i + num_nl_eq_constr] +
               num_user_primary_fns] =
          (bestIneqs[i] - constraint_map_offsets[i + num_nl_eq_constr]) /
...
void set_variables(const VectorType& source, Model& model, Variables& vars) {
  ...
  copy_variables(source, discIntVars, int_set_bits, set_int_vars, num_cont_vars,
                 num_disc_int_vars);
  ...
  for (i = 0; i < num_disc_real_vars; i++)
    discRealVars[i] = set_index_to_value(
        source[i + num_cont_vars + num_disc_int_vars], set_real_vars[i]);
  vars.discrete_real_variables(discRealVars);

  for (i = 0; i < num_disc_string_vars; i++)
    vars.discrete_string_variable(
        set_index_to_value(
            source[i + num_cont_vars + num_disc_int_vars + num_disc_real_vars],
            set_string_vars[i]),
        i);
}
...
/** copy the various pieces comprising Dakota::Variables into a concatenated TPL
 * vector */
template <typename VectorType>
void get_variables(Model& model, VectorType& vec) {
  ...
  const StringMultiArrayConstView dsvars =
  ...
  if ((ModelUtils::cv(model) != cvars.length()) ||
    ...
    Cerr << "\nget_variables Error: model variables have inconsistent lengths."
         << std::endl;
    abort_handler(-1);
  }
  ...
  const StringSetArray& pt_set_string =
      ModelUtils::discrete_set_string_values(model);
  ...
  offset = cvars.length();
  copy_variables(divars, int_set_bits, pt_set_int, vec, offset,
                 divars.length());
...
/** Data adapter to transfer data from Dakota to third-party opt packages */
template <typename vectorType>
void get_responses(const Model& model, const RealVector& dak_fn_vals,
                   const std::vector<int> constraint_map_indices,
                   const std::vector<double> constraint_map_multipliers,
                   const std::vector<double> constraint_map_offsets,
                   vectorType& f_vec, vectorType& cEqs_vec,
  ...
  const BoolDeque& max_sense = model.primary_response_fn_sense();
  f_vec[0] =
      (!max_sense.empty() && max_sense[0]) ? -dak_fn_vals[0] : dak_fn_vals[0];

  // Get best Nonlinear Equality Constraints - see comments in
  // set_best_responses
  cEqs_vec.resize(num_nl_eq_constr);
  for (int i = 0; i < cEqs_vec.size(); i++)
    cEqs_vec[i] = constraint_map_offsets[i] +
                  constraint_map_multipliers[i] *
                      dak_fn_vals[constraint_map_indices[i] + 1];

  // Get best Nonlinear Inequality Constraints - see comments in
  // set_best_responses
  cIneqs_vec.resize(constraint_map_indices.size() - num_nl_eq_constr);
  for (int i = 0; i < cIneqs_vec.size(); i++)
    cIneqs_vec[i] =
        constraint_map_offsets[i + num_nl_eq_constr] +
        constraint_map_multipliers[i + num_nl_eq_constr] *
...
/** Data adapter to transfer data from Dakota to third-party opt packages */
template <typename VecT>
void get_nonlinear_eq_constraints(const Model& model, VecT& values, Real scale,
                                  int offset = -1) {
  if (-1 == offset) offset = ModelUtils::num_linear_eq_constraints(model);
  ...
  for (int i = 0; i < num_nonlinear_eq; i++)
    values[i + offset] =
        curr_resp_vals[i + 1 + num_nonlinear_ineq] + scale * nln_eq_targets[i];
}
...
void get_nonlinear_eq_constraints(Model& model,
                                  ...
                                  VecT& values, Real scale, int offset = 0) {
  const RealVector& nln_eq_targets =
      ModelUtils::nonlinear_eq_constraint_targets(model);
...
void get_nonlinear_ineq_constraints(const Model& model, VecT& values) {
  ...
  copy_data_partial(curr_resp_vals, 1, values, num_linear_ineq,
                    num_nonlinear_ineq);
}

//----------------------------------------------------------------

///  Would like to combine the previous adapter with this one (based on
///  APPSOptimizer and COLINOptimizer) and then see how much more generalization
///  is needed to support other TPLs like JEGA

/** Data adapter to transfer data from Dakota to third-party opt packages */
template <typename VecT>
void get_nonlinear_bounds(Model& model, VecT& nonlin_ineq_lower,
                          VecT& nonlin_ineq_upper, VecT& nonlin_eq_targets) {
