...
NonDStochCollocation::NonDStochCollocation(ProblemDescDB& problem_db,
  ...
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, sc_set, sc_view, approx_type, approx_order,
      corr_type, corr_order, data_order, outputLevel, pt_reuse, empty_str,
...
// Note: non-zero seed would be needed for expansionSampler, if defined
{
  ...
  ShortArray sc_asv(g_u_model->qoi(),
                    3);  // TO DO: consider passing in data_mode
  ActiveSet sc_set(sc_asv, recast_set.derivative_vector());
  const ShortShortPair& sc_view = g_u_model->current_variables().view();
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, sc_set, sc_view, approx_type, approx_order,
...
NonDStochCollocation::NonDStochCollocation(unsigned short method_name,
...
NonDStochCollocation::NonDStochCollocation(
    ...
    short exp_coeffs_approach, const RealVector& dim_pref, short refine_type,
...
void NonDStochCollocation::config_integration(
    unsigned short quad_order, unsigned short ssg_level,
    ...
    switch (expansionBasisType) {
      ...
      case Pecos::DEFAULT_BASIS:
        ...
        if (u_space_type == STD_UNIFORM_U && refineControl &&
            ruleNestingOverride !=
                Pecos::NON_NESTED) {  // TO DO: rm this constraint
          expansionCoeffsApproach = Pecos::HIERARCHICAL_SPARSE_GRID;
...
void NonDStochCollocation::config_integration(
    short exp_coeffs_approach, unsigned short num_int,
...
void NonDStochCollocation::resolve_inputs(short& u_space_type,
                                          short& data_order) {
...
void NonDStochCollocation::initialize_u_space_model() {
  ...
  if (expansionBasisType == Pecos::HIERARCHICAL_INTERPOLANT && refineControl &&
...
void NonDStochCollocation::initialize_covariance() {
  ...
  for (i = 0; i < numFunctions; ++i) {
    ...
    for (j = 0; j <= i; ++j) pa_rep_i->initialize_covariance(poly_approxs[j]);
  }
}
  ...
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    if (pa_rep_i->expansion_coefficient_flag()) {
      if (combined_stats)
        ...
        deltaRespMean[i] = (allVars) ? pa_rep_i->delta_combined_mean(initialPtU)
                                     : pa_rep_i->delta_combined_mean();
      else  // refinement assessed for impact on the current expansion
        deltaRespMean[i] = (allVars) ? pa_rep_i->delta_mean(initialPtU)
                                     : pa_rep_i->delta_mean();
...
void NonDStochCollocation::compute_delta_variance(bool update_ref,
                                                  bool print_metric) {
  std::vector<Approximation>& poly_approxs = uSpaceModel->approximations();
  bool warn_flag = false,
       combined_stats = (statsMetricMode == Pecos::COMBINED_EXPANSION_STATS);
  ...
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    if (pa_rep_i->expansion_coefficient_flag()) {
      ...
      if (update_ref) {
        ...
        if (combined_stats)
          pa_rep_i->combined_moment(respVariance[i], 1);
        else
...
void NonDStochCollocation::compute_delta_covariance(bool update_ref,
                                                    bool print_metric) {
  std::vector<Approximation>& poly_approxs = uSpaceModel->approximations();
  bool warn_flag = false,
       combined_stats = (statsMetricMode == Pecos::COMBINED_EXPANSION_STATS);
  ...
  for (i = 0; i < numFunctions; ++i) {
    ...
    if (pa_rep_i->expansion_coefficient_flag()) {
      for (j = 0; j <= i; ++j) {
        ...
        if (approx_j.expansion_coefficient_flag()) {
          if (combined_stats)
            ...
            delta = (allVars) ? pa_rep_i->delta_combined_covariance(initialPtU,
                                                                    approx_j)
                              : pa_rep_i->delta_combined_covariance(approx_j);
          else  // refinement assessed for impact on the current expansion
            delta = (allVars) ? pa_rep_i->delta_covariance(initialPtU, approx_j)
      ...
      warn_flag = true;
      for (j = 0; j <= i; ++j) deltaRespCovariance(i, j) = 0.;
    }
  }
...
Real NonDStochCollocation::compute_covariance_metric(bool revert,
                                                     bool print_metric) {
    ...
    switch (covarianceControl) {
      case DIAGONAL_COVARIANCE:
        if (relativeMetric)  // norm of reference variance, bounded from zero
          scale =
              std::max(Pecos::SMALL_NUMBER_SQ, respVariance.normFrobenius());
...
Real NonDStochCollocation::compute_level_mappings_metric(bool revert,
                                                         bool print_metric) {
  ...
  if (expansionBasisType == Pecos::HIERARCHICAL_INTERPOLANT) {
    ...
    if (refineMetric == Pecos::MIXED_STATS_METRIC) {
      ...
      switch (covarianceControl) {
        case DIAGONAL_COVARIANCE:
          compute_delta_variance(update_ref, false);
          break;
        case FULL_COVARIANCE:
          compute_delta_covariance(update_ref, false);
          break;
      }
    }
    ...
    if (beta_map) {  // hierarchical increments in beta-bar->z and z-bar->beta
      ...
      if (numerical_map) {  // merge in z-bar->p,beta* & p-bar,beta*-bar->z
        // metric_roll_up(REFINEMENT_RESULTS); // TO DO: support combined exp in
        // numerical stats
        compute_numerical_level_mappings();
        pull_level_mappings(level_maps_new,
                            offset);  // analytic overlaid at end
        deltaLevelMaps = level_maps_new;
        deltaLevelMaps -= level_maps_ref;
      } else {
      ...
      bool warn_flag = false,
           combined_stats =
               (statsMetricMode == Pecos::COMBINED_EXPANSION_STATS);
      std::vector<Approximation>& poly_approxs = uSpaceModel->approximations();
        ...
        if (pa_rep_i->expansion_coefficient_flag()) {
          if (respLevelTarget == RELIABILITIES)
            for (j = 0; j < rl_len; ++j, ++cntr) {
              ...
              if (combined_stats)
                delta = deltaLevelMaps[cntr] =
                    (allVars) ? pa_rep_i->delta_combined_beta(initialPtU,
                                                              cdfFlag, z_bar)
                              : pa_rep_i->delta_combined_beta(cdfFlag, z_bar);
              else
                delta = deltaLevelMaps[cntr] =
                    (allVars) ? pa_rep_i->delta_beta(initialPtU, cdfFlag, z_bar)
          ...
          for (j = 0; j < bl_len; ++j, ++cntr) {
            ...
            if (combined_stats)
              delta = deltaLevelMaps[cntr] =
                  (allVars) ? pa_rep_i->delta_combined_z(initialPtU, cdfFlag,
                                                         beta_bar)
                            : pa_rep_i->delta_combined_z(cdfFlag, beta_bar);
            else
              delta = deltaLevelMaps[cntr] =
                  (allVars) ? pa_rep_i->delta_z(initialPtU, cdfFlag, beta_bar)
...
void NonDStochCollocation::analytic_delta_level_mappings(
    const RealVector& level_maps_ref, RealVector& level_maps_new) {
    ...
    if (respLevelTarget == RELIABILITIES)
      for (j = 0; j < rl_len; ++j, ++cntr) {
        ...
        if (std::abs(ref) == Pecos::LARGE_NUMBER) {
          ...
          if (combined_stats)
            level_maps_new[cntr] =
                (allVars) ? pa_rep_i->combined_beta(initialPtU, cdfFlag, z_bar)
                          : pa_rep_i->combined_beta(cdfFlag, z_bar);
          else
            level_maps_new[cntr] =
                (allVars) ? pa_rep_i->beta(initialPtU, cdfFlag, z_bar)
                          : pa_rep_i->beta(cdfFlag, z_bar);
