...
void PredatorPreyModel::set_time(Real final_time, Real dt) {
  ...
  for (int n = 0; n < N_ - 1; n++) dtVector_[n] = time_[n + 1] - time_[n];
  ...
  */
  ...
  for (int i = 0; i < num_quadrature_points; i++) {
    for (int k = 0; k < N_ - 1; k++) {
      for (int j = 0; j < 3; j++) {
        int idx = 3 * k + j;
        fhat_ptr[idx] = quadrature_points[i] * f1_ptr[idx] +
                        (1. - quadrature_points[i]) * f2_ptr[idx];
        ahat_ptr[idx] = quadrature_points[i] * a1_ptr[idx] +
                        (1. - quadrature_points[i]) * a2_ptr[idx];
    ...
    for (int k = 0; k < N_ - 1; k++) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        sum += dtVector_[k] *
               ((f2_ptr[idx] - f1_ptr[idx]) / dtVector_[k] - rr_ptr[idx]) *
               ahat_ptr[idx];
      }
...
void PredatorPreyModel::compute_error_estimates(
    const RealMatrix &coordinates, const RealMatrix &forward_solutions,
    const RealMatrix &adjoint_solutions, RealMatrix &error_estimates) {
  ...
  for (int k = 0; k < num_pts; k++) {
    ...
    error_estimates(k, 0) =
        compute_error_estimate(coordinate, forward_solution, adjoint_solution);
  }
}

void PredatorPreyModel::evaluate(const RealVector &x, RealVector &result) {
  dtVector_.sizeUninitialized(N_ - 1);
  for (int n = 0; n < N_ - 1; n++) dtVector_[n] = time_[n + 1] - time_[n];
  ...
  sol(0, 0) = initialConditions_[0];
  sol(1, 0) = initialConditions_[1];
  ...
  while (ct < finalTime_) {
    ...
    if (prog > 0) {
      ...
      ppsol[0] = asol(0, N_ - prog - 1);
      ppsol[1] = asol(1, N_ - prog - 1);
