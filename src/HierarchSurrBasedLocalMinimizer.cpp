...
void HierarchSurrBasedLocalMinimizer::pre_run() {
  ...
  for (i = 0; i < num_tr; ++i) {
    ...
    tr_data.trust_region_factor(
        origTrustRegionFactor[i]);  // sets NEW_TR_FACTOR
...
void HierarchSurrBasedLocalMinimizer::post_run(std::ostream& s) {
  ...
  // if (recastSubProb) ModelUtils::continuous_variables(iteratedModel,
  // initialPoint);
...
void HierarchSurrBasedLocalMinimizer::update_trust_region(
  ...
  for (index = tr_index_start; index >= min; --index) {
    ...
    if (new_trust_region)  // nested levels at/below this level must update TR
                           // bnds
      parent_update = true;
    ...
    if (nestedTrustRegions && next_index < num_tr) {
      if (parent_update)  // update if any TR factors at/above level have
                          // changed
    ...
    // if !nested and !minimizeIndex, then no parent constraints, only global
      ...
      if (new_trust_region)  // update only if this level's TR factor has
                             // changed
...
void HierarchSurrBasedLocalMinimizer::build_center_truth(size_t tr_index) {
  ...
  ModelUtils::active_variables(*iteratedModel, tr_data.vars_center());
...
void HierarchSurrBasedLocalMinimizer::find_center_truth(size_t tr_index,
  ...
  bool truth_found;
  if (search_db)
    truth_found = find_truth_response(
        v_center, tr_data.response_center(UNCORR_TRUTH_RESPONSE));
  else {
    truth_found = true;
...
void HierarchSurrBasedLocalMinimizer::find_star_truth(size_t tr_index,
  ...
  if (search_db)
    truth_found = find_truth_response(
        v_star, tr_data.response_star(UNCORR_TRUTH_RESPONSE));
  else {
    truth_found = true;
...
void HierarchSurrBasedLocalMinimizer::find_center_approx(size_t tr_index) {
  ...
  bool approx_found = find_approx_response(
      v_center, tr_data.response_center(UNCORR_APPROX_RESPONSE));
  ...
  if (!approx_found) {
    ...
    ModelUtils::active_variables(*iteratedModel, v_center);
...
void HierarchSurrBasedLocalMinimizer::find_star_approx(size_t tr_index) {
  ...
  bool approx_found = find_approx_response(
      v_star, tr_data.response_star(UNCORR_APPROX_RESPONSE));
  ...
  if (!approx_found) {
    ...
    // iteratedModel.component_parallel_mode(SURROGATE_MODEL_MODE);
    iteratedModel->surrogate_response_mode(UNCORRECTED_SURROGATE);
    ModelUtils::active_variables(*iteratedModel, v_star);
...
RealVector HierarchSurrBasedLocalMinimizer::multigrid_recursion(
    const RealVector& x0_k, int k)
...
RealVector HierarchSurrBasedLocalMinimizer::linesearch(const RealVector& xk,
                                                       const RealVector& pk,
                                                       double alpha0) {
  ...
  for (i = 0; i < len; ++i) new_xk[i] += alpha0 * pk[i];
  return new_xk;
}

RealVector HierarchSurrBasedLocalMinimizer::optimize(const RealVector& x,
                                                     size_t max_iter,
                                                     int index) {
