...
class Analyzer : public Iterator {
 ...
 protected:
  ...
  Analyzer(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
  ...
  virtual void archive_model_variables(const Model&,
                                       size_t idx) const { /* no-op */ }

  /// archive model evaluation responses
  virtual void archive_model_response(const Response&,
                                      size_t idx) const { /* no-op */ }
...
inline void Analyzer::clear_batches() {
  ...
  batchSamplesMap.clear();
  batchVariablesMap.clear();
...
inline const VariablesArray& Analyzer::variables_array_results() {
  ...
  RealPairPRPMultiMap::const_iterator cit;
  size_t i;
  for (cit = bestVarsRespMap.begin(), i = 0; cit != bestVarsRespMap.end();
       ++cit, ++i)
...
inline bool Analyzer::returns_multiple_points() const { return true; }

inline void Analyzer::response_results_active_set(const ActiveSet& set) {
  bestVarsRespMap.begin()->second.active_set(set);
}

inline bool Analyzer::compact_mode() const { return compactMode; }

inline void Analyzer::log_response(const Model& model, IntResponseMap& resp_map,
                                   size_t i, bool log_resp_flag,
                                   bool log_best_flag) {
...
inline void Analyzer::log_response_map(const RealMatrix& samples,
...
inline void Analyzer::log_response_map(const VariablesArray& vars_array,
                                       const IntResponseMap& resp_map,
                                       bool log_best_flag) {
  bool db_act = resultsDB.active();
  if (!log_best_flag && !db_act) return;
...
inline void Analyzer::log_response_map(const IntRealVectorMap& rv_map,
                                       const IntResponseMap& resp_map,
                                       bool log_best_flag) {
  bool db_act = resultsDB.active();
  if (!log_best_flag && !db_act) return;
  ...
  for (i = 0, v_cit = rv_map.begin(), r_cit = resp_map.begin();
       v_cit != rv_map.end() && r_cit != resp_map.end();
       ++i, ++v_cit, ++r_cit) {
    if (log_best_flag)
...
inline void Analyzer::log_response_map(const IntVariablesMap& vars_map,
                                       const IntResponseMap& resp_map,
                                       bool log_best_flag) {
  bool db_act = resultsDB.active();
  if (!log_best_flag && !db_act) return;
  ...
  for (i = 0, v_cit = vars_map.begin(), r_cit = resp_map.begin();
       v_cit != vars_map.end() && r_cit != resp_map.end();
       ++i, ++v_cit, ++r_cit) {
