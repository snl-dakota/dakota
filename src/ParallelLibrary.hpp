...
class ParallelLevel {
 ...
 public:
  ...
  const MPI_Comm& server_intra_communicator()
      const;                             ///< return serverIntraComm
...
inline ParallelLevel::ParallelLevel()
    : ownCommFlag(true),
      dedicatedSchedulerFlag(false),
      commSplitFlag(false),
      serverLeaderFlag(true),
      messagePass(false),
      idlePartition(false),
      numServers(0),
      procsPerServer(0),
...
inline void ParallelLevel::copy_config(const ParallelLevel& pl) {
  ...
  commSplitFlag = pl.commSplitFlag;
  serverLeaderFlag = pl.serverLeaderFlag;
  messagePass = pl.messagePass;
  idlePartition = pl.idlePartition;
  numServers = pl.numServers;
  procsPerServer = pl.procsPerServer;
...
inline ParallelLevel::ParallelLevel(const ParallelLevel& pl) { alias(pl); }
...
inline bool ParallelLevel::server_leader() const { return serverLeaderFlag; }

inline bool ParallelLevel::message_pass() const { return messagePass; }

inline bool ParallelLevel::idle_partition() const { return idlePartition; }

inline int ParallelLevel::num_servers() const { return numServers; }

inline int ParallelLevel::processors_per_server() const {
  return procsPerServer;
}

inline int ParallelLevel::processor_remainder() const { return procRemainder; }
...
inline int ParallelLevel::server_id() const { return serverId; }
...
inline MPIUnpackBuffer& operator>>(MPIUnpackBuffer& s, ParallelLevel& pl) {
  pl.read(s);
  return s;
}
...
/// write(MPIPackBuffer&).
inline MPIPackBuffer& operator<<(MPIPackBuffer& s, const ParallelLevel& pl) {
  pl.write(s);
  return s;
}
...
inline ParallelConfiguration& ParallelConfiguration::operator=(
    const ParallelConfiguration& pc) {
  assign(pc);
  return *this;
}
...
inline const ParallelLevel& ParallelConfiguration::mi_parallel_level(
    size_t index) const {
  return (index == _NPOS) ? *miPLIters.back() : *miPLIters[index];
}

inline size_t ParallelConfiguration::mi_parallel_level_index(
    ParLevLIter pl_iter) const {
  size_t i, len = miPLIters.size();
  for (i = 0; i < len; ++i)
    if (miPLIters[i] == pl_iter) return i;
  return _NPOS;
}
...
inline bool ParallelConfiguration::mi_parallel_level_defined(
    size_t index) const {
...
/** If a meaningful index is not provided, return the last mi parallel
    level.  This is useful within the Model context, for which we need
    the lowest level partition after any meta-iterator recursions. */
inline ParLevLIter ParallelConfiguration::mi_parallel_level_iterator(
    size_t index) const {
  return (index == _NPOS) ? miPLIters.back() : miPLIters[index];
}
...
class ParallelLibrary {
 public:
  ...
  const ParallelLevel& init_iterator_communicators(
      int iterator_servers, int procs_per_iterator, int min_procs_per_iterator,
  ...
  const ParallelLevel& init_evaluation_communicators(
      int evaluation_servers, int procs_per_evaluation, int min_procs_per_eval,
      int max_procs_per_eval, int max_evaluation_concurrency,
      int asynch_local_evaluation_concurrency, short default_config,
      short evaluation_scheduling, bool peer_dynamic_avail);

  /// split an evaluation communicator into analysis communicators
  const ParallelLevel& init_analysis_communicators(
      int analysis_servers, int procs_per_analysis, int min_procs_per_analysis,
  ...
  const UserModes& user_modes()
      const;  ///< return user_modes from programOptions
  ...
  void send(int& send_int, int dest, int tag, const ParallelLevel& parent_pl,
            const ParallelLevel& child_pl);

  /// nonblocking buffer send at the current communication level
  void isend(MPIPackBuffer& send_buff, int dest, int tag, MPI_Request& send_req,
             const ParallelLevel& parent_pl, const ParallelLevel& child_pl);
  /// nonblocking integer send at the current communication level
  void isend(int& send_int, int dest, int tag, MPI_Request& send_req,
             const ParallelLevel& parent_pl, const ParallelLevel& child_pl);

  /// blocking buffer receive at the current communication level
  void recv(MPIUnpackBuffer& recv_buff, int source, int tag, MPI_Status& status,
            const ParallelLevel& parent_pl, const ParallelLevel& child_pl);
  /// blocking integer receive at the current communication level
  void recv(int& recv_int, int source, int tag, MPI_Status& status,
            const ParallelLevel& parent_pl, const ParallelLevel& child_pl);
  ...
  void irecv(int& recv_int, int source, int tag, MPI_Request& recv_req,
             const ParallelLevel& parent_pl, const ParallelLevel& child_pl);
  ...
  void waitsome(int num_sends, std::vector<MPI_Request>& recv_requests,
                int& num_recvs, std::vector<int>& index_array,
...
inline bool ParallelLibrary::is_null() const { return dummyFlag; }
...
inline void ParallelLibrary::parallel_configuration_iterator(
...
inline const ParallelConfiguration& ParallelLibrary::parallel_configuration()
    const {
  return *currPCIter;
}
...
inline bool ParallelLibrary::parallel_configuration_is_complete() {
  ...
  for (i = 1; i < num_mipl;
       ++i) {  // skip w_pl (dedicated scheduler not supported)
...
inline void ParallelLibrary::increment_parallel_configuration(
  ...
  for (i = 0; i < num_mi_pl; ++i)
    if (pc.miPLIters[i]->messagePass) ++pc.numParallelLevels;
...
inline void ParallelLibrary::increment_parallel_configuration() {
  ParLevLIter pl_iter = (parallelConfigurations.empty())
                            ? --parallelLevels.end()
                            : currPCIter->miPLIters.back();  // last to include
...
inline const ParallelLevel& ParallelLibrary::init_iterator_communicators(
    int iterator_servers, int procs_per_iterator, int min_procs_per_iterator,
    int max_procs_per_iterator, int max_iterator_concurrency,
    short default_config, short iterator_scheduling, bool peer_dynamic_avail) {
...
inline const ParallelLevel& ParallelLibrary::init_evaluation_communicators(
    int evaluation_servers, int procs_per_evaluation, int min_procs_per_eval,
    int max_procs_per_eval, int max_evaluation_concurrency,
    int asynch_local_evaluation_concurrency, short default_config,
    short evaluation_scheduling, bool peer_dynamic_avail) {
...
inline const ParallelLevel& ParallelLibrary::init_analysis_communicators(
    int analysis_servers, int procs_per_analysis, int min_procs_per_analysis,
    int max_procs_per_analysis, int max_analysis_concurrency,
    int asynch_local_analysis_concurrency, short default_config,
    short analysis_scheduling, bool peer_dynamic_avail) {
...
inline void ParallelLibrary::check_error(const String& err_source,
                                         int err_code) {
...
inline void ParallelLibrary::send(MPIPackBuffer& send_buff, int dest, int tag,
                                  const ParallelLevel& parent_pl,
                                  const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  int err_code = 0;
  if (child_pl.commSplitFlag)
    err_code =
        (parent_pl.serverCommRank)
            ? MPI_Send((void*)send_buff.buf(), send_buff.size(), MPI_PACKED, 0,
                       tag,
                       child_pl.hubServerInterComm)
            :  // servers/peers 2 through n
            MPI_Send(
                (void*)send_buff.buf(), send_buff.size(), MPI_PACKED, 0, tag,
...
inline void ParallelLibrary::send(int& send_int, int dest, int tag,
                                  const ParallelLevel& parent_pl,
                                  const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  int err_code = 0;
...
inline void ParallelLibrary::send_mi(int& send_int, int dest, int tag,
                                     size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
      (index) ? *currPCIter->miPLIters[index - 1] : *parallelLevels.begin();
  send(send_int, dest, tag, parent_pl, *currPCIter->miPLIters[index]);
}

inline void ParallelLibrary::send_mi(MPIPackBuffer& send_buff, int dest,
                                     int tag, size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
      (index) ? *currPCIter->miPLIters[index - 1] : *parallelLevels.begin();
...
inline void ParallelLibrary::send_ie(MPIPackBuffer& send_buff, int dest,
                                     int tag) {
...
inline void ParallelLibrary::isend(MPIPackBuffer& send_buff, int dest, int tag,
                                   MPI_Request& send_req,
                                   const ParallelLevel& parent_pl,
                                   const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  ...
  if (child_pl.commSplitFlag)
    err_code =
        (parent_pl.serverCommRank)
            ? MPI_Isend((void*)send_buff.buf(), send_buff.size(), MPI_PACKED, 0,
                        tag, child_pl.hubServerInterComm, &send_req)
            :  // servers/peers 2 -> n
            MPI_Isend((void*)send_buff.buf(), send_buff.size(), MPI_PACKED, 0,
                      tag, child_pl.hubServerInterComms[dest - 1],
                      &send_req);  // sched/peer 1
  else
...
inline void ParallelLibrary::isend(int& send_int, int dest, int tag,
                                   MPI_Request& send_req,
                                   const ParallelLevel& parent_pl,
                                   const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  ...
  if (child_pl.commSplitFlag)
    err_code = (parent_pl.serverCommRank)
                   ? MPI_Isend((void*)&send_int, 1, MPI_INT, 0, tag,
                               child_pl.hubServerInterComm, &send_req)
                   :  // servers/peers 2 -> n
                   MPI_Isend((void*)&send_int, 1, MPI_INT, 0, tag,
                             child_pl.hubServerInterComms[dest - 1],
                             &send_req);  // sched/peer 1
  else
...
inline void ParallelLibrary::isend_mi(int& send_int, int dest, int tag,
                                      MPI_Request& send_req, size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
      (index) ? *currPCIter->miPLIters[index - 1] : *parallelLevels.begin();
...
inline void ParallelLibrary::isend_mi(MPIPackBuffer& send_buff, int dest,
...
inline void ParallelLibrary::isend_ie(int& send_int, int dest, int tag,
                                      MPI_Request& send_req) {
  isend(send_int, dest, tag, send_req, *currPCIter->miPLIters.back(),
        *currPCIter->iePLIter);
}

inline void ParallelLibrary::isend_ie(MPIPackBuffer& send_buff, int dest,
                                      int tag, MPI_Request& send_req) {
  isend(send_buff, dest, tag, send_req, *currPCIter->miPLIters.back(),
        *currPCIter->iePLIter);
}

inline void ParallelLibrary::isend_ea(int& send_int, int dest, int tag,
                                      MPI_Request& send_req) {
  isend(send_int, dest, tag, send_req, *currPCIter->iePLIter,
        *currPCIter->eaPLIter);
}

inline void ParallelLibrary::recv(MPIUnpackBuffer& recv_buff, int source,
  ...
  if (child_pl.commSplitFlag)
    err_code =
        (parent_pl.serverCommRank)
            ? MPI_Recv((void*)recv_buff.buf(), recv_buff.size(), MPI_PACKED, 0,
                       tag, child_pl.hubServerInterComm, &status)
            :  // servers/peers 2 -> n
            MPI_Recv((void*)recv_buff.buf(), recv_buff.size(), MPI_PACKED, 0,
                     tag, child_pl.hubServerInterComms[source - 1],
                     &status);  // sched/peer 1
  else
...
inline void ParallelLibrary::recv(int& recv_int, int source, int tag,
                                  MPI_Status& status,
                                  const ParallelLevel& parent_pl,
                                  const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  ...
  if (child_pl.commSplitFlag)
    err_code =
        (parent_pl.serverCommRank)
            ? MPI_Recv((void*)&recv_int, 1, MPI_INT, 0,
                       tag,  // servers/peers 2 -> n
...
inline void ParallelLibrary::recv_mi(int& recv_int, int source, int tag,
                                     MPI_Status& status, size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
      (index) ? *currPCIter->miPLIters[index - 1] : *parallelLevels.begin();
...
inline void ParallelLibrary::recv_mi(MPIUnpackBuffer& recv_buff, int source,
                                     int tag, MPI_Status& status,
                                     size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
...
inline void ParallelLibrary::recv_ie(int& recv_int, int source, int tag,
                                     MPI_Status& status) {
  recv(recv_int, source, tag, status, *currPCIter->miPLIters.back(),
       *currPCIter->iePLIter);
}

inline void ParallelLibrary::recv_ie(MPIUnpackBuffer& recv_buff, int source,
                                     int tag, MPI_Status& status) {
  recv(recv_buff, source, tag, status, *currPCIter->miPLIters.back(),
       *currPCIter->iePLIter);
}

inline void ParallelLibrary::recv_ea(int& recv_int, int source, int tag,
                                     MPI_Status& status) {
  recv(recv_int, source, tag, status, *currPCIter->iePLIter,
       *currPCIter->eaPLIter);
}

inline void ParallelLibrary::irecv(MPIUnpackBuffer& recv_buff, int source,
                                   int tag, MPI_Request& recv_req,
                                   const ParallelLevel& parent_pl,
                                   const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  int err_code = 0;
  if (child_pl.commSplitFlag)
    err_code =
        (parent_pl.serverCommRank)
            ? MPI_Irecv((void*)recv_buff.buf(), recv_buff.size(), MPI_PACKED, 0,
                        tag, child_pl.hubServerInterComm, &recv_req)
            :  // servers/peers 2 -> n
            MPI_Irecv((void*)recv_buff.buf(), recv_buff.size(), MPI_PACKED, 0,
                      tag, child_pl.hubServerInterComms[source - 1],
                      &recv_req);  // sched/peer1
  else
...
inline void ParallelLibrary::irecv(int& recv_int, int source, int tag,
                                   MPI_Request& recv_req,
                                   const ParallelLevel& parent_pl,
                                   const ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  ...
  if (child_pl.commSplitFlag)
    err_code =
        (parent_pl.serverCommRank)
            ? MPI_Irecv((void*)&recv_int, 1, MPI_INT, 0,
                        tag,  // servers/peers 2 -> n
                        child_pl.hubServerInterComm, &recv_req)
            : MPI_Irecv((void*)&recv_int, 1, MPI_INT, 0,
                        tag,  // scheduler/peer 1
...
inline void ParallelLibrary::irecv_mi(int& recv_int, int source, int tag,
                                      MPI_Request& recv_req, size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
      (index) ? *currPCIter->miPLIters[index - 1] : *parallelLevels.begin();
...
inline void ParallelLibrary::irecv_mi(MPIUnpackBuffer& recv_buff, int source,
                                      int tag, MPI_Request& recv_req,
                                      size_t index) {
  check_mi_index(index);
  const ParallelLevel& parent_pl =
...
inline void ParallelLibrary::irecv_ie(int& recv_int, int source, int tag,
                                      MPI_Request& recv_req) {
  irecv(recv_int, source, tag, recv_req, *currPCIter->miPLIters.back(),
        *currPCIter->iePLIter);
}

inline void ParallelLibrary::irecv_ie(MPIUnpackBuffer& recv_buff, int source,
                                      int tag, MPI_Request& recv_req) {
  irecv(recv_buff, source, tag, recv_req, *currPCIter->miPLIters.back(),
        *currPCIter->iePLIter);
}

inline void ParallelLibrary::irecv_ea(int& recv_int, int source, int tag,
                                      MPI_Request& recv_req) {
...
inline void ParallelLibrary::bcast(MPIPackBuffer& send_buff,
                                   const ParallelLevel& pl) {
  bcast(send_buff, pl.serverIntraComm);
}

inline void ParallelLibrary::bcast(MPIUnpackBuffer& recv_buff,
                                   const ParallelLevel& pl) {
...
inline void ParallelLibrary::bcast_hs(MPIPackBuffer& send_buff,
                                      const ParallelLevel& pl) {
  bcast(send_buff, pl.hubServerIntraComm);
}

inline void ParallelLibrary::bcast_hs(MPIUnpackBuffer& recv_buff,
                                      const ParallelLevel& pl) {
...
inline void ParallelLibrary::bcast_mi(MPIUnpackBuffer& recv_buff,
                                      size_t index) {
  check_mi_index(index);
...
inline void ParallelLibrary::reduce_sum(double* local_vals, double* sum_vals,
                                        int num_vals, const MPI_Comm& comm) {
#ifdef DAKOTA_HAVE_MPI
  int err_code =
...
inline void ParallelLibrary::reduce_sum_ea(double* local_vals, double* sum_vals,
                                           int num_vals) {
  reduce_sum(local_vals, sum_vals, num_vals,
             currPCIter->eaPLIter->hubServerIntraComm);
}

inline void ParallelLibrary::reduce_sum_a(double* local_vals, double* sum_vals,
                                          int num_vals) {
  reduce_sum(local_vals, sum_vals, num_vals,
             currPCIter->eaPLIter->serverIntraComm);
}

inline void ParallelLibrary::test(MPI_Request& request, int& test_flag,
                                  MPI_Status& status) {
#ifdef DAKOTA_HAVE_MPI
...
inline void ParallelLibrary::waitall(int num_recvs,
...
//{
// #ifdef DAKOTA_HAVE_MPI
//   int err_code = MPI_Waitsome(num_sends, recv_requests, &num_recvs,
//   index_array,
//			      status_array);
...
inline void ParallelLibrary::waitsome(int num_sends,
                                      ...
                                      std::vector<MPI_Status>& status_array) {
#ifdef DAKOTA_HAVE_MPI
  int err_code = MPI_Waitsome(num_sends, recv_requests.data(), &num_recvs,
                              index_array.data(), status_array.data());
  check_error("MPI_Waitsome", err_code);
#endif  // DAKOTA_HAVE_MPI
}
...
inline void ParallelLibrary::alias_as_server_comm(
    const ParallelLevel& parent_pl, ParallelLevel& child_pl) {
...
inline void ParallelLibrary::copy_as_server_comm(const ParallelLevel& parent_pl,
                                                 ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
...
inline void ParallelLibrary::alias_as_hub_server_comm(
    const ParallelLevel& parent_pl, ParallelLevel& child_pl) {
...
inline void ParallelLibrary::copy_as_hub_server_comm(
    const ParallelLevel& parent_pl, ParallelLevel& child_pl) {
#ifdef DAKOTA_HAVE_MPI
  if (mpiManager.mpirun_flag() && parent_pl.serverIntraComm != MPI_COMM_NULL) {
    // single-processor servers are defined for assignment of distributed work
