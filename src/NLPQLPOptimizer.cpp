...
static const char rcsId[] =
    "@(#) $Id: NLPQLPOptimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
void QL_F77(int&, int&, int&, int&, int&, int&, double*, double*, double*,
...
NLPQLPOptimizer::NLPQLPOptimizer(ProblemDescDB& problem_db,
                                 ParallelLibrary& parallel_lib,
                                 std::shared_ptr<Model> model)
    : Optimizer(problem_db, parallel_lib, model,
                std::shared_ptr<TraitsBase>(new NLPQLPTraits())) {
  initialize();
}

NLPQLPOptimizer::NLPQLPOptimizer(std::shared_ptr<Model> model)
    : Optimizer(NLPQL_SQP, model,
                std::shared_ptr<TraitsBase>(new NLPQLPTraits())) {
  initialize();
}
...
void NLPQLPOptimizer::allocate_constraints() {
  ...
  configure_inequality_constraint_maps(
      *iteratedModel, bigRealBoundSize, CONSTRAINT_TYPE::LINEAR,
      linIneqConMappingIndices, linIneqConMappingMultipliers,
  ...
  numNlpqlConstr = numEqConstraints + constraintMapIndices.size() +
...
void NLPQLPOptimizer::allocate_workspace() {
  ...
  LWA = (int)std::floor(1.5 * (double)(NMAX * NMAX)) + 10 * NMAX + 23 * N +
        5 * numNlpqlConstr + 4 * MMAX + 217;
  LKWA = N + 10;
  ...
  // Need to initialize this data to avoid valgrind errors (TPL should do it but
  // doesn't)
  for (int i = 0; i < MMAX * NMAX; ++i) DG[i] = 0;
  U = new Real[MNN2];
  ...
  for (int i = 0; i < LACTIVE; ++i)
    ACTIVE[i] = 1;  // These need to be non-zero so that all constraints are
                    // active. - RWH
}
...
void NLPQLPOptimizer::check_sub_iterator_conflict() {
  ...
  if (sub_iterator && (sub_iterator->method_name() == NLPQL_SQP ||
...
void NLPQLPOptimizer::core_run() {
  ...
  for (fn_eval_cntr = 1; fn_eval_cntr <= maxFunctionEvals; fn_eval_cntr++) {
    ...
    if (IFAIL == 0 || IFAIL == -1) {
      ...
      if (num_nln_eq) {
        // This is a bit ungainly but is needed to use the adapter (for now) -
        // RWH
        const RealVector sliced_local_fns(
            Teuchos::View, const_cast<Real*>(&local_fns[num_nln_ineq + 1]),
            num_nln_eq);
      ...
      size_t cntr = numEqConstraints;
      for (; my_i_iter != constraintMapIndices.end();
    ...
    if (IFAIL == 0 || IFAIL == -2) {
      ...
      size_t index;
      if (max_flag)
        for (j = 0; j < numContinuousVars; j++) DF[j] = -local_grads(j, 0);
      else
        for (j = 0; j < numContinuousVars; j++) DF[j] = local_grads(j, 0);
