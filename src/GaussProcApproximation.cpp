...
Real getdist(const RealVector& x1, const RealVector& x2) {
  ...
  if (d == 0) Cerr << "Zero dimension in getdist in GaussProcApproximation\n";
  Real result;
  result = 0.0;
  for (j = 0; j < d; j++) {
...
Real mindistindx(const RealVector& x, const RealMatrix& xset,
  ...
  n = xset.numRows();
  nindx = indx.size();
  if (nindx > n) Cerr << "Size error in mindistinx in GaussProcApproximation\n";
  Real dist, result = std::numeric_limits<Real>::max();
  RealVector xp(d);
  for (i = 0; i < nindx; i++) {
...
Real getRmax(const RealMatrix& xset) {
  ...
  n = xset.numRows();
  if (n == 0 || d == 0)
    Cerr << "Zero size in getRmax in GaussProcApproximation.  n:" << n
         << "  d:" << d << std::endl;
...
GaussProcApproximation::GaussProcApproximation(
    ...
    : Approximation(BaseConstructor(), problem_db, shared_data, approx_label),
      usePointSelection(
          problem_db.get_bool("model.surrogate.point_selection")) {
...
#ifdef HAVE_NCSU
  Cout << "Using NCSU DIRECT to optimize correlation coefficients."
       << std::endl;
#else
...
void GaussProcApproximation::build() {
  ...
  for (i = 0; i < numObs; ++i) {
    ...
    for (j = 0; j < num_v; j++) trainPoints(i, j) = c_vars[j];
...
Real GaussProcApproximation::value(const Variables& vars) {
  GPmodel_apply(vars.continuous_variables(), false, false);
  return approxValue;
}

const RealVector& GaussProcApproximation::gradient(const Variables& vars) {
  GPmodel_apply(vars.continuous_variables(), false, true);
  return approxGradient;
}

Real GaussProcApproximation::prediction_variance(const Variables& vars) {
  GPmodel_apply(vars.continuous_variables(), true, false);
...
#ifdef DEBUG
  Cout << "Theta:" << std::endl;
  for (size_t i = 0; i < num_v; i++) Cout << thetaParams[i] << std::endl;
#endif  // DEBUG
}
...
void GaussProcApproximation::normalize_training_data() {
  ...
  for (i = 0; i < num_v; i++) {
    Real sum = 0.;
    for (j = 0; j < numObs; j++) sum += normTrainPoints(j, i);
  ...
  for (i = 0; i < num_v; i++)
    for (j = 0; j < numObs; j++) normTrainPoints(j, i) /= trainStdvs(i);
...
void GaussProcApproximation::get_trend() {
  ...
  for (size_t i = 0; i < numObs; i++) trendFunction(i, 0) = 1.;
...
void GaussProcApproximation::optimize_theta_global() {
  ...
  RealVector lin_ineq_lb, lin_ineq_ub, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub,
      nln_eq_tgt;
  ...
  nll_optimizer = std::make_shared<NCSUOptimizer>(
      ...
      lin_eq_coeffs, lin_eq_tgt, nln_ineq_lb, nln_ineq_ub, nln_eq_tgt, max_iter,
      max_eval, negloglikNCSU);
...
void GaussProcApproximation::get_cov_matrix() {
  ...
  for (j = 0; j < numObs; j++) {
    for (k = 0; k < numObs; k++) Cout << covMatrix(j, k) << " ";
    Cout << '\n';
  }
  Cout << '\n';
#endif  // DEBUG_FULL
...
void GaussProcApproximation::get_beta_coefficients() {
  ...
  if (ordinary) {
    ...
    for (size_t i = 0; i < numObs; i++) identity_matrix(i, i) = 1.;
  ...
  for (size_t i = 0; i < trend_dim; i++) betaCoeffs(i, 0) = temphold5(i, 0);
...
void GaussProcApproximation::get_process_variance() {
  RealMatrix YFb(numObs, 1, false), temphold3(1, 1, false);
...
void GaussProcApproximation::get_cov_vector() {
  ...
  for (size_t j = 0; j < numObs; j++) Cout << covVector(j, 0) << " ";
  Cout << '\n';
#endif  // DEBUG_FULL
}
...
void GaussProcApproximation::predict(bool variance_flag, bool gradients_flag) {
  ...
  switch (trendOrder) {
    ...
    case 1:
      ...
      f_xstar(0, 0) = 1.;
      for (j = 0; j < num_v; j++) f_xstar(0, j + 1) = approxPoint(0, j);
      break;
    case 2:
  ...
  f_beta.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1., f_xstar, betaCoeffs,
                  0.);
  ...
  if (gradients_flag) {
    ...
    for (i = 0; i < num_v; i++) {
      ...
      for (j = 0; j < numObs; j++) gradCovVector_i(j, 0) = gradCovVector(j, i);
      ...
      switch (trendOrder) {
        ...
        case 1:  // derivative of linear trend
          approxGradient[i] += betaCoeffs(i + 1, 0) / trainStdvs(i);
          break;
  ...
  if (variance_flag) {
    ...
    if (full_var_flag) {
      ...
      for (size_t i = 0; i < trend_dim; i++) f_xstar_T(i, 0) = f_xstar(0, i);
      ...
      temp10.multiply(Teuchos::TRANS, Teuchos::NO_TRANS, 1., temp9, f_FT_Rinv_r,
                      0.);
    ...
    if (approxVariance < 1.e-9) approxVariance = 1.e-9;
  }
}
  ...
  for (i = 0; i < numObs; i++)
    for (j = 0; j < num_v; j++)
      gradCovVector(i, j) = covVector(i, 0) * -2. * std::exp(thetaParams[j]) *
                            (approxPoint(0, j) - normTrainPoints(i, j)) /
                            trainStdvs(j);
...
#ifdef HAVE_OPTPP
  for (i = 0; i < 3; i++) {
    for (j = 0; j < num_v; j++) thetaParams[j] = THETA_INIT_STARTS[i];
    nll_optimizer = std::make_shared<SNLLOptimizer>(
        ...
        nln_ineq_lower_bnds, nln_ineq_upper_bnds, nln_eq_targets, negloglik,
        constraint_eval);
    nll_optimizer->run();  // no pl_iter needed for this optimization
    const Variables& vars_star = nll_optimizer->variables_results();
    const Response& resp_star = nll_optimizer->response_results();
    ...
    if (nll < nll_best) {
      nll_best = nll;
      for (j = 0; j < num_v; j++) theta_best[j] = thetaParams[j];
    }
  }
  // Now put the best run back into thetaParams and nll
  for (j = 0; j < num_v; j++) thetaParams[j] = theta_best[j];
  nll = nll_best;
#endif  // OPTPP
...
void GaussProcApproximation::calc_grad_nll() {
  ...
  Real det = 1.;
  for (i = 0; i < numObs; i++) det = det * (*covSlvr.getFactoredMatrix())(i, i);
  det = det * det;
  ...
  if (cholFlag == 0) {  // ok to go ahead with gradient calcs
    ...
    for (i = 0; i < num_v; i++) {
      for (k = 0; k < numObs; k++) {
        for (j = k; j < numObs; j++) {
          ...
          Rk(j, k) =
              -std::exp(thetaParams[i]) * pt_diff * pt_diff * covMatrix(j, k);
      ...
      trace_i = 0.;
      for (j = 0; j < numObs; j++) trace_i += trace(j, j);

      temphold5.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1., Rk, Rinv_Y,
                         0.);
    ...
    for (i = 0; i < num_v; i++) gradNegLogLikTheta(i, 0) = -1.e3;
  }
}

#ifdef HAVE_OPTPP
void GaussProcApproximation::negloglik(
    int mode, int n, const Teuchos::SerialDenseVector<int, double>& x, Real& fx,
    Teuchos::SerialDenseVector<int, double>& grad_f, int& result_mode) {
  for (size_t i = 0; i < n; i++) GPinstance->thetaParams[i] = x(i + 1);
  GPinstance->get_cov_matrix();
  GPinstance->get_cholesky_factor();  // get the factored matrix only once
...
void GaussProcApproximation::constraint_eval(
    int mode, int n, const Teuchos::SerialDenseVector<int, double>& x,
    Teuchos::SerialDenseVector<int, double>& g,
    Teuchos::SerialDenseMatrix<int, double>& grad_c, int& res_mode) {}
#endif  // HAVE_OPTPP
...
// theta).
{
  ...
  do {
    ...
    if (numObs < N_MAX_MLE) {
      ...
      // optimize_theta_multipoint();
#ifdef DEBUG_FULL
      Cout << "New theta values:  " << thetaParams[0] << "  " << thetaParams[1]
           << std::endl;
#endif
    }
    get_cov_matrix();
    ...
    // Need to allow maxdelta to go up one or two times to avoid stopping too
    // early
  ...
  for (i = 0; i < num_v; ++i)
    Cout << i + 1 << ":  " << thetaParams[i] << std::endl;
#endif  // DEBUG_FULL
  ...
  if (numObs < numObsAll / 2.5 && numObs < 100 * num_v) {
    ...
    Cerr << "***Only " << numObs << " of " << numObsAll << " points were used"
         << std::endl;
  }
}
...
// distance calculation routines
{
  ...
  for (i = 0; i < numObs; i++) pointsAddedIndex.push_back(i);
}
...
// training points and find the errors
{
  ...
  for (i = 0; i < numObsAll; i++) {
    for (j = 0; j < num_v; j++) xpred[j] = trainPoints(i, j);
...
void indexx(IterIn first, IterIn last, IterOut out) {
  ...
  for (int i = 0; i < s.size(); ++i) s[i] = std::make_pair(first + i, i);
  std::sort(s.begin(), s.end(), ComparePair1stDeref<IterIn, int>());
  for (int i = 0; i < s.size(); ++i, ++out) *out = s[i].second;
}
}  // namespace idx_table
...
// Uses unsorted errors in delta, finds which points to add, and adds them
{
  ...
  for (i = 3; i < ntest + 3; i++) {
    ...
    if (dist > 0.5 * Rmax || added_index.size() == 0)
      addpoint(itest, added_index);
  }
...
// Adds a point to the effective training set.  Returns 1 on success.
{
  ...
  for (i = 0; i < numObs; i++) {
    if (pnum == pointsAddedIndex[i]) already_in = true;
  }
...
#ifdef DEBUG_FULL
    Cout << "Attempted to add a point already in the set: " << pnum
         << std::endl;
#endif
    return 0;
  }
...
void GaussProcApproximation::lhood_2d_grid_eval() {
  ...
  do {
    do {
      ...
      get_cholesky_factor();
      nll = calc_nll();
      if (nll > 10000) nll = 1000.0;
      fout << thetaParams[0] << " " << thetaParams[1] << " " << nll
           << std::endl;
      thetaParams[0] += eps;
...
void GaussProcApproximation::writeCovMat(char filename[]) {
  ...
  size_t i, j;
  for (i = 0; i < numObs; i++) {
    for (j = 0; j < numObs; j++) fout << covMatrix(i, j) << "\t";
    fout << std::endl;
  }
  fout.close();
}
