...
class NonDBayesCalibration : public NonDCalibration {
 public:
  ...
  NonDBayesCalibration(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
 ...
 protected:
  ...
  void choose_batch_from_mutual_info(int random_seed, int num_it, int max_hifi,
                                     int num_hifi, RealMatrix& mi_chain,
  ...
  void build_hi2lo_xmatrix(RealMatrix& Xmatrix, int i,
  ...
  void filter_chain(const RealMatrix& acceptance_chain,
  ...
  void filter_fnvals(const RealMatrix& accepted_fn_vals,
  ...
  RealMatrix predVals;
  /// cached filtered function values for printing (may be a view of
  /// acceptedFnVals)
  ...
  static void ann_dist(const ANNpointArray matrix1, const ANNpointArray matrix2,
                       RealVector& distances, int NX, int NY, int dim2,
                       IntVector& k, double eps);
  static void ann_dist(const ANNpointArray matrix1, const ANNpointArray matrix2,
                       RealVector& distances, Int2DArray& indices, int NX,
                       int NY, int dim2, IntVector& k, double eps);
  Real kl_est;
...
inline std::shared_ptr<Model> NonDBayesCalibration::algorithm_space_model() {
...
Real NonDBayesCalibration::prior_density(const VectorType& vec) {
  ...
  if (no_mask)
    for (v = 0; v < num_rv; ++v) {
#ifdef DEBUG
      Cout << "Variable " << v + 1 << " = " << vec[v]
           << " prior density = " << mv_dist.pdf(vec[v], v) << std::endl;
#endif  // DEBUG
...
inline Real NonDBayesCalibration::prior_density(const RealVector& vec) {
  ...
  const Pecos::MultivariateDistribution& mv_dist =
      (standardizedSpace)
          ? residualModel->multivariate_distribution()   // u_dist
...
#ifdef DEBUG
      Cout << "Variable " << v + 1 << " = " << vec[v]
           << " prior density = " << mv_dist.pdf(vec[v], v)
           << " log prior density = " << mv_dist.log_pdf(vec[v], v)
           << std::endl;
#endif  // DEBUG
...
template <typename Engine>
void NonDBayesCalibration::prior_sample(Engine& rng,
                                        RealVector& prior_samples) {
...
template <typename VectorType1, typename VectorType2>
void NonDBayesCalibration::augment_gradient_with_log_prior(
    VectorType1& log_grad, const VectorType2& vec) {
...
template <typename MatrixType, typename VectorType>
void NonDBayesCalibration::augment_hessian_with_log_prior(
    MatrixType& log_hess, const VectorType& vec) {
  // neg log posterior = neg log likelihood + neg log prior = misfit - log prior
...
inline void NonDBayesCalibration::get_positive_definite_covariance_from_hessian(
    const RealSymMatrix& hessian, RealSymMatrix& covariance) {
