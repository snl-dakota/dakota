...
std::vector<double> get_variables_values(
    const Dakota::LibraryEnvironment& env) {
...
py::array_t<double> get_variables_values_numpy(
    const Dakota::LibraryEnvironment& env) {
  const Variables& vars = env.variables_results();
  const RealVector& var_vals = vars.continuous_variables();
  ...
  for (size_t i = 0; i < buffer.shape[0]; ++i) ptr[i] = var_vals[i];
...
PYBIND11_MODULE(environment, m) {
  ...
  m.def("version", &Dakota::python::print_version,
  ...
  py::class_<Dakota::ExecutableEnvironment>(m, "CommandLine")
      .def(py::init([](const std::string& input_filename) {
        int argc = 2;
        std::string infile_copy(input_filename);
        char* argv[] = {"dakota", const_cast<char*>(infile_copy.data()), NULL};
  ...
  py::class_<Dakota::Variables>(m, "Variables")
      .def(py::init([]() { return new Dakota::Variables(); }))
  ...
  py::class_<Dakota::Response>(m, "Response")
      .def(py::init([]() { return new Dakota::Response(); }))

      .def("function_value",
           static_cast<const Dakota::Real& (Dakota::Response::*)(size_t) const>(
               &Dakota::Response::function_value),
           "Return function value by index", py::arg("i"));

  // demo a library environment that models opt_tpl_test semantics
  py::class_<Dakota::LibraryEnvironment>(m, "study")
      .def(py::init([](py::object callback, const std::string& input_string) {
             assert(!input_string.empty());
             auto p_libEnv = Dakota::python::create_libEnv(input_string);

             // Associate the single python callback with all Pybind11Interface
             // interfaces
             Dakota::InterfaceList& interfaces =
                 Dakota::Interface::interface_cache(
                     p_libEnv->problem_description_db());
             for (auto& interface : interfaces) {
               auto py11_int =
                   std::dynamic_pointer_cast<Dakota::Pybind11Interface>(
                       (*Dakota::Interface::interface_cache(
                             p_libEnv->problem_description_db())
                             .begin()));

               if (py11_int) py11_int->register_pybind11_callback_fn(callback);
             }
      ...
      .def(py::init([](py::dict callbacks, const std::string& input_string) {
             assert(!input_string.empty());
             auto p_libEnv = Dakota::python::create_libEnv(input_string);

             // Associate callbacks with interface specs
             auto callbacks_map =
                 callbacks.cast<std::map<std::string, py::function> >();
             Dakota::InterfaceList& interfaces =
                 Dakota::Interface::interface_cache(
                     p_libEnv->problem_description_db());
             for (auto& interface : interfaces) {
               ...
               if (py11_int) {
                 for (auto const& idrv : interface->analysis_drivers()) {
                   if (callbacks_map.count(idrv) > 0)
                     py11_int->register_pybind11_callback_fn(
                         callbacks_map[idrv]);
                   else {
                     Cout << "Warning: Could not find a pybind11 callback \""
                          << idrv
                          << "\" needed "
                             "by Dakota interface \""
                          << interface->interface_id()
                          << "\".\n"
                             "... will try to use module:function interface "
                             "specification.\n";
                   }
                 }
               }
...
#ifdef DAKOTA_PYTHON_NUMPY
  m.def("get_variable_values_np", &Dakota::python::get_variables_values_numpy,
        "Get active continuous Variable values");
#endif

  m.def("get_variable_values", &Dakota::python::get_variables_values,
        "Get active continuous Variable values");

  m.def("get_response_fn_val", &Dakota::python::get_response_fn_val,
