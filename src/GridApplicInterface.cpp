...
GridApplicInterface::GridApplicInterface(const ProblemDescDB& problem_db,
  ...
  get_jobs_completed =
      (get_jobs_completed_t)dlsym(handle, "get_jobs_completed");
  if ((error = dlerror()) != NULL) {
    Cerr << "Problem loading get_jobs_completed function: " << error
         << std::endl;
    abort_handler(-1);
  }
  int status = (*start_grid_computing)(
      programNames[0].data(), paramsFileName.data(), resultsFileName.data());
  // fileSaveFlag=true;
}
...
void GridApplicInterface::derived_map(const Variables& vars,
                                      const ActiveSet& set, Response& response,
                                      int fn_eval_id) {
  //
...
bool GridApplicInterface::grid_file_test(const String& root_file) {
  ...
  if (num_progs > 1 && oFilterName().empty()) {
    ...
    for (size_t i = 0; i < num_progs; i++) {
      ...
      if (stat((char*)tagged_file.data(), &buf) == -1) return false;
    }
    return true;
#else
...
extern "C" int stop_grid_computing() { return 0; }

/** sample function prototype for submitting a grid evaluation */
extern "C" int perform_analysis(char* iteration_num) {
  spawn_evaluation_to_shell(true);
  return 0;
}
#endif
