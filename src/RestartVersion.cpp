...
RestartVersion RestartVersion::check_restart_version(
    const std::string& rst_filename) {
  std::ifstream ifs(rst_filename, std::ios::binary);
  if (!ifs.good()) {
    Cerr << "\nError: could not open restart file '" << rst_filename
  ...
  try {
    ...
    // else rstFirstVer# <= read version <= curr ver
      ...
      Cerr << "\nError: cannot read restart file '" << rst_filename
           ...
           << rst_ver.dakotaRelease << "\n  with restart version "
           << rst_ver.restartVersion << "." << std::endl;
      // abort_handler(IO_ERROR);
    } else {
      Cout << "Reading restart file '" << rst_filename
           << "' containing: " << rst_ver;
    }
