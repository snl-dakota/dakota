...
ProbabilityTransformModel::ProbabilityTransformModel(
    std::shared_ptr<Model> x_model, short u_space_type,
    // const ShortShortPair& recast_vars_view,
    bool truncate_bnds, Real bnd)
    : RecastModel(x_model),  // minimal initialization
                             // RecastModel(x_model, recast_vars_view), // No:
                             // for recasts limited to view
      distParamDerivs(NO_DERIVS),
      truncatedBounds(truncate_bnds),
      boundVal(bnd) {
...
void ProbabilityTransformModel::initialize_dakota_variable_types() {
  ...
  size_t i, num_cdv, num_ddiv, num_ddsv, num_ddrv, num_cauv, num_dauiv,
      num_dausv, num_daurv, num_ceuv, num_deuiv, num_deusv, num_deurv, num_csv,
      num_dsiv, num_dssv, num_dsrv, rv_cntr = 0, cv_cntr = 0, div_cntr = 0,
...
void ProbabilityTransformModel::update_model_bounds(bool truncate_bnds,
                                                    Real bnd) {
  ...
  RealVector c_l_bnds(num_cv, false);
  c_l_bnds = -1.;
  RealVector c_u_bnds(num_cv, false);
  c_u_bnds = 1.;
  ...
  size_t i, num_cdv, num_ddiv, num_ddsv, num_ddrv, num_cauv, num_dauiv,
      num_dausv, num_daurv, num_ceuv, num_deuiv, num_deusv, num_deurv, num_csv,
      num_dsiv, num_dssv, num_dsrv, cv_cntr = 0, rv_cntr = 0;
  u_svd.design_counts(num_cdv, num_ddiv, num_ddsv, num_ddrv);
  u_svd.aleatory_uncertain_counts(num_cauv, num_dauiv, num_dausv, num_daurv);
  u_svd.epistemic_uncertain_counts(num_ceuv, num_deuiv, num_deusv, num_deurv);
  u_svd.state_counts(num_csv, num_dsiv, num_dssv, num_dsrv);
        ...
        switch (u_types[rv_cntr]) {
          ...
          case Pecos::BOUNDED_LOGNORMAL: {
            ...
            if (u_bnd < dbl_inf)
              ...
              rv_i.pull_parameter(Pecos::LN_MEAN, mean);
              rv_i.pull_parameter(Pecos::LN_STD_DEV, stdev);
              // Note: as for NIDR initialization, we use mean,std_dev
              // parameters rather than computing actual mean,std_dev of bounded
              // distribution
              c_u_bnds[cv_cntr] = mean + bnd * stdev;
            }
            break;
          ...
          case Pecos::HISTOGRAM_BIN:
            ...
            c_l_bnds[cv_cntr] =
                ModelUtils::continuous_lower_bound(*subModel, cv_cntr);
            c_u_bnds[cv_cntr] =
                ModelUtils::continuous_upper_bound(*subModel, cv_cntr);
            break;
          ...
          case Pecos::GUMBEL: {  // unbounded distribution
            ...
            Pecos::GumbelRandomVariable::moments_from_params(alpha, beta, mean,
                                                             stdev);
          ...
          case Pecos::FRECHET: {  // semibounded distribution
            ...
            Pecos::FrechetRandomVariable::moments_from_params(alpha, beta, mean,
                                                              stdev);
            c_l_bnds[cv_cntr] = 0.;
            c_u_bnds[cv_cntr] = mean + bnd * stdev;
            break;
          ...
          case Pecos::WEIBULL: {  // semibounded distribution
            ...
            Pecos::WeibullRandomVariable::moments_from_params(alpha, beta, mean,
                                                              stdev);
            c_l_bnds[cv_cntr] = 0.;
            c_u_bnds[cv_cntr] = mean + bnd * stdev;
            break;
      ...
      for (i = 0; i < num_cauv; ++i, ++cv_cntr, ++rv_cntr) {
        ...
        switch (u_types[rv_cntr]) {
          ...
          case Pecos::GUMBEL:  // unbounded distributions
            c_l_bnds[cv_cntr] = -dbl_inf;
            c_u_bnds[cv_cntr] = dbl_inf;
            break;
...
void ProbabilityTransformModel::initialize_distribution_types(
  ...
  for (i = 0; i < num_rv; ++i)
    if (no_mask || active_rv[i]) switch (u_space_type) {
        case STD_NORMAL_U:
        case STD_UNIFORM_U:
          switch (x_types[i]) {
            case Pecos::DISCRETE_RANGE:
            case Pecos::DISCRETE_SET_INT:
            case Pecos::DISCRETE_SET_STRING:
            case Pecos::DISCRETE_SET_REAL:
            case Pecos::POISSON:
            case Pecos::BINOMIAL:
            ...
            case Pecos::CONTINUOUS_INTERVAL_UNCERTAIN:
              ...
              break;
            default:
              u_types[i] = (u_space_type == STD_UNIFORM_U) ? Pecos::STD_UNIFORM
                                                           : Pecos::STD_NORMAL;
              break;
          }
          break;
        case PARTIAL_ASKEY_U:  // used for cases with limited distrib support
                               // (C3)
          switch (x_types[i]) {
            case Pecos::NORMAL:
            case Pecos::BOUNDED_NORMAL:
            ...
            case Pecos::GUMBEL:
            case Pecos::FRECHET:
            case Pecos::WEIBULL:  // unbounded or semi-bounded dist; bounded
                                  // N/logN
              u_types[i] = Pecos::STD_NORMAL;
              break;
            case Pecos::UNIFORM:
            case Pecos::LOGUNIFORM:
            case Pecos::TRIANGULAR:
        ...
        case ASKEY_U:
          switch (x_types[i]) {
            ...
            case Pecos::EXPONENTIAL:
              ...
              break;
            case Pecos::BETA:
              u_types[i] = Pecos::STD_BETA;
              break;
            case Pecos::GAMMA:
              u_types[i] = Pecos::STD_GAMMA;
              break;
            // TO DO: discrete types ddv, dauv, deuv, dsv
    ...
    for (i = 0, corr_i = 0; i < num_rv; ++i) {
      ...
      if (distribution_error) {
        Cerr << "Error: correlation warping for Nataf variable transformation "
             ...
             << "not currently supported.  Error detected for variable "
             << i + 1 << "." << std::endl;
        err_flag = true;
      }
    }
    if (err_flag) abort_handler(MODEL_ERROR);
  }
}

unsigned short ProbabilityTransformModel::pecos_to_dakota_variable_type(
    unsigned short pecos_var_type, size_t rv_index) {
  ...
  switch (pecos_var_type) {
    case Pecos::CONTINUOUS_RANGE:  // non-unique mapping
      return (rv_index < vc_totals[TOTAL_CDV])
                 ?  // not subject to active subsets
                 CONTINUOUS_DESIGN
                 : CONTINUOUS_STATE;
      break;
    ...
    // discrete epistemic
    case Pecos::DISCRETE_INTERVAL_UNCERTAIN:
      return DISCRETE_INTERVAL_UNCERTAIN;
      break;
    case Pecos::DISCRETE_UNCERTAIN_SET_INT:
      return DISCRETE_UNCERTAIN_SET_INT;
      break;
    case Pecos::DISCRETE_UNCERTAIN_SET_STRING:
      return DISCRETE_UNCERTAIN_SET_STRING;
      break;
    case Pecos::DISCRETE_UNCERTAIN_SET_REAL:
      return DISCRETE_UNCERTAIN_SET_REAL;
      break;
    default:
...
void ProbabilityTransformModel::resp_x_to_u_mapping(const Variables& x_vars,
  ...
  for (i = 0; i < num_fns; ++i) {
    if (u_asv[i] & 2) u_grad_flag = true;
    if (u_asv[i] & 4) u_hess_flag = true;
  }
  ...
  if (map_derivs) {
    ...
    if (ptmInstance->distParamDerivs > NO_DERIVS)
      ptmInstance->natafTransform.jacobian_dX_dS(
          x_cv, jacobian_xs, x_cv_ids, u_cv_ids, x_acv_ids,
  ...
  for (i = 0; i < num_fns; ++i) {
    ...
    if (x_asv_val & 2) fn_grad_x = x_response.function_gradient_view(i);
    ...
    if (u_asv_val & 2) {
      ...
      if (map_derivs) {  // perform transformation
        ...
        if (ptmInstance->distParamDerivs > NO_DERIVS)  // transform subset
          ptmInstance->natafTransform.trans_grad_X_to_S(
              fn_grad_x, fn_grad_us, jacobian_xs, x_dvv, x_cv_ids, u_cv_ids,
              x_acv_ids, ptmInstance->primaryACVarMapIndices,
              ptmInstance->secondaryACVarMapTargets);
        else  // transform subset of components
          ptmInstance->natafTransform.trans_grad_X_to_U(
              fn_grad_x, x_cv_ids, fn_grad_us, jacobian_xu, x_dvv);
    ...
    if (u_asv_val & 4) {
      if (!(x_asv_val & 4) ||
          (map_derivs && ptmInstance->nonlinearVarsMapping &&
           !(x_asv_val & 2))) {
      ...
      if (map_derivs) {  // perform transformation
        ...
        if (ptmInstance->distParamDerivs > NO_DERIVS) {  // transform subset
          ...
          ptmInstance->natafTransform.trans_hess_X_to_U(
              fn_hess_x, x_cv_ids, fn_hess_us, jacobian_xu, hessian_xu,
              fn_grad_x, x_dvv);
