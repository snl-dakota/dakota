...
Approximation::Approximation(BaseConstructor, const ProblemDescDB& problem_db,
    ...
    : approxData(true),
      approxLabel(approx_label),
      sharedDataRep(shared_data.data_rep()) { /* empty ctor */ }
...
/** This constructor is the one which must build the base class data
    ...
    base class constructor calling get_approx() again).  Since the
    letter IS the representation, its rep pointer is set to NULL. */
Approximation::Approximation(NoDBBaseConstructor,
                             const SharedApproxData& shared_data)
    : approxData(true),
      sharedDataRep(shared_data.data_rep()) { /* empty ctor */ }
...
Approximation::Approximation() { /* empty ctor */ }
...
Approximation::Approximation(ProblemDescDB& problem_db,
                             const SharedApproxData& shared_data,
                             const String& approx_label)
...
std::shared_ptr<Approximation> Approximation::get_approx(
  ...
  if (pw_decomp) {
    return std::make_shared<VPSApproximation>(problem_db, shared_data,
                                              approx_label);
  } else {
    const String& approx_type = shared_data.data_rep()->approxType;
    if (approx_type == "local_taylor")
      return std::make_shared<TaylorApproximation>(problem_db, shared_data,
                                                   approx_label);
    else if (approx_type == "multipoint_tana")
      return std::make_shared<TANA3Approximation>(problem_db, shared_data,
                                                  approx_label);
    else if (approx_type == "multipoint_qmea")
      return std::make_shared<QMEApproximation>(problem_db, shared_data,
                                                approx_label);
    else if (strends(approx_type, "_orthogonal_polynomial") ||
             strends(approx_type, "_interpolation_polynomial"))
      return std::make_shared<PecosApproximation>(problem_db, shared_data,
                                                  approx_label);
    else if (approx_type == "global_gaussian")
      return std::make_shared<GaussProcApproximation>(problem_db, shared_data,
                                                      approx_label);
#ifdef HAVE_C3
    else if (approx_type == "global_function_train")
      return std::make_shared<C3Approximation>(problem_db, shared_data,
                                               approx_label);
#endif
#ifdef HAVE_SURFPACK
      ...
      return std::make_shared<SurfpackApproximation>(problem_db, shared_data,
                                                     approx_label);
#endif  // HAVE_SURFPACK
#ifdef HAVE_DAKOTA_SURROGATES
#ifdef HAVE_ROL
    else if (approx_type == "global_exp_gauss_proc")
      return std::make_shared<SurrogatesGPApprox>(problem_db, shared_data,
                                                  approx_label);
#endif
    else if (approx_type == "global_exp_poly")
      return std::make_shared<SurrogatesPolyApprox>(problem_db, shared_data,
                                                    approx_label);
#ifdef HAVE_DAKOTA_PYTHON_SURROGATES
    else if (approx_type == "global_exp_python")
      return std::make_shared<SurrogatesPythonApprox>(problem_db, shared_data,
                                                      approx_label);
#endif  // HAVE_DAKOTA_PYTHON_SURROGATES
#endif  // HAVE_DAKOTA_SURROGATES
    else {
...
Approximation::Approximation(const Approximation& approx)
    : approxRep(approx.approxRep) { /* empty ctor */ }
...
Approximation::~Approximation() { /* empty dtor */ }
...
void Approximation::export_model(const StringArray& var_labels,
...
void Approximation::export_model(const Variables& vars, const String& fn_label,
                                 const String& export_prefix,
                                 const unsigned short export_format) {
  if (approxRep)
...
void Approximation::replace(const IntResponsePair& response_pr,
                            size_t fn_index) {
  if (approxRep)
    approxRep->replace(response_pr, fn_index);
  else {
    Pecos::SurrogateDataResp sdr = response_to_sdr(
        response_pr.second,
        fn_index);  // *** Note: SHALLOW_COPY *** the referenced responses will
                    // be cleared from the Model queue but *may* persist in the
                    // eval cache; this is what
                    // ApproximationInterface::cache_lookup() manages ... Only
                    // matters for gradient/Hessian views ...
...
void Approximation::pop_data(bool save_data) {
  if (approxRep)
    approxRep->pop_data(save_data);
  else
    approxData.pop(sharedDataRep->activeKey, save_data);
}
...
const RealVector& Approximation::mean_gradient(const RealVector& x,
                                               const SizetArray& ind) {
  if (!approxRep) {
...
const RealVector& Approximation::variance_gradient(const RealVector& x,
                                                   const SizetArray& ind) {
  if (!approxRep) {
...
Real Approximation::combined_covariance(const RealVector& x,
...
void Approximation::compute_moments(const RealVector& x, bool full_stats,
                                    bool combined_stats) {
  if (approxRep)
...
bool Approximation::advancement_available() {
  if (approxRep)
    return approxRep->advancement_available();
  else
...
RealArray Approximation::cv_diagnostic(const StringArray& metric_types,
                                       unsigned num_folds) {
  if (!approxRep) {
...
void Approximation::primary_diagnostics(size_t fn_index) {
  if (approxRep) approxRep->primary_diagnostics(fn_index);
  // else no-op
}

RealArray Approximation::challenge_diagnostic(
  ...
  return approxRep->challenge_diagnostic(metric_types, challenge_points,
                                         challenge_resps);
}

void Approximation::challenge_diagnostics(size_t fn_index,
...
void Approximation::coefficient_labels(
    std::vector<std::string>& coeff_labels) const {
  if (approxRep)
...
int Approximation::min_points(bool constraint_flag) const {
  if (approxRep)
    ...
    if (constraint_flag) coeffs -= num_constraints();
    ...
    return (data_per_pt > 1) ? (int)std::ceil((Real)coeffs / (Real)data_per_pt)
                             : coeffs;
  }
}
...
Pecos::SurrogateDataVars Approximation::variables_to_sdv(
    const Real* sample_c_vars) {
...
Pecos::SurrogateDataVars Approximation::variables_to_sdv(
  ...
  if (vars.cv() + vars.div() + vars.drv() == num_v)
    return Pecos::SurrogateDataVars(
        vars.continuous_variables(), vars.discrete_int_variables(),
        vars.discrete_real_variables(), Pecos::SHALLOW_COPY);
  else if (vars.acv() + vars.adiv() + vars.adrv() == num_v)
    return Pecos::SurrogateDataVars(
        vars.all_continuous_variables(), vars.all_discrete_int_variables(),
        vars.all_discrete_real_variables(), Pecos::SHALLOW_COPY);
...
Pecos::SurrogateDataResp Approximation::response_to_sdr(
    const Response& response, size_t fn_index) {
  short asv_val = response.active_set_request_vector()[fn_index];
  switch (asv_val) {
    case 0:  // should not happen: ASV dropouts managed at higher level
      return Pecos::SurrogateDataResp();
      break;
...
void Approximation::add_array(const RealMatrix& sample_vars, bool v_copy,
