...
void print_expected_format(std::ostream& s, unsigned short tabular_format,
  ...
  if (tabular_format > TABULAR_NONE) {
    if (tabular_format & TABULAR_HEADER) s << "\n  * header row with labels";
  ...
  if (num_rows) s << "; " << num_rows << " rows";
  if (num_cols) s << "; " << num_cols << " columns";
  s << std::endl;
}
...
void write_data_tabular(std::ostream& tabular_ostream, const Variables& vars,
                        const String& iface_id, const Response& response,
...
void write_data_tabular(std::ostream& tabular_ostream, const Variables& vars,
                        const String& iface_id, size_t counter,
                        unsigned short tabular_format) {
  write_leading_columns(tabular_ostream, counter, iface_id, tabular_format);
...
void write_eol(std::ostream& tabular_ostream) { tabular_ostream << std::endl; }
...
void write_data_tabular(const std::string& output_filename,
  ...
  for (size_t row = 0; row < num_coeff_rows; ++row) {
    for (size_t fn_ind = 0; fn_ind < num_fns; ++fn_ind)
      Dakota::write_data_tabular(output_stream, &output_coeffs[fn_ind][row], 1);
    Dakota::write_data_tabular(output_stream, &output_indices[row][0],
                               num_vars);
...
void read_leading_columns(std::istream& input_stream,
  ...
  if (tabular_format & TABULAR_IFACE_ID) {
    ...
    // (Dakota 6.1 used EMPTY for missing ID)
    if (iface_id == "EMPTY") iface_id = "NO_ID";
  } else
    iface_id = "NO_ID";
}
...
bool exists_extra_data(std::istream& input_stream) {
  ...
  while (input_stream.good() && !input_stream.eof()) {
    try {
      ...
      if (!extra_data.empty()) return true;
...
std::vector<size_t> find_vars_map(
...
void print_expected_labels(
    bool active_only, const StringArray& expected_vars,
    const StringArray::const_iterator& read_vars_begin,
    const StringArray::const_iterator& header_fields_end) {
  std::ostream_iterator<String> out_it(Cout, " ");
  Cout << "\nExpected labels (for " << ((active_only) ? "active" : "all")
       << " variables):\n  ";
  std::copy(expected_vars.begin(), expected_vars.end(), out_it);
  Cout << std::endl
       << "Instead found these in header (including "
...
std::vector<size_t> validate_header(std::ifstream& data_stream,
  ...
  auto read_vars_begin = (num_lead < read_fields)
                             ? header_fields.begin() + num_lead
                             : header_fields.end();

  bool vars_equal = (num_lead + num_vars > read_fields)
                        ? false
                        : std::equal(expected_vars.begin(), expected_vars.end(),
                                     read_vars_begin);

  bool vars_permuted =
      (num_lead + num_vars > read_fields)
          ? false
...
void read_data_tabular(const std::string& input_filename,
  ...
  if (exists_extra_data(input_stream))
    print_unexpected_data(Cout, input_filename, context_message,
                          tabular_format);
...
void read_data_tabular(const std::string& input_filename,
                       ...
                       bool verbose, bool use_var_labels, bool active_only) {
  ...
  size_t num_vars = active_only
                        ? vars.cv() + vars.div() + vars.dsv() + vars.drv()
                        : vars.tv();
  ...
  try {
    ...
    while (input_stream.good() && !input_stream.eof()) {
      ...
      size_t num_read =
          strsplit(row_str).size();  // TODO: count without storing
      ...
      std::istringstream row_iss(
          var_inds.empty() ? row_str
      ...
      const IntVector& di_vars = active_only
                                     ? vars.discrete_int_variables()
                                     : vars.all_discrete_int_variables();
      const RealVector& dr_vars = active_only
      ...
      for (size_t fi = 0; fi < num_fns; ++fi) {
        ...
        if (row_iss >> read_value) work_resp_vec(fi) = read_value;
      }
...
void read_data_tabular(const std::string& input_filename,
  ...
  if (input_coeffs.size() != num_fns) input_coeffs.resize(num_fns);
  for (size_t fn_ind = 0; fn_ind < num_fns; ++fn_ind) {
...
size_t read_data_tabular(const std::string& input_filename,
  ...
  for (size_t i = 0; i < num_evals; ++i) dsva[i] = list_dsv_points[i];
...
std::pair<size_t, bool> read_data_tabular(const std::string& input_filename,
  ...
  try {
    ...
    input_stream >> std::ws;  // advance to next readable input
    while (configs_read < max_configs && input_stream.good() &&
           !input_stream.eof()) {
      // discard the row labels (typically eval and iface ID)
      read_leading_columns(input_stream, tabular_format);
