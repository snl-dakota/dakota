...
static const char rcsId[] =
    "@(#) $Id: CONMINOptimizer.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
CONMINOptimizer::CONMINOptimizer(ProblemDescDB& problem_db,
                                 ParallelLibrary& parallel_lib,
                                 std::shared_ptr<Model> model)
    : Optimizer(problem_db, parallel_lib, model,
                std::shared_ptr<TraitsBase>(new CONMINTraits())) {
...
CONMINOptimizer::CONMINOptimizer(const String& method_string,
                                 std::shared_ptr<Model> model)
    : Optimizer(method_string_to_enum(method_string), model,
                std::shared_ptr<TraitsBase>(new CONMINTraits())) {
  initialize();
}  // convenience fn for shared ctor code
...
void CONMINOptimizer::allocate_constraints() {
  ...
  // Augment nonlinear inequality maps (set in
  // Optimizer::configure_constraint_maps) with additional constraint info ...
  configure_equality_constraints(CONSTRAINT_TYPE::NONLINEAR, num_nln_ineq);
  numConminLinConstr = 2 * num_lin_eq;
  numConminLinConstr +=
      configure_inequality_constraints(CONSTRAINT_TYPE::LINEAR);
...
void CONMINOptimizer::initialize_run() {
  ...
  size_t i;
  for (i = 0; i < numConminConstr; i++) IC[i] = ISC[i] = 0;
...
void CONMINOptimizer::core_run() {
  ...
  for (fn_eval_cntr = 1; fn_eval_cntr <= maxFunctionEvals; fn_eval_cntr++) {
    ...
    if (conminInfo == 2) {
      ...
      for (conmin_constr = 0; conmin_constr < numConminConstr;
           conmin_constr++) {
