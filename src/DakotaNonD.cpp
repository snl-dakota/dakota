...
static const char rcsId[] =
    "@(#) $Id: DakotaNonD.cpp 7024 2010-10-16 01:24:42Z mseldre $";
...
NonD::NonD(ProblemDescDB& problem_db, ParallelLibrary& parallel_lib,
           std::shared_ptr<Model> model)
    : Analyzer(problem_db, parallel_lib, model),
      respLevelTarget(
          problem_db.get_short("method.nond.response_level_target")),
      ...
      cdfFlag(problem_db.get_short("method.nond.distribution") !=
              COMPLEMENTARY),
      pdfOutput(false),
  ...
  distribute_levels(requestedGenRelLevels,
                    !cdfFlag);  // descending if cumulative
  ...
  if (totalLevelRequests && outputLevel >= NORMAL_OUTPUT) pdfOutput = true;
}
...
NonD::NonD(unsigned short method_name, std::shared_ptr<Model> model,
    ...
    : Analyzer(method_name, model, approx_view),
      totalLevelRequests(0),
      cdfFlag(true),
      pdfOutput(false),
      finalMomentsType(Pecos::STANDARD_MOMENTS) {
...
void NonD::requested_levels(const RealVectorArray& req_resp_levels,
  ...
  if (totalLevelRequests && pdf_output) pdfOutput = true;
  ...
  if (num_level_arrays != numFunctions) {
    ...
    // default is no output level calculations if no requested input levels.
      ...
      for (i = 0; i < numFunctions; ++i) {
        ...
        for (j = 0; j < new_len; ++j) vec_i[j] = level_array0[i * new_len + j];
      }
    } else {
    ...
    if (len_i > 1) {
      ...
      if (ascending)
        ...
        std::sort(vec_i, vec_i + len_i,
                  std::greater<Real>());  // descending sort
    }
  }
}

void NonD::construct_lhs(std::shared_ptr<Iterator>& u_space_sampler,
  ...
  u_space_sampler =
      std::make_shared<NonDLHSSampling>(u_model, sample_type, num_samples, seed,
                                        rng, vary_pattern, sampling_vars_mode);
}

void NonD::initialize_response_covariance() {}  // default is no-op
    ...
    for (i = 0; i < numFunctions; ++i) {
      ...
      for (j = 0; j < num_levels; ++j, ++cntr) {
        switch (respLevelTarget) {
          case PROBABILITIES:
            std::sprintf(lev_tag, "_plev%zu", j + 1);
            break;
          case RELIABILITIES:
            std::sprintf(lev_tag, "_blev%zu", j + 1);
            break;
          case GEN_RELIABILITIES:
            std::sprintf(lev_tag, "_b*lev%zu", j + 1);
            break;
        }
...
void NonD::configure_1d_sequence(size_t& num_steps, size_t& secondary_index,
                                 short& seq_type) {
  ...
  if (ml || iteratedModel->multilevel_multifidelity()) {
    ...
    num_steps = num_hf_lev;
    secondary_index = num_mf - 1;
    if (ml && num_mf > 1)
...
void NonD::configure_2d_sequence(size_t& total_steps, size_t& secondary_index,
                                 short& seq_type) {
  if (iteratedModel->multilevel_multifidelity()) {
    ...
    size_t num_mf = sub_models.size(),
           ...
           num_cv_lev =
               (num_mf > 1)
                   ? std::min(num_hf_lev, sub_models.front()->solution_levels())
                   : 0;
...
void NonD::configure_enumeration(size_t& num_combinations, short& seq_type) {
  seq_type = Pecos::FORM_RESOLUTION_ENUMERATION;

  // Enumerate both model forms and discretization levels
  num_combinations = 0;
  ModelList& sub_models =
      iteratedModel->subordinate_models(false);  // includes HF
...
short NonD::query_cost(size_t num_steps, short seq_type, RealVector& seq_cost,
  ...
  switch (seq_type) {
    ...
    case Pecos::RESOLUTION_LEVEL_1D_SEQUENCE: {  // ML
      ...
      if (model_cost_spec[m])
        user_spec = true;
      else if (cost_md_indices[m].first != SZ_MAX)
        md_recover = true;
      break;
    }
    case Pecos::MODEL_FORM_1D_SEQUENCE: {  // MF
      ...
      for (m = 0, m_iter = sub_models.begin(); m < num_mf; ++m, ++m_iter) {
        seq_cost[m] =
            (*m_iter)->solution_level_cost();  // act soln index; 0 if !fnd
    ...
    case Pecos::FORM_RESOLUTION_2D_SEQUENCE: {  // MLMF
      ...
      auto hf_model = sub_models.back();
      // Cost specifications are optional; match to number of soln control
      // values
      RealVector lf_costs = sub_models.front()->solution_level_costs(),
                 hf_costs = sub_models.back()->solution_level_costs();
      m = 0;
      model_cost_spec[m] = valid_costs(lf_costs);
      if (model_cost_spec[m])
        user_spec = true;
      else if (cost_md_indices[m].first != SZ_MAX)
        md_recover = true;
      m = num_mf - 1;
      model_cost_spec[m] = valid_costs(hf_costs);
      if (model_cost_spec[m])
        user_spec = true;
      else if (cost_md_indices[m].first != SZ_MAX)
        md_recover = true;
    ...
    case Pecos::FORM_RESOLUTION_ENUMERATION: {
      ModelLIter m_iter;
      size_t cntr;
      for (m_iter = sub_models.begin(), cntr = 0; m_iter != sub_models.end();
           ++m_iter)
      ...
      for (m = 0, cntr = 0, m_iter = sub_models.begin();
           m_iter != sub_models.end(); ++m, ++m_iter) {
  ...
  if (user_spec && md_recover)
    return MIXED_COST_SPEC_RECOVERY;
  else if (user_spec)
    return USER_COST_SPEC;
  else if (md_recover)
    return ONLINE_COST_RECOVERY;
  else
    return NO_COST_SOURCE;
}

void NonD::test_cost(short seq_type, const BitArray& model_cost_spec,
...
void NonD::average_online_cost(const RealVector& accum_cost,
  ...
  for (step = 0; step < num_steps; ++step) {
    ...
    if (costMetadataIndices[mf].first != SZ_MAX)
      seq_cost[step] =
          (num_cost[step]) ? accum_cost[step] / num_cost[step] : 0.;
...
void NonD::load_pilot_sample(const SizetArray& pilot_spec, size_t num_groups,
...
void NonD::load_pilot_sample(const SizetArray& pilot_spec, short seq_type,
  ...
  if (pilot_size <= 1) {
    ...
    for (i = 0; i < num_mf; ++i) delta_N_l[i].assign(N_l[i].size(), num_samp);
  } else {
    ...
    for (i = 0; i < num_mf; ++i) {
      ...
      num_total_lev += num_lev;
      num_prev_lev = num_lev;
    }

    if (same_lev && pilot_size == num_lev)
      for (j = 0; j < num_lev; ++j) {
        num_samp = pilot_spec[j];
        for (i = 0; i < num_mf; ++i) delta_N_l[i][j] = num_samp;
      }
    else if (pilot_size == num_total_lev) {
      size_t cntr = 0;
      for (i = 0; i < num_mf; ++i) {
        SizetArray& delta_N_li = delta_N_l[i];
        num_lev = delta_N_li.size();
        for (j = 0; j < num_lev; ++j, ++cntr) delta_N_li[j] = pilot_spec[cntr];
      }
    } else {
      Cerr << "Error: inconsistent pilot sample size (" << pilot_size
...
void NonD::resize_final_statistics_gradients() {
  ...
  for (i = 0; i < num_final_stats; i++)
    ...
    {
      final_grad_flag = true;
      break;
    }
  finalStatistics.reshape(num_final_stats, final_dvv.size(), final_grad_flag,
                          false);  // no final Hessians
}
  ...
  for (j = 0; j < rl_len; ++j, ++cntr) {
    ...
    switch (respLevelTargetReduce) {
      ...
      case SYSTEM_PARALLEL:  // system p_fail = product of component p_fail
        for (i = 0; i < numFunctions; ++i) system_p *= computedProbLevels[i][j];
        break;
    }
#ifdef DEBUG
    ...
    switch (respLevelTarget) {
      case PROBABILITIES:
        finalStatistics.function_value(system_p, cntr);
        break;
      case RELIABILITIES:
      case GEN_RELIABILITIES:
    ...
    if (final_asv[cntr] & 2) {
      ...
      for (s = 0; s < numFunctions; ++s) {
        ...
        index += rl_len + requestedProbLevels[s].length() +
                 requestedRelLevels[s].length() +
                 requestedGenRelLevels[s].length();
#ifdef DEBUG
      ...
      for (v = 0; v < num_deriv_vars; ++v) {
        ...
        Real& sum = final_stat_grad[v];
        sum = 0.;
        for (s = 0; s < numFunctions; ++s) {
          prod = 1.;
          switch (respLevelTargetReduce) {
            case SYSTEM_SERIES:  // system p_success = prod of component
                                 // p_success
              for (p = 0; p < numFunctions; ++p)
                prod *= (p == s) ? -component_grad[p][v]
                                 : 1. - computedProbLevels[p][l];
              break;
            case SYSTEM_PARALLEL:  // system p_fail = product of component
                                   // p_fail
              for (p = 0; p < numFunctions; ++p)
                prod *=
                    (p == s) ? component_grad[p][v] : computedProbLevels[p][l];
              break;
          }
          sum += prod;
        }
...
void NonD::initialize_level_mappings() {
  ...
  if (computedRespLevels.empty() || computedProbLevels.empty() ||
    ...
    for (size_t i = 0; i < numFunctions; ++i) {
      switch (respLevelTarget) {
        case PROBABILITIES:
          computedProbLevels[i].resize(requestedRespLevels[i].length());
          break;
        case RELIABILITIES:
          computedRelLevels[i].resize(requestedRespLevels[i].length());
          break;
        case GEN_RELIABILITIES:
          computedGenRelLevels[i].resize(requestedRespLevels[i].length());
          break;
      }
...
void NonD::compute_densities(const RealRealPairArray& min_max_fns,
  ...
  for (i = 0; i < numFunctions; ++i) {
    ...
    cdf_map.clear();
    min = min_max_fns[i].first;
    max = min_max_fns[i].second;
    if (all_levels_computed) {
      ...
      if (!prob_refinement || !rl_len) {  // don't combine refined/unrefined
        ...
        for (j = rl_len; j < total_len; ++j) {
          z = comp_rlev_i[j];
          // if (z >= min && z <= max) // exclude any extrapolations outside
          // bnds
          cdf_map[z] = (cdfFlag) ? comp_plev_i[j] : 1. - comp_plev_i[j];
        }
      }
      ...
      switch (respLevelTarget) {
        case PROBABILITIES:
          ...
          break;
        // case RELIABILITIES: // exclude reliability level mappings from
        // cdf_map
        case GEN_RELIABILITIES:
          for (j = 0; j < rl_len; ++j) {
            z = req_rlev_i[j];  // request may be outside extreme values
    ...
    if (!cdf_map.empty()) {
      ...
      prev_r = it->first;
      it_last = --cdf_map.end();
      last_r = it_last->first;
      // compute computedPDF{Abscissas,Ordinates} from bin counts and widths
      core_pdf_bins = cdf_map.size() - 1;
      pdf_size = core_pdf_bins;
      if (min < prev_r) ++pdf_size;
      if (max > last_r) ++pdf_size;
      RealVector& abs_i = computedPDFAbscissas[i];
      abs_i.resize(pdf_size + 1);
      RealVector& ord_i = computedPDFOrdinates[i];
      ord_i.resize(pdf_size);
      if (min < prev_r) {  // first bin accumulates p0 over [min,lev0]
        offset = 1;
        prev_p = it->second;
        abs_i[0] = min;
        ord_i[0] = prev_p / (prev_r - min);
...
void NonD::print_level_mappings(std::ostream& s, String qoi_type,
  ...
  for (size_t i = 0; i < numFunctions; ++i)
    if (!requestedRespLevels[i].empty() || !requestedProbLevels[i].empty() ||
      ...
      if (outputLevel >= VERBOSE_OUTPUT) level_mappings_file(i, qoi_labels[i]);
    }
}
...
void NonD::print_level_mappings(std::ostream& s, const RealVector& level_maps,
                                bool moment_offset,
  ...
  size_t i, j, cntr, width = write_precision + 7, w2p2 = 2 * width + 2,
                     w3p4 = 3 * width + 4;
...
void NonD::print_densities(std::ostream& s, String qoi_type,
                           const StringArray& pdf_labels) const {
  if (!pdfOutput) return;

  size_t i, j, wpp7 = write_precision + 7,
               num_qoi = computedPDFOrdinates.size();
  if (num_qoi)
    s << std::scientific << std::setprecision(write_precision)
      << "\nProbability Density Function (PDF) histograms for each " << qoi_type
      << ":\n";
  for (i = 0; i < num_qoi; ++i) {
    ...
    if (pdf_len) {
      s << "PDF for " << pdf_labels[i] << ":\n"
        ...
        << "          ---------          ---------      -------------\n";
      for (j = 0; j < pdf_len; ++j)
        s << "  " << std::setw(wpp7) << abs_i[j] << "  " << std::setw(wpp7)
          << abs_i[j + 1] << "  " << std::setw(wpp7) << ord_i[j] << '\n';
    }
  }
}
...
void NonD::print_level_map(std::ostream& s, size_t fn_index,
  ...
  for (j = 0; j < num_gen_rel_levels;
       j++)  // map from 1 requested to 1 computed
    s << "  " << std::setw(width) << computedRespLevels[fn_index][j + offset]
...
void NonD::print_system_mappings(std::ostream& s) const {
  ...
  if (respLevelTargetReduce && rl_len) {
    ...
    if (cdfFlag)
      s << "Cumulative distribution metrics ";
    else
      s << "Complementary cumulative distribution metrics ";
    if (respLevelTargetReduce == SYSTEM_SERIES)
      s << "for series ";
    ...
    for (i = 0; i < rl_len; ++i, ++cntr) {
      switch (respLevelTarget) {
        case PROBABILITIES:
          ...
          gen_rel = -Pecos::NormalRandomVariable::inverse_std_cdf(prob);
          break;
        default:
          gen_rel = final_stats[cntr];
          prob = Pecos::NormalRandomVariable::std_cdf(-gen_rel);
          break;
      }
...
void NonD::print_multilevel_evaluation_summary(std::ostream& s,
                                               const SizetArray& N_m) const {
  size_t j, width = write_precision + 7, num_lev = N_m.size();
  for (j = 0; j < num_lev; ++j)
    s << "                     " << std::setw(width) << N_m[j] << "  QoI_lev"
      << j << '\n';  // QoI counts
}
  ...
  for (j = 0; j < num_lev; ++j) {
    ...
    if (!N_j.empty()) {
      ...
      if (!homogeneous(N_j)) {  // print all counts in this 1D array
        ...
        for (size_t q = 1; q < num_q; ++q) s << ' ' << N_j[q];
      }
      s << "  QoI_lev" << j << '\n';
    }
  }
...
void NonD::print_multilevel_discrepancy_summary(std::ostream& s,
                                                const SizetArray& N_m) const {
...
void NonD::print_multilevel_discrepancy_summary(std::ostream& s,
  ...
  for (j = 0; j < num_lev; ++j) {
    ...
    print_multilevel_row(s, N_mj);
    s << "  DeltaQoI_lev" << j;
    if (j < num_discrep)
      print_multilevel_row(s, N_mj, N_m[j + 1]);
    else
      print_multilevel_row(s, N_mj);
    s << "  QoI_lev" << j << '\n';
  }
...
void NonD::print_multilevel_discrepancy_summary(
    std::ostream& s, const Sizet2DArray& N_m, const Sizet2DArray& N_mp1) const {
  ...
  for (j = 0; j < num_lev; ++j) {  // should be 1 for current use cases
    ...
    print_multilevel_row(s, N_mj);
    s << "  DeltaQoI_lev" << j;
    print_multilevel_row(s, N_mj, N_mp1[j]);
    s << "  QoI_lev" << j << '\n';
  }
}

void NonD::print_multilevel_row(std::ostream& s, const SizetArray& N_j) const {
  ...
  switch (requested_sub_method) {
    ...
    case SUBMETHOD_NPSOL_OPTPP:
      ...
      if (have_npsol && have_optpp)
        ...
        Cerr << "\nError: this executable not configured with OPT++ or NPSOL "
                "for "
    ...
    case SUBMETHOD_DIRECT_NPSOL_OPTPP:
      ...
      if (have_ncsu) {
        if (have_npsol && have_optpp)
          ...
          assigned_sub_method = SUBMETHOD_DIRECT_NPSOL;
        else if (have_optpp)
          assigned_sub_method = SUBMETHOD_DIRECT_OPTPP;
        else
        ...
        case SUBMETHOD_DIRECT_NPSOL:
          if (have_ncsu && have_npsol)
            ...
            if (have_optpp)
              ...
              assigned_sub_method = SUBMETHOD_DIRECT;
          } else if (have_npsol)
            assigned_sub_method = SUBMETHOD_NPSOL;
          else if (have_optpp)
            assigned_sub_method = SUBMETHOD_OPTPP;
...
void NonD::archive_allocate_mappings() {
  ...
  if (req_resp) {
    ...
    resultsDB.array_allocate<RealMatrix>(run_identifier(), data_name,
                                         numFunctions, md);
  }
  if (req_prob) {
    // mapping per function, possibly empty
...
void NonD::archive_to_resp(size_t i, size_t inc_id) {
  ...
  if (num_prob_levels > 0) {
    ...
    resultsDB.array_insert<RealMatrix>(run_identifier(),
    ...
    RealVector result(Teuchos::View, &computedRespLevels[i][0],
                      num_prob_levels);
    resultsDB.insert(run_identifier(), location, result, scale);
  }
  ...
  if (num_rel_levels > 0) {
    ...
    RealVector result(Teuchos::View, &computedRespLevels[i][0] + offset,
                      num_rel_levels);
  ...
  if (num_gen_rel_levels > 0) {
    ...
    resultsDB.array_insert<RealMatrix>(
        run_identifier(), resultsNames.map_genrel_resp, i, mapping);

    scale.emplace(
        0, RealScale("gen_reliability_levels", requestedGenRelLevels[i]));
    RealVector result(Teuchos::View, &computedRespLevels[i][0] + offset,
                      num_gen_rel_levels);
...
{
  if (!resultsDB.active() || !pdfOutput ||
      computedPDFOrdinates[i].length() == 0)
    return;
  ...
  resultsDB.array_insert<RealMatrix>(run_identifier(),
                                     resultsNames.pdf_histograms, i, pdf);
  ...
  location.push_back(labels[i]);
  DimScaleMap scales;
  scales.emplace(0, RealScale("lower_bounds", &computedPDFAbscissas[i][0],
                              pdf_len, ScaleScope::UNSHARED));
  scales.emplace(0, RealScale("upper_bounds", &computedPDFAbscissas[i][1],
                              pdf_len, ScaleScope::UNSHARED));
...
void NonD::archive_equiv_hf_evals(const Real equiv_hf_evals) {
  if (!resultsDB.active()) return;
  resultsDB.add_metadata_to_execution(
      run_identifier(),
