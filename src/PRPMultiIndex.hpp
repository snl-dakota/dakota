...
/** a global function to compare the ActiveSet of a particular database_pr
    ...
    (search_set). */
inline bool set_compare(const ParamResponsePair& database_pr,
...
inline bool id_vars_exact_compare(const ParamResponsePair& database_pr,
  ...
  // fidelity).
  if (search_pr.interface_id() != database_pr.interface_id()) return false;

  // For Boost hashing, need exact binary equality (not tolerance-based)
  if (search_pr.variables() != database_pr.variables()) return false;
...
struct partial_prp_hash {
  ...
  std::size_t operator()(const ParamResponsePair& prp) const {
    return hash_value(prp);
  }  // ONLY interfaceId & Vars used for hash_value
};
...
typedef bmi::multi_index_container<
    ...
    bmi::indexed_by<
        // sorted by increasing evalId/interfaceId value; can be non-unique due
        // to restart database and restarted run having different evals with the
        // same evalId/interfaceId (due to modified execution settings)
        bmi::ordered_non_unique<
            bmi::tag<ordered>,
            bmi::const_mem_fun<
                Dakota::ParamResponsePair, const IntStringPair&,
                &Dakota::ParamResponsePair::eval_interface_ids> >,
        // hashed using partial_prp_hash and compared using
        // partial_prp_equality; can be non-unique due to multiple evals with
        // same interfaceId/variables but distinct active set
...
inline PRPCacheHIter lookup_by_val(PRPMultiIndexCache& prp_cache,
                                   const ParamResponsePair& search_pr) {
  ...
  while (prp_hash_it0 != prp_hash_it1) {
    if (set_compare(*prp_hash_it0, search_pr.active_set())) return prp_hash_it0;
    ++prp_hash_it0;
  }
  return prp_cache.get<hashed>().end();
}
...
inline PRPCacheOIter lookup_by_nearby_val(PRPMultiIndexCache& prp_cache,
                                          const String& search_interface_id,
                                          const Variables& search_vars,
                                          const ActiveSet& search_set,
                                          Real tol) {
...
inline PRPCacheOIter lookup_by_ids(PRPMultiIndexCache& prp_cache,
                                   const IntStringPair& search_ids) {
  ...
  if (search_ids.first >
      0)  // positive ids (evals from current exec) are unique
    return prp_cache.get<ordered>().find(search_ids);
  else {  // negative (restart) and 0 (file import) ids are non-unique in
          // general
    ...
    */
    ...
    abort_handler(-1);
    return prp_cache.get<ordered>().end();
  }
}
...
/** Lookup occurs in two steps: (1) PRPMultiIndexQueue lookup based on
    strict equality in interface id and variables, and (2) set_compare()
    post-processing based on ActiveSet subset logic. */
inline PRPQueueHIter lookup_by_val(PRPMultiIndexQueue& prp_queue,
                                   const ParamResponsePair& search_pr) {
...
inline PRPQueueOIter lookup_by_eval_id(PRPMultiIndexQueue& prp_queue,
                                       int search_id) {
