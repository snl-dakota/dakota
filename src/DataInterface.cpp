...
DataInterfaceRep::DataInterfaceRep()
    : interfaceType(DEFAULT_INTERFACE),
      allowExistingResultsFlag(false),
      verbatimFlag(false),
      parametersFileFormat(PARAMETERS_FILE_STANDARD),
      resultsFileFormat(RESULTS_FILE_STANDARD),
      dakotaResultsFileLabeled(false),
      fileTagFlag(false),
      fileSaveFlag(false),
      batchEvalFlag(false),
      asynchFlag(false),
      asynchLocalEvalConcurrency(0),
      asynchLocalEvalScheduling(DEFAULT_SCHEDULING),
      asynchLocalAnalysisConcurrency(0),
      evalServers(0),
      evalScheduling(DEFAULT_SCHEDULING),
      procsPerEval(0),
      analysisServers(0),
      analysisScheduling(DEFAULT_SCHEDULING),
      procsPerAnalysis(0),
      failAction("abort"),
      retryLimit(1),
      activeSetVectorFlag(true),
      evalCacheFlag(true),
      nearbyEvalCacheFlag(false),
...
void DataInterfaceRep::write(MPIPackBuffer& s) const {
  s << idInterface << interfaceType << algebraicMappings << analysisDrivers
    ...
    << parametersFileFormat << resultsFileFormat << dakotaResultsFileLabeled
    ...
    << useWorkdir << workDir << dirTag << dirSave << linkFiles << copyFiles
    << templateReplace << pluginLibraryPath << numpyFlag;
}
...
DataInterface::DataInterface()
    : dataIfaceRep(new DataInterfaceRep()) { /* empty ctor */ }

DataInterface::DataInterface(const DataInterface& data_resp)
    : dataIfaceRep(data_resp.dataIfaceRep) { /* empty ctor */ }
...
DataInterface::~DataInterface() { /* empty dtor */ }
