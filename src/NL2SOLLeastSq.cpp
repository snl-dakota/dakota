...
NL2SOLLeastSq::NL2SOLLeastSq(ProblemDescDB &problem_db,
    ...
    : LeastSq(problem_db, parallel_lib, model,
      ...
      // output controls
      auxprt(31),
      outlev(1),  // normal/verbose/debug
      // finite differencing
      dltfdj(0.),
      delta0(0.),
      dltfdc(0.),
      // max limits
      mxfcal(maxFunctionEvals),
...
NL2SOLLeastSq::NL2SOLLeastSq(std::shared_ptr<Model> model)
    : LeastSq(NL2SOL, model,
              std::shared_ptr<TraitsBase>(new NL2SOLLeastSqTraits())),
      // output controls
      auxprt(31),
      outlev(1),  // normal/verbose/debug
...
struct Nl2Misc {
  ...
  int newR;      ///< set to 1 if on next call we should check on updating the
                 ///< current view
...
void NL2SOLLeastSq::calcr(int *np, int *pp, Real *x, int *nfp, Real *r, int *ui,
                          void *ur, Vf vf) {
  using std::isfinite;
  ...
  if (nfc != nf) {
    ...
    rc = R->r;
    xc = R->x;
    for (i = 0; i < p; i++) xc[i] = x[i];
    for (i = 0; i < n; i++) r[i] = rc[i] = lf[i];
    if (spec) {
      J = R->J;
      ...
      for (i = 0; i < n; i++) {
        Gradi = lg[i];
        for (j = 0, k = i; j < p; j++, k += n) J[k] = Gradi[j];
      }
    }
  } else
    for (i = 0; i < n; i++) r[i] = lf[i];
}

void NL2SOLLeastSq::calcj(int *np, int *pp, Real *x, int *nfp, Real *J, int *ui,
                          void *ur, Vf vf) {
  using std::isfinite;
...
void NL2SOLLeastSq::core_run() {
  ...
  q.newR = 0;
  q.specgrad =
      (speculativeFlag || iteratedModel->gradient_type() == "analytic") ? 2 : 0;
  if (vendorNumericalGradFlag) q.specgrad = 0;
  ...
  x = (Real *)malloc(L = (lv + 5 * p + 4 * n + lb + 4 * Jlen) * sizeof(Real) +
                         liv * sizeof(int));
  if (!x) {
  ...
  if (fprec > macheps && fprec < 1) {
    ...
    if (vendorNumericalGradFlag) v[v_dltfdj] = t;
    if (i) v[v_delta0] = t;
    t = std::pow(fprec, 1. / 3.);
    if (i) v[v_dltfdc] = t;
    t *= t;
    if (t > 1e-10) v[v_rfctol] = t;
  ...
  if (afctol > 0) v[v_afctol] = afctol;
  if (dltfdc > 0 && i) v[v_dltfdc] = dltfdc;
  if (vendorNumericalGradFlag && dltfdj > 0) v[v_dltfdj] = dltfdj;
  if (delta0 > 0 && i) v[v_delta0] = delta0;
  if (lmax0 > 0) v[v_lmax0] = lmax0;
  if (lmaxs > 0) v[v_lmaxs] = lmaxs;
  if (mxfcal > 0) iv[iv_mxfcal] = mxfcal;
  if (mxiter > 0) iv[iv_mxiter] = mxiter;
  if (rfctol > 0) v[v_rfctol] = rfctol;
  if (sctol > 0) v[v_sctol] = sctol;
  if (xctol > 0) v[v_xctol] = xctol;
  if (xftol > 0) v[v_xftol] = xftol;
