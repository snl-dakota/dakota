...
static const char rcsId[] =
    "@(#) $Id: NonDMultifidelitySampling.cpp 7035 2010-10-22 21:45:39Z mseldre "
    "$";
...
NonDMultifidelitySampling::NonDMultifidelitySampling(
    ...
    : NonDNonHierarchSampling(problem_db, parallel_lib, model),
      numericalSolveMode(
          problem_db.get_ushort("method.nond.numerical_solve_mode")) {
  ...
  for (size_t i = 0; i < numApprox; ++i) approxSet[i] = i;
...
void NonDMultifidelitySampling::multifidelity_mc_online_pilot() {
  ...
  while (numSamples && mlmfIter <= maxIterations) {
    ...
    accumulate_mf_sums(sum_L_baseline, sum_H, sum_LL, sum_LH, sum_HH,
                       N_H_actual);
    N_H_alloc +=
        (backfillFailures && mlmfIter)
            ? one_sided_delta(N_H_alloc, mfmcSolnData.solution_reference(),
                              relaxFactor)
            : numSamples;
    ...
    numSamples = one_sided_delta(N_H_actual, mfmcSolnData.solution_reference(),
                                 relaxFactor);
    // numSamples = std::min(num_samp_budget, num_samp_accuracy); // enforce
    // both
    ++mlmfIter;
    advance_relaxation();
  }
  ...
  if (finalStatsType == QOI_STATISTICS) {
    ...
    compute_mf_controls(sum_L_baseline, sum_H, sum_LL, sum_LH, N_H_actual,
                        beta);
...
void NonDMultifidelitySampling::multifidelity_mc_offline_pilot() {
  ...
  RealVector sum_HH_pilot(numFunctions);
  RealMatrix var_L;
  ...
  compute_LH_correlation(sum_L_pilot[1], sum_H_pilot[1], sum_LL_pilot[1],
                         sum_LH_pilot[1], sum_HH_pilot, N_pilot, var_L, varH,
                         rho2LH);
  ...
  numSamples =
      std::max(one_sided_delta(N_H_actual, mfmcSolnData.solution_reference()),
               (size_t)2);  // no relax
  shared_increment("mf_");  // spans ALL models, blocking
...
/** LF only */
void NonDMultifidelitySampling::update_projected_lf_samples(
    const MFSolutionData& soln, const SizetArray& N_H_actual, size_t& N_H_alloc,
    // SizetArray& delta_N_L_actual,
    Real& delta_equiv_hf) {
  Sizet2DArray N_L_actual;
  inflate(N_H_actual, N_L_actual);
  ...
  for (approx = 0; approx < numApprox; ++approx) {
    ...
    actual_incr = (backfillFailures) ? one_sided_delta(N_L_actual_a, lf_target)
                                     : alloc_incr;
...
/** LF and HF */
void NonDMultifidelitySampling::update_projected_samples(
    const MFSolutionData& soln, const SizetArray& N_H_actual, size_t& N_H_alloc,
  ...
  size_t alloc_incr = one_sided_delta(N_H_alloc, hf_target),
         actual_incr = (backfillFailures)
                           ? one_sided_delta(N_H_actual, hf_target)
                           : alloc_incr;
...
void NonDMultifidelitySampling::approx_increments(
    ...
    Sizet2DArray& N_L_actual_refined, SizetArray& N_L_alloc_refined,
  ...
  switch (optSubProblemForm) {
    ...
    case REORDERED_ANALYTIC_SOLUTION:  // rho ordering
      update_model_groups(corrApproxSequence);
      break;
    default:  // numerical MFMC: on the fly reordering for r_i
      // final numerical soln may not be last eval: update final sequence
      ordered_approx_sequence(soln.solution_ratios(), ratioApproxSequence,
                              true);
      update_model_groups(ratioApproxSequence);
      break;
  }
...
void NonDMultifidelitySampling::compute_mf_controls(
    const IntRealMatrixMap& sum_L_covar, const IntRealVectorMap& sum_H_covar,
    const IntRealMatrixMap& sum_LL_covar, const IntRealMatrixMap& sum_LH_covar,
  ...
  for (int mom = 1; mom <= 4; ++mom) {
    ...
    for (qoi = 0; qoi < numFunctions; ++qoi) {
      RealVector& beta_mq = beta_m[qoi];
      beta_mq.sizeUninitialized(numApprox);
      sum_H_mq = sum_H_m[qoi];
      N_base_q = N_covar[qoi];
      for (approx = 0; approx < numApprox; ++approx)
...
void NonDMultifidelitySampling::estimator_variance_ratios(
  ...
  case ANALYTIC_SOLUTION:
    for (qoi=0; qoi<numFunctions; ++qoi) {
      ...
      for (i=0, ip1=1; ip1<numApprox; ++i, ++ip1) {
        r_ip1 = eval_ratios(qoi, ip1); // *** taking credit for QoI tailoring in
estvar R_sq += (r_i - r_ip1) / (r_i * r_ip1) * rho2_LH(qoi, i); r_i = r_ip1;
      }
  ...
  case REORDERED_ANALYTIC_SOLUTION: {
    ...
    for (i=0, ip1=1; ip1<numApprox; ++i, ++ip1) {
      ...
      // Note: monotonicity in reordered r_i is enforced in
compute_allocations()
  ...
  default: {
    ...
    for (qoi=0; qoi<numFunctions; ++qoi) {
      ...
      for (i=0, ip1=1; ip1<numApprox; ++i, ++ip1) {
        approx_ip1 = (ordered) ? ip1 : approx_sequence[ip1];
        r_ip1 = eval_ratios(qoi, approx_ip1); // *** taking credit for QoI
tailored solution in estvar R_sq += (r_i - r_ip1) / (r_i * r_ip1) * rho2_LH(qoi,
approx); r_i = r_ip1;  approx = approx_ip1;
      }
...
void NonDMultifidelitySampling::accumulate_mf_sums(
        ...
        for (approx = 0; approx < numApprox; ++approx) {
          ...
          while (lb_ord || ll_ord || lh_ord) {
            ...
            if (lb_ord == active_ord) {  // support general key sequence
              lb_it->second(qoi, approx) += lf_prod;
              ++lb_it;
              lb_ord = (lb_it == sum_L_baseline.end()) ? 0 : lb_it->first;
            }
            // Low-Low
            if (ll_ord == active_ord) {  // support general key sequence
              ll_it->second(qoi, approx) += lf_prod * lf_prod;
              ++ll_it;
              ll_ord = (ll_it == sum_LL.end()) ? 0 : ll_it->first;
            }
            // Low-High
...
void NonDMultifidelitySampling::accumulate_mf_sums(
    RealMatrix& sum_L_baseline, RealVector& sum_H, RealMatrix& sum_LL,
    RealMatrix& sum_LH, RealVector& sum_HH, SizetArray& N_shared) {
  // uses one set of allResponses with QoI aggregation across all Models,
  // ordered by unorderedModels[i-1], i=1:numApprox --> truthModel
...
void NonDMultifidelitySampling::accumulate_mf_sums(
    ...
    const IntResponseMap& resp_map, const SizetArray& approx_sequence,
        ...
        if (isfinite(fn_val)) {  // neither NaN nor +/-Inf
          ...
          while (ls_ord || lr_ord) {
            ...
            if (ls_ord == active_ord) {  // support general key sequence
              ls_it->second(qoi, approx) += prod;
              ++ls_it;
              ls_ord = (ls_it == sum_L_shared.end()) ? 0 : ls_it->first;
            }
            // Low refined
            if (lr_ord == active_ord) {  // support general key sequence
              lr_it->second(qoi, approx) += prod;
              ++lr_it;
              lr_ord = (lr_it == sum_L_refined.end()) ? 0 : lr_it->first;
            }
            prod *= fn_val;
...
void NonDMultifidelitySampling::compute_LH_correlation(
    const RealMatrix& sum_L_shared, const RealVector& sum_H,
    const RealMatrix& sum_LL, const RealMatrix& sum_LH,
    const RealVector& sum_HH, const SizetArray& N_shared, RealMatrix& var_L,
...
void NonDMultifidelitySampling::compute_allocations(const RealMatrix& rho2_LH,
  ...
  switch (numericalSolveMode) {
    ...
    case NUMERICAL_FALLBACK:  // default
      ...
      // Ordering in avg_eval_ratios is then used for pyramid sampling
      // downstream > ascending order in rho required all QoI for all Approx
      ...
      break;
    case REORDERED_FALLBACK:  // not currently in XML spec (numerical FB
                              // preferred)
      ordered_rho = ordered_approx_sequence(rho2_LH);
      if (ordered_rho) {
        Cout << "MFMC: model sequence provided is ordered in Low-High "
  ...
  switch (optSubProblemForm) {
    case ANALYTIC_SOLUTION:
      ...
      process_analytic_allocations(rho2_LH, var_H, N_H, cost, avg_eval_ratios,
                                   soln);
...
void NonDMultifidelitySampling::mfmc_numerical_solution(
    const RealMatrix& rho2_LH, const RealVector& cost, MFSolutionData& soln) {
  ...
  if (mlmfIter == 0) {
    ...
    bool lower_bounded_r = true,
         monotonic_r = (budget_constrained) ? false : true;
...
void NonDMultifidelitySampling::process_analytic_allocations(
    const RealMatrix& rho2_LH, const RealVector& var_H, const SizetArray& N_H,
    const RealVector& cost, RealVector& avg_eval_ratios, MFSolutionData& soln) {
...
void NonDMultifidelitySampling::mfmc_estimator_variance(
    const RealMatrix& rho2_LH, const RealVector& var_H, const SizetArray& N_H,
    MFSolutionData& soln) {
  ...
  switch (optSubProblemForm) {
    ...
    case REORDERED_ANALYTIC_SOLUTION: {
      ...
      // if (converged) {
      mfmc_estvar_ratios(rho2_LH, soln.solution_ratios(), corrApproxSequence,
                         soln);
      ...
      if (outputLevel >= NORMAL_OUTPUT) {
        ...
        for (size_t qoi = 0; qoi < numFunctions; ++qoi)
          Cout << "QoI " << qoi + 1
               << ": variance reduction factor = " << estvar_ratios[qoi]
               << '\n';
        Cout << std::endl;
      }
      break;
      ...
      // Numerical MFMC: mfmc_numerical_solution() uses
      // process_model_allocations() to store optimized estvar/estvar_ratios in
      // soln.  In this case, there is no emerge_from_pilot() modification so
      // estvar remains consistent, including pilot projections that target this
      // optimized state.
  }
...
void NonDMultifidelitySampling::update_model_groups() {
  ...
  if (modelGroups.size() != numGroups) modelGroups.resize(numGroups);
  for (size_t g = 0; g < numGroups; ++g) mfmc_model_group(g, modelGroups[g]);
}

void NonDMultifidelitySampling::update_model_groups(
    const SizetArray& approx_sequence) {
  if (approx_sequence.empty()) update_model_groups();
...
void NonDMultifidelitySampling::emerge_from_pilot(const SizetArray& N_H,
                                                  RealVector& avg_eval_ratios,
                                                  Real& avg_hf_target,
                                                  Real offline_N_lwr) {
  ...
  if (avg_N_H > avg_hf_target) {  // replace N* and rescale r*
    ...
    bool ordered = ratioApproxSequence.empty();
    unsigned short approx;
    for (int i = numApprox - 1; i >= 0; --i) {
      approx = (ordered) ? i : ratioApproxSequence[i];
      Real& r_i = avg_eval_ratios[approx];
      N_i = r_i * avg_hf_target;
...
void NonDMultifidelitySampling::mfmc_estvar_ratios(
    const RealMatrix& rho2_LH, const RealVector& avg_eval_ratios,
  ...
  Real R_sq, r_i, r_ip1;
  size_t qoi, approx, approx_ip1, ip1;
  ...
  switch (optSubProblemForm) {
    ...
    case REORDERED_ANALYTIC_SOLUTION: {
      RealVector avg_rho2_LH;
      average(rho2_LH, 0, avg_rho2_LH);  // avg over QoI
      approx = (ordered) ? 0 : approx_sequence[0];
      ...
      for (ip1 = 1; ip1 < numApprox; ++ip1) {
        ...
        R_sq +=
            (r_i - r_ip1) / (r_i * r_ip1) * avg_rho2_LH[approx];  // see below
        r_i = r_ip1;
        approx = approx_ip1;
      }
      R_sq += (r_i - 1.) / r_i * avg_rho2_LH[approx];
