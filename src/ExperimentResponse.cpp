...
static const char rcsId[] =
    "@(#) $Id: ExperimentResponse.cpp 7029 2010-10-22 00:17:02Z mseldre $";

namespace Dakota {

ExperimentResponse::ExperimentResponse(const Variables& vars,
...
ExperimentResponse::ExperimentResponse(const SharedResponseData& srd,
                                       const ActiveSet& set)
...
void ExperimentResponse::set_scalar_covariance(RealVector& scalarSigmas) {
  ...
  expDataCovariance.set_covariance_matrices(
      matrices, diagonals, scalarSigmas, matrix_map_indices,
...
void ExperimentResponse::set_full_covariance(std::vector<RealMatrix>& matrices,
  ...
  expDataCovariance.set_covariance_matrices(
      matrices, diagonals, scalars, matrix_map_indices, diagonal_map_indices,
      scalar_map_indices);
  // Might make this depend on a verbosity output level - RWH
  // expDataCovariance.print_cov();
}
...
void ExperimentResponse::apply_covariance_inv_sqrt(
    const RealVector& residuals, RealVector& weighted_residuals) const {
  expDataCovariance.apply_experiment_covariance_inverse_sqrt(
      residuals, weighted_residuals);
}

void ExperimentResponse::apply_covariance_inv_sqrt(
    const RealMatrix& gradients, RealMatrix& weighted_gradients) const {
  expDataCovariance.apply_experiment_covariance_inverse_sqrt_to_gradients(
...
void ExperimentResponse::apply_covariance_inv_sqrt(
  ...
  expDataCovariance.apply_experiment_covariance_inverse_sqrt_to_hessians(
      hessians, weighted_hessians);
}

void ExperimentResponse::get_covariance_diagonal(RealVector& diagonal) const {
