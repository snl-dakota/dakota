...
static const char rcsId[] =
    "@(#) $Id: DakotaConstraints.cpp 7029 2010-10-22 00:17:02Z mseldre $";
...
Constraints::Constraints(BaseConstructor, const ProblemDescDB& problem_db,
                         const SharedVariablesData& svd)
    : sharedVarsData(svd),
      numNonlinearIneqCons(problem_db.get_sizet(
...
/** This constructor is the one which must build the base class data
    ...
    base class constructor calling get_constraints() again).  Since
    the letter IS the representation, its rep pointer is set to NULL. */
Constraints::Constraints(BaseConstructor, const SharedVariablesData& svd)
    : sharedVarsData(svd),
      numNonlinearIneqCons(0),
      numNonlinearEqCons(0),
      numLinearIneqCons(0),
      numLinearEqCons(0) {
  shape_bounds();  // size all*{Lower,Upper}Bnds arrays
  build_views();   // construct active/inactive views of all arrays
...
/** The default constructor: constraintsRep is NULL in this case (a
    ...
    object). */
Constraints::Constraints() { /* empty ctor */ }
...
Constraints::Constraints(const ProblemDescDB& problem_db,
                         const SharedVariablesData& svd)
...
std::shared_ptr<Constraints> Constraints::get_constraints(
    const ProblemDescDB& problem_db, const SharedVariablesData& svd) {
...
Constraints::Constraints(const SharedVariablesData& svd)
  ...
  if (!constraintsRep) abort_handler(CONS_ERROR);
}
...
Constraints::~Constraints() { /* empty dtor */ }
...
void Constraints::build_active_views() {
  ...
  if (num_cv) {
    ...
    continuousLowerBnds =
        RealVector(Teuchos::View, &allContinuousLowerBnds[cv_start], num_cv);
    continuousUpperBnds =
        RealVector(Teuchos::View, &allContinuousUpperBnds[cv_start], num_cv);
  ...
  if (num_div) {
    ...
    discreteIntLowerBnds =
        IntVector(Teuchos::View, &allDiscreteIntLowerBnds[div_start], num_div);
    discreteIntUpperBnds =
        IntVector(Teuchos::View, &allDiscreteIntUpperBnds[div_start], num_div);
  } else  // return to default empty initialization
    discreteIntLowerBnds = discreteIntUpperBnds = IntVector();
  if (num_drv) {
    size_t drv_start = sharedVarsData.drv_start();
    discreteRealLowerBnds = RealVector(
        Teuchos::View, &allDiscreteRealLowerBnds[drv_start], num_drv);
    discreteRealUpperBnds = RealVector(
        Teuchos::View, &allDiscreteRealUpperBnds[drv_start], num_drv);
  } else  // return to default empty initialization
...
void Constraints::build_inactive_views() {
  ...
  if (sharedVarsData.view().second == MIXED_ALL ||
    ...
    Cerr << "Error: inactive view cannot be ALL in VarConstraints."
         << std::endl;
    abort_handler(CONS_ERROR);
  }
  ...
  if (num_icv) {
    ...
    inactiveContinuousLowerBnds =
        RealVector(Teuchos::View, &allContinuousLowerBnds[icv_start], num_icv);
    inactiveContinuousUpperBnds =
        RealVector(Teuchos::View, &allContinuousUpperBnds[icv_start], num_icv);
  } else  // return to default empty initialization
    inactiveContinuousLowerBnds = inactiveContinuousUpperBnds = RealVector();
  if (num_idiv) {
    size_t idiv_start = sharedVarsData.idiv_start();
    inactiveDiscreteIntLowerBnds = IntVector(
        Teuchos::View, &allDiscreteIntLowerBnds[idiv_start], num_idiv);
    inactiveDiscreteIntUpperBnds = IntVector(
        Teuchos::View, &allDiscreteIntUpperBnds[idiv_start], num_idiv);
  } else  // return to default empty initialization
    inactiveDiscreteIntLowerBnds = inactiveDiscreteIntUpperBnds = IntVector();
  if (num_idrv) {
    size_t idrv_start = sharedVarsData.idrv_start();
    inactiveDiscreteRealLowerBnds = RealVector(
        Teuchos::View, &allDiscreteRealLowerBnds[idrv_start], num_idrv);
    inactiveDiscreteRealUpperBnds = RealVector(
        Teuchos::View, &allDiscreteRealUpperBnds[idrv_start], num_idrv);
  } else  // return to default empty initialization
        ...
        const SharedVariablesData& svd)
{
  if (constraintsRep) // envelope
    ...
    reshape_linear(num_lin_ineq_cons, num_lin_eq_cons); // simple resize, no
mapping
  }
}
*/

void Constraints::reshape(size_t num_nln_ineq_cons, size_t num_nln_eq_cons,
                          const SharedVariablesData& svd) {
  if (constraintsRep)  // envelope
...
void Constraints::reshape_nonlinear(size_t num_nln_ineq_cons,
...
/** Resizes the linear and nonlinear constraint arrays at the base
    class.  Does NOT currently resize the derived bounds arrays. */
void Constraints::reshape_linear(size_t num_lin_ineq_cons,
...
/** Resizes the linear coefficient arrays based on change in active
    variables view. */
void Constraints::reshape_update_linear(const SharedVariablesData& prev_svd,
    ...
    if ((prev_active_view == RELAXED_ALL || prev_active_view == MIXED_ALL) &&
      ...
      for (col = 0; col < num_drv; ++col, ++curr_cntr, ++prev_cntr) {
        ...
        copy_data(prev_lin_eq_coeffs[prev_cntr], linearEqConCoeffs[curr_cntr],
               ...
               prev_active_view >=
                   RELAXED_DESIGN) {  // inflate (pad with zeros)
      prev_cntr = 0;
      curr_cntr = prev_svd.cv_start();
      for (col = 0; col < num_cv; ++col, ++curr_cntr, ++prev_cntr) {
        copy_data(prev_lin_ineq_coeffs[prev_cntr],
                  linearIneqConCoeffs[curr_cntr], numLinearIneqCons);
        copy_data(prev_lin_eq_coeffs[prev_cntr], linearEqConCoeffs[curr_cntr],
...
void Constraints::manage_linear_constraints(const ProblemDescDB& problem_db) {
  ...
  size_t num_vars =
      sharedVarsData.cv() + sharedVarsData.div() + sharedVarsData.drv();
