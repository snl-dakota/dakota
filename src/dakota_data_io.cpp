...
void read_sized_data(std::istream& s, RealVectorArray& va, size_t num_rows,
                     int num_cols) {
  if (va.size() != num_rows) va.resize(num_rows);

  for (size_t i = 0; i < num_rows; ++i) {
    RealVector& v = va[i];
    if (v.length() != num_cols) v.sizeUninitialized(num_cols);
...
void read_fixed_rowsize_data(std::istream& s, RealVectorArray& va, int num_cols,
                             bool row_major) {
  if (!va.empty()) va.clear();
  ...
  if (row_major)
    ...
    for (int i = 0; i < num_cols; ++i) {
      ...
      for (int j = 0; j < num_rows; ++j) v[j] = working_va[j][i];
      va[i] = v;
    }
  }
}
...
void read_unsized_data(std::istream& s, RealVectorArray& va, bool row_major) {
  ...
  int num_tokens = 0;
  for (const std::string& t : tokens) num_tokens++;
...
// This version uses a single configuration file adhering to an expected
// num_expts X ncv format
...
void read_coord_values(const std::string& basename, int expt_num,
  ...
  std::ifstream s;
  std::string filename =
      basename + "." + convert_to_string(expt_num) + ".coords";
...
void read_covariance(const std::string& basename, int expt_num,
  ...
  std::ifstream s;
  std::string filename =
      basename + "." + convert_to_string(expt_num) + ".sigma";
...
int count_rows(String file_name) {
  ...
  String line;
  while (std::getline(file, line)) ++nbOfLines;
  return nbOfLines;
}
