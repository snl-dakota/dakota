...
static const char rcsId[] =
    "@(#) $Id: NonDIntegration.cpp,v 1.57 2004/06/21 19:57:32 mseldre Exp $";
...
NonDIntegration::NonDIntegration(ProblemDescDB& problem_db,
...
NonDIntegration::NonDIntegration(unsigned short method_name,
                                 std::shared_ptr<Model> model)
    : NonD(method_name, model),
      numIntegrations(0)  //, standAloneMode(false)
{
...
void NonDIntegration::print_points_weights(const String& tabular_name) {
  ...
  if (num_pts && num_vars) {
    ...
    if (weights) pts_wts_file << std::setw(write_precision + 6) << "weight ";
...
void NonDIntegration::increment_grid_preference(const RealVector& dim_pref) {
  ...
  for (i = 0; i < num_pref; ++i) aniso_wts[i] = 1. / dim_pref[i];
...
void NonDIntegration::increment_grid_weights(const RealVector& aniso_wts) {
  ...
  for (i = 0; i < num_wts; ++i) dim_pref[i] = 1. / aniso_wts[i];
