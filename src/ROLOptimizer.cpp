...
void ROLOptimizer::core_run() {
  ...
  // Should we attempt to remove this call from the constructor to avoid
  // redundancy? - RWH
  set_problem();
...
void ROLOptimizer::set_problem() {
  ...
  if ((num_ineq_const > 0) || ((num_eq_const > 0) && (boundConstraintFlag)))
    problemType = TYPE_EB;
  else {
  ...
  if ((problemType == TYPE_B) || (problemType == TYPE_EB)) {
    Teuchos::RCP<std::vector<Real> > l_rcp(
        new std::vector<Real>(numContinuousVars, 0.0));
    Teuchos::RCP<std::vector<Real> > u_rcp(
        new std::vector<Real>(numContinuousVars, 0.0));
    ...
    for (size_t i = 0; i < numContinuousVars; i++) {
      if ((*l_rcp)[i] < rol_ninf) (*l_rcp)[i] = rol_ninf;
      if ((*u_rcp)[i] > rol_inf) (*u_rcp)[i] = rol_inf;
    }
  ...
  if (num_ineq_const > 0) {
    ...
    Teuchos::RCP<std::vector<Real> > ineq_l_rcp(
        new std::vector<Real>(num_ineq_const, 0.0));
    Teuchos::RCP<std::vector<Real> > ineq_u_rcp(
        new std::vector<Real>(num_ineq_const, 0.0));
    ...
    for (size_t i = 0; i < num_ineq_const; i++) {
      if ((*ineq_l_rcp)[i] < rol_ninf) (*ineq_l_rcp)[i] = rol_ninf;
      if ((*ineq_u_rcp)[i] > rol_inf) (*ineq_u_rcp)[i] = rol_inf;
    }

    // Set the ROL::BoundConstraint object.
    Teuchos::RCP<ROL::Vector<Real> > ineq_lower_bounds(
        new ROL::StdVector<Real>(ineq_l_rcp));
    Teuchos::RCP<ROL::Vector<Real> > ineq_upper_bounds(
        new ROL::StdVector<Real>(ineq_u_rcp));
...
void ROLOptimizer::set_rol_parameters() {
  ...
  if (iteratedModel->hessian_type() == "none") {
    optSolverParams.sublist("General").sublist("Secant").set(
        "Type", "Limited-Memory BFGS");
    optSolverParams.sublist("General").sublist("Secant").set("Use as Hessian",
                                                             true);
  }
  ...
  if (problemType == TYPE_U) {
    ...
    optSolverParams.sublist("Step")
        .sublist("Trust Region")
  ...
  // Set the verbosity level.
  optSolverParams.sublist("General").set("Print Verbosity",
                                         outputLevel < VERBOSE_OUTPUT ? 0 : 1);
...
void update_model(Model &model, const std::vector<Real> &x) {
  ...
  size_t num_cv = ModelUtils::cv(model);
  for (size_t i = 0; i < num_cv; ++i)
    ModelUtils::continuous_variable(model, x[i], i);
  ...
  if (is_seq_dup) model.auto_graphics(orig_auto_graphics_flag);
...
// Constructor.
DakotaROLObjective::DakotaROLObjective(Model &model) : dakotaModel(model) {}

// Compute objective value and return to ROL.
Real DakotaROLObjective::value(const std::vector<Real> &x, Real &tol) {
...
void DakotaROLObjectiveGrad::gradient(std::vector<Real> &g,
...
void DakotaROLObjectiveHess::hessVec(std::vector<Real> &hv,
                                     const std::vector<Real> &v,
                                     const std::vector<Real> &x, Real &tol) {
  // Make sure the Hessian has been evaluated and get it from Dakota
  // Response.
  update_model(dakotaModel, x);
  const RealSymMatrix &hess_f =
      dakotaModel.current_response().function_hessian(0);
...
void DakotaROLObjectiveHess::invHessVec(std::vector<Real> &h,
                                        const std::vector<Real> &v,
                                        const std::vector<Real> &x, Real &tol) {
  Cerr << "\nError: DakotaROLObjectiveHess::invHessVec is not currently "
          "supported."
       << "  logic error.\n";
  abort_handler(METHOD_ERROR);
...
void DakotaROLIneqConstraints::value(std::vector<Real> &c,
                                     const std::vector<Real> &x, Real &tol) {
  // Evaluate nonlinear constraints.
  update_model(dakotaModel, x);

  // Matrix-vector multiply to get linear constraint values.
  apply_linear_constraints(dakotaModel, CONSTRAINT_EQUALITY_TYPE::INEQUALITY, x,
                           c);
...
// to ROL.
void DakotaROLIneqConstraintsGrad::applyJacobian(std::vector<Real> &jv,
                                                 const std::vector<Real> &v,
                                                 const std::vector<Real> &x,
                                                 Real &tol) {
  // Matrix-vector multiply to apply linear constraint Jacobian.
  const RealMatrix &lin_ineq_coeffs =
      ModelUtils::linear_ineq_constraint_coeffs(dakotaModel);
  apply_matrix_partial(lin_ineq_coeffs, v, jv);

  // Apply nonlinear constraint Jacobian.
  if (haveNlnConst) {
    update_model(dakotaModel, x);
    apply_nonlinear_constraints(dakotaModel,
                                CONSTRAINT_EQUALITY_TYPE::INEQUALITY, v, jv);
  }
...
void DakotaROLIneqConstraintsGrad::applyAdjointJacobian(
  ...
  if (haveNlnConst) {
    ...
    apply_nonlinear_constraints(
        dakotaModel, CONSTRAINT_EQUALITY_TYPE::INEQUALITY, v, ajv, true);
  }
...
void DakotaROLIneqConstraintsHess::applyAdjointHessian(
    std::vector<Real> &ahuv, const std::vector<Real> &u,
  ...
  if (haveNlnConst) {
    ...
    for (size_t i = 1;
         i < ModelUtils::num_nonlinear_ineq_constraints(dakotaModel); ++i) {
      RealSymMatrix temp_hu(
          dakotaModel.current_response().function_hessian(1 + i));
      temp_hu *= u[i];
      hu += temp_hu;
    }
...
void DakotaROLEqConstraints::value(std::vector<Real> &c,
                                   const std::vector<Real> &x, Real &tol) {
  // Evaluate nonlinear constraints.
  update_model(dakotaModel, x);

  // Matrix-vector multiply to get linear constraint values.
  apply_linear_constraints(dakotaModel, CONSTRAINT_EQUALITY_TYPE::EQUALITY, x,
                           c);
...
// to ROL.
void DakotaROLEqConstraintsGrad::applyJacobian(std::vector<Real> &jv,
                                               const std::vector<Real> &v,
                                               const std::vector<Real> &x,
                                               Real &tol) {
  // Matrix-vector multiply to apply linear constraint Jacobian.
  const RealMatrix &lin_eq_coeffs =
      ModelUtils::linear_eq_constraint_coeffs(dakotaModel);
  apply_matrix_partial(lin_eq_coeffs, v, jv);

  // Apply nonlinear constraint Jacobian.
  if (haveNlnConst) {
    update_model(dakotaModel, x);
    apply_nonlinear_constraints(dakotaModel, CONSTRAINT_EQUALITY_TYPE::EQUALITY,
                                v, jv);
  }
...
// Jacobian and return to ROL.
void DakotaROLEqConstraintsGrad::applyAdjointJacobian(
    std::vector<Real> &ajv, const std::vector<Real> &v,
    const std::vector<Real> &x, Real &tol) {
  // Must init since are merging the effect of linear and nonlinear
...
// Multiply a vector by the transpose of the inequality constraint
// Hessian and return to ROL.
void DakotaROLEqConstraintsHess::applyAdjointHessian(std::vector<Real> &ahuv,
  ...
  // apply nonlinear constraint Hessian (might be empty)
  if (haveNlnConst) {
    size_t num_nln_ineq_constraints =
        ModelUtils::num_nonlinear_ineq_constraints(dakotaModel);
    // make sure that model is current
    update_model(dakotaModel, x);

    RealSymMatrix hu(dakotaModel.current_response().function_hessian(
        1 + num_nln_ineq_constraints));
    hu *= u[0];

    for (size_t i = 1;
         i < ModelUtils::num_nonlinear_eq_constraints(dakotaModel); ++i) {
      RealSymMatrix temp_hu(dakotaModel.current_response().function_hessian(
          1 + num_nln_ineq_constraints + i));
      temp_hu *= u[i];
      hu += temp_hu;
    }
