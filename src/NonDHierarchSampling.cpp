...
ensemble_sample_batch(const String& prepend, int batch_id, bool new_samples)
{
  if (new_samples) {
    ...
    if (exportSampleSets) {
      if (pIteratedModel->active_truth_key())
	export_all_samples(prepend, pIteratedModel->active_truth_model(),
			   mlmfIter, batch_id);
      size_t i, num_active_surr = pIteratedModel->active_surrogate_keys();
  ...
  evaluate_batch(*pIteratedModel, batch_id); // excludes synchronize
}
