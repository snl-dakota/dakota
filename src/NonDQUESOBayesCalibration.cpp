...
NonDQUESOBayesCalibration::NonDQUESOBayesCalibration(
    ...
    : NonDBayesCalibration(problem_db, parallel_lib, model),
      ...
      propCovUpdatePeriod(
          probDescDB.get_int("method.nond.prop_cov_update_period")),
      ...
      advancedOptionsFile(
          probDescDB.get_string("method.advanced_options_file")) {
  ...
  if (proposalCovarType.empty()) {
    if (emulatorType)
      proposalCovarType = "derivatives";  // misfit Hessian
    else
      proposalCovarType = "prior";  // prior covariance
  }
  ...
  if (propCovUpdatePeriod < std::numeric_limits<int>::max() &&
    ...
    Cout << "\nWarning: QUESO proposal covariance update_period >= "
            "chain_samples;"
  ...
  if (found_error) abort_handler(METHOD_ERROR);
...
void NonDQUESOBayesCalibration::run_chain() {
  ...
  if (proposalCovarType == "derivatives") precondition_proposal(0);
...
void NonDQUESOBayesCalibration::init_queso_environment() {
  ...
  if (parallelLib.mpirun_flag()) {
    ...
    if (mcmcType == "multilevel")
      quesoEnv =
          std::make_shared<QUESO::FullEnvironment>("ml.inp", "", nullptr);
    else  // dram, dr, am, or mh
      quesoEnv = std::make_shared<QUESO::FullEnvironment>(
  ...
  if (mcmcType == "multilevel")
    ...
    quesoEnv = std::make_shared<QUESO::FullEnvironment>(aof_cstr, "",
...
void NonDQUESOBayesCalibration::init_precond_request_value() {
  ...
  switch (emulatorType) {
    ...
    case KRIGING_EMULATOR:
      precondRequestValue = 7;
      break;  // emulator Hessian support
    case SC_EMULATOR:
    case MF_SC_EMULATOR:
    case GP_EMULATOR:
      precondRequestValue = 2;
      break;  // emulator Hessians not supported yet
    case NO_EMULATOR:
...
void NonDQUESOBayesCalibration::init_bayesian_solver() {
  ...
  if (proposalCovarType == "derivatives") init_precond_request_value();
...
void NonDQUESOBayesCalibration::specify_posterior() {
  ...
  postRv = std::make_shared<
      QUESO::GenericVectorRV<QUESO::GslVector, QUESO::GslMatrix>>("post_",
                                                                  *paramSpace);
...
void NonDQUESOBayesCalibration::precondition_proposal(
  ...
  */
  ...
  for (i = 0; i < nv; ++i)
    for (j = 0; j < nv; ++j) (*proposalCovMatrix)(i, j) = prop_covar(i, j);
}
...
void NonDQUESOBayesCalibration::run_queso_solver() {
  ...
  if (mcmcType == "multilevel")
    ...
    inverseProb->solveWithBayesMetropolisHastings(
        calIpMhOptionsValues.get(), *paramInitials, proposalCovMatrix.get());
...
void NonDQUESOBayesCalibration::filter_chain_by_conditioning() {
  ...
  if (outputLevel >= NORMAL_OUTPUT)
    Cout << "Extracting unique samples from MCMC chain containing " << num_mcmc
         << " samples.\n";

  // BMA TODO: determine if the conditioning alg can handle duplicates
  // Note this isn't an array of unique samples, rather has no consecutive
  // repeats May not be needed any longer...
  ...
  RealVector empty_rv;
  mcmc_chain.getPositionValues(0,
                               prev_q_sample);  // extract vector from sequence
...
void NonDQUESOBayesCalibration::specify_prior() {
  ...
  paramSpace =
      std::make_shared<QUESO::VectorSpace<QUESO::GslVector, QUESO::GslMatrix>>(
  ...
  paramDomain =
      std::make_shared<QUESO::BoxSubset<QUESO::GslVector, QUESO::GslMatrix>>(
...
void NonDQUESOBayesCalibration::user_proposal_covariance(
  ...
  /*if( (proposalCovMatrix->numRowsLocal()  != numContinuousVars) ||
    ...
    throw std::runtime_error("Queso vector space is not consistent with
    parameter dimension.");
  */
    ...
    if (use_file) {
      ...
      if (values_from_file.size() != 1) {
        if (values_from_file.size() == numContinuousVars)
          ...
          throw std::runtime_error(
              "\"diagonal\" Queso covariance file data should have either 1 "
              "column (or row) and " +
      ...
      if (row_data) {
        ...
        if (values_from_file[0].length() != numContinuousVars)
          throw std::runtime_error(
              "\"diagonal\" Queso covariance file data should have " +
  ...
  } else  // "matrix" == input_fmt
  {
    if (use_file) {
      ...
      if (values_from_file.size() != numContinuousVars)
        throw std::runtime_error(
            "\"matrix\" Queso covariance file data should have " +
...
void NonDQUESOBayesCalibration::set_ip_options() {
  ...
  // File-based power user parameters have the final say
  if (!advancedOptionsFile.empty()) calIpOptionsValues->parse(*quesoEnv, "");
...
void NonDQUESOBayesCalibration::set_mh_options() {
  ...
  calIpMhOptionsValues->m_rawChainSize =
      (chainSamples > 0) ? chainSamples : 1000;
  ...
  calIpMhOptionsValues->m_rawChainDataOutputFileName =
      "QuesoDiagnostics/raw_chain";
  ...
  if (false) {  // TO DO: add user spec to support chain filtering?
    ...
    calIpMhOptionsValues->m_filteredChainDataOutputFileName =
        "QuesoDiagnostics/filtered_chain";
...
double NonDQUESOBayesCalibration::dakotaLogLikelihood(
  ...
  // in any inner models.
  RealVector& all_params = nonDQUESOInstance->residualModel->current_variables()
  ...
  if (nonDQUESOInstance->outputLevel >= DEBUG_OUTPUT) {
    ...
    size_t num_total_params = nonDQUESOInstance->numContinuousVars +
                              nonDQUESOInstance->numHyperparams;
...
void NonDQUESOBayesCalibration::copy_gsl(const QUESO::GslVector& qv,
                                         RealVector& rv) {
  size_t i, size_qv = qv.sizeLocal();
  if (size_qv != rv.length()) rv.sizeUninitialized(size_qv);
  for (i = 0; i < size_qv; ++i) rv[i] = qv[i];
}

void NonDQUESOBayesCalibration::copy_gsl(const RealVector& rv,
                                         QUESO::GslVector& qv) {
  size_t i, size_rv = rv.length();
  // this resize is not general, but is adequate for current uses:
  if (size_rv != qv.sizeLocal()) qv = paramSpace->zeroVector();  //(size_rv);
  for (i = 0; i < size_rv; ++i) qv[i] = rv[i];
}

void NonDQUESOBayesCalibration::copy_gsl_partial(const QUESO::GslVector& qv,
                                                 size_t start_qv,
                                                 RealVector& rv) {
  // copy part of qv into all of rv
  size_t ri, qi, size_rv = rv.length();
  for (ri = 0, qi = start_qv; ri < size_rv; ++ri, ++qi) rv[ri] = qv[qi];
}

void NonDQUESOBayesCalibration::copy_gsl_partial(const RealVector& rv,
                                                 QUESO::GslVector& qv,
                                                 size_t start_qv) {
  // copy all of rv into part of qv
  size_t ri, qi, size_rv = rv.length();
  for (ri = 0, qi = start_qv; ri < size_rv; ++ri, ++qi) qv[qi] = rv[ri];
}

void NonDQUESOBayesCalibration::copy_gsl(const QUESO::GslVector& qv,
                                         RealMatrix& rm, int col) {
  size_t i, size_qv = qv.sizeLocal();
  ...
  Real* rm_c = rm[col];
  for (i = 0; i < size_qv; ++i) rm_c[i] = qv[i];
}

bool NonDQUESOBayesCalibration::equal_gsl(const QUESO::GslVector& qv1,
                                          const QUESO::GslVector& qv2) {
  size_t size_qv1 = qv1.sizeLocal();
  if (size_qv1 != qv2.sizeLocal()) return false;
