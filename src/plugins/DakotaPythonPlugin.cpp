...
void DakotaPythonPlugin::initialize() {
  ...
  if (num_drivers != 1) {
    throw(
        std::runtime_error("Python plugin supports "
...
void DakotaPythonPlugin::set_python_function(
    std::string const& py_module_fn_str) {
...
std::vector<DakotaPlugins::EvalResponse> DakotaPythonPlugin::evaluate(
  ...
  for (size_t i = 0; i < num_requests; ++i) {
    ...
    unpack_python_response(requests[i].activeSet, num_derivs, py_responses[i],
                           responses[i]);
  }
...
void DakotaPythonPlugin::unpack_python_response(
    const std::vector<short>& active_set, size_t const num_derivs,
    py::dict const& py_response, DakotaPlugins::EvalResponse& response) {
  ...
  if (expect_derivative(active_set, 1)) {
    if (py_response.contains("fns")) {
      ...
      if (values.size() != num_fns) {
        throw(
            std::runtime_error("Python Plugin [\"fns\"]: "
      ...
      throw(
          std::runtime_error("Python Plugin: required key "
  ...
  if (expect_derivative(active_set, 2)) {
    if (py_response.contains("fnGrads")) {
      ...
      if (grads.size() != num_fns) {
        throw(
            std::runtime_error("Python Plugin [\"fnGrads\"]: "
                               "incorrect size for # of functions"));
      }
      for (size_t i = 0; i < num_fns; ++i) {
  ...
  if (expect_derivative(active_set, 4)) {
    if (py_response.contains("fnHessians")) {
      ...
      if (hess.size() != num_fns) {
        throw(
            std::runtime_error("Python Plugin [\"fnHessians\"]: "
                               "incorrect size for # of functions"));
      }
      for (size_t i = 0; i < num_fns; ++i) {
