...
NCSUOptimizer::NCSUOptimizer(ProblemDescDB& problem_db,
...
NCSUOptimizer::NCSUOptimizer(std::shared_ptr<Model> model, size_t max_iter,
                             size_t max_eval, double min_box_size,
                             double vol_box_size, double solution_target)
    : Optimizer(NCSU_DIRECT, model,
                std::shared_ptr<TraitsBase>(new NCSUTraits())),
      setUpType(SETUP_MODEL),
      minBoxSize(min_box_size),
      volBoxSize(vol_box_size),
      solutionTarget(solution_target),
      userObjectiveEval(NULL) {
...
NCSUOptimizer::NCSUOptimizer(  // const RealVector& initial_pt,
    ...
    double (*user_obj_eval)(const RealVector& x), double min_box_size,
    double vol_box_size, double solution_target)
    : Optimizer(NCSU_DIRECT, var_l_bnds.length(), 0, 0, 0,
                ...
                std::shared_ptr<TraitsBase>(new NCSUTraits())),
      setUpType(SETUP_USERFUNC),
      minBoxSize(min_box_size),
      volBoxSize(vol_box_size),
      solutionTarget(solution_target),
      lowerBounds(var_l_bnds),
      upperBounds(var_u_bnds),
      linIneqCoeffs(lin_ineq_coeffs),
      // linear and nonlinear constraints are cached even though not directly
      // used
...
void NCSUOptimizer::update_callback_data(
...
int NCSUOptimizer::objective_eval(int* n, double c[], double l[], double u[],
                                  int point[], int* maxI, int* start,
                                  int* maxfunc, double fvec[], int iidata[],
                                  int* iisize, double ddata[], int* idsize,
                                  char cdata[], int* icsize) {
  ...
  for (int j = 0; j < np; j++) {
    if (*start == 1)
      for (int i = 0; i < nx; i++) local_des_vars[i] = (c[i] + u[i]) * l[i];
    else {
      for (int i = 0; i < nx; i++) {
    ...
    if (ncsudirectInstance->setUpType == SETUP_MODEL) {
      ModelUtils::continuous_variables(*ncsudirectInstance->iteratedModel,
                                       local_des_vars);
      ...
      if (ncsudirectInstance->iteratedModel->asynch_flag())
        ...
        Real fn_val = ncsudirectInstance->iteratedModel->current_response()
                          .function_value(0);
...
void NCSUOptimizer::core_run() {
  ...
  NCSU_DIRECT_F77(objective_eval, local_des_vars.values(), num_cv, eps,
                  ...
                  solutionTarget, fglper, volper, sigmaper, idata, isize, ddata,
                  dsize, cdata, csize, quiet_flag);

  if (ierror < 0) {
    Cerr << "NCSU DIRECT failed with fatal error code " << ierror << "\n";
    switch (ierror) {
      case -1:
        Cerr << "(variable lower bounds must be strictly less than upper "
                "bounds)";
        break;
      case -2:
