...
static const char rcsId[] =
    "@(#) $Id: NonDMultilevelSampling.cpp 7035 2010-10-22 21:45:39Z mseldre $";
...
NonDMultilevelSampling::NonDMultilevelSampling(ProblemDescDB& problem_db,
  ...
  switch (allocationTarget) {
    case TARGET_MEAN: {
      ...
      size_t i, vec_ctr = 0;
      for (i = 0; i < numFunctions; ++i) scalarizationCoeffs(i, 2 * i) = 1.;
      break;
    }
    ...
    case TARGET_SIGMA: {
      ...
      scalarizationCoeffs = 0;
      size_t i, vec_ctr = 0;
      for (i = 0; i < numFunctions; ++i) scalarizationCoeffs(i, 2 * i + 1) = 1.;
      break;
    }
    case TARGET_SCALARIZATION: {
      ...
      if (finalMomentsType != Pecos::STANDARD_MOMENTS) {
        Cerr << "\nError: Scalarization not available with setting final_"
             << "moments=central. Use final_moments=standard instead."
             << std::endl;
        abort_handler(METHOD_ERROR);
      }
      if (qoiAggregation == QOI_AGGREGATION_SUM) {
        Cerr << "\nError: Scalarization not available with setting qoi_"
             << "aggregation=sum. Use qoi_aggregation=max instead."
             << std::endl;
        abort_handler(METHOD_ERROR);
      }
      ...
      if (scalarization_resp_vector.empty() ||
          scalarization_resp_vector.length() !=
              numFunctions * (2 * numFunctions)) {
        Cerr << "\n Warning: no or incomplete mappings provided for "
                "scalarization"
        ...
        for (i = 0; i < numFunctions; ++i) {
          for (j = 0; j < numFunctions; ++j) {
            scalarizationCoeffs(i, 2 * j) =
                scalarization_resp_vector[vec_ctr++];
            scalarizationCoeffs(i, 2 * j + 1) =
                scalarization_resp_vector[vec_ctr++];
          }
        }
      }
  ...
  iteratedModel->ensemble_precedence(
      MULTILEVEL_PRECEDENCE);  // prefer ML over MF
}
...
void NonDMultilevelSampling::core_run() {
  ...
  costSource =
      configure_cost(numSteps, sequenceType, sequenceCost, costMetadataIndices);
  ...
  switch (pilotMgmtMode) {
    case ONLINE_PILOT:
      // if (subIteratorFlag) multilevel_mc_Qsum(); // error est, unbiased
      // central else                 multilevel_mc_Ysum(); // lighter weight
      multilevel_mc_online_pilot();
      break;
      break;
    case OFFLINE_PILOT:
...
void NonDMultilevelSampling::assign_active_key() {
  std::vector<Pecos::ActiveKey> seq_keys(numSteps);
  switch (sequenceType) {
    case Pecos::RESOLUTION_LEVEL_1D_SEQUENCE:
      for (size_t s = 0; s < numSteps; ++s)
        seq_keys[s].form_key(0, secondaryIndex, s);
      break;
  ...
  while (!zeros(delta_N_l) && mlmfIter <= maxIterations) {
    ...
    for (step=0; step<numSteps; ++step) { // step is reference to lev
      ...
      if (numSamples) {
        ...
        accumulate_ml_Ysums(sum_Y, sum_YY, lev, 0, N_l[step]);
...
void NonDMultilevelSampling::multilevel_mc_online_pilot()  //_Qsum()
{
  ...
  RealMatrix var_Y, var_qoi;
  RealVector eps_sq_div_2;
  Sizet2DArray N_l_actual;
  SizetArray delta_N_l, N_l_alloc;

  load_pilot_sample(pilotSamples, numSteps, delta_N_l);
  while (!zeros(delta_N_l) && mlmfIter <= maxIterations) {
    // loop over levels and compute sample increments; pilot=online
    evaluate_levels(sum_Ql, sum_Qlm1, sum_QlQlm1, N_l_actual, N_l_actual,
                    N_l_alloc, N_l_alloc, delta_N_l, var_Y, var_qoi,
                    eps_sq_div_2, true, true);
    advance_relaxation();
  }
...
void NonDMultilevelSampling::multilevel_mc_offline_pilot() {
  ...
  // -----------------------------------------
  load_pilot_sample(pilotSamples, numSteps, delta_N_l);
  evaluate_levels(sum_Ql, sum_Qlm1, sum_QlQlm1, N_actual_pilot, N_actual_online,
                  N_alloc_pilot, N_alloc_online, delta_N_l, var_Y, var_qoi,
                  eps_sq_div_2, false, false);
  ...
  for (step = 0; step < numSteps; ++step) {
    accumulate_ml_Qsums(batchResponsesMap[step], sum_Ql, sum_Qlm1, sum_QlQlm1,
                        step, N_actual_online[step]);
    numSamples = delta_N_l[step];
  ...
  inflate_sequence_samples(N_actual_online, multilev, secondaryIndex,
                           NLevActual);
...
void NonDMultilevelSampling::multilevel_mc_pilot_projection() {
  ...
  if (pilotMgmtMode == OFFLINE_PILOT ||  // redirected here for ESTIMATOR_PERF
    ...
    SizetArray N_alloc_pilot;  // segregate
    evaluate_levels(sum_Ql, sum_Qlm1, sum_QlQlm1, N_actual_pilot, N_actual,
                    N_alloc_pilot, N_alloc, delta_N_l, var_Y, var_qoi,
                    eps_sq_div_2, false, false);
    compute_moments(sum_Ql, sum_Qlm1, sum_QlQlm1, N_actual_pilot);  // only varH
  } else {  // ONLINE_PILOT_PROJECTION
    evaluate_levels(sum_Ql, sum_Qlm1, sum_QlQlm1, N_actual, N_actual, N_alloc,
                    N_alloc,  // pilot is online
...
void NonDMultilevelSampling::evaluate_levels(
    IntRealMatrixMap& sum_Ql, IntRealMatrixMap& sum_Qlm1,
    IntIntPairRealMatrixMap& sum_QlQlm1, Sizet2DArray& N_actual_pilot,
    Sizet2DArray& N_actual_online, SizetArray& N_alloc_pilot,
    SizetArray& N_alloc_online, SizetArray& delta_N_l, RealMatrix& var_Y,
    RealMatrix& var_qoi, RealVector& eps_sq_div_2, bool increment_cost,
    bool pilot_estvar) {
  ...
  if (N_alloc_pilot.empty()) N_alloc_pilot.assign(numSteps, 0);
  if (N_alloc_online.empty()) N_alloc_online.assign(numSteps, 0);
  if (mlmfIter == 0) {
    if (!budget_constrained) {
      agg_est_var0.size(numFunctions);
      eps_sq_div_2.size(numFunctions);
    }
    var_Y.shapeUninitialized(numFunctions, numSteps);
    var_qoi.shapeUninitialized(numFunctions, numSteps);
  }

  // Previous use of BYPASS_SURROGATE (1x response for lev 0) and
  // AGGREGATED_MODEL_PAIR (2x response for lev >= 1) has now been replaced
  // with AGGREGATED_MODELS as for nonhierarchical approaches.  In this case,
  // activeSet identifies active model or model pair within the full model set
  // for Analyzer::evaluate_batch()
  // > Since ML groups use independent sample sets (no cross-group roll up as
  //   in MFMC, ACV-MF), the "horizontal" group to "vertical" model accumulation
  ...
  for (step = 0; step < numSteps; ++step) {
    numSamples = delta_N_l[step];
    if (numSamples) {
      IntResponseMap& resp_map =
          batchResponsesMap[step];  // step used as group id
      accumulate_ml_Qsums(resp_map, sum_Ql, sum_Qlm1, sum_QlQlm1, step,
                          N_actual_pilot[step]);
      if (backfillFailures && mlmfIter)
        N_alloc_pilot[step] +=
            allocation_increment(N_alloc_pilot[step], NTargetQoI[step]);
      else
        N_alloc_pilot[step] += numSamples;
      ...
      if (mlmfIter == 0 && !budget_constrained)  // MSE ref is MC applied to HF
  ...
  if (increment_cost) {
    ...
    for (step = 0; step < numSteps; ++step)
      increment_ml_equivalent_cost(delta_N_l[step],
                                   level_cost(sequenceCost, step), ref_cost,
                                   equivHFEvals);
  }
  ...
  if (mlmfIter == 0) {
    ...
    if (!budget_constrained)  // set target eps^2 / 2 = estvar0 * rel tol
      set_convergence_tol(agg_est_var0, sequenceCost, eps_sq_div_2);
  }
  // update targets based on variance estimates
  if (budget_constrained)
    compute_sample_allocation_target(var_qoi, sequenceCost, N_actual_online,
                                     N_alloc_online, delta_N_l);
  else
    compute_sample_allocation_target(
        sum_Ql, sum_Qlm1, sum_QlQlm1, eps_sq_div_2, var_qoi, sequenceCost,
        N_actual_pilot, N_actual_online, N_alloc_online, delta_N_l);
                  ...
                  short seq_type)
{
  ...
  if ( (seq_type == Pecos::MODEL_FORM_1D_SEQUENCE       && form == 0) ||
    ...
    if (seq_type == Pecos::MODEL_FORM_1D_SEQUENCE && lev == SZ_MAX) {
...
void NonDMultilevelSampling::ml_increments(SizetArray& delta_N_l,
                                           String prepend) {
  if (mlmfIter == 0)
  ...
  for (step = 0; step < num_steps; ++step) {
    ...
    if (numSamples) {
      ...
      // AGGREGATED_MODELS using step, step-1)
      ml_active_set(step);
      ensemble_sample_batch(prepend, step);  // index is group_id
    }
  }

  if (iteratedModel->asynch_flag())
    synchronize_batches(
        *iteratedModel);  // schedule all groups (return ignored)
}
...
void NonDMultilevelSampling::initialize_ml_Ysums(IntRealMatrixMap& sum_Y,
                                                 size_t num_lev,
                                                 size_t num_mom) {
...
void NonDMultilevelSampling::initialize_ml_Qsums(
    IntRealMatrixMap& sum_Ql, IntRealMatrixMap& sum_Qlm1,
  ...
  for (i = 1; i <= 2; ++i)
    for (j = 1; j <= 2; ++j) {
      ...
      sum_QlQlm1.insert(empty_iirm_pr)
          .first->second.shape(numFunctions, num_lev);
    }
}
...
void NonDMultilevelSampling::store_evaluations(const size_t lev) {
  ...
  if (it != levQoisamplesmatrixMap.end()) {
    it->second.reshape((lev > 0 ? 2 : 1) * numFunctions,
                       eval_index + numSamples);
  } else {
    Cerr << "NonDMultilevelSampling::store_evaluations: Key not found."
         << std::endl;
    abort_handler(METHOD_ERROR);
  }
  ...
  for (IntRespMCIter r_it = allResponses.begin(); r_it != allResponses.end();
    ...
    for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
      // Cout << "Evals lev: " << lev << " ctr " << eval_index << " val: " <<
      // fn_vals[qoi] << std::endl;
      it->second(qoi, eval_index) = fn_vals[qoi];
      if (lev > 0)
        it->second(qoi + numFunctions, eval_index) =
...
void NonDMultilevelSampling::reset_ml_Qsums(
    IntRealMatrixMap& sum_Ql, IntRealMatrixMap& sum_Qlm1,
    IntIntPairRealMatrixMap& sum_QlQlm1) {
  // reset accumulators to zero
  IntRMMIter q_it;
  IntIntPairRealMatrixMap::iterator qq_it;
  for (q_it = sum_Ql.begin(); q_it != sum_Ql.end(); ++q_it) q_it->second = 0.;
...
void NonDMultilevelSampling::accumulate_ml_Qsums(const IntResponseMap& resp_map,
                                                 IntRealMatrixMap& sum_Q,
                                                 size_t lev,
                                                 SizetArray& num_Q) {
  using std::isfinite;
  Real q_l, q_l_prod;
  int ord, active_ord;
  size_t qoi, offset_l = lev * numFunctions;
  IntRespMCIter r_it;
  IntRMMIter q_it;
...
void NonDMultilevelSampling::accumulate_ml_Qsums(
    const IntResponseMap& resp_map, IntRealMatrixMap& sum_Ql,
    IntRealMatrixMap& sum_Qlm1, IntIntPairRealMatrixMap& sum_QlQlm1, size_t lev,
    SizetArray& num_Q) {
  if (lev == 0)
    ...
    size_t qoi, offset_lm1 = (lev - 1) * numFunctions,
                offset_l = lev * numFunctions;
    IntRespMCIter r_it;
    ...
    if (outputLevel == DEBUG_OUTPUT)
      Cout << "Accumulated sums (Ql[1,2], Qlm1[1,2]):\n"
           << sum_Ql[1] << sum_Ql[2] << sum_Qlm1[1] << sum_Qlm1[2] << std::endl;
  }
}

void NonDMultilevelSampling::accumulate_ml_Ysums(const IntResponseMap& resp_map,
                                                 IntRealMatrixMap& sum_Y,
                                                 IntRealMatrixMap& sum_YY,
                                                 size_t lev, size_t lev_offset,
                                                 SizetArray& num_Y) {
  using std::isfinite;
  Real q_l, q_l_prod;
  int y_ord, yy_ord, active_ord;
  size_t qoi, offset_l = (lev + lev_offset) * numFunctions;
  IntRespMCIter r_it;
  IntRMMIter y_it, yy_it;
        ...
        if (isfinite(q_lm1) && isfinite(q_l)) {  // neither NaN nor +/-Inf
          ...
          while (y_it != sum_Y.end() || yy_it != sum_YY.end()) {
            ...
            if (yy_ord == active_ord) {
              ...
              yy_it->second(qoi, lev) +=
                  delta_prod * delta_prod;  // (HF^p-LF^p)^2
              ++yy_it;
              yy_ord = (yy_it == sum_YY.end()) ? 0 : yy_it->first;
            }
            q_l_prod *= q_l;
  ...
  if (lev == 0) {
    for (r_it = resp_map.begin(); r_it != resp_map.end(); ++r_it) {
      ...
      for (qoi = 0; qoi < numFunctions; ++qoi) {
        ...
        if (isfinite(q_l)) {  // neither NaN nor +/-Inf
          // add to sum_YY: running sums across all sample increments
          sum_YY(qoi, lev) += q_l_prod * q_l_prod;
...
void NonDMultilevelSampling::accumulate_ml_Ysums(const IntResponseMap& resp_map,
  ...
  if (lev == 0) {
    for (r_it = resp_map.begin(); r_it != resp_map.end(); ++r_it) {
      const RealVector& fn_vals = r_it->second.function_values();
      for (qoi = 0; qoi < numFunctions; ++qoi) {
        q_l = fn_vals[qoi + offset_l];
        if (isfinite(q_l)) {  // neither NaN nor +/-Inf
          ++num_Y[qoi];
          sum_Y(qoi, lev) += q_l;         // add to sum_Y
          sum_YY(qoi, lev) += q_l * q_l;  // add to sum_YY
        }
      }
    }
  } else {
...
void NonDMultilevelSampling::aggregate_variance_target_Qsum(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& N_l,
    const size_t step, RealMatrix& agg_var_qoi) {
  // compute estimator variance from current sample accumulation:
  ...
  switch (allocationTarget) {
    ...
    case TARGET_VARIANCE:
      for (size_t qoi = 0; qoi < numFunctions; ++qoi)
        agg_var_qoi(qoi, step) = variance_variance_Qsum(
            sum_Ql, sum_Qlm1, sum_QlQlm1, N_l, step, qoi);
      break;
    case TARGET_SIGMA:
      for (size_t qoi = 0; qoi < numFunctions; ++qoi)
        agg_var_qoi(qoi, step) =
    ...
    case TARGET_SCALARIZATION:
      for (size_t qoi = 0; qoi < numFunctions; ++qoi)
        agg_var_qoi(qoi, step) = variance_scalarization_Qsum(
            sum_Ql, sum_Qlm1, sum_QlQlm1, N_l, step, qoi);
      break;
    default:
...
Real NonDMultilevelSampling::variance_mean_Qsum(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& N_l,
...
Real NonDMultilevelSampling::variance_variance_Qsum(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& N_l,
    const size_t step, const size_t qoi) {
  Real place_holder;
  Real var_l =
      ...
      N_l[step]
         [qoi];  // As described in the paper by Krumscheid, Pisaroni, Nobile

  return var_l;
}

Real NonDMultilevelSampling::variance_sigma_Qsum(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& N_l,
    const size_t step, const size_t qoi) {
  Real place_holder;
  ...
  for (size_t cur_step = 0; cur_step < N_l.size(); ++cur_step)
    var_l +=
        var_lev_l(sum_Ql.at(1)[cur_step][qoi], sum_Qlm1.at(1)[cur_step][qoi],
                  sum_Ql.at(2)[cur_step][qoi], sum_Qlm1.at(2)[cur_step][qoi],
                  N_l[cur_step][qoi]);
...
Real NonDMultilevelSampling::variance_scalarization_Qsum(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& N_l,
    const size_t step, const size_t qoi) {
  ...
  /// For TARGET_SCALARIZATION we have the special case that we can also combine
  /// scalarization over multiple qoi This is respresented in the scalarization
  /// response mapping stored in scalarizationCoeffs This is for now neglecting
  /// cross terms for covariance terms inbetween different qois, e.g. V[mu_1 + 2
  /// sigma_1 + 3 mu_2] = V[mu_1] + V[2 sigma_1] + 2 Cov[mu_1, 2 sigma_1] + V[3
  /// mu_2] + 2 Cov[2 mu_1, 3 mu_2] + 2 Cov[2 sigma_1, 3 mu_2] \approx V[mu_1] +
  /// V[2 sigma_1] + 2 Cov[mu_1, 2 sigma_1] + V[3 mu_2] (What we do)
  ...
  for (size_t cur_qoi = 0; cur_qoi < numFunctions; ++cur_qoi) {
    ...
    var_of_mean_l = scalarizationCoeffs(qoi, cur_qoi_offset) == 0
                        ? 0
                        : variance_mean_Qsum(sum_Ql, sum_Qlm1, sum_QlQlm1, N_l,
                                             step, cur_qoi);

    if (var_of_mean_l < 0) {
      Cout << "NonDMultilevelSampling::aggregate_variance_scalarization_Qsum("
              "qoi, cur_qoi, lev) = ("
           << qoi << ", " << cur_qoi << ", " << step << "): var_of_var < 0"
           << std::endl;
    }
    check_negative(var_of_mean_l);

    // Var[Sigma_l - Sigma_(l-1)]
    var_of_sigma_l = scalarizationCoeffs(qoi, cur_qoi_offset + 1) == 0
                         ? 0
                         : variance_sigma_Qsum(sum_Ql, sum_Qlm1, sum_QlQlm1,
                                               N_l, step, cur_qoi);
    ...
    switch (cov_approximation_type) {
      case COV_BOOTSTRAP:
        cov_estim =
            (scalarizationCoeffs(qoi, cur_qoi_offset) == 0) ||
                    (scalarizationCoeffs(qoi, cur_qoi_offset + 1) == 0)
                ? 0
                : compute_bootstrap_covariance(
                      step, cur_qoi, levQoisamplesmatrixMap, N_l[step][cur_qoi],
                      false, dummy_grad, &(++bootstrapSeed)) *
                      N_l[step][cur_qoi];
        break;
      case COV_PEARSON:
      ...
      case COV_CORRLIFT:
        ...
        cov_estim = compute_cov_mean_sigma(
                        sum_Ql, sum_Qlm1, sum_QlQlm1, N_l[step][cur_qoi],
                        N_l[step][cur_qoi], cur_qoi, step, false, dummy_grad) *
                    N_l[step][cur_qoi];
        //}
        break;
    }

    // Cout << "Mean, sigma, Varvar vs bootstrap: qoi: " << qoi << ": " <<
    // var_of_mean_l << ", " << var_of_sigma_l << ", " <<
    // upper_bound_cov_of_mean_sigma << " vs. \n"; //<< cov_bootstrap <<
    // std::endl;
    ...
    /*if(cur_qoi == qoi){
      Cout << "aggregate_variance_scalarization_Qsum: qoi: " << qoi << " step: "
    << step << " cur_qoi: " << cur_qoi
        << " variances: " << var_of_mean_l << ", " << var_of_sigma_l << ", " <<
    cov_estim << std::endl; Cout << scalarizationCoeffs(qoi, cur_qoi_offset) <<
    ", " << scalarizationCoeffs(qoi, cur_qoi_offset+1) << std::endl; Cout <<
    var_of_scalarization_cur << "\n";
    }*/
  }
  return var_of_scalarization_l;  // Multiplication by N_l as described in the
                                  // paper by Krumscheid, Pisaroni, Nobile is
                                  // already done in submethods
}

Real NonDMultilevelSampling::compute_bootstrap_covariance(
    const size_t step, const size_t qoi,
  ...
  std::map<int, RealMatrix>::const_iterator it =
      lev_qoisamplematrix_map.find(step);
  ...
  for (int bs_resample = 0; bs_resample < nb_bs_samples; ++bs_resample) {
    for (int resample = 0; resample < nb_samples; ++resample) {
      ...
      bs_samples_lm1[resample] =
          (step > 0) ? it->second(qoi + nb_functions, bs_sample_idx) : 0;
    }
    ...
    if (step > 0) {
      meanlm1_bs[bs_resample] = compute_mean(
          bs_samples_lm1, N, compute_gradient, meanlm1_bs_grad[bs_resample]);
      sigmalm1_bs[bs_resample] = compute_std(
          bs_samples_lm1, N, compute_gradient, sigmalm1_bs_grad[bs_resample]);
    }
  }
  ...
  */
  if (compute_gradient) {
    ...
    for (int bs_resample = 0; bs_resample < nb_bs_samples; ++bs_resample) {
      covmeanlsigmal_grad +=
          (meanl_bs_grad[bs_resample] - mean_meanlm1_bs_grad) *
              (sigmal_bs[bs_resample] - mean_sigmal_bs) +
          (meanl_bs[bs_resample] - mean_meanl_bs) *
              (sigmal_bs_grad[bs_resample] - mean_sigmal_bs_grad);

      if (step > 0) {
        covmeanlm1sigmal_grad +=
            (meanlm1_bs_grad[bs_resample] - mean_meanlm1_bs_grad) *
                (sigmal_bs[bs_resample] - mean_sigmal_bs) +
            (meanlm1_bs[bs_resample] - mean_meanlm1_bs) *
                (sigmal_bs_grad[bs_resample] - mean_sigmal_bs_grad);

        covmeanlsigmalm1_grad +=
            (meanl_bs_grad[bs_resample] - mean_meanl_bs_grad) *
  ...
  return covmeanlsigmal - covmeanlm1sigmal - covmeanlsigmalm1 +
         covmeanlm1sigmalm1;
}
...
Real NonDMultilevelSampling::compute_cov(const RealVector& samples_X,
                                         const RealVector& samples_Y) {
...
size_t NonDMultilevelSampling::allocation_increment(size_t N_l_alloc,
                                                    const Real* N_l_target) {
  switch (qoiAggregation) {
    ...
    case QOI_AGGREGATION_MAX:
      ...
      break;
    default:
      Cerr << "Error: QoI aggregation mode " << qoiAggregation
           << " not supported"
...
void NonDMultilevelSampling::compute_sample_allocation_target(
    const RealMatrix& var_qoi, const RealVector& cost,
    const Sizet2DArray& N_actual, const SizetArray& N_alloc,
    SizetArray& delta_N_l) {
  ...
  switch (qoiAggregation) {
    case QOI_AGGREGATION_SUM: {
      ...
      factor = budget_cost / sum_sqrt_var_cost;
      for (step = 0; step < num_steps; ++step) {
    ...
    case QOI_AGGREGATION_MAX: {
      ...
      for (qoi = 0; qoi < numFunctions; ++qoi) {
        for (step = 0; step < num_steps; ++step)
          sum_sqrt_var_cost[qoi] +=
              std::sqrt(var_qoi(qoi, step) * lev_costs[step]);
        factor = budget_cost / sum_sqrt_var_cost[qoi];
        for (step = 0; step < num_steps; ++step) {
          N_target = NTargetQoI(qoi, step) =
              std::sqrt(var_qoi(qoi, step) / lev_costs[step]) * factor;
          delta_N_l_qoi[step][qoi] =
              (backfillFailures)
...
void NonDMultilevelSampling::compute_sample_allocation_target(
    ...
    const RealVector& cost, const Sizet2DArray& N_actual_pilot,
    const Sizet2DArray& N_actual_online, const SizetArray& N_alloc,
    SizetArray& delta_N_l) {
  const size_t num_steps = var_qoi.numCols(),
  ...
  if (qoiAggregation == QOI_AGGREGATION_SUM) {
    for (size_t step = 0; step < num_steps; ++step) {
      ...
      sum_sqrt_var_cost[0] +=
          std::sqrt(agg_var_qoi(0, step) * level_cost_vec[step]);
    }
  } else if (qoiAggregation == QOI_AGGREGATION_MAX) {
    for (size_t qoi = 0; qoi < nb_aggregation_qois; ++qoi) {
      ...
      for (size_t step = 0; step < num_steps; ++step) {
        sum_sqrt_var_cost[qoi] +=
            std::sqrt(var_qoi(qoi, step) * level_cost_vec[step]);
        agg_var_qoi(qoi, step) = var_qoi(qoi, step);
        if (outputLevel == DEBUG_OUTPUT) {
          Cout << "\n\tN_target for Qoi: " << qoi
               << ", with agg_var_qoi_in: " << var_qoi(qoi, step) << std::endl;
          Cout << "\n\tN_target for Qoi: " << qoi
               << ", with level_cost: " << level_cost_vec[step] << std::endl;
          Cout << "\n\tN_target for Qoi: " << qoi
               << ", with agg_var_qoi: " << sum_sqrt_var_cost << std::endl;
        }
      }
    }
  } else {
  ...
  for (size_t qoi = 0; qoi < nb_aggregation_qois; ++qoi) {
    // if(allocationTarget == TARGET_MEAN || allocationTarget ==
    // TARGET_VARIANCE){
    fact_qoi = Pecos::is_small(eps_sq_div_2[qoi])
                   ? 0
                   : sum_sqrt_var_cost[qoi] / eps_sq_div_2[qoi];
    if (outputLevel == DEBUG_OUTPUT) {
      Cout << "\n\tN_target for Qoi: " << qoi
           << ", with sum_sqrt_var_cost: " << sum_sqrt_var_cost[qoi]
           << std::endl;
      Cout << "\n\tN_target for Qoi: " << qoi
           << ", with eps_sq_div_2: " << eps_sq_div_2[qoi] << std::endl;
      Cout << "\n\tN_target for Qoi: " << qoi << ", with lagrange: " << fact_qoi
           << std::endl;
    }
    for (size_t step = 0; step < num_steps; ++step) {
      if (convergenceTolTarget == VARIANCE_CONSTRAINT_TARGET) {
        NTargetQoI(qoi, step) =
            Pecos::is_small(fact_qoi)
                ? 0
                : std::sqrt(agg_var_qoi(qoi, step) / level_cost_vec[step]) *
                      fact_qoi;
        ...
        Cout << "NonDMultilevelSampling::compute_sample_allocation_target: "
                "convergenceTolTarget is not known.\n";
        abort_handler(INTERFACE_ERROR);
      }

      NTargetQoI(qoi, step) =
          NTargetQoI(qoi, step) < 6 ? 6 : NTargetQoI(qoi, step);
    ...
    if ((allocationTarget == TARGET_VARIANCE ||
        ...
        (have_npsol || have_optpp) && useTargetVarianceOptimizationFlag) {
      size_t qoi_copy = qoi;
      size_t qoiAggregation_copy = qoiAggregation;
      size_t numFunctions_copy = numFunctions;
      if (outputLevel == DEBUG_OUTPUT) {
        Cout << "Numerical Optimization for sample allocation targeting "
             << (allocationTarget == TARGET_VARIANCE
                     ? "variance"
                     : (allocationTarget == TARGET_SIGMA ? "sigma"
                                                         : "scalarization"))
             << " using " << (have_npsol ? "NPSOL" : "OPTPP") << std::endl;
      ...
      for (size_t step = 0; step < num_steps; ++step) {
        ...
        initial_point[step] =
            8. > NTargetQoI(qoi, step) ? 8 : NTargetQoI(qoi, step);
      }

      RealVector var_lower_bnds, var_upper_bnds, lin_ineq_lower_bnds,
      ...
      */
      ...
      // Number of nonlinear equality constraints = 1, s.t. c_eq: c_1(Nlq) =
      // convergenceTol;
      nonlin_eq_targets.size(1);                 // init to 0
      nonlin_eq_targets[0] = eps_sq_div_2[qoi];  // convergenceTol;

      // bootstrapSeed++;
      assign_static_member(nonlin_eq_targets[0], qoi_copy, qoiAggregation_copy,
                           numFunctions_copy, level_cost_vec, sum_Ql, sum_Qlm1,
                           sum_QlQlm1, pilot_samples, scalarizationCoeffs);

      std::unique_ptr<Iterator> optimizer;

      void (*objective_function_npsol_ptr)(int&, int&, double*, double&,
                                           double*, int&) = nullptr;
      void (*constraint_function_npsol_ptr)(int&, int&, int&, int&, int*,
                                            double*, double*, double*, int&) =
          nullptr;
      void (*objective_function_optpp_ptr)(int, int, const RealVector&, double&,
                                           RealVector&, int&) = nullptr;
      void (*constraint_function_optpp_ptr)(int, int, const RealVector&,
                                            RealVector&, RealMatrix&, int&) =
          nullptr;
        ...
        switch (allocationTarget) {
          ...
          case TARGET_SCALARIZATION:
            objective_function_npsol_ptr =
                &target_scalarization_objective_eval_npsol;
            break;
          default:
            break;
        }
      ...
      optimizer.reset(new NPSOLOptimizer(
          initial_point, var_lower_bnds, var_upper_bnds, lin_ineq_coeffs,
          lin_ineq_lower_bnds, lin_ineq_upper_bnds, lin_eq_coeffs,
          lin_eq_targets, nonlin_ineq_lower_bnds, nonlin_ineq_upper_bnds,
          nonlin_eq_targets, objective_function_npsol_ptr,
          constraint_function_npsol_ptr, 3,
          1e-15)  // derivative_level = 3 means user_supplied gradients
      );
#elif HAVE_OPTPP
      if (convergenceTolTarget == VARIANCE_CONSTRAINT_TARGET) {
        ...
        switch (allocationTarget) {
          ...
          case TARGET_SCALARIZATION:
            objective_function_optpp_ptr =
                &target_scalarization_objective_eval_optpp;
            break;
          default:
            break;
        }
      ...
      optimizer.reset(new SNLLOptimizer(
          initial_point, var_lower_bnds, var_upper_bnds, lin_ineq_coeffs,
          lin_ineq_lower_bnds, lin_ineq_upper_bnds, lin_eq_coeffs,
          lin_eq_targets, nonlin_ineq_lower_bnds, nonlin_ineq_upper_bnds,
          nonlin_eq_targets, objective_function_optpp_ptr,
          constraint_function_optpp_ptr));
      // 10000, 100000, 1.e-14,
      // 1.e-14, 100000));
#endif
      ...
      if (outputLevel == DEBUG_OUTPUT) {
        ...
        Cout << optimizer->variables_results().continuous_variables()
             << std::endl;
        Cout << "Objective: " << optimizer->response_results().function_value(0)
             << std::endl;
        Cout << "Constraint: "
             << optimizer->response_results().function_value(1) << std::endl;
      }
      Cout << "Relative Constraint violation: "
           << std::abs(1 - optimizer->response_results().function_value(1) /
                               nonlin_eq_targets[0])
           << std::endl;
      Cout << "\n";

      if (std::abs(1. - optimizer->response_results().function_value(1) /
                            nonlin_eq_targets[0]) > 1.0e-5) {
        // if (outputLevel == DEBUG_OUTPUT) Cout << "Relative Constraint
        // violation violated: Switching to log scale " << std::endl;
        /*for (size_t step = 0; step < num_steps; ++step) {
          if(allocationTarget == TARGET_VARIANCE){
            initial_point[step] = 8. > NTargetQoI(qoi, step) ? 8 :
        NTargetQoI(qoi, step); }else{ initial_point[step] = pilot_samples[step];
        //8. > NTargetQoI(qoi, step) ? 8 : NTargetQoI(qoi, step);
          }
        }*/
        nonlin_eq_targets[0] =
            std::log(eps_sq_div_2[qoi]);  // std::log(convergenceTol);
#ifdef HAVE_NPSOL
        if (convergenceTolTarget == VARIANCE_CONSTRAINT_TARGET) {
          objective_function_npsol_ptr = &target_cost_objective_eval_npsol;
          switch (allocationTarget) {
          ...
          switch (allocationTarget) {
            case TARGET_VARIANCE:
              objective_function_npsol_ptr =
                  &target_var_objective_eval_logscale_npsol;
              break;
            case TARGET_SIGMA:
              objective_function_npsol_ptr =
                  &target_sigma_objective_eval_logscale_npsol;
              break;
            case TARGET_SCALARIZATION:
              objective_function_npsol_ptr =
                  &target_scalarization_objective_eval_logscale_npsol;
              break;
            default:
              break;
          }
        ...
        if (convergenceTolTarget == VARIANCE_CONSTRAINT_TARGET) {
          ...
          switch (allocationTarget) {
            case TARGET_VARIANCE:
              objective_function_optpp_ptr =
                  &target_var_objective_eval_logscale_optpp;
              break;
            case TARGET_SIGMA:
              objective_function_optpp_ptr =
                  &target_sigma_objective_eval_logscale_optpp;
              break;
            case TARGET_SCALARIZATION:
              objective_function_optpp_ptr =
                  &target_scalarization_objective_eval_logscale_optpp;
              break;
            default:
              break;
          }
        ...
        if (outputLevel == DEBUG_OUTPUT) {
          ...
          Cout << optimizer->variables_results().continuous_variables()
               << std::endl;
          Cout << "Objective: "
               << optimizer->response_results().function_value(0) << std::endl;
          Cout << "Constraint: "
               << optimizer->response_results().function_value(1) << std::endl;
          Cout << "Relative Constraint violation: "
               << std::abs(1 - optimizer->response_results().function_value(1) /
        ...
        Cout << "Relative Log-Constraint violation: "
             << std::abs(1 - optimizer->response_results().function_value(1) /
                                 nonlin_eq_targets[0])
             << std::endl;
      }
      ...
      for (size_t step = 0; step < num_steps; ++step) {
        NTargetQoI(qoi, step) =
            optimizer->variables_results().continuous_variable(step);
      }
    }
    for (size_t step = 0; step < num_steps; ++step) {
      if (std::isnan(NTargetQoI(qoi, step)) ||
          std::isinf(NTargetQoI(qoi, step))) {
  ...
  for (size_t qoi = 0; qoi < nb_aggregation_qois; ++qoi) {
    for (size_t step = 0; step < num_steps; ++step) {
      if (allocationTarget == TARGET_MEAN) {
        ...
        Cout << "NonDMultilevelSampling::compute_sample_allocation_target: "
                "allocationTarget is not implemented.\n";
        abort_handler(METHOD_ERROR);
      }
    }
  }
  ...
  if (qoiAggregation == QOI_AGGREGATION_SUM) {
    ...
    for (size_t step = 0; step < num_steps; ++step) {
      ...
      for (size_t qoi = 1; qoi < nb_aggregation_qois; ++qoi) {
        max_qoi_idx =
            delta_N_l_qoi(qoi, step) > delta_N_l_qoi(max_qoi_idx, step)
                ? qoi
                : max_qoi_idx;
      }
      // max_qoi_idx = 2;
      // Cout << "NonDMultilevelSampling::compute_sample_allocation_target:
      // hardcoded max_qoi_idx\n";
...
void NonDMultilevelSampling::compute_moments(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& N_l) {
  // RealMatrix Q_raw_mom(4, numFunctions);
  ...
  for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
    ...
    for (size_t step = 0; step < num_steps; ++step) {
      ...
      if (outputLevel == DEBUG_OUTPUT)
        Cout << "CM_l   for level " << step << ": " << cm1l << ' ' << cm2l
             << ' ' << cm3l << ' ' << cm4l << '\n';
      if (step) {
        uncentered_to_centered(
            sum_Q1lm1(qoi, step) / Nlq, sum_Q2lm1(qoi, step) / Nlq,
            sum_Q3lm1(qoi, step) / Nlq, sum_Q4lm1(qoi, step) / Nlq, cm1l, cm2l,
            cm3l, cm4l, Nlq);
        cm1 -= cm1l;
        cm2 -= cm2l;
        cm3 -= cm3l;
        cm4 -= cm4l;
        if (outputLevel == DEBUG_OUTPUT)
          Cout << "CM_lm1 for level " << step << ": " << cm1l << ' ' << cm2l
               << ' ' << cm3l << ' ' << cm4l << '\n';
      }
    }
    if (cm2 < 0) {
    ...
    if (finalMomentsType == Pecos::CENTRAL_MOMENTS) {
      ...
      centered_to_standard(cm1, cm2, cm3, cm4, mom_q[0], mom_q[1], mom_q[2],
                           mom_q[3]);
  }
}

void NonDMultilevelSampling::compute_error_estimates(
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const Sizet2DArray& num_Q) {
  if (!finalMomentsType) return;
  ...
  Real agg_estim_var_mean, agg_estim_var_var, agg_estim_var_sigma,
      agg_estim_cov_scalarization, var_Yl, cm1l, cm2l, cm3l, cm4l, cm1lm1,
      cm2lm1, cm3lm1, cm4lm1, cm1l_sq, cm1lm1_sq, cm2l_sq, cm2lm1_sq, var_Ql,
      var_Qlm1, mu_Q2l, mu_Q2lm1, mu_Q2lQ2lm1, mu_Q1lm1_mu_Q2lQ1lm1,
      mu_Q1lm1_mu_Q1lm1_muQ2l, mu_Q1l_mu_Q1lQ2lm1, mu_Q1l_mu_Q1l_mu_Q2lm1,
      mu_Q1l_mu_Qlm1_mu_Q1lQ1lm1, mu_Q1l_mu_Q1l_mu_Q1lm1_muQ1lm1,
      mu_Q2l_muQ2lm1, mu_Q1lQ1lm1_mu_Q1lQ1lm1, mu_P2lP2lm1, var_P2l, var_P2lm1,
      covar_P2lP2lm1, term, bessel_corr, a_div_b, b_div_a, dummy_grad;
  ...
  const RealMatrix &sum_Q1l = sum_Ql.at(1), &sum_Q1lm1 = sum_Qlm1.at(1),
                   ...
                   &sum_Q1lQ1lm1 = sum_QlQlm1.at(pr11),
                   &sum_Q1lQ2lm1 = sum_QlQlm1.at(pr12),
                   &sum_Q2lQ1lm1 = sum_QlQlm1.at(pr21),
                   &sum_Q2lQ2lm1 = sum_QlQlm1.at(pr22);
  for (qoi = 0; qoi < numFunctions; ++qoi) {
    ...
    for (lev = 0; lev < num_lev; ++lev) {
      Nlq = num_Q[lev][qoi];
      agg_estim_var_mean +=
          variance_mean_Qsum(sum_Ql, sum_Qlm1, sum_QlQlm1, num_Q, lev, qoi) /
          Nlq;
    }
    ...
    if (agg_estim_var_mean < 0) {
      Cout << "NonDMultilevelSampling::compute_error_estimates:mean(qoi, lev) "
              "= ("
           << qoi << ", " << lev << "): agg_estim_var_mean < 0" << std::endl;
    }
    check_negative(agg_estim_var_mean);
    finalStatErrors(2 * qoi, 2 * qoi) =
        std::sqrt(agg_estim_var_mean);  // std error
    if (outputLevel >= DEBUG_OUTPUT) {
    ...
    for (lev = 0; lev < num_lev; ++lev) {
      Nlq = num_Q[lev][qoi];
      agg_estim_var_var += variance_variance_Qsum(sum_Ql, sum_Qlm1, sum_QlQlm1,
                                                  num_Q, lev, qoi) /
                           Nlq;
    }

    if (agg_estim_var_var < 0) {
      Cout << "NonDMultilevelSampling::compute_error_estimates:variance/"
              "std(qoi, lev) = ("
           << qoi << ", " << lev << "): agg_estim_var_var < 0" << std::endl;
    }
    ...
    if (finalMomentsType == Pecos::STANDARD_MOMENTS && mom2 > 0.) {
      ...
      finalStatErrors(2 * qoi + 1, 2 * qoi + 1) =
          std::sqrt(agg_estim_var_sigma);

      if (outputLevel >= DEBUG_OUTPUT)
        Cout << "Estimator SE for stddev = "
             << finalStatErrors(2 * qoi + 1, 2 * qoi + 1) << "\n";
    } else  // std error of variance estimator
      finalStatErrors(2 * qoi + 1, 2 * qoi + 1) = std::sqrt(agg_estim_var_var);

    if (scalarizationCoeffs(qoi, 2 * qoi) != 0 &&
        scalarizationCoeffs(qoi, 2 * qoi + 1) != 0 &&
        finalMomentsType == Pecos::STANDARD_MOMENTS) {
      agg_estim_cov_scalarization = 0;
      switch (cov_approximation_type) {
        case COV_BOOTSTRAP:
          for (lev = 0; lev < num_lev; ++lev) {
            agg_estim_cov_scalarization += compute_bootstrap_covariance(
                lev, qoi, levQoisamplesmatrixMap, num_Q[lev][qoi], false,
                dummy_grad, &(++bootstrapSeed));
          }
          break;
        case COV_PEARSON:
          agg_estim_cov_scalarization =
              std::sqrt(agg_estim_var_mean * agg_estim_var_sigma);
          break;
        case COV_CORRLIFT:
          for (lev = 0; lev < num_lev; ++lev) {
            agg_estim_cov_scalarization += compute_cov_mean_sigma(
                sum_Ql, sum_Qlm1, sum_QlQlm1, num_Q[lev][qoi], num_Q[lev][qoi],
                qoi, lev, false, dummy_grad);
          }
      }

      finalStatErrors(2 * qoi + 1, 2 * qoi) = agg_estim_cov_scalarization;
      if (outputLevel >= DEBUG_OUTPUT)
        Cout << "Estimator Cov for Cov[mean, stddev] = "
             << finalStatErrors(2 * qoi + 1, 2 * qoi) << "\n";
    } else {
    ...
    //    + requestedRelLevels[qoi].length() +
    //    requestedGenRelLevels[qoi].length();
  }
}
...
void NonDMultilevelSampling::assign_static_member(
    const Real& conv_tol, size_t& qoi, const size_t& qoi_aggregation,
    const size_t& num_functions, const RealVector& level_cost_vec,
    const IntRealMatrixMap& sum_Ql, const IntRealMatrixMap& sum_Qlm1,
    const IntIntPairRealMatrixMap& sum_QlQlm1, const RealVector& pilot_samples,
...
void NonDMultilevelSampling::assign_static_member_problem18(
    Real& var_L_exact, Real& var_H_exact, Real& mu_four_L_exact,
    Real& mu_four_H_exact, Real& Ax, RealVector& level_cost_vec) const {
...
void NonDMultilevelSampling::target_cost_objective_eval_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
...
void NonDMultilevelSampling::target_cost_constraint_eval_npsol(
    int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
  ...
  target_cost_constraint_eval_optpp(mode, n, optpp_x, optpp_g, optpp_grad_g,
                                    nstate);
...
void NonDMultilevelSampling::target_var_constraint_eval_npsol(
    int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
    double* grad_g, int& nstate) {
  RealVector optpp_x;
  RealVector optpp_g;
  ...
  target_var_constraint_eval_optpp(mode, n, optpp_x, optpp_g, optpp_grad_g,
                                   nstate);
...
void NonDMultilevelSampling::target_var_objective_eval_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
  Real optpp_f;
  ...
  target_var_objective_eval_optpp(mode, n, optpp_x, optpp_f, optpp_grad_f,
                                  nstate);
...
void NonDMultilevelSampling::target_sigma_constraint_eval_npsol(
    int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
    double* grad_g, int& nstate) {
  RealVector optpp_x;
  RealVector optpp_g;
  ...
  target_sigma_constraint_eval_optpp(mode, n, optpp_x, optpp_g, optpp_grad_g,
                                     nstate);
...
void NonDMultilevelSampling::target_sigma_objective_eval_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
  Real optpp_f;
  RealVector optpp_grad_f(n);
  ...
  target_sigma_objective_eval_optpp(mode, n, optpp_x, optpp_f, optpp_grad_f,
                                    nstate);
...
void NonDMultilevelSampling::target_scalarization_constraint_eval_npsol(
    int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
    double* grad_g, int& nstate) {
  RealVector optpp_x;
  RealVector optpp_g;
  ...
  target_scalarization_constraint_eval_optpp(mode, n, optpp_x, optpp_g,
                                             optpp_grad_g, nstate);
...
void NonDMultilevelSampling::target_scalarization_objective_eval_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
  Real optpp_f;
  RealVector optpp_grad_f(n);
  ...
  target_scalarization_objective_eval_optpp(mode, n, optpp_x, optpp_f,
                                            optpp_grad_f, nstate);
...
void NonDMultilevelSampling::target_var_constraint_eval_logscale_npsol(
    int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
    double* grad_g, int& nstate) {
  RealVector optpp_x;
  RealVector optpp_g;
  ...
  target_var_constraint_eval_logscale_optpp(mode, n, optpp_x, optpp_g,
                                            optpp_grad_g, nstate);
...
void NonDMultilevelSampling::target_var_objective_eval_logscale_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
  Real optpp_f;
  RealVector optpp_grad_f(n);
  ...
  target_var_objective_eval_logscale_optpp(mode, n, optpp_x, optpp_f,
                                           optpp_grad_f, nstate);
...
void NonDMultilevelSampling::target_sigma_constraint_eval_logscale_npsol(
    int& mode, int& m, int& n, int& ldJ, int* needc, double* x, double* g,
    double* grad_g, int& nstate) {
  RealVector optpp_x;
  RealVector optpp_g;
  ...
  target_sigma_constraint_eval_logscale_optpp(mode, n, optpp_x, optpp_g,
                                              optpp_grad_g, nstate);
...
void NonDMultilevelSampling::target_sigma_objective_eval_logscale_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
  Real optpp_f;
  RealVector optpp_grad_f(n);
  ...
  target_sigma_objective_eval_logscale_optpp(mode, n, optpp_x, optpp_f,
                                             optpp_grad_f, nstate);
...
void NonDMultilevelSampling::
  ...
  target_scalarization_constraint_eval_logscale_optpp(mode, n, optpp_x, optpp_g,
                                                      optpp_grad_g, nstate);
...
void NonDMultilevelSampling::target_scalarization_objective_eval_logscale_npsol(
    int& mode, int& n, double* x, double& f, double* gradf, int& nstate) {
  RealVector optpp_x;
  Real optpp_f;
  RealVector optpp_grad_f(n);
  ...
  target_scalarization_objective_eval_logscale_optpp(mode, n, optpp_x, optpp_f,
                                                     optpp_grad_f, nstate);
...
void NonDMultilevelSampling::target_cost_objective_eval_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  f = 0;
...
void NonDMultilevelSampling::target_cost_constraint_eval_optpp(
...
void NonDMultilevelSampling::target_var_constraint_eval_logscale_optpp(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
...
void NonDMultilevelSampling::target_var_objective_eval_logscale_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  Real agg_estim_var = 0;
  size_t num_lev = n;
...
void NonDMultilevelSampling::target_sigma_constraint_eval_logscale_optpp(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
  Real agg_estim_var = 0;
  size_t num_lev = n;
...
void NonDMultilevelSampling::target_sigma_objective_eval_logscale_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  Real agg_estim_var = 0;
  size_t num_lev = n;
...
void NonDMultilevelSampling::
    target_scalarization_constraint_eval_logscale_optpp(int mode, int n,
                                                        const RealVector& x,
                                                        RealVector& g,
                                                        RealMatrix& grad_g,
                                                        int& result_mode) {
  Real agg_estim_var = 0;
  size_t num_lev = n;
  target_scalarization_constraint_eval_optpp(mode, n, x, g, grad_g,
                                             result_mode);
  agg_estim_var = g[0];
#ifdef HAVE_NPSOL
#elif HAVE_OPTPP
...
void NonDMultilevelSampling::target_scalarization_objective_eval_logscale_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  Real agg_estim_var = 0;
  size_t num_lev = n;
...
void NonDMultilevelSampling::target_var_constraint_eval_optpp(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
  ...
  target_var_objective_eval_optpp(mode, n, x, g_to_f, grad_g_to_grad_f,
                                  result_mode);
  g[0] = g_to_f;
  if (compute_gradient) {
...
void NonDMultilevelSampling::target_var_objective_eval_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  bool compute_gradient = false;
#ifdef HAVE_NPSOL
  ...
  if (qoiAggregation == QOI_AGGREGATION_SUM) {
    ...
    for (size_t qoi = 0; qoi < nb_qois; ++qoi) {
      agg_estim_var_l[0] =
          var_of_var_ml_l0(*static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1,
                           Nlq_pilot, Nlq, qoi, compute_gradient, grad_f[0]);
      agg_estim_var += agg_estim_var_l[0];
      for (lev = 1; lev < num_lev; ++lev) {
        Nlq = x[lev];
        Nlq_pilot = (*static_Nlq_pilot)[lev];

        agg_estim_var_l[lev] = var_of_var_ml_l(
            *static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1, Nlq_pilot,
            Nlq, qoi, lev, compute_gradient, grad_f[lev]);
    ...
    for (lev = 1; lev < num_lev; ++lev) {
      ...
      agg_estim_var_l[lev] = var_of_var_ml_l(
          *static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1, Nlq_pilot, Nlq,
          qoi, lev, compute_gradient, grad_f[lev]);
      agg_estim_var += agg_estim_var_l[lev];
    }
  } else {
    Cout << "NonDMultilevelSampling::multilevel_mc_Qsum: qoiAggregation is not "
            "known.\n";
    abort_handler(METHOD_ERROR);
  }
...
void NonDMultilevelSampling::target_sigma_constraint_eval_optpp(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
  bool compute_gradient = false;
#ifdef HAVE_NPSOL
  ...
  RealVector grad_g_to_grad_f;
  grad_g_to_grad_f.resize(n);
  target_sigma_objective_eval_optpp(mode, n, x, g_to_f, grad_g_to_grad_f,
                                    result_mode);
  g[0] = g_to_f;
  if (compute_gradient) {
    for (size_t lev = 0; lev < n; ++lev) {
...
void NonDMultilevelSampling::target_sigma_objective_eval_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  bool compute_gradient = false;
#ifdef HAVE_NPSOL
  ...
  if (qoiAggregation == QOI_AGGREGATION_SUM) {
    ...
    for (size_t qoi = 0; qoi < nb_qois; ++qoi) {
      agg_estim_var_of_var_l[0] = var_of_var_ml_l0(
          *static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1, Nlq_pilot,
          Nlq_pilot, qoi, compute_gradient, gradient_var_var);
      agg_estim_var_of_var += agg_estim_var_of_var_l[0];

      agg_estim_var_l[0] = variance_Ysum_static(
          (*static_sum_Ql).at(1)[0][qoi], (*static_sum_Ql).at(1)[0][qoi],
          Nlq_pilot, Nlq, compute_gradient, gradient_var);
      agg_estim_var += agg_estim_var_l[0];
      ...
      if (compute_gradient) {
        grad_f[0] +=
            agg_estim_var_l[0] > 0
                ? 1. / 4. *
                      (
                          //-1./(agg_estim_var_l[0] * agg_estim_var_l[0]) *
                          // gradient_var * agg_estim_var_of_var_l[0]
                          //+ //TODO SCALAR BUGFIX
      ...
      for (lev = 1; lev < num_lev; ++lev) {
        ...
        agg_estim_var_of_var_l[lev] = var_of_var_ml_l(
            *static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1, Nlq_pilot,
            Nlq, qoi, lev, compute_gradient, gradient_var_var);
        agg_estim_var_of_var += agg_estim_var_of_var_l[lev];

        agg_estim_var_l[lev] =
            var_lev_l_static((*static_sum_Ql).at(1)[lev][qoi],
                             (*static_sum_Qlm1).at(1)[lev][qoi],
                             (*static_sum_Ql).at(2)[lev][qoi],
                             (*static_sum_Qlm1).at(2)[lev][qoi], Nlq_pilot,
                             Nlq_pilot, compute_gradient, gradient_var);
        agg_estim_var += agg_estim_var_l[lev];
        ...
        if (compute_gradient) {
          grad_f[lev] +=
              agg_estim_var_l[0] > 0
                  ? 1. / 4. *
                        (
                            //-1./(agg_estim_var_l[lev] * agg_estim_var_l[lev])
                            //* gradient_var * agg_estim_var_of_var_l[lev]
                            //+ //TODO SCALAR BUGFIX
    ...
    agg_estim_var_of_var_l[0] = var_of_var_ml_l0(
        *static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1, Nlq_pilot, Nlq,
        qoi, compute_gradient, gradient_var_var);
    agg_estim_var_of_var += agg_estim_var_of_var_l[0];

    grad_var_var[0] = gradient_var_var;

    agg_estim_var_l[0] = variance_Ysum_static(
        (*static_sum_Ql).at(1)[0][qoi], (*static_sum_Ql).at(2)[0][qoi],
        Nlq_pilot, Nlq_pilot, compute_gradient,
        gradient_var);  // TODO_SCALARBUGFIX
    grad_var[0] = gradient_var;

    // Cout << "Sigma Variance terms: " << lev << ": " << agg_estim_var_l[0] <<
    // ", " << agg_estim_var_of_var_l[0] << "\n";
    ...
    for (lev = 1; lev < num_lev; ++lev) {
      ...
      agg_estim_var_of_var_l[lev] = var_of_var_ml_l(
          *static_sum_Ql, *static_sum_Qlm1, *static_sum_QlQlm1, Nlq_pilot, Nlq,
          qoi, lev, compute_gradient, gradient_var_var);

      agg_estim_var_of_var += agg_estim_var_of_var_l[lev];

      grad_var_var[lev] = gradient_var_var;

      agg_estim_var_l[lev] = var_lev_l_static(
          (*static_sum_Ql).at(1)[lev][qoi], (*static_sum_Qlm1).at(1)[lev][qoi],
          (*static_sum_Ql).at(2)[lev][qoi], (*static_sum_Qlm1).at(2)[lev][qoi],
          Nlq_pilot, Nlq_pilot, compute_gradient,
          gradient_var);  // TODO_SCALARBUGFIX

      agg_estim_var += agg_estim_var_l[lev];

      // Cout << "Sigma Variance terms: " << lev << ": " << agg_estim_var_l[lev]
      // << ", " << agg_estim_var_of_var_l[lev] << "\n";
    ...
    if (compute_gradient) {
      for (lev = 0; lev < num_lev; ++lev) {
        grad_f[lev] = agg_estim_var > 0
                          ? 1. / 4. *
                                ((grad_var_var[lev] * agg_estim_var -
                                  agg_estim_var_of_var * grad_var[lev]) /
                                 (agg_estim_var * agg_estim_var))
                          : 0;
      }
    }
  } else {
    Cout << "NonDMultilevelSampling::target_sigma_objective_eval_optpp: "
            "qoiAggregation is not known.\n";
    abort_handler(METHOD_ERROR);
  }

  /*Cout << "NonDMultilevelSampling::target_sigma_objective_eval_optpp Opt: qoi:
  " << qoi << ": ("; for (lev = 0; lev < num_lev; ++lev) Cout << x[lev] << ((lev
  == num_lev-1) ? "" : ", "); Cout << ")\n\tValues:" << agg_estim_var << ", " <<
  agg_estim_var_of_var << ", ";
  */

  if (agg_estim_var_of_var < 0) {
    Cout << "NonDMultilevelSampling::target_sigma_objective_eval_optpp: "
            "agg_estim_var_of_var < 0: "
         << agg_estim_var_of_var << "\n";
  ...
  f = agg_estim_var > 0 ? 1. / 4. * agg_estim_var_of_var / agg_estim_var
                        : 0;  // - (*static_eps_sq_div_2);
  // f = agg_estim_var != 0 ? 1./4. * agg_estim_var_of_var/agg_estim_var : 0;
  // Cout << f << "\n";
  // exit(-1);
}

void NonDMultilevelSampling::target_scalarization_constraint_eval_optpp(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
  bool compute_gradient = false;
#ifdef HAVE_NPSOL
  ...
  RealVector grad_g_to_grad_f;
  grad_g_to_grad_f.resize(n);
  target_scalarization_objective_eval_optpp(mode, n, x, g_to_f,
                                            grad_g_to_grad_f, result_mode);
  g[0] = g_to_f;
  if (compute_gradient) {
    for (size_t lev = 0; lev < n; ++lev) {
      grad_g[0][lev] = grad_g_to_grad_f[lev];
...
void NonDMultilevelSampling::target_scalarization_objective_eval_optpp(
    int mode, int n, const RealVector& x, double& f, RealVector& grad_f,
    int& result_mode) {
  bool compute_gradient = false;
#ifdef HAVE_NPSOL
  ...
  for (size_t cur_qoi = 0; cur_qoi < nb_qois; ++cur_qoi) {
    ...
    if (qoiAggregation == QOI_AGGREGATION_SUM) {
      for (size_t sum_qoi = 0;
        ...
        for (lev = 0; lev < num_lev; ++lev) {
          ...
          f_var = (lev == 0)
                      ? variance_Ysum_static((*static_sum_Ql).at(1)[0][sum_qoi],
                                             (*static_sum_Ql).at(2)[0][sum_qoi],
                                             Nlq_pilot, Nlq, compute_gradient,
                                             cur_grad_var)
                      : variance_Qsum_static(
                            (*static_sum_Ql).at(1)[lev][sum_qoi],
                            (*static_sum_Qlm1).at(1)[lev][sum_qoi],
                            (*static_sum_Ql).at(2)[lev][sum_qoi],
                            (*static_sum_QlQlm1).at(pr11)[lev][sum_qoi],
                            (*static_sum_Qlm1).at(2)[lev][sum_qoi], Nlq_pilot,
                            Nlq, compute_gradient, cur_grad_var);
      ...
      for (lev = 0; lev < num_lev && (*static_scalarization_response_mapping)(
                                         qoi, cur_qoi_offset) != 0;
           ++lev) {
        Nlq = x[lev];
        Nlq_pilot = (*static_Nlq_pilot)[lev];
        f_var = (lev == 0)
                    ? variance_Ysum_static((*static_sum_Ql).at(1)[0][cur_qoi],
                                           (*static_sum_Ql).at(2)[0][cur_qoi],
                                           Nlq_pilot, Nlq, compute_gradient,
                                           cur_grad_var)
                    : variance_Qsum_static(
                          (*static_sum_Ql).at(1)[lev][cur_qoi],
                          (*static_sum_Qlm1).at(1)[lev][cur_qoi],
                          (*static_sum_Ql).at(2)[lev][cur_qoi],
                          (*static_sum_QlQlm1).at(pr11)[lev][cur_qoi],
                          (*static_sum_Qlm1).at(2)[lev][cur_qoi], Nlq_pilot,
                          Nlq, compute_gradient, cur_grad_var);
        f_mean += f_var / Nlq;

        // Cout << "Varvar vs bootstrap Opt: qoi: " << cur_qoi << "lev" << lev
        // << ": (" << x[0] << ", " << x[1] << "): " << f_var << std::endl;
        if (compute_gradient) {
          grad_f_mean[lev] = (Nlq * cur_grad_var - f_var) / (Nlq * Nlq);
        }
      }
    } else {
      Cout << "NonDMultilevelSampling::target_scalarization_objective_eval_"
              "optpp: qoiAggregation is not known.\n";
      abort_handler(METHOD_ERROR);
    }
    ...
    if ((*static_scalarization_response_mapping)(qoi, cur_qoi_offset + 1) !=
        0) {
      *static_qoi = cur_qoi;
      target_sigma_objective_eval_optpp(mode, n, x, f_sigma, grad_f_sigma,
                                        result_mode);
    ...
    if (cov_scaling != 0 &&
        (*static_scalarization_response_mapping)(qoi, cur_qoi_offset) != 0 &&
        (*static_scalarization_response_mapping)(qoi, cur_qoi_offset + 1) !=
            0) {
      switch (*static_cov_approximation_type) {
        case COV_BOOTSTRAP: {
          Real grad_f_bootstrap_cov_tmp = 0;
          for (lev = 0; lev < num_lev; ++lev) {
            // TODO_SCALARBUGFIX x[lev] -> (*static_Nlq_pilot)[lev]: Results in
            // a zero gradient and constant over N bootstrap estimation.
            f_cov_estimate += compute_bootstrap_covariance(
                lev, cur_qoi, *static_levQoisamplesmatrixMap,
                (*static_Nlq_pilot)[lev], compute_gradient,
                grad_f_bootstrap_cov_tmp, static_randomSeed);
            if (compute_gradient) {
        ...
        case COV_PEARSON: {
          ...
          if (compute_gradient) {
            for (lev = 0; lev < num_lev; ++lev) {
              grad_f_cov_estimate[lev] = f_cov_estimate > 0.
                                             ? 1. / (2. * f_cov_estimate) *
                                                   (grad_f_mean[lev] * f_sigma +
                                                    f_mean * grad_f_sigma[lev])
                                             : 0;
            }
          }
        } break;
    ...
    Real f_tmp =
        (*static_scalarization_response_mapping)(qoi, cur_qoi_offset) *
            (*static_scalarization_response_mapping)(qoi, cur_qoi_offset) *
            f_mean +
        (*static_scalarization_response_mapping)(qoi, cur_qoi_offset + 1) *
            (*static_scalarization_response_mapping)(qoi, cur_qoi_offset + 1) *
    ...
    switch (*static_cov_approximation_type) {
      case COV_BOOTSTRAP:
        f_tmp +=
            cov_scaling * 2.0 *
            ((*static_scalarization_response_mapping)(qoi, cur_qoi_offset)) *
            ((*static_scalarization_response_mapping)(qoi,
                                                      cur_qoi_offset + 1)) *
            f_cov_estimate;
        break;
      case COV_CORRLIFT:
        ...
        break;
      case COV_PEARSON:
        f_tmp += cov_scaling * 2.0 *
                 std::abs((*static_scalarization_response_mapping)(
                     qoi, cur_qoi_offset)) *
                 std::abs((*static_scalarization_response_mapping)(
                     qoi, cur_qoi_offset + 1)) *
                 f_cov_estimate;
        break;
    }

    /*Cout << "NonDMultilevelSampling::target_scalarization_objective_eval_optpp
    Opt: qoi: " << qoi << " cur_qoi: " << cur_qoi << ": ("; for (lev = 0; lev <
    num_lev; ++lev) Cout << x[lev] << ((lev == num_lev-1) ? "" : ", "); Cout <<
    ")\n\tValues:" << f_mean << ", " << f_sigma << ", " << f_cov_estimate << " =
    " << f_tmp << std::endl; Cout << "\tGradients: "; for (lev = 0; lev <
    num_lev; ++lev){ Cout << ((lev > 0) ? "\t\t" : "")<< grad_f_mean[lev] <<  ",
    "; Cout << grad_f_sigma[lev] <<  ", "; Cout << grad_f_cov_estimate[lev] <<
    ((lev == num_lev-1) ? "\n" : "\n");
    }*/
    ...
    if (compute_gradient) {
      for (lev = 0; lev < num_lev; ++lev) {
        ...
        switch (*static_cov_approximation_type) {
          case COV_BOOTSTRAP:
            grad_f[lev] += cov_scaling * 2.0 *
                           ((*static_scalarization_response_mapping)(
                               qoi, cur_qoi_offset)) *
                           ((*static_scalarization_response_mapping)(
                               qoi, cur_qoi_offset + 1)) *
                           grad_f_cov_estimate[lev];
            break;
          case COV_CORRLIFT:
            grad_f[lev] += cov_scaling * 2.0 *
                           ((*static_scalarization_response_mapping)(
  ...
  Real f_fd_minus = 0;
  Real f_fd = 0;
  Real fd_forward, fd_backward, fd_central, f_fd_plus_central,
      f_fd_minus_central;
  ...
  /*for (lev = 0; lev < num_lev && compute_gradient; ++lev) {
    x_fd[lev] = x[lev] + h;
    target_scalarization_objective_eval_optpp_fd(mode, n, x_fd, fd_forward,
  result_mode); x_fd[lev] = x[lev] - h;
    target_scalarization_objective_eval_optpp_fd(mode, n, x_fd, fd_backward,
  result_mode); Cout << "FD Opt: (qoi: " << qoi << ", lev: " << lev << ") : (";
    Cout << "\tGradients: ";
    Cout << grad_f[lev] <<  "vs. " << (fd_forward - fd_backward)/(2.*h) <<
  std::endl;
  } */
}

void NonDMultilevelSampling::target_scalarization_objective_eval_optpp_fd(
    int mode, int n, const RealVector& x, double& f, int& result_mode) {
  bool compute_gradient = false;
  ...
  for (size_t cur_qoi = 0; cur_qoi < nb_qois; ++cur_qoi) {
    ...
    for (lev = 0; lev < num_lev && (*static_scalarization_response_mapping)(
      ...
      Nlq = x[lev];
      Nlq_pilot = (*static_Nlq_pilot)[lev];
      f_var = (lev == 0) ? variance_Ysum_static(
                               (*static_sum_Ql).at(1)[0][cur_qoi],
                               (*static_sum_Ql).at(2)[0][cur_qoi], Nlq_pilot,
                               Nlq, compute_gradient, cur_grad_var)
                         : variance_Qsum_static(
                               (*static_sum_Ql).at(1)[lev][cur_qoi],
                               (*static_sum_Qlm1).at(1)[lev][cur_qoi],
                               (*static_sum_Ql).at(2)[lev][cur_qoi],
                               (*static_sum_QlQlm1).at(pr11)[lev][cur_qoi],
                               (*static_sum_Qlm1).at(2)[lev][cur_qoi],
                               Nlq_pilot, Nlq, compute_gradient, cur_grad_var);
      f_mean += f_var / Nlq;
...
void NonDMultilevelSampling::target_var_constraint_eval_optpp_problem18(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
  bool compute_gradient = false;
  ...
  if (compute_gradient) {
    Hl1_deriv = -1. / (Nlq * Nlq);
    Hl2_deriv = ((Nlq * (Nlq - 1.)) - (Nlq - 3.) * (2. * Nlq - 1)) /
                (std::pow(Nlq * (Nlq - 1.), 2));
    grad_g[0][lev] = 1. / var_of_var_l0 * (Hl1_deriv * C1 - Hl2_deriv * C2);
  }
  ...
  Real Cost1H =
      std::pow(0.5, 12) / 13. * Ax_squared - Ax_squared * var_H_squared;
  Real Cost2H = Ax_squared * var_H_squared;
  Real var_of_var_l1 = Hl1 * C1 - Hl2 * C2 - 2. * Cost1H / Nlq -
                       2. * Cost2H / (Nlq * (Nlq - 1.));
  if (compute_gradient) {
    Hl1_deriv = -1. / (Nlq * Nlq);
    Hl2_deriv = ((Nlq * (Nlq - 1.)) - (Nlq - 3.) * (2. * Nlq - 1)) /
    ...
    grad_g[0][lev] =
        1. / var_of_var_l1 *
        (Hl1_deriv * C1 - Hl2_deriv * C2 - 2. * Cost1H * (-1 / (Nlq * Nlq)) -
...
void NonDMultilevelSampling::target_sigma_constraint_eval_optpp_problem18(
    int mode, int n, const RealVector& x, RealVector& g, RealMatrix& grad_g,
    int& result_mode) {
  bool compute_gradient = false;
  ...
  if (compute_gradient) {
    ...
    Hl2_deriv = ((Nlq * (Nlq - 1.)) - (Nlq - 3.) * (2. * Nlq - 1)) /
                (std::pow(Nlq * (Nlq - 1.), 2));
    grad_g[0][lev] =
        1. / (4. * var_H * var_H) * ((Hl1_deriv * C1 - Hl2_deriv * C2) * var_H);
  }
...
double NonDMultilevelSampling::exact_var_of_var_problem18(
    const RealVector& Nl) {
  Real Ax = *static_Ax;
  Real var_L = *static_var_L;
  Real var_H = *static_var_H;
...
double NonDMultilevelSampling::exact_var_of_sigma_problem18(
    const RealVector& Nl) {
  Real Ax = *static_Ax;
  Real var_L = *static_var_L;
  Real var_H = *static_var_H;
