...
NonDC3FunctionTrain::NonDC3FunctionTrain(ProblemDescDB& problem_db,
  ...
  short u_space_type =
      PARTIAL_ASKEY_U;  // probDescDB.get_short("method.nond.expansion_type");
  ...
  uSpaceModel = std::make_shared<DataFitSurrModel>(
      u_space_sampler, g_u_model, ft_set, ft_view, approx_type, start_orders,
...
NonDC3FunctionTrain::NonDC3FunctionTrain(unsigned short method_name,
...
size_t NonDC3FunctionTrain::regression_size() {
  ...
  return SharedC3ApproxData::regression_size(
      numContinuousVars, regress_r, maxRankSpec, regress_o, maxOrderSpec);
}
...
void NonDC3FunctionTrain::resolve_refinement() {
  ...
  switch (refineType) {
    case Pecos::P_REFINEMENT:
      switch (refineControl) {
        case Pecos::UNIFORM_CONTROL:  // only uniform p-refine supported at this
                                      // time
          c3AdvancementType = probDescDB.get_short(
              "method.nond.c3function_train.advancement_type");
          switch (c3AdvancementType) {
            case START_ORDER_ADVANCEMENT:  // use with adapt_order is a poor
                                           // choice
              if (adapt_o) rm_adapt_err = true;
              break;
            case START_RANK_ADVANCEMENT:  // use with adapt_rank is a poor
                                          // choice
            ...
            case MAX_ORDER_ADVANCEMENT:  // require adapt_order
              ...
              break;
            case MAX_RANK_ORDER_ADVANCEMENT:  // require adapt_rank +
                                              // adapt_order
              if (!adapt_r || !adapt_o) add_adapt_err = true;
              break;
            case NO_C3_ADVANCEMENT:
              refine_err = true;
              break;  // no default assign since spec is reqd
          }
          break;
        default:
          refine_err = true;
  ...
  if (add_adapt_err)
    Cerr << "Error: adapt_rank/adapt_order specification required for C3 "
         << "advancement type " << c3AdvancementType << "." << std::endl;
  if (refine_err || rm_adapt_err || add_adapt_err) abort_handler(METHOD_ERROR);
}

void NonDC3FunctionTrain::resolve_inputs(short& u_space_type,
                                         short& data_order) {
...
bool NonDC3FunctionTrain::config_regression(
    size_t colloc_pts, size_t regress_size, int seed,
  ...
  if (probDescDB.get_bool("method.nond.tensor_grid")) {
    ...
    RealVector dim_pref;
    Pecos::anisotropic_order_to_dimension_preference(dim_quad_order, quad_order,
                                                     dim_pref);
...
void NonDC3FunctionTrain::initialize_c3_start_orders(
    const UShortArray& start_orders) {
...
void NonDC3FunctionTrain::initialize_c3_db_options() {
  ...
  shared_data_rep->set_parameter(
      "regress_type", probDescDB.get_short("method.nond.regression_type"));
  shared_data_rep->set_parameter(
  ...
  shared_data_rep->set_parameter(
      "response_scaling", probDescDB.get_bool("method.nond.response_scaling"));

  short comb_type =
      Pecos::ADD_COMBINE;  // for now; pass short (enum = ambiguous)
  ...
  shared_data_rep->set_parameter(
      "max_cv_rank",
      probDescDB.get_sizet("method.nond.cross_validation.max_rank_candidates"));
  shared_data_rep->set_parameter(
      "max_cv_order", probDescDB.get_ushort(
                          "method.nond.cross_validation.max_order_candidates"));
  // shared_data_rep->infer_max_cross_validation_ranges();
}

void NonDC3FunctionTrain::push_c3_start_orders(
    const UShortArray& start_orders) {
  // These are passed in since they may be a scalar or part of a sequence:
  std::shared_ptr<SharedC3ApproxData> shared_data_rep =
      std::static_pointer_cast<SharedC3ApproxData>(
...
void NonDC3FunctionTrain::update_samples_from_order_increment() {
  ...
  // rebuild_global())
  // prevSamplesOnModel = numSamplesOnModel;//requires level mgmt for
  // persistence
...
void NonDC3FunctionTrain::sample_allocation_metric(Real& regress_metric,
                                                   Real power) {
  ...
  switch (c3AdvancementType) {
    ...
    // candidates are generated when max rank and/or max order are active
    // bounds.
    case MAX_RANK_ADVANCEMENT:  // incl. refine{Type,Control},adaptRank
                                // dependency
      regress_metric = shared_data_rep->max_rank_regression_size();
      break;
    case MAX_ORDER_ADVANCEMENT:  // incl. refine{Type,Control},adaptOrder
                                 // dependency
      regress_metric = shared_data_rep->max_order_regression_size();
      break;
    case MAX_RANK_ORDER_ADVANCEMENT:  // includes refine{T,C},adapt{R,O}
                                      // dependency
      regress_metric = shared_data_rep->max_regression_size();
      break;
    // Default approach increments samples based on recovered rank/order after
    // CV:
    default: {
      ...
      for (size_t qoi = 0; qoi < numFunctions; ++qoi) {
        ...
        if (outputLevel >= DEBUG_OUTPUT)
          Cout << "System size(" /*lev " << lev << ", "*/ << "qoi "
               << qoi
               /* << ", iter " << iter */
               << ") = " << regress_q << '\n';
        if (pow_inf) {
          if (regress_q > max) max = regress_q;
        } else
...
void NonDC3FunctionTrain::print_moments(std::ostream& s) {
  ...
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    s << ' ' << std::setw(width) << moments[0] << ' ' << std::setw(width)
      << sqrt(moments[1]) << ' ' << std::setw(width)
      << poly_approx_i->skewness() << ' ' << std::setw(width)
      << poly_approx_i->kurtosis() - 3.;
    s << '\n';
  }
}
...
void NonDC3FunctionTrain::print_sobol_indices(std::ostream& s) {
  ...
  for (size_t i = 0; i < numFunctions; ++i) {
    ...
    for (size_t j = 0; j < numContinuousVars; ++j)
      s << "                     " << std::setw(wpp7)
        << poly_approx_i->main_sobol_index(j) / var << ' ' << std::setw(wpp7)
        << poly_approx_i->total_sobol_index(j) << ' ' << cv_labels[j] << '\n';
