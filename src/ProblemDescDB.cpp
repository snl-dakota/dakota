...
static const char rcsId[] =
    "@(#) $Id: ProblemDescDB.cpp 7007 2010-10-06 15:54:39Z wjbohnh $";
...
ProblemDescDB::ProblemDescDB(BaseConstructor, int world_size, int world_rank)
    : environmentCntr(0),
      ...
      worldRank(world_rank) { /* empty ctor */ }
...
std::shared_ptr<ProblemDescDB> ProblemDescDB::get_db(int world_size,
                                                     int world_rank) {
...
ProblemDescDB::~ProblemDescDB() {
  if (this == Dak_pddb) Dak_pddb = NULL;
  if (dbRep.use_count() == 1) delete_study_components(*this);
}
...
/** DB setup phase 1: parse the input file and execute callback
    ...
    Rank 0 only. */
void ProblemDescDB::parse_inputs(const std::string_view input_string,
                                 const std::string_view parser_options,
                                 bool command_line_run,
                                 DbCallbackFunctionPtr callback,
                                 void* callback_data) {
  if (dbRep) {
    dbRep->parse_inputs(input_string, parser_options, command_line_run,
    ...
    if (callback && dbRep->worldRank == 0) (*callback)(this, callback_data);
  } else {
...
void ProblemDescDB::check_and_broadcast(const UserModes& user_modes) {
  if (dbRep)
    ...
    if (worldRank == 0) check_input(user_modes);
...
void ProblemDescDB::derived_parse_inputs(const std::string_view dakota_input,
                                         const std::string_view parser_options,
                                         bool command_line_run) {
  if (dbRep)
...
void ProblemDescDB::check_input(const UserModes& user_modes) {
  if (dbRep)
    ...
    if (num_errors) {
      Cerr << num_errors << " input specification errors detected."
           << std::endl;
...
void ProblemDescDB::set_db_list_nodes(const String& method_tag) {
  if (dbRep) dbRep->set_db_list_nodes(method_tag);
...
void ProblemDescDB::set_db_method_node(const String& method_tag) {
  if (dbRep) dbRep->set_db_method_node(method_tag);
  // for simplicity in client logic, allow NO_SPECIFICATION case to fall
      ...
      if (dataMethodList.size() == 1)  // no ambiguity if only one spec
        ...
        if (dataMethodIter == dataMethodList.end()) {
          ...
          --dataMethodIter;  // last entry in list
                   ...
                   std::count_if(
                       dataMethodList.begin(), dataMethodList.end(),
...
void ProblemDescDB::set_db_model_nodes(const String& model_tag) {
  if (dbRep) dbRep->set_db_model_nodes(model_tag);
  // for simplicity in client logic, allow NO_SPECIFICATION case to fall
  ...
  // within a recursion do not alter list node sequencing).
    ...
    if (model_tag.empty() ||
        model_tag == "NO_MODEL_ID") {  // no pointer specification
      ...
      if (dataModelList.size() == 1)  // no ambiguity if only one spec
        ...
        if (dataModelIter == dataModelList.end()) {
          ...
          --dataModelIter;  // last entry in list
                   ...
                   std::count_if(
                       dataModelList.begin(), dataModelList.end(),
...
void ProblemDescDB::set_db_variables_node(const String& variables_tag) {
  if (dbRep) dbRep->set_db_variables_node(variables_tag);
  // for simplicity in client logic, allow NO_SPECIFICATION case to fall
  ...
  // instances within a recursion do not alter list node sequencing).
    ...
    if (variables_tag.empty()) {          // no pointer specification
      if (dataVariablesList.size() == 1)  // no ambiguity if only one spec
        ...
        dataVariablesIter = std::find_if(
            dataVariablesList.begin(), dataVariablesList.end(),
      ...
      std::list<DataVariables>::iterator dv_it = std::find_if(
          dataVariablesList.begin(), dataVariablesList.end(),
          boost::bind(DataVariables::id_compare, _1, variables_tag));
...
void ProblemDescDB::set_db_interface_node(const String& interface_tag) {
  if (dbRep) dbRep->set_db_interface_node(interface_tag);
  // for simplicity in client logic, allow NO_SPECIFICATION case to fall
  ...
  // instances within a recursion do not alter list node sequencing).
    ...
    if (interface_tag.empty() ||
        interface_tag == "NO_ID") {       // no pointer specification
      if (dataInterfaceList.size() == 1)  // no ambiguity if only one spec
        ...
        dataInterfaceIter = std::find_if(
            dataInterfaceList.begin(), dataInterfaceList.end(),
        ...
        if (dataInterfaceIter == dataInterfaceList.end()) {
          if (worldRank == 0 && MoRep.modelType == "simulation")
        ...
        } else if (worldRank == 0 && MoRep.modelType == "simulation" &&
      ...
      std::list<DataInterface>::iterator di_it = std::find_if(
          dataInterfaceList.begin(), dataInterfaceList.end(),
          boost::bind(DataInterface::id_compare, _1, interface_tag));
...
void ProblemDescDB::set_db_responses_node(const String& responses_tag) {
  if (dbRep) dbRep->set_db_responses_node(responses_tag);
  // for simplicity in client logic, allow NO_SPECIFICATION case to fall
  ...
  // instances within a recursion do not alter list node sequencing).
    ...
    if (responses_tag.empty()) {          // no pointer specification
      if (dataResponsesList.size() == 1)  // no ambiguity if only one spec
        ...
        dataResponsesIter = std::find_if(
            dataResponsesList.begin(), dataResponsesList.end(),
      ...
      std::list<DataResponses>::iterator dr_it = std::find_if(
          dataResponsesList.begin(), dataResponsesList.end(),
          boost::bind(DataResponses::id_compare, _1, responses_tag));
...
int ProblemDescDB::max_procs_per_ea() {
  ...
  return max_procs_per_level(
      ...
      std::max(1,
               (int)get_sa("interface.application.analysis_drivers").size()));
}
...
int ProblemDescDB::max_procs_per_ie(int max_eval_concurrency) {
  ...
  return max_procs_per_level(
      max_ea, ppe_spec, get_int("interface.evaluation_servers"),
      get_short("interface.evaluation_scheduling"),
...
static void Bad_name(const String& entry_name, const String& where) {
  Cerr << "\nBad entry_name '" << entry_name << "' in ProblemDescDB::" << where
       << std::endl;
  abort_handler(PARSE_ERROR);
}
...
std::pair<std::string, std::string> split_entry_name(
    const std::string& entry_name, const std::string& context_msg) {
  ...
  const std::string entry =
      entry_name.substr(first_dot + 1, entry_name.size() - first_dot - 1);
...
T& ProblemDescDB::get(
  ...
  if (!db_rep) Null_rep(context_msg);
...
const RealMatrixArray& ProblemDescDB::get_rma(const String& entry_name) const {
  return get<RealMatrixArray>(
      "get_rma()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"discrete_design_set_int.adjacency_matrix",
        P_VAR discreteDesignSetIntAdj},
       {"discrete_design_set_real.adjacency_matrix",
        P_VAR discreteDesignSetRealAdj},
       {"discrete_design_set_str.adjacency_matrix",
        P_VAR discreteDesignSetStrAdj}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
}
...
const RealVector& ProblemDescDB::get_rv(const String& entry_name) const {
  return get<const RealVector>(
      ...
      {/* variables */
       ...
       {"discrete_design_set_real.initial_point",
        P_VAR discreteDesignSetRealVars},
       ...
       {"discrete_state_set_real.initial_state",
        P_VAR discreteStateSetRealVars},
  ...
  return get<const IntVector>(
      ...
      {/* variables */
       ...
       {"discrete_design_range.lower_bounds",
        P_VAR discreteDesignRangeLowerBnds},
       {"discrete_design_range.upper_bounds",
        P_VAR discreteDesignRangeUpperBnds},
       {"discrete_design_set_int.initial_point",
        P_VAR discreteDesignSetIntVars},
       ...
       {"discrete_state_set_int.lower_bounds",
        P_VAR discreteStateSetIntLowerBnds},
       {"discrete_state_set_int.upper_bounds",
        P_VAR discreteStateSetIntUpperBnds},
       ...
       {"hypergeometric_uncertain.total_population",
        P_VAR hyperGeomUncTotalPop},
       {"negative_binomial_uncertain.num_trials",
        P_VAR negBinomialUncNumTrials}},
      {/* interface */},
      {/* responses */
...
const BitArray& ProblemDescDB::get_ba(const String& entry_name) const {
  return get<const BitArray>(
      "get_ba()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       ...
       {"discrete_interval_uncertain.categorical",
        P_VAR discreteIntervalUncCat},
...
const SizetArray& ProblemDescDB::get_sza(const String& entry_name) const {
  return get<const SizetArray>(
      ...
      {/* model */}, {/* variables */}, {/* interface */}, {/* responses */},
      entry_name, dbRep);
}
...
const RealSymMatrix& ProblemDescDB::get_rsm(const String& entry_name) const {
  return get<const RealSymMatrix>(
      "get_rsm()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
...
const IntVectorArray& ProblemDescDB::get_iva(const String& entry_name) const {
  ...
  return get<const IntVectorArray>("get_iva()", {/* environment */},
                                   {/* method */}, {/* model */},
                                   {/* variables */}, {/* interface */},
                                   {/* responses */}, entry_name, dbRep);
}

const IntSet& ProblemDescDB::get_is(const String& entry_name) const {
  return get<const IntSet>(
      "get_is()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */}, {/* interface */},
      {/* responses */
...
const IntSetArray& ProblemDescDB::get_isa(const String& entry_name) const {
  return get<const IntSetArray>(
      "get_isa()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
...
const RealSetArray& ProblemDescDB::get_rsa(const String& entry_name) const {
  return get<const RealSetArray>(
      "get_rsa()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
...
const IntRealMapArray& ProblemDescDB::get_irma(const String& entry_name) const {
  return get<const IntRealMapArray>(
      "get_irma()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"discrete_uncertain_set_int.values_probs",
        P_VAR discreteUncSetIntValuesProbs},
       {"histogram_uncertain.point_int_pairs",
        P_VAR histogramUncPointIntPairs}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
}

const StringRealMapArray& ProblemDescDB::get_srma(
    const String& entry_name) const {
  return get<const StringRealMapArray>(
      "get_srma()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"discrete_uncertain_set_string.values_probs",
        P_VAR discreteUncSetStrValuesProbs},
       {"histogram_uncertain.point_string_pairs",
        P_VAR histogramUncPointStrPairs}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
}

const RealRealMapArray& ProblemDescDB::get_rrma(
    const String& entry_name) const {
  return get<const RealRealMapArray>(
      "get_rrma()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       ...
       {"histogram_uncertain.point_real_pairs",
        P_VAR histogramUncPointRealPairs}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
}

const RealRealPairRealMapArray& ProblemDescDB::get_rrrma(
    const String& entry_name) const {
  return get<const RealRealPairRealMapArray>(
      "get_rrrma()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
...
const IntIntPairRealMapArray& ProblemDescDB::get_iirma(
    const String& entry_name) const {
  return get<const IntIntPairRealMapArray>(
      "get_iirma()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
...
const StringArray& ProblemDescDB::get_sa(const String& entry_name) const {
  return get<const StringArray>(
      ...
      {/* variables */
       {"continuous_aleatory_uncertain.labels",
        P_VAR continuousAleatoryUncLabels},
       ...
       {"discrete_design_set_string.initial_point",
        P_VAR discreteDesignSetStrVars},
       {"discrete_design_set_string.labels", P_VAR discreteDesignSetStrLabels},
       {"discrete_design_set_string.lower_bounds",
        P_VAR discreteDesignSetStrLowerBnds},
       {"discrete_design_set_string.upper_bounds",
        P_VAR discreteDesignSetStrUpperBnds},
       ...
       {"discrete_state_set_string.initial_state",
        P_VAR discreteStateSetStrVars},
       {"discrete_state_set_string.labels", P_VAR discreteStateSetStrLabels},
       {"discrete_state_set_string.lower_bounds",
        P_VAR discreteStateSetStrLowerBnds},
       {"discrete_state_set_string.upper_bounds",
        P_VAR discreteStateSetStrUpperBnds},
       {"discrete_uncertain_set_string.initial_point",
        P_VAR discreteUncSetStrVars},
...
const String2DArray& ProblemDescDB::get_s2a(const String& entry_name) const {
  return get<const String2DArray>(
      "get_s2a()", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */},
      {/* interface */
...
const String& ProblemDescDB::get_string(const String& entry_name) const {
  return get<const String>(
      ...
      {/* method */
       ...
       {"nond.posterior_density_export_file",
        P_MET posteriorDensityExportFilename},
       {"nond.posterior_samples_export_file",
        P_MET posteriorSamplesExportFilename},
       {"nond.posterior_samples_import_file",
        P_MET posteriorSamplesImportFilename},
      ...
      {/* model */
       ...
       {"optional_interface_responses_pointer",
        P_MOD optionalInterfRespPointer},
       ...
       {"surrogate.export_approx_variance_file",
        P_MOD exportApproxVarianceFile},
...
const Real& ProblemDescDB::get_real(const String& entry_name) const {
  return get<const Real>(
      ...
      {/* method */
       ...
       {"nond.mlmcmc_greedy_resampling_factor",
        P_MET mlmcmcGreedyResamplingFactor},
      ...
      {/* model */
       ...
       {"adapted_basis.truncation_tolerance",
        P_MOD adaptedBasisTruncationTolerance},
...
int ProblemDescDB::get_int(const String& entry_name) const {
  return get<int>(
      ...
      {/* interface */
       ...
       {"asynch_local_analysis_concurrency",
        P_INT asynchLocalAnalysisConcurrency},
       {"asynch_local_evaluation_concurrency",
        P_INT asynchLocalEvalConcurrency},
...
short ProblemDescDB::get_short(const String& entry_name) const {
  return get<short>(
      ...
      {/* method */
       ...
       {"nond.multilevel_discrepancy_emulation",
        P_MET multilevDiscrepEmulation},
...
unsigned short ProblemDescDB::get_ushort(const String& entry_name) const {
  return get<unsigned short>(
      ...
      {/* model */
       ...
       {"surrogate.export_approx_variance_format",
        P_MOD exportApproxVarianceFormat},
...
size_t ProblemDescDB::get_sizet(const String& entry_name) const {
  ...
  if (block == "variables") {
    if (!dbRep) Null_rep("get_sizet()");
    if (dbRep->variablesDBLocked) Locked_db();
  ...
  return get<size_t>(
      ...
      {/* responses */
       ...
       {"num_nonlinear_inequality_constraints",
        P_RES numNonlinearIneqConstraints},
...
bool ProblemDescDB::get_bool(const String& entry_name) const {
  return get<bool>(
      ...
      {/* method */
       ...
       {"nond.allocation_target.optimization",
        P_MET useTargetVarianceOptimizationFlag},
      ...
      {/* model */
       ...
       {"active_subspace.truncation_method.constantine",
        P_MOD subspaceIdConstantine},
       ...
       {"surrogate.challenge_use_variable_labels",
        P_MOD importChalUseVariableLabels},
...
void** ProblemDescDB::get_voidss(const String& entry_name) const {
  if (entry_name == "method.dl_solver.dlLib") {
    if (dbRep->methodDBLocked) Locked_db();
...
void ProblemDescDB::set(const String& entry_name, const BitArray& ba) {
  BitArray& rep_ba = get<BitArray>(
      "set(BitArray&)", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"binomial_uncertain.categorical", P_VAR binomialUncCat},
       {"discrete_design_range.categorical", P_VAR discreteDesignRangeCat},
       {"discrete_design_set_int.categorical", P_VAR discreteDesignSetIntCat},
...
void ProblemDescDB::set(const String& entry_name, const RealSymMatrix& rsm) {
  RealSymMatrix& rep_rsm = get<RealSymMatrix>(
      "set(RealSymMatrix&)", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"uncertain.correlation_matrix", P_VAR uncertainCorrelations}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
...
void ProblemDescDB::set(const String& entry_name, const RealVectorArray& rva) {
  RealVectorArray& rep_rva = get<RealVectorArray>(
      "set(RealVectorArray&)", {/* environment */},
      {/* method */
       {"nond.gen_reliability_levels", P_MET genReliabilityLevels},
       {"nond.probability_levels", P_MET probabilityLevels},
       {"nond.reliability_levels", P_MET reliabilityLevels},
...
void ProblemDescDB::set(const String& entry_name, const IntVectorArray& iva) {
  IntVectorArray& rep_iva = get<IntVectorArray>(
      "set(IntVectorArray&)", {/* environment */}, {/* method */},
      {/* model */}, {/* variables */}, {/* interface */}, {/* responses */},
      entry_name, dbRep);
...
void ProblemDescDB::set(const String& entry_name, const IntSetArray& isa) {
  IntSetArray& rep_isa = get<IntSetArray>(
      "set(IntSetArray&)", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"discrete_design_set_int.values", P_VAR discreteDesignSetInt},
       {"discrete_state_set_int.values", P_VAR discreteStateSetInt}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
...
void ProblemDescDB::set(const String& entry_name, const RealSetArray& rsa) {
  RealSetArray& rep_rsa = get<RealSetArray>(
      "set(RealSetArray&)", {/* environment */}, {/* method */}, {/* model */},
      {/* variables */
       {"discrete_design_set_real.values", P_VAR discreteDesignSetReal},
       {"discrete_state_set_real.values", P_VAR discreteStateSetReal}},
      {/* interface */}, {/* responses */}, entry_name, dbRep);
...
void ProblemDescDB::set(const String& entry_name, const IntRealMapArray& irma) {
  IntRealMapArray& rep_irma = get<IntRealMapArray>(
      "set(IntRealMapArray&)", {/* environment */}, {/* method */},
      {/* model */},
      {/* variables */
       {"discrete_uncertain_set_int.values_probs",
        P_VAR discreteUncSetIntValuesProbs},
...
void ProblemDescDB::set(const String& entry_name,
                        const StringRealMapArray& srma) {
  StringRealMapArray& rep_srma = get<StringRealMapArray>(
      "set(StringRealMapArray&)", {/* environment */}, {/* method */},
      {/* model */},
      {/* variables */
       {"histogram_uncertain.point_string_pairs",
        P_VAR histogramUncPointStrPairs}},
...
void ProblemDescDB::set(const String& entry_name,
  ...
  RealRealMapArray& rep_rrma = get<RealRealMapArray>(
      "set(RealRealMapArray&)", {/* environment */}, {/* method */},
      {/* model */},
      {/* variables */
       {"discrete_uncertain_set_real.values_probs",
        P_VAR discreteUncSetRealValuesProbs}},
...
void ProblemDescDB::set(const String& entry_name,
                        const RealRealPairRealMapArray& rrrma) {
  RealRealPairRealMapArray& rep_rrrma = get<RealRealPairRealMapArray>(
      "set(RealRealPairRealMapArray&)", {/* environment */}, {/* method */},
      {/* model */},
      {/* variables */
       {"continuous_interval_uncertain.basic_probs",
        P_VAR continuousIntervalUncBasicProbs}},
...
void ProblemDescDB::set(const String& entry_name,
                        const IntIntPairRealMapArray& iirma) {
  IntIntPairRealMapArray& rep_iirma = get<IntIntPairRealMapArray>(
      "set(IntIntPairRealMapArray&)", {/* environment */}, {/* method */},
      {/* model */},
      {/* variables */
       {"discrete_interval_uncertain.basic_probs",
        P_VAR discreteIntervalUncBasicProbs}},
...
void ProblemDescDB::set(const String& entry_name, const StringArray& sa) {
  StringArray& rep_sa = get<StringArray>(
      "set(StringArray&)", {/* environment */}, {/* method */},
      {/* model */
...
void ProblemDescDB::enforce_unique_ids() {
  ...
  if (found_error) abort_handler(PARSE_ERROR);
}
