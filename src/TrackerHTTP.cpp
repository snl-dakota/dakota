...
TrackerHTTP::TrackerHTTP()
    : curlPtr(NULL),
      devNull(NULL),
      timeoutSeconds(2),
      dakotaVersion("unknown"),
      outputLevel(TH_SILENT_OUTPUT) { /* no-op */ }
...
void TrackerHTTP::url_add_field(std::string& url, const char* keyword,
                                const std::string& value, bool delimit) const {
