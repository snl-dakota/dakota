...
static const char rcsId[] =
    "@(#) $Id: NonDAdaptImpSampling.cpp 4058 2006-10-26 01:39:40Z lpswile $";
...
NonDAdaptImpSampling::NonDAdaptImpSampling(ProblemDescDB& problem_db,
...
NonDAdaptImpSampling::NonDAdaptImpSampling(
    std::shared_ptr<Model> model, unsigned short sample_type,
    int refine_samples, int refine_seed, const String& rng, bool vary_pattern,
    unsigned short is_type, bool cdf_flag, bool x_space_model,
    bool use_model_bounds, bool track_extreme)
    : NonDSampling(IMPORTANCE_SAMPLING, model, sample_type, 0, refine_seed, rng,
                   vary_pattern,
                   ALEATORY_UNCERTAIN),  // only sample aleatory vars
      importanceSamplingType(is_type),
      initLHS(false),
      useModelBounds(use_model_bounds),
      invertProb(false),
      trackExtremeValues(track_extreme),
  ...
  if (refineSamples) maxEvalConcurrency *= refineSamples;
}
...
void NonDAdaptImpSampling::derived_init_communicators(ParLevLIter pl_iter) {
  ...
  if (initLHS) uSpaceModel->init_communicators(pl_iter, numSamples);
...
void NonDAdaptImpSampling::derived_free_communicators(ParLevLIter pl_iter) {
  ...
  if (initLHS) uSpaceModel->free_communicators(pl_iter, numSamples);
}

/** Initializes data using a vector array of starting points. */
void NonDAdaptImpSampling::initialize(const RealVectorArray& acv_points,
                                      bool x_space_data, size_t resp_index,
                                      Real initial_prob,
  ...
  if (startCAUV) {  // store inactive variable values, if present
    ...
    for (i = 0; i < startCAUV; i++) designPoint[i] = acv_pt_0[i];
  }
...
void NonDAdaptImpSampling::initialize(const RealMatrix& acv_points,
                                      bool x_space_data, size_t resp_index,
                                      Real initial_prob,
                                      Real failure_threshold) {
...
void NonDAdaptImpSampling::initialize(const RealVector& acv_point,
                                      bool x_space_data, size_t resp_index,
                                      Real initial_prob,
                                      Real failure_threshold) {
  size_t j, cntr;
  ...
  if (startCAUV) {  // store inactive variable values, if present
    designPoint.sizeUninitialized(startCAUV);
    for (j = 0; j < startCAUV; ++j) designPoint[j] = acv_point[j];
  }
...
void NonDAdaptImpSampling::core_run() {
  ...
  if (initLHS) {
    ...
    for (resp_fn_count = 0; resp_fn_count < numFunctions; resp_fn_count++) {
      ...
      size_t i;
      IntRespMIter it;
      for (i = 0, it = allResponses.begin(); it != allResponses.end();
           ++i, ++it)
...
void NonDAdaptImpSampling::select_rep_points(
    const RealVectorArray& var_samples_u, const RealVector& fn_samples) {
  ...
  RealArray fail_betas;
  fail_betas.reserve(num_samples);
  SizetArray fail_indices;
  fail_indices.reserve(num_samples);
  RealArray safe_dist;
  safe_dist.reserve(num_samples);
  SizetArray safe_indices;
  safe_indices.reserve(num_samples);
  ...
  for (i = 0; i < new_rep_pts; ++i) {
    size_t idx = min_indx[i],
           pt_idx = (fail_count > 0) ? fail_indices[idx] : safe_indices[idx];
    // TMW: Modified this to exclude the previous repPointsU
...
void NonDAdaptImpSampling::converge_statistics(bool cov_flag) {
  ...
  size_t total_samples = 0,
         max_iter =
             (maxIterations == SZ_MAX) ? 100 : maxIterations,  // default to 100
  ...
  while (!converged && total_samples < max_samples) {
    ...
    if (importanceSamplingType == IS)  // for IS, only perform one iteration
      ...
      if (p > 0. && p < 1. && old_p > 0. && old_p < 1. &&
        ...
        p_converged = true;
      // else if (p > 0. && p < 1. && std::abs(p - old_p) < p_abs_tol)//
      // absolute
      //   p_converged = true;
...
void NonDAdaptImpSampling::generate_samples(RealVectorArray& var_samples_u) {
  ...
  if (useModelBounds) {
    ...
    const RealVector& c_l_bnds =
        ModelUtils::continuous_lower_bounds(*uSpaceModel);
    const RealVector& c_u_bnds =
        ModelUtils::continuous_upper_bounds(*uSpaceModel);
    for (i = 0; i < numCAUV; ++i) {
    ...
    for (i = 0, j = startCAUV; i < numCAUV; ++i, ++j) {
      ...
      n_l_bnds[i] = u_bnds.first;
      n_u_bnds[i] = u_bnds.second;
    }
  }
  ...
  for (i = 0, cntr = 0; i < num_rep_pts; ++i) {
    ...
    if (num_rep_samples) {
      ...
      samplerDriver->generate_normal_samples(
          repPointsU[i], n_std_devs, n_l_bnds, n_u_bnds, correl,
          num_rep_samples, lhs_samples_array);
...
void NonDAdaptImpSampling::evaluate_samples(
    const RealVectorArray& var_samples_u, RealVector& fn_samples) {
  ...
  set.request_value(1, respFnIndex);
...
void NonDAdaptImpSampling::calculate_statistics(
    const RealVectorArray& var_samples_u, const RealVector& fn_samples,
    size_t total_samples, Real& sum_prob, Real& prob, bool compute_cov,
  ...
  if (compute_cov) failure_ratios.reserve(batch_size);
  ...
  for (i = 0; i < batch_size; i++) {
    ...
    if ((fn_samples[i] < failThresh &&
      ...
      if (compute_cov) failure_ratios.push_back(pdf_ratio);
    }
  }
...
Real NonDAdaptImpSampling::recentered_density(
    const RealVector& sample_point, const RealRealPairArray& cauv_u_bnds) {
  ...
  for (i = 0; i < num_rep_pts; ++i) {
    ...
    for (j = 0; j < numCAUV; ++j) {
      ...
      rep_pdf *= Pecos::BoundedNormalRandomVariable::pdf(
          sample_point[j], rep_pt_i[j], stdev, bnds_j.first, bnds_j.second);
    }
