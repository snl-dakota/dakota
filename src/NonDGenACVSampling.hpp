...
class NonDGenACVSampling : public NonDACVSampling {
 ...
 private:
  ...
  /// create an ordered list of roots that enable ordered sample increments
  /// by ensuring that root sample levels are defined
  void unroll_reverse_dag_from_root(unsigned short root, UShortArray& group);
  /// create an ordered list of roots that enable ordered sample increments
  ...
  */
  void compute_C_G_c_g(const RealSymMatrix& C, const RealSymMatrix& G,
                       ...
                       const UShortArray& approx_set, RealSymMatrix& C_G,
                       RealVector& c_g);
...
inline Real NonDGenACVSampling::inactive_budget_deduction() const {
  ...
  for (size_t approx = 0; approx < numApprox; ++approx)
    if (cntr < num_approx && approx == approx_set[cntr])
      ++cntr;  // ordered seq
    else
      deduct += sequenceCost[approx] * NApproxAlloc[approx] / cost_H;
...
inline const MFSolutionData& NonDGenACVSampling::final_solution_data() const {
  std::pair<UShortArray, UShortArray> key(activeModelSetIter->first,
                                          *activeDAGIter);
  return dagSolns.at(key);
}

inline void NonDGenACVSampling::print_variance_reduction(
    std::ostream& s) const {
...
inline void NonDGenACVSampling::inflate_approx_set(
    const UShortArray& approx_set, SizetArray& index_map) {
...
inline void NonDGenACVSampling::inflate_variables(
    const RealVector& cd_vars, RealVector& N_vec,
    const UShortArray& approx_set) {
  size_t i, num_approx = approx_set.size(), num_cdv = cd_vars.length();
  if (N_vec.length() == numGroups)
    N_vec = 0.;
  else
    N_vec.size(numGroups);
  for (i = 0; i < num_approx; ++i) N_vec[approx_set[i]] = cd_vars[i];
...
inline void NonDGenACVSampling::compute_C_G_c_g(
    const RealSymMatrix& C, const RealSymMatrix& G, const RealMatrix& c,
    const RealVector& g, size_t qoi, const UShortArray& approx_set,
    RealSymMatrix& C_G, RealVector& c_g) {
  size_t i, j, n = G.numRows();
  unsigned short approx_i;
  C_G.shapeUninitialized(n);
  c_g.sizeUninitialized(n);
  for (i = 0; i < n; ++i) {
...
inline Real NonDGenACVSampling::solve_for_triple_product(
    const RealSymMatrix& C, const RealSymMatrix& G, const RealMatrix& c,
    const RealVector& g, size_t qoi, const UShortArray& approx_set) {
  RealSymMatrix C_G;
  RealVector c_g, lhs;
  ...
  Real trip_prod = 0.;
  for (i = 0; i < n; ++i) trip_prod += c_g(i) * lhs(i);
...
inline Real NonDGenACVSampling::compute_R_sq(const RealSymMatrix& C,
                                             const RealSymMatrix& G,
                                             const RealMatrix& c,
                                             const RealVector& g, size_t qoi,
                                             const UShortArray& approx_set,
                                             Real var_H_q, Real N_H) {
  return solve_for_triple_product(C, G, c, g, qoi, approx_set) * N_H / var_H_q;
}

inline void NonDGenACVSampling::precompute_genacv_controls(
    const RealVector& soln_vars) {
...
inline void NonDGenACVSampling::solve_for_genacv_control(
    const RealSymMatrix& cov_LL, const RealSymMatrix& G,
    const RealMatrix& cov_LH, const RealVector& g, size_t qoi,
    const UShortArray& approx_set, RealVector& beta) {
  RealSymMatrix C_G;
  RealVector c_g;
...
inline void NonDGenACVSampling::compute_genacv_control(
    const RealMatrix& sum_L_m, Real sum_H_mq, const RealSymMatrix& sum_LL_mq,
    const RealMatrix& sum_LH_m, size_t N_shared_q, size_t mom, size_t qoi,
    const UShortArray& approx_set, RealVector& beta) {
  if (mom == 1)  // online|offline covariances available for mean
    solve_for_genacv_control(covLL[qoi], GMat, covLH, gVec, qoi, approx_set,
                             beta);
...
inline void NonDGenACVSampling::print_dag(const UShortArray& dag,
                                          const UShortArray& approx_set) const {
