...
#include "EmbeddedHybridStrategy.H"
...
static const char rcsId[]="@(#) $Id: DakotaStrategy.C 6749 2010-05-03 17:11:57Z briadam $";
  ...
  numIteratorJobs(1), maxConcurrency(1), strategyRep(NULL), referenceCount(1)
{
  ...
  if (db_write_precision > 0) {
    if (db_write_precision > 16) {
      Cout << "\nWarning: requested output_precision exceeds DAKOTA's internal "
...
/** Used in main.C instantiation to build the envelope.  This constructor only
