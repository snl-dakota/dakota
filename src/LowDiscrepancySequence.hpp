...
class LowDiscrepancySequence {
 public:
  ...
  LowDiscrepancySequence(int mMax, int dMax, int seedValue, short outputLevel)
      : mMax(mMax), dMax(dMax), seedValue(seedValue), outputLevel(outputLevel) {
  ...
  void get_points(RealMatrix& points) { get_points(points.numCols(), points); }
  ...
  void get_points(const size_t nMin, const size_t nMax, RealMatrix& points) {
    ...
    if (outputLevel >= Pecos::VERBOSE_OUTPUT) {
      Cout << "Successfully generated " << points.numCols()
           << " low-discrepancy points in " << points.numRows()
           << " dimensions:" << std::endl;
 ...
 protected:
  ...
  void check_sizes(const size_t nMin, const size_t nMax, RealMatrix& points) {
    ...
    if (dimension > dMax) {
      Cerr << "\nError: this low-discrepancy sequence can only generate "
           << "points in dimension " << dMax << " or less, got " << dimension
           << "." << std::endl;
      abort_handler(METHOD_ERROR);
    }
    ...
    if (numPoints != nMax - nMin) {
      Cerr << "\nError: requested low-discrepancy points between index " << nMin
  ...
  virtual void unsafe_get_points(const size_t nMin, const size_t nMax,
                                 RealMatrix& points) = 0;
