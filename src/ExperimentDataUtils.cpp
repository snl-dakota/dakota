...
void copy_field_data(const RealVector &fn_vals, RealMatrix &fn_grad,
  ...
  for (size_t i = 0; i < num_fns; i++) {
    ...
    if (asv[i] & 2) {
      ...
      resp_grad = 0.0;
      for (size_t j = 0; j < num_sm_cv; ++j) resp_grad[j] = fn_grad(j, i);
    }
    if (asv[i] & 4) {
...
void interpolate_simulation_field_data(const Response &sim_resp,
  ...
  if (total_asv & 2) sim_grads = sim_resp.field_gradients_view(field_num);

  RealSymMatrixArray sim_hessians;
  if (total_asv & 4) sim_hessians = sim_resp.field_hessians_view(field_num);
  ...
  linear_interpolate_1d(sim_coords, sim_vals, sim_grads, sim_hessians,
                        exp_coords, interp_vals, interp_grads, interp_hessians);

  size_t field_size = interp_vals.length();

  copy_field_data(interp_vals, interp_grads, interp_hessians,
                  interp_resp_offset, field_size, total_asv, interp_resp);
}
...
void linear_interpolate_1d(const RealMatrix &build_pts,
                           ...
                           const RealMatrix &pred_pts, RealVector &pred_vals,
  ...
  bool interp_grads =
      ((build_grads.numRows() > 0) && (build_grads.numCols() > 0));
  bool interp_hessians = (build_hessians.size() > 0);

  if ((interp_hessians) && (!interp_grads))
    throw(std::runtime_error(
        "Hessians were provided, but gradients were missing"));
  ...
  // The following is for when we active multivariate interpolation of field
  // data
  // if ( pred_pts.numCols()!=build_pts.numCols() )
  //  throw( std::runtime_error("build pts and pred pts must have the same
  //  number of columns") );
  ...
  if ((interp_grads) && ((pred_grads.numRows() != num_vars) ||
    ...
    if (pred_pts_1d[i] <= build_pts_1d[0]) {
      ...
      if (interp_hessians) {
        ...
        for (int k = 0; k < build_grads.numRows(); k++) {
          for (int j = 0; j <= k; j++) {
            pred_hessians[i](j, k) =
                build_hessians[index](j, k) +
                ((build_hessians[index + 1](j, k) -
                  build_hessians[index](j, k)) /
                 (build_pts_1d[index + 1] - build_pts_1d[index])) *
                    (pred_pts_1d[i] - build_pts_1d[index]);
          }
...
void CovarianceMatrix::copy(const CovarianceMatrix &source) {
  ...
  if (source.covDiagonal_.length() > 0) {
    ...
    factor_covariance_matrix();  // The source has already been factored
                                 // in-place? RWH
  } else
...
void CovarianceMatrix::dense_covariance(RealSymMatrix &cov) const {
  ...
  if (cov.numRows() != numDOF_) cov.shape(numDOF_);
  cov = 0.0;
  if (!covIsDiagonal_) {
    for (int i = 0; i < numDOF_; i++)
      for (int j = 0; j < i; j++) cov(i, j) = covMatrix_(i, j);
  } else {
    for (int i = 0; i < numDOF_; i++) {
...
Real CovarianceMatrix::apply_covariance_inverse(
    const RealVector &vector) const {
  RealVector result;
...
void CovarianceMatrix::invert_cholesky_factor() {
  ...
  la.TRTRI(covCholFactor_.UPLO(), 'N', numDOF_, cholFactorInv_.values(),
...
void CovarianceMatrix::apply_covariance_inverse_sqrt(const RealVector &vector,
  ...
  if (result.length() != numDOF_) result.sizeUninitialized(numDOF_);
  if (covIsDiagonal_) {
    for (int i = 0; i < numDOF_; i++)
      result[i] = vector[i] / std::sqrt(covDiagonal_[i]);
  } else {
    result.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0, cholFactorInv_,
                    vector, 0.0);
  }
}
...
void CovarianceMatrix::apply_covariance_inverse_sqrt_to_gradients(
  ...
  if (covIsDiagonal_) {
    ...
    // so compute C = (AB')' = BA'
    result.multiply(Teuchos::NO_TRANS, Teuchos::TRANS, 1.0, gradients,
                    cholFactorInv_, 0.0);
  }
}
...
int CovarianceMatrix::num_dof() const { return numDOF_; }
...
void CovarianceMatrix::get_main_diagonal(RealVector &diagonal) const {
  ...
  if (diagonal.length() != num_dof()) diagonal.sizeUninitialized(num_dof());

  if (covIsDiagonal_) {
    for (int i = 0; i < num_dof(); i++) diagonal[i] = covDiagonal_[i];
  } else {
    for (int i = 0; i < num_dof(); i++) diagonal[i] = covMatrix_(i, i);
  }
}
...
void CovarianceMatrix::as_correlation(RealSymMatrix &corr_mat) const {
  corr_mat = 0.0;
  if (covIsDiagonal_) {
    for (int i = 0; i < num_dof(); ++i) corr_mat(i, i) = 1.0;
  } else {
...
Real CovarianceMatrix::determinant() const {
  Real det = 1.0;
  if (covIsDiagonal_) {
    for (int i = 0; i < num_dof(); i++) det *= covDiagonal_[i];
  } else {
    for (int i = 0; i < num_dof(); i++)
...
Real CovarianceMatrix::log_determinant() const {
  Real log_det = 0.0;
  if (covIsDiagonal_) {
    for (int i = 0; i < num_dof(); i++) log_det += std::log(covDiagonal_[i]);
  } else {
    for (int i = 0; i < num_dof(); i++)
      log_det +=
          std::log(covCholFactor_(i, i)) + std::log(covCholFactor_(i, i));
  }
  return log_det;
}

ExperimentCovariance &ExperimentCovariance::operator=(
    const ExperimentCovariance &source) {
  if (this == &source) return *this;  // Special case of source same as target
...
void ExperimentCovariance::set_covariance_matrices(
    std::vector<RealMatrix> &matrices, std::vector<RealVector> &diagonals,
    RealVector &scalars, IntVector matrix_map_indices,
    IntVector diagonal_map_indices, IntVector scalar_map_indices) {
...
Real ExperimentCovariance::apply_experiment_covariance(
    const RealVector &vector) const {
  if (vector.length() != num_dof()) {
    throw(
        std::runtime_error("apply_covariance_inverse: vector is inconsistent "
...
void ExperimentCovariance::apply_experiment_covariance_inverse_sqrt(
  ...
  if (vector.length() != num_dof())
    throw(
        std::runtime_error("apply_covariance_inverse_sqrt: vector is "
                           "inconsistent with covariance matrix"));
...
void ExperimentCovariance::
    apply_experiment_covariance_inverse_sqrt_to_gradients(
        const RealMatrix &gradients, RealMatrix &result) const {
  if (gradients.numCols() != num_dof())
    throw(
        std::runtime_error("apply_covariance_inverse_sqrt_to_gradients: "
                           "gradients is inconsistent with covariance matrix"));
  ...
  for (int i = 0; i < covMatrices_.size(); i++) {
    int num_dof = covMatrices_[i].num_dof();
    RealMatrix sub_matrix(Teuchos::View, gradients, num_grads, num_dof, 0,
                          shift);
...
void ExperimentCovariance::apply_experiment_covariance_inverse_sqrt_to_hessians(
  ...
  if (hessians.size() != num_dof())
    throw(
        std::runtime_error("apply_covariance_inverse_sqrt_to_hessians: "
                           "hessians is inconsistent with covariance matrix"));
...
void ExperimentCovariance::get_main_diagonal(RealVector &diagonal) const {
  ...
  // Extract the diagonal of each sub block of the experimental covariance
  // matrix
...
void ExperimentCovariance::dense_covariance(RealSymMatrix &cov_mat) const {
  // reshape could be dangerous as will disconnect any Teuchos::View
  if (cov_mat.numRows() != num_dof()) cov_mat.shape(num_dof());
  int global_row_num = 0;
  for (int i = 0; i < covMatrices_.size(); ++i) {
...
void ExperimentCovariance::as_correlation(RealSymMatrix &corr_mat) const {
  // reshape could be dangerous as will disconnect any Teuchos::View
  if (corr_mat.numRows() != num_dof()) corr_mat.shape(num_dof());
  int global_row_num = 0;
  for (int i = 0; i < covMatrices_.size(); ++i) {
...
Real ExperimentCovariance::determinant() const {
  Real det = 1.0;
  for (int i = 0; i < numBlocks_; ++i) det *= covMatrices_[i].determinant();
  return det;
}
